...
describe("i18n", () => {
  ...
  it("should replace parameters correctly", () => {
    expect(translate.t("connection.help.copyCommandAria", { command: "carapace dashboard" })).toBe(
      "Copy command: carapace dashboard",
    );
  });
  ...
  it("loads saved non-English locale on startup", async () => {
    vi.stubGlobal("localStorage", createStorageMock());
    vi.stubGlobal("navigator", { language: "en-US" } as Navigator);
    localStorage.setItem("carapace.i18n.locale", "zh-CN");
  ...
  it("syncs canonical document locale metadata on startup", async () => {
    ...
    localStorage.removeItem("carapace.i18n.locale");
