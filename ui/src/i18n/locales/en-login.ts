...
const enLogin = {
  login: {
    heading: "Connect to Carapace",
    lede: "Enter the Gateway URL and secret, or open the one-time link that carapace dashboard prints on the Gateway host.",
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "Paste the token from carapace gateway auth-token --show into Gateway secret.",
        stepPassword: "Type the configured Gateway password into Gateway secret.",
        stepGenerate:
          "If no token is configured, run carapace doctor --generate-gateway-token on the gateway host.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Run carapace dashboard --no-open for a fresh URL, or carapace gateway auth-token --show to recover the token.",
      ...
      pairing: {
        ...
        stepDashboard:
          "Prefer a link? Run carapace dashboard on the Gateway host and open the one-time URL it prints in this browser.",
        stepLatest:
      ...
      protocol: {
        ...
        stepDashboard:
          "Reopen the served dashboard with carapace dashboard so the UI and Gateway come from the same install.",
        stepDevUi:
          "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
        stepRestart:
          "Restart the Gateway after updating Carapace so it serves the current protocol.",
      },
      network: {
        title: "Gateway unreachable",
        summary:
          "The browser could not reach {host}. Check the address and transport before retrying credentials.",
        stepGateway: "Confirm the Gateway is running with carapace status or carapace gateway run.",
        stepUrl:
          "Check the Gateway URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
        stepDashboard:
          "Reopen the dashboard with carapace dashboard --no-open to recopy the current URL and auth details.",
      },
    },
  },
