...
function getTestApi(): TranslateTestApi {
  const api = (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("carapace.i18nManagerTestApi")
  ];
  if (!api) {
...
export function captureI18nStateForTesting(): () => Promise<void> {
  ...
  const previousPreference = storage?.getItem("carapace.i18n.locale") ?? null;
  ...
  return async () => {
    ...
    if (previousPreference === null) {
      ...
      storage?.setItem("carapace.i18n.locale", previousPreference);
    }
  };
}
