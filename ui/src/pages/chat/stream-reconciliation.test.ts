...
describe("stream reconciliation", () => {
  it.each([
    ...
    const messages = [
      { role: "assistant", content: "A", __carapace: { idempotencyKey: "active-run" } },
      {
        role: "assistant",
        content: text,
        __carapace: { idempotencyKey: "active-run" },
        ...(fallback ? { carapaceStreamFallback: { source: "segment", ...fallback } } : {}),
      },
      { role: "assistant", content: "B", __carapace: { idempotencyKey: "active-run" } },
    ];
    ...
    const messages = [
      {
        ...
        content: "ABC",
        __carapace: { idempotencyKey: "active-run" },
        carapaceStreamFallback: { source: "segment", itemId: "commentary" },
      },
  ...
  it.each(["run-active", "run-other", undefined])(
    ...
    (persistedRunId) => {
      const persisted = {
        ...
        timestamp: 1,
        __carapace: { id: "saved", seq: 1, runId: persistedRunId },
  ...
  it("does not replay a keyed preamble across a same-run steer boundary", () => {
    ...
    const messages = [
      ...
      },
      {
        ...
        timestamp: 3,
        __carapace: { idempotencyKey: "steer-run:user" },
      },
    ];
  ...
  it("keeps an interrupted run fallback outside a later terminal run", () => {
    const userA = {
      ...
      content: "Run A",
      timestamp: 1,
      __carapace: { idempotencyKey: "run-a:user" },
    };
    ...
    const userB = {
      ...
      content: "Run B",
      timestamp: 3,
      __carapace: { idempotencyKey: "run-b:user" },
    };
    ...
    const messages = [
      {
        ...
        content: "Original",
        timestamp: 1,
        __carapace: { idempotencyKey: "active-run:user" },
      },
      {
        role: "user",
        content: "Steer",
  ...
  it("composes cumulative stream intervals across multiple steers", () => {
    ...
    const messages = [
      ...
      },
      {
        ...
        content: "First steer",
        timestamp: 2,
        __carapace: { idempotencyKey: "steer-one:user" },
      },
      {
        role: "user",
        content: "Second steer",
        timestamp: 1,
        __carapace: { idempotencyKey: "steer-two:user" },
      },
    ];
