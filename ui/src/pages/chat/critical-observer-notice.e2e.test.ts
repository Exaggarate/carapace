...
async function waitForToastUpdate(page: Page): Promise<void> {
  await page.locator("carapace-toast-host").evaluate(async (element) => {
...
async function emitObserverAndReadToast(
  ...
  return await page.locator("carapace-toast-host").evaluate(
    async (element, params) => {
      const host = element as HTMLElement & { updateComplete: Promise<unknown> };
      const app = document.querySelector("carapace-app-shell") as
        | (HTMLElement & {
  ...
  it("keeps a critical notice above a shadow-root modal through nested overlay hides", async () => {
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        const modal = page.locator("#toast-shadow-owner").locator("carapace-modal-dialog");
        ...
        const host = modal.locator(":scope > carapace-toast-host");
        ...
        const appToast = page.locator(".shell > carapace-toast-host .app-toast");
