...
describe("realtime Talk conversation", () => {
  ...
  it("keeps per-character assistant deltas verbatim across punctuation boundaries", () => {
    ...
    for (const [index, char] of "Version 1.2 is on github.com/Exaggarate/carapace today.".split("").entries()) {
      state = updateRealtimeTalkConversation(state, {
        role: "assistant",
        text: char,
        final: false,
    ...
    expect(state.entries).toMatchObject([
      { role: "assistant", text: "Version 1.2 is on github.com/Exaggarate/carapace today.", isStreaming: true },
    ]);
  });
