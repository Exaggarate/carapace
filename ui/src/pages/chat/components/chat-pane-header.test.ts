...
afterEach(() => {
  ...
  vi.restoreAllMocks();
  Reflect.deleteProperty(window, "__CARAPACE_NATIVE_WEB_CHROME__");
});
...
describe("chat pane header", () => {
  it("hides the gateway picker without capability and with one gateway", () => {
    Object.assign(window, { __CARAPACE_NATIVE_WEB_CHROME__: true });
  ...
  it("re-renders gateway rows from a changed snapshot property", () => {
    ...
    current = next;
    window.dispatchEvent(new CustomEvent("carapace:native-gateways-changed", { detail: next }));
  ...
  it("renders an editable title and workspace chip", () => {
    ...
    expect(chip?.textContent?.trim()).toContain("carapace");
    title?.click();
  ...
  it("replaces the header owner avatar when visibility is available", () => {
    ...
    expect(container.querySelector("carapace-session-owner-chip")).toBeNull();
    expect(container.querySelector('[data-slot="sharing"]')?.parentElement?.className).toBe(
      "chat-pane__header-leading",
    );
  });
  ...
  it("renders the permanent owner chip only when attribution chrome is enabled", () => {
    ...
    expect(shown.container.querySelector("carapace-session-owner-chip")).not.toBeNull();

    const dormant = mountHeader({
      showOwnerChip: false,
      session: row({ createdActor: { type: "human", id: "profile-ada", label: "Ada" } }),
    });
    expect(dormant.container.querySelector("carapace-session-owner-chip")).toBeNull();
  });
  ...
  it("renders the bounded static participant facepile beside the owner", async () => {
    ...
    expect(mounted.container.querySelector("carapace-session-owner-chip")).not.toBeNull();
    expect(
  ...
  it("renders the durable session actor avatar with the header attribution semantics", async () => {
    ...
    await vi.waitFor(() => {
      expect(mounted.container.querySelector("carapace-session-owner-chip img")).not.toBeNull();
    });
  ...
  it("hides one branch and lists multiple branches with the active tip marked", () => {
    ...
    // wrapper (like carapace-tooltip) has a zero rect and pins the menu to the
...
describe("chat pane workspace resolution", () => {
    ...
    expect(
      resolveSessionWorkspace({
        session: row({
          ...
          worktree: { id: "wt-1", branch: "title-bar", repoRoot: "/src/carapace" },
        }),
      }),
    ).toEqual({ root: "/tmp/worktrees/title-bar", label: "carapace" });
  });
  ...
  it("does not substitute the agent workspace for a missing worktree checkout", () => {
    expect(
      resolveSessionWorkspace({
        session: row({
          worktree: { id: "wt-missing", branch: "feature", repoRoot: "/src/carapace" },
        }),
  ...
  it("matches the gateway root order: spawned workspace before spawned cwd", () => {
    expect(
      resolveSessionWorkspace({
        session: row({
          spawnedWorkspaceDir: "/src/carapace",
          spawnedCwd: "/src/carapace/packages/nested",
        }),
      }),
    ).toEqual({ root: "/src/carapace", label: "carapace" });
    // execCwd is exec-node routing state; it never overrides local facts.
    expect(
      resolveSessionWorkspace({
        session: row({ execCwd: "/remote/stale", spawnedCwd: "/src/carapace" }),
      }),
    ).toEqual({ root: "/src/carapace", label: "carapace" });
  });
  ...
  it("prefers exec cwd and falls back to the agent workspace", () => {
    ...
    expect(resolveSessionWorkspace({ session: row(), agentWorkspace: "/src/carapace" })).toEqual({
      root: "/src/carapace",
      label: "carapace",
    });
  });
