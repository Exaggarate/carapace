...
import type { SystemChangeEntry, SystemChangesListResult } from "@carapace/gateway-protocol";
...
export class CustodianPage extends CarapaceLightDomElement {
  ...
  private synchronizeHistoryClient(): void {
    ...
    const available =
      client !== null && isGatewayMethodAdvertised(snapshot, "carapace.changes.list") === true;
  ...
  private async loadHistory(reset: boolean): Promise<void> {
    ...
    const isCurrent = () =>
      ...
      this.historyAvailable;
    try {
      const result = await client.request<SystemChangesListResult>("carapace.changes.list", {
      ...
      >
        ...
        >
          ...
          <div class="custodian__header-actions">
            ${
              this.onboarding
                ? html`<carapace-sidebar-attention></carapace-sidebar-attention>`
                : nothing
            }
            ${
              this.historyAvailable
...
if (!customElements.get("carapace-custodian-page")) {
  customElements.define("carapace-custodian-page", CustodianPage);
}

declare global {
  interface HTMLElementTagNameMap {
    "carapace-custodian-page": CustodianPage;
  }
}
