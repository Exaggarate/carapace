...
describe("catalog session menu", () => {
  it("renders an anchored Web Awesome dropdown and focuses the first item", async () => {
    ...
    render(
      html`<carapace-catalog-session-menu .lastActive=${"57d"}></carapace-catalog-session-menu>`,
      container,
    );
    const menu = container.querySelector("carapace-catalog-session-menu") as CatalogMenuElement;
  ...
  it.each([
    ...
    render(
      html`<carapace-catalog-session-menu
        .onAction=${onAction}
        .onClose=${() => {
          backingState = null;
          order.push("close");
