...
describe("carapace-board-view", () => {
  ...
  it("renders an ungranted widget in the shared sandbox without popup authority", async () => {
    const view = await mount({
      ...
      widgetFrameUrl: () => "/__carapace__/board/session/alpha/index.html?bt=ticket",
    });
  ...
  it("updates the sandbox bridge when the application Gateway client reconnects", async () => {
    ...
    send({ type: "carapace:widget-bridge-port-offer" }, [bridgeChannel.port1]);
  ...
  it("requests a fresh frame ticket after iframe errors or 401 loads", async () => {
    ...
    const view = await mount({
      ...
      widgetFrameUrl: () => "/__carapace__/board/session/status/index.html?bt=expired",
    });
    const frame = view.querySelector("iframe");
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/__carapace__/board/session/status/index.html?bt=expired",
      { cache: "no-store" },
    );
  });
  ...
  it("bounds repeated frame ticket refreshes after persistent 401 responses", async () => {
    const frameLoadFailed = vi.fn(async () => undefined);
    let frameStatus = 401;
    let frameUrl = "/__carapace__/board/session/status/index.html?bt=expired";
    vi.stubGlobal(
      "fetch",
      vi.fn(
    ...
    frameStatus = 200;
    frameUrl = "/__carapace__/board/session/status/index.html?bt=fresh";
  ...
  it("ignores authorization results from a superseded widget frame", async () => {
    ...
    const view = await mount({
      ...
      widgetFrameUrl: (_name, revision) =>
        `/__carapace__/board/session/status/index.html?revision=${revision}`,
    });
    const frame = view.querySelector("iframe");
    frame?.dispatchEvent(new Event("load"));
  ...
  it("preserves each widget cell and iframe identity when order changes", async () => {
    const view = await mount();
    const before = [...view.querySelectorAll("carapace-board-widget-cell")].find(
  ...
  it("calls applyOps from the kebab remove action", async () => {
    ...
    const firstCell = view.querySelector("carapace-board-widget-cell");
  ...
  it("renders MCP App widgets through the bridge element while approval is pending", async () => {
    ...
    expect(view.querySelector("iframe")).toBeNull();
    app?.dispatchEvent(
      new CustomEvent("carapace-mcp-app-view-expired", { bubbles: true, composed: true }),
    );
    await vi.waitFor(() =>
  ...
  it("shows granted capabilities in a compact chip and tooltip", async () => {
    ...
    const tooltip = chip?.closest("carapace-tooltip") as
  ...
  it("replaces live-region content for repeated identical announcements", async () => {
    ...
    const cell = secondCell?.closest("carapace-board-widget-cell");
