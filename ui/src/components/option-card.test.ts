...
describe("option card", () => {
  ...
  it("accents and focuses the recommended choice, then emits its value", async () => {
    ...
    render(
      html`<carapace-option-card
        .props=${{
          header: "Access",
          question: "How should Carapace help?",
          options: [
    ...
    const card = container.querySelector("carapace-option-card")!;
  ...
  it("always renders a skip affordance and emits dismissal", async () => {
    ...
    await container.querySelector("carapace-option-card")!.updateComplete;
