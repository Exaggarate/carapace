...
class CarapaceMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--carapace-mascot-size, 120px);
      height: var(--carapace-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--carapace-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("carapace-mascot")) {
  customElements.define("carapace-mascot", CarapaceMascot);
}
