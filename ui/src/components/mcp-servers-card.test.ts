...
type McpServersCard = HTMLElementTagNameMap["carapace-mcp-servers-card"];
...
async function mountCard(
  ...
  const card = document.createElement("carapace-mcp-servers-card");
...
describe("carapace-mcp-servers-card", () => {
  beforeEach(async () => {
  ...
  it("renders rich rows without exposing URL credentials or stdio arguments", async () => {
    ...
    expect(docs.textContent).toContain("carapace mcp login docs");
    ...
    expect(local.textContent).toContain("carapace mcp probe local");
    ...
    expect(hostile.textContent).toContain("carapace mcp probe 'docs; echo unsafe'");
  });
  ...
  it("renders the empty state when no servers are configured", async () => {
    ...
    expect(setupLink?.href).toBe("https://github.com/Exaggarate/carapace");
