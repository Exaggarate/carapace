...
afterEach(() => {
  ...
  Reflect.deleteProperty(window, "__CARAPACE_NATIVE_HISTORY__");
  Reflect.deleteProperty(window, "__CARAPACE_NATIVE_EMBED__");
});
...
describe("native web chrome capability", () => {
  it.each([
    ...
    Object.assign(window, { __CARAPACE_NATIVE_EMBED__: host });
  ...
  it("requires the document-start capability flag", () => {
    expect(isNativeWebChromeHost()).toBe(false);
    (window as TestNativeWindow)["__CARAPACE_NATIVE_WEB_CHROME__"] = true;
  ...
  it("reads native history state and defaults safely", () => {
    ...
    (window as TestNativeWindow)["__CARAPACE_NATIVE_HISTORY__"] = {
      canGoBack: true,
      canGoForward: false,
    };
