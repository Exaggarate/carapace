// Carapace Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "carapace-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__CARAPACE_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Carapace", body: event.data.text() };
  }

  const title = data.title || "Carapace";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "carapace-notification",
