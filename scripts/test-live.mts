...
function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with CARAPACE_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args: TestLiveArgs, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    CARAPACE_LIVE_TEST: baseEnv.CARAPACE_LIVE_TEST || "1",
    CARAPACE_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.CARAPACE_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { CARAPACE_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.CARAPACE_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid CARAPACE_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
