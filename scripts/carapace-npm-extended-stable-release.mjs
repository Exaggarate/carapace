...
export function validateNpmPreflightDistTag({ manifest, npmDistTag }) {
  ...
  if (
    ...
    ["beta", "latest"].includes(npmDistTag) &&
    manifest.pluginSdkApi?.schema === "carapace.plugin-sdk-api-release-evidence-set/v1" &&
    selectors &&
    Object.keys(selectors).length === 2 &&
    ["beta", "latest"].every(
      (selector) => selectors[selector]?.schema === "carapace.plugin-sdk-api-release-evidence/v1",
    )
  ) {
    return;
  }
...
export async function verifyExtendedStableRegistryReadback({
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const exactResult = await query(`carapace@${expectedVersion}`);
    const extendedStableResult = await query("carapace@extended-stable");
  ...
  throw new Error(
    `npm registry did not converge to carapace@${expectedVersion} and carapace@extended-stable=${expectedVersion} after ${attempts} attempts (exact=${exactVersion}, extended-stable=${extendedStableSelector}).`,
  );
}
...
export function extendedStableSelectorRepairCommand(expectedVersion) {
  ...
  return `npm dist-tag add carapace@${parsed.version} extended-stable`;
}
...
async function main() {
  ...
  if (command === "capture-selector") {
    const previous = capturePriorExtendedStableSelector({
      query: () =>
        spawnSync("npm", ["view", "carapace", "dist-tags", "--json"], { encoding: "utf8" }),
    });
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(`carapace-npm-extended-stable-release: ${message}`);
    process.exit(1);
  }
}
