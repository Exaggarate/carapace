...
export interface CarapaceNpmResumeValidationInput {
...
const WORKFLOW_PATH = ".github/workflows/carapace-npm-release.yml";
...
function requiredString(value: unknown, label: string): string {
  if (typeof value !== "string" || value.length === 0) {
    fail(`Carapace npm resume run is missing ${label}.`);
  }
  return value;
}
...
function requiredSha(value: unknown, label: string): string {
  ...
  if (!SHA_PATTERN.test(sha)) {
    fail(`Carapace npm resume run has invalid ${label}.`);
  }
  return sha;
}

export function validateCarapaceNpmResumeRun({
  canonicalWorkflowId,
  compareStatus,
  jobs,
  run,
  ...
  if (!workflowRefMatch || workflowFullRef !== `refs/tags/${workflowRef}`) {
    fail(`Carapace npm resume run has an untrusted workflow ref: ${url}`);
  }
  ...
  if (
    ...
    fail(`Carapace npm resume run has an untrusted workflow identity: ${url}`);
  }

  const tagObjectSha = requiredSha(tagRef?.object?.sha, "tooling tag object SHA");
  if (tagRef?.object?.type === "commit") {
    if (tagObjectSha !== sha) {
      fail(`Carapace npm resume run protected tooling tag moved after dispatch: ${url}`);
    }
    ...
    if (
      ...
      fail(
        `Carapace npm resume run is not bound to a real, main-reachable protected tooling tag: ${url}`,
      );
    }
  } else {
    fail(`Carapace npm resume run tooling ref is not a protected tag: ${url}`);
  }
  ...
  if (
    ...
    fail(`Carapace npm resume run lacks successful parent release approval validation: ${url}`);
  }
...
export function runCarapaceNpmResumeGh(
  args: string[],
  params: {
...
export function resolveCarapaceNpmResumeRun({
  repo,
  runId,
  trustedWorkflowFullRef,
  trustedWorkflowRef,
  runGh = runCarapaceNpmResumeGh,
}: {
  repo: string;
  runId: string;
  ...
  if (!/^[1-9][0-9]*$/u.test(runId)) {
    fail("Carapace npm resume run id must be a positive integer.");
  }
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repo)) {
    fail("Carapace npm resume repository must be owner/name.");
  }
  ...
  if (!trustedRefMatch || trustedWorkflowFullRef !== `refs/tags/${trustedWorkflowRef}`) {
    fail(
      "Carapace npm resume trusted workflow identity must be an exact protected release-publish tag.",
    );
  }
  ...
  return validateCarapaceNpmResumeRun({
...
function main(argv: string[] = process.argv.slice(2)): void {
  const result = resolveCarapaceNpmResumeRun(parseArgs(argv));
