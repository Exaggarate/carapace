...
// Test Force script supports Carapace repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured Carapace gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.CARAPACE_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `carapace-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/test-projects.mts"], {
    stdio: "inherit",
    env: {
      ...process.env,
      CARAPACE_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
