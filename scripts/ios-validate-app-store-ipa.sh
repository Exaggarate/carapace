...
usage() {
  cat <<'EOF'
Usage:
  scripts/ios-validate-app-store-ipa.sh --ipa apps/ios/build/app-store/Carapace-<version>.ipa \
...
EXPECTED_BUNDLE_ID="ai.carapacefoundation.app"
EXPECTED_PROFILE_NAME="Carapace App Store ai.carapacefoundation.app"
EXPECTED_APP_GROUP="group.ai.carapacefoundation.app.shared"
...
tmp_dir="$(mktemp -d -t carapace-ios-ipa.XXXXXX)"
...
assert_build_provenance() {
  local commit
  local timestamp
  commit="$(plist_value "${info_plist}" "CarapaceGitCommit")"
  timestamp="$(plist_value "${info_plist}" "CarapaceBuildTimestamp")"
  if [[ ! "${commit}" =~ ^[0-9a-f]{40}$ ]]; then
    echo "Invalid IPA: CarapaceGitCommit must be a full lowercase commit SHA; got ${commit:-missing}." >&2
    exit 1
  fi
  if [[ ! "${timestamp}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{3})?Z$ ]]; then
    echo "Invalid IPA: CarapaceBuildTimestamp must be a canonical UTC timestamp; got ${timestamp:-missing}." >&2
    exit 1
  fi
...
assert_plist_string "${info_plist}" "CFBundleDisplayName" "Carapace" "display name mismatch"
assert_plist_string "${info_plist}" "CarapacePushMode" "${EXPECTED_PUSH_MODE}" "push mode mismatch"
...
assert_build_provenance
assert_localized_plists_resolve_build_settings
assert_plist_empty_or_absent "${info_plist}" "CarapacePushRelayBaseURL" "push relay URL override"
assert_plist_key_absent "${info_plist}" "CarapacePushTransport" "legacy push transport"
assert_plist_key_absent "${info_plist}" "CarapacePushDistribution" "legacy push distribution"
assert_plist_key_absent "${info_plist}" "CarapacePushAPNsEnvironment" "legacy APNs environment"
assert_plist_key_absent "${info_plist}" "CarapacePushRelayProfile" "legacy relay profile"
assert_plist_key_absent "${info_plist}" "CarapacePushProofPolicy" "legacy proof policy"
