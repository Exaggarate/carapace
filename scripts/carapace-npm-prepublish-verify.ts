...
// Carapace Npm Prepublish Verify script supports Carapace repository automation.
...
import { resolveNpmCommandInvocation } from "./carapace-npm-release-check.ts";
...
type CarapaceNpmPrepublishVerifyArgs =
  | {
...
export function carapaceNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/carapace-npm-prepublish-verify.ts <tarball.tgz> [expected-version] [dependency.tgz ...]";
}

export function parseCarapaceNpmPrepublishVerifyArgs(
  argv: readonly string[],
): CarapaceNpmPrepublishVerifyArgs {
  ...
  if (!tarballPath) {
    throw new Error(carapaceNpmPrepublishVerifyUsage());
  }
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown carapace npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown carapace npm prepublish verifier option: ${expectedVersion}`);
  }
...
export function assertPreparedCarapaceAiDependency(params: {
  aiManifest: PackedPackageJson;
  rootManifest: PackedPackageJson;
}): void {
  if (params.aiManifest.name !== "@carapace/ai" || !params.aiManifest.version) {
    throw new Error("Prepared dependency tarball must contain @carapace/ai with a version.");
  }
  if (params.rootManifest.name !== "carapace") {
    throw new Error("Prepared root tarball must contain the carapace package.");
  }
  if (!params.rootManifest.version || params.rootManifest.version !== params.aiManifest.version) {
    throw new Error(
      `Prepared root and @carapace/ai tarballs must both be version ${params.aiManifest.version}.`,
    );
  }
  if (params.rootManifest.dependencies?.["@carapace/ai"] !== params.aiManifest.version) {
    throw new Error(
      `Prepared root tarball must depend on exact @carapace/ai@${params.aiManifest.version}.`,
    );
  }
}
...
function main(argv = process.argv.slice(2)): void {
  const args = parseCarapaceNpmPrepublishVerifyArgs(argv);
  if (args.help) {
    console.log(carapaceNpmPrepublishVerifyUsage());
    return;
  }

  const workingDir = mkdtempSync(join(tmpdir(), "carapace-prepublish-"));
  ...
  try {
    ...
    if (usesPreparedLocalDependencyInstall(args.dependencyTarballPaths.length)) {
        ...
        `${JSON.stringify(
          {
            private: true,
            dependencies: {
              "@carapace/ai": pathToFileURL(aiTarballPath).href,
              carapace: pathToFileURL(realpathSync(args.tarballPath)).href,
            },
          },
          null,
          2,
      ...
      packageRoot = join(prefixDir, "node_modules", "carapace");
      const binaryPath = join(
        prefixDir,
        "node_modules",
        ".bin",
        process.platform === "win32" ? "carapace.cmd" : "carapace",
      );
      binaryInvocation =
      ...
      packageRoot = join(globalRoot, "carapace");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed carapace binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `carapace-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`carapace-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
