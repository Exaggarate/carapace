...
async function main(argv: string[]) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `carapace@${previousVersion}` : "carapace@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.CARAPACE_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.CARAPACE_RELEASE_CHECK_RUNNER ?? "",
