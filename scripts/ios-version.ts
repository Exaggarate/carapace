// Ios Version script supports Carapace repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `CARAPACE_IOS_VERSION=${version.canonicalVersion}`,
        `CARAPACE_APP_STORE_REVISION=${version.appStoreRevision ?? ""}`,
        `CARAPACE_APP_STORE_VERSION=${version.appStoreVersion ?? ""}`,
        `CARAPACE_MARKETING_VERSION=${version.marketingVersion}`,
        `CARAPACE_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
