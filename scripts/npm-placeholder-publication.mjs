...
const MANIFEST_SCHEMA = "carapace.npm-placeholder-publication/v1";
const PACKAGE_VERSION = "0.0.0";
const PUBLISH_TAG = "placeholder";
const WORKFLOW_PATH = ".github/workflows/npm-placeholder-bootstrap.yml";
const PACKAGE_NAME_RE = /^@carapace\/[a-z0-9][a-z0-9._-]*$/u;
...
function placeholderPackageJson(packageName) {
  return {
    ...
    description: "Reserved package name for an official Carapace plugin.",
    license: "MIT",
    repository: {
      type: "git",
      url: "git+https://github.com/Exaggarate/carapace.git",
    },
    publishConfig: {
      access: "public",
      tag: PUBLISH_TAG,
...
function placeholderReadme(packageName) {
  return `# ${packageName}\n\nReserved placeholder for the official Carapace plugin package. Use a published release version instead.\n`;
}
...
export function createPlaceholderTarball(packageName) {
  if (!PACKAGE_NAME_RE.test(packageName)) {
    throw new Error(`Invalid Carapace package name: ${packageName}`);
  }
...
export function resolveSelectedPackageSources(repoRoot, packageNames) {
  ...
  for (const catalogName of ["plugin", "provider", "channel"]) {
    ...
    for (const entry of catalog.entries) {
      if (
        ...
        entry.carapace?.install?.npmSpec === entry.name
      ) {
  ...
  return packageNames.map((packageName) => {
    ...
    if (
      ...
      match.packageJson.carapace?.install?.npmSpec !== packageName ||
      match.packageJson.carapace?.build?.bundledDist !== false ||
      match.packageJson.carapace?.release?.publishToNpm !== true
    ) {
...
export async function publishPlaceholders(params) {
  ...
  const publishHome = mkdtempSync(
    join(resolve(params.tempRoot ?? tmpdir()), "carapace-npm-placeholder-"),
  );
  try {
