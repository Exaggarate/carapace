// Anthropic Prompt Probe script supports Carapace repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// CARAPACE_PROMPT_TRANSPORT=direct|gateway
// CARAPACE_PROMPT_MODE=extra
// CARAPACE_PROMPT_TEXT='...'
// CARAPACE_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.CARAPACE_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.CARAPACE_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.CARAPACE_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.CARAPACE_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "CARAPACE_PROMPT_CAPTURE",
  raw: process.env.CARAPACE_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "CARAPACE_PROMPT_INCLUDE_RAW",
  raw: process.env.CARAPACE_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "CARAPACE_PROMPT_KEEP_TMP",
  raw: process.env.CARAPACE_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.CARAPACE_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "CARAPACE_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.CARAPACE_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "CARAPACE_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.CARAPACE_PROMPT_GATEWAY_TIMEOUT_MS,
});
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  fallback: 2 * 1024 * 1024,
  label: "CARAPACE_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.CARAPACE_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.CARAPACE_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.CARAPACE_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.CARAPACE_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set CARAPACE_LIVE_SETUP_TOKEN_VALUE or CARAPACE_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["carapace.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        CARAPACE_AGENT_DIR: params.agentDir,
        ...
        CARAPACE_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      detached: process.platform !== "win32",
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing CARAPACE_PROMPT_TEXT or CARAPACE_PROMPT_LIST_JSON");
  }
