...
LIVE_GATEWAY_MAX_MODELS="$(carapace_live_read_positive_int_env CARAPACE_LIVE_GATEWAY_MAX_MODELS 8)"
LIVE_GATEWAY_STEP_TIMEOUT_MS="$(carapace_live_read_positive_int_env CARAPACE_LIVE_GATEWAY_STEP_TIMEOUT_MS 45000)"
LIVE_GATEWAY_MODEL_TIMEOUT_MS="$(carapace_live_read_positive_int_env CARAPACE_LIVE_GATEWAY_MODEL_TIMEOUT_MS 90000)"
...
carapace_live_collect_auth_for_providers "${CARAPACE_LIVE_GATEWAY_PROVIDERS:-}"
...
carapace_live_prepare_cli_backend_docker_packages \
  "${CARAPACE_LIVE_GATEWAY_PROVIDERS:-}" \
  "${CARAPACE_LIVE_GATEWAY_MODELS:-}"
carapace_live_stage_gemini_auth
carapace_live_run_staged_script scripts/test-live -- src/gateway/gateway-models.profiles.live.test.ts
EOF
...
if [[ "${CARAPACE_FROZEN_TARGET_LIVE_CLI_BACKEND_PACKAGE_MODE:-current}" == "legacy" ]]; then
  FROZEN_TARGET_DOCKER_ENV+=( -e "CARAPACE_FROZEN_TARGET_LIVE_CLI_BACKEND_PACKAGE_MODE=legacy" )
fi
carapace_live_init_docker_run_args DOCKER_RUN_ARGS "${CARAPACE_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e CARAPACE_LIVE_GATEWAY_MODELS="${CARAPACE_LIVE_GATEWAY_MODELS:-modern}" \
  -e CARAPACE_LIVE_GATEWAY_PROVIDERS="${CARAPACE_LIVE_GATEWAY_PROVIDERS:-}" \
  -e CARAPACE_LIVE_GATEWAY_THINKING="${CARAPACE_LIVE_GATEWAY_THINKING:-}" \
  -e CARAPACE_LIVE_GATEWAY_SMOKE="${CARAPACE_LIVE_GATEWAY_SMOKE:-1}" \
  -e CARAPACE_LIVE_GATEWAY_MAX_MODELS="$LIVE_GATEWAY_MAX_MODELS" \
  -e CARAPACE_LIVE_GATEWAY_HEARTBEAT_MS="${CARAPACE_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e CARAPACE_LIVE_GATEWAY_STEP_TIMEOUT_MS="$LIVE_GATEWAY_STEP_TIMEOUT_MS" \
  -e CARAPACE_LIVE_GATEWAY_MODEL_TIMEOUT_MS="$LIVE_GATEWAY_MODEL_TIMEOUT_MS" \
  -e CARAPACE_VITEST_FS_MODULE_CACHE=0)
carapace_live_append_array DOCKER_RUN_ARGS FROZEN_TARGET_DOCKER_ENV
