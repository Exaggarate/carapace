 ...
 * `carapace update`. Use it to verify the completion installation flow
...
\`carapace update\`, \`carapace doctor\`, and \`carapace onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Carapace Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`carapace completion --install\` later to enable.`));
    return;
  }
