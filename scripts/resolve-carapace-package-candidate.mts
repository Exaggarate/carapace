...
const DEFAULT_OUTPUT_NAME = "carapace-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "CARAPACE_TRUSTED_PACKAGE_TOKEN";
...
export const CARAPACE_PACKAGE_SPEC_RE =
  /^carapace@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node --import tsx scripts/resolve-carapace-package-candidate.mts --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedCarapaceTarballFilename(value: unknown) {
  ...
  if (
    ...
    throw new Error(
      `npm pack reported unsafe Carapace tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateCarapacePackageSpec(spec: string) {
  if (!CARAPACE_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be carapace@alpha, carapace@beta, carapace@extended-stable, carapace@latest, or an exact Carapace release version; got: ${spec}`,
    );
  }
}
...
export function resolveNpmPackageCandidatePackRunner(
  ...
  validateCarapacePackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref: string) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Carapace branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref: string) {
  ...
  const sourceDir = path.join(
    ...
    `carapace-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir: string, packOutput: string, outputName: string) {
  ...
  if (parsed !== undefined) {
    ...
    if (packedFilename) {
      filename = resolvePackedCarapaceTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("carapace-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedCarapaceTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename =
      entries
        .filter((entry) => {
          try {
            return resolvePackedCarapaceTarballFilename(entry) === entry;
          } catch {
            return false;
          }
        })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no Carapace tarball in ${outputDir}`);
  }
...
export const cleanPackedCarapaceTarballsForTest = cleanPackedCarapaceTarballs;
...
async function resolveCandidate(options: PackageCandidateOptions) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-carapace-for-docker.mjs",
      ...
      await cleanPackedCarapaceTarballs(outputDir);
  ...
  console.error(`Checking Carapace package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-carapace-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "carapace") {
    throw new Error(`package candidate must be named "carapace"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
