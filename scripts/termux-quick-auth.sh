...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/carapace/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Carapace Re-Auth" -c "After getting key, run: ssh $SERVER '~/carapace/scripts/mobile-reauth.sh'" --id carapace-auth
        ;;
    *)
