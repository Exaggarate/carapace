...
// Carapace Npm Release Check script supports Carapace repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/Exaggarate/carapace";
...
const skipPackValidationEnv = "CARAPACE_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return readPositiveEnvInt(
    "CARAPACE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "carapace") {
    errors.push(`package.json name must be "carapace"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.carapace !== "carapace.mjs") {
    errors.push(
      `package.json bin.carapace must be "carapace.mjs"; found "${pkg.bin?.carapace ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`carapace-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `carapace-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
