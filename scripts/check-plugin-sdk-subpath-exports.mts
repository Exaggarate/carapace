...
function parsePluginSdkSubpath(specifier: string): string | null {
  return specifier.match(/^@?carapace\/plugin-sdk\/(.+)$/u)?.[1] ?? null;
}
...
async function main(): Promise<void> {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced carapace/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced carapace/plugin-sdk/<subpath> must be public or use its required private boundary.",
  );
