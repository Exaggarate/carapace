#!/usr/bin/env node
// Builds the canonical Carapace package artifact used by Docker E2E.
...
const AI_RUNTIME_PACKAGE = "@carapace/ai";
const AI_RUNTIME_BACKUP_DIR = ".carapace-ai-package-backup";
...
const PACKAGE_BUILD_PLUGIN_SELECTION_ENV_NAMES = [
  "CARAPACE_EXTENSIONS",
  "CARAPACE_DOCKER_BUILD_EXTENSIONS",
...
function resolvePackedCarapaceFileName(value: string) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("carapace-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    !/^carapace-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(`npm pack reported unsafe Carapace tarball filename: ${filename}`);
  }
  return filename;
}
...
async function newestCarapaceTarball(outputDir: string, packOutput: string) {
  let fromOutput = "";
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedCarapaceFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedCarapaceFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Carapace tarball in ${outputDir}`);
  }
...
async function cleanPackedCarapaceTarballs(outputDir: string) {
  let entries: string[];
  try {
...
function isPackedAiRuntimeTarball(filename: string) {
  return /^carapace-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
  ...
  return run(
    ...
    process.cwd(),
    {
      ...
      timeoutMs: resolveTimeoutMs(
        "CARAPACE_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@carapace", "ai");
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@carapace/ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @carapace/ai as a dependency");
  }
  ...
  try {
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @carapace/ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @carapace/ai package must declare a version");
    }
...
async function normalizeCarapaceTarballModes(tarballPath: string) {
  ...
  const stageDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-package-modes-"));
  try {
    ...
    if (stagedFileCount === 0) {
      throw new Error(`packed Carapace tarball has no file entries: ${tarballPath}`);
    }
...
export async function packCarapacePackageForDocker(
  sourcePath: string,
  outputPath: string,
  packageOptions: PackageOptions = {},
) {
  ...
  console.error("==> Packing Carapace package");
  ...
  try {
    console.error("==> Writing Carapace package inventory");
  ...
  try {
    ...
    try {
      await cleanPackedCarapaceTarballs(outputPath);
      if (packageOptions.bundlePlugins?.length) {
    ...
    let tarball = await newestCarapaceTarball(
      outputPath,
    ...
    await (packageOptions.normalizeTarballModes ?? normalizeCarapaceTarballModes)(tarball);
    if (packageOptions.packJsonPath) {
      ...
      packReceiptDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-npm-pack-receipt-"));
...
export async function writePackageInventoryForDocker(
  ...
  await runImpl(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "CARAPACE_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
...
async function main() {
  ...
  console.error("==> Checking Carapace package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-carapace-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "CARAPACE_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> Carapace package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
