...
function usage(): string {
  return "Usage: node scripts/check-carapace-package-tarball.mjs [--require-bundled-workspace-deps] <carapace.tgz>";
}
...
function parseArgs(argv: string[]) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown Carapace package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected Carapace package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`Carapace package tarball does not exist: ${tarball}`);
}
...
const REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES = ["@carapace/ai"];
...
const REQUIRED_BUNDLED_WORKSPACE_RUNTIME_ENTRIES = new Map([
  [
    "@carapace/ai",
    [
      { specifier: "@carapace/ai", entry: "dist/index.mjs" },
      { specifier: "@carapace/ai/providers", entry: "dist/providers.mjs" },
      {
        specifier: "@carapace/ai/transports",
        entry: "dist/transports.mjs",
        whenExported: "./transports",
      },
      {
        specifier: "@carapace/ai/internal/openai-responses-payload-policy",
        entry: "dist/internal/openai-responses-payload-policy.mjs",
        whenExported: "./internal/openai-responses-payload-policy",
      },
      {
        specifier: "@carapace/ai/internal/runtime",
        entry: "dist/internal/runtime.mjs",
      },
      {
        specifier: "@carapace/ai/internal/tool-schema",
...
function collectRequiredBundledWorkspaceDependencyErrors(
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the Carapace workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.CARAPACE_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
...
function runPhase<Result>(label: string, action: () => Result): Result {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-carapace-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const archiveRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-package-tarball-"));
...
try {
  ...
  fail(`Carapace package tarball preflight failed:\n${coerceErrorMessage(error)}`);
}
...
if (hasShrinkwrap && declaresShrinkwrap) {
  try {
    ...
    if (shrinkwrap.name !== "carapace") {
      errors.push("npm-shrinkwrap.json root name must be carapace");
    }
    ...
    if (!rootPackage || rootPackage.name !== "carapace") {
      errors.push("npm-shrinkwrap.json packages root must name carapace");
    }
...
try {
  ...
  if (phaseTimingsEnabled) {
    console.error(
      `check-carapace-package-tarball: npm pack inventory (npm ${npmInventory.npmVersion}) completed in ${npmInventory.durationMs}ms`,
    );
  }
...
if (errors.length > 0) {
  fs.rmSync(archiveRoot, { recursive: true, force: true });
  fail(`Carapace package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Carapace package tarball integrity warning: ${warning}`);
}
fs.rmSync(archiveRoot, { recursive: true, force: true });
console.log("Carapace package tarball integrity passed.");
