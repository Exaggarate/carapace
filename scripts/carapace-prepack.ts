...
// Carapace Prepack script supports Carapace repository automation.
...
import { restorePrepackArtifacts } from "./carapace-postpack.mjs";
...
const ALLOW_UNRELEASED_CHANGELOG_ENV = "CARAPACE_PREPACK_ALLOW_UNRELEASED_CHANGELOG";
const PREPARED_RELEASE_ENV = "CARAPACE_PREPACK_PREPARED";
const OCM_INTERNAL_NPM_BIN_ENV = "OCM_INTERNAL_NPM_BIN";
const OCM_WORKSPACE_DIRS_ENV = "CARAPACE_OCM_WORKSPACE_DEPENDENCY_DIRS";
...
const SELF_CONTAINED_SOURCE_PACK_COMMAND =
  "node scripts/package-carapace-for-docker.mjs --allow-unreleased-changelog";
...
export function collectSourcePackWorkspaceDependencyErrors(
  ...
  const aiDependency = packageJson.dependencies?.["@carapace/ai"];
  if (typeof aiDependency !== "string" || !aiDependency.trim().startsWith("workspace:")) {
    return [];
  }
  if (ocmExternalizesWorkspacePackage("@carapace/ai", env)) {
    return [];
  }
  return [
    `plain root packing cannot safely resolve @carapace/ai from ${aiDependency}: pnpm rewrites the workspace dependency to an exact version without bundling the package`,
    `use \`${SELF_CONTAINED_SOURCE_PACK_COMMAND}\` for a self-contained source package; official npm release automation prepares and publishes @carapace/ai separately`,
  ];
}
...
export function resolvePrepackCommandTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveEnvInt(
    "CARAPACE_PREPACK_COMMAND_TIMEOUT_MS",
    env,
