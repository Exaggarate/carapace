// Debug Claude Usage script supports Carapace repository automation.
...
Options:
  --agent <id>          Carapace agent id to inspect (default: main)
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.CARAPACE_STATE_DIR?.trim() || path.join(os.homedir(), ".carapace");
...
const resolveFetchTimeoutMs = (raw = process.env.CARAPACE_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "CARAPACE_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
