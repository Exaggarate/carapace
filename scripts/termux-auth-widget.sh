...
# Carapace Auth Widget for Termux
...
# Check auth status
termux-toast "Checking Carapace auth..."

STATUS=$(ssh "$SERVER" '$HOME/carapace/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/carapace/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CARAPACE_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/carapace/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CARAPACE_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Carapace won't work until re-auth"
                ;;
        esac
        ;;
