#!/usr/bin/env bash
# Rootless Carapace in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-carapace-podman.sh launch        # Start gateway
#   ./scripts/run-carapace-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   carapace --container carapace dashboard --no-open
#   carapace --container carapace channels login
#
...
run_podman_detached() {
  carapace_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      CARAPACE_GATEWAY_TOKEN|CARAPACE_PODMAN_CONTAINER|CARAPACE_PODMAN_IMAGE|CARAPACE_IMAGE|CARAPACE_PODMAN_PULL|CARAPACE_PODMAN_RUN_TIMEOUT|CARAPACE_PODMAN_GATEWAY_HOST_PORT|CARAPACE_GATEWAY_PORT|CARAPACE_PODMAN_BRIDGE_HOST_PORT|CARAPACE_BRIDGE_PORT|CARAPACE_GATEWAY_BIND|CARAPACE_PODMAN_USERNS|CARAPACE_BIND_MOUNT_OPTIONS|CARAPACE_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-carapace-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${CARAPACE_CONFIG_DIR:-$EFFECTIVE_HOME/.carapace}"
ENV_FILE="${CARAPACE_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${CARAPACE_CONFIG_DIR:-$EFFECTIVE_HOME/.carapace}"
ENV_FILE="${CARAPACE_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${CARAPACE_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${CARAPACE_PODMAN_CONTAINER:-carapace}"
CARAPACE_IMAGE="${CARAPACE_PODMAN_IMAGE:-${CARAPACE_IMAGE:-carapace:local}}"
PODMAN_PULL="${CARAPACE_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${CARAPACE_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${CARAPACE_PODMAN_GATEWAY_HOST_PORT:-${CARAPACE_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${CARAPACE_PODMAN_BRIDGE_HOST_PORT:-${CARAPACE_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${CARAPACE_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$CARAPACE_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  CARAPACE_CONTAINER="" CARAPACE_CONFIG_DIR="$config_dir" \
    carapace config get gateway.bind 2>/dev/null || true
}
...
# CARAPACE_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${CARAPACE_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'CARAPACE_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v carapace >/dev/null 2>&1; then
    echo "Warning: carapace not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    CARAPACE_CONTAINER="" CARAPACE_CONFIG_DIR="$config_dir" \
      carapace config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    CARAPACE_CONTAINER="" CARAPACE_CONFIG_DIR="$config_dir" \
      carapace config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  CARAPACE_CONTAINER="" CARAPACE_CONFIG_DIR="$config_dir" \
    carapace config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${CARAPACE_GATEWAY_TOKEN:-}" ]]; then
  export CARAPACE_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CARAPACE_GATEWAY_TOKEN" "$CARAPACE_GATEWAY_TOKEN"
  echo "Generated CARAPACE_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/carapace.json"
...
PODMAN_USERNS="${CARAPACE_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CARAPACE_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CARAPACE_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${CARAPACE_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${CARAPACE_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$CARAPACE_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.carapace/.npm \
    -e CARAPACE_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.carapace:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.carapace/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$CARAPACE_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME carapace dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME carapace devices approve --latest  # if pairing required"
