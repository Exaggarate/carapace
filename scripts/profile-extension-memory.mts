...
const RSS_MARKER = "__CARAPACE_MAX_RSS_KB__=";
...
function defaultJsonReportPath(): string {
  return path.join(
    os.tmpdir(),
    `carapace-extension-memory-${process.pid}-${Date.now()}-${randomUUID()}.json`,
  );
}
...
async function main(): Promise<void> {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "carapace-extension-memory-"));
