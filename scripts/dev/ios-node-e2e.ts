// Ios Node E2E script supports Carapace repository automation.
...
const urlRaw = getArg("--url") ?? process.env.CARAPACE_GATEWAY_URL;
const token = getArg("--token") ?? process.env.CARAPACE_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.CARAPACE_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "carapace ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "carapace-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Carapace E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
