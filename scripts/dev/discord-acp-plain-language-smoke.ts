#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Carapace repository automation.
...
type DriverMode = "token" | "webhook" | "carapace";
...
const DEFAULT_CARAPACE_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  ...
  const home = process.env.CARAPACE_HOME?.trim() || process.env.HOME || "";
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "carapace") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or carapace.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver carapace] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Carapace spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|carapace> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override Carapace state dir for plugin-state polling\n" +
    "  --carapace-bin <path>        Carapace CLI binary for driver=carapace (default: carapace)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  CARAPACE_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  CARAPACE_DISCORD_SMOKE_DRIVER\n" +
    "  CARAPACE_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  CARAPACE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  CARAPACE_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  CARAPACE_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  CARAPACE_DISCORD_SMOKE_AGENT\n" +
    "  CARAPACE_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  CARAPACE_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  CARAPACE_DISCORD_SMOKE_POLL_MS\n" +
    "  CARAPACE_STATE_DIR\n" +
    "  CARAPACE_DISCORD_SMOKE_CARAPACE_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.CARAPACE_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.CARAPACE_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.CARAPACE_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.CARAPACE_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.CARAPACE_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.CARAPACE_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const carapaceBin =
    resolveArg("--carapace-bin", argv) ||
    process.env.CARAPACE_DISCORD_SMOKE_CARAPACE_BIN ||
    "carapace";
...
async function carapaceCliJson<T>(params: {
  carapaceBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.carapaceBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_CARAPACE_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`carapace ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithCarapace(params: {
  carapaceBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await carapaceCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    carapaceBin: params.carapaceBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "carapace") {
    return await readMessagesWithCarapace({
      carapaceBin: params.args.carapaceBin,
...
async function run(argv = process.argv.slice(2)): Promise<SuccessResult | FailureResult> {
  ...
  const bindingsStore = createPluginStateKeyedStore<ThreadBindingRecord>("discord", {
    ...
    env: { ...process.env, CARAPACE_STATE_DIR: args.stateDir },
  });
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `carapace-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await carapaceCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("carapace message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "carapace"
            ? await readMessagesWithCarapace({
                carapaceBin: args.carapaceBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Carapace.`,
        diagnostics: {
          bindingCandidates: [
            {
