...
unset CARAPACE_GATEWAY_TOKEN CARAPACE_GATEWAY_PASSWORD CARAPACE_GATEWAY_URL CARAPACE_GATEWAY_PORT
unset CARAPACE_CONFIG_PATH CARAPACE_STATE_DIR CARAPACE_PROFILE CARAPACE_HOME CARAPACE_AGENT_DIR
...
NODE
  ...
  CARAPACE_CU_RIG_PLATFORM="${rig_values[0]}"
  CARAPACE_CU_RIG_ROOT="${rig_values[1]}"
  CARAPACE_CU_RIG_PROFILE="${rig_values[2]}"
  CARAPACE_CU_RIG_PORT="${rig_values[3]}"
  CARAPACE_CU_RIG_APP="${rig_values[4]}"
  CARAPACE_CU_RIG_APP_STATE="${rig_values[5]}"
  CARAPACE_CU_RIG_GATEWAY_CONFIG="${rig_values[6]}"
  CARAPACE_CU_RIG_AGENT_STATE="${rig_values[7]}"
  CARAPACE_CU_RIG_NODE_CONFIG="${rig_values[8]}"
  CARAPACE_CU_RIG_NODE_STATE="${rig_values[9]}"
  CARAPACE_CU_RIG_DISPLAY="${rig_values[10]}"
  CARAPACE_CU_RIG_GATEWAY_STATE="${rig_values[11]}"
  [[ "$CARAPACE_CU_RIG_ROOT" == "$repo_root" ]] ||
    fail "rig belongs to a different checkout: $CARAPACE_CU_RIG_ROOT"
  [[ "$CARAPACE_CU_RIG_PROFILE" =~ ^[A-Za-z0-9][A-Za-z0-9_-]+$ ]] || fail "invalid rig profile"
  [[ "$CARAPACE_CU_RIG_PORT" =~ ^[0-9]+$ ]] || fail "invalid rig port"
  ((CARAPACE_CU_RIG_PORT >= 1024 && CARAPACE_CU_RIG_PORT <= 65535)) || fail "invalid rig port"
  ((CARAPACE_CU_RIG_PORT != 18789)) || fail "operator port is not valid rig state"
  [[ "$CARAPACE_CU_RIG_GATEWAY_CONFIG" == "$scratch/gateway.json" ]] ||
    fail "rig gateway config is outside its scratch directory"
  [[ "$CARAPACE_CU_RIG_AGENT_STATE" == "$scratch/agent-state" ]] ||
    fail "rig agent state is outside its scratch directory"
  case "$CARAPACE_CU_RIG_PLATFORM" in
    macos)
      [[ "$CARAPACE_CU_RIG_APP" = /* ]] || fail "invalid rig app path"
      [[ "$CARAPACE_CU_RIG_APP_STATE" == "$HOME/.carapace-$CARAPACE_CU_RIG_PROFILE" ]] ||
        fail "rig app state does not match its profile"
      [[ "$CARAPACE_CU_RIG_GATEWAY_STATE" == "$CARAPACE_CU_RIG_APP_STATE" ]] ||
        fail "invalid macOS gateway state"
      ;;
    linux)
      [[ "$CARAPACE_CU_RIG_NODE_CONFIG" == "$scratch/node.json" ]] ||
        fail "rig node config is outside its scratch directory"
      [[ "$CARAPACE_CU_RIG_NODE_STATE" == "$scratch/node-state" ]] ||
        fail "rig node state is outside its scratch directory"
      [[ "$CARAPACE_CU_RIG_GATEWAY_STATE" == "$scratch/gateway-state" ]] ||
        fail "rig gateway state is outside its scratch directory"
      [[ -n "$CARAPACE_CU_RIG_DISPLAY" ]] || fail "invalid Linux display"
      ;;
...
prepare() {
  ...
  local app_executable="$app_path/Contents/MacOS/Carapace"
  ...
  local app_state="$HOME/.carapace-$profile"
  local defaults_domain="ai.carapace.mac.profile.$profile"
  ...
  local app_config="$app_state/carapace.json"
  ...
  defaults write "$defaults_domain" carapace.macNodeIdentityProfile -string node
  defaults write "$defaults_domain" carapace.connectionMode -string remote
  defaults write "$defaults_domain" carapace.pauseEnabled -bool false
  defaults write "$defaults_domain" carapace.computerControlEnabled -bool true
  defaults write "$defaults_domain" carapace.computerControlProvider -string "$provider"
  defaults write "$defaults_domain" carapace.gatewayProjectRootPath -string "$repo_root"
  defaults write "$defaults_domain" carapace.onboardingSeen -bool true
  defaults write "$defaults_domain" carapace.onboardingVersion -int 8
...
NODE
  ...
  echo "fixture: $0 fixture $scratch 'Carapace CUA X11 Target' 'Carapace X11 Sentinel' 'W3-LINUX BEFORE'"
  echo "nodes:   $0 nodes $scratch"
}
...
run_gateway() {
  [[ $# -eq 1 ]] || { usage; exit 2; }
  load_rig "$1"
  require_unoccupied_port "$CARAPACE_CU_RIG_PORT"
  exec env \
    CARAPACE_CONFIG_PATH="$CARAPACE_CU_RIG_GATEWAY_CONFIG" \
    CARAPACE_STATE_DIR="$CARAPACE_CU_RIG_GATEWAY_STATE" \
    node "$repo_root/scripts/run-node.mjs" --profile "$CARAPACE_CU_RIG_PROFILE" \
      gateway run --port "$CARAPACE_CU_RIG_PORT" --auth token --bind loopback --verbose
}

run_app() {
  [[ $# -ge 1 && $# -le 2 ]] || { usage; exit 2; }
  load_rig "$1"
  [[ "$CARAPACE_CU_RIG_PLATFORM" == "macos" ]] || fail "app is available only for macOS rigs"
  local provider="${2:-peekaboo}"
  validate_provider "$provider"
  defaults write "ai.carapace.mac.profile.$CARAPACE_CU_RIG_PROFILE" \
    carapace.computerControlProvider -string "$provider"
  exec env CARAPACE_PROFILE="$CARAPACE_CU_RIG_PROFILE" \
    "$CARAPACE_CU_RIG_APP/Contents/MacOS/Carapace"
}

run_node() {
  [[ $# -eq 1 ]] || { usage; exit 2; }
  load_rig "$1"
  [[ "$CARAPACE_CU_RIG_PLATFORM" == "linux" ]] || fail "node is available only for Linux rigs"
  require_linux_x11
  [[ "$DISPLAY" == "$CARAPACE_CU_RIG_DISPLAY" ]] || fail "DISPLAY does not match rig state"
  exec env \
    DISPLAY="$CARAPACE_CU_RIG_DISPLAY" \
    CARAPACE_CONFIG_PATH="$CARAPACE_CU_RIG_NODE_CONFIG" \
    CARAPACE_STATE_DIR="$CARAPACE_CU_RIG_NODE_STATE" \
    node "$repo_root/scripts/run-node.mjs" --profile "$CARAPACE_CU_RIG_PROFILE" \
      node run --host 127.0.0.1 --port "$CARAPACE_CU_RIG_PORT" \
      --display-name "Carapace CUA X11 Live Proof"
}
...
run_fixture() {
  ...
  local scratch="$1"
  load_rig "$scratch"
  [[ "$CARAPACE_CU_RIG_PLATFORM" == "linux" ]] || fail "fixture is available only for Linux rigs"
  require_linux_x11
  [[ "$DISPLAY" == "$CARAPACE_CU_RIG_DISPLAY" ]] || fail "DISPLAY does not match rig state"
...
run_nodes() {
  ...
  exec env \
    CARAPACE_CONFIG_PATH="$CARAPACE_CU_RIG_GATEWAY_CONFIG" \
    CARAPACE_STATE_DIR="$scratch/cli-state" \
...
run_approve() {
  ...
  exec env \
    CARAPACE_CONFIG_PATH="$CARAPACE_CU_RIG_GATEWAY_CONFIG" \
    CARAPACE_STATE_DIR="$CARAPACE_CU_RIG_AGENT_STATE" \
...
run_proof() {
  ...
  local provider="$2"
  validate_provider "$provider"
  if [[ "$CARAPACE_CU_RIG_PLATFORM" == "linux" ]]; then
