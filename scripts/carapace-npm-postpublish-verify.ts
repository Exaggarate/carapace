...
// Carapace Npm Postpublish Verify script supports Carapace repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./carapace-npm-release-check.ts";
...
const PACKAGED_BUNDLED_PLUGIN_ARTIFACTS = new Set(
  listBundledPluginPackArtifacts({
    env: { ...process.env, CARAPACE_BUNDLED_PLUGIN_BUILD_IDS: undefined },
  }),
);
...
type CarapaceNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function carapaceNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/carapace-npm-postpublish-verify.ts <version>";
}

export function parseCarapaceNpmPostpublishVerifyArgs(
  argv: readonly string[],
): CarapaceNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(carapaceNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown carapace npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected carapace npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `carapace@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`carapace@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/Exaggarate/carapace";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/carapace-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted Carapace GitHub release workflow.`,
    );
  }
...
function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@carapace/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "carapace.cmd")
    : pathPosix.join(prefixDir, "bin", "carapace");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  await verifyNpmProvenanceAttestation({
    ...
    integrity,
    attestations,
    expectedWorkflowRef: process.env.CARAPACE_NPM_EXPECTED_WORKFLOW_REF,
    expectedWorkflowSha: process.env.CARAPACE_NPM_EXPECTED_WORKFLOW_SHA,
  });
  console.log(
    `carapace-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `carapace-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "carapace");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed carapace binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`carapace-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseCarapaceNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(carapaceNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `carapace-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`carapace-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
