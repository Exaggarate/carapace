...
- regenerates apps/ios/Carapace.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical Carapace Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${CARAPACE_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
RELEASE_GIT_COMMIT="$(CARAPACE_REQUIRE_BUILD_METADATA=1 carapace_resolve_git_commit "${ROOT_DIR}")"
...
${RELEASE_SIGNING_XCCONFIG}
CARAPACE_DEVELOPMENT_TEAM = ${TEAM_ID}
CARAPACE_IOS_SELECTED_TEAM = ${TEAM_ID}
CARAPACE_APP_BUNDLE_ID = ai.carapacefoundation.app
CARAPACE_SHARE_BUNDLE_ID = ai.carapacefoundation.app.share
CARAPACE_ACTIVITY_WIDGET_BUNDLE_ID = ai.carapacefoundation.app.activitywidget
CARAPACE_WATCH_APP_BUNDLE_ID = ai.carapacefoundation.app.watchkitapp
CARAPACE_CODE_SIGN_ENTITLEMENTS = Sources/CarapaceAppAttest.entitlements
CARAPACE_APNS_ENTITLEMENT_ENVIRONMENT = production
CARAPACE_APP_ATTEST_ENVIRONMENT = production
CARAPACE_PUSH_MODE = appStore
CARAPACE_PUSH_RELAY_BASE_URL =
EOF
