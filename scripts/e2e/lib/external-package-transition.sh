...
carapace_e2e_external_package_transition() (
  ...
  local backup_dir
  backup_dir="$(mktemp -d "${TMPDIR:-/tmp}/carapace-transition-backup.XXXXXX")" || return "$?"
  trap 'rm -rf "$backup_dir"' EXIT
  chmod 700 "$backup_dir" || return "$?"
  carapace backup create --verify --output "$backup_dir/before.tar.gz" --json \
  ...
  carapace_e2e_install_package "$evidence_dir/install.log" "external candidate installation" || return "$?"
