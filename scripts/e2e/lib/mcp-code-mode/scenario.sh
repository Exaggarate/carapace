...
if [ "$mode" = mock ]; then
  mock_pid="$(carapace_e2e_start_mock_openai "$mock_port" /tmp/mcp-code-mode-mock-openai.log)"
...
carapace_prepublish_plugin_registry_start_mounted \
  "$log_prefix-plugin-registry" plugin_registry_pid '["@carapace/codex"]'
...
carapace_e2e_fixture_plugin_command carapace_e2e_run_command node "$entry" -- \
  plugins install "${codex_install_args[@]}" >"$log_prefix-plugin-install.log" 2>&1

gateway_pid="$(carapace_e2e_start_gateway "$entry" "$port" "$log_prefix-gateway.log")"
carapace_e2e_wait_gateway_ready "$gateway_pid" "$log_prefix-gateway.log" 480 "$port"
