...
dump_debug_logs() {
  ...
  for file in \
    ...
    "$CLIENT_REJECT_LOG" \
    "$MOCK_REQUEST_LOG" \
    "$CARAPACE_STATE_DIR/carapace.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
      CARAPACE_E2E_LOG_TAIL_BYTES=8192 carapace_e2e_print_log "$file" >&2
    fi
  done
...
PORT="$PORT" CARAPACE_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >"$CLIENT_SUCCESS_LOG" 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" CARAPACE_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >"$CLIENT_REJECT_LOG" 2>&1
