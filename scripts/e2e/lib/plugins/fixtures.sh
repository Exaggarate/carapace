CARAPACE_PLUGINS_FIXTURE_PID_FILES=()
CARAPACE_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
CARAPACE_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

carapace_plugins_read_positive_int_env() {
...
carapace_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
carapace_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${CARAPACE_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      carapace_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}
...
carapace_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
carapace_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

carapace_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(carapace_plugins_read_positive_int_env CARAPACE_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(carapace_plugins_read_nonnegative_decimal_env CARAPACE_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F carapace_e2e_stop_process >/dev/null 2>&1; then
    carapace_e2e_stop_process "$pid"
    return "$?"
  fi
  carapace_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! carapace_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return 0; }
    sleep "$interval"
  done
  carapace_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

carapace_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(carapace_plugins_read_positive_int_env CARAPACE_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
carapace_plugins_validate_fixture_log_print_bytes() {
  carapace_plugins_read_positive_int_env CARAPACE_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

carapace_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  CARAPACE_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  carapace_plugins_install_fixture_cleanup_trap
}

carapace_plugins_install_fixture_cleanup_trap() {
  if [[ "${CARAPACE_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *carapace_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "CARAPACE_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  CARAPACE_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap carapace_plugins_fixture_exit_trap EXIT
}

carapace_plugins_fixture_exit_trap() {
  local status="$?"
  carapace_plugins_cleanup_fixture_servers
  if [[ -n "${CARAPACE_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$CARAPACE_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.carapace.extensions = ["./index.js", " "];
...
start_npm_fixture_registry() {
  ...
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$server_pid" 2>/dev/null; then
      carapace_plugins_print_fixture_log "$server_log"
      return 1
    fi
    sleep 0.1
  done
