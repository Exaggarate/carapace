...
const pluginRecordSnapshot = () => {
  const config = readJson(carapacePath("carapace.json"));
...
function carapacePath(...parts) {
  return path.join(home, ".carapace", ...parts);
}
...
function seedInstallState() {
  const pluginRoot = carapacePath("extensions", "lossless-claw");
  const pluginSource = path.join(pluginRoot, "index.js");
  const pluginManifest = path.join(pluginRoot, "carapace.plugin.json");
  ...
  writeJson(process.env.CARAPACE_CONFIG_PATH, { plugins: {} });
  const index = {
    version: 1,
    warning: "DO NOT EDIT. This file is generated by Carapace plugin registry commands.",
...
function assertCorruptTargetUnavailable(updateJsonPath, pluginId) {
  ...
  if (
    ...
    !details.includes(`requires @carapace/${pluginId}@0.0.1 for core `) ||
    !details.includes(`Package not found on npm: @carapace/${pluginId}@0.0.1`) ||
...
function isCorruptPluginDisabledAfterUpdate(evidence, pluginId) {
  ...
  return (
    ...
    message.includes("Carapace will continue without it")
  );
}
...
function assertCorruptPluginDetails(plugins, pluginId) {
  ...
  const expectedFragments = disabledAfterFailure
    ? [
        ...
        "Carapace will continue without it",
