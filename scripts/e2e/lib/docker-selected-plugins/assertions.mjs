...
const expectedCommit = process.env.CARAPACE_E2E_EXPECTED_GIT_COMMIT?.toLowerCase();
const expectedBuiltAt = new Date(
  process.env.CARAPACE_E2E_EXPECTED_BUILD_TIMESTAMP ?? "",
).toISOString();
...
for (const [pluginId, expected] of Object.entries(selected)) {
  ...
  assertFile(path.join(pluginRoot, "carapace.plugin.json"));
  assertFile(path.join(pluginRoot, "package.json"));

  const manifest = readJson(path.join(pluginRoot, "carapace.plugin.json"));
  ...
  assert(
    packageJson.carapace?.extensions?.includes("./index.js"),
    `${pluginId} package entry was not rewritten to ./index.js`,
  );
  if (expected.entries.includes("setup-entry.js")) {
    assert(
      packageJson.carapace?.setupEntry === "./setup-entry.js",
      `${pluginId} setup entry was not rewritten to ./setup-entry.js`,
    );
  }

  const inspect = readJson(`/tmp/carapace-${pluginId}-inspect.json`);
