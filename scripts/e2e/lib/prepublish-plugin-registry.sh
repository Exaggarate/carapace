...
carapace_prepublish_plugin_registry_configure_docker_args() {
  ...
  local source_sha="${CARAPACE_DOCKER_E2E_SELECTED_SHA:-}"
  local candidate_version="${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_CANDIDATE_VERSION:-}"
  local manifest_sha256="${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_MANIFEST_SHA256:-}"
  ...
  CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_DOCKER_ARGS=(
    -e CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_DIR=/tmp/carapace-prepublish-plugin-registry
    -e CARAPACE_DOCKER_E2E_SELECTED_SHA="$source_sha"
    -e CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_CANDIDATE_VERSION="$candidate_version"
    -e CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_MANIFEST_SHA256="$manifest_sha256"
    -v "$resolved_registry_dir:/tmp/carapace-prepublish-plugin-registry:ro"
    --entrypoint /opt/carapace-e2e/scripts/e2e/lib/prepublish-plugin-registry.sh
  )
}
...
carapace_prepublish_plugin_registry_start() {
  ...
  local artifact_script="${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_ARTIFACT_SCRIPT:-$helper_dir/../../prepublish-plugin-registry-artifact.mjs}"
  local server_script="${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_SERVER_SCRIPT:-$helper_dir/plugins/npm-registry-server.mjs}"
  local required_packages_json="${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_REQUIRED_PACKAGES_JSON:-[]}" registry_args=()
...
for (const entry of manifest.packages) {
  ...
  if (entry.name === "carapace") continue;
...
NODE
  ...
  local merge_upstream="${CARAPACE_NPM_REGISTRY_MERGE_UPSTREAM:-versions}"
  local dist_tags="${CARAPACE_NPM_REGISTRY_DIST_TAGS-}"
  if [ -z "${CARAPACE_NPM_REGISTRY_DIST_TAGS+x}" ]; then
  ...
  CARAPACE_NPM_REGISTRY_DIST_TAGS="$dist_tags" \
    CARAPACE_NPM_REGISTRY_MERGE_UPSTREAM="${artifact_dir:+$merge_upstream}" \
...
carapace_prepublish_plugin_registry_run_mounted() (
  set -euo pipefail
  local registry_root registry_pid="" command_pid=""
  registry_root="$(mktemp -d "${TMPDIR:-/tmp}/carapace-prepublish-registry.XXXXXX")"
  ...
  CARAPACE_NPM_REGISTRY_DIST_TAGS="" CARAPACE_NPM_REGISTRY_MERGE_UPSTREAM=1 \
    carapace_prepublish_plugin_registry_start_mounted "$registry_root" registry_pid '[]'
  if [ -n "$registry_pid" ]; then
    export CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_URL="$NPM_CONFIG_REGISTRY"
  fi
  "$@" <&0 &
  command_pid="$!"
  wait "$command_pid"
...
carapace_prepublish_plugin_registry_start_mounted() {
  ...
  CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_REQUIRED_PACKAGES_JSON="$required_packages_json" \
    carapace_prepublish_plugin_registry_start \
