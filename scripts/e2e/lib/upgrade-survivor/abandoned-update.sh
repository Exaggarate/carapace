...
run_abandoned_update_survivor() {
  if [ "$baseline_version" != "2026.9.2" ] || [ "$UPDATE_RESTART_MODE" != "auto-auth" ]; then
    echo "abandoned-update requires baseline carapace@2026.9.2 and auto-auth service mode" >&2
    return 1
  fi
  ...
  phase configure-gateway carapace config set gateway "$gateway_config" --strict-json
  phase disable-fixture-plugins carapace config set plugins.enabled false --strict-json
  ...
  carapace_e2e_maybe_timeout "$COMMAND_TIMEOUT" carapace update --dry-run --yes --no-restart \
    --tag "$(candidate_update_spec)" --json >"$ARTIFACT_ROOT/baseline-preview.json" 2>"$ARTIFACT_ROOT/baseline-preview.err"
  phase seed-abandoned-run node "$helper" seed "$CARAPACE_STATE_DIR" "$ARTIFACT_ROOT"
  ...
  phase assert-preserved-run node "$helper" preserved "$CARAPACE_STATE_DIR" "$ARTIFACT_ROOT"
  ...
  phase assert-no-automatic-recovery node "$helper" preserved "$CARAPACE_STATE_DIR" "$ARTIFACT_ROOT"
  ...
  carapace_e2e_maybe_timeout "$COMMAND_TIMEOUT" carapace update repair --yes --json \
    >"$REPAIR_JSON" 2>"$ARTIFACT_ROOT/repair.err" || repair_status=$?
  printf '%s\n' "$repair_status" >"$ARTIFACT_ROOT/repair.exit"
  carapace_e2e_maybe_timeout "$COMMAND_TIMEOUT" carapace update status --json \
  ...
  phase assert-ledger-recovery node "$helper" recovered "$CARAPACE_STATE_DIR" "$ARTIFACT_ROOT"
