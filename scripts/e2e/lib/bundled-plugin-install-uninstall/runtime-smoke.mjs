...
const RUNTIME_PORT_BASE_ENV = "CARAPACE_BUNDLED_PLUGIN_RUNTIME_PORT_BASE";
...
const WATCHDOG_MS = readPositiveIntEnv("CARAPACE_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", 1000);
const READY_TIMEOUT_MS = readPositiveIntEnv("CARAPACE_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000);
const RPC_TIMEOUT_MS = readPositiveIntEnv("CARAPACE_BUNDLED_PLUGIN_RUNTIME_RPC_MS", 60000);
const RPC_READY_TIMEOUT_MS = readPositiveIntEnv(
  "CARAPACE_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS",
  210000,
);
const COMMAND_TIMEOUT_MS = readPositiveIntEnv("CARAPACE_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", 120000);
const HTTP_PROBE_TIMEOUT_MS = readPositiveIntEnv("CARAPACE_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", 5000);
...
function manifestPath(pluginDir, pluginRoot) {
  const candidates = [
    ...(isNonEmptyString(pluginRoot) ? [path.join(pluginRoot, "carapace.plugin.json")] : []),
    path.join(process.cwd(), "dist", "extensions", pluginDir, "carapace.plugin.json"),
    path.join(process.cwd(), "dist-runtime", "extensions", pluginDir, "carapace.plugin.json"),
  ];
...
function configPathFromEnv(env = process.env) {
  return (
    env.CARAPACE_CONFIG_PATH || path.join(env.HOME || os.homedir(), ".carapace", "carapace.json")
  );
}
...
export function readSmokeTtsConfig(config) {
  return process.env.CARAPACE_FROZEN_PLUGIN_PRERELEASE_FIXTURE_DIALECT === "legacy"
    ? config.messages?.tts
    : config.tts;
}
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        CARAPACE_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
export async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
  ...
  try {
    const { stdout } = await runCommand("node", args, {
      env: {
        ...
        ...options.env,
        CARAPACE_NO_ONBOARD: "1",
        CARAPACE_STATE_DIR: rpcStateDir,
      },
    });
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex, pluginRoot) {
  ...
  const entrypoint = process.env.CARAPACE_ENTRY;
  if (!entrypoint) {
    throw new Error("missing CARAPACE_ENTRY");
  }
  ...
  const logPath = `/tmp/carapace-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex, pluginRoot) {
  ...
  const logPath = `/tmp/carapace-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const logPath = `/tmp/carapace-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
export function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `carapace-${label}-`));
