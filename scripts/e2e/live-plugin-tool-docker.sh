...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-live-plugin-tool-e2e" CARAPACE_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${CARAPACE_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${CARAPACE_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${CARAPACE_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${CARAPACE_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${CARAPACE_TESTBOX_PROFILE_FILE:-$HOME/.carapace-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${CARAPACE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "CARAPACE_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires CARAPACE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e CARAPACE_LIVE_PLUGIN_TOOL_MODEL="${CARAPACE_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.6-luna}" \
  ...
  -e "CARAPACE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${CARAPACE_LIVE_PLUGIN_TOOL_MODEL:?missing CARAPACE_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@carapace/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="Carapace E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${CARAPACE_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/carapace-agent.json ]; then
    echo "--- /tmp/carapace-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/carapace-agent.json >&2 || true
    echo >&2
  fi
  carapace_e2e_dump_logs \
    /tmp/carapace-install.log \
    /tmp/carapace-plugin-install.log \
    /tmp/carapace-plugin-enable.log \
    /tmp/carapace-plugins-list.json \
    /tmp/carapace-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/carapace-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/carapace-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
carapace_e2e_fixture_plugin_command carapace -- plugins install "npm-pack:$plugin_tgz" --force >/tmp/carapace-plugin-install.log 2>&1
node --disable-warning=ExperimentalWarning scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
carapace plugins enable "$PLUGIN_ID" >/tmp/carapace-plugin-enable.log 2>&1
carapace plugins list --json >/tmp/carapace-plugins-list.json
carapace plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/carapace-plugin-inspect.json
...
carapace agent --local \
  ...
  --thinking off \
  --timeout "${CARAPACE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/carapace-agent.json 2>/tmp/carapace-agent.err
