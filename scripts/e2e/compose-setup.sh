...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-docker-e2e-functional:local")"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz compose-setup "${CARAPACE_CURRENT_PACKAGE_TGZ:-}")"
IDENTITY_PATH="${CARAPACE_DOCKER_ARTIFACT_IDENTITY_PATH:-$ROOT_DIR/.artifacts/docker-tests/compose-setup-identities.json}"
PROJECT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/carapace-compose-proof.XXXXXX")"
PROJECT_NAME="carapace-compose-proof-$$"
...
const actual =
  kind === "compose"
    ? payload?.services?.["carapace-gateway"]?.healthcheck?.test
...
wait_for_gateway_health() {
  ...
  "${COMPOSE[@]}" logs --no-color carapace-gateway >&2 || true
  return 1
}
...
assert_probe_endpoints() {
  local label="$1"
  local output_path="$2"
  "${COMPOSE[@]}" exec -T carapace-gateway node - "$label" >"$output_path" <<'NODE'
const label = process.argv[2];
...
cat >"$PROJECT_DIR/config/carapace.json" <<EOF
{
  "gateway": {
    "mode": "local",
...
export CARAPACE_CONFIG_DIR="$PROJECT_DIR/config"
export CARAPACE_WORKSPACE_DIR="$PROJECT_DIR/config/workspace"
export CARAPACE_AUTH_PROFILE_SECRET_DIR="$PROJECT_DIR/auth-profile"
export CARAPACE_GATEWAY_TOKEN="$TOKEN"
export CARAPACE_BRIDGE_PORT=0
export CARAPACE_MSTEAMS_PORT=0
export CARAPACE_DISABLE_BONJOUR=1
export CARAPACE_CURRENT_PACKAGE_TGZ="$PACKAGE_TGZ"
...
CARAPACE_GATEWAY_PORT="$(node <<'NODE'
...
export CARAPACE_GATEWAY_PORT
printf 'CARAPACE_GATEWAY_PORT=%s\n' "$CARAPACE_GATEWAY_PORT" >"$PROJECT_DIR/.env"
...
"${COMPOSE[@]}" up -d --no-build carapace-gateway
GATEWAY_ID="$("${COMPOSE[@]}" ps -q carapace-gateway)"
if [ -z "$GATEWAY_ID" ]; then
  echo "Compose did not create carapace-gateway" >&2
  exit 1
fi
...
"${COMPOSE[@]}" exec -T carapace-gateway sh -lc 'node dist/index.js gateway health --token "$CARAPACE_GATEWAY_TOKEN"'
assert_auth_rejected \
  "gateway service" \
  "$PROJECT_DIR/gateway-wrong-token.json" \
  "$PROJECT_DIR/gateway-wrong-token.err" \
  "${COMPOSE[@]}" exec -T carapace-gateway \
  node dist/index.js gateway health --token "$WRONG_TOKEN" --json
"${COMPOSE[@]}" exec -T carapace-gateway node dist/index.js gateway health --token "$TOKEN" --json >"$PROJECT_DIR/gateway-health.json"
...
assert_auth_rejected \
  ...
  carapace-cli gateway health --token "$WRONG_TOKEN" --json
"${COMPOSE[@]}" run -T --no-deps --name "$CLI_NAME" carapace-cli gateway health --token "$TOKEN" --json >"$PROJECT_DIR/cli-health.json"
...
"${COMPOSE[@]}" logs --no-color carapace-gateway >"$PROJECT_DIR/gateway-compose.log"
...
GATEWAY_VERSION="$("${COMPOSE[@]}" exec -T carapace-gateway node -p "require('./package.json').version")"
...
node --import tsx "$ROOT_DIR/scripts/e2e/lib/docker-artifact-proof/write-identities.ts" \
  ...
  --detail "gateway:carapaceVersion=$GATEWAY_VERSION" \
