...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-kitchen-sink-rpc-e2e" CARAPACE_KITCHEN_SINK_RPC_E2E_IMAGE)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env CARAPACE_KITCHEN_SINK_MAX_MEMORY_MIB 2048)"
...
CONTAINER_NAME="carapace-kitchen-sink-rpc-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-kitchen-sink-rpc.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-kitchen-sink-rpc-stats.XXXXXX")"
...
DOCKER_ENV_ARGS=(
  ...
  -e CARAPACE_ENTRY=/app/carapace.mjs
)
capability_status=0
...
for env_name in \
  CARAPACE_KITCHEN_SINK_NPM_SPEC \
  CARAPACE_KITCHEN_SINK_PLUGIN_ID \
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" "${DOCKER_ENV_ARGS[@]}" -i "$IMAGE_NAME" \
  bash -lc "source scripts/lib/carapace-e2e-instance.sh; carapace_e2e_run_script_entrypoint scripts/e2e/kitchen-sink-rpc-walk" >"$RUN_LOG" 2>&1 &
docker_pid="$!"
