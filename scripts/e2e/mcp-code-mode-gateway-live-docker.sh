...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-mcp-code-mode-gateway-live-e2e" CARAPACE_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env CARAPACE_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="carapace-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${CARAPACE_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${CARAPACE_TESTBOX_PROFILE_FILE:-$HOME/.carapace-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t carapace-mcp-code-mode-live-log.XXXXXX)"
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "CARAPACE_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "CARAPACE_MCP_CODE_MODE_MODEL=${CARAPACE_MCP_CODE_MODE_LIVE_MODEL:-carapace/main}" \
  "${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_DOCKER_ARGS[@]}" \
  "$IMAGE_NAME" \
