...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-plugin-update-e2e" CARAPACE_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${CARAPACE_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${CARAPACE_CURRENT_PACKAGE_TGZ:-}")"
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
