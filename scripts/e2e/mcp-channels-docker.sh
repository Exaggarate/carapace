...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-mcp-channels-e2e" CARAPACE_IMAGE)"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="carapace-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t carapace-mcp-client-log.XXXXXX)"
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-channels empty)"
DOCKER_ENV_ARGS=()
capability_status=0
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(carapace_e2e_start_mock_openai \"\$mock_port\" /tmp/mcp-channels-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(carapace_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-channels-gateway.log)\"
    carapace_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-channels-gateway.log 480 $PORT
