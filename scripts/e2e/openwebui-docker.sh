#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Carapace Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-openwebui-e2e" CARAPACE_OPENWEBUI_E2E_IMAGE)"
...
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env CARAPACE_OPENWEBUI_MAX_MEMORY_MIB 8192)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env CARAPACE_OPENWEBUI_MAX_CPU_PERCENT 1600)"
...
MODEL="${CARAPACE_OPENWEBUI_MODEL:-openai/gpt-5.6-luna}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${CARAPACE_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="$(docker_e2e_read_tcp_port_env CARAPACE_OPENWEBUI_GATEWAY_PORT 18789)"
WEBUI_PORT="$(docker_e2e_read_tcp_port_env CARAPACE_OPENWEBUI_PORT 8080)"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${CARAPACE_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${CARAPACE_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="carapace-openwebui-e2e-$$"
GW_NAME="carapace-openwebui-gateway-$$"
OW_NAME="carapace-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${CARAPACE_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
DOCKER_PULL_TIMEOUT="${CARAPACE_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROBE_FETCH_TIMEOUT_MS="${CARAPACE_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL * 1000 + 60000))}"
validate_positive_int CARAPACE_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"
DOCKER_COMMAND_TIMEOUT="${CARAPACE_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL + 90))s}"
...
PROFILE_FILE="${CARAPACE_TESTBOX_PROFILE_FILE:-$HOME/.carapace-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/carapace-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/carapace-openwebui-config.XXXXXX.json)"
    CARAPACE_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    ...
    carapace_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "CARAPACE_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Carapace E2E" \
  -e WEBUI_SECRET_KEY="carapace-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Carapace E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="carapace/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "CARAPACE_HTTP_PROBE_BEARER='$TOKEN' CARAPACE_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Carapace smoke..."
set +e
