...
VM_NAME="Windows 11"
CARAPACE_PARALLELS_WINDOWS_API=1
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TODAY="$(date +%F)"
CLEAN_SNAPSHOT="windows-11-clean-os-${TODAY}"
BASELINE_SNAPSHOT="pre-carapace-native-e2e-${TODAY}"
GUEST_PROFILE=""
GUEST_PROFILE_PS=""
GUEST_ARCH=""
...
SNAPSHOT=""
SECURE_STAGE_DIR="C:/ProgramData/CarapacePrerequisiteInstallers"
COMMAND="help"
if [[ "${CARAPACE_PARALLELS_WINDOWS_LIBRARY_ONLY:-0}" != "1" ]]; then
  COMMAND="${1:-help}"
  if [[ $# -gt 0 ]]; then
    shift
  fi
...
Commands:
  ...
  prepare     Create a clean snapshot, provision the reusable Carapace baseline, and snapshot it.
  verify      Verify base prerequisites and prove the guest contains no Carapace product state.
...
It installs only reusable Carapace prerequisites: WSL platform/package, Git, and Node/npm.
It refuses to create the baseline when the guest contains an Carapace CLI, app package, process,
...
create_clean_snapshot_if_raw() {
  ...
  create_snapshot "$CLEAN_SNAPSHOT" "Clean Windows baseline before Carapace development prerequisites."
}
...
if (Get-Command carapace.cmd -ErrorAction SilentlyContinue) { $dirty.Add('carapace.cmd on PATH') }
if (Test-Path (Join-Path $env:USERPROFILE '.carapace')) { $dirty.Add('Carapace CLI state directory exists') }
if (Test-Path (Join-Path $env:APPDATA 'CarapaceTray')) { $dirty.Add('CarapaceTray AppData exists') }
if (Test-Path (Join-Path $env:APPDATA 'CarapaceTray-Dev')) { $dirty.Add('CarapaceTray-Dev AppData exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'CarapaceTray')) { $dirty.Add('Carapace Companion install/state directory exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'CarapaceTray-Dev')) { $dirty.Add('Carapace Companion dev install/state directory exists') }
if (Get-AppxPackage -Name '*Carapace*' -ErrorAction SilentlyContinue) { $dirty.Add('Carapace app package installed') }
if (Get-Process -Name '*Carapace*' -ErrorAction SilentlyContinue) { $dirty.Add('Carapace process running') }
$uninstallRoots = @(
  'HKCU:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/WOW6432Node/Microsoft/Windows/CurrentVersion/Uninstall/*'
)
if (Get-ItemProperty $uninstallRoots -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like 'Carapace Companion*' }) {
  $dirty.Add('Carapace Companion uninstall registration exists')
}
...
ensure_wsl_package() {
  ...
  run_windows_installer msiexec.exe /i 'C:\ProgramData\CarapacePrerequisiteInstallers\WSL.msi' /qn /norestart '/L*v' 'C:\Windows\Temp\carapace-wsl-install.log'
  finish_installer_reboot
  ...
  ensure_wsl_default_version
  guest_system_ps "Remove-Item -LiteralPath '${wsl_msi}','C:/Windows/Temp/carapace-wsl-install.log' -Force -ErrorAction SilentlyContinue"
}
...
winget_download() {
  ...
  local download_dir="${GUEST_PROFILE//\//\\}\\Downloads\\CarapacePrereqs"
  guest_user_ps "if (Test-Path -LiteralPath '${GUEST_PROFILE_PS}/Downloads/CarapacePrereqs') { Remove-Item -LiteralPath '${GUEST_PROFILE_PS}/Downloads/CarapacePrereqs' -Recurse -Force }"
...
downloaded_installer() {
  local pattern="$1"
  guest_user_ps "Get-ChildItem -LiteralPath '${GUEST_PROFILE_PS}/Downloads/CarapacePrereqs' -File | Where-Object { \$_.Name -like '${pattern}' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1 -ExpandProperty FullName" | tr -d '\r' | tail -n 1
}
...
ensure_node() {
  ...
  guest_node_supported || die "installed Node.js does not satisfy the Carapace runtime requirement"
}
...
prepare() {
  ...
  ensure_wsl_default_version
  verify_baseline
  create_snapshot "$BASELINE_SNAPSHOT" "E2E-ready Carapace Windows baseline with WSL 2, Git, Node/npm, and no Carapace product state."
