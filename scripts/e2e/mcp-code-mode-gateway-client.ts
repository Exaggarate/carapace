// Mcp Code Mode Gateway Client script supports Carapace repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    ...
    timeoutMs: readPositiveIntEnv("CARAPACE_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.CARAPACE_MCP_CODE_MODE_MODEL?.trim() || "carapace/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing CARAPACE_STATE_DIR");
  ...
  const response = await fetchJson(`${gatewayUrl.replace(/\/$/, "")}/v1/responses`, {
    ...
    headers: {
      ...
      "x-carapace-session-key": MCP_CODE_MODE_SESSION_KEY,
