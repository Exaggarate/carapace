...
// Linux Smoke script supports Carapace repository automation.
...
function parseCarapacePackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareCarapacePackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseCarapacePackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  private disableBonjour = parseBoolEnv(process.env.CARAPACE_PARALLELS_LINUX_DISABLE_BONJOUR);
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("carapace-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("carapace-parallels-linux-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("carapace-main-fresh.tgz"),
    );
    ...
    await this.phase("fresh.install-companions", 600, () =>
      installSmokeRuntimeCompanions({
        ...
        readCli: (args) => this.guestExec(["carapace", ...args]),
        installCli: (args) => {
          this.guestExec(["carapace", ...args]);
        },
      }),
    );
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("carapace-main-upgrade.tgz"),
    );
...
npm uninstall -g carapace >/dev/null 2>&1 || true
rm -rf /root/.carapace /root/.npm/_cacache
rm -f /tmp/carapace-parallels-linux-gateway.log`);
  }

  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/carapace-install.sh");
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "CARAPACE_NO_ONBOARD=1",
        "bash",
        "/tmp/carapace-install.sh",
        "--version",
    ...
    this.guestExec(["carapace", "--version"]);
  }
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["carapace", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.carapace/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@carapace/test-bad-plugin","version":"1.0.0","carapace":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/carapace.plugin.json" <<'JSON'
...
config_path = Path("/root/.carapace/carapace.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseCarapacePackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareCarapacePackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " CARAPACE_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "carapace gateway run" >/dev/null 2>&1 || true
rm -f /tmp/carapace-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env CARAPACE_HOME=/root CARAPACE_STATE_DIR=/root/.carapace CARAPACE_CONFIG_PATH=/root/.carapace/carapace.json CARAPACE_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
            this.auth.apiKeyValue,
          )} carapace gateway run --bind loopback --port 18789 --force >/tmp/carapace-parallels-linux-gateway.log 2>&1`,
        ) +
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["carapace", "gateway", "status", "--help"], { check: false });
  ...
  private verifyGatewayStatus(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guest.run(
        ["carapace", "gateway", "status", "--deep", "--require-rpc", "--timeout", "15000"],
        { check: false },
      );
      if (result.status === 0) {
        return;
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.carapace/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.carapace/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    ...
    this.guestExec(["carapace", "models", "set", this.auth.modelId]);
...
JSON
carapace config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["carapace", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env CARAPACE_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} carapace agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    return await extractLastCarapaceVersion(
      this.runDir,
      phaseId,
      /(Carapace [^\r\n]+ \([0-9a-f]{7,}\))/g,
    );
