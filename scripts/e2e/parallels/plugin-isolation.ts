// Plugin Isolation script supports Carapace repository automation.
...
export function posixCodexPlatformPackageRepairFunction(): string {
  ...
  state_home="\${CARAPACE_PARALLELS_HOME:-\${HOME:-}}"
  codex_manifest=""
  for candidate in "$state_home"/.carapace/npm/projects/*/node_modules/@carapace/codex/package.json; do
  ...
  project_root="\${codex_manifest%/node_modules/@carapace/codex/package.json}"
  cache_dir="$(mktemp -d "\${TMPDIR:-/tmp}/carapace-npm-cache.XXXXXX")"
...
export function windowsCodexPlatformPackageRepairFunction(): string {
  ...
  $projectsRoot = Join-Path $env:USERPROFILE '.carapace\npm\projects'
  $codexManifest = Get-ChildItem -Path $projectsRoot -Filter package.json -File -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match 'node_modules[\\/]@carapace[\\/]codex[\\/]package\.json$' } |
  ...
  $cacheDir = Join-Path ([System.IO.Path]::GetTempPath()) ('carapace-npm-cache-' + [guid]::NewGuid().ToString('N'))
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `CARAPACE_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:CARAPACE_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) ('carapace-parallels-plugin-isolation-' + [guid]::NewGuid().ToString('N') + '.cjs')
try {
@'
...
if ($LASTEXITCODE -ne 0) { throw "plugin isolation failed with exit code $LASTEXITCODE" }
  ...
  Remove-Item Env:CARAPACE_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue
}`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.CARAPACE_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.CARAPACE_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.CARAPACE_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@carapace", "codex"), {
  recursive: true,
  force: true,
});
