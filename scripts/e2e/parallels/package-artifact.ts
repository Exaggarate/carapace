// Package Artifact script supports Carapace repository automation.
...
export function resolveCarapaceRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("carapace@") ? rawValue.slice("carapace@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`carapace@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "carapace", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no carapace registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packCarapace(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "carapace-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `carapace-main-${shortHead}.tgz`);
...
async function acquirePackageLock(
  ...
  const timeoutMs = readPositiveIntEnv("CARAPACE_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("CARAPACE_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
