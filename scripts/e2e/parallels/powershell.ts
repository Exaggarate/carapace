// Powershell script supports Carapace repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:CARAPACE_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.carapace\\carapace.json' }
$agentTurnVersionText = Invoke-Carapace --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Carapace\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:CARAPACE_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:CARAPACE_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:CARAPACE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'carapace-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.CARAPACE_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.CARAPACE_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.CARAPACE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@carapace", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "carapace" };
  } else {
...
Remove-Item Env:CARAPACE_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:CARAPACE_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:CARAPACE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsCarapaceResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-CarapaceCommand {
  if ($script:CarapaceResolvedCommand) { return $script:CarapaceResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\carapace.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\carapace.ps1'
  }
  foreach ($name in @('carapace.cmd', 'carapace.ps1', 'carapace')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'carapace.cmd'
    $shimCandidates += Join-Path $npmPrefix 'carapace.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:CarapaceResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:CarapaceResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\carapace\carapace.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:CarapaceResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:CarapaceResolvedCommand
    }
  }
  throw 'carapace command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Carapace {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $CarapaceArgs)
  $command = Resolve-CarapaceCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @CarapaceArgs
    } else {
      & $command.Path @CarapaceArgs
    }
  } finally {
