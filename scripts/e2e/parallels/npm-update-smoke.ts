...
// Npm Update Smoke script supports Carapace repository automation.
...
import {
  ...
  resolveCarapaceRegistryVersion,
  resolveProviderAuth,
...
function resolveUpdateTimeouts(): UpdateTimeouts {
  return (cachedUpdateTimeouts ??= (() => {
    const seconds = readPositiveIntEnv("CARAPACE_PARALLELS_NPM_UPDATE_TIMEOUT_S", 2700);
...
function resolveFreshLaneTimeoutKillGraceMs(): number {
  return (cachedFreshLaneTimeoutKillGraceMs ??= readPositiveIntEnv(
    "CARAPACE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
    2_000,
  ));
}
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return resolveSecondsTimerMs(
    readPositiveIntEnv("CARAPACE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds),
  );
}
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: carapace@latest
  --update-target <target>    Target passed to guest 'carapace update --tag'.
...
function carapaceVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseCarapacePackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveCarapaceRegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("carapace-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("carapace-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    this.packageSpec = this.options.packageSpec || `carapace@${this.latestVersion}`;
    ...
    say(`Run same-guest carapace update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          CARAPACE_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    if (this.targetTarballPath) {
      ...
      this.artifact = await packCarapace({
      ...
      this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
        ? ""
        : resolveCarapaceRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
    ...
    if (this.targetTarballPath) {
      this.freshTargetSpec = this.targetRegistryUrl
        ? `carapace@${this.updateTargetPackageVersion}`
        : this.updateTargetTarball;
    }
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("carapace@") ? target : `carapace@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      this.macosVm,
      script,
      "carapace-parallels-npm-update-macos",
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveCarapaceRegistryVersion(this.packageSpec);
    const target = resolveCarapaceRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to carapace@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastCarapaceVersionFromLog(logPath);
  }
  ...
  private async configureTargets(): Promise<void> {
    ...
    if (this.options.betaValidation) {
      const version = resolveCarapaceRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `carapace@${version}`;
      say(`Beta validation target: carapace@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveCarapaceRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveCarapaceRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `carapace@${version}` : this.options.freshTargetSpec;
    }
  }

  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.CARAPACE_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion =
      this.targetTarballVersion ||
      (this.freshTargetSpec
        ? parseCarapacePackageSpecVersion(this.freshTargetSpec)
        : parseCarapacePackageSpecVersion(this.options.updateTarget));
    const targetFamily = carapaceVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = carapaceVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set CARAPACE_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
