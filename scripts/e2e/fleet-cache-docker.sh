...
scratch="$(mktemp -d /tmp/carapace-fleet-cache-e2e.XXXXXX)"
chmod 755 "$scratch"
PACKAGE_TGZ=""
cli_entry="$scratch/host-runtime/node_modules/carapace/carapace.mjs"
tenant=""
case_dir=""
uid=1501
interrupted=false
...
fleet() (
  ...
  exec timeout --foreground 180s sudo -n setpriv --reuid="$uid" --regid="$uid" --groups="$socket_gid" \
    env -i PATH="$PATH" HOME="$case_dir/home" CARAPACE_HOME="$case_dir/home" \
...
capture() {
  ...
  fleet logs "$tenant"
  docker inspect "carapace-cell-$tenant" | jq '.[0] | {
    Id, State, Image, user: .Config.User,
    environment: [.Config.Env[] | select(startswith("CARAPACE_GATEWAY_TOKEN=") | not)],
...
cleanup_cell() {
  ...
  if docker container inspect "carapace-cell-$tenant" >/dev/null 2>&1; then
    echo "Fleet proof left a cell container behind: $tenant" >&2
    return 1
  fi
  if docker network inspect "carapace-cell-$tenant-net" >/dev/null 2>&1; then
...
assert_cell() {
  ...
  docker inspect "carapace-cell-$tenant" | \
  ...
  docker exec "carapace-cell-$tenant" node -e '
...
run_case() {
  ...
  fleet start "$tenant"
  local port
  port="$(docker inspect "carapace-cell-$tenant" --format '{{(index (index .NetworkSettings.Ports "18789/tcp") 0).HostPort}}')"
...
run_previous_default_case() {
  ...
  local cache=""
  local keys=""
  local expected_cache=/home/node/.carapace/cache
  local -a overrides=()
  ...
  cli_entry="$scratch/previous-runtime/node_modules/carapace/carapace.mjs"
  fleet create "$tenant" --image "$image" --gateway-token fleet-cache-synthetic-token \
    "${overrides[@]}" --json
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz fleet-cache "${CARAPACE_CURRENT_PACKAGE_TGZ:-}")"
...
' "$scratch/host-runtime/node_modules/carapace/dist/build-info.json" "$source_sha"
umask 077
...
image_tag="ghcr.io/carapace/carapace:2026.8.2"
...
run_case mismatch 1501 /home/node/.carapace/cache '' ''
run_case override 1501 /home/node/.carapace/operator-cache XDG_CACHE_HOME '' \
  --env XDG_CACHE_HOME=/home/node/.carapace/operator-cache
run_case equal 1501 /home/node/.carapace/cache XDG_CACHE_HOME '' \
  --env XDG_CACHE_HOME=/home/node/.carapace/cache
run_case temporary 1501 /home/node/.carapace/cache TMPDIR /home/node/.carapace/operator-tmp \
  --env TMPDIR=/home/node/.carapace/operator-tmp
run_case image-user 1000 /home/node/.carapace/cache '' ''
run_case root-invoker 0 /home/node/.carapace/cache '' ''

(
  umask 022
  timeout --foreground 600s npm install --prefix "$scratch/previous-runtime" --no-save --no-package-lock \
    --no-audit --no-fund carapace@2026.8.2
)
"$node_bin" -e '
  const assert = require("node:assert/strict");
...
' "$scratch/previous-runtime/node_modules/carapace/package.json"
