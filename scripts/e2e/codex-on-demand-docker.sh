#!/usr/bin/env bash
# Installs Carapace and Codex from npm artifacts with explicit capability consent,
...
CODEX_ASSERTIONS="$(carapace_resolve_frozen_target_file "$TARGET_ROOT_DIR" \
  scripts/e2e/lib/codex-on-demand/assertions.mjs \
  "$ROOT_DIR/scripts/e2e/lib/codex-on-demand/assertions.mjs")"

IMAGE_NAME="$(docker_e2e_resolve_image "carapace-codex-on-demand-e2e" CARAPACE_CODEX_ON_DEMAND_E2E_IMAGE)"
DOCKER_TARGET="${CARAPACE_CODEX_ON_DEMAND_DOCKER_TARGET:-bare}"
HOST_BUILD="${CARAPACE_CODEX_ON_DEMAND_HOST_BUILD:-1}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${CARAPACE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "CARAPACE_CODEX_ON_DEMAND_HOST_BUILD=0 requires CARAPACE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
if [ -z "${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_DIR:-}" ] &&
  [ -z "${CARAPACE_CURRENT_PACKAGE_TGZ:-}" ] &&
  [ "$HOST_BUILD" != "0" ]; then
  AUTO_PREPUBLISH_PLUGIN_REGISTRY_ROOT="$(
    mktemp -d "${TMPDIR:-/tmp}/carapace-codex-on-demand-plugin-registry.XXXXXX"
  )"
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-on-demand empty)"

echo "Running Codex on-demand Docker E2E..."
if ! docker_e2e_run_with_harness \
  -v "${CARAPACE_DOCKER_E2E_REPO_ROOT:-$ROOT_DIR}/extensions/codex/package.json:/tmp/carapace-candidate-codex-package.json:ro" \
...
export OPENAI_API_KEY="sk-carapace-codex-on-demand-e2e"
...
dump_debug_logs() {
  ...
  carapace_e2e_dump_logs \
    ...
    /tmp/carapace-codex-registry/server.log \
...
configure_plugin_registry() {
  carapace_prepublish_plugin_registry_start_mounted \
    /tmp/carapace-codex-registry plugin_registry_pid '["@carapace/codex"]'
}
...
carapace_e2e_assert_dep_absent "@carapace/codex" "$HOME/.carapace" "$NPM_CONFIG_PREFIX"
carapace_e2e_assert_dep_absent "@openai/codex" "$HOME/.carapace" "$NPM_CONFIG_PREFIX"
...
codex_install_args=("@carapace/codex")
if [ -n "${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_DIR:-}" ]; then
  codex_install_args=("npm:@carapace/codex@${CARAPACE_PREPUBLISH_PLUGIN_REGISTRY_CANDIDATE_VERSION:?missing candidate version}" --pin)
fi
...
carapace plugins inspect codex --runtime --json >/tmp/carapace-codex-inspect.json
