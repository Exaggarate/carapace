#!/usr/bin/env bash
# Verifies embedded Carapace bundle MCP tool materialization and tool-policy behavior
...
carapace_resolve_frozen_core_harness_capabilities "$SOURCE_ROOT"
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-agent-bundle-mcp-tools-e2e" CARAPACE_IMAGE)"
CONTAINER_NAME="carapace-agent-bundle-mcp-tools-e2e-$$"
RUN_LOG="$(mktemp -t carapace-agent-bundle-mcp-tools-log.XXXXXX)"
...
CARAPACE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 agent-bundle-mcp-tools empty)"
CLIENT_PATH="test/e2e/qa-lab/runtime/agent-bundle-mcp-tools-docker-client.ts"
CLIENT_MOUNT_ARGS=()
if [ "$CARAPACE_FROZEN_TARGET_AGENT_BUNDLE_MCP_MODE" = "legacy" ]; then
  ...
  LEGACY_CLIENT_SOURCE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/carapace-frozen-agent-bundle-mcp-tools.XXXXXX")"
  # Preserve its root package.json so tsx retains the selected release's ESM scope,
  # then link the package-owned dependencies.
  git -C "$SOURCE_ROOT" archive "$CARAPACE_SELECTED_SHA" -- package.json scripts/e2e |
    tar -x -C "$LEGACY_CLIENT_SOURCE_ROOT"
  LEGACY_CLIENT_ROOT="/tmp/carapace-frozen-agent-bundle-mcp-tools"
...
echo "Running in-container Carapace bundle MCP tool availability smoke..."
...
if [ "$status" -ne 0 ]; then
  echo "Docker Carapace bundle MCP tool availability smoke failed"
