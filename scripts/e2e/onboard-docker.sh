...
IMAGE_NAME="$(docker_e2e_resolve_image "carapace-onboard-e2e" CARAPACE_ONBOARD_E2E_IMAGE)"
CARAPACE_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env CARAPACE_ONBOARD_MAX_MEMORY_MIB 2048)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env CARAPACE_ONBOARD_MAX_CPU_PERCENT 1200)"
DOCKER_RUN_TIMEOUT="${CARAPACE_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
...
ONBOARD_CASES="${CARAPACE_ONBOARD_E2E_CASES:-}"
if [ -z "$ONBOARD_CASES" ]; then
  ONBOARD_CASES="$CARAPACE_FROZEN_TARGET_ONBOARD_CASES"
fi
CONTAINER_NAME="carapace-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/carapace-onboard-stats.XXXXXX")"
...
if [ -n "$ONBOARD_CASES" ]; then
  ONBOARD_CASE_ENV_ARGS+=(-e "CARAPACE_ONBOARD_E2E_CASES=$ONBOARD_CASES")
fi
