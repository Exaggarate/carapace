...
APP_NAME="${IOS_APP_NAME:-Carapace}"
...
Options:
  ...
  --push-relay-base-url <url>
      ...
      Defaults to https://github.com/Exaggarate/carapace.
  --simulator-proof-secret-env <name>
      Environment variable that contains the simulator proof secret.
      Defaults to CARAPACE_SIMULATOR_PUSH_PROOF_SECRET.
  -h, --help
      Show this help.
EOF
}
...
push_relay_base_url="${CARAPACE_PUSH_SANDBOX_RELAY_BASE_URL:-https://github.com/Exaggarate/carapace}"
simulator_proof_secret_env="${CARAPACE_SIMULATOR_PUSH_PROOF_SECRET_ENV:-CARAPACE_SIMULATOR_PUSH_PROOF_SECRET}"
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  ...
  xcodebuild_overrides+=(
    "CARAPACE_PUSH_MODE=simulatorSandbox"
    "CARAPACE_PUSH_RELAY_BASE_URL=${push_relay_base_url}"
    "CARAPACE_APNS_ENTITLEMENT_ENVIRONMENT=development"
  )
fi

unset "${simulator_proof_secret_env}"
if [[ "${simulator_proof_secret_env}" != "CARAPACE_SIMULATOR_PUSH_PROOF_SECRET" ]]; then
  unset CARAPACE_SIMULATOR_PUSH_PROOF_SECRET
fi
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  # shellcheck disable=SC2086
  SIMCTL_CHILD_CARAPACE_SIMULATOR_PUSH_PROOF_SECRET="${simulator_proof_secret}" \
