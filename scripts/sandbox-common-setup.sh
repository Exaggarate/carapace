...
BASE_IMAGE="${BASE_IMAGE:-carapace-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-carapace-sandbox-common:bookworm-slim}"
...
CARAPACE_DOCKER_BUILD_USE_BUILDX="${CARAPACE_DOCKER_BUILD_USE_BUILDX:-0}"
CARAPACE_DOCKER_BUILD_CACHE_FROM="${CARAPACE_DOCKER_BUILD_CACHE_FROM:-}"
CARAPACE_DOCKER_BUILD_CACHE_TO="${CARAPACE_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=carapace.sandbox=1)
NOTE
