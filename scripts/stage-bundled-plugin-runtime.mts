...
function ensureCarapaceExtensionAlias(params: { distExtensionsRoot: string; repoRoot: string }) {
  ...
  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "carapace");
...
function shouldCopyRuntimeFile(relativePath: string) {
  return (
    ...
    isPathOrNestedPath(relativePath, "carapace.plugin.json") ||
...
export function stageBundledPluginRuntime(params: { cwd?: string; repoRoot?: string } = {}) {
  ...
  ensureCarapaceExtensionAlias({ repoRoot, distExtensionsRoot });
