...
if [[ "${mode}" == "--pack" && -z "${CARAPACE_PLUGIN_NPM_PACK_OUTPUT_DIR:-}" ]]; then
  echo "--pack requires CARAPACE_PLUGIN_NPM_PACK_OUTPUT_DIR" >&2
  exit 2
fi
...
const plan = resolveNpmPublishPlan(
  ...
  process.env.CARAPACE_PLUGIN_NPM_PUBLISH_TAG,
);
...
if [[ "${CARAPACE_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${CARAPACE_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
check_package_npm_lock() {
  ...
  (
    cd "${repo_root}"
    CARAPACE_NPM_PACKAGE_LOCK_REPO_ROOT="${repo_root}" \
...
if [[ "${CARAPACE_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
verify_release_tooling_identity() {
  if [[ "${CARAPACE_RELEASE_TOOLING_IDENTITY_REQUIRED:-}" != "true" ]]; then
    return 0
  fi
  identity_args=(
    verify
    --repository "${CARAPACE_RELEASE_TOOLING_REPOSITORY:-}"
    --workflow-ref "${CARAPACE_RELEASE_TOOLING_REF:-}"
    --workflow-full-ref "${CARAPACE_RELEASE_TOOLING_FULL_REF:-}"
    --workflow-sha "${CARAPACE_RELEASE_TOOLING_SHA:-}"
    --release-publish-run-id "${CARAPACE_RELEASE_PUBLISH_RUN_ID:-}"
    --release-publish-run-attempt "${CARAPACE_RELEASE_PUBLISH_RUN_ATTEMPT:-}"
    --release-publish-ref "${CARAPACE_RELEASE_PUBLISH_REF:-}"
  ...
  if [[ "${CARAPACE_RELEASE_TOOLING_ALLOW_PREVALIDATED_REF:-}" == "true" ]]; then
...
if [[ "${mode}" == "--pack" || "${mode}" == "--pack-dry-run" ]]; then
  ...
  if [[ "${mode}" == "--pack-dry-run" ]]; then
    ...
    mkdir -p "${CARAPACE_PLUGIN_NPM_PACK_OUTPUT_DIR}"
    pack_output_dir="$(cd "${CARAPACE_PLUGIN_NPM_PACK_OUTPUT_DIR}" && pwd)"
