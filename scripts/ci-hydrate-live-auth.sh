...
profile_path="${1:-${RUNNER_TEMP:-/tmp}/carapace-live.profile}"
...
activate_claude_oauth_access_token() {
  local credentials="${CARAPACE_CLAUDE_CREDENTIALS_JSON:-}"
...
write_secret_file "$HOME/.codex/auth.json" CARAPACE_CODEX_AUTH_JSON
write_secret_file "$HOME/.codex/config.toml" CARAPACE_CODEX_CONFIG_TOML
write_secret_file "$HOME/.claude.json" CARAPACE_CLAUDE_JSON
write_secret_file "$HOME/.claude/.credentials.json" CARAPACE_CLAUDE_CREDENTIALS_JSON
write_secret_file "$HOME/.claude/settings.json" CARAPACE_CLAUDE_SETTINGS_JSON
write_secret_file "$HOME/.claude/settings.local.json" CARAPACE_CLAUDE_SETTINGS_LOCAL_JSON
write_secret_file "$HOME/.gemini/settings.json" CARAPACE_GEMINI_SETTINGS_JSON

if [[ -n "${GITHUB_ENV:-}" ]]; then
  {
    echo "CARAPACE_PROFILE_FILE=$profile_path"
  } >>"$GITHUB_ENV"
fi
