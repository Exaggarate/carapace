...
async function readPackage(rootDir: string, packageDir: string) {
  ...
  const release = isRecord(manifest.carapace) ? manifest.carapace.release : undefined;
...
function renderMarkdown(report: NpmPackageLocksReport) {
  return `${[
    ...
    "",
    `Source: \`${report.sourceSha}\` (Carapace ${report.packageVersion}).`,
    ...
    "Verify dependency-evidence-manifest.json releaseSha equals sourceSha and the Carapace commit you pin.",
