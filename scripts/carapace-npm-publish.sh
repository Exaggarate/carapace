...
usage() {
  echo "usage: bash scripts/carapace-npm-publish.sh --publish [package.tgz]"
}
...
publish_plan="$(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${CARAPACE_NPM_PUBLISH_TAG:-}" \
    BYPASS_EXTENDED_STABLE_GUARD="${BYPASS_EXTENDED_STABLE_GUARD:-}" \
    node scripts/carapace-npm-extended-stable-release.mjs publish-plan
)"
