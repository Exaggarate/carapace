...
require_swift_toolchain() {
  ...
  if ! xcodebuild_version="$(xcrun xcodebuild -version 2>&1)"; then
    ...
    echo "ERROR: Carapace macOS app packaging requires a full Xcode developer directory." >&2
  ...
  if ! swift_version="$(swift --version 2>&1)"; then
    printf '%s\n' "$swift_version" >&2
    echo "ERROR: Carapace macOS app packaging requires Swift tools ${REQUIRED_SWIFT_TOOLS_MAJOR}.${REQUIRED_SWIFT_TOOLS_MINOR}+." >&2
  ...
  if [[ -z "$major_minor" ]]; then
    ...
    echo "       Carapace macOS app packaging requires Swift tools ${REQUIRED_SWIFT_TOOLS_MAJOR}.${REQUIRED_SWIFT_TOOLS_MINOR}+." >&2
    return 1
  fi
  ...
  if [[ -z "$xcode_major_minor" ]]; then
    printf '%s\n' "$xcodebuild_version" >&2
    echo "ERROR: Could not parse selected Xcode version; Carapace macOS app packaging requires Xcode 26.4+." >&2
    return 1
  fi
  ...
  if (( xcode_major < REQUIRED_XCODE_MAJOR )) ||
    ...
    printf '%s\n' "$xcodebuild_version" >&2
    echo "ERROR: Carapace macOS app packaging requires Xcode 26.4+; current Xcode is ${xcode_major}.${xcode_minor}." >&2
    return 1
  fi
}
