// Verifies installed packages can bootstrap the default Carapace workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params: { packageRoot: string }): void {
  const tempRoot = mkdtempSync(join(tmpdir(), "carapace-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "carapace.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".carapace", "workspace");
