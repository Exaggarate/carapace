...
const LIVE_DOCKER_DEFAULT_HARNESS_DIR =
  ...
  process.env.CARAPACE_DOCKER_E2E_REPO_ROOT
    ? ".release-harness"
    : ".";
...
const CODEX_HARNESS_API_KEY_ENV = "CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key";
const npmOnboardLaneOptions = {
  prepublishPluginPackages: ["@carapace/codex"],
  resources: ["service"],
  stateScenario: "empty",
  weight: 3,
...
export function liveDockerScriptCommand(
  ...
  const skipBuild = options.skipBuild === false ? "" : "CARAPACE_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c '${shellPrelude}harness="\${CARAPACE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; CARAPACE_LIVE_DOCKER_REPO_ROOT="\${CARAPACE_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}

function upgradeSurvivorScriptCommand(envPrefix = "", shellPrelude = "") {
  const rootPrefix = 'CARAPACE_DOCKER_E2E_REPO_ROOT="${CARAPACE_DOCKER_E2E_REPO_ROOT:-$PWD}"';
...
function releaseTypedOnboardingLane() {
  return npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-typed-onboarding",
    {
...
function createPackageUpdateMaintenanceLanes() {
  return [
    npmLane("doctor-switch", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
      stateScenario: "empty",
      weight: 3,
    }),
    npmLane(
      "update-channel-switch",
      "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
      {
    ...
    npmLane("skill-install", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install", {
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `CARAPACE_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} CARAPACE_BUNDLED_PLUGIN_SWEEP_INDEX=${index} CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
function liveOpenAiChatToolsLane() {
  return liveLane(
    "openai-chat-tools",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-chat-tools",
    {
...
function mcpCodeModeGatewayLane() {
  return serviceLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-code-mode-gateway",
    {
      prepublishPluginPackages: ["@carapace/codex"],
      resources: ["npm"],
      stateScenario: "empty",
...
function liveMcpCodeModeGatewayLane() {
  return liveLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-mcp-code-mode-gateway",
    {
...
function kitchenSinkRpcLane() {
  return serviceLane(
    "kitchen-sink-rpc",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 25 * 60 * 1000,
...
export const mainLanes: DockerE2eLane[] = [
  lane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=0 pnpm test:docker:selected-plugins",
    {
      e2eImageKind: false,
      estimateSeconds: 600,
      resources: ["docker"],
  ...
  serviceLane("compose-setup", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:compose-setup", {
    stateScenario: "empty",
    timeoutMs: 20 * 60 * 1000,
    weight: 3,
  }),
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:cli-installer-distribution",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=0 pnpm test:docker:package-install",
    {
      e2eImageKind: false,
      needsPackage: true,
      stateScenario: "empty",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "CARAPACE_LIVE_GATEWAY_PROVIDERS=claude-cli,google-gemini-cli",
    ),
    {
      providers: ["claude-cli", "google-gemini-cli"],
      timeoutMs: LIVE_PROFILE_TIMEOUT_MS,
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "CARAPACE_LIVE_CLI_BACKEND_ADVISORY=1 CARAPACE_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1 CARAPACE_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    provider: "openai",
    resources: ["service"],
  ...
  serviceLane("onboard", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane("codex-on-demand", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand", {
    prepublishPluginPackages: ["@carapace/codex"],
    resources: ["service"],
    stateScenario: "empty",
    weight: 3,
  ...
  serviceLane(
    "codex-media-path",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-media-path",
    {
      prepublishPluginPackages: ["@carapace/codex"],
      resources: ["npm"],
      stateScenario: "empty",
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    npmOnboardLaneOptions,
  ),
  npmLane(
  ...
  npmLane(
    ...
    liveDockerScriptCommand(
      ...
      "CARAPACE_NPM_ONBOARD_CHANNEL=discord CARAPACE_NPM_ONBOARD_USE_SOURCE_PLUGIN_PACKAGE=1",
    ),
    {
      ...npmOnboardLaneOptions,
      prepublishPluginPackages: ["@carapace/codex", "@carapace/discord"],
    },
  ),
  npmLane(
    ...
    liveDockerScriptCommand(
      "e2e/npm-onboard-channel-agent-docker.sh",
      "CARAPACE_NPM_ONBOARD_CHANNEL=slack CARAPACE_NPM_ONBOARD_USE_SOURCE_PLUGIN_PACKAGE=1",
    ),
    {
      ...npmOnboardLaneOptions,
      prepublishPluginPackages: ["@carapace/codex", "@carapace/slack"],
    },
  ),
  npmLane(
    "release-user-journey",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-media-memory",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-upgrade-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-plugin-marketplace",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  serviceLane("gateway-network", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  ...
  serviceLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:sandbox-browser-sidecar",
    {
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
      weight: 4,
  ...
  serviceLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:agent-bundle-mcp-tools",
    {
      stateScenario: "empty",
    },
  ),
  lane("system-agent-rescue", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:system-agent-rescue", {
    stateScenario: "empty",
  }),
  serviceLane(
    "cron-mcp-cleanup",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
  ...
  lane("plugins", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "CARAPACE_PLUGINS_E2E_CLAWHUB=0 CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  npmLane("multi-node-update", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:multi-node-update", {
    stateScenario: "empty",
    timeoutMs: 15 * 60 * 1000,
    weight: 3,
  }),
  lane("openai-image-auth", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "system-agent-first-run",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:system-agent-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane(
  ...
  liveLane("npm-telegram-live", "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-telegram-live", {
    e2eImageKind: "bare",
    provider: "openai",
...
export const tailLanes: DockerE2eLane[] = [
  serviceLane(
    ...
    "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key CARAPACE_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE=1 CARAPACE_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 CARAPACE_LIVE_CODEX_HARNESS_MCP_PROBE=0 CARAPACE_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0 CARAPACE_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-codex-harness-docker.sh",
      `${CODEX_HARNESS_API_KEY_ENV} CARAPACE_LIVE_CODEX_BIND=1 CARAPACE_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts`,
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "CARAPACE_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "CARAPACE_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "CARAPACE_LIVE_ACP_BIND_AGENT=droid CARAPACE_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "CARAPACE_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "CARAPACE_LIVE_ACP_BIND_AGENT=opencode CARAPACE_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const primaryReleasePathChunks: Record<string, DockerE2eLane[]> = {
  core: [
    scheduledLane("qr", { command: "CARAPACE_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr" }),
    ...scheduledLaneList(
      "onboard",
      "gateway-network",
      "config-reload",
...
export function releasePathChunkLanes(
  ...
  if (!base) {
    throw new Error(
      `CARAPACE_DOCKER_ALL_CHUNK must be one of: ${[
