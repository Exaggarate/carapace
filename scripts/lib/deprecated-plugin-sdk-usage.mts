...
export function buildDeprecatedPluginSdkModuleSpecifiers(
  ...
  const unscoped = [
    ...
    ...deprecatedSubpaths.map((subpath) => `carapace/plugin-sdk/${subpath}`),
  ];
  // tsconfig aliases the scoped @carapace/plugin-sdk package to the same
...
export const BANNED_INTERNAL_PLUGIN_SDK_FACADE_MODULES: BannedInternalPluginSdkFacadeModule[] = [
  // Reply facades: canonical seams are carapace/plugin-sdk/channel-inbound and
  // carapace/plugin-sdk/channel-outbound (defineChannelMessageAdapter family).
  {
    modulePath: "src/plugin-sdk/channel-message",
    canonical: "carapace/plugin-sdk/channel-outbound",
  },
  {
  ...
  },
  {
    ...
    canonical: "carapace/plugin-sdk/channel-inbound",
  },
  {
