...
replace_mac_app_bundle() {
  ...
  if [[ -e "$target" ]]; then
    mv "$target" "$previous/Carapace.app" || return 1
  fi
  if ! mv "$staged" "$target"; then
    if [[ -e "$previous/Carapace.app" ]]; then
      mv "$previous/Carapace.app" "$target" || {
        echo "ERROR: Restore the previous app from $previous/Carapace.app" >&2
        return 1
      }
    fi
    rmdir "$previous"
