...
# build the runner/functional images with the prepared Carapace package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${CARAPACE_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$CARAPACE_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${CARAPACE_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker_e2e_docker_cmd image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because CARAPACE_DOCKER_BUILD_ON_MISSING/CARAPACE_TESTBOX allows fallback."
      else
  ...
  if [ "$target" = "functional" ]; then
    ...
    if [ -z "${CARAPACE_CURRENT_PACKAGE_TGZ:-}" ]; then
    ...
    build_args+=(--build-context "carapace_package=$package_context")
    build_args+=(--build-arg "CARAPACE_FS_SAFE_NATIVE_CONTRACT=${CARAPACE_FS_SAFE_NATIVE_CONTRACT:-required}")
  fi
...
docker_e2e_test_state_entrypoint() {
  ...
  for extension in mts mjs; do
    entrypoint="$ROOT_DIR/scripts/lib/carapace-test-state.$extension"
  ...
  echo "Carapace test-state entrypoint not found under $ROOT_DIR/scripts/lib" >&2
  return 1
}
