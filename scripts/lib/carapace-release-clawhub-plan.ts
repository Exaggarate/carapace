// Carapace release ClawHub plan script supports release workflow routing.
...
type CarapaceReleaseClawHubPlanArgs = {
...
type CarapaceReleaseClawHubPlan = {
  warnings: string[];
  bootstrapWorkflowSha: string;
...
type CarapaceReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
type CarapaceReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildCarapaceReleaseClawHubRuntimeState(
  args: CarapaceReleaseClawHubRuntimeStateArgs,
): CarapaceReleaseClawHubRuntimeState {
...
export function parseCarapaceReleaseClawHubPlanArgs(
  argv: string[],
): CarapaceReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildCarapaceReleaseClawHubPlan(
  args: CarapaceReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<CarapaceReleaseClawHubPlan> {
