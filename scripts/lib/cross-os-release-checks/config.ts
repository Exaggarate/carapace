...
export const PUBLISHED_INSTALLER_BASE_URL = "https://github.com/Exaggarate/carapace";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "CARAPACE_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
export const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "CARAPACE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
const providerConfig = {
  openai: {
    ...
    requiredCompanionPackages: ["@carapace/codex"],
...
export function resolveProviderConfig(provider: string, env = process.env): ProviderConfig | null {
  ...
  const providerEnvKey = `CARAPACE_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.CARAPACE_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
export function buildReleaseProviderConfigOverride(providerMeta: ProviderConfig) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "carapace" } } : {}),
    models: [],
...
export const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.carapace-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "CARAPACE_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("CARAPACE_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.CARAPACE_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.CARAPACE_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.CARAPACE_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
export function buildRealUpdateEnv(env: NodeJS.ProcessEnv) {
  ...
  delete updateEnv.CARAPACE_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]carapace\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/carapace[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec: string) {
  ...
  if (!trimmed || trimmed === "carapace@latest") {
    return "";
  }
  if (trimmed.startsWith("carapace@")) {
    return trimmed.slice("carapace@".length);
  }
  return trimmed;
}
