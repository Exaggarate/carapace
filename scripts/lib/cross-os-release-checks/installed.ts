...
export async function resolveInstallerTargetVersion(params: {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "carapace@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view carapace@latest version did not return a version.");
  }
...
export function buildInstallerSmokeScript(
  ...
  if ((params.platform ?? process.platform) === "win32") {
    return `
$installerPath = Join-Path ([System.IO.Path]::GetTempPath()) ("carapace-installer-" + [guid]::NewGuid().ToString("N") + ".ps1")
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "curl.exe failed to download the Carapace installer (exit $LASTEXITCODE)"
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'carapace.cmd'),
      (Join-Path $npmPrefix 'carapace.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command carapace -ErrorAction Stop
...
Write-Output "__CARAPACE_PATH__=$commandPath"
...
export async function verifyFreshShellCommand(params: {
  ...
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__CARAPACE_PATH__=") ?? "",
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed carapace path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const cliPath = parseMarkerLine(result.stdout, "__CARAPACE_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed carapace path from fresh POSIX shell.");
  }
...
export async function ensureDevUpdateGitInstall(params: {
  ...
  // The dev-update lane must prove that `carapace update --channel dev` landed on
