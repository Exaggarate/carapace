...
async function configureDiscordSmoke(params: {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gatewayLogPath = join(
      params.cwd,
      `.carapace/logs/${params.lane.name}-discord-gateway.log`,
    );
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const title = html.includes("<title>Carapace Control</title>");
  const app = html.includes("<carapace-app></carapace-app>");
...
export async function maybeRunDiscordRoundtrip(params: {
  ...
  const token =
    process.env.CARAPACE_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.CARAPACE_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.CARAPACE_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
