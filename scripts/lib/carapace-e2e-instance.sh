...
carapace_e2e_eval_test_state_from_b64() {
  local encoded="${1:?missing Carapace test-state script}"
  local decoded
  if ! decoded="$(printf '%s' "$encoded" | base64 -d)"; then
    echo "Invalid Carapace test-state base64 payload" >&2
    return 1
  fi
  if [ -z "${decoded//[[:space:]]/}" ]; then
    echo "Carapace test-state base64 payload decoded to an empty script" >&2
    return 1
  fi
  eval "$decoded"
}
carapace_e2e_read_positive_int_env() {
...
carapace_e2e_read_nonnegative_int_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [ -z "${!name+x}" ]; then
...
carapace_e2e_resolve_entrypoint() {
  local entry
  for entry in dist/index.mjs dist/index.js; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "Carapace entrypoint not found under dist/" >&2
  return 1
}
carapace_e2e_run_script_entrypoint() {
  local stem="${1:?missing Carapace E2E script stem}"
  shift
  if [ -f "$stem.mts" ]; then
    tsx "$stem.mts" "$@"
    return
  ...
  echo "Carapace E2E script entrypoint not found: $stem.{mts,mjs}" >&2
  return 1
}
carapace_e2e_package_root() {
  local prefix="${1:-}"
  if [ -n "$prefix" ]; then
...
carapace_e2e_package_entrypoint() {
  local root="${1:?missing package root}"
  local entry
  for entry in "$root/dist/index.mjs" "$root/dist/index.js"; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "Carapace package entrypoint not found under $root/dist/" >&2
  return 1
}
carapace_e2e_maybe_timeout() {
  ...
  if [ -z "$timeout_bin" ]; then
    if command -v node >/dev/null 2>&1; then
      echo "timeout command not found; using Node watchdog for Carapace E2E command timeout $timeout_value" >&2
      if [[ "$1" != */* ]]; then
...
const killGraceMs = Number.parseInt(
  process.env.CARAPACE_E2E_TIMEOUT_KILL_GRACE_MS || "30000",
  10,
);
...
const timer = setTimeout(() => {
  timedOut = true;
  console.error(`Carapace E2E command timed out after ${timeoutValue}`);
  killChild("SIGTERM");
...
child.on("error", (error) => {
  ...
  process.exit(127);
    ...
    echo "timeout command not found and Node is unavailable; cannot bound Carapace E2E command after $timeout_value" >&2
    return 127
  fi
...
carapace_e2e_print_log() {
  ...
  max_bytes="$(carapace_e2e_read_nonnegative_int_env CARAPACE_E2E_LOG_TAIL_BYTES 262144)" || return $?
  max_lines="$(carapace_e2e_read_nonnegative_int_env CARAPACE_E2E_LOG_TAIL_LINES 120)" || return $?
  [ -f "$path" ] || return 0
  echo "--- $path ---"
  redactor_module="${CARAPACE_E2E_REDACTOR_MODULE:-$(carapace_e2e_package_root)/dist/plugin-sdk/logging-core.js}"
...
carapace_e2e_install_package() {
  local log_file="$1"
  local label="${2:-mounted Carapace package}"
  local prefix="${3:-}"
  local package_tgz="${CARAPACE_CURRENT_PACKAGE_TGZ:?missing CARAPACE_CURRENT_PACKAGE_TGZ}"
  local timeout_value="${CARAPACE_E2E_NPM_INSTALL_TIMEOUT:-600s}"
  local args=(-g)
  if [ -n "$prefix" ]; then
    args+=("--prefix" "$prefix")
  fi
  echo "Installing $label..."
  if carapace_e2e_maybe_timeout "$timeout_value" npm install "${args[@]}" "$package_tgz" --no-fund --no-audit >"$log_file" 2>&1; then
    return 0
  else
...
carapace_e2e_write_state_env() {
  local target="${1:-/tmp/carapace-test-state-env}"
  {
    printf 'export HOME=%q\n' "$HOME"
    printf 'export CARAPACE_HOME=%q\n' "$CARAPACE_HOME"
    printf 'export CARAPACE_STATE_DIR=%q\n' "$CARAPACE_STATE_DIR"
    printf 'export CARAPACE_CONFIG_PATH=%q\n' "$CARAPACE_CONFIG_PATH"
    printf 'export CARAPACE_AGENT_DIR=%q\n' "${CARAPACE_AGENT_DIR-}"
  } >"$target"
}
carapace_e2e_install_trash_shim() {
  local shim_dir="${CARAPACE_E2E_BIN_DIR:-}"
  if [ -z "$shim_dir" ]; then
    if [ -n "${CARAPACE_STATE_DIR:-}" ]; then
      shim_dir="$CARAPACE_STATE_DIR/e2e-bin"
    else
      shim_dir="$(mktemp -d "${TMPDIR:-/tmp}/carapace-bin.XXXXXX")"
    fi
    CARAPACE_E2E_BIN_DIR="$shim_dir"
    export CARAPACE_E2E_BIN_DIR
  fi
  case ":$PATH:" in
    *":$shim_dir:"*) ;;
...
carapace_e2e_run_script_with_pty() {
  local command="$1"
  local log_path="$2"
  local timeout_value="${CARAPACE_E2E_COMMAND_TIMEOUT:-300s}"
  if script --version >/dev/null 2>&1; then
    carapace_e2e_maybe_timeout "$timeout_value" script -q -f -c "$command" "$log_path"
  elif node -e 'import("@lydell/node-pty")' >/dev/null 2>&1; then
    carapace_e2e_maybe_timeout "$timeout_value" node scripts/e2e/lib/run-with-pty.mjs "$log_path" /bin/bash -lc "$command"
  else
    carapace_e2e_maybe_timeout "$timeout_value" script -q -F "$log_path" /bin/bash -lc "$command"
  fi
}
carapace_e2e_start_tracked_process() {
  local log_path="${1:?missing Carapace E2E process log path}"
  shift
...
if (!command) {
  console.error("missing command for Carapace E2E tracked process");
  process.exit(1);
}
...
carapace_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
  carapace_e2e_signal_process "$pid" TERM
  for _ in $(seq 1 40); do
    ! carapace_e2e_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return 0; }
    sleep 0.25
  done
  carapace_e2e_signal_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}
carapace_e2e_terminate_gateways() {
  carapace_e2e_stop_process "${1:-}"
}
carapace_e2e_start_mock_openai() { carapace_e2e_start_tracked_process "$2" env "MOCK_PORT=$1" node scripts/e2e/mock-openai-server.mjs; }
carapace_e2e_wait_mock_openai() {
  local port="$1" attempts="${2:-80}" timeout_ms="${3:-400}" _
  local base_url="${4:-http://127.0.0.1:${port}}"
  for _ in $(seq 1 "$attempts"); do
    carapace_e2e_probe_http "${base_url}/health" ok "$timeout_ms" && return 0
    sleep 0.1
  done
  carapace_e2e_probe_http "${base_url}/health" ok "$timeout_ms"
}
carapace_e2e_start_gateway() { carapace_e2e_start_tracked_process "$3" node "$1" gateway --port "$2" --bind loopback --allow-unconfigured; }
carapace_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
carapace_e2e_gateway_log_port_from_text() {
    ...
    if [ "$saw_ready_log" != "true" ] && [ -f "$log" ]; then
      ...
      if [ "$log_bytes" -gt "$ready_scan_offset" ]; then
        ...
        if [ -n "$ready_log_lines" ]; then
          saw_ready_log=true
          [ -n "$ready_port" ] || ready_port="$(printf "%s" "$ready_log_lines" | carapace_e2e_gateway_log_port_from_text)"
        fi
      fi
    fi
    if [ "$saw_ready_log" = "true" ]; then
      [ -n "$ready_port" ] || ready_port="${CARAPACE_E2E_GATEWAY_READY_PORT:-18789}"
      if [ "$readiness_mode" = "legacy-ready-log-ok" ]; then
        carapace_e2e_probe_tcp 127.0.0.1 "$ready_port" 400 && return 0
      else
        carapace_e2e_probe_http "http://127.0.0.1:${ready_port}/readyz" ok 400 && return 0
      fi
    fi
    sleep 0.25
  done
...
carapace_e2e_probe_http_status() {
  carapace_e2e_probe_http "$1" "${2:-200}" "${3:-400}"
}
carapace_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
carapace_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
carapace_e2e_assert_log_not_contains() {
  ! grep -q "$2" "$1" || { echo "Unexpected log output: $2"; exit 1; }
}
carapace_e2e_run_logged() {
  local label="$1" log_root="${CARAPACE_E2E_LOG_DIR:-${TMPDIR:-/tmp}}" log_path safe_label
  shift
  safe_label="${label//[^A-Za-z0-9_.-]/-}"
  [ -n "$safe_label" ] || safe_label="command"
  mkdir -p "$log_root"
  log_path="$(mktemp "$log_root/carapace-${safe_label}.XXXXXX.log")"
  CARAPACE_E2E_LAST_LOG_PATH="$log_path"
  export CARAPACE_E2E_LAST_LOG_PATH
  carapace_e2e_run_command "$@" >"$log_path" 2>&1 || { carapace_e2e_print_log "$log_path"; exit 1; }
}
carapace_e2e_run_command() {
  local timeout_value="${CARAPACE_E2E_COMMAND_TIMEOUT:-300s}"
  carapace_e2e_maybe_timeout "$timeout_value" "$@"
}
carapace_e2e_fixture_plugin_command() {
  ...
  if [[ -n "$consent" ]]; then
    ...
    CARAPACE_E2E_LAST_FIXTURE_PLUGIN_CAPABILITY_CONSENT_SUPPORTED=0
  fi
  "${runner[@]}" "$@"
}
carapace_e2e_enable_carapace_cli_timeout() {
  CARAPACE_E2E_CLI_BIN="$(type -P carapace)"
  if [ -z "$CARAPACE_E2E_CLI_BIN" ]; then
    echo "Carapace CLI binary not found on PATH" >&2
    return 1
  fi
  export CARAPACE_E2E_CLI_BIN
  carapace() {
    carapace_e2e_run_command "$CARAPACE_E2E_CLI_BIN" "$@"
  }
}
carapace_e2e_dump_logs() {
  local path
  for path in "$@"; do
    carapace_e2e_print_log "$path"
  done
}
