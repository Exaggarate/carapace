...
const GATEWAY_STARTUP_HEALTH_RUNTIME_ENV = {
  CARAPACE_VITEST_NO_OUTPUT_TIMEOUT_MS: "60000",
};
...
const AGENTS_EMBEDDED_AGENT_ENV = {
  CARAPACE_VITEST_NO_OUTPUT_TIMEOUT_MS: "660000",
};
...
const PINNED_COMPACT_GROUP_ENV = { CARAPACE_VITEST_MAX_WORKERS: "2" };
...
function resolveAgentCoreShardName(file: string): string {
  ...
  if (
    ...
    name.startsWith("carapace-tools") ||
...
function resolveInfraShardName(file: string): string {
  ...
  if (
    ...
    name.startsWith("tmp-carapace-dir")
  ) {
  ...
  if (
    ...
    name.startsWith("carapace-exec-env") ||
