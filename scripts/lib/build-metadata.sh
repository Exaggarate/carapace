...
carapace_trim_build_metadata_value() {
  local value="${1:-}"
...
carapace_is_full_git_commit() {
  [[ "${1:-}" =~ ^[0-9a-fA-F]{40}$ ]]
}

carapace_normalize_utc_build_timestamp() {
  local value="${1:-}"
  local LC_ALL=C
...
carapace_is_utc_build_timestamp() {
  carapace_normalize_utc_build_timestamp "${1:-}" >/dev/null
}

carapace_resolve_git_commit() {
  local root_dir="$1"
  local candidate
  local source_name
  for source_name in GIT_COMMIT GIT_SHA; do
    candidate="$(carapace_trim_build_metadata_value "${!source_name:-}")"
    [[ -n "${candidate}" ]] && break
  done
  if [[ -n "${candidate}" ]]; then
    if ! carapace_is_full_git_commit "${candidate}"; then
  ...
  if [[ -n "${candidate}" ]] && ! carapace_is_full_git_commit "${candidate}"; then
  ...
  if [[ -z "${candidate}" ]]; then
    candidate="$(carapace_trim_build_metadata_value "${GITHUB_SHA:-}")"
    if [[ -n "${candidate}" ]] && ! carapace_is_full_git_commit "${candidate}"; then
  ...
  if [[ -z "${candidate}" ]]; then
    if [[ "${CARAPACE_REQUIRE_BUILD_METADATA:-0}" == "1" ]]; then
...
carapace_resolve_build_timestamp() {
  local candidate
  candidate="$(carapace_trim_build_metadata_value "${CARAPACE_BUILD_TIMESTAMP:-}")"
  if [[ -n "${candidate}" ]]; then
    if ! candidate="$(carapace_normalize_utc_build_timestamp "${candidate}")"; then
      echo "ERROR: CARAPACE_BUILD_TIMESTAMP must be an ISO-8601 UTC timestamp." >&2
      return 1
    fi
  else
