...
const ROOT_DIR = path.resolve(
  process.env.CARAPACE_NPM_PACKAGE_LOCK_REPO_ROOT?.trim() || SCRIPT_ROOT_DIR,
);
...
const NPM_LOCK_WORKER_KIND = "carapace-npm-lock-package";
...
export function createNpmLockExecOptions(
  ...
  return {
    ...
    maxBuffer: readPositiveIntEnv(
      "CARAPACE_NPM_LOCK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout: readPositiveIntEnv(
      "CARAPACE_NPM_LOCK_COMMAND_TIMEOUT_MS",
...
export function generateNpmPackageLock(packageDir: string, options: NpmLockOptions = {}) {
  const tempDir = mkdtempSync(path.join(tmpdir(), "carapace-npm-lock-"));
  try {
...
export function listManagedNpmLockPackageDirs(rootDir = ROOT_DIR) {
  ...
  return ["extensions", "packages"]
    ...
    .filter((packageDir) => {
      ...
      return recordAt(recordAt(packageJson, "carapace"), "release")?.publishToNpm === true;
    })
    .toSorted((left, right) => left.localeCompare(right));
}
...
export function resolveNpmLockJobs(
  ...
  const raw = rawValue ?? env.CARAPACE_NPM_LOCK_JOBS ?? String(fallback);
  const rawText =
    typeof raw === "string" || typeof raw === "number" || typeof raw === "bigint"
      ? String(raw)
      : "";
  const jobs = readPositiveIntEnv("CARAPACE_NPM_LOCK_JOBS", rawText, {
    CARAPACE_NPM_LOCK_JOBS: rawText,
  });
  if (jobs > NPM_LOCK_MAX_JOBS) {
    throw new Error(`invalid CARAPACE_NPM_LOCK_JOBS: ${rawText}; maximum is ${NPM_LOCK_MAX_JOBS}`);
  }
  return jobs;
}
...
async function runPackageWorker(packageDir: string, rootDir: string) {
  return await new Promise<string>((resolve, reject) => {
    const worker = new Worker(new URL(import.meta.url), {
      ...
      env: { ...process.env, CARAPACE_NPM_PACKAGE_LOCK_REPO_ROOT: rootDir },
      workerData: {
