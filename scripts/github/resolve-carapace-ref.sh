...
REMOTE_URL="${CARAPACE_REF_REMOTE:-https://github.com/Exaggarate/carapace.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-carapace-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Carapace branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Carapace ref: ${REF}" >&2
exit 1
