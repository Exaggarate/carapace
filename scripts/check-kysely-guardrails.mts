...
const rawSqliteAllowPathGroups = {
  ...
  "SQLite database lifecycle, schema, transactions, and pragmas": [
    ...
    "src/state/carapace-agent-db-maintenance.ts",
    "src/state/carapace-agent-db-registry.ts",
    "src/state/carapace-agent-db-registry-listing.ts",
    "src/state/carapace-agent-db-schema-helpers.ts",
    "src/state/carapace-agent-db-schema.ts",
    "src/state/carapace-agent-db-session-nodes-migration.ts",
    "src/state/carapace-agent-db-session-migrations.ts",
    "src/state/carapace-agent-db-session-provenance.ts",
    "src/state/carapace-agent-db.ts",
    "src/state/carapace-state-db-audit-migration.ts",
    "src/state/carapace-state-db-delivery-queue-backfill.ts",
    "src/state/carapace-state-db-legacy-backfills.ts",
    "src/state/carapace-state-db-maintenance.ts",
    "src/state/carapace-state-db-operator-approval-migration.ts",
    "src/state/carapace-state-db-schema-additive.ts",
    "src/state/carapace-state-db-schema-helpers.ts",
    "src/state/carapace-state-db-schema-repair.ts",
    "src/state/carapace-state-db-schema-v12-foldin.ts",
    "src/state/carapace-state-db-schema-v13-widerow.ts",
    "src/state/carapace-state-db-startup-checkpoint.ts",
    "src/state/carapace-state-db-table-retirements.ts",
    "src/state/carapace-state-db-fast-path.ts",
    "src/state/carapace-state-db.ts",
    "src/state/carapace-state-ownership-operations.ts",
  ...
  "read-only shared state database access": [
    ...
    "src/state/carapace-agent-db-readonly.ts",
    "src/state/carapace-state-db-readonly.ts",
  ],
  ...
  "read-only schema preflight and integrity verification access": [
    "src/state/carapace-database-preflight.ts",
    "src/state/carapace-database-verify.worker.ts",
    "src/state/carapace-state-ownership.ts",
  ],
  "quarantine store must work when other databases are damaged": [
    "src/state/carapace-quarantine-store.ts",
  ],
...
function collectImports(sourceFile: SourceFile) {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (
      ...
      source.endsWith("carapace-state-db.js")
    ) {
      hasSqliteContext = true;
    }
  }
