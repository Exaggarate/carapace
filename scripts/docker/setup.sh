...
EXTRA_MOUNTS="${CARAPACE_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${CARAPACE_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${CARAPACE_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${CARAPACE_DOCKER_SOCKET:-}"
TIMEZONE="${CARAPACE_TZ:-}"
RAW_SKIP_ONBOARDING="${CARAPACE_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${CARAPACE_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="carapace-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="carapace-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$CARAPACE_CONFIG_DIR/carapace.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == CARAPACE_GATEWAY_TOKEN=* ]]; then
      token="${line#CARAPACE_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${CARAPACE_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$CARAPACE_GATEWAY_PORT" "$CARAPACE_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$CARAPACE_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$CARAPACE_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network carapace-cli service because it
  ...
  # Host CARAPACE_* paths are Compose bind-mount sources. Setup-time CLI writes
  # must still resolve state/config paths inside the container.
  run_prestart_gateway \
    -e HOME=/home/node \
    -e CARAPACE_HOME=/home/node \
    -e CARAPACE_STATE_DIR=/home/node/.carapace \
    -e CARAPACE_CONFIG_PATH=/home/node/.carapace/carapace.json \
    -e CARAPACE_CONFIG_DIR=/home/node/.carapace \
    -e CARAPACE_WORKSPACE_DIR=/home/node/.carapace/workspace \
    --entrypoint node carapace-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" carapace-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" carapace-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node carapace-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:carapace" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.carapace.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker carapace-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "CARAPACE_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "CARAPACE_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
CARAPACE_CONFIG_DIR="${CARAPACE_CONFIG_DIR:-$HOME/.carapace}"
CARAPACE_WORKSPACE_DIR="${CARAPACE_WORKSPACE_DIR:-$HOME/.carapace/workspace}"
CARAPACE_AUTH_PROFILE_SECRET_DIR="${CARAPACE_AUTH_PROFILE_SECRET_DIR:-$HOME/.carapace-auth-profile-secrets}"

validate_mount_path_value "CARAPACE_CONFIG_DIR" "$CARAPACE_CONFIG_DIR"
validate_mount_path_value "CARAPACE_WORKSPACE_DIR" "$CARAPACE_WORKSPACE_DIR"
validate_mount_path_value "CARAPACE_AUTH_PROFILE_SECRET_DIR" "$CARAPACE_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "CARAPACE_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "CARAPACE_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "CARAPACE_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "CARAPACE_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "CARAPACE_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$CARAPACE_CONFIG_DIR"
mkdir -p "$CARAPACE_WORKSPACE_DIR"
mkdir -p "$CARAPACE_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$CARAPACE_CONFIG_DIR/identity"
mkdir -p "$CARAPACE_CONFIG_DIR/agents/main/agent"
mkdir -p "$CARAPACE_CONFIG_DIR/agents/main/sessions"

export CARAPACE_CONFIG_DIR
export CARAPACE_WORKSPACE_DIR
export CARAPACE_AUTH_PROFILE_SECRET_DIR
export CARAPACE_GATEWAY_PORT="${CARAPACE_GATEWAY_PORT:-18789}"
export CARAPACE_BRIDGE_PORT="${CARAPACE_BRIDGE_PORT:-18790}"
export CARAPACE_GATEWAY_BIND="${CARAPACE_GATEWAY_BIND:-lan}"
export CARAPACE_DISABLE_BONJOUR="${CARAPACE_DISABLE_BONJOUR:-}"
export CARAPACE_IMAGE="$IMAGE_NAME"
export CARAPACE_IMAGE_APT_PACKAGES="${CARAPACE_IMAGE_APT_PACKAGES-${CARAPACE_DOCKER_APT_PACKAGES:-}}"
export CARAPACE_IMAGE_PIP_PACKAGES="${CARAPACE_IMAGE_PIP_PACKAGES:-}"
export CARAPACE_EXTENSIONS="${CARAPACE_EXTENSIONS:-}"
export CARAPACE_DOCKER_BUILD_NODE_OPTIONS="${CARAPACE_DOCKER_BUILD_NODE_OPTIONS---max-old-space-size=8192}"
export CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB="${CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB:-}"
export CARAPACE_DOCKER_BUILD_SKIP_DTS="${CARAPACE_DOCKER_BUILD_SKIP_DTS:-1}"
export CARAPACE_INSTALL_BROWSER="${CARAPACE_INSTALL_BROWSER:-}"
export CARAPACE_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export CARAPACE_HOME_VOLUME="$HOME_VOLUME_NAME"
export CARAPACE_ALLOW_INSECURE_PRIVATE_WS="${CARAPACE_ALLOW_INSECURE_PRIVATE_WS:-}"
export CARAPACE_SANDBOX="$SANDBOX_ENABLED"
export CARAPACE_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export CARAPACE_DOCKER_SETUP=1
export CARAPACE_TZ="$TIMEZONE"
...
export CARAPACE_OTEL_PRELOADED="${CARAPACE_OTEL_PRELOADED:-}"
export CARAPACE_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${CARAPACE_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    CARAPACE_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $CARAPACE_CONFIG_DIR/carapace.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      CARAPACE_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      CARAPACE_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      CARAPACE_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${CARAPACE_CONFIG_DIR}:/home/node/.carapace"
    gateway_workspace_mount="${CARAPACE_WORKSPACE_DIR}:/home/node/.carapace/workspace"
    gateway_auth_profile_secret_mount="${CARAPACE_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/carapace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${CARAPACE_INSTALL_DOCKER_CLI:-}" ]]; then
    export CARAPACE_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  CARAPACE_AUTH_PROFILE_SECRET_DIR \
  CARAPACE_GATEWAY_PORT \
  CARAPACE_BRIDGE_PORT \
  CARAPACE_GATEWAY_BIND \
  CARAPACE_DISABLE_BONJOUR \
  CARAPACE_GATEWAY_TOKEN \
  CARAPACE_IMAGE \
  CARAPACE_EXTRA_MOUNTS \
  CARAPACE_HOME_VOLUME \
  CARAPACE_IMAGE_APT_PACKAGES \
  CARAPACE_IMAGE_PIP_PACKAGES \
  CARAPACE_EXTENSIONS \
  CARAPACE_DOCKER_BUILD_NODE_OPTIONS \
  CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB \
  CARAPACE_DOCKER_BUILD_SKIP_DTS \
  CARAPACE_INSTALL_BROWSER \
  CARAPACE_SANDBOX \
  CARAPACE_DOCKER_SOCKET \
  DOCKER_GID \
  CARAPACE_INSTALL_DOCKER_CLI \
  ...
  CARAPACE_OTEL_PRELOADED \
  CARAPACE_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "carapace:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  BUILD_GIT_COMMIT="$(carapace_resolve_git_commit "$ROOT_DIR")"
  BUILD_TIMESTAMP="$(carapace_resolve_build_timestamp)"
  PROVENANCE_BUILD_ARGS=(--build-arg "CARAPACE_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}")
  if [[ "$BUILD_GIT_COMMIT" =~ ^[0-9a-fA-F]{40}$ ]]; then
    PROVENANCE_BUILD_ARGS+=(--build-arg "GIT_COMMIT=${BUILD_GIT_COMMIT}")
  fi
  run_docker_build \
    "${PROVENANCE_BUILD_ARGS[@]}" \
    --build-arg "CARAPACE_IMAGE_APT_PACKAGES=${CARAPACE_IMAGE_APT_PACKAGES}" \
    --build-arg "CARAPACE_IMAGE_PIP_PACKAGES=${CARAPACE_IMAGE_PIP_PACKAGES}" \
    --build-arg "CARAPACE_EXTENSIONS=${CARAPACE_EXTENSIONS}" \
    --build-arg "CARAPACE_DOCKER_BUILD_NODE_OPTIONS=${CARAPACE_DOCKER_BUILD_NODE_OPTIONS}" \
    --build-arg "CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=${CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB}" \
    --build-arg "CARAPACE_DOCKER_BUILD_SKIP_DTS=${CARAPACE_DOCKER_BUILD_SKIP_DTS}" \
    --build-arg "CARAPACE_INSTALL_BROWSER=${CARAPACE_INSTALL_BROWSER}" \
    --build-arg "CARAPACE_INSTALL_DOCKER_CLI=${CARAPACE_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Carapace metadata subdirectory
# (.carapace/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh carapace-gateway -c \
  'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export PATH; \
   /usr/bin/find -P /home/node/.carapace -xdev \( ! -path /home/node/.carapace/workspace -o -prune \) -execdir /usr/bin/chown -h node:node {} +; \
   /usr/bin/chown -h node:node /home/node/.config; \
   /usr/bin/find -P /home/node/.config/carapace -xdev -execdir /usr/bin/chown -h node:node {} +; \
   if [ -d /home/node/.carapace/workspace/.carapace ] && [ ! -L /home/node/.carapace/workspace/.carapace ]; then \
     /usr/bin/find -P /home/node/.carapace/workspace/.carapace -xdev -execdir /usr/bin/chown -h node:node {} +; \
   fi || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (CARAPACE_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from CARAPACE_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $CARAPACE_GATEWAY_BIND"
  if is_truthy_value "$CARAPACE_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (CARAPACE_DISABLE_BONJOUR=$CARAPACE_DISABLE_BONJOUR)."
  elif [[ -z "$CARAPACE_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (CARAPACE_DISABLE_BONJOUR=$CARAPACE_DISABLE_BONJOUR)."
  fi
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm carapace-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm carapace-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm carapace-cli channels add --channel discord --token <token>"
echo "Docs: https://github.com/Exaggarate/carapace"
...
# --- Sandbox setup (opt-in via CARAPACE_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: CARAPACE_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker carapace-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg CARAPACE_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (CARAPACE_IMAGE=carapace:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Carapace config.
...
echo "Config: $CARAPACE_CONFIG_DIR"
echo "Workspace: $CARAPACE_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f carapace-gateway"
echo "  ${COMPOSE_HINT} exec carapace-gateway sh -lc 'node dist/index.js gateway health --token \"\$CARAPACE_GATEWAY_TOKEN\"'"
