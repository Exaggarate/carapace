---
summary: "Synthetic Slack-class channel plugin for deterministic Carapace QA scenarios"
title: "QA channel"
read_when:
...
- `botUserId` - synthetic bot user id used in target grammar (default: `carapace`).
- `botDisplayName` - display name for outbound messages (default: `Carapace QA`).
...
```bash
pnpm carapace qa suite --scenario channel-participant-identity-inspection
```
