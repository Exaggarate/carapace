...
read_when:
  - Connecting Carapace to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects Carapace to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an Carapace agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.

## Quick setup

In ClickClack, open **Workspace settings → Integrations → Carapace**, create a
bot using **Setup code (recommended)**, and copy the generated command:

```bash
carapace channels add clickclack --code 'https://clickclack.example.com/#XXXX-XXXX-XXXX'
```
...
```bash
carapace channels add clickclack --code 'https://api.example.com/services/clickclack/api/bot-setup-codes/claim#XXXX-XXXX-XXXX'
```

The setup code is single-use and expires after 10 minutes. Carapace claims it,
...
For versioned exact endpoints, Carapace validates and saves the canonical API
base returned by ClickClack, including any path prefix. The setup code itself is
not stored in Carapace config.
...
If Carapace is already running, ClickClack connects automatically and no second
...
```bash
carapace channels add clickclack --code XXXX-XXXX-XXXX --base-url https://clickclack.example.com
```
...
Choose **Manual token** in ClickClack when configuring a non-Carapace client or
when you explicitly need to manage the token yourself:

```bash
carapace channels add clickclack --base-url https://clickclack.example.com --token ccb_... --workspace default
```
...
carapace channels add clickclack --base-url https://clickclack.example.com --workspace default --use-env
carapace gateway
```
...
Use `apiBaseUrl` when ClickClack and the Carapace gateway run on the same host
...
```json5
{
  channels: {
    clickclack: {
      baseUrl: "https://github.com/Exaggarate/carapace",
...
The public hostname can remain fully auth-gated for browser users. Carapace
...
ClickClack in channel setup or running `carapace plugins enable clickclack`
...
`carapace plugins install @carapace/clickclack` follows the normal
...
Enable discussions on one ClickClack account to give each Carapace session a
  ...
  channels: {
    clickclack: {
      ...
      discussions: {
        enabled: true,
        workspace: "default",
        controlUrlBase: "https://github.com/Exaggarate/carapace",
        section: "Sessions",
      },
    },
  },
...
or deleting an Carapace session never archives or replaces the ClickClack
...
concrete Carapace session id, so resetting a reusable session key rotates the
...
updates and return them in channel responses. Carapace verifies that contract
...
was created.
That reference contains a durable per-Carapace-installation namespace plus a
...
workspace, Carapace records the old channel as revoked and releases the binding
...
At gateway startup, each configured account publishes Carapace's native
...
the deterministic Carapace run id `clickclack:<message-id>`. Each model call is
...
Agent replies containing media use required durable delivery. Carapace assigns
...
Carapace does not reread the original local path or remote media URL.
...
Carapace leaves the delivery unresolved rather than risking a duplicate; update
...
Accepted bot messages also pass through Carapace's shared bot-pair loop guard.
...
Carapace inbound turns, even when their author bot is explicitly allowed.
...
remote media URLs follow Carapace's normal media-access policy. Set
...
```bash
carapace message send --channel clickclack --target channel:general --message "hello"
carapace message send --channel clickclack --target dm:usr_123 --message "hello"
carapace message send --channel clickclack --target thread:msg_123 --message "following up"
```
...
Carapace only needs current `bot:write` for normal agent chat and command-menu sync. Add `agent_activity:write` when enabling [native progress and agent activity rows](#native-progress-and-agent-activity-rows).
