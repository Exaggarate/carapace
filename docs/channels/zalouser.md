...
read_when:
  - Setting up Zalo Personal for Carapace
...
- Pin a version: `carapace plugins install @carapace/zalouser@<version>`
- From a source checkout: `carapace plugins install ./path/to/local/zalouser-plugin`
...
Carapace stores each raw `zca-js` message callback before processing it. Pending messages resume from the account queue after a Gateway restart, and processing stays serialized per direct chat or group.

The `zca-js` socket listener does not expose a delivery acknowledgement or automatically replay old messages after reconnect. The durable queue therefore protects the local crash window after a callback reaches Carapace; it cannot recover a message the socket never delivered. Replay tombstones are mostly a safeguard against a repeated callback with the same Zalo message id.
...
- `carapace pairing list zalouser`
- `carapace pairing approve zalouser <code>`
...
<Note>
`channels.zalouser.groups.<id>.allow` is a legacy field name; current config uses `enabled`. `carapace doctor --fix` migrates `allow` to `enabled` automatically.
</Note>
...
- When a group message is skipped because a mention is required, Carapace stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Carapace state. Example:
...
Profile names select the saved Zalo login credentials in Carapace state. Resolution order:
...
- Carapace sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Carapace sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `carapace channels logout --channel zalouser && carapace channels login --channel zalouser`
