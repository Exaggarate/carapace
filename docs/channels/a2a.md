---
summary: "Connect external agents to Carapace through the A2A 1.0 JSON-RPC protocol"
read_when:
  - You want an A2A-compliant agent to discover and message Carapace
  - You need to configure authenticated A2A peers or outbound agent messages
title: "A2A"
---

The A2A channel plugin connects Carapace to other agents through the Linux Foundation [Agent2Agent protocol](https://a2a-protocol.org). External agents discover the gateway through a public Agent Card and submit authenticated text tasks using the A2A 1.0 JSON-RPC binding. Carapace can also send messages to configured peer agents.

## Quick setup

Add the bundled plugin to your Carapace configuration and define a separate bearer token for each trusted peer:
...
```json5
{
  channels: {
    a2a: {
      enabled: true,
      advertisedUrl: "https://carapace.example.com",
      peers: {
        hermes: {
...
The card advertises the gateway JSON-RPC endpoint, supported text input and output, and one skill for each exposed Carapace agent. Set `channels.a2a.exposeAgents` to an array of agent IDs to limit which agents appear. If unset or empty, all configured agents are advertised.
...
Add a peer URL when Carapace should send messages to another A2A agent. Set `outboundToken` when the remote agent requires its own bearer token:
...
| `peers.<name>.token`         | string   | required | Bearer token required when this peer sends requests to Carapace.               |
| `peers.<name>.url`           | string   | -        | Peer JSON-RPC endpoint for outbound messages.                                  |
| `peers.<name>.outboundToken` | string   | -        | Bearer token Carapace sends to the configured peer URL.                        |
...
Outbound destinations come only from operator-configured peer URLs. Inbound callers cannot supply a proxy target or redirect Carapace to another destination.
