...
read_when:
  - You want to connect Carapace to a Raft workspace
...
Raft connects an Carapace agent to a Raft External Agent through the local
...
```bash
carapace plugins install @carapace/raft
...
- The Raft CLI installed on the same host as the Carapace Gateway, on the
  service's `PATH`.
...
```bash
carapace channels add --channel raft
```
...
7. Starts one serialized Carapace agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The Carapace turn
...
```bash
raft --profile carapace message check
raft --profile carapace message send
```

<Note>
Raft is not a push-message transport. Carapace does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that Carapace can find the CLI and has a configured profile:

```bash
carapace channels status --probe
carapace plugins inspect raft --runtime --json
```
