---
summary: "Messaging platforms Carapace can connect to"
read_when:
  - You want to choose a chat channel for Carapace
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

Carapace can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
(`carapace plugins install @carapace/<id>`) or on demand during
`carapace onboard` / `carapace channels add`, then need a Gateway restart.
"External plugin" channels are maintained outside the Carapace repo.
...
Run `carapace channels add` with no flags to pick a channel from a list
...
[`carapace channels`](/cli/channels).
...
- [A2A](/channels/a2a) - Connect external agents to Carapace through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/channels/buzz) - Connect Carapace agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new Carapace iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and Carapace config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between Carapace agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and Carapace config (official plugin).
...
- [WeChat](/channels/wechat) - WeChat channel setup through the external carapace-weixin plugin (external plugin).
...
- [Zalo ClawBot](/channels/zaloclawbot) - Zalo ClawBot channel setup through the external carapace-zaloclawbot plugin (external plugin).
...
- Channels can run simultaneously; configure multiple and Carapace will route per chat.
