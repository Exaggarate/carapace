...
read_when:
  - You want Carapace to receive DMs via Nostr
  - You're setting up decentralized messaging
title: "Nostr"
---

Nostr is a downloadable channel plugin (`@carapace/nostr`) that lets Carapace receive and answer NIP-04 encrypted direct messages over Nostr relays. One account per gateway; DMs only.

## Install

```bash
carapace plugins install @carapace/nostr
```
...
```bash
carapace plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`carapace onboard`) and `carapace channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
carapace channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
carapace channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "carapace@example.com",
        lud16: "carapace@example.com",
      },
    },
  },
}
...
1. Note the bot pubkey from gateway logs or `carapace channels status` (hex; convert to npub in your client if needed).
