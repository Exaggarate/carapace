...
Entries are matched with the destination channel's normal `allowFrom` rules. Carapace does not translate sender ids between channels: if Alice has a Telegram id and a Discord id, list both ids under the matching channel keys.
...
`discord.channelAudience` means "allow Discord DM senders who can currently view this guild channel." Carapace resolves the sender through Discord at authorization time and applies Discord `ViewChannel` permission rules. `membership` is optional and defaults to `canViewChannel`.
...
```typescript
import { resolveAccessGroupAllowFromState } from "carapace/plugin-sdk/access-groups";
...
Run `carapace doctor` after editing access-control config. It catches many invalid allowlist and policy combinations before runtime.
