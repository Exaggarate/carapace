...
Nextcloud Talk is a downloadable channel plugin (`@carapace/nextcloud-talk`) that connects Carapace to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.

## Install

```bash
carapace plugins install @carapace/nextcloud-talk
```
...
```bash
carapace plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Carapace" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Carapace:
   ...
   ```bash
   carapace channels add --channel nextcloud-talk \
...
- Message webhooks return HTTP 200 only after the raw event is durably stored; storage failures return HTTP 500. The durable `200` carries `x-carapace-delivery-accepted: durable`, so reverse proxies can require the marker to distinguish Carapace acceptance from a generic `200`. Unsupported non-message events return HTTP 200 without the marker and are logged as ignored.
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `carapace channels status` probes the bot and warns when the `response` feature is missing.

## Access control (DMs)

- Default: `channels.nextcloud-talk.dmPolicy = "pairing"`. Unknown senders get a pairing code.
- Approve via:
  - `carapace pairing list nextcloud-talk`
  - `carapace pairing approve nextcloud-talk <CODE>`
