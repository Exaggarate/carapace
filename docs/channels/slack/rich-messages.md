...
The Block Kit surfaces Carapace renders natively in Slack.
...
renders line, bar, area, and pie charts in messages. Carapace maps the portable
...
block. Standard presentation sends to other Carapace channels receive that same
deterministic chart data as text unless they advertise native chart support. If
Slack rejects the chart with `invalid_blocks` during a phased rollout, Carapace
...
a presentation contains more than two valid charts, Carapace keeps their order
...
renders structured rows and columns in messages. Carapace maps an explicit
...
Carapace maps header and string cells to Slack `raw_text` cells. Numeric cells
...
If Slack rejects native chart or table blocks with `invalid_blocks`, Carapace
...
Markdown pipe tables remain authored text; Carapace does not guess at table
structure or cell types. Existing trusted Slack-native producers can continue
to pass raw blocks through `channelData.slack.blocks`; Carapace derives fallback
...
block in the message's top-level blocks or attachments. Carapace renders those
...
`view_submission` and `view_closed` lifecycle events before Carapace compacts
the payload for the agent-visible system event. Use one of these routing
patterns when opening a Slack modal:

- Set `callback_id` to `carapace:<namespace>:<payload>`.
...
When those buttons are present, they are the primary approval UX; Carapace
