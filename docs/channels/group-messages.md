...
Goal: let Carapace sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
  ...
  agents: {
    entries: {
      main: {
        default: true,
        groupChat: {
          mentionPatterns: ["@?carapace", "\\+?15555550123"],
        },
      },
    },
  },
...
1. Add your WhatsApp account (the one running Carapace) to the group.
2. Say `@carapace ...` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@carapace` ping in the group and confirm a reply that references the sender name.
