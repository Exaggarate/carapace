...
Zalo ships as a bundled plugin in current Carapace releases, so packaged builds do not need a separate install.

On an older build or a custom install that excludes Zalo, install the npm package directly:

- Install: `carapace plugins install @carapace/zalo`
- Pinned version: `carapace plugins install @carapace/zalo@2026.6.11`
- From a local checkout: `carapace plugins install ./path/to/local/zalo-plugin`
...
- Pairing: unknown senders get a pairing code; messages are ignored until approved. Codes expire after 1 hour.
  - `carapace pairing list zalo`
  - `carapace pairing approve zalo <CODE>`
...
- Reported real-world caveat: on some Marketplace-bot setups the bot could not be added to a group at all. If you hit that, verify with your bot's Zalo Bot Platform settings; it is a platform-side constraint, not an Carapace policy.
...
- Webhook mode: set `channels.zalo.webhookUrl` and `channels.zalo.webhookSecret`.
  ...
  - HTTP 200 is returned only after the raw event is durably stored; storage failures return HTTP 500. The durable `200` carries `x-carapace-delivery-accepted: durable`, so reverse proxies can require it to distinguish Carapace acceptance from a generic `200` (authentication, validation, and storage-error responses omit it).
...
```bash
carapace message send --channel zalo --target 123456789 --message "hi"
```
...
- Check the token: `carapace channels status --probe`
- Verify the sender is approved (pairing or `allowFrom`)
- Check gateway logs: `carapace logs --follow`
