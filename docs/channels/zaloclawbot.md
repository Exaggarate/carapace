---
summary: "Zalo ClawBot channel setup through the external carapace-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the carapace-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

Carapace connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/carapace-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `carapace-zaloclawbot`.

## Compatibility

| Plugin Version | Carapace Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
- Node.js >= 22
- [Carapace](../install/index.md) installed (`carapace` CLI available)
...
```bash
carapace plugins install "@zalo-platforms/carapace-zaloclawbot@0.1.4"
```

Use the exact pinned version so Carapace verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
carapace config set plugins.entries.carapace-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
carapace channels login --channel carapace-zaloclawbot
```
...
The plugin manages bot credentials under the Carapace state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of Carapace state.

This plugin's runtime lives entirely in the external `@zalo-platforms/carapace-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against Carapace core source.
...
- **Gateway fails to load:** confirm your Carapace host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
