...
This hub links the core docs for how Carapace connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`carapace gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Hosted widget documents and A2UI renderer assets** are served on the same port as the Gateway (`/__carapace__/canvas/`, `/__carapace__/a2ui/`), protected by Gateway auth when bound beyond loopback.
