...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - Legacy `rotateBytes` is rejected by the current schema; `carapace doctor --fix` removes it from older configs.
