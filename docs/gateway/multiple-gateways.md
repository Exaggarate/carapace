---
summary: "Run multiple Carapace Gateways on one host (isolation, ports, and profiles)"
read_when:
...
carapace --profile rescue onboard
carapace --profile rescue gateway install --port 19789
```
...
During `carapace --profile rescue onboard`:
...
- Profile/config file
- State directory
- Workspace (default: `~/.carapace/workspace-rescue`)
...
# extra gateway
carapace --profile ops setup
carapace --profile ops gateway --port 19789
```

Named profiles on both sides also work:

```bash
carapace --profile main setup
carapace --profile main gateway --port 18789
...
carapace --profile ops gateway install --port 19789
```
...
| Setting                      | Purpose                              |
| ---------------------------- | ------------------------------------ |
| `CARAPACE_CONFIG_PATH`       | Per-instance config file             |
| `CARAPACE_STATE_DIR`         | Per-instance sessions, creds, caches |
...
`CARAPACE_ALLOW_MULTI_GATEWAY=1` skips the per-config singleton.

<Warning>
`CARAPACE_STATE_DIR` alone does not isolate a managed Gateway service. Service names follow the profile, not the state directory. For onboarding or service-install tests, use a dedicated named profile and unique ports, or an isolated machine. Do not install or restart the default service against a temporary state directory.
</Warning>

## Port mapping (derived)

Base port = `gateway.port` (or `CARAPACE_GATEWAY_PORT` / `--port`).
...
```bash
CARAPACE_CONFIG_PATH=~/.carapace/main.json \
CARAPACE_STATE_DIR=~/.carapace \
carapace gateway --port 18789

CARAPACE_CONFIG_PATH=~/.carapace/rescue.json \
CARAPACE_STATE_DIR=~/.carapace-rescue \
carapace gateway --port 19789
```
...
carapace --profile rescue gateway status --deep
carapace --profile rescue gateway probe
carapace status
carapace --profile rescue status
carapace --profile rescue browser status
```

- `gateway status --deep` catches stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when Carapace cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
