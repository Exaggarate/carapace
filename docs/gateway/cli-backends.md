...
Carapace can run a local AI CLI as a text-only fallback when API providers are down, rate-limited, or misbehaving. It is intentionally conservative:

- Carapace tools are not injected directly, but a backend with `bundleMcp: true` can receive gateway tools through a loopback MCP bridge.
...
```bash
carapace agent --agent main --message "hi" --model claude-cli/claude-sonnet-5
```
...
mechanics in `carapace.json`.

Carapace auto-loads an owning bundled plugin when model selection or a
...
Credentials remain in Carapace auth profiles or the owning plugin's config.
...
2. Builds a system prompt using the same Carapace prompt and workspace context.
...
carapace config unset agents.defaults.timeoutSeconds

# Or choose an explicit 12-hour limit:
carapace config set agents.defaults.timeoutSeconds 43200
```

Background work started inside a CLI is still part of that CLI subprocess. If the parent turn reaches its overall limit, Carapace stops the subprocess and its CLI-internal background tasks together. For durable long work, use a detached Carapace [sub-agent](/tools/subagents) or [ACP agent](/tools/acp-agents); detached sub-agents have no run timeout by default.

The `carapace agent` command also has its own request deadline. Its 600-second fallback default applies to that command invocation, not to ordinary Gateway turns; see [`carapace agent`](/cli/agent).
...
subscription. Carapace uses a non-secret route marker. It never reads, persists,
...
receives it as a native hook attachment, while Carapace history preserves the original user message. The
...
Carapace replays eligible notes from the active reset/compaction window, with a
...
bindings do not track which Carapace notes the native session has consumed.
Transient runtime context and notes excluded from model context are not replayed.

Keep Claude Code updated, especially if Carapace reports an incompatible
installed executable:

```bash
claude --version
claude update
# Restart the Carapace gateway after updating.
```

The bundled `claude-cli` backend prefers Claude Code's native skill resolver. When the current skills snapshot has at least one selected skill with a materialized path, Carapace passes a temporary Claude Code plugin via `--plugin-dir` and omits the duplicate Carapace skills catalog from the appended system prompt. Without a materialized plugin skill, Carapace keeps the prompt catalog as a fallback. Skill env/API key overrides still apply to the child process environment for the run.

Carapace disables Claude Code's built-in Git workflow instructions and startup
...
Carapace always launches Claude Code with its default permission mode.
Carapace's permission responses and `PreToolUse` hook keep native tools under
...
arguments projected into their Carapace equivalents. Per-agent and session
restrictions still override broader global policy. Carapace-owned MCP tools
...
Claude's native `AskUserQuestion` uses Carapace's structured question flow. When
Carapace rejects malformed questions, it reports the failed field and
...
When the effective exec ask setting is `on-miss` or `always`, Carapace relays
...
```bash
carapace approvals allowlist add --agent main /usr/local/bin/gog
```

Carapace reuses its exec shell evaluator and allows a call without prompting
...
not sandboxed execution by Carapace. Claude Code owns cwd, PATH, environment,
...
Native session compatibility and permission to replay saved Carapace history are separate. Clearing or replacing a native binding does not establish ownership of older transcript rows. Carapace records a private account fingerprint and contiguous transcript coverage before an admitted CLI turn, then advances coverage with that turn’s canonical writes. It never stores credential values in this metadata.
...
Serialization: `serialize: true` keeps same-lane runs ordered (most CLIs serialize on one provider lane). Carapace also drops stored CLI session reuse when the selected auth identity changes, including a changed auth profile id, static API key, static token, or OAuth account identity when the CLI exposes one; OAuth access/refresh token rotation alone does not cut the session. If a CLI has no stable OAuth account id, Carapace lets that CLI enforce its own resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in [`agents.defaults.model.fallbacks`](/concepts/model-failover), Carapace seeds the next attempt with a context prelude harvested from Claude Code's local JSONL transcript (under `~/.claude/projects/`, keyed per workspace). This supplies CLI-owned context that may not be present in Carapace's SQLite session transcript.
...
Carapace writes base64 images to temp files. If `imageArg` is set, those paths are passed as CLI args; if not, Carapace appends the file paths to the prompt (path injection), which works for CLIs that auto-load local files from plain paths.
...
- For Gemini CLI JSON output, Carapace reads reply text from `response` and usage from `stats` when `usage` is missing or empty. The bundled Gemini CLI adapter uses `stream-json`.
...
Anthropic owns `claude-cli` and Google owns `google-gemini-cli`. OpenAI Codex agent runs use the Codex app-server harness through `openai/*`; Carapace no longer registers a bundled `codex-cli` backend.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `command`             | `claude`                                                                                                                                                                                                      |
| `args`                | `-p --output-format stream-json --include-partial-messages --verbose --setting-sources user --allowedTools mcp__carapace__* --disallowedTools ScheduleWakeup,CronCreate,Bash(run_in_background:true),Monitor` |
| `output`              | `jsonl`                                                                                                                                                                                                       |
| `input`               | `stdin`                                                                                                                                                                                                       |
| `modelArg`            | `--model`                                                                                                                                                                                                     |
...
valid legacy Gemini CLI OAuth profiles remain runtime-compatible, but Carapace
...
- Usage falls back to `stats` when `usage` is absent or empty; `stats.cached` normalizes into Carapace `cacheRead`, and if `stats.input` is missing, input tokens derive from `stats.input_tokens - stats.cached`.
...
Add the atomic `manualCompaction` capability only when its command compacts the resumed session in place. Its `input` selects the transport the backend command actually recognizes, and `validateOutput` must require a positive backend acknowledgement rather than treating a zero exit as success. Carapace runs it as an internal control operation: it is not written as a user turn and does not run agent or context-engine turn hooks.

## Bundle MCP overlays

CLI backends do not receive Carapace tool calls directly, but a backend can opt into a generated MCP config overlay with `bundleMcp: true`. Current bundled behavior:

- `claude-cli`: generated strict MCP config file.
- `google-gemini-cli`: generated Gemini system settings file.

When bundle MCP is enabled, Carapace:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process, authenticated with a per-run context grant (`CARAPACE_MCP_TOKEN`) active only for the current execution attempt;
...
Carapace lists each server through its session-scoped runtime, assigns the same
...
operator policy in Carapace configuration.
...
Carapace tool through the grant-scoped MCP server. This keeps filesystem,
process, exec, approval, and sandbox policy inside Carapace instead of widening
...
If no MCP servers are enabled, Carapace still injects a strict config when a backend opts into bundle MCP, so background runs stay isolated.
...
exceeds its deadline, Carapace refuses replacement, including from a later run.
...
login. Carapace uses a non-secret route marker and never reads, persists,
...
Explicit Carapace-managed API-key and token profiles continue to use the
protected, per-invocation credential-forwarding CLI path.

## Reseed history cap

When a fresh CLI session is seeded from a prior Carapace transcript (for example after a `session_expired` retry), the rendered `<conversation_history>` block is capped to keep reseed prompts from exploding. The default is 12,288 characters (about 3,000 tokens).
...
- Carapace does not inject tool calls into the CLI backend protocol. Backends only see gateway tools when they opt into `bundleMcp: true`.
...
redacted stderr diagnostic when available. Check the run error or `carapace logs`
