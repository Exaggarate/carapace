...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or CARAPACE_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `CARAPACE_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `gateway.auth.identityScopes`: maps a verified trusted-proxy user or Tailscale WhoIs login to connection-only operator scopes. Email keys match case-insensitively; other identities match exactly. For trusted-proxy Control UI connections, `x-carapace-scopes` caps device enrollment or upgrade requests and the final device-plus-identity session scopes. Grants do not create or modify pairing records. Token, password, and no-auth connections have no verified identity and receive no grant.
...
- `tailscale.mode`: `serve` (tailnet only, loopback bind) or `funnel` (public, requires auth).
  Carapace holds the route as a foreground claim, so startup fails unless the
...
- `tailscale.preserveFunnel`: deprecated migration guard. When `true` and
  `tailscale.mode = "serve"`, Carapace checks `tailscale funnel status` before
  ...
  provenance. Carapace leaves the external route unchanged and warns. The
  ...
  `CARAPACE_GATEWAY_PASSWORD`, and set `gateway.auth.mode` to `password`. Then
  run `carapace config set gateway.tailscale.mode funnel`, followed by
  `carapace config unset gateway.tailscale.preserveFunnel`. Default `false`.
...
- `terminal.shell`: optional shell executable. When unset, Carapace uses `$SHELL` on Unix and `%ComSpec%` on Windows. Changes hot-apply to newly opened terminals; existing terminals keep running their original shell.
...
- `gateway.push.apns.relay.baseUrl`: base HTTPS URL for the external APNs relay used after relay-backed iOS builds publish registrations to the gateway. Public App Store builds use the hosted Carapace relay. Custom relay URLs must match a deliberately separate iOS build/deployment path whose relay URL points at that relay.
- `gateway.push.apns.relay.timeoutMs`: gateway-to-relay send timeout in milliseconds. Defaults to `10000`.
- Relay-backed registrations are delegated to a specific gateway identity. The paired iOS app fetches `gateway.identity.get`, includes that identity in the relay registration, and forwards a registration-scoped send grant to the gateway. Another gateway cannot reuse that stored registration.
- `CARAPACE_APNS_RELAY_BASE_URL` / `CARAPACE_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `CARAPACE_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `CARAPACE_HANDSHAKE_TIMEOUT_MS`: optional environment override for the built-in pre-auth Gateway WebSocket handshake timeout.
...
- `gateway.nodes.pairing.sshVerify`: SSH-verified auto-approval for first-time node device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only on an exact `carapace node identity` device-key match. Same eligibility floor as `autoApproveCidrs`; probes are limited to private/CGNAT source addresses unless `cidrs` overrides them. Set `false` to disable, or `{ user, identity, timeoutMs, cidrs }` to tune. See [Node pairing](/gateway/pairing#ssh-verified-device-auto-approval-default).
...
```bash
CARAPACE_CONFIG_PATH=~/.carapace/a.json \
CARAPACE_STATE_DIR=~/.carapace-a \
carapace gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.carapace-dev` + port `19001`), `--profile <name>` (uses `~/.carapace-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/carapace/tls/server.crt",
      keyPath: "/etc/carapace/tls/server.key",
      caPath: "/etc/carapace/tls/ca-bundle.crt",
    },
  },
}
```
...
The earlier `"restart"` and `"hot"` values are retired; [`carapace doctor --fix`](/cli/doctor) maps both to `"hybrid"`.

Reload debounce and in-flight operation deferral are no longer configurable and run behind built-in defaults. [`carapace doctor --fix`](/cli/doctor) removes the retired `debounceMs` and `deferralTimeoutMs` keys from older config files.
