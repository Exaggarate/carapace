...
Optionally commit `.carapace/portals.json` to the workspace repository so the agent can discover the available development servers:
...
- SSH-backed `remote-exec` placements, including Codex sessions, do not run the Carapace worker tool loop, so the `portal` tool does not apply there. Move the session back to the Gateway with `sessions.move` when a Gateway-hosted portal is needed.
