---
summary: "Pre-flight and rollback checklist before exposing an Carapace Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.carapace`).
...
- Backup location for `~/.carapace/carapace.json` and credentials.
...
```bash
carapace gateway probe --url ws://127.0.0.1:18789 --token "$CARAPACE_GATEWAY_TOKEN"
```
...
Run `carapace security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `carapace security audit --deep`.
...
5. Re-run `carapace security audit --deep`.
