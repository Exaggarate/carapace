...
Tune it under `gateway.auth.rateLimit` in `carapace.json`:
...
proxy is not configured in `gateway.trustedProxies`, Carapace cannot safely
...
proxy overwrite or safely rebuild forwarding headers. Carapace then restores
...
Carapace-managed Tailscale Serve and Funnel use a separate private loopback
...
non-loopback forwarded client address. Carapace then uses that client address
...
`gateway.tailscale.mode: "serve"` or `"funnel"` when Carapace should own the
