...
Carapace supports OAuth and API keys for model providers. For an always-on gateway host, an API key is the most predictable option; subscription/OAuth flows work too when they match your provider account model.
...
2. Put it on the **gateway host** (the machine running `carapace gateway`):
...
3. If the gateway runs under systemd/launchd, put the key in `~/.carapace/.env` so the daemon can read it:

```bash
cat >> ~/.carapace/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
`carapace onboard` can also store API keys for daemon use if you don't want to manage env vars yourself. See [Environment variables](/help/environment) for the full env-loading precedence (`env.shellEnv`, `~/.carapace/.env`, systemd/launchd).
...
This is two steps: log Claude Code into Anthropic on the host, then tell Carapace to route Anthropic models through the local `claude-cli` backend.

Carapace never reads, stores, refreshes, or forwards the native login tokens. The installed `claude` process reads and refreshes its own login. `CLAUDE_CONFIG_DIR` selects a separate Claude login when set on the Gateway process. Carapace-managed setup tokens and API keys remain separate credentials.
...
```bash
carapace models auth paste-token --provider openrouter
```

Carapace reads auth profiles from each agent's `carapace-agent.sqlite`. Endpoint details (`baseUrl`, `api`, model ids, headers, timeouts) belong under `models.providers.<id>` in `carapace.json` or `models.json`, not in auth profiles.

If an older install still has `auth-profiles.json`, `auth-state.json`, or a flat shape like `{ "openrouter": { "apiKey": "..." } }`, run `carapace doctor --fix` to import it into SQLite; doctor keeps timestamped backups beside the original JSON files.

External auth routes such as Bedrock `auth: "aws-sdk"` aren't credentials. For a named Bedrock route, set `auth.profiles.<id>.mode: "aws-sdk"` in `carapace.json` — don't write `type: "aws-sdk"` into the auth profile store. `carapace doctor --fix` migrates legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but Carapace can't resolve a probeable model for that provider, probe reports `status: no_model`.
...
1. `CARAPACE_LIVE_<PROVIDER>_KEY` (single override, pins one key)
...
Carapace rotates to the next key only when the error message matches: `rate_limit`, `rate limit`, `429`, `quota exceeded`/`quota_exceeded`, `resource exhausted`/`resource_exhausted`, or `too many requests`. Other errors are not retried with alternate keys. If all keys fail, the final error from the last attempt is returned.
...
```bash
carapace models auth login --provider anthropic --force
```
...
Use `--agent <id>` to target a specific agent; omit it to use the configured default agent. `carapace models status --probe` shows omitted stored profiles as `excluded_by_auth_order` rather than silently skipping them.
...
Run `carapace models status` to see which profile is expiring. If an Anthropic token profile is missing or expired, refresh it via setup-token or migrate to an Anthropic API key.
