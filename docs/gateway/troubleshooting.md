...
- `carapace gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `carapace doctor` reports no blocking config/service issues.
- `carapace channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
- `Update restart` in `carapace status` / `carapace status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run carapace doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `carapace doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.
...
`carapace gateway status --deep` and the startup logs.
...
Use when a gateway service unexpectedly stops after an update, or logs show one `carapace` binary is older than the version that last wrote `carapace.json`.

Carapace stamps config writes with `meta.lastTouchedVersion`. Read-only commands can inspect a config written by a newer Carapace, but process and service mutations refuse to run from an older binary. Blocked actions: gateway service start/stop/restart/uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which carapace
carapace --version
carapace gateway status --deep
carapace config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `carapace` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `carapace` binary.
  </Step>
</Steps>
...
- `Established clients:` in `carapace gateway status --deep` or `Gateway clients` in `carapace doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer Carapace install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale Carapace client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed Carapace: local dashboards, editors, app-server helpers, or long-running `carapace logs --follow` shells.
3. Re-run `carapace gateway status --deep` or `carapace doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.carapace/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
carapace config get skills.load
```
...
carapace config get agents.defaults.models
```
...
Do not assume this is always an Carapace configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path Carapace uses, and avoid disabling protection for the whole site.
...
- Carapace model runs fail only on normal agent turns.
...
carapace infer model run --model <provider/model> --prompt "hi" --json
carapace logs --follow
```

Look for:

- Direct tiny calls succeed, but Carapace runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Carapace retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but Carapace agent runs fail with backend/model crashes (for example Gemma on some `llama-server` builds behind `llmman`): Carapace transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Carapace's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Carapace agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
If `curl` returns Carapace HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `carapace gateway start` and recheck `carapace gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | On the Gateway host, run `carapace gateway auth-token --show` in an interactive terminal, paste the output into the client, and retry.                                                                                                                                                   |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `carapace devices list` then `carapace devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `carapace devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `carapace devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
carapace logs --follow
carapace doctor
carapace gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `carapace onboard --mode local` / `carapace setup` to restamp the expected local-mode config. If you are running Carapace via Podman, the default config path is `~/.carapace/carapace.json`.
    ...
    - `System-level Carapace gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `CARAPACE_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `carapace doctor --fix` or `carapace gateway install --force`, then restart the gateway service.
...
Use when channels (Telegram, WhatsApp, etc.) on a macOS host go quiet for minutes to hours at a time, and the gateway appears to come back the moment you open the Control UI, SSH in, or otherwise interact with the host. There is usually no obvious symptom in `carapace status` because by the time you look the gateway is alive again.

```bash
ls ~/.carapace/logs/stability/ | tail -5
carapace gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.carapace.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.carapace/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/carapace/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.carapace.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
...
Use this when a macOS install keeps restarting every few seconds, `carapace`
...
This was observed on older installs where both `ai.carapace.gateway` and
`ai.carapace.node` LaunchAgents were active and each injected
`CARAPACE_LAUNCHD_LABEL`. In that state Carapace can detect launchd
...
```bash
for i in 1 2 3 4; do
  ps aux | grep 'carapace.*index.js' | grep -v grep | awk '{print $2}'
  sleep 10
done

carapace gateway status --deep
carapace node status
launchctl print gui/$UID/ai.carapace.gateway | grep -E 'state|last exit|runs'
tail -n 80 ~/Library/Logs/carapace/gateway.log
```
...
- Both `~/Library/LaunchAgents/ai.carapace.gateway.plist` and
  `~/Library/LaunchAgents/ai.carapace.node.plist` loaded at the same time on a
...
1. If this host should only run the Gateway service, remove the managed node
   service through Carapace. **Skip this step** if you actively rely on the node
   service for remote node features; uninstalling it stops those features on
   this host:

   ```bash
   carapace node uninstall
   ```

2. Install a persistent Gateway wrapper that clears the inherited launchd
   markers before starting Carapace. Use the supported `--wrapper` option; do
   not edit the generated file under `~/.carapace/service-env/`, because service
   reinstall, update, and doctor repair regenerate that file:

   ```bash
   mkdir -p ~/.local/bin
   cat >~/.local/bin/carapace-launchd-workaround <<'EOF'
   #!/bin/sh
   set -eu
   unset CARAPACE_LAUNCHD_LABEL LAUNCH_JOB_LABEL LAUNCH_JOB_NAME XPC_SERVICE_NAME || true
   exec carapace "$@"
   EOF
   chmod 700 ~/.local/bin/carapace-launchd-workaround

   carapace gateway install \
     --wrapper ~/.local/bin/carapace-launchd-workaround \
     --force
   ```
...
3. Verify that the Gateway is stable and serving RPC, not merely listening:
   ...
   ```bash
   carapace gateway status --deep --require-rpc
...
4. After upgrading to a release where the underlying dual-LaunchAgent loop is
   ...
   ```bash
   CARAPACE_WRAPPER= carapace gateway install --force
   rm ~/.local/bin/carapace-launchd-workaround
   ```
...
- `critical memory pressure bundle written` appears shortly before restart → Carapace captured a pre-OOM stability bundle. Inspect it with `carapace gateway stability --bundle latest`.
- `memory pressure: level=critical` appears in gateway logs → Carapace detected critical memory pressure and recorded the available in-process memory facts.
...
- `V8 heap:` used bytes are close to the heap limit → lower prompt/session pressure or reduce concurrent work first. For a managed service, compare the configured controls and install-time recommendation in `Gateway heap:` from `carapace gateway status` with the runtime measurement. Reinstalling preserves existing stored heap settings; it does not automatically replace an older value with the current recommendation.
...
- Critical memory pressure appears in logs but no bundle exists → capture `carapace gateway diagnostics export` after the event for the available operational evidence.
...
```bash
NODE_OPTIONS="--max-old-space-size=16384" carapace gateway run
```

For a custom supervisor or Docker runtime command, place `--max-old-space-size=16384` immediately after `node`, before the Carapace entry script, or set `NODE_OPTIONS` in that process or container's launch environment. Docker image build-time heap options do not configure the runtime Gateway. An Carapace config or dotenv value loaded after Node starts cannot resize its heap. `NODE_OPTIONS` can also reach spawned Node children, so prefer a direct Node argument when only the Gateway should receive the budget.
...
- A timestamped `carapace.json.rejected.*` file beside the active config.
- A timestamped `carapace.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- Carapace keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Carapace-owned write.
    - Gateway startup leaves `carapace.json` unchanged and fails closed when safe legacy-key migration cannot produce a fully valid config.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Carapace-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `carapace doctor --fix` owns repairs beyond automatic legacy-key migration. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Carapace rotates older `.clobbered.*` files so the newest repaired payload is still available.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Carapace-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Carapace-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    An interactive startup can offer to run `carapace doctor --fix` and retry once when automatic legacy-key migration is not enough. Non-interactive startup prints the repair command instead.

    1. Run `carapace doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `carapace config set` or `config.patch`.
    3. Run `carapace config validate` before restarting.
...
Use when `carapace gateway probe` reaches something, but still prints a warning block.

```bash
carapace gateway probe
carapace gateway probe --json
carapace gateway probe --ssh user@gateway-host
```
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or Carapace could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
carapace system heartbeat last
carapace logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → heartbeat monitor scratch only contains blank, comment, header, fence, or empty-checklist scaffolding, so Carapace skips the model call.
...
carapace browser start --browser-profile carapace
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `carapace browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Carapace-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    carapace gateway status
    carapace config get gateway.mode
    carapace config get gateway.remote.url
    carapace config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    carapace config get gateway.bind
    carapace config get gateway.auth.mode
    carapace config get gateway.auth.token
    carapace gateway status
    carapace logs --follow
    ```
