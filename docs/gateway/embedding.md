---
summary: "Supervise the Carapace Gateway as a child process from Electron or another host app"
read_when:
  - Embedding Carapace in a desktop or server application
...
title: "Embedding Carapace"
---

An embedding host should supervise the installed `carapace` executable, use the
...
and upgrades explicit without depending on Carapace's private state layout.
...
const packageEntry = fileURLToPath(import.meta.resolve("carapace"));
const carapaceEntry = resolve(dirname(packageEntry), "..", "carapace.mjs");
const gateway = spawn(hostNodeExecutable, [carapaceEntry, "gateway", "--allow-unconfigured"], {
  env: {
    ...process.env,
    CARAPACE_DISABLE_BONJOUR: "1",
    CARAPACE_EXEC_SHELL_SNAPSHOT: "0",
...
Resolve Carapace through the installed package as shown; do not assume that a
project-local `carapace` binary is on the host process's `PATH`. The example
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CARAPACE_DISABLE_BONJOUR=1`     | Disables Gateway-owned LAN multicast advertising when the host owns discovery.                                                                                                             |
| `CARAPACE_NO_RESPAWN=1`          | In an unmanaged embedding child, prevents Carapace from handing an update restart to a detached child. Routine restarts remain in process, so the host keeps ownership of the tracked PID. |
| `CARAPACE_EXEC_SHELL_SNAPSHOT=0` | Disables login-shell snapshot capture for host exec commands.                                                                                                                              |
| `CARAPACE_SKIP_CHANNELS=1`       | Skips channel startup and reload. Set it only when the embedding app wants a control-plane or WebChat-only Gateway.                                                                        |
...
`CARAPACE_EXEC_SHELL_SNAPSHOT=0` in the Gateway child's environment, not only in
...
1. Run `carapace doctor --fix --yes --non-interactive` against the same config and
...
`@carapace/gateway-protocol/startup-unavailable` or the reference client's built-in
...
Keep the Gateway as the only owner of Carapace state. Common embedding operations
...
under `~/.carapace` to implement app features. Those layouts are private runtime
implementation details and can move or change without protocol compatibility.

## Install; do not flatten

The root `carapace` package is not a single-file vendoring target. Bundled runtime
files under `dist/extensions` retain bare self-imports such as
`carapace/plugin-sdk/*`, while the npm package intentionally excludes
per-extension `node_modules` trees.

Install Carapace through npm, pnpm, or another normal Node package installation so
Node can resolve the package exports and root dependency tree. Spawn the installed
`carapace` executable. Do not copy only `dist`, flatten the package into an app
