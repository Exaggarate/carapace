...
Carapace supports additive SecretRefs so supported credentials do not need to live as plaintext in configuration.
...
<Warning>
Plaintext credentials remain agent-readable when they sit in files the agent can inspect, including `carapace.json`, `.env`, retired auth-profile JSON archives, or generated `agents/*/agent/models.json` files. SecretRefs reduce that local blast radius once every supported credential is migrated and `carapace secrets audit --check` reports no plaintext residue.
</Warning>
...
For model-provider credentials backed by SecretRefs, Carapace mints an opaque, process-local sentinel during model-auth resolution. Auth storage, stream options, SDK configuration, logs, error objects, and most runtime introspection therefore see a value such as `oc-sent-v2.<authenticated-ciphertext>.end`, not the provider credential. The guarded model fetch and managed local-provider health probes replace known sentinels in URL and header values immediately before each request leaves the process.

Unknown sentinel-shaped values fail closed before network activity. Carapace refuses to send the request rather than forwarding an unresolved sentinel to a provider. Resolved secret values are also registered for exact-value log redaction as a defense in depth measure.

Provider adapters use the latest injection point their SDK supports:

- SDKs with a custom fetch option receive Carapace's guarded fetch, so the SDK retains the sentinel.
- SDKs without a custom fetch option unwrap the sentinel immediately before client construction. Plugin-owned provider streams and agent harnesses unwrap at the final core-owned handoff because those transports do not share Carapace's guarded fetch.
...
Set `CARAPACE_SECRET_SENTINELS=off` (also accepts `0` or `false`, case-insensitive) to disable model-provider sentinel minting during incident response or compatibility troubleshooting. This switch disables neither exact-value redaction registration nor protected-store sealing for Gateway-hosted subprocesses.
...
- Legacy plaintext residue is scrubbed from `carapace.json`, the SQLite auth-profile store, `.env`, and generated `models.json` files. Retired auth JSON is doctor-owned migration input and is never rewritten by `secrets apply`.
- `carapace secrets audit --check` is clean after migration.
...
- Active `gateway.auth.token` / `gateway.auth.password` SecretRefs stay authoritative over `CARAPACE_GATEWAY_TOKEN` / `CARAPACE_GATEWAY_PASSWORD`; environment credentials are fallbacks when the corresponding local config input is absent.
...
- Generated gateway token: setup mints `gateway.auth.token` itself, so reference mode has nothing to prompt for. With `CARAPACE_GATEWAY_TOKEN` exported it writes an `env` ref to that variable, keeping a later rotation authoritative; otherwise it writes the token to the secret store under `CARAPACE_GATEWAY_TOKEN` and stores a `store` ref. An existing store entry is reused rather than rotated, so re-running setup never invalidates already-paired clients.
...
- Windows fail-closed: if ACL verification is unavailable for the path, resolution fails. Move the secret to a path whose ACLs Carapace can verify; there is no provider-level bypass.
...
- Windows fail-closed: if ACL verification is unavailable for the command path, resolution fails. Use a command path whose ACLs Carapace can verify; there is no provider-level bypass.
- Plugin-managed exec providers can use `pluginIntegration` instead of a copied `command`/`args`. Carapace resolves the current command details from the installed plugin manifest during startup/reload; if the plugin is disabled, removed, untrusted, or no longer declares the integration, active SecretRefs on that provider fail closed.
...
`code` is an optional machine-readable diagnostic. Carapace displays the recognized
...
- Reads values from Carapace's shared state SQLite database.
...
- **Agent-readable environment** (`kind: "env"`) values remain visible to administrators in the Control UI and can be returned by `store list` and `store get`. Carapace adds them as plaintext to Gateway-hosted commands run through its exec tool, after inherited process values and before explicit per-call env. The agent can print, transmit, or persist these values. Protected host keys are ignored with a visible warning.

Agent-readable environment values do not reach Codex native shell, the Codex sandbox exec-server, ACP children such as Claude Code, Carapace sandbox exec, or remote `node` exec. Those paths assemble a different child environment. In eligible Codex app-server turns, use `gateway_exec` to deliberately re-enter the Carapace Gateway execution path; `gateway_process` provides the existing per-session background follow-up. Native Codex shell remains preferred for ordinary local work. Gateway-hosted exec captures the store snapshot on its first execution in a run. Later additions, replacements, deletions, and host edits require a new run; storing a credential does not refresh an already-captured exec snapshot.
...
Reference an entry from `carapace.json` with the `store` source:
...
Credential prompts are bound to the exact requesting authority and cancel when it closes. A committed answer is terminal even if the subsequent runtime refresh fails. The saved value remains; resolve the provider error and retry `carapace secrets reload`, not the answer. Use the tool's returned full SecretRef, including its provider alias.

<Warning>
Store values are not encrypted at rest. They are stored unencrypted in the shared state SQLite database (`state/carapace.sqlite`), protected by the same `0600` file and `0700` directory permissions as other credentials in that database. Operators who need stronger storage isolation should use an external exec provider such as the [1Password plugin](/plugins/onepassword) or [Vault SecretRefs](/plugins/vault).
</Warning>

## Secret egress proxy

The secret egress proxy lets Gateway-hosted agent subprocesses use shared-store `secret` entries without receiving their plaintext. Carapace puts the existing authenticated sentinel in the subprocess environment, then a Gateway-owned loopback proxy replaces it in request URLs, headers, and streamed bodies immediately before egress.
...
```bash
carapace config set secrets.egressProxy.enabled true --strict-json
...
In the agent environment, `$OPENAI_API_KEY` is an `oc-sent-v2...end` sentinel. The proxy replaces it with the stored value only for `api.openai.com`. A request to an unbound host is refused with `Secret "OPENAI_API_KEY" is not allowed for host "<host>". Run: carapace secrets store set OPENAI_API_KEY --allow-host <host>`.
...
When enabled, Carapace adds these values to Gateway-hosted exec environments:
...
The CA is generated once per Gateway start under the state directory with a ten-year certificate validity window. Its key is still process-owned, not retained for ten years. One-day leaf certificates renew on demand within their final hour without replacing the CA or interrupting established TLS connections. This keeps already-running subprocesses trusting the same issuer across renewal. Its directory is mode `0700`, its private keys are mode `0600`, it is removed during Gateway shutdown, and Carapace never installs it in a system trust store. Requests fail closed when a sentinel cannot be authenticated or resolved; the proxy never forwards or silently strips an unresolved sentinel. Request bodies are scanned as a stream with a bounded carry window, so substitution also works when a sentinel crosses chunk boundaries or appears in a large upload.

`carapace status` and `carapace doctor` report certificate preparation failures and warn when the process CA is within seven days of expiry. Failed preparation refuses the new CONNECT request with an actionable error; the next request can retry after OpenSSL, filesystem access, or clock problems are corrected. An expired or not-yet-valid CA requires checking the system clock and restarting the Gateway, not disabling TLS verification. Gateway RPC can remain reachable while protected egress is degraded. For a read-only, machine-readable probe, run `carapace doctor --lint --only core/doctor/gateway-health --json`; the default JSON checks do not probe the running Gateway.
...
<AccordionGroup>
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    Use a resolver wrapper to map SecretRef ids to Bitwarden Secrets Manager item keys. The repository includes `scripts/secrets/carapace-bws-resolver.mjs`; install or copy it to an absolute trusted path on the host that runs the Gateway.
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/carapace-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "carapace/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    The resolver batches requested ids, runs `bws secret list`, and returns values for matching secret `key` fields. Use keys that satisfy the exec SecretRef id contract, such as `carapace/providers/openai/apiKey`; env-var-style keys with underscores are rejected before the resolver runs. If more than one visible Bitwarden secret shares the requested key, the resolver fails that id as ambiguous instead of guessing. After updating config, verify the resolver path:
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/carapace"],
  ...
  <Accordion title="password-store (`pass`)">
    Use a small resolver wrapper to map SecretRef ids directly to `pass` entries. Save this as an executable at an absolute path that passes your exec-provider path checks, for example `/usr/local/bin/carapace-pass-resolver`. The `#!/usr/bin/env node` shebang resolves `node` from the resolver process `PATH`, so include `PATH` in `passEnv`. If `pass` is not on that `PATH`, set `PASS_BIN` in the parent environment and include it in `passEnv` too:
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/carapace-pass-resolver",
...
- Carapace resolves these refs during sandbox activation, not lazily on each SSH call.
...
- The redaction sentinel `__CARAPACE_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when SQLite auth-profile credentials take precedence over `carapace.json` refs)
...
When reload-time activation fails after a healthy state, Carapace enters degraded secrets state, emitting one-shot system events and log codes:
...
<Tabs>
  <Tab title="Strict command paths">
    For example `carapace memory` remote-memory paths and `carapace qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `carapace status`, `carapace status --all`, `carapace channels status`, `carapace channels resolve`, `carapace security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable.
...
- Snapshot refresh after backend secret rotation is handled by `carapace secrets reload`.
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    carapace secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
If you save a plan instead of applying during `configure`, apply that saved plan with `carapace secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - Plaintext values at rest (`carapace.json`, SQLite auth-profile rows, `.env`, and generated `agents/*/agent/models.json`).
    - Plaintext sensitive provider header residues in generated `models.json` entries.
    - Unresolved refs.
    - Precedence shadowing (SQLite auth profiles taking priority over `carapace.json` refs).
    - Store residue (a stored name still has an equivalent plaintext value in config).

    Exec note: by default, audit skips exec SecretRef resolvability checks to avoid command side effects. Use `carapace secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - Lets you select supported secret-bearing fields in `carapace.json` plus the SQLite auth-profile store for one agent scope.
    ...
    - `carapace secrets configure --agent <id>`

    `configure` apply defaults:

    - Scrub matching static credentials from SQLite auth-profile rows for targeted providers.
    - Leave retired `auth.json` untouched; run `carapace doctor --fix` to migrate and archive it.
...
<Warning>
Carapace intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
