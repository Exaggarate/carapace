...
Carapace can use Bonjour (mDNS/DNS-SD) to discover an active gateway (WebSocket endpoint). Multicast `local.` browsing is a **LAN-only convenience**: the bundled `bonjour` plugin owns LAN advertising, auto-starting on macOS hosts and opt-in on Linux, Windows, and containerized gateway deployments. The same beacon can also publish through a configured wide-area DNS-SD domain for cross-network discovery. Discovery is best-effort and does **not** replace SSH or Tailnet-based connectivity.
...
2. Publish DNS-SD records for `_carapace-gw._tcp` under a dedicated zone (example: `carapace.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that DNS server for clients, including iOS.

`carapace.internal.` above is just an example — Carapace supports any discovery domain. iOS/Android nodes browse both `local.` and your configured wide-area domain.
...
Setting `discovery.wideArea.domain` enables Gateway wide-area publishing. The `CARAPACE_WIDE_AREA_DOMAIN` environment variable supplies a default for CLI discovery and DNS setup; it does not enable Gateway publishing by itself.
...
- serve your chosen domain (example: `carapace.internal.`) from `~/.carapace/dns/<domain>.db`
...
```bash
dns-sd -B _carapace-gw._tcp carapace.internal.
dig @<TAILNET_IPV4> -p 53 _carapace-gw._tcp.carapace.internal PTR +short
```
...
Once clients accept Tailnet DNS, iOS nodes and CLI discovery can browse `_carapace-gw._tcp` in your discovery domain without multicast.

### Gateway listener security

The gateway WS port (default `18789`) binds to loopback by default. For LAN/Tailnet access, bind explicitly and keep auth enabled. For Tailnet-only setups, set `gateway.bind: "tailnet"` in `~/.carapace/carapace.json` and restart the gateway (or the macOS menubar app).

## What advertises

Only the gateway advertises `_carapace-gw._tcp`. LAN multicast advertising comes from the bundled `bonjour` plugin when enabled; wide-area DNS-SD publishing stays gateway-owned.

## Service types

- `_carapace-gw._tcp` - gateway transport beacon, used by macOS/iOS/Android nodes.
...
```bash
# Browse instances
dns-sd -B _carapace-gw._tcp local.

# Resolve one instance (replace <instance>)
dns-sd -L "<instance>" _carapace-gw._tcp local.
```
...
Carapace starts each Bonjour service once and leaves probing, retry, name-conflict resolution, and interface-change republishing to the mDNS responder. This avoids overlapping publish attempts during normal network churn. Repeated internal self-probe messages are suppressed so they cannot flood the gateway log.

When multiple Carapace gateways advertise from the same host, Bonjour may append suffixes such as `(2)` or `(3)` to keep service instance names unique. Those suffixes are normal conflict resolution and do not indicate duplicate OCM supervision.

Bonjour uses the system hostname for the advertised `.local` host when it's a valid DNS label. If the system hostname contains spaces, underscores, or another invalid DNS-label character, Carapace falls back to `carapace.local`. Set `CARAPACE_MDNS_HOSTNAME=<name>` before starting the gateway when you need an explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_carapace-gw._tcp`.
...
```bash
carapace plugins enable bonjour
```
...
Use plugin configuration when you intentionally want to turn off the bundled LAN discovery plugin for that Carapace config:

```bash
carapace plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected containers when `CARAPACE_DISABLE_BONJOUR` is unset. Docker bridge networks usually don't forward mDNS multicast (`224.0.0.251:5353`) between the container and the LAN, so advertising from the container rarely makes discovery work.
...
- Disabling Bonjour doesn't change `gateway.bind`; Docker still defaults to `CARAPACE_GATEWAY_BIND=lan` so the published host port works.
...
- Reusing the same `CARAPACE_CONFIG_DIR` outside Docker doesn't persist the container auto-disable policy.
- Set `CARAPACE_DISABLE_BONJOUR=0` only for host networking, macvlan, or another network where mDNS multicast is known to pass; set it to `1` to force-disable.
...
1. Confirm whether the gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep CARAPACE_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising with `CARAPACE_DISABLE_BONJOUR=0`, test multicast from the host:

   ```bash
   dns-sd -B _carapace-gw._tcp local.
   ```

   If browsing is empty, or Gateway logs show repeated ciao probe failures, restore `CARAPACE_DISABLE_BONJOUR=1` and use a direct or Tailnet route.

## Common failure modes

- **Bonjour doesn't cross networks**: use Tailnet or SSH.
- **Multicast blocked**: some Wi-Fi networks disable mDNS.
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast, container bridges, WSL, or interface churn can leave the responder in a non-announced state. The gateway remains available through direct, SSH, Tailnet, or wide-area DNS-SD routes; disable LAN Bonjour with `discovery.mdns.mode: "off"` or `CARAPACE_DISABLE_BONJOUR=1` when multicast is unavailable.
- **Docker bridge networking**: Bonjour auto-disables in detected containers. Set `CARAPACE_DISABLE_BONJOUR=0` only for host, macvlan, or another mDNS-capable network.
...
| Setting                                              | Effect                                                                            |
| ---------------------------------------------------- | --------------------------------------------------------------------------------- |
| `carapace plugins enable bonjour`                    | Enables the bundled LAN discovery plugin on hosts where it isn't default-enabled. |
| `carapace plugins disable bonjour`                   | Disables LAN multicast advertising by disabling the bundled plugin.               |
| `CARAPACE_DISABLE_BONJOUR=1` (or `true`/`yes`/`on`)  | Disables LAN multicast advertising without changing plugin config.                |
| `CARAPACE_DISABLE_BONJOUR=0` (or `false`/`no`/`off`) | Forces LAN multicast advertising on, including inside detected containers.        |
| `discovery.mdns.mode`                                | `off` \| `minimal` (default) \| `full` — see modes above.                         |
| `gateway.bind`                                       | Controls the gateway bind mode in `~/.carapace/carapace.json`.                    |
| `CARAPACE_SSH_PORT`                                  | Overrides the SSH port when `sshPort` is advertised (full mode).                  |
| `CARAPACE_TAILNET_DNS`                               | Publishes a MagicDNS hint in TXT when mDNS full mode is enabled.                  |
| `CARAPACE_CLI_PATH`                                  | Overrides the advertised CLI path (full mode).                                    |

macOS hosts auto-start the bundled LAN discovery plugin by default. When the Bonjour plugin is enabled and `CARAPACE_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers (Docker, Fly.io machines, and common container runtimes).
