---
summary: "Use OpenShell as a managed sandbox backend for Carapace agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend: Carapace delegates sandbox lifecycle to
...
Carapace Gateway, which continues to run the agent and host-side plugins.
...
- `openshell` CLI installed and on the Carapace Gateway process's `PATH` (or a
...
- A supported compute runtime on the OpenShell gateway host when using local
  sandboxes
- Carapace Gateway running on the host
...
Before configuring Carapace, verify the OpenShell CLI as the same operating
system user that runs the Carapace Gateway:
...
The Carapace Gateway service must see the same OpenShell CLI, gateway
...
```bash
carapace plugins install @carapace/openshell-sandbox
```
...
Validate the configuration and restart the Carapace Gateway:
...
On the next agent turn Carapace creates an OpenShell sandbox and routes tool
execution through it. Verify both the plugin and the effective sandbox:

```bash
carapace plugins inspect openshell --runtime --json
carapace sandbox list
carapace sandbox explain
openshell sandbox list
```

`carapace sandbox list` is empty until a sandboxed agent turn first needs a
runtime.
...
- Before `exec`, Carapace syncs the local workspace into the sandbox.
- After `exec`, Carapace syncs the remote workspace back to local.
- Within one Carapace Gateway process, commands and file-tool operations sharing
...
Best for development workflows: local edits outside Carapace show up on the
...
- On first use after sandbox creation, Carapace seeds the remote workspace
...
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. Carapace does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside Carapace after the initial seed is invisible to the remote sandbox. Run `carapace sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"carapace"`  | Sandbox source for first-time create                                                   |
...
| `policy`                  | `string`                 | unset         | Path to a sandbox policy YAML file on the Carapace Gateway host                        |
...
rejected. Choose distinct, non-overlapping directories because Carapace manages
...
as `/etc/carapace/openshell-policy.yaml`; relative paths are resolved from the
...
it cannot select different OpenShell workspaces per Carapace agent or session.
Changing it does not migrate existing sandboxes. Delete Carapace's OpenShell
    ...
    entries: {
      openshell: {
        ...
        config: {
          ...
          workspace: "research",
          policy: "/etc/carapace/openshell-policy.yaml",
        },
      },
    },
  },
...
carapace sandbox recreate --all

# Recreate only one agent or the exact session scope shown by sandbox list
carapace sandbox recreate --agent researcher
carapace sandbox recreate --session "agent:researcher:main"
```
...
Carapace keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. Carapace preserves the registered runtime identity, refuses to create
a replacement implicitly, and reports the scoped `carapace sandbox recreate`
...
If OpenShell cannot delete a sandbox, Carapace reports the failure and keeps the
...
and rerun the scoped `carapace sandbox recreate` command. Do not switch the
...
the Carapace Gateway host instead of being copied into an untrusted sandbox.
...
Carapace does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the Carapace filesystem bridge must provide:
...
First separate Carapace Gateway health, plugin activation, and OpenShell
...
- **Plugin missing or backend unavailable:** Install
  `@carapace/openshell-sandbox`, set `plugins.entries.openshell.enabled: true`,
  validate the config, and restart the Carapace Gateway. Run
  `carapace plugins inspect openshell --runtime --json` to check the running
...
- **Workspace missing or the wrong sandbox list:** Verify the selected
  ...
  enabling them in Carapace. Remember that the Gateway service may not inherit
...
- **Mirror synchronization reports recovery paths:** Carapace kept a host shadow
  ...
  Carapace preserves remaining workspace entries instead of overwriting them
...
- **Recreate or prune cannot delete a sandbox:** Restore access to the original
  ...
  the scoped recreation. Carapace retains the registry entry until deletion
  succeeds.

## How it works

1. Carapace runs `sandbox get` for the sandbox name (with the selected
...
2. Carapace runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `carapace sandbox` commands
