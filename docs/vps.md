---
summary: "Run Carapace on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Carapace
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Carapace Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Carapace on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Carapace bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/carapace-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `CARAPACE_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
- Service env for a stable startup path: `CARAPACE_NO_RESPAWN=1` and
...
The standard `carapace onboard --install-daemon` path installs a systemd user
...
`sudo systemctl edit carapace-gateway.service`.
