...
in memory and records it in the hidden `carapace.cache-ttl` transcript marker so
...
and the default endpoint or `api.anthropic.com`, Carapace delegates pruning to
Anthropic's [server-side tool-result clearing](https://platform.claude.com/docs/en/build-with-claude/context-editing).
Carapace opens no new client-side pruning rounds, and the server clears old
...
Carapace derives the request parameters without adding config options:
...
justify the new cache write. When clearing occurs, Carapace logs this info line:
...
Carapace also builds a separate idempotent replay view for sessions that persist raw image blocks or prompt-hydration media markers in history.
...
If you set `agents.defaults.contextPruning.mode` or `agents.defaults.heartbeat.every` yourself, Carapace does not override them. This auto-default only fires for Anthropic-family auth; other providers get pruning `off` unless you configure it.
