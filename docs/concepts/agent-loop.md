...
System prompt is built from Carapace's base prompt, skills prompt, bootstrap context, and per-run overrides. Model-specific limits and compaction reserve tokens are enforced. See [System prompt](/concepts/system-prompt) for what the model sees.

## Hooks

Carapace has two in-process hook systems:
...
| `tool_result_persist`                                   | Synchronously transforms tool results before they are written to an Carapace-owned session transcript.                                                                                                                                                                                                                                                                                                              |
...
Harnesses can adapt these hooks. The Codex app-server harness keeps Carapace plugin hooks as the compatibility contract for documented mirrored surfaces; Codex native hooks are a separate, lower-level Codex mechanism.
...
The built-in Carapace harness publishes its execution deadline to the queue.
...
The model idle and provider HTTP rows describe the built-in Carapace model
path. Codex owns its native stream deadlines and network retries. After exact
native terminal receipt, Carapace allows two minutes for local settlement.
...
not protect expired Carapace-owned requests or tools, cancellation, terminal
