...
This is separate from `~/.carapace/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.carapace/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.carapace/workspace`
- If `CARAPACE_PROFILE` is set and not `"default"`, the default becomes `~/.carapace-<profile>/workspace`.
- `CARAPACE_WORKSPACE_DIR` overrides both of the above when set.
- A non-default `CARAPACE_STATE_DIR` keeps the default workspace at `<state-dir>/workspace`, including scheduled maintenance and the initial `main` agent entry.
...
Override in `~/.carapace/carapace.json`:
...
`carapace onboard`, `carapace configure`, or `carapace setup` create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/carapace`. Each agent uses one resolved workspace; keeping extra directories does not merge their persona or memory files into the active workspace.

<Note>
Keep each agent's workspace path explicit when retaining older directories. Before switching back to an older workspace, stop the Gateway, configure the intended path, run [`carapace doctor --fix`](/cli/doctor) to migrate retired setup state, and restart. Doctor also discovers legacy setup files in a still-configured `agents.defaults.workspace` root even when no agent currently uses that root directly. Archive unused folders only after verifying which files you want to retain.
</Note>
...
Standard files Carapace expects inside the workspace:
...
<Note>
If a required bootstrap file is missing, Carapace injects a "missing file" marker into the session and continues. Optional `USER.md` and `MEMORY.md` files are omitted when absent. Large bootstrap files are truncated when injected; adjust general limits with `agents.defaults.bootstrapMaxChars` (default: `20000`) and `agents.defaults.bootstrapTotalMaxChars` (default: `60000`). `USER.md` keeps its separate 4,000-character cap. `carapace setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.carapace/` and should NOT be committed to the workspace repo:

- `~/.carapace/carapace.json` (config)
- `~/.carapace/state/carapace.sqlite` (shared workspace setup state and attestations)
- `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite` (model auth profiles, routing state, standing intents, and other agent-scoped durability)
- `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite` (session rows, transcripts, and per-agent runtime state)
- `~/.carapace/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.carapace/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.carapace/agents/<agentId>/sessions/` (legacy migration sources and archive/support artifacts)
- `~/.carapace/skills/` (managed skills)

If you need to migrate sessions or config, copy them separately and keep them out of version control.

Older Carapace releases wrote `carapace-workspace-state.json`,
`.carapace/workspace-state.json`, and `.attested` workspace sidecars. Current
runtime uses only the shared SQLite database for that state. If Doctor reports
one of these files, run `carapace doctor --fix`; Doctor imports valid legacy
  ...
  <Step title="Add a private remote">
    <Tabs>
      ...
      <Tab title="GitHub CLI (gh)">
        ```bash
        gh auth login
        gh repo create carapace-workspace --private --source . --remote origin --push
        ```
      </Tab>
...
- Anything under `~/.carapace/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.carapace/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.carapace/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.entries.<agentId>.workspace` to the cloned path in `~/.carapace/carapace.json` for the agent that should use it. A sole agent without a per-agent override can use `agents.defaults.workspace` instead; in a multi-agent roster, that setting only changes the base directory for unpinned entries.
  </Step>
  <Step title="Verify the workspace">
    Run `carapace agents list` and confirm that the intended agent points to the cloned path before starting the Gateway. Moving an existing workspace does not require rerunning onboarding.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite`
    from the old machine separately. Copy `~/.carapace/agents/<agentId>/sessions/`
