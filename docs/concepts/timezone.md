---
summary: "Where timezones show up in Carapace — envelopes, tool payloads, system prompt"
read_when:
...
Carapace standardizes timestamps so the model sees a **single reference time** instead of a mix of provider-local clocks. Three surfaces show timezones, each with its own purpose:
...
If `userTimezone` is unset, Carapace resolves the host timezone at runtime via
