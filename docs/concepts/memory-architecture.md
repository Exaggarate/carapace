---
summary: "End-to-end architecture of Carapace memory: tiers, provenance, dreaming, recall lanes, the user model, and standing intents"
title: "Memory architecture"
sidebarTitle: "Memory architecture"
read_when:
  - You want the complete picture of how Carapace memory works end to end
...
Carapace memory is a set of plain files and one SQLite index, organized into
...
2. **Writing is the hard part.** Retrieval over notes files is competitive
   ...
   arXiv:2410.10813). Carapace therefore moves curation off the busy reply
   path and into a dedicated background pass.
3. **The write path is the security boundary.** Content-level scanning of
   memory cannot catch poisoned facts reliably, so Carapace enforces
...
so `carapace memory forget` can remove tracked entries derived from selected
...
```markdown
- Use the release helper for package validation. <!-- project: github.com/Exaggarate/carapace -->
```
...
event benchmarks). Carapace therefore compiles intentions out of the model:
...
measure poorly. Carapace defends structurally:
...
more conservatively they write; Carapace keeps those properties even with
