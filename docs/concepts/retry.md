...
The [model failover controller](/concepts/model-failover#model-fallback) owns this recovery budget. Once it is exhausted, Carapace follows eligible auth-profile or model fallback paths, or surfaces the final failure. Native harnesses may retry individual requests internally before returning a terminal failure to Carapace; those internal retries are separate from Carapace's continuation budget.
...
For SDK calls that retain internal retries, Stainless-based SDKs such as Anthropic and OpenAI can receive `retry-after-ms` or `retry-after` on retryable responses (`408`, `409`, `429`, and `5xx`). When that wait is longer than 60 seconds, Carapace injects `x-should-retry: false` so the SDK returns control promptly. Override this SDK-only cap with `CARAPACE_SDK_RETRY_MAX_WAIT_SECONDS=<seconds>`. Set it to `0`, `false`, `off`, `none`, or `disabled` to let those SDK calls honor long `Retry-After` sleeps internally.
...
Discord and Telegram channel retry timings are built in and are not configurable in `carapace.json`.

The embedded runtime's existing session setting `retry.provider.maxRetries` overrides its recovery retry budget; `0` disables retries, and rate limits remain capped at 10 total attempts. This is an embedded session setting, not an `carapace.json` key, and it does not configure native harness request retries. Automatic recovery requires no new configuration.
