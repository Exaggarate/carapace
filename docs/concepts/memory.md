---
summary: "How Carapace remembers things across sessions"
...
Carapace remembers things by writing plain Markdown files in your agent's
workspace (default `~/.carapace/workspace`). The model only remembers what gets
...
If `MEMORY.md` grows past the bootstrap file budget, Carapace keeps the file on
...
prompt budget. Use `/context list`, `/context detail`, or `carapace doctor` to
...
instead open **Settings → Ask Carapace** and say `import memory`; this narrower
chat wizard requires completed onboarding, copies only new detected memory, and
reports per-source failures or possible partial copies. Carapace copies only
Markdown memory:
...
Carapace approval settings, sandboxing, and scheduled tasks for hard
...
<Info>
Carapace uses OpenAI embeddings by default. Set
...
Carapace runs a silent turn that reminds the agent to save important context
...
- **Live dreaming** works from short-term dreaming state in SQLite plugin
  ...
  `memory/.dreams/`; run `carapace doctor --fix` before using that old state.
...
```bash
carapace memory rem-backfill --rollback
carapace memory rem-backfill --rollback-short-term
```

## CLI

```bash
carapace memory status          # Check index status and provider
carapace memory search "query"  # Search from the command line
carapace memory index --force   # Rebuild the index
```
