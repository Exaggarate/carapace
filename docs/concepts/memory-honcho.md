...
[Honcho](https://honcho.dev) adds AI-native memory to Carapace through an
...
```bash
carapace plugins install @honcho-ai/carapace-honcho
carapace honcho setup
...
Settings live under `plugins.entries["carapace-honcho"].config`:
    ...
    entries: {
      "carapace-honcho": {
        config: {
          ...
          workspaceId: "carapace", // memory isolation
...
`IDENTITY.md`, `memory/`), `carapace honcho setup` detects and offers to
migrate them.
...
During conversation, Honcho tools query the service during Carapace's
...
```bash
carapace honcho setup                        # Configure API key and migrate files
carapace honcho status                       # Check connection status
carapace honcho ask <question>               # Query Honcho about the user
carapace honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/carapace-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Carapace integration guide](https://docs.honcho.dev/v3/guides/integrations/carapace)
