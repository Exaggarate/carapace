...
TypeBox is a TypeScript-first schema library. Carapace uses it to define the **Gateway WebSocket protocol** (handshake, request/response, server events). Those schemas drive **runtime validation** (TypeBox Compile), **JSON Schema export**, and **Swift codegen** for the macOS app. One source of truth; everything else is generated.
...
- Generated Swift models: `apps/shared/CarapaceKit/Sources/CarapaceProtocol/GatewayModels.swift`
...
- [`protocol.schema.json`](https://unpkg.com/@carapace/gateway-protocol@beta/protocol.schema.json)
