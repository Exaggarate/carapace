...
<AccordionGroup>
  <Accordion title="Automations not firing">
    - Check `cron.enabled` and the `CARAPACE_SKIP_CRON` env var.
    - Confirm the Gateway is running continuously.
    - For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
    - `reason: not-due` in run output means the manual run was checked with `carapace automations run <jobId> --due` and the job was not due yet.
  ...
  <Accordion title="Job fired but no delivery">
    ...
    - If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`), Carapace suppresses direct outbound delivery and the fallback queued-summary path, so nothing is posted back to chat.
  ...
  <Accordion title="Automations or heartbeat appear to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, Carapace can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
