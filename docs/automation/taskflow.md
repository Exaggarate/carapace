...
read_when:
  ...
  - You encounter Task Flow or carapace tasks flow in release notes or docs
...
Flow records persist in the shared SQLite state database (`~/.carapace/state/carapace.sqlite`, `flow_runs` table) alongside task records, so progress survives gateway restarts. Each write bumps the flow's `revision`; concurrent writers that pass a stale expected revision get a conflict and must re-read. WAL growth is bounded by SQLite autocheckpointing plus periodic passive checkpoints, with truncate checkpoints on shutdown. The legacy `flows/registry.sqlite` sidecar from older installs is imported by `carapace doctor`.
...
| Command                           | Description                                                             |
| --------------------------------- | ----------------------------------------------------------------------- |
| `carapace tasks flow list`        | Tracked flows with sync mode, status, revision, controller, task counts |
| `carapace tasks flow show <id>`   | Inspect one flow by flow id or owner key, including linked tasks        |
| `carapace tasks flow cancel <id>` | Cancel a running flow and its active tasks                              |

Flows are also covered by `carapace tasks audit` (stale or broken flow findings) and `carapace tasks maintenance` (finalizes stuck cancels, prunes terminal flows after 7 days).
...
Example automation job (`carapace automations`; `carapace cron` remains an alias):
...
- Browser availability and profile choice, for example `carapace` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
...
Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `carapace tasks` to inspect individual task records and `carapace tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) - the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) - CLI command reference for `carapace tasks flow`
