---
summary: "Advanced setup and development workflows for Carapace"
read_when:
...
- **Tailoring lives outside the repo:** keep your config and workspace in `~/.carapace/carapace.json` and `~/.carapace/workspace/` so repo updates don't touch them.
...
- `pnpm` required for source checkouts. Carapace loads bundled plugins from the
...
- **Config:** `~/.carapace/carapace.json` (JSON/JSON5-ish)
- **Workspace:** `~/.carapace/workspace` (skills, prompts, memories; make it a private git repo)
...
```bash
pnpm carapace setup --baseline
```

(Bare `carapace setup`, without `--baseline`, is an alias for `carapace onboard` and runs the full interactive wizard.)
...
```bash
node carapace.mjs gateway --port 18789 --verbose
```

## Stable workflow (macOS app first)

1. Install + launch **Carapace.app** (menu bar).
...
- Run `carapace setup`, then `carapace channels login`, then start the Gateway manually (`carapace gateway`).
...
```bash
pnpm install
# First run only (or after resetting local Carapace config/workspace)
pnpm carapace setup
pnpm gateway:watch
```
...
- It starts or restarts the Gateway watch process in a named tmux session,
  `carapace-gateway-watch-main`, and auto-attaches from interactive terminals.
- Non-interactive shells stay detached and print
  `tmux attach -t carapace-gateway-watch-main`. Run
  `CARAPACE_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch` to keep an interactive
...
- It stops the active profile's installed Gateway service before it takes over
  ...
  Run `pnpm carapace gateway start` when you finish watching.
...
- If the watched Gateway exits during startup, `gateway:watch` runs
  `carapace doctor --fix --non-interactive` once and retries. Set
  `CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR=0` to disable that dev-only repair pass.

TypeScript rebuilds triggered by `pnpm carapace ...` or `pnpm gateway:watch` preserve existing `dist/control-ui` assets. When the Gateway starts, it rebuilds missing, incomplete, or stale bundled UI assets before serving them. Headless commands do not rebuild the UI. Run `pnpm ui:build` after `ui/` changes, or use `pnpm ui:dev` while developing the Control UI.
...
- **Wrong developer CLI:** When `PATH` includes `node_modules/.bin`, `codex` can
  ...
  Carapace's [managed Codex app-server](/plugins/codex-harness-reference#app-server-transport)
...
- **Where state lives:**
  - Channel/provider state: `~/.carapace/credentials/`
  - Model auth profiles: SQLite auth stores (shared: `~/.carapace/state/carapace.sqlite`; agent-local: `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite`)
  - Sessions and transcripts: `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite`
...
- **Pairing allowlists**:
  - `~/.carapace/credentials/<channel>-allowFrom.json` (default account)
  - `~/.carapace/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
...
- **File-backed secrets payload (optional)**: `~/.carapace/secrets.json`
- **Legacy OAuth import**: `~/.carapace/credentials/oauth.json`
  More detail: [Security](/gateway/security#credential-storage-map).

## Updating (without wrecking your setup)

- Keep `~/.carapace/workspace` and `~/.carapace/` as "your stuff"; don't put personal prompts/config into the `carapace` repo.
...
- [Carapace assistant setup](/start/carapace)
