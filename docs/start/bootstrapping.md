...
On the first run against a brand-new workspace (default `~/.carapace/workspace`),
Carapace:
...
- Persists the agreed identity twice: into `IDENTITY.md` and `SOUL.md` (what the
  agent reads about itself) and via `carapace agents set-identity` (what channels
  and the UI display).
- Reads app recommendations already stored during onboarding without rescanning.
  Official plugins use `carapace plugins install <id>`; third-party ClawHub
...
For embedded or local-model runs, Carapace keeps `BOOTSTRAP.md` out of the
...
host (for example, `user@gateway-host:~/.carapace/workspace`).
</Note>
