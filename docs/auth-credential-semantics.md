...
- `carapace doctor` auth checks (`doctor-auth`)
...
Agent auth inheritance is read-through. When an agent has no local profile, it resolves profiles from the shared auth store at runtime without copying secret material into its own credential store (`agents/<agentId>/agent/carapace-agent.sqlite`). The shared store lives in `state/carapace.sqlite` after `carapace doctor --fix` performs the one-time relocation. Until then, doctor reports the legacy `agents/main/agent/carapace-agent.sqlite` owner and leaves that agent undeletable.

Explicit copy flows, such as `carapace agents add`, use this portability policy:
...
Do not write `type: "aws-sdk"` into the credential store; stored credentials are only `api_key`, `token`, or `oauth`. If a legacy `auth-profiles.json` has such a marker, `carapace doctor --fix` moves it to `auth.profiles` and removes the marker from the store.
...
- A valid session user pin is an explicit per-session exception: Carapace tries that profile first even when it is omitted from the provider order, then uses the ordered same-provider profiles as retry candidates. A cooldown or disabled window applies only to the affected profile; it does not suppress its eligible siblings.
...
and run `carapace secrets reload` before retrying discovery.
...
- If a provider has credentials but Carapace cannot resolve a probeable model candidate for it, `models status --probe` reports `status: no_model` with `reasonCode: no_model`.
...
- `/models` reuses external login evidence already prepared with its catalog, so those providers remain visible without a second Carapace login. Opening the default menu does not repeat external CLI discovery; explicit auth order and route compatibility still apply.
