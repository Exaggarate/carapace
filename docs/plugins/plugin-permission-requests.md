...
| Codex native permission requests | Codex asks before native shell, file, MCP, or app-server actions.        | Codex app-server or native hook approval handling, routed through plugin approvals when Carapace owns the prompt. |
| MCP approval elicitations        | A Codex MCP server requests approval for a tool call.                    | MCP approval responses bridged through Carapace plugin approvals.                                                 |
...
runs after the model selects a tool and before Carapace executes it:
...
to 12. If sanitization would exceed a bound, Carapace omits the scope while
preserving the normal approval prompt.

## Decision behavior

Carapace creates a pending approval with a `plugin:` ID, delivers it to the
...
Carapace treats `allow-once` and `allow-always` as approval decisions for the
...
If the hook also returns `params`, Carapace snapshots the base parameters and
...
- Codex app-server approval requests route through Carapace after Codex review.
