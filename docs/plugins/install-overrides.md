...
[`carapace plugins install`](/cli/plugins).
...
```bash
export CARAPACE_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export CARAPACE_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/carapace-codex-2026.5.8.tgz",
  "carapace-web-search": "npm:@carapace/web-search@2026.5.8"
}'
```
...
When a setup-time flow installs a plugin whose id appears in the map, Carapace
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents an Carapace-owned package, an override is
...
- Workspace `.env` files cannot enable install overrides; both env vars are on
  ...
  remote test command that launches Carapace.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Carapace state:

```bash
npm pack extensions/codex --pack-destination /tmp

CARAPACE_STATE_DIR="$(mktemp -d)" \
CARAPACE_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
CARAPACE_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/carapace-codex-2026.5.8.tgz"}' \
pnpm carapace onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$CARAPACE_STATE_DIR/npm/projects" -path '*/node_modules/@carapace/codex/package.json' -print
grep -R '"@carapace/codex"' "$CARAPACE_STATE_DIR/npm/projects"/*/package-lock.json
```
