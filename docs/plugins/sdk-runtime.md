...
`api.runtime.version` is the current Carapace product version, sourced from the shared version resolver so plugins see the same value the CLI reports.
...
<ParamField path="api.config" type="CarapaceConfig">
