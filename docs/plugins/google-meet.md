...
read_when:
  - You want an Carapace agent to join a Google Meet call
  - You want an Carapace agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

The `google-meet` plugin joins explicit Meet URLs on behalf of an Carapace agent. It is deliberately narrow:
...
- `mode: "agent"` (default) transcribes participant speech with a realtime provider, routes it to the configured Carapace agent, and speaks the answer with regular Carapace TTS. `mode: "bidi"` lets a realtime voice model answer directly. `mode: "transcribe"` joins observe-only with no talk-back.
...
```bash
carapace plugins install npm:@carapace/google-meet
...
Carapace provisions an `Carapace Meeting Audio` null sink and matching source in that desktop user's audio session. Run the Gateway or paired node as the same user that runs Chrome.
...
Run `carapace plugins disable google-meet` if you do not want the plugin active.

Check setup, then join:

```bash
carapace googlemeet setup
carapace googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
carapace googlemeet setup --transport chrome-node --mode transcribe
```
...
```bash
carapace googlemeet create --transport chrome-node --mode agent
carapace googlemeet create --no-join
```
...
- **`browser`**: used without OAuth credentials. Carapace opens `https://meet.google.com/new` on the pinned Chrome node and waits for Google to redirect to a real meeting-code URL; the Carapace Chrome profile on that node must already be signed in to Google. Join and create both reuse an existing Meet tab (or an in-progress `.../new` / Google account prompt tab) before opening a new one; tab matching ignores harmless query strings like `authuser`.
...
```bash
carapace googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
This only applies to API-created rooms, so OAuth must be configured. If you authenticated before this option existed, rerun `carapace googlemeet auth login --json` after adding the `meetings.space.settings` scope to your OAuth consent screen.
...
Set `"mode": "transcribe"` to skip the duplex realtime bridge (no virtual-audio requirement, no talk-back). Transcribe-mode Chrome joins also skip Carapace's microphone/camera permission grant and the Meet **Use microphone** path; if Meet shows the audio-choice interstitial, automation tries **Continue without microphone** first. Managed Chrome transports install a best-effort Meet caption observer in every mode so durable notes are available without changing the live agent-consult path. `googlemeet status --json` and `googlemeet doctor` report `captioning`, `captionsEnabledAttempted`, `transcriptLines`, `lastCaptionAt`, `lastCaptionSpeaker`, `lastCaptionText`, and a `recentTranscript` tail.

For the bounded session transcript, read the exact tracked Meet tab:

```bash
carapace googlemeet transcript <session-id>
carapace googlemeet transcript <session-id> --since <next-index> --json
```

The observer keeps at most 2,000 completed caption lines in the Meet page. Visible progressive text stays in the status health tail until the caption row completes, so saving `nextIndex` cannot skip a later text expansion; leaving finalizes visible rows before the snapshot. `droppedLines` reports lines lost from the head when the cap is exceeded. The bounded `googlemeet transcript` tail still keeps only the four most recently ended sessions and resets with the Gateway. Separately, Carapace appends completed caption rows to the shared state database throughout the meeting and writes a derived summary on leave. Use [`carapace transcripts`](/cli/transcripts) to inspect or export those durable notes.
...
```bash
carapace googlemeet test-listen <meet-url> --transport chrome-node
```
...
Local Chrome joins through the signed-in Carapace browser profile and routes its microphone and speaker through the native backend selected by `chrome.audioBackend`. The default shared loopback device is enough for a first smoke test but can echo; use separate virtual devices or a Loopback-style graph for clean duplex audio.
...
| Runs where           | What                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------- |
| Gateway host         | Carapace Gateway, agent workspace, model/API keys, realtime provider, Google Meet plugin config |
| Parallels macOS VM   | Carapace CLI/node host, Chrome, SoX, BlackHole 2ch, a Chrome profile signed in to Google        |
...
```bash
carapace plugins install npm:@carapace/google-meet
carapace node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
CARAPACE_ALLOW_INSECURE_PRIVATE_WS=1 \
  carapace node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```

Use the same flag when installing as a LaunchAgent (it is process environment, stored in the LaunchAgent environment when present on the install command, not an `carapace.json` setting):

```bash
CARAPACE_ALLOW_INSECURE_PRIVATE_WS=1 \
  carapace node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
...
```bash
carapace googlemeet test-speech https://meet.google.com/abc-defg-hij
```
...
If `chromeNode.node` is omitted, Carapace auto-selects only when exactly one connected node advertises both `googlemeet.chrome` and browser control; pin `chromeNode.node` (node id, display name, or remote IP) when several capable nodes are connected.
...
| `No connected Google Meet-capable node`                  | Install `npm:@carapace/google-meet` in the VM, run `carapace plugins enable browser`, start `carapace node run`, and approve pairing. If Google Meet was explicitly disabled, enable it too. Confirm `gateway.nodes.commands.allow` includes `googlemeet.chrome` and `browser.proxy`. |
...
| Chrome opens but cannot join                             | Sign in to the browser profile in the VM, or keep `chrome.guestName` set. Guest auto-join uses Carapace browser automation through the node browser proxy; point the node's `browser.defaultProfile` (or a named existing-session profile) at the profile you want.                   |
| Duplicate Meet tabs                                      | Leave `chrome.reuseExistingTab: true`. Carapace activates an existing tab for the same URL, and creation reuses an in-progress `.../new` or Google account prompt tab, before opening another.                                                                                        |
| No audio                                                 | Route Meet mic/speaker through the virtual audio path used by Carapace; use separate virtual devices or Loopback-style routing for clean duplex audio.                                                                                                                                |

## Install notes

The Chrome talk-back default uses host audio tools that Carapace does not bundle or redistribute:
...
- `pactl`, `pacat`, and `parec`: Linux PulseAudio utilities used against PipeWire-Pulse to provision and stream through `Carapace Meeting Audio`.

SoX is licensed `LGPL-2.0-only AND GPL-2.0-only`; BlackHole is GPL-3.0. If you build an installer or appliance that bundles BlackHole with Carapace, review BlackHole's upstream licensing or get a separate license from Existential Audio.
...
Opens the Meet URL through Carapace browser control and joins as the signed-in Carapace browser profile. Before launch, the plugin checks or provisions the host's native virtual-audio backend and then runs any configured audio bridge health/startup command. For local Chrome, pick the profile with `browser.defaultProfile`; `chrome.browserProfile` is passed to `chrome-node` hosts instead.

```bash
carapace googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
carapace googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Chrome mic/speaker audio routes through the local Carapace audio bridge. If the native backend is unavailable, the join fails with a setup error instead of joining without an audio path.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Carapace agent. Be brief.",
...
Provide Twilio credentials through environment to keep secrets out of `carapace.json`:
...
carapace plugins list | grep -E 'google-meet|voice-call'
carapace googlemeet setup
```
...
`meetings.space.created` is required by `spaces.create`. `meetings.space.readonly` resolves Meet URLs/codes to spaces. `meetings.space.settings` lets Carapace pass `SpaceConfig` settings such as `accessType` during API room creation. `meetings.conference.media.readonly` is for Meet Media API preflight and media work; Google may require Developer Preview enrollment for actual Media API use. `calendar.events.readonly` is only needed for `--today`/`--event` calendar lookup. `drive.meet.readonly` is only needed for `--include-doc-bodies` export. If you only need browser-based Chrome joins, skip OAuth entirely.
...
```bash
carapace googlemeet auth login --json
```
...
```bash
CARAPACE_GOOGLE_MEET_CLIENT_ID="your-client-id" \
CARAPACE_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
carapace googlemeet auth login --json --manual
```
...
```bash
carapace googlemeet doctor --oauth --create-space --json
```

Prove read access to an existing space:

```bash
carapace googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
carapace googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```

A `403` from these checks usually means the Meet REST API is disabled, the refresh token is missing the required scope, or the Google account cannot access that space. A refresh-token error means rerun `carapace googlemeet auth login --json` and store the new `oauth` block.

No OAuth is needed for the browser fallback; Google auth there comes from the signed-in Chrome profile on the selected node, not Carapace config.

These environment variables are accepted as fallbacks:

- `CARAPACE_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `CARAPACE_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `CARAPACE_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `CARAPACE_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `CARAPACE_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `CARAPACE_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `CARAPACE_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`

### Resolve, preflight, and read artifacts

```bash
carapace googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
carapace googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

After Meet has created conference records:

```bash
carapace googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
carapace googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
carapace googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
carapace googlemeet latest --today
carapace googlemeet calendar-events --today --json
carapace googlemeet artifacts --event "Weekly sync"
carapace googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
carapace googlemeet latest --meeting https://meet.google.com/abc-defg-hij
carapace googlemeet artifacts --conference-record conferenceRecords/abc123 --json
carapace googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Close the room for an API-created space:

```bash
carapace googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```

Calls `spaces.endActiveConference` and requires OAuth with the `meetings.space.created` scope for a space the authorized account can manage. Accepts a Meet URL, meeting code, or `spaces/{id}` and resolves it to the API space resource first. This is separate from `googlemeet leave`: `leave` stops Carapace's local/session participation; `end-active-conference` asks Google Meet to end the active conference for the space.

Write a readable report:

```bash
carapace googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
carapace googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
carapace googlemeet export --conference-record conferenceRecords/abc123 \
...
CARAPACE_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
pnpm test:live -- extensions/google-meet/google-meet.live.test.ts
```

```bash
carapace googlemeet setup --transport chrome-node --mode transcribe
carapace googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

| Variable                                                                                                                  | Purpose                                                                |
| ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `CARAPACE_LIVE_TEST=1`                                                                                                    | Enables guarded live tests                                             |
| `CARAPACE_GOOGLE_MEET_LIVE_MEETING`                                                                                       | Retained Meet URL, code, or `spaces/{id}`                              |
| `CARAPACE_GOOGLE_MEET_CLIENT_ID` / `GOOGLE_MEET_CLIENT_ID`                                                                | OAuth client id                                                        |
| `CARAPACE_GOOGLE_MEET_REFRESH_TOKEN` / `GOOGLE_MEET_REFRESH_TOKEN`                                                        | Refresh token                                                          |
| `CARAPACE_GOOGLE_MEET_CLIENT_SECRET`, `CARAPACE_GOOGLE_MEET_ACCESS_TOKEN`, `CARAPACE_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` | Optional; same fallback names without the `CARAPACE_` prefix also work |
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Carapace browser profile, then retry meeting creation.",
  "manualAction": {
    "reason": "google-login-required",
    "message": "Sign in to Google in the Carapace browser profile, then retry meeting creation."
  },
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
Creating joins by default, but Chrome/Chrome-node still needs a signed-in Google profile to join through the browser; if signed out, Carapace returns `manualAction` or a browser fallback error and asks the operator to finish Google login before retrying.
...
The common Chrome agent path only needs the plugin enabled, BlackHole, SoX, a realtime provider key, and a configured Carapace TTS provider:
...
| `chrome.guestName`                | `"Carapace Agent"`                       | Shown on the signed-out Meet guest screen                                                                                                                                                                         |
...
| `chrome.audioInputCommand`        | generated native command                 | SoX/CoreAudio on macOS; `parec` from the Carapace PipeWire-Pulse source on Linux                                                                                                                                  |
| `chrome.audioOutputCommand`       | generated native command                 | SoX/CoreAudio on macOS; `pacat` into the Carapace PipeWire-Pulse sink on Linux                                                                                                                                    |
...
| `realtime.instructions`           | brief spoken-reply instructions          | Tells the model to speak briefly and use `carapace_agent_consult` for deeper answers                                                                                                                              |
...
| `realtime.agentId`                | `"main"`                                 | Carapace agent id used for `carapace_agent_consult`                                                                                                                                                               |
...
An `carapace doctor --fix` migration exists for the legacy `realtime.provider: "google"` shape: it moves that intent to `realtime.voiceProvider: "google"` plus `realtime.transcriptionProvider: "openai"` when those fields are not already set.
...
For Chrome transports, `leave` keeps a reused user-owned tab open after clicking Meet's Leave call button. Tabs opened by Carapace are closed after departure.

Use `transport: "chrome"` when Chrome runs on the Gateway host, `transport: "chrome-node"` when it runs on a paired node. In both cases the model providers and `carapace_agent_consult` run on the Gateway host, so model credentials stay there. Agent-mode logs include the resolved transcription provider/model at bridge startup and the TTS provider/model/voice/output format/sample rate after each synthesized reply. Raw `mode: "realtime"` is still accepted as a legacy compatibility alias for `mode: "agent"`, but it is no longer advertised in the tool's `mode` enum.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage` | Whether managed Chrome speech is allowed now; `speechReady: false` means Carapace did not send the intro/test phrase   |
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Carapace agent | Normal Carapace TTS runtime            | You want "my agent is in the meeting" behavior        |
...
```bash
carapace googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
carapace googlemeet setup
carapace nodes status --connected
carapace nodes invoke \
  --node parallels-macos \
...
- `carapace logs --follow` shows DTMF TwiML served before realtime TwiML, then a realtime bridge with the initial greeting queued.
...
```bash
carapace plugins list | grep google-meet
carapace googlemeet setup
```
...
```bash
carapace plugins install npm:@carapace/google-meet
carapace plugins enable browser
CARAPACE_ALLOW_INSECURE_PRIVATE_WS=1 \
  carapace node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
- **API creation**: `oauth.clientId` and `oauth.refreshToken` (or matching `CARAPACE_GOOGLE_MEET_*` env vars) are present, and the refresh token was minted after create support was added; older tokens may lack `meetings.space.created`, so rerun `carapace googlemeet auth login --json`.
- **Browser fallback**: `defaultTransport: "chrome-node"` and `chromeNode.node` point at a connected node with `browser.proxy` and `googlemeet.chrome`; the Carapace Chrome profile on that node is signed in and can open `https://meet.google.com/new`.
...
- **Audio-choice interstitial**: if Meet shows "Do you want people to hear you in the meeting?", leave the tab open. Carapace should click **Use microphone** or (create-only) **Continue without microphone** and keep waiting for the generated URL; if it cannot, the error should mention `meet-audio-choice-required`, not `google-login-required`.

### Agent joins but does not talk

```bash
carapace googlemeet setup
carapace googlemeet doctor
```

Use `mode: "agent"` for the STT -> Carapace agent -> TTS path, `mode: "bidi"` for the direct realtime voice fallback. `mode: "transcribe"` intentionally starts no talk-back bridge. For observe-only debugging, run `carapace googlemeet status --json <session-id>` after participants speak and check `captioning`, `transcriptLines`, `lastCaptionText`. If `inCall` is true but `transcriptLines` stays `0`, Meet captions may be disabled, no one has spoken since the observer was installed, the Meet UI changed, or live captions are unavailable for the meeting language/account.

`googlemeet test-speech` always checks the realtime path and reports whether bridge output bytes were observed for that invocation. If `speechOutputVerified` is false and `speechOutputTimedOut` is true, the realtime provider may have accepted the utterance but Carapace did not see new output bytes reach the Chrome audio bridge.
...
```bash
carapace googlemeet recover-tab
carapace googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
- `carapace googlemeet doctor <session-id>`: confirm the delegated Twilio call ID, whether DTMF was queued, and whether the intro greeting was requested.
- `carapace voicecall status --call-id <id>`: confirm the call is still active.
- `carapace voicecall tail`: confirm Twilio webhooks are arriving at the Gateway.
- `carapace logs --follow`: look for the Twilio Meet sequence: Google Meet delegates the join, Voice Call stores and serves pre-connect DTMF TwiML, Voice Call serves realtime TwiML for the Twilio call, then Google Meet requests intro speech with `voicecall.speak`.
- Re-run `carapace googlemeet setup --transport twilio`; a green setup check is required but does not prove the meeting PIN sequence is correct.
