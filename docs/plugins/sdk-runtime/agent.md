...
snapshots; Carapace owns all persistence and lifecycle coordination.
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` is the neutral helper for starting a normal Carapace agent turn from plugin code. It uses the same provider/model resolution and agent-harness selection as channel-triggered replies.
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Before advertising an ACP-backed action, use `resolveAcpSessionAvailability(...)` from `carapace/plugin-sdk/acp-runtime`. It applies the canonical enablement, dispatch, allowed-agent, registered-backend, and backend-health checks; recheck it immediately before creating the session.

    ACP manager inputs accept an optional `agentId` identifying the Carapace session owner; `agent` selects the external harness. Carry the resolved owner from `resolveSession(...)` through subsequent calls, including controls and cleanup. `expectedOwnerKey` retains its parent-session meaning.
    ...
    `loadTranscriptEventsSync(...)` is available for synchronous doctor and repair paths that cannot use the async transcript runtime. It returns raw `SessionStoreTranscriptEvent` records and does not consult runtime `session.store`; pass `storePath` for a non-default store. Normal plugin runtime code should prefer `carapace/plugin-sdk/session-transcript-runtime`.
