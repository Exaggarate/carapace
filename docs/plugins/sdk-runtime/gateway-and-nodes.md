...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.nodes">
    ...
    without raising its per-message ceiling. Carapace splits each binary message
    ...
    `openDuplex` resolves. Before a listener is registered, Carapace buffers at
    ...
    Inside the Gateway this runtime is in-process. In plugin CLI commands it calls the configured Gateway over RPC, so commands such as `carapace googlemeet recover-tab` can inspect paired nodes from the terminal. Node commands still go through normal Gateway node pairing, command allowlists, plugin node-invoke policies, and node-local command handling.
...
Carapace calls a service's `stop()` at most once per startup attempt, including when a replacement
