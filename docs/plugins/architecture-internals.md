...
At startup, Carapace does roughly this:
...
The manifest is the control-plane source of truth. Carapace uses it to:
...
derived set is empty, Carapace keeps the scope empty instead of widening to
...
Carapace still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Carapace calls hooks in this rough order.
...
| _(built-in model lookup)_         | Carapace tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Carapace refreshers                                                                                          |
...
- Model references apply manifest-declared model-ID normalization once before
  ...
  model ID; an empty result keeps it unchanged. Carapace does not try other
...
- `normalizeConfig` uses the owning bundled provider's lightweight policy surface
  first. If that surface has no `normalizeConfig` hook, Carapace may call the
...
that still runs on Carapace's normal inference loop.

`resolveUsageAuth` decides whether Carapace should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Carapace's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Carapace adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while Carapace owns the provider/runtime boundary.
...
- Carapace only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Carapace plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope. The default surface (`gatewayRuntimeScopeSurface: "write-default"`) is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) and any non-trusted-proxy auth method get a single `operator.write` scope, even if the caller sends `x-carapace-scopes`
  - `trusted-proxy` callers without an explicit `x-carapace-scopes` header also keep the legacy `operator.write`-only surface
  - `trusted-proxy` callers that do send `x-carapace-scopes` get the declared scopes instead
  - a route can opt into `gatewayRuntimeScopeSurface: "trusted-operator"` to always honor `x-carapace-scopes` for identity-bearing auth modes (falling back to the full CLI default scope set when the header is absent)
...
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, opt into `trusted-operator` scope surface, require an identity-bearing auth mode, and document the explicit `x-carapace-scopes` header contract.
...
Use narrow SDK subpaths instead of the monolithic `carapace/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                            | Purpose                                      |
| ---------------------------------- | -------------------------------------------- |
| `carapace/plugin-sdk/plugin-entry` | Plugin registration primitives               |
| `carapace/plugin-sdk/channel-core` | Channel entry/build helpers                  |
| `carapace/plugin-sdk/core`         | Generic shared helpers and umbrella contract |
...
<Info>
`carapace/plugin-sdk/channel-lifecycle`, small channel helper facades,
`carapace/plugin-sdk/config-runtime`, and `carapace/plugin-sdk/infra-runtime`
...
External plugins should only import `carapace/plugin-sdk/*` subpaths. Never
...
`carapace/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Carapace writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Carapace's
...
- if both `catalog` and `discovery` are registered, Carapace uses `catalog`
  and emits a warning
...
- Read-only command paths such as `carapace status`, `carapace status --all`,
  `carapace channels status`, `carapace channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `carapace.extensions`:
...
Security guardrail: every `carapace.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `carapace plugins install` installs plugin dependencies with a
...
Optional: `carapace.setupEntry` can point at a lightweight setup-only module.
When Carapace needs setup surfaces for a disabled channel plugin, or
...
Channel plugins can advertise setup/discovery metadata via `carapace.channel` and
install hints via `carapace.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@carapace/nextcloud-talk",
  "carapace": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@carapace/nextcloud-talk",
...
Useful `carapace.channel` fields beyond the minimal example:
...
Carapace can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.carapace/mpm/plugins.json`
- `~/.carapace/mpm/catalog.json`
- `~/.carapace/plugins/catalog.json`

Or point `CARAPACE_PLUGIN_CATALOG_PATHS` (or `CARAPACE_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "carapace": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `carapace.install` block. The
...
2. **Add typed plugin registration/runtime surfaces.** Extend
   `CarapacePluginApi` and/or `api.runtime` with the smallest useful typed
   capability surface.
...
This is how Carapace stays opinionated without becoming hardcoded to one
