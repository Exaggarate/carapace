...
| `dangerousFlags`              | No       | `object[]` | Config literals that `carapace doctor` should flag as insecure or dangerous when enabled. See below.                                                                                                                                   |
...
The map key is the integration id. If `providerAlias` is omitted, Carapace uses the integration id as the SecretRef provider alias. Provider aliases must match the normal SecretRef provider alias pattern, for example `team-secrets` or `onepassword-work`.

When an operator selects the preset, Carapace writes a provider reference like:
