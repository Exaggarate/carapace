...
read_when:
  - You want Carapace to share the native Codex home
...
Opt-in features that connect an Carapace agent to native Codex state and Codex-owned capabilities. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## Share threads with Codex Desktop and CLI

The default `appServer.homeScope: "agent"` isolates each Carapace agent from
...
that home's native Codex auth, config, plugins, and thread store. Carapace does
not inject an Carapace auth profile into this app-server, even when the agent's
...
- A Platform (API-key) route refuses a native home signed in with a ChatGPT
  ...
  and let Carapace inject the key it already holds.

A stored OpenAI profile is fine alongside `homeScope: "user"`; Carapace keeps it
for agent-scoped connections and simply does not hand it to the native home. Use
`carapace models auth list --provider openai` to inspect stored profiles and
`carapace models auth logout <profileId> --yes` to remove one you no longer want.
...
Carapace; the fork attaches to the current Carapace session and remains readable
...
`sessionCatalog.enabled: false` to remove it from the Carapace sidebar without
...
native configuration; the outer Carapace model and fallback chain never replace
...
capabilities in the same Codex thread as the Carapace harness turn. Carapace
does not translate Codex plugins into synthetic `codex_plugin_*` Carapace
dynamic tools.
...
Thread app config is computed when Carapace establishes a Codex harness
...
Short version: Carapace does not vendor the desktop-control app or execute
