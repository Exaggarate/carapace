...
| `clearEnv`                       | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Carapace builds its inherited environment. Carapace keeps the selected `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                                                              |
| `codeModeOnly`                   | `false`                                                | Opt into Codex's code-mode-only tool surface. Ordinary Carapace dynamic tools remain available through nested `tools.*` calls; `carapace_direct` tools stay directly model-visible.                                                                                                                                                                                                                                                |
...
| `sandbox`                        | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an Carapace sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the Carapace sandbox egress setting.                                                                                                                        |
...
contract. When enabled, Carapace also sets `features.network_proxy.enabled`
and `default_permissions` in the Codex thread config so the generated
permission profile can start Codex managed networking. By default, Carapace
generates a collision-resistant `carapace-network-<fingerprint>` profile
