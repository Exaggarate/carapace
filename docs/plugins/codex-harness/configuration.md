...
| Need                                                | Set                                                                                                       | Where                              |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                                  | `plugins.entries.codex.enabled: true`                                                                     | Carapace config                    |
...
| Keep an allowlisted plugin install                  | Include `codex` in `plugins.allow`                                                                        | Carapace config                    |
...
| Sign in with ChatGPT/Codex OAuth                    | `carapace models auth login --provider openai`                                                            | CLI auth profile                   |
| Add API-key backup for Codex runs                   | `openai:*` API-key profile listed after subscription auth in `auth.order.openai`                          | CLI auth profile + Carapace config |
| Fail closed when Codex is unavailable               | Provider or model `agentRuntime.id: "codex"`                                                              | Carapace model/provider config     |
| Use direct OpenAI API traffic                       | Provider or model `agentRuntime.id: "carapace"` with normal OpenAI auth                                   | Carapace model/provider config     |
...
Carapace applies Codex restrictions per turn, not as a permanent session mode.
...
temporary, Carapace preserves the normal thread binding and uses a temporary
restricted thread where necessary.

An ordinary **policy-restricted turn** occurs when an explicit Carapace tool
...
containing only audited Carapace-owned tools can also stay on the normal native
...
For an ordinary policy-restricted turn, Carapace disables Codex native Code
...
MCP tools that pass the effective policy move to Carapace's dynamic surface for
that turn. Other Carapace dynamic tools use the same policy. The bounded workspace `AGENTS.md`
...
**Ring zero** is stronger and separate. It is the host-owned Carapace system
agent used for setup and repair operations. The host activates it with the
single `carapace` tool; normal agent config cannot opt a chat into ring zero.
...
normal app-server threads, Carapace raises Codex's aggregate root-to-working-
...
so Carapace supplies the bounded workspace `AGENTS.md` snapshot as thread-level
...
Carapace ignores those local summarizer overrides at runtime, and
`carapace doctor --fix` removes them when the agent uses Codex.
...
window. When the model entry has no authored cap, Carapace sends no override.
...
`agents.defaults.compaction.provider`. `carapace doctor --fix` migrates the
...
Carapace asks Codex to interrupt the native turn and keeps the per-thread fence
...
For the Carapace Codex app-server harness, keep the default agent-scoped Codex
home and let Carapace inject an `openai` API-key profile. Create the profile by
...
configuration unit. Apply them together. Carapace can keep embedded and native
...
produced `5442` output tokens, and Carapace sent the Codex app-server tier
...
output pricing. Carapace currently sends the wire value
