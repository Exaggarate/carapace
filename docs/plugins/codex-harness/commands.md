...
supports Carapace text commands.
...
- `/codex goal` reads or updates the attached thread's native Codex goal. Codex automatic goal continuation stays disabled; Carapace does not own autonomous follow-on turns yet.
- `/codex resume <thread-id>` attaches the current Carapace session to an
...
The target native thread must be idle. Carapace coordinates attachment, resume,
...
resume` or `/codex bind`. Carapace reports that restriction and keeps the current
...
or Codex cannot confirm that it applied the configuration, Carapace reports
...
If an ordinary Carapace-managed native thread was deleted, the next turn starts
...
`/fast` controls the shared Carapace policy. A directive-only `/fast off`
persists `off` in the Carapace session and sends `null` on affected Codex
harness turns to clear the Carapace-owned service-tier override. `/fast default`
...
is not a synonym for `/fast off`, and it does not change the shared Carapace
