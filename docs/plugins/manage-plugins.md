---
summary: "Manage Carapace plugins from the Control UI or CLI"
read_when:
...
rules, and edge cases, see [`carapace plugins`](/cli/plugins).
...
configured Control UI base path. For example, a base path of `/carapace` uses
`/carapace/settings/plugins`. The page has two tabs:
...
- **Discover** is the store: featured plugins included with Carapace, official
...
or **Disable**. Workboard, for example, is included with Carapace and disabled
...
OAuth-backed MCP connectors still need a one-time `carapace mcp login <name>`
...
`plugins list` is a cold inventory check: what Carapace can discover from
...
an install hint (`carapace plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
carapace plugins enable <plugin-id>
carapace plugins disable <plugin-id>
```
...
Carapace asks you to review a third-party plugin's declared capabilities before
...
Outside AI onboarding, bundled plugins and verified first-party plugins from Carapace's official
catalog do not require this capability review during install, enable, update,
or Doctor repair. For separately installed first-party plugins, Carapace checks
...
be reused. Run `carapace plugins enable <plugin-id>` to activate it afterward.
...
Carapace cannot carry acceptance forward from an artifact it cannot verify.
...
```bash
carapace plugins install clawhub:<package> --accept-capabilities
carapace plugins update <plugin-id> --accept-capabilities
...
`carapace plugins install --link <path>` creates a managed install record and
...
carapace plugins install clawhub:<package>@1.2.3
carapace plugins install clawhub:<package>@beta

# Install from npm.
carapace plugins install npm:<package>
carapace plugins install npm:@scope/carapace-plugin@1.2.3
carapace plugins install npm:@carapace/codex

# Install from a local npm-pack artifact.
carapace plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
carapace plugins install git:github.com/acme/carapace-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case Carapace uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. Carapace's bundled and official
...
ClawHub, or hook-pack install, use `carapace plugins update` instead. With
...
Carapace records the install but leaves the plugin disabled. Configure
`plugins.entries.<id>.config`, then run `carapace plugins enable <id>`. If an
...
```bash
carapace plugins update <plugin-id>
carapace plugins update <npm-package-or-spec>
carapace plugins update --all
carapace plugins update <plugin-id> --dry-run
```
...
If Carapace cannot prove exactly one package owner and a complete child list,
update and uninstall fail closed without changing package files, config, or the
installed index. Run `carapace plugins registry --refresh`, inspect
`carapace plugins doctor`, and use `carapace doctor --fix` for repairable legacy
index state. If the ambiguity remains, reinstall the package before retrying.

`carapace plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official Carapace
...
```bash
carapace plugins update @scope/carapace-plugin@beta
carapace plugins update @scope/carapace-plugin
```
...
See [`carapace plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
carapace plugins uninstall <plugin-id> --dry-run
carapace plugins uninstall <plugin-id>
carapace plugins uninstall <plugin-id> --keep-files
```
...
may break those Claws; use `carapace claws status` to review ownership first.
Removing a Claw releases its plugin reference but retains the process-wide
plugin by default.

In Nix mode (`CARAPACE_NIX_MODE=1`), plugin install, update, uninstall,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want Carapace-native discovery, scan summaries, versions, and hints     | `carapace plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `carapace plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `carapace plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `carapace plugins install npm:@acme/carapace-plugin`           |
...
ClawHub is the primary public discovery surface for Carapace plugins. Publish
...
Native npm plugins must ship a plugin manifest (`carapace.plugin.json`) plus
`package.json` metadata before publishing:

```json package.json
{
  "name": "@acme/carapace-plugin",
  "version": "1.0.0",
  "type": "module",
  "carapace": {
...
carapace plugins install npm:@acme/carapace-plugin
carapace plugins install npm:@acme/carapace-plugin@beta
carapace plugins install npm:@acme/carapace-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `carapace.plugin.json`) and first publish
  workflow.
