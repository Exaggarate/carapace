...
read_when:
  ...
  - You need to map a third-party CLI into Carapace's text fallback runner
---

CLI backend plugins let Carapace call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Carapace at the plugin runtime module              |
| Manifest ownership   | `carapace.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `carapace.runtimeExtensions` pointing at the
  ...
  <Step title="Declare backend ownership">
    ```json carapace.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Carapace",
    ...
    `cliBackends` is the runtime ownership list; it lets Carapace auto-load the
  ...
  <Step title="Register the backend">
    ...
    function buildAcmeCliBackend(): Parameters<CarapacePluginApi["registerCliBackend"]>[0] {
      return {
        id: "acme-cli",
        liveTest: {
    ...
    adapter is authoritative plugin code; Carapace config selects the backend
...
`CliBackendConfig` describes how Carapace should launch and parse the CLI. The
...
| `modelAliases`                                            | Map Carapace model ids to CLI-native ids                                          |
| `sessionArgs`                                             | How to pass a session id using `{sessionId}`                                      |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields Carapace reads from CLI output                                        |
...
- Leave it undefined or set it to `"replace-binding"` to preserve the legacy
  clear-and-reseed behavior. Carapace clears the persisted binding and retries
...
| `defaultAuthProfileId`             | Prefer a specific Carapace auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Carapace's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own automatic compaction - Carapace defers                 |
...
operator-authored input. Carapace's policy and observation hooks still receive the
...
authority. Carapace retains cancellation, watchdogs, session policy, and MCP
...
command. Carapace hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise Carapace fails closed when BTW
...
`ctx.toolAvailability.carapace` is the exact list of Carapace tool names. The
host independently limits the generated MCP configuration and grant to that
Carapace list; plugins must not translate it in core or add transport prefixes.
...
- `toolAvailabilityEnforcement: "prepare-execution"` requires
  ...
  Carapace cleans up the staged resources before launch.

Runtime caps such as cron `toolsAllow` are normalized and group-expanded by
Carapace before this contract is built. Native tools are disabled, and a
...
The host-prepared instruction snapshot must remain authoritative. Carapace
...
disables its native tools and serves the selected Carapace tools through the
...
Codex native code mode projects `read` and `exec` after Carapace explicitly
...
Tool events describe work the backend already performed. Carapace renders and
...
### `ownsNativeCompaction`: opting out of Carapace compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Carapace's safeguard summarizer never runs
...
`manualCompaction`, so an explicit Carapace `/compact` resumes the
...
over-budget session can stay over budget or go stale (Carapace no longer
rescues it):
...
CLI backends do not receive Carapace tools by default. If the CLI can consume
...
"always-on"` so Carapace can fail closed when a caller requires no native
...
Put credentials in Carapace auth profiles or plugin-owned config. Ensure the
...
```bash
carapace plugins inspect acme-cli --runtime --json
carapace agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `carapace.extensions` and built runtime entries for published packages</Check>
<Check>`carapace.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
