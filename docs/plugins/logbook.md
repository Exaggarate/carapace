...
Carapace-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`carapace node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `carapace models auth login --provider openai`; see
...
```bash
carapace plugins enable codex
carapace plugins enable logbook
```
...
carapace plugins inspect logbook --runtime --json
...
1. `carapace plugins list --enabled` includes `logbook`.
