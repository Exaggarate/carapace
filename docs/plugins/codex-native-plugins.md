---
summary: "Configure native Codex plugins for Codex-mode Carapace agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Carapace agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are discovering or installing a Codex marketplace plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Carapace agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Carapace turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. Carapace does not translate
Codex plugins into synthetic `codex_plugin_*` Carapace dynamic tools.
...
- Migration supports only `openai-curated` plugins that it observed as
  ...
  Carapace treats both names as the one curated catalog, so configured
...
`codexPlugins` has no effect on Carapace-provider runs, ACP conversation
...
```bash
carapace migrate codex --dry-run --verify-plugin-apps
```
...
owner does not need to add that marketplace to Carapace configuration before
...
allowlisted, those requirements still apply; Carapace does not bypass them.
...
Carapace install command afterward to authorize the already-installed plugin.
Carapace keeps apps hidden when the response omits the exact marketplace,
...
When an authenticated owner creates an automation from a Codex turn, Carapace
...
Jobs created before app authority capture may keep their ordinary Carapace
...
the `operator.admin` scope. Carapace's reserved `/codex` command is dispatched
...
again. `enable` and `disable` change Carapace's persisted policy; qualified
...
| Enabled    | Codex reports the plugin enabled, and Carapace config allows it for Codex harness turns.                                           |
...
- During planning, Carapace reads source Codex `plugin/read` details and
...
For explicitly approved plugins from any discovered marketplace, Carapace uses
...
- Carapace does not add new Git or local marketplace sources in this flow.
  Additional sources must already be configured in Codex or be discoverable
  from the bound repository.

## App inventory and ownership

Carapace first reads and caches one `plugin/installed` snapshot scoped to the
...
Carapace reads installed app runtime state through `app/installed` and fetches
...
configured curated plugins are installed, Carapace combines their cache
invalidations into a single app-inventory refresh. Ordinary cached reads do
not force a connector refresh for every new thread. Carapace caches the
...
A plugin app is exposed only when Carapace can map it back to the configured
...
and callable for that thread. Carapace does not install, authenticate, or enable
...
names, Carapace uses its `plugin/installed` snapshot and reads only that exact
...
Carapace injects a restrictive `config.apps` patch for the Codex thread:
...
snapshot while `_default` is disabled. Carapace provisionally admits only
...
non-callable, Carapace logs one warning listing the unavailable apps and
...
or used. Carapace deletes a failed persistent provisional thread, unsubscribes
...
get `open_world_enabled: true`. Carapace does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: Carapace returns a deterministic decline.
- `true`: Carapace auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: Carapace exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into Carapace plugin
  approvals before returning the Codex approval response.
- `"ask"`: Carapace uses the same Codex write/destructive gating as
  ...
  For each admitted app using `"ask"`, Carapace selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  Carapace; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in Carapace.                                                      |
...
| `plugin_detail_unavailable`                       | Carapace could not read the exact configured plugin's ownership details.                                                             | Inspect the target app-server's `plugin/installed` and `plugin/read` responses.                                        |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | Carapace schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account-wide default can become callable after Carapace starts and verifies
...
`/reset`. Carapace does not authenticate plugin apps on the owner's behalf.

For `plugin_detail_unavailable`, verify that the exact installed marketplace
and plugin identity select a matching `plugin/read` result. Carapace keeps
...
Codex thread bindings keep the app config they started with until Carapace
