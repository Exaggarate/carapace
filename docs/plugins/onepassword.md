...
read_when:
  ...
  - You want Carapace config credentials to resolve from 1Password
  - You need per-secret approval policy and audit history
  - You are configuring a 1Password service account for Carapace
...
- Service-account authentication only. The token stays in a local credentials
  file and is never accepted in `carapace.json`.
- Curated agent registry only. Agents can list configured slugs, but the plugin
  ...
  explicitly stored on registered Carapace credential targets.
- Per-slug `auto`, `approve`, or `deny` policy.
- Approval grants expire. A cached value never bypasses current policy.
- Every access attempt is recorded in Carapace's shared SQLite state. Audit
...
- After the current tool execution, Carapace-owned transcript persistence
...
Create the token directory and file under the Carapace state directory:
...
When `CARAPACE_STATE_DIR` is set, replace `~/.carapace` with that directory.
...
Carapace's shared exec-id grammar in a plugin-local opaque form and decodes them
...
```bash
carapace onepassword secretref status --json
```

## Configure registered secrets

Add plugin config to `carapace.json`:
...
approval. Carapace offers allow always only when the caller has a concrete agent
...
```bash
carapace onepassword audit
carapace onepassword audit --limit 100
```
