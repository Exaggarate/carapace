...
Carapace Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
1. Open **Plugins** in the Control UI, or use `/settings/plugins` relative to
   the configured Control UI base path. For example, a base path of `/carapace`
   uses `/carapace/settings/plugins`.
2. Find **Workboard** and choose **Enable**. Because Workboard is included with
   Carapace, it does not need an **Install** action.
...
```bash
carapace plugins disable workboard
carapace gateway restart
```
...
that Gateway's Carapace state (see [Storage](#storage)).
...
Carapace subagent sessions still own execution. One dispatch pass:
...
Gateway (`CARAPACE_GATEWAY_URL` or `gateway.mode: remote`) apply, the CLI runs
...
`defaultAssignee`, and `orchestratorProfile`. Carapace records this intent and
...
carapace workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
carapace workboard show <card-id> [--json]
carapace workboard move <card-id> --status <status> [--json]
carapace workboard dispatch [--board <id>] [--json]
```
...
under the Carapace state directory: boards, cards, labels, lifecycle events,
...
`carapace doctor --fix` to migrate the shipped legacy plugin-state namespaces
