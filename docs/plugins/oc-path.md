---
summary: "Bundled `oc-path` plugin: ships the `carapace path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`carapace path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Carapace repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `carapace path … --json` instead of carrying separate markdown, JSONC,
...
carapace path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
carapace path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `carapace` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/carapace.plugin.json`:
...
lazily the first time you run `carapace path …`, so installs that never use
...
state. Bare `carapace path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
carapace plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `carapace path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__CARAPACE_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`carapace path` CLI reference](/cli/path)
