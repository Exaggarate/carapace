---
summary: "Install and use Agent Plugins, Codex, Claude, and Cursor bundles as Carapace plugins"
read_when:
  - You want to install an Agent Plugins, Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Carapace maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Carapace can install plugins from four external ecosystems: the vendor-neutral
[**Agent Plugins**](https://agent-plugins.org) standard, plus **Codex**,
**Claude**, and **Cursor**. These are called **bundles** - content and metadata
packs that Carapace maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Carapace plugins. Native plugins run
...
Cursor format. Instead of requiring authors to rewrite them as native Carapace
plugins, Carapace detects these formats and maps their supported content into
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    carapace plugins install ./my-bundle

    # Archive
    carapace plugins install ./my-bundle.tgz

    # Claude marketplace
    carapace plugins marketplace list <source>
    carapace plugins install <plugin> --marketplace <source>
    ```
...
## What Carapace maps from bundles

Not every bundle feature runs in Carapace today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Carapace skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | Carapace-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded Carapace settings; supported stdio and HTTP servers loaded | All formats    |
| Env contract  | `PLUGIN_ROOT` and `PLUGIN_DATA` env vars plus placeholder expansion for stdio MCP servers         | Agent Plugins  |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded Carapace LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded Carapace defaults                                     | Claude         |

#### Skill content

- Bundle skill roots load as normal Carapace skill roots.
- Claude `commands/` roots are treated as additional skill roots.
- Cursor `.cursor/commands/` roots are treated as additional skill roots.

Claude markdown command files and Cursor command markdown both work through the
normal Carapace skill loader.

#### Hook packs

Bundle hook roots work **only** when they use the normal Carapace hook-pack
layout: `HOOK.md` plus `handler.ts` or `handler.js`. Today this is primarily
the Codex-compatible case.

#### MCP for embedded Carapace

- Enabled bundles can contribute MCP server config.
- Carapace merges bundle MCP config into the effective embedded Carapace
  settings as `mcpServers`.
- Carapace exposes supported bundle MCP tools during embedded Carapace agent
...
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Carapace config. `carapace mcp set` and `carapace doctor --fix` normalize the common alias.
...
Carapace registers bundle MCP tools with provider-safe names in the form
...
#### Embedded Carapace settings

Claude `settings.json` is imported as default embedded Carapace settings when
the bundle is enabled. Carapace sanitizes shell override keys before applying
them:

- `shellPath`
- `shellCommandPrefix`

#### Embedded Carapace LSP

- Enabled Claude bundles can contribute LSP server config.
- Carapace loads `.lsp.json` plus any manifest-declared `lspServers` paths.
- Bundle LSP config is merged into the effective embedded Carapace LSP
  defaults.
- Only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `carapace plugins inspect <id>`.

### Detected but not executed

These are recognized and shown in diagnostics, but Carapace does not run them:
...
<AccordionGroup>
  <Accordion title="Agent Plugins bundles">
    ...
    - The manifest is strict JSON (not JSON5). Carapace requires a non-empty
    ...
    - stdio servers launch with `PLUGIN_ROOT` (the plugin root) and
      `PLUGIN_DATA` (a persistent per-plugin data directory Carapace creates
    ...
    - Carapace reads `extensions["ai.carapace"]`; it currently supports
  ...
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Carapace best when they use skill roots and Carapace-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded Carapace settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded Carapace
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded Carapace LSP defaults
...
Carapace checks for native plugin format first:

1. `carapace.plugin.json` or a valid `package.json` with `carapace.extensions` - treated as a **native plugin**
...
bundle markers, Carapace uses the native path. This prevents dual-format
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `carapace plugins install` and ship everything
  they need in the installed plugin directory.
- Carapace-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `carapace doctor --fix` removes stale local bundled-plugin install records
...
- Carapace does **not** load arbitrary bundle runtime modules in-process.
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `carapace plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Carapace settings from `settings.json` are supported. Carapace does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Carapace hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
