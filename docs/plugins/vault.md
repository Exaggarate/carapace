...
read_when:
  - You want Carapace to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets Carapace resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. Carapace stores Vault
references in config, keeps resolved values in the in-memory secrets snapshot,
and does not write the resolved API keys back to `carapace.json`.

Use this when you already run Vault or want model provider keys to live outside
Carapace config files. For the SecretRef runtime model, see
...
Enable the bundled plugin before running the `carapace vault` commands:

```bash
carapace plugins enable vault
```

## Store a provider key in Vault

Carapace defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `CARAPACE_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the Carapace defaults, this
SecretRef id:
...
Use a scoped client token for Carapace, not a root token. For the default KV v2
...
that starts Carapace. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
```bash
export VAULT_ADDR=https://vault.example.com
export CARAPACE_VAULT_AUTH_METHOD=token_file
...
These variables must be present when Carapace starts. The Vault plugin forwards
...
```bash
export VAULT_ADDR=https://vault.example.com
export CARAPACE_VAULT_AUTH_METHOD=jwt
export CARAPACE_VAULT_AUTH_MOUNT=jwt
export CARAPACE_VAULT_AUTH_ROLE=carapace
export CARAPACE_VAULT_JWT_FILE=/var/run/secrets/tokens/vault
```
...
```bash
export VAULT_ADDR=https://vault.example.com
export CARAPACE_VAULT_AUTH_METHOD=kubernetes
export CARAPACE_VAULT_AUTH_ROLE=carapace
```

Set `CARAPACE_VAULT_AUTH_MOUNT` only when Vault mounted Kubernetes auth somewhere
other than `auth/kubernetes`. Set `CARAPACE_VAULT_JWT_FILE` only when the service
...
export CARAPACE_VAULT_KV_MOUNT=secret
export CARAPACE_VAULT_KV_VERSION=2
```

Check what the current shell can see:

```bash
carapace vault status
```
...
```bash
carapace vault status --provider-alias corp-vault
```

`carapace vault status` never prints `VAULT_TOKEN`; it reports only whether the
...
```bash
carapace secrets apply --from ./vault-secrets-plan.json --dry-run --allow-exec
carapace secrets apply --from ./vault-secrets-plan.json --allow-exec
carapace secrets audit --check --allow-exec
carapace secrets reload
```

Use `--allow-exec` because the Vault plugin resolves through an Carapace-managed
exec SecretRef provider.

If the Gateway is not running yet, start it normally after applying the plan
instead of running `carapace secrets reload`.
...
```bash
carapace vault setup --openai-id providers/openai/apiKey
carapace vault setup --anthropic-id providers/anthropic/apiKey
carapace vault setup --openrouter-id providers/openrouter/apiKey
```
...
Bare target paths apply to `carapace.json`. Use
`auth-profiles:<agentId>:<path>` for existing SQLite auth-profile targets.
The target path must be a registered Carapace SecretRef target. The setup
command does not create arbitrary named secrets in Carapace; Vault remains the
secret store, and Carapace stores SecretRefs only on supported config fields.
...
```bash
export CARAPACE_VAULT_KV_VERSION=1
```
...
- one auth source:
  - `VAULT_TOKEN`
  - `CARAPACE_VAULT_AUTH_METHOD=token_file` plus `VAULT_TOKEN_FILE`
  - `CARAPACE_VAULT_AUTH_METHOD=jwt` plus `CARAPACE_VAULT_AUTH_MOUNT`,
    `CARAPACE_VAULT_AUTH_ROLE`, and `CARAPACE_VAULT_JWT_FILE`
  - `CARAPACE_VAULT_AUTH_METHOD=kubernetes` plus `CARAPACE_VAULT_AUTH_ROLE`; optionally
    override `CARAPACE_VAULT_AUTH_MOUNT` or `CARAPACE_VAULT_JWT_FILE`
- optional `VAULT_NAMESPACE`, `CARAPACE_VAULT_KV_MOUNT`, and
  `CARAPACE_VAULT_KV_VERSION`

When using Kubernetes, prefer `CARAPACE_VAULT_AUTH_METHOD=kubernetes`
when Vault has Kubernetes auth configured for the cluster. Use
`CARAPACE_VAULT_AUTH_METHOD=jwt` only when Vault is configured to treat the cluster
...
deployment config. Carapace does not require a fixed mount, role, or path: each
Gateway environment can set its own `CARAPACE_VAULT_KV_MOUNT`,
`CARAPACE_VAULT_AUTH_ROLE`, and SecretRef ids. If one shared Gateway must resolve
