---
summary: "How Carapace installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Carapace installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Carapace handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the Carapace package directory.
...
- SDK/core imports are peer or supplied Carapace imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into Carapace-owned package roots.

Carapace owns only the plugin lifecycle:
...
Carapace uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.carapace/npm/projects/<encoded-package>`.
- git packages clone under `~/.carapace/git`.
...
```bash
cd ~/.carapace/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`carapace plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: Carapace reads the tarball's npm
...
`~/.carapace/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. Carapace scans the managed project
...
Carapace-owned npm plugin packages never ship npm lockfiles. The repository
...
lockfiles according to their own packaging policy; Carapace's installer leaves
...
Carapace-owned npm plugin packages can also publish with explicit
...
`carapace.release.bundleRuntimeDependencies: false`; they still ship a
...
`carapace` package also resolves dependencies at install time and does not
bundle its full dependency tree. See
[dependency locking](/gateway/security/dependency-locking).

Plugins that import `carapace/plugin-sdk/*` declare `carapace` as a peer
dependency. Carapace does not let npm install a separate registry copy of the
...
resolution/materialization, and Carapace reasserts plugin-local
`node_modules/carapace` links for installed packages that declare the host
...
Local plugins are developer-controlled directories. Carapace never runs
...
carapace plugins install <source>
carapace doctor --fix
```
...
Lightweight and core-critical bundled plugins ship as part of Carapace. They
...
be declared in the root Carapace package's `dependencies` or
...
In source checkouts, use `pnpm install` followed by `pnpm build`. Carapace
...
| ----------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------ |
| Global npm install                              | Built runtime tree inside the package                | Root Carapace package for internal bundled runtime     |
...
| `carapace plugins install ...`                  | Managed npm project/git/ClawHub root                 | The plugin install/update flow                         |

For the global npm row, use
`npm install -g carapace --allow-scripts=carapace` on npm 12 or npm 11.16+.
On npm 11.15 and earlier, omit `--allow-scripts=carapace`. Plugin dependency
...
It only links the checkout as `node_modules/carapace` for a real immediate
source package that declares `carapace` in `peerDependencies` or `dependencies`.
...
Older Carapace versions generated bundled-plugin dependency roots at startup
...
`.carapace-install-stage*` directories under `dist/extensions`, `dist` files
...
Older npm installs also used a shared `~/.carapace/npm/node_modules` root.
