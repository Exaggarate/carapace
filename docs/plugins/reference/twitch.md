---
summary: "Carapace Twitch channel plugin for chat and moderation workflows."
read_when:
  - You are installing, configuring, or auditing the twitch plugin
title: "Twitch plugin reference"
---

<!-- Generated file. Do not edit by hand.
Run `pnpm plugins:inventory:gen` to rebuild it. Hand-written text survives only
between the carapace-plugin-reference:manual-start and
carapace-plugin-reference:manual-end comment markers. -->

Carapace Twitch channel plugin for chat and moderation workflows.

## Distribution

- Package: `@carapace/twitch`
- Install route: npm or ClawHub

## Surface

- Channels: `twitch`

## Related docs

- [twitch](/channels/twitch)
