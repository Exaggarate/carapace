...
- Package: `@carapace/policy`
- Install route: included in Carapace

## Surface

- CLI commands: `carapace policy`

<!-- carapace-plugin-reference:manual-start -->

## Behavior

The Policy plugin contributes doctor health checks for policy-managed Carapace
...
posture, ingress/channel access posture, data-handling posture, and Carapace config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
Carapace settings and workspace declarations as evidence, and reports drift
through `carapace policy check` and `carapace doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`carapace policy check`, `watch`, and workspace-relative `compare` accept
`--agent <id>`. Explicit multi-agent fleets must select the workspace owner;
the plugin does not infer one from roster order.

`carapace policy compare --baseline <file>` compares one policy file to another
