---
summary: "Adds Microsoft Foundry model provider support to Carapace."
read_when:
...
Adds Microsoft Foundry model provider support to Carapace.

## Distribution

- Package: `@carapace/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Carapace refreshes Microsoft Foundry runtime tokens through
...
Carapace uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Carapace
...
deployment through onboarding or include model metadata so Carapace can verify
