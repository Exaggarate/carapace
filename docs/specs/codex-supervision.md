...
summary: "Architecture and product boundary for supervising native Codex sessions from Carapace."
read_when:
...
Codex supervision lets an Carapace operator discover native Codex sessions,
create a Gateway-local branch, or continue an eligible paired-node thread
through the normal Carapace Chat surface.
Codex App Server remains the thread and model-loop owner. Carapace supplies the
...
- native thread bindings to Carapace sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
Carapace Codex plugin -> supervision App Server connection
...
Ordinary Carapace Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> Carapace Chat and channel delivery
```
...
process-local; a thread unknown to Carapace's supervision process is `notLoaded`
...
unbounded `thread/read`. Carapace also rejects any serialized item page above
...
eventually discovered without importing them into Carapace storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the Carapace client to live thread requests, or
answer an approval.
...
```text
carapace codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [gateway-options]
carapace codex continue <thread-id> [--agent <id>] [--host <id>] [--json] [gateway-options]
carapace codex archive <thread-id> --confirm-no-other-runner [--agent <id>] [--host <id>] [--json] [gateway-options]
```
...
The notice supersedes only earlier Carapace generic policy. Independent managed,
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   Carapace Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the Carapace `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, Carapace's cwd, policy, config, environment, the
   full Carapace harness tool surface, and exactly the model and provider
...
omit Carapace model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, Carapace accepts
that native persisted selection. The outer Carapace model and fallback chain
...
Deleting or resetting the Carapace entry would otherwise discard the native
...
canonical harness thread on the first turn prevents Carapace from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and Carapace catalogs.
...
known active Carapace binding owner for the exact target or any non-archived
spawned descendant also rejects archive. Carapace paginates Codex's experimental
...
the owner/admin check and are blocked while Carapace sandboxing is active.

The node runner rejects overlapping Carapace resume turns for the same thread
...
`carapace doctor --fix` migrates shipped `plugins.entries.codex-supervisor`
...
- audited handoff between Codex Desktop, CLI, and Carapace
...
- Later resumes omit Carapace model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer Carapace model or fallback chain.
