...
audio and video players in Carapace chat clients, see
[Media playback](/nodes/media-playback).

## What it does

When audio understanding is enabled (or auto-detected), Carapace:
...
If you have not configured models and `tools.media.audio.enabled` is not `false`, Carapace auto-detects in this order and stops at the first working option:
...
3. **Local CLIs** (only if no provider auth resolved). Carapace builds an ordered fallback list:
...
Install/link provenance is capability evidence, not execution evidence. It never moves a candidate ahead of CPU sherpa by itself. Carapace does not load a model during setup or status checks just to probe a backend.
Auto-detected whisper.cpp keeps its normal model-run logs enabled so Carapace can record the upstream `using … backend` line. Explicit CLI entries keep their configured output flags.
...
```bash
carapace capability audio providers
...
1. List the agent's OpenAI profiles so you can copy the exact OAuth profile ID:
   ...
   ```bash
   carapace models auth list --agent AGENT_NAME_HERE --provider openai
   ```
...
2. Create a dedicated API-key profile. This command prompts for the key; paste it
   ...
   ```bash
   carapace models auth paste-api-key --agent AGENT_NAME_HERE --provider openai --profile-id openai:CUSTOM_PROFILE_NAME_HERE
   ```

   Example:

   ```bash
   carapace models auth paste-api-key --agent smith --provider openai --profile-id openai:audio
   ```
...
3. Put the OAuth profile first and the audio API-key profile second in the agent's
   ...
   ```bash
   carapace models auth order set --agent AGENT_NAME_HERE --provider openai openai:YOUR_OPENAI_ACCOUNT_EMAIL_ADDRESS openai:CUSTOM_PROFILE_NAME_HERE
   ```

   Example:

   ```bash
   carapace models auth order set --agent smith --provider openai openai:youremailaddress@email.com openai:audio
   ```
...
The `profile` field is not required when Carapace can unambiguously select a
...
- CLI `args` should use `{{AttachmentPath}}` for the local audio file path. Run `carapace doctor --fix` to migrate deprecated `{input}` placeholders from older `audio.transcription.command` configs (retired key: `audio.transcription`, replaced by `tools.media.models`). `{{MediaPath}}` remains a deprecated compatibility alias.
- `tools.media.concurrency` bounds media tasks; it is not a GPU scheduler.

### Resident local STT

Auto-detected local STT remains process-per-request. Carapace does not currently manage a resident whisper.cpp server because the standard Homebrew `whisper-cpp` package disables that server, while the upstream example has no configured bounded admission queue. A plugin-owned resident lifecycle needs a maintained packaged worker with health/startup, model residency, bounded queueing, cancellation/timeout, loopback-only no-auth operation, and no cloud fallback before it can be enabled safely.
...
Lowercase variables take precedence over uppercase; `NO_PROXY`/`no_proxy` entries (hostnames, `*.suffix`, or `host:port`) bypass the proxy. If no proxy env vars are set, direct egress is used. If proxy setup fails (malformed URL), Carapace logs a warning and falls back to direct fetch.

## Mention detection in groups

On channels that support audio preflight, Carapace transcribes audio **before** checking for mentions when `requireMention: true` is set for a group chat. This lets a captionless voice note pass the mention gate when its transcript contains a configured mention pattern. Channel-specific docs describe transports that require a typed mention instead.

**How it works:**

1. If a voice message has no text body and the group requires mentions, Carapace performs a preflight transcription of the first audio attachment.
