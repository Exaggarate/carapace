...
Carapace supports camera capture for agent workflows on paired **iOS**, **Android**, **macOS**, and **Linux** nodes: capture a photo (`jpg`) or a short video clip (`mp4`, with optional audio) via Gateway `node.invoke`.
...
```bash
carapace nodes camera snap --node <id>                 # default: one node-selected photo
carapace nodes camera snap --node <id> --facing front
carapace nodes camera snap --node <id> --facing both   # front then back (2 saved paths)
carapace nodes camera clip --node <id> --duration 3000
carapace nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`carapace.cameraEnabled`).
  - Default: **off**.
  - When off: camera requests return `CAMERA_DISABLED: enable Camera in Settings`.

### CLI helper (node invoke)

Use the main `carapace` CLI to invoke camera commands on the macOS node.

```bash
carapace nodes camera list --node <id>                     # list camera ids
carapace nodes camera snap --node <id>                     # prints saved path
carapace nodes camera snap --node <id> --max-width 1280
carapace nodes camera snap --node <id> --delay-ms 2000
carapace nodes camera snap --node <id> --device-id <id>
carapace nodes camera clip --node <id> --duration 10s       # prints saved path
carapace nodes camera clip --node <id> --duration-ms 3000   # prints saved path (legacy flag)
carapace nodes camera clip --node <id> --device-id <id>
carapace nodes camera clip --node <id> --no-audio
```

- `carapace nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
If a macOS external camera starts a photo session in portrait, Carapace selects an advertised landscape format with transposed dimensions and the same encoding, when one exists. Already-landscape, built-in, and Continuity Camera formats are unchanged. Without an exact counterpart, or if the camera cannot be reconfigured, capture keeps the negotiated format. `--max-width` still only limits the returned JPEG width; it does not choose a camera format.
...
Always pass an explicit `deviceId` returned by `camera.list`. Carapace never chooses a default camera for physical movement.
...
The allow entry alone does not widen an existing node approval. After the updated Mac reconnects and declares PTZ control, run `carapace nodes pending`, then approve the widened surface with `carapace nodes approve <requestId>`.
...
The bundled Linux Node plugin adds camera capture to the CLI `carapace node` service. It works on a headless host and does not require the Linux desktop app.
...
```bash
carapace nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
