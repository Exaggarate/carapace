...
read_when:
  - You want Carapace to identify the active Mac
...
the most recent physical mouse or keyboard input. Carapace uses that signal to
...
- The Carapace macOS app is paired and connected in node mode.
- **Settings -> Permissions -> Active computer detection** is enabled. It is off by default.
- **Accessibility** permission is granted to the signed Carapace app.
...
3. Move the mouse or press a key on that Mac, then run:
   ...
   carapace nodes describe --node <node-id-or-name>
   ```
...
used for UI automation. Carapace sends idle duration, not input content. It does not send key values,
...
Carapace waits 750 milliseconds so the connecting Mac can submit its first
...
- If no active Mac is available or primary delivery fails, Carapace waits five
...
| Symptom                                   | Check                                                                                                                                                                |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No row is marked `active`                 | Confirm active computer detection is enabled, a native macOS node is connected, and `carapace nodes describe --node <id>` shows `permissions.accessibility: true`.   |
| The wrong Mac remains active              | Use that Mac physically, wait for the coalescing window, then rerun `carapace nodes status`. Synthetic computer-control actions do not count.                        |
