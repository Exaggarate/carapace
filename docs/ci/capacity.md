...
Carapace's current GitHub runner-registration bucket reports 10,000 self-hosted
...
`carapace` organization, so adding another Blacksmith installation does not add
a new bucket.
...
Eligible `control-ui` rows request `blacksmith-32vcpu-ubuntu-2404`; the browser-extension row keeps the 8-vCPU request and the real-Gateway job keeps 16. Backend, event, contributor-trust and cache-write boundaries are unchanged, including hybrid first attempts and trusted contributor forks. In [run 33692146223](https://github.com/Exaggarate/carapace/actions/runs/33692146223), the two slowest UI rows requested the 8-vCPU label but reported two CPUs; their 356/383-second test steps set the 8:20 non-Windows wall. The same run's 32-vCPU jobs reported eight CPUs. The larger request added no workers. In [run 33695337496](https://github.com/Exaggarate/carapace/actions/runs/33695337496), all twelve UI rows reported eight CPUs and finished by 4:38 from workflow creation, with 102–145-second test steps. That margin supports consolidating to six rows; reduced-row timings still require native proof. Stale file weights also need the existing refit's independent-run and replacement thresholds, rather than a one-run manual adjustment.
...
invalid timing files, or `CARAPACE_CI_TEST_TIMINGS=0`, use the cold-start estimates
...
runs using `CARAPACE_CI_RUNNER_BACKEND=github`, can fill it naturally. Once recorded,
