...
read_when:
  ...
  - Cannot access github.com/Exaggarate/carapace, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="Recommended way to install and set up Carapace">
    ```bash
    ...
    command? Run `carapace onboard --install-daemon` instead of the two
    ...
    No global install yet? Run `pnpm carapace onboard` instead. If Control UI assets are
...
<a id="i-am-stuck" />
  ...
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    **local config or environment issues** a remote helper cannot inspect, so this beats
    ...
    | Command | Shows |
    | --- | --- |
    | `carapace status` | Gateway/agent health + basic config snapshot |
    | `carapace status --all` | Full read-only diagnosis, pasteable |
    | `carapace models status` | Provider auth + model availability |
    | `carapace doctor` | Validates and repairs common config/state issues |
    | `carapace logs --follow` | Live log tail |
    | `carapace gateway status --deep` | Deep gateway/config/plugin health check |
    | `carapace health --verbose` | Detailed health report |

    Found a real bug or fix? File an issue or send a PR:
    [Issues](https://github.com/Exaggarate/carapace/issues) /
    [Pull requests](https://github.com/Exaggarate/carapace/pulls).
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `CARAPACE_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `CARAPACE_GATEWAY_PASSWORD`).
    - No shared secret configured yet? Run `carapace doctor --generate-gateway-token` (or `carapace doctor --fix --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `carapace gateway --tailscale serve`, open `https://<magicdns>/`. With `gateway.auth.allowTailscale: true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes a trusted gateway host); HTTP APIs still need shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
  ...
  <Accordion title="Heartbeat keeps skipping. What do the skip reasons mean?">
    ...
    Older heartbeat `tasks:` blocks migrate to independently scheduled cron jobs with `carapace doctor --fix`.
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    TUI without sending anything — add a provider by running `carapace onboard` again.
  ...
  <Accordion title="Can I migrate my setup to a new machine without redoing onboarding?">
    ...
    1. Install Carapace on the new machine.
    2. Copy `$CARAPACE_STATE_DIR` (default: `~/.carapace`) from the old machine.
    3. Copy your workspace (default: `~/.carapace/workspace`).
    4. Run `carapace doctor` and restart the Gateway service.
    ...
    **memory + bootstrap files**, but not session history or auth. Those live under
    `~/.carapace/` (for example `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/Exaggarate/carapace/blob/main/CHANGELOG.md](https://github.com/Exaggarate/carapace/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access github.com/Exaggarate/carapace (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `github.com/Exaggarate/carapace` via Xfinity
    Advanced Security. Disable it or allowlist `github.com/Exaggarate/carapace`, then retry. Help us
    ...
    [https://github.com/Exaggarate/carapace/tree/main/docs](https://github.com/Exaggarate/carapace/tree/main/docs)
  ...
  <Accordion title="Difference between stable and beta">
    ...
    **same version** after promotion.
    ...
    See what changed: [CHANGELOG.md](https://github.com/Exaggarate/carapace/blob/main/CHANGELOG.md).
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published to npm it uses dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --beta
    ```
    ...
    Windows installer (PowerShell): `iwr -useb https://github.com/Exaggarate/carapace | iex`
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --beta --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or carapace not recognized">
    ...
    **2) carapace is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    Still reproducing this on latest Carapace? Track/report it: [Issue #30640](https://github.com/Exaggarate/carapace/issues/30640).
  ...
  <Accordion title="How do I install Carapace on Linux?">
  ...
  <Accordion title="How do I install Carapace on a VPS?">
  ...
  <Accordion title="Can I ask Carapace to update itself?">
    ...
    carapace update --channel stable|extended-stable|beta|dev
    carapace update --tag <dist-tag|version>
    ...
    ```bash
    carapace update --yes --no-restart
    carapace gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `carapace onboard` is the recommended setup path. On a fresh local install it
    ...
    `carapace onboard --classic` for its Workspace, Model/Auth, Gateway,
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. Run Carapace with **API keys** (Anthropic/OpenAI/others) or **local-only models**
    ...
    Carapace also supports hosted subscription-style options including **Qwen Cloud
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    Yes. Carapace supports Claude CLI reuse for Pro/Max/Team/Enterprise plans. Anthropic
    currently treats the `claude -p` path Carapace uses as subscription-plan usage subject
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic setup-token auth is also still a supported token path, but Carapace prefers
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Set a **fallback model** so Carapace keeps replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. Carapace has a bundled **Amazon Bedrock (Converse)** provider. With AWS env
    markers present (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, `AWS_BEARER_TOKEN_BEDROCK`),
    Carapace auto-enables the implicit Bedrock provider for model discovery; otherwise
  ...
  <Accordion title="How does Codex auth work?">
    Carapace supports **OpenAI Codex** via OAuth (ChatGPT sign-in). A fresh
    ...
    `openai/gpt-5.5` explicitly; Carapace does not silently downgrade. Legacy
    Codex-prefixed model refs are legacy config repaired by `carapace doctor
  ...
  <Accordion title="Why does Carapace still mention legacy OpenAI Codex prefix?">
    ...
    - Legacy `openai-codex/*` model refs = legacy route repaired by `carapace doctor --fix`.
    - `openai/gpt-5.5` plus an ordered `openai` API-key profile = API-key auth for an OpenAI agent model.
    - Legacy `openai-codex` auth profile ids = legacy ids migrated by `carapace doctor --fix`.

    Want direct OpenAI Platform billing? Set `OPENAI_API_KEY`. Want ChatGPT/Codex
    subscription auth? Run `carapace models auth login --provider openai`. Keep
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    `carapace models status` shows the currently visible provider usage/quota windows, but
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    tools/workflows like Carapace. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="Can I use Gemini CLI or Antigravity OAuth?">
    ...
    Gemini CLI OAuth profiles remain executable for compatibility, but Carapace
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Carapace needs large context + strong safety; small cards truncate context
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Carapace runs on macOS or Linux (Windows via WSL2). A Mac mini is a popular
  ...
  <Accordion title="If I buy a Mac mini to run Carapace, can I connect it to my MacBook Pro?">
    ...
    node host and pairs to the Gateway. Check with `carapace nodes status` / `carapace nodes list`.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    not the bot username. Setup asks for numeric user IDs only; `carapace doctor --fix`
    can try to resolve legacy `@username` entries.

    Safer (no third-party bot): DM your bot, run `carapace logs --follow`, read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Carapace instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    Running Carapace via systemd: make sure the service PATH includes
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes, with `carapace update --channel ...` on an existing install. This does **not
    delete your data** - only the Carapace code install changes. State (`~/.carapace`) and
    workspace (`~/.carapace/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --install-method git
    curl -fsSL --proto '=https' --tlsv1.2 https://github.com/Exaggarate/carapace | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="How important is it to run Carapace on a dedicated machine?">
  ...
  <Accordion title="Can I run Carapace in a VM and what are the requirements?">
