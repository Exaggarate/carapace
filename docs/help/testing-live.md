...
- Bring your own gateway: use the in-process gateway (Layer 2 below) or start a
  dev instance with an isolated state dir (`CARAPACE_STATE_DIR=<scratch>`) and a
  free port. Do not bind the default gateway port (18789) while a real gateway
  is running on it.
- Do not `carapace gateway stop`/`restart` (or `launchctl`/`systemctl`/tmux
...
```bash
pnpm carapace infer tts convert --local --json \
  --text "Carapace live smoke." \
  --output /tmp/carapace-live-smoke.mp3
```

Safe voice-call readiness smoke:

```bash
pnpm carapace voicecall setup --json
pnpm carapace voicecall smoke --to "+15555550123"
```
...
- Optional target overrides:
  ...
  - `CARAPACE_ANDROID_GATEWAY_URL` / `CARAPACE_ANDROID_GATEWAY_TOKEN` / `CARAPACE_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `CARAPACE_LIVE_TEST=1` if invoking Vitest directly)
  - Set `CARAPACE_LIVE_MODELS=modern`, `small`, or `all` (alias for `modern`) to actually run this suite; otherwise it skips, so `pnpm test:live` on its own stays focused on gateway smoke.
- How to select models:
  - `CARAPACE_LIVE_MODELS=modern` runs the curated high-signal priority list (see [Live: model matrix](#live-model-matrix-what-we-cover))
  - `CARAPACE_LIVE_MODELS=small` runs the curated small-model priority list
  - `CARAPACE_LIVE_MODELS=all` is an alias for `modern`
  - or `CARAPACE_LIVE_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Local Ollama small-model runs default to `http://127.0.0.1:11434`; set `CARAPACE_LIVE_OLLAMA_BASE_URL` only for LAN, custom, or Ollama Cloud endpoints.
  - Modern/all and small sweeps default to their curated-list length as a cap; set `CARAPACE_LIVE_MAX_MODELS=0` for an exhaustive selected-profile sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `CARAPACE_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `CARAPACE_LIVE_MODEL_CONCURRENCY` to override.
- How to select providers:
  - `CARAPACE_LIVE_PROVIDERS="google,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `CARAPACE_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates "provider API is broken / key is invalid" from "gateway agent pipeline is broken"
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@carapace" actually does)
...
- How to select models:
  ...
  - `CARAPACE_LIVE_GATEWAY_MODELS=small` runs the curated small-model list through the full gateway+agent pipeline
  - `CARAPACE_LIVE_GATEWAY_MODELS=all` is an alias for `modern`
  - Or set `CARAPACE_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all and small gateway sweeps default to their curated-list length as a cap; set `CARAPACE_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive selected sweep or a positive number for a smaller cap.
- How to select providers (avoid "OpenRouter everything"):
  - `CARAPACE_LIVE_GATEWAY_PROVIDERS="google,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `CARAPACE_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6"`
  - `CARAPACE_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `CARAPACE_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json"]'`
  - `CARAPACE_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Off by default in Docker recipes.
  - `CARAPACE_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `CARAPACE_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `CARAPACE_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `CARAPACE_LIVE_CLI_BACKEND_CACHE_PROBE=1` to run a fresh Claude CLI turn, a tool-bearing warmup resume, and a no-tool settlement resume before requiring at least 90% prompt-cache reuse on the following dirty-workspace resume. It also verifies that a thinking-level change rotates the live-session generation and that the next steady resume restores at least 90% reuse. This probe disables the image, MCP, and model-switch probes.
  - `CARAPACE_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Off by default, including in Docker recipes.
  - `CARAPACE_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Off by default in Docker recipes.
...
settings Carapace gives Gemini, then runs `gemini --debug mcp list` to prove a
...
- It resolves CLI smoke metadata from the owning plugin, then installs the matching Linux CLI package (`@anthropic-ai/claude-code` or `@google/gemini-cli`) into a cached writable prefix at `CARAPACE_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/carapace/docker-cli-tools`).
...
- Enable:
  - `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_APNS_REACHABILITY=1 pnpm test:live src/infra/push-apns-http2.live.test.ts`
- Optional timeout:
  - `CARAPACE_LIVE_APNS_TIMEOUT_MS=30000`
...
- Overrides:
  ...
  - `CARAPACE_LIVE_ACP_BIND_AGENT=codex`
  - `CARAPACE_LIVE_ACP_BIND_AGENT=droid`
  - `CARAPACE_LIVE_ACP_BIND_AGENT=gemini`
  - `CARAPACE_LIVE_ACP_BIND_AGENT=opencode`
  - `CARAPACE_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `CARAPACE_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `CARAPACE_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.6-luna`
  - `CARAPACE_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `CARAPACE_LIVE_ACP_BIND_IMAGE_PROBE=1` (or `on`/`true`/`yes`) to force the image probe on; any other value forces it off. Runs by default for every agent except `opencode`.
  - `CARAPACE_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `CARAPACE_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.6-luna`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `CARAPACE_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `CARAPACE_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `CARAPACE_LIVE_ACP_BIND_AGENTS=claude`, `CARAPACE_LIVE_ACP_BIND_AGENTS=codex`, `CARAPACE_LIVE_ACP_BIND_AGENTS=droid`, `CARAPACE_LIVE_ACP_BIND_AGENTS=gemini`, or `CARAPACE_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- The OpenCode Docker variant is a strict single-agent regression lane. It writes a temporary `OPENCODE_CONFIG_CONTENT` default model from `CARAPACE_LIVE_ACP_BIND_OPENCODE_MODEL` (default `opencode/kimi-k2.6`).
- Direct `acpx` CLI calls are only a manual/workaround path for comparing behavior outside the Gateway. The Docker ACP bind smoke exercises Carapace's embedded `acpx` runtime backend.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - send a second turn to the same Carapace session and verify the app-server
    thread can resume
...
- Enable: `CARAPACE_LIVE_CODEX_HARNESS=1`
- Harness baseline model: `openai/gpt-5.6-luna`
- Fresh OpenAI API-key selection default: `openai/gpt-5.6-sol`
- Default thinking: `low`
- Model override: `CARAPACE_LIVE_CODEX_HARNESS_MODEL=openai/<model>`
- Thinking override: `CARAPACE_LIVE_CODEX_HARNESS_THINKING=<level>`
- Non-default model effort assertion:
  `CARAPACE_LIVE_CODEX_HARNESS_EXPECTED_EFFORT=<level>`
- Matrix override: `CARAPACE_LIVE_CODEX_HARNESS_TARGETS=<model>=<thinking>,...`
- Auth mode: `CARAPACE_LIVE_CODEX_HARNESS_AUTH=codex-auth` (default) uses the
  copied Codex login; `api-key` uses `OPENAI_API_KEY` through Codex app-server.
- Optional image probe: `CARAPACE_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `CARAPACE_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `CARAPACE_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
- Optional resume stress: `CARAPACE_LIVE_CODEX_HARNESS_RESUME_STRESS=1` adds
  ...
  `CARAPACE_LIVE_CODEX_HARNESS_RESUME_STRESS_HISTORY_TURNS` (1-20) and
  `CARAPACE_LIVE_CODEX_HARNESS_RESUME_STRESS_RESTARTS` (1-10).
- Optional fan-out stress: set `CARAPACE_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1`
  and `CARAPACE_LIVE_CODEX_HARNESS_SUBAGENT_COUNT` (1-12). The harness starts
  every child concurrently, waits for every terminal run, and verifies each
  unique child reply and native thread identity.
- Optional compaction stress: `CARAPACE_LIVE_CODEX_HARNESS_COMPACTION_STRESS=1`
  ...
  `CARAPACE_LIVE_CODEX_HARNESS_COMPACTION_STRESS_TURNS` (1-8) and
  `CARAPACE_LIVE_CODEX_HARNESS_LARGE_OUTPUT_BYTES` (100000-800000).
- Full direct-API context: `CARAPACE_LIVE_CODEX_HARNESS_FULL_CONTEXT=1` applies
  ...
  requires
  `CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key` plus an absolute
  `CARAPACE_LIVE_CODEX_HARNESS_MODEL_CATALOG` path. The catalog must expose the
...
- Optional loop-relay opt-out probe:
  `CARAPACE_LIVE_CODEX_HARNESS_DISABLE_LOOP_RELAY=1`
...
- The smoke forces provider/model `agentRuntime.id: "codex"` so a broken Codex
  harness cannot pass by silently falling back to Carapace.
- Auth: Codex app-server auth from the local Codex subscription login, or
  `OPENAI_API_KEY` when `CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key`. Docker can
...
CARAPACE_LIVE_CODEX_HARNESS=1 \
  CARAPACE_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  CARAPACE_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  CARAPACE_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
...
```bash
CARAPACE_LIVE_CODEX_HARNESS_RESUME_STRESS=1 \
pnpm test:docker:live-codex-harness
```
...
```bash
CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key \
  CARAPACE_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 \
  CARAPACE_LIVE_CODEX_HARNESS_SUBAGENT_COUNT=8 \
  CARAPACE_LIVE_CODEX_HARNESS_RESUME_STRESS=1 \
  CARAPACE_LIVE_CODEX_HARNESS_COMPACTION_STRESS=1 \
  pnpm test:docker:live-codex-harness
```
...
CARAPACE_LIVE_CODEX_HARNESS=1 \
  CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key \
  CARAPACE_LIVE_CODEX_HARNESS_FULL_CONTEXT=1 \
  CARAPACE_LIVE_CODEX_HARNESS_MODEL_CATALOG=/absolute/path/to/models-api-1m.json \
  CARAPACE_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.6-sol \
  CARAPACE_LIVE_CODEX_HARNESS_THINKING=low \
  CARAPACE_LIVE_CODEX_HARNESS_COMPACTION_STRESS_TURNS=8 \
  CARAPACE_LIVE_CODEX_HARNESS_LARGE_OUTPUT_BYTES=800000 \
  CARAPACE_LIVE_CODEX_HARNESS_DEBUG=1 \
...
```bash
CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key \
  CARAPACE_LIVE_CODEX_HARNESS_TARGETS='openai/gpt-5.6-sol=ultra,openai/gpt-5.6-terra=ultra,openai/gpt-5.6-luna=max' \
  pnpm test:docker:live-codex-harness
```

## Live: OpenAI long context

- Goal: validate exact-model embedded Carapace execution through a
...
- Enable: `CARAPACE_LIVE_OPENAI_LONG_CONTEXT=1`
- Profiles: `CARAPACE_LIVE_OPENAI_LONG_CONTEXT_PROFILE=full` selects exact
...
- Metrics: `CARAPACE_LIVE_OPENAI_LONG_CONTEXT_METRICS=1` emits phase timing and
  token observations. These measurements are informational, not pass/fail
  latency targets.
- Long output: `CARAPACE_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1` requires a
  deterministic response between 4000 and 8000 output tokens.
- Optional raw read-tool stress:
  `CARAPACE_LIVE_OPENAI_LONG_CONTEXT_TOOL_OUTPUT=1`. It is not part of the
...
```bash
CARAPACE_LIVE_OPENAI_LONG_CONTEXT=1 \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_PROFILE=full \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_METRICS=1 \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1 \
...
```bash
CARAPACE_LIVE_OPENAI_LONG_CONTEXT=1 \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_PROFILE=reduced \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_METRICS=1 \
  CARAPACE_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1 \
  node --import tsx scripts/test-live.mts --quiet src/gateway/gateway-openai-long-context.live.test.ts
```
...
- Runtime and model identity are exact: embedded Carapace or native Codex as
...
- Embedded Carapace receives and persists a first-class encrypted Responses
...
```bash
CARAPACE_LIVE_GATEWAY_OPENAI_API_DEFAULT=1 \
  CARAPACE_LIVE_GATEWAY_PROVIDERS=openai \
  CARAPACE_LIVE_GATEWAY_THINKING=off \
  pnpm test:live -- src/gateway/gateway-models.profiles.live.test.ts
```

This proof leaves `CARAPACE_LIVE_GATEWAY_MODELS` unset, resolves the model through
...
GPT-5.6 embedded Carapace matrix:

```bash
CARAPACE_LIVE_GATEWAY_THINKING=ultra \
  CARAPACE_LIVE_GATEWAY_PROVIDERS=openai \
  CARAPACE_LIVE_GATEWAY_MODELS='openai/gpt-5.6-sol,openai/gpt-5.6-terra,openai/gpt-5.6-luna' \
  pnpm test:live -- src/gateway/gateway-models.profiles.live.test.ts
```
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `CARAPACE_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `CARAPACE_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `CARAPACE_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
- Docker uses the same explicit Codex runtime config, so legacy aliases or Carapace
...
- Single model, direct (no gateway):
  - `CARAPACE_LIVE_MODELS="openai/gpt-5.6-luna" pnpm test:live src/agents/models.profiles.live.test.ts`

- Small-model direct profile:
  - `CARAPACE_LIVE_MODELS=small pnpm test:live src/agents/models.profiles.live.test.ts`

- Small-model gateway profile:
  - `CARAPACE_LIVE_GATEWAY_MODELS=small pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Ollama Cloud API smoke:
  - `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_OLLAMA=1 CARAPACE_LIVE_OLLAMA_BASE_URL=https://ollama.com CARAPACE_LIVE_OLLAMA_MODEL=glm-5.1:cloud CARAPACE_LIVE_OLLAMA_WEB_SEARCH=0 pnpm test:live -- extensions/ollama/ollama.live.test.ts`

- Single model, gateway smoke:
  - `CARAPACE_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `CARAPACE_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,google/gemini-3.5-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Z.AI Coding Plan GLM-5.3 direct smoke:
  - `ZAI_CODING_LIVE_TEST=1 pnpm test:live src/agents/zai.live.test.ts`

- Google focus:
  - Gemini (API key): `CARAPACE_LIVE_GATEWAY_MODELS="google/gemini-3.5-flash" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google adaptive thinking smoke (`qa manual` from the private QA CLI - requires `CARAPACE_ENABLE_PRIVATE_QA_CLI=1` and a source checkout; see [QA overview](/concepts/qa-e2e-automation)):
  - Gemini 3 dynamic default: `CARAPACE_ENABLE_PRIVATE_QA_CLI=1 pnpm carapace qa manual --provider-mode live-frontier --model google/gemini-3.1-pro-preview --alt-model google/gemini-3.1-pro-preview --message '/think adaptive Reply exactly: GEMINI_ADAPTIVE_OK' --timeout-ms 180000`
  - Gemini 2.5 dynamic budget: `CARAPACE_ENABLE_PRIVATE_QA_CLI=1 pnpm carapace qa manual --provider-mode live-frontier --model google/gemini-2.5-flash --alt-model google/gemini-2.5-flash --message '/think adaptive Reply exactly: GEMINI25_ADAPTIVE_OK' --timeout-ms 180000`
...
- Gemini API vs Gemini CLI:
  - API: Carapace calls Google's hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by "Gemini".
  - CLI: Carapace shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).

## Live: model matrix (what we cover)

Live is opt-in, so there is no fixed "CI model list." `CARAPACE_LIVE_MODELS=modern` / `CARAPACE_LIVE_GATEWAY_MODELS=modern` (and their `all` alias) run the curated priority list from `HIGH_SIGNAL_LIVE_MODEL_PRIORITY` in `src/agents/test-helpers/live-model-dynamic-candidates.ts`, in this priority order:
...
The curated **small-model** list (`CARAPACE_LIVE_MODELS=small` / `CARAPACE_LIVE_GATEWAY_MODELS=small`), from `SMALL_LIVE_MODEL_PRIORITY`:
...
- Include at least one image-capable model (Claude/Gemini/OpenAI-family vision variants, etc.) in `CARAPACE_LIVE_GATEWAY_MODELS` to exercise the image probe.

Run gateway smoke with tools + image across a hand-picked cross-provider set:

```bash
CARAPACE_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3.5-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts
```
...
- OpenRouter: `openrouter/...` (hundreds of models; use `carapace models scan` to find tool+image capable candidates)
...
- If a live test says "no creds", debug the same way you'd debug `carapace models list` / model selection.
- An Carapace live suite that cannot resolve its credentials must skip visibly in the reporter with Vitest test-context `skip(reason)` or fail; it must never pass green without reaching the provider, because a green run that did not reach the provider is not live evidence.

- Per-agent auth profiles: SQLite credential rows in `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite` (this is what "profile keys" means in the live tests)
- Config: `~/.carapace/carapace.json` (or `CARAPACE_CONFIG_PATH`)
- Legacy OAuth dir: `~/.carapace/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
Docker runners below with an explicit `CARAPACE_PROFILE_FILE`.
...
- Enable: `CARAPACE_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `CARAPACE_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `CARAPACE_LIVE_IMAGE_GENERATION_PROVIDERS="deepinfra"`
  - `CARAPACE_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `CARAPACE_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `CARAPACE_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

For the shipped CLI path, add an `infer` smoke after the provider/runtime live
test passes:

```bash
CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_INFER_CLI_TEST=1 pnpm test:live -- test/image-generation.infer-cli.live.test.ts
carapace infer image providers --json
carapace infer image generate \
  --model google/gemini-3.1-flash-image \
  --prompt "Minimal flat test image: one blue square on a white background, no text." \
  --output ./carapace-infer-image-smoke.png \
  --json
```
...
- Enable: `CARAPACE_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `CARAPACE_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `CARAPACE_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `CARAPACE_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `CARAPACE_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
...
- Scope:
  ...
  - Defaults to the release-safe smoke path: one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `CARAPACE_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `CARAPACE_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` (or clear the skip list) to run it explicitly
  - Uses already-exported provider env vars before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in SQLite auth stores do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `CARAPACE_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - That file runs `veo3` text-to-video plus a `kling` image-to-video lane that uses a remote image URL fixture by default (`CARAPACE_LIVE_VYDRA_KLING_IMAGE_URL` to override).
  - Provider-specific xAI coverage:
    - `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_XAI_VIDEO=1 pnpm test:live -- extensions/xai/xai.live.test.ts -t "classic Grok Imagine"`
    ...
    - `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_XAI_VIDEO=1 pnpm test:live -- extensions/xai/xai.live.test.ts -t "Grok Imagine Video 1.5"`
...
- Optional narrowing:
  - `CARAPACE_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `CARAPACE_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `CARAPACE_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `CARAPACE_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `CARAPACE_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
