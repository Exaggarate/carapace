...
Set `CARAPACE_PLUGIN_LIFECYCLE_TRACE=1` for a phase-by-phase breakdown of plugin metadata, discovery, registry, runtime mirror, config mutation, and refresh work. Writes to stderr, so JSON command output stays parseable.
Plugin load failures include their stack trace while this trace is enabled.

```bash
CARAPACE_PLUGIN_LIFECYCLE_TRACE=1 carapace plugins install tokenjuice --force
```
...
Use this before reaching for a CPU profiler. From a source checkout, measure the built runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm carapace ...` also measures source-runner overhead.
...
```bash
CARAPACE_DIAGNOSTICS=plugin.load-profile carapace plugins list
```
...
For one-off profiling through the normal source runner, set `CARAPACE_RUN_NODE_CPU_PROF_DIR`:

```bash
CARAPACE_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm carapace status
```
...
```bash
CARAPACE_TRACE_SYNC_IO=1 pnpm carapace gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched Gateway child; set `CARAPACE_TRACE_SYNC_IO=1` when you want sync I/O trace output in watch mode too.
...
the built runtime through `pnpm carapace <command>`. The repository's typecheck
...
By default this starts or restarts a tmux session named `carapace-gateway-watch-<profile>` (for example `carapace-gateway-watch-main`), with a port suffix such as `carapace-gateway-watch-dev-19001` added only when `CARAPACE_GATEWAY_PORT` differs from the default port `18789`. It auto-attaches from interactive terminals; non-interactive shells, CI, and agent exec calls stay detached and print attach instructions instead:

```bash
tmux attach -t carapace-gateway-watch-main
# Read recent output without attaching
tmux capture-pane -ep -t carapace-gateway-watch-main -S -200
```
...
```bash
pnpm carapace gateway start
```

When an explicit `--port` or `CARAPACE_GATEWAY_PORT` differs from the installed service's effective port, the wrapper leaves the service running so both Gateways can run side by side.
...
# or
CARAPACE_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Raw mode does not manage the installed service. Run `pnpm carapace gateway stop` first when it uses the same port.

Keep tmux management but disable auto-attach:

```bash
CARAPACE_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
Benchmark mode suppresses sync-I/O trace spam by default. Set `CARAPACE_TRACE_SYNC_IO=1` with `--benchmark` to get both CPU profiles and sync-I/O stack traces; in benchmark mode those trace blocks go to `gateway-watch-output.log` under the benchmark directory (filtered from the terminal pane), while normal Gateway logs stay visible.

The tmux wrapper carries common non-secret runtime selectors into the pane, including `CARAPACE_PROFILE`, `CARAPACE_CONFIG_PATH`, `CARAPACE_STATE_DIR`, `CARAPACE_GATEWAY_PORT`, and `CARAPACE_SKIP_CHANNELS`. Put provider credentials in your normal profile/config, or use raw foreground mode for one-off ephemeral secrets.

If the watched Gateway exits during startup, the watcher runs `carapace doctor --fix --non-interactive` once and restarts the Gateway child. Set `CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR=0` to see the original startup failure without the dev-only repair pass.
...
The watcher restarts on build-relevant files under `src/`, extension source files, extension `package.json` and `carapace.plugin.json` metadata, `tsconfig.json`, `package.json`, and `tsdown.config.ts`. Extension metadata changes restart the gateway without forcing a rebuild; source and config changes still rebuild `dist` first.
...
When you run `pnpm carapace`, `pnpm dev`, or a Gateway development runner from
...
The runners supply the existing `CARAPACE_DEV_SOURCE_ROOT` selector unless you
...
`pnpm carapace plugins inspect <id> --json` to check the selected source and origin.

Two **separate** `--dev` flags:

- **Global `--dev` (profile):** isolates state under `~/.carapace-dev` and defaults the gateway port to `19001` (derived ports shift with it).
...
```bash
pnpm gateway:dev
CARAPACE_PROFILE=dev carapace tui
```

Without a global install, run the CLI via `pnpm carapace ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `CARAPACE_PROFILE=dev`
   - `CARAPACE_STATE_DIR=~/.carapace-dev`
   - `CARAPACE_CONFIG_PATH=~/.carapace-dev/carapace.json`
   - `CARAPACE_GATEWAY_PORT=19001` (browser/canvas ports shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - `pnpm gateway:dev` also sets `CARAPACE_SKIP_CHANNELS=1` to skip channel providers.
...
```bash
CARAPACE_PROFILE=dev carapace gateway --dev --reset
```
...
Carapace can log the **raw assistant stream** before any filtering/formatting. This is the best way to see whether reasoning is arriving as plain text deltas (or as separate thinking blocks).
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.carapace/logs/raw-stream.jsonl
```
...
CARAPACE_RAW_STREAM_PATH=~/.carapace/logs/raw-stream.jsonl
```

Default file: `~/.carapace/logs/raw-stream.jsonl`
...
- To use the built CLI for other tasks (for example `dashboard --no-open` if your debug session spawns a new auth token), run it from another terminal: `node ./carapace.mjs` or an alias like `alias carapace-build="node $(pwd)/carapace.mjs"`.
