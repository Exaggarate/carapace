...
- Prefer CLI surfaces when they exist (example: `carapace models status --check`).
...
- `scripts/claude-auth-status.sh [full|json|simple]` - checks Claude Code + Carapace auth status.
- `scripts/auth-monitor.sh` - polls status and sends a notification (via Carapace send, and/or ntfy.sh) when the token nears expiry. Env: `WARN_HOURS` (default `2`), `NOTIFY_PHONE`, `NOTIFY_NTFY`. Run on a schedule via the bundled `scripts/systemd/carapace-auth-monitor.{service,timer}` (every 30 minutes).
...
- `CARAPACE_GH_READ_APP_ID`
- `CARAPACE_GH_READ_PRIVATE_KEY_FILE`

Optional env:

- `CARAPACE_GH_READ_INSTALLATION_ID` when you want to skip repo-based installation lookup
- `CARAPACE_GH_READ_PERMISSIONS` as a comma-separated override for the read permission subset to request
...
- `scripts/gh-read run list -R carapace/carapace`
- `scripts/gh-read api repos/carapace/carapace/pulls/123`
