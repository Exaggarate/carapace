...
- Maintainers author `/clawhub/**` pages in [carapace/clawhub](https://github.com/Exaggarate/carapace/clawhub/tree/main/docs). `scripts/docs-sync-publish.mjs` replaces the entire publish `docs/clawhub/` tree from that source. Do not keep authored copies here.
- This repo therefore holds no `/clawhub/**` page sources, even though `docs/docs.json` lists them in the navigation. A local docs preview and `pnpm docs:check-links` report those routes as missing until you point `CARAPACE_DOCS_SYNC_CLAWHUB_REPO` at a ClawHub checkout.
- Keep Carapace-specific skill and plugin guidance in the owning Carapace docs, such as `docs/cli/skills.md` and `docs/cli/plugins.md`. That guidance covers installation, update, verification, removal, and release trust. Standalone ClawHub CLI and publishing reference belongs upstream.
- For links into `/clawhub/**`, run `pnpm docs:check-links:anchors` with `CARAPACE_DOCS_SYNC_CLAWHUB_REPO` pointing to the actual ClawHub source checkout. Local-only pages cannot prove published ClawHub routes or anchors.

## Published Link Rules

- The publish pipeline pushes docs to `https://github.com/Exaggarate/carapace` from the `carapace/docs` mirror.
...
- CI verifies JSON5 and JSON config fences that look like whole `carapace.json` documents against the schema. `pnpm docs:check-config-examples` runs that verification. Deliberately partial or legacy snippets opt out with `validate=false` in the fence info string.
...
- `scripts/docs-sync-publish.mjs` excludes and prunes `docs/internal/**` from the public `carapace/docs` publish repo if a page is force-added later.
...
- `.github/workflows/maturity-scorecard.yml` renders artifact previews and can open generated-doc PRs. `.github/workflows/carapace-release-checks.yml` dispatches it for release QA.
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `carapace/docs` repo (often cloned locally as `../carapace-docs`).
- Do not add or edit localized docs under `docs/<locale>/**` here.
- Treat Carapace-owned English docs in this repo plus glossary files as the source of truth. ClawHub English sources follow Source Ownership above.
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `carapace/docs`.
