...
summary: "CLI reference for `carapace skills` (search/install/update/verify/list/info/check/library/workshop)"
read_when:
...
Use [`carapace plugins`](/cli/plugins) for plugin packages. The standalone
...
carapace skills search --limit 20 --json
carapace skills install @owner/<slug>
carapace skills install @owner/<slug> --version <version>
carapace skills install skills-sh:<owner>/<repo>/<slug>
carapace skills install git:owner/repo
carapace skills install git:owner/repo@main
carapace skills install ./path/to/skill --as custom-name
carapace skills install @owner/<slug> --force
carapace skills install @owner/<slug> --force-install
carapace skills install @owner/<slug> --acknowledge-install-policy-warning
carapace skills install @owner/<slug> --agent <id>
carapace skills install @owner/<slug> --global
carapace skills update @owner/<slug>
carapace skills update @owner/<slug> --force
carapace skills update @owner/<slug> --force-install
carapace skills update @owner/<slug> --acknowledge-install-policy-warning
carapace skills update @owner/<slug> --global
carapace skills update --all
carapace skills update --all --agent <id>
carapace skills update --all --global
carapace skills verify @owner/<slug>
carapace skills verify @owner/<slug> --json
carapace skills verify @owner/<slug> --version <version>
carapace skills verify @owner/<slug> --tag <tag>
carapace skills verify @owner/<slug> --card
carapace skills verify @owner/<slug> --global
carapace skills list
carapace skills list --eligible
carapace skills list --json
carapace skills list --verbose
carapace skills list --agent <id>
carapace skills info <name>
carapace skills info <name> --json
carapace skills info <name> --agent <id>
carapace skills check
carapace skills check --agent <id>
carapace skills check --json
carapace skills curator status
carapace skills curator status --json
carapace skills workshop propose-create --name "qa-check" --description "QA checklist" --proposal ./PROPOSAL.md
carapace skills workshop propose-update qa-check --proposal ./PROPOSAL.md
...
Carapace does not download from skills.sh. These entries are shown as
...
and bare `carapace skills` request the selected Gateway's authoritative skill
inventory. A configured remote Gateway or an explicit `CARAPACE_GATEWAY_URL`
...
or zip/archive paths, and `carapace skills update` updates ClawHub-tracked
installs only.
...
Carapace prints the reason and findings, then asks `type: '<skill>' to install
anyway` (or `update anyway`). If the fully rendered review exceeds 4,000
characters, Carapace fails closed before prompting; reduce or coalesce the
...
| `verify` provenance              | When ClawHub returns server-resolved source provenance, verify JSON also includes a commit-pinned `carapace.verifiedSourceUrl`. Unavailable or self-declared source URLs stay only in the raw provenance envelope and are not promoted.                                                                                           |
...
| Skill Card fingerprint           | Installed ClawHub bundles can include a generated `skill-card.md`. Carapace treats verification as a ClawHub server decision and does not reject an installed skill just because that generated card changes the bundle fingerprint.                                                                                              |
...
Official ClawHub skill publishers and bundled Carapace skill sources bypass
...
agent-specific skill, or the Carapace state directory for a shared skill
installed with `--global`:

```bash
clawhub --workdir /path/to/agent-workspace uninstall @owner/my-skill
clawhub --workdir ~/.carapace uninstall @owner/my-skill
```

If `CARAPACE_STATE_DIR` is set, use that configured state directory for shared
skills instead:

```bash
clawhub --workdir "$CARAPACE_STATE_DIR" uninstall @owner/my-skill
```
...
`carapace skills library` manages the identified caller's skills on the selected
...
```bash
carapace skills library --help
```

List your library:

```bash
carapace skills library list --scope mine --json
```

Create a private skill from a directory containing `SKILL.md`:

```bash
carapace skills library create ./my-skill --slug my-skill
```

Read its stable ID and current revision before editing:

```bash
carapace skills library read <skill-id> --json
```

Update only the instructions while preserving supporting files:

```bash
carapace skills library update <skill-id> ./SKILL.md --expected-revision <revision-hash>
```
...
```bash
carapace skills library import ./my-skill.zip --slug my-skill
```

Import a ClawHub skill without publishing your library:

```bash
carapace skills library import @owner/<slug> --clawhub --slug my-skill
```
...
```bash
carapace skills library attach --session <session-key> --skill-id <skill-id> --revision <revision-hash>
```

`detach` takes the same session and skill ID. Refresh one selected skill:

```bash
carapace skills library refresh --session <session-key> --skill-id <skill-id>
```
...
`carapace skills workshop` manages pending skill proposals for the selected
