---
summary: "CLI reference for `carapace wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, ChatGPT import, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `carapace wiki`
title: "Wiki"
---
...
carapace wiki okf import ./knowledge-catalog/okf/bundles/ga4
carapace wiki compile
carapace wiki lint
carapace wiki search "alpha"
carapace wiki search "who should I ask about Teams?" --mode route-question
carapace wiki get entity.alpha --from 1 --lines 80

carapace wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

carapace wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

carapace wiki bridge import
carapace wiki unsafe-local import
carapace wiki chatgpt import --export ./chatgpt-export --dry-run
carapace wiki chatgpt rollback <run-id>

carapace wiki obsidian status
carapace wiki obsidian search "alpha"
carapace wiki obsidian open syntheses/alpha-summary.md
carapace wiki obsidian command workspace:quick-switcher
carapace wiki obsidian daily
```
...
```bash
carapace wiki status --agent support
carapace wiki search "refund policy" --agent support
carapace wiki ingest ./campaign-notes.md --agent marketing
```
...
carapace wiki okf import ./bundles/ga4 --json
carapace wiki search "BigQuery Table" --mode source-evidence --json
carapace wiki get <path-from-json-result>
```

### `wiki compile`

Rebuild indexes, related blocks, dashboards, and the compiled query/prompt snapshot. The snapshot is persisted in Carapace's shared SQLite plugin state and kept in memory for synchronous prompt projection; it does not create cache files in the vault.
...
Use `wiki search` for wiki-specific ranking and provenance. For one broad shared recall pass, prefer `carapace memory search` when the active memory plugin exposes shared search.
...
```bash
carapace wiki search "bgroux" --mode find-person
carapace wiki search "who knows Teams rollout?" --mode route-question
carapace wiki search "maintainer-whois" --mode source-evidence
carapace wiki search "strong route Teams" --mode raw-claim --json
```
...
carapace wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
```bash
carapace wiki chatgpt import --export ./chatgpt-export
carapace wiki chatgpt import --export ./conversations.json --dry-run
```
...
Roll back a previously applied ChatGPT import run, removing pages it created and restoring pages it overwrote. Pages changed after import are moved under the run's `.carapace-wiki/import-runs/<run-id>/recovered/` directory instead of being deleted. Recovery paths remain in the command result on retries and later `alreadyRolledBack` responses. Interrupted runs remain `rolling_back` while target recovery or derived-artifact compilation is incomplete. A persisted process-restart fence separates those phases: after it, retries rebuild indexes and compiled caches without rewriting source pages or moving later pathname writes. A later normal compile may refresh machine-managed Related blocks. This covers in-process failure and process restart after ordinary filesystem calls return, not kernel or host power-loss ordering.
...
`carapace wiki` behavior is shaped by:
