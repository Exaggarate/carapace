...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Carapace Gateway.

`carapace acp` speaks ACP over stdio for IDEs and forwards prompts to the Gateway over WebSocket, keeping ACP sessions mapped to Gateway session keys. It is a Gateway-backed ACP bridge, not a full ACP-native editor runtime: it focuses on session routing, prompt delivery, and streaming updates.

If you want an external MCP client to talk directly to Carapace channel conversations instead of hosting an ACP harness session, use [`carapace mcp serve`](/cli/mcp) instead.

## What this is not

`carapace acp` means Carapace acts as an ACP server: an IDE or ACP client connects to Carapace, and Carapace forwards that work into a Gateway session.

This is different from [ACP Agents](/tools/acp-agents), where Carapace runs an external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Carapace: use `carapace acp`
- Carapace should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Carapace parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Carapace Gateway or agent instead.                                                                                                                          |
...
# Remote Gateway
carapace acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
carapace acp --url wss://gateway-host:18789 --token-file ~/.carapace/gateway.token

# Attach to an existing session key
carapace acp --session agent:main:main

# Attach by label (must already exist)
carapace acp --session-label "support inbox"

# Reset the session key before the first prompt
carapace acp --session agent:main:main --reset-session
```
...
carapace acp client --server-args --url wss://gateway-host:18789 --token-file ~/.carapace/gateway.token

# Override the server command (default: carapace)
carapace acp client --server "node" --server-args carapace.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Carapace through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive `carapace acp` over stdio with an ACP JSON-RPC client. Cover `initialize`, `session/new`, `session/list` with an absolute `cwd`, `session/resume`, `session/close`, duplicate close, and missing resume.
...
Avoid using `carapace gateway call sessions.list` as the only ACP proof. That CLI path may request a fresh-token operator scope upgrade; ACP bridge correctness is proven by ACP stdio frames plus the Gateway `sessions.list` log.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want it to drive an Carapace Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `carapace acp` over stdio.

Example config (persisted):

```bash
carapace config set gateway.remote.url wss://gateway-host:18789
carapace config set gateway.remote.token <token>
```
...
```bash
carapace acp --session agent:main:main
carapace acp --session agent:design:main
carapace acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Carapace plugin tools or selected built-in tools such as `cron`, enable the gateway-side ACPX MCP bridges instead of trying to pass per-session `mcpServers`. See [ACP Agents](/tools/acp-agents-setup#plugin-tools-mcp-bridge) and [Carapace tools MCP bridge](/tools/acp-agents-setup#carapace-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your Carapace bot over ACP, use `acpx` with its built-in `carapace` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx carapace` at `carapace acp`.
3. Target the Carapace session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Carapace ACP session
acpx carapace exec "Summarize the active Carapace session state."

# Persistent named session for follow-up turns
acpx carapace sessions ensure --name codex-bridge
acpx carapace -s codex-bridge --cwd /path/to/repo \
  "Ask my Carapace work agent for recent context relevant to this repo."
```

If you want `acpx carapace` to target a specific Gateway and session key every time, override the `carapace` agent command in `~/.acpx/config.json`:

```json validate=false
{
  "agents": {
    "carapace": {
      "command": "env CARAPACE_HIDE_BANNER=1 CARAPACE_SUPPRESS_NOTES=1 carapace acp --url ws://127.0.0.1:18789 --token-file ~/.carapace/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Carapace checkout, use the direct CLI entrypoint instead of the dev runner so the ACP stream stays clean:

```bash
env CARAPACE_HIDE_BANNER=1 CARAPACE_SUPPRESS_NOTES=1 node carapace.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client pull contextual information from an Carapace agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Carapace ACP" to start a thread.
...
- `--token` and `--password` can be visible in local process listings on some systems. Prefer `--token-file`/`--password-file` or environment variables (`CARAPACE_GATEWAY_TOKEN`, `CARAPACE_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: `gateway.auth.*` then env (`CARAPACE_GATEWAY_*`), falling back to `gateway.remote.*` only when `gateway.auth.*` is unset (a configured-but-unresolved local SecretRef fails closed instead of silently falling back)
...
- `--server <command>`: ACP server command (default: `carapace`).
- `--server-args <args...>`: extra arguments passed to the ACP server.
- `--server-verbose`: enable verbose logging on the ACP server.
- `--verbose, -v`: verbose client logging.
- `carapace acp client` sets `CARAPACE_SHELL=acp-client` on the spawned bridge process, which can be used for context-specific shell/profile rules.
