...
until a message is processed. Use `carapace channels status --probe`,
`carapace status --deep`, or `carapace health --verbose` when you need live
channel connectivity.

```bash
carapace sessions
carapace sessions --agent work
carapace sessions --all-agents
carapace sessions --active 120
carapace sessions --limit 25
carapace sessions --store ./tmp/sessions.json
carapace sessions --json
```
...
Long keys show their beginning and end; use `carapace sessions --json` for complete
session keys.
...
and suffixless custom selectors. Carapace resolves that selector to its physical
...
`carapace sessions` and the Gateway `sessions.list` RPC are bounded by default
...
`carapace sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.carapace/agents/main/agent/carapace-agent.sqlite" },
    { "agentId": "work", "path": "/home/user/.carapace/agents/work/agent/carapace-agent.sqlite" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
```bash
carapace sessions archive "agent:main:scratch-1"
carapace sessions archive "agent:main:scratch-1" "agent:main:scratch-2"
carapace sessions archive "agent:work:scratch-1" --agent work
carapace sessions archive "agent:main:scratch-1" --dry-run
carapace sessions archive "agent:main:scratch-1" --json
```
...
```bash
carapace sessions delete "agent:main:scratch-1"
carapace sessions delete "agent:main:scratch-1" "agent:main:scratch-2" --yes
carapace sessions delete "agent:work:scratch-1" --agent work --yes
carapace sessions delete "agent:main:scratch-1" --dry-run
carapace sessions delete "agent:main:scratch-1" --yes --json
```
...
```json
{
  ...
  "results": [
    { "key": "agent:main:scratch-1", "ok": true, "status": "archived" },
    {
      ...
      "ok": false,
      "status": "not_found",
      "error": "Session not found. Run carapace sessions list --json to choose a valid key."
    }
  ]
}
```
...
```bash
carapace sessions tail
carapace sessions tail --follow
carapace sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
carapace sessions --agent work tail --follow
carapace sessions --all-agents tail --follow
```

`carapace sessions tail` renders recent runtime trajectory events as compact
...
```bash
carapace sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
carapace sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.carapace/trajectory-exports/` under the selected workspace.
...
carapace sessions cleanup --agent work --dry-run
carapace sessions cleanup --all-agents --dry-run
carapace sessions cleanup --enforce
carapace sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
carapace sessions cleanup --dry-run --fix-dm-scope
carapace sessions cleanup --json
```

`carapace sessions cleanup` uses `session.maintenance` settings from config
([Configuration reference](/gateway/config-agents/sessions#session)):

- Scope note: `carapace sessions cleanup` maintains session stores,
...
`carapace sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.carapace/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.carapace/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
...
Reclaim context budget for a wedged or oversized session. `carapace sessions
compact <key>` is the first-class wrapper around the `sessions.compact`
Gateway RPC and requires a running Gateway.

```bash
carapace sessions compact "agent:main:main"
carapace sessions compact "agent:main:main" --max-lines 200
carapace sessions compact "agent:work:main" --agent work --json
```
...
<Note>
`carapace agent --message '/compact ...'` is **not** a compaction path. Slash
...
`carapace gateway call sessions.compact --params '<json>'` accepts:
