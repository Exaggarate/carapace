---
summary: "CLI reference for `carapace daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `carapace daemon ...` in scripts
...
Legacy alias for Gateway service management. `carapace daemon ...` maps to the same service-control commands as `carapace gateway ...`. Prefer [`carapace gateway`](/cli/gateway) for current docs and examples.

## Usage

```bash
carapace daemon status
carapace daemon install
carapace daemon start
carapace daemon stop
carapace daemon restart
carapace daemon uninstall
```
