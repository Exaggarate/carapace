---
summary: "CLI reference for `carapace backup` (archives, SQLite snapshots, and Git history)"
read_when:
  - You want a first-class backup archive for local Carapace state
  - You need a compact, verified snapshot of one Carapace SQLite database
  - You want scheduled, versioned database backups in an operator-owned Git repository
  - You want to preview which paths would be included before reset or uninstall
  - You want to restore from a `.tar.gz` archive previously created by `carapace backup`
title: "Backup"
---

# `carapace backup`

Create a local backup archive for Carapace state, config, auth profiles, channel/provider credentials, sessions, and optionally workspaces.

```bash
carapace backup create
carapace backup create --output ~/Backups
carapace backup create --dry-run --json
carapace backup create --verify
carapace backup create --no-include-workspace
carapace backup create --only-config
carapace backup verify ./2026-03-09T08-00-00.000+08-00-carapace-backup.tar.gz
carapace backup restore ./2026-03-09T08-00-00.000+08-00-carapace-backup.tar.gz --target ./restored-carapace
carapace backup sqlite create --global --repository ~/Backups/carapace-sqlite
carapace backup sqlite create --agent main --repository ~/Backups/carapace-sqlite
carapace backup sqlite list --repository ~/Backups/carapace-sqlite
carapace backup sqlite verify ~/Backups/carapace-sqlite/<snapshot-id>
carapace backup sqlite verify ~/Backups/carapace-sqlite/<snapshot-id> --scratch ~/Private/carapace-scratch
carapace backup sqlite restore ~/Backups/carapace-sqlite/<snapshot-id> --target ./restored/carapace.sqlite
carapace backup git init --repository ~/Backups/carapace-git --remote <private-git-url>
carapace backup git create --repository ~/Backups/carapace-git --all --push
carapace backup git log --repository ~/Backups/carapace-git
carapace backup git verify --repository ~/Backups/carapace-git --global
carapace backup git restore --repository ~/Backups/carapace-git --agent main --target ./restored/agent.sqlite
carapace backup enable --repository ~/Backups/carapace-git --every 24h --push
carapace backup disable
```
...
- `carapace backup verify <archive>` checks that the archive contains exactly one root manifest, rejects traversal-style archive paths, unsafe symbolic links, and SQLite sidecars, confirms every manifest-declared payload exists, validates every SQLite snapshot's file shape, and runs full integrity and role checks on canonical Carapace databases. Dedicated plugin schemas remain opaque because they may require owner-defined SQLite capabilities. `carapace backup create --verify` runs that validation immediately after writing the archive.
- `carapace backup create --only-config` backs up just the active JSON config file.
...
<Warning>
  ...
  are not archived; after activation, run `carapace plugins update <id>` or
  reinstall with `carapace plugins install <spec> --force`. The generated
  `plugin-skills/` symlink index is also omitted; run `carapace skills list` or
...
restored state asset into place or point `CARAPACE_STATE_DIR` at that asset,
then run `carapace doctor` before restarting. Use `manifest.json` as the source
...
Use `carapace backup sqlite` when you need a portable artifact for one Carapace-owned SQLite database instead of a broad state archive.

Snapshot creation accepts exactly one named source. Agent sources always use
the current configuration's resolved `<agentDir>/carapace-agent.sqlite`, even
when `agentDir` is outside the state directory:

| Command                                                         | Database               |
| --------------------------------------------------------------- | ---------------------- |
| `carapace backup sqlite create --global --repository <dir>`     | Shared Carapace state  |
| `carapace backup sqlite create --agent <id> --repository <dir>` | One per-agent database |
...
SQLite snapshots can contain auth profiles, session state, plugin state, and other sensitive records. Protect repositories with the same permissions, encryption, retention policy, and destination restrictions as the live Carapace state directory.

### Verify and restore

```bash
carapace backup sqlite verify <snapshot-directory>
carapace backup sqlite restore <snapshot-directory> --target <new-database-path>
```

Verification checks the strict manifest shape, artifact size and SHA-256, SQLite integrity, foreign keys, schema version, database role and owner, and Carapace-owned index definitions.
...
Restore repeats verification and writes only to a fresh target. It refuses an existing target, `-wal`, `-shm`, or `-journal` sidecar and never performs an in-place replacement of a live Carapace database. The target parent has the same path-security requirements as verification scratch. Activating a restored database remains an explicit offline operator step.
...
`carapace backup git` stores deterministic, per-table JSONL dumps in a plain Git repository owned by the operator. One repository can hold the shared database and every per-agent database:
...
world-writable. Carapace checks this when initializing or adopting a repository
...
The repository must be dedicated to Carapace backups. An existing `global/` or
`agents/<agentId>/` scope is backup-owned only when it is empty or contains a
valid schema-version-1 `manifest.json`. Carapace refuses to replace any other
...
```bash
carapace backup git log --repository ~/Backups/carapace-git --limit 20
carapace backup git verify --repository ~/Backups/carapace-git --ref <commit> --global
carapace backup git verify --repository ~/Backups/carapace-git --ref <commit> --agent main
```
...
Git. Carapace reports the failure without returning partial history entries.
...
```bash
carapace backup git restore --repository ~/Backups/carapace-git --ref <commit> --global --target ./restored/carapace.sqlite
```
...
`carapace status` shows one `Backups` overview row, and `carapace status --json` includes the latest attempt and latest successful run. `carapace doctor` prints an informational hint when no successful backup is recorded or the newest successful backup is more than 14 days old. Recording is best-effort: a record-write failure prints a warning but never changes a successful backup into a failed command.

## What gets backed up

`carapace backup create` plans sources from your local Carapace install:

- The state directory (usually `~/.carapace`)
...
`<agentDir>/carapace-agent.sqlite`. The default agent root is
...
Carapace builds one immutable, configuration-derived ownership inventory before
...
During archive creation, Carapace excludes known live-mutation paths before `tar` reads them. This avoids races between a file's recorded size and concurrent writes. The filter applies these state-relative rules under each backed-up state directory:
...
leases are removed, sidecars are omitted, and canonical Carapace databases must
...
Installed plugin source and manifest files under the state directory's `extensions/` tree are included, but their nested `node_modules/` dependency trees are skipped as rebuildable install artifacts. After restoring an archive, use `carapace plugins update <id>` or reinstall with `carapace plugins install <spec> --force` if a restored plugin reports missing dependencies.

The state directory's `plugin-skills/` root is a generated, Carapace-owned symlink index, not authoritative state. Backup creation reports and omits that root because its absolute targets are specific to the source installation. After activating restored state, run `carapace skills list` or start an agent session to rebuild the links from current plugin metadata.
...
Local edits inside a managed `dev/` checkout are developer source, not Carapace product state, and are not included. Commit and push those edits or copy the checkout separately before relying on a state backup.

## Invalid config behavior

`carapace backup` bypasses the normal config preflight so it can still help during recovery. Workspace discovery depends on a valid config, so `carapace backup create` fails fast when the config file exists but is invalid and workspace backup is still enabled.
...
Carapace does not enforce a built-in maximum backup size or per-file size limit. An archive write that produces no data for five minutes fails and removes its partial temporary file instead of hanging indefinitely. Practical limits otherwise come from:

- Available space for the temporary archive write plus the final archive
- Time to walk large workspace trees and compress them into a `.tar.gz`
- Time to rescan the archive with `--verify` or `carapace backup verify`
- Destination filesystem behavior: Carapace requires no-overwrite hard-link publication so a final archive path never exposes an in-progress copy; unsupported filesystems fail with an actionable error
...
- [CLI reference](/cli)
- [Migrating an Carapace install](/install/migrating)
