---
summary: "CLI reference for `carapace promos` (list and claim promotional model offers)"
read_when:
...
```bash
carapace promos list
carapace promos claim <slug>
carapace promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed Carapace version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `carapace onboard` non-interactive flags; to keep the
...
`carapace models set <model>`.

## Passive discovery in `models list`

`carapace models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `carapace promos list`. Offers you have already listed or claimed are never
  announced again.
