---
summary: "CLI reference for `carapace configure` (interactive configuration prompts)"
read_when:
...
Use `carapace onboard` or `carapace setup` for the full guided first-run journey, `carapace setup --baseline` for the baseline config/workspace only, and `carapace channels add` when you only need channel account setup.

<Tip>
`carapace config` with no subcommand opens the same wizard. Use `carapace config get|set|unset` for non-interactive edits.
</Tip>

## Options

Before `carapace configure` changes local credentials or configuration, Carapace compares the selected CLI state/config paths with the local Gateway or its installed service. A proven mismatch stops before the write. A remote Gateway or an authenticated path that cannot be verified produces a warning instead.

This comparison also applies when `CARAPACE_HOME` relocates the CLI's default state directory. The installed service's recorded environment determines its paths, including while the Gateway is stopped; the CLI's path overrides do not replace them. If the service definition or its recorded paths cannot be verified, Carapace warns and leaves configuration available. Inspect the service with `carapace gateway status --deep` before relying on local changes to reach it.
...
carapace configure --section model --section channels
carapace configure --section gateway --section daemon
```
...
<Note>
`carapace configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `carapace config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>

## Gateway section

First-run `carapace onboard` and `carapace setup` generate a Gateway secret in
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `carapace models auth login --provider <id> --set-default` or `carapace models set <model>` to intentionally change the default model.
</Note>
...
`carapace configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
...
- After successful daemon setup, the final Gateway status uses the same platform-specific startup grace period as onboarding before reporting reachability. Service installation and Gateway reachability are separate outcomes; if the Gateway is still not detected, run `carapace health` to check it again.
