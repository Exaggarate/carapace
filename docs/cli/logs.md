---
summary: "CLI reference for `carapace logs` (tail gateway logs via RPC)"
read_when:
...
- `--port <port>`: select a local Gateway port, overriding the configured remote URL and `CARAPACE_GATEWAY_URL`; cannot be combined with `--url`
...
carapace logs --follow --interval 2000
carapace logs --limit 500 --max-bytes 500000
carapace logs --json
carapace logs --plain
carapace logs --no-color
carapace logs --utc
carapace logs --follow --local-time
carapace logs --port 19083 --json
carapace logs --url ws://127.0.0.1:18789 --token "$CARAPACE_GATEWAY_TOKEN"
```

The selected root profile matches the Gateway's rolling file: the default
profile uses `carapace-YYYY-MM-DD.log`, while named profiles use
`carapace-<profile>-YYYY-MM-DD.log` (for example,
`carapace-dev-YYYY-MM-DD.log`).

## Fallback and recovery behavior

- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `carapace logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets never use this fallback.
