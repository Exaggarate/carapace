---
summary: "CLI reference for `carapace workboard` cards, dispatch, and worker runs"
read_when:
...
`carapace workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.

Enable the plugin before using the command:

```bash
carapace plugins enable workboard
...
```bash
carapace workboard list [--board <id>] [--status <status>] [--include-archived] [--json]
carapace workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
carapace workboard show <id> [--json]
carapace workboard move <id> --status <status> [--json]
carapace workboard dispatch [--board <id>] [--max-starts <count>] [--admin] [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
carapace workboard list
carapace workboard list --board default --status ready
carapace workboard list --json
```
...
```bash
carapace workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
carapace workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
carapace workboard show 7f4a2c10
carapace workboard show 7f4a2c10 --json
```
...
```bash
carapace workboard move 7f4a2c10 --status review
carapace workboard move 7f4a2c10 --status done --json
```
...
```bash
carapace workboard dispatch
carapace workboard dispatch --json
carapace workboard dispatch --max-starts 10
carapace workboard dispatch --admin
carapace workboard dispatch --url http://127.0.0.1:18789 --token "$CARAPACE_GATEWAY_TOKEN"
```
...
Local `list`, `create`, `show`, and `move` commands operate on the local Carapace state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `carapace` command when you need a different state root.
...
```bash
carapace plugins inspect workboard --runtime --json
```
...
Then retry `carapace workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.
...
```bash
carapace workboard list --status ready
```
