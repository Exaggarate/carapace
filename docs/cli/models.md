---
summary: "CLI reference for `carapace models` (status/list/set/scan, aliases, fallbacks, shared auth, personal accounts)"
read_when:
...
```bash
carapace models --json
carapace models status
carapace models list
carapace models refresh
carapace models set <model-or-alias>
carapace models set-image <model-or-alias>
carapace models scan
```
...
For `models status`, `CARAPACE_AGENT_DIR` overrides the inspected auth directory when `--agent` is omitted. A matching configured `agentDir` retains that agent's ownership during credential refresh. An explicit `--agent <id>` takes precedence over the environment override.
...
| `--agent <id>`            | Configured agent id; overrides `CARAPACE_AGENT_DIR`.                                                                                     |
...
Direct `models status --probe` runs create temporary internal sessions in the selected agent's canonical database, so the command requires exclusive ownership of the configured state directory. Stop a running Gateway with `carapace gateway stop` before probing; the command removes its internal sessions and releases the state lock when it finishes or is interrupted.
...
- `no_model`: provider auth exists, but Carapace could not resolve a probeable model candidate for that provider.

For OpenAI ChatGPT/Codex OAuth troubleshooting, `carapace models status`, `carapace models auth list --provider openai`, and `carapace config get agents.defaults.model --json` are the quickest way to confirm whether an agent has a usable `openai` OAuth profile for `openai/*` through the native Codex runtime. See [OpenAI provider setup](/providers/openai#check-and-recover-codex-oauth-routing).

### List

`carapace models list` is read-only: it reads config, auth profiles, existing catalog state, and provider-owned catalog rows, but never rewrites `models.json`.

`carapace models refresh [--json]` forces an immediate hosted catalog check. Like `scan`, it rejects `--agent` because the hosted catalog is global, not agent-scoped.
...
[`carapace/catalog`](https://github.com/Exaggarate/carapace/catalog), where each content
...
- If you omit the provider, Carapace resolves the input as an alias first, then as a unique configured-provider match for that exact model id, and only then falls back to the configured default provider with a deprecation warning. If that provider no longer exposes the configured default model, Carapace falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
By default Carapace tries to probe tool and image support with live model calls. If no OpenRouter key is configured, the command falls back to metadata-only output and explains that `:free` models still require `OPENROUTER_API_KEY` for probes and inference.
...
```bash
carapace models aliases list [--json] [--plain]
carapace models aliases add <alias> <model-or-alias>
carapace models aliases remove <alias>
```
...
```bash
carapace models fallbacks list [--json] [--plain]
carapace models fallbacks add <model-or-alias>
carapace models fallbacks remove <model-or-alias>
carapace models fallbacks clear
```

Manages `agents.defaults.model.fallbacks`. `carapace models image-fallbacks list|add|remove|clear` manages the parallel `agents.defaults.imageModel.fallbacks` list with the same subcommand shape.
...
| System / agent | `models auth login --provider <id> [--agent <id>]` | The Carapace installation on the machine running the command.       |

To configure system/agent credentials for a remote server, run `models auth` on that server with its Carapace state/config. Configuring a remote Gateway URL on your laptop does not make `models auth` write to the server.

```bash
carapace models accounts list
carapace models accounts login
carapace models accounts login anthropic --method api-key
carapace models accounts login openai --method device-code
carapace models accounts login xai --method api-key
carapace models accounts use <account-id>
carapace models accounts clear-default <provider>
```
...
If no person is identified, the command stops before provider sign-in and explains how to use an identity-bearing endpoint. It does not infer ownership or change Gateway authentication. These commands do not accept `--agent` or an owner id, and they do not modify local shared auth stores or model config. The top-level `carapace connect` command enrolls a node; it is not a personal-account login.
...
```bash
carapace models accounts --timeout 45000 list --json
carapace models accounts list --timeout 45000 --json
```

## Auth profiles

These commands manage **System / agent** credentials, not personal Gateway accounts. Before provider sign-in, `models auth login` shows the selected agent and that it is operating on the machine running Carapace.

Before a `models auth` command changes the local auth store, Carapace compares the selected CLI state/config paths with the local Gateway or its installed service. A proven mismatch stops before the write. A remote Gateway or an authenticated path that cannot be verified produces a warning instead.

```bash
carapace models auth add
carapace models auth list [--provider <id>] [--json]
carapace models auth login --provider <id> [--agent <agentId>]
carapace models auth login --provider openai --profile-id openai:work
carapace models auth login-github-copilot
carapace models auth logout <profileId> [--yes]
carapace models auth paste-api-key --provider <id>
carapace models auth setup-token --provider <id>
carapace models auth paste-token --provider <id>
carapace models auth order get --provider <id>
carapace models auth order set --provider <id> <profileIds...>
carapace models auth order clear --provider <id>
```
...
`models auth login` runs a provider plugin's auth flow (OAuth/API key). Use `carapace plugins list` to see which providers are installed. `login` accepts `--profile-id <id>` for providers that support named profiles during login (use this to keep multiple logins for the same provider separate), `--method <id>` to pick a specific auth method, `--device-code` as a shortcut for `--method device-code`, `--set-default` to apply the provider's recommended default model, and `--force` to remove existing profiles for that provider first (use when a cached OAuth profile is stuck or you want to switch accounts).

For the shared-main agent, `--force` clears the provider's shared credentials and main-agent local overrides, including their order and health state. For another agent it clears only that agent's local profiles, leaving shared credentials unchanged. A busy auth store stops the command before login starts; close other Carapace commands using the same state directory and retry. SQLite lock diagnostics can name either the shared state database or an agent database, so checking only the legacy auth file for open handles does not rule out contention.
...
Use either `carapace models auth --agent <id> <subcommand>` or `carapace models auth <subcommand> --agent <id>` to target a specific configured agent store. Both forms are supported by `add`, `list`, `login`, `logout`, `paste-api-key`, `setup-token`, `paste-token`, `login-github-copilot`, and `order get`/`set`/`clear`.

For OpenAI models, `--provider openai` defaults to ChatGPT/Codex account login. Use `--method api-key` only when you want to add an OpenAI API-key profile, usually as a backup for Codex subscription limits. Run `carapace doctor --fix` to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
carapace models auth login --provider openai --set-default
carapace models auth login --provider openai --method api-key
carapace models auth paste-api-key --provider openai
carapace models auth list --provider openai
carapace models auth logout openai:manual --yes
```

Notes:

- `paste-api-key` accepts API keys generated elsewhere, prompts for the key value, and writes it to the default profile id `<provider>:manual` unless you pass `--profile-id`. In automation, pipe the key on stdin, for example `printf "%s\n" "$OPENAI_API_KEY" | carapace models auth paste-api-key --provider openai`.
...
- Anthropic: `setup-token`/`paste-token` are supported Carapace auth paths for `anthropic`, but Carapace prefers reusing the Claude CLI (`claude -p`) on the host when it is available.
