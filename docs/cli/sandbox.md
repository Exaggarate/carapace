...
[`carapace agent exec`](/cli/agent#agent-exec) does not use these configured runtimes. Its isolated implicit policy config turns the agent sandbox off, allows full Gateway-host execution, and restricts filesystem tools to `--cwd`.
...
```bash
carapace sandbox list
carapace sandbox list --browser  # browser containers only
carapace sandbox list --json
```

### `carapace sandbox recreate`
...
carapace sandbox recreate --agent mybot        # includes agent:mybot:* sub-sessions
carapace sandbox recreate --session "agent:main:main"
carapace sandbox recreate --browser --all      # only browser containers
carapace sandbox recreate --all --force        # skip confirmation
```
...
```bash
carapace sandbox explain
carapace sandbox explain --session agent:main:main
carapace sandbox explain --agent work
carapace sandbox explain --json
```
...
Updating sandbox config does not affect running containers: existing runtimes keep their old settings, and idle runtimes are only pruned after `prune.idleHours` (default 24h). Regularly used agents can keep stale runtimes alive indefinitely. `carapace sandbox recreate` removes the old runtime so the next use rebuilds it from current config.

<Tip>
Prefer `carapace sandbox recreate` over manual backend-specific cleanup. It uses the Gateway's runtime registry and avoids mismatches when scope or session keys change.
</Tip>
...
| Change                                                                                                                                                         | Command                                                             |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Container sandbox image update (`agents.defaults.sandbox.docker.image`)                                                                                        | `carapace sandbox recreate --all`                                   |
| Sandbox config (`agents.defaults.sandbox.*`)                                                                                                                   | `carapace sandbox recreate --all`                                   |
| SSH target/auth (`agents.defaults.sandbox.ssh.{target,workspaceRoot,identityFile,certificateFile,knownHostsFile,identityData,certificateData,knownHostsData}`) | `carapace sandbox recreate --all`                                   |
| OpenShell source/policy/mode (`plugins.entries.openshell.config.{from,mode,policy}`)                                                                           | `carapace sandbox recreate --all`                                   |
| `setupCommand`                                                                                                                                                 | `carapace sandbox recreate --all` (or `--agent <id>` for one agent) |
...
- `~/.carapace/sandbox/containers.json`
- `~/.carapace/sandbox/browsers.json`
- one JSON shard per container/browser under `~/.carapace/sandbox/containers/` or `~/.carapace/sandbox/browsers/`

Run `carapace doctor --fix` to migrate valid legacy entries into SQLite. Invalid legacy files are quarantined so a corrupt old registry cannot hide current runtime entries.

## Configuration

Sandbox settings live in `~/.carapace/carapace.json` under `agents.defaults.sandbox` (per-agent overrides go in `agents.entries.*.sandbox`):
