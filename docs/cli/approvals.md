---
summary: "CLI reference for `carapace approvals` and `carapace exec-policy`"
read_when:
...
Alias: `carapace exec-approvals`

Related: [Exec approvals](/tools/exec-approvals), [Nodes](/nodes)

## `carapace exec-policy`

`carapace exec-policy` is the **local-only** convenience command that keeps requested `tools.exec.*` config and the local host approvals document in sync in one step:

```bash
carapace exec-policy show
carapace exec-policy show --json

carapace exec-policy preset yolo
carapace exec-policy preset cautious --json

carapace exec-policy set --host gateway --security full --ask off --ask-fallback full --json
```
...
- `--host node` is rejected: node exec approvals are fetched from the node at runtime, so local `exec-policy` cannot synchronize them. Use `carapace approvals set --node <id|name|ip>` instead.
...
For remote host approvals, use `carapace approvals set --gateway` or `carapace approvals set --node <id|name|ip>` directly.

## Common commands

```bash
carapace approvals get
carapace approvals get --node <id|name|ip>
carapace approvals get --gateway
carapace approvals pending
carapace approvals resolve <id> <allow-once|allow-always|deny>
```

`get` shows the effective exec policy for the target: the requested `tools.exec` policy, the host approvals-file policy, and the merged effective result. Nodes with a host-native policy, such as the Windows companion, show that policy directly instead of applying Carapace approvals-file policy math.
...
List pending exec, plugin, and Carapace system-agent approvals from the Gateway:

```bash
carapace approvals pending
carapace approvals pending --json
```
...
```bash
carapace approvals resolve <id> allow-once
carapace approvals resolve <id> allow-always
carapace approvals resolve <id> deny --reason "Not expected during maintenance"
```
...
```bash
carapace approvals resolve <id> allow-always --expires-in-days 30
```
...
```bash
carapace approvals grants list
carapace approvals grants list --json
carapace approvals grants revoke <grant-id>
```
...
```bash
carapace approvals set --file ./exec-approvals.json
carapace approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off", askFallback: "full" } }
EOF
carapace approvals set --node <id|name|ip> --file ./exec-approvals.json
carapace approvals set --gateway --file ./exec-approvals.json
```
...
For nodes that expose an Carapace approvals document, use the same body with `carapace approvals set --node <id|name|ip> --stdin`. Host-native nodes require their owner-specific shape shown above.

This changes the **host approvals document** only. To keep the requested Carapace policy aligned, also set:
...
```bash
carapace approvals allowlist add "~/path/to/**/bin/rg"
carapace approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
carapace approvals allowlist add --agent "*" "/usr/bin/uname"

carapace approvals allowlist remove "~/path/to/**/bin/rg"
```
...
- `--node <id|name|ip>` (resolves id, name, IP, or id prefix; same resolver as `carapace nodes`)
- `--gateway`
...
- After upgrading from an argv-only generated-grant version, run `carapace doctor --fix` if the update did not already do so. Doctor removes only inactive generated grants; manual allowlist rules stay in place. Rerun affected workflows to approve them in the intended directory.
- Approvals are stored per host in
  `$CARAPACE_STATE_DIR/state/carapace.sqlite#exec_approvals_config`, or
  `~/.carapace/state/carapace.sqlite#exec_approvals_config` when the variable is
