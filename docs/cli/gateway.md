---
summary: "Carapace Gateway CLI (`carapace gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Carapace's WebSocket server (channels, nodes, sessions, hooks). All subcommands below live under `carapace gateway ...`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Carapace advertises and finds gateways.
  </Card>
...
```bash
carapace gateway
carapace gateway run   # equivalent, explicit form
```

<AccordionGroup>
  <Accordion title="Startup behavior">
    - Refuses to start unless `gateway.mode=local` is set in `~/.carapace/carapace.json`. Use `--allow-unconfigured` for ad-hoc/dev runs; it bypasses the guard without writing or repairing config.
    ...
    - If automatic migration cannot make the config valid, an interactive terminal can offer to run `carapace doctor --fix` and retry startup once after consent. Non-interactive runs print the command instead. If the repaired config is still invalid, startup remains stopped.
    - `carapace onboard --mode local` and `carapace setup` write `gateway.mode=local`. If the config file exists but `gateway.mode` is missing, that is treated as damaged/clobbered config and the Gateway refuses to guess `local` for you — re-run onboarding, set the key manually, or pass `--allow-unconfigured`.
    ...
    - `SIGUSR1` triggers an in-process restart when authorized. `commands.restart` (default: enabled) gates externally-sent `SIGUSR1`; set it to `false` to block manual OS-signal restarts. The agent-facing `gateway` tool is read-only; agents request restart through the `carapace` delegation tool. Effective Full Access, including Default (Full Access), authorizes permitted delegated changes without an approval prompt; restricted runs require human approval. See [Delegated setup and repair](/gateway/permission-modes#delegated-setup-and-repair).
...
<ParamField path="--token <token>" type="string">
  Shared token for `connect.params.auth.token`. Defaults to `CARAPACE_GATEWAY_TOKEN` when set.
</ParamField>
...
```bash
carapace gateway auth-token --show
```

The command resolves `gateway.auth.token`, `CARAPACE_GATEWAY_TOKEN`, and configured SecretRefs, then prints only the token. It requires an interactive terminal and refuses redirected or piped output so the credential does not silently enter command logs. Treat the terminal output as a secret.

If no persistent token is configured, run `carapace doctor --generate-gateway-token`, restart the Gateway, and then rerun the command. Generic `carapace config get` output remains redacted, including `--json`.

## Restart the Gateway

```bash
carapace gateway restart
carapace gateway restart --safe
carapace gateway restart --safe --skip-deferral
carapace gateway restart --force
carapace gateway restart --wait 30s
```
...
On Windows, a plain restart launched from a Gateway service process, including an agent's shell command, automatically uses the safe restart path. The running Gateway owns the deferred Scheduled Task handoff, so stopping its process tree cannot kill the caller before relaunch. This requires a reachable Gateway; the command acknowledges the restart request, not successor health. Use `carapace gateway status` afterward to verify recovery.

On macOS, when `carapace gateway restart`, `stop`, `install`, or `uninstall` runs inside the managed LaunchAgent's process tree, including an agent's shell command, Carapace detects that from launchd's service environment or, when a hand-written plist omits those variables, from process ancestry against the PID launchd reports for the job. Restart hands off to a detached helper so `kickstart -k` cannot kill the caller. Stop, install, and uninstall refuse and ask you to run the command from an external shell.
...
Service management (`install`, `start`, `stop`, `restart`, `uninstall`, Doctor service repair, and self-update service handling) belongs to the install that owns the host service. That is the canonical `.carapace` directory under the OS account home, or the `.carapace-<profile>` directory a named profile projects there. Named profiles use distinct native service identities.

`CARAPACE_HOME`, or an `CARAPACE_STATE_DIR` or `CARAPACE_CONFIG_PATH` that points elsewhere, is treated as isolated state and skipped. A relocated or copied state tree cannot adopt and rewrite the account's host service.
...
Named profiles must also use the native service identity derived from `CARAPACE_PROFILE`. Unset `CARAPACE_LAUNCHD_LABEL`, `CARAPACE_SYSTEMD_UNIT`, or `CARAPACE_WINDOWS_TASK_NAME` before service management; custom identities remain available for the default profile or runtime-only/external-supervisor setups.

On Linux, `carapace gateway install --force` refuses a sealed systemd service
...
Set `CARAPACE_SUPERVISOR_MODE=external` only when another process manager owns the Gateway lifecycle. In this mode:

- `carapace gateway restart` preserves the existing safe, forced, and bounded-wait behavior while targeting the verified running Gateway instead of launchd, systemd, or Task Scheduler. Exact-lock restart delivery runs inside that Gateway, so a replacement CLI does not migrate shared state before the old process hands off.
- Native service install, start, stop, and uninstall operations are refused with guidance to use the external supervisor.
- Carapace self-update is refused so the supervisor can stop the Gateway, replace and finalize the runtime, and restart it safely.
...
For upgrades and rollbacks, have the supervisor create a consolidated WAL-consistent copied snapshot with no SQLite sidecars, then run the target release's own `carapace database preflight <copied-state.sqlite> --json` before activation. Numeric schema versions alone do not prove that a same-version additive shape is compatible. See [Database schemas](/reference/database-schemas).

`CARAPACE_SERVICE_REPAIR_POLICY=external` remains a separate Doctor repair policy. It does not declare runtime ownership; supervisors that need both behaviors should set both variables.

External supervisors can negotiate and consume restart handoffs through the hidden machine contract:

```bash
carapace gateway restart-handoff capabilities --json
carapace gateway restart-handoff consume --expected-pid <pid> --json
```
...
- `CARAPACE_GATEWAY_STARTUP_TRACE=1` logs phase timings during startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings (installed-index, manifest registry, startup planning, owner-map work).
- `CARAPACE_GATEWAY_RESTART_TRACE=1` logs `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics. Ordinary stops also start a fresh trace with `stop.signal.received` and `stop.drain` timing. Named shutdown steps and coarse close phases emit `.begin` before waiting, then a duration when they settle; an unmatched begin identifies an entered phase that has not settled. These phases do not time every nested cleanup operation individually.
- `CARAPACE_DIAGNOSTICS=timeline` with `CARAPACE_DIAGNOSTICS_TIMELINE_PATH=<path>` writes a best-effort JSONL startup diagnostics timeline for external QA harnesses (equivalent to config `diagnostics.flags: ["timeline"]`; the path is still env-only). Add `CARAPACE_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
carapace gateway health --url ws://127.0.0.1:18789
carapace gateway health --port 18789
```
...
<ParamField path="--port <port>" type="number">
  Target a local loopback Gateway on this port. Overrides `CARAPACE_GATEWAY_URL` and `CARAPACE_GATEWAY_PORT` for this call.
</ParamField>
...
```bash
carapace gateway usage-cost
carapace gateway usage-cost --days 7
carapace gateway usage-cost --agent work --json
carapace gateway usage-cost --all-agents
carapace gateway usage-cost --json
```
...
```bash
carapace gateway stability
carapace gateway stability --type payload.large
carapace gateway stability --bundle latest
carapace gateway stability --bundle latest --export
carapace gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - Fatal Gateway exits, shutdown timeouts, and restart startup failures write the same diagnostic snapshot to `~/.carapace/logs/stability/carapace-stability-*.json` when the recorder has events. Inspect the newest bundle with `carapace gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` apply to bundle output too.
...
carapace gateway diagnostics export --output carapace-diagnostics.zip
carapace gateway diagnostics export --json
```
...
carapace gateway status --require-rpc
carapace gateway status --port 19001
```
...
carapace gateway probe --port 18789
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or Carapace could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this.
    ...
    - `local_tls_runtime_unavailable`: local Gateway TLS is enabled but Carapace could not load the local certificate fingerprint.
...
Carapace launches only an SSH client found in OS-managed system directories. On native Windows,
...
```bash
carapace gateway call status
carapace gateway call health --port 18999
carapace gateway call logs.tail --params '{"limit": 200}'
```
...
<ParamField path="--port <port>" type="number">
  Target a local loopback Gateway on this port. Overrides `CARAPACE_GATEWAY_URL` and `CARAPACE_GATEWAY_PORT` for this call. Cannot combine with `--url`.
</ParamField>
<ParamField path="--token <token>" type="string">
  Gateway token.
</ParamField>
...
`carapace.setup.detect` uses a 40-second default so the Gateway can finish its
...
```bash
carapace gateway suspend
carapace gateway suspend --request-id snapshot-2026-08-11 --wait 30
carapace gateway suspend --port 18999 --json
```
...
carapace gateway resume <suspensionId> --port 18999 --json
```
...
Carapace binary. Start and restart continue to validate plugin configuration.
...
`carapace gateway status --deep` from the account and profile that own it.
...
or unsupported values before reinstalling; Carapace cannot infer their intended
...
Gateway-hosted Carapace chat controls the exact Gateway serving that session.
...
After an unexpected disconnect, check `carapace gateway status` and the native
...
Use `--wrapper` when the managed service must start through another executable, for example a secrets manager shim or a run-as helper. The wrapper receives the normal Gateway args and is responsible for eventually exec'ing `carapace` or Node with those args.

```bash
cat > ~/.local/bin/carapace-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- carapace "$@"
EOF
chmod +x ~/.local/bin/carapace-doppler

carapace gateway install --wrapper ~/.local/bin/carapace-doppler --force
carapace gateway restart
```

You can also set the wrapper through the environment. `gateway install` validates that the path is an executable file, writes the wrapper into the service `ProgramArguments`, and persists `CARAPACE_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor repairs.

```bash
CARAPACE_WRAPPER="$HOME/.local/bin/carapace-doppler" carapace gateway install --force
carapace doctor
```

To remove a persisted wrapper, clear `CARAPACE_WRAPPER` while reinstalling:
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - If `gateway start` or `gateway restart` needs to repair a stale service definition, the command refuses when the invoking shell resolves a different state directory, config path, or port than the installed service. Match or unset the conflicting environment overrides, or use `carapace gateway install --force` to retarget the service intentionally.
  ...
  <Accordion title="Managed Gateway heap sizing">
    ...
    - Ambient installer `NODE_OPTIONS` and the installer's own Node arguments are not saved as Gateway heap settings. The budget is chosen at installation and takes effect when the service process starts; it is not recalculated while the Gateway runs. Upgrading Carapace alone does not resize a running Gateway, and foreground launches do not replace themselves to apply this policy.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `CARAPACE_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `CARAPACE_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_carapace-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (wide-area Bonjour): choose a domain (example: `carapace.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
carapace gateway discover --timeout 4000
carapace gateway discover --json | jq '.beacons[].wsUrl'
```
