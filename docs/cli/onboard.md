---
summary: "CLI reference for `carapace onboard` (interactive onboarding)"
read_when:
  - You want to establish inference, then finish setup with Carapace
title: "Onboard"
---
...
Carapace to configure the rest. `carapace setup` reaches this flow on fresh
systems or whenever an onboarding option is present; configured systems use
bare `carapace setup` for system-agent chat. `carapace setup --baseline` only
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Carapace onboarding fits together.
  </Card>
...
```bash
carapace onboard
carapace onboard --tui
carapace onboard --classic
carapace onboard --modern
carapace onboard --flow quickstart
carapace onboard --agent-name robby
carapace onboard --flow manual
carapace onboard --flow import
carapace onboard --import-from hermes --import-source ~/.hermes
carapace onboard --skip-bootstrap
carapace onboard recommendations --json
carapace onboard recommendations acknowledge
carapace onboard recommendations acknowledge --retry "<failed-id>"
carapace onboard recommendations refresh
carapace onboard --mode remote --remote-url wss://gateway-host:18789
```

`carapace onboard recommendations` reads pending app-recommendation matches
...
`carapace onboard recommendations refresh` clears the stored offer so the next
...
`carapace onboard recommendations acknowledge` marks the stored offer answered.
...
`carapace skills verify "@owner/slug"` succeeds for the same
publisher-qualified recommendation ID and its JSON output reports
`carapace.resolution.source: "installed"`. Registry verification alone is not
...
- `--agent-name <name>`: names the first agent when no roster exists. Interactive
  ...
  `carapace doctor --fix` first when creation reports legacy-session or
...
- `--modern` is a compatibility alias for the Carapace conversational setup
  assistant. It uses the same live-inference gate as `carapace setup` and
...
Plain `carapace onboard` starts the guided flow. It shows the security notice,
...
API-key or token methods use the same live completion path. Carapace persists
...
or AI chat. Rerun `carapace onboard` when you are ready to connect AI; interrupted
baseline setup resumes under its existing onboarding owner.

In guided mode, `--workspace <dir>` supplies Carapace's proposed workspace
and the isolated inference context. It is not persisted until you approve the
Carapace setup proposal. Classic and noninteractive onboarding persist their
...
the same memory-only scope. (A full [`carapace migrate`](/cli/migrate) run is
...
the same plan the conversational `carapace setup` chat would apply on "yes" —
...
If applying setup fails, onboarding falls back to the conversational Carapace
chat to finish interactively. Channels, agents,
plugins, and other optional features remain Carapace chat territory: run
`carapace` and use `open channel wizard for <channel>` to hand channel
credential collection to a masked terminal wizard. To change the model
provider or its authentication, exit Carapace and run `carapace onboard`;
Carapace does not open the guided or classic provider flows.

On a configured install, running `carapace onboard` again offers the current
...
Use `carapace onboard --classic` for provider-specific auth, channels, skills,
remote Gateway setup, imports, or full Gateway controls. For conversational
non-inference setup and repair, run `carapace setup`; `carapace onboard
...
Carapace will not start until its own live inference check passes.

In an interactive terminal, bare `carapace` (no subcommand) routes by config
state:
...
- If the config file exists but fails validation, it starts the classic
  onboarding path with `carapace doctor` guidance. Carapace needs working
...
- If the config file is valid, it opens the normal agent TUI. A reachable
  ...
  onboarding or Carapace. On a configured install, reach Carapace with
  `/carapace` inside the TUI or `carapace setup`.
...
another provider. Check the remote Gateway, then rerun bare `carapace` in the
connected terminal. If the error says the Gateway did not provide a boot identity,
update and restart that Gateway first.

Plaintext `ws://` is accepted for loopback, private IP literals, `.local`, and Tailnet `*.ts.net` gateway URLs. For other trusted private-DNS names, set `CARAPACE_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.

## Reset

```bash
carapace onboard --reset
carapace onboard --reset --reset-scope full
```
...
```bash
CARAPACE_LOCALE=zh-CN carapace onboard
CARAPACE_LOCALE=en carapace onboard # Explicit English override
```
...
Carapace's official plugin catalog. If the required plugin is already installed
but needs approval to enable it, use
`carapace plugins enable <plugin-id> --accept-capabilities` instead. The flag
...
`carapace channels add`. Bundled plugins do not need this review. See
...
- `--allow-unconfigured` is a separate `carapace gateway run` escape hatch; it does not let onboarding skip `gateway.mode`.

```bash
export OPENAI_API_KEY="your-provider-key"
export CARAPACE_GATEWAY_TOKEN="your-token"
carapace onboard --non-interactive --accept-risk --skip-health \
  --mode local \
  --auth-choice openai-api-key \
  --secret-input-mode ref \
...
- `--install-daemon` starts the managed gateway install path first. With no daemon flag, a local gateway must already be running (for example `carapace gateway run`).
...
With `--modern`, JSON is a one-shot Carapace overview and exits after that
...
- Fastest first chat: `carapace dashboard` (Control UI, no channel setup).
...
Use `carapace configure` later for targeted non-inference changes and `carapace
channels add` for channel-only setup. For model provider or auth route changes,
run `carapace onboard` instead.

```bash
carapace channels add
carapace configure
carapace agents add <name>
```
