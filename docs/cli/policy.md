---
summary: "CLI reference for `carapace policy` conformance checks"
read_when:
  - You want to check Carapace settings against an authored policy.jsonc
...
`carapace policy` is provided by the bundled Policy plugin. It is an enterprise
conformance layer over existing Carapace settings, not a second configuration
system. You author requirements in `policy.jsonc`; Carapace observes the active
...
does not attest per-agent credential stores such as `carapace-agent.sqlite`.
...
Separately, [`carapace agent exec`](/cli/agent#agent-exec) applies an isolated
...
```bash
carapace plugins enable policy
```
...
- Secret and auth-profile evidence records provider/source posture and
  ...
  per-agent credential stores such as `carapace-agent.sqlite`.
...
- Routing probes reuse Carapace's runtime binding resolver. Routing evidence
...
observed state is existing Carapace config or workspace metadata.
...
If an `agentIds` entry is not present in `agents.entries.*`, Carapace evaluates
...
| `routing.probes[].route`            | The public Carapace route resolver                  | Describe a representative inbound route without sending a message.     |
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.commands.deny`                 | Require exact node command ids such as `system.run` to be denied in Carapace config. |
...
denied by Carapace config. A deployment that intentionally allows a privileged
...
| Policy field                                        | Observed state                                                                                                   | Use when                                                               |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `dataHandling.sensitiveLogging.requireRedaction`    | Runtime invariant `oc://carapace.invariant/logging/redaction`                                                    | Set to `true` to record the requirement; Carapace always satisfies it. |
...
`~/.carapace/state/carapace.sqlite` by default, or the same database under
`$CARAPACE_STATE_DIR/state` when `CARAPACE_STATE_DIR` is set. Findings keep the
...
```bash
carapace policy check
carapace policy check --agent ops
carapace policy check --json
carapace policy check --severity-min error
```
...
`carapace doctor --lint` when the Policy plugin is enabled.
...
without the flag; Carapace never selects an arbitrary first agent.

Compare an operator policy file against an authored baseline:

```bash
carapace policy compare --baseline official.policy.jsonc
carapace policy compare --baseline official.policy.jsonc --agent ops
carapace policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```
...
```json
{
  ...
  "evidence": {
    ...
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://carapace.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://carapace.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.6-sol",
        "source": "oc://carapace.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://carapace.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://carapace.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "source": "oc://carapace.config/agents/defaults/sandbox/workspaceAccess",
        "scope": "defaults",
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://carapace.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
        ...
        "kind": "toolDeny",
        "source": "oc://carapace.config/tools/deny",
        "scope": "defaults",
        "tool": "exec",
        "denied": true,
        "explicit": true
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://carapace.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://carapace.config/models/providers/openai/apiKey",
        "kind": "input",
        "source": "oc://carapace.config/models/providers/openai/apiKey",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://carapace.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Carapace state used by the checks, and
...
`carapace policy watch` re-runs the check and reports when current evidence no
longer matches `expectedAttestationHash`:

```bash
carapace policy watch --json
carapace policy watch --agent ops --json
```
...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by Carapace config.                 |
...
```json
{
  ...
  "ocPath": "oc://carapace.config/channels/telegram",
  "target": "oc://carapace.config/channels/telegram",
...
```json
{
  ...
  "target": "oc://carapace.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://carapace.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "source": "policy",
  "path": "carapace config",
  "ocPath": "oc://carapace.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
  "target": "oc://carapace.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "source": "policy",
  "path": "carapace config",
  "ocPath": "oc://carapace.config/gateway/bind",
  "target": "oc://carapace.config/gateway/bind",
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by Carapace config.",
  "source": "policy",
  "path": "carapace config",
  "ocPath": "oc://carapace.config/gateway/nodes/commands/deny",
  "target": "oc://carapace.config/gateway/nodes/commands/deny",
...
```json
{
  ...
  "source": "policy",
  "path": "carapace config",
  "ocPath": "oc://carapace.config/agents/defaults/sandbox/workspaceAccess",
  "target": "oc://carapace.config/agents/defaults/sandbox/workspaceAccess",
...
Sensitive log redaction is unconditional in Carapace, so nothing can report it
as disabled. The key stays a supported policy rule: `carapace policy` validates
its shape, `carapace policy compare` still requires a candidate policy to be at
least as strict as the baseline for it, and `carapace policy check` records the
runtime invariant `oc://carapace.invariant/logging/redaction` in the
