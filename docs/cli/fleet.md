---
summary: "CLI reference for provisioning and managing isolated per-tenant Carapace cells"
read_when:
...
`carapace fleet` manages complete Carapace instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Fleet supports Docker and Podman. The default image is `ghcr.io/carapace/carapace:latest`.
...
```bash
carapace fleet create acme
carapace fleet status acme
carapace fleet list
```
...
The ID becomes part of the container name: `carapace-cell-<tenant>`.
...
carapace fleet create acme \
  ...
  --env CARAPACE_DISABLE_BONJOUR=1
```
...
Fleet defaults `XDG_CACHE_HOME` to `/home/node/.carapace/cache` on the tenant state mount. An explicit `--env XDG_CACHE_HOME=...` value survives upgrade and restore, including a value equal to the current default.

### Create options

| Option                    | Default                               | Description                                                                                    |
| ------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `--image <ref>`           | `ghcr.io/carapace/carapace:latest`    | Container image for the cell.                                                                  |
...
Create and upgrade accept digest-pinned image references such as `--image ghcr.io/carapace/carapace@sha256:<digest>`. Fleet passes the image reference through verbatim to Docker or Podman, which lets an operator keep a cell on immutable image bytes instead of a moving tag.
...
```bash
carapace fleet status acme
carapace fleet status acme --json
```
...
```bash
carapace fleet logs acme
carapace fleet logs acme --follow
carapace fleet logs acme --tail 200
carapace fleet logs acme --since 10m
```
...
```bash
carapace fleet start acme
carapace fleet stop acme
carapace fleet restart acme
```
...
```bash
carapace fleet upgrade acme
```

Move the cell to another image:

```bash
carapace fleet upgrade acme --image ghcr.io/carapace/carapace:<version>
```
...
The Gateway token is intentionally not stored in the fleet registry. Before removing the old container, Fleet reads its environment and carries `CARAPACE_GATEWAY_TOKEN` into the replacement. Do not manually remove the old container before an upgrade if the token exists nowhere else you control.
...
```bash
carapace fleet stop acme
carapace fleet backup acme --out ./acme.tgz
```

Restore that archive into the registered cell:

```bash
carapace fleet restore acme --from ./acme.tgz
```
...
```bash
carapace fleet doctor
carapace fleet doctor acme --json
```
...
```bash
carapace fleet rm acme
```

A running container requires `--force`:

```bash
carapace fleet rm acme --force
```

Permanently remove the cell data as well:

```bash
carapace fleet rm acme --purge-data --force
```
...
Cell state and the legacy auth-profile encryption key use separate per-tenant host paths under the active Carapace state directory:
...
The first directory is mounted at `/home/node/.carapace`. The second is mounted at `/home/node/.config/carapace`, matching the official Docker setup's legacy OAuth migration-key mount. Both directories survive normal removal and upgrade; `fleet rm --purge-data --force` deletes both after separate containment checks.
...
| `XDG_CACHE_HOME`         | `/home/node/.carapace/cache`         |
| `CARAPACE_GATEWAY_TOKEN` | Generated or supplied cell token     |

`XDG_CACHE_HOME` is the one overrideable generated default in this table. The other listed container-path variables and `CARAPACE_GATEWAY_TOKEN` remain Fleet-managed.
