---
summary: "CLI reference for `carapace agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
Manage isolated agents (workspaces + auth + routing). Running `carapace agents` with no subcommand is equivalent to `carapace agents list`.
...
carapace agents add work --workspace ~/.carapace/workspace-work
carapace agents add work --workspace ~/.carapace/workspace-work --bind telegram:*
carapace agents add ops --workspace ~/.carapace/workspace-ops --bind telegram:ops --non-interactive
carapace agents bindings
carapace agents bind --agent work --bind telegram:ops
carapace agents unbind --agent work --bind telegram:ops
carapace agents set-identity --workspace ~/.carapace/workspace --from-identity
carapace agents set-identity --agent main --avatar avatars/carapace.png
carapace agents delete work
```
...
- `main` is an ordinary agent id. Recreating it after another agent owns the installation can require `carapace doctor --fix` to repair legacy session or shared-auth ownership first.
...
- On installations that have not migrated shared auth yet, the legacy owner cannot be deleted. Run `carapace doctor --fix`; after relocation into shared state SQLite, `main` follows the same deletion rules as any other agent.
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.entries.*.skills` in `carapace.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents/workspace-and-bootstrap#agents-defaults-skills).

List bindings:

```bash
carapace agents bindings
carapace agents bindings --agent work
carapace agents bindings --json
```

Add bindings:

```bash
carapace agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
carapace agents add work --workspace ~/.carapace/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), Carapace resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, Carapace targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, Carapace upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
carapace agents bind --agent work --bind telegram:*
...
carapace agents bind --agent work --bind telegram:alerts
```
...
```bash
carapace agents unbind --agent work --bind telegram:ops
carapace agents unbind --agent work --all
```
...
- Example path: `~/.carapace/workspace/IDENTITY.md`
...
- Relocate an existing agent with `carapace config set agents.entries.<id>.workspace <dir>`, then follow the CLI restart hint and confirm with `carapace agents list`.
...
```bash
carapace agents set-identity --agent main --name "Carapace" --emoji "🦞" --avatar avatars/carapace.png
```

Relocate the stored workspace:

```bash
carapace config set agents.entries.work.workspace ~/.carapace/workspace-work
carapace agents list
```
