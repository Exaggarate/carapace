---
summary: "CLI reference for `carapace attach` (launch Claude Code with a scoped Gateway MCP grant)"
read_when:
  - You want Claude Code to use Carapace Gateway MCP tools
  - You need a temporary session-bound MCP grant for an external harness
title: "Attach CLI"
---

`carapace attach` launches Claude Code with a strict temporary MCP config bound to one Gateway session.

```sh
carapace attach [target]
```
...
```sh
carapace attach
carapace attach https://gateway.example/dashboard/main/movies-a1166b81
carapace attach movies-a1166b81
carapace attach --session agent:main:telegram:123 --ttl 600000
carapace attach --print-config
```
...
The bearer token is passed through environment variables, not argv. Carapace launches Claude Code with `--strict-mcp-config --mcp-config <path>` so ambient Claude MCP servers do not join the attached session. Normal launches (without `--print-config`) revoke the grant when the Claude Code process exits.
...
session but does not identify an active Carapace run or its original caller
