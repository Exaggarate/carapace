---
summary: "CLI reference for `carapace proxy`, including operator-managed proxy validation and the local debug proxy capture inspector"
read_when:
  - You need to validate operator-managed proxy routing before deployment
  - You need to capture Carapace transport traffic locally for debugging
...
```bash
carapace proxy validate [--json] [--proxy-url <url>] [--proxy-ca-file <path>] [--allowed-url <url>] [--denied-url <url>] [--apns-reachable] [--apns-authority <url>] [--timeout-ms <ms>]
carapace proxy start [--host <host>] [--port <port>]
carapace proxy run [--host <host>] [--port <port>] -- <cmd...>
carapace proxy coverage [--json]
carapace proxy sessions [--limit <count>] [--json]
carapace proxy query --preset <name> [--session <id>] [--json]
carapace proxy blob --id <blobId>
carapace proxy purge
```
...
Checks the effective operator-managed proxy URL from `--proxy-url`, config (`proxy.proxyUrl`), or `CARAPACE_PROXY_URL`, in that precedence order. Reports a config problem if no proxy is enabled and configured; pass `--proxy-url` for a one-off preflight without touching config.

Managed proxy URLs use `http://` for a plain forward-proxy listener, or `https://` when Carapace must open TLS to the proxy endpoint itself before sending proxy requests. Use `--proxy-ca-file` to trust a private CA for that TLS connection.
...
The debug proxy's direct upstream forwarding opens upstream sockets for diagnostics. When Carapace managed proxy mode is active, direct forwarding for proxy requests and CONNECT tunnels is disabled by default; set `CARAPACE_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY=1` only for approved local diagnostics.
