---
summary: "CLI reference and security model for the inference-backed Carapace setup and repair helper"
read_when:
  - You finished inference setup and want Carapace to configure the rest
  - You need to inspect or repair Carapace with the local setup agent
  - You are designing or enabling message-channel rescue mode
title: "Carapace setup agent"
---

# `carapace setup`

Carapace ships with a built-in system agent — it speaks as "Carapace" — for
...
Running `carapace` with no subcommand routes based on config state:
...
- Config exists but fails validation: starts classic onboarding, which reports the issues and directs you to `carapace doctor`.
- Config exists and is valid: opens the normal agent TUI. A reachable
  ...
  without onboarding or Carapace. Use `/carapace` inside the TUI, or run
  `carapace setup` directly, to reach Carapace later.

Running `carapace setup` first live-tests the configured default model. A passing turn starts Carapace. An interactive failure opens guided inference setup and hands off to Carapace after a candidate passes. One-shot, JSON, and other noninteractive requests fail with instructions to run `carapace onboard` when inference is unavailable. `carapace --help` and `carapace --version` keep their normal fast paths.

If inference plugin loading or owner verification fails, the error includes the underlying cause after applying Carapace's error redaction. One-shot text and JSON output retain that detail alongside onboarding guidance.

Noninteractive bare `carapace` (no TTY) exits with a short message instead of printing root help: it points to non-interactive onboarding on a fresh or invalid install, or to `carapace agent --local ...` when config is valid.

`carapace onboard --modern` remains a compatibility alias for Carapace, but uses the same inference gate: working inference opens the chat, interactive failures start guided inference setup, and noninteractive failures exit with onboarding guidance. `carapace onboard --classic` opens the full step-by-step wizard.

## What Carapace shows

Interactive Carapace opens the same TUI shell as `carapace tui`, with an Carapace chat backend. The startup greeting covers:

- config validity and the default agent
- the verified model Carapace is using
...
Carapace uses the same reference discovery as regular agents: in a Git checkout it points at local `docs/` and the source tree; in an npm install it uses bundled docs and links to [https://github.com/Exaggarate/carapace](https://github.com/Exaggarate/carapace), with guidance to check source when docs are not enough.
...
carapace setup
carapace setup --json
carapace setup --message "models"
carapace setup --message "validate config"
carapace setup --message "setup workspace ~/path/to/work" --yes
carapace setup --message "set default model openai/gpt-5.6" --yes
carapace onboard --modern
```

Inside the Carapace TUI:
...
config set-ref gateway.auth.token env CARAPACE_GATEWAY_TOKEN
gateway status
configure gateway
open gateway wizard
restart gateway
...
plugin install clawhub:carapace-codex-app-server
talk to work agent
...
Carapace uses typed operations instead of editing config ad hoc.
...
In direct Carapace chat, persistent operations require conversational approval (or `--yes` for a one-shot command): write config, `config set`, `config set-ref`, setup/onboarding bootstrap, change the default model, start/stop/restart the Gateway, create agents, and install plugins.
...
mode. Restricted runs still require approval in the Carapace operator UI. Replying
"yes" in the delegated chat cannot authorize a change. When approval is required,
run `carapace dashboard` on the Gateway host to review it, or run the change
directly with `carapace setup` there. Independent filesystem and sandbox boundaries,
...
Configured agents can ask Carapace to create another agent through their
`carapace` tool. The request enters the same typed create-agent operation and
host authorization flow; any approval summary names the requesting agent.
Carapace remains the executor, and authorized creation records that requesting
agent as the new agent's creator.

Delegated creation remains tied to the requesting run. If that run ends or loses
authority during preparation, Carapace stops before starting the next persistent
write. A write already in progress may finish, and workspace files created earlier
are not automatically removed. Check `carapace agents list` before retrying from
an active run; an agent whose creation already completed is not removed when its
requesting run ends.

Doctor repairs are unavailable inside Carapace because they can rewrite the provider, authentication, or default-agent inference route powering the session. Exit Carapace and run `carapace doctor --fix` in a terminal. Read-only `doctor` remains available inside Carapace.

New agents inherit the live-verified default inference route. The agent ids `carapace` and `crestodian` are reserved for the system agent and cannot be created as normal agents. The retired id remains blocked so an old config cannot claim it.
...
configure or repair provider/auth access, exit Carapace and run
`carapace onboard`.
...
refused for the same reason; exit Carapace and run
`carapace plugins uninstall <id>` from a terminal.
...
The Ask Carapace page can show recent applied system-agent operations, Doctor
migrations, Settings and CLI config writes, and manual edits to
`carapace.json`. The config journal detects external edits while the Gateway
is watching, during an Carapace-owned write, or at the next startup after an
offline edit.

History is stored in the `diagnostic_events` table of the shared
`~/.carapace/state/carapace.sqlite` database, under the `system-agent-audit`
...
until they reach a secret. The local Carapace TUI does not accept sensitive wizard answers
...
`carapace channels add --channel <channel>` or
`carapace configure --section web` or `carapace configure --section gateway`
later.
...
when it finishes, run `carapace gateway restart` to apply the saved settings.

Carapace never changes provider/auth access from inside its own session: the
...
starting a wizard or writing config. Exit Carapace and run `carapace
onboard`; onboarding stages the credentials and saves only a route that
completes a real live turn. Start Carapace again after onboarding succeeds.
...
Carapace stops before starting the next persistent effect. Earlier completed
effects remain, including an agent whose creation already finished; setup may
still be incomplete. Check `carapace agents list` and `status`, then request
setup again from an active run and approve the new request, or finish directly
with `carapace setup` on the Gateway host. If cancellation deferred legacy
history migration for a newly named agent, the next Gateway startup retries it;
use `carapace doctor --fix` on the same state/config to finish it sooner.

If inference is missing or its live check fails, leave Carapace and run `carapace onboard`. Guided onboarding tries the configured model first, then authenticated subscription CLIs, API keys, and remaining supported CLIs; it asks each candidate for a real reply and persists only a passing route. Carapace starts immediately after that boundary and can then configure the workspace, Gateway, channels, agents, plugins, and other optional features.
...
the `carapace.setup.detect` and `carapace.setup.activate` Gateway methods:
...
credential, and provider/runtime state needed for that route after the test passes. Workspace and Gateway defaults remain for Carapace. A failing candidate
...
working and Carapace has started; existing plugin policy and explicit
supervision opt-outs remain untouched during inference setup.

## AI conversation

Interactive Carapace's free-form conversation runs through the same agent loop as regular Carapace agents, restricted to one ring-zero Carapace authority tool, `carapace`, that wraps the typed operations. Read actions run freely, mutations require your conversational approval for that exact operation (see Operations and approval), and every applied write is audited and re-validated. The agent session persists, so Carapace has real multi-turn memory. If the verified inference route later stops working, return to `carapace onboard` and repair it before continuing.
...
restriction directly: the run carries an Carapace tool allow-list with only
the `carapace` tool. For Codex, Carapace also disables environments, native
...
harnesses do not consume Carapace's allow-list,
so Carapace admits only backends whose own tool-selection contract can prove
the same restriction:

- Selectable backends, including Claude Code, launch with an empty native-tool
  selection and one MCP tool, `carapace`. Claude's generated MCP config is
  applied with `--strict-mcp-config`, so no other MCP servers are loaded.
- Backends that declare no native tools receive the same dedicated Carapace
  MCP server.
- Always-on or unknown native-tool backends fail closed before inference; they
  cannot host an Carapace session.

Only Carapace sessions get the carapace MCP server; normal agent runs
...
a single Carapace authority tool plus the inert native planning utility. In all
three cases, setup writes remain confined to Carapace's audited approval
contract.
...
Use a natural-language selector to leave Carapace and open the normal TUI:
...
`carapace tui`, `carapace chat`, and `carapace terminal` open the normal agent TUI directly; they do not start Carapace. After switching into the normal TUI, `/carapace` returns to Carapace, optionally with a follow-up request:

```text
/carapace
/carapace restart gateway
```

## Message rescue mode

Message rescue mode is the message-channel entrypoint for Carapace: use it when your normal agent is dead but a trusted channel (for example WhatsApp) still receives commands.

This is a deterministic emergency command handler, not the conversational
Carapace agent. It does not bootstrap a fresh setup or relax the inference
gate for Carapace chat.

Supported command: `/carapace <request>`. Rescue accepts the exact typed command grammar only — natural language is rejected with a hint, never guessed into an operation, and no model is ever consulted.

```text
You, in a trusted owner DM: /carapace status
Carapace: Carapace rescue mode. Gateway reachable: no. Config valid: no.
You: /carapace restart gateway
Carapace: Plan: restart the Gateway. Reply /carapace yes to apply.
You: /carapace yes
Carapace: Applied. Audit entry written.
```
...
/carapace create agent work workspace ~/path/to/work
```
...
- Disabled when sandboxing is active for the agent/session; Carapace refuses remote rescue and points to local CLI repair.
...
- Plugin search and list are read-only. Plugin install is always local-only (blocked in rescue, even when otherwise enabled) because it downloads executable code. Plugin uninstall is refused in both local Carapace and rescue; run `carapace plugins uninstall <id>` from a terminal.
- Remote rescue cannot open the local TUI or switch into an interactive agent session; use local `carapace` for agent handoff.
...
expire after 15 minutes. `carapace doctor --fix` removes the retired
...
An opt-in live channel command-surface smoke checks `/carapace status` plus a persistent approval roundtrip through the rescue handler:
...
That packaged-CLI lane starts with an empty state dir and proves Carapace
...
Carapace agent/tool/approval conversation. The QA Lab scenario below redirects
