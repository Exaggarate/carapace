---
summary: "CLI reference for `carapace devices` (device pairing + token rotation/revocation)"
read_when:
...
### `carapace devices approve [requestId] [--latest]`
...
```bash
carapace devices approve
carapace devices approve <requestId>
carapace devices approve --latest
```

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Carapace supersedes the previous pending entry with a new `requestId`. Run `carapace devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, Carapace keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `carapace devices list`, or preview with `--latest`, before approving.
...
```bash
carapace devices join-code
carapace devices join-code --json
```

### `carapace devices remove <deviceId>`

Remove one paired device entry.

```bash
carapace devices remove <deviceId>
carapace devices remove <deviceId> --json
```
...
### `carapace devices rename --device <id> --name <label>`
...
```bash
carapace devices rename --device <deviceId> --name "Kitchen Mac"
carapace devices rename --device <deviceId> --name "Kitchen Mac" --json
```
...
### `carapace devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
carapace devices clear --yes
carapace devices clear --yes --pending
carapace devices clear --yes --pending --json
```

`--pending` also rejects all pending pairing requests.

### `carapace devices rotate --device <id> --role <role> [--scope <scope...>]`

Rotate a device token for a role, optionally updating its scopes.

```bash
carapace devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `carapace devices revoke --device <id> --role <role>`

Revoke a device token for a role.

```bash
carapace devices revoke --device <deviceId> --role node
```
...
## Paperclip / `carapace_gateway` first-run approval

Paperclip agents connecting through the `carapace_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `carapace_gateway_pairing_required`, approve the pending device and retry.

```bash
carapace devices approve --latest
```

The preview prints the exact `carapace devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
carapace devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `carapace devices list` first to confirm a pending request exists.
