---
summary: "Back up Carapace state: archives, per-database snapshots, scheduling, offsite copies, and continuous replication"
read_when:
  - You want a backup routine for an Carapace install instead of a one-off archive
  - You want scheduled, offsite, or continuous backups without copying the whole database every time
  - You need to restore Carapace state from a backup
title: "Backups"
---

# Backups

Carapace keeps its authoritative state in SQLite: one global control-plane
database under the state directory (usually `~/.carapace`), plus one database
per configured agent at `<agentDir>/carapace-agent.sqlite`. Agent directories
...
- One-off, everything, portable: `carapace backup create` archive.
- One database, compact and verified: `carapace backup sqlite create`.
- Versioned and incremental by content: `carapace backup git create`.
...
the state directory. Carapace-owned SQLite databases, including agent databases
...
`carapace backup restore` as the disaster-recovery primitive for rebuilding a
...
SHA-256 that `carapace backup sqlite verify` rechecks later.
...
```bash
carapace backup git init --repository ~/Backups/carapace-git --remote git@github.com:you/carapace-backups.git
...
```bash
0 3 * * * carapace backup sqlite create --global --repository "$HOME/Backups/carapace-sqlite" --json >> "$HOME/Backups/carapace-backup.log" 2>&1
5 3 * * * carapace backup sqlite create --agent main --repository "$HOME/Backups/carapace-sqlite" --json >> "$HOME/Backups/carapace-backup.log" 2>&1
```
...
also recorded in the shared state database. `carapace status` shows the newest
attempt, and `carapace doctor` suggests a one-off or scheduled backup when no
...
```bash
rclone sync ~/Backups/carapace-sqlite remote:carapace-backups/sqlite
```
...
pushes only content changes by construction. Carapace stages only the
...
Use a repository dedicated to Carapace backups. Existing `global/` and
`agents/<agentId>/` scopes must be empty or contain a valid schema-version-1
Carapace backup manifest. Carapace refuses to replace any other scope, and an
...
carapace backup git restore --repository ~/Backups/carapace-git --ref <commit> --agent main --target ./restored-agent.sqlite
```
...
SQLite. It runs alongside the Gateway with no Carapace changes: it watches
...
Litestream's one hard requirement is WAL mode, which Carapace uses on local
filesystems; on network-backed storage such as NFS or SMB, Carapace falls
...
```yaml
dbs:
  - path: /home/user/.carapace/state/carapace.sqlite
    replicas:
      - url: s3://carapace-backups/state
  - path: /home/user/.carapace/agents/main/agent/carapace-agent.sqlite
    replicas:
      - url: s3://carapace-backups/agents/main
```
...
```bash
litestream restore -o ./restored-carapace.sqlite s3://carapace-backups/state
```

For an agent with a custom `agentDir`, replace the example's default agent
database path with its configured `<agentDir>/carapace-agent.sqlite`.
...
required on the origin. Carapace uses WAL on local filesystems but
...
```bash
sqlite3 ~/.carapace/state/carapace.sqlite "PRAGMA journal_mode;"
```
...
```bash
sqlite3_rsync 'user@gateway-host:~/.carapace/state/carapace.sqlite' ./replica/carapace.sqlite
```
...
Two caveats. First, deltas are page-based, and Carapace's databases run
...
Start only from an archive you created or otherwise trust. `carapace backup
...
```bash
ARCHIVE=./2026-03-09T08-00-00.000+08-00-carapace-backup.tar.gz
carapace backup restore "$ARCHIVE" --target ./restored-carapace
```

The target must not exist or must be empty, and it must not be inside the live
state directory or any configured live agent directory. Carapace verifies
...
<Warning>
  ...
  reinstall with `carapace plugins install <spec> --force`. Run `carapace
...
state asset into place, or point `CARAPACE_STATE_DIR` at that asset. Restore
...
Run `carapace doctor` before restarting the Gateway. See
[Updating](/install/updating#rollback) for the rollback workflow.

### Restore a database

For a snapshot, `carapace backup sqlite restore <snapshot-directory> --target
<new-database-path>` writes a re-verified database to a fresh target. For Git
history, `carapace backup git restore --repository <dir> --ref <commit>
...
start the Gateway and check `carapace health` and `carapace doctor`.

After restoring onto a different Carapace version, preflight the database
first with `carapace database preflight`; see
