---
summary: "Deploy Carapace on Railway with one-click template"
read_when:
  - Deploying Carapace to Railway
  - You want a one-click cloud deploy with browser-based Control UI
title: "Railway"
---

Deploy Carapace on Railway with a one-click template and access it through the web Control UI. This is the easiest "no terminal on the server" path: Railway runs the Gateway for you.
...
<Step title="Add a volume">
  ...
  <Step title="Set variables">
    ...
    - `CARAPACE_GATEWAY_PORT=8080` (required -- must match the port in Public Networking)
    - `CARAPACE_GATEWAY_TOKEN` (required; treat as an admin secret)
    - `CARAPACE_STATE_DIR=/data/.carapace` (required for persistent state)
    - `CARAPACE_WORKSPACE_DIR=/data/workspace` (required for a persistent workspace)
...
<Step title="Enable public networking">
  ...
  <Step title="Connect">
    ...
    Open `https://<your-railway-domain>/carapace` and connect using the configured shared secret. The template uses `CARAPACE_GATEWAY_TOKEN` by default; if you replace it with password auth, use that password instead.
...
- Persistent storage via the Railway Volume (`/data`), so `carapace.json`, shared and per-agent SQLite auth stores, channel/provider state, sessions, and workspace survive redeploys

## Connect a channel

Use the Control UI at `/carapace` or run `carapace onboard` via Railway's shell for channel setup instructions:
