...
read_when:
  - You want to use Hugging Face Inference with Carapace
  - You need the HF token env var or CLI auth choice
title: "Hugging Face (inference)"
---

[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers) exposes an OpenAI-compatible chat completions router in front of many hosted models (DeepSeek, Llama, and more) under one token. Carapace talks to the **chat completions endpoint only**; for text-to-image, embeddings, or speech use the [HF inference clients](https://huggingface.co/docs/api-inference/quicktour) directly.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    carapace onboard --auth-choice huggingface-api-key
    ```

  </Step>
  <Step title="Select a default model">
    In the **Default Hugging Face model** dropdown, pick a model. The list loads from the Inference API when your token is valid; otherwise Carapace shows the built-in catalog below. Your choice is saved as `agents.defaults.model.primary`:
  ...
  <Step title="Verify the model is available">
    ```bash
    carapace models list --provider huggingface
    ```
  </Step>
</Steps>
...
Model refs use the form `huggingface/<org>/<model>` (Hub-style IDs). Carapace's built-in catalog:
...
<Tip>
When your token is valid, Carapace also discovers any other model from **GET** `https://router.huggingface.co/v1/models` at onboarding time and Gateway startup, so your catalog can include far more than the three models above. You can append `:fastest` or `:cheapest` to any model id; HF's router routes to the matching inference provider. Set your default provider order in [Inference Provider settings](https://hf.co/settings/inference-providers).
</Tip>
...
<AccordionGroup>
  <Accordion title="Model discovery and onboarding dropdown">
    Carapace discovers models with:
    ...
    ```bash
    carapace config set plugins.entries.huggingface.config.discovery.enabled false
    ```

  </Accordion>

  <Accordion title="Model names, aliases, and policy suffixes">
    - **Name from API:** discovered models use the API's `name`, `title`, or `display_name` when present; otherwise Carapace derives a name from the model id (e.g. `deepseek-ai/DeepSeek-R1` becomes "DeepSeek R1").
    ...
    - **Policy suffixes:** `:fastest` and `:cheapest` are HF router conventions, not something Carapace rewrites: the suffix is sent verbatim as part of the model id and HF's router picks the matching inference provider. Add each variant as its own entry under `models.providers.huggingface.models` (or in `model.primary`) if you want a distinct alias per suffix.
  ...
  <Accordion title="Environment and daemon setup">
    If the Gateway runs as a daemon (launchd/systemd), make sure `HUGGINGFACE_HUB_TOKEN` or `HF_TOKEN` is available to that process (for example, in `~/.carapace/.env` or via `env.shellEnv`).

    <Note>
    Carapace accepts both `HUGGINGFACE_HUB_TOKEN` and `HF_TOKEN`. If both are set, `HUGGINGFACE_HUB_TOKEN` takes precedence.
    </Note>
