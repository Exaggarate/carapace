---
summary: "Use Vydra image, video, and speech in Carapace"
read_when:
  - You want Vydra media generation in Carapace
...
Carapace uses the same `VYDRA_API_KEY` for all three capabilities.

| Property        | Value                                                                     |
| --------------- | ------------------------------------------------------------------------- |
| Provider id     | `vydra`                                                                   |
| Plugin          | `@carapace/vydra-provider`                                                |
...
<Steps>
  ...
  <Step title="Run interactive onboarding">
    ```bash
    carapace onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export CARAPACE_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
