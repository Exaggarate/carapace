---
summary: "fal image, video, and music generation setup in Carapace"
title: "Fal"
read_when:
  - You want to use fal image generation in Carapace
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

Carapace ships a bundled `fal` provider for hosted image, video, and music
generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    carapace onboard --auth-choice fal-api-key
    ```
...
Krea 2 models use fal's native Krea payload schema. Carapace sends
...
`aspectRatio`; Carapace maps `size` to the closest supported Krea aspect ratio
...
control in Carapace, so `background: "transparent"` is reported as an ignored
override for fal models.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
Carapace rejects `outputFormat` overrides for Krea requests.
