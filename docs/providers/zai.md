---
summary: "Use Z.AI (GLM models) with Carapace"
read_when:
  - You want Z.AI / GLM models in Carapace
...
Carapace uses the `zai` provider with a Z.AI API key.

| Property | Value                                        |
| -------- | -------------------------------------------- |
| Provider | `zai`                                        |
| Package  | `@carapace/zai-provider`                     |
| Auth     | `ZAI_API_KEY` (legacy alias: `Z_AI_API_KEY`) |
| API      | Z.AI Chat Completions (Bearer auth)          |

## GLM models

GLM is a model family, not a separate provider. In Carapace, GLM models use
...
```bash
carapace plugins install @carapace/zai-provider
```

<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Carapace probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        carapace onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        carapace models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        carapace onboard --auth-choice zai-coding-cn

        # General API
        carapace onboard --auth-choice zai-global

        # General API CN (China region)
        carapace onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
`https://api.z.ai/api/anthropic`. Carapace's Z.AI choices use the documented
...
- [General-purpose agent tools](https://docs.z.ai/devpack/tool/others),
  including Carapace, are served on a best-effort basis. During high inference
...
```bash
carapace models list --all --provider zai
carapace config get models.providers.zai.baseUrl
```
...
<Tip>
`zai-api-key` lets Carapace detect the matching Z.AI endpoint from the key and
...
| `zai/glm-5-turbo`   | Carapace-optimized text model; 200K context        |
...
GLM-5.3. Run
`carapace models list --all --provider zai` to see the catalog known to your
installed version.
</Note>
...
<Tabs>
  <Tab title="GLM-5.3 and Flash">
    Levels: `low`, `high`, and `max` (default `max`). Carapace maps these to
  ...
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). Carapace maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, Carapace sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Carapace copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Carapace config schema, including provider and model settings.
  </Card>
</CardGroup>
