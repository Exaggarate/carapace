...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    carapace config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
  ...
  <Step title="Start searching">
    ...
    ```bash
    carapace config set tools.web.search.provider perplexity
    ```
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    ...
    imported. Set the key in `~/.carapace/.env` or via `env.shellEnv` so the
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    (prefix `sk-or-`) instead of a native Perplexity key. Carapace detects the
