---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with Carapace"
read_when:
  - You want ElevenLabs text-to-speech in Carapace
...
Carapace uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | Carapace surface                                                     | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. Carapace keeps
...
returned audio stream instead of waiting for Carapace to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; Carapace omits that parameter for
...
Carapace sends multipart audio to ElevenLabs `/v1/speech-to-text` with
