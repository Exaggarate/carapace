...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with Carapace
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. Carapace provides the
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carapace onboard --auth-choice longcat-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    carapace models list --provider longcat
    ```
  </Step>
</Steps>
...
LongCat exposes binary thinking control. Carapace maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. LongCat does not currently document
`reasoning_effort`, so Carapace does not send it.

LongCat returns reasoning in `reasoning_content`. Carapace preserves that field
...
model through an OpenAI-compatible runtime and use Carapace's existing
...
<AccordionGroup>
  <Accordion title="The key works in a shell but not in the Gateway">
    ...
    variable. Put `LONGCAT_API_KEY` in `~/.carapace/.env`, configure it through
  ...
  <Accordion title="The model does not appear">
    Run `carapace plugins list` and confirm the `longcat` plugin is
    enabled, then run `carapace models list --provider longcat`.
  </Accordion>
</AccordionGroup>
