---
summary: "Use Qianfan's unified API to access many models in Carapace"
read_when:
...
Qianfan is Baidu's MaaS platform: a unified, OpenAI-compatible API that routes requests to many models behind a single endpoint and API key. Carapace ships it as the official external plugin `@carapace/qianfan-provider`.
...
```bash
carapace plugins install @carapace/qianfan-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carapace onboard --auth-choice qianfan-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    carapace models list --provider qianfan
    ```
  </Step>
</Steps>
