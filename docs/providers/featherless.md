...
read_when:
  - You want to use Featherless AI with Carapace
...
OpenAI-compatible API. Carapace installs Featherless as an official external
...
```bash
carapace onboard --auth-choice featherless-api-key
```
...
```bash
carapace models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. Carapace configures its
...
Carapace deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.carapace/.env` or another
