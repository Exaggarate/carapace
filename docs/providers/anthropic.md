---
summary: "Use Anthropic Claude via API keys or Claude CLI in Carapace"
read_when:
  - You want to use Anthropic models in Carapace
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Carapace supports two auth routes:
...
Carapace detects the available Anthropic credential and selects the matching usage surface:

- Carapace-managed subscription/setup credentials show quota windows and optional extra-usage budget.
- Native Claude CLI logins stay under Claude's exclusive refresh control, so Carapace does not poll their quota endpoint.
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Carapace's session-derived estimated cost.

<Warning>
Claude Code owns its existing login and subscription; Carapace does not persist
...
<Tabs>
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        carapace onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        carapace models list --provider anthropic
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      <Step title="Ensure Claude CLI is installed and logged in">
        Carapace communicates directly with the installed Claude Code executable.
        ...
        Carapace so the gateway launches the new binary:
      ...
      <Step title="Run onboarding">
        ...
        through Carapace's direct CLI transport. Carapace uses a non-secret route
        ...
        file-descriptor forwarding. Native-tool approvals remain under Carapace
        control. Schema-valid native calls pass through Carapace's canonical
    ...
    <Warning>
    Claude CLI reuse expects the Carapace process to run on the same host as the
    ...
    a provider with Carapace-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    ```bash
    carapace models auth login --provider anthropic --method setup-token
    ```
    ...
    Carapace release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Carapace also supports subscription-style options from
...
```bash
carapace models set anthropic/claude-fable-5-1
```
...
`fable-5` and `anthropic/claude-fable-5` selections still use Fable 5; Carapace
...
Carapace's Anthropic adapter converts a forced tool choice to `auto`
...
blocks. Claude Code manages this history for the CLI runtime. Carapace's
...
invalidated replayed thinking server-side, and Carapace logs a warning with the
...
On other eligible routes, a client-side prune is a one-time prefix edit. Carapace
...
Anthropic drops those blocks unbilled, and Carapace's embedded runtime omits
...
direct API-key routes Anthropic drops those blocks server-side and Carapace
...
No additional Carapace config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside Carapace appear
...
`sessions.catalog.continue`. Carapace re-resolves the local catalog record,
...
The new Carapace session starts with the catalog title and color. Continuing an
already adopted session preserves any title or color changes made in Carapace.
...
and its local `claude` executable resolves. Carapace re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Carapace never modifies Claude Desktop
...
With an Anthropic API key configured, Carapace refreshes the Claude catalog from
Anthropic's models endpoint, so newly published snapshots of supported model
families appear without an Carapace release. Models the shipped catalog already
...
capabilities match the request shaping Carapace would apply to it. A brand-new
model generation therefore stays hidden until Carapace adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading Carapace can therefore move a
...
higher native effort levels. Carapace omits manual thinking budgets, custom
...
effort instead. Carapace also omits caller-selected sampling parameters for
both Fable versions.

`anthropic/claude-mythos-5` is a limited-access model with the same always-on
adaptive-thinking contract. Carapace defaults to `high`, maps `/think off` and
...
Claude Opus 4.8 keeps thinking off by default in Carapace. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Carapace sends
...
another Claude model. Carapace opts into Anthropic's recommended per-category
...
1. For every direct API-key request to `anthropic/claude-opus-5`,
   `anthropic/claude-fable-5-1`, or `anthropic/claude-fable-5`, Carapace sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Carapace's normal [model failover](/concepts/model-failover).
...
4. If the recommended model declines as well, the turn surfaces the refusal
   as an error. Carapace does not retry a final refusal or advance to another
   configured model.

The fallback happens at the Anthropic API level, so the serving model does not
need to be in your configured Carapace fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. Carapace
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into Carapace's per-turn estimate.
...
Carapace supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  ...
  <Accordion title="Bedrock Claude notes">
    ...
    - Supported Nova models offer opt-in explicit caching: set `cacheRetention` explicitly to `short` or `long` for system/message checkpoints with a five-minute TTL. Unset retention adds no checkpoints. Nova explicit caching has not been live-verified against AWS by Carapace maintainers yet. Other non-Claude models remain at `cacheRetention: "none"`; see [Bedrock prompt caching](/reference/prompt-caching#amazon-bedrock) for model IDs, AWS limits, and the live acceptance proof gap.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, Carapace's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. Carapace applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so Carapace omits both fields.
  ...
  <Accordion title="Server-side compaction">
    ...
    Carapace adds the `compact-2026-01-12` beta header and sends an Anthropic
    `context_management` compaction edit. When compaction occurs, Carapace
    ...
    When `anthropicCompactThreshold` is omitted, Carapace uses
    ...
    <Warning>
    Anthropic server-side compaction is a beta feature and Carapace never
    ...
    proxies, Bedrock, Vertex, and Foundry are excluded. Carapace does not send
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Carapace
    ...
    When an image or PDF is attached to a conversation, Carapace automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Carapace sizes these models
    ...
    these models and Carapace no longer sends the retired
    ...
    extended context. Carapace uses CLI-owned metadata and configured limits;
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Carapace strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  <Accordion title="Claude CLI OAuth session expired or could not be refreshed">
    ...
    Claude Code owns its login and refresh lifecycle; do not copy an OAuth token into Carapace.
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent**; new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `carapace models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `carapace models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `carapace models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
