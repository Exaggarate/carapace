---
summary: "Use NovitaAI's OpenAI-compatible API with Carapace"
read_when:
  - You want to run Carapace with NovitaAI models
...
Carapace provides NovitaAI through the official external
`@carapace/novita-provider` plugin. Model refs use the
...
```bash
carapace plugins install @carapace/novita-provider
...
```bash
carapace onboard --auth-choice novita-api-key
```
...
| Setting       | Value                             |
| ------------- | --------------------------------- |
| Plugin        | `@carapace/novita-provider`       |
| Provider id   | `novita`                          |
...
```bash
carapace models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `carapace onboard --auth-choice novita-api-key` if the stored profile is
  stale.
