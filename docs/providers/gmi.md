---
summary: "Use GMI Cloud's OpenAI-compatible API with Carapace"
read_when:
  - You want to run Carapace with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In Carapace it is an official external provider
...
primary provider does. Carapace owns the provider id, auth profile, aliases,
...
```bash
carapace plugins install @carapace/gmi-provider
...
```bash
carapace onboard --auth-choice gmi-api-key
```
...
```bash
carapace models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  Carapace, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `carapace models list --provider gmi`.
