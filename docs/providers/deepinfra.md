---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Carapace"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Carapace
title: "DeepInfra"
---
...
```bash
carapace onboard --deepinfra-api-key <key>
```
...
live from `https://api.deepinfra.com/v1/openai/models?sort_by=carapace&filter=with_meta`
...
| Surface                  | Default model                                                                  | Carapace config/tool                                  |
| ------------------------ | ------------------------------------------------------------------------------ | ----------------------------------------------------- |
...
native model types. Carapace has no provider contract for those categories
yet, so this plugin does not register them.

## Available models

Carapace discovers DeepInfra models dynamically once a key is configured. Use
`/models deepinfra` or `carapace models list --provider deepinfra` to see the
current list.
...
discount expiry remain unknown; Carapace does not guess context tiers or parse
...
Carapace's registered catalog hook explicitly selects `discoveryMode: "strict"`
