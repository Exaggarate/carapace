---
summary: "Use Venice AI privacy-focused models in Carapace"
read_when:
  - You want privacy-focused inference in Carapace
...
<Steps>
  ...
  <Step title="Configure Carapace">
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        carapace onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    carapace agent --model venice/zai-org-glm-4.7 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
carapace models set venice/zai-org-glm-4.7
carapace models list --all --provider venice
```

You can also run `carapace configure` and pick **Model/auth provider > Venice AI**.
...
The bundled catalog above is a manifest-backed seed list. At runtime Carapace
...
Carapace catalog advertises only the canonical model IDs returned by `/models`.

## DeepSeek V4 replay behavior

If Venice exposes DeepSeek V4 models such as `deepseek-v4-pro` or
`deepseek-v4-flash`, Carapace fills the required `reasoning_content` replay
...
Carapace reads live prices from Venice's public
...
carapace agent --model venice/zai-org-glm-4.7 --message "Quick health check"

# Claude Opus via Venice (anonymized)
carapace agent --model venice/claude-opus-5 --message "Summarize this task"

# Uncensored model
carapace agent --model venice/venice-uncensored-1-2 --message "Draft options"

# Vision model with image
carapace agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Coding model
carapace agent --model venice/qwen3-coder-480b-a35b-instruct-turbo --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    carapace models list --provider venice
    ```
  ...
  <Accordion title="Model not available">
    Run `carapace models list --all --provider venice` to see currently
