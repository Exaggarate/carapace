...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in Carapace
---

ClawRouter gives Carapace one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on Carapace usage surfaces.
...
Carapace host. The plugin ships bundled with Carapace (`enabledByDefault: true`);
you only need an issued ClawRouter credential.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in Carapace)           |
| Auth          | `CLAWROUTER_API_KEY`                     |
| Default URL   | `https://github.com/Exaggarate/carapace`         |
...
<Steps>
  ...
  <Step title="Configure Carapace">
    ...
    carapace onboard --auth-choice clawrouter-api-key
    carapace plugins enable clawrouter
    ```
    ...
    ClawRouter origin; the default is `https://github.com/Exaggarate/carapace`.

  </Step>
  <Step title="List granted models">
    ```bash
    carapace models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    carapace models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `carapace agent --model clawrouter/<provider>/<model> --message "..."`.
...
SecretRef in `carapace.json`. The canonical managed fields are:
...
```bash
carapace config patch --file ./clawrouter.patch.json5 --dry-run --json
carapace config patch --file ./clawrouter.patch.json5
```
...
such as `CARAPACE_EXTENSIONS=clickclack`, `slack`, or `msteams`; see
...
# Carapace gateway startup readiness only; no model call is made.
curl -fsS http://127.0.0.1:18789/readyz

# Credential-scoped catalog discovery.
carapace models list --all --provider clawrouter --json

# Minimal real inference probe through the configured ClawRouter provider.
carapace models status --probe --probe-provider clawrouter --probe-max-tokens 8 --json

# Workload canary using an exact granted model ref.
carapace agent --agent main \
...
`X-Request-ID`, so an Carapace model-call event can be joined to ClawRouter's
...
supported request routes. Carapace does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an Carapace model when:
...
Adding a model to a supported ClawRouter provider needs no Carapace release:
...
A model's optional `displayName` is its picker label; without it, Carapace uses
...
ClawRouter owns upstream credentials; its catalog tells Carapace which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | Carapace transport     |
| -------------------------------------------------------- | ---------------------- |
...
unsupported request format is intentionally not advertised as an Carapace
...
ClawRouter's `/v1/usage` response feeds the normal Carapace provider-usage
...
```bash
carapace status --usage
carapace models status
```

The same provider snapshot is available to `/status` in chat and Carapace's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; Carapace does not fall back to upstream provider keys.                                          |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    Carapace usage and status surfaces.
  </Card>
</CardGroup>
