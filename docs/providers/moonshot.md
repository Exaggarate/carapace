...
and `max` (the default). Carapace exposes those exact levels and maps `/think
    ...
    **Best for:** Kimi K3 and K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      <Step title="Install the plugin">
        ```bash
        carapace plugins install @carapace/moonshot-provider
      ...
      <Step title="Run onboarding">
        ```bash
        carapace onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        carapace onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      <Step title="Confirm the Kimi K3 default">
        ...
        ```bash
        carapace models set moonshot/kimi-k3
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        carapace models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        CARAPACE_CONFIG_PATH=/tmp/carapace-kimi/carapace.json \
        CARAPACE_STATE_DIR=/tmp/carapace-kimi \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        carapace onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        carapace models list --provider kimi
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    Moonshot API Kimi K3 always reasons at maximum effort. Carapace exposes only
    ...
    so Carapace preserves the requested `tool_choice`. For multi-turn tool use,
    Carapace preserves the assistant reasoning content required by Moonshot's
    replay contract.

    Kimi K2.7 Code always uses native thinking. Moonshot requires clients to
    omit the `thinking` field for this model, so Carapace exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; Carapace omits configured
    ...
    Carapace maps runtime `/think` levels for those models:
    ...
    default strategy. Carapace only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while Carapace omits the entire
    `thinking` field.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    Carapace keys this off the endpoint host, not the provider id, so a custom
