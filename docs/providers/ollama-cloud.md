---
summary: "Use Ollama Cloud directly with Carapace"
read_when:
...
Carapace registers `ollama-cloud` as its own provider id so cloud-only
...
```bash
carapace onboard --auth-choice ollama-cloud --ollama-cloud-api-key "<key>"
```
...
- You want the same native Ollama chat API shape Carapace uses for local
...
Carapace discovers Ollama Cloud models live from the hosted catalog:

```bash
carapace models list --provider ollama-cloud
carapace models set ollama-cloud/kimi-k2.6
```
...
Carapace offers bundled suggestions and records the discovery failure. A
...
CARAPACE_LIVE_OLLAMA_MODEL=kimi-k2.6 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

The cloud smoke runs text, native stream, and web search; set
`CARAPACE_LIVE_OLLAMA_WEB_SEARCH=0` to skip web search. It skips embeddings by
default for `https://ollama.com` because Ollama Cloud API keys may not
authorize `/api/embed`; force them with `CARAPACE_LIVE_OLLAMA_EMBEDDINGS=1`.
...
- Unknown model errors: run `carapace models list --provider ollama-cloud` and
