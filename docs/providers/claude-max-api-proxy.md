...
**claude-max-api-proxy** is a community npm package (not an Carapace plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for Carapace's
...
<Steps>
  ...
  <Step title="Configure Carapace">
    Point Carapace at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not Carapace's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses Carapace's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - Carapace's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native Carapace integration with Claude CLI or API keys.
  </Card>
