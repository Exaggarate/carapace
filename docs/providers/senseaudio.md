...
SenseAudio transcribes inbound audio and voice-note attachments through Carapace's shared `tools.media.audio` pipeline. Carapace posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Carapace uploads the
...
<Note>
SenseAudio is batch STT only in Carapace. Voice Call realtime transcription
