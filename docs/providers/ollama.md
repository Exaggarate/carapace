---
summary: "Run Carapace with Ollama (cloud and local models)"
read_when:
  - You want to run Carapace with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

Carapace talks to Ollama's native API (`/api/chat`), not the OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. Carapace uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Tab title="Onboarding (recommended)">
    <Steps>
      <Step title="Run onboarding">
        ...
        On a fresh guided setup, Carapace first checks the default or configured
        ...
        a real completion before Carapace saves it; discovery never pulls or
        loads an idle model.
      ...
      <Step title="Verify">
        ```bash
        carapace models list --provider ollama
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Set a credential">
        ...
        Or in config: `carapace config set models.providers.ollama.apiKey "OLLAMA_API_KEY"`.
      </Step>
      <Step title="Select the model">
        ```bash
        carapace models list
        carapace models set ollama/gemma4
        ```
...
Carapace prompts for the base URL, discovers local models, and checks
...
For cloud-only access without a local daemon, use `carapace onboard --auth-choice ollama-cloud` and see [Ollama Cloud](/providers/ollama-cloud) — that path does not need `ollama signin` or a running server:

```bash
carapace onboard --auth-choice ollama-cloud
carapace models set ollama-cloud/kimi-k2.5:cloud
```

The cloud model list shown during `carapace onboard` is populated live from
...
models at setup time, Carapace falls back to its hardcoded suggested list so
...
defined, Carapace discovers models from `http://127.0.0.1:11434`:
...
| Token limits         | `maxTokens` defaults to Carapace's Ollama max-token cap                                                                                                                                                                                                                                       |
...
hand-written `models.json` entry; Carapace resolves it live. For signed-in
...
`carapace infer audio transcribe` for audio):
...
provider and `/api/tags` is unreachable, Carapace records that run as
...
```bash
CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_OLLAMA=1 CARAPACE_LIVE_OLLAMA_WEB_SEARCH=0 \
  pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

For Ollama Cloud, point the same live test at the hosted endpoint (skips
embeddings by default; force with `CARAPACE_LIVE_OLLAMA_EMBEDDINGS=1` since a
cloud key may not authorize `/api/embed`):

```bash
export OLLAMA_API_KEY='<your-ollama-cloud-api-key>'
CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_OLLAMA=1 \
CARAPACE_LIVE_OLLAMA_BASE_URL=https://ollama.com \
CARAPACE_LIVE_OLLAMA_MODEL=glm-5.1:cloud \
CARAPACE_LIVE_OLLAMA_WEB_SEARCH=1 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```
...
<Steps>
  ...
  <Step title="Connect the node host">
    ...
    carapace devices approve <deviceRequestId>
    carapace nodes pending
    carapace nodes approve <nodeRequestId>
    ...
    `ollama.models` and `ollama.chat`, check `carapace nodes pending` again.
...
```bash
carapace config set plugins.entries.ollama.config.nodeInference.enabled false
```

Restart the node (`carapace node restart`, or stop/rerun `carapace node run`
...
surface may need `carapace nodes pending` approval again after reconnect.
...
media-understanding provider, so Carapace can route explicit image-description
...
carapace infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --json
```
...
that already support native vision. If the call fails, Carapace can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for Carapace's
...
```bash
CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_OLLAMA_IMAGE=1 \
...
Carapace rejects image-description requests for models not marked
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry; Carapace fills it in for availability checks.
    </Tip>
...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextTokens` caps Carapace's active-input budget; `params.num_ctx` sets
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    Carapace strips the active provider prefix (falling back to a bare
  ...
  <Accordion title="Small local model profile">
    ...
    Large file reads use Carapace's context-based paging. The native adapter
...
prefix, such as `ollama-spark/qwen3:32b`, Carapace strips that prefix before
...
# Carapace catalog and selected model
carapace models list --provider ollama
...
works but Carapace does not, check whether the Gateway runs on a different
machine, container, or service account.

## Ollama Web Search

Carapace bundles **Ollama Web Search** as a `web_search` provider.
...
Choose it during `carapace onboard` or `carapace configure --section web`, or set:
...
For a self-hosted host, Carapace first tries the local `/api/experimental/web_search`
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    Carapace injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, Carapace uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to Carapace's default Ollama context
    window.
    ...
    native window), so Carapace can override a smaller Modelfile context even
    ...
    `carapace doctor --fix`. Doctor preserves current `contextTokens` caps without
    ...
    With `temperature: 0`, Carapace still normalizes `top_p` to `1` for greedy
    ...
    Carapace then attempts compaction and retries, or reports the failure.
  ...
  <Accordion title="Thinking control">
    ...
    Carapace forwards thinking as Ollama expects it: top-level `think`, not
    ...
    ```bash
    carapace agent --model ollama/gemma4 --thinking off
    carapace agent --model ollama/gemma4 --thinking low
    ```
    ...
    thinking for a specific model. Carapace preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use Carapace's
  ...
  <Accordion title="Streaming configuration">
    ...
    and `carapace agent --thinking off` send top-level `think: false` unless
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    Carapace logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not Carapace">
  ...
  <Accordion title="Kimi or GLM returns garbled symbols">
    ...
    ```bash
    carapace infer model run --model ollama/kimi-k2.5:cloud --prompt "Reply with exactly: ok" --json
    carapace models set ollama/gemma4
    ```
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` overrides it. Cap both Carapace's budget and Ollama's request
    ...
    Lower the model entry's `contextTokens` if Carapace sends too much prompt. Lower
