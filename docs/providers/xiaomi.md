---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with Carapace"
read_when:
  - You want Xiaomi MiMo models in Carapace
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    carapace onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    carapace onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    carapace onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    carapace onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    carapace models list --provider xiaomi
    carapace models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
Carapace's [`/think` directive](/tools/thinking) with levels `off`,
...
Carapace sends `speakerVoice` for that model.
...
the desired voice description; Carapace sends it as the `user` message, sends
...
Matrix, Telegram, and WhatsApp), Carapace transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.carapace/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.carapace/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Carapace configuration reference.
  </Card>
