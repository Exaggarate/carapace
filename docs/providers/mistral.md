---
summary: "Use Mistral models and Voxtral transcription with Carapace"
read_when:
  - You want to use Mistral models in Carapace
...
| ---------------- | ------------------------------------------- |
| Provider id      | `mistral`                                   |
| Plugin           | `@carapace/mistral-provider`                |
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    carapace onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    carapace onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    carapace models list --provider mistral
    ```
  </Step>
</Steps>
...
```bash
carapace models list --all --provider mistral --plain
```
...
<Note>
Carapace defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call can forward Twilio media frames directly. Use `encoding: "pcm_s16le"` and a matching `sampleRate` only if your upstream stream is already raw PCM.
</Note>
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Carapace maps the session **thinking** level to Mistral's API:

    | Carapace thinking level                                              | Mistral `reasoning_effort` |
    | ----------------------------------------------------------------------- | --------------------------- |
    | **off** / **minimal**                                                 | `none`                      |
    | **low** / **medium** / **high** / **xhigh** / **adaptive** / **max** | `high`                       |

    <Warning>
    Avoid combining Medium 3.5 reasoning mode with `temperature: 0`; the Mistral HTTP API has been reported to reject `reasoning_effort="high"` plus `temperature: 0` with a 400 response. Leave temperature unset, or turn thinking off/minimal so Carapace sends `reasoning_effort: "none"` before you set a low temperature.
    </Warning>
