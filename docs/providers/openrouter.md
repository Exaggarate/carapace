---
summary: "Use OpenRouter's unified API to access many models in Carapace"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Carapace
...
OpenAI-compatible, so Carapace talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        carapace onboard --auth-choice openrouter-oauth
        ```

        Carapace opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, Carapace prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        carapace models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        carapace onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Carapace sends canonical OpenRouter image requests to the dedicated image API
...
optional `media_type`; when `media_type` is absent, Carapace sniffs the image
...
Carapace submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. Carapace sends `modalities:
...
Carapace sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one Carapace model ref to several OpenRouter models in
...
`openrouter/fusion`, so the Carapace model ref carries both the Carapace
provider prefix and the upstream OpenRouter namespace:

```bash
carapace models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: Carapace turns can include
...
Carapace adds a sanitized system-prompt note listing the configured analysis
...
```bash
carapace infer model run --local \
...
login flow that issues an OpenRouter API key, so Carapace stores the result in
...
```bash
carapace models auth login --provider openrouter --method oauth
carapace models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), Carapace adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Carapace
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Carapace sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Carapace maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. Carapace sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Carapace keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Carapace forwards that object to OpenRouter as the request `provider`
