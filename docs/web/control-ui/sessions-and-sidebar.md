...
machine's native account, model, and configuration; Carapace does not translate
model or authentication settings or automatically adopt the native session.
Native draft text is not sent to Carapace for automatic title preparation.
...
No matching Carapace model route is required. Each machine has one launch
...
and resuming existing sessions with their original profile. An Carapace agent's
...
Carapace worker placement, cloud/Auto selection, attachment submission, model
...
Carapace never starts a Chat or substitutes another host or home directory.
