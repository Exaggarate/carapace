...
```bash
carapace tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `carapace chat` and `carapace terminal` are aliases for `carapace tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- Bare `carapace` (no subcommand) picks a target automatically: an unconfigured install runs inference onboarding; invalid config opens classic doctor guidance; a reachable configured Gateway opens this TUI shell in gateway mode; otherwise a configured local model opens it in local mode.
...
- Delivery is fixed for the whole TUI session at launch: start with `carapace tui --deliver` to turn it on. There is no `/deliver` slash command or Settings toggle to flip it mid-session; restart the TUI to change it.
...
- `/carapace [request]` returns from the normal agent TUI to the [Carapace](#carapace-setup-and-repair-helper) setup/repair chat, optionally forwarding one request.
...
- Local shell commands receive `CARAPACE_SHELL=tui-local` in their environment.
- A lone `!` is sent as a normal message; leading spaces do not trigger local exec.

## Carapace setup and repair helper

Carapace is the ring-zero setup/repair assistant, exposed as `carapace setup` after the configured default model passes a live inference check. If inference is unavailable, an interactive invocation returns to inference onboarding and automation fails with repair guidance. It runs inside the same local TUI shell as `carapace tui --local`, backed by an AI agent restricted to Carapace's typed, approval-gated operations:

```bash
carapace setup                       # start interactively
carapace setup -m "status"           # run one request and exit
carapace setup -m "set default model openai/gpt-5.2" --yes   # apply a config write
```
...
- From inside Carapace, an `open-tui` request (for example, asking to talk to a normal agent) exits Carapace and opens the regular agent TUI; use `/carapace` there to come back.
...
If `carapace config validate` is already failing, start with `carapace configure` or `carapace doctor --fix` first; `carapace chat` still needs a loadable config to start.
...
4. Apply narrow changes with `carapace config set` or `carapace configure`, then rerun `!carapace config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!carapace doctor --fix`.

Tips:

- Prefer `carapace config set` or `carapace configure` over hand-editing `carapace.json`.
- `carapace docs "<query>"` searches the live docs index from the same machine.
- `carapace config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `CARAPACE_THEME=light` before launching `carapace tui`.
- To force the original dark palette instead, set `CARAPACE_THEME=dark`.
...
- The TUI connects with client id `carapace-tui` under the coarse `ui` client mode (the same mode Control UI and WebChat use for Gateway policy).
...
- Check the Gateway logs: `carapace logs --follow`.
- Confirm the agent can run: `carapace status` and `carapace models status`.
...
- No agents in picker: check `carapace agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `carapace.json`
