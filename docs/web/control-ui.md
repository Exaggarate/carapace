...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/carapace`)
...
Closed Terminal, Browser, Desktop, and Home/Ask Carapace panels initialize when you open them rather than during initial navigation. Panels saved as open still restore after a reload.
...
If the page fails to load, start the Gateway first: `carapace gateway`.

<Note>
On native Windows LAN binds, Windows Firewall or organization-managed Group Policy can still block the advertised LAN URL even when `127.0.0.1` works on the Gateway host. Run `carapace gateway status --deep` on the Windows host; it reports likely-blocked ports, profile mismatches, and local firewall rules that policy may ignore.
</Note>
...
- [Panels and docks](/web/control-ui/panels) — Ask Carapace, the Home dock, the operator terminal, and the browser panel.
...
- <a id="carapace-system-care" />[Carapace system care](/web/control-ui/panels#carapace-system-care)
...
- <a id="carapace-chat-workspace-startup" />[Carapace Chat workspace startup](/web/control-ui/sessions-and-sidebar#carapace-chat-workspace-startup)
