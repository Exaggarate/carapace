...
- In scope: strictly user-supplied credentials that Carapace does not mint or rotate.
...
### `carapace.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
machine-state value,
not `carapace.json`. They accept the same SecretInput forms and resolve through
...
- In `carapace.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `carapace doctor --fix` to migrate valid markers.
