---
summary: "Default Carapace agent instructions and skills roster for the personal assistant setup"
title: "Default AGENTS.md"
read_when:
  - Starting a new Carapace agent session
  - Enabling or auditing default skills
---

## First run (recommended)

Carapace agents use a workspace directory. Default: `~/.carapace/workspace` (configurable via `agents.defaults.workspace`, supports `~`).

1. Create the workspace:

```bash
mkdir -p ~/.carapace/workspace
```

2. Copy the default workspace templates into it:

```bash
cp docs/reference/templates/AGENTS.md ~/.carapace/workspace/AGENTS.md
cp docs/reference/templates/SOUL.md ~/.carapace/workspace/SOUL.md
```

3. Optional: use this file's personal-assistant skill roster instead of the generic template:

```bash
cp docs/reference/AGENTS.default.md ~/.carapace/workspace/AGENTS.md
```
...
Before proposing or building a custom system, feature, workflow, tool, integration, or automation, check for open-source projects, maintained libraries, existing Carapace plugins, or free platforms that already solve it well enough. Prefer those when adequate. Build custom only when existing options are unsuitable, too expensive, unmaintained, unsafe, non-compliant, or the user explicitly asks for custom. Avoid paid-service recommendations unless the user explicitly approves spend. Keep this lightweight, a preflight gate, not a research assignment.
...
- The macOS app manages permissions (screen recording, notifications, microphone) and exposes the `carapace` CLI via its bundled binary.
...
- Prefer the `carapace` CLI for scripting; the desktop app handles permissions.
...
- For browser-driven verification, use the `carapace browser` CLI (bundled `browser` plugin) with the Carapace-managed Chrome/Brave/Edge/Chromium profile.
