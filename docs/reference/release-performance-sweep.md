...
read_when:
  ...
  - You need the numbers behind the Carapace performance and dependency blog post
...
This page captures the evidence behind the May 2026 Carapace performance,
...
- **Release performance sweep:** GitHub Releases from `v2026.5.28` back through
  stable `v2026.4.23`, using the `Carapace Performance` workflow,
...
- **npm package size sweep:** `npm pack carapace@<version> --dry-run --json`
...
<CardGroup cols={2}>
  ...
  <Card title="Latest stable install" icon="hard-drive">
    **361.7MiB fresh install**
    ...
    Cuts the nested Carapace dependency tree sharply from the `2026.5.22`
...
<CardGroup cols={2}>
  ...
  <Card title="Nested tree" icon="unplug">
    Nested `carapace/node_modules` fell from **656.1MiB** to **259.7MiB** in
...
| Nested `carapace/node_modules` from `2026.5.22` |   911.8MB |     259.7MiB | 71.5% lower |
| Nested `carapace/node_modules` from `2026.5.27` |  656.1MiB |     259.7MiB | 60.4% lower |
...
| Point              | Installed deps | Fresh install | Carapace package | Nested `carapace/node_modules` | Root shrinkwrap | Canvas install behavior                   |
| ------------------ | -------------: | ------------: | ---------------: | -----------------------------: | --------------- | ----------------------------------------- |
...
`2026.5.20` shipped with no root shrinkwrap and no large nested Carapace
dependency tree. `2026.5.22` introduced root shrinkwrap and installed 911.8MB
under nested `carapace/node_modules`. `2026.5.28` keeps shrinkwrap and still
installs 259.7MiB under nested `carapace/node_modules`, but no longer installs
...
that made npm materialize a large nested Carapace dependency tree and all 12
