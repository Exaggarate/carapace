...
Carapace exposes four user-facing update channels:
...
```bash
gh workflow run carapace-npm-release.yml \
  --ref extended-stable/YYYY.M.33 \
  -f tag=vYYYY.M.P \
...
```bash
node --import tsx scripts/carapace-npm-postpublish-verify.ts YYYY.M.P
npm view carapace@YYYY.M.P version --userconfig "$(mktemp)"
npm view carapace@extended-stable version --userconfig "$(mktemp)"
```
...
`npm dist-tag add carapace@YYYY.M.P extended-stable` repair command printed in
...
`Carapace Release Publish`. Its Docker-only extended-stable path rechecks the
...
```bash
gh workflow run carapace-release-publish.yml \
  --ref main \
  -f tag=vYYYY.M.P \
  -f preflight_run_id=<npm-preflight-run-id> \
  -f full_release_validation_run_id=<full-validation-run-id> \
...
9. Create the protected lightweight tooling tag at the recorded Tooling SHA using the [publish automation commands](#regular-release-publish-automation). Run the candidate helper against the untagged Release SHA with the successful Release-SHA validation parent and that tooling tag:
   ...
   Then run the post-publish package acceptance against the published `carapace@YYYY.M.PATCH-beta.N` or `carapace@beta` package. If a pushed or published prerelease needs a fix, cut the next matching prerelease number; never delete or rewrite the old one.
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and `CARAPACE_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main` contains the shipped version and changelog before calling the stable release done.
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof for a package candidate while release work continues. Use `source=npm` for `carapace@beta`, `carapace@latest`, or an exact release version; `source=ref` to pack a trusted `package_ref` branch/tag/SHA with the current `workflow_ref` harness; `source=url` for a public HTTPS tarball with a required SHA-256 and strict public URL policy; `source=trusted-url` for a named trusted-source policy using required `trusted_source_id` and SHA-256; or `source=artifact` for a tarball uploaded by another GitHub Actions run.
  ...
  ```bash
  gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=carapace@beta -f suite_profile=product -f published_upgrade_survivor_baseline=carapace@2026.4.26 -f telegram_mode=mock-openai
  ```
...
- That workflow runs `CARAPACE_LIVE_TEST=1 CARAPACE_LIVE_CACHE_TEST=1 pnpm test:live:cache` using both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` workflow secrets.
- npm release preflight no longer waits on the separate release checks lane.
- Before tagging a release candidate locally, run `RELEASE_TAG=vYYYY.M.PATCH-beta.N pnpm release:fast-pretag-check`. The helper runs the fast release guardrails, plugin npm/ClawHub release checks, build, UI build, and `release:carapace:npm:check` in the order that catches common approval-blocking mistakes before the GitHub publish workflow starts.
- Plugin `carapace.release.requireLatestDependencies` declarations remain release metadata, but npm `latest` drift is advisory. Checks warn with the plugin, dependency, pinned version, and current latest version; a failed latest lookup also warns and does not establish that the pin is unusable. Full Release Validation's Codex lanes validate the `@carapace/codex` harness pin. Keep that frozen, tested pin when upstream publishes a newer version. Missing or malformed required runtime dependency metadata, package/install failures, and failed required validation lanes still block release.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/carapace-npm-release-check.ts` (or the matching prerelease/correction tag) before approval.
- After npm publish, run `node --import tsx scripts/carapace-npm-postpublish-verify.ts YYYY.M.PATCH` (or the matching beta/correction version) to verify the published registry install path in a fresh temp prefix.
- After a beta publish, run `CARAPACE_NPM_TELEGRAM_PACKAGE_SPEC=carapace@YYYY.M.PATCH-beta.N CARAPACE_NPM_TELEGRAM_CREDENTIAL_ROLE=maintainer pnpm test:docker:npm-telegram-live` with `CARAPACE_QA_CONVEX_SITE_URL` and `CARAPACE_QA_CONVEX_SECRET_MAINTAINER` set. This verifies installed-package onboarding, Telegram setup, and real Telegram E2E against the published npm package using the shared Test Server userbot pool. CI uses the `ci` role and `CARAPACE_QA_CONVEX_SECRET_CI` instead.
...
- Maintainer release automation uses preflight-then-promote:
  ...
  - Token-based npm dist-tag mutation lives in `carapace/releases/.github/workflows/carapace-npm-dist-tags.yml` because `npm dist-tag add` still needs `NPM_TOKEN` while the source repo keeps OIDC-only publish.
  ...
  - Real macOS publish must pass successful macOS `preflight_run_id` and `validate_run_id` in `carapace/releases`. These app gates run independently and never hold npm or GitHub release finalization.
...
pnpm ci:full-release \
  ...
  --target-ref release/YYYY.M.PATCH \
  --workflow-sha "$TOOLING_SHA" \
  -f release_package_spec=carapace@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=carapace@YYYY.M.PATCH-beta.N \
...
For bounded recovery, pass `rerun_group` to the umbrella. Supported controller groups are `ci`, `plugin-prerelease`, `install-smoke`, `cross-os`, `live-e2e`, `package`, `qa-parity`, `qa-live`, `npm-telegram`, and `performance`; use `all` only for deliberate full validation. The removed `release-checks` aggregate handle is invalid because it silently selected every release-check lane and its package/Docker setup. `qa` remains available only as a direct `Carapace Release Checks` manual aggregate, not as an umbrella/controller retry API. Focused `npm-telegram` reruns require `release_package_spec` or `npm_telegram_package_spec`; all-group runs use Package Acceptance Telegram E2E except beta without soak, where it is deferred. Focused cross-OS reruns can add `cross_os_suite_filter=windows/packaged-upgrade` or another OS/suite filter. Live and QA-live filters are valid only with their owning group. Cross-OS filters also work with `rerun_group=all`: add `-f cross_os_suite_filter=ubuntu,macos` to exclude Windows. `npm-stable-v1` and `npm-beta-v1` qualification is preserved when all three Linux suites remain selected; omitted advisory lanes are not run, never passed. Mismatches fail before scheduling and never become an unfiltered broad run. QA release-check failures block normal release validation, including Carapace dynamic tool drift in the core runtime-pair lane. Tideclaw alpha runs may still treat non-package-safety release-check lanes as advisory. With `release_profile=beta`, the `Run repo/live E2E validation` live-provider suites are advisory (warnings, not blockers); stable and full profiles keep them blocking. When `live_suite_filter` explicitly requests a gated QA live lane such as Discord, WhatsApp, or Slack, the matching `CARAPACE_RELEASE_QA_*_LIVE_CI_ENABLED` repo variable must be enabled; otherwise input capture fails instead of silently skipping the lane.

### Vitest

The Vitest box is the manual `CI` child workflow. Manual CI bypasses changed scoping and selects the normal test graph for the release candidate: Linux Node shards, bundled-plugin shards, plugin and channel contract shards, Node 24 minimum compatibility, `check-*`, `check-additional-*`, built-artifact smoke checks, docs checks, Python skills, Windows, macOS, and Control UI i18n. Under `npm-beta-v1`, the umbrella passes `release_scope=npm-beta` and `include_android=false`: native Swift/CarapaceKit, iOS, Android, and native i18n CI lanes are deferred; macOS and Windows Node checks remain. Other Full Release Validation runs use full CI with Android. Standalone manual CI defaults to full coverage and requires `include_android=true` for Android.
...
The Docker box lives in `Carapace Release Checks` through `carapace-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `Carapace Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
The Package box is the installable-product gate. It is backed by `Package Acceptance` and the resolver `scripts/resolve-carapace-package-candidate.mts`. The resolver normalizes a candidate into the `package-under-test` tarball consumed by Docker E2E, validates the package inventory, records the package version and SHA-256, and keeps the workflow harness ref separate from the package source ref.

Supported candidate sources:

- `source=npm`: `carapace@beta`, `carapace@latest`, or an exact Carapace release version
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=carapace@2026.4.26
```
...
`Carapace Release Publish` is the normal mutating entrypoint. The monthly
...
6. After plugin npm succeeds, dispatch `Carapace NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt. ClawHub proceeds in parallel.
...
gh workflow run carapace-release-publish.yml \
  ...
  -f preflight_run_id=<successful-carapace-npm-preflight-run-id> \
...
To attach Windows assets later or retry a failed promotion, use the exact Carapace tag, exact published Windows source tag, and approved installer digests:
...
gh workflow run windows-node-release.yml \
  ...
  -f expected_installer_digests='{"CarapaceCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","CarapaceCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}'
```

Never substitute `latest` for either tag. Monitor the Windows run and its verification evidence separately; an unsuccessful promotion leaves the npm package and GitHub release published. macOS recovery uses `carapace/releases/.github/workflows/carapace-macos-validate.yml` and `carapace-macos-publish.yml`, preserving the successful macOS preflight and validation run IDs when promoting prepared assets.
...
For a selected plugin repair, use `Carapace Release Publish` with `publish_carapace_npm=false`, `plugin_publish_scope=selected`, and `plugins=@carapace/name`. The parent rejects selected scope when `publish_carapace_npm=true` so the core package cannot ship without every publishable official plugin, including `@carapace/diffs-language-pack`. `Plugin NPM Release` also supports direct focused repair dispatch.
...
release tag plus the parent `Carapace Release Publish` run id, attempt, and
...
`Carapace NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `Carapace Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`Carapace Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Carapace NPM Release` preflight run id; required when `publish_carapace_npm=true` or `plugin_publish_scope=all-publishable`
- `full_release_validation_run_id`: successful `Full Release Validation` run id; required when `publish_carapace_npm=true` or `plugin_publish_scope=all-publishable`
...
- `windows_node_tag`: optional exact non-prerelease `carapace/carapace-windows-node` release tag for detached Windows promotion after stable GitHub publication; omit both Windows inputs to skip dispatch
...
- `npm_dist_tag`: npm target tag for the Carapace package, one of `alpha`, `beta`, `latest`, or `extended-stable`
- `publish_docker_only`: beta, regular stable (`latest`), or extended-stable recovery/closeout path. It requires `publish_carapace_npm=false`, complete preflight and Full Release Validation evidence, then verifies the exact npm package, selected dist-tag, and tarball digest before invoking Docker publication.
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only for focused plugin-only repair work with `publish_carapace_npm=false`
- `plugins`: comma-separated `@carapace/*` package names when `plugin_publish_scope=selected`
- `publish_carapace_npm`: defaults to `true`; set `false` only when using the workflow as a plugin-only repair orchestrator
...
`Carapace Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks require the resolved commit to be reachable from an Carapace branch or release tag.
...
- For `Carapace NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `Carapace Release Checks` and `Full Release Validation` are always validation-only
...
1. Run `Carapace NPM Release` with `preflight_only=true`. Before a tag exists, you may use the current full workflow-branch commit SHA for a validation-only dry run of the preflight workflow.
...
4. Optionally select the exact non-prerelease `carapace/carapace-windows-node` release tag whose signed x64 and ARM64 installers should attach after publication. Save it as `windows_node_tag`, with the validated `windows_node_installer_digests` map. The release-candidate helper records both when given `--windows-node-tag`; omit the option if Windows is not ready.
5. Save the successful `preflight_run_id`, `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`.
6. Run `Carapace Release Publish` from the protected `release-publish/<sha12>-<epoch>` tooling tag with the same `tag`, the same `npm_dist_tag`, the optional Windows input pair, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It starts plugin npm and ClawHub in parallel, then promotes the prepared Carapace npm package once plugin npm succeeds. GitHub finalization waits for npm and Docker evidence; apps attach independently afterward.
7. If the release landed on `beta`, use the `carapace/releases/.github/workflows/carapace-npm-dist-tags.yml` workflow to promote that stable version from `beta` to `latest`.
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/carapace-npm-release.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/carapace-npm-release.yml)
- [`.github/workflows/carapace-release-checks.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/carapace-release-checks.yml)
- [`.github/workflows/carapace-cross-os-release-checks-reusable.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/carapace-cross-os-release-checks-reusable.yml)
- [`.github/workflows/docker-release.yml`](https://github.com/Exaggarate/carapace/blob/main/.github/workflows/docker-release.yml)
- [`scripts/resolve-carapace-package-candidate.mts`](https://github.com/Exaggarate/carapace/blob/main/scripts/resolve-carapace-package-candidate.mts)
- [`scripts/carapace-npm-release-check.ts`](https://github.com/Exaggarate/carapace/blob/main/scripts/carapace-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/Exaggarate/carapace/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/Exaggarate/carapace/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in [`carapace/maintainers/release/README.md`](https://github.com/Exaggarate/carapace/maintainers/blob/main/release/README.md) for the actual runbook.
