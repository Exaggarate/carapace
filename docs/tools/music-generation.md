...
generated audio is still missing from the reply, Carapace sends an
...
<Note>
Not all providers support all parameters. Carapace still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Carapace
...
Provider request timeouts are operator configuration only. Carapace uses
...
- **Status lookup:** `carapace tasks list` or `carapace tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Carapace injects an internal completion event back
...
```bash
carapace tasks list
carapace tasks show <taskId>
carapace tasks cancel <taskId>
```
...
For `music_generate`, Carapace tries providers in this order:
...
```bash
CARAPACE_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
