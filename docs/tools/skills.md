...
frontmatter and a markdown body. Carapace loads bundled skills plus any local
...
Skill roots support grouped layouts. Carapace discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Carapace
  skill root. Use `carapace migrate plan codex` to inventory those skills, then
  `carapace migrate codex` to copy them into your Carapace workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active Carapace
skills directory (`~/.carapace/skills` by default; profile environment overrides
...
When `CARAPACE_STATE_DIR` points somewhere other than the default
`~/.carapace`, session skill indexes exclude home-scoped personal or
...
`state/carapace.sqlite`, and immutable bundles under
...
`carapace.plugin.json` (paths relative to the plugin root). Plugin skills load
...
`metadata.carapace.requires` in its frontmatter, same as any other skill.
...
Carapace resolves explicit references from authorized senders on every channel
...
distinct skills; Carapace returns a visible error instead of ignoring extra or
...
`carapace skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `carapace skills install @owner/<slug>`                |
| Install an external skills.sh ref  | `carapace skills install skills-sh:owner/repo/slug`    |
| Install from a Git repository      | `carapace skills install git:owner/repo@ref`           |
| Install a local skill directory    | `carapace skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `carapace skills install @owner/<slug> --global`       |
| Update all workspace skills        | `carapace skills update --all`                         |
| Update a shared managed skill      | `carapace skills update @owner/<slug> --global`        |
| Update all shared managed skills   | `carapace skills update --all --global`                |
| Verify a skill's trust envelope    | `carapace skills verify @owner/<slug>`                 |
| Print the generated Skill Card     | `carapace skills verify @owner/<slug> --card`          |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `carapace skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.carapace/skills` directory, visible to all local agents unless agent
    ...
    `carapace skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `carapace skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `carapace.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.carapace/skills` and personal `~/.agents/skills` may contain
...
<Note>
  Carapace follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Carapace keeps the skill's instructions out of the agent's normal
...
Carapace filters skills at load time using `metadata.carapace` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.carapace` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `carapace.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.carapace` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Carapace lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `carapace.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Carapace does not auto-install Homebrew or translate brew
    ...
    - **Go:** Carapace requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, Carapace installs Go via
      ...
      dependency always targets a dedicated Carapace-managed bin directory
    ...
    - **Download:** `url` (required), `sha256` (optional 64-character hexadecimal
      ...
      `~/.carapace/tools/<skillKey>`). Existing specs without `sha256` keep the
...
<Note>
  ...
  `metadata.carapace.skillKey`, use that key under `skills.entries` instead.
  Quote hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, Carapace:

<Steps>
  <Step title="Reads skill metadata">
    Carapace resolves the effective skill list for the agent, applying gating
...
`--plugin-dir`. Sessions with library selections use Carapace's prompt catalog
and pinned revision paths instead. Carapace omits `--plugin-dir` for those
...
Carapace snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Carapace refreshes the snapshot to keep visible skills
aligned.

When native watch capacity is exhausted, Carapace logs one warning and stops
...
<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Carapace watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Carapace can treat macOS-only skills as eligible when
    ...
    answering bin probes, Carapace clears its cached bin matches.
...
When skills are eligible, Carapace injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), Carapace first preserves as many skill
...
note pointing at `carapace skills check` whenever compact formatting or list
truncation is required.

Keep descriptions short and descriptive to minimize prompt overhead.

For small context windows, the Carapace embedded runtime further shortens the
