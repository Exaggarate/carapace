...
read_when:
  - Looking for an overview of Carapace's media capabilities
...
Carapace generates images, videos, and music, understands inbound media
...
Carapace until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, Carapace submits the request to the provider, returns a task
...
Carapace wakes the agent with the generated media paths so it can tell the
...
Carapace sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; Carapace has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in Carapace until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
