---
summary: "Install, configure, and manage Carapace plugins"
read_when:
...
Plugins extend Carapace with channels, model providers, agent harnesses, tools,
...
- an Carapace checkout or installation with the `carapace` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    ```bash
    carapace plugins search "calendar"
    ```
    ...
    they match an official plugin id. Raw `@carapace/*` specs that match a
  ...
  <Step title="Install the plugin">
    ```bash
    # From ClawHub.
    carapace plugins install clawhub:<package>
    ...
    carapace plugins install ./my-plugin
    carapace plugins install --link ./my-plugin
    ```

    Treat plugin installs like running code. Prefer pinned versions for
    reproducible production installs. ClawHub packages and Carapace's
  ...
  <Step title="Configure and enable it">
    ...
    before the plugin can load. `carapace plugins install` adds the installed
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Carapace-native discovery, scans, version metadata, and install hints | `carapace plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `carapace plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `carapace plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `carapace plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `carapace plugins install <plugin> --marketplace <source>`     |
...
bare spec installs through npm during the launch cutover. Raw `@carapace/*`
specs that match bundled plugins also resolve to the bundled copy before npm
fallback. Use `npm:@carapace/<plugin>@<version>` to deliberately install the
external npm package instead of the bundled copy. Use `clawhub:`, `npm:`,
`git:`, or `npm-pack:` for deterministic source selection. See
[`carapace plugins`](/cli/plugins#install) for the full command contract.

For npm installs, unpinned specs and `@latest` choose the newest stable
package that advertises compatibility with this Carapace build. If npm's
current latest release declares a newer `carapace.compat.pluginApi` or
`carapace.install.minHostVersion` than this build supports, Carapace scans
...
warning. Plugin
`before_install` hooks run later, and only in Carapace processes where plugin
...
snippet. Run [`carapace plugins list --enabled --verbose`](/cli/plugins#list)
or [`carapace plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `carapace.json`. The same
...
`plugins.allow` or reinstall from a trusted source so Carapace records install
provenance.

Run `carapace doctor` or `carapace doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

Carapace recognizes two plugin formats:

| Format                 | How it loads                                                                                | Use when                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Carapace plugin | `carapace.plugin.json` plus a runtime module loaded in process                              | You are installing or building Carapace-specific runtime capabilities  |
| Compatible bundle      | Agent Plugins, Codex, Claude, or Cursor plugin layout mapped into Carapace plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `carapace plugins list`, `carapace plugins inspect`,
`carapace plugins enable`, and `carapace plugins disable`. See
...
Plugin-managed internal hooks show up in `carapace hooks list` with
`plugin:<id>`. You cannot enable or disable them through `carapace hooks`;
...
`carapace.plugin.json`, then enable the plugin and include it in
...
`carapace plugins inspect <id> --runtime --json`, then trigger an event to verify
the running process. See [Plugin hooks](/plugins/hooks#quick-start) for a complete
example.

## Verify the active Gateway

`carapace plugins list` and plain `carapace plugins inspect` read cold config,
...
sure any manual restart targets the actual `carapace gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                                   |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `carapace plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart after install, update, or source changes; for config changes, confirm reload mode and plugin restart prefixes |
| Duplicate channel or tool ownership diagnostics appear         | Run `carapace plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement                    |
...
| Config is invalid during install                               | Read the validation message and run `carapace doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload                   |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `carapace plugins registry --refresh`                                  |
| `CARAPACE_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                                    |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `carapace plugins update <id>`, reinstall the source, or install local plugin dependencies yourself               |

When an enabled managed plugin fails payload verification during Gateway
startup, Carapace quarantines that exact installed plugin root for the boot and
continues serving other plugins. `carapace status --all`, `carapace health`,
and `carapace doctor` report it as `configured-unavailable`. Fix or reinstall
...
`carapace doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, Carapace keeps that request
...
Carapace needs at runtime. Update or reinstall after the publisher ships
...
| Reason                  | Remedy                                                                                                                                                                                                               |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `record-missing`        | Align CLI and Gateway state paths if they differ; otherwise reinstall through `carapace plugins install` so the install is recorded.                                                                                 |
| `provenance-missing`    | Run `carapace doctor --fix` with the Gateway's state/config paths. Doctor repairs catalog-proven legacy ClawHub records; unverifiable records require reinstalling from the official npm package or ClawHub listing. |
...
and validation follows with `plugin present but blocked`, Carapace found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run Carapace
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Carapace config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Carapace as root, repair the managed plugin root to
...
After fixing ownership, rerun `carapace doctor --fix` or
`carapace plugins registry --refresh` so the persisted plugin registry
...
```bash
carapace config set logging.level trace
...
Carapace caches successful plugin tool factory results for repeated
...
- [`carapace plugins`](/cli/plugins) - full CLI reference
