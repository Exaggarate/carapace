---
summary: "Tool Search: compact large Carapace tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Carapace agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Carapace tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Carapace runs
---

Tool Search is an experimental Carapace agent runtime feature. It gives agents one
...
This page documents Carapace Tool Search. It is not the Codex-native tool
...
For the generic Carapace runtime that exposes a QuickJS-WASI `exec`/`wait`
...
When enabled for Carapace runs, the model automatically receives a bounded
...
JavaScript body in an isolated Node subprocess with an `carapace.tools` bridge:

```js
const hits = await carapace.tools.search("create a GitHub issue");
const tool = await carapace.tools.describe(hits[0].id);
return await carapace.tools.call(tool.id, {
...
entry remains searchable and callable. Invalid arguments for Carapace-owned
...
The catalog can include catalog-eligible Carapace tools, plugin tools, MCP
...
Carapace. Direct-only tools remain model-visible and are not added to the
catalog.

Codex harness runs do not receive these experimental Carapace Tool Search
controls. Carapace passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Carapace embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Carapace and plugin tools.
...
7. Expose the Carapace code bridge, the structured fallback tools, or the
   compact directory surface alongside those stable, directly callable tools.

At execution time every real tool call returns to Carapace. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`carapace.tools.call(...)` crosses the bridge back into the Gateway, where the
...
All modes use the same policy-filtered catalog and normal Carapace execution
...
for the current run while Carapace tools, plugin tools, and MCP tools can be
...
catalog includes catalog-eligible Carapace, MCP, and client tools after normal
...
and built from the already policy-filtered catalog. Carapace reuses the
...
authorized catalog changes, Carapace builds a new directory for the new
snapshot.

## API

`carapace.tools.search(query, options?)`
...
Carapace core or plugin tool may also include a compact `output` hint, such as
...
```js
const hits = await carapace.tools.search("calendar event", { limit: 5 });
```

`carapace.tools.describe(id)`
...
```js
const calendarCreate = await carapace.tools.describe("mcp:calendar:create_event");
```

`carapace.tools.call(id, args)`

Calls a selected tool through Carapace and returns the raw `{ tool, result }`
envelope. JSON-returning tools normally place their value in
`result.details`. Carapace validates a trusted core or plugin tool's declared
...
If a trusted tool also declares `outputSchema`, Carapace compiles that schema
...
```js
await carapace.tools.call(calendarCreate.id, {
  summary: "Planning",
...
hidden directory tool name directly, Carapace resolves it from the authorized
catalog before normal execution.
Directory-mode client tool names must not collide with Carapace, plugin, or MCP
...
network grants, and no child-process or worker grants. Carapace enforces a
...
- `carapace.tools.search`
- `carapace.tools.describe`
- `carapace.tools.call`

Normal Carapace behavior still applies to final calls:
...
Enable Tool Search explicitly for Carapace runs with the default code bridge:

```bash
carapace config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Carapace runs:
...
Use the compact directory surface instead for Carapace runs:
...
- `sources`: catalog entry counts split into `carapace`, `mcp`, and `client`
...
operation. Carapace does not record serialized tool or prompt byte counts. The
...
- whether the result came from Carapace, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves all three paths with the Carapace runtime:

```bash
pnpm carapace qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
