...
read_when:
  ...
  - Comparing Carapace exec approvals with ACPX harness permissions
...
```bash
carapace config set tools.exec.mode auto
...
Set ACPX permissions separately from Carapace exec approvals:
...
| Use Codex/Carapace auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of Carapace config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
