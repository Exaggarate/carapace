...
`carapace agent` runs a single agent turn from the command line without an
...
output projection, and process status, use [`carapace agent exec`](/cli/agent#agent-exec).
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    carapace agent --agent main --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    carapace agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    carapace agent --session-id abc123 --message "Continue the task"
  ...
  <Step title="Deliver the reply to a channel">
    ...
    carapace agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    carapace agent --agent ops --message "Generate report" \
...
carapace agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `carapace agent` flag and option reference.
  </Card>
