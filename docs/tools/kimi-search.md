...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.carapace/.env`), or configure via:
...
Choosing **Kimi** during `carapace onboard` or `carapace configure --section web`
also prompts for:
...
Carapace only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), Carapace returns a
