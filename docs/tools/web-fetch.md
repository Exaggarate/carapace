...
Set `tools.web.fetch.cacheTtlMinutes: 0` to bypass Carapace's fetch cache for both
...
Carapace passes cancellation to fallback providers. Providers that honor the
...
connection cleanup, Carapace rejects the call instead of returning success or
...
`plugins.entries.firecrawl.config.webFetch` via `carapace doctor --fix`.
...
- If `provider` is omitted, Carapace auto-detects the first ready web-fetch
...
In this mode, Carapace still applies hostname-based SSRF checks before sending
