---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for Carapace browser control on Linux"
...
```json
{ "error": "Error: Failed to start Chrome CDP on port 18800 for profile \"carapace\"." }
```
...
Snap's AppArmor confinement interferes with how Carapace spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. Carapace removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  If you set `CARAPACE_BROWSER_HEADLESS=0`, `browser.headless: false`, or
  `browser.profiles.<name>.headless: false`, remove that headed override, set
  `CARAPACE_BROWSER_HEADLESS=1`, start `Xvfb`, run
  `carapace browser start --headless` for a one-shot managed launch, or run
  Carapace in a real desktop session.
...
If you must keep snap Chromium, configure Carapace to attach to a
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.carapace/browser/carapace/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/carapace-browser.service
[Unit]
Description=Carapace Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.carapace/browser/carapace/user-data about:blank
Restart=on-failure
RestartSec=5
...
```bash
systemctl --user enable --now carapace-browser.service
```
...
| `CARAPACE_BROWSER_HEADLESS` | Per-process override for local managed browser headless mode         | unset                                                              |
...
1. Use the managed browser instead:
   `carapace browser --browser-profile carapace start` (or set
...
- Local `carapace`-driver profiles get a `cdpPort` allocated when Carapace
