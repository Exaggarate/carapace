...
read_when:
  - Running Carapace Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Carapace Gateway runs inside WSL2, Chrome runs
on Windows, and browser control must cross the WSL2/Windows boundary. Several
independent problems can surface at once (see
[issue #39369](https://github.com/Exaggarate/carapace/issues/39369)): CDP
...
- Carapace points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\Carapace\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. Carapace is not the
problem yet.
...
that before touching Carapace config.

### Layer 3: configure the correct browser profile

Point Carapace at the address reachable from WSL2:
...
- use HTTP(S) when you want Carapace to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting Carapace to succeed
...
```bash
carapace browser --browser-profile remote open https://example.com
carapace browser --browser-profile remote tabs
```
...
| stale viewport / dark-mode / locale / offline overrides after a remote session          | run `carapace browser --browser-profile remote stop` to close the session and release the cached Playwright/CDP connection without restarting the Gateway or the external browser |
...
3. Carapace config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug Carapace config or Control UI auth.
