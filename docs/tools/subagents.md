...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    - `sessions_spawn` returns a run id after startup is accepted, without waiting for the child task to finish. Spawns from an Carapace cloud worker can first wait for child provisioning and node enrollment.
    ...
    - On completion, Carapace best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - Carapace hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Carapace first tries to wake/steer that run instead of starting a second visible reply path.
    ...
    - If an outbound hook intentionally suppresses a completion, the child can remain completed while its task delivery is marked `failed` with the suppression reason. Carapace does not retry or start another requester turn to bypass that decision. Inspect the task error and hook policy before manually retrying.
    - Blocked canonical results are retained for 7 days. Operators can retry or intentionally dismiss them from the Tasks page or with `carapace tasks retry` / `carapace tasks dismiss`; retry can duplicate a visible result after an ambiguous provider acknowledgement.
    - Delivery keeps the resolved requester route: thread-bound or conversation-bound completion routes win when available. If the completion origin only provides a channel, Carapace fills the missing target/account from the requester session's recorded delivery context so direct delivery still works.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Carapace hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.entries.*` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Carapace config agents from `agents_list`.
...
- **Run timeout:** pass `runTimeoutSeconds` to set a timeout for a specific native, ACP, or visible sub-agent run. When omitted, Carapace uses `agents.defaults.subagents.runTimeoutSeconds` if configured; otherwise it falls back to `0` (no timeout). An explicit `0` disables the timeout for that run.
- **Process lifetime:** a detached Carapace sub-agent has its own run lifecycle. A background task created inside an external CLI backend is different: it shares the parent CLI subprocess and stops if that parent reaches `agents.defaults.timeoutSeconds`.
...
`agents.defaults.subagents.delegationMode` controls prompt guidance only; it does not change tool policy or enforce delegation. With no explicit setting, Carapace uses `prefer` in each agent's main session and `suggest` in every other session.
...
<ParamField path="expectsCompletionMessage" type="boolean" default="true">
  Set `false` for fire-and-forget children. When the child finishes, Carapace skips the completion handoff to the requester (no announce or steer turn), records the delivery as not required, and still runs child cleanup. Inspect such children with `subagents` or `sessions_history`. `collect: true` always uses `false`.
</ParamField>
...
collectors require `agents_wait`, or an awaited `agents.run()` in Carapace
...
When active children exist, Carapace injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Carapace creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish Carapace, Codex
...
and `agents_list` omits it. Run `carapace doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, Carapace falls back to that session's requester when available.
...
target agent first. After that, Carapace applies the sub-agent restriction
layer.
...
Carapace warns when the delivery backlog reaches 25 and blocks new subagent
spawns at 50 until operators retry or dismiss enough retained deliveries. It
does not prune results to make room.

## Liveness and recovery

Carapace does not treat `endedAt` absence as permanent proof that a
...
For sub-agents that announce completion, Carapace also attempts a notice to the
...
rapid re-wedge window, Carapace persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`carapace tasks maintenance --apply` to reconcile the task record, or
`carapace doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
