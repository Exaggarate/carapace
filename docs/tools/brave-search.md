...
Carapace supports Brave Search API as a `web_search` provider.
...
or gateway. Carapace appends `/res/v1/web/search` or `/res/v1/llm/context` to
...
- Results expose `published` from Brave's ISO publication metadata: `page_age`
  ...
  timestamps remain offsetless; Carapace does not invent a timezone. Verify the
  source date when freshness matters.

- Carapace uses the Brave **Search** plan. If you have a legacy subscription (e.g. the original Free plan with 2,000 queries/month), it remains valid but does not include newer features like LLM Context or higher rate limits.
