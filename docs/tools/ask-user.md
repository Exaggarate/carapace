...
- Discord and Slack render native buttons for a single-choice, single-question
  prompt.
- For a question created by an active Carapace run, a plain-text reply works on
...
Carapace run's creator binding. Answer those using the question controls in the
Control UI or native app, not an ordinary channel message.

Carapace always enables a free-text **Other** answer. The agent must not add an
