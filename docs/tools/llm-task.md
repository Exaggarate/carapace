...
Run `carapace doctor --fix` once for llm-task entries created by older releases.
...
or deliver model output to a channel. Carapace uses the selected provider,
...
`carapace.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside Carapace, this nested CLI
...
- Lobster steps that do not rely on nested `carapace.invoke` calls.
...
- **No tools**: the selected runtime must expose a literal empty model-callable
  tool surface. Carapace rejects tool-shaped results instead of treating them as
  task output.
