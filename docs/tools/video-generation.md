...
Carapace agents generate videos from text prompts, reference images, or
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    carapace config set agents.defaults.mediaModels.video.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. Carapace submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Carapace wakes the same session with an internal completion event.
4. The agent reports it through the session's normal visible-reply mode:
   ...
   generated media is still missing from the completion reply, Carapace sends
...
generation, or `carapace tasks list` / `carapace tasks show <lookup>` from the
...
Generated video files save under Carapace-managed media storage when the
...
provider also returns a hosted output URL, Carapace delivers that URL instead
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Carapace normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `360P`, `480P`, `540P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Carapace normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Carapace uses `agents.defaults.mediaModels.video.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. Carapace normalizes duration to
...
For `video_generate`, Carapace resolves the model in this order:
...
Carapace does not append auto-detected providers.
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus plugin">
    Requires the official `@carapace/byteplus-provider` plugin.
  ...
  <Accordion title="BytePlus Seedance 1.5 plugin">
    Requires the [`@carapace/byteplus-modelark`](https://www.npmjs.com/package/@carapace/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Carapace waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Carapace submits the
...
```bash
CARAPACE_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `CARAPACE_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `CARAPACE_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Carapace config:
...
```bash
carapace config set agents.defaults.mediaModels.video.primary "qwen/wan2.6-t2v"
```
