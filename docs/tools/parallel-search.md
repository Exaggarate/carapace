      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; Carapace appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.carapace/.env`.
...
Cloudflare AI Gateway). Carapace normalizes bare hosts by prepending
...
- Both providers return a `session_id`; Carapace surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- Carapace always forwards a resolved result count to Parallel as
  ...
  Carapace's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
