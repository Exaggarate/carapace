...
providers. In chat sessions it runs asynchronously: Carapace records a
...
session is inactive or its active wake fails, Carapace sends an idempotent
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, Carapace routes image requests
...
`carapace infer image generate` or `carapace infer image edit`.
...
nearby geometry option instead of the exact requested one, Carapace remaps to
...
For `image_generate`, Carapace tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Timeouts">
    ...
    by Carapace's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, Carapace reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Carapace
    ...
    Carapace forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Carapace maps
    ...
    valid. When only `aspectRatio` is specified, Carapace still selects
    ...
    transparency-capable OpenAI image model. Carapace routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so Carapace can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    Carapace forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. Carapace sends:
    ...
    Krea; Carapace maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Carapace-managed image attachments

    Carapace intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Carapace image generation" size=3840x2160 count=1
```
  </Tab>
...
on `carapace infer image edit`; `--openai-background` remains as an
