...
- Provider notes:
  ...
  - GPT-6 Astra and GPT-5.6 Sol and Terra expose native `/think ultra` through the Codex runtime with either Platform API-key or ChatGPT subscription auth. Carapace preserves Ultra for ordinary turns; `/btw` intentionally runs at `off`. Codex owns proactive delegation and the model-specific inference effort (Astra uses `xhigh`). Ultra is not sent as a raw Responses API reasoning effort. GPT-5.6 Luna exposes levels through `max` because its Codex catalog does not advertise Ultra.
  - The embedded Carapace runtime exposes logical `/think ultra` for GPT-6 Astra and GPT-5.6 Sol, Terra, and Luna. It sends provider max effort and adds run-scoped proactive sub-agent orchestration guidance.
...
- **Embedded Carapace**: the resolved level is passed to the in-process Carapace agent runtime.
...
- Carapace resolves fast mode in this order:
  ...
  6. Fallback: `off`
- Valid model-scoped `params.fastMode` / `params.fast_mode` values and valid cutoff keys are typed agent-runtime controls. They do not count as authored provider request params and do not select Carapace or Codex by themselves. Pin `agentRuntime.id: "carapace"` or `agentRuntime.id: "codex"` when a recipe depends on one runtime.
...
- For `openai/*`, fast mode maps to OpenAI API Fast mode (formerly Priority processing). Carapace currently sends `service_tier=priority` on supported Responses requests.
- On Codex harness turns, the shared runtime control supersedes a configured native app-server tier: Fast on sends `priority`, Fast off sends `null` to clear the Carapace-owned tier, and auto decides for each model call. A configured Codex tier is used only when no shared Fast-mode run control is supplied. See [Codex harness](/plugins/codex-harness/commands#shared-fast-mode-and-codex-fast-mode).
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. Carapace still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
- `/status` reports the resolved Carapace policy (`on`, `off`, or `auto`) and the selected runtime. It does not report the upstream service tier actually honored or returned for a completed request. See [OpenAI Fast mode](/providers/openai#advanced-configuration) for provider details.
...
- Provider plugins that proxy Claude models should reuse `resolveClaudeThinkingProfile(modelId)` from `carapace/plugin-sdk/provider-model-shared` so direct Anthropic and proxy catalogs stay aligned.
