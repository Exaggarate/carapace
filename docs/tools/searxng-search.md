...
Carapace supports [SearXNG](https://docs.searxng.org/) as a **self-hosted,
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    carapace plugins install @carapace/searxng-plugin
    ```
  </Step>
...
- **Category fallback** -- if a non-`general` category request succeeds but
  returns zero results, Carapace retries the same query once with `general`
