...
Numeric values must be positive integers. Carapace bounds
...
To use the `agents.run`, `phase`, and `log` guest globals, enable Carapace Code
...
hints only when its catalog contains the native Carapace `sessions_spawn`
...
permitted by the requester's `subagents.allowAgents` policy. Carapace rejects
...
Carapace starts up to `maxConcurrent` children for the group and queues the rest
in submission order.
...
`agents_wait`, or await `agents.run()` in Carapace Code Mode. Do not use
...
tool surface, cheaper model, or tighter sandbox policy. Carapace does not ship
...
For structured output, Carapace adds a synthetic `structured_output` tool to
...
You can use Swarm without Carapace Code Mode. Its core tools are
...
Codex Code Mode automatically exposes eligible dynamic Carapace tools under
`tools.*`. It does not use Carapace's QuickJS guest API or require
...
upgrading Carapace, so collector spawn fields remain unavailable on that thread.
...
With the currently supported Codex runtime, dynamic Carapace tool results reach
...
Carapace sub-agents; it does not support ACP runtime, thread binding, visible
