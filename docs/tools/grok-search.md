...
Carapace supports Grok as a `web_search` provider, using xAI web-grounded
...
lets Carapace reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `carapace onboard` or `carapace configure --section
web` lets Carapace reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

Carapace then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    carapace onboard --auth-choice xai-oauth
...
**Credential alternatives:** `carapace models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. Carapace
