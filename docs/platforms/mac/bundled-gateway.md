...
read_when:
  - Packaging Carapace.app
  - Debugging the macOS gateway launchd service
  - Installing the gateway CLI for macOS
title: "Gateway on macOS"
---

Carapace.app bundles a private Node runtime and matching Carapace package for
...
The **Gateway remains external**. The app uses an external `carapace` CLI to
...
user-space Node runtime and the matching `carapace` CLI under `~/.carapace`,
...
runtime and matching Carapace package. The bundled installer owns that setup;
...
Node 24.16+ or Node 26.1+. Install `carapace` globally:
...
Label: `ai.carapace.gateway` (default profile), or `ai.carapace.<profile>`
for a named profile.

Plist location (per-user): `~/Library/LaunchAgents/ai.carapace.gateway.plist`
(or `ai.carapace.<profile>.plist`).

The macOS app owns LaunchAgent install/update for the default profile in
Local mode. The CLI can also install it directly: `carapace gateway install`
(named profiles are selected via the `CARAPACE_PROFILE` env var).

Behavior:

- "Carapace Active" enables/disables the LaunchAgent.
...
effect. The override persists in `~/.carapace/disable-launchagent`; remove that
file to restore app-managed launchd behavior.

Logging:

- launchd stdout: `~/Library/Logs/carapace/gateway.log` (profiles use
  `gateway-<profile>.log`)
- launchd stderr: suppressed
- If the host loops with repeated `EADDRINUSE` or fast restarts, check for
  duplicate `ai.carapace.gateway` / `ai.carapace.node` LaunchAgents and the
...
Keep Carapace state on a local, non-synced disk. Avoid iCloud Drive and other
cloud-synced folders; sync latency and file locks can affect sessions,
credentials, and Gateway state.

Set `CARAPACE_STATE_DIR` to a local path only when you need an override.
`carapace doctor` warns about common cloud-synced state paths and recommends
...
carapace-mac primary show --json
carapace-mac gateway list --json
```

The app's CLI installer links `carapace-mac` beside its profile-managed
`carapace` command. You can also run
`/Applications/Carapace.app/Contents/MacOS/carapace-mac` directly. See
...
```bash
cd apps/macos
swift run carapace-mac connect --json
swift run carapace-mac discover --timeout 3000 --json
```
...
discovery output with `carapace gateway discover --json` when you need to
...
carapace gateway --port 18999 --bind loopback
```

Then:

```bash
carapace gateway call health --port 18999 --timeout 3000
```
