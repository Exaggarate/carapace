...
[`scripts/package-mac-app.sh`](https://github.com/Exaggarate/carapace/blob/main/scripts/package-mac-app.sh) builds a staged app, calls [`scripts/codesign-mac-app.sh`](https://github.com/Exaggarate/carapace/blob/main/scripts/codesign-mac-app.sh), and verifies the signed worker before replacing `dist/Carapace.app`. TCC permissions are tied to the bundle ID and code signature; keeping both stable (and the app at a fixed path) across rebuilds keeps macOS from forgetting TCC grants (notifications, accessibility, screen recording, mic, speech).

- Debug bundle identifier defaults to `ai.carapace.mac.debug` (override with `BUNDLE_ID=...`).
...
- Stamps Info.plist with `CarapaceBuildTimestamp` (ISO8601 UTC) and `CarapaceGitCommit` (full 40-character hexadecimal commit, or `unknown` for local builds when unavailable) so the standard About panel can show the build timestamp and git commit.
