...
- Same path: run a release app from `/Applications/Carapace.app`; keep development builds at one fixed path such as `dist/Carapace.app`.
- Same bundle identifier: release builds use `ai.carapace.mac`; development builds default to `ai.carapace.mac.debug`. Each has a separate permission identity.
...
1. Click **Grant** in Carapace.
2. If macOS opens System Settings, enable the running Carapace app under **Privacy & Security -> Screen & System Audio Recording** (called **Screen Recording** on older macOS versions).
3. Return to Carapace and retry the screenshot. **Dashboard → Settings → This Mac → Permissions** shows the refreshed access status.

After an explicit **Grant** request, Carapace checks ScreenCaptureKit as well as the macOS permission preflight. This lets it recognize access when the preflight still reports an old denial. Passive status checks do not initiate this probe before you request access.

If access still appears missing, quit and reopen Carapace from the same app path. Some macOS permission changes require an app restart before capture works. If both release and development builds are installed, grant access to the build you are actually running: approving `/Applications/Carapace.app` does not grant access to a development build with a different bundle identifier.

## Accessibility grants for Node and CLI runtimes

Prefer granting Accessibility to Carapace.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
macOS keeps Accessibility, Event Posting, input listening, and Screen Recording in separate TCC buckets. One successful grant does not prove the others are usable. Carapace's Computer Control status checks Accessibility, Event Posting, and Screen Recording separately; this is why screenshots can succeed while clicks and typing fail.

An Accessibility row can also remain visibly enabled while its code requirement is pinned to an older build. When Carapace reports **Accessibility grant may be stale**, select Carapace under **System Settings -> Privacy & Security -> Accessibility**, remove it with **-**, then re-add `/Applications/Carapace.app`. Quit and reopen Carapace afterward because Accessibility trust can remain cached in the running process.
...
Example resets (using Carapace's bundle ID, `ai.carapace.mac`):

```bash
sudo tccutil reset Accessibility ai.carapace.mac
sudo tccutil reset ScreenCapture ai.carapace.mac
...
Workaround: move files into the Carapace workspace (`~/.carapace/workspace`) if you want to avoid per-folder grants.
