...
Carapace can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the Carapace iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the Carapace watchOS app is
...
`carapace.json`. Preserve any commands already present:
...
3. Read the disclosure, then choose which Health categories Carapace may read
   in Apple's permission sheet.

The switch records your explicit Carapace sharing choice. It does not claim
...
```bash
carapace nodes describe --node "<iOS device name>"
```
...
- Carapace requests read access only. It cannot add or modify Health data.
- Carapace reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. Carapace cannot distinguish those cases.
...
Run `carapace nodes pending` and approve any capability update, then inspect
`carapace nodes describe --node "<iOS device name>"` again.
...
Carapace's access in Apple's Health settings, but do not treat an empty result
