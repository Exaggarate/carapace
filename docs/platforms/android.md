...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.carapace.app&hl=en_IN) and as a signed standalone APK on supported [GitHub Releases](https://github.com/Exaggarate/carapace/releases). It is a companion node and requires a running Carapace Gateway. Source: [apps/android](https://github.com/Exaggarate/carapace/tree/main/apps/android) ([build instructions](https://github.com/Exaggarate/carapace/blob/main/apps/android/README.md)).
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.carapace.app&hl=en_IN) or `Carapace-Android.apk` from a supported [GitHub Release](https://github.com/Exaggarate/carapace/releases), [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- **Settings → Carapace** opens a dedicated Gateway settings assistant when the operator connection has `operator.admin` and the Gateway supports `carapace.chat`. Its setup conversation stays separate from ordinary Chat, redacts secret replies locally, and moves to Chat only after you tap **Open Chat**.
...
Regular final and correction GitHub Releases include a universal `Carapace-Android.apk` and `Carapace-Android-SHA256SUMS.txt`. The APK is built from the release tag, signed with the Carapace Android release key, and carries GitHub Actions provenance.

Choose a [release](https://github.com/Exaggarate/carapace/releases) that lists both assets, then download and verify that exact tag before sideloading:
...
sha256sum --check Carapace-Android-SHA256SUMS.txt
gh attestation verify Carapace-Android.apk \
  --repo carapace/carapace \
  --signer-workflow carapace/carapace/.github/workflows/android-release.yml \
...
workflow, separate from the Carapace node connection. It is useful when the Android device and the
...
- The `carapace` CLI available on the gateway machine (or via SSH), to approve pairing requests.
...
```bash
carapace gateway discover --json
```
...
1. Set up a DNS-SD zone (example `carapace.internal.`) on the gateway host and publish `_carapace-gw._tcp` records.
...
with `carapace qr`, then scan or paste it on that page and reconnect. Operators
...
Android supports launching Carapace from the system assistant trigger (Google Assistant). Holding the home button (or another `ACTION_ASSIST` trigger) opens the app; saying "Hey Google, ask Carapace `<prompt>`" matches the app's declared App Actions query pattern and hands the prompt into the chat composer without auto-sending it.
...
<Note>
App Actions availability depends on the device, Google Play Services version, and whether the user has set Carapace as the default assistant app.
</Note>

## Notification forwarding

Android can forward device notifications to the gateway as `node.event` items. This is configured **on the device**, in the app's Settings sheet — not in gateway/`carapace.json` config.

| Setting                     | Description                                                                                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Forward Notification Events | Master toggle. Off by default; requires Notification Listener Access to be granted first.                                                                                                              |
| Package Filter              | **Allowlist** (only listed package IDs forwarded) or **Blocklist** (default: all packages except listed IDs). Carapace's own package is always excluded in Blocklist mode to prevent forwarding loops. |
...
WhatsApp, WhatsApp Business, Telegram, Telegram X, Discord, and Signal notifications are always excluded. Their messages are already owned by native Carapace channel sessions; forwarding the Android notification as a separate node event could route a reply through the wrong conversation.
