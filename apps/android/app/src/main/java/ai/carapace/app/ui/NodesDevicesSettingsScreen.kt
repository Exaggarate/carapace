...
import ai.carapace.app.GatewayDevicePairingCapabilities
import ai.carapace.app.GatewayDevicePairingMutation
import ai.carapace.app.GatewayDeviceTokenSummary
...
import ai.carapace.app.GatewayPairedDeviceSummary
import ai.carapace.app.GatewayPendingDeviceSummary
import ai.carapace.app.MainViewModel
import ai.carapace.app.canApproveGatewayDevicePairing
...
private fun nodeSubtitle(node: GatewayNodeSummary): String {
  ...
  val version = node.version?.let { "Carapace $it" }
...
internal fun devicePairingAdminUnavailableText(): String =
  nativeString(
    "Device pairing actions are unavailable in this Gateway session. Run carapace devices list on the Gateway host and manage the request there. Node capability approval is separate and still uses nodes approve <request id>.",
  )
