...
import ai.carapace.app.gateway.GatewayBootstrapHandoff
import ai.carapace.app.gateway.GatewayCustomHeaders
import ai.carapace.app.gateway.GatewayRegistryStore
import ai.carapace.app.gateway.GatewayStoreMigration
import ai.carapace.app.node.parseHexColorArgb
...
class SecurePrefs(
  ...
  companion object {
    ...
    private const val plainPrefsName = "carapace.node"
    private const val securePrefsName = "carapace.node.secure"
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block Carapace's own notifications in blocklist mode to prevent forwarding loops.
