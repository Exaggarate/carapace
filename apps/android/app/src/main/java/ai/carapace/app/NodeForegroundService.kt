...
import ai.carapace.app.i18n.nativeLocaleChanges
...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    ...
    val initial =
      buildNotification(
        title = nativeString("Carapace Node"),
      ...
      scope.launch {
        val runtime =
          try {
            ...
            Log.e("CarapaceNodeService", "Failed to restore node runtime", err)
      ...
      val title =
        when {
          state.connected && state.mode == VoiceCaptureMode.TalkMode -> {
            nativeString("Carapace Node · Talk")
          }

          state.connected -> {
            nativeString("Carapace Node · Connected")
          }

          else -> {
            nativeString("Carapace Node")
          }
        }
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = nativeString("Carapace node connection status")
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.carapace.app.action.STOP"
    private const val ACTION_RESUME = "ai.carapace.app.action.RESUME"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.carapace.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.carapace.app.extra.VOICE_CAPTURE_MODE"
