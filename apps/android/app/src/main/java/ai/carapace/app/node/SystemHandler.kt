...
import ai.carapace.app.mainActivityPendingIntent
...
private const val NOTIFICATION_CHANNEL_BASE_ID = "carapace.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> {
          Triple("passive", NotificationManager.IMPORTANCE_LOW, nativeString("Carapace Passive"))
        }
        ...
        "timesensitive" -> {
          Triple(
            ...
            nativeString("Carapace Time Sensitive"),
          )
        }

        else -> {
          Triple("active", NotificationManager.IMPORTANCE_DEFAULT, nativeString("Carapace Active"))
        }
      }
  ...
  fun handleSystemNotify(paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return try {
      ...
      GatewaySession.InvokeResult.error(
        code = "NOT_AUTHORIZED",
        message = "NOT_AUTHORIZED: enable Carapace notifications and the selected priority in Android Settings",
      )
    } catch (err: Throwable) {
      GatewaySession.InvokeResult.error(
        code = "UNAVAILABLE",
