...
import ai.carapace.app.gatewayConnectionStatusForDisplay
...
internal fun carapaceAndroidVersionLabel(): String {
...
internal fun gatewayNodeApprovalCommand(
  ...
  when (state) {
    ...
    -> normalizeGatewayApprovalRequestId(requestId)?.let { "carapace nodes approve $it" } ?: "carapace nodes status"

    GatewayNodeApprovalState.Unapproved -> "carapace nodes status"
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return nativeString(
    "Help diagnose this Carapace Android gateway connection failure.\n\n" +
      "Please:\n" +
      ...
      "- tell me whether `carapace devices list` should show a pending pairing request\n" +
      "- if more signal is needed, ask for `carapace qr --json`, `carapace devices list`, and `carapace nodes status`\n" +
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Carapace gateway diagnostics", report))
