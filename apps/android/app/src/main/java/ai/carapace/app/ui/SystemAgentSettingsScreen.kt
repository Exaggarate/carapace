...
import ai.carapace.app.systemagent.SystemAgentChatMessage
import ai.carapace.app.systemagent.SystemAgentChatQuestion
import ai.carapace.app.systemagent.SystemAgentChatQuestionOption
...
private fun SystemAgentAccessGate(state: SystemAgentChatState) {
  ...
  val detail =
    when (state.access) {
      SystemAgentChatAccess.Disconnected -> nativeString("Connect this phone to a Gateway before opening Carapace.")
      ...
      SystemAgentChatAccess.CheckingGateway -> nativeString("Checking whether this Gateway supports the Carapace settings assistant.")
      SystemAgentChatAccess.GatewayUpdateRequired -> nativeString("Update this Gateway to use the Carapace settings assistant.")
      SystemAgentChatAccess.Ready -> ""
    }
      ...
      items(rows, key = { it.key }) { row ->
        when (row) {
          ...
          SystemAgentConversationRow.Working -> {
            Text(nativeString("Carapace is working…"), style = ClawTheme.type.caption, color = ClawTheme.colors.textMuted)
          }
    ...
    state.handoff?.let {
      ClawPanel(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
          Text(nativeString("Carapace is ready to continue in your ordinary chat."), style = ClawTheme.type.body, color = ClawTheme.colors.textMuted)
    ...
    ClawTextField(
      ...
      placeholder =
        if (state.expectsSensitiveReply) {
          ...
          nativeString("Reply to Carapace…")
        },
