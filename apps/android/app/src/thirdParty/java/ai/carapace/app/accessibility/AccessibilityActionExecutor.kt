package ai.carapace.app.accessibility
...
class AccessibilityActionExecutor internal constructor(
  private val connectionProvider: () -> AccessibilityServiceConnection<CarapaceAccessibilityService> = {
    CarapaceAccessibilityService.connection.value
  },
  private val captureSnapshot: (CarapaceAccessibilityService) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val foregroundPackageProvider: (CarapaceAccessibilityService) -> String? =
    CarapaceAccessibilityService::foregroundPackageName,
  private val uiEpochProvider: () -> Long = { CarapaceAccessibilityService.uiEpoch },
) : AutoCloseable {
  ...
  private fun coordinateGesturePreflight(service: CarapaceAccessibilityService): ActionResult? {
  ...
  private fun nodeActionPackagePreflight(service: CarapaceAccessibilityService): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = foregroundPackageProvider(service)
    if (expectedPackage == null || currentPackage == null || expectedPackage != currentPackage) {
      return ActionResult(
  ...
  private suspend fun dispatchGesture(
    service: CarapaceAccessibilityService,
