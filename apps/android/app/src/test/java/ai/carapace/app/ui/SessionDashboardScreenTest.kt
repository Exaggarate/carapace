...
import ai.carapace.app.buildNodeMainSessionKey
...
class SessionDashboardScreenTest {
  ...
  fun originRuleDropsBasePathAndKeepsPort() {
    assertEquals(
      ...
      controlUiOriginRule("https://gateway.example.com:8443/carapace"),
    )
  ...
  fun dashboardUrlKeepsConfiguredControlUiBasePathAndDropsOldQuery() {
    val url =
      sessionDashboardUrl(
        baseUrl = "https://gateway.example.com:8443/carapace?stale=true#old",
        sessionKey = "agent:main:qa",
      )

    assertEquals(
      "https://gateway.example.com:8443/carapace/dashboard/main/~key/qa",
      url,
    )
  }
