...
import ai.carapace.app.chat.parseChatMessageContent
...
class ChatMessageViewsTest {
  ...
  fun transcriptBubblesExposeSpeakerWithoutReplacingMessageText() {
    ...
    val assistantBubble = composeRule.onNode(hasContentDescription("Carapace") and hasText("assistant body")).assertExists()
    composeRule.onNode(hasContentDescription("System") and hasText("system body")).assertExists()
    composeRule.onNode(hasContentDescription("Carapace") and hasText("live body")).assertExists()
    ...
    composeRule.onAllNodesWithText("Carapace", useUnmergedTree = true).assertCountEquals(0)
    ...
    composeRule.onAllNodesWithText("Carapace · Live", useUnmergedTree = true).assertCountEquals(1)
  ...
  fun attachmentOnlyUserTurnsRetainEntryActionsWithoutEmptyTextActions() {
    ...
    listOf("assistant.pdf", "unpersisted.pdf", "disabled.pdf").forEach { fileName ->
      val speaker = if (fileName == "assistant.pdf") "Carapace" else "You"
      val semantics =
        composeRule
  ...
  fun outboxBubbleExposesSpeakerWithoutReplacingStatusOrActions() {
    ...
    composeRule
      .onNode(
        hasContentDescription("Carapace") and
  ...
  fun attachmentOnlyAssistantTurnShowsDocumentFilenameWithoutLoadingItsUrl() {
    ...
    composeRule
      .onNode(hasContentDescription("Carapace") and hasText("quarterly-report.pdf"))
      .assertIsDisplayed()
  ...
  fun omittedImageOnlyTurnsRemainVisibleWithoutLoadingBeyondTheImageCap() {
    ...
    composeRule.onNode(hasContentDescription("Carapace") and hasText("Attachment")).assertIsDisplayed()
