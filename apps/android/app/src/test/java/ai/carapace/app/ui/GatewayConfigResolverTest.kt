...
class GatewayConfigResolverTest {
  ...
  fun parseGatewayEndpointPreservesDecodedContextPath() {
    val parsed = parseGatewayEndpoint("wss://gateway.example/carapace%20gateway")

    assertEquals("/carapace%20gateway", parsed?.contextPath)
    assertEquals("https://gateway.example/carapace%20gateway", parsed?.displayUrl)
  }

  @Test
  fun parseGatewayEndpointPreservesEscapedPathDelimiter() {
    val parsed = parseGatewayEndpoint("wss://gateway.example/carapace%2Fgateway")

    assertEquals("/carapace%2Fgateway", parsed?.contextPath)
    assertEquals("https://gateway.example/carapace%2Fgateway", parsed?.displayUrl)
  }

  @Test
  fun parseGatewayEndpointPreservesRepeatedLeadingPathSlashes() {
    val parsed = parseGatewayEndpoint("wss://gateway.example//carapace")

    assertEquals("//carapace", parsed?.contextPath)
    assertEquals("https://gateway.example//carapace", parsed?.displayUrl)
  }
  ...
  fun resolveGatewayConnectConfigPreservesSetupContextPath() {
    val resolved =
      resolveConnectConfigFixture(
        useSetupCode = true,
        setupCode = setupCode("wss://gateway.example/carapace-gw"),
      )

    assertEquals("gateway.example", resolved?.host)
    assertEquals(443, resolved?.port)
    assertEquals(true, resolved?.tls)
    assertEquals("/carapace-gw", resolved?.contextPath)
  }
  ...
  fun composeGatewayManualUrlRejectsPastedAuthorityUserInfoQueriesAndFragments() {
    val hosts =
      listOf(
        ...
        "wss://user@gateway.example/carapace-gw",
        "wss://gateway.example/carapace-gw?mode=manual",
        "wss://gateway.example/carapace-gw#fragment",
      )
