...
import ai.carapace.app.MainActivity
...
class SystemHandlerTest {
  ...
  fun handleSystemNotify_rejectsUnauthorized() {
    ...
    for (permissionGranted in listOf(false, true)) {
      ...
      val result = handler.handleSystemNotify("""{"title":"Carapace","body":"hi"}""")
  ...
  fun handleSystemNotify_rejectsInvalidRequestObject() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Carapace"}""")
  ...
  fun handleSystemNotify_postsNotification() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Carapace","body":"done","priority":"active"}""")
  ...
  fun handleSystemNotify_rejectsBlockedSelectedChannels() {
    ...
    for ((priority, suffix) in priorities) {
      val channelId = "carapace.system.notify.$suffix"
      ...
      val result = handler.handleSystemNotify("""{"title":"Carapace","body":"blocked"$priorityField}""")
  ...
  fun handleSystemNotify_postsAllowedChannelWhenAnotherPriorityIsBlocked() {
    ...
    manager.createNotificationChannel(
      NotificationChannel("carapace.system.notify.active", "Blocked", NotificationManager.IMPORTANCE_NONE),
    )
    val handler = SystemHandler(context)

    val result = handler.handleSystemNotify("""{"title":"Carapace","body":"allowed","priority":"passive"}""")

    assertTrue(result.ok)
    assertEquals(NotificationManager.IMPORTANCE_LOW, manager.getNotificationChannel("carapace.system.notify.passive").importance)
    val notification = manager.activeNotifications.single().notification
    assertEquals("carapace.system.notify.passive", notification.channelId)
  ...
  fun handleSystemNotify_trimsAndPassesOptionalFields() {
    ...
    val result =
      handler.handleSystemNotify(
        """{"title":" Carapace ","body":" done ","priority":" passive ","sound":" silent "}""",
      )

    assertTrue(result.ok)
    assertEquals("Carapace", poster.lastRequest?.title)
  ...
  fun buildSystemNotificationSetsImmutableAppLaunchIntent() {
    ...
    val notification =
      buildSystemNotification(
        appContext = context,
        channelId = "test",
        request = SystemNotifyRequest("Carapace", "done", sound = null, priority = null),
      )
  ...
  fun handleSystemNotify_returnsUnauthorizedWhenPostFailsPermission() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Carapace","body":"done"}""")
