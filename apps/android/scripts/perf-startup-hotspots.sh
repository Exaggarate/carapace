...
PACKAGE="ai.carapace.app.debug"
ACTIVITY="ai.carapace.app.MainActivity"
DURATION_SECONDS="10"
OUTPUT_PERF_DATA=""
...
Default package/activity target Carapace Android startup.
EOF
}
...
if [[ -z "$OUTPUT_PERF_DATA" ]]; then
  OUTPUT_PERF_DATA="/tmp/carapace-startup-$(date +%Y%m%d-%H%M%S).perf.data"
fi
...
tmp_dir="$(mktemp -d -t carapace-android-hotspots.XXXXXX)"
