...
class CarapaceTileService :
  Material3TileService(
    allowDynamicTheme = false,
    defaultColorScheme = carapaceTileColorScheme,
  ) {
  override suspend fun MaterialScope.tileResponse(requestParams: RequestBuilders.TileRequest): TileBuilders.Tile {
    val talkAction = wearLaunchAction(this@CarapaceTileService, WearLaunchTarget.Voice)
    val openAction = wearLaunchAction(this@CarapaceTileService, WearLaunchTarget.Chat)
    val talkClickable = clickable(action = talkAction, id = "talk_carapace")
    val openClickable = clickable(action = openAction, id = "open_carapace")
    val layout =
      primaryLayout(
          ...
          avatarButton(
            ...
            avatarContent = {
              avatarImage(
                ...
                protoLayoutResourceId = "carapace_core_mascot",
              )
            },
            labelContent = {
              text(
...
private val carapaceTileColorScheme =
  ColorScheme(
