...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "gateway-discovery")
    ...
    public func start() {
        ...
        self.browserSession.start(
            queueLabelPrefix: "ai.carapace.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = CarapaceBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = CarapaceBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = CarapaceBonjour.wideAreaGatewayServiceDomain else { return false }
