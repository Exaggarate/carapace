...
/// An explicit config path wins; otherwise the selected state directory owns carapace.json.
func resolveCarapaceConfigURL() -> URL {
    if let configPath = carapaceEnvironmentPath("CARAPACE_CONFIG_PATH") {
        return URL(fileURLWithPath: NSString(string: configPath).expandingTildeInPath)
    }
    let stateDir = carapaceEnvironmentPath("CARAPACE_STATE_DIR").map {
        URL(fileURLWithPath: NSString(string: $0).expandingTildeInPath, isDirectory: true)
    } ?? FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".carapace", isDirectory: true)
    return stateDir.appendingPathComponent("carapace.json")
}

private func carapaceEnvironmentPath(_ key: String) -> String? {
...
func loadGatewayConfig() -> GatewayConfig {
    guard let data = try? Data(contentsOf: resolveCarapaceConfigURL()) else { return GatewayConfig() }
