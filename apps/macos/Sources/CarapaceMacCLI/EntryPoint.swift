...
struct CarapaceMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("carapace-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      carapace-mac status [--json] [--profile <name>] [--no-launch]
      carapace-mac primary show|set|clear [options]
      carapace-mac gateway list|add|remove|reconnect [options]
        Run carapace-mac primary --help for app control options.
      carapace-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      carapace-mac discover [--timeout <ms>] [--json] [--include-local]
      carapace-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      carapace-mac connect
      carapace-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      carapace-mac connect --url ws://127.0.0.1:18789 --json
      carapace-mac discover --timeout 3000 --json
      carapace-mac wizard --mode local
    """)
}
