...
private let appDefaultsSuites = ["ai.carapace.mac", "ai.carapace.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              carapace-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              carapace-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
                                          [--password <password>] [--project-root <path>] [--cli-path <path>] [--json]

            Offline preconfiguration; prefer carapace-mac primary set when the app is running.
            ...
            Options:
              ...
              --project-root <p>  Remote Carapace checkout for CLI commands.
              --cli-path <path>   Remote carapace executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = resolveCarapaceConfigURL()
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, targetChanged: Bool, suites: [String]) {
    for suite in suites {
        ...
        let previousTarget = defaults.string(forKey: "carapace.remoteTarget") ?? ""
        if targetChanged || previousTarget != target {
            for key in ["carapace.remoteIdentity", "carapace.remoteProjectRoot", "carapace.remoteCliPath"] {
                defaults.removeObject(forKey: key)
            }
        }
        defaults.set("remote", forKey: "carapace.connectionMode")
        setDefaultString(defaults, key: "carapace.remoteTarget", value: target)
        defaults.set(true, forKey: "carapace.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "carapace.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "carapace.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "carapace.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "carapace.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("Carapace macOS Remote Config")
