...
func runMacControl(_ args: [String]) {
    do {
           ...
           let text = String(data: data, encoding: .utf8)
        {
            ...
            fputs("carapace-mac: \(controlError.message)\n", stderr)
        }
...
private func printMacControlResult(_ data: Data, operation: String, primaryOnly: Bool) throws {
    ...
    if operation == "status", !primaryOnly {
        ...
        print("Carapace \(status.app.version) (\(status.app.build)) · profile \(status.app.profile)")
...
private func printMacControlUsage() {
    print("""
    carapace-mac app control

    Usage:
      carapace-mac status
      carapace-mac primary show|clear
      carapace-mac primary set --ssh-target user@host[:port]
        [--remote-port PORT] [--local-port PORT] [--identity PATH]
        [--ssh-host-key-policy strict|openssh] [secret options]
      carapace-mac primary set --direct-url ws://host|wss://host [--tls-fingerprint SHA256] [secret options]
      carapace-mac primary set --local
      carapace-mac gateway list
      carapace-mac gateway add NAME --url https://host[/base]|wss://host [--browser] [secret options]
      carapace-mac gateway remove ID_OR_NAME [--yes]
      carapace-mac gateway reconnect ID_OR_NAME

    Global options (before or after commands):
      --profile NAME     Overrides CARAPACE_PROFILE; default profile when unset.
