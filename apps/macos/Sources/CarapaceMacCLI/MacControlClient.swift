...
struct MacControlClient {
    ...
    static func applicationBundle(executableURL: URL) -> URL {
        ...
        return URL(fileURLWithPath: "/Applications/Carapace.app")
    }
    ...
    private func launchApp(deadline: ContinuousClock.Instant) throws {
        ...
        process.arguments = [
            "-gj", "-n", "-a", bundle.path,
            "--env", "CARAPACE_PROFILE=\(self.options.profile.name ?? "default")", "--args", "--background-only",
        ]
    ...
    private static func unreachable() -> MacControlError {
        MacControlError(
            code: "unreachable",
            message: """
            Carapace app is not reachable for this profile. \
