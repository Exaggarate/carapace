...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.carapace", category: "gateway.env")
    ...
    private static func selectedGatewayPort(root: [String: Any] = CarapaceConfigFile.loadDict()) -> Int {
        self.resolvedGatewayPort(
            environment: ProcessInfo.processInfo.environment,
            configPort: CarapaceConfigFile.gatewayPort(root: root),
    ...
    {
        if let raw = environment["CARAPACE_GATEWAY_PORT"] {
    ...
    private static func resolveEnvironment(searchPaths: [String]) async -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.carapaceExecutable(searchPaths: searchPaths)
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "carapace CLI not found in PATH; install the CLI.")
            }
            ...
            if let gatewayBin, installedRaw == nil {
                let message = "Carapace Gateway at \(gatewayBin) could not be verified; reinstall or repair it."
                return GatewayEnvironmentStatus(
                    kind: .error(message),
                    nodeVersion: runtime.version.description,
                    gatewayVersion: nil,
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("carapace ") {
            normalized = String(normalized.dropFirst("carapace ".count))
        }
