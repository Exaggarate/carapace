...
extension OnboardingView {
    ...
    func startExistingCLIActivationIfNeeded() {
        ...
        OnboardingController.shared.busyReason = "Carapace is starting the Gateway service."
        cliStatus = "Starting Carapace Gateway…"
        ...
        afterFreshInstall: Bool) -> (ready: Bool, status: String)
    {
        switch result {
        case .ready:
            (true, "Carapace Gateway is ready.")
        case .deferred:
            (false, "Carapace is paused. Resume it, then retry setup to start the Gateway.")
        case let .failed(reason):
            (false, Self.gatewayStartFailureMessage(
                prefix: "Carapace \(afterFreshInstall ? "was" : "is") installed, " +
    ...
    func runCLIInstall() async {
        ...
        OnboardingController.shared.busyReason = "Carapace is installing the Gateway service."
