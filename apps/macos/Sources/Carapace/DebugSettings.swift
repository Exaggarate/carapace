...
struct DebugSettings: View {
        ...
        Section("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(String(
                    ...
                    When enabled, Carapace won't install or manage %@. \
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "carapace://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (carapace://…) are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
    ...
    private var pathsSection: some View {
        Section("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Carapace project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to carapace repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/carapace").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.carapace/carapace.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        Section("Quick actions") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting Carapace after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    if AppProfile.current.isActive {
                        ...
                        Button {
                            LaunchdManager.startCarapace()
                        } label: {
                            Label("Restart Carapace", systemImage: "arrow.counterclockwise")
                        }
                        .buttonStyle(.bordered)
                        .controlSize(.small)
                    }
    ...
    private func loadSessionStorePath() {
        let parsed = CarapaceConfigFile.loadDict()
        guard
