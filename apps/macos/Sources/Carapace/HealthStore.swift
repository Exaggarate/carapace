...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "ai.carapace", category: "health")
    ...
    private var presentation: (state: HealthState, summary: String) {
        ...
        if link.summary.linked == false {
            ...
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return (.degraded("Not linked"), "\(label) ok · Not linked — run carapace login")
            }
            return (.linkingNeeded, "Not linked — run carapace login")
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart Carapace to bring it back."
            }
    ...
    func describeFailure(from snap: HealthSnapshot, fallback: String?) -> String {
        if let link = self.resolveHealthChannel(snap) {
            if link.summary.linked == false { return "Not linked — run carapace login" }
