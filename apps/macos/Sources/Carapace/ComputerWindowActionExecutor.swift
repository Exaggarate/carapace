...
extension CarapaceComputerActParams {
...
extension CarapaceComputerAction {
...
final class ComputerWindowActionExecutor {
    ...
    private func listApps() async throws -> CarapaceComputerActResult {
        ...
        return CarapaceComputerActResult(ok: true, details: [
    ...
    private func listWindows() async throws -> CarapaceComputerActResult {
        ...
        return CarapaceComputerActResult(ok: true, details: details)
    }

    private func getAccessibilityTree(
        _ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult
    {
    ...
    private func getCursorPosition() -> CarapaceComputerActResult {
        ...
        _ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult
    {
        ...
        let elements = zip(bounded, issued.elementRefs).map { element, ref in
            CarapaceComputerObservationElement(
                elementRef: ref,
        ...
        return CarapaceComputerActResult(
            ok: true,
            observation: CarapaceComputerObservation(
                kind: "window",
    ...
    private func launchApp(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
        ...
        return CarapaceComputerActResult(ok: true, effect: .confirmed, details: [
            "name": AnyCodable(launched.name),
            "pid": AnyCodable(Int(launched.processIdentifier)),
        ])
    }

    private func killApp(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
        ...
        _ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult
    {
        ...
        return CarapaceComputerActResult(
            ...
            escalation: confirmed ? nil : CarapaceComputerEscalation(
                recommended: "desktop",
    ...
    private func click(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
        let target = try self.requiredWindow(params)
        ...
        if mode == .foreground {
            ...
            return CarapaceComputerActResult(ok: true, effect: .unverifiable, details: [
    ...
    private func type(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
    ...
    private func key(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
    ...
    private func setValue(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
    ...
    private func invokeMenu(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
        guard params.deliveryMode != .foreground else {
            throw ComputerActionService.ComputerActionError.unsupportedAction(params.action)
        }
        let target = try self.requiredWindow(params)
    ...
    private func scroll(_ params: CarapaceComputerActParams) async throws -> CarapaceComputerActResult {
        let windowRef = try Self.require(params.windowRef, field: "windowRef")
        let target = try self.resolveWindow(windowRef)
    ...
    private func requiredWindow(_ params: CarapaceComputerActParams) throws -> WindowTarget {
    ...
    private func focusElement(
        _ elementRef: String,
        params: CarapaceComputerActParams,
        target: WindowTarget,
        foreground: Bool) async throws -> CarapaceComputerActResult
    {
        let windowRef = try Self.require(params.windowRef, field: "windowRef")
        let observation = try self.resolveObservation(params.observationId, windowRef: windowRef)
        ...
        if foreground {
            ...
            return CarapaceComputerActResult(ok: true, effect: .unverifiable)
        }
        guard let identity = target.window.mutationIdentity else {
            throw ComputerActionService.ComputerActionError.staleObservation
        }
...
extension ComputerWindowActionExecutor {
    private static func result(
        ...
        background: Bool) -> CarapaceComputerActResult
    {
        ...
        if outcome.effect == .refused {
            return background
                ...
                : CarapaceComputerActResult(
                    ...
                    escalation: CarapaceComputerEscalation(
                        recommended: "desktop",
                        reasonCode: "foreground_ineffective"))
        }
        let effect: CarapaceComputerActionEffect = switch outcome.effect {
        ...
        let escalation = background && outcome.effect == .suspectedNoop
            ? CarapaceComputerEscalation(recommended: "foreground", reasonCode: "suspected_noop")
            : nil
        var details: [String: AnyCodable] = [
    ...
    private static func backgroundEscalation(reason: String) -> CarapaceComputerActResult {
        CarapaceComputerActResult(
            ok: false,
            effect: .suspectedNoop,
            escalation: CarapaceComputerEscalation(
    ...
    private static func observationLimits(
        _ params: CarapaceComputerActParams) throws -> (depth: Int, maxElements: Int)
    {
    ...
    private static func bounds(_ rect: CGRect) -> CarapaceComputerBounds {
        CarapaceComputerBounds(
    ...
    private static func scrollDirection(
        _ direction: CarapaceComputerScrollDirection) -> PeekabooFoundation.ScrollDirection
    {
        switch direction {
        case .up: .up
        case .down: .down
