...
struct TailscaleIntegrationSection: View {
    ...
    private var passwordRow: some View {
        LabeledContent {
            ...
            Text("Password")
            Text("Stored in ~/.carapace/carapace.json. Prefer CARAPACE_GATEWAY_PASSWORD for production.")
        }
    }
        ...
        isPaused: Bool) -> GatewayTailscaleApplyMessages
    {
        ...
        let statusMessage = if connectionMode == .local, !isPaused {
            "Saved to ~/.carapace/carapace.json. Restarting gateway…"
        } else {
            "Saved to ~/.carapace/carapace.json. Restart the gateway to apply."
        }
        return GatewayTailscaleApplyMessages(
