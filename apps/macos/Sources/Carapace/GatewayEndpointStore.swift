...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.carapace", category: "gateway-endpoint")
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        ...
        let raw = env["CARAPACE_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
                ...
                !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "CARAPACE_GATEWAY_PASSWORD",
    ...
    {
        let serviceEnv = launchdSnapshot?.environment ?? [:]
        let raw = env["CARAPACE_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = AppDefaults.standard.bool(forKey: "carapace.onboardingSeen")
...
extension GatewayEndpointStore {
    ...
    {
        if let envBind = env["CARAPACE_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["CARAPACE_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
...
extension GatewayEndpointStore {
    static func localConfig() -> GatewayConnection.Config {
        self.localConfig(
            root: CarapaceConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment,
            launchdSnapshot: GatewayLaunchAgentManager.launchdConfigSnapshot(),
