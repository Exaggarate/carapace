...
struct StatusMenuHeaderView: View {
    ...
    private var statusHeading: some View {
        HStack(alignment: .center, spacing: 8) {
            ...
            Toggle(String(localized: "Carapace active"), isOn: Binding(
                ...
                .accessibilityLabel(String(localized: "Carapace active"))
        }
    }
    ...
    private var statusTitle: String {
        if self.state.connectionMode == .unconfigured {
            return String(localized: "Carapace Not Configured")
        }
        if self.state.isPaused {
            return String(localized: "Carapace Paused")
        }
        if self.isSleeping {
            return String(localized: "Carapace Sleeping")
        }
    ...
    private var macNodeStatus: (label: String, diagnostic: String?, color: Color)? {
        ...
        if let node = self.nodesStore.nodes.first(where: { $0.nodeId == deviceID }) {
            ...
            let requiredCommands = [
                ...
                CarapaceSystemCommand.which.rawValue,
            ]
