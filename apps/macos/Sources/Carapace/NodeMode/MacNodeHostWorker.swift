...
extension Notification.Name {
    static let carapaceNodeHostManifestChanged = Notification.Name("carapace.node-host-worker.manifest-changed")
    static let carapaceNodeHostWorkerFailed = Notification.Name("carapace.node-host-worker.failed")
    static let carapaceNodeHostWorkerRetryExhausted = Notification.Name(
        "carapace.node-host-worker.retry-exhausted")
}
...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.carapace.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.carapace.node-host-worker.writer")
    ...
    private func startLocked(launch: MacNodeHostWorkerLaunch) {
        ...
        environment["CARAPACE_NODE_EXEC_HOST"] = "app"
        environment["CARAPACE_NODE_EXEC_FALLBACK"] = "0"
    ...
    private func handleMessageLocked(_ message: [String: Any]) {
        ...
        case "ready", "manifest":
            ...
            if type == "ready" {
                ...
                NotificationCenter.default.post(name: .carapaceNodeHostManifestChanged, object: nil)
            }
        case "invoke-result":
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = CarapaceNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : CarapaceNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: CarapaceNodeError(code: .unavailable, message: message))
    }
