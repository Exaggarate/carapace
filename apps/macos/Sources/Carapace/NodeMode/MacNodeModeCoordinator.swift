...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "mac-node")
    ...
    override private convenience init() {
        ...
        let nodeHostWorker = MacNodeHostWorker(session: session) { generation in
            NotificationCenter.default.post(name: .carapaceNodeHostWorkerFailed, object: NSNumber(value: generation))
        }
        self.init(
            session: session,
        ...
        nodeHostWorkerRetryPolicy: MacNodeHostWorkerRetryPolicy = MacNodeHostWorkerRetryPolicy())
    {
        ...
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .carapacePermissionsChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostManifestChanged),
            name: .carapaceNodeHostManifestChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostWorkerFailed),
            name: .carapaceNodeHostWorkerFailed,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .carapaceConfigDidChange,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .carapaceCuaDriverAvailabilityChanged,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = CarapaceConfigFile.browserControlEnabled()
        ...
        claudeSessionCatalogEnabled: Bool) async throws -> ConnectionAttempt?
    {
        ...
        if !nativeCaps.contains(CarapaceCapability.computer.rawValue) {
            ...
            onInvoke: { [weak self] req in
                guard let self else {
                    return BridgeInvokeResponse(
                        id: req.id,
                        ok: false,
                        error: CarapaceNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                }
...
extension MacNodeModeCoordinator {
        ...
        claudeSessionCatalogEnabled: Bool) -> [String]
    {
        ...
        return Self.resolvedCaps(
            ...
            computerControlEnabled: computerControlEnabled,
            computerControlProvider: computerControlProvider,
            locationMode: CarapaceLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    private func handleNodeHostWorkerFailure(configurationGeneration: UInt64) {
        ...
        case let .giveUp(unexpectedExitCount):
            ...
            self.notificationCenter.post(
                name: .carapaceNodeHostWorkerRetryExhausted,
                object: self,
...
extension MacNodeModeCoordinator {
    ...
    nonisolated static func resolvedCaps(
        ...
        computerControlEnabled: Bool,
        computerControlProvider: ComputerControlProvider = .peekaboo,
        locationMode: CarapaceLocationMode,
        connectionMode: AppState.ConnectionMode,
        codexThreadCatalogEnabled: Bool = false,
        claudeSessionCatalogEnabled: Bool = false) -> [String]
    {
        var caps: [String] = [
            CarapaceCapability.canvas.rawValue,
            CarapaceCapability.screen.rawValue,
        ]
        ...
        if computerControlEnabled, computerControlProvider == .peekaboo {
            caps.append(CarapaceCapability.computer.rawValue)
        }
        ...
        computerControlProvider: ComputerControlProvider = .peekaboo) -> [String]
    {
        ...
        commands.append(CarapaceSystemCommand.notify.rawValue)

        let capsSet = Set(caps)
        if capsSet.contains(CarapaceCapability.camera.rawValue) {
            ...
            commands.append(CarapaceCameraCommand.ptzStatus.rawValue)
            commands.append(CarapaceCameraCommand.ptzControl.rawValue)
        }
        if capsSet.contains(CarapaceCapability.location.rawValue) {
        ...
        if capsSet.contains(CarapaceCapability.computer.rawValue) {
            commands.append(CarapaceComputerCommand.act.rawValue)
        }
        ...
        for provider: ComputerControlProvider) -> MacNodeHostManifest?
    {
        ...
        return MacNodeHostManifest(
            ...
            caps: manifest.caps.filter { $0 != CarapaceCapability.computer.rawValue },
    ...
    nonisolated static func computerUseDescriptor(
        ...
        commands: [String],
        workerManifest: MacNodeHostManifest?) -> CarapaceProtocol.AnyCodable?
    {
        guard commands.contains(MacNodeScreenCommand.snapshot.rawValue),
              commands.contains(CarapaceComputerCommand.act.rawValue)
        else { return nil }
