...
enum MacNodeCodexThreadCatalog {
    ...
    private static let commandEnvironmentKey = "CARAPACE_CODEX_APP_SERVER_BIN"
    private static let argumentsEnvironmentKey = "CARAPACE_CODEX_APP_SERVER_ARGS"
    ...
    static func shouldAdvertise(root: [String: Any]? = nil) -> Bool {
        let root = root ?? CarapaceConfigFile.loadDict()
        guard CarapaceConfigFile.configuredBundledPluginAllowed(
...
extension MacNodeCodexThreadCatalog {
    ...
    private static func configuredPlugin(root: [String: Any]) throws -> ConfiguredPlugin? {
        guard let entry = CarapaceConfigFile.pluginEntry(
            MacNodeCodexThreadCatalogContract.pluginId,
            root: root)
        else { return nil }
