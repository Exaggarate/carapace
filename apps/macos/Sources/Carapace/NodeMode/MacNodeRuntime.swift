...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case CarapaceCanvasCommand.present.rawValue,
                 CarapaceCanvasCommand.hide.rawValue,
            ...
            case CarapaceCameraCommand.snap.rawValue,
                 ...
                 CarapaceCameraCommand.ptzStatus.rawValue,
                 CarapaceCameraCommand.ptzControl.rawValue:
                return try await handleCameraInvoke(req)
            case CarapaceLocationCommand.get.rawValue:
            ...
            case CarapaceComputerCommand.act.rawValue:
                return try await handleComputerActInvoke(req)
            case CarapaceSystemCommand.notify.rawValue:
            ...
            let code: CarapaceNodeErrorCode = switch error {
    ...
    private static let canvasCommands: Set<String> = [
        CarapaceCanvasCommand.present.rawValue,
        CarapaceCanvasCommand.hide.rawValue,
        CarapaceCanvasCommand.navigate.rawValue,
    ]
...
extension MacNodeRuntime {
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarapaceCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(CarapaceCanvasPresentParams.self, from: req.paramsJSON)) ??
                CarapaceCanvasPresentParams()
        ...
        case CarapaceCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CarapaceCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(CarapaceCanvasNavigateParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CarapaceCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CarapaceCameraCommand.ptzStatus.rawValue:
            let params: CarapaceCameraPTZStatusParams
            do {
                params = try Self.decodeParams(CarapaceCameraPTZStatusParams.self, from: req.paramsJSON)
            } catch {
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CarapaceCameraCommand.ptzControl.rawValue:
            let params: CarapaceCameraPTZControlParams
            do {
                params = try Self.decodeParams(CarapaceCameraPTZControlParams.self, from: req.paramsJSON)
            } catch {
    ...
    private func handleComputerActInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params: CarapaceComputerActParams
        do {
            params = try Self.decodeParams(CarapaceComputerActParams.self, from: req.paramsJSON)
        } catch {
            return Self.errorResponse(
                req,
                code: .invalidRequest,
        ...
        do {
            ...
            case .accessibilityNotTrusted:
                return Self.errorResponse(
                    req,
                    code: .unavailable,
                    message: "ACCESSIBILITY_REQUIRED: grant Accessibility permission to Carapace")
            ...
            case .postEventAccessDenied:
                return Self.errorResponse(
                    req,
                    code: .unavailable,
                    message: "POST_EVENT_REQUIRED: macOS denied Event Posting access; re-grant Carapace "
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> CarapaceLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: CarapaceNodeError(code: code, message: message))
    }
}
