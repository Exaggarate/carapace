...
extension OnboardingAISetupModel {
    ...
    enum ProviderWizardKind: Equatable {
        ...
        var startMethod: String {
            switch self {
            case .auth: "carapace.setup.auth.start"
            case .prepare: "carapace.setup.prepare.start"
            case .activation: "carapace.setup.activate.start"
            }
        }
    }
    ...
    var busyReason: String? {
        ...
        {
            "Carapace is testing your AI connection."
        } else if self.activeAuthOption != nil {
            self.isPreparingModel
                ? "Carapace is preparing a local model."
                : "Carapace is completing provider sign-in."
        } else {
            nil
        }
    }
    ...
    /// Carapace rather than the existing-Gateway onboarding bypass.
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older Carapace version that doesn’t support " +
                "app-guided setup. Update Carapace on the gateway, then try again."
        }
        return raw.isEmpty
    ...
    {
        await gateway.supportsServerMethod("carapace.setup.activate.start", ifCurrentServerLease: serverLease) == true
            ...
            return Self.setupAdmissionIsBusy(response) || code == "UNKNOWN_METHOD" ||
                (code == "INVALID_REQUEST" &&
                    (message.contains("unknown method") ||
                        message.contains("invalid carapace.setup.activate params")))
        }
        return error is GatewayConnectAuthError ||
            error is GatewayTLSValidationError ||
            error is CarapaceChatTransportSendError
    }
    ...
    static func providerAuthCancellationUnconfirmed() -> Failure {
        Failure(
            summary: "Carapace couldn’t confirm cancellation. Setup may still be running. Try Cancel again.",
            detail: nil)
    }
...
enum OnboardingAISetupError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .providerCatalogUnavailable:
            "The Gateway is running an older Carapace version that doesn’t provide the " +
                "supported provider list. Update Carapace on the gateway, then try again."
        }
    }
}
