...
extension Notification.Name {
    static let carapaceCuaDriverAvailabilityChanged = Notification.Name(
        "carapace.cua-driver.availability-changed")
}
...
/// Owns the embedded CUA daemon as a direct Carapace.app child so macOS TCC
...
final class CuaDriverHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "cua-driver-host")
    ...
    private func setReadyEndpoint(_ endpoint: CuaDriverWorkerEndpoint?) {
        ...
        self.notificationCenter.post(name: .carapaceCuaDriverAvailabilityChanged, object: nil)
    }
        ...
        inheritedEnvironment: [String: String] = ProcessInfo.processInfo.environment) -> CuaDriverProcessLaunch
    {
        ...
        // Carapace command arming, pairing, and tool policy own authorization upstream, matching the
        ...
        return CuaDriverProcessLaunch(
            ...
            arguments: [
                ...
                // Carapace records the spawned pid itself so reaping can attribute
        ...
        onTermination: @escaping @Sendable (Int32) -> Void) throws -> any CuaDriverProcessControlling
    {
        ...
        let logger = Logger(subsystem: "ai.carapace", category: "cua-driver-host")
    ...
    static func createSocketDirectory(in applicationSupportURL: URL) throws -> CuaDriverSocketDirectory {
        let carapaceRoot = applicationSupportURL.appendingPathComponent("Carapace", isDirectory: true)
        let root = carapaceRoot.appendingPathComponent("cua", isDirectory: true)
        for directory in [applicationSupportURL, carapaceRoot, root] {
            var status = stat()
