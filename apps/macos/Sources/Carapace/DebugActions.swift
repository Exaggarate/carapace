...
enum DebugActions {
    private static let verboseDefaultsKey = "carapace.debug.verboseMain"
    private static let onboardingSeenKey = "carapace.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = CarapacePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func restartApp() {
        ...
        ] + (AppProfile.current.name.map { ["--env", "CARAPACE_PROFILE=\($0)"] } ?? []) + [url.path]
        try? task.run()
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CarapacePaths.configURL
        guard
