...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.carapace", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "CARAPACE_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "CARAPACE_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "CARAPACE_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "CARAPACE_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "CARAPACE_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "CARAPACE_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "CARAPACE_REPLACEMENT_SUPERVISOR_PLIST"
    nonisolated static let relocationRelaunchArgument = "--carapace-relocation-relaunch"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        {
            self.showFailure(
                "Carapace is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
        switch recommendation {
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["CARAPACE_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "Carapace couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "Carapace is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older Carapace in Applications?"
            : "Install Carapace in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. Carapace will replace it and reopen from Applications."
            : "Carapace will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func startSupervisorRestorationWatcher(_ supervisor: KeepAliveSupervisor) -> Bool {
        ...
        watcher.arguments = [
            ...
            "carapace-supervisor-restoration",
        ...
        reason: String) async
    {
        ...
        case .stopMonitoring:
            ...
            self.logger.critical(
                ...
                Quit and reopen Carapace to load the installed replacement.
                """)
        case .terminateForSupervisor:
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move Carapace to Applications"
