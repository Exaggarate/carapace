...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    private func updateStatusAppearance() {
        ...
        button.toolTip = self.state.voiceWakeMeterActive
            ? String(localized: "Carapace - Voice Wake live meter active")
            : String(localized: "Carapace")
    }
    ...
    private func scheduleDebugMenuOpen() {
        ...
        if flag("CARAPACE_DEBUG_MENU_FIXTURES", "--debug-menu-fixtures") {
            CronJobsStore.shared.seedDebugFixtureJobs()
        }
        if flag("CARAPACE_DEBUG_OPEN_MENU", "--debug-open-menu") {
        ...
        if flag("CARAPACE_DEBUG_PROBE_RIGHTCLICK", "--debug-probe-rightclick") {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
                guard let self, let button = self.statusItem?.button,
