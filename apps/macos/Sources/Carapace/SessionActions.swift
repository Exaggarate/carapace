...
enum SessionActions {
    ...
    {
        let request = CarapaceChatGatewayRequests.patchSessionPreferences(
            sessionKey: key,
            agentID: nil,
    ...
    static func resetSession(key: String) async throws {
        let request = CarapaceChatGatewayRequests.resetSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }

    static func deleteSession(key: String) async throws {
        let request = CarapaceChatGatewayRequests.deleteSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }
    ...
    static func openSessionLogInCode(sessionId: String, storePath: String?) {
        let candidates: [URL] = {
            ...
            urls.append(CarapacePaths.stateDirURL.appendingPathComponent("sessions/\(sessionId).jsonl"))
            return urls
        }()
