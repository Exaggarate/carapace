...
enum GatewayLaunchAgentManager {
    ...
    private static let logger = Logger(subsystem: "ai.carapace", category: "gateway.launchd")
    ...
    private static var disableLaunchAgentMarkerURL: URL {
        ...
        #endif
        return self.disableLaunchAgentMarkerURL(in: CarapacePaths.stateDirURL)
    }
    ...
    private static func isCanonicalGatewayClaim(_ snapshot: LaunchAgentPlistSnapshot) -> Bool {
        snapshot.environment["CARAPACE_SERVICE_MARKER"] == "carapace" &&
            snapshot.environment["CARAPACE_SERVICE_KIND"] == "gateway" &&
            snapshot.programArguments.contains("gateway")
    }

    private static var generatedEnvironmentDirectoryURL: URL {
        CarapacePaths.stateDirURL.appendingPathComponent("service-env", isDirectory: true)
    }
...
extension GatewayLaunchAgentManager {
    ...
    private static func gatewayPort(from service: [String: Any]) -> Int? {
        ...
        return self.validGatewayPort(environment?["CARAPACE_GATEWAY_PORT"] as? String)
    }
