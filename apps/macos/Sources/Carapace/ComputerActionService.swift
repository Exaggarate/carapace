...
final class ComputerActionExecutionQueue {
    typealias Operation = @MainActor (CarapaceComputerActParams, UInt64) async throws
    ...
    private struct QueuedAction {
        ...
        let continuation: CheckedContinuation<CarapaceComputerActResult, Error>
    ...
    private var currentActionTask: Task<CarapaceComputerActResult, Error>?
    ...
    func perform(
        ...
        operation: @escaping Operation) async throws -> CarapaceComputerActResult
    {
        let actionID = UUID()
        let cancellationState = ComputerActionCancellationState()
    ...
    private func drain() async {
        while !self.pendingActions.isEmpty {
            ...
            let outcome: Result<CarapaceComputerActResult, Error>
            do {
...
final class ComputerActionService {
    enum ComputerActionError: LocalizedError {
        ...
        case unsupportedAction(CarapaceComputerAction)
        case refused(String)
    ...
    func typeTextForTesting(
        _ text: String,
        lifecycleGeneration: UInt64 = 0) async throws -> CarapaceComputerActResult
    {
        let params = CarapaceComputerActParams(action: .type, text: text)
