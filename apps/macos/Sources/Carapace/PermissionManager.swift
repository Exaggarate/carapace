...
extension Notification.Name {
    static let carapacePermissionsChanged = Notification.Name("carapace.permissions.changed")
}
...
enum PermissionManager {
    ...
    static func ensure(_ caps: [Capability], interactive: Bool) async -> [Capability: Bool] {
        ...
        if interactive {
            await MainActor.run {
                NotificationCenter.default.post(name: .carapacePermissionsChanged, object: nil)
            }
        }
        return results
    }
