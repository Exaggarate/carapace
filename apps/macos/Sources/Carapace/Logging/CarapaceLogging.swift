...
enum CarapaceLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = CarapaceOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CarapaceFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = CarapaceLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CarapaceOSLogHandler: AppLogLevelBackedHandler {
...
struct CarapaceFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]

    func log(event: LogEvent) {
        guard AppLogSettings.fileLoggingEnabled() else { return }
        let (subsystem, category) = CarapaceLogging.parseLabel(self.label)
