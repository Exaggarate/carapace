...
private let gatewayConnectionLogger = Logger(subsystem: "ai.carapace", category: "gateway.connection")
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> CarapaceChatSessionRoutingIdentity
    {
        let data = try await request(
            CarapaceChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try CarapaceChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
...
extension GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: CarapaceProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(CarapaceGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> CarapaceSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return CarapaceSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> CarapaceChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(CarapaceChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> CarapaceChatSendResponse
    {
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(CarapaceChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(CarapaceChatSendResponse.self, from: data)
    }
