...
final class PostUpdateModel {
    ...
    var title = String(localized: "Finishing your Carapace update")
...
final class PostUpdateController: NSObject, NSWindowDelegate {
    ...
    static let updateGuideURL = URL(string: "https://github.com/Exaggarate/carapace")!
    ...
    private func show() {
        ...
        window.title = String(localized: "Carapace updated")
    ...
    private func run() {
        ...
        self.model.title = String(localized: "Finishing your Carapace update")
    ...
    private func finishUpdate(receipt: PostAppUpdateReceipt) async {
        ...
        let ownsManagedRuntime = Self.ownsManagedRuntime(
            ...
            gatewayUpdateChannel: CarapaceConfigFile.gatewayUpdateChannel(),
        ...
        case .install:
            ...
            guard installed else {
                self.fail(
                    ...
                    details: String(localized: "The managed Carapace runtime could not be reinstalled."))
                return
            }
        }
        ...
        connectionMode: AppState.ConnectionMode)
    {
        ...
        self.model.message = connectionMode == .local
            ? String(format: String(localized: "Carapace %@ and its Gateway are ready."), receipt.toVersion)
            : String(format: String(localized: "Carapace %@ and its Mac node runtime are ready."), receipt.toVersion)
        self.model.details = switch (notification, notificationRetryScheduled) {
        case (.retryLater, true):
            String(localized: "Your agent could not be notified yet. Carapace will retry after the next app launch.")
        case (.retryLater, false):
            if connectionMode == .local {
                String(
                    localized: """
                    Carapace could not notify your agent automatically. \
        ...
        case (.deliveryUnconfirmed, _):
            String(
                localized: """
                Carapace could not confirm the agent notification. \
                It will not retry, to avoid a duplicate welcome.
                """)
        case (.skippedUnsupportedGateway, _):
            String(
                localized: "The remote Gateway is older than this Mac app, so Carapace skipped the agent notification.")
        case (.skippedWhilePaused, _):
            String(localized: "The Gateway remains paused, so Carapace did not wake your agent.")
        default:
            nil
        }
    }
    ...
    private func verifyRuntime(connectionMode: AppState.ConnectionMode) async -> Bool {
        ...
        guard ControlChannel.shared.state == .connected else {
            self.fail(
                ...
                details: String(
                    localized: "Carapace installed the update but could not verify the Gateway connection."))
            return false
        }
        return true
    }
        ...
        receipt: PostAppUpdateReceipt) async -> PostUpdateNotificationOutcome
    {
        ...
        do {
            guard let selected = try await Self.preferredNotificationSession(loadPage: { offset in
                var params: [String: CarapaceKit.AnyCodable] = [
                    "limit": CarapaceKit.AnyCodable(100),
                    "includeGlobal": CarapaceKit.AnyCodable(false),
                    "includeUnknown": CarapaceKit.AnyCodable(false),
                    "configuredAgentsOnly": CarapaceKit.AnyCodable(true),
                    "requireLastInteraction": CarapaceKit.AnyCodable(true),
                    "sortBy": CarapaceKit.AnyCodable("lastInteractionAt"),
                ]
                if offset > 0 {
                    params["offset"] = CarapaceKit.AnyCodable(offset)
                }
        ...
        let text =
            "Carapace updated to \(version). Briefly welcome the user back and say you are updated, " +
        ...
        do {
            _ = try await GatewayConnection.shared.request(
                method: "system-event",
                params: [
                    "text": CarapaceKit.AnyCodable(text),
                    "sessionKey": CarapaceKit.AnyCodable(session.key),
                    "wake": CarapaceKit.AnyCodable(true),
                ],
                ifCurrentServerLease: serverLease)
            return .delivered
        } catch {
    ...
    static func notificationSendFailureOutcome(_ error: Error) -> PostUpdateNotificationOutcome {
        if error is CarapaceChatTransportSendError || error is GatewayResponseError {
            return .retryLater
        }
        ...
        receipt: PostAppUpdateReceipt)
    {
        ...
        case .local:
            self.fail(
                ...
                details: String(
                    localized: """
                    Carapace could not read the Gateway service ownership record. \
        ...
        case .remote:
            self.fail(
                ...
                details: String(
                    localized: """
                    Carapace could not read the node service ownership record. \
...
private struct PostUpdateView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            GlowingCarapaceIcon(size: 150, mood: self.model.mood)
                .frame(height: 205)
