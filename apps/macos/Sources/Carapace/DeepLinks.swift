...
private let deepLinkLogger = Logger(subsystem: "ai.carapace", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        case let .agent(link):
            guard !AppStateStore.shared.isPaused else {
                self.presentAlert(title: "Carapace is paused", message: "Unpause Carapace to run agent actions.")
                return
            }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Carapace agent?", message: body) else { return }
        }
