...
final class ExecApprovalQueueStore {
    ...
    @ObservationIgnored private let logger = Logger(subsystem: "ai.carapace", category: "exec-approvals.queue")
    ...
    {
        let method = kind == .exec ? "exec.approval.list" : "carapace.approval.list"
        if kind == .systemAgent,
    ...
    private func handle(delivery: GatewayConnection.PushDelivery) {
        ...
        switch event.event {
        case "exec.approval.requested", "carapace.approval.requested":
            do {
                let request = try GatewayPayloadDecoding.decode(payload, as: ExecApprovalQueueItem.self)
                let kind: ExecApprovalQueueItem.ApprovalKind = event.event == "carapace.approval.requested"
                    ? .systemAgent
                    : .exec
        ...
        case "exec.approval.resolved", "carapace.approval.resolved":
            guard let resolved = try? GatewayPayloadDecoding.decode(payload, as: ResolvedApproval.self) else {
                return
            }
            self.refreshGeneration &+= 1
            let kind: ExecApprovalQueueItem.ApprovalKind = event.event == "carapace.approval.resolved"
                ? .systemAgent
                : .exec
