...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        onboardingPage {
            VStack(spacing: 18) {
                ...
                Label {
                    Text(
                        "Carapace can take actions using the permissions and services you enable. " +
    ...
    func connectionPage() -> some View {
        onboardingPage {
            Text("Where should your assistant live?")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Most people pick this Mac — Carapace installs everything and keeps it " +
            ...
            if self.selectedConnectionMode == .unconfigured {
                Text("OK — Carapace won’t start anything yet. Pick Local or Remote later in the Connection window.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .onChange(of: self.state.connectionMode) { _, newValue in
            ...
            // retires route-owned AI/Carapace state. This nested observer owns probe copy only.
        ...
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Verify Carapace can reach this gateway.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer(minLength: 0)
            ...
            GridRow {
                ...
                Text(
                    "The current gateway.remote.token value is not plain text. "
                        + "Carapace for macOS cannot use it directly; "
        ...
        if self.state.remoteTransport == .ssh {
            ...
            GridRow {
                Text("Project root")
                    .font(.callout.weight(.semibold))
                    .frame(width: labelWidth, alignment: .leading)
                TextField("/home/you/Projects/carapace", text: self.$state.remoteProjectRoot)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: fieldWidth)
            }
            GridRow {
                Text("CLI path")
                    ...
                    .frame(width: labelWidth, alignment: .leading)
                TextField(
                    "/Applications/Carapace.app/.../carapace",
        ...
        action: @escaping () -> Void) -> some View
    {
        Button {
            ...
            .carapaceSelectableRowChrome(selected: selected)
        }
        .buttonStyle(.plain)
    }

    func cliPage() -> some View {
        let detail = "Carapace is setting up its Gateway background service on this Mac. " +
            "Published Stable and Beta installs are usually quick. " +
            "Dev (Git main) downloads and builds Carapace from source, so allow several minutes " +
        ...
        onboardingPage {
            ...
            self.onboardingCard {
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the Carapace menu bar icon for the compact chat panel and status.",
