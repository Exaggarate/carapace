...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CARAPACE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/carapace")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("carapace-stdout.log")
    }

    private static var gatewayLog: URL {
        let suffix = AppProfile.current.name.map { "-\($0)" } ?? ""
        return logDir.appendingPathComponent("carapace-gateway\(suffix).log")
    }
    ...
    /// Returns the newest log file under /tmp/carapace/ (rolling or stdout), or nil if none exist.
