...
extension Notification.Name {
    static let carapaceCLIInstalled = Notification.Name("carapace.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "Carapace Gateway \(version) is ready."
            case .missing:
                "Carapace Gateway is not installed yet."
            case .unusable:
                "The Carapace Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("carapace").path
        ...
        profile: AppProfile = .current) -> String
    {
        URL(fileURLWithPath: self.installPrefix(homeDirectory: homeDirectory, profile: profile))
            .appendingPathComponent("bin/carapace")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing Carapace CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall Carapace.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed carapace \($0)." } ?? "Installed carapace."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .carapaceCLIInstalled, object: nil)
            return true
        }
    ...
    private static func installBundledMacCLI(prefix: String) throws {
        ...
        let source = Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/carapace-mac")
        ...
        let destination = URL(fileURLWithPath: prefix).appendingPathComponent("bin/carapace-mac")
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the Carapace Gateway update…")
            : String(format: String(localized: "Updating the Carapace Gateway to %@…"), targetVersion))
        ...
        await statusHandler(String(
            format: String(localized: "Carapace Gateway %@ is installed."), installedVersion))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/carapace")
            .path
    }
    ...
    nonisolated static func installStatus(forEventLine line: String) -> String? {
        ...
        case ("git-clone", "start"): "Downloading Carapace source…"
        case ("git-update", "start"): "Updating Carapace source…"
        case ("dependencies", "start"): "Installing dependencies…"
        case ("control-ui", "start"): "Building interface…"
        case ("cli-build", "start"): "Building Carapace CLI…"
        case ("carapace", "retry"): "Retrying Carapace CLI install…"
        case ("disk-space", "warn"): "Couldn’t verify free disk space; continuing…"
        case ("git-update", "warn"): "Using the existing modified Carapace source…"
