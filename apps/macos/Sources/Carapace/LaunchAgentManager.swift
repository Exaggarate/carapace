...
enum LaunchAgentManager {
    private static let logger = Logger(subsystem: "ai.carapace", category: "app.login-agent")
    private static var plistURL: URL {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Library/LaunchAgents/ai.carapace.mac.plist")
    }
        ...
        preferredPaths: [String] = CommandResolver.preferredPaths()) -> String
    {
        ...
        <dict>
          <key>Label</key>
          <string>ai.carapace.mac</string>
          <key>ProgramArguments</key>
          <array>
            <string>\(self.escapePlistText(bundlePath))/Contents/MacOS/Carapace</string>
          </array>
    ...
    private static func profileEnvironmentPlistEntries() -> String {
        ["CARAPACE_CONFIG_PATH", "CARAPACE_STATE_DIR"].compactMap { key in
            guard let value = CarapaceEnv.path(key) else { return nil }
            return """
