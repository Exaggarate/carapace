...
enum ChromeExtensionSetup {
    ...
    enum SetupError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .missingCLI: "Install the Carapace CLI on this Mac, then try setup again."
            case .unavailable:
                "Chrome setup could not finish. Run carapace browser extension install on this Mac for details."
    ...
    static func install(isCurrent: () -> Bool) async throws -> Result {
        ...
        let executable: String? = if case let .ready(location, _) = await CLIInstaller.status() {
            ...
            CommandResolver.findExecutable(named: "carapace", searchPaths: CommandResolver.preferredPaths())
        }
