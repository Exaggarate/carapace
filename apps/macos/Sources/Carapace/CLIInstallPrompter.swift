...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "cli.prompt")
        ...
        presentingSheetOn window: NSWindow?) async -> CLIInstaller.InstallTarget?
    {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install Carapace CLI?"
        ...
        presentingSheetOn window: NSWindow?) async -> CLIInstaller.Channel?
    {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose Carapace CLI channel"
        alert.informativeText =
            "This is an unreleased Carapace build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            await status.set("Starting Carapace Gateway…")
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: Starting Carapace Gateway…")
            }
            ...
            case .ready:
                "Carapace Gateway is ready."
            case .deferred:
                "Carapace is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "Carapace was installed, but the Gateway did not start. Open Settings to retry."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
