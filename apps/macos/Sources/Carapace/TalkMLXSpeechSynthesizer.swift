...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["CARAPACE_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("carapace-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["carapace-mlx-tts"],
            displayName: "carapace-mlx-tts")
    }
