...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "portguard")
        ...
        currentAppPID: Int32? = nil) -> TunnelRecordAction
    {
        ...
        // Any other live parent may be a concurrent Carapace instance (prod + dev).
        return .keep
    }
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "carapace", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("carapace-gateway")
                || cmd.contains("carapace-gateway")
            {
                return true
            }
            if self.isNodeCarapaceGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("carapace"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeCarapaceGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isCarapaceDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isCarapaceDistEntrypointToken(_ token: String) -> Bool {
    ...
    private nonisolated static func openRecordStore() throws -> PortGuardianRecordStore {
        guard !self.hasLegacyCarapaceAppProcess() else {
            throw PortGuardianStoreError(
                "Quit older Carapace app copies before opening the SQLite PortGuardian ledger")
        }
    ...
    private nonisolated static func requirePostSpawnCompatibility() throws {
        guard !self.hasLegacyCarapaceAppProcess(),
              !FileManager.default.fileExists(atPath: PortGuardianRecordStore.liveLegacyRecordURL.path)
        else {
            throw PortGuardianStoreError(
                "Older Carapace storage appeared after tunnel preflight; SSH launch cancelled")
        }
    }
    ...
    private nonisolated static func hasLegacyCarapaceAppProcess() -> Bool {
    ...
    private nonisolated static func runningPortGuardianStorageVersion(pid: pid_t) -> Int? {
        ...
        guard SecCodeCopyStaticCode(code, SecCSFlags(), &staticCode) == errSecSuccess,
              ...
              let version = securedInfo["CarapacePortGuardianStorageVersion"] as? NSNumber
        else { return nil }
        ...
        storageVersion: Int?) -> Bool
    {
        guard let bundleIdentifier,
              bundleIdentifier == "ai.carapace.mac" || bundleIdentifier.hasPrefix("ai.carapace.mac.")
        else { return false }
