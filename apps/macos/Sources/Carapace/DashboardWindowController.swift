...
/// pinned at `--carapace-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "carapaceLink"
    private static let windowDragMessageHandlerName = "carapaceWindowDrag"
    private static let updateMessageHandlerName = "carapaceUpdate"
    private static let commandsMessageHandlerName = "carapaceCommands"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(Self.scopedDashboardScript(
            "window.dispatchEvent(new CustomEvent('carapace:native-update-availability-changed'))",
            url: self.currentURL))
    }
    ...
    fileprivate func receiveLinkMessage(_ message: WKScriptMessage) {
        ...
        case .inline, .external:
            // Older Control UI bundles still post inline; Mac tabs now use carapaceBrowser.
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(Self.scopedDashboardScript(
                "window.dispatchEvent(new CustomEvent('carapace:native-update-declined'))",
                url: self.currentURL))
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(Self.scopedDashboardScript(
            """
            window.__CARAPACE_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('carapace:native-history-state', \
            {detail:window.__CARAPACE_NATIVE_HISTORY__}));
            """, url: self.currentURL))
    }
        ...
        reusing existingWindow: NSWindow?) -> NSWindow
    {
        ...
        // (--carapace-native-titlebar-height); without it they hug the top edge.
...
extension DashboardWindowController {
    static func installNativeChromeScript(into userContentController: WKUserContentController, url: URL) {
        ...
        window.__CARAPACE_NATIVE_WEB_CHROME__ = true;
        window.addEventListener('carapace:native-commands-state', () => {
          window.webkit.messageHandlers.carapaceCommands.postMessage({type: 'commands-state'});
        });
        """
        ...
        // `html.carapace-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.carapace-native-macos .sidebar-shell,
          html.carapace-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--carapace-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("carapace-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "carapace-native-macos-chrome";
...
extension DashboardWindowController {
    ...
    private static func makeJavaScriptConfirmAlert(message: String, host: String?) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = "Carapace Dashboard"
    ...
    private func refreshNativeCommandReadiness() {
        ...
        Task { @MainActor [weak self] in
            ...
            let ready = try? await self.webView.evaluateJavaScript(Self.scopedDashboardScript(
                "return window.__CARAPACE_NATIVE_COMMANDS_READY__ === true;", url: sourceURL))
    ...
    private func evaluateNativeNavigation(_ navigation: DashboardNativeNavigation) {
        ...
        let script =
            """
            return !window.dispatchEvent(new CustomEvent('carapace:native-navigate', {
              cancelable: true,
