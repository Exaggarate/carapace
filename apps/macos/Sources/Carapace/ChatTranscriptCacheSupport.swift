...
/// shared with iOS through CarapaceKit.
enum MacChatTranscriptCache {
    ...
    @MainActor private static var databasesByDirectory: [String: CarapaceClientDatabases] = [:]
    @MainActor private static var storesByKey: [String: CarapaceChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `CARAPACE_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        self.gatewayID(root: CarapaceConfigFile.loadDict())
    }
    ...
    static func gatewayID(root: [String: Any]) -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: CarapaceConfigFile.stateDirURL(),
    ...
    static func makeContext(gatewayID: String) -> Context? {
        ...
        let root = base.appendingPathComponent("Carapace", isDirectory: true)
    ...
    @MainActor
    static func store(databaseDirectoryURL: URL, gatewayID: String) -> CarapaceChatSQLiteTranscriptCache? {
        guard let databases = self.databases(
            directoryURL: databaseDirectoryURL,
    ...
    private static func store(
        ...
        gatewayID: String) -> CarapaceChatSQLiteTranscriptCache
    {
    ...
    private static func databases(
        directoryURL: URL,
        legacyDirectoryURL: URL) -> CarapaceClientDatabases?
    {
        ...
        guard let databases = try? CarapaceClientDatabases(
