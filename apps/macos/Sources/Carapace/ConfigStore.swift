...
enum ConfigStore {
    ...
    static func load(gateway: GatewayConnection = .shared) async -> Document {
        ...
        do {
            ...
            origin.lease = nil
            return Document(root: CarapaceConfigFile.loadDict(), origin: origin, hash: nil, readError: nil)
        }
    }
    ...
    static func save(_ document: Document, allowGatewayAuthMutation: Bool = false) async throws {
        ...
        #endif
        notificationCenter.post(name: .carapaceConfigDidChange, object: nil)
    }
    ...
    private static func saveLocal(_ document: Document, allowGatewayAuthMutation: Bool) throws {
        ...
        guard CarapaceConfigFile.saveDict(
...
extension Notification.Name {
    static let carapaceConfigDidChange = Notification.Name("carapace.config.did-change")
}
