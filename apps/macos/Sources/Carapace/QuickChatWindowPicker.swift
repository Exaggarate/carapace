...
final class QuickChatWindowPicker {
    ...
    {
        let logging = LoggingService(subsystem: "ai.carapace.quickchat-picker")
    ...
    private func requestScreenRecordingPermission(mode: QuickChatCapturePickerMode) async {
        ...
        alert.messageText = mode == .window
            ? String(localized: "Allow Carapace to capture windows")
            : String(localized: "Allow Carapace to capture a screen area")
