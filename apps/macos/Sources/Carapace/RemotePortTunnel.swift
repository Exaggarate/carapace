...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.carapace", category: "remote.tunnel")
    ...
    static func configuration(remotePort: Int) throws -> Configuration {
        ...
        let preferredLocalPort = CarapaceConfigFile.gatewayPort(root: root)
        ...
        root: [String: Any]) -> Int?
    {
        ...
        guard let sshKey = CarapaceConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = CarapaceConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.carapace.remote.tunnel.port", qos: .utility)
            do {
