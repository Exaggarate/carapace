...
@main
enum CarapaceProcessMain {
    static func main() {
        if let status = CarapaceProcessEntrypoint.run(arguments: CommandLine.arguments, launchApplication: {
            CarapaceApp.main()
        }) {
            Darwin.exit(status)
        }
...
enum CarapaceProcessEntrypoint {
...
struct CarapaceApp: App {
    ...
    private static let logger = Logger(subsystem: "ai.carapace", category: "app")
    ...
    init() {
        ...
        if let error = AppProfile.current.validationError {
            if launchPlan.isElevationHost {
                fputs("Carapace elevation host profile is invalid: \(error.localizedDescription)\n", stderr)
                Darwin.exit(2)
            }
            let alert = NSAlert()
            alert.alertStyle = .critical
            alert.messageText = "Carapace profile is invalid"
            alert.informativeText = error.localizedDescription
            alert.runModal()
            Darwin.exit(2)
        }
        if AppProfile.current.isActive,
           !DeviceIdentityStore.configureStateDirectory(CarapacePaths.stateDirURL)
        {
        ...
        CarapaceLogging.bootstrapIfNeeded()
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let webChatAutoLogger = Logger(subsystem: "ai.carapace", category: "Chat")
    ...
    override init() {
        ...
        if hasReplacementMetadata, !isReplacementHandoff {
            fputs("Carapace replacement handoff authentication failed.\n", stderr)
            Darwin.exit(2)
        }
        ...
        if let exitCode = Self.processExitCode(for: ownership) {
            fputs("Carapace profile is already running.\n", stderr)
        ...
        if let instanceOwnershipFailure {
            if AppLaunchRuntimePlan.current.isElevationHost {
                fputs(
                    "Carapace elevation host could not claim its instance lock: \(instanceOwnershipFailure)\n",
                    stderr)
                Darwin.exit(2)
            }
            let alert = NSAlert()
            alert.alertStyle = .critical
            alert.messageText = "Carapace could not claim its instance lock"
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if !AppProfile.current.isActive, !launchPlan.isElevationHost {
            ...
            case let .continueLaunch(startUpdater):
                if startUpdater, launchPlan.allowsUpdater {
                    if CarapaceConfigFile.gatewayUpdateChannel() == nil {
        ...
        if launchPlan.allowsAutomaticPresentation,
           ProcessInfo.processInfo.environment["CARAPACE_DEBUG_PAIRING_DEMO"] == "1"
        {
