...
final class SparkleUpdaterController: NSObject, UpdaterProviding {
        ...
        onStart: (() -> Void)? = nil)
    {
        self.gatewayUpdateChannelResolver = gatewayUpdateChannelResolver ?? {
            ...
            return CarapaceConfigFile.normalizedGatewayUpdateChannel(response.effectiveChannel)
        }
...
extension SparkleUpdaterController: SPUUpdaterDelegate {
    func allowedChannels(for _: SPUUpdater) -> Set<String> {
        allowedSparkleChannels(
            forGatewayUpdateChannel: self.gatewayUpdateChannel ?? CarapaceConfigFile.gatewayUpdateChannel())
    }

    func bestValidUpdate(in appcast: SUAppcast, for _: SPUUpdater) -> SUAppcastItem? {
        guard self.gatewayUpdateChannel ?? CarapaceConfigFile.gatewayUpdateChannel() == "extended-stable" else {
            return nil
        }
