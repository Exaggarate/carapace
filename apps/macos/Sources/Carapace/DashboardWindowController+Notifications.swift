...
extension DashboardWindowController {
    static let notificationsMessageHandlerName = "carapaceNotifications"
    ...
    private func publishNotificationsStatus() async {
        ...
        _ = try? await self.webView.evaluateJavaScript(Self.scopedDashboardScript(
            """
            window.__CARAPACE_NATIVE_NOTIFICATIONS__ = \(json);
            window.dispatchEvent(new CustomEvent('carapace:native-notifications-status', \
            {detail:window.__CARAPACE_NATIVE_NOTIFICATIONS__}));
            """, url: self.currentURL))
    }
}
