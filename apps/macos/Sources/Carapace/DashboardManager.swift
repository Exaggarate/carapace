...
private let dashboardManagerLogger = Logger(subsystem: "ai.carapace", category: "DashboardManager")
...
final class DashboardManager {
        ...
        let endpoint = GatewayConnection.EndpointSnapshot(
            ...
            tls: GatewayTLSRoute.resolve(
                ...
                configuredFingerprint: mode == .remote
                    ? GatewayRemoteConfig.resolveTLSFingerprint(root: CarapaceConfigFile.loadDict())
                    : nil),
            routeAuthority: nil,
    ...
    func showConfiguredWindowIfPossible() -> Bool {
        ...
        self.presentDashboard(
            configuration: WindowConfiguration(
                url: url, auth: auth, tlsParams: endpoint.tls?.params, mode: mode, displayName: "Carapace"),
            endpoint: endpoint,
            target: .primary,
...
extension DashboardManager {
        ...
        reusingWindow: NSWindow? = nil) -> DashboardWindowController
    {
        self.makeController(
            configuration: WindowConfiguration(
                url: url, auth: auth, tlsParams: tlsParams, mode: mode, displayName: "Carapace"),
            target: .primary,
...
extension DashboardManager {
        ...
        token: String?) async throws -> WindowConfiguration
    {
        ...
        let name = target == .primary ? "Carapace"
