...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.carapace.mac`) even if the shipped
    ...
    {
        if environment["CARAPACE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "carapace.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "carapace.nixMode") { return true }
