...
enum CommandResolver {
    private static let projectRootDefaultsKey = "carapace.gatewayProjectRootPath"
    private static let helperName = "carapace"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let carapaceEntry = root.appendingPathComponent("carapace.mjs").path
        if FileManager().isReadableFile(atPath: carapaceEntry) { return carapaceEntry }
        let binEntry = root.appendingPathComponent("bin/carapace.js").path
        ...
        homeDirectory: URL = FileManager.default.homeDirectoryForCurrentUser) -> URL
    {
        ...
        let fallback = homeDirectory.appendingPathComponent("Projects/carapace")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedCarapaceExecutable(
        ...
        profile: AppProfile = .current) -> [String]
    {
        ...
        let managedPaths = self.carapaceManagedPaths(home: home, profile: profile)
        // Other profiles' managed trees must not leak in via stale validation or the
        // inherited shell PATH (the CLI installer adds ~/.carapace/bin to shell profiles).
    ...
    static func validatedCarapaceExecutable(
    ...
    /// Exactly the AppProfile.stateDirectoryURL namespace; ~/.carapace2 is not managed.
    private static func isManagedDirectoryName(_ name: String) -> Bool {
        name == ".carapace" || name.hasPrefix(".carapace-")
    }

    private static func carapaceManagedPaths(home: URL, profile: AppProfile) -> [String] {
    ...
    static func carapaceExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCarapaceExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("carapace.mjs").path
        ...
        searchPaths: [String]? = nil) async throws -> MacNodeHostWorkerLaunch
    {
        ...
        #else
        throw MacNodeHostWorker.WorkerError.unavailable(reason: "The node worker requires a packaged Carapace.app")
        #endif
    }
    ...
    static func carapaceNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = AppDefaults.standard,
        ...
        profile: AppProfile = .current) async -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let carapacePath = projectCarapaceExecutable(projectRoot: root) {
            return profile.localCLICommand(prefix: [carapacePath], arguments: [subcommand] + extraArgs)
        }
        if let carapacePath = carapaceExecutable(searchPaths: searchPaths) {
            return profile.localCLICommand(prefix: [carapacePath], arguments: [subcommand] + extraArgs)
        }
        ...
        case .success:
            let missingEntry = """
            carapace CLI not found. Install the CLI, or run pnpm build in an Carapace source checkout.
            """
    ...
    static func carapaceCommand(
        ...
        profile: AppProfile = .current) async -> [String]
    {
        await self.carapaceNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real carapace CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/carapace"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/carapace.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/carapace.mjs"
            node "$PRJ/carapace.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/carapace.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/carapace.js"
            node "$PRJ/bin/carapace.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent carapace"
          pnpm --silent carapace \(quotedArgs);
        else
          echo "carapace CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? CarapaceConfigFile.loadDict()
