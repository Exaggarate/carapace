...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CarapaceProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "control")
        ...
        ifCurrentServerLease lease: GatewayConnection.ServerLease? = nil) async throws -> Data
    {
        try await self.performRequest(ifCurrentServerLease: lease) {
            let rawParams = params?.reduce(into: [String: CarapaceKit.AnyCodable]()) {
                $0[$1.key] = CarapaceKit.AnyCodable($1.value.base)
            }
            if let lease {
    ...
    private func reportFailure(_ error: Error, generation: UInt64) -> String {
        ...
        let message = Self.friendlyGatewayMessage(error, configRoot: CarapaceConfigFile.loadDict())
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock(configRoot: configRoot) {
                    return """
                    macOS is blocking Carapace Local Network access.
                    Allow Carapace in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private func fetchProfileAccentHex(serverLease: GatewayConnection.ServerLease) async -> String? {
        do {
            let data = try await self.gateway.request(
                ...
                params: ["keys": CarapaceKit.AnyCodable(["ui.accent"])],
                timeoutMs: 8000,
    ...
    private static func bridgeToProtocolArgs(
        _ value: CarapaceProtocol.AnyCodable?) -> [String: CarapaceProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CarapaceProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CarapaceKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CarapaceProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlChannelStateDidChange = Notification.Name("carapace.control-channel.state-did-change")
}
