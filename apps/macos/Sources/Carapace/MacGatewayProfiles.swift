...
enum MacGatewayProfileError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .unsupportedRegistryVersion(version):
            "Gateway profiles were written by a newer Carapace version (schema \(version))."
...
actor MacGatewayProfileStore {
    ...
    static let willChangePrincipalNotification = Notification.Name("carapace.gateway-profiles.will-change-principal")
    static let didChangeNotification = Notification.Name("carapace.gateway-profiles.did-change")
    ...
    #if DEBUG
    private static let baseService = "ai.carapace.gateway-profiles.debug"
    #else
    private static let baseService = "ai.carapace.gateway-profiles"
    #endif
