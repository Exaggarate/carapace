...
struct OnboardingAISetupView: View {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(self.model.waitingForPendingActivationDeadline
                        ...
                        .font(.callout.weight(.semibold))
                    Text(self.model.waitingForPendingActivationDeadline
                        ? "Carapace will check again before changing any inference settings."
    ...
    private func candidateRow(_ candidate: OnboardingAISetupModel.Candidate) -> some View {
        ...
        .carapaceSelectableRowChrome(selected: selected && !Self.isFailed(status))
    }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                ForEach(self.model.prepareOptions) { option in
                    Button {
                        ...
                        .carapaceSelectableRowChrome(selected: false)
                    }
                    .buttonStyle(.plain)
        ...
        if !self.model.authOptions.isEmpty || !self.model.manualProviders.isEmpty {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Connect an AI provider")
                    .font(.headline)
                Text(
                    "Choose any supported provider. Carapace asks before installing a provider plugin, " +
    ...
    private var apiKeysRow: some View {
        Button {
            ...
            .carapaceSelectableRowChrome(selected: self.model.showManualEntry)
        }
        .buttonStyle(.plain)
        .disabled(self.model.isBusy)
    }
    ...
    private var manualProviderHelp: String {
        ...
        guard let hint, !hint.isEmpty else {
            return "Paste the key or token here, and Carapace checks it with a real test question."
        }
        return "\(hint). Paste it here, and Carapace checks it with a real test question."
    }
}

/// Friendly error presentation with a consistent docs escape hatch.
/// Every onboarding failure points at a github.com/Exaggarate/carapace page so people are
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 14) {
                    ...
                    Button("Open help…") {
                        if let url = URL(string: "https://github.com/Exaggarate/carapace\(docsSlug)") {
