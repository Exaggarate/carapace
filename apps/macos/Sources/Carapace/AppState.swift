...
final class AppState {
    private static let logger = Logger(subsystem: "ai.carapace", category: "app-state")
    ...
    init(
        ...
        gatewayConfigSaver: @escaping ([String: Any], Bool) -> Bool = {
            CarapaceConfigFile.saveDict($0, allowGatewayModeRemoval: $1)
        })
    {
        ...
        let configRoot = CarapaceConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            CarapaceConfigFile.canonicalHostForComparison(host) ==
                CarapaceConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
...
extension AppState {
    ...
    private func startConfigWatcher() {
        let configUrl = CarapaceConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .carapaceConfigDidChange, object: nil)
    }
    ...
    func setLocationMode(_ mode: CarapaceLocationMode, requestIsCurrent: @MainActor () -> Bool) async {
...
extension AppState {
    ...
    func setPrimaryGateway(_ configuration: PrimaryGatewayControlConfiguration) throws {
        ...
        let currentRoot = CarapaceConfigFile.loadDict()
        ...
        let effectiveLocalPort = GatewayEnvironment.resolvedGatewayPort(
            ...
            configPort: configuration.requestedLocalPort ?? CarapaceConfigFile.gatewayPort(root: currentRoot),
