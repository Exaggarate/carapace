...
struct QuickChatModelControlSnapshot: Sendable {
    let models: [CarapaceChatModelChoice]
    let currentModelSelectionID: String?
    let currentThinkingLevel: String?
    let thinkingOptions: [CarapaceChatThinkingLevelOption]
...
enum QuickChatModelControlLogic {
    static let baseThinkingOptions = ["off", "minimal", "low", "medium", "high"].map {
        CarapaceChatThinkingLevelOption(id: $0, label: $0)
    }

    static func snapshot(
        target: QuickChatRoutingTarget,
        models: [CarapaceChatModelChoice],
        sessions: CarapaceChatSessionsListResponse,
        ...
        currentSessionSelectionID: String? = nil) -> QuickChatModelPatchDecision
    {
        ...
        if selectionID == CarapaceChatViewModel.defaultModelSelectionID {
    ...
    private static func sessionEntry(
        target: QuickChatRoutingTarget,
        sessions: [CarapaceChatSessionEntry]) -> CarapaceChatSessionEntry?
    {
    ...
    private static func agent(target: QuickChatRoutingTarget, agents: AgentsListResult?) -> AgentSummary? {
        ...
        let targetAgentID = self.normalized(target.agentID) ??
            CarapaceChatSessionKey.agentID(from: target.sessionKey) ??
    ...
    private static func thinkingOptions(
        entry: CarapaceChatSessionEntry?,
        agent: AgentSummary?,
        defaults: CarapaceChatSessionsDefaults?) -> [CarapaceChatThinkingLevelOption]
    {
        let agentOptions = agent?.thinkinglevels?.compactMap { option -> CarapaceChatThinkingLevelOption? in
            ...
            return CarapaceChatThinkingLevelOption(id: id, label: label)
        }
        let options = entry?.thinkingLevels ??
            entry?.thinkingOptions?.map { CarapaceChatThinkingLevelOption(id: $0, label: $0) } ??
            agentOptions ??
            defaults?.thinkingLevels ??
            defaults?.thinkingOptions?.map { CarapaceChatThinkingLevelOption(id: $0, label: $0) } ??
