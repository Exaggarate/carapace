...
final class CookieSyncManager: NSObject {
    ...
    var isAvailable: Bool {
        ...
        #if DEBUG
        if CommandResolver.projectCarapaceExecutable() != nil { return true }
        #endif
        return CLIInstaller.installedLocation() != nil
    }

    @ObservationIgnored private let logger = Logger(subsystem: "ai.carapace", category: "cookie-sync")
    @ObservationIgnored private let queue = DispatchQueue(label: "ai.carapace.cookie-sync")
    ...
    private func reconcile(generation: UInt64) async {
        ...
        guard let executable = await self.resolveLocalCarapaceExecutable(),
              !Task.isCancelled,
              generation == self.reconcileGeneration
        else {
            if !Task.isCancelled, generation == self.reconcileGeneration {
                self.state = .error("Carapace CLI not found locally; cookie sync needs the CLI on this Mac")
            }
            return
        }
    ...
    private func resolveLocalCarapaceExecutable() async -> String? {
        #if DEBUG
        if let executable = CommandResolver.projectCarapaceExecutable() {
            return executable
        }
        #endif
    ...
    private func launch(executable: String, intent: SyncIntent) {
        ...
        environment["CARAPACE_GATEWAY_URL"] = intent.endpoint.url.absoluteString
        environment.removeValue(forKey: "CARAPACE_GATEWAY_TOKEN")
        environment.removeValue(forKey: "CARAPACE_GATEWAY_PASSWORD")
        if let token = intent.endpoint.token {
            environment["CARAPACE_GATEWAY_TOKEN"] = token
        } else if let password = intent.endpoint.password {
            environment["CARAPACE_GATEWAY_PASSWORD"] = password
        }
