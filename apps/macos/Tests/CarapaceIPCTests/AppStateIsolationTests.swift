...
struct AppStateIsolationTests {
    ...
    func `preview constructor uses launch namespace and owned config`() async throws {
        ...
        let fm = FileManager()
        let home = try #require(CarapaceEnv.path("HOME"))
        ...
        let configURL = fixture.appendingPathComponent("carapace.json")
        let seededKeys = [
        ...
        let launchState = try await TestIsolation.withEnvValues([:]) {
            let home = try #require(CarapaceEnv.path("HOME"))
            #expect(CarapaceEnv.path("CFFIXED_USER_HOME") == home)
            ...
            let tmp = try #require(CarapaceEnv.path("TMPDIR"))
            #expect(URL(fileURLWithPath: tmp).resolvingSymlinksInPath().path ==
                root.appendingPathComponent("tmp").resolvingSymlinksInPath().path)
            #expect(CarapacePaths.stateDirURL == root.appendingPathComponent("state", isDirectory: true))
            #expect(CarapacePaths.configURL == CarapacePaths.stateDirURL.appendingPathComponent("carapace.json"))
            return CarapacePaths.stateDirURL
        }

        let fixtureState = try await TestIsolation.withIsolatedState(
            env: ["CARAPACE_CONFIG_PATH": configURL.path],
            defaults: defaults)
        {
            ...
            let stateDirectory = CarapacePaths.stateDirURL
            ...
            #expect(try fm.contentsOfDirectory(atPath: fixture.path).contains {
                $0.hasPrefix("carapace.json.clobbered.")
            })
            return stateDirectory
        }
        #expect(!fm.fileExists(atPath: fixtureState.path))
        await TestIsolation.withEnvValues([:]) {
            #expect(CarapacePaths.stateDirURL == launchState)
    ...
    func `config fixture cleans audit after throwing body`() async throws {
        ...
        do {
            try await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": configPath]) {
                fixtureState = CarapacePaths.stateDirURL
                #expect(CarapaceConfigFile.saveDict(["gateway": ["mode": "local"]]))
                await Task.yield()
                #expect(fm.fileExists(atPath: CarapacePaths.stateDirURL
