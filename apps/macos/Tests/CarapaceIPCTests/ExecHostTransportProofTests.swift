...
struct ExecHostTransportProofTests {
    @Test(.enabled(if: ProcessInfo.processInfo.environment["CARAPACE_EXEC_HOST_NATIVE_PROOF"] == "1"))
    func `native execution completes after caller response loss`() async throws {
        let environment = ProcessInfo.processInfo.environment
        let statePath = try #require(environment["CARAPACE_STATE_DIR"])
        ...
        for key in ["HOME", "CFFIXED_USER_HOME", "CARAPACE_HOME"] {
