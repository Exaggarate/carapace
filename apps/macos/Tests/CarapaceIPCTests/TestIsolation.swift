...
enum TestIsolation {
    ...
    {
        precondition(!env.keys.contains("CARAPACE_PROFILE"), "Select the app profile before launching the test process")
        ...
        let ownedStateDirectory: URL? = if let configPath = env["CARAPACE_CONFIG_PATH"] ?? nil,
                                           !configPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
                                           !env.keys.contains("CARAPACE_STATE_DIR")
        {
            FileManager().temporaryDirectory
                .appendingPathComponent("carapace-config-state-\(UUID().uuidString)", isDirectory: true)
        } else {
            nil
        }
        if let ownedStateDirectory {
            env["CARAPACE_STATE_DIR"] = ownedStateDirectory.path
        }
        defer {
            if let ownedStateDirectory {
    ...
    nonisolated static func tempConfigPath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("carapace-test-config-\(UUID().uuidString).json")
            .path
    }
}
...
struct ExecApprovalsStateIsolationTrait: TestTrait, TestScoping {
        ...
        performing function: @Sendable () async throws -> Void) async throws
    {
        let stateDirectory = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-approvals-state-\(UUID().uuidString)", isDirectory: true)
