...
struct CLIInstallerTests {
    @Test func `managed install locations follow the app profile`() {
        ...
        let cases = [
            (AppProfile(environment: [:]), "/Users/Test User/.carapace"),
            (
                AppProfile(environment: [
                    "CARAPACE_PROFILE": "onboardtest",
                    "CARAPACE_STATE_DIR": "/tmp/ignored-state",
                ]),
                "/Users/Test User/.carapace-onboardtest"),
        ]
        ...
        for (profile, expectedPrefix) in cases {
            ...
            #expect(CLIInstaller.managedExecutableLocation(homeDirectory: home, profile: profile) ==
                "\(expectedPrefix)/bin/carapace")

            let command = CLIInstaller.installScriptCommand(
                target: .exact("2026.7.3"),
                prefix: prefix,
                scriptPath: "/Applications/Carapace.app/Contents/Resources/install-cli.sh")
    ...
    @Test func `installed location finds executable`() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "carapace-cli-installer-\(UUID().uuidString)")
        defer { try? fm.removeItem(at: root) }

        let binDir = root.appendingPathComponent("bin")
        try fm.createDirectory(at: binDir, withIntermediateDirectories: true)
        let cli = binDir.appendingPathComponent("carapace")
    ...
    @Test func `installer command runs the signed bundled script without a shell pipeline`() {
        let command = CLIInstaller.installScriptCommand(
            ...
            prefix: "/Users/Test User/.carapace",
            scriptPath: "/Applications/Carapace.app/Contents/Resources/install-cli.sh",
            compatibleWith: "2026.7.4")

        #expect(command == [
            "/bin/bash",
            "/Applications/Carapace.app/Contents/Resources/install-cli.sh",
            "--json",
            "--no-onboard",
            "--prefix",
            "/Users/Test User/.carapace",
            "--version",
            "2026.7.3-beta.1",
        ])
    ...
    @Test func `dev installer uses a managed git main checkout`() {
        ...
        #expect(command.suffix(5) == [
            ...
            "git",
            "--git-dir",
            "/Users/Test User/.carapace/dev/carapace",
        ])
    }
    ...
    @Test func `installer events map to concise live status`() {
        let cases: [(String, String)] = [
            ...
            (#"{"event":"step","name":"git-clone","status":"start"}"#, "Downloading Carapace source…"),
            (#"{"event":"step","name":"git-update","status":"start"}"#, "Updating Carapace source…"),
            ...
            (#"{"event":"step","name":"cli-build","status":"start"}"#, "Building Carapace CLI…"),
            (#"{"event":"step","name":"carapace","status":"retry"}"#, "Retrying Carapace CLI install…"),
            (
                #"{"event":"step","name":"disk-space","status":"warn"}"#,
                "Couldn’t verify free disk space; continuing…"),
            (
                #"{"event":"step","name":"git-update","status":"warn"}"#,
                "Using the existing modified Carapace source…"),
            (
    ...
    @Test func `managed update uses the canonical updater without accepting downgrades`() {
        let command = CLIInstaller.managedUpdateCommand(
            executable: "/Users/Test User/.carapace/bin/carapace",
            targetVersion: "2026.7.4")

        #expect(command == [
            "/Users/Test User/.carapace/bin/carapace",
            "update",
            "--tag",
            "2026.7.4",
            "--json",
        ...
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/carapace",
            targetVersion: "2026.7.4",
            profile: profile) == [
            "/opt/carapace", "--profile", "work", "update", "--tag", "2026.7.4",
            "--json", "--timeout", "900",
        ])
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/carapace",
            ...
            repair: true,
            profile: profile) == [
            "/opt/carapace", "--profile", "work", "update", "repair", "--json",
            "--timeout", "900", "--yes",
        ])
    }
    ...
    @Test func `validated CLI cache changes only when the ready tuple changes`() throws {
        ...
        let initialLocation = "/Users/test/.local/bin/carapace"
        ...
        let updatedLocation = "/opt/homebrew/bin/carapace"
        CLIInstaller.rememberValidated(
    ...
    @Test func `managed setup requires a parseable compatible version`() {
        let location = "/Users/test/.carapace/bin/carapace"

        #expect(CLIInstaller.classifyVersion(
            location: location,
            output: "Carapace 2026.7.3\n",
    ...
    {
        let root = FileManager().temporaryDirectory.appendingPathComponent(
            "carapace-old-node-cli-\(UUID().uuidString)")
        defer { try? FileManager().removeItem(at: root) }
        try FileManager().createDirectory(at: root, withIntermediateDirectories: true)
        let executable = root.appendingPathComponent("carapace")
    ...
    @Test func `CLI probe preserves environment and resolves shebang tools beside executable`() {
        let location = "/custom/bin/carapace"
    ...
    @Test func `managed CLI probe prefers its private runtime`() {
        let executable = "/Users/test/.carapace/bin/carapace"
        let environment = CLIInstaller.probeEnvironment(
            location: executable,
            processEnvironment: [:],
            preferredPaths: ["/Users/test/.nvm/versions/node/v20/bin", "/usr/bin"],
            managedExecutable: executable,
            managedRuntimeDirectory: "/Users/test/.carapace/tools/node/bin")

        #expect(environment["PATH"] == [
            "/Users/test/.carapace/bin",
            "/Users/test/.carapace/tools/node/bin",
