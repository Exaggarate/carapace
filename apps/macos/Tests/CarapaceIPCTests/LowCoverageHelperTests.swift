...
struct LowCoverageHelperTests {
    private typealias ProtoAnyCodable = CarapaceProtocol.AnyCodable
    ...
    @Test func `shell executor stops before spawn when final preflight fails`() async {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-shell-preflight-\(UUID().uuidString)")
    ...
    @Test func `port guardian classifies tunnel records for reaping`() {
        ...
        // Parent alive (e.g. a concurrent Carapace instance) → hands off.
        #expect(PortGuardian.classifyTunnelRecord(
            record,
