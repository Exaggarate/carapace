...
@Suite(.serialized) struct NodeServiceManagerTests {
    @Test func `absent node service performs no CLI lifecycle work`() async throws {
        ...
        try await TestIsolation.withIsolatedState(
            ...
            defaults: ["carapace.gatewayProjectRootPath": nil])
        {
    ...
    @Test func `builds node service commands with current CLI shape`() async throws {
        try await TestIsolation.withUserDefaultsValues(["carapace.gatewayProjectRootPath": nil]) {
            ...
            #expect(start == [carapacePath.path, "node", "start", "--json"])

            let stop = await NodeServiceManager._testServiceCommand(["stop"])
            #expect(stop == [carapacePath.path, "node", "stop", "--json"])

            let restart = await NodeServiceManager._testServiceCommand(["restart"])
            #expect(restart == [carapacePath.path, "node", "restart", "--json"])
        }
    }
    ...
    func `node lifecycle commands finish in request order`(_ transition: String) async throws {
        ...
        try await TestIsolation.withIsolatedState(
            env: [
                ...
                "CARAPACE_NODE_SERVICE_TEST_ROOT": root.path,
                "CARAPACE_NODE_SERVICE_DELAYED_ACTION": previousAction,
            ],
            defaults: ["carapace.gatewayProjectRootPath": nil])
        {
            CommandResolver.setProjectRoot(root.path)
            ...
            #!/bin/sh
            action="$2"
            : > "$CARAPACE_NODE_SERVICE_TEST_ROOT/$action.entered"
            if [ "$action" = "$CARAPACE_NODE_SERVICE_DELAYED_ACTION" ]; then
              while [ ! -f "$CARAPACE_NODE_SERVICE_TEST_ROOT/release" ]; do
                sleep 0.01
              done
            fi
            if [ "$action" = "stop" ]; then
              printf stopped > "$CARAPACE_NODE_SERVICE_TEST_ROOT/state"
            else
              printf running > "$CARAPACE_NODE_SERVICE_TEST_ROOT/state"
            fi
            printf '{"ok":true}'
            """
    ...
    func `node lifecycle respects process exit and optional JSON status`(_ scenario: String) async throws {
        ...
        try await TestIsolation.withIsolatedState(
            env: ["HOME": root.path, "CFFIXED_USER_HOME": root.path, "CARAPACE_NODE_SERVICE_TEST_CASE": scenario],
            defaults: ["carapace.gatewayProjectRootPath": nil])
        {
            CommandResolver.setProjectRoot(root.path)
            let executable = root.appendingPathComponent("node_modules/.bin/carapace")
            ...
            let script = """
            #!/bin/sh
            case "$CARAPACE_NODE_SERVICE_TEST_CASE" in
              ...
              json-failure-with-hints*)
                printf '{"ok":false,"error":"Node service not installed.",'
                printf '"hints":["carapace node install","carapace node start","third hint"]}'
                if [ "$CARAPACE_NODE_SERVICE_TEST_CASE" = "json-failure-with-hints-and-exit" ]; then exit 1; fi
                ;;
              json-failure-hints-only)
                printf '{"ok":false,"hints":["carapace node install","carapace node start"]}'
                ;;
              not-loaded-*)
                printf '{"ok":true,"result":"not-loaded","message":"Node service not loaded.",'
                printf '"hints":["carapace node install","carapace node start"]}'
                ;;
            ...
            case "json-failure-with-hints", "json-failure-with-hints-and-exit":
                "Node service not installed. (carapace node install · carapace node start)"
            case "json-failure-hints-only": "carapace node install · carapace node start"
            case "not-loaded-start":
                "Node service not loaded. (carapace node install · carapace node start)"
            default: nil
            }
    ...
    @Test func `reads node service ownership command directly from launchd`() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-node-\(UUID().uuidString).plist")
