...
struct MacNodeCodexThreadCatalogTests {
    ...
    private func makeFakeCodex(_ script: String) throws -> FakeCodex {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-fake-codex-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `resolves and runs the configured Codex App Server without a shell`() async throws {
        let clearEnvSentinel = "CARAPACE_CODEX_CATALOG_CLEAR_ENV_SENTINEL"
        ...
        let fake = try makeAppServer(
            ...
            [ "$2" = "--stdio" ] || exit 11
            [ -z "${CARAPACE_CODEX_CATALOG_CLEAR_ENV_SENTINEL+x}" ] || exit 12
            """#,
            body: #"""
    ...
    @Test func `uses official environment command and argument fallbacks`() throws {
        ...
        let resolved = try MacNodeCodexThreadCatalog.resolveInvocation(
            root: [:],
            environment: [
                "CARAPACE_CODEX_APP_SERVER_BIN": " \(fake.executable.path) ",
                "CARAPACE_CODEX_APP_SERVER_ARGS": #"custom-app-server "--listen" 'stdio://'"#,
            ],
            searchPaths: [],
    ...
    @Test func `blank configured command falls back to the environment command`() throws {
        ...
        let resolved = try MacNodeCodexThreadCatalog.resolveInvocation(
            root: root,
            environment: ["CARAPACE_CODEX_APP_SERVER_BIN": fallback.executable.path],
            searchPaths: [])
