...
struct QuickChatPowerFeaturesTests {
    private static let solModelChoice = CarapaceChatModelChoice(
        modelID: "gpt-5.6-sol",
        name: "Sol",
        provider: "openai",
    ...
    @Test func `paste rejects a stale assistant and the Carapace process`() {
        let messages = [
            Self.message(role: "user", text: "Question", idempotencyKey: "send-1"),
    ...
    @Test func `model thinking options reject an unsupported explicit override`() {
        let options = [CarapaceChatThinkingLevelOption(id: "off", label: "Off")]
