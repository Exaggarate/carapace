...
struct OnboardingViewSmokeTests {
    ...
    @Test func `foreign local listener is not advertised as attachable`() {
        let profile = AppProfile(environment: ["CARAPACE_PROFILE": "p2380"])
    ...
    @Test func `paused gateway keeps CLI setup and recovery visible after every install path`() {
        for afterFreshInstall in [false, true] {
            ...
            #expect(outcome.status == "Carapace is paused. Resume it, then retry setup to start the Gateway.")
        }
    }
    ...
    @Test func `local gateway activation preserves readiness and concrete failure reasons`() {
        for afterFreshInstall in [false, true] {
            let ready = OnboardingView.localGatewayActivationOutcome(
                ...
                afterFreshInstall: afterFreshInstall)
            #expect(ready.ready)
            #expect(ready.status == "Carapace Gateway is ready.")
            ...
            #expect(failure.status == (afterFreshInstall
                    ? "Carapace was installed, but the Gateway did not start. Retry setup. (launchd disabled)"
                    : "Carapace is installed, but the Gateway did not start. Retry setup. (launchd disabled)"))
        }
    }
    ...
    @Test func `gateway start failure message retains the concrete reason`() {
        #expect(
            OnboardingView.gatewayStartFailureMessage(
                prefix: "Carapace was installed, but the Gateway did not start. Retry setup.",
        ...
        #expect(
            OnboardingView.gatewayStartFailureMessage(
                prefix: "Carapace was installed, but the Gateway did not start. Retry setup.",
                reason: nil) ==
                "Carapace was installed, but the Gateway did not start. Retry setup.")
        #expect(
            OnboardingView.gatewayStartFailureMessage(
                prefix: "Carapace was installed, but the Gateway did not start. Retry setup.",
                reason: "") ==
