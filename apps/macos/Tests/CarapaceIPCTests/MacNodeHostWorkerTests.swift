...
struct MacNodeHostWorkerTests {
    ...
    @Test(arguments: [
        CarapaceSystemCommand.run.rawValue,
        "mcp.tools.call.v1",
    ...
    @Test(arguments: [MacNodeScreenCommand.snapshot.rawValue, CarapaceComputerCommand.act.rawValue])
    ...
    @Test func `provider selection filters command ownership and publishes each provider descriptor`() throws {
        let descriptor = CarapaceProtocol.AnyCodable([
            "contractVersion": CarapaceProtocol.AnyCodable(2),
        ])
        let manifest = MacNodeHostManifest(
            version: "test",
            caps: ["screen", "computer"],
            commands: [MacNodeScreenCommand.snapshot.rawValue, CarapaceComputerCommand.act.rawValue],
        ...
        #expect(!peekaboo.commands.contains(CarapaceComputerCommand.act.rawValue))
    ...
    @Test func `elevation host never advertises a persisted CUA provider`() throws {
        ...
        let provider = ComputerControlProvider.current(
            defaults: defaults,
            cuaAvailable: true,
            launchPlan: AppLaunchRuntimePlan(arguments: ["Carapace", "--elevation-host"]))
        #expect(provider == .peekaboo)

        let cuaDescriptor = CarapaceProtocol.AnyCodable(["provider": "cua"])
        let manifest = MacNodeHostManifest(
            version: "test",
            caps: ["screen", "computer"],
            commands: [MacNodeScreenCommand.snapshot.rawValue, CarapaceComputerCommand.act.rawValue],
    ...
    @Test func `worker forces app exec host without fallback or startup respawn`() async throws {
        let worker = MacNodeHostWorker(session: GatewayNodeSession())
        let script = """
        test "$CARAPACE_NODE_EXEC_HOST" = app || exit 42
        test "$CARAPACE_NODE_EXEC_FALLBACK" = 0 || exit 43
        test "$CARAPACE_NO_RESPAWN" = 1 || exit 46
    ...
    @Test func `worker strips inherited CUA values and receives only the app-provided endpoint`() async throws {
        let endpoint = CuaDriverWorkerEndpoint(
            ...
            binaryPath: "/Applications/Carapace.app/Contents/Resources/cua-driver")
        ...
        let script = """
        test "$CARAPACE_CUA_DRIVER_ENDPOINT" = "$1" || exit 41
        test "$(env | grep -Ec '^(CARAPACE_)?CUA_DRIVER_')" = 1 || exit 42
    ...
    @Test func `unbound worker strips every inherited CUA endpoint value`() async throws {
        ...
        let script = """
        test "$(env | grep -Ec '^(CARAPACE_)?CUA_DRIVER_')" = 0 || exit 41
        printf '%s\\n' '{"type":"ready","version":"test","manifest":{"caps":[],"commands":[],"pathEnv":"/usr/bin:/bin"},"inventory":{"skills":null,"pluginTools":[]}}'
        while IFS= read -r line; do :; done
        """
    ...
    @Test func `worker cancellation settles when the child suppresses its result`() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-worker-cancel-\(UUID().uuidString)")
    ...
    @Test func `worker deinit terminates its owned process`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-worker-deinit-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `stop cancels a changed launch during worker cleanup`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-worker-restart-stop-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    @Test func `stop waits for the worker leader and reaps its descendants`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-worker-stop-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
