import Foundation
import Testing
@testable import CarapaceDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._carapace-gw._tcp.carapace.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.carapace.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Carapace)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/carapace/src/entry.ts\""
                }
                return ""
            })

        // Env mutation must hold TestIsolationLock; raw setenv races parallel
        // tests scanning environ (e.g. CARAPACE_CONFIG_PATH readers).
        let beacons = await TestIsolation.withEnvValues(
            ["CARAPACE_WIDE_AREA_DOMAIN": "carapace.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Carapace)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/carapace/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._carapace-gw._tcp.carapace.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
