...
struct ComputerActionServiceTests {
    ...
    private final class ActionTaskBox {
        var task: Task<CarapaceComputerActResult, Error>?
    }
    ...
    private func unsupportedAction(_ error: Error?) -> CarapaceComputerAction? {
    ...
    @Test func `capture grant distinguishes a stale accessibility grant`() {
        ...
        Carapace may already appear enabled under System Settings → Privacy & Security → Accessibility. \
        If so, the grant is pinned to an older build: select Carapace, remove it with −, then re-add \
    ...
    @Test func `missing accessibility and capture is a plain missing permission`() {
        ...
        Grant access in System Settings → Privacy & Security, then reopen Carapace.
        """)
    ...
    @Test func `coordinate input requires the current display frame identity`() throws {
        ...
        let missing = CarapaceComputerActParams(
            action: .leftClick,
            x: 1,
            y: 2,
            refWidth: 1280)
        ...
        let stale = CarapaceComputerActParams(
            action: .leftClick,
        ...
        let current = CarapaceComputerActParams(
            action: .leftClick,
        ...
        let missingScale = CarapaceComputerActParams(
            action: .leftClick,
            displayFrameId: currentFrameId,
            x: 1,
            y: 2)
        ...
        let display = CarapaceComputerDisplayGeometry(
            originX: 0,
            originY: 0,
            widthPoints: 1280,
            heightPoints: 800)
        let screenshotFrameId = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let mismatchedScaleFrameId = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
            display: display)
        let mismatchedScale = CarapaceComputerActParams(
            action: .leftClick,
        ...
        let keyboard = CarapaceComputerActParams(action: .type, text: "hello")
    ...
    @Test func `computer actions execute in FIFO order without overlap`() async throws {
        ...
        let firstParams = CarapaceComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let secondParams = CarapaceComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `window authority rejects a result after lifecycle revocation`() async {
        ...
        do {
            _ = try await service.perform(
                CarapaceComputerActParams(action: .getCursorPosition),
    ...
    @Test func `window executor rejects foreground accessibility-only actions`() async {
        let service = ComputerWindowActionExecutor()
        for action in [CarapaceComputerAction.setValue, .invokeMenu] {
            let outcomeError: Error?
            do {
                _ = try await service.perform(
                    CarapaceComputerActParams(action: action, deliveryMode: .foreground),
                    lifecycleGeneration: 0,
                    checkExecutionAllowed: {})
                outcomeError = nil
            } catch {
    ...
    @Test func `cancelled queued action never executes`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: { true })
        let firstParams = CarapaceComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let cancelledParams = CarapaceComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `cancelled active action releases held input before it settles`() async {
        ...
        let params = CarapaceComputerActParams(action: .leftMouseDown, x: 1, y: 0, refWidth: 1280)
    ...
    @Test func `new lifecycle generation cancels old work before fresh action`() async throws {
        ...
        let releaseProbe = LifecycleReleaseProbe(allowed: true)
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let oldParams = CarapaceComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let freshParams = CarapaceComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `failed lifecycle mouse up blocks newer generation until retry succeeds`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let params = CarapaceComputerActParams(action: .type, x: 2, y: 0, refWidth: 1280)
