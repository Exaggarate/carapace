...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `carapace --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("Carapace 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  Carapace 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("Carapace 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("Carapace 2026.4.2-1 (d74a122)")
    ...
    @Test func `failed global version probe does not borrow the local package version`() async throws {
        let projectRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-gateway-environment-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `failed gateway probe cannot validate version output from a broken executable`() async throws {
        ...
        let gateway = root.appendingPathComponent("carapace")
        try "#!/bin/sh\necho Carapace 2026.7.30\nexit 1\n"
    ...
    @Test func `broken global gateway remains an actionable error beside a valid checkout`() async throws {
        ...
        let gateway = bin.appendingPathComponent("carapace")
        ...
        await TestIsolation.withIsolatedState(
            defaults: ["carapace.gatewayProjectRootPath": root.path])
        {
    ...
    @Test func `named profiles derive stable distinct gateway ports after explicit precedence`() {
        ...
        let personal = AppProfile(environment: ["CARAPACE_PROFILE": "personal"])
        ...
        #expect(GatewayEnvironment.resolvedGatewayPort(
            environment: ["CARAPACE_GATEWAY_PORT": "21001"],
            configPort: 22001,
            storedPort: 23001,
            profile: work) == 21001)
        #expect(GatewayEnvironment.resolvedGatewayPort(
