...
struct PortGuardianRecordStoreTests {
    @Test
    func `storage marker distinguishes older Carapace writers from aligned copies`() {
        ...
        #expect(PortGuardian.usesLegacyPortGuardianStorage(
            ...
            storageVersion: nil))
        #expect(PortGuardian.usesLegacyPortGuardianStorage(
            bundleIdentifier: "ai.carapace.mac.debug",
            storageVersion: 1))
        #expect(!PortGuardian.usesLegacyPortGuardianStorage(
            bundleIdentifier: "ai.carapace.mac",
            storageVersion: 2))
    ...
    func `storage marker in app bundle matches runtime capability`() throws {
        let macOSRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let infoURL = macOSRoot.appendingPathComponent("Sources/Carapace/Resources/Info.plist")
        let data = try Data(contentsOf: infoURL)
        let values = try #require(
            PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
        let version = try #require(values["CarapacePortGuardianStorageVersion"] as? NSNumber)
    ...
    private static func fixture() throws -> Fixture {
        ...
        return Fixture(
            root: root,
            databaseURL: root.appendingPathComponent("state/carapace.sqlite"),
