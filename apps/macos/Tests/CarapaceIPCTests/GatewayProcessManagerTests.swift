...
struct GatewayProcessManagerTests {
    ...
    @Test func `colliding profile ports cannot attach another profile gateway`() {
        let first = AppProfile(environment: ["CARAPACE_PROFILE": "p1402"])
        let second = AppProfile(environment: ["CARAPACE_PROFILE": "p2380"])
        ...
        _ body: () async throws -> T) async throws -> T
    {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-launchagent-marker-\(UUID().uuidString)")
    ...
    private func gatewayDescriptor(
        pid: Int32,
        command: String = "carapace-gateway",
        executablePath: String = "/tmp/carapace-gateway") -> PortGuardian.Descriptor
    {
        ...
        configAudit: String = #"{"ok":true,"issues":[]}"#) -> String
    {
        ...
        {"ok":true,"service":{
          ...
          "command":{"programArguments":["carapace","gateway","--port","\(port)"]},
        ...
        try await self.withLaunchAgentEnvironment {
            let staleStatuses = [
                ...
                {"ok":true,"service":{
                  ...
                  "command":{"programArguments":["carapace","gateway","--port","19084"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """,
                """
    ...
    @Test func `readiness refreshes when the endpoint pid changes during the probe`() async throws {
        try await self.withLaunchAgentEnvironment {
                ...
                GatewayTestWebSocketTask(
                    sendHook: { task, message, sendIndex in
                        ...
                        let replacement = PortGuardian.Descriptor(
                            ...
                            executablePath: "/tmp/carapace-gateway")
            ...
            let stateDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("carapace-gateway-pid-refresh-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `transient unavailable health response retries until ready`() async throws {
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-gateway-ready-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: stateDir) }
        try await self.withLaunchAgentEnvironment {
    ...
    @Test func `failed reattachment releases departed independent Gateway ownership`() async throws {
        ...
        try await self.withLaunchAgentEnvironment(port: port) {
            ...
            let configPath = try #require(ProcessInfo.processInfo.environment["CARAPACE_CONFIG_PATH"])
    ...
    @Test func `identity conflict paths cannot select Gateway auth guidance`() async throws {
        ...
        let reason = try await self.attachFailureReason {
            throw NSError(
                domain: "ai.carapace.device-identity-store",
                code: 2,
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        ...
        do {
            ...
            do {
                ...
                #expect(details.contains("pid 4343 carapace-gateway @ /tmp/carapace-gateway"))
                #expect(manager._testControlChannelRefreshForces().last == true)
                await cleanup()
            } catch {
                await cleanup()
