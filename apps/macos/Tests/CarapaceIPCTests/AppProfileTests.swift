...
struct AppProfileTests {
    @Test func `default profile preserves historical identities`() {
        ...
        for raw in [nil, "", " default ", "Default"] {
            let profile = AppProfile(environment: raw.map { ["CARAPACE_PROFILE": $0] } ?? [:])
            #expect(profile.name == nil)
            #expect(profile.validationError == nil)
            #expect(profile.gatewayLaunchAgentLabel == "ai.carapace.gateway")
            #expect(profile.defaultsSuiteName == nil)
            #expect(profile.keychainService(base: "ai.carapace.test") == "ai.carapace.test")
            #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
                "ai.carapace.tls-pinning")
    ...
    @Test func `current profile scopes credential service identities`() {
        ...
        #if DEBUG
        #expect(MacGatewayProfileStore.service == "ai.carapace.gateway-profiles.debug\(suffix)")
        #expect(GatewayActivationBindingKeyStore.service == "ai.carapace.onboarding-route-binding.debug\(suffix)")
        #else
        #expect(MacGatewayProfileStore.service == "ai.carapace.gateway-profiles\(suffix)")
        #expect(GatewayActivationBindingKeyStore.service == "ai.carapace.onboarding-route-binding\(suffix)")
        #endif
    }

    @Test func `named profile owns every persistent namespace`() {
        let profile = AppProfile(environment: ["CARAPACE_PROFILE": "work_2"])
        #expect(profile.name == "work_2")
        #expect(profile.validationError == nil)
        #expect(profile.gatewayLaunchAgentLabel == "ai.carapace.work_2")
        #expect(profile.defaultsSuiteName == "ai.carapace.mac.profile.work_2")
        #expect(profile.keychainService(base: "ai.carapace.test") == "ai.carapace.test.profile.work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")).path ==
            "/Users/test/.carapace-work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")) !=
            AppProfile(environment: ["CARAPACE_PROFILE": "personal"])
            .stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")))
        #expect(profile.cliRootArguments == ["--profile", "work_2"])
        #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
            "ai.carapace.tls-pinning.profile.work_2")
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            prefix: ["/opt/carapace"],
            profile: profile) == [
            "/opt/carapace", "--profile", "work_2", "node", "worker",
        ])
    }
    ...
    @Test func `default worker commands preserve exact argument shapes`() {
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            prefix: ["/opt/carapace"],
            profile: profile) == ["/opt/carapace", "node", "worker"])
    }
    ...
    @Test func `invalid and colliding profile names fail closed`() {
        ...
        for raw in invalid {
            let profile = AppProfile(environment: ["CARAPACE_PROFILE": raw])
            #expect(profile.name == nil)
            #expect(profile.validationError != nil)
        }
        #expect(AppProfile(environment: ["CARAPACE_PROFILE": String(repeating: "a", count: 64)]).name != nil)
    }
    ...
    @Test func `colliding profile apps cannot reserve the same exact port`() throws {
        ...
        let first = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["CARAPACE_PROFILE": "p1402"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
        let second = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["CARAPACE_PROFILE": "p2380"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
    ...
    @Test func `persisted other profile gateway reserves its exact port`() throws {
        ...
        let serviceEnv = home.appendingPathComponent(".carapace-p1402/service-env", isDirectory: true)
        try FileManager.default.createDirectory(at: serviceEnv, withIntermediateDirectories: true)
        let wrapper = serviceEnv.appendingPathComponent("ai.carapace.p1402-env-wrapper.sh")
        let environment = serviceEnv.appendingPathComponent("ai.carapace.p1402.env")
        try Data("#!/bin/sh\n".utf8).write(to: wrapper)
        try Data("""
        export CARAPACE_SERVICE_MARKER='carapace'
        export CARAPACE_SERVICE_KIND='gateway'
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": [
                    ...
                    "node", "carapace.mjs", "gateway", "--port", "55636",
                ],
            ],
            format: .xml,
            options: 0)
    ...
    @MainActor @Test func `profile lock excludes only the same profile`() throws {
        ...
        let workProfile = AppProfile(environment: ["CARAPACE_PROFILE": "work"])
        ...
        #expect(workProfile.instanceLockURL(systemTemporaryDirectory: root) ==
            AppProfile(environment: [
                ...
                "CARAPACE_STATE_DIR": "/different/state",
    ...
    @Test func `profile lock reports unsafe ownership errors`() throws {
        ...
        let unsafeURL = AppProfile(environment: ["CARAPACE_PROFILE": "unsafe"])
            .instanceLockURL(systemTemporaryDirectory: root)
    ...
    @Test func `long profile approvals socket uses deterministic private short path`() {
        ...
        #expect(first == second)
        #expect(first.hasPrefix("/tmp/carapace-\(geteuid())/exec-approvals-"))
