...
struct ExecApprovalPolicySnapshotTests {
    ...
    func `portable snapshot round trips canonical policy semantics`() {
        ...
        #expect(portable.allowlistRules == [
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/usr/bin/tool"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(
                pattern: "/usr/bin/tool",
                argPattern: "^ok$",
                source: .allowAlways),
        ])
    ...
    func `portable snapshot decode deduplicates and sorts rules by utf8 bytes`() throws {
        ...
        let portable = try JSONDecoder().decode(
            CarapaceSystemRunApprovalPolicySnapshot.self,
            from: data)

        #expect(portable.allowlistRules == [
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/", argPattern: "A"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/", argPattern: "é"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/A"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/A", source: .allowAlways),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/e\u{0301}"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/ä"),
            CarapaceSystemRunApprovalPolicySnapshot.Rule(pattern: "/é"),
        ])
