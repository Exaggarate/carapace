...
extension DashboardWindowOwnershipTests {
    ...
    private static func checkDeviceSettingsSettlement(_ transition: String) async throws {
        ...
        defer { server.stop() }
        let autosaveName = "CarapaceDashboardWindow-Test-\(UUID().uuidString)"
        ...
        function recordDeviceReply(type, request) {
          window.webkit.messageHandlers.carapaceDeviceSettings.postMessage(request).then(
        ...
        case "revoke-domains":
            ...
            return await window.webkit.messageHandlers.carapaceDeviceSettings.postMessage({
