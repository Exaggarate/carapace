...
struct DashboardSandboxNavigationTests {
    ...
    @Test func `sign-in documents cannot observe native data or consume pending commands`() async throws {
        let server = try await DashboardHTTPFixture.start(
            ...
            window.commands = 0;
            window.addEventListener('carapace:native-new-session', () => window.commands++);
            window.addEventListener('carapace:native-navigate', event => {
              window.navigation = event.detail.path;
              event.preventDefault();
            });
            window.__CARAPACE_NATIVE_COMMANDS_READY__ = location.pathname.startsWith('/control/');
        ...
        [window.__CARAPACE_NATIVE_CONTROL_AUTH__, window.__CARAPACE_NATIVE_GATEWAYS__,
         window.__CARAPACE_NATIVE_WEB_CHROME__, window.__CARAPACE_NATIVE_HISTORY__,
         window.__CARAPACE_NATIVE_NOTIFICATIONS__].some(value => value !== undefined)
        """) as? Bool
        ...
        #expect(try await controller.webView.evaluateJavaScript(
            "window.__CARAPACE_NATIVE_GATEWAYS__.gateways[0].name") as? String == "Private Gateway")
        #expect(try await controller.webView.evaluateJavaScript(
            "window.__CARAPACE_NATIVE_CONTROL_AUTH__.token === null && " +
                "window.__CARAPACE_NATIVE_CONTROL_AUTH__.password === null") as? Bool == true)
    }
    ...
    @Test func `same URL sign in retains commands until the current document installs its shell`() async throws {
        let server = try await DashboardHTTPFixture.start(
            ...
            window.commands = [];
            window.mountShell = () => {
              window.addEventListener('carapace:native-new-session', () => window.commands.push('new'));
              window.addEventListener('carapace:native-toggle-search', event => {
                window.commands.push('search'); event.preventDefault();
              });
              window.__CARAPACE_NATIVE_COMMANDS_READY__ = true;
              window.dispatchEvent(new Event('carapace:native-commands-state'));
            };
            </script></body></html>
            """, contentSecurityPolicy: "default-src 'none'; script-src 'unsafe-inline'")
        ...
        try await self.waitForDocument(
            controller, url: url, ready: "window.__CARAPACE_NATIVE_COMMANDS_READY__ === undefined")
        controller.dispatchNativeCommand(.newSession)
        ...
        _ = try await controller.webView.evaluateJavaScript(
            "window.webkit.messageHandlers.carapaceCommands.postMessage({type: 'commands-state'}); null")
        #expect(controller._testPendingNativeCommands == [.newSession])
    ...
    @Test func `blob documents cannot inherit the root dashboard native credentials`() async throws {
        ...
        #expect(try await controller.webView.evaluateJavaScript("""
        [window.__CARAPACE_NATIVE_CONTROL_AUTH__, window.__CARAPACE_NATIVE_WEB_CHROME__,
         window.__CARAPACE_NATIVE_HISTORY__].every(value => value === undefined)
        """) as? Bool == true)
    ...
    func `sandbox navigation requires a trusted dashboard subframe`(_ address: String) throws {
        let dashboard = try #require(URL(string: "https://carapace.example/control/"))
    ...
    func `dashboard source trust preserves the browser pathname`(_ mountPath: String) throws {
        let dashboard = try #require(URL(string: "https://carapace.example\(mountPath)"))
        ...
        let encodedSeparator = try #require(URL(string: "https://carapace.example\(mountPath.dropLast())%2Fchat"))
            ...
            addEventListener('message', event => {
              ...
              addEventListener('message', reply => {
                if (reply.source === inner.contentWindow && reply.data === 'app-ready') {
                  parent.postMessage({ready: true, nativeAuth: !!window.__CARAPACE_NATIVE_CONTROL_AUTH__}, '*');
                }
              });
