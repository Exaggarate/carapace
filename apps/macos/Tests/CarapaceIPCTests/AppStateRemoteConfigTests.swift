...
struct AppStateRemoteConfigTests {
    ...
    func `route edit during config read fails the source snapshot closed`() async {
        ...
        await TestIsolation.withIsolatedState(
            env: ["CARAPACE_CONFIG_PATH": configPath],
    ...
    func `remote identity edit updates canonical SSH config before routing resumes`() async {
            ...
            defaults: [connectionModeKey: AppState.ConnectionMode.remote.rawValue])
        {
            ...
            let remote = (CarapaceConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
    ...
    func `incomplete direct URL edits survive sync until completed`(savedURL: String?) async {
        let configPath = TestIsolation.tempConfigPath()
        await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            var remote: [String: Any] = ["transport": "direct"]
            remote["url"] = savedURL
            #expect(CarapaceConfigFile.saveDict(["gateway": ["mode": "remote", "remote": remote]]))
            let state = AppState(preview: true)
            state._testEnableGatewayConfigSync()
            ...
            for draft in ["w", "ws", "wss", "wss:", "wss:/", "wss://"] {
                ...
                #expect(GatewayRemoteConfig.resolveUrlString(root: CarapaceConfigFile.loadDict()) == savedURL)
            }
            ...
            #expect(state.gatewayConfigIsCurrentForRouting)
            #expect(GatewayRemoteConfig.resolveUrlString(root: CarapaceConfigFile.loadDict()) == state.remoteUrl)
        }
    }
    ...
    func `incomplete direct URL retains ownership across external config edits`() async {
        ...
        await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            ...
            externalRoot = CarapaceConfigFile.loadDict()
            ...
            #expect(GatewayRemoteConfig.resolveUrlString(root: CarapaceConfigFile.loadDict()) == remote["url"])
        ...
        storedTokenIsReference: Bool) async throws
    {
        ...
        try await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            ...
            let persisted = CarapaceConfigFile.loadDict()
    ...
    func `rejected primary replacement preserves the canonical connection without a rollback write`() async throws {
        ...
        try await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            ...
            let original = CarapaceConfigFile.loadDict()
            var saveAttempts = 0
            ...
            #expect(NSDictionary(dictionary: CarapaceConfigFile.loadDict()).isEqual(to: original))
    ...
    func `external token edit after successful sync is not reverted`() async {
        ...
        await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            ...
            let persistedRemote = (CarapaceConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
        ...
        await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configPath]) {
            ...
            let state = AppState(
                ...
                gatewayConfigSaver: { root, _ in
                    rejectSaves ? false : CarapaceConfigFile.saveDict(root)
                })
            ...
            state._testEnableGatewayConfigSync()
            #expect(!state.syncGatewayConfigNow())
            var persistedRemote = (CarapaceConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
            #expect(persistedRemote?["token"] as? String == "external-token")
            #expect(state.remoteToken == "app-token")
            ...
            #expect(state.keepGatewayConfigEdits())
            persistedRemote = (CarapaceConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
