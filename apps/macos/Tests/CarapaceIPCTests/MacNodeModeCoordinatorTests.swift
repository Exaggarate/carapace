...
private actor CoordinatorFailingStartWorkerProbe: MacNodeHostWorking {
    ...
    static let failureDiagnostic = "[carapace] state database uses newer schema version 10"
...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `terminal worker failure is reported instead of scheduling another restart`() async throws {
        ...
        try coordinator.prepareNodeHostWorkerRetryForTesting(
            command: ["/usr/local/bin/carapace", "node", "worker"])
        await confirmation("terminal worker failure") { confirmed in
            let observer = notificationCenter.addObserver(
                forName: .carapaceNodeHostWorkerRetryExhausted,
                object: coordinator,
                queue: nil)
            { notification in
    ...
    @Test @MainActor func `worker cannot restart before its crash backoff expires`() async throws {
        ...
        let command = ["/usr/local/bin/carapace", "node", "worker"]
    ...
    @Test func `worker stderr never becomes part of the operator status headline`() throws {
        let diagnostic = "[carapace] bootstrap failed: state database uses a newer schema"
    ...
    @Test func `operator diagnostics preserve complete lines within their display budget`() throws {
        let inputLines = (0..<10).map { index in
            "[carapace] line \(index): " + String(repeating: "bootstrap failure details ", count: 5)
        }
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(CarapaceCapability.browser.rawValue))
        #expect(!commands.contains(CarapaceBrowserCommand.proxy.rawValue))
        ...
        #expect(!commands.contains(CarapaceFileSystemCommand.listDir.rawValue))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(CarapaceConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(CarapaceConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(CarapaceConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!CarapaceConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(CarapaceCapability.computer.rawValue))
        #expect(enabledCommands.contains(CarapaceComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(CarapaceCapability.computer.rawValue))
        #expect(!disabledCommands.contains(CarapaceComputerCommand.act.rawValue))
    }
    ...
    @Test func `camera cap gates capture and PTZ commands`() {
        let enabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let enabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: enabledCaps)
        #expect(enabledCommands.contains(CarapaceCameraCommand.list.rawValue))
        #expect(enabledCommands.contains(CarapaceCameraCommand.ptzStatus.rawValue))
        #expect(enabledCommands.contains(CarapaceCameraCommand.ptzControl.rawValue))
        ...
        let disabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let disabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: disabledCaps)
        #expect(!disabledCommands.contains(CarapaceCameraCommand.ptzStatus.rawValue))
        #expect(!disabledCommands.contains(CarapaceCameraCommand.ptzControl.rawValue))
    }
