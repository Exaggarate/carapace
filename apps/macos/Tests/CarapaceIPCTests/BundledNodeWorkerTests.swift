...
struct BundledNodeWorkerTests {
    private func makeBundle(at root: URL, builtAt: String, command: String) throws -> URL {
        let info: [String: Any] = [
            "CFBundleIdentifier": "ai.carapace.mac.debug",
            "CFBundleExecutable": "Carapace",
            "CFBundlePackageType": "APPL",
            "CFBundleShortVersionString": "2026.8.1",
            "CFBundleVersion": "1",
            "CarapaceGitCommit": String(repeating: "a", count: 40),
            "CarapaceBuildTimestamp": builtAt,
            "CarapaceWorkerBuildID": builtAt,
        ]
        ...
        let dist = runtime.appendingPathComponent("lib/node_modules/carapace/dist")
    ...
    @Test func `dirty same-SHA rebuild selects relocated worker over accepted external CLI`() async throws {
        ...
        let external = root.appendingPathComponent("external/carapace")
        ...
        for (index, command) in ["worker.before", "worker.dirty"].enumerated() {
            ...
            let app = source.appendingPathComponent("Carapace.app")
            _ = try self.makeBundle(at: app, builtAt: "2026-08-27T00:00:0\(index).000Z", command: command)
            let relocated = root.appendingPathComponent("relocated-\(index)/Carapace.app")
            try FileManager.default.createDirectory(
    ...
    func `incomplete payload never falls back to development source`(failure: String) async throws {
        let root = try makeTempDirForTests()
        defer { try? FileManager.default.removeItem(at: root) }
        let app = root.appendingPathComponent("Carapace.app")
