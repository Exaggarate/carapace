...
struct OnboardingDashboardHandoffTests {
        ...
        receiptDuringActivation: Bool) async throws
    {
        ...
        #expect(await methods.snapshot().filter { $0 != "health" } == [
            "agents.list", "agents.list", "carapace.setup.detect",
        ])
        ...
        #expect(await methods.snapshot().filter { $0 != "health" } == [
            "agents.list", "agents.list", "carapace.setup.detect", "carapace.setup.activate",
        ])
        await gateway.shutdown()
    }

    @Test func `relaunch with pending inference resumes Carapace`() async throws {
    ...
    @Test func `superseded resume cannot finish a replacement route handoff`() async throws {
        ...
        let session = GatewayTestWebSocketSession(taskFactory: {
            GatewayTestWebSocketTask(sendHook: { task, message, sendIndex in
                ...
                await methods.record(method)
                if respondToOnboardingHealth(task: task, id: id, method: method) { return }
                guard method == "carapace.setup.verify" else { return }
                await gate.wait()
                task.emitReceiveSuccess(.data(verifiedInferenceResponse(id: id)))
            })
        })
        ...
        for _ in 0..<200 {
            if await methods.snapshot() == ["health", "carapace.setup.verify"] {
                break
            }
            try? await Task.sleep(nanoseconds: 5_000_000)
        }
        ...
        #expect(await methods.snapshot() == ["health", "carapace.setup.verify"])
    }
