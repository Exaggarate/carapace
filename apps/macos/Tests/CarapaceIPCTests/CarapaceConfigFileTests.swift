...
@Suite(.serialized)
struct CarapaceConfigFileTests {
    private func makeConfigOverridePath() -> String {
        ...
        return temporaryRoot
            .appendingPathComponent("carapace-config-\(UUID().uuidString)")
            .appendingPathComponent("carapace.json")
            .path
    }
    ...
    func `fresh config defaults native discovery off and preserves explicit choice`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            "CARAPACE_CONFIG_PATH": override,
            "CARAPACE_STATE_DIR": directory.path,
        ]) {
            #expect(CarapaceConfigFile.saveDict([
    ...
    func `dangling config link is not initialized as a fresh file`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            "CARAPACE_CONFIG_PATH": override,
            "CARAPACE_STATE_DIR": url.deletingLastPathComponent().path,
        ]) {
            #expect(!CarapaceConfigFile.saveDict(["browser": ["enabled": false]]))
    ...
    func `existing unversioned config keeps omitted discovery preferences`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(CarapaceConfigFile.saveDict(["browser": ["enabled": false]]))
            #expect(CarapaceConfigFile.loadDict()["plugins"] == nil)
        }
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": override]) {
            #expect(CarapaceConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": override]) {
            #expect(CarapaceConfigFile.browserControlEnabled() == true)
            CarapaceConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(CarapaceConfigFile.browserControlEnabled() == false)
            CarapaceConfigFile.setBrowserControlEnabled(true)
            #expect(CarapaceConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": override]) {
            ...
            #expect(CarapaceConfigFile.remoteGatewayPort() == 19999)
            #expect(CarapaceConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(CarapaceConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(CarapaceConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(CarapaceConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(CarapaceConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-state-\(UUID().uuidString)", isDirectory: true)
            .path
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(CarapaceConfigFile.stateDirURL().path == dir)
            #expect(CarapaceConfigFile.url().path == "\(dir)/carapace.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("carapace.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "CARAPACE_STATE_DIR": stateDir.path,
            "CARAPACE_CONFIG_PATH": configPath.path,
        ]) {
            CarapaceConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-carapace-config-file")
    ...
    func `save dict removes retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": configPath]) {
            #expect(CarapaceConfigFile.saveDict([
                "gateway": ["mode": "local"],
    ...
    func `gateway start migration repairs existing retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["CARAPACE_CONFIG_PATH": configPath]) {
            #expect(CarapaceConfigFile.migrateRetiredAppMetadataForGatewayStart())
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async {
        ...
        await TestIsolation.withEnvValues([
            ...
            let root = CarapaceConfigFile.loadDict()
            ...
            let allowedRoot = CarapaceConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try CarapaceConfigFile.withTestingFileLock {
                let loaded = CarapaceConfigFile.loadDict()
    ...
    func `load dict skips unchanged forensic fingerprints`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try CarapaceConfigFile.withTestingFileLock {
                let before = CarapaceConfigFile.testingConfigObservationCount()
                _ = CarapaceConfigFile.loadDict()
                let afterFirstRead = CarapaceConfigFile.testingConfigObservationCount()
                _ = CarapaceConfigFile.loadDict()
                let afterUnchangedRead = CarapaceConfigFile.testingConfigObservationCount()
                ...
                _ = CarapaceConfigFile.loadDict()
                let afterMetadataChange = CarapaceConfigFile.testingConfigObservationCount()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = CarapaceConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
    ...
    func `save dict requires explicit allowance for primary clear and keeps other writers guarded`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let replacement = try selection.replacingRoot(CarapaceConfigFile.loadDict(), effectiveLocalPort: 18789)
            #expect(CarapaceConfigFile.saveDict(replacement.root, allowGatewayModeRemoval: selection.isClear))
            let cleared = CarapaceConfigFile.loadDict()
            ...
            #expect(CarapaceConfigFile.saveDict(direct.root))
            #expect(GatewayRemoteConfig
                .resolveUrlString(root: CarapaceConfigFile.loadDict()) == "wss://gateway.example/")
            #expect(!CarapaceConfigFile.saveDict(replacement.root))
        }
    }
}
