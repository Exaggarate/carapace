...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `named profiles do not inherit the default development checkout`() throws {
        ...
        let checkout = home.appendingPathComponent("Projects/carapace")
        try FileManager.default.createDirectory(at: checkout, withIntermediateDirectories: true)
        let (suiteName, defaults) = self.makeDefaults()
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let defaultProfile = AppProfile(environment: [:])
        let namedProfile = AppProfile(environment: ["CARAPACE_PROFILE": "isolated"])
        ...
        #expect(CommandResolver.projectRoot(
            ...
            homeDirectory: home).path == home.appendingPathComponent(".carapace-isolated").path)

        defaults.set(checkout.path, forKey: "carapace.gatewayProjectRootPath")
        #expect(CommandResolver.projectRoot(
            defaults: defaults,
            profile: namedProfile,
            homeDirectory: home).path == checkout.path)
    ...
    @Test func `prefers open claw binary`() async throws {
        ...
        let carapacePath = tmp.appendingPathComponent("node_modules/.bin/carapace")
        try makeExecutableForTests(at: carapacePath)

        let searchPaths = [tmp.appendingPathComponent("node_modules/.bin").path]
        let cmd = await CommandResolver.carapaceCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([carapacePath.path, "gateway"]))
    }
    ...
    @Test func `source checkout worker uses the freshness aware runner`() async throws {
        ...
        let sourceEntrypoint = tmp.appendingPathComponent("carapace.mjs")
        let distEntrypoint = tmp.appendingPathComponent("dist/entry.js")
        let projectExecutable = tmp.appendingPathComponent("node_modules/.bin/carapace")
    ...
    @Test func `falls back to node and script`() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/carapace.js")
    ...
    @Test func `prefers open claw binary over pnpm`() async throws {
        ...
        let carapacePath = binDir.appendingPathComponent("carapace")
        ...
        #expect(cmd.prefix(2).elementsEqual([carapacePath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "carapace", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "carapace", "health", "--json"]))
    ...
    @Test func `missing CLI explains install and source checkout paths`() async throws {
        ...
        #expect(cmd.last?.contains("carapace CLI not found") == true)
        #expect(cmd.last?.contains("Install the CLI") == true)
        #expect(cmd.last?.contains("run pnpm build in an Carapace source checkout") == true)
    }
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        let home = try makeTempDirForTests()
        defer { try? FileManager.default.removeItem(at: home) }
        let managedBin = home.appendingPathComponent(".carapace/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("carapace")
    ...
    @Test func `managed paths follow the app profile`() throws {
        ...
        let hostBase = home.appendingPathComponent(".carapace")
        let profileBase = home.appendingPathComponent(".carapace-onboardtest")
        for base in [hostBase, profileBase] {
            try makeExecutableForTests(at: base.appendingPathComponent("bin/carapace"))
            try FileManager().createDirectory(
        ...
        let cases = [
            ...
            (AppProfile(environment: ["CARAPACE_PROFILE": "onboardtest"]), profileBase, hostBase),
        ]
        ...
        let namedProfile = AppProfile(environment: ["CARAPACE_PROFILE": "onboardtest"])
        let staleCases = [
        ...
        for (profile, staleBase) in staleCases {
            ...
            let paths = CommandResolver.preferredPaths(
                ...
                projectRoot: home,
                validatedExecutable: staleBase.appendingPathComponent("bin/carapace").path,
                profile: profile)
            #expect(!paths.contains(staleBase.appendingPathComponent("bin").path))
            #expect(paths.contains("/usr/bin"))
        }

        // ~/.carapace2 is a lookalike, not the managed profile namespace; validation must survive.
        for external in ["custom/bin/carapace", ".carapace2/bin/carapace"] {
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        let executable = root.appendingPathComponent("carapace")
        ...
        #expect(CommandResolver.validatedCarapaceExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() async {
        ...
        defaults.set("carapace@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/carapace", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "carapace@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/carapace'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("carapace"))
    ...
    @Test func `remote SSH with explicit blank target fails closed`() async throws {
        ...
        let tmp = try makeTempDirForTests()
        defer { try? FileManager.default.removeItem(at: tmp) }
        let localCarapace = tmp.appendingPathComponent("node_modules/.bin/carapace")
        try makeExecutableForTests(at: localCarapace)
    ...
    @Test func `direct remote route uses local CLI despite stale SSH defaults`() async throws {
        ...
        #expect(command == [localCarapace.path, "status"])
    }
    ...
    @Test func `config root local overrides remote defaults`() async throws {
        ...
        #expect(cmd.first == carapacePath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
