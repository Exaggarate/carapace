...
struct MacNodeRuntimeTests {
    ...
    actor CanvasRefreshProbe {
        ...
        func refresh() -> String? {
            self.calls += 1
            return "http://127.0.0.1:18789/__carapace__/cap/refreshed-token"
        }
    }

    actor CanvasReconnectProbe {
        private var surfaceURL = "http://127.0.0.1:18789/__carapace__/cap/old-token"
        ...
        func reconnectDuringRefresh() -> String? {
            self.surfaceURL = "http://127.0.0.1:18789/__carapace__/cap/new-token"
            return nil
        }
    }
    ...
    final class MainActorServicesProbe: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        typealias SnapshotInspection = (Int?, Int?, Double?, CarapaceScreenSnapshotFormat?) -> Void
        ...
        var receivedParams: CarapaceComputerActParams?
        var actError: Error?
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: CarapaceScreenSnapshotFormat?) async throws -> ScreenSnapshotResult
        {
            ...
            outPath _: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("carapace-test-screen-record-\(UUID().uuidString).mp4")
        ...
        func performComputerAct(
            _ params: CarapaceComputerActParams,
            lifecycleGeneration: UInt64) async throws -> CarapaceComputerActResult
        {
            ...
            return CarapaceComputerActResult(ok: true)
        }
    ...
    @Test func `hosted Canvas commands refresh capability and preserve target components`() async throws {
        ...
        let resolver = MacNodeCanvasHostedSurfaceResolver(
            currentSurfaceURL: { "http://127.0.0.1:18789/__carapace__/cap/current-token" },
            refreshSurfaceURL: { _ in await probe.refresh() })

        let resolved = try await resolver.resolveTarget(
            "/__carapace__/canvas/demo%20page.html?mode=proof#result")
        #expect(resolved?.absoluteString ==
            "http://127.0.0.1:18789/__carapace__/cap/refreshed-token/__carapace__/canvas/demo%20page.html?mode=proof#result")
    ...
    @Test func `hosted Canvas commands use replacement route after refresh fails`() async throws {
        ...
        let resolved = try await resolver.resolveTarget("/__carapace__/canvas/demo.html")

        #expect(resolved?.absoluteString ==
            "http://127.0.0.1:18789/__carapace__/cap/new-token/__carapace__/canvas/demo.html")
    }
    ...
    @Test func `Canvas commands reject arbitrary web targets`() async {
        await TestIsolation.withUserDefaultsValues([canvasEnabledKey: true]) {
            let response = await self.invoke(
                MacNodeRuntime(),
                "req-canvas-external",
                CarapaceCanvasCommand.navigate.rawValue,
    ...
    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = CarapaceSystemNotifyParams(title: "", body: "")
        let response = try await invoke(
            runtime, "req-3", CarapaceSystemCommand.notify.rawValue, params: params)
        #expect(response.ok == false)
    }
    ...
    @Test @MainActor func `cancelled notification cannot present an overlay`() async throws {
        ...
        let runtime = MacNodeRuntime()
        let params = CarapaceSystemNotifyParams(
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await self.invoke(runtime, "req-4", CarapaceCameraCommand.list.rawValue)
            #expect(response.ok == false)
    ...
    @Test func `handle location invoke applies authorization required by mode`() async throws {
        ...
        let cases: [(mode: CarapaceLocationMode, status: CLAuthorizationStatus, accepted: Bool)] = [
        ...
        for testCase in cases {
            await TestIsolation.withUserDefaultsValues([locationModeKey: testCase.mode.rawValue]) {
                ...
                let response = await self.invoke(
                    runtime, "req-location", CarapaceLocationCommand.get.rawValue)
    ...
    @Test func `handle invoke rejects computer act when control disabled`() async throws {
        ...
        let params = CarapaceComputerActParams(action: .leftClick, x: 5, y: 6, refWidth: 1280)
        let response = try await invoke(
            runtime, "req-computer-disabled", CarapaceComputerCommand.act.rawValue, params: params)
    ...
    @Test func `handle invoke routes computer act to the injected services when enabled`() async throws {
        ...
        let params = CarapaceComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
        let response = try await invoke(
            runtime, "req-computer-ok", CarapaceComputerCommand.act.rawValue, params: params)
        ...
        let payloadJSON = try #require(response.payloadJSON)
        let result = try JSONDecoder().decode(CarapaceComputerActResult.self, from: Data(payloadJSON.utf8))
    ...
    @Test func `provider selection owns both snapshot and action without cross-provider fallback`() async throws {
        let commands: Set<String> = [MacNodeScreenCommand.snapshot.rawValue, CarapaceComputerCommand.act.rawValue]
        ...
        let action = CarapaceComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
        ...
        #expect(try await (self.invoke(
            cuaRuntime,
            "cua-action",
            CarapaceComputerCommand.act.rawValue,
            params: action)).ok)
    ...
    @Test func `concurrent invokes share one main actor services initialization`() async throws {
        ...
        let first = Task {
            await self.invoke(runtime, "req-computer-single-flight-1", CarapaceComputerCommand.act.rawValue, json)
        }
        try #require(await waitForCount(1, counter: factoryCalls))
        let second = Task {
            await self.invoke(runtime, "req-computer-single-flight-2", CarapaceComputerCommand.act.rawValue, json)
        }
    ...
    @Test func `lifecycle release invalidates first invoke awaiting service initialization`() async throws {
        ...
        let runtime = MacNodeRuntime(
            ...
            computerControlEnabled: { true })
        let params = CarapaceComputerActParams(action: .leftMouseDown, x: 12, y: 34, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let invoke = Task {
            await self.invoke(runtime, "req-computer-release-during-init", CarapaceComputerCommand.act.rawValue, json)
        }
        try #require(await waitForCount(1, counter: factoryCalls))
    ...
    @Test func `handle invoke maps accessibility denial to unavailable`() async throws {
        ...
        let params = CarapaceComputerActParams(action: .leftClick, x: 1, y: 1, refWidth: 1280)
        let response = try await invoke(
            runtime, "req-computer-ax", CarapaceComputerCommand.act.rawValue, params: params)

        #expect(response.ok == false)
        #expect(response.error?.code == .unavailable)
        #expect(response.error?.message == "ACCESSIBILITY_REQUIRED: grant Accessibility permission to Carapace")
    }
    ...
    @Test func `handle invoke rejects malformed computer act params`() async {
        ...
        let response = await invoke(
            runtime, "req-computer-bad", CarapaceComputerCommand.act.rawValue, #"{"action":"#)
