...
struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: CarapaceProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: CarapaceProtocol.AnyCodable(["triggers": [
            "carapace",
            "computer",
        ]]))
        ...
        #expect(updated == ["carapace", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: CarapaceProtocol.AnyCodable(["unexpected": 123]))
