...
struct CanvasWindowSmokeTests {
    @Test func `panel controller shows and hides`() throws {
        let root = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-canvas-test-\(UUID().uuidString)")
    ...
    @Test func `hosted Canvas URL resolver keeps capability scope`() throws {
        let surface = "https://gateway.example/root/__carapace__/cap/token%20value"
        let canvas = try #require(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/demo%20page.html?mode=proof#result"))
        #expect(canvas.absoluteString ==
            "https://gateway.example/root/__carapace__/cap/token%20value/__carapace__/canvas/demo%20page.html?mode=proof#result")
        ...
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/../other/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/%252e%252e/other/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/%25252525252e%25252525252e/other/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: "https://gateway.example/not-capability-scoped",
            target: "/__carapace__/canvas/") == nil)
    }
    ...
    @Test func `Canvas target resolver accepts only hosted paths and app-local URLs`() {
        #expect(CanvasHostedURLResolver.isHostedTarget(
            "/__carapace__/canvas/documents/widget/index.html"))
        #expect(CanvasHostedURLResolver.isAppLocalTarget("carapace-canvas://main/widget/index.html"))
        ...
        #expect(!CanvasHostedURLResolver.isAppLocalTarget("carapace-canvas:///widget.html"))
        #expect(!CanvasHostedURLResolver.isAppLocalTarget("carapace-canvas://main/%252e%252e/secret"))
    }
}
