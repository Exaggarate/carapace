import Foundation
import Testing
@testable import CarapaceMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "carapace.connectionMode") == "remote")
                #expect(defaults.string(forKey: "carapace.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "carapace.onboardingSeen") == true)
                #expect(defaults.string(forKey: "carapace.remoteCliPath") == "/opt/homebrew/bin/carapace")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote clears host defaults only when target changes`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-retarget-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configURL.path]) {
            try configureRemote(
                .init(
                    ...
                    cliPath: "/opt/first/carapace"),
                defaultsSuites: [suite])
            try configureRemote(.init(sshTarget: "alice@first.example"), defaultsSuites: [suite])
            #expect(defaults.string(forKey: "carapace.remoteIdentity") == "/tmp/first-identity")
            #expect(defaults.string(forKey: "carapace.remoteProjectRoot") == "/srv/first")
            #expect(defaults.string(forKey: "carapace.remoteCliPath") == "/opt/first/carapace")

            try configureRemote(
                .init(sshTarget: "alice@second.example", cliPath: "/opt/second/carapace"),
                defaultsSuites: [suite])
            #expect(defaults.string(forKey: "carapace.remoteIdentity") == nil)
            #expect(defaults.string(forKey: "carapace.remoteProjectRoot") == nil)
            #expect(defaults.string(forKey: "carapace.remoteCliPath") == "/opt/second/carapace")

            try configureRemote(.init(directUrl: "wss://third.example"), defaultsSuites: [suite])
            #expect(defaults.string(forKey: "carapace.remoteTarget") == nil)
            #expect(defaults.string(forKey: "carapace.remoteIdentity") == nil)
            #expect(defaults.string(forKey: "carapace.remoteProjectRoot") == nil)
            #expect(defaults.string(forKey: "carapace.remoteCliPath") == nil)

            try configureRemote(
                .init(directUrl: "wss://third.example", projectRoot: "/srv/third"), defaultsSuites: [suite])
            try configureRemote(.init(directUrl: "wss://third.example"), defaultsSuites: [suite])
            #expect(defaults.string(forKey: "carapace.remoteProjectRoot") == "/srv/third")
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote writes state dir config when config path is unset`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-state-\(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("carapace.json")
        ...
        try await TestIsolation.withIsolatedState(env: [
            ...
            try #require(resolveCarapaceConfigURL().path == configURL.path)
            let output = try configureRemote(
                .init(
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["CARAPACE_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
...
struct GatewayConfigTests {
    @Test @MainActor func `config path wins when both config and state dir are set`() async throws {
        let rootDir = FileManager().temporaryDirectory
            .appendingPathComponent("carapace-cli-config-precedence-\(UUID().uuidString)", isDirectory: true)
        let explicitConfigURL = rootDir.appendingPathComponent("explicit.json")
        let stateConfigURL = rootDir.appendingPathComponent("state/carapace.json")
        ...
        await TestIsolation.withEnvValues([
            "CARAPACE_CONFIG_PATH": explicitConfigURL.path,
            "CARAPACE_STATE_DIR": stateConfigURL.deletingLastPathComponent().path,
        ]) {
    ...
    @Test @MainActor func `config path trims surrounding whitespace and expands tilde`() async {
        let relativePath = ".carapace-cli-config-\(UUID().uuidString)/carapace.json"

        await TestIsolation.withEnvValues([
            "CARAPACE_CONFIG_PATH": "  ~/\(relativePath)\n",
            "CARAPACE_STATE_DIR": "/tmp/carapace-unused-state-dir",
        ]) {
            let expectedURL = FileManager().homeDirectoryForCurrentUser.appendingPathComponent(relativePath)
            #expect(resolveCarapaceConfigURL().path == expectedURL.path)
        }
    }

    @Test @MainActor func `state dir trims surrounding whitespace for connect and wizard`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("carapace cli state profile \(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("carapace.json")
        defer { try? FileManager().removeItem(at: stateDir) }
