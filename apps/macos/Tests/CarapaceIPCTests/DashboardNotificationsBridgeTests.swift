...
struct DashboardNotificationsBridgeTests {
    ...
    @Test func `notification navigation keeps the originating Gateway and mount after auth refresh`() throws {
        var components = try #require(URLComponents(string: "https://gateway.example/carapace/"))
        ...
        let refreshed = try #require(URL(string: "https://gateway.example/carapace/#token=after"))
        let otherGateway = try #require(URL(string: "https://other.example/carapace/"))
        ...
        #expect(route.absoluteString == "https://gateway.example/carapace/")
