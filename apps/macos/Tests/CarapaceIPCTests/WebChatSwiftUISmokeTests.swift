...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: CarapaceChatTransport {
        func requestHistory(sessionKey: String) async throws -> CarapaceChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [CarapaceChatAttachmentPayload]) async throws -> CarapaceChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(CarapaceChatSendResponse.self, from: Data(json.utf8))
        }
    ...
    @Test func `window controller merges titlebar and keeps toolbar controls`() throws {
        ...
        #expect(window.title == "Studio — Carapace")
        window.title = "main"
        #expect(window.title == "Studio — Carapace")
    ...
    @Test func `max and Ultra thinking preferences survive reopen`() throws {
        ...
        for level in ["max", "ultra"] {
            defaults.set(level, forKey: "carapace.webchat.thinkingLevel")
    ...
    @Test func `verbosity preference survives reopen`() throws {
        ...
        defaults.set("full", forKey: "carapace.webchat.verboseLevel")
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == "full")
        defaults.set("invalid", forKey: "carapace.webchat.verboseLevel")
    ...
    @Test func `inherited verbosity preference clears persisted override`() throws {
        ...
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == nil)
        #expect(defaults.object(forKey: "carapace.webchat.verboseLevel") == nil)
    }
}
