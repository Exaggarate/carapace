...
struct WebChatWindowLifetimeTests {
    ...
    @Test func `admitting a replacement primary Gateway retires only its native window`() async throws {
        try await withIsolatedWebChatProfile { manager, profile in
            ...
            let originalGatewayID = GatewayDiscoveryPreferences.deviceAuthGatewayID(root: CarapaceConfigFile.loadDict())
...
private final class ClosingWindowChatTransport: @unchecked Sendable, CarapaceChatTransport {
    ...
    func requestHistory(sessionKey: String) async throws -> CarapaceChatHistoryPayload {
        ...
        return try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: Data("""
...
private actor ClosingWindowTranscriptCache: CarapaceChatTranscriptCache {
    private(set) var savedTranscripts: [[CarapaceChatMessage]] = []
    ...
    func storeSessions(_: [CarapaceChatSessionEntry]) async {}
