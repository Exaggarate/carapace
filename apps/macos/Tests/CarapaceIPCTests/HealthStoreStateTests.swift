...
struct HealthStoreStateTests {
    @Test(.enabled(if: ProcessInfo.processInfo.environment["CARAPACE_TEST_HEALTH_RENDER_DIR"] != nil))
    @MainActor func `health settings render`() async throws {
        let environment = ProcessInfo.processInfo.environment
        let output = try URL(fileURLWithPath: #require(environment["CARAPACE_TEST_HEALTH_RENDER_DIR"]))
        ...
        try #require(CarapacePaths.stateDirURL == home.appendingPathComponent("state", isDirectory: true))
        try #require(CarapaceConfigFile.loadDict().isEmpty)
        let canary = try #require(environment["CARAPACE_TEST_HEALTH_DENIED_FILE"])
    ...
    @Test @MainActor func `fallback channel with last error is not healthy`() throws {
        try Self.withChannel([
            ...
            #expect(store.summaryLine == "Not linked — run carapace login")
        }
    }
    ...
    @MainActor func `enabled and unspecified channels remain eligible`(enabled: Bool?) throws {
        ...
        try Self.withSnapshot([
            ...
            #expect(store.summaryLine == "Telegram ok · Not linked — run carapace login")
        }
    }
