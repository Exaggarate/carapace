...
struct StatusMenuWidthTests {
    ...
    private static let workerDiagnostic = [
        "[carapace] bootstrap failed: state database uses a newer schema version than this installed CLI",
        "[carapace] run carapace doctor to inspect the current installation and its persisted local state",
        "[carapace] check that the application and worker are using the same installed Carapace release",
    ].joined(separator: "\n")
    ...
    @Test func `long native and hosted content cannot expand the status menu`() throws {
        ...
        let approval = ExecApprovalQueueItem(
            ...
            request: ExecApprovalPromptRequest(
                command: "carapace doctor --fix --verbose --check-every-registered-capability-host",
