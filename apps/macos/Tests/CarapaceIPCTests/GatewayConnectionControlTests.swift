...
func makeGatewayGenerationSnapshot(version: String, mainSessionKey: String? = nil) -> HelloOk {
    HelloOk(
        type: "hello-ok",
        _protocol: 3,
        server: ["version": CarapaceProtocol.AnyCodable(version)],
        features: [:],
        snapshot: Snapshot(
            presence: [],
            health: [String: CarapaceProtocol.AnyCodable](),
            stateversion: StateVersion(presence: 0, health: 0),
            uptimems: 0,
            configpath: nil,
            statedir: nil,
            sessiondefaults: mainSessionKey.map { ["mainSessionKey": CarapaceProtocol.AnyCodable($0)] },
            authmode: nil,
            updateavailable: nil),
        controluitabs: nil,
...
@Suite(.serialized) struct GatewayConnectionControlTests {
    ...
    @Test func `operator widget capability refresh is shared and retained`() async throws {
        let rawOldSurface = "http://127.0.0.1:18789/__carapace__/cap/old-token"
        let rawNewSurface = "http://127.0.0.1:18789/__carapace__/cap/new-token"
        let oldSurface = "https://gateway.example.invalid:9443/__carapace__/cap/old-token"
        let newSurface = "https://gateway.example.invalid:9443/__carapace__/cap/new-token"
        let recorder = WebSocketMessageRecorder()
        let session = GatewayTestWebSocketSession(taskFactory: {
            GatewayTestWebSocketTask(
                sendHook: { task, message, sendIndex in
...
extension GatewayConnectionControlTests {
    ...
    @Test func `routing identity decodes agent and contract from one response`() throws {
        ...
        let identity = try CarapaceChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
        ...
        operation: (GatewayConnection, GatewayTestWebSocketSession) async throws -> T) async throws -> T
    {
        let isolatedState = FileManager.default.temporaryDirectory
            .appendingPathComponent("carapace-gateway-recovery-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: isolatedState, withIntermediateDirectories: true)
        let configURL = isolatedState.appendingPathComponent("carapace.json")
        ...
        return try await TestIsolation.withEnvValues([
            "CARAPACE_CONFIG_PATH": configURL.path,
            "CARAPACE_STATE_DIR": isolatedState.path,
        ]) {
