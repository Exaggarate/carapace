...
struct ApplicationRelocatorTests {
    ...
    private let current = ApplicationRelocator.ApplicationIdentity(
        bundleIdentifier: "ai.carapace.mac",
        buildVersion: "100"
    )
    ...
    func `stable application locations continue normally`() {
        let paths = [
            ...
            "/Users/tester/Applications/Carapace.app",
            "/Users/tester/Tools/Carapace.app",
            "/Volumes/External/Apps/Carapace.app",
        ]
        for path in paths {
    ...
    func `debug and test builds never relocate`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Carapace.app",
                debugOrTesting: true
            )
        )
    ...
    func `transient copy offers system installation when available`() {
        let destination = URL(fileURLWithPath: "/Applications/Carapace.app")
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Carapace.app",
    ...
    func `read only mounted copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Volumes/Carapace/Carapace.app",
                candidates: [missing(destination, writable: true)],
                readOnlyVolume: true
            )
        )
    ...
    func `translocated copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/private/var/folders/x/AppTranslocation/y/d/Carapace.app",
                candidates: [missing(destination, writable: true)]
            )
        )
    ...
    func `relaunch marker stops a repeated transient handoff`() {
        let destination = URL(fileURLWithPath: "/Applications/Carapace.app")
        let markedArguments = [
            "/private/var/folders/x/AppTranslocation/y/d/Carapace.app/Contents/MacOS/Carapace",
    ...
    func `different same named app is never replaced`() {
        let systemDestination = URL(fileURLWithPath: "/Applications/Carapace.app")
        let userDestination = home.appendingPathComponent("Applications/Carapace.app")
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Desktop/Carapace.app",
                candidates: [
    ...
    func `unwritable destinations require manual installation`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Carapace.app",
                candidates: [
                    missing(URL(fileURLWithPath: "/Applications/Carapace.app"), writable: false),
                    missing(home.appendingPathComponent("Applications/Carapace.app"), writable: false),
                ]
            )
        )
    ...
    func `unauthenticated replacement marker cannot bypass duplicate launch rejection`() {
        let forgedHandoff = [
            "CARAPACE_REPLACEMENT_SOURCE_BUNDLE": "/Applications/Carapace.app",
            "CARAPACE_REPLACEMENT_PARENT_PID": "1",
            "CARAPACE_REPLACEMENT_CODE_HASH": "ZmFrZQ==",
            "CARAPACE_REPLACEMENT_READY_FD": "19",
        ]
        ...
        #expect(!ApplicationRelocator.acceptReplacementHandoff(environment: [
            "CARAPACE_REPLACEMENT_SOURCE_BUNDLE": "relative/Carapace.app",
        ]))
    }
    ...
    func `replacement requirement preserves Developer ID certificate class`() {
        ...
        #expect(ApplicationRelocator.developerIDRequirementString(
            bundleIdentifier: "ai.carapace.mac\" or true",
            teamIdentifier: "Y5PE65HELJ"
        ) == nil)
    }
    ...
    func `bundle file reference stays bound after canonical path replacement`() throws {
        ...
        let canonicalBundle = root.appendingPathComponent("Carapace.app", isDirectory: true)
        let archivedBundle = root.appendingPathComponent("Carapace.previous.app", isDirectory: true)
        let executableRelativePath = "Contents/MacOS/Carapace"
    ...
    func `replacement handoff launches the bound executable without filesystem locks`() {
        let reference = ApplicationRelocator.BundleFileReference(
            deviceIdentifier: 42,
            fileIdentifier: 99,
            executableRelativePath: "Contents/MacOS/Carapace"
        )

        #expect(reference.bundleURL.path == "/.vol/42/99")
        #expect(reference.executableURL.path == "/.vol/42/99/Contents/MacOS/Carapace")
    }
    ...
    func `verified KeepAlive launchd supervisor retains relaunch ownership`() throws {
        ...
        let executable = root.appendingPathComponent("Carapace.app/Contents/MacOS/Carapace")
        let serviceName = "ai.carapace.mac.test-node"
        ...
        #expect(ApplicationRelocator.relaunchStrategy(
            xpcServiceName: "application.ai.carapace.mac.123",
            executableURL: executable,
            homeDirectory: home
        ) == .openAfterTermination)
        #expect(ApplicationRelocator.relaunchStrategy(
    ...
    func `replacement metadata is read from disk instead of Bundle cache`() throws {
        ...
        let bundleURL = root.appendingPathComponent("Carapace.app")
        ...
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.carapace.mac",
            executableURL: contentsURL.appendingPathComponent("MacOS/Carapace")
        ))

        try writeInfoPlist(
            at: contentsURL.appendingPathComponent("Info.plist"),
            bundleIdentifier: "ai.carapace.mac.replaced",
            executable: "CarapaceNext"
        )
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.carapace.mac.replaced",
            executableURL: contentsURL.appendingPathComponent("MacOS/CarapaceNext")
        ))
    }
