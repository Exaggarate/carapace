...
private actor ApprovalGatewayRequestLog {
    ...
    func listResponse(method: String) throws -> String {
        ...
        let requests = method == "carapace.approval.list" ? self.makeListedSystemRequests() : self.makeListedRequests()
...
struct ExecApprovalQueueStoreTests {
    ...
    @Test func `system agent approvals resolve through the unified kind-aware gateway method`() async throws {
        let fixture = ApprovalGatewayFixture(advertisedMethods: ["carapace.approval.list"])
        try await fixture.withStore { store in
            store.start()
            await store.refresh()

            let request = ApprovalFixtureRequest(id: "system", allowedDecisions: ["allow-once", "deny"])
            try await fixture.sendEvent(name: "carapace.approval.requested", payload: request.json)
    ...
    func `reconnect reloads advertised system approvals with current authority`(supportsSystemList: Bool) async throws {
        let fixture = ApprovalGatewayFixture(
            ...
            advertisedMethods: supportsSystemList ? ["carapace.approval.list"] : [])
        try await fixture.withStore { store in
            store.start()
            ...
            if supportsSystemList {
                ...
                try await fixture.sendEvent(name: "carapace.approval.requested", payload: event.json)
                try #require(await self.waitUntil {
            ...
            let listsBeforeReconnect = await fixture.requestLog.requests(method: "carapace.approval.list")
            await store.refresh()
            ...
            let systemLists = await fixture.requestLog.requests(method: "carapace.approval.list")
    ...
    @Test func `a replacement hello reloads both approval kinds without reopening the menu`() async throws {
        ...
        let fixture = ApprovalGatewayFixture(
            ...
            advertisedMethods: ["carapace.approval.list"])
        try await fixture.withStore { store in
            store.start()
            await store.refresh()
