...
extension CarapaceChatViewModel {
    ...
    public func removeAttachment(_ id: CarapacePendingAttachment.ID) {
        attachments.removeAll { $0.id == id }
        applyDeferredExternalStateIfReady()
    }

    func restoreEditorAttachments(_ editorAttachments: [CarapaceChatEditorAttachment]?) {
        self.attachments = (editorAttachments ?? []).enumerated().compactMap { index, attachment in
            ...
            return CarapacePendingAttachment(
                url: nil,
                data: data,
    ...
    public func addVoiceNoteAttachment(fileURL: URL, durationSeconds: Double) async {
        ...
        let normalizedDuration = durationSeconds.isFinite
            ? min(max(0, durationSeconds), CarapaceVoiceNoteRecorder.maximumDurationSeconds)
            : 0
        attachments.append(
        ...
        expectedSession: SessionSnapshot?) async
    {
        ...
        guard self.ownsAttachmentSession(expectedSession) else { return }
        self.attachments.append(CarapacePendingAttachment(
            url: nil,
            data: data,
            fileName: fileName,
    ...
    static func previewImage(data: Data) -> CarapacePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
        ...
        fileExtension: String) async -> Data?
    {
        await Task.detached(priority: .userInitiated) {
            let fileURL = FileManager.default.temporaryDirectory
                .appendingPathComponent("carapace-upload-preview-\(UUID().uuidString)")
