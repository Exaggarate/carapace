...
extension EnvironmentValues {
    @Entry var carapaceChatDesktopLayout = false
}
...
enum CarapaceChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("CarapaceChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("CarapaceChatTheme.onboardingAssistantBubble"),
...
enum CarapacePlatformImageFactory {
    static func image(_ image: CarapacePlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
