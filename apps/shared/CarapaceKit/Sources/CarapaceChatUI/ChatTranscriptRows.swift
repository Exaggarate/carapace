...
enum ChatTranscriptRow: Hashable, Identifiable {
    ...
    case message(CarapaceChatMessage)
    ...
    init?(_ message: CarapaceChatMessage) {
    ...
    static func build(from messages: [CarapaceChatMessage]) -> [Self] {
        messages.compactMap(Self.init)
    }

    private static func savedTokens(for marker: CarapaceChatHistoryMarker) -> Double? {
