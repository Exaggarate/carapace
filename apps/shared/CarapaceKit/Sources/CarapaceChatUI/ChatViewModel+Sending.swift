...
struct SlashFilterCache {
    let query: String
    let filter: CarapaceChatCommandFilter
    let result: [CarapaceChatCommandChoice]
}

private let chatSendingLogger = Logger(subsystem: "ai.carapace", category: "CarapaceChatUI")
...
extension CarapaceChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: CarapaceChatCommandFilter) -> [CarapaceChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: CarapaceChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [CarapaceChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [CarapaceChatCommandChoice],
        ...
        filter: CarapaceChatCommandFilter) -> [CarapaceChatCommandChoice]
    {
        ...
        let effectiveFilter: CarapaceChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, CarapaceChatCommandChoice)? in
    ...
    private static func command(
        _ command: CarapaceChatCommandChoice,
        isIncludedIn filter: CarapaceChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        ...
        let replyTarget: CarapaceChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [CarapaceChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard !mustPreserveOutboxOrder else {
            ...
            errorText = reason ?? CarapaceChatTransportUpgradeMessage.routingContract
            return .stop
        }
    ...
    private static func userContent(
        messageText: String,
        attachments: [CarapacePendingAttachment],
        encodedAttachments: [CarapaceChatAttachmentPayload]) -> [CarapaceChatMessageContent]
    {
        var content: [CarapaceChatMessageContent] = [
    ...
    private func deliverLiveSend(_ attempt: LiveSendAttempt) async {
        ...
        var durableSessionSettingsExpectation: CarapaceChatSessionSettingsExpectation?
        do {
            if let settingsError = await waitForCapabilitySettingsBarrier(in: sessionKey) {
                await self.handleLiveSendFailure(
                    NSError(
                        domain: "CarapaceChatCapabilitySettings",
                        code: 1,
    ...
    private func handleLiveSendResponse(
        _ response: CarapaceChatSendResponse,
    ...
    private func handleLiveSendFailure(
        ...
        durableSessionSettingsExpectation: CarapaceChatSessionSettingsExpectation? = nil,
        canPreserveInOutbox: Bool = true) async
    {
           ...
           !(error is GatewayResponseError)
        {
            ...
            let deliveryIsAmbiguous = !(error is CarapaceChatTransportSendError)
