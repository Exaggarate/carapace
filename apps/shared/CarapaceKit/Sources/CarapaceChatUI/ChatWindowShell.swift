...
/// using `CarapaceChatView` directly.
@MainActor
public struct CarapaceChatWindowShell: View {
    public nonisolated static let assistantTraceDefaultsKey = "carapace.webchat.showAssistantTrace"
    public nonisolated static let assistantReasoningDefaultsKey = "carapace.webchat.showAssistantReasoning"
    public nonisolated static let assistantToolActivityDefaultsKey = "carapace.webchat.showAssistantToolActivity"
    ...
    private let emptyAssistantPrompts: [CarapaceChatView.StarterPrompt]
    ...
    public init(
        ...
        emptyAssistantPrompts: [CarapaceChatView.StarterPrompt] = [],
    ...
    public var body: some View {
        NavigationSplitView {
            ...
            CarapaceChatView(
                ...
                .environment(\.carapaceChatDesktopLayout, true)
    ...
    private var activeSessionEntry: CarapaceChatSessionEntry? {
    ...
    private var detailToolbar: some ToolbarContent {
        if CarapaceSessionColor(name: self.activeSessionEntry?.color) != nil {
            ToolbarItem(placement: .principal) {
                HStack(spacing: 6) {
                    CarapaceSessionColorDot(color: self.activeSessionEntry?.color)
                    Text(self.activeSessionTitle)
                        .font(CarapaceChatTypography.body.weight(.semibold))
                        .lineLimit(1)
                }
            }
        }
    ...
    private var sessionActionsMenu: some View {
        Menu {
            ...
            CarapaceSessionColorMenu(color: self.activeSessionEntry?.color) { color in
...
func chatWindowActionLabel(_ title: LocalizedStringKey, systemImage: String) -> some View {
    Label {
        Text(title)
            .font(CarapaceChatTypography.body(size: 13, weight: .regular, relativeTo: .body))
    } icon: {
