...
struct ChatMarkdownRenderer: View {
    ...
    enum Typography: Equatable {
        ...
        var proseFont: Font {
            self == .response
                ? CarapaceChatTypography.body
                : CarapaceChatTypography.callout.italic()
        }
        ...
        func headingFont(level: Int) -> Font {
            ...
            case .hierarchy: CarapaceChatTypography.heading(level: level)
    ...
    struct InlineMathTypography: Equatable {
        static let body = Self(size: CarapaceChatTypography.bodySize, relativeTo: .body)
    ...
    private var linkColor: Color {
        self.context == .user ? self.textColor : CarapaceChatTheme.accent
    }
}
...
private struct ChatMarkdownDisclosureView: View {
    ...
    var body: some View {
        ...
        .tint(self.context == .user ? self.textColor : CarapaceChatTheme.accent)
    }
}
    ...
    var body: some View {
        ForEach(self.images, id: \.id) { item in
            if let img = item.image {
                CarapacePlatformImageFactory.image(img)
                    .resizable()
                    .scaledToFit()
                    .frame(maxHeight: 260)
