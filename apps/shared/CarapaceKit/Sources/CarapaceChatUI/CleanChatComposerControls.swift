        ...
        self.platformSurface(content)
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .strokeBorder(CarapaceChatTheme.composerBorder, lineWidth: 1))
...
struct CompactChatAttachmentLabel: View {
    var body: some View {
        Image(systemName: "plus")
            .font(CarapaceChatTypography.display(size: 20, weight: .semibold, relativeTo: .body))
...
struct CleanChatContextUsageLabel: View {
    let usage: CarapaceChatContextUsage

    var body: some View {
        ZStack {
            Circle()
                .stroke(CarapaceChatTheme.muted.opacity(0.2), lineWidth: 2.5)
            Circle()
    ...
    private var tint: Color {
        guard let percent = self.usage.percentUsed else { return CarapaceChatTheme.muted }
        if percent >= 90 { return CarapaceChatTheme.danger }
        #if os(macOS)
        if percent >= 75 { return CarapaceChatTheme.warning }
        #else
        if percent >= 80 { return CarapaceChatTheme.warning }
        #endif
        return CarapaceChatTheme.success
    }
...
struct CarapaceChatAttachmentsStrip: View {
    let attachments: [CarapacePendingAttachment]
    let onRemove: @MainActor (CarapacePendingAttachment.ID) -> Void

    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(self.attachments, id: \CarapacePendingAttachment.id) { attachment in
                    HStack(spacing: 6) {
                        if let image = attachment.preview {
                            CarapacePlatformImageFactory.image(image)
                                .resizable()
                                .scaledToFill()
                            ...
                            if let duration = attachment.durationSeconds {
                                Text(carapaceVoiceNoteDurationLabel(duration))
                                    .font(CarapaceChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(CarapaceChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
...
#if !os(macOS)
struct CarapaceChatAttachmentMenu<ExtraItems: View>: View {
...
#if canImport(UIKit)
struct CarapaceChatCameraPicker: UIViewControllerRepresentable {
    ...
    final class Coordinator: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
        let parent: CarapaceChatCameraPicker

        init(parent: CarapaceChatCameraPicker) {
            self.parent = parent
        }
...
struct CarapaceChatMicButton: View {
    ...
    private var label: some View {
        ...
        return Image(systemName: showsStop ? "stop.fill" : "mic")
            .font(CarapaceChatTypography.display(size: 17, weight: .medium, relativeTo: .body))
            .foregroundStyle(showsStop ? CarapaceChatTheme.accent : .secondary)
    ...
    private func voiceNoteAction(_ voiceNoteControl: CarapaceChatVoiceNoteControl) -> some View {
        Button {
    ...
    private func isVoiceNoteRecordingEnabled(_ voiceNoteControl: CarapaceChatVoiceNoteControl) -> Bool {
...
private struct UnifiedChatMicMetadata: ViewModifier {
    let control: CarapaceChatDictationControl
    let isPending: Bool
