...
private let chatSessionActionsLogger = Logger(
    subsystem: "ai.carapace",
    category: "CarapaceChat")
...
extension CarapaceChatViewModel {
    ...
    func performStartNewSession(
        ...
        routeLease: CarapaceChatNewSessionRouteLease? = nil) async -> Bool
    {
        ...
        let currentAgentID = (
            CarapaceChatSessionKey.agentID(from: self.sessionKey) ??
                self.activeAgentId ??
                CarapaceChatSessionKey.agentID(from: self.resolvedMainSessionKey))?
            .lowercased()
    ...
    static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "CarapaceChatTransport"
    ...
    func startNewSession(
        ...
        using routeLease: CarapaceChatNewSessionRouteLease) async -> Bool
    {
        await self.performStartNewSession(
            agentID: agentID,
            worktree: worktree,
    ...
    func newSessionRouteLease() async throws -> CarapaceChatNewSessionRouteLease {
    ...
    public func fetchSessionGroups() async throws -> [CarapaceChatSessionGroup] {
        let routeLease = try await self.sessionGroupsRouteLease()
        return try await self.fetchSessionGroups(using: routeLease)
    }

    func sessionGroupsRouteLease() async throws -> CarapaceChatSessionGroupsRouteLease {
    ...
    func fetchSessionGroups(
        using routeLease: CarapaceChatSessionGroupsRouteLease) async throws -> [CarapaceChatSessionGroup]
    {
    ...
    func performSessionBatch(
        sessions selectedSessions: [CarapaceChatSessionEntry],
        action: ChatSessionBatchAction) async -> ChatSessionBatchResult
    {
        ...
        case .pin, .unpin:
            ...
            self.sessions = CarapaceChatSessionListOrganizer.organize(self.sessions)
        case .archive, .delete:
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [CarapaceChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return CarapaceChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return CarapaceChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func rewindToMessage(_ message: CarapaceChatMessage) async {
    ...
    func refreshSessionBranches(
        ...
        preBootstrapBranchState: CarapaceChatOutboxBranchState?) async -> Bool
    {
        let refreshGeneration = self.beginSessionBranchesRefresh()
        return await self.performSessionBranchesRefresh(
            for: session,
    ...
    public func forkAtMessage(_ message: CarapaceChatMessage) async {
        guard let entryID = Self.sessionMutationEntryID(for: message) else { return }
        guard self.canPerformMessageSessionAction else { return }
        guard self.canCreateSessionForImmediateSwitch() else { return }
        let initiatingSession = self.currentSessionSnapshot()
    ...
    private static func sessionMutationEntryID(for message: CarapaceChatMessage) -> String? {
    ...
    public func setSessionArchived(_ session: CarapaceChatSessionEntry, archived: Bool) {
        let key = session.key
        guard archived else {
    ...
    @discardableResult
    public func restoreSession(_ session: CarapaceChatSessionEntry) async -> Bool {
        guard let expectedSessionID = session.sessionId?
            .trimmingCharacters(in: .whitespacesAndNewlines),
            !expectedSessionID.isEmpty
        else {
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: CarapaceChatSessionEntry?) async
    {
