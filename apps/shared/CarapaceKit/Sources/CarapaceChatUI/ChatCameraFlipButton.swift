...
struct ChatCameraFlipButton: View {
    let control: CarapaceChatTalkControl
    let controlHeight: CGFloat
    let visualSize: CGFloat

    static func isAvailable(for control: CarapaceChatTalkControl) -> Bool {
    ...
    var body: some View {
        Button {
            ...
            Image(systemName: "arrow.triangle.2.circlepath.camera")
                .font(CarapaceChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                .foregroundStyle(.primary)
                .frame(width: self.visualSize, height: self.visualSize)
                .background {
                    Circle()
                        .fill(CarapaceChatTheme.accent.opacity(0.12))
                }
...
struct ChatTalkButton: View {
    ...
    private func compactButton(controlHeight: CGFloat, iconControlSize: CGFloat) -> some View {
        Button {
            ...
            ChatTalkButtonGlyph(control: self.control)
                ...
                .background {
                    Circle()
                        .fill(self.control.isEnabled ? self.fill : AnyShapeStyle(CarapaceChatTheme.accent))
    ...
    private var fill: AnyShapeStyle {
        if self.control.isEnabled {
            return AnyShapeStyle(CarapaceChatTheme.userBubble)
        }
        if !self.control.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return CarapaceChatTheme.subtleCard
    }
    ...
    private var stroke: Color {
        ...
        return CarapaceChatTheme.composerBorder
    }
...
enum ChatDictationActions {
    static func start(
        _ control: CarapaceChatDictationControl,
    ...
    static func cancel(
        task: Binding<Task<Void, Never>?>,
        control: CarapaceChatDictationControl?)
    {
        guard task.wrappedValue != nil else { return }
