...
extension CarapaceWatchMessageJournal {
    ...
    public func importLegacy(_ prepared: CarapaceWatchMessageLegacyImport, nowMs: Int64) throws {
        guard nowMs >= 0, nowMs <= Int64.max - CarapaceWatchChatDeliveryCodec.lifetimeMs else {
            throw CarapaceWatchChatDeliveryError(code: "clock_error", message: "Check the date and time on iPhone.")
        }
        var recent: [String] = []
        ...
        try self.queue.write { db in
            ...
            for message in prepared.messages {
                ...
                if let existing {
                    ...
                    if existing.phase == .needsReview {
                        let original = try CarapaceWatchMessageLegacyImport.Message(
            ...
            for id in recentIDs {
                ...
                try db.execute(
                    ...
                    """,
                    arguments: [id, nowMs, nowMs + CarapaceWatchChatDeliveryCodec.lifetimeMs])
            }
            try db.execute(
    ...
    private static func legacyContentHash(_ message: CarapaceWatchMessageLegacyImport.Message) -> String {
        var bytes = Data()
    ...
    private static func legacyImportConflict() -> CarapaceWatchChatDeliveryError {
