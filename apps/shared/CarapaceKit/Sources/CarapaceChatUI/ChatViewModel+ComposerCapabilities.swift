...
extension CarapaceChatViewModel {
    ...
    var composerCapabilityCatalog: CarapaceChatComposerCapabilityCatalog {
    ...
    func composerSessionSettingsExpectation() -> CarapaceChatSessionSettingsExpectation? {
        ...
        return CarapaceChatSessionSettingsExpectation(
            permissionMode: entry.permissionMode,
            toolOverrides: entry.toolOverrides)
    }

    func durableSessionSettingsExpectation() -> CarapaceChatSessionSettingsExpectation {
        let entry = self.currentSessionEntry()
        return CarapaceChatSessionSettingsExpectation(
            permissionMode: entry?.permissionMode,
            toolOverrides: entry?.toolOverrides)
    ...
    func composerPermissionDisabledReason(_ mode: CarapaceChatPermissionMode?) -> String? {
    ...
    func composerSkillDisabledReason(_ skill: CarapaceChatComposerSkill) -> String? {
    ...
    func composerSkillStatusMessage(_ skill: CarapaceChatComposerSkill) -> String? {
    ...
    var composerPermissionMode: CarapaceChatPermissionMode? {
        self.currentSessionEntry()?.permissionMode
    }

    var composerToolOverrides: CarapaceChatSessionToolOverrides {
        self.confirmedCapabilityToolOverridesByTarget[self.currentModelPatchTarget()]?.overrides ??
            self.currentSessionEntry()?.toolOverrides ?? CarapaceChatSessionToolOverrides()
    }
    ...
    func composerSkillEnabled(_ skill: CarapaceChatComposerSkill) -> Bool {
    ...
    func composerConnectorEnabled(_ connector: CarapaceChatComposerConnector) -> Bool {
    ...
    func loadComposerCapabilities(force: Bool = false) async {
        ...
        self.composerCapabilityState.catalog = CarapaceChatComposerCapabilityCatalog()
        self.composerCapabilityState.notice = nil
    ...
    func selectComposerPermissionMode(_ mode: CarapaceChatPermissionMode?) {
        ...
        self.performComposerCapabilityPatch(
            CarapaceChatSessionSettingsPatch(permissionMode: .some(mode)),
    ...
    func toggleComposerSkill(_ skill: CarapaceChatComposerSkill) {
        guard self.composerCapabilityControlsAvailable,
              self.composerCapabilityCatalog.toolOverrideMutationAvailable,
    ...
    func toggleComposerConnector(_ connector: CarapaceChatComposerConnector) {
        guard self.composerCapabilityControlsAvailable,
              self.composerCapabilityCatalog.toolOverrideMutationAvailable
        else { return }
        var next = self.composerToolOverrides
    ...
    func clearComposerToolOverrides() {
        ...
        self.performComposerCapabilityPatch(
            CarapaceChatSessionSettingsPatch(toolOverrides: .some(nil)),
            permissionMode: nil,
            toolOverrides: .some(nil),
            notice: String(localized: "Tool overrides will be cleared for the next run."))
    }

    private func patchComposerToolOverrides(_ overrides: CarapaceChatSessionToolOverrides) {
        let normalized = overrides.isEmpty ? nil : overrides
        self.performComposerCapabilityPatch(
            CarapaceChatSessionSettingsPatch(toolOverrides: .some(normalized)),
            permissionMode: nil,
        ...
        notice: String?)
    {
        ...
        let ownerID = self.composerCapabilityOwnerID
        let expectedToolOverrides: CarapaceChatSessionToolOverrides?? = toolOverrides == nil
            ? nil
            : .some(self.currentSessionEntry()?.toolOverrides)
        let expectedPermissionMode: CarapaceChatPermissionMode?? = permissionMode == nil
            ? nil
            ...
            do {
                ...
                if let outbox = self.outbox, let originalOutboxScope {
                    let parked = await outbox.parkQueuedCommands(
                        ...
                        lastError: CarapaceChatSQLiteTranscriptCache.outboxSettingsChangedError)
                    guard parked else {
                        throw NSError(
                            domain: "CarapaceChatOutbox",
                            code: 1,
                            userInfo: [
    ...
    func recordCapabilityPatchFailure(
        ...
        outboxScope: CarapaceChatOutboxScope?,
