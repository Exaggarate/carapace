        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                if let language = self.block.language {
                    Text(language).font(CarapaceChatTypography.caption2)
                }
                Spacer(minLength: 0)
...
struct ChatMathBlockView: View {
    ...
    @ScaledMetric(relativeTo: .body) private var fontSize: CGFloat = CarapaceChatTypography.bodySize
...
struct ChatMarkdownTableView: View {
    ...
    private func cell(_ text: String, isHeader: Bool) -> some View {
        Text(self.inlineMarkdown(text))
            .font(isHeader ? CarapaceChatTypography.footnoteSemiBold : CarapaceChatTypography.footnote)
            .foregroundStyle(CarapaceChatTheme.assistantText)
            .textSelection(.enabled)
    }
