...
private struct CarapaceSendableCameraImage: @unchecked Sendable {
    let value: UIImage
}

private struct CarapaceVideoTransfer: Sendable, Transferable {
    let url: URL
    ...
    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(contentType: .movie) { video in
            ...
            let destination = FileManager.default.temporaryDirectory
                .appendingPathComponent("carapace-picker-video-\(UUID().uuidString)")
                .appendingPathExtension(fileExtension.isEmpty ? "mov" : fileExtension)
            try FileManager.default.copyItem(at: received.file, to: destination)
            return CarapaceVideoTransfer(url: destination)
        }
    }
}
#endif
...
@MainActor
private struct CarapaceChatAttachmentCaptureOwner {
    let viewModel: CarapaceChatViewModel
    let session: CarapaceChatViewModel.SessionSnapshot
}
...
struct CarapaceChatComposerPresentationOwner: Equatable {
    let viewModelID: ObjectIdentifier
    let session: CarapaceChatViewModel.SessionSnapshot

    @MainActor
    init(viewModel: CarapaceChatViewModel) {
...
@MainActor
struct CarapaceChatComposer: View {
    @Environment(\.carapaceChatDesktopLayout) private var isDesktopLayout
    @Bindable var viewModel: CarapaceChatViewModel
    let style: CarapaceChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: CarapaceChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: CarapaceChatTalkControl?
    let dictationControl: CarapaceChatDictationControl?
    let voiceNoteControl: CarapaceChatVoiceNoteControl?
    var focusRequest = 0
    ...
    @State private var photoPickerOwner: CarapaceChatAttachmentCaptureOwner?
    @State private var fileImporterOwner: CarapaceChatAttachmentCaptureOwner?
    #if canImport(UIKit)
    @State private var cameraEncodingTask: Task<Void, Never>?
    @State private var cameraEncodingGeneration = UUID()
    @State private var cameraCaptureOwner: CarapaceChatAttachmentCaptureOwner?
    #endif
    ...
    private var platformComposer: some View {
        self.styledComposer
            ...
            .fileImporter(
                ...
                allowedContentTypes: CarapaceChatPickerAttachmentMetadata.allowedFileContentTypes,
            ...
            .fullScreenCover(
                ...
                content: {
                    ...
                    CarapaceChatCameraPicker { image in
        ...
        VStack(alignment: .leading, spacing: 4) {
            if self.composerChrome == .clean {
                ...
                if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                    CarapaceVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
    ...
    private var presentationOwner: CarapaceChatComposerPresentationOwner {
        CarapaceChatComposerPresentationOwner(viewModel: self.viewModel)
    }

    #if !os(macOS)
    private func attachmentCaptureOwner() -> CarapaceChatAttachmentCaptureOwner {
        CarapaceChatAttachmentCaptureOwner(
        ...
        if self.composerChrome == .full {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(CarapaceChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(CarapaceChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        ...
                        if self.viewModel.modelSelectionID != CarapaceChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? CarapaceChatTheme.warning : CarapaceChatTheme.muted
        return ZStack {
            Circle()
                .stroke(CarapaceChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(CarapaceChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(CarapaceChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(CarapaceChatTheme.composerBorder)))
    ...
    private func talkHelpText(_ talkControl: CarapaceChatTalkControl) -> String {
...
extension CarapaceChatComposer {
                      ...
                      self.viewModel.slashCommands.isEmpty
            {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Commands unavailable")
                        .font(CarapaceChatTypography.footnoteSemiBold)
                    Text(error)
                        .font(CarapaceChatTypography.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(3)
    ...
    private func selectSlashCommand(_ command: CarapaceChatCommandChoice) {
...
extension CarapaceChatComposer {
            ...
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(CarapaceChatTypography.display(size: 17, weight: .semibold, relativeTo: .body))
                }
            }
            .buttonStyle(.plain)
            ...
            Button {
                ...
                if self.viewModel.isSending {
                    ...
                    Image(systemName: "arrow.up")
                        .font(CarapaceChatTypography.display(size: 18, weight: .semibold, relativeTo: .body))
                }
            }
            .buttonStyle(.plain)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? CarapaceChatTheme.userBubble
    }
    ...
    private var sendButtonForeground: Color {
        ...
        return self.canSendMessage ? CarapaceChatTheme.userText(on: self.userAccent) : disabledInk
    }
    ...
    func pickFilesMac() {
        ...
        panel.allowedContentTypes = CarapaceChatPickerAttachmentMetadata.allowedFileContentTypes
        panel.begin { resp in
    ...
    private func handleFileImport(
        ...
        owner: CarapaceChatAttachmentCaptureOwner?)
    {
        guard self.isAttachmentInputEnabled else { return }
        guard let owner,
    ...
    #if canImport(UIKit)
    private func addCameraImage(_ image: UIImage, owner: CarapaceChatAttachmentCaptureOwner) {
        guard self.isAttachmentInputEnabled else { return }
        guard self.viewModel === owner.viewModel,
              owner.viewModel.isCurrentSession(owner.session)
        else { return }
        self.cancelActiveCameraEncoding()
        let sendableImage = CarapaceSendableCameraImage(value: image)
    ...
    private func loadPhotosPickerItems(
        _ items: [PhotosPickerItem],
        owner: CarapaceChatAttachmentCaptureOwner) async
    {
        guard self.isAttachmentInputEnabled else {
            self.pickerItems = []
            return
        ...
        for item in items {
            do {
                ...
                if type.conforms(to: .movie) {
                    guard let transfer = try await item.loadTransferable(type: CarapaceVideoTransfer.self)
                    else { continue }
                    defer { try? FileManager.default.removeItem(at: transfer.url) }
                    let metadata = CarapaceChatPickerAttachmentMetadata.resolve(
                        contentType: type,
                    ...
                    let metadata = CarapaceChatPickerAttachmentMetadata.resolve(contentType: type)
