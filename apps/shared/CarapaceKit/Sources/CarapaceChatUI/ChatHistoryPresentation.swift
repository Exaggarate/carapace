...
public enum CarapaceChatHistoryPresentation {
    private static let previewItemLimit = 5

    private struct MessageEntry {
        var message: CarapaceChatMessage
        var text: String
        var runID: String?
    ...
    public nonisolated static func replyText(
        from rawMessages: [CarapaceKit.AnyCodable],
        runID: String,
        submittedText: String? = nil,
        submittedAtMs: Int64? = nil,
        inputConsumptions: [CarapaceChatHistoryPayload.InputConsumption]? = nil) -> String?
    {
    ...
    public nonisolated static func makeWatchItems(
        from rawMessages: [CarapaceKit.AnyCodable]) -> [CarapaceWatchChatItem]
    {
    ...
    private nonisolated static func decodeMessage(_ raw: CarapaceKit.AnyCodable) -> MessageEntry? {
        ...
        return MessageEntry(
            message: message,
            text: text,
            runID: fields?["__carapace"]?.dictionaryValue?["runId"]?.stringValue,
            isMessageToolMirror: fields?["carapaceMessageToolMirror"]?.dictionaryValue != nil)
    }
