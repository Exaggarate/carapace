                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            if !model.partialTranscript.isEmpty {
                                Text(model.partialTranscript)
                                    .font(CarapaceChatTypography.caption.italic())
    ...
    private func header(model: ChatTalkTranscriptStripModel, showsDisclosure: Bool) -> some View {
        HStack(spacing: 7) {
            ...
            if let collapsedText = model.collapsedText, !self.isExpanded {
                Text(collapsedText)
                    .font(CarapaceChatTypography.caption)
                    .foregroundStyle(model.partialTranscript.isEmpty ? .secondary : CarapaceChatTheme.accent)
...
extension View {
    func chatTalkInputDeviceMenu(_ control: CarapaceChatTalkControl) -> some View {
