...
extension CarapaceChatViewModel {
    ...
    func refreshProgressCard(from info: CarapaceChatSessionInfo?, for request: HistoryRequest) {
        ...
        if let key = info?.key,
           let owner = info?.agentId ?? CarapaceChatSessionKey.agentID(from: key),
           request.session.deliveryAgentID == nil || request.session.deliveryAgentID == owner,
           key == "global" || CarapaceChatSessionKey.agentID(from: key) == owner
        {
            self.preparedProgressCardTarget = (
                request.session,
                request.progressCardGeneration,
                CarapaceChatSessionTarget(sessionKey: key, agentID: owner))
        }
    ...
    private func progressCardTarget(for session: SessionSnapshot) -> CarapaceChatSessionTarget? {
        ...
        return CarapaceChatSessionTarget(sessionKey: "global", agentID: owner)
    }
        ...
        requestID: UInt64) async
    {
        ...
        do {
            ...
            if error is CarapaceChatProgressCardError, self.errorText == nil {
