...
extension CarapaceChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [CarapaceChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CarapaceChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    static func stripInboundMetadata(from message: CarapaceChatMessage) -> CarapaceChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CarapaceChatMessageContent in
    ...
    static func messageContentFingerprint(for message: CarapaceChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: CarapaceChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
            let artifactId = (item.artifactId ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: CarapaceChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: CarapaceChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func isFinalAssistantMessage(_ message: CarapaceChatMessage) -> Bool {
    ...
    private static func finalMessageRunID(for message: CarapaceChatMessage) -> String? {
    ...
    static func finalMessageReconciliationKey(for message: CarapaceChatMessage) -> String? {
        guard self.isFinalAssistantMessage(message) else { return nil }
    ...
    static func adoptingCanonicalMessage(
        _ incoming: CarapaceChatMessage,
        over existing: CarapaceChatMessage) -> CarapaceChatMessage
    {
        CarapaceChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [CarapaceChatMessageContent],
        from existing: [CarapaceChatMessageContent]) -> [CarapaceChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [CarapaceChatMessage]) -> Range<[CarapaceChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [CarapaceChatMessage]) -> [CarapaceChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [CarapaceChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: CarapaceChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: CarapaceChatMessage) -> Bool {
    ...
    static func reconcileMessageIDs(
        previous: [CarapaceChatMessage],
        incoming: [CarapaceChatMessage]) -> [CarapaceChatMessage]
    {
        guard !previous.isEmpty, !incoming.isEmpty else { return incoming }

        var previousMessagesByKey: [String: [CarapaceChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [CarapaceChatMessage],
        incoming: [CarapaceChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [CarapaceChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [CarapaceChatMessage]) -> [CarapaceChatMessage] {
    ...
    static func dedupeKey(for message: CarapaceChatMessage) -> String? {
...
extension CarapaceChatViewModel {
    ...
    func applyHistoryPayload(
        _ payload: CarapaceChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [CarapaceChatMessage]) -> [CarapaceChatMessage]
    {
