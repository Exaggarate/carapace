...
private let outboxLogger = Logger(subsystem: "ai.carapace", category: "CarapaceChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum CarapaceChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension CarapaceChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> CarapaceChatOutboxMessageState? {
        self.outboxStatesByMessageID[messageID]
    }

    static func activeBranchLeafEntryID(in branches: [CarapaceChatSessionBranch]) -> String? {
    ...
    func outboxBranchScope(for session: SessionSnapshot) -> CarapaceChatOutboxScope? {
        let agentID = self.outboxAgentID(for: session)
        guard !self.outboxRequiresAgentID(for: session) || agentID != nil else { return nil }
        return CarapaceChatOutboxScope(sessionKey: session.key, agentID: agentID)
    }

    private static func outboxBranchScope(for command: CarapaceChatOutboxCommand) -> CarapaceChatOutboxScope {
        CarapaceChatOutboxScope(sessionKey: command.sessionKey, agentID: command.agentID)
    }
    ...
    func captureOutboxBranchState(for session: SessionSnapshot) async -> CarapaceChatOutboxBranchState? {
    ...
    func observeOutboxTranscriptTip(_ message: CarapaceChatMessage, session: SessionSnapshot) {
    ...
    func reconcileOutboxBranchScope(
        _ session: SessionSnapshot,
        branches: [CarapaceChatSessionBranch],
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [CarapacePendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = CarapaceChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
        ...
        deliveryIsAmbiguous: Bool) async -> Bool
    {
        ...
        let command = CarapaceChatOutboxCommand(
            ...
            lastError: deliveryIsAmbiguous
                ? CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError
                : nil)
        await self.waitForBootstrapOutboxBranchCapture(for: session)
    ...
    func confirmOutboxCommands(in messages: [CarapaceChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [CarapaceChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: CarapaceChatMessage,
        for command: CarapaceChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any CarapaceChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [CarapaceChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [CarapaceChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: CarapaceChatOutboxCommand) -> CarapaceChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: CarapaceChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: CarapaceChatOutboxCommand,
        outbox: any CarapaceChatCommandOutbox,
        routeLease: CarapaceChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        ...
        {
            let reason = CarapaceChatSQLiteTranscriptCache.outboxSettingsGatewayUpgradeRequiredError
            let message = CarapaceChatSQLiteTranscriptCache.outboxDisplayError(reason)
        ...
        {
            let reason = CarapaceChatSQLiteTranscriptCache.outboxSettingsReviewRequiredError
            let message = CarapaceChatSQLiteTranscriptCache.outboxDisplayError(reason)
            let update = await self.failOutboxCommand(
                command,
                outbox: outbox,
        ...
        do {
            ...
            if error.detailsReason == CarapaceChatSessionRoutingContract.changedErrorReason {
                let parked = await self.parkOutboxCommandForChangedTarget(command, outbox: outbox)
                return parked ? .continueFlush : .stop
            }
            if error.detailsReason == CarapaceChatSessionSettingsContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: CarapaceChatOutboxCommand,
        outbox: any CarapaceChatCommandOutbox) async -> OutboxFlushDisposition
    {
        ...
        outbox: any CarapaceChatCommandOutbox) async -> OutboxFlushDisposition
    {
        let update = await self.failOutboxCommand(
            ...
            outbox: outbox,
            retryCount: command.retryCount,
            reason: CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
        self.applyTransportHealth(false)
        guard update == .updated else { return .stop }
        return .stopAndReconcile(Self.deliveryTarget(for: command))
    }

    private static func needsOutboxDeliveryReconciliation(_ command: CarapaceChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: CarapaceChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func failOutboxCommand(
        ...
        outbox: any CarapaceChatCommandOutbox,
        retryCount: Int,
        reason: String) async -> CarapaceChatOutboxUpdateResult
    {
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: CarapaceChatOutboxCommand,
        outbox: any CarapaceChatCommandOutbox) async -> Bool
    {
        await self.failOutboxCommand(
            command,
            outbox: outbox,
            retryCount: command.retryCount,
            reason: CarapaceChatSQLiteTranscriptCache.outboxChangedTargetError) != .unavailable
    }
        ...
        outbox: any CarapaceChatCommandOutbox) async -> Bool
    {
        await self.failOutboxCommand(
            ...
            outbox: outbox,
            retryCount: command.retryCount,
            reason: CarapaceChatSQLiteTranscriptCache.outboxSettingsChangedError) != .unavailable
    }
    ...
    private func recordOutboxRejection(
        of command: CarapaceChatOutboxCommand,
        outbox: any CarapaceChatCommandOutbox,
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return CarapaceChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if CarapaceChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: CarapaceChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: CarapaceChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: CarapaceChatOutboxCommand)
        -> CarapaceChatOutboxMessageState
    {
        ...
        case .awaitingConfirmation:
            .confirming
        case .failed where command.lastError == CarapaceChatSQLiteTranscriptCache.outboxChangedTargetError:
            .failed(reason: "Gateway session routing changed; review and retry this message.")
        case .failed:
            .failed(reason: CarapaceChatSQLiteTranscriptCache.outboxDisplayError(command.lastError))
        }
    }
