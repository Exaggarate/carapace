...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: CarapaceChatSessionEntry?
    @State private var sessionPendingRename: CarapaceChatSessionEntry?
    ...
    @AppStorage("carapace.chat.collapsedSessionGroups") private var collapsedSessionGroups = ""
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(CarapaceChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        if node.badges.queuedCount > 0 {
            Image(systemName: "hourglass")
                .foregroundStyle(CarapaceChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: CarapaceChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: CarapaceChatSessionEntry) -> String? {
        var parts: [String] = []
