...
public enum CarapaceMascotMood: String, CaseIterable, Equatable, Sendable {
...
enum CarapaceMascotEffect: Equatable {
    case none
    case sparkles
    case hearts
    case zzz
...
final class CarapaceMascotAnimator {
    ...
    private(set) var mood: CarapaceMascotMood = .idle
    ...
    private var activeGesture: CarapaceMascotGesture?
    private var activeGestureStart: TimeInterval = 0
    private var pendingGesture: CarapaceMascotGesture?
    ...
    private var lastClickReaction: CarapaceMascotGesture?
    ...
    private var requestedAccessory: CarapaceMascotAccessory = .none
    ...
    func setMood(_ mood: CarapaceMascotMood, at time: TimeInterval) {
    ...
    func setAccessory(_ accessory: CarapaceMascotAccessory, at time: TimeInterval) {
    ...
    func handleTap(at time: TimeInterval) {
        ...
        var reactions: [CarapaceMascotGesture] = [.hop, .wave, .wink]
    ...
    func pose(at time: TimeInterval) -> CarapaceMascotPose {
        ...
        let effectiveMood: CarapaceMascotMood = dozing ? .sleepy : self.mood
        ...
        if self.requestedAccessory != .none {
            ...
            pose.accessoryAmount = CarapaceMascotGesture.easeOut(progress)
        }
    ...
    private func advanceSchedules(at time: TimeInterval) {
        ...
        self.blinkStarts.removeAll { time - $0 > CarapaceMascotGesture.blinkDuration }
    ...
    private func randomQuirk() -> CarapaceMascotGesture {
    ...
    private func startGesture(_ gesture: CarapaceMascotGesture, at time: TimeInterval) {
        self.activeGesture = gesture
        self.activeGestureStart = time
    }

    private static func entranceGesture(for mood: CarapaceMascotMood) -> CarapaceMascotGesture? {
        switch mood {
        case .happy: .hop
    ...
    private func basePose(for mood: CarapaceMascotMood, at time: TimeInterval) -> CarapaceMascotPose {
        ...
        case .working:
            ...
            if phase < 0.05 {
                ...
                pose.rightClawDegrees = -6 - 28 * CarapaceMascotGesture.easeInOut((phase - 0.05) / 0.55)
            } else if phase < 0.72 {
                let strike = ((phase - 0.60) / 0.12).clamped(to: 0...1)
                pose.rightClawDegrees = -34 + 46 * strike * strike
            } else {
                pose.rightClawDegrees = 12 - 18 * CarapaceMascotGesture.easeInOut((phase - 0.72) / 0.28)
            }
            pose.leftClawDegrees = 4 + 2 * sin(2 * .pi * phase)
            let impact = CarapaceMascotGesture.bell(((phase - 0.72) / 0.14).clamped(to: 0...1))
    ...
    private func applyGaze(
        _ pose: inout CarapaceMascotPose,
        mood: CarapaceMascotMood,
    ...
    private func applyBlinks(_ pose: inout CarapaceMascotPose, at time: TimeInterval) {
        guard pose.happyEyes < 0.6 else { return }
        for start in self.blinkStarts {
            let progress = (time - start) / CarapaceMascotGesture.blinkDuration
            guard progress >= 0, progress <= 1 else { continue }
            let closure = CarapaceMascotGesture.bell(CGFloat(progress))
    ...
    private func applyDizzy(_ pose: inout CarapaceMascotPose, at time: TimeInterval) {
...
enum CarapaceMascotGesture: Equatable {
    case wave
    case hop
    case wink
    case celebrate
    ...
    func apply(to pose: inout CarapaceMascotPose, progress p: CGFloat) {
        switch self {
        case .wave:
        ...
        case .sneeze:
            if p < 0.42 {
                let inhale = CarapaceMascotGesture.easeInOut(p / 0.42)
                ...
                let recover = 1 - CarapaceMascotGesture.easeInOut((p - 0.58) / 0.42)
        ...
        case .sigh:
            let rise = CarapaceMascotGesture.easeInOut((p / 0.3).clamped(to: 0...1))
            let fall = CarapaceMascotGesture.easeInOut(((p - 0.3) / 0.45).clamped(to: 0...1))
