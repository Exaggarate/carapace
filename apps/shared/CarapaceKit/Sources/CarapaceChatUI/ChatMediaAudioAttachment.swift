...
struct ChatMediaAudioAttachment: View {
    ...
    let playback: CarapaceChatPlaybackMode?
    let resolverReady: Bool
    let playbackAllowed: @MainActor @Sendable () -> Bool
    let load: @MainActor @Sendable (String) async throws -> CarapaceChatLoadedMedia?
    ...
    private func playbackPositionLabel(_ player: ChatMediaAudioPlayer) -> String {
        let elapsed = carapaceVoiceNoteDurationLabel(player.currentTime)
        let total = carapaceVoiceNoteDurationLabel(player.duration)
