...
private let transportEventsLogger = Logger(subsystem: "ai.carapace", category: "CarapaceChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: CarapaceChatViewModel?

    init(_ value: CarapaceChatViewModel) {
        self.value = value
    }
}
...
extension CarapaceChatViewModel {
    ...
    func handleTransportEvent(_ evt: CarapaceChatTransportEvent) {
    ...
    private func applySessionChangeProjection(
        _ change: CarapaceChatSessionsChangedEvent,
    ...
    private func handleSessionsChangedEvent(_ change: CarapaceChatSessionsChangedEvent) {
        ...
        runID: String?) -> LifecycleSessionMergeResult
    {
        ...
        self.sessions = CarapaceChatSessionListOrganizer.organize(updated)
    ...
    private func lifecycleSnapshotMatchesEvent(
        _ snapshot: CarapaceChatSessionEntry,
        change: CarapaceChatSessionsChangedEvent) -> Bool
    {
    ...
    private func lifecycleSessionIndex(
        _ snapshot: CarapaceChatSessionEntry,
        change: CarapaceChatSessionsChangedEvent) -> Int?
    {
    ...
    private static func lifecycleSnapshotRejection(
        snapshot: CarapaceChatSessionEntry,
        existing: CarapaceChatSessionEntry,
        phase: String,
        runID: String?) -> LifecycleSessionMergeResult?
    {
        if phase == "start" {
    ...
    private static func mergedLifecycleSession(
        ...
        color: String?,
        colorPresent: Bool) -> CarapaceChatSessionEntry
    {
        var merged = existing
        if colorPresent {
            merged.color = color
    ...
    private func handleSessionMessageEvent(_ payload: CarapaceSessionMessageEventPayload) {
        ...
        if isCurrentSession, payload.hasActiveRun != nil || payload.activeRunIdsPresent {
            let change = CarapaceChatSessionsChangedEvent(
    ...
    private func handleChatEvent(_ chat: CarapaceChatEventPayload) {
        ...
        if chat.state == "delta", let runID = explicitRunID {
            ...
            self.adoptRun(
                runId: runID,
                bufferedText: CarapaceChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        let hapticEvent: CarapaceChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: CarapaceChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = CarapaceChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: CarapaceChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: CarapaceChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: CarapaceChatMessage) -> CarapaceChatMessage {
    ...
    private func handleAgentEvent(_ evt: CarapaceAgentEventPayload) {
        if evt.stream == "usage" {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CarapaceChatPendingToolCall(
    ...
    private func handleAgentUsageEvent(_ evt: CarapaceAgentEventPayload) {
    ...
    private func handleAgentLifecycleEvent(
        _ evt: CarapaceAgentEventPayload,
        isPendingRun: Bool,
    ...
    private static func agentLifecycleErrorMessage(_ evt: CarapaceAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: CarapaceChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: CarapaceChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: CarapaceChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: CarapaceChatRunTerminalState) {
        let hapticEvent: CarapaceChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [CarapaceChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [CarapaceChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [CarapaceChatMessage].Index,
        in messages: [CarapaceChatMessage]) -> LatestUserTurn?
    {
    ...
    static func hasAnsweredUser(
        ...
        in messages: [CarapaceChatMessage])
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [CarapaceChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [CarapaceChatMessage].Index,
        in messages: [CarapaceChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [CarapaceChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: CarapaceChatMessage) -> CarapaceChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> CarapaceChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> CarapaceChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: CarapaceChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any CarapaceChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any CarapaceChatTransport) async
    {
        var terminalState: CarapaceChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(CarapaceChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: CarapaceChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: CarapaceChatHaptics.Event? = nil)
    {
    ...
    private func removePendingRun(
        _ runId: String,
        hapticEvent: CarapaceChatHaptics.Event?)
    {
