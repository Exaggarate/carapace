...
private let databaseLogger = Logger(subsystem: "ai.carapace", category: "CarapaceClientDatabases")
...
public final class CarapaceClientDatabases: @unchecked Sendable {
    ...
    public let watchMessages: CarapaceWatchMessageJournal
        ...
        registeredGatewayIDs: [String]? = nil) throws
    {
        ...
        self.watchMessages = CarapaceWatchMessageJournal(queue: self.stateQueue)
    ...
    public func store(gatewayID: String) -> CarapaceChatSQLiteTranscriptCache {
        CarapaceChatSQLiteTranscriptCache(databases: self, gatewayID: gatewayID)
    }
    ...
    public func loadSessionRoutingIdentity(
        gatewayID: String) -> CarapaceChatSessionRoutingIdentity?
    {
        do {
            return try self.stateQueue.read { db in
                ...
                return CarapaceChatSessionRoutingIdentity(
                    scope: row["scope"],
...
extension CarapaceClientDatabases {
    ...
    private static func openStateDatabase(at url: URL) throws -> DatabaseQueue {
        let queue = try DatabaseQueue(
            path: url.path,
            configuration: self.configuration(label: "Carapace.client-state"))
    ...
    private static func openRepairableCacheDatabase(at url: URL) throws -> DatabaseQueue {
        do {
            let queue = try DatabaseQueue(
                path: url.path,
                configuration: self.configuration(label: "Carapace.gateway-cache"))
...
extension CarapaceClientDatabases {
    ...
    private struct LegacyCommand {
        ...
        var agentID: String
        var text: String
        var attachments: [CarapaceChatOutboxAttachment]
        var thinking: String
    ...
    private static func readLegacySnapshot(at url: URL) throws -> LegacySnapshot? {
        var configuration = Configuration()
        configuration.label = "Carapace.legacy-chat-import"
            ...
            if try db.tableExists("outbox_commands") {
                ...
                for row in rows {
                    ...
                    let attachments = try JSONDecoder().decode(
                        [CarapaceChatOutboxAttachment].self,
                        from: Data(attachmentsJSON.utf8))
                    let originalStatus: String = row["status"]
                    guard CarapaceChatOutboxCommand.Status(rawValue: originalStatus) != nil else {
                    ...
                    let status = lacksVerifiedTarget ? CarapaceChatOutboxCommand.Status.failed.rawValue : originalStatus
                    let lastError: String = if lacksVerifiedTarget {
                        if originalStatus == CarapaceChatOutboxCommand.Status.sending.rawValue ||
                            originalStatus == CarapaceChatOutboxCommand.Status.awaitingConfirmation.rawValue ||
                            originalError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError
                        {
                            CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError
                        } else {
                            CarapaceChatSQLiteTranscriptCache.outboxUnknownTargetError
                        }
                    } else {
                        originalError
                    }
