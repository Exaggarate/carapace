...
public struct CarapaceChatDisplayOptions: OptionSet, Sendable {
...
@MainActor
public struct CarapaceChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @Environment(\.carapaceChatDesktopLayout) private var isDesktopLayout
    ...
    #if os(iOS)
    @State private var selectTextMessage: CarapaceChatMessage?
    #endif
    ...
    private let displayOptions: CarapaceChatDisplayOptions
    ...
    private let talkControl: CarapaceChatTalkControl?
    private let dictationControl: CarapaceChatDictationControl?
    private let voiceNoteControl: CarapaceChatVoiceNoteControl?
    ...
    public init(
        ...
        displayOptions: CarapaceChatDisplayOptions? = nil,
        ...
        talkControl: CarapaceChatTalkControl? = nil,
        dictationControl: CarapaceChatDictationControl? = nil,
        voiceNoteControl: CarapaceChatVoiceNoteControl? = nil,
        speech: CarapaceChatSpeechController? = nil,
    ...
    private var swarmProgress: some View {
        ...
        if !groups.isEmpty {
            CarapaceChatSwarmProgressView(groups: groups)
        }
    }

    private var composer: some View {
        CarapaceChatComposer(
    ...
    private var messageListRows: some View {
                ...
                self.messageRow(for: message, contextWindowTokens: contextWindowTokens)
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(CarapaceChatTheme.accent.opacity(self.searchMessageID == message.id ? 0.08 : 0)))
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .strokeBorder(
                                CarapaceChatTheme.accent.opacity(self.searchMessageID == message.id ? 0.55 : 0),
                                lineWidth: 1))
        ...
        CarapaceQuestionCards(viewModel: self.viewModel)
    ...
    private func messageActionsMenu(for message: CarapaceChatMessage) -> some View {
        Menu {
    ...
    @ViewBuilder
    private func messageMenuActions(for message: CarapaceChatMessage) -> some View {
    ...
    @ViewBuilder
    private func listenMessageButton(for message: CarapaceChatMessage) -> some View {
    ...
    private func isListenable(_ msg: CarapaceChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: CarapaceChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Image(systemName: "arrow.down")
                ...
                .background(
                    Circle()
                        .fill(CarapaceChatTheme.subtleCard)
        ...
        .buttonStyle(.plain)
        .foregroundStyle(CarapaceChatTheme.assistantText)
...
extension CarapaceChatView {
    private func mergeToolResults(in messages: [CarapaceChatMessage]) -> [CarapaceChatMessage] {
        var result: [CarapaceChatMessage] = []
        ...
        for message in messages {
            ...
            let merged = CarapaceChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: CarapaceChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CarapaceChatMessage) -> Bool {
    ...
    private func primaryText(in message: CarapaceChatMessage) -> String {
        ChatMessageVisibleText.displayText(
            in: message,
            includeThinking: self.displayOptions.contains(.reasoning))
    }

    private func hasInlineAttachments(in message: CarapaceChatMessage) -> Bool {
        message.content.contains(where: \.isInlineAttachment)
    }

    private func toolCalls(in message: CarapaceChatMessage) -> [CarapaceChatMessageContent] {
    ...
    private func inlineToolResults(in message: CarapaceChatMessage) -> [CarapaceChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CarapaceChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CarapaceChatMessage) -> String {
        self.primaryText(in: message)
    }

    @ViewBuilder
    private func copyMessageButton(for message: CarapaceChatMessage) -> some View {
        ...
        if !text.isEmpty {
            Button {
                ...
                Label {
                    Text("Copy Message")
                        .font(CarapaceChatTypography.body)
                } icon: {
    ...
    #if os(iOS)
    @ViewBuilder
    private func selectTextButton(for message: CarapaceChatMessage) -> some View {
        if !ChatMessageVisibleText.copyText(in: message).isEmpty {
            Button {
                self.selectTextMessage = message
            } label: {
                Label {
                    Text("Select Text").font(CarapaceChatTypography.body)
                } icon: {
    ...
    @ViewBuilder
    private func openFullMessageButton(for message: CarapaceChatMessage) -> some View {
    ...
    @ViewBuilder
    private func rewindMessageButton(for message: CarapaceChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
    ...
    @ViewBuilder
    private func forkMessageButton(for message: CarapaceChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
           message.transcriptMessageID?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
        {
    ...
    @ViewBuilder
    private func replyMessageButton(for message: CarapaceChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [CarapaceChatView.StarterPrompt]
    let onPrompt: (CarapaceChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(CarapaceChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(CarapaceChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(CarapaceChatTypography.headline)
        } description: {
            Text(self.message)
                .font(CarapaceChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(CarapaceChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: self.systemImage)
                .font(CarapaceChatTypography.display(size: 15, weight: .semibold, relativeTo: .subheadline))
