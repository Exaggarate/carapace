...
public enum ChatSessionSidebarModel {
    ...
    public static func sections(
        ...
        groups: [CarapaceChatSessionGroup] = [],
    ...
    static func tree(from sessions: [CarapaceChatSessionEntry]) -> [Node] {
        ...
        var entriesByKey: [String: CarapaceChatSessionEntry] = [:]
        ...
        var childrenByParent: [String: [CarapaceChatSessionEntry]] = [:]
        for session in sessions {
        ...
        func build(_ session: CarapaceChatSessionEntry, ancestors: Set<String>) -> Node {
    ...
    private static func node(session: CarapaceChatSessionEntry, children: [Node]) -> Node {
    ...
    public static func displayName(for session: CarapaceChatSessionEntry) -> String {
    ...
    public static func workSubtitle(for session: CarapaceChatSessionEntry) -> String? {
        ...
        let shortBranch = branch.map { $0.hasPrefix("carapace/") ? String($0.dropFirst("carapace/".count)) : $0 }
    ...
    public static func subtitle(
        for session: CarapaceChatSessionEntry,
        workSubtitle: String?,
    ...
    public static func applying(
        ...
        to sessions: [CarapaceChatSessionEntry],
        activeAgentId: String? = nil) -> [CarapaceChatSessionEntry]
    {
        ...
        let candidate = CarapaceChatSessionObserverDigest(digest)
    ...
    public static func reconcilingGlobalObserverDigestOwner(
        in sessions: [CarapaceChatSessionEntry],
        activeAgentId: String?) -> [CarapaceChatSessionEntry]
    {
    ...
    private static func reconcilingGlobalObserverDigestOwner(
        ...
        adoptOwnerless: Bool) -> [CarapaceChatSessionEntry]
    {
        ...
        updated[index].observerDigest = if digestAgentId == nil, adoptOwnerless {
            CarapaceChatSessionObserverDigest(
    ...
    private static func observerProjection(
        for change: CarapaceChatSessionsChangedEvent,
        activeAgentId: String?) -> (digest: CarapaceChatSessionObserverDigest?, present: Bool)?
    {
    ...
    public static func applying(
        sessionChange change: CarapaceChatSessionsChangedEvent,
        to sessions: [CarapaceChatSessionEntry],
        activeAgentId: String? = nil) -> [CarapaceChatSessionEntry]?
    {
    ...
    private static func visibleObserverDigest(
        for session: CarapaceChatSessionEntry) -> CarapaceChatSessionObserverDigest?
    {
    ...
    private static func activeAgentStatus(
        _ status: CarapaceChatSessionAgentStatus?,
        now: Double) -> CarapaceChatSessionAgentStatus?
    {
        guard let status,
    ...
    private static func unreadFailureReason(for session: CarapaceChatSessionEntry) -> String? {
        let status = session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func isRunning(_ session: CarapaceChatSessionEntry) -> Bool {
        session.hasActiveRun == true ||
            session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "running"
    }

    private static func isNewer(
        _ candidate: CarapaceChatSessionObserverDigest,
        than previous: CarapaceChatSessionObserverDigest) -> Bool
    {
    ...
    private static func visibleSessions(
        ...
        query: String) -> [CarapaceChatSessionEntry]
    {
           ...
           !currentSessionKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        {
            ...
            entries.append(CarapaceChatSessionEntry.placeholder(key: currentSessionKey))
        }
        // Gateway, cached lists, iOS, and macOS must share the same pin
        // chronology, stable key ties, and searchable session fields.
        return CarapaceChatSessionListOrganizer.filter(
            CarapaceChatSessionListOrganizer.organize(entries),
            search: query)
    }
}
