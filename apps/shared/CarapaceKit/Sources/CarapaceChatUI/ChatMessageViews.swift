    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(self.notice.body)
                .font(CarapaceChatTypography.footnote)
...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(CarapaceChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? CarapaceChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? CarapaceChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
struct ChatMessageBubble: View {
    ...
    let displayOptions: CarapaceChatDisplayOptions
    ...
    let inlineWidgetResourceResolver: @MainActor @Sendable (
        String,
        CarapaceChatWidgetResource?) async -> CarapaceChatWidgetResource?
    ...
    let loadMediaArtifact: @MainActor @Sendable (
        String,
        CarapaceChatMediaKind,
        CarapaceChatPlaybackMode?) async throws -> CarapaceChatLoadedMedia?
...
private struct ChatMessageBody: View {
    @Environment(\.carapaceAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: CarapaceChatMessage
    let isUser: Bool
    let style: CarapaceChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser
            ? CarapaceChatTheme.userText(on: self.userAccent)
            : CarapaceChatTheme.assistantText
    ...
    private var toolActivityItems: [ChatToolActivityItem] {
        ...
        // (CarapaceChatView.mergeToolResults); this branch is the orphan
    ...
    private var inlineAttachments: [CarapaceChatMessageContent] {
        self.message.content.filter(\.isInlineAttachment)
    }

    private var visibleInlineAttachments: [CarapaceChatMessageContent] {
        var imageCount = 0
    ...
    private var inlineWidgets: [CarapaceChatCanvasPreview] {
    ...
    private var toolCalls: [CarapaceChatMessageContent] {
    ...
    private var inlineToolResults: [CarapaceChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func usageTint(_ pressure: ChatMessageUsagePresentation.Pressure) -> Color {
        switch pressure {
        case .normal:
            CarapaceChatTheme.muted
        case .warning:
            CarapaceChatTheme.warning
        case .danger:
            CarapaceChatTheme.danger
        }
    }

    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CarapaceChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CarapaceChatTheme.onboardingAssistantBubble
        }
        return CarapaceChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CarapaceChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CarapaceChatMessageContent
    let isUser: Bool
    let userAccent: Color?
    ...
    private var fallbackRow: some View {
        HStack(spacing: 8) {
            ...
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(carapaceVoiceNoteDurationLabel(durationSeconds))
                    .font(CarapaceChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? CarapaceChatTheme.userText(on: self.userAccent).opacity(0.72)
                            : CarapaceChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
struct ChatOutboxStatusLabel: View {
    let state: CarapaceChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(CarapaceChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct CarapaceAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var carapaceAssistantBubblesInCleanChrome: Bool {
        get { self[CarapaceAssistantBubblesInCleanChromeKey.self] }
        set { self[CarapaceAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.carapaceAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CarapaceChatPendingToolCall]
