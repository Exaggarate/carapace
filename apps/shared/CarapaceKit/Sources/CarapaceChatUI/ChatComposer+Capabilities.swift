...
extension CarapaceChatComposer {
    ...
    var cleanInlinePermissionMenu: some View {
        if self.viewModel.composerCapabilityControlsAvailable {
            Menu {
                ...
                ForEach(CarapaceChatPermissionMode.allCases, id: \.rawValue) { mode in
    ...
    private var cleanInlinePermissionGlyph: some View {
        ZStack {
            ...
            Image(systemName: self.cleanInlinePermissionOverlaySymbol)
                .font(CarapaceChatTypography.display(size: 7, weight: .bold, relativeTo: .caption2))
                .offset(y: -1)
        }
    ...
    private func permissionButton(
        _ mode: CarapaceChatPermissionMode?,
    ...
    private func composerConnectorMenu(_ connector: CarapaceChatComposerConnector) -> some View {
