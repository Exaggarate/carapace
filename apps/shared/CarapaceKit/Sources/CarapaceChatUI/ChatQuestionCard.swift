...
public enum CarapaceQuestionCardStatus: Sendable, Equatable {
    case pending
    case submitting
    case answered
...
@MainActor
@Observable
public final class CarapaceQuestionCardModel: Identifiable {
    ...
    public func status(at date: Date = Date()) -> CarapaceQuestionCardStatus {
    ...
    public func apply(resolved: CarapaceQuestionResolvedEvent) {
...
struct CarapaceQuestionCard: View {
    @Bindable private var model: CarapaceQuestionCardModel
    private let onSubmit: @MainActor @Sendable (CarapaceQuestionCardModel) async -> Void
    private let onSkip: (@MainActor @Sendable (CarapaceQuestionCardModel) async -> Void)?
    #if os(macOS)
    @FocusState private var focusedQuestionID: String?
    #endif

    init(
        model: CarapaceQuestionCardModel,
        onSubmit: @escaping @MainActor @Sendable (CarapaceQuestionCardModel) async -> Void,
        onSkip: @escaping @MainActor @Sendable (CarapaceQuestionCardModel) async -> Void)
    {
        self.model = model
    ...
    private var pendingCard: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .padding(16)
            .background(CarapaceChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private var terminalSummary: some View {
            ...
            ForEach(self.model.record.questions, id: \.questionid) { question in
                HStack(alignment: .firstTextBaseline, spacing: 5) {
                    Text(verbatim: "\(question.header):")
                        .font(CarapaceChatTypography.body(size: 14, weight: .semibold, relativeTo: .callout))
                    Text(self.model.terminalSummaryText(for: question))
                        .font(CarapaceChatTypography.body(size: 14, weight: .regular, relativeTo: .callout))
        ...
        .padding(.vertical, 9)
        .background(CarapaceChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 12))
    ...
    private func questionSection(_ question: Question, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(question.header.uppercased())
                ...
                .foregroundStyle(CarapaceChatTheme.accent)
            ...
            if question.options.isEmpty || question.isother == true {
                if question.issecret == true {
                    ...
                    {
                        Text("Secret value").font(CarapaceChatTypography.body)
                    }
                    .font(CarapaceChatTypography.body)
        ...
        now: Date) -> some View
    {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if let reason = store.reason, !reason.isEmpty {
                Text(reason).font(CarapaceChatTypography.body)
            }
            if let existing = question.secretstoreexisting {
                ...
                Text(String(format: String(localized: "Replaces %@ — last updated %@"), store.name, updated))
                    .font(CarapaceChatTypography.captionSemiBold)
                    .foregroundStyle(CarapaceChatTheme.danger)
            ...
            if protectedSecret {
                Text("Allowed HTTPS hosts").font(CarapaceChatTypography.captionSemiBold)
                TextField(text: self.$model.secretStoreAllowedHostsText, axis: .vertical) {
                    Text("api.example.com, uploads.example.com").font(CarapaceChatTypography.body)
                }
                .font(CarapaceChatTypography.body)
                .textFieldStyle(.roundedBorder)
                .autocorrectionDisabled()
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: selected
                    ...
                    .foregroundStyle(selected ? CarapaceChatTheme.accent : .secondary)
                VStack(alignment: .leading, spacing: 2) {
                    Text(option.label).font(CarapaceChatTypography.body)
...
@MainActor
struct CarapaceQuestionCards: View {
    let viewModel: CarapaceChatViewModel

    var body: some View {
        ForEach(self.viewModel.visibleQuestionCards) { card in
            CarapaceQuestionCard(model: card) { [weak viewModel = self.viewModel] model in
...
extension CarapaceChatViewModel {
    public var visibleQuestionCards: [CarapaceQuestionCardModel] {
    ...
    private func fetchMissingQuestionLookups(
        _ models: [CarapaceQuestionCardModel]) async
        -> [(CarapaceQuestionCardModel, QuestionLookupResult)]
    {
        ...
        var lookups: [(CarapaceQuestionCardModel, QuestionLookupResult)] = []
        for model in models {
            do {
    ...
    private func applyQuestionRefresh(
        ...
        lookups: [(CarapaceQuestionCardModel, QuestionLookupResult)]) -> QuestionRefreshApplyResult
    {
        var changed = false
        for record in records {
            if let model = self.questionCards.first(where: { $0.id == record.id }) {
                ...
                self.questionCards.append(CarapaceQuestionCardModel(record: record))
                changed = true
            }
        }
        var complete = true
    ...
    func resolveQuestionEvent(_ event: CarapaceQuestionResolvedEvent) {
    ...
    func submitQuestion(_ model: CarapaceQuestionCardModel) async {
    ...
    func skipQuestion(_ model: CarapaceQuestionCardModel) async {
    ...
    func expireQuestionIfNeeded(
        _ model: CarapaceQuestionCardModel,
        at date: Date = Date())
    {
