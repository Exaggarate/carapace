...
public struct CarapaceChatSpeechClip: Equatable, Sendable {
...
public typealias CarapaceChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> CarapaceChatSpeechClip
...
@MainActor
@Observable
public final class CarapaceChatSpeechController: ChatMediaPlaybackOwner {
    ...
    private let synthesize: CarapaceChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping CarapaceChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping CarapaceChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: CarapaceChatSpeechClip? = await {
            do {
