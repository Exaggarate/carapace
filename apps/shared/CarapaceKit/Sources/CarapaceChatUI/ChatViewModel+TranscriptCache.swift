...
extension CarapaceChatViewModel {
    ...
    func replaceMessages(_ messages: [CarapaceChatMessage]) {
    ...
    nonisolated static func durableSessionCacheProjection(
        _ session: CarapaceChatSessionEntry) -> CarapaceChatSessionEntry
    {
        ...
        canonicalMessageIdempotencyKeys: Set<String>)
    {
        ...
        let branchStateTask: Task<CarapaceChatOutboxBranchState?, Never>? = if let outbox, let scope {
    ...
    func persistSessionsToCache(_ sessions: [CarapaceChatSessionEntry], agentID: String?) {
    ...
    func paintFromCacheIfNeeded(session: SessionSnapshot) {
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                let organized = CarapaceChatSessionListOrganizer.organize(durableSessions)
    ...
    static func transcriptCacheAgentID(sessionKey: String, agentID: String?) -> String? {
        guard CarapaceChatSessionKey.agentID(from: sessionKey) == nil else { return nil }
