...
enum ChatToolActivity {
    static func items(
        calls: [CarapaceChatMessageContent],
        results: [CarapaceChatMessageContent]) -> [ChatToolActivityItem]
    {
...
private struct ChatToolActivityRowContent: View {
                    ...
                    if self.resolvedDiff != nil {
                        ...
                        if !self.formattedResult.isEmpty {
                            Text(self.formattedResult)
                                .font(CarapaceChatTypography.mono(size: 12, relativeTo: .footnote))
    ...
    private var collapsedRow: some View {
        HStack(alignment: .firstTextBaseline, spacing: 7) {
            ...
            Image(systemName: Self.symbol(forToolName: self.item.name))
                ...
                .foregroundStyle(self.item.isError ? CarapaceChatTheme.danger : Color.secondary)

            Text(self.display.title)
                .font(CarapaceChatTypography.footnoteSemiBold)
                .foregroundStyle(
                    self.item.isError ? CarapaceChatTheme.danger : CarapaceChatTheme.assistantText)
                .lineLimit(1)

            if let detailLine = self.detailLine {
                Text(detailLine)
                    .font(CarapaceChatTypography.mono(size: 12, relativeTo: .footnote))
                    .foregroundStyle(CarapaceChatTheme.assistantText.opacity(0.62))
                    .lineLimit(1)
                    .truncationMode(.tail)
            }
    ...
    private func diffRow(_ line: ChatToolDiffLine) -> some View {
        if line.kind == .file {
            Text(verbatim: String(line.text.unicodeScalars.prefix(2000)))
                .font(CarapaceChatTypography.mono(size: 11, relativeTo: .caption))
                .fontWeight(.semibold)
                .foregroundStyle(.secondary)
            ...
            HStack(spacing: 6) {
                if let lineNo = line.lineNo {
                    Text(verbatim: "\(lineNo)")
                        .font(CarapaceChatTypography.mono(size: 10, relativeTo: .caption2))
                        .foregroundStyle(.secondary.opacity(0.6))
    ...
    private func diffBackground(_ kind: ChatToolDiffLineKind) -> Color {
        switch kind {
        case .add:
            CarapaceChatTheme.success.opacity(0.14)
        case .del:
            CarapaceChatTheme.danger.opacity(0.12)
        case .ctx, .file, .skip:
            .clear
        }
    }
    ...
    var body: some View {
        Group {
            Text(verbatim: "+\(self.stat.added)")
                .foregroundStyle(CarapaceChatTheme.success.opacity(0.9))
            Text(verbatim: "−\(self.stat.removed)")
                .foregroundStyle(CarapaceChatTheme.danger.opacity(0.9))
        }
        .font(CarapaceChatTypography.mono(size: 12, relativeTo: .footnote))
        .lineLimit(1)
    }
