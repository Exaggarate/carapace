...
public enum CarapaceSessionColor: String, CaseIterable, Sendable {
...
public struct CarapaceSessionColorStripe: View {
    ...
    public var body: some View {
        if let color = CarapaceSessionColor(name: self.color) {
...
public struct CarapaceSessionColorDot: View {
    @Environment(\.colorScheme) private var colorScheme
    private let color: String?
...
public struct CarapaceSessionColorMenu: View {
    @Environment(\.colorScheme) private var colorScheme
    private let color: String?
    ...
    public var body: some View {
        Menu {
            Picker(selection: Binding(
                get: { CarapaceSessionColor(name: self.color)?.rawValue },
                set: { self.onSelect($0) }))
            {
                Text("Default")
                    .font(CarapaceChatTypography.body)
                    .tag(String?.none)
                ForEach(CarapaceSessionColor.allCases, id: \.self) { color in
                    Label {
                        Text(color.label)
                            .font(CarapaceChatTypography.body)
                    } icon: {
