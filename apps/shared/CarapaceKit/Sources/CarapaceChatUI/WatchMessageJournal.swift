...
public actor CarapaceWatchMessageJournal {
    struct ClaimIdentity: Equatable, Sendable {
        let commandId: String
        let fingerprint: Data
        let owner: CarapaceWatchMessageOwner
        let attemptVersion: Int64
        let expiresAtMs: Int64

        init?(_ entry: CarapaceWatchMessageEntry) throws {
            ...
            self.fingerprint = try CarapaceWatchMessageJournal.fingerprint(command)
            self.owner = owner
    ...
    private static let logger = Logger(subsystem: "ai.carapace", category: "WatchMessageJournal")

    init(queue: DatabaseQueue) {
        self.queue = queue
    }

    public func route(gatewayStableID: String) async throws -> CarapaceWatchMessageRoute? {
        try await self.queue.write { db in
            ...
            guard let row = try Row.fetchOne(
                ...
                let identity = CarapaceChatSessionRoutingIdentity(
                    scope: row["scope"],
            ...
            return CarapaceWatchMessageRoute(
                owner: CarapaceWatchMessageOwner(gatewayStableID: gatewayStableID, routeGeneration: generation),
    ...
    {
        try CarapaceWatchChatDeliveryCodec.validateCommand(command, nowMs: nowMs)
        ...
        let receiptJSON = try Self.encode(CarapaceWatchChatDeliveryReceipt(
        ...
        return try await self.queue.write { db in
            ...
            guard unexpired < CarapaceWatchChatDeliveryCodec.maxUnexpiredCommands,
                  pending < CarapaceWatchChatDeliveryCodec.maxPendingCommands
            else {
                throw CarapaceWatchChatDeliveryError(
    ...
    public func claim(_ command: CarapaceWatchChatDeliveryCommand, nowMs: Int64) async throws
        -> CarapaceWatchMessageEntry?
    {
        try await self.pruneExpired(nowMs: nowMs)
        return try await self.queue.write { db in
            ...
            guard let entry = try Self.fetch(db, id: command.commandId), entry.command == command,
                  entry.owner == CarapaceWatchMessageOwner(context: command.context), entry.phase == .queued,
                  entry.expiresAtMs == command.expiresAtMs, command.expiresAtMs > max(nowMs, Self.nowMs)
            else { return nil }
            do {
                try Self.requireContext(db, context: command.context, matchRoutingContract: true)
            } catch let error as CarapaceWatchChatDeliveryError where error.code == "routing_changed" {
    ...
    public func releaseNotDispatched(_ entry: CarapaceWatchMessageEntry) async throws -> CarapaceWatchMessageMutation {
    ...
    public func recordAccepted(_ entry: CarapaceWatchMessageEntry, runID: String) async throws
        -> CarapaceWatchMessageMutation
    {
        guard !runID.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw CarapaceWatchChatDeliveryError(code: "invalid_run", message: "The Gateway returned no run identity.")
        }
    ...
    public func recordTerminal(
        _ entry: CarapaceWatchMessageEntry,
        outcome: CarapaceWatchChatDeliveryOutcome,
        nowMs: Int64) async throws -> CarapaceWatchMessageMutation
    {
        let bounded: CarapaceWatchChatDeliveryOutcome
        if case let .reply(text) = outcome {
            let text = CarapaceWatchChatDeliveryCodec.boundedReplyText(text)
    ...
    public func acknowledge(_ ack: CarapaceWatchChatDeliveryReceiptAck) async throws -> CarapaceWatchMessageMutation {
        try CarapaceWatchChatDeliveryCodec.validateReceiptAck(ack)
        return try await self.queue.write { db in
    ...
    public func entries(owner: CarapaceWatchMessageOwner? = nil) async throws -> [CarapaceWatchMessageEntry] {
    ...
    public func resumableEntry(
        id: String, context: CarapaceWatchChatDeliveryContext) async throws -> CarapaceWatchMessageEntry?
    {
        try await self.pruneExpired(nowMs: Self.nowMs)
        return try await self.queue.read { db in
            guard let entry = try Self.fetch(db, id: id),
                  entry.owner == CarapaceWatchMessageOwner(context: context), entry.command?.context == context,
    ...
    public func changes(owner: CarapaceWatchMessageOwner? = nil) async throws
        -> some AsyncSequence<[CarapaceWatchMessageEntry], any Error> & Sendable
    {
        try await self.pruneExpired(nowMs: Self.nowMs)
    ...
    public func accepted(owner: CarapaceWatchMessageOwner) async throws -> [CarapaceWatchMessageEntry] {
        try await self.entries(in: .accepted, owner: owner)
    }

    public func pendingReceipts(owner: CarapaceWatchMessageOwner? = nil) async throws -> [CarapaceWatchMessageEntry] {
        let now = Self.nowMs
    ...
    public func discard(
        id: String,
        exactOwner: CarapaceWatchMessageOwner?) async throws -> CarapaceWatchMessageMutation
    {
        try await self.queue.write { db in
    ...
    private func entries(
        in phase: CarapaceWatchMessagePhase,
        owner: CarapaceWatchMessageOwner) async throws -> [CarapaceWatchMessageEntry]
    {
        let now = Self.nowMs
        return try await self.entries(owner: owner).filter {
    ...
    private func writeSettlement(
        ...
        updates: @escaping @Sendable (Database, CarapaceWatchMessageEntry) throws -> CarapaceWatchMessageMutation)
        async throws -> CarapaceWatchMessageMutation
    {
