...
extension CarapaceChatViewModel {
    static let baseThinkingLevelOptions: [CarapaceChatThinkingLevelOption] = [
        CarapaceChatThinkingLevelOption(id: "off", label: "off"),
        CarapaceChatThinkingLevelOption(id: "minimal", label: "minimal"),
    ...
    func performSelectThinkingLevel(_ level: String) {
        ...
        if acceptedThinkingLevelsByTarget[target] == nil {
            ...
            acceptedSettingsPatchResultsByTarget[target] = CarapaceChatModelPatchResult(
    ...
    private func mergedThinkingPatchSuccess(
        _ patchResult: CarapaceChatModelPatchResult?,
        acceptedLevel: String,
        target: ModelPatchTarget) -> CarapaceChatModelPatchResult
    {
    ...
    func updateCurrentSessionThinkingLevels(
        _ thinkingLevels: [CarapaceChatThinkingLevelOption],
        sessionKey: String,
    ...
    func inactiveSettingsStateKey(for target: ModelPatchTarget) -> String? {
        if target.agentID != nil || target.sessionRoutingContract != nil {
            guard CarapaceChatSessionKey.agentID(from: target.canonicalSessionKey) != nil else { return nil }
        }
        ...
        sessionRoutingContract: String? = nil) -> String
    {
        ...
        let session: CarapaceChatSessionEntry?
    ...
    private func thinkingLevelWithoutGatewayMetadata(
        _ level: String,
        target: ModelPatchTarget,
        session: CarapaceChatSessionEntry?) -> String?
    {
    ...
    private func thinkingPickerIsAvailable(
        ...
        modelChoice: CarapaceChatModelChoice?) -> Bool
    {
        let resolved = self.resolvedThinkingLevelOptions(for: session)
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [CarapaceChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: CarapaceChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: CarapaceChatSessionEntry?) -> CarapaceChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> CarapaceChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: CarapaceChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [CarapaceChatThinkingLevelOption]?) -> [CarapaceChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return CarapaceChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [CarapaceChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return CarapaceChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [CarapaceChatThinkingLevelOption],
        current: String) -> [CarapaceChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [CarapaceChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [CarapaceChatThinkingLevelOption]) -> [CarapaceChatThinkingLevelOption]
    {
        var result: [CarapaceChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    static func normalizedThinkingLevel(
        _ level: String?,
        options: [CarapaceChatThinkingLevelOption],
