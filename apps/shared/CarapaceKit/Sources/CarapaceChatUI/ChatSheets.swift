...
public struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [CarapaceChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: CarapaceChatSessionEntry?
    ...
    @State private var inspectedSession: CarapaceChatSessionEntry?
    @State private var isPresentingGroups = false

    public init(viewModel: CarapaceChatViewModel) {
    ...
    private var displayedSessions: [CarapaceChatSessionEntry] {
    ...
    public var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: CarapaceChatViewModel.sessionListFetchLimit)
            }
    ...
    private func interactiveSessionRow(_ session: CarapaceChatSessionEntry) -> some View {
        ...
        return Button {
            ...
            .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                    ...
                    mainSessionKey: self.viewModel.resolvedMainSessionKey)
                {
                    ...
                    .tint(session.isArchived ? CarapaceChatTheme.accent : CarapaceChatTheme.danger)
                }
            }
            .contextMenu {
                CarapaceSessionColorMenu(color: session.color) { color in
                    Task {
    ...
    private func sessionRowContent(_ session: CarapaceChatSessionEntry) -> some View {
        HStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                Text(session.displayName ?? session.key)
                    .font(CarapaceChatTypography.mono(size: 17, relativeTo: .body))
                    .lineLimit(1)
