...
public enum CarapaceChatSessionKey {
...
public enum CarapaceChatGatewayPayloadCodec {
    public static func decodeAgentsList(_ data: Data) throws -> CarapaceChatAgentsListResponse {
        let result = try JSONDecoder().decode(AgentsListResult.self, from: data)
        return CarapaceChatAgentsListResponse(
    ...
    public static func decodeProgressCard(_ data: Data, agentID: String?) throws -> ProgressCard? {
        ...
        if let agentID,
           CarapaceChatSessionKey.agentID(from: card.sessionkey)?.lowercased() != agentID.lowercased()
        {
            throw NSError(domain: "CarapaceChatTransport", code: 0, userInfo: [
    ...
    public static func decodeAgentWaitObservation(_ data: Data) throws -> CarapaceChatRunObservation {
        let decoded = try JSONDecoder().decode(AgentWaitResponse.self, from: data)
        return CarapaceChatRunObservation.fromWaitResponse(
    ...
    public static func decodeModelChoices(_ data: Data) throws -> [CarapaceChatModelChoice] {
        let decoded = try JSONDecoder().decode(ModelsListResult.self, from: data)
        return decoded.models.map(self.modelChoice)
    }

    public static func decodeChatMetadataModelChoices(_ data: Data) throws -> [CarapaceChatModelChoice] {
    ...
    public static func decodeSessionRoutingIdentity(_ data: Data) throws -> CarapaceChatSessionRoutingIdentity {
    ...
    public static func modelChoice(_ model: ModelChoice) -> CarapaceChatModelChoice {
        let name = model.name.trimmingCharacters(in: .whitespacesAndNewlines)
        return CarapaceChatModelChoice(
            modelID: model.id,
    ...
    public static func commandChoice(_ entry: CommandEntry) -> CarapaceChatCommandChoice {
        let sourceValue = (entry.source.value as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let source: CarapaceChatCommandChoice.Source = switch sourceValue {
        case "native":
            .command
        case "skill":
            .skill
        ...
        return CarapaceChatCommandChoice(
            id: id,
            name: entry.name,
            textAliases: aliases,
    ...
    public static func event(from frame: EventFrame) -> CarapaceChatTransportEvent? {
        switch frame.event {
        case "tick":
            return .tick
        ...
        case "sessions.changed":
            guard let payload = frame.payload,
                  let change = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: CarapaceChatSessionsChangedEvent.self)
            else { return nil }
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: CarapaceGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = frame.payload,
        ...
        case "session.message":
            guard let payload = frame.payload,
                  let message = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: CarapaceSessionMessageEventPayload.self)
            else { return nil }
                   ...
                   !messageID.isEmpty
            {
                ...
                return .sessionMessage(CarapaceSessionMessageEventPayload(
        ...
        case "agent":
            guard let payload = frame.payload,
                  let agent = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: CarapaceAgentEventPayload.self)
            else { return nil }
            return .agent(agent)
    ...
    private static func secondaryEvent(from frame: EventFrame) -> CarapaceChatTransportEvent? {
        guard let payload = frame.payload else { return nil }
        switch frame.event {
        case "task":
            return (try? GatewayPayloadDecoding.decode(payload, as: CarapaceChatTaskEvent.self))
                .map(CarapaceChatTransportEvent.task)
        case "question.requested":
            return (try? GatewayPayloadDecoding.decode(payload, as: QuestionRecord.self))
                .map(CarapaceChatTransportEvent.questionRequested)
        case "question.resolved":
            return (try? GatewayPayloadDecoding.decode(payload, as: CarapaceQuestionResolvedEvent.self))
                .map(CarapaceChatTransportEvent.questionResolved)
        default:
            return nil
        }
    }
