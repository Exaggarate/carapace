...
private let cacheLogger = Logger(subsystem: "ai.carapace", category: "CarapaceChatTranscriptCache")

final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<CarapaceChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<CarapaceChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<CarapaceChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: CarapaceChatOutboxChange) {
        self.lock.lock()
...
public actor CarapaceChatSQLiteTranscriptCache: CarapaceChatTranscriptCache,
    CarapaceChatCanonicalTranscriptMerging,
    CarapaceChatCommandOutbox
{
    ...
    private let databases: CarapaceClientDatabases
    ...
    init(databases: CarapaceClientDatabases, gatewayID: String) {
    ...
    public func loadSessions() async -> [CarapaceChatSessionEntry] {
    ...
    public func loadSessions(agentID: String?) async -> [CarapaceChatSessionEntry] {
        ...
        do {
            return try await self.databases.cacheQueue.write { db in
                try CarapaceClientDatabases.ensureAgentSessionCacheSchema(db)
                ...
                do {
                    return try rows.map { row in
                        ...
                        return try JSONDecoder().decode(CarapaceChatSessionEntry.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func loadTranscript(sessionKey: String) async -> [CarapaceChatMessage] {
        ...
        do {
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        let payload: String = row["payload_json"]
                        return try JSONDecoder().decode(CarapaceChatMessage.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func storeSessions(_ sessions: [CarapaceChatSessionEntry]) async {
        ...
        do {
            try await self.databases.cacheQueue.write { db in
                let rows = try Row.fetchAll(
                    ...
                    """,
                    arguments: [gatewayID, sessionKey, normalizedAgentID])
                var cached = rows.compactMap { row -> CarapaceChatMessage? in
                    let payload: String = row["payload_json"]
                    return try? JSONDecoder().decode(CarapaceChatMessage.self, from: Data(payload.utf8))
                }
    ...
    public func loadSessionRoutingIdentity() async -> CarapaceChatSessionRoutingIdentity? {
        guard !self.isRetired else { return nil }
        return self.databases.loadSessionRoutingIdentity(gatewayID: self.gatewayID)
    }

    public func storeSessionRoutingIdentity(_ identity: CarapaceChatSessionRoutingIdentity) async {
        guard !self.isRetired else { return }
        let gatewayID = self.gatewayID
        do {
...
extension CarapaceChatSQLiteTranscriptCache {
    ...
    private nonisolated static func replaceTranscript(
        ...
        sessionKey: String,
        agentID: String,
        messages: [CarapaceChatMessage]) throws
    {
...
extension CarapaceChatSQLiteTranscriptCache {
    ...
    public nonisolated func changes() -> AsyncStream<CarapaceChatOutboxChange> {
        self.storeChangeHub.stream()
    }

    public func enqueueCommand(_ command: CarapaceChatOutboxCommand) async -> Bool {
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let scope = CarapaceChatOutboxScope(
    ...
    public func loadCommands() async -> [CarapaceChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [CarapaceChatOutboxCommand]? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
    ...
    public func claimNextCommand() async -> CarapaceChatOutboxCommand? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> (
                CarapaceChatOutboxCommand?,
                [CarapaceChatOutboxScope]) in
                try Self.applyOutboxStaleness(db, gatewayID: gatewayID)
        ...
        replacementID: String?) async -> CarapaceChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == CarapaceChatOutboxCommand
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let retryScope = CarapaceChatOutboxScope(
    ...
    public func cancelCommand(id: String) async -> CarapaceChatOutboxUpdateResult {
        guard !self.isRetired else { return .unavailable }
    ...
    public func branchState(
        for scope: CarapaceChatOutboxScope) async -> CarapaceChatOutboxBranchState?
    {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                state = try CarapaceChatOutboxBranchState(
                    epoch: state.epoch,
    ...
    public func beginBranchSwitch(_ scope: CarapaceChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func cancelBranchSwitch(_ scope: CarapaceChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func demoteBranchSwitchToReconcile(_ scope: CarapaceChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
            let changed = try await databases.stateQueue.write { db -> Bool in
        ...
        lastError: String) async -> [CarapaceChatOutboxCommand]?
    {
        ...
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> ([CarapaceChatOutboxCommand], Bool) in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
...
extension CarapaceChatSQLiteTranscriptCache {
    private func transitionClaimedCommand(
        id: String,
        attemptVersion: Int,
        status: CarapaceChatOutboxCommand.Status,
        retryCount: Int,
        lastError: String?) async -> CarapaceChatOutboxUpdateResult
    {
    ...
    private nonisolated static func readCommands(
        _ db: Database,
        gatewayID: String) throws -> [CarapaceChatOutboxCommand]
    {
        let rows = try Row.fetchAll(
            db,
            sql: """
        ...
        gatewayID: String) throws -> CarapaceChatOutboxCommand
    {
        ...
        guard let status = CarapaceChatOutboxCommand.Status(rawValue: statusRaw) else {
            throw DatabaseError(message: "unknown outbox status")
        }
        let lastError: String = row["last_error"]
        let expectedSettingsJSON: String? = row["expected_settings_json"]
        return try CarapaceChatOutboxCommand(
            id: id,
    ...
    private nonisolated static func ensureBranchScope(
        _ db: Database,
        gatewayID: String,
        scope: CarapaceChatOutboxScope) throws
    {
        try db.execute(
            sql: """
    ...
    private nonisolated static func readBranchState(
        _ db: Database,
        gatewayID: String,
        scope: CarapaceChatOutboxScope) throws -> CarapaceChatOutboxBranchState
    {
        guard let row = try Row.fetchOne(
            db,
            sql: """
        ...
        return CarapaceChatOutboxBranchState(
    ...
    private nonisolated static func expireBranchSwitchLeases(
        _ db: Database,
        gatewayID: String,
        scope: CarapaceChatOutboxScope? = nil) throws -> [CarapaceChatOutboxScope]
    {
        ...
        return rows.map { row in
            CarapaceChatOutboxScope(sessionKey: row["session_key"], agentID: row["agent_id"])
        }
    }
...
extension CarapaceChatSQLiteTranscriptCache {
    static func boundedSessions(_ sessions: [CarapaceChatSessionEntry]) -> [CarapaceChatSessionEntry] {
    ...
    private nonisolated static func sessionsOwnedBy(
        ...
        agentID: String) -> [CarapaceChatSessionEntry]?
    {
        guard !agentID.isEmpty else { return sessions }
        var owned: [CarapaceChatSessionEntry] = []
        owned.reserveCapacity(sessions.count)
        for var session in sessions {
            let rowAgentID = self.normalizedAgentID(session.agentId)
            let keyAgentID = self.normalizedAgentID(CarapaceChatSessionKey.agentID(from: session.key))
    ...
    private static func attachmentByteCount(_ attachments: [CarapaceChatOutboxAttachment]) -> Int? {
        var total = 0
