...
enum CarapaceChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct CarapaceChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: CarapaceChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<CarapaceChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: CarapaceChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<CarapaceChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum CarapaceChatWidgetURLResolver {
    private static let documentsPath = "/__carapace__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: CarapaceChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__carapace__",
    ...
    private static func resolve(
        ...
        role: CarapaceChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<CarapaceChatWidgetSurfaceRole>) -> CarapaceChatWidgetResource?
    {
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: CarapaceChatWidgetResource?,
        blockedRoles: Set<CarapaceChatWidgetSurfaceRole>,
        attemptedRoles: Set<CarapaceChatWidgetSurfaceRole>) -> CarapaceChatWidgetResource?
    {
        [
            (role: CarapaceChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: CarapaceChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: CarapaceChatWidgetResource,
        for failedResource: CarapaceChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            if let title = self.preview.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
                Text(title)
                    ...
                    .fontWeight(.semibold)
                    .foregroundStyle(CarapaceChatTheme.muted)
            }
    ...
    #if canImport(WebKit) && (os(iOS) || os(macOS))
    private func renderedWidget(resource: CarapaceChatWidgetResource) -> some View {
        ChatInlineWidgetWebView(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(CarapaceChatTheme.muted.opacity(0.24), lineWidth: 1)
            }
    ...
    private func setResolvedResource(_ resource: CarapaceChatWidgetResource?) {
        #if canImport(WebKit) && (os(iOS) || os(macOS))
    ...
    private func handleLoadFailure(resource: CarapaceChatWidgetResource) {
...
extension CarapaceChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: CarapaceChatWidgetResource {
        didSet {
    ...
    init(
        resource: CarapaceChatWidgetResource,
