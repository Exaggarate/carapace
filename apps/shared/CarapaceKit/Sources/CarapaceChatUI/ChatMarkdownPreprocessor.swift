...
enum ChatMarkdownPreprocessor {
    ...
    private static let inboundContextMarker = "\u{27E6}carapace:ctx\u{27E7}"
    ...
    struct InlineImage: Identifiable {
        let id = UUID()
        let label: String
        let image: CarapacePlatformImage?
    }
    ...
    private static func inlineImage(label: String, source: String) -> InlineImage? {
        ...
        let image = Data(base64Encoded: b64).flatMap(CarapacePlatformImage.init(data:))
