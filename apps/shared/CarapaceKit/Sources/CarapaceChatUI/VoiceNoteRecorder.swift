...
public struct CarapaceVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class CarapaceVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: CarapaceVoiceNoteRecording)
        case staging(recording: CarapaceVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: CarapaceVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = CarapaceVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: CarapaceVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> CarapaceVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: CarapaceVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> CarapaceVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = CarapaceVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class CarapaceVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "CarapaceVoiceNoteAudioCapture",
                    code: 1,
...
func carapaceVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
