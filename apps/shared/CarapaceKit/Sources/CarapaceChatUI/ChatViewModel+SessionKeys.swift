...
extension CarapaceChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: CarapaceChatSessionEntry?) -> Double? {
    ...
    public func currentSessionEntry() -> CarapaceChatSessionEntry? {
    ...
    func sessionMutationIdentity(for key: String, listedKey: String? = nil) -> String {
        ...
        return self.modelPatchTarget(
            ...
            agentID: CarapaceChatSessionKey.agentID(from: key) ?? self.activeAgentId,
            sessionRoutingContract: nil).canonicalSessionKey
    }

    func applyingLocalUnreadOverrides(
        to sessions: [CarapaceChatSessionEntry]) -> [CarapaceChatSessionEntry]
    {
        ...
        sessionRoutingContract: String?) -> ModelPatchTarget
    {
        ...
        if CarapaceChatSessionKey.agentID(from: candidate) != nil || normalizedCandidate == "unknown" {
            targetKey = candidate
            targetAgentID = nil
    ...
    func successfulModelPatchResult(
        ...
        session: CarapaceChatSessionEntry?) -> CarapaceChatModelPatchResult?
    {
        guard let session,
        ...
        syncSelection: Bool)
    {
        ...
        var updated = existingIndex.map { self.sessions[$0] } ?? CarapaceChatSessionEntry.placeholder(key: sessionKey)
    ...
    public var sessionChoices: [CarapaceChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = CarapaceChatSessionListOrganizer.organize(sessions)
        let mainSessionKey = resolvedMainSessionKey

        var result: [CarapaceChatSessionEntry] = []
        ...
        if let main = sorted.first(where: { $0.key == mainSessionKey }) {
            ...
            result.append(CarapaceChatSessionEntry.placeholder(key: mainSessionKey))
        ...
        if !included.contains(sessionKey) {
            if let current = sorted.first(where: { $0.key == self.sessionKey }) {
                ...
                result.append(CarapaceChatSessionEntry.placeholder(key: sessionKey))
            }
        }
