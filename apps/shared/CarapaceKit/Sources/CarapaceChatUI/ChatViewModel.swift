...
let chatUILogger = Logger(subsystem: "ai.carapace", category: "CarapaceChatUI")

@MainActor
@Observable
public final class CarapaceChatViewModel {
    ...
    public internal(set) var messages: [CarapaceChatMessage] = []
    ...
    public internal(set) var replyTarget: CarapaceChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [CarapaceChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public internal(set) var modelChoices: [CarapaceChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [CarapaceChatCommandChoice] = []
    ...
    public var attachments: [CarapacePendingAttachment] = []
    ...
    public internal(set) var sessionBranches: [CarapaceChatSessionBranch] = []
    ...
    public internal(set) var questionCards: [CarapaceQuestionCardModel] = []
    ...
    @ObservationIgnored
    var preparedProgressCardTarget: (session: SessionSnapshot, generation: UInt64, target: CarapaceChatSessionTarget)?
    @ObservationIgnored
    ...
    public private(set) var pendingToolCalls: [CarapaceChatPendingToolCall] = []
    ...
    public internal(set) var sessions: [CarapaceChatSessionEntry] = [] {
        didSet {
            syncContextUsageFraction()
            syncActiveSessionRunIDsFromCurrentSession()
        }
    ...
    public internal(set) var swarmSessions: [CarapaceChatSessionEntry] = []
    var activeSwarmGroups: [CarapaceChatSwarmGroup] = []
    var swarmActivityState = CarapaceChatSwarmActivityState()
    @ObservationIgnored
    ...
    let composerCapabilityState = CarapaceChatComposerCapabilityState()
    ...
    let transport: any CarapaceChatTransport
    let haptics: CarapaceChatHaptics
    let transcriptCache: (any CarapaceChatTranscriptCache)?
    let outbox: (any CarapaceChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: CarapaceChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    @ObservationIgnored
    var reconciledOutboxBranchScopes: Set<CarapaceChatOutboxScope> = []
    @ObservationIgnored
    var reconcilingOutboxBranchScopes: Set<CarapaceChatOutboxScope> = []
    @ObservationIgnored
    var outboxBranchReconcileRetryAttempts: [CarapaceChatOutboxScope: Int] = [:]
    @ObservationIgnored
    var outboxBranchReconcileRetryTasks: [CarapaceChatOutboxScope: Task<Void, Never>] = [:]
    @ObservationIgnored
    ...
    var bootstrapOutboxBranchStateCapture: (
        ...
        task: Task<CarapaceChatOutboxBranchState?, Never>)?
    @ObservationIgnored
    ...
    var sessionDefaults: CarapaceChatSessionsDefaults? {
    ...
    let onToolActivity: CarapaceChatToolActivityHandler?
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: CarapaceChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: CarapaceChatModelPatchResult] = [:]
    ...
    enum ToolOverridesState {
        case none
        case value(CarapaceChatSessionToolOverrides)

        var overrides: CarapaceChatSessionToolOverrides? {
    ...
    struct FastModeState {
        let override: CarapaceChatFastMode?
        let effective: CarapaceChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<CarapaceChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: CarapaceChatMessage]
    ...
    var pendingToolCallsById: [String: CarapaceChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: CarapaceChatHaptics = CarapaceChatHaptics(),
        ...
        onToolActivity: CarapaceChatToolActivityHandler? = nil,
    ...
    private var usesMutableAgentRouting: Bool {
        CarapaceChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        if CarapaceChatSessionKey.agentID(from: sessionKey) == nil {
            return true
        }
        ...
        let contractMainKey = CarapaceChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
...
extension CarapaceChatViewModel {
    ...
    func appendMessage(_ message: CarapaceChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: CarapaceChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    private func bootstrap(context: BootstrapContext) async {
        ...
        do {
            ...
            if self.errorText != CarapaceChatTransportUpgradeMessage.progressCardAgentScope {
                self.errorText = nil
            }
        } catch {
            guard self.isCurrentBootstrap(context) else { return }
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: CarapaceChatSessionsListResponse
            do {
            ...
            let organized = CarapaceChatSessionListOrganizer.organize(res.sessions)
    ...
    private func performSelectModel(
        ...
        routeLease: CarapaceChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw CarapaceChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: CarapaceChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (CarapaceChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func generatedNewSessionKey(agentID explicitAgentID: String? = nil) -> String {
        ...
        guard let agentID = explicitAgentID ??
            CarapaceChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            CarapaceChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ CarapaceChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: CarapaceChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: CarapaceChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = CarapaceChatModelPatchResult(
