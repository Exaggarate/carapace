...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.carapace", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking CarapaceKit window.
    @discardableResult
...
extension GatewayNodeSession {
    ...
    private func decodeInvokeRequest(from payload: CarapaceProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: CarapaceProtocol.AnyCodable) throws -> T {
        do {
