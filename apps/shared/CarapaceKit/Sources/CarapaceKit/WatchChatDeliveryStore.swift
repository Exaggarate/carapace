...
public actor CarapaceWatchChatDeliveryStore {
    ...
    private var connection: CarapaceNativeStateSQLite?

    public init(databaseURL: URL = URL.applicationSupportDirectory
        .appendingPathComponent("Carapace/watch-chat-delivery.sqlite"))
    {
        self.databaseURL = databaseURL
    }

    public func enqueue(_ command: CarapaceWatchChatDeliveryCommand, nowMs: Int64) throws {
        ...
        try database.withImmediateTransaction {
            ...
            guard try count.step() == .row,
                  count.int64(at: 0) < Int64(CarapaceWatchChatDeliveryCodec.maxUnexpiredCommands),
                  count.int64(at: 1) < Int64(CarapaceWatchChatDeliveryCodec.maxPendingCommands)
            else {
    ...
    public func pendingCommands(nowMs: Int64) throws -> [CarapaceWatchChatDeliveryCommand] {
        let database = try self.database()
    ...
    public func isPending(_ command: CarapaceWatchChatDeliveryCommand, nowMs: Int64) throws -> Bool {
    ...
    public func record(
        ...
        nowMs: Int64) throws -> CarapaceWatchChatDeliveryReceiptAck?
    {
        ...
        return try database.withImmediateTransaction {
            ...
            if case .rejected = receipt.state {
                ...
                try CarapaceWatchChatDeliveryCodec.validateCommand(existing.command, nowMs: nowMs)
    ...
    public func receipt(context: CarapaceWatchChatDeliveryContext, commandId: String) throws
    ...
    private static func acknowledgment(
        _ receipt: CarapaceWatchChatDeliveryReceipt) -> CarapaceWatchChatDeliveryReceiptAck?
    {
        receipt.terminal.map {
            CarapaceWatchChatDeliveryReceiptAck(
    ...
    private static func json(_ value: some Encodable) throws -> String {
        ...
        guard let json = String(bytes: data, encoding: .utf8) else {
            throw CarapaceNativeStateError("Could not encode the Watch delivery record")
        }
        return json
    }
    ...
    private func entry(
        _ gatewayID: String,
        _ commandID: String,
        database: CarapaceNativeStateSQLite) throws -> Entry?
    {
        let query = try database.prepare("""
    ...
    private static func read(_ query: CarapaceNativeStateSQLiteStatement) throws -> [Entry] {
        var entries: [Entry] = []
        while try query.step() == .row {
            ...
            try CarapaceWatchChatDeliveryCodec.validateCommandStructure(command)
    ...
    private func database() throws -> CarapaceNativeStateSQLite {
        if let connection { return connection }
        let database = try CarapaceNativeStateSQLite(databaseURL: self.databaseURL)
        try database.withImmediateTransaction {
            ...
            case 0:
                ...
                else { throw CarapaceNativeStateError("Unrecognized Watch delivery database") }
            ...
            case 1:
                break
            default:
                throw CarapaceNativeStateError("Update Carapace to open this Watch delivery database")
            }
        }
        guard try database.scalarText("PRAGMA journal_mode = WAL") == "wal" else {
            throw CarapaceNativeStateError("Could not enable the Watch delivery journal")
        }
    ...
    private static func conflict() -> CarapaceWatchChatDeliveryError {
