...
enum DeviceIdentityPaths {
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(CarapaceAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("carapace", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps CARAPACE_STATE_DIR.
    ...
    static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("Carapace", isDirectory: true)
        }
        return nil
    }
    ...
    static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: CarapaceAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("Carapace", isDirectory: true)
    }
...
public enum DeviceIdentityStore {
    ...
    public static func loadOrCreate(profile: GatewayDeviceIdentityProfile) -> DeviceIdentity {
        do {
            ...
            preconditionFailure("Could not persist the Carapace device identity: \(error.localizedDescription)")
        }
    }
