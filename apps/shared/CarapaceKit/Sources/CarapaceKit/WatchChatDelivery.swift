...
public struct CarapaceWatchChatDeliveryContext: Codable, Sendable, Hashable {
...
public enum CarapaceWatchChatDeliveryKind: String, Codable, Sendable {
    case chat
    case quickReply
}

public enum CarapaceWatchChatDeliveryBody: Codable, Sendable, Equatable {
    case chat(text: String)
    case quickReply(promptId: String, actionId: String, actionLabel: String?, note: String?)

    public var kind: CarapaceWatchChatDeliveryKind {
        switch self {
        case .chat: .chat
...
public struct CarapaceWatchChatDeliveryCommand: Codable, Sendable, Equatable {
    public let type: CarapaceWatchPayloadType
    public let context: CarapaceWatchChatDeliveryContext
    public let commandId: String
    public let submittedAtMs: Int64
    public let body: CarapaceWatchChatDeliveryBody

    public init(
        context: CarapaceWatchChatDeliveryContext,
        commandId: String,
        submittedAtMs: Int64,
        body: CarapaceWatchChatDeliveryBody)
    {
    ...
    public var expiresAtMs: Int64 {
        let sum = self.submittedAtMs.addingReportingOverflow(CarapaceWatchChatDeliveryCodec.lifetimeMs)
...
public enum CarapaceWatchChatDeliveryOutcome: Codable, Sendable, Equatable {
...
public struct CarapaceWatchChatDeliveryTerminal: Codable, Sendable, Equatable {
    public let receiptId: String
    public let outcome: CarapaceWatchChatDeliveryOutcome
...
public struct CarapaceWatchChatDeliveryReceipt: Codable, Sendable, Equatable {
    public enum State: Codable, Sendable, Equatable {
        case admitted(atMs: Int64)
        case terminal(CarapaceWatchChatDeliveryTerminal)
    ...
    public init(context: CarapaceWatchChatDeliveryContext, commandId: String, state: State) {
    ...
    public var terminal: CarapaceWatchChatDeliveryTerminal? {
    ...
    public var outcome: CarapaceWatchChatDeliveryOutcome? {
        switch self.state {
        case .admitted: nil
...
public struct CarapaceWatchChatDeliveryReceiptAck: Codable, Sendable, Equatable {
    public let type: CarapaceWatchPayloadType
    public let context: CarapaceWatchChatDeliveryContext
    public let commandId: String
    public let receiptId: String

    public init(context: CarapaceWatchChatDeliveryContext, commandId: String, receiptId: String) {
...
public struct CarapaceWatchChatDeliveryError: Error, LocalizedError, Sendable, Equatable {
...
public enum CarapaceWatchChatDeliveryCodec {
    ...
    public static func decodeContext(_ payload: [String: Any]) throws -> CarapaceWatchChatDeliveryContext {
        let context: CarapaceWatchChatDeliveryContext = try self.decode(payload)
        try self.validateContext(context)
        return context
    }

    public static func decodeCommandStructure(_ payload: [String: Any]) throws -> CarapaceWatchChatDeliveryCommand {
        let command: CarapaceWatchChatDeliveryCommand = try self.decode(payload)
        try self.validateCommandStructure(command)
        return command
    }

    public static func decodeReceipt(_ payload: [String: Any]) throws -> CarapaceWatchChatDeliveryReceipt {
        let receipt: CarapaceWatchChatDeliveryReceipt = try self.decode(payload)
        try self.validateReceipt(receipt)
        return receipt
    }

    public static func decodeReceiptAck(_ payload: [String: Any]) throws -> CarapaceWatchChatDeliveryReceiptAck {
        let ack: CarapaceWatchChatDeliveryReceiptAck = try self.decode(payload)
        try self.validateReceiptAck(ack)
        return ack
    }

    public static func validateReceiptAck(_ ack: CarapaceWatchChatDeliveryReceiptAck) throws {
    ...
    public static func validateContext(_ context: CarapaceWatchChatDeliveryContext) throws {
        guard context.version == 1 else {
            throw CarapaceWatchChatDeliveryError(
                code: "upgrade_required",
                message: String(localized: "Update Carapace on iPhone and Watch."))
        }
    ...
    public static func validateCommand(_ command: CarapaceWatchChatDeliveryCommand, nowMs: Int64) throws {
        try self.validateCommandStructure(command)
        guard nowMs >= 0,
    ...
    public static func validateCommandStructure(_ command: CarapaceWatchChatDeliveryCommand) throws {
    ...
    public static func validateReceipt(_ receipt: CarapaceWatchChatDeliveryReceipt) throws {
    ...
    private static func invalidPayload() -> CarapaceWatchChatDeliveryError {
        CarapaceWatchChatDeliveryError(
            code: "invalid_payload", message: String(localized: "Invalid Watch chat delivery message."))
    }

    private static func tooLarge() -> CarapaceWatchChatDeliveryError {
        CarapaceWatchChatDeliveryError(
            code: "too_large",
