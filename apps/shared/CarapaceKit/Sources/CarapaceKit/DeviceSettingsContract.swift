...
public enum DeviceSettingsLocationMode: String, CaseIterable, Encodable, Sendable {
    ...
    public init(_ mode: CarapaceLocationMode) {
        switch mode {
        case .off: self = .off
    ...
    public var nativeMode: CarapaceLocationMode {
        switch self {
        case .off: .off
...
public struct DeviceSettingsSnapshot: Encodable, Sendable {
    ...
    public func javaScript() throws -> String {
        ...
        return "window.__CARAPACE_NATIVE_DEVICE_SETTINGS__ = \(json); " +
            "window.dispatchEvent(new CustomEvent('carapace:native-device-settings-changed', " +
            "{detail: window.__CARAPACE_NATIVE_DEVICE_SETTINGS__}));"
    }
}
