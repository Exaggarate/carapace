...
struct GatewayNodeInvocationRegistry {
    ...
    mutating func register(requestID: String, command: String, admissionGeneration: UInt64) -> UUID? {
        let waitsForRouteTeardown = command == CarapaceComputerCommand.act.rawValue ||
            command == CarapaceCameraCommand.ptzControl.rawValue ||
            CarapaceTalkCommand(rawValue: command) != nil
        guard waitsForRouteTeardown || command == CarapaceSystemCommand.notify.rawValue ||
            command == CarapaceChatCommand.push.rawValue || command == CarapaceWatchCommand.notify.rawValue
        else { return nil }
        let id = UUID()
