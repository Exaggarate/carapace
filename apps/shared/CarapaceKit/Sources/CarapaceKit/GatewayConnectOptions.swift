...
public enum CarapaceGatewayClientCapability {
...
extension GatewayConnectOptions {
    ...
    func applyOptionalConnectParams(to params: inout [String: CarapaceProtocol.AnyCodable]) {
        if !self.commands.isEmpty {
            params["commands"] = CarapaceProtocol.AnyCodable(self.commands)
        }
        ...
        if let pathEnv = self.pathEnv?.trimmingCharacters(in: .whitespacesAndNewlines),
           !pathEnv.isEmpty
        {
            params["pathEnv"] = CarapaceProtocol.AnyCodable(pathEnv)
        }
        if !self.permissions.isEmpty {
            params["permissions"] = CarapaceProtocol.AnyCodable(self.permissions)
        }
    }
}
