...
public final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "carapace_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.carapacefoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable], lifecycleGeneration: UInt64) async {
        ...
        do {
            ...
            let result: [String: AnyCodable] = completion.failed
                ? ["error": AnyCodable("Carapace tool call failed")]
                : ["text": AnyCodable(completion.text ?? "Carapace finished with no text.")]
