            ...
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        ...
                        fallback: "https://github.com/Exaggarate/carapace"),
        ...
        case .authTokenNotConfigured:
            self.problem(
                .init(
                    ...
                    actionCommand: authError.actionCommand
                        ?? "carapace config set gateway.auth.token <new-token>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://github.com/Exaggarate/carapace"),
                    requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            self.problem(
                .init(
                    ...
                    actionLabel: authError.actionLabel ?? "Fix on gateway",
                    actionCommand: authError.actionCommand
                        ?? "carapace config set gateway.auth.password <new-password>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://github.com/Exaggarate/carapace"),
                    requestId: authError.requestId,
...
extension GatewayConnectionProblemMapper {
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "Carapace could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                messagePresentation: .localizedFormat(
                    "Carapace could not read the gateway certificate for %@.",
                    [failure.host]),
                actionCommand: nil,
                docsURL: URL(string: "https://github.com/Exaggarate/carapace"),
                retryable: true,
        ...
        case .pinStorageUnavailable:
            return GatewayConnectionProblem(
                ...
                owner: .unknown,
                title: "Gateway certificate unavailable",
                message: "Carapace could not securely save the TLS certificate pin for \(failure.host).",
                actionLabel: "Retry",
                titlePresentation: .localized("Gateway certificate unavailable"),
                messagePresentation: .localizedFormat(
                    "Carapace could not securely save the TLS certificate pin for %@.",
                    [failure.host]),
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            (
                ...
                "Carapace could not reach the gateway over the current network.",
                "Check network")
        case .websocketCancelled:
    ...
    private static func protocolMismatchProblem(for authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
           ...
           clientMax < expected
        {
            ...
            message = authError.userMessageOverride
                ?? "This app is older than the gateway. Update Carapace on this device, then retry."
            owner = .iphone
                  ...
                  clientMin > expected
        {
            ...
            message = authError.userMessageOverride
                ?? "The gateway is older than this app. Update Carapace on the gateway host, then retry."
            owner = .gateway
            actionLabel = authError.actionLabel ?? "Copy update command"
            actionCommand = authError.actionCommand ?? "carapace update"
        } else {
            title = authError.titleOverride ?? "Carapace update required"
            message = authError.userMessageOverride
                ?? "The app and gateway use incompatible protocol versions. Update Carapace on both, then retry."
            owner = .both
            actionLabel = authError.actionLabel ?? "Update Carapace"
            actionCommand = authError.actionCommand
        }
        return self.problem(
            .init(
    ...
    private static func approvalCommand(requestId: String?) -> String {
        self.nonEmpty(requestId).map { "carapace devices approve \($0)" }
