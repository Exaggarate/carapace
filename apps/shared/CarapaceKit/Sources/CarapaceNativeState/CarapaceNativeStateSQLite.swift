...
public struct CarapaceNativeStateError: Error, LocalizedError, Sendable {
...
public enum CarapaceNativeStateCanonicalTable: Sendable {
...
public enum CarapaceNativeStateSQLiteStep: Equatable, Sendable {
    case row
    case done
}

public enum CarapaceNativeStateSQLiteValueType: Equatable, Sendable {
    case integer
    case float
    case text
    case blob
...
public final class CarapaceNativeStateSQLite: @unchecked Sendable {
    // Keep aligned with CARAPACE_STATE_SCHEMA_VERSION. Native clients never upgrade this database.
    ...
    private static let canonicalTables = [
        CarapaceNativeStateSQLite.deviceAuthTokens,
        CarapaceNativeStateSQLite.deviceIdentities,
        CarapaceNativeStateSQLite.execApprovalsConfig,
        CarapaceNativeStateSQLite.macosPortGuardianRecords,
    ]
        ...
        createIfMissing: Bool = true) throws
    {
        ...
        guard result == SQLITE_OK, let database else {
            ...
            throw CarapaceNativeStateError("Could not open native state database: \(detail)")
        }
    ...
    public func ensureCanonicalTable(
        _ table: CarapaceNativeStateCanonicalTable,
        allowVersionZeroCreation: Bool = true) throws
    {
        try self.withConnectionLock {
            ...
            guard userVersion <= Self.maximumSupportedSchemaVersion else {
                throw CarapaceNativeStateError(
            ...
            if userVersion == 0 {
                ...
                guard try self.schemaObjectExists(type: "table", name: descriptor.name) else {
                    throw CarapaceNativeStateError(
                        "Versioned Carapace state database is missing \(descriptor.name)")
                }
            }
    ...
    public func validateCanonicalTable(_ table: CarapaceNativeStateCanonicalTable) throws {
        try self.withConnectionLock {
            try self.validateTableShape(Self.descriptor(table))
        }
    }

    public func prepare(_ sql: String) throws -> CarapaceNativeStateSQLiteStatement {
        try self.withConnectionLock {
            ...
            return CarapaceNativeStateSQLiteStatement(connection: self, statement: statement)
        }
    }
    ...
    public func execute(_ sql: String) throws {
        try self.withConnectionLock {
            ...
            guard result == SQLITE_OK else {
                ...
                throw CarapaceNativeStateError("SQLite operation failed: \(detail)")
            }
        }
    }
    ...
    public func scalarInt64(_ sql: String) throws -> Int64 {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .row,
                ...
                throw CarapaceNativeStateError("SQLite integer query did not return one integer row")
            }
            let value = statement.int64(at: 0)
            guard try statement.step() == .done else {
                throw CarapaceNativeStateError("SQLite integer query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func scalarText(_ sql: String) throws -> String? {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw CarapaceNativeStateError("SQLite text query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func schemaObjectExists(type: String, name: String) throws -> Bool {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw CarapaceNativeStateError("SQLite schema query returned multiple rows")
            }
            return true
        }
    }

    fileprivate func databaseError(operation: String) -> CarapaceNativeStateError {
        CarapaceNativeStateError(
    ...
    private static func descriptor(_ table: CarapaceNativeStateCanonicalTable) -> CanonicalTable {
        switch table {
    ...
    private func validateVersionZeroOwnership() throws {
        ...
        guard objects.isSubset(of: allowedObjects) else {
            throw CarapaceNativeStateError(
                "Schema version zero database contains objects not owned by a native Carapace store")
        }
    ...
    private func validateSharedDatabaseMetadata(userVersion: Int64) throws {
        guard try self.schemaObjectExists(type: "table", name: "schema_meta") else {
            throw CarapaceNativeStateError("Versioned Carapace state database is missing schema_meta")
        }
        let statement = try self.prepare(
        ...
        guard try statement.step() == .row,
            ...
            throw CarapaceNativeStateError(
                "Carapace state database schema metadata does not match its global schema version")
        }
    }

    private func validateTableShape(_ table: CanonicalTable) throws {
        guard try self.tableColumns(table.name) == table.columns else {
            throw CarapaceNativeStateError("\(table.name) has an incompatible schema")
        }
        ...
        guard normalizedTableSQL.hasSuffix(") STRICT") else {
            throw CarapaceNativeStateError("\(table.name) must be a STRICT table")
        }
        guard try self.validRequiredIndex(table) else {
            throw CarapaceNativeStateError("\(table.indexName ?? table.name) has an incompatible index schema")
        }
    }
    ...
    static func secureDatabaseFiles(
        ...
        setAttributes: ([FileAttributeKey: Any], String) throws -> Void = {
            try CarapaceNativeStateSQLite.setPrivateAttributes($0, atPath: $1)
        }) throws
    {
...
public final class CarapaceNativeStateSQLiteStatement {
    private let connection: CarapaceNativeStateSQLite
    private let statement: OpaquePointer

    fileprivate init(connection: CarapaceNativeStateSQLite, statement: OpaquePointer) {
    ...
    public func step() throws -> CarapaceNativeStateSQLiteStep {
    ...
    public func valueType(at column: Int32) -> CarapaceNativeStateSQLiteValueType {
    ...
    public func requiredText(at column: Int32, field: String) throws -> String {
        try self.connection.withConnectionLock {
            guard self.valueType(at: column) == .text,
                ...
                throw CarapaceNativeStateError("SQLite \(field) must be text")
            }
