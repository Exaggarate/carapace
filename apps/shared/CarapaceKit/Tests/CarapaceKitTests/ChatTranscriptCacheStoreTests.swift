...
func cacheSessionEntry(
    key: String,
    updatedAt: Double,
    agentID: String? = nil) -> CarapaceChatSessionEntry
{
...
private func messageTexts(_ messages: [CarapaceChatMessage]) -> [String] {
...
extension CarapaceChatSQLiteTranscriptCache {
    fileprivate func storeTestTranscript(
        sessionKey: String,
        agentID: String? = nil,
        messages: [CarapaceChatMessage]) async
    {
...
private func outboxCommand(
    ...
    status: CarapaceChatOutboxCommand.Status = .queued) -> CarapaceChatOutboxCommand
{
    CarapaceChatOutboxCommand(
        id: id,
        sessionKey: sessionKey,
...
class ClientDatabaseTestSuite: TemporaryDatabaseTestSuite {
    ...
    let store: CarapaceChatSQLiteTranscriptCache

    required init() throws {
        let directory = try makeDatabaseDirectory()
        let databases: CarapaceClientDatabases
        do {
            databases = try CarapaceClientDatabases(directoryURL: directory)
        } catch {
...
private func retryExpectation(_ command: CarapaceChatOutboxCommand) -> CarapaceChatOutboxRetryExpectation {
    CarapaceChatOutboxRetryExpectation(
...
private func forgottenGatewayProbe(
    in databases: CarapaceClientDatabases,
    identity: String) async throws -> (hash: String?, gatewayID: String?, cleanupPhase: Int?)
{
    try await databases.stateQueue.read { db in
        let row = try Row.fetchOne(
            ...
            arguments: [CarapaceClientDatabases.gatewayIdentityHash(identity)])
...
extension CarapaceChatSQLiteTranscriptCache {
    fileprivate func reconcileForTest(
        ...
        branchLeafEntryIDs: Set<String> = ["leaf-b"]) async -> [CarapaceChatOutboxCommand]?
    {
...
final class ChatTranscriptCacheStoreTests: ClientDatabaseTestSuite, @unchecked Sendable {
    ...
    @Test func `full removal deletes both databases legacy files and sidecars`() async throws {
        ...
        try CarapaceClientDatabases.removeDatabaseFiles(in: directory)

        for filename in [
            CarapaceClientDatabases.gatewayCacheFilename,
            CarapaceClientDatabases.clientStateFilename,
    ...
    @Test func `agent session snapshots preserve another agents offline roster`() async throws {
        ...
        let reopened = try CarapaceClientDatabases(directoryURL: directory)
    ...
    @Test func `agent session owner partitions remain bounded`() async throws {
        for index in 0...CarapaceChatSQLiteTranscriptCache.maxCachedSessionOwners {
        ...
        #expect(counts.0 == CarapaceChatSQLiteTranscriptCache.maxCachedSessionOwners)
        #expect(counts.1 == CarapaceChatSQLiteTranscriptCache.maxCachedSessionOwners)
    }
    ...
    @Test func `cache format mismatch rebuilds without touching client state`() async throws {
        let stateIdentity = try #require(CarapaceChatSessionRoutingIdentity(
            scope: "per-sender",
            mainSessionKey: "main",
            defaultAgentID: "main"))
    ...
    @Test func `corrupt cache rebuilds while corrupt client state is preserved`() async throws {
        ...
        let repaired = try CarapaceClientDatabases(directoryURL: cacheDirectory)
        ...
        #expect(throws: (any Error).self) {
            _ = try CarapaceClientDatabases(directoryURL: stateDirectory)
        }
    ...
    @Test func `cache bounds sessions messages and transcript partitions`() async {
        let sessions = (0..<(CarapaceChatSQLiteTranscriptCache.maxCachedSessions + 10)).map {
            cacheSessionEntry(key: "s\($0)", updatedAt: Double($0))
        }
        await store.storeSessions(sessions)
        #expect(await store.loadSessions().count == CarapaceChatSQLiteTranscriptCache.maxCachedSessions)
        #expect(await store.loadSessions().contains(where: { $0.key == "s0" }) == false)

        let messages = (0..<(CarapaceChatSQLiteTranscriptCache.maxCachedMessagesPerSession + 20)).map {
        ...
        #expect(await store.loadTranscript(sessionKey: "bounded").count ==
            CarapaceChatSQLiteTranscriptCache.maxCachedMessagesPerSession)
        #expect(await messageTexts(store.loadTranscript(sessionKey: "bounded")).first == "m20")

        for index in 0...CarapaceChatSQLiteTranscriptCache.maxCachedTranscripts {
            await store.storeTestTranscript(
        ...
        #expect(await store.loadTranscript(
            sessionKey: "partition-\(CarapaceChatSQLiteTranscriptCache.maxCachedTranscripts)").isEmpty == false)
    }
    ...
    @Test func `cache projection strips payloads and keeps bounded diffs`() throws {
        ...
        let cached = try #require(CarapaceChatSQLiteTranscriptCache.cacheableMessages([message]).first)
    ...
    @Test func `staged gateway removal reconciles against the pairing registry`() async throws {
        ...
        let registered = try CarapaceClientDatabases(
        ...
        let forgotten = try CarapaceClientDatabases(
            directoryURL: directory,
        ...
        #expect(probe.hash == CarapaceClientDatabases.gatewayIdentityHash("gw-a") && probe.gatewayID == nil)
    }
    ...
    @Test func `staged gateway removal uses exact registry identifier bytes`() async throws {
        ...
        let recovered = try CarapaceClientDatabases(
            directoryURL: directory,
        ...
        #expect(probe.hash == CarapaceClientDatabases.gatewayIdentityHash(composedGatewayID) && probe.gatewayID == nil)
    }
    ...
    @Test func `hash only scrub marker remains recoverable`() async throws {
        ...
        let recovered = try CarapaceClientDatabases(directoryURL: directory)
        let probe = try await forgottenGatewayProbe(in: recovered, identity: "gw-a")
        #expect(probe.cleanupPhase == 0)
    }
    ...
    @Test func `gateway removal scrubs its payloads from shared database files`() async throws {
        ...
        let attachment = CarapaceChatOutboxAttachment(
            type: "file",
...
final class ClientDatabaseLegacyImportTests: TemporaryDatabaseTestSuite, @unchecked Sendable {
    @Test func `legacy v1 cache is discarded`() async throws {
        ...
        let databases = try CarapaceClientDatabases(directoryURL: directory)
    ...
    @Test func `legacy v2 outbox imports parked into client state`() async throws {
        ...
        #expect(commands.map(\.lastError) == [CarapaceChatSQLiteTranscriptCache.outboxUnknownTargetError])
    ...
    @Test func `forget removes an unreadable per gateway legacy database and sidecars`() throws {
        ...
        let legacyURL = CarapaceClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: gatewayID,
        ...
        let databases = try CarapaceClientDatabases(
        ...
        } == CarapaceClientDatabases.gatewayIdentityHash(gatewayID))
        try databases.close()
        for suffix in ["", "-wal", "-shm"] {
            let url = URL(fileURLWithPath: databaseDirectory
                .appendingPathComponent(CarapaceClientDatabases.clientStateFilename).path + suffix)
    ...
    @Test func `legacy import accepts only gateways in the pairing registry`() async throws {
        let keptURL = CarapaceClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: "gw-kept",
            directoryURL: directory)
        let orphanedURL = CarapaceClientDatabases.legacyPerGatewayDatabaseURL(
    ...
    @Test func `unknown or corrupt legacy files remain untouched`() async throws {
        ...
        _ = try CarapaceClientDatabases(directoryURL: directory)
        ...
        let databases = try CarapaceClientDatabases(directoryURL: corruptDirectory)
...
final class ChatCommandOutboxStoreTests: ClientDatabaseTestSuite, @unchecked Sendable {
    @Test func `session settings expectation survives a cold outbox reopen`() async throws {
        let expectation = CarapaceChatSessionSettingsExpectation(
            permissionMode: .guarded,
            toolOverrides: CarapaceChatSessionToolOverrides(
                webSearch: false,
                mcpToolsDeny: ["github": ["delete_issue"]]))
        #expect(await store.enqueueCommand(CarapaceChatOutboxCommand(
            id: "settings-bound",
            sessionKey: "main",
            deliverySessionKey: "agent:main:main",
            routingContract: "per-sender|main|main",
    ...
    @Test func `legacy null settings row cannot claim or retry without current client authorization`() async throws {
        ...
        let claimed = try CarapaceClientDatabases(directoryURL: directory)
        let claimedCommand = try #require(await claimed.store(gatewayID: "gw-a").loadCommands().first)
        #expect(claimedCommand.status == .failed)
        #expect(claimedCommand.lastError == CarapaceChatSQLiteTranscriptCache.outboxSettingsUpgradeRequiredError)
        ...
        let retried = try CarapaceClientDatabases(directoryURL: directory)
    ...
    @Test func `pre v7 upgrade reopens and fences a settings bound retry`() async throws {
        ...
        let upgraded = try CarapaceClientDatabases(directoryURL: directory)
        ...
        #expect(await upgradedStore.enqueueCommand(CarapaceChatOutboxCommand(
        ...
        #expect(failed.lastError == CarapaceChatSQLiteTranscriptCache.outboxSettingsUpgradeRequiredError)
    ...
    @Test func `nil agent rows use the canonical empty scope owner`() async throws {
        let scope = CarapaceChatOutboxScope(sessionKey: "main", agentID: nil)
    ...
    @Test func `parked accepted rows mint retry identity while queued rows keep it`() async throws {
        let sendingScope = CarapaceChatOutboxScope(sessionKey: "sending", agentID: "main")
        ...
        #expect(await store.markCommandRetriedIfPresent(
            ...
            expectedSessionSettings: CarapaceChatSessionSettingsExpectation(permissionMode: nil, toolOverrides: nil),
        ...
        let queuedScope = CarapaceChatOutboxScope(sessionKey: "queued", agentID: "main")
        ...
        let requeuedScope = CarapaceChatOutboxScope(sessionKey: "requeued", agentID: "main")
    ...
    @Test func `parked accepted attachment follows the fresh retry identity`() async throws {
        let scope = CarapaceChatOutboxScope(sessionKey: "main", agentID: "main")
        let attachment = CarapaceChatOutboxAttachment(
            type: "audio",
            mimeType: "audio/m4a",
            fileName: "note.m4a",
    ...
    @Test func `empty root reconcile permits first row and parks a wiped scope`() async throws {
        let root = CarapaceChatOutboxScope(sessionKey: "root", agentID: "main")
        ...
        let wiped = CarapaceChatOutboxScope(sessionKey: "wiped", agentID: "main")
    ...
    @Test func `interrupted sends fail closed once`() async throws {
        ...
        #expect(recovered.lastError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(await store.recoverInterruptedSends())
    }
    ...
    @Test func `stale queued and acknowledged commands require user action`() async throws {
        let old = Date().timeIntervalSince1970 - CarapaceChatSQLiteTranscriptCache.outboxCommandMaxAge - 1
        ...
        #expect(commandsByID["old-queued"]?.lastError == CarapaceChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(commandsByID["old-ack"]?.status == .failed)
        #expect(commandsByID["old-ack"]?.lastError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    @Test func `queue and attachment budgets are gateway scoped`() async {
        let storeA = databases.store(gatewayID: "gw-a")
        let storeB = databases.store(gatewayID: "gw-b")
        for index in 0..<CarapaceChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        let oversized = CarapaceChatOutboxAttachment(
            type: "file",
            mimeType: "application/octet-stream",
            fileName: "large.bin",
            data: Data(count: CarapaceChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand + 1))
        #expect(await storeB.enqueueCommand(outboxCommand(
            id: "too-large",
            text: "large",
            attachments: [oversized])) == false)
        #expect(CarapaceChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 1,
            queuedBytes: CarapaceChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
        #expect(!CarapaceChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 2,
            queuedBytes: CarapaceChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
    }
...
final class WatchMessageJournalStoreTests: ClientDatabaseTestSuite, @unchecked Sendable {
    ...
    private var journal: CarapaceWatchMessageJournal {
        self.databases.watchMessages
    }

    private func context(gatewayID: String = "gw-a") async throws -> CarapaceWatchChatDeliveryContext {
        try await self.journal.importLegacy(.init(messages: [], recentMessageIDs: []), nowMs: self.now)
        let identity = try #require(CarapaceChatSessionRoutingIdentity(contract: "per-sender|main|main"))
        ...
        return try CarapaceWatchChatDeliveryContext(
    ...
    {
        try await CarapaceWatchChatDeliveryCommand(
            context: self.context(gatewayID: gatewayID), commandId: id, submittedAtMs: self.now,
            body: .chat(text: text))
    }

    private func claim(_ command: CarapaceWatchChatDeliveryCommand) async throws -> CarapaceWatchMessageEntry {
    ...
    @Test func `Watch admission receipt and immutable payload survive reopen`() async throws {
        ...
        let changed = CarapaceWatchChatDeliveryCommand(
    ...
    @Test func `accepted Watch run resumes readback and terminal receipt replays without another claim`() async throws {
        ...
        let ack = try CarapaceWatchChatDeliveryReceiptAck(
    ...
    @Test func `waiting Watch chat does not block a fresh quick reply`() async throws {
        ...
        let reply = CarapaceWatchChatDeliveryCommand(
    ...
    @Test(arguments: [false, true], [CarapaceWatchChatDeliveryKind.chat, .quickReply])
    func `dismiss preserves the committed Watch receipt and its original expiry`(
        acknowledged: Bool, kind: CarapaceWatchChatDeliveryKind) async throws
    {
        let original = try await self.command(text: "caf\u{E9}")
        let body: CarapaceWatchChatDeliveryBody = kind == .chat ? original.body :
            .quickReply(promptId: "prompt", actionId: "approve", actionLabel: nil, note: nil)
        let input = CarapaceWatchChatDeliveryCommand(
            context: original.context, commandId: original.commandId,
            submittedAtMs: original.submittedAtMs, body: body)
        let changedBody: CarapaceWatchChatDeliveryBody = kind == .chat ? .chat(text: "cafe\u{301}") :
        ...
        func requireIdentityConflict(in journal: CarapaceWatchMessageJournal) async throws {
        ...
        let watch = CarapaceWatchChatDeliveryStore(
        ...
        let owner = CarapaceWatchMessageOwner(context: input.context)
        let accepted = try #require(try await self.journal.accepted(owner: owner).first)
        let outcome: CarapaceWatchChatDeliveryOutcome = kind == .chat ?
        ...
        let reopened = try CarapaceClientDatabases(directoryURL: self.directory)
        defer { try? reopened.close() }
        ...
        let differentContext = try await CarapaceWatchChatDeliveryCommand(
    ...
    func `cancelable Forget preserves late settlement without granting a new dispatch`(settlement: String) async throws {
        ...
        let expectedPhase: CarapaceWatchMessagePhase
        switch settlement {
        case "accepted":
        ...
        settlement: String, disposition: String) async throws
    {
        ...
        case "retry":
            ...
            let expectedPhase: CarapaceWatchMessagePhase = switch settlement {
        ...
        case "expiry":
            ...
            let next = CarapaceWatchChatDeliveryCommand(
    ...
    @Test func `legacy import marker survives discard Forget and delayed defaults cleanup`() async throws {
        let snapshot = CarapaceWatchMessageLegacyImport(messages: [
        ...
        #expect(try await self.journal
            .pruneExpired(nowMs: self.now + 2 * CarapaceWatchChatDeliveryCodec.lifetimeMs) == 2)
        ...
        try await reopened.watchMessages.importLegacy(
            snapshot,
            nowMs: self.now + 3 * CarapaceWatchChatDeliveryCodec.lifetimeMs)
        #expect(try await reopened.watchMessages.entries().isEmpty)
        try await reopened.watchMessages.importLegacy(
            .init(messages: [snapshot.messages[2]], recentMessageIDs: []),
            nowMs: self.now + 4 * CarapaceWatchChatDeliveryCodec.lifetimeMs)
        #expect(try await reopened.watchMessages.entries().isEmpty)

        let later = CarapaceWatchMessageLegacyImport(messages: [
            .init(id: "later-old-writer", gatewayStableID: "gw-a", text: "new unsent text", submittedAtMs: nil),
        ], recentMessageIDs: [])
        let laterTime = self.now + 5 * CarapaceWatchChatDeliveryCodec.lifetimeMs
    ...
    func `legacy recent metadata cannot erase an imported content fingerprint`(changedField: String) async throws {
        let original = CarapaceWatchMessageLegacyImport.Message(
        ...
        let changed = CarapaceWatchMessageLegacyImport.Message(
            id: original.id,
        ...
        let fresh = CarapaceWatchMessageLegacyImport.Message(
    ...
    func `unseen legacy IDs from a forgotten Gateway remain unimported after re-pair`(repaired: Bool) async throws {
        ...
        let ambiguous = CarapaceWatchMessageLegacyImport.Message(
    ...
    @Test func `legacy import identities preserve exact UTF8 bytes`() async throws {
        let first = CarapaceWatchMessageLegacyImport.Message(
            id: "legacy-\u{E9}", gatewayStableID: "gw-a", text: "first", submittedAtMs: nil)
        let second = CarapaceWatchMessageLegacyImport.Message(
    ...
    @Test func `Watch expiry never renews a replayed command`() async throws {
        ...
        let future = CarapaceWatchChatDeliveryCommand(
            context: input.context, commandId: "future",
            submittedAtMs: self.now + CarapaceWatchChatDeliveryCodec.maxFutureSkewMs + 1,
            body: .chat(text: "future"))
        await #expect(throws: CarapaceWatchChatDeliveryError.self) { _ = try await self.journal.admit(
            future,
            nowMs: self.now) }
        #expect(try await self.journal.entries().isEmpty)
    }
    ...
    @Test func `Watch pending capacity rejects before admission and accepted work frees a slot`() async throws {
        ...
        let extra = CarapaceWatchChatDeliveryCommand(
            context: target, commandId: "overflow", submittedAtMs: now, body: .chat(text: "overflow"))
        await #expect(throws: CarapaceWatchChatDeliveryError.self) { _ = try await self.journal.admit(
            extra,
            nowMs: self.now) }
        #expect(try await self.journal.entries().count == CarapaceWatchChatDeliveryCodec.maxPendingCommands)
    ...
    @Test func `changed routing settles a queued Watch command without dispatch`() async throws {
        ...
        _ = try await self.journal.admit(input, nowMs: self.now)
        try await store
            .storeSessionRoutingIdentity(#require(CarapaceChatSessionRoutingIdentity(contract: "global|main|other")))
    ...
    @Test(arguments: [
        ...
        let past = self.now - CarapaceWatchChatDeliveryCodec.lifetimeMs - 1000
        let input = CarapaceWatchChatDeliveryCommand(
        ...
        let entries: [CarapaceWatchMessageEntry]
        if subscription {
            let observation = try await self.journal.changes()
            var changes = observation.makeAsyncIterator()
    ...
    @Test func `actual v8 migrations reopen the additive Watch schema and preserve its generation`() async throws {
        ...
        let older = try DatabaseQueue(path: directory
            .appendingPathComponent(CarapaceClientDatabases.clientStateFilename).path)
        defer { try? older.close() }
        var migrator = DatabaseMigrator()
        CarapaceClientDatabases.registerClientStateMigrationsV1ThroughV5(&migrator)
        CarapaceClientDatabases.registerClientStateMigrationsV6ThroughV8(&migrator)
        ...
        try await store
            .storeSessionRoutingIdentity(#require(CarapaceChatSessionRoutingIdentity(contract: "per-sender|main|main")))
