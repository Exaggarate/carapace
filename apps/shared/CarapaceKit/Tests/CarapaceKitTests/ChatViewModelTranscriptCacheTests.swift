...
private func visibleTexts(_ vm: CarapaceChatViewModel) async -> [String] {
...
private actor TestTranscriptCache: CarapaceChatTranscriptCache {
    private var transcripts: [String: [CarapaceChatMessage]]
    private var sessions: [CarapaceChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscripts: [[CarapaceChatMessage]] = []

    init(
        transcripts: [String: [CarapaceChatMessage]] = [:],
    ...
    func loadSessions(agentID _: String?) async -> [CarapaceChatSessionEntry] {
    ...
    func storeSessions(_ sessions: [CarapaceChatSessionEntry], agentID _: String?) async {
...
private actor LegacyTranscriptCache: CarapaceChatTranscriptCache {
    private var sessions: [CarapaceChatSessionEntry]

    init(sessions: [CarapaceChatSessionEntry] = []) {
        self.sessions = sessions
    }
    ...
    func loadTranscript(sessionKey _: String) async -> [CarapaceChatMessage] {
        []
    }
    ...
    func storeCanonicalTranscript(
        sessionKey _: String,
        agentID _: String?,
        messages _: [CarapaceChatMessage],
        canonicalMessageIdempotencyKeys _: Set<String>) async
    {}
}

/// Minimal FIFO transport whose history responses can be gated during cold open.
private final class GatedHistoryChatTransport: @unchecked Sendable, CarapaceChatTransport {
    private let historyResult: @Sendable (String, Int) async throws -> CarapaceChatHistoryPayload
    ...
    init(historyResult: @escaping @Sendable (String, Int) async throws -> CarapaceChatHistoryPayload) {
    ...
    {
        CarapaceChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func makeViewModel(
    ...
    cache: any CarapaceChatTranscriptCache,
    load: Bool = true) -> CarapaceChatViewModel
{
...
struct ChatViewModelTranscriptCacheTests {
    ...
    @Test func `prior agent cache cannot repaint global digest after owner switch`() async throws {
        ...
        global.observerDigest = CarapaceChatSessionObserverDigest(
            agentId: "main",
            runId: "run-main",
            revision: 3,
            updatedAt: 1000,
    ...
    @Test func `session cache strips active markers and preserves terminal recap`() {
        ...
        let projected = CarapaceChatViewModel.durableSessionCacheProjection(active)
        ...
        let terminalProjection = CarapaceChatViewModel.durableSessionCacheProjection(terminal)
