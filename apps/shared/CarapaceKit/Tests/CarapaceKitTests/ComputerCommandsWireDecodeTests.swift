...
struct ComputerCommandsWireDecodeTests {
    private struct DecodeCase {
        let json: String
        let action: CarapaceComputerAction
        var app: String?
        var windowRef: String?
        var elementRef: String?
        var observationId: String?
        var deliveryMode: CarapaceComputerDeliveryMode?
        var query: String?
        var depth: Int?
        var maxElements: Int?
        var value: String?
    ...
    private func decode(_ testCase: DecodeCase) throws -> CarapaceComputerActParams {
        try JSONDecoder().decode(CarapaceComputerActParams.self, from: Data(testCase.json.utf8))
    }

    private func expectFields(_ params: CarapaceComputerActParams, match testCase: DecodeCase) {
    ...
    @Test func `rejects unknown and native-unimplemented action names`() throws {
        for action in ["totally_unknown", "browser_click", "start_recording"] {
            ...
            #expect(throws: DecodingError.self) {
                _ = try JSONDecoder().decode(CarapaceComputerActParams.self, from: data)
            }
        }
    }
    ...
    @Test func `action raw values match the frozen computer use v2 contract`() {
        ...
        #expect(CarapaceComputerAction.allCases.map(\.rawValue) == frozenActionNames)
    }
}
