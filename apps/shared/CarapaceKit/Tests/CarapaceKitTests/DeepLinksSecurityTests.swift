...
@Suite struct DeepLinksSecurityTests {
    ...
    @Test func dashboardDeepLinkParses() {
        let url = URL(string: "carapace://dashboard")!
        #expect(DeepLinkParser.parse(url) == .dashboard)
    }

    @Test func debugDashboardDeepLinkParses() {
        let url = URL(string: "carapace-debug://dashboard")!
        #expect(DeepLinkParser.parse(url) == .dashboard)
    }

    @Test(arguments: ["carapace", "carapace-debug"])
    func gatewayAddDeepLinkPreservesAddressAndLabel(scheme: String) throws {
        ...
        components.queryItems = [
            URLQueryItem(name: "url", value: "HTTPS://Gateway.Example:8443/carapace%20gateway/"),
        ...
        #expect(link.url.absoluteString == "https://gateway.example:8443/carapace%20gateway/")
    ...
    @Test func gatewayAddDeepLinkRejectsPasswordCredentials() throws {
        ...
        var link = try #require(URLComponents(string: "carapace://gateway/add"))
    ...
    @Test(arguments: [
        "carapace://gateway/add?url=https%3A%2F%2Fgateway.example&token=secret",
        "carapace://gateway/add?url=https%3A%2F%2Fgateway.example&password=secret",
        "carapace://gateway/add?url=https%3A%2F%2Fgateway.example&url=https%3A%2F%2Fother.example",
        "carapace://gateway/add?url=https%3A%2F%2Fgateway.example&name=One&name=Two",
        "carapace://gateway/add?url=https%3A%2F%2Fgateway.example#secret",
        "carapace://user@gateway/add?url=https%3A%2F%2Fgateway.example",
        "carapace://gateway:443/add?url=https%3A%2F%2Fgateway.example",
        "carapace://gateway/add?host=gateway.example&tls=1&token=secret",
    ])
    ...
    @Test func gatewayDeepLinkUsesTlsDefaultPortWhenPortMissing() {
        let link = gatewayLink(from: "carapace://gateway?host=gateway.example.com&tls=1")
        #expect(link?.port == 443)
        #expect(link?.tls == true)
    }

    @Test func gatewayDeepLinkUsesPlaintextDefaultPortWhenPortMissing() {
        let link = gatewayLink(from: "carapace://gateway?host=127.0.0.1&tls=0")
        #expect(link?.port == 18789)
        #expect(link?.tls == false)
    }

    @Test func gatewayDeepLinkPreservesExplicitTlsPort() {
        let link = gatewayLink(from: "carapace://gateway?host=gateway.example.com&port=18789&tls=1")
        #expect(link?.port == 18789)
        #expect(link?.tls == true)
    }
    ...
    @Test func gatewayDeepLinkRejectsInsecureNonLoopbackWs() {
        let url = URL(
            string: "carapace://gateway?host=attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkRejectsInsecurePrefixBypassHost() {
        let url = URL(
            string: "carapace://gateway?host=127.attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "carapace://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodePreservesPrimaryGatewayContextPath() {
        let payload = #"{"url":"wss://gateway.example/carapace-gw","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/carapace-gw")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/carapace-gw")
    }

    @Test func setupCodeDecodesGatewayContextPathExactlyOnce() {
        let payload = #"{"url":"wss://gateway.example/carapace%20gateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/carapace%20gateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/carapace%20gateway")
    }

    @Test func setupCodePreservesEscapedGatewayPathDelimiter() {
        let payload = #"{"url":"wss://gateway.example/carapace%2Fgateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/carapace%2Fgateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/carapace%2Fgateway")
    }

    @Test func setupCodePreservesNonUTF8GatewayPathOctet() {
        let payload = #"{"url":"wss://gateway.example/carapace%FFgateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/carapace%FFgateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/carapace%FFgateway")
    }
    ...
    @Test func setupCodeAllowsMDNSWs() {
        let payload = #"{"url":"ws://carapace.local:18789","bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "carapace.local",
                port: 18789,
    ...
    @Test func setupCodeRejectsGatewayURLMetadata() {
        let urls = [
            "wss://user@gateway.example/carapace-gw",
            "wss://gateway.example/carapace-gw?mode=setup",
            "wss://gateway.example/carapace-gw#fragment",
        ]
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"carapace.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "carapace.local",
                port: 18789,
