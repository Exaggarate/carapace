...
struct ChatGatewayRequestTests {
    ...
    {
        let request = CarapaceChatGatewayRequests.progressCardGet(sessionKey: key, agentID: owner)
    ...
    @Test func `progress payloads validate the captured agent without replacing null`() throws {
        let valid = Data(
            #"{"card":{"sessionKey":"agent:research:global","revision":1,"updatedAt":10,"markdown":"Research","steps":[]}}"#
                .utf8)
        #expect(try CarapaceChatGatewayPayloadCodec.decodeProgressCard(valid, agentID: "research")?
            .markdown == "Research")
        #expect(throws: (any Error).self) {
            try CarapaceChatGatewayPayloadCodec.decodeProgressCard(valid, agentID: "main")
        }
        #expect(try CarapaceChatGatewayPayloadCodec.decodeProgressCard(
    ...
    func `question host consent uses protocol field`(hosts: [String]?) throws {
        let request = CarapaceChatGatewayRequests.resolveQuestion(
    ...
    @Test func `question answer response decodes canonical marker`() throws {
        ...
        let answers = try CarapaceChatGatewayPayloadCodec.decodeQuestionAnswer(data)
        ...
        for invalid in [
            ...
            #expect(throws: (any Error).self) {
                try CarapaceChatGatewayPayloadCodec.decodeQuestionAnswer(Data(invalid.utf8))
            }
        }
    }
    ...
    @Test func `models list scopes worker catalogs and preserves default scope`() {
        let worker = CarapaceChatGatewayRequests.modelsList(agentID: " worker ")
        let defaultAgent = CarapaceChatGatewayRequests.modelsList(agentID: nil)
    ...
    @Test func `session observation requests encode global subscription and actual visibility`() {
        let subscribe = CarapaceChatGatewayRequests.subscribeSessions()
        let visible = CarapaceChatGatewayRequests.setSessionObserverVisibility(true)
        let hidden = CarapaceChatGatewayRequests.setSessionObserverVisibility(false)
        let longerSubscription = CarapaceChatGatewayRequests.subscribeSessions(timeoutMs: 12000)
        let longerVisibility = CarapaceChatGatewayRequests.setSessionObserverVisibility(
            true,
            timeoutMs: 12000)
    ...
    @Test func `session targets share normalization while preserving platform routing policy`() {
        #expect(CarapaceChatSessionTarget.resolve(
            " Matrix:Channel:Room ",
    ...
    @Test func `list sessions request normalizes optional filters`() {
        ...
        let unscoped = CarapaceChatGatewayRequests.sessionsList(
            limit: nil,
            search: nil,
            archived: false,
            agentID: "   ")
    ...
    @Test func `session patch request preserves explicit null clearing`() {
        let request = CarapaceChatGatewayRequests.patchSession(
            sessionKey: "global",
            agentID: "reviewer",
            label: .some(nil),
            category: .some(nil),
    ...
    @Test func `session read routing separates explicit automatic and legacy requests`() {
        #expect(CarapaceChatSessionUnreadPatch.routed(
            unread: false,
    ...
    @Test func `settings patch request encodes default model as null`() {
        let request = CarapaceChatGatewayRequests.patchSessionSettings(
    ...
    @Test func `settings patch request encodes fast values and explicit resets`() {
        let reset = CarapaceChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            thinkingLevel: .some(nil),
            fastMode: .some(nil),
            verboseLevel: .some(nil))
        let automatic = CarapaceChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
    ...
    @Test func `settings patch request preserves permission and sparse tool overrides`() throws {
        ...
        let request = CarapaceChatGatewayRequests.patchSessionSettings(
            ...
            expectedToolOverrides: .some(CarapaceChatSessionToolOverrides(webSearch: false)),
        ...
        let releasedGateway = CarapaceChatGatewayRequests.patchSessionSettings(
            sessionKey: "global",
            agentID: "reviewer",
            expectedSessionID: "sess-global",
    ...
    @Test func `composer catalog requests preserve their owner scope`() {
        let skills = CarapaceChatGatewayRequests.composerSkillsStatus(agentID: "reviewer")
        let config = CarapaceChatGatewayRequests.composerConfigGet()
        let tools = CarapaceChatGatewayRequests.composerToolsEffective(
    ...
    @Test func `fork and create requests preserve routing identity`() {
        let fork = CarapaceChatGatewayRequests.forkSession(
        ...
        let activeFork = CarapaceChatGatewayRequests.forkSession(
            parentSessionKey: "agent:reviewer:telegram:group:1",
            agentID: "reviewer",
            fromLastCompleted: true)
        #expect(activeFork.params["forkFrom"]?.value as? String == "last-completed")

        let create = CarapaceChatGatewayRequests.createSession(
    ...
    @Test func `message rewind and fork requests preserve routing identity`() {
        let rewind = CarapaceChatGatewayRequests.rewindSession(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ",
            entryId: " message-42 ")
        let fork = CarapaceChatGatewayRequests.forkAtMessage(
            sessionKey: "global",
            agentID: nil,
            entryId: "message-43")
    ...
    @Test func `branch list and switch requests preserve routing identity`() {
        let list = CarapaceChatGatewayRequests.listSessionBranches(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ")
        let switchBranch = CarapaceChatGatewayRequests.switchSessionBranch(
            sessionKey: "global",
            agentID: nil,
            leafEntryId: " leaf-42 ")
    ...
    @Test func `session group requests encode exact gateway contracts`() {
        let list = CarapaceChatGatewayRequests.sessionGroupsList()
        let put = CarapaceChatGatewayRequests.sessionGroupsPut(names: ["Work", "Personal"])
        let rename = CarapaceChatGatewayRequests.sessionGroupsRename(name: "Work", to: "Projects")
        let delete = CarapaceChatGatewayRequests.sessionGroupsDelete(name: "Personal")
    ...
    private actor MutationRequestRecorder {
        var requests: [CarapaceChatGatewayRequest] = []

        func send(_ request: CarapaceChatGatewayRequest) -> Data {
    ...
    @Test func `gateway mutation lease forwards nullable fields and request timeouts`() async throws {
        ...
        let lease = CarapaceChatSessionMutationRouteLease(
            sessionTarget: {
    ...
    @Test func `unknown captured read capability only blocks read mutations`() async throws {
        let recorder = MutationRequestRecorder()
        let lease = CarapaceChatSessionMutationRouteLease(
            sessionTarget: { CarapaceChatSessionTarget(sessionKey: $0, agentID: nil) },
        ...
        try await lease.patchSession(
            ...
            archived: nil,
            unread: true)
        await #expect(throws: CarapaceChatTransportSendError.self) {
            try await lease.patchSession(
                key: "agent:reviewer:work",
                expectedMarkedUnreadAt: .some(nil),
                label: nil,
    ...
    @Test func `rename clear archive delete and fork use session mutation contracts`() {
        let rename = CarapaceChatGatewayRequests.patchSession(
            ...
            archived: nil,
            unreadPatch: nil)
        let archive = CarapaceChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            ...
            archived: true,
            unreadPatch: nil)
        let restore = CarapaceChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            expectedSessionID: "session-child",
            label: nil,
        ...
        let fork = CarapaceChatGatewayRequests.forkSession(
            ...
            agentID: nil)
        let delete = CarapaceChatGatewayRequests.deleteSession(
            sessionKey: "agent:main:child",
            agentID: nil)
    ...
    @Test func `chat metadata request selects session agent before fallback`() {
        let scoped = CarapaceChatGatewayRequests.chatMetadata(
            sessionKey: "agent:reviewer:main",
        ...
        let global = CarapaceChatGatewayRequests.chatMetadata(
            sessionKey: "global",
    ...
    @Test func `commands request selects session agent before fallback`() {
        let scoped = CarapaceChatGatewayRequests.commandsList(
            sessionKey: "agent:reviewer:main",
            fallbackAgentID: "fallback")
        ...
        let global = CarapaceChatGatewayRequests.commandsList(
            sessionKey: "global",
            fallbackAgentID: "reviewer")
        #expect(global.params["agentId"]?.value as? String == "reviewer")
    }
    ...
    @Test func `send request omits inherited thinking override`() {
        let inherited = CarapaceChatGatewayRequests.sendMessage(
            sessionKey: "global",
            agentID: nil,
    ...
    @Test func `question get request carries id`() {
        let request = CarapaceChatGatewayRequests.questionGet(id: "ask_123")
    ...
    @Test func `question cancel request uses resolve cancel contract`() {
        let request = CarapaceChatGatewayRequests.cancelQuestion(id: "ask_123")
    ...
    @Test func `long running requests share exact gateway timeout margins`() {
        #expect(CarapaceChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 1).timeoutMs == 5001)
        #expect(CarapaceChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 30000).timeoutMs == 35000)
        #expect(CarapaceChatGatewayRequests.compactSession(
            sessionKey: "main",
...
struct ChatGatewayPayloadCodecTests {
    ...
    func `history decoding preserves optional input consumption receipts`(consumptions: [[String: String]]?) throws {
        ...
        let payload = try JSONDecoder().decode(
            CarapaceChatHistoryPayload.self,
    ...
    @Test func `session row decodes permission and every tool override family`() throws {
        let row = try JSONDecoder().decode(CarapaceChatSessionEntry.self, from: Data(#"""
        {
        ...
        #expect(row.toolOverrides == CarapaceChatSessionToolOverrides(
            webSearch: false,
            skills: ["release": false],
            mcpServers: ["github": true],
            mcpToolsDeny: ["github": ["create_issue"]]))
    ...
    @Test func `session key extracts canonical agent identity`() {
        #expect(CarapaceChatSessionKey.agentID(from: " agent:Reviewer:main ") == "Reviewer")
        #expect(CarapaceChatSessionKey.agentID(from: "agent::main") == nil)
        #expect(CarapaceChatSessionKey.agentID(from: "global") == nil)
    }

    @Test func `agent wait distinguishes terminal and retryable timeouts`() throws {
        #expect(try CarapaceChatGatewayPayloadCodec.decodeAgentWaitObservation(
    ...
    @Test func `routing identity decodes agent and canonical contract`() throws {
        let identity = try CarapaceChatGatewayPayloadCodec.decodeSessionRoutingIdentity(
    ...
    @Test func `model choices preserve metadata and replace blank names`() throws {
        let payload = Data(
            #"{"models":[{"id":"gpt-5","name":"  ","provider":"openai","available":false,"unavailableReason":"missing-auth","unavailableUntil":1234,"contextWindow":200000,"reasoning":true}]}"#
                .utf8)
        let choices = try CarapaceChatGatewayPayloadCodec.decodeModelChoices(payload)
        let metadataChoices = try CarapaceChatGatewayPayloadCodec.decodeChatMetadataModelChoices(payload)

        #expect(choices == [CarapaceChatModelChoice(
            modelID: "gpt-5",
            name: "gpt-5",
            provider: "openai",
            available: false,
    ...
    @Test func `command choice normalizes source aliases and identity`() {
        let choice = CarapaceChatGatewayPayloadCodec.commandChoice(CommandEntry(
            name: "review",
            textaliases: [" /review ", ""],
    ...
    @Test func `event frames map to shared chat transport events`() {
        ...
        guard case let .sessionsChanged(change) = CarapaceChatGatewayPayloadCodec.event(from: sessionsChanged)
        else {
        ...
        guard case let .sessionsChanged(note) = CarapaceChatGatewayPayloadCodec.event(from: swarmNote)
        else {
        ...
        guard case let .sessionsChanged(lifecycle) = CarapaceChatGatewayPayloadCodec.event(
        ...
        guard case let .chat(payload) = CarapaceChatGatewayPayloadCodec.event(from: chat) else {
        ...
        guard case let .sessionObserver(digest) = CarapaceChatGatewayPayloadCodec.event(from: observer)
        else {
        ...
        guard case let .task(.upserted(summary)) = CarapaceChatGatewayPayloadCodec.event(from: task)
        else {
        ...
        guard case let .progressCardChanged(event) = CarapaceChatGatewayPayloadCodec.event(
            from: progressCard)
        else {
        ...
        guard case .chatMetadataChanged = CarapaceChatGatewayPayloadCodec.event(from: EventFrame(
            type: "event",
        ...
        #expect(CarapaceChatGatewayPayloadCodec.event(from: EventFrame(
            type: "event",
            event: "unknown")) == nil)
    }

    @Test func `session change decoding distinguishes absent fields from explicit clears`() {
        func decode(_ fields: [String: AnyCodable]) throws -> CarapaceChatSessionsChangedEvent {
    ...
    @Test func `session change decodes nested fallback with outer null precedence and no reason`() throws {
        ...
        let event = try JSONDecoder().decode(CarapaceChatSessionsChangedEvent.self, from: data)
    ...
    @Test func `session change remains codable without exposing presence flags`() throws {
        let event = CarapaceChatSessionsChangedEvent(
            sessionKey: "agent:main:work",
            agentId: "main",
            reason: "run-progress",
            updatedAt: 200,
        ...
        #expect(try JSONDecoder().decode(CarapaceChatSessionsChangedEvent.self, from: data) == event)
    }
}
