...
struct ChatSwarmProgressTests {
    ...
    @Test func `activity notes decorate children in observation order`() {
        ...
        var activity = CarapaceChatSwarmActivityState()

        let observedPhase = activity.observe(CarapaceChatSessionsChangedEvent(
            sessionKey: "agent:main:parent",
            reason: "swarm-note",
            swarmGroupId: groupID,
            kind: "phase",
            text: "Research"))
        let observedChild = activity.observe(CarapaceChatSessionsChangedEvent(
            sessionKey: "agent:main:child",
            reason: "create",
            swarmGroupId: groupID))
        let observedLog = activity.observe(CarapaceChatSessionsChangedEvent(
            sessionKey: "agent:main:parent",
            reason: "swarm-note",
            swarmGroupId: groupID,
            kind: "log",
    ...
    @Test func `projection renders active group states and hides terminal groups`() {
        ...
        let groups = buildCarapaceChatSwarmGroups(sessions: sessions) { $0 == "agent:main:parent" }
    ...
    @Test func `child pager repeats from zero when rows move across offsets`() async throws {
        ...
        var call = 0
        let rows = try await CarapaceChatChildSessionPager.collect { offset in
            let page = pages[call]
            call += 1
    ...
    @Test func `metadata capability defaults missing Swarm support to disabled`() throws {
        let capabilities = try JSONDecoder().decode(
            CarapaceChatMetadataCapabilities.self,
            from: Data("{}".utf8))
    ...
    @Test func `Swarm events ignore other parents`() {
        ...
        let ownChild = CarapaceChatSessionsChangedEvent(
            sessionKey: "agent:main:child",
            parentSessionKey: current,
            reason: "create",
            swarmGroupId: "custom-group")
        let otherPhase = CarapaceChatSessionsChangedEvent(
    ...
    @Test func `projection uses derived title before the raw session key`() {
        ...
        let group = buildCarapaceChatSwarmGroups(sessions: [row]) { $0 == "agent:main:parent" }.single
    ...
    @Test func `projection caps historical dots while retaining active workers`() {
        ...
        let group = buildCarapaceChatSwarmGroups(
