...
private final class ContextUsageTestTransport: @unchecked Sendable, CarapaceChatTransport {
...
struct ChatContextUsageTests {
    private func message(
        ...
        usage: CarapaceChatUsage? = nil) -> CarapaceChatMessage
    {
        CarapaceChatMessage(
            id: UUID(),
            role: role,
    ...
    private func usage(
        ...
        cacheWrite: Int? = nil,
        total: Int? = nil,
        costTotal: Double? = nil) throws -> CarapaceChatUsage
    {
        ...
        return try JSONDecoder().decode(CarapaceChatUsage.self, from: data)
    }
    ...
    @Test @MainActor func `view model resolves context totals through a selected global alias`() {
        let vm = CarapaceChatViewModel(
            ...
            activeAgentId: "ops")
        vm.sessions = [CarapaceChatSessionEntry(
