...
/// Replays scripted gateway traffic against `CarapaceChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, CarapaceChatTransport {
    private actor State {
        var history: CarapaceChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: CarapaceChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: CarapaceChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> CarapaceChatHistoryPayload {
    ...
    init(
        history: CarapaceChatHistoryPayload,
    ...
    func setHistory(_ payload: CarapaceChatHistoryPayload) async {
        ...
        attachments _: [CarapaceChatAttachmentPayload]) async throws -> CarapaceChatSendResponse
    {
        ...
        return CarapaceChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        CarapaceChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> CarapaceChatHistoryPayload
{
...
private func replayDurableMessage(
    ...
    idempotencyKey: String? = nil,
    runId: String? = nil,
    emptyThinking: Bool = false) -> CarapaceChatMessage
{
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> CarapaceChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> CarapaceChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> CarapaceChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: CarapaceChatHistoryPayload = replayHistory(),
        transcriptCache: (any CarapaceChatTranscriptCache)? = nil) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: transport, transcriptCache: transcriptCache)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (CarapaceChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension CarapaceChatViewModel {
    fileprivate var replayAssistantRows: [CarapaceChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [CarapaceChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [CarapaceChatMessage] {
...
struct ChatStreamReplayTests {
        ...
        let frame = EventFrame(
            ...
            payload: AnyCodable([
                ...
                "message": [
                    ...
                    "content": [],
                    "timestamp": 1,
                    "__carapace": ["kind": "reset", "id": "live-reset"],
                ],
            ]))
        let event = try #require(CarapaceChatGatewayPayloadCodec.event(from: frame))
    ...
    func `intermediate tool rows cannot consume a final from the same run`(finalFirst: Bool) async throws {
        ...
        let toolMessage: CarapaceChatMessage = try ChatPayloadDecoding.decode(AnyCodable([
            "role": "assistant",
            "content": [
            ...
            "timestamp": now + 2000,
            "stopReason": "toolUse",
            "__carapace": ["runId": runId],
        ] as [String: Any]))
        let toolEvent = CarapaceChatTransportEvent.sessionMessage(CarapaceSessionMessageEventPayload(
            sessionKey: "main",
            message: toolMessage,
        ...
        canonicalViaLiveEvent: Bool) async throws
    {
        ...
        let reopened = try CarapaceClientDatabases(directoryURL: database.directory)
        ...
        let vm = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: transport, transcriptCache: reopenedStore)
        }
        ...
        if canonicalViaLiveEvent {
            ...
            try transport.emit(#require(CarapaceChatGatewayPayloadCodec.event(from: frame)))
