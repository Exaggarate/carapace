...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testSuccessiveRecordingsUseUniqueTemporaryFiles() async throws {
        let capture = FakeVoiceNoteAudioCapture()
        let recorder = CarapaceVoiceNoteRecorder(
            capture: capture,
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(carapaceVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(carapaceVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(carapaceVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(carapaceVoiceNoteDurationLabel(-1), "0:00")
    }

    func testUnifiedMicKeepsVoiceNoteAvailableWithoutDictation() {
        XCTAssertFalse(CarapaceChatMicButton.dictationActionEnabled(
        ...
        XCTAssertTrue(CarapaceChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
    ...
    func testUnifiedMicPreventsCompetingVoiceCapture() {
        XCTAssertFalse(CarapaceChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
            isAttachmentInputEnabled: true,
        ...
        XCTAssertTrue(CarapaceChatMicButton.dictationActionEnabled(
    ...
    func testUnifiedMicCancelsPendingDictationStart() {
        XCTAssertEqual(CarapaceChatMicButton.dictationPrimaryAction(
            isPending: true,
    ...
    func testCompactTalkControlYieldsToLocalVoiceCapture() {
        XCTAssertTrue(CarapaceChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
            isLocalVoiceCaptureActive: false))
        XCTAssertFalse(CarapaceChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = CarapaceVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = CarapaceChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = CarapaceVoiceNoteButton(
            control: control,
            compact: false,
            isComposerEnabled: true,
