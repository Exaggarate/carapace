...
private actor UnreadTestTransportState {
    ...
    var sessionOverride: [CarapaceChatSessionEntry]?
...
private final class UnreadTestTransport: @unchecked Sendable, CarapaceChatTransport {
    ...
    func setSessions(_ sessions: [CarapaceChatSessionEntry]) async {
...
extension UnreadTestTransportState {
    ...
    fileprivate func setSessions(_ sessions: [CarapaceChatSessionEntry]) {
...
struct ChatViewModelUnreadTests {
    ...
    @Test func `failed route lease preserves mutation queue ordering`() async throws {
        ...
        let firstLease = CarapaceChatSessionMutationRouteLease { _, _, _, _, _, _, _, _, _ in
            await recorder.append("first-start")
            try await Task.sleep(for: .milliseconds(100))
            await recorder.append("first-end")
        }
        let thirdLease = CarapaceChatSessionMutationRouteLease { _, _, _, _, _, _, _, _, _ in
            await recorder.append("third")
        }

        let first = queue.reserve(
            routeLease: Task<CarapaceChatSessionMutationRouteLease?, Never> { firstLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
        let second = queue.reserve(
            routeLease: Task<CarapaceChatSessionMutationRouteLease?, Never> { nil },
            queueKey: "a",
            routeKey: "a",
            unread: true)
        let third = queue.reserve(
            routeLease: Task<CarapaceChatSessionMutationRouteLease?, Never> { thirdLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
    ...
    private func viewModel(
        ...
        transport: UnreadTestTransport) -> CarapaceChatViewModel
    {
    ...
    private func entry(
        ...
        pinned: Bool? = nil) -> CarapaceChatSessionEntry
    {
