...
struct ChatGatewayTransportTests {
    @Test(arguments: [CarapaceChatSessionTargetPolicy.preserveBareKeys, .scopeBareKeysToSelectedAgent])
    func `shared operations preserve platform targeting through the transport protocol`(
        policy: CarapaceChatSessionTargetPolicy) async throws
    {
        let recorder = RequestRecorder()
        let transport: any CarapaceChatTransport = Transport(policy: policy, request: { request in
    ...
    @Test func `branch operations use the platform authority hook and propagate its rejection`() async throws {
        let recorder = RequestRecorder()
        let transport: any CarapaceChatTransport = Transport(
    ...
    private actor RequestRecorder {
        ...
        func append(_ request: CarapaceChatGatewayRequest) {
            self.requests.append(request)
        }
    }

    private struct Transport: CarapaceChatGatewayTransport {
        var policy: CarapaceChatSessionTargetPolicy = .preserveBareKeys
        var request: @Sendable (CarapaceChatGatewayRequest) async throws -> Data
        var actionRequest: (@Sendable (CarapaceChatGatewayRequest) async throws -> Data)?
