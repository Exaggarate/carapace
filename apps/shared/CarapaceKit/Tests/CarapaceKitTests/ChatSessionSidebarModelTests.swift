...
struct ChatSessionSidebarModelTests {
    private func entry(
        ...
        endedAt: Double? = nil,
        hasActiveSubagentRun: Bool? = nil) -> CarapaceChatSessionEntry
    {
    ...
    @Test func `gateway groups preserve canonical session order`() {
        let sections = ChatSessionSidebarModel.sections(
            ...
            currentSessionKey: "recent",
            groups: [CarapaceChatSessionGroup(name: "Projects", position: 0)],
            query: "")
    ...
    @Test func `session list hierarchy fields decode with gateway spellings`() throws {
        let data = try #require("""
        {
          ...
          "runtimeMs": 2000,
          "agentRuntime": {"id": "codex", "fallback": "carapace", "source": "session"},
    ...
    @Test func `inspector derives gateway metadata without composer overrides`() {
        let session = CarapaceChatSessionEntry(
            ...
            category: "Projects",
            hasActiveRun: true,
            worktree: CarapaceChatSessionWorktree(
                id: "wt-1",
                branch: "feature/review",
                repoRoot: "/repo"),
            runtimeMs: 1500,
            agentRuntime: CarapaceChatAgentRuntime(id: "codex", fallback: nil, source: "session"))
    ...
    @Test func `partial session change preserves observer and status metadata`() throws {
        let agentStatus = CarapaceChatSessionAgentStatus(
            note: "Waiting on review",
            expiresAt: 5000,
            attention: "hand")
        let observerDigest = CarapaceChatSessionObserverDigest(
            runId: "run-1",
            revision: 3,
            updatedAt: 300,
            headline: "Reviewing",
    ...
    func `session color survives coding and partial events but clears on tombstone`(nested: Bool) throws {
        ...
        for (field, expected) in [
            ...
            let change = try decoder.decode(CarapaceChatSessionsChangedEvent.self, from: Data(payload.utf8))
    ...
    @Test func `active run id tombstone clears exact ids while omission is inert`() throws {
        ...
        let omitted = try decoder.decode(
            CarapaceChatSessionsChangedEvent.self,
    ...
    @Test func `subtitle precedence keeps attention and status above observer digest`() {
        let digest = CarapaceChatSessionObserverDigest(
            ...
            headline: "Observer",
            health: "on-track")
        let agent = CarapaceChatSessionAgentStatus(
            note: "Agent note",
            expiresAt: 10000,
            attention: nil)
