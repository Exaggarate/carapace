...
func makeOutboxStore() throws -> (
    ...
    databases: CarapaceClientDatabases,
    directory: URL)
{
...
func outboxTestCommand(
    ...
    expectedSessionSettings: CarapaceChatSessionSettingsExpectation? = CarapaceChatSessionSettingsExpectation(
        permissionMode: nil,
        toolOverrides: nil)) -> CarapaceChatOutboxCommand
{
...
private func userTexts(_ vm: CarapaceChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
actor OutboxTransportState {
    ...
    var sentSessionSettings: [CarapaceChatSessionSettingsExpectation?] = []
...
final class OutboxTestTransport: @unchecked Sendable, CarapaceChatTransport {
    let state: OutboxTransportState
    private let sessions: [CarapaceChatSessionEntry]
    ...
    {
        self.composerCapabilityCatalog ?? CarapaceChatComposerCapabilityCatalog()
    }
    ...
    func listCommands(sessionKey _: String) async throws -> [CarapaceChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> CarapaceChatHistoryPayload
    {
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__carapace": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return CarapaceChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        expectedRoute: Int?) async throws -> CarapaceChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(CarapaceChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendSettingsChanged {
            throw GatewayResponseError(
                ...
                details: ["reason": AnyCodable(CarapaceChatSessionSettingsContract.changedErrorReason)])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return CarapaceChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return CarapaceChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
func outboxSessionEntry(
    ...
    permissionMode: CarapaceChatPermissionMode? = nil) -> CarapaceChatSessionEntry
{
    CarapaceChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { CarapaceChatThinkingLevelOption(id: $0, label: $0) },
        permissionMode: permissionMode)
}

func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any CarapaceChatCommandOutbox,
    transcriptCache: (any CarapaceChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> CarapaceChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: CarapaceChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
actor ScriptedOutbox: CarapaceChatCommandOutbox {
    enum Forwarding { case full, minimal, holdingCancellation }

    private nonisolated let base: CarapaceChatSQLiteTranscriptCache
    ...
    private var terminalWriteResult: CarapaceChatOutboxUpdateResult = .updated
    ...
    init(base: CarapaceChatSQLiteTranscriptCache, forwarding: Forwarding = .full) {
        self.base = base
        self.forwarding = forwarding
    }

    nonisolated func changes() -> AsyncStream<CarapaceChatOutboxChange> {
        self.base.changes()
    }
    ...
    func setTerminalWriteResult(_ result: CarapaceChatOutboxUpdateResult) {
    ...
    func markCommandAwaitingConfirmation(id: String, attemptVersion: Int) async -> CarapaceChatOutboxUpdateResult {
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        #expect(commands.map(\.expectedSessionSettings) == [
            CarapaceChatSessionSettingsExpectation(permissionMode: nil, toolOverrides: nil),
        ])
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        let (store, _, databaseDirectory) = try makeOutboxStore()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        let message = CarapaceChatTransportUpgradeMessage.routingContract
    ...
    @Test @MainActor func `branch listing unsupported matcher rejects scope denials and accepts definitive absence`() {
        #expect(!CarapaceChatViewModel.branchListingIsUnsupported(GatewayResponseError(
            method: "sessions.branches.list",
            code: "INVALID_REQUEST",
            message: "missing scope: operator.admin",
            details: nil)))
        ...
        #expect(CarapaceChatViewModel.branchListingIsUnsupported(GatewayResponseError(
            ...
            message: "sessions.branches.list is unsupported on this gateway",
            details: nil)))
        #expect(CarapaceChatViewModel.branchListingIsUnsupported(
            CarapaceChatViewModel.BranchListingUnadvertisedError()))
    }
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: CarapaceChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = CarapaceChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: CarapaceChatOutboxScope(sessionKey: "main", agentID: "main")))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: CarapaceChatOutboxScope(sessionKey: "main", agentID: nil)))
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            CarapaceChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == CarapaceChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(CarapaceChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: CarapaceChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(CarapaceChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: CarapaceChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = CarapaceChatMessage(
            role: "user",
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == CarapaceChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(CarapaceChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: CarapaceChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = ScriptedOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            CarapaceChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            CarapaceChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(CarapaceSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == CarapaceChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    func `queued send transport failure fails closed until explicit retry`(retryBeforeReconnect: Bool) async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == CarapaceChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
    ...
    @Test func `deleting a queued message removes bubble and durable row`() async throws {
        ...
        let staleMessage = CarapaceChatMessage(
            role: "user",
            content: [CarapaceChatMessageContent(
                type: "text",
...
extension ChatViewModelOutboxTests {
    ...
    @Test func `send right after a session switch still queues behind that session's backlog`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: CarapaceChatOutboxScope(sessionKey: "second", agentID: nil)))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: CarapaceChatOutboxScope(sessionKey: "second", agentID: "main")))
