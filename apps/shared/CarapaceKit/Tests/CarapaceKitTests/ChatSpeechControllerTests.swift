...
private final class GatedClipPlayer: ChatSpeechClipPlaying {
    var playedClips: [CarapaceChatSpeechClip] = []
    var stopCount = 0
    private var pending: CheckedContinuation<Bool, Never>?

    func play(clip: CarapaceChatSpeechClip) async -> Bool {
...
private struct SpeechHarness {
    let controller: CarapaceChatSpeechController
    let clipPlayer: GatedClipPlayer
    let localSpeaker: RecordingLocalSpeaker

    init(synthesize: @escaping CarapaceChatSpeechSynthesis) {
        let clipPlayer = GatedClipPlayer()
        let localSpeaker = RecordingLocalSpeaker()
        self.clipPlayer = clipPlayer
        self.localSpeaker = localSpeaker
        self.controller = CarapaceChatSpeechController(
...
private func waitForPhase(
    _ controller: CarapaceChatSpeechController,
    _ expected: CarapaceChatSpeechController.Phase) async -> Bool
{
    for _ in 0..<200 {
...
struct ChatSpeechControllerTests {
    ...
    @Test func `falls back to local speech when clip is unplayable`() async {
        let clip = CarapaceChatSpeechClip(data: Data([1]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let messageID = UUID()
    ...
    @Test func `toggle while active stops without fallback`() async {
        let clip = CarapaceChatSpeechClip(data: Data([5]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let messageID = UUID()
    ...
    @Test func `starting another message supersedes the first`() async {
        let clip = CarapaceChatSpeechClip(data: Data([7]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let first = UUID()
        let second = UUID()
    ...
    @Test func `blank text stays idle`() async {
        let harness = SpeechHarness { _ in
            CarapaceChatSpeechClip(data: Data([1]), mimeType: nil)
        }
    ...
    @Test func `identifies container hints and headerless formats`() {
        let mp3 = CarapaceChatSpeechClip(
            data: Data([1]),
            outputFormat: "mp3",
            mimeType: "audio/mpeg",
            fileExtension: ".mp3")
        let pcm = CarapaceChatSpeechClip(
            data: Data([1]),
            outputFormat: "pcm",
            mimeType: "audio/pcm",
            fileExtension: ".pcm")
        let azureRaw = CarapaceChatSpeechClip(
            data: Data([1]),
            outputFormat: "raw-8khz-8bit-mono-mulaw",
            fileExtension: ".pcm")
        let sampledPCM = CarapaceChatSpeechClip(
            data: Data([1]),
            outputFormat: "pcm_24000",
            fileExtension: ".pcm")
        let rawULaw = CarapaceChatSpeechClip(
            data: Data([1]),
