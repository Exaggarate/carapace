...
struct ChatInlineWidgetTests {
    ...
    @Test func `decodes projected canvas widget block`() throws {
          ...
          "content": [
            {"type": "text", "text": "Done"},
            {
              ...
              "preview": {
                ...
                "title": "Status",
                "preferredHeight": 240,
                "url": "/__carapace__/canvas/documents/widget-1/index.html",
                "sandbox": "scripts"
              }
            }
          ]
        ...
        let message = try JSONDecoder().decode(CarapaceChatMessage.self, from: data)
        let preview = try #require(message.content.last?.preview)
        #expect(preview.inlineWidgetPath == "/__carapace__/canvas/documents/widget-1/index.html")
        #expect(preview.inlineWidgetHeight == 240)

        let unsafe = CarapaceChatCanvasPreview(
            kind: "canvas",
            surface: "assistant_message",
            render: "url",
            title: nil,
    ...
    @Test func `resolves only capability scoped widget documents`() {
        let surface = "https://gateway.example/__carapace__/cap/token"
        let target = "/__carapace__/canvas/documents/widget-1/index.html"

        #expect(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: target)?.absoluteString ==
            "https://gateway.example/__carapace__/cap/token/__carapace__/canvas/documents/widget-1/index.html")
        #expect(CarapaceChatWidgetURLResolver.resolve(surfaceURL: "https://gateway.example", target: target) == nil)
        #expect(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "https://attacker.example/widget.html") == nil)
        #expect(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/a2ui/index.html") == nil)
        #expect(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/documents/%252e%252e/index.html") == nil)
        #expect(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__carapace__/canvas/documents/%2525252525252525252525252525252525/index.html") == nil)
    }
    ...
    @Test func `resource equality ignores internal recovery metadata`() throws {
        ...
        let publicResource = CarapaceChatWidgetResource(url: url)
        let trackedResource = CarapaceChatWidgetResource(
            url: url,
    ...
    @Test func `uses replacement route after capability refresh loses its lease`() async throws {
        let target = "/__carapace__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://gateway.example/__carapace__/cap/old"
        let newSurface = "https://gateway.example/__carapace__/cap/new"
        let failedURL = try #require(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = CarapaceChatWidgetResource(url: failedURL)
        ...
        let resolved = await CarapaceChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(resolved?.url == CarapaceChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))
    ...
    @Test func `accepts a same URL widget route when it acquires a TLS pin`() async throws {
        ...
        let initialResource = await CarapaceChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
    ...
    @Test func `refreshes node once before trying the operator fallback`() async {
        ...
        let oldSurface = "https://gateway.example/__carapace__/cap/old"
        let newSurface = "https://gateway.example/__carapace__/cap/new"
        let fallbackSurface = "https://operator.example/__carapace__/cap/fallback"
        let probe = ChatWidgetRouteReconnectProbe(
            route: GatewayCanvasHostRoute(url: oldSurface, tlsFingerprintSHA256: nil),
            replacement: GatewayCanvasHostRoute(url: newSurface, tlsFingerprintSHA256: nil),
            operatorRoute: GatewayCanvasHostRoute(url: fallbackSurface, tlsFingerprintSHA256: nil))
        let initialNode = await CarapaceChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
            refreshNodeSurfaceRoute: { _ in nil },
        ...
        let refreshedNode = await CarapaceChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(refreshedNode?.url == CarapaceChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))

        let fallback = await CarapaceChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(fallback?.url == CarapaceChatWidgetURLResolver.resolve(
    ...
    @Test func `refreshes the operator capability when the node route is unavailable`() async throws {
        let target = "/__carapace__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://operator.example/__carapace__/cap/old"
        let newSurface = "https://operator.example/__carapace__/cap/new"
        let failedURL = try #require(CarapaceChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = CarapaceChatWidgetResource(url: failedURL)
    ...
    @Test func `rejects a TLS pin on a cleartext widget route`() async {
        ...
        let route = GatewayCanvasHostRoute(
            url: "http://gateway.example/__carapace__/cap/token",
    ...
    @Test @MainActor func `widget snapshots reject a changed document or generation before capture`() throws {
        let fixture = try ChatInlineWidgetSnapshotFixture()
        let replacement = try CarapaceChatWidgetResource(
            url: #require(URL(string: "https://gateway.example/replacement")))
        let changedPin = CarapaceChatWidgetResource(
    ...
    @Test @MainActor func `widget snapshot completions cannot outlive document or TLS replacements`() throws {
        ...
        let replacements = [
            CarapaceChatWidgetResource(url: newURL),
            CarapaceChatWidgetResource(url: oldURL, tlsFingerprintSHA256: String(repeating: "ab", count: 32)),
        ]
...
private final class ChatInlineWidgetSnapshotFixture {
    let resource: CarapaceChatWidgetResource
    let generation = UUID()
    ...
    init() throws {
        self.resource = try CarapaceChatWidgetResource(
    ...
    func request(
        ...
        generation: UUID? = nil,
        resource: CarapaceChatWidgetResource? = nil) -> ChatInlineWidgetSnapshotRequest
    {
    ...
    func capture(
        ...
        generation: UUID? = nil,
        resource: CarapaceChatWidgetResource? = nil)
    {
        self.owner.capture(
            request,
