...
struct ComputerInputGeometryTests {
    private func display(
        ...
        originY: Double = 0,
        width: Double,
        height: Double) -> CarapaceComputerDisplayGeometry
    {
        CarapaceComputerDisplayGeometry(
            originX: originX,
            originY: originY,
            widthPoints: width,
            heightPoints: height)
    ...
    private func capturedWidth(refWidth: Int?, sourceWidth: Double, sourceHeight: Double = 0) -> Double {
        CarapaceComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
    ...
    @Test func `display frame identity changes with physical display geometry`() {
        ...
        let original = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let repeated = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let reindexed = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 43,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let resized = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 1920,
            sourceHeight: 1200,
            referenceWidth: 1280,
            display: display)
        let moved = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
        ...
        let rescaled = CarapaceComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
    ...
    @Test func `maps one to one when capture matches display`() {
        ...
        let mapped = CarapaceComputerInputGeometry.mapReferencePointToGlobal(
            x: 100,
            y: 200,
    ...
    @Test func `accepts finite positive mapping geometry with negative display origins`() {
        ...
        #expect(CarapaceComputerInputGeometry.isValidMappingGeometry(
            sourceWidth: 2560,
            sourceHeight: 1440,
            display: display))
    }
    ...
    @Test func `clamps far-edge and out-of-bounds points strictly inside the display`() {
        ...
        let farEdge = CarapaceComputerInputGeometry.clampToDisplay(
            x: 100 + 1280,
            y: 200 + 800,
            display: display)
        ...
        let negative = CarapaceComputerInputGeometry.clampToDisplay(x: 99, y: 199, display: display)
        #expect(negative.x == 100)
        #expect(negative.y == 200)
        // A point already inside is unchanged.
        let inside = CarapaceComputerInputGeometry.clampToDisplay(x: 640, y: 400, display: display)
        #expect(inside.x == 640)
        #expect(inside.y == 400)
    }
    ...
    @Test func `decodes partial act params`() throws {
        ...
        let params = try JSONDecoder().decode(CarapaceComputerActParams.self, from: data)
    ...
    @Test func `decodes scroll and hold actions`() throws {
        ...
        let scrollParams = try JSONDecoder().decode(CarapaceComputerActParams.self, from: scroll)
        ...
        let holdParams = try JSONDecoder().decode(CarapaceComputerActParams.self, from: hold)
    ...
    @Test func `input result encoding uses the canonical success envelope`() throws {
        let data = try JSONEncoder().encode(CarapaceComputerActResult(ok: true))
    ...
    @Test func `v2 result encoding matches the shared result envelope`() throws {
        let result = CarapaceComputerActResult(
            ok: false,
            ...
            observation: CarapaceComputerObservation(
                ...
                height: 20,
                observationId: "observation-1",
                elements: [CarapaceComputerObservationElement(
                    elementRef: "element-1",
                    role: "button",
                    label: "Save",
                    bounds: CarapaceComputerBounds(x: 1, y: 2, width: 3, height: 4))]),
