...
struct ChatQuestionCardTests {
    @Test func `question card single select and other are exclusive`() {
        let model = CarapaceQuestionCardModel(record: questionRecord())
    ...
    @Test func `question card multi select uses declared option order`() {
        let model = CarapaceQuestionCardModel(record: questionRecord(multiSelect: true))
    ...
    @Test func `question card maps expiry and answer origin`() {
        ...
        let expired = CarapaceQuestionCardModel(record: questionRecord(expiresAtMs: 1_499_000))
        #expect(expired.status(at: now) == .expired)
        #expect(expired.remainingSeconds(at: now) == 0)

        let remote = CarapaceQuestionCardModel(record: questionRecord())
        remote.apply(resolved: CarapaceQuestionResolvedEvent(id: remote.id, status: .answered))
        #expect(remote.status(at: Date(timeIntervalSince1970: 1500)) == .answeredElsewhere)

        let local = CarapaceQuestionCardModel(record: questionRecord())
        local.markAnsweredLocally(answers: QuestionAnswers(answers: ["meal": AnyCodable(["Pizza"])]))
        local.apply(resolved: CarapaceQuestionResolvedEvent(id: local.id, status: .answered))
    ...
    @Test func `question card pending refresh preserves submission`() {
        let model = CarapaceQuestionCardModel(record: questionRecord(expiresAtMs: Int.max))
        model.toggleOption(questionID: "meal", label: "Pizza")
    ...
    @Test func `question card preserves run identity across local terminal records`() {
        let answered = CarapaceQuestionCardModel(record: questionRecord())
        ...
        let skipped = CarapaceQuestionCardModel(record: questionRecord())
        skipped.markSkippedLocally()
        #expect(skipped.record.runid == "run-question")

        let elsewhere = CarapaceQuestionCardModel(record: questionRecord())
        elsewhere.markAnsweredElsewhere()
        #expect(elsewhere.record.runid == "run-question")

        let resolved = CarapaceQuestionCardModel(record: questionRecord())
        resolved.apply(resolved: .init(id: resolved.id, status: .answered))
        #expect(resolved.record.runid == "run-question")

        let refreshed = CarapaceQuestionCardModel(record: questionRecord(answers: .init(answers: [:])))
    ...
    @Test func `question card locally expired state remains terminal`() {
        ...
        let model = CarapaceQuestionCardModel(record: questionRecord(expiresAtMs: 1_500_000))
    ...
    @Test func `question completions override unavailable recovery race`() throws {
        ...
        let answeredElsewhere = CarapaceQuestionCardModel(record: questionRecord())
    ...
    @Test func `question card terminal summaries prefer resolved answers`() {
        ...
        let answered = CarapaceQuestionCardModel(record: questionRecord(status: .answered, answers: answers))
        let question = answered.record.questions[0]
        #expect(answered.terminalSummaryText(for: question) == "Pizza, extra hot")

        let elsewhere = CarapaceQuestionCardModel(record: questionRecord(status: .answered))
        #expect(elsewhere.terminalSummaryText(for: question) == "Answered elsewhere")

        let skipped = CarapaceQuestionCardModel(record: questionRecord(status: .cancelled))
        #expect(skipped.terminalSummaryText(for: question) == "Skipped")

        let expired = CarapaceQuestionCardModel(record: questionRecord(status: .expired))
        #expect(expired.terminalSummaryText(for: question) == "Expired")

        let unavailable = CarapaceQuestionCardModel(record: questionRecord())
    ...
    func `secret draft preserves bytes`(value: String) {
        let model = CarapaceQuestionCardModel(record: questionRecord(isSecret: true))
    ...
    @Test func `host consent preserves proposal and allows empty override`() {
        ...
        #expect(CarapaceQuestionCardModel(record: questionRecord()).secretStoreAllowedHosts == nil)
    }
