...
private actor AttachmentSendCapture {
    private(set) var attachments: [CarapaceChatAttachmentPayload] = []
    private(set) var sends = 0

    func store(_ attachments: [CarapaceChatAttachmentPayload]) {
    ...
    func first() -> CarapaceChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: CarapaceChatTransport {
        ...
        attachments: [CarapaceChatAttachmentPayload]) async throws -> CarapaceChatSendResponse
    {
        ...
        return CarapaceChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() throws -> CarapaceChatSQLiteTranscriptCache {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("attachment-outbox-\(UUID().uuidString)", isDirectory: true)
    return try CarapaceClientDatabases(directoryURL: directory).store(gatewayID: "attachment-tests")
}

@MainActor
private func makeDurableAttachmentViewModel(
    transport: AttachmentProcessingTransport,
    outbox: CarapaceChatSQLiteTranscriptCache) -> CarapaceChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testVideoFileUsesServerDefaultTwentyMiBCap() async throws {
        ...
        try Data(count: CarapaceChatViewModel.maxVideoAttachmentBytes + 1).write(to: fileURL)
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testUnsupportedAudioFileIsRejectedBeforeReadingItsPayload() async throws {
        ...
        try handle.truncate(atOffset: UInt64(CarapaceChatViewModel.maxVideoAttachmentBytes * 10))
        try handle.close()
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() async {
        let state = await MainActor.run {
            ...
            let contractViewModel = CarapaceChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let contractAttachment = CarapacePendingAttachment(
                url: nil,
            ...
            let agentViewModel = CarapaceChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let agentAttachment = CarapacePendingAttachment(
                url: nil,
    ...
    func testIndeterminateOutboxRouteRetainsAttachmentAndRetriesOnceAvailable() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = CarapacePendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.jpg",
                mimeType: "image/jpeg",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = CarapacePendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = try makeAttachmentOutbox()
        var firstViewModel: CarapaceChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__carapace":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = CarapaceChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let incoming = try JSONDecoder().decode(
            CarapaceChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__carapace":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))
