...
private final class ComposerParityTransport: @unchecked Sendable, CarapaceChatTransport {
        ...
        attachments _: [CarapaceChatAttachmentPayload]) async throws -> CarapaceChatSendResponse
    {
        ...
        return CarapaceChatSendResponse(runId: idempotencyKey, status: "started")
    }
...
struct ChatReplyQuoteTests {
    @Test func `single line quote matches web fixture and escapes attribution`() {
        let target = CarapaceChatReplyTarget(
            messageID: UUID(),
            text: "quoted body",
            senderLabel: "A *B* [C]")
...
struct ChatComposerStateTests {
    @Test func `model selection target describes only gateway owned values`() {
        let viewModel = CarapaceChatViewModel(sessionKey: "main", transport: ComposerParityTransport())
        ...
        for (target, expected) in expectations {
            viewModel.sessionDefaults = CarapaceChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func `model selection matches qualified choices after gateway readback`() {
        ...
        #expect(CarapaceChatViewModel.modelSelectionMatches(
    ...
    @Test func `file picker allows images and movie containers only`() {
        #expect(CarapaceChatPickerAttachmentMetadata.allowedFileContentTypes == [
            .image,
            .movie,
            .mpeg4Movie,
            .quickTimeMovie,
        ])
        #expect(!CarapaceChatPickerAttachmentMetadata.allowedFileContentTypes.contains(.audiovisualContent))
        #expect(!CarapaceChatPickerAttachmentMetadata.allowedFileContentTypes.contains(.audio))
    }

    @Test func `tagless movie metadata stages as video`() async throws {
        let metadata = CarapaceChatPickerAttachmentMetadata.resolve(contentType: .movie)
        ...
        let vm = CarapaceChatViewModel(sessionKey: "main", transport: ComposerParityTransport())
    ...
    @Test func `accepted normal send quotes and clears reply and stored draft`() async throws {
        ...
        let vm = CarapaceChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.input = "continue"
    ...
    @Test func `accepted attachment only send quotes and clears reply`() async throws {
        ...
        let vm = CarapaceChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.attachments = [CarapacePendingAttachment(
            url: nil,
            data: Data([1, 2, 3]),
            fileName: "image.png",
            mimeType: "image/png",
