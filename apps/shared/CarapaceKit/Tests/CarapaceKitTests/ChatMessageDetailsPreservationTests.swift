...
/// Tool-result diff metadata rides on `CarapaceChatMessage.details`; every
...
struct ChatMessageDetailsPreservationTests {
    private func toolResultMessage(id: UUID = UUID()) -> CarapaceChatMessage {
        CarapaceChatMessage(
            id: id,
            role: "toolResult",
            content: [
    ...
    private func systemNoticeMessage(id: UUID = UUID()) -> CarapaceChatMessage {
        CarapaceChatMessage(
            id: id,
            role: "user",
            content: [
    ...
    @MainActor @Test func `decode pipeline keeps message details`() throws {
        ...
        let decoded = CarapaceChatViewModel.decodeMessages(anyMessages)
    ...
    @MainActor @Test func `canonical adoption keeps incoming details`() {
        ...
        let adoptedNotice = CarapaceChatViewModel.adoptingCanonicalMessage(
            incomingNotice,
