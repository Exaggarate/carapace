...
private func nodeConnectOptions(
    caps: [String] = [],
    commands: [String] = [],
    clientId: String = "carapace-ios-test",
...
struct GatewayNodeSessionTests {
    ...
    @Test func `operator canvas refresh uses the operator surface method`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__carapace__/cap/operator-token",
                ],
            ])
    ...
    @Test func `canvas surface refresh is shared across callers with different timeouts`() async throws {
        ...
        let gateway = GatewayNodeSession()
        let options = nodeConnectOptions(caps: ["canvas"], clientId: "carapace-macos", clientDisplayName: "macOS Test")
        ...
        task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__carapace__/cap/new-token",
                ],
            ])
    ...
    @Test func `last timed out surface waiter releases stalled rotation for retry`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__carapace__/cap/retry-token",
                ],
            ])
    ...
    @Test func `lagging canvas refresh reuses the rotated capability`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__carapace__/cap/old-token",
                ],
            ])
    ...
    func `watch approval warning text is optional and round trips`() throws {
        let legacy = try JSONDecoder().decode(
            CarapaceWatchExecApprovalItem.self,
    ...
    func `watch approval recovery schema carries exact resolution attempt identifiers`() throws {
        ...
        let prompt = CarapaceWatchExecApprovalPromptMessage(
        ...
        let request = CarapaceWatchExecApprovalSnapshotRequestMessage(
            requestId: "request",
    ...
    func `route switch rejects a decoded invoke before detached admission`() async throws {
        ...
        func retireBeforeDetachedAdmission(to gateway: isolated GatewayNodeSession) async {
            await gateway._test_handlePush(
                nodeInvokePush(id: "stale-computer", command: CarapaceComputerCommand.act.rawValue),
                socketGeneration: 1)
            await gateway.disconnect()
        }
        ...
        replacementSocket.emitInvokeRequest(id: "current-computer", command: CarapaceComputerCommand.act.rawValue)
    ...
    @Test(arguments: [
        ...
        CarapaceCameraCommand.ptzControl.rawValue,
    ...
    func `route switch cancels queued device work and waits for invoke cleanup`(command: String) async throws {
        ...
        let options = nodeConnectOptions(commands: [command], clientId: "carapace-macos")
        ...
        try await gateway.connectForTest(
            ...
            onInvoke: { request in
                ...
                return BridgeInvokeResponse(
                    id: request.id,
                    ok: false,
                    error: CarapaceNodeError(code: .unavailable, message: "UNAVAILABLE: route changed"))
            })
            ...
            onInvoke: { request in
                ...
                if Task.isCancelled {
                    ...
                    return BridgeInvokeResponse(
                        id: request.id,
                        ok: false,
                        error: CarapaceNodeError(code: .unavailable, message: "UNAVAILABLE: canceled"))
                }
    ...
    func `node invoke cancellation before detached admission prevents the side effect`() async throws {
        ...
        let admissions = DisconnectProbe()
        let command = CarapaceCameraCommand.ptzControl.rawValue
        try await gateway.connectForTest(
            testURL("ws://gateway.example.invalid"),
            options: nodeConnectOptions(commands: [command], clientId: "carapace-macos"),
            session: session,
            onInvoke: { request in
        ...
        command: String) async throws
    {
        ...
        let cancellation = InvokeCancellationFlag()
        let options = nodeConnectOptions(commands: [command], clientId: "carapace-ios")
        try await gateway.connectForTest(
            testURL("ws://first.example.invalid"),
            options: options,
            session: session,
    ...
    func `replacement invoke fails promptly while disconnect lifecycle is blocked`() async throws {
        ...
        #expect(params["ok"] as? Bool == false)
        #expect(error["code"] as? String == CarapaceNodeErrorCode.notReady.rawValue)
    ...
    func `route bound node event request distinguishes handled and legacy acknowledgements`() async throws {
        let session = FakeGatewayWebSocketSession()
        let gateway = GatewayNodeSession()
        let options = nodeConnectOptions(clientId: "carapace-macos-test", clientDisplayName: "macOS Test")
    ...
    func `node invoke requests keep receiving while system run is blocked`() async throws {
        ...
        #expect(error["code"] as? String == CarapaceNodeErrorCode.unavailable.rawValue)
    ...
    func `computer invoke receipts deduplicate in flight and after reconnect`() async throws {
        ...
        #expect(mismatchError["code"] as? String == CarapaceNodeErrorCode.invalidRequest.rawValue)
        #expect(await probe.count() == 1)
    ...
    func `share extension identity profile uses separate node identity and token store`() async throws {
        ...
        let options = nodeConnectOptions(
            clientId: "carapace-ios",
            clientDisplayName: "Carapace Share",
    ...
    func `connect failure preserves protocol mismatch details`() async throws {
        ...
        do {
            ...
            #expect(problem?
                .message == "This app is older than the gateway. Update Carapace on this device, then retry.")
    ...
    func `failed device token write retains issuance without claiming persistence`() async throws {
        ...
        let stateDir = try #require(ProcessInfo.processInfo.environment["CARAPACE_STATE_DIR"])
    ...
    func `normalize canvas host url preserves explicit secure canvas port`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "https://canvas.example.com:9443/__carapace__/cap/token",
            against: URL(string: "wss://gateway.example.com"))

        #expect(normalized == "https://canvas.example.com:9443/__carapace__/cap/token")
    }

    @Test
    func `normalize canvas host url backfills gateway host for loopback canvas`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "http://127.0.0.1:18789/__carapace__/cap/token",
            against: URL(string: "wss://gateway.example.com:7443"))

        #expect(normalized == "https://gateway.example.com:7443/__carapace__/cap/token")
    }
    ...
    func `gateway TLS pin applies only to the same canvas endpoint`() {
        ...
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:7443/__carapace__/cap/token",
            gatewayURL: gateway) == fingerprint)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://canvas.example.com:7443/__carapace__/cap/token",
            gatewayURL: gateway) == nil)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:9443/__carapace__/cap/token",
            gatewayURL: gateway) == nil)
    }
    ...
    func `main session key follows the current node snapshot route`() async throws {
        ...
        let options = nodeConnectOptions(clientId: "carapace-macos", clientDisplayName: "macOS Test")
