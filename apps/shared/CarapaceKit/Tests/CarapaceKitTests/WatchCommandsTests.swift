...
struct WatchCommandsTests {
    @Test func `app snapshot dual writes semantic and legacy status fields`() throws {
        let message = CarapaceWatchAppSnapshotMessage(
            ...
            chatStatus: CarapaceWatchAppStatus(code: .chatNoMessages),
    ...
    @Test func `semantic chat status always writes the shipped text field`() throws {
        let message = CarapaceWatchAppSnapshotMessage(
            ...
            talkSpeaking: false,
            pendingApprovalCount: 0,
            chatStatus: CarapaceWatchAppStatus(code: .chatUnavailable))
    ...
    @Test func `unknown semantic statuses fall back to legacy text`() throws {
        ...
        let message = try JSONDecoder().decode(
            CarapaceWatchAppSnapshotMessage.self,
            from: Data(json.utf8))

        #expect(message.gatewayStatus == CarapaceWatchAppStatus(
            code: .legacy,
            verbatim: "Future gateway state"))
        #expect(message.talkStatus == CarapaceWatchAppStatus(
            code: .legacy,
            verbatim: "Future Talk state"))
        #expect(message.chatStatus == CarapaceWatchAppStatus(
            code: .legacy,
