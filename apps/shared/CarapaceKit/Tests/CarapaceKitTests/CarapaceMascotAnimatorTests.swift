...
struct CarapaceMascotAnimatorTests {
    private func makeAnimator(seed: UInt64 = 7, interactive: Bool = false) -> CarapaceMascotAnimator {
        CarapaceMascotAnimator(seed: seed, hourOfDay: 12, allowsAutoSleep: interactive)
    }

    @Test func `poses stay inside drawable bounds for every mood`() {
        for mood in CarapaceMascotMood.allCases {
    ...
    @Test func `sleepy mood wears the nightcap`() {
        ...
        let staticPose = CarapaceMascotPose.staticPose(for: .sleepy)
    ...
    @Test func `static poses carry the mood signature`() {
        let sad = CarapaceMascotPose.staticPose(for: .sad)
        #expect(sad.antennaDroop > 0)
        #expect(sad.mouthCurve < 0)
        let celebrating = CarapaceMascotPose.staticPose(for: .celebrating)
        #expect(celebrating.leftClawDegrees > 0)
        #expect(celebrating.mouthCurve > 0)
        let idle = CarapaceMascotPose.staticPose(for: .idle)
        #expect(idle == CarapaceMascotPose())
        let working = CarapaceMascotPose.staticPose(for: .working)
