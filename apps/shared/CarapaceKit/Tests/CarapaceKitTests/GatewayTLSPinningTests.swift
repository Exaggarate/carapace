...
struct GatewayTLSPinningTests {
    ...
    @Test func `keychain namespace configures once and fails closed after use`() {
        ...
        let workService = state.service(base: "ai.carapace.tls-pinning")
        ...
        #expect(workService == "ai.carapace.tls-pinning.profile.work")
        #expect(configuredWorkAfterUse)
        #expect(!configuredDefaultAfterUse)

        var usedDefault = GatewayTLSKeychainNamespaceState()
        let defaultService = usedDefault.service(base: "ai.carapace.tls-pinning")
        ...
        #expect(defaultService == "ai.carapace.tls-pinning")
