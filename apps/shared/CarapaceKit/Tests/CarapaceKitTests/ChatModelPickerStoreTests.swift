...
private func pickerModel(_ selectionID: String) -> CarapaceChatModelChoice {
...
@Suite struct ChatModelPickerStoreTests {
    ...
    @Test func `recents dedupe move to front cap and skip invalid ids`() throws {
        try withPickerStore { store, _ in
            store.recordRecent("")
            store.recordRecent(CarapaceChatViewModel.defaultModelSelectionID)
    ...
    @Test func `qualified global default matches provider model pair`() {
        ...
        let qualifiedModelID = CarapaceChatModelChoice(
            modelID: "openai/gpt",
            name: "GPT",
            provider: "openai",
            contextWindow: nil)
