...
private func sessionActionOutboxCommand(
    id: String,
    text: String) -> CarapaceChatOutboxCommand
{
...
private final class LegacyForkTransport: @unchecked Sendable, CarapaceChatTransport {
    let state = LegacyForkTransportState()

    func requestHistory(sessionKey _: String) async throws -> CarapaceChatHistoryPayload {
...
private final class SessionActionTransport: @unchecked Sendable, CarapaceChatTransport {
    ...
    private let rewindEditorAttachments: [CarapaceChatEditorAttachment]?
    private let forkAtMessageSessionKey: String
    private let forkAtMessageEditorText: String?
    private let forkAtMessageEditorAttachments: [CarapaceChatEditorAttachment]?
    private let branches: [CarapaceChatSessionBranch]
    private let branchListResponses: [[CarapaceChatSessionBranch]]
    ...
    init(
        ...
        rewindEditorAttachments: [CarapaceChatEditorAttachment]? = nil,
        forkAtMessageSessionKey: String = "forked-at-message",
        forkAtMessageEditorText: String? = "forked draft",
        forkAtMessageEditorAttachments: [CarapaceChatEditorAttachment]? = nil,
        branches: [CarapaceChatSessionBranch] = [],
        branchListResponses: [[CarapaceChatSessionBranch]] = [],
        ...
        entryId: String) async throws -> CarapaceChatRewindResponse
    {
        ...
        return CarapaceChatRewindResponse(
        ...
        entryId: String) async throws -> CarapaceChatForkAtMessageResponse
    {
        ...
        return CarapaceChatForkAtMessageResponse(
        ...
        agentID _: String?) async throws -> CarapaceChatSessionBranchesResponse
    {
        ...
        let branches = self.branchListResponses.indices.contains(callIndex)
            ...
            : self.branches
        return CarapaceChatSessionBranchesResponse(branches: branches)
    }
    ...
    func acquireSessionGroupsRouteLease() async -> CarapaceChatSessionGroupsRouteLease? {
        ...
        return CarapaceChatSessionGroupsRouteLease(
            listGroups: {
                CarapaceChatSessionGroupsResponse(groups: [
                    CarapaceChatSessionGroup(name: "Existing", position: 0),
                ])
            },
            putGroups: { names in
                await state.recordGroupPut(names)
                return CarapaceChatSessionGroupsMutationResponse(
                    ok: true,
                    groups: names.enumerated().map {
                        CarapaceChatSessionGroup(name: $0.element, position: $0.offset)
                    },
                    updatedSessions: nil)
            },
            renameGroup: { _, _ in
                CarapaceChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            },
            deleteGroup: { _ in
                CarapaceChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            })
        ...
        return CarapaceChatNewSessionRouteLease(
            listAgents: {
                CarapaceChatAgentsListResponse(
                    defaultId: "worker",
                    agents: [CarapaceChatAgentChoice(id: "worker", workspaceGit: true)])
            },
            createSession: { key, _, agentID, parentKey, _, _ in
                ...
                return CarapaceChatCreateSessionResponse(ok: true, key: key, sessionId: nil)
            })
    }
...
struct ChatViewModelSessionActionTests {
    ...
    @Test func `remote group mutations bump the catalog revision`() async {
        ...
        let viewModel = await MainActor.run {
            CarapaceChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    @Test func `batch delete rejects current session while attachment owner is pinned`() async {
        let transport = SessionActionTransport()
        let viewModel = CarapaceChatViewModel(sessionKey: "worker", transport: transport)
        viewModel.attachments = [CarapacePendingAttachment(
            url: nil,
            data: Data([1]),
            fileName: "draft.png",
            mimeType: "image/png",
    ...
    @Test func `rewind seeds editor and refreshes history`() async {
        ...
        let transport = SessionActionTransport(
            ...
            rewindEditorAttachments: [
                CarapaceChatEditorAttachment(
                    mimeType: "image/png",
                    data: imageData.base64EncodedString()),
                CarapaceChatEditorAttachment(mimeType: "image/png", data: "%%%"),
            ])
        let viewModel = CarapaceChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `fork at message waits for current session outbox confirmation`() async throws {
        let directory = try makeSessionActionOutboxDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try CarapaceClientDatabases(directoryURL: directory).store(gatewayID: "gw-test")
        #expect(await store.enqueueCommand(sessionActionOutboxCommand(
            id: "fork-pending",
    ...
    @Test func `rewind bumps branch epoch and parks a racing enqueue`() async throws {
        ...
        #expect(CarapaceChatSQLiteTranscriptCache.outboxDisplayError(racedCommand.lastError) ==
    ...
    @Test func `branch message count uses localized singular and plural forms`() {
        #expect(CarapaceChatComposer.branchMessageCount(1) == "1 message")
        #expect(CarapaceChatComposer.branchMessageCount(2) == "2 messages")
    }
    ...
    @Test func `fork at message switches and seeds editor`() async {
        ...
        let transport = SessionActionTransport(
            ...
            forkAtMessageEditorAttachments: [CarapaceChatEditorAttachment(
                mimeType: "image/webp",
                data: imageData.base64EncodedString())])
        let viewModel = CarapaceChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `boundary-aware fork remains compatible with legacy transports`() async throws {
        ...
        let transport: any CarapaceChatTransport = legacy
    ...
    private func waitForBranchSwitchActivityToClear(
        _ viewModel: CarapaceChatViewModel,
        timeout: Duration = .seconds(15)) async -> Bool
    {
        let clock = ContinuousClock()
        let deadline = clock.now + timeout
    ...
    private func userMessage(entryID: String) -> CarapaceChatMessage {
        CarapaceChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func confirmingMessage(commandID: String) -> CarapaceChatMessage {
        CarapaceChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func branches(activeLeafEntryID: String = "leaf-active") -> [CarapaceChatSessionBranch] {
        [
            CarapaceChatSessionBranch(
    ...
    private func entry(
        ...
        hasActiveRun: Bool? = nil) -> CarapaceChatSessionEntry
    {
