    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__carapace"] = ["idempotencyKey": idempotencyKey]
    }
...
private func chatTextModelMessage(
    ...
    text: String,
    timestamp: Double,
    idempotencyKey: String? = nil) -> CarapaceChatMessage
{
    CarapaceChatMessage(
        role: role,
        content: [
...
private func historyPayload(
    ...
    agentId: String? = nil) -> CarapaceChatHistoryPayload
{
...
private func usageEvent(runId: String, outputTokens: Int, seq: Int) -> CarapaceAgentEventPayload {
    CarapaceAgentEventPayload(
        runId: runId,
        seq: seq,
        stream: "usage",
        ts: seq,
...
private func lifecycleSessionEntry(
    ...
    outputTokens: Int? = nil) -> CarapaceChatSessionEntry
{
...
private func sessionEntry(
    ...
    thinkingLevels: [CarapaceChatThinkingLevelOption]? = nil,
    thinkingOptions: [String]? = nil,
    thinkingDefault: String? = nil,
    verboseLevel: String? = nil,
    fastMode: CarapaceChatFastMode? = nil,
    effectiveFastMode: CarapaceChatFastMode? = nil,
    totalTokens: Int? = nil,
    totalTokensFresh: Bool? = nil,
    contextTokens: Int? = nil,
    permissionMode: CarapaceChatPermissionMode? = nil,
    toolOverrides: CarapaceChatSessionToolOverrides? = nil,
    hasActiveRun: Bool? = nil,
    activeRunIds: [String]? = nil) -> CarapaceChatSessionEntry
{
    CarapaceChatSessionEntry(
        key: key,
        kind: nil,
...
private func sessionsResponse(
    _ sessions: [CarapaceChatSessionEntry],
    ts: Double? = nil,
    defaults: CarapaceChatSessionsDefaults? = nil) -> CarapaceChatSessionsListResponse
{
...
private func thinkingOption(_ id: String, label: String? = nil) -> CarapaceChatThinkingLevelOption {
    CarapaceChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> CarapaceChatModelChoice
{
...
private func openAIModelPatchResult(
    _ model: String,
    thinking: String?,
    levels: [CarapaceChatThinkingLevelOption]? = nil) -> CarapaceChatModelPatchResult
{
    CarapaceChatModelPatchResult(
...
private func historyPayloadWithoutRunState(
    ...
    sessionId: String? = "sess-main",
    messages: [AnyCodable] = [],
    thinkingLevel: String = "off") -> CarapaceChatHistoryPayload
{
    CarapaceChatHistoryPayload(
        sessionKey: sessionKey,
        sessionId: sessionId,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: CarapaceChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> CarapaceChatCommandChoice
{
    CarapaceChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [CarapaceChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [CarapaceChatSessionsListResponse] = [],
    modelResponses: [[CarapaceChatModelChoice]] = [],
    modelAvailabilityIsSessionScoped: Bool = false,
    modelCatalogHook: (@Sendable (Int) async throws -> CarapaceChatModelCatalogSnapshot?)? = nil,
    modelPatchResults: [CarapaceChatModelPatchResult?] = [],
    thinkingPatchResults: [CarapaceChatModelPatchResult?] = [],
    commandResponses: [[CarapaceChatCommandChoice]] = [],
    ...
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> CarapaceChatHistoryPayload?)? = nil,
    ...
    sessionSettingsPatchHook: (
        @Sendable (CarapaceChatSessionSettingsPatch) async throws -> CarapaceChatModelPatchResult?)? = nil,
    composerCapabilityCatalog: CarapaceChatComposerCapabilityCatalog? = nil,
    composerCapabilityCatalogHook: (
        @Sendable (String, String?) async -> CarapaceChatComposerCapabilityCatalog)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> CarapaceChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> CarapaceChatSendResponse)? = nil,
    sendMessageStatus: String = "ok",
    waitForRunCompletionHook: (@Sendable (String, Int) async -> CarapaceChatRunObservation)? = nil,
    acquireSessionSettingsRouteLeaseHook: (@Sendable () async -> Void)? = nil,
    swarmEnabledHook: (@Sendable (String) async throws -> Bool)? = nil,
    listChildSessionsHook: (@Sendable (String) async throws -> [CarapaceChatSessionEntry])? = nil,
    ...
    -> (TestChatTransport, CarapaceChatViewModel)
{
...
private func sendUserMessage(_ vm: CarapaceChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private func legacyPlanEvent(
    ...
    seq: Int = 1,
    timestamp: Int? = 1000) -> CarapaceChatTransportEvent
{
    ...
    return .agent(CarapaceAgentEventPayload(
        runId: runId,
        seq: seq,
        stream: "plan",
        ts: timestamp,
...
private actor TestChatTransportState {
    ...
    var sentSettingsExpectations: [CarapaceChatSessionSettingsExpectation?] = []
    ...
    var sessionSettingsPatches: [CarapaceChatSessionSettingsPatch] = []
...
private final class TestChatTransport: @unchecked Sendable, CarapaceChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CarapaceChatHistoryPayload]
    private let sessionsResponses: [CarapaceChatSessionsListResponse]
    private let modelResponses: [[CarapaceChatModelChoice]]
    private let modelAvailabilityIsSessionScoped: Bool
    private let modelCatalogHook: (@Sendable (Int) async throws -> CarapaceChatModelCatalogSnapshot?)?
    private let modelPatchResults: [CarapaceChatModelPatchResult?]
    private let thinkingPatchResults: [CarapaceChatModelPatchResult?]
    private let commandResponses: [[CarapaceChatCommandChoice]]
    ...
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> CarapaceChatHistoryPayload?)?
    ...
    private let sessionSettingsPatchHook:
        (@Sendable (CarapaceChatSessionSettingsPatch) async throws -> CarapaceChatModelPatchResult?)?
    private let composerCapabilityCatalog: CarapaceChatComposerCapabilityCatalog?
    private let composerCapabilityCatalogHook:
        (@Sendable (String, String?) async -> CarapaceChatComposerCapabilityCatalog)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> CarapaceChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> CarapaceChatSendResponse)?
    private let sendMessageStatus: String
    private let waitForRunCompletionHook:
        (@Sendable (String, Int) async -> CarapaceChatRunObservation)?
    ...
    private let listChildSessionsHook: (@Sendable (String) async throws -> [CarapaceChatSessionEntry])?
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<CarapaceChatTransportEvent>.Continuation!
        ...
        worktree _: Bool?) async throws -> CarapaceChatCreateSessionResponse
    {
        ...
        return CarapaceChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CarapaceChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CarapaceChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        agentID: String?) async -> CarapaceChatComposerCapabilityCatalog
    {
        ...
        return self.composerCapabilityCatalog ?? CarapaceChatComposerCapabilityCatalog()
    }
        ...
        attachments _: [CarapaceChatAttachmentPayload]) async throws -> CarapaceChatSendResponse
    {
        ...
        return CarapaceChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
        ...
        archived: Bool) async throws -> CarapaceChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? CarapaceChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listCommands(sessionKey: String) async throws -> [CarapaceChatCommandChoice] {
        ...
        patch: CarapaceChatSessionSettingsPatch) async throws -> CarapaceChatModelPatchResult?
    {
        ...
        var result: CarapaceChatModelPatchResult?
        ...
        if let thinkingLevelUpdate = patch.thinkingLevel {
            ...
            let thinkingResult = try await patchSessionThinking(
                sessionKey: sessionKey,
                thinkingLevel: thinkingLevel)
            result = CarapaceChatModelPatchResult(
    ...
    private func patchSessionThinking(
        sessionKey: String,
        thinkingLevel: String) async throws -> CarapaceChatModelPatchResult?
    {
    ...
    func emit(_ evt: CarapaceChatTransportEvent) {
    ...
    func sentSettingsExpectations() async -> [CarapaceChatSessionSettingsExpectation?] {
    ...
    func sessionSettingsPatches() async -> [CarapaceChatSessionSettingsPatch] {
...
extension TestChatTransportState {
    ...
    fileprivate func sessionSettingsPatchesAppend(
        _ patch: CarapaceChatSessionSettingsPatch,
        sessionKey: String,
        agentID: String?)
    {
    ...
    fileprivate func sentSettingsExpectationsAppend(_ value: CarapaceChatSessionSettingsExpectation?) {
...
struct ChatViewModelTests {
    ...
    @Test @MainActor func `progress upgrade hint survives bootstrap and clears only its own error`() async throws {
        ...
        #expect(vm.errorText == CarapaceChatTransportUpgradeMessage.progressCardAgentScope)
    ...
    @Test @MainActor func `history canonical progress tuple survives delayed routing`() async throws {
        let history = try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: Data(
        ...
        sequenceGap: Bool) async throws
    {
        let history = try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: Data(
            #"{"sessionKey":"agent:research:main","messages":[],"sessionInfo":{"key":"agent:research:workbench","agentId":"research","hasActiveRun":false}}"#
                .utf8))
        let oldHistory = try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: Data(
            #"{"sessionKey":"agent:research:main","messages":[],"sessionInfo":{"key":"global","agentId":"research","hasActiveRun":false}}"#
                .utf8))
        let historyGate = SessionSubscribeGate()
    ...
    @Test @MainActor func `per sender global progress card preserves selected owner`() async throws {
        let (_, vm) = await makeViewModel(
            ...
            fetchProgressCardHook: { key, agentID in
                let owner = agentID ?? CarapaceChatSessionKey.agentID(from: key) ?? "main"
    ...
    @Test @MainActor func `subagent task events filter by session and retain terminal activity`() {
        let viewModel = CarapaceChatViewModel(
            sessionKey: "main",
    ...
    @Test @MainActor func `tool input delta updates the matching pending edit diff`() {
        ...
        viewModel.handleTransportEvent(.agent(CarapaceAgentEventPayload(
            runId: "run-1",
            seq: 1,
            stream: "tool",
            ts: 1000,
    ...
    @Test @MainActor func `transient Swarm capability failure preserves state and retries until explicit false`() async throws {
        ...
        let viewModel = CarapaceChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test @MainActor func `older Swarm capability completion cannot undo a newer disable`() async throws {
        ...
        let viewModel = CarapaceChatViewModel(sessionKey: "global", transport: transport, activeAgentId: "research")
    ...
    @Test @MainActor func `global Swarm activity follows the selected agent owner`() async {
        ...
        let foreign = CarapaceChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "main",
            reason: "swarm-note",
        ...
        let selected = CarapaceChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "work",
            reason: "swarm-note",
            swarmGroupId: "swarm:global:turn-1",
    ...
    @Test @MainActor func `credential submission retains only gateway answers and sends host consent`() async throws {
        ...
        let transport = TestChatTransport(
            ...
            resolveQuestionHook: { id, answers, hosts in
                ...
                return try CarapaceChatGatewayPayloadCodec.decodeQuestionAnswer(response)
            })
        let viewModel = CarapaceChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            CarapaceChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `session message tombstone clears stale exact run ids`() throws {
        ...
        let omitted = try JSONDecoder().decode(
            CarapaceSessionMessageEventPayload.self,
            from: Data(
    ...
    @Test @MainActor func `remote lifecycle merges terminal recap metadata`() {
        ...
        let activeHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "remote-run", text: "older working text"))
    ...
    @Test @MainActor func `advertised terminal chat retires the run without clearing local work`() {
        ...
        viewModel.handleTransportEvent(.chat(CarapaceChatEventPayload(
            runId: "remote-run",
            sessionKey: "main",
            state: "final",
            message: nil,
    ...
    @Test @MainActor func `terminal chat without run ID requires one current local owner`() {
        let viewModel = CarapaceChatViewModel(
            ...
            transport: TestChatTransport(historyResponses: []))
        viewModel.pendingRuns = ["run-a", "run-b"]
        let terminal = CarapaceChatEventPayload(
            runId: nil,
            sessionKey: nil,
            state: "final",
            message: nil,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: CarapaceChatViewModel? = CarapaceChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(CarapaceChatHistoryPayload.self, from: data)
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(CarapaceChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "resumed partial"))
        let historyCalls = AsyncCounter()
        let (_, vm) = await makeViewModel(
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: CarapaceChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: CarapaceChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground synthesizes activity when no run snapshot or local run exists`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                CarapaceSessionMessageEventPayload(
                    sessionKey: "main",
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: CarapaceChatInFlightRun(
                runId: "run-quiet",
                text: ""))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)],
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return CarapaceChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                CarapaceChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: CarapaceChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                CarapaceChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test @MainActor func `global session changes reconcile nested digest ownership`() async {
        ...
        selected.observerDigest = CarapaceChatSessionObserverDigest(
            agentId: "work",
            runId: "run-work",
            revision: 2,
            updatedAt: 200,
        ...
        vm.handleTransportEvent(.sessionsChanged(.init(
            ...
            agentId: "work",
            updatedAt: 900,
            observerDigest: CarapaceChatSessionObserverDigest(
                agentId: "main",
                runId: "run-work",
                revision: 9,
                updatedAt: 900,
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: CarapaceChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: CarapaceChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: CarapaceChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: CarapaceChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: CarapaceChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> CarapaceChatMessage {
        ...
        #expect(
            CarapaceChatMessage.errorDisplayText(
        ...
        #expect(
            CarapaceChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func `composer presentation owner changes when the model is replaced for the same session`() async {
        ...
        let firstOwner = await MainActor.run {
            CarapaceChatComposerPresentationOwner(viewModel: first)
        }
        let replacementOwner = await MainActor.run {
            CarapaceChatComposerPresentationOwner(viewModel: replacement)
        }
    ...
    @Test func `composer presentation owner changes with same-session agent routing`() async {
        ...
        let alphaOwner = await MainActor.run {
            CarapaceChatComposerPresentationOwner(viewModel: vm)
        }
        let betaOwner = await MainActor.run {
            vm.syncActiveAgentId("beta")
            return CarapaceChatComposerPresentationOwner(viewModel: vm)
        }
    ...
    @Test func `balances tool activity when a terminal event clears pending calls`() async throws {
        ...
        transport.emit(.chat(CarapaceChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: nil,
    ...
    @Test func `duplicate final events append one provisional reply`() async throws {
        ...
        await sendUserMessage(vm, text: "hello")
        let runId = try await waitForLastSentRunId(transport)
        let final = CarapaceChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: chatTextMessage(
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                CarapaceChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
        ...
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, runIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: CarapaceChatInFlightRun(runId: runId, text: "Let me inspect it."))
            },
            sendMessageStatus: "pending")
    ...
    func `agent assistant text owns the run instead of cumulative chat buffers`(delivery: String) async throws {
        ...
        let history = historyPayload(
            inFlightRun: delivery == "delta-only" ? nil : CarapaceChatInFlightRun(runId: runId, text: "seed"))
        let (_, vm) = await makeViewModel(historyResponses: [history])
        try await loadAndWaitBootstrap(vm: vm)
        defer { vm.detachTransport() }
        let agent = CarapaceChatTransportEvent.agent(CarapaceAgentEventPayload(
            runId: runId, seq: 1, stream: "assistant", ts: 1,
            data: ["text": AnyCodable("Here is the result")]))
        let delta = CarapaceChatTransportEvent.chat(CarapaceChatEventPayload(
    ...
    @Test @MainActor func `detached transport ignores late events and in-flight history`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                ...
                _ = await historyStarted.increment()
                await historyGate.wait()
                return historyPayload(inFlightRun: CarapaceChatInFlightRun(runId: "retired-run", text: "stale"))
            })
        try await loadAndWaitBootstrap(vm: vm)
        ...
        vm.detachTransport()
        vm.detachTransport()
        let lateEvent = CarapaceChatTransportEvent.chat(CarapaceChatEventPayload(
    ...
    func `terminal event for another run preserves active streaming and tools`(state: String) async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                CarapaceChatSendResponse(runId: activeRunId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return CarapaceChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                CarapaceChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `terminal timeout send ack surfaces error and allows next send`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                ...
                return CarapaceChatSendResponse(
                    runId: runId,
    ...
    @Test @MainActor func `repeated refresh preserves an unconfirmed same text turn after pending retirement`() {
        ...
        let whilePending = CarapaceChatViewModel.reconcileRunRefreshMessages(
            previous: previous,
        ...
        let afterRetirement = CarapaceChatViewModel.reconcileRunRefreshMessages(
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(CarapaceChatTransportEvent.seqGap)
    ...
    @Test func `composer capabilities patch permission and sparse tool overrides`() async throws {
        let skill = CarapaceChatComposerSkill(
            key: "release",
            name: "Release",
            baseEnabled: true,
            missingDependencies: false,
            blocked: false)
        let disabledSkill = CarapaceChatComposerSkill(
            key: "disabled",
            name: "Disabled",
            baseEnabled: false,
            missingDependencies: false,
            blocked: false)
        let catalog = CarapaceChatComposerCapabilityCatalog(
    ...
    @Test func `restrictive composer patch serializes ahead of immediate send`() async throws {
        ...
        #expect(await transport.sentSettingsExpectations() == [
            CarapaceChatSessionSettingsExpectation(
                permissionMode: .guarded,
                toolOverrides: nil),
        ])
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                throw GatewayResponseError(
                    ...
                    details: [
                        "reason": AnyCodable(CarapaceChatSessionSettingsContract.changedErrorReason),
                    ])
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await vm.loadComposerCapabilities()
    ...
    @Test func `write scope permits model but keeps effort settings admin only`() async throws {
        let writeCatalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
    ...
    @Test func `composer connector and tool access mutations preserve effective state`() async throws {
        let tool = CarapaceChatComposerTool(
            name: "create_issue",
            label: "Create issue",
            baseEnabled: true,
            sessionDenied: true)
        let connector = CarapaceChatComposerConnector(
            name: "github",
            baseEnabled: true,
            tools: [tool])
        let catalog = CarapaceChatComposerCapabilityCatalog(
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sessionsResponses: [sessionsResponse([
                sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    toolOverrides: CarapaceChatSessionToolOverrides(
    ...
    @Test func `composer tool toggles use authoritative session denial instead of catalog baseline`() async throws {
        ...
        let catalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
            connectors: [CarapaceChatComposerConnector(
                name: "github",
                baseEnabled: true,
                tools: [tool])],
            connectorsAvailable: true,
            ...
            sessionSettingsCASAvailable: true,
            toolOverrideMutationAvailable: true)
        let denied = CarapaceChatSessionToolOverrides(
            mcpToolsDeny: ["github": ["create_issue"]])
        let (transport, vm) = await makeViewModel(
            historyResponses: [historyPayload()],
            sessionsResponses: [sessionsResponse([
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sessionsResponses: [sessionsResponse([
                sessionEntry(
                    ...
                    updatedAt: 1,
                    sessionId: "sess-main",
                    toolOverrides: CarapaceChatSessionToolOverrides(webSearch: false)),
            ])],
            composerCapabilityCatalog: catalog)
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await vm.loadComposerCapabilities()
    ...
    @Test func `composer tool state toggles and clears an inherited effective denial`() async throws {
        let deniedTool = CarapaceChatComposerTool(
            name: "create_issue",
            label: "Create issue",
            sessionDenied: true)
        let allowedTool = CarapaceChatComposerTool(
            name: "create_issue",
            label: "Create issue")
        let catalog: @Sendable (CarapaceChatComposerTool) -> CarapaceChatComposerCapabilityCatalog = { tool in
    ...
    @Test func `composer capability reasons name access and skill blockers`() async {
        let missing = CarapaceChatComposerSkill(
            key: "missing",
            name: "Missing",
            baseEnabled: true,
            missingDependencies: true,
            blocked: false)
        let blocked = CarapaceChatComposerSkill(
            key: "blocked",
            name: "Blocked",
            baseEnabled: true,
            missingDependencies: false,
            blocked: true)
        let disabled = CarapaceChatComposerSkill(
            key: "disabled",
            name: "Disabled",
            baseEnabled: false,
            missingDependencies: false,
    ...
    @Test func `clear tool overrides is disabled and cannot dispatch without mutation access`() async throws {
        let overrides = CarapaceChatSessionToolOverrides(webSearch: false)
        let catalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
            toolOverrideMutationAvailable: false)
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `same session reconnect invalidates and reloads composer capability scopes`() async throws {
        let calls = AsyncCounter()
        let adminCatalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
            permissionMutationAvailable: true,
            toolOverrideMutationAvailable: true,
            canSelectFullPermission: true)
        let downgradedCatalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
            permissionMutationAvailable: true,
            toolOverrideMutationAvailable: false,
            canSelectFullPermission: false)
    ...
    @Test func `composer capabilities fail closed and discard a stale catalog`() async throws {
        let gate = AsyncGate()
        let calls = AsyncCounter()
        let loadedCatalog = CarapaceChatComposerCapabilityCatalog(
            sessionSettingsAvailable: true,
            webSearchBaseEnabled: true,
            skills: [CarapaceChatComposerSkill(
                key: "stale",
                name: "Stale",
                baseEnabled: true,
                missingDependencies: false,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = sessionsResponse(
            ...
            ts: now,
            defaults: CarapaceChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil))
        let models = [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(CarapaceChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CarapaceChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = sessionsResponse(
            ...
            ts: now,
            defaults: CarapaceChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil))
        let models = [
            modelChoice(id: "gpt-4.1-mini", name: "GPT-4.1 mini", provider: "openai"),
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: CarapaceChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test @MainActor func `Ultra is canonical while ultrathink remains a high alias`() {
        #expect(CarapaceChatViewModel.normalizedThinkingLevel("ultra") == "ultra")
        #expect(CarapaceChatViewModel.normalizedThinkingLevel("ULTRA") == "ultra")
        #expect(CarapaceChatViewModel.normalizedThinkingLevel("ultrathink") == "high")
        #expect(
            CarapaceChatViewModel.normalizedThinkingLevel(
                "ultra",
    ...
    @Test func `decodes authoritative model patch thinking state`() throws {
        ...
        let result = try JSONDecoder().decode(CarapaceChatModelPatchResult.self, from: data)
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            CarapaceChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == CarapaceChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `default settings patch returns accepted thinking state`() async throws {
        ...
        let result = try await transport.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            patch: CarapaceChatSessionSettingsPatch(thinkingLevel: .some("high")))
    ...
    @Test func `default thinking selection clears override and adopts resolved level`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectThinkingLevel(CarapaceChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")

        #expect(await MainActor.run { vm.thinkingSelectionID } ==
            CarapaceChatViewModel.inheritedThinkingSelectionID)
        #expect(await MainActor.run { vm.thinkingLevel } == "medium")
        #expect(await MainActor.run { vm.sessions.first?.thinkingLevel } == nil)
    ...
    @Test func `inherited verbosity does not masquerade as persisted override`() async throws {
        ...
        #expect(await MainActor.run { vm.verboseLevel } == CarapaceChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity rejection restores inherited overrides`() async throws {
        ...
        #expect(await MainActor.run { vm.fastModeSelectionID } == CarapaceChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity default selections clear overrides`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectFastMode(CarapaceChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.fastMode } == nil)
        ...
        await MainActor.run {
            vm.selectVerboseLevel(CarapaceChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.verboseLevel } == nil)
        #expect(await MainActor.run { vm.verboseLevel } == CarapaceChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        let history = historyPayloadWithoutRunState(
            messages: [
                AnyCodable([
                    ...
                    Conversation info: \u{27E6}carapace:ctx\u{27E7}
                    ```json
                    { \"sender\": \"carapace-ios\" }
                    ```
        ...
        let vm = await MainActor.run { CarapaceChatViewModel(sessionKey: "main", transport: transport) }
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = CarapaceChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = CarapaceChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(CarapaceChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
