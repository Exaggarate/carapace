...
struct GatewayErrorsTests {
    ...
    @Test func `connect auth error preserves structured metadata`() {
        let error = GatewayConnectAuthError(
            ...
            actionCommand: "carapace devices approve req-123",
            docsURLString: "https://github.com/Exaggarate/carapace",
        ...
        #expect(error.actionCommand == "carapace devices approve req-123")
        #expect(error.docsURLString == "https://github.com/Exaggarate/carapace")
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update Carapace on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update Carapace on the gateway host, then retry.")
        #expect(problem?.actionLabel == "Copy update command")
        #expect(problem?.actionCommand == "carapace update")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update Carapace on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `pairing problem uses structured request metadata`() {
        ...
        #expect(problem?.pauseReconnect == true)
        #expect(problem?.actionCommand == "carapace devices approve req-123")
    }
    ...
    private static let troubleshootingDocs = "https://github.com/Exaggarate/carapace"
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            facts = (
                "Gateway is not reachable", "Carapace could not reach the gateway over the current network.",
                "Check network")
