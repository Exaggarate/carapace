import Foundation
import Testing
@testable import CarapaceNativeState
#if canImport(Darwin)
import Darwin
import Darwin.membership
#endif

struct CarapaceNativeStateSQLiteTests {
    @Test
    ...
    func `version zero rejects incomplete known sibling schema`() throws {
        try self.withDatabase { database in
            ...
            #expect(throws: CarapaceNativeStateError.self) {
                try database.ensureCanonicalTable(.deviceIdentities)
            }
        }
    }
    ...
    func `concurrent version zero stores compose different tables`() async throws {
        ...
        let databaseURL = directory.appendingPathComponent("carapace.sqlite", isDirectory: false)

        try await withThrowingTaskGroup(of: Void.self) { group in
            for table in [
                CarapaceNativeStateCanonicalTable.deviceIdentities,
                .macosPortGuardianRecords,
            ] {
                group.addTask {
                    let database = try CarapaceNativeStateSQLite(databaseURL: databaseURL)
                    try database.withImmediateTransaction {
    ...
    func `vanished transient sidecar does not fail committed write cleanup`(missing: NSError) throws {
        let databaseURL = URL(fileURLWithPath: "/tmp/carapace-native-state.sqlite")
        var attemptedPaths: [String] = []

        try CarapaceNativeStateSQLite.secureDatabaseFiles(
            databaseURL,
    ...
    func `new private state removes inherited grants without changing ancestors`() throws {
        ...
        let databaseURL = parent.appendingPathComponent("carapace.sqlite")
        let database = try CarapaceNativeStateSQLite(databaseURL: databaseURL)
        ...
        journalMode: String) throws
    {
        ...
        let database = try CarapaceNativeStateSQLite(databaseURL: databaseURL, createIfMissing: createIfMissing)
    ...
    func `mixed ACL preserves ordering and owner access`(principal: GrantPrincipal) throws {
        try self.withDatabaseURL { database, databaseURL in
            ...
            try CarapaceNativeStateSQLite.secureDatabaseFiles(databaseURL)
    ...
    private func withDatabase(
        body: (CarapaceNativeStateSQLite) throws -> Void) throws
    {
        try self.withDatabaseURL { database, _ in try body(database) }
    }

    private func withDatabaseURL(
        body: (CarapaceNativeStateSQLite, URL) throws -> Void) throws
    {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
