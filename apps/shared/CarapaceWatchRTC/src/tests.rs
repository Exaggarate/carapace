...
struct Client(*mut CarapaceRTC);

impl Drop for Client {
    fn drop(&mut self) {
        unsafe { carapace_rtc_free(self.0) }
    }
}
...
impl Fixture {
    fn new(addresses: &[SocketAddr], scenario: Scenario) -> Self {
        let client = Client(carapace_rtc_create());
        ...
        fixture.poll_client();
        assert_eq!(unsafe { carapace_rtc_offer(fixture.client.0) }, 0);
        fixture.poll_client();
        let mut length = 0;
        let bytes = unsafe { carapace_rtc_description(fixture.client.0, &mut length) };
        ...
        fixture.answer_result =
            unsafe { carapace_rtc_answer(fixture.client.0, text.as_ptr(), text.len()) };
        fixture.poll_client();
        fixture
    }
    ...
    fn poll_client(&mut self) {
        loop {
            let mut output = CarapaceRTCOutput::default();
            assert_eq!(unsafe { carapace_rtc_poll(self.client.0, &mut output) }, 0);
    ...
    fn resolve(&mut self, index: usize, address: SocketAddr) -> i32 {
        let result =
            unsafe { carapace_rtc_resolve_remote_address(self.client.0, index, address.into()) };
        self.poll_client();
        result
    }
    ...
    fn inventory(&self) -> Vec<SocketAddr> {
        ...
        for index in 0..=100 {
            let mut address = CarapaceRTCAddress::default();
            let code = unsafe { carapace_rtc_remote_address(self.client.0, index, &mut address) };
            if code == 1 {
                return result;
            }
            assert_eq!(code, 0);
...
fn prove_media(scenario: Scenario) {
    ...
    assert_eq!(
        unsafe { carapace_rtc_add_candidate(fixture.client.0, client.into()) },
        0
    );
    fixture.poll_client();
    ...
    while Instant::now() < deadline {
        ...
        assert_eq!(unsafe { carapace_rtc_timeout(fixture.client.0) }, 0);
        fixture.poll_client();
        fixture
            .peer
...
fn ice_credentials_do_not_repeat_with_noncryptographic_rng_state() {
    let credentials = || {
        ...
        let client = Client(carapace_rtc_create());
        assert!(!client.0.is_null());
        assert_eq!(unsafe { carapace_rtc_offer(client.0) }, 0);
        let mut length = 0;
        let bytes = unsafe { carapace_rtc_description(client.0, &mut length) };
        let text =
...
fn panic_retires_engine_without_losing_description() {
    ...
    assert_eq!(unsafe { carapace_rtc_timeout(fixture.client.0) }, -1);
    let mut address = CarapaceRTCAddress::default();
    assert_eq!(
        unsafe { carapace_rtc_remote_address(fixture.client.0, 0, &mut address) },
        -1
    );
