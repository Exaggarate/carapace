...
```sh
apps/shared/CarapaceWatchRTC/build.sh watchos /tmp/watch-rtc-device arm64 arm64_32
apps/shared/CarapaceWatchRTC/build.sh watchsimulator /tmp/watch-rtc-simulator arm64 x86_64
```

Each command writes `libcarapace_watch_rtc.a` and its Cargo build cache to the
...
directory to the library search path. Link `libcarapace_watch_rtc.a`, Security,
...
```sh
cargo +nightly-2026-09-05 test --locked --manifest-path apps/shared/CarapaceWatchRTC/Cargo.toml --lib -- --test-threads=1
```
