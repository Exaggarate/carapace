...
const GATEWAY_SERVICE_TYPE: &str = "_carapace-gw._tcp.local.";
...
impl GatewayDiscovery {
    fn start(&self) -> Result<(), String> {
        ...
        let thread = thread::Builder::new()
            .name("carapace-gateway-discovery".to_string())
            .spawn(move || {
...
fn prettify_instance_name(name: &str) -> String {
    ...
    strip_ascii_suffix(without_conflict, " (Carapace)")
        .trim()
        .to_string()
}
...
mod tests {
    ...
    fn decodes_and_prettifies_fallback_name() {
        assert_eq!(
            service_instance_name("Peter\\032Studio\\032(Carapace)._carapace-gw._tcp.local."),
            "Peter Studio"
        );
        assert_eq!(prettify_instance_name("Studio (Carapace) (2)"), "Studio");
    }
}
