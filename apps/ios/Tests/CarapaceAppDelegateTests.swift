...
@Suite(.serialized) struct CarapaceAppDelegateTests {
    ...
    @Test @MainActor func `resolves registry model before view task assigns delegate model`() {
        ...
        CarapaceAppModelRegistry.appModel = registryModel
        defer { CarapaceAppModelRegistry.appModel = nil }
    ...
    @Test @MainActor func `stages a gateway URL when the model is ready`() async throws {
        ...
        let url = try #require(URL(
            string: "carapace://gateway?host=gateway.example.com&port=443&tls=1&token=tok"))
    ...
    @Test @MainActor func `live voice intent survives cold launch and waits for an active scene`() async throws {
        try await withUserDefaults(["talk.enabled": false]) {
            let previousModel = CarapaceAppModelRegistry.appModel
            CarapaceAppModelRegistry.appModel = nil
            defer { CarapaceAppModelRegistry.appModel = previousModel }
            ...
            CarapaceAppModelRegistry.appModel = replacement
    ...
    func `warm live voice intent opens the selected chat without toggling existing voice`() async throws {
        try await withUserDefaults(["talk.enabled": false]) {
            ...
            defer {
                ...
                CarapaceAppModelRegistry.appModel = previousModel
            }
