...
struct WatchChatStatusLocalizationTests {
    ...
    @Test func `gateway presentation localizes key and keeps backend override verbatim`() {
        let localized = CarapaceWatchAppStatus(
            code: .gatewayProblem,
            localizationKey: "Gateway update required")
        let backendOverride = CarapaceWatchAppStatus(
            code: .gatewayProblem,
            verbatim: "Gateway says update channel beta")
        let localizedTalkFailure = CarapaceWatchAppStatus(
            code: .talkFailure,
    ...
    private static func semanticPayload() -> [String: Any] {
        [
            ...
            "gatewayStatus": [
                "code": CarapaceWatchAppStatusCode.gatewayProblem.rawValue,
                "localizationKey": "Gateway update required",
            ],
            "gatewayConnected": false,
            "agentName": "Main",
            ...
            "sessionKey": "main",
            "talkStatus": [
                "code": CarapaceWatchAppStatusCode.talkOff.rawValue,
            ],
            "talkEnabled": false,
            "talkListening": false,
            "talkSpeaking": false,
            "pendingApprovalCount": 0,
            "chatStatus": [
                "code": CarapaceWatchAppStatusCode.chatConnectIPhone.rawValue,
            ],
        ]
    }
