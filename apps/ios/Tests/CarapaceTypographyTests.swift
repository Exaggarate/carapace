...
struct RootSidebarTypographyTests {
    @Test func `root sidebar uses branded typography`() throws {
        ...
        #expect(sidebar.contains("CarapaceType.headline"))
        #expect(sidebar.contains("CarapaceType.subheadSemiBold"))
        #expect(sidebar.contains("CarapaceType.captionMedium"))
        #expect(sidebar.contains("CarapaceType.caption2Bold"))
        #expect(!sidebar.contains(".font(."))
    }
}

struct CarapaceTypographyTests {
    @Test func `thread controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(CarapaceType.subhead)"))
        #expect(support.contains(".font(CarapaceType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(CarapaceType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in CarapaceType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["CarapaceActivityWidget", "CarapaceWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "CarapaceActivityWidget" ? "  CarapaceWatchApp:" : "  CarapaceTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("CarapaceActivityType.subheadSemiBold"))
        #expect(activitySource.contains("CarapaceActivityType.subheadBold"))
        #expect(activitySource.contains("CarapaceActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/CarapaceTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("CarapaceType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/CarapaceProComponents.swift"),
            encoding: .utf8)
        ...
        let approvalDialog = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/CarapaceDocsScreen.swift"), encoding: .utf8)
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatMessageViews.swift"),
            encoding: .utf8)
        let chatWorkingClawView = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatWorkingClawView.swift"),
            encoding: .utf8)
        let chatMarkdownRenderer = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatMarkdownRenderer.swift"),
            encoding: .utf8)

        #expect(proComponents.contains(".font(CarapaceType.subheadSemiBold)"))
        ...
        #expect(quickSetup.contains(".font(CarapaceType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(CarapaceType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(CarapaceType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(CarapaceType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(CarapaceType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(CarapaceType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(CarapaceType.body)"))
        for label in ["Gateway", "Approvals", "Open Gateway"] {
            let text = try Self.extract(settingsHub, from: "Text(\"\(label)\")", to: "}")
            #expect(text.contains(".font(CarapaceType.subheadSemiBold)"))
        }
        ...
        #expect(upgradeWarning.contains(".font(CarapaceType.footnote)"))
        let approvalBadge = try Self.extract(
            settingsHub,
            from: "Text(self.appModel.pendingExecApprovalCount.formatted())",
            to: "}")
        #expect(approvalBadge.contains(".font(CarapaceType.captionSemiBold)"))
        let dashboardClose = try Self.extract(dashboardPage, from: "Text(\"Done\")", to: "}")
        #expect(dashboardClose.contains(".font(CarapaceType.subheadSemiBold)"))
        #expect(settingsSections.contains("Text(warningText)"))
        #expect(settingsSections.contains(".font(CarapaceType.caption)"))
        #expect(approvalDialog.contains("Text(warningText)"))
        #expect(approvalDialog.contains(".font(CarapaceType.footnote)"))
        ...
        #expect(settingsSections.contains("CarapaceToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains(".font(CarapaceType.subhead)"))
        ...
        let reconnect = try Self.extract(
            ...
            to: "}")
        #expect(reconnect.contains(".font(CarapaceType.body)"))
        ...
        #expect(settingsSections.contains(".font(CarapaceType.subheadSemiBold)"))
        #expect(settingsSupport.contains("struct SettingsBuildMetadataStrip"))
        #expect(settingsSupport.contains(".font(CarapaceType.caption2SemiBold)"))
        #expect(settingsSupport.contains(".font(CarapaceType.monoSmall)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(CarapaceType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(CarapaceType.captionSemiBold)"))

        #expect(docs.contains(".font(CarapaceType.body)"))

        #expect(chatMessageViews.contains("typography: segment.kind.markdownTypography"))
        #expect(chatMessageViews.contains(".font(CarapaceChatTypography.caption)"))
        #expect(chatMessageViews.contains("chat-user-message-disclosure-toggle"))
        #expect(chatMarkdownRenderer.contains("CarapaceChatTypography.callout.italic()"))
        #expect(!chatMessageViews.contains("font: .body"))
        #expect(!chatMessageViews.contains("Font.body"))
        #expect(!chatMessageViews.contains("Font.callout"))
        #expect(chatWorkingClawView.contains(".font(CarapaceChatTypography.caption)"))
        #expect(chatWorkingClawView.contains(".font(CarapaceChatTypography.captionSemiBold)"))
    ...
    @Test func `chat model menu uses branded typography`() throws {
        ...
        #expect(menu.contains(".font(CarapaceType.body)"))
        #expect(menu.contains(".font(CarapaceType.caption)"))
    }
    ...
    @Test func `chat copy and select text surfaces use branded typography`() throws {
        let selectableTextSheet = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatSelectableTextSheet.swift"),
            encoding: .utf8)
        let chatView = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatView.swift"),
            encoding: .utf8)
        let markdownBlockViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatMarkdownBlockViews.swift"),
            encoding: .utf8)
        let mermaidBlockView = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI/ChatMermaidBlockView.swift"),
            encoding: .utf8)

        #expect(selectableTextSheet.contains("CarapaceChatTypography.bodyUIFont"))
        #expect(selectableTextSheet.contains("Text(\"Close\")"))
        #expect(selectableTextSheet.contains(".font(CarapaceChatTypography.body)"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/CarapaceLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/CarapaceActivityTypography.swift")
    }
    ...
    private static func appSourceURL() -> URL {
        self.sourceURL("CarapaceApp.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/CarapaceKit/Sources/CarapaceChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders(in source: String, relativePath: String) -> [String] {
        let fontTokens = ["CarapaceType", "CarapaceChatTypography", "WatchClawType", "typography."]
    ...
    private static func hasAllowedBrandedFontParameter(_ fontCall: String, relativePath: String) -> Bool {
        switch relativePath {
        case "apps/ios/Sources/Design/CarapaceProComponents.swift":
            fontCall.contains(".font(self.titleFont)") ||
                fontCall.contains(".font(self.subtitleFont)")
        case "apps/shared/CarapaceKit/Sources/CarapaceChatUI/ChatMarkdownRenderer.swift":
