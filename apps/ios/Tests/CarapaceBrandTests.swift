...
struct CarapaceBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", CarapaceBrand.uiAccentForeground),
            ("accentHot", CarapaceBrand.uiAccentHotForeground),
            ("ok", CarapaceBrand.uiOK),
            ("warn", CarapaceBrand.uiWarn),
            ("danger", CarapaceBrand.uiDanger),
            ("info", CarapaceBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                CarapaceBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(CarapaceBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(CarapaceBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
