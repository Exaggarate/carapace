...
@MainActor
final class CarapaceSnapshotUITests: XCTestCase {
    ...
    func testOnboardingExplainsCapabilitiesAndTrust() {
        ...
        app.launchArguments += ["--carapace-reset-onboarding"]
        app.launch()
        self.app = app
    ...
    func testSidebarAgentSelectorShowsAllAgentsAndKeepsFooterVisible() throws {
        ...
        let gatewayFooter = try XCTUnwrap(self.app?.buttons.matching(
            NSPredicate(format: "label CONTAINS %@", "Carapace Gateway")).firstMatch)
    ...
    func testVoiceNoteDraftKeepsStopAvailableDuringActiveResponse() throws {
        ...
        self.launchApp(
            ...
            additionalArguments: ["--carapace-hold-initial-chat-run"])
    ...
    func testKeyboardOpenPreservesTranscriptAndFollowsLiveEdgeAfterSend() throws {
        ...
        self.launchApp(
            ...
            additionalArguments: ["--carapace-long-chat-fixture"])
        let app = try XCTUnwrap(self.app)

        let latestSeededReply = app.staticTexts["CARAPACE_LONG_CHAT_LATEST"]
    ...
    func testExistingSessionRestoresLatestOutput() throws {
        ...
        let latest = app.staticTexts["CARAPACE_LONG_CHAT_LATEST"]
    ...
    func testEmptyChatStarterPromptSendsMessage() throws {
        self.launchApp(
            ...
            additionalArguments: ["--carapace-empty-chat-fixture"])
        ...
        starter.tap()
        let sentText = "Summarize the current Carapace status and tell me what needs attention."
    ...
    func testEmptyChatStarterPromptsLocalizeInGerman() throws {
        ...
        let starter = try XCTUnwrap(self.app?.buttons["Carapace-Status prüfen"])
        XCTAssertTrue(starter.exists)
        starter.tap()
        XCTAssertTrue(
            self.app?.staticTexts[
                "Fasse den aktuellen Carapace-Status zusammen und sage mir, was Aufmerksamkeit erfordert.",
    ...
    func testLiveGatewayChatRoundTripAndControlOverview() throws {
        ...
        for index in 0..<3 {
            let seedMarker = "CARAPACE_E2E_SEED_\(index)_\(Int(Date().timeIntervalSince1970 * 1000))"
        ...
        let replyMarker = "CARAPACE_E2E_OK_\(Int(Date().timeIntervalSince1970 * 1000))"
        self.sendLiveGatewayMessage(
    ...
    func testManualAuthRetryUsesEditedToken() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["CARAPACE_IOS_RETRY_E2E"] == "1",
            "Set CARAPACE_IOS_RETRY_E2E=1 with a local token-auth Gateway on port 18920")
        let token = try XCTUnwrap(ProcessInfo.processInfo.environment["CARAPACE_IOS_RETRY_TOKEN"])
...
extension CarapaceSnapshotUITests {
    ...
    func testUnavailableModelIsDisabledInChatActions() throws {
        self.launchApp(
            for: Self.chatScreenshotTarget,
            additionalArguments: [
                "--carapace-unavailable-model-fixture",
                "-carapace.chat.modelFavorites", "",
                "-carapace.chat.modelRecents", "",
            ])
        let app = try XCTUnwrap(self.app)
    ...
    func testSelectedAuthFailedModelDisablesOnlineSend() throws {
        self.launchApp(
            for: Self.chatScreenshotTarget,
            additionalArguments: ["--carapace-selected-model-auth-failure-fixture"])
        let app = try XCTUnwrap(self.app)
        let input = self.chatMessageInput(in: app)
        XCTAssertTrue(input.waitForExistence(timeout: 8))
        input.tap()
    ...
    func testModelSelectionTargetVariantsPersistAcrossBothPickers() throws {
        for (target, disclosure) in [
            ...
            self.launchApp(
                for: Self.chatScreenshotTarget,
                additionalArguments: [
                    "--carapace-model-selection-target", target,
                    "-carapace.chat.modelFavorites", "",
                    "-carapace.chat.modelRecents", "",
                ])
            let app = try XCTUnwrap(self.app)
    ...
    private func verifyApprovalNotificationsNavigation(fromOverview: Bool) async throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["CARAPACE_IOS_APPROVAL_FIXTURE_URL"] != nil,
            "Requires a task-owned synthetic Gateway approval fixture")
        let fixtureURL = try XCTUnwrap(
            ProcessInfo.processInfo.environment["CARAPACE_IOS_APPROVAL_FIXTURE_URL"]
        ...
        additionalArguments: [String] = [])
    {
        ...
        XCTAssertTrue(
            ...
            "Carapace root readiness marker did not appear")
        if screenshotMode {
        ...
        additionalArguments: [String]) -> XCUIApplication
    {
        ...
        app.launchArguments += [
            ...
            "hidden",
            "--carapace-ui-test-readiness",
        ]
        if screenshotMode {
            app.launchArguments.append("--carapace-screenshot-mode")
        }
        app.launchArguments += additionalArguments
        if let appearance {
            app.launchArguments += ["--carapace-appearance", appearance]
        }
        return app
    }
        ...
        beforeCapture: ((XCUIApplication) -> Void)? = nil)
    {
        ...
        guard let app = self.app else {
            XCTFail("Carapace is not running for screenshot target \(target.name)")
            return
        }
        beforeCapture?(app)
        ...
        line: UInt = #line)
    {
        ...
        XCTAssertTrue(
            ...
            "Carapace did not terminate before the next launch",
            file: file,
            line: line)
        self.app = nil
    }
    ...
    {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["CARAPACE_IOS_LIVE_GATEWAY"] == "1",
            "Set CARAPACE_IOS_LIVE_GATEWAY=1 and provide a fresh setup code")

        if let setupCode = ProcessInfo.processInfo.environment["CARAPACE_IOS_LIVE_SETUP_CODE"] {
