...
- iOS SwiftUI text should use branded typography helpers, not bare system fonts. Use `CarapaceType` in `apps/ios/Sources/**`, `CarapaceChatTypography` in shared chat UI, `WatchClawType` in watch UI, and `CarapaceActivityType` in Live Activity UI.
...
- When touching iOS text surfaces, update/keep `apps/ios/Tests/CarapaceTypographyTests.swift` so bare text/control regressions are caught, then run the focused typography test.
...
- Do not add Carapace, Carapace Foundation, or other first-party/self-owned license entries. The screen is for third-party/open-source dependency acknowledgements.
- When adding, removing, or upgrading iOS dependencies, audit whether `apps/ios/Resources/Licenses/` needs updates. Exclude dependencies owned by Carapace Foundation from the published license list.
