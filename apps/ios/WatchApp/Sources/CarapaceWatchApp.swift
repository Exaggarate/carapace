...
enum WatchScreenshotMode {
    private static let defaultsKey = "carapace.watch.screenshotMode"
    static let approvals = ProcessInfo.processInfo.arguments.contains(
        "--carapace-watch-approval-screenshot-mode")
        || ProcessInfo.processInfo.environment["CARAPACE_WATCH_APPROVAL_SCREENSHOT_MODE"] == "1"
    static let enabled = ProcessInfo.processInfo.arguments.contains("--carapace-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["CARAPACE_WATCH_SCREENSHOT_MODE"] == "1"
    ...
    fileprivate func configureScreenshotFixture(includeApproval: Bool = false) {
        ...
        self.consume(
            appSnapshot: WatchAppSnapshotMessage(
                ...
                gatewayStableID: "watch-screenshot-gateway",
                talkStatus: CarapaceWatchAppStatus(code: .talkReady),
                talkEnabled: true,
                talkListening: false,
                talkSpeaking: false,
