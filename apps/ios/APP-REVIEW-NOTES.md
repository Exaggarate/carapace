...
Use these steps to exercise the live Carapace iOS App Review Gateway.

## Demo Account / Setup

Use the Carapace iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open Carapace`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable Carapace Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether Carapace may send notifications, tap `Allow`.
...
command `printf 'Carapace App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send Carapace to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while Carapace is backgrounded.
Reopen Carapace, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send Carapace to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `Carapace` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because Carapace ends the Live Activity after
...
4. Immediately send Carapace to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `Carapace`
notification with this body:

```text
Carapace App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If Carapace opens on
...
3. Immediately send Carapace to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `Carapace` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If Carapace opens on
   `Control`, tap `Chat`.
...
Carapace does not present an aggregate permissions page during onboarding.
...
the native iOS authorization prompt, with no additional Carapace dialog or
deferral action.
...
permission sheet. Carapace requests read-only access to steps, sleep, resting
...
4. Tap `Share`.
5. Tap `Carapace`.
6. Confirm the Carapace share extension appears and shows
   `Edit text, then tap Send.` and `Send to Carapace`.
7. Tap `Send to Carapace`.

Expected result: the Carapace share extension sends the shared Safari page to
the live review Gateway and shows `Sent to Carapace.` Returning to Carapace
