    ...
    private var authStep: some View {
        Section {
            ...
            if let problem = self.currentProblem {
                ...
                Text("Carapace is checking gateway and node access.")
                    .font(CarapaceType.footnote)
                    .foregroundStyle(.secondary)
            }
        } header: {
        ...
        if self.issue.needsPairing {
            Section {
                ...
                let requestLine: String = {
                    ...
                    return String(localized: "Request ID: check `carapace devices list`.")
                }()
                let commandLine = self.currentProblem?.actionCommand ?? "carapace devices approve <requestId>"
...
extension OnboardingWizardView {
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
               ...
               let port = active.port
            {
                ...
                self.manualHost = "carapace.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "carapace.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "carapace.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "carapace.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "carapace.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
