...
struct OnboardingActivationCanvas<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .background(CarapaceBrand.activationCanvasGradient.ignoresSafeArea())
        }
    }
}
...
private struct OnboardingHeroGlyph: View {
    ...
    var body: some View {
        CarapaceActivationGlyph(size: 78, mood: self.mood, interactive: true)
    }
}
...
private typealias OnboardingPrimaryButtonStyle = CarapacePrimaryActionButtonStyle
...
private enum OnboardingIntroPanelStyle {
    ...
    static let panelFill = CarapaceBrand.activationNeutralSurface
    static let iconFill = CarapaceBrand.activationNeutralInsetSurface
    static let stroke = CarapaceBrand.activationNeutralStroke
}
...
private struct OnboardingSafetyRow: View {
    ...
    var body: some View {
        HStack(spacing: OnboardingIntroPanelStyle.contentSpacing) {
            OnboardingIntroIcon(
                symbol: self.symbol,
                tint: CarapaceBrand.activationPrimaryAction)
            ...
            HStack(alignment: .top, spacing: OnboardingIntroPanelStyle.contentSpacing) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        ...
                        .foregroundStyle(.primary)
                    (
                        Text("The connected Carapace agent can use device capabilities you enable.")
                            + Text(verbatim: " ")
                            + Text(
...
private struct OnboardingCommandChip: View {
    ...
    private let command = "carapace qr"
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Button {
                ...
                Image(systemName: self.didCopy ? "checkmark" : "doc.on.doc")
                    .font(CarapaceType.subheadSemiBold)
                    .foregroundStyle(
                        self.didCopy ? CarapaceBrand.activationPrimaryAction : Color.secondary.opacity(0.56))
        ...
        .foregroundStyle(CarapaceBrand.activationPrimaryAction)
        .padding(.horizontal, 16)
        .frame(maxWidth: .infinity, alignment: .leading)
        .frame(height: 54)
        .background {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(CarapaceBrand.activationNeutralSurface)
        }
        .overlay {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(CarapaceBrand.activationNeutralStroke, lineWidth: 0.5)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(spacing: 36) {
                            ...
                            if self.isConnecting {
                                HStack(spacing: 8) {
                                    ProgressView()
                                        ...
                                        .tint(CarapaceBrand.activationPrimaryActionText)
                                    Text("Connecting…")
                                        .font(CarapaceType.subheadSemiBold)
                                }
                            } else {
                    ...
                    VStack(spacing: 14) {
                        ...
                        .buttonStyle(CarapaceSecondaryActionButtonStyle(height: 54, shadowOpacity: 0.018))
                        .disabled(self.isConnecting)
                    }
                }
                .padding(.top, 46)
        ...
        OnboardingActivationCanvas {
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .bottomTrailing) {
                    CarapaceActivationGlyph(size: 86, mood: .celebrating, interactive: true)
                        .shadow(color: CarapaceBrand.activationGlow.opacity(0.18), radius: 12, x: 0, y: 6)

                    Image(systemName: "checkmark")
                        .font(CarapaceType.headlineBold)
                        ...
                        .overlay {
                            Circle()
                                .stroke(CarapaceBrand.activationCanvas, lineWidth: 3)
                        }
                }
                .padding(.bottom, 22)
...
struct OnboardingModeIcon: View {
    ...
    var body: some View {
        Image(systemName: self.symbol)
            .font(CarapaceType.subheadSemiBold)
            .foregroundStyle(self.selected ? CarapaceBrand.activationPrimaryActionText : .secondary)
            .frame(width: 34, height: 34)
            .background {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(self.selected ? CarapaceBrand.activationPrimaryGradient : CarapaceBrand
                        .activationNeutralGradient)
                    .shadow(
                        color: self.selected ? CarapaceBrand.activationGlow.opacity(0.18) : .clear,
                        radius: 5,
                        x: 0,
                        y: 2)
            }
            .overlay {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        self.selected ? Color.white.opacity(0.30) : CarapaceBrand.activationNeutralStroke,
                        lineWidth: 0.5)
            }
    }
}
