...
final class RemindersService: RemindersServicing {
    ...
    func list(params: CarapaceRemindersListParams) async throws -> CarapaceRemindersListPayload {
        ...
        let payload: [CarapaceReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return CarapaceReminderPayload(
        ...
        return CarapaceRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CarapaceRemindersAddParams) async throws -> CarapaceRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = Self.date(fromDueComponents: reminder.dueDateComponents)
        let payload = CarapaceReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CarapaceRemindersAddPayload(reminder: payload)
    }
