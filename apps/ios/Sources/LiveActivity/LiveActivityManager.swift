...
final class LiveActivityManager {
    ...
    private struct StatusPresentation {
        let status: CarapaceActivityAttributes.ContentState.Status
        let verbatimDetail: String?
    }

    private let logger = Logger(subsystem: "ai.carapacefoundation.app", category: "LiveActivity")
    ...
    private var currentActivity: Activity<CarapaceActivityAttributes>?
    private var currentState: CarapaceActivityAttributes.ContentState?
    ...
    private func startActivity(_ request: LiveActivityPresentationRequest) {
        ...
        do {
            let activity = try Activity.request(
                attributes: CarapaceActivityAttributes(
    ...
    private func enqueueLatestUpdate(
        activity: Activity<CarapaceActivityAttributes>,
        state: CarapaceActivityAttributes.ContentState,
        staleDate: Date?)
    {
    ...
    private func startUpdateWorker(activity: Activity<CarapaceActivityAttributes>, generation: UInt64) {
    ...
    private func endCurrentActivity(reason: String) {
        ...
        let finalState = CarapaceActivityAttributes.ContentState(
            status: .disconnected,
            verbatimDetail: nil,
            startedAt: startedAt)
        Task {
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<CarapaceActivityAttributes>.activities
    ...
    nonisolated static func shouldHydrate(
        status: CarapaceActivityAttributes.ContentState.Status,
        startedAt: Date,
        staleDate: Date?,
        now: Date,
    ...
    nonisolated static func shouldResetVoiceSamples(
        previousStatus: CarapaceActivityAttributes.ContentState.Status?) -> Bool
    {
        previousStatus == nil
    }
    ...
    private func end(activity: Activity<CarapaceActivityAttributes>) {
    ...
    private static func voiceDetail(
        _ value: String,
        status: CarapaceActivityAttributes.ContentState.Status) -> String?
    {
