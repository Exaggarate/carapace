...
final class MotionService: MotionServicing {
    func activities(params: CarapaceMotionActivityParams) async throws -> CarapaceMotionActivityPayload {
        ...
        let mapped: [CarapaceMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return CarapaceMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = CarapacePedometerPayload(
