...
struct GatewayQuickSetupSheet: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    if let candidate = self.bestCandidate {
                        ...
                        if availability.canConnect {
                            ...
                            .buttonStyle(CarapacePrimaryActionButtonStyle())
                            ...
                            .buttonStyle(CarapaceSecondaryActionButtonStyle())
                        }
            ...
            .background(CarapaceBrand.activationCanvasGradient)
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    ...
                    .buttonStyle(CarapaceCloseButtonStyle())
                }
            }
        }
    ...
    static func headerMood(
        connecting: Bool,
        hasError: Bool,
        hasCandidate: Bool) -> CarapaceMascotMood
    {
        if connecting {
            .working
        } else if hasError {
...
private struct GatewayQuickSetupHeader: View {
    ...
    let mood: CarapaceMascotMood

    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ZStack(alignment: .bottomTrailing) {
                CarapaceActivationGlyph(size: 70, mood: self.mood, interactive: true)
                    .shadow(color: CarapaceBrand.activationGlow.opacity(0.18), radius: 10, x: 0, y: 5)

                Image(systemName: "antenna.radiowaves.left.and.right")
                    .font(CarapaceType.caption2SemiBold)
                    .foregroundStyle(CarapaceBrand.activationPrimaryActionText)
                    .frame(width: 28, height: 28)
                    .background {
                        Circle()
                            .fill(CarapaceBrand.activationPrimaryGradient)
                    }
                    .overlay {
                        Circle()
    ...
    private var subtitle: LocalizedStringKey {
        if self.hasCandidate {
            return "Carapace found a gateway on this network. Review it, then pair this iPhone as a secure node."
        }
        return "Carapace is searching the local network and tailnet for a Gateway you can trust."
    }
}
...
private struct GatewayQuickSetupCandidatePanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top, spacing: 12) {
                Image(systemName: "network")
                    ...
                    .shadow(color: CarapaceBrand.activationGlow.opacity(0.18), radius: 6, x: 0, y: 3)
        ...
        .carapaceCraftSurface(cornerRadius: 22)
        .accessibilityElement(children: .combine)
    }
...
private struct GatewayQuickSetupHairlineDivider: View {
    var body: some View {
        Rectangle()
            .fill(CarapaceBrand.activationNeutralDivider)
            .frame(height: 0.5)
            .frame(maxWidth: .infinity)
    }
}
        ...
        Text(self.text)
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(CarapaceBrand.activationGlow.opacity(0.10))
            }
            .overlay {
                Capsule(style: .continuous)
                    .stroke(CarapaceBrand.activationHairline, lineWidth: 0.6)
            }
    }
}
    ...
    var body: some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(CarapaceBrand.activationInsetGradient)
        }
        .overlay {
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .stroke(CarapaceBrand.activationHairline, lineWidth: 0.7)
        }
    }
}
...
private struct GatewayQuickSetupEmptyState: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                GatewayQuickSetupInstructionRow(text: "Run carapace gateway --port 18789.")
...
extension GatewayDiscoveryModel.DiscoveredGateway {
    fileprivate static let previewGateway = GatewayDiscoveryModel.DiscoveredGateway(
        name: "Studio Gateway",
        endpoint: .hostPort(
            host: .name("carapace.local", nil),
            port: 18789),
        stableID: "preview-gateway",
        debugID: "carapace.local",
        lanHost: "carapace.local",
        tailnetDns: nil,
        gatewayPort: 18789,
        tlsEnabled: true,
