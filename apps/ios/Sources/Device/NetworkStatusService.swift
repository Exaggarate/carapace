...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async throws -> CarapaceNetworkStatusPayload {
        guard timeoutMs > 0 else { throw URLError(.timedOut) }

        return try await withCheckedThrowingContinuation { cont in
            let monitor = NWPathMonitor()
            let queue = DispatchQueue(label: "ai.carapacefoundation.app.network-status")
    ...
    private static func payload(from path: NWPath) -> CarapaceNetworkStatusPayload {
        let status: CarapaceNetworkPathStatus = switch path.status {
