...
struct CommandSessionSection: Identifiable {
    ...
    let id: ID
    let title: String
    let entries: [CarapaceChatSessionEntry]
    let showsHeader: Bool
}

enum CommandSessionGrouping {
    static func sections(
        from entries: [CarapaceChatSessionEntry],
    ...
    static func previewOrder(_ entries: [CarapaceChatSessionEntry]) -> [CarapaceChatSessionEntry] {
    ...
    static func previewSelection(
        _ entries: [CarapaceChatSessionEntry],
        currentKey: String,
        limit: Int = 3) -> [CarapaceChatSessionEntry]
    {
    ...
    static func members(
        of group: String,
        in lists: [[CarapaceChatSessionEntry]]) -> [CarapaceChatSessionEntry]
    {
    ...
    static func activityTimestamp(_ entry: CarapaceChatSessionEntry) -> Double {
        entry.lastActivityAt ?? entry.updatedAt ?? 0
    }

    private static func sortedByActivity(
        _ entries: [CarapaceChatSessionEntry]) -> [CarapaceChatSessionEntry]
    {
        entries.sorted { lhs, rhs in
            let left = self.activityTimestamp(lhs)
            let right = self.activityTimestamp(rhs)
