...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--carapace-appearance") else {
            return nil
        }
...
extension TalkWaveformPalette {
    ...
    static let carapaceBrand = TalkWaveformPalette(
        active: [
            CarapaceBrand.carapaceCoral,
            CarapaceBrand.carapaceSea,
            CarapaceBrand.accent,
        ],
        inactive: [
...
struct CarapaceActivationGlyph: View {
    let size: CGFloat
    var mood: CarapaceMascotMood = .idle
    /// Opt-in tap Easter eggs; leave off when the glyph sits inside a control.
    var interactive = false

    var body: some View {
        CarapaceMascotView(floats: false, mood: self.mood, interactive: self.interactive)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: CarapaceBrand.activationGlow.opacity(0.18),
...
extension View {
    func carapaceSheetChrome() -> some View {
        self
            .tint(CarapaceBrand.accent)
            .background {
                CarapaceBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func carapaceCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(CarapaceCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct CarapaceCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(CarapaceBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(CarapaceBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct CarapacePrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? CarapaceBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? CarapaceBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : CarapaceBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? CarapaceBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? CarapaceBrand.activationPrimaryAction.opacity(0.72)
                            : CarapaceBrand.activationNeutralStroke,
                        lineWidth: 0.75)
            }
...
struct CarapaceSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(CarapaceType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? CarapaceBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(CarapaceBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                CarapaceBrand.activationSecondaryActionTop,
                CarapaceBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct CarapaceCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(
                self.isEnabled
                    ? CarapaceBrand.activationPrimaryAction
                    : Color.secondary)
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(CarapaceBrand.activationNeutralGradient)
                    .shadow(
            ...
            .overlay {
                Capsule(style: .continuous)
                    .stroke(CarapaceBrand.activationNeutralStroke, lineWidth: 0.6)
            }
