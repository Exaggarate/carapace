...
enum WatchPromptAction: Sendable {
    case delivery(CarapaceWatchChatDeliveryCommand)
...
@MainActor
final class CarapaceAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.carapacefoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.carapacefoundation.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.carapacefoundation.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? CarapaceAppModelRegistry.appModel
    }
    ...
    private func registerForRemoteNotificationsIfEnrollmentReady(_ application: UIApplication) async {
        ...
        guard !PushBuildConfig.current.usesCarapaceHostedRelay
        ...
        userInfo: [AnyHashable: Any]) -> WatchPromptAction?
    {
        ...
        guard let payload = userInfo[WatchPromptNotificationBridge.chatDeliveryContextKey] as? [String: Any],
              let context = try? CarapaceWatchChatDeliveryCodec.decodeContext(payload),
        ...
        return .delivery(CarapaceWatchChatDeliveryCommand(
            context: context,
...
enum WatchPromptNotificationBridge {
    static let typeKey = "carapace.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "carapace.watch.promptId"
    static let sessionKeyKey = "carapace.watch.sessionKey"
    static let gatewayStableIDKey = "carapace.watch.gatewayStableID"
    static let chatDeliveryContextKey = "carapace.watch.chatDeliveryContext"
    static let actionPrimaryIDKey = "carapace.watch.action.primary.id"
    static let actionPrimaryLabelKey = "carapace.watch.action.primary.label"
    static let actionSecondaryIDKey = "carapace.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "carapace.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "carapace.watch.action.primary"
    static let actionSecondaryIdentifier = "carapace.watch.action.secondary"
    static let actionIdentifierPrefix = "carapace.watch.action."
    static let actionIDKeyPrefix = "carapace.watch.action.id."
    static let actionLabelKeyPrefix = "carapace.watch.action.label."
    static let categoryPrefix = "carapace.watch.prompt.category."
        ...
        notificationCenter: NotificationCentering) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CarapaceWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CarapaceWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Carapace" : title
        content.body = body
    ...
    static func publishAdmissionFailure(upgradeRequired: Bool, notificationCenter: NotificationCentering) async {
        ...
        content.body = upgradeRequired
            ? String(localized: "Update Carapace on iPhone and Apple Watch, then request a new prompt.")
            : String(localized: "Open Carapace on iPhone and try the action again.")
    ...
    private static func categoryActions(_ actions: [CarapaceWatchAction]) -> [UNNotificationAction] {
...
struct CarapaceApp: App {
    ...
    @UIApplicationDelegateAdaptor(CarapaceAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        (UserDefaults(suiteName: CarapaceAppGroup.identifier) ?? .standard)
            .removeObject(forKey: "share.defaultInstruction")
        CarapaceType.installUIKitAppearance()
        let appModel = NodeAppModel(audioAdmissionInitiallyAllowed: false)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--carapace-reset-onboarding") {
        ...
        #endif
        CarapaceAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        CarapaceType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--carapace-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--carapace-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private static var liveActivityVoicePreviewEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--carapace-live-activity-voice-preview")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = CarapaceBrand.uiAccent
        }
    }
...
extension CarapaceApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Carapace: installing uncaught exception handler")
