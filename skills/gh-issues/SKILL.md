...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${CARAPACE_CONFIG_PATH:-${CARAPACE_STATE_DIR:-$HOME/.carapace}/carapace.json}"
...
```text
${CARAPACE_STATE_DIR:-$HOME/.carapace}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with carapace message send if route provided.
```
