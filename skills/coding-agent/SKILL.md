...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.carapace`, `$CARAPACE_STATE_DIR`, or active Carapace state dirs.
...
- Worker must send completion/failure via `carapace message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/carapace`; use an isolated checkout.
...
6. Launch the worker in the isolated worktree, never the primary checkout. For Carapace, the primary checkout under `~/Projects/carapace` remains forbidden.
...
carapace message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use carapace system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t carapace-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
...
OpenAI auth overrides. Never export the worker home into the Carapace Gateway
environment.
