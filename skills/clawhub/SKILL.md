...
Use `carapace skills` to discover and manage skills for the current Carapace
...
```bash
carapace skills search "postgres backups"
```
...
```bash
carapace skills verify my-skill
carapace skills install my-skill
carapace skills install my-skill --version 1.2.3
```
...
Use the original agent workspace for agent-specific skills or the Carapace
...
If `CARAPACE_STATE_DIR` is set, use its value instead of `~/.carapace`:
...
- `carapace skills install` installs into the active workspace by default.
