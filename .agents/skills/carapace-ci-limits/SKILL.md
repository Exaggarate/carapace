---
name: carapace-ci-limits
description: Manage Carapace GitHub Actions and Blacksmith CI capacity, runner-registration budgets, fanout caps, main-push single-flight, shard sizing, hosted-runner offload, queue health, and safe ramp-down/ramp-up changes. Use when tuning `.github/workflows/*`, `docs/ci.md`, CI runner labels, matrix `max-parallel`, ClawSweeper/Blacksmith burst protection, CodeQL runner placement, or investigating slow/queued Carapace CI.
---

# Carapace CI Limits

Use this skill for CI capacity changes, not ordinary test failure triage. The
goal is to keep Carapace fast while distinguishing runner registration, runner
...
- GitHub runner registrations for `carapace` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `carapace`
...
- **Hosted Mac exact dependencies (2026-09-01):** The same-head publisher and
  consumer in [run 33458856298](https://github.com/Exaggarate/carapace/actions/runs/33458856298)
...
- **Actions-artifact checkout (2026-08-16):** Do not recommend replacing the
  ...
  artifact. [PR #124818](https://github.com/Exaggarate/carapace/pull/124818)
  ...
  worse: [run 31971531521](https://github.com/Exaggarate/carapace/actions/runs/31971531521)
...
ghx run list -R carapace/carapace --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx run list -R carapace/clawsweeper --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx api repos/carapace/clawsweeper/actions/runs/<run-id>/jobs --paginate --jq '.jobs[] | {id,name,status,conclusion,labels,created_at,started_at,completed_at,runner_name,runner_group_name}'
blacksmith testbox list --all
curl -fsS https://github.com/Exaggarate/carapace | jq '{generated_at,fleet,diagnostics:{errors:.diagnostics.errors}}'
curl -fsS https://github.com/Exaggarate/carapace | jq '{generated_at,review:.lanes.review,publication:.lanes.publication,state_writer,state_append}'
...
- **Carapace test runtime:** jobs start quickly but one lane dominates wall time.
  Use `$carapace-test-performance` instead of runner tuning.
- **Real failing CI:** one job fails after starting. Use `$github:gh-fix-ci` or
  `$carapace-testing`, not this skill.
...
ClawSweeper, ClawHub, Clownfish, Carapace RTT, and Clawbench.
...
7. Split truly slow tests with `$carapace-test-performance`; do not hide a slow
...
- CPU-heavy test-type, core test-type stripe, runtime-topology, and npm preflight
  ...
  [run 33538827388](https://github.com/Exaggarate/carapace/actions/runs/33538827388)
...
- `CARAPACE_CI_RUNNER_BACKEND=github` routes every configurable `ci.yml` job
...
```bash
gh variable set CARAPACE_CI_RUNNER_BACKEND --repo carapace/carapace --body github
```
...
```bash
gh variable delete CARAPACE_CI_RUNNER_BACKEND --repo carapace/carapace
```
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/carapace-ci-limits/SKILL.md .agents/skills/carapace-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R carapace/carapace --watch --interval 15
```
...
CARAPACE_TESTBOX=1 scripts/pr prepare-run <pr>
```
...
```bash
ghx run list -R carapace/carapace --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
for repo in carapace/clawsweeper carapace/clawhub carapace/clownfish carapace/carapace-rtt carapace/clawbench; do
  ghx run list -R "$repo" --limit 12 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
done
curl -fsS https://github.com/Exaggarate/carapace | jq '.'
```
