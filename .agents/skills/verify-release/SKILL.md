---
name: verify-release
description: "Verify regular or extended-stable Carapace releases against the exact publication surfaces, workflow identities, package provenance, smoke tests, and live Gateway behavior expected for that release track."
---

# Verify Release

Use this when asked whether an Carapace release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-carapace-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo carapace/carapace --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
...
2. Root npm:
   - `npm view carapace@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     download `https://api.github.com/repos/carapace/carapace/tarball/v<VERSION>`
     into `/tmp/carapace-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `carapace.release.publishToNpm === true` and
     `carapace.release.publishToClawHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/carapace/carapace/actions/runs/<RUN>/jobs --paginate`.
...
4. **Provenance:** from trusted current tooling, run
   `node --import tsx scripts/carapace-npm-postpublish-verify.ts <VERSION>`.
...
1. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package carapace@<VERSION> -- carapace --version`.
...
2. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/carapace-release-smoke/home CARAPACE_WORKSPACE=/tmp/carapace-release-smoke/work pnpm carapace --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `carapace --dev gateway health --json`.
