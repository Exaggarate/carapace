---
name: carapace-qa-testing
description: Run, watch, debug, extend, or explain Carapace qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
5. If the user wants to watch the live UI, find the current `carapace-qa` listen port and report `http://127.0.0.1:<port>`.
...
- That item is the first place to look for:
  ...
  - `CARAPACE_QA_PROVIDER_MODE`
  - `CARAPACE_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Carapace`
  - item: `CARAPACE_QA_CONVEX_SITE_URL`
  - item: `CARAPACE_QA_CONVEX_SECRET_MAINTAINER`
  - item: `CARAPACE_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `CARAPACE QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `CARAPACE_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `CARAPACE_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Carapace/CARAPACE_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
CARAPACE_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
CARAPACE_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
CARAPACE_QA_PROVIDER_MODE="mock-openai" \
CARAPACE_NPM_TELEGRAM_PACKAGE_SPEC="carapace@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `CARAPACE_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `CARAPACE_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `CARAPACE_QA_PROVIDER_MODE`, map it explicitly to `CARAPACE_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `CARAPACE_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo carapace/carapace --ref main \
  -f package_spec=carapace@YYYY.M.D-beta.N \
  -f package_label=carapace@YYYY.M.D-beta.N \
...
```bash
gh api repos/carapace/carapace/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Carapace WhatsApp accounts in the shared pool.
...
- Validate with `pnpm carapace qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- Packaged `pnpm build` omits QA Lab + qa-channel by design (source-checkout
  only). To exercise `carapace qa`/qa-channel from a built dist, build with
  `CARAPACE_BUILD_PRIVATE_QA=1 pnpm build` (emits `dist/plugin-sdk/qa-lab.js`,
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Carapace identity combines with the character.
...
```bash
pnpm carapace qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `CARAPACE_HOME` sandboxed.
