...
Use `$release-carapace-ci` for workflow dispatch, manifests, and failed-child
...
Keep plugin `carapace.release.requireLatestDependencies` declarations. Upstream
...
keep those enabled. `CARAPACE_INSTALL_SMOKE_SKIP_NONROOT=1` is the existing
...
```bash
node --import tsx scripts/carapace-npm-postpublish-verify.ts <published-version>
```
...
`CARAPACE_LIVE_VIDEO_GENERATION_FULL_MODES=1`. Use `$one-password` before
...
- Dispatch **NPM Telegram Beta E2E** from main with
  `package_spec=carapace@<beta-version>`, `provider_mode=mock-openai`; require
...
- Use `$carapace-parallels-smoke` for published-package install/update with both
...
`$carapace-pr-maintainer` / `$carapace-ci-limits`; never synthesize prepare
