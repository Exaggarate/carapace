...
`carapace` npm package, official npm plugins, and matching Docker Gateway
...
- Read `docs/reference/RELEASING.md`,
  `scripts/carapace-npm-extended-stable-release.mjs`, and the relevant release
...
- Cover the core `carapace` package and every npm-publishable official plugin
...
- Read `SECURITY.md` and use `$security-triage` for security candidates. Route
  unpublished advisory work through `$carapace-ghsa-maintainer`; never expose
  private details before the security owner authorizes disclosure.
- Use `$carapace-testing` for proof selection, `$autoreview` before handoff,
  and `$carapace-pr-maintainer` for GitHub operations.
...
dist_tags=$(npm view carapace dist-tags --json --userconfig "$npm_userconfig")
published_version=$(printf '%s' "$dist_tags" | jq -r '."extended-stable" // empty')
if [[ -n "$published_version" ]]; then
  npm view "carapace@${published_version}" version \
...
- mode, published `carapace@extended-stable` version or approved bootstrap
