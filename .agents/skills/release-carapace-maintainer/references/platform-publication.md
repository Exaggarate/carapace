...
Use `$release-carapace-mac` for public handoff validation, release-ops
...
```bash
gh workflow run windows-node-release.yml --repo carapace/carapace --ref main \
  -f tag=vYYYY.M.PATCH \
...
`CarapaceCompanion-SHA256SUMS.txt`, expected Foundation Authenticode signer, and
