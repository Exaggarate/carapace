...
`$release-carapace-ci` for dispatch/recovery.
...
An early `Carapace Performance` run is optional beta confidence:
...
If notes change after Code qualification, use `$carapace-changelog-update`
...
Dispatch `.github/workflows/carapace-release-publish.yml` using the candidate
...
Campaign generation belongs to `$carapace-release-validation` and is not a
publish blocker. Requested Discord announcements use
`$release-carapace-announcement`; requested X posts use `$release-tweets`.
