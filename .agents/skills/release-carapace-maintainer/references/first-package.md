...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`carapace.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
- Bootstrap a new ClawHub package only from the trusted workflow source:
  `gh workflow run plugin-clawhub-new.yml --ref main -f plugins=@carapace/name -f ref=<full-release-sha> -f pretag_validation=true -f dry_run=true`.
