...
3. Make `CHANGELOG.md`'s `## YYYY.M.PATCH` section on `main` exactly match the
   ...
   `CARAPACE_ALLOW_ROOT_CHANGELOG_PR=1` remains an explicit override
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and
   `CARAPACE_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `carapace-stable-main-closeout.yml` starts from the `main` push carrying the
