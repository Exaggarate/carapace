---
name: release-carapace-maintainer
description: "Prepare, publish, recover, or verify Carapace beta, stable, and extended-stable releases, including approved backports."
---

# Carapace Release Maintainer
...
- Validation selection or failed proof: [validation and confidence](references/validation.md), with `$release-carapace-ci` for workflow execution and immutable manifests.
- Interrupted publication or registry promotion: [publication recovery](references/publication-recovery.md).
- Native assets: [platform publication](references/platform-publication.md), with `$release-carapace-mac` for macOS operations.
- Stable postpublish synchronization: [main closeout](references/stable-main-closeout.md).
- Release notes: `$carapace-changelog-update`. Requested announcements: `$release-carapace-announcement` for Discord, `$release-tweets` for X. Announcements never gate publication and require explicit posting authorization.
- Published artifact verification: `$verify-release`. GHSA operations: `$carapace-ghsa-maintainer` only with explicit security-workflow authorization.
