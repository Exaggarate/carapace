---
name: release-carapace-announcement
description: "Draft or post Carapace beta, stable, or extended-stable Discord release announcements from changelog, tag, registry, and validation evidence. Use when announcing a release, release candidate, or asking what users should test after an Carapace release."
---

# Carapace Release Announcement

Use with `release-carapace-maintainer` after a release is live.
...
- exact version, e.g. `Carapace 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  carapace update --channel beta --yes
  carapace --version
  ```
- fresh install path:
  `Install from https://github.com/Exaggarate/carapace`
- GitHub release link
...
```sh
carapace update --channel stable --yes
carapace --version
```

Fresh installs still point to `https://github.com/Exaggarate/carapace`.
...
sending it. Never use Carapace channel sends, bots, webhooks, relays, or tokens.
