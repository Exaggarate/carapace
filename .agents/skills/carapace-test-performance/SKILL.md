---
name: carapace-test-performance
description: Benchmark, diagnose, and optimize Carapace test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Carapace Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `CARAPACE_VITEST_IMPORT_DURATIONS=1 CARAPACE_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
3. Treat plugin-inspector failures as product signals:
   ...
   - command output should include wall time, exit code, and peak RSS when
     available.
4. Follow `$carapace-testing` for host selection. Trusted source benchmarks
...
5. If plugin performance is package-artifact sensitive, switch to
   `release-carapace-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `CARAPACE_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap evidence   | real leak vs retained module graph | `carapace-test-heap-leaks` workflow                                         |
...
Use `carapace-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `CARAPACE_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  CARAPACE_VITEST_IMPORT_DURATIONS=1 \
