...
`$carapace-testing` and `$crabbox`. Do not weaken guards to accommodate missing
...
hosted Testbox, use the documented `CARAPACE_PR_GATES_REMOTE=testbox` path.
...
```bash
git rev-parse refs/carapace/pr-merge-outcomes/<PR>
