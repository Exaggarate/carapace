...
const DEFAULT_EXPECTED_ORIGIN = "carapace/carapace";
...
const GATEWAY_STARTUP_TRACE_ENV = "CARAPACE_GATEWAY_STARTUP_TRACE";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    changedPath.startsWith("apps/shared/CarapaceKit/Sources/CarapaceProtocol/"),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/CarapaceKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `carapace-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "carapace-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.carapace-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".carapace"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.carapace.gateway.plist");
  let plistStat;
  try {
  ...
  const stateDir =
    typeof environmentVariables?.CARAPACE_STATE_DIR === "string"
      ? environmentVariables.CARAPACE_STATE_DIR
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.CARAPACE_CONFIG_PATH === "string"
      ? plist.EnvironmentVariables.CARAPACE_CONFIG_PATH
      : path.join(gatewayCommand.stateDir, "carapace.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.carapace.gateway.plist"))) {
    return null;
  }
...
function prepareLaunchAgentEntrypointReplacement(deployment, entrypoint, options = {}) {
  const temporaryPath = `${deployment.plistPath}.carapace-live-updater-${randomUUID()}`;
  ...
  try {
    ...
    const restore = () => {
      ...
      const rollbackPath = `${deployment.plistPath}.carapace-live-updater-rollback-${randomUUID()}`;
      try {
...
export function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  delete baseEnv.CARAPACE_GATEWAY_URL;
  delete baseEnv.CARAPACE_GATEWAY_TOKEN;
  delete baseEnv.CARAPACE_GATEWAY_PASSWORD;
  delete baseEnv.CARAPACE_CONFIG_PATH;
  delete baseEnv.CARAPACE_GATEWAY_PORT;
  ...
  if (wrapperPath && envFilePath) {
    ...
    try {
      const configuredPath = execFileSync(
        executable,
        [...wrapperPrefix, "/usr/bin/printenv", "CARAPACE_CONFIG_PATH"],
        boundedSyncOptions({
          encoding: "utf8",
          env: baseEnv,
  ...
  const overlayPath = path.join(
    ...
    `.carapace-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
  ...
  const envOverrides = [`CARAPACE_CONFIG_PATH=${overlayPath}`, `CARAPACE_GATEWAY_PORT=${port}`];
  const env = Object.assign(baseEnv, {
    CARAPACE_CONFIG_PATH: overlayPath,
...
export function prepareGatewaySuspension(
  ...
  const requestId = `carapace-live-updater-${randomUUID()}`;
...
async function runBuildWithPreservedMacApp(
  ...
  const appBundle = path.join(checkout, "dist/Carapace.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.carapace-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function armLaunchdEnvironmentRestore(name, previousValue) {
  const markerPath = path.join(
    tmpdir(),
    `.carapace-launchd-env-restore-${process.pid}-${randomUUID()}`,
  );
...
async function verifyGatewayDeepRpc(runCommand, checkout, expectedSha, deployment, now) {
  ...
  if (deployment) {
    ...
    await runUpdateCommand(
      ...
      "gateway.status",
      "pnpm",
      ["carapace", "gateway", "status", "--deep", "--require-rpc", "--json"],
      checkout,
      {
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "carapace") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/carapace", path.join(tmpdir(), "carapace")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `carapace-${date}.log`);
...
async function defaultVerifyMacTarget(checkout) {
  await delay(10_000);
  const executable = path.join(checkout, "dist/Carapace.app/Contents/MacOS/Carapace");
