...
```bash
export CARAPACE_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.carapace/carapace.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env CARAPACE_PARALLELS_DISCORD_TOKEN \
...
- Use the `carapace` wrapper for guest `message send/read`; `node carapace.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/carapace-parallels-smoke.*`
...
- host inbound nonce appears in `carapace message read` output
