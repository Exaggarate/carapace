---
name: discord-clawd
description: Use to talk to the Discord-backed Carapace agent/session; not for archive search.
---
...
Use the Carapace relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/carapace-relay/scripts/carapace_relay.py targets
python3 skills/carapace-relay/scripts/carapace_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/carapace-relay/scripts/carapace_relay.py ask \
  --target maintainers \
