...
export function defaultGithubSnapshotPath(base, target, gitCommonDir) {
  ...
  return path.resolve(gitCommonDir, "carapace-release-cache", defaultName);
}
...
function closingReferencesIn(text) {
  ...
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:carapace\/carapace)?#\d+)(?:\s*(?:,|and)\s*(?:(?:carapace\/carapace)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "carapace", name: "carapace") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "carapace", name: "carapace") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "carapace", name: "carapace") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "carapace", name: "carapace") {
