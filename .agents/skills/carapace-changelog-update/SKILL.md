---
name: carapace-changelog-update
description: Regenerate Carapace release changelog sections from git history before beta, stable, or extended-stable releases.
---

# Carapace Changelog Update
...
Use it with `release-carapace-maintainer`; this skill owns changelog content,
...
3. Generate the complete contribution record and editorial manifest before
   ...
   ```bash
   node --import tsx .agents/skills/carapace-changelog-update/scripts/verify-release-notes.mjs \
     --base <base-tag> \
     --target <target-ref> \
     --main-ref origin/main \
     --version <YYYY.M.PATCH> \
     --manifest /tmp/carapace-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@codex`, `@carapace`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `carapace/carapace`;
  cross-repository references such as `carapace/imsg#141` remain literal
