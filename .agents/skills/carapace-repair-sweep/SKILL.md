---
name: carapace-repair-sweep
description: "Run scoped Carapace issue/PR repair campaigns: coordinate workers, prove root causes, and land or close verified work under the requested authority."
---

# Carapace Repair Sweep

Use for a multi-item repair campaign. Root `AGENTS.md` owns repair and safety
policy; `$carapace-pr-maintainer` owns item review, GitHub writes, and landing.
...
Use `$gitcrawl` for available archive discovery, `$carapace-testing` for proof,
...
Choose proportional proof through `$carapace-testing`. Trusted development can
