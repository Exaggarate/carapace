...
open_external_terminal() {
  ...
  if [[ $system_name == "Darwin" ]] && command -v open >/dev/null 2>&1; then
    ...
    attach_dir=$(mktemp -d "${TMPDIR:-/tmp}/carapace-tui-prototype.XXXXXX")
  ...
  if [[ $system_name == "Linux" && -n ${WSL_DISTRO_NAME:-} ]] &&
    ...
    if wt.exe new-tab --title "Carapace TUI prototypes" \
  ...
  if [[ $system_name == "Linux" ]]; then
    if command -v xdg-terminal-exec >/dev/null 2>&1; then
      if xdg-terminal-exec --title="Carapace TUI prototypes" \
        tmux attach-session -t "=$session_name"; then
        return 0
      fi
    fi
