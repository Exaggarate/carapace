---
name: prototype-carapace-tui
description: Build throwaway, fixture-driven Carapace Clack or Pi TUI prototypes and compare multiple interactive variants side by side in tmux without running the full application or touching live state.
user-invocable: true
disable-model-invocation: true
---

# Prototype Carapace TUI

Use this skill to isolate one Carapace terminal surface, render it with the
real Carapace TUI stack, and compare two to six design variants in tmux.
...
- Use the real Carapace renderer, theme, copy, and component where practical.
...
- Do not run the full Carapace application when the isolated surface is enough.
...
- Agent shell, chat, overlay, editor, selector, or `src/tui/**` component:
  use `@earendil-works/pi-tui` and the existing Carapace component.
...
```bash
.agents/skills/prototype-carapace-tui/scripts/launch-tmux-grid.sh \
  --open \
