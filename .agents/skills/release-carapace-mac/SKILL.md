---
name: release-carapace-mac
description: "Run or recover Carapace macOS release signing, notarization, appcast, and asset promotion."
---

# Carapace Mac Release

Use with `$release-carapace-maintainer`, `$release-carapace-ci`, `$one-password`, and `$release-private` if it exists when stable macOS assets, release-ops mac preflight, notarization, appcast promotion, or mac release recovery is involved.
...
Target release-ops repo environment: `carapace/releases`, env `mac-release`.
...
- `carapace/carapace` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `carapace/carapace` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `carapace/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `carapace/carapace` GitHub release.
...
- Carapace uses `scripts/notarize-mac-artifact.sh`.
...
```bash
gh workflow run macos-release.yml --repo carapace/carapace \
...
```bash
gh workflow run carapace-macos-publish.yml --repo carapace/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run carapace-macos-validate.yml --repo carapace/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `carapace/releases` publish/validate workflows run from their own
  trusted `main` workflow ref. Real publish has a guard that rejects any other
  workflow ref. That displayed `main` ref is expected; the public Carapace
  source is selected by `tag` and optional `source_ref`.

## Verify

- `gh release view vYYYY.M.PATCH --repo carapace/carapace` shows zip, dmg, dSYM zip, not draft, not prerelease.
- Public `main` `appcast.xml` points at `Carapace-YYYY.M.PATCH.zip`.
