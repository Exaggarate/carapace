---
name: release-carapace-plugin-testing
description: Plan and run pre-release Carapace plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# Carapace Pre-Release Plugin Testing
...
release?" It complements `carapace-testing`; use that skill too when choosing
...
the main Carapace checkout. Do not run `pnpm install` there. For broad or
...
```bash
CARAPACE_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
CARAPACE_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
CARAPACE_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
CARAPACE_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=carapace@beta` for published beta proof. Keep
...
ghx workflow run plugin-npm-release.yml \
  ...
  -f plugins=@carapace/meta-provider \
  -f ref="${release_sha}" \
...
Start packaged Carapace in Docker with clean state:
...
- `carapace status --json` includes plugin diagnostics
- `carapace plugins inspect --all --json` is parseable
...
`carapace/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
