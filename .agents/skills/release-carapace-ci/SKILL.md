---
name: release-carapace-ci
description: "Run, watch, debug, and summarize Carapace full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# Carapace Release CI

Use this with `$release-carapace-maintainer` and `$carapace-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
- Reused Testboxes are provenance-gated after their first successful run.
  ...
  `CARAPACE_TESTBOX_ALLOW_STALE=1` for release evidence.
...
```bash
CARAPACE_BUILD_PRIVATE_QA=1 pnpm build
```
...
```bash
node .agents/skills/release-carapace-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
Publish with `carapace-release-publish.yml` using `release_profile=from-validation`
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo carapace/carapace --json jobs \
...
7. If a required PR CI run is capacity-stalled with queued jobs and no active
   ...
   `gh api 'repos/carapace/carapace/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
   ...
   `gh workflow run ci.yml --repo carapace/carapace --ref <pr-head-branch> -f
