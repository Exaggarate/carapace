---
name: claw-score
description: Audit or refresh Carapace maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the Carapace maturity scorecard in this repo.
This is the carapace-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by Carapace QA workflows.
...
Run from the carapace repo root.
...
```bash
pnpm carapace qa coverage --json
```
...
By default, Completeness measures how fully Carapace exposes the intended
...
tree to carapace. Evidence-enriched scorecard outputs belong in short-lived
