...
- WSL Setup and Updates: WSL2 + Ubuntu installation, Node runtime, Linux install flow inside WSL2, WSL2 runtime boundary, WSL2 network-family requirements, Source install and build inside WSL2, carapace update, npm/pnpm/git package-root, Managed systemd Gateway restart, Service metadata refresh, Package-manager caveats
...
- Diagnostics and Repair: carapace doctor, carapace status, carapace logs, SecretRef, WSL/systemd unavailable hints, Operator repair guidance after WSL2 service
