...
- Gateway Service Lifecycle: Per-user Gateway LaunchAgent install, launchctl bootstrap, LaunchAgent labels, Gateway token/env handling, App-managed LaunchAgent handoff, carapace update package/git handoff, Managed service refresh, Stale updater launchd job detection, carapace uninstall, Stranded service recovery
- Diagnostics and Observability: LaunchAgent log paths, carapace gateway status --deep, Gateway silently stops responding, Stale updater jobs
