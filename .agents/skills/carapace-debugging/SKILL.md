---
name: carapace-debugging
description: Debug Carapace model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code, including fetching stored sessions, transcripts, and attachments as evidence.
---

# Carapace Debugging

Use this skill when Carapace behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `carapace logs`, and targeted debug flags.
- `docs/reference/test.md` for local test commands.
- `docs/tools/code-mode.md` for code-mode exec/wait and tool catalog rules.
- Use `$carapace-testing` for choosing test lanes.
...
CARAPACE_DEBUG_MODEL_PAYLOAD=full-redacted CARAPACE_DEBUG_SSE=peek carapace gateway
```

Useful flags:

- `CARAPACE_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `CARAPACE_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `CARAPACE_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `CARAPACE_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `CARAPACE_DEBUG_SSE=events`: first-event and stream-completion timing.
- `CARAPACE_DEBUG_SSE=peek`: first five redacted SSE events.
- `CARAPACE_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
...
```bash
carapace sessions list --agent <agentId> --json
carapace sessions tail
carapace sessions export-trajectory
```

Docs: `docs/reference/database-schemas.md` for the store layout,
../../../docs/cli/sessions.md for the CLI.

Raw store (when the CLI is unavailable, e.g. inspecting a remote host over
SSH, or you need event-level detail):

- Per-agent data plane: `~/.carapace/agents/<agentId>/agent/carapace-agent.sqlite`.
  Canonical schema: `src/state/carapace-agent-schema.sql`.
- Hosted/systemd installs keep state under the service user's home (e.g.
  `/home/carapace/.carapace/...`), not root's — root may carry a separate
  stray install with different agents. If the expected agent dir is missing,
  locate the real DB: `find / -maxdepth 6 -name carapace-agent.sqlite`.
...
- Sender provenance: real user messages carry `message.__carapace`
...
- User-attached images are not `image` content parts. They ride the message
  envelope: `message.__carapace.media[]` entries with `url`
