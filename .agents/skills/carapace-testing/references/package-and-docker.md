...
recovery belong to `$release-carapace-ci`; plugin release coverage belongs to
`$release-carapace-plugin-testing`. Apply the entrypoint's source-trust and
...
- `source=npm`: select the requested dist-tag or exact Carapace release version.
  Resolve current tags with `npm view carapace dist-tags --json --prefer-online`
...
`carapace plugins install npm-pack:<path.tgz> --force` to exercise the managed
...
`docker_lanes` in `carapace-live-and-e2e-checks-reusable.yml` and disable unrelated
...
```bash
CARAPACE_DOCKER_ALL_LANES=<lane> node scripts/test-docker-all.mjs --plan-json
```
...
mount the operator's `~/.carapace` and claim port 18789; do not reuse them for
...
`CARAPACE_DOCKER_E2E_WORKFLOW_REF` explicitly selects another trusted definition.
