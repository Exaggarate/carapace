---
name: carapace-testing
description: Choose proportional Carapace tests and checks, diagnose failures, and route environment-sensitive or release proof to its owner.
---
...
- Remote leases and credentials: [`$crabbox`](../crabbox/SKILL.md), with the
  Carapace bootstrap binding below.
...
- Release candidates, full-validation dispatch, evidence identity or recovery:
  [`$release-carapace-ci`](../release-carapace-ci/SKILL.md). A narrow green rerun
  does not itself authorize publication. Do not substitute moving `main` for
  the recorded candidate or Tooling SHA.
- Plugin release matrix: [`$release-carapace-plugin-testing`](../release-carapace-plugin-testing/SKILL.md).
- New or changed Docker lanes: [`$carapace-docker-e2e-authoring`](../carapace-docker-e2e-authoring/SKILL.md).
...
For untrusted Carapace AWS proof, supply the clean trusted `main` copy of
...
fallback `CARAPACE_TEST_CHANGED_BROAD=1 pnpm test:changed`.
...
`CARAPACE_VITEST_FS_MODULE_CACHE_PATH`. Checks can schedule Vitest too.
For worker-sensitive failures, `CARAPACE_VITEST_MAX_WORKERS=1 pnpm test <path>`
