...
description: "Triage Carapace security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof."
---

# Security Triage

Use when reviewing Carapace security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/carapace/carapace/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view carapace version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/carapace/carapace/security-advisories/<GHSA>
gh api /repos/carapace/carapace/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view carapace version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo carapace/carapace --match title,body,comments -- "<terms>"
gh search prs --repo carapace/carapace --match title,body,comments -- "<terms>"
```
