---
name: clawsweeper
description: "Use for all ClawSweeper work: Carapace issue/PR sweep reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one Carapace
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `carapace/clawsweeper` for comment-router
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/carapace-carapace/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/carapace/inbox/clawsweeper-carapace-carapace-123.md
pnpm run repair:dispatch -- jobs/carapace/inbox/clawsweeper-carapace-carapace-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo carapace/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo carapace/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo carapace/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo carapace/clawsweeper --body 1
```
...
@carapace-clawsweeper fix ci
@carapace-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo carapace/carapace --lookback-minutes 180
pnpm run repair:comment-router -- --repo carapace/carapace --execute --wait-for-capacity
```
...
leave the local Carapace checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central Carapace security handling.
...
```bash
gh run list --repo carapace/clawsweeper --workflow sweep.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo carapace/clawsweeper --workflow repair-cluster-worker.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo carapace/clawsweeper --workflow repair-comment-router.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo carapace/clawsweeper --workflow github-activity.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo carapace/carapace --workflow "ClawSweeper Dispatch" \
