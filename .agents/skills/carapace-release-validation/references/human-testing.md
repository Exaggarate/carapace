...
`.artifacts/carapace-release-validation/<stable-train>-<timestamp>.md`. Fill its
...
at `{{VALIDATION_GUIDANCE}}`. In those bytes, replace `{{CARAPACE}}` with
`ocm @<test-env> --` for either OCM lane or `carapace` for the plain in-place
lane. Replace `{{RESTART_GATEWAY}}` with `ocm service restart <test-env>` for
either OCM lane or `carapace gateway restart` for the plain in-place lane. Use
...
surface, then immediately search open and closed `carapace/carapace` issues with
