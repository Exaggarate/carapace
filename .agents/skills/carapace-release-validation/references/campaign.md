...
`<!-- carapace-release-validation:<stable-train> -->`. Read the current beta
...
Otherwise run `gh api 'repos/carapace/carapace/releases?per_page=100'` once and
...
```sh
gh api 'repos/carapace/carapace/issues?state=open&labels=release-validation&per_page=2'
```
...
```text
Issue: https://github.com/Exaggarate/carapace/issues/<number>
```
