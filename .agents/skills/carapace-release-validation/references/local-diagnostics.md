...
test gateway. It installs Carapace's diagnostics-otel plugin only in the
...
3. Install the current official ClawHub package into the fixture only:
   `ocm @<test-env> -- plugins install clawhub:@carapace/diagnostics-otel`.
...
4. Replace only the fixture's `diagnostics.otel` object with this exact JSON
   ...
   ```json
   {
     ...
     "serviceName": "carapace-release-validation",
     "traces": true,
     "metrics": true,
     "logs": true,
     "logsExporter": "otlp",
...
gateway, personal Carapace home, and shared GitHub issue remain untouched.
