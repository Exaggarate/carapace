...
2. Fetch `../../../../docs/maturity/scorecard.md`. Extract the live
...
5. Render each selected surface as:
   ...
   `{{CARAPACE}}` wherever the tester should invoke the selected gateway and
   `{{RESTART_GATEWAY}}` for its restart command. Do not assume OCM, add other
   execution placeholders, or say only "use" or "verify."

6. Replace the issue title with `Carapace <YYYY.M.D> beta feedback`. Render the
   body in this order, with no beta-history section:

   ```md
   <!-- carapace-release-validation:<stable-train> -->
...
7. Write this exact JSON shape to `RELEASE_VALIDATION_ARTIFACT_PATH`:
   ...
   ```json
   {
     ...
     "title": "Carapace <YYYY.M.D> beta feedback",
     "body": "<rendered body>"
   }
   ```
