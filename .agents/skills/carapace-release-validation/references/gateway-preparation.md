...
the plain personal gateway with `carapace --version` and `carapace gateway
status --json --no-probe`. When OCM is available, also use `ocm adopt inspect
~/.carapace --json` to resolve aliases safely.

Read only each gateway's display name, Carapace version, and running/stopped
...
show `Personal ~/.carapace` with its known version and state. Ask which gateway
...
Reply exactly `install OCM` to let me install the Carapace Manager CLI, or
...
```sh
curl -fsSL https://github.com/Exaggarate/carapace/ocm/releases/latest/download/install.sh | bash
ocm --version
```
...
# Plain source only:
ocm adopt plan --name <test-env> ~/.carapace --json
ocm adopt import --name <test-env> ~/.carapace --json
```
...
stop <source-env>`; for the plain source, use `carapace gateway stop`. There is
no `ocm stop` command.

### In-place gateway

Do not copy the selected gateway. A plain gateway will use its own `carapace`
...
```sh
main_sha="$(git ls-remote https://github.com/Exaggarate/carapace.git refs/heads/main | awk 'NR == 1 { print $1 }')"
test "$(printf '%s' "$main_sha" | wc -c | tr -d ' ')" = 40
main_checkout="$(mktemp -d "${TMPDIR:-/tmp}/carapace-release-validation-main.XXXXXX")"
git -C "$main_checkout" init -q
git -C "$main_checkout" remote add origin https://github.com/Exaggarate/carapace.git
...
First inspect `carapace update status --json`. Create a full verified backup in
a private owner-only directory outside `~/.carapace`, retain its resulting
archive path, and never expose that path in GitHub output:

```sh
carapace backup create --output <private-backup-dir> --verify
CARAPACE_UPDATE_DEV_TARGET_REF="$main_sha" carapace update --channel dev --dry-run --json
```
...
```sh
CARAPACE_UPDATE_DEV_TARGET_REF="$main_sha" carapace update --channel dev --yes --json
...
Report every error immediately, including errors recovered by a retry. Carapace
