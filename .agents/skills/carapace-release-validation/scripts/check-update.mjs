...
async function readMetadataJson(root, filename, isValid) {
  for (const directory of CARAPACE_METADATA_DIRECTORIES) {
...
async function collectCarapaceTreeEntries(directory, root = directory) {
  const collected = [];
  const entries = await readdir(directory, { withFileTypes: true });
  entries.sort((left, right) => (left.name < right.name ? -1 : left.name > right.name ? 1 : 0));
  for (const entry of entries) {
    if (directory === root && CARAPACE_METADATA_DIRECTORIES.has(entry.name)) continue;
    ...
    if (stat.isDirectory()) {
      ...
      collected.push(...(await collectCarapaceTreeEntries(path, root)));
      continue;
    }
    if (stat.nlink > 1) {
...
// Keep this byte-compatible with Carapace's ClawHub update guard so the prompt
// never offers an unforced update that Carapace will refuse as locally modified.
async function digestCarapaceSkillTree(directory) {
  const entries = await collectCarapaceTreeEntries(directory);
...
// Installed skill copies run outside Carapace's package graph, so mirror only
...
async function hasLocalModifications(origin, directory = skillDirectory) {
  return (await digestCarapaceSkillTree(directory)) !== origin.fileTreeSha256;
}
...
function effectiveHomeDirectory() {
  const osHome =
    ...
    homedir();
  const configuredHome = normalizedHomeValue(process.env.CARAPACE_HOME);
...
// state override, config-file directory, then the effective Carapace home.
function configuredGlobalInstallRoot() {
  const stateDirectory = process.env.CARAPACE_STATE_DIR?.trim();
  if (stateDirectory) return resolveConfiguredPath(stateDirectory);
  const configPath = process.env.CARAPACE_CONFIG_PATH?.trim();
  if (configPath) return dirname(resolveConfiguredPath(configPath));
  return join(effectiveHomeDirectory(), ".carapace");
}
...
function print(value) {
  console.log(
    JSON.stringify(
      {
        schema: "carapace.release-validation-skill-update/v1",
        ...value,
      },
      null,
      2,
  ...
  try {
    ...
    const response = await fetch(detailUrl, {
      headers: {
        ...
        "User-Agent": "carapace-release-validation-skill",
      },
