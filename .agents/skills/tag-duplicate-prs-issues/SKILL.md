...
description: Use gitcrawl to search duplicate Carapace PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
`prtags` lives outside the Carapace organization, so this install stays pinned to a reviewed
...
Use `gitcrawl` first because it is the local Carapace history and clustering source.
...
```bash
gitcrawl threads carapace/carapace --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors carapace/carapace --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search carapace/carapace --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search carapace/carapace --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail carapace/carapace --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo carapace/carapace --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo carapace/carapace --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo carapace/carapace --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R carapace/carapace "<problem phrase>" --types group --limit 10
prtags search similar -R carapace/carapace "<problem summary>" --types group --limit 10
prtags group list -R carapace/carapace
```
...
```bash
prtags group create -R carapace/carapace \
  --kind mixed \
...
```bash
prtags field ensure -R carapace/carapace --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R carapace/carapace --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R carapace/carapace --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R carapace/carapace --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R carapace/carapace --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R carapace/carapace --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R carapace/carapace --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R carapace/carapace --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R carapace/carapace --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R carapace/carapace <pr-number> \
...
```bash
prtags annotation issue set -R carapace/carapace <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R carapace/carapace
```
