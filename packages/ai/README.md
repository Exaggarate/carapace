...
Carapace. The package supports isolated runtime instances; importing it does not
...
`@carapace/ai/event-stream`, `@carapace/ai/transports`, and
`@carapace/ai/validation`. No second Carapace runtime package is required.

Provider ids, credentials, model catalogs, retries, and failover remain
application concerns. Carapace supplies those policies around this package.
...
The explicit `@carapace/ai/internal/anthropic`, `google-model-family`, `openai`,
`openai-responses-payload-policy`, `retry-after`, `runtime`, `shared`, and
`tool-schema` subpaths exist for the Carapace application itself.
They carry no semver guarantee and can change or disappear in any release; do
not depend on them outside Carapace.
