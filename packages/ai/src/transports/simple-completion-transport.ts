 ...
 *
 * Registers provider-specific stream functions and rewrites models that need Carapace-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "carapace-provider-simple:";
const PROVIDER_STREAM_API_PREFIX = "carapace-provider-stream:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `carapace-anthropic-vertex-simple:${suffix}`;
}
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use Carapace's transport so resolved request auth is applied.
  ...
  const streamFn = createCarapaceTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
