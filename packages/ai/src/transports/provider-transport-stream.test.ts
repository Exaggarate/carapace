...
import type { Api, Model } from "@carapace/llm-core";
...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Carapace transport aliases when needed.
    const cases = [
      {
        ...
        alias: "carapace-openai-responses-transport",
      },
      {
        ...
        alias: "carapace-openai-chatgpt-responses-transport",
      },
      {
        ...
        alias: "carapace-openai-completions-transport",
      },
      {
        ...
        alias: "carapace-azure-openai-responses-transport",
      },
      {
        ...
        alias: "carapace-anthropic-messages-transport",
      },
      {
        ...
        alias: "carapace-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Carapace transport", () => {
    const cases = [
      buildModel("openai-responses", {
        id: "gpt-5.4",
        provider: "openai",
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createCarapaceTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Carapace simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("carapace-openai-completions-transport");
    expect(preparedModel.provider).toBe("llmman");
    expect(preparedModel.id).toBe("gemma4");
  });

  it("keeps Codex defaults on the Carapace transport until Carapace preserves attribution", () => {
