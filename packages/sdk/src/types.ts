...
import type { GatewayClientRequestOptions } from "@carapace/gateway-client";
import type {
...
/** Minimal transport interface consumed by the Carapace SDK client. */
export type CarapaceTransport = {
  request<T = unknown>(
    method: string,
    params?: unknown,
...
export type ConnectableCarapaceTransport = CarapaceTransport & {
...
export type RuntimeSelection =
  | "auto"
  | { type: "embedded"; id: "carapace" | "codex" | (string & {}) }
...
export type CarapaceEvent<TData = unknown> = {
  version: 1;
  id: string;
  ts: number;
  type: CarapaceEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
