...
// High-level Carapace SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Carapace SDK client. */
export type CarapaceOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
...
function resolveGatewayUrl(options: CarapaceOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Carapace Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Carapace Gateway yet`);
}
...
type RunTerminalSource = { kind: "canonical" } | { kind: "chat"; eventType: CarapaceEvent["type"] };
...
function readChatProjection(event: CarapaceEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: CarapaceEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: CarapaceEvent): boolean {
  return (
...
export class Carapace {
  ...
  private readonly transport: CarapaceTransport;
  private readonly normalizedEvents = new EventHub<CarapaceEvent>();
  private readonly replayByRunId = new Map<string, CarapaceEvent[]>();
  ...
  constructor(options: CarapaceOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: CarapaceEvent) => boolean): AsyncIterable<CarapaceEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: CarapaceEvent) => boolean,
  ): AsyncIterable<CarapaceEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("Carapace SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: CarapaceEvent): CarapaceEvent | undefined => {
    ...
    const matches = (event: CarapaceEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: CarapaceEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): CarapaceEvent[] {
...
export class Agent {
  constructor(
    private readonly client: Carapace,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: Carapace) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<CarapaceEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Carapace,
