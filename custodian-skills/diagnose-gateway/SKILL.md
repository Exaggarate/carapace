...
carapace channels logs --channel <id>
```
...
On managed installs, bounded recent logs: `./scripts/clawlog.sh` (repo checkout) or the log path printed at gateway startup (`/tmp/carapace/carapace-<date>.log` by default).
...
Translate each finding into the next action, naming the responsible skill when one exists: `configure-channel`, `add-model-provider`, or `cloud-image-bake`. Recommend `carapace doctor --fix --non-interactive` only as a separately approved step.
...
Further reference: ../../docs/gateway/troubleshooting.md
