...
carapace config get channels --json        # "Config path not found" is normal before first setup
```
...
```
carapace config schema --json | jq '.properties.channels.properties.telegram'
```
...
```
carapace config set channels.telegram.botToken --ref-provider default --ref-source env --ref-id TELEGRAM_BOT_TOKEN
```

With the bot connected and DM policy `pairing`, ask the operator to DM it. Read `Your Telegram user id` in the pairing reply, or run `carapace logs --follow` and read `senderUserId` in that sender's `telegram pairing request` entry. Stop following once captured; keep unrelated logs private. If the current policy prevents this flow, use an already verified ID or report the discovery blocker; do not broaden access.
...
```
carapace config set channels.telegram.allowFrom '["123456789"]' --strict-json
```

Multi-field changes in one validated write:

```
carapace config patch --stdin <<'JSON'
{ channels: { telegram: { enabled: true, groupPolicy: "allowlist" } } }
JSON
```

In-session alternative: call the `connect_channel` tool action with the channel id — the operator enters the token in a masked prompt, never in chat. Avoid `carapace channels add --token <value>`: it puts the secret in argv and process listings.
...
`doctor --lint` can exit `1` for findings: read the report and continue the remaining checks. Ordinary `doctor` and `doctor --non-interactive` can write config/state; do not use them for diagnosis before approval. Apply `carapace doctor --fix --non-interactive` only after explicit approval, then re-check status.
...
```
carapace message send --channel telegram --target <chatId> --message "Carapace channel test — please ignore" --dry-run
carapace message send --channel telegram --target <chatId> --message "Carapace channel test — please ignore"
```
...
Further reference: ../../docs/channels/telegram.md (and the matching page for other channels)
