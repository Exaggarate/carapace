...
const entryContractMocks = vi.hoisted(() => ({
  registerFull: undefined as ((api: CarapacePluginApi) => void) | undefined,
}));

vi.mock("carapace/plugin-sdk/channel-entry-contract", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("carapace/plugin-sdk/channel-entry-contract")>();
  return {
    ...actual,
