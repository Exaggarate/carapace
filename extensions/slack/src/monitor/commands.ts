...
import type { SlackSlashCommandConfig } from "carapace/plugin-sdk/config-contracts";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "carapace",
  );
  const name = normalizedName || "carapace";
  return {
