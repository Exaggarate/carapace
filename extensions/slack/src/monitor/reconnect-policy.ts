...
export function formatSlackSocketModeSharedConnectionWarning(activeConnections: number): string {
  return [
    ...
    "ensure every Carapace gateway sharing this app has equivalent routing and authorization, or use a separate Slack app per gateway, one relay ingress, or HTTP Request URLs behind a load balancer",
 ...
 * Transient request and HTTP failures stay in Carapace's reconnect loop.
 */
