...
vi.mock("./shared.js", async (importOriginal) => {
  ...
  const { isRecord } = await import("carapace/plugin-sdk/string-coerce-runtime");
  return {
    ...actual,
...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the Carapace User-Agent to guarded Graph attachment fetches", async () => {
    ...
    for (const [call] of guardCalls) {
      ...
      expect((headers as Headers).get("User-Agent")).toMatch(
        /^teams\.ts\[apps\]\/.+ Carapace\/.+$/,
      );
    }
  });

  it("adds the Carapace User-Agent to Graph shares downloads for reference attachments", async () => {
    mockGraphMediaFetch({
      messageId: "msg-share",
      messageResponse: {
        body: {},
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ Carapace\/.+$/);
  });
});
