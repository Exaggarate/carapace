...
function resolveCarapaceVersion(): string {
  try {
 ...
 * and appends the Carapace version.
 *
 * Format: "teams.ts[apps]/<sdk-version> Carapace/<carapace-version>"
 * Example: "teams.ts[apps]/2.0.5 Carapace/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} Carapace/${resolveCarapaceVersion()}`;
  return cachedUserAgent;
}
 ...
 * only contribute the Carapace piece — passing the full `buildUserAgent()`
 * would double-print the SDK token.
 *
 * Format: "Carapace/<carapace-version>"
 */
export function buildCarapaceUserAgentFragment(): string {
  return `Carapace/${resolveCarapaceVersion()}`;
}
