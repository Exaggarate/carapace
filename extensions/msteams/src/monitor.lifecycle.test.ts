...
function createConfig(port: number): CarapaceConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: CarapaceConfig,
  patch: NonNullable<NonNullable<CarapaceConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): CarapaceConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { carapacePollId: "poll-1", choices: "0" } } },
      },
    });
