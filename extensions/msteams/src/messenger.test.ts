...
import { SILENT_REPLY_TOKEN } from "carapace/plugin-sdk/reply-chunking";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("requires SharePoint storage for channel files", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredCarapaceTmpDir(), "msteams-storage-"));
    ...
    it("marks local activity preparation failures as never dispatched", async () => {
      ...
      const missingPath = path.join(resolvePreferredCarapaceTmpDir(), "missing-msteams-file.txt");
    ...
    it("loads uppercase file URLs before sending personal images", async () => {
      const tmpDir = await mkdtemp(
        path.join(resolvePreferredCarapaceTmpDir(), "msteams-file-url-"),
      );
    ...
    it("does not claim no dispatch after an earlier batch message was sent", async () => {
      ...
      const missingPath = path.join(resolvePreferredCarapaceTmpDir(), "missing-second-file.txt");
    ...
    it("retries media preparation but reuses it after provider dispatch starts", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredCarapaceTmpDir(), "msteams-retry-"));
