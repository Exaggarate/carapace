...
import { keepHttpServerTaskAlive, waitUntilAbort } from "carapace/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. Carapace passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires Carapace channel runtime support. Update Carapace and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. Carapace has no
      ...
      // Raft owns wake metadata and its schema evolution. Carapace accepts only
