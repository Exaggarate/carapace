...
import type { CarapacePluginCommandDefinition } from "carapace/plugin-sdk/core";
...
function createApi(run = vi.fn().mockResolvedValue({ runId: "run-1" })): CarapacePluginApi {
  return {
...
describe("handleWorkboardCommand", () => {
  ...
  it("attests the default agent for an unassigned slash-command card", async () => {
    ...
    let command: CarapacePluginCommandDefinition | undefined;
    const api = {
      registerCommand: vi.fn((definition: CarapacePluginCommandDefinition) => {
        command = definition;
      }),
      runtime: {
        subagent: { run },
