...
import { createPluginStateKeyedStoreForTests as createPluginStateKeyedStore } from "carapace/plugin-sdk/plugin-state-test-runtime";
import type {
...
describe("workboard doctor contract", () => {
  it("migrates shipped .28 plugin-state workboard data into sqlite", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-workboard-doctor-"));
