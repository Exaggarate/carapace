...
/** Retired Carapace auth profile replaced by Claude CLI's native login. */
...
/** Non-secret marker telling Carapace that the installed Claude CLI owns auth. */
export const CLAUDE_CLI_NATIVE_AUTH_MARKER = ["carapace", "claude-cli-native-auth"].join(":");
...
// steer Carapace-managed Claude CLI runs toward a different provider,
...
/** Environment variables removed before launching Carapace-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  // Re-injected per run from Carapace's canonical context budget.
  ...
  // Re-injected per run from Carapace's effective thinking level.
  "MAX_THINKING_TOKENS",
