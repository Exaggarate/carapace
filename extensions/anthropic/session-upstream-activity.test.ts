...
const claudeUpstreamWorkspaceRoot = resolvePreferredCarapaceTmpDir();

function createClaudeUpstreamWorkspace(label?: string) {
  return tempWorkspace({
    rootDir: claudeUpstreamWorkspaceRoot,
    prefix: `carapace-claude-upstream-${label ? `${label}-` : ""}`,
  });
}
...
describe("Claude upstream activity", () => {
  ...
  it("filters Carapace-authored rows by normalized transcript text", async () => {
