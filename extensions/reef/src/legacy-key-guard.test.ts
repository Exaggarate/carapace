...
describe("Reef legacy key guard", () => {
  ...
  it("blocks identity generation while a legacy keys file awaits Doctor", async () => {
    const stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-keys-"));
    tempDirs.push(stateRoot);
    const legacyDir = path.join(stateRoot, ".carapace", "data", "reef");
  ...
  it("uses the configured legacy directory when one is present", async () => {
    const stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-keys-"));
    const legacyDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-custom-"));
  ...
  it("blocks when the legacy keys path exists but is not a regular file", async () => {
    const stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-keys-"));
    tempDirs.push(stateRoot);
    fs.mkdirSync(path.join(stateRoot, ".carapace", "data", "reef", "keys.json"), {
      recursive: true,
    });
  ...
  it("ignores default-home keys for an isolated active state", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-home-"));
    const isolatedStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-reef-isolated-"));
    tempDirs.push(homeDir, isolatedStateDir);
    const legacyDir = path.join(homeDir, ".carapace", "data", "reef");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(path.join(legacyDir, "keys.json"), "{}");

    await expect(
      assertLegacyReefKeysMigrated(undefined, { CARAPACE_STATE_DIR: isolatedStateDir }, homeDir),
  ...
  it("honors explicitly configured default-home keys for an isolated active state", async () => {
    ...
    await expect(
      assertLegacyReefKeysMigrated(legacyDir, { CARAPACE_STATE_DIR: isolatedStateDir }, homeDir),
