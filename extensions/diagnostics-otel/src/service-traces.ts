...
export function createDiagnosticsTraceRuntime(tracer: Tracer) {
  ...
  // whose trace context came from Carapace-owned scope (metadata.trustedTraceContext).
  ...
  const addRunAttrs = (
    ...
    if (evt.provider) {
      spanAttrs["carapace.provider"] = evt.provider;
    }
    if (evt.model) {
      spanAttrs["carapace.model"] = evt.model;
    }
    if (evt.channel) {
      spanAttrs["carapace.channel"] = evt.channel;
    }
    if (evt.trigger) {
      spanAttrs["carapace.trigger"] = evt.trigger;
    }
  };
  ...
  const paramsSummaryAttrs = (
    ...
    return {
      "carapace.tool.params.kind": summary.kind,
      ...("length" in summary ? { "carapace.tool.params.length": summary.length } : {}),
    };
  };
  return {
    tracer,
