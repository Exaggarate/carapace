...
export function createDiagnosticsLogExporter(params: {
  ...
  if (logsEnabled) {
    ...
    if (logsToOtlp) {
      ...
      otelLogger = logProvider.getLogger("carapace");
    }
    ...
    const buildDiagnosticLogRecord = (
      ...
      assignOtelLogAttribute(attributes, "carapace.log.level", logLevelName);
      if (evt.loggerName) {
        assignOtelLogAttribute(attributes, "carapace.logger", evt.loggerName);
      }
      if (evt.loggerParents?.length) {
        assignOtelLogAttribute(attributes, "carapace.logger.parents", evt.loggerParents.join("."));
      }
