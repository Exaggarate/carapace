...
export function addTraceAttributes(
  ...
  attributes["carapace.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["carapace.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["carapace.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["carapace.traceFlags"] = normalized.traceFlags;
  }
}
