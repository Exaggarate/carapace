...
const detectResourcesMock = vi.hoisted(() =>
  vi.fn((_options: { detectors?: unknown[] }) => ({
    attributes: { "carapace.test.detected": "1" },
    merge: vi.fn((configured: { attributes?: Record<string, unknown> }) => ({
      attributes: {
        "carapace.test.detected": "1",
...
vi.mock("carapace/plugin-sdk/fetch-runtime", () => ({
...
const ORIGINAL_CARAPACE_OTEL_PRELOADED = process.env.CARAPACE_OTEL_PRELOADED;
...
afterAll(() => {
  ...
  vi.doUnmock("carapace/plugin-sdk/fetch-runtime");
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.CARAPACE_OTEL_PRELOADED;
  ...
  afterEach(async () => {
    ...
    resetDiagnosticEventsForTest();
    if (ORIGINAL_CARAPACE_OTEL_PRELOADED === undefined) {
      delete process.env.CARAPACE_OTEL_PRELOADED;
    } else {
      process.env.CARAPACE_OTEL_PRELOADED = ORIGINAL_CARAPACE_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["carapace.provider"]).toBe("openai");
    for (const key of [
      "carapace.callId",
      "carapace.call_id",
      "carapace.chatId",
      ...
      "carapace.messageId",
      "carapace.message_id",
      "carapace.parentSpanId",
      "carapace.parent_span_id",
      "carapace.runId",
      "carapace.run_id",
      "carapace.sessionId",
      "carapace.session_id",
  ...
  test.each([
    {
      ...
      expectedTokenName: "carapace.tokens",
      expectedDurationName: "carapace.run.duration_ms",
    },
    {
    ...
    {
      metricNamePrefix: "acme.carapace.",
      expectedTokenName: "acme.carapace.tokens",
      expectedDurationName: "acme.carapace.run.duration_ms",
    },
  ])(
    "replaces the default Carapace metric prefix with $metricNamePrefix",
    async ({ metricNamePrefix, expectedTokenName, expectedDurationName }) => {
      ...
      expect(telemetryState.counters.has("carapace.tokens")).toBe(
        expectedTokenName === "carapace.tokens",
      );
    },
  );
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("carapace.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "carapace.channel": "telegram",
      "carapace.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("carapace.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "carapace.channel": "telegram",
      "carapace.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("carapace.message.queued")?.add).toHaveBeenCalledWith(1, {
      "carapace.channel": "telegram",
      "carapace.source": "telegram",
    });
    expect(telemetryState.histograms.get("carapace.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("carapace.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "carapace.channel": "telegram",
      "carapace.source": "telegram",
    });
    expect(telemetryState.histograms.get("carapace.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "carapace.lane": "main",
    });
    expect(telemetryState.counters.get("carapace.message.processed")?.add).toHaveBeenCalledWith(1, {
      "carapace.channel": "telegram",
      "carapace.outcome": "completed",
    });
    expect(telemetryState.counters.get("carapace.message.received")?.add).toHaveBeenCalledWith(1, {
      "carapace.channel": "telegram",
      "carapace.source": "webhook",
    });
    expect(telemetryState.counters.get("carapace.message.received")?.add).toHaveBeenCalledWith(1, {
      "carapace.channel": "unknown",
      "carapace.source": "unknown",
    });
    expect(
      telemetryState.counters.get("carapace.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("carapace.message.dispatch.completed")?.add,
    ...
    expect(
      ...
      expect.objectContaining({
        "carapace.channel": "unknown",
        "carapace.reason": "none",
        "carapace.source": "unknown",
      }),
    );
    expect(
      telemetryState.histograms.get("carapace.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "carapace.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("carapace.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "carapace.channel": "telegram",
      "carapace.outcome": "completed",
    });
    expect(telemetryState.histograms.get("carapace.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "carapace.lane": "main",
      },
    );
    expect(telemetryState.counters.get("carapace.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("carapace.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "carapace.state": "processing",
    });
    expect(
      telemetryState.histograms.get("carapace.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "carapace.state": "processing",
    });
    expect(telemetryState.counters.get("carapace.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "carapace.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("carapace.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "carapace.agent": "agent.default",
        "carapace.channel": "telegram",
        "carapace.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("carapace.webhook.processed");
    expect(spanNames).toContain("carapace.message.processed");
    expect(spanNames).toContain("carapace.session.stuck");
    const webhookSpanOptions = startedSpanOptions("carapace.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("carapace.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("carapace.message.processed");
    expect(messageSpanOptions?.attributes?.["carapace.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["carapace.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("carapace.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("carapace.messageId");
  ...
  test("surfaces bridge cleanup failure after attempting every shutdown phase", async () => {
    ...
    await service.start(ctx);
    emitRunStarted();
    const runSpan = spanByName("carapace.run");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.CARAPACE_OTEL_PRELOADED = "1";
    const { service, ctx } = await startServiceFixture(["traces", "metrics", "logs"]);
    ...
    const runDurationRecordCall = lastHistogramRecord("carapace.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["carapace.provider"]).toBe("openai");
    expect(runDurationAttributes?.["carapace.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("carapace.run");
    expect(runSpanOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("carapace.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "carapace.exporter": "diagnostics-otel",
      "carapace.signal": "logs",
      "carapace.status": "started",
      "carapace.reason": "configured",
    });
  ...
  test("coalesces multi-transport logs into one public lifecycle", async () => {
    ...
    expect(telemetryState.counters.has("carapace.telemetry.exporter.events")).toBe(false);
  ...
  test.each([" TRUE ", "TrUe"])(
    "disables every Carapace-owned telemetry route for OTEL_SDK_DISABLED=%j",
    async (value) => {
      const events: TelemetryExporterEvent[] = [];
      const unsubscribe = onInternalDiagnosticEvent((event) => {
        if (event.type === "telemetry.exporter") {
  ...
  test("preserves preloaded trace and metric ownership while disabling plugin logs", async () => {
    ...
    expect(meterProviderCtor).not.toHaveBeenCalled();
    expect(unhandledRejectionHandlerState.register).not.toHaveBeenCalled();
    expect(lastHistogramRecord("carapace.run.duration_ms")?.[0]).toBe(100);
    expect(startedSpanOptions("carapace.run")?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(logEmit).not.toHaveBeenCalled();
  ...
  test("retires trace ownership across external, unsupported, and supported restarts", async () => {
    ...
    try {
      ...
      process.env.CARAPACE_OTEL_PRELOADED = "1";
      await service.start(ctx);
      process.env.CARAPACE_OTEL_PRELOADED = "0";
      delete ctx.config.diagnostics!.otel!.protocol;
      process.env.OTEL_EXPORTER_OTLP_PROTOCOL = "grpc";
      await service.start(ctx);
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("carapace.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "carapace.security.event_id": "security-event-1",
      "carapace.security.category": "tool",
      "carapace.security.action": "tool.execution.blocked",
      "carapace.security.outcome": "denied",
      "carapace.security.severity": "medium",
      "carapace.security.reason": "tools.deny",
      "carapace.security.actor.kind": "agent",
      "carapace.security.actor.id_hash": "agent-hash-1",
      "carapace.security.actor.role": "operator",
      "carapace.security.actor.scopes": "operator.read,operator.approvals",
      "carapace.security.target.kind": "plugin",
      "carapace.security.target.name": "@acme/security-event-plugin",
      "carapace.security.target.owner": "plugin-installer",
      "carapace.security.policy.id": "tools.exec",
      "carapace.security.policy.decision": "deny",
      "carapace.security.policy.reason": "allowlist.miss",
      "carapace.security.control.id": "exec-approval",
      "carapace.security.control.family": "approval",
      "carapace.security.attribute.params_kind": "object",
      "carapace.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "carapace.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("keeps explicit HTTP exporters canonical when ambient protocol is gRPC", async () => {
    ...
    emitEvent("log.record", {
      message: "Carapace-owned OTLP log",
    });
    await flushDiagnosticEvents();
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("carapace.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "carapace.security.event_id": "security-event-stdout",
        "carapace.security.category": "tool",
        "carapace.security.action": "tool.execution.blocked",
        "carapace.security.outcome": "denied",
        "carapace.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "carapace.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("carapace.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "carapace.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("carapace.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "carapace.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("carapace.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "carapace.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("carapace.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["carapace.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["carapace.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["carapace.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "carapace.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("carapace.payload.large")?.add).toHaveBeenCalledWith(1, {
      "carapace.payload.action": "rejected",
      "carapace.payload.surface": "gateway.frame",
      "carapace.channel": "web",
      "carapace.plugin": "none",
      "carapace.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("carapace.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "carapace.exporter": "diagnostics-otel",
      "carapace.signal": "logs",
      "carapace.status": "failure",
      "carapace.reason": "emit_failed",
      "carapace.errorCategory": "TypeError",
    });
    expect(
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    await startServiceFixture(["metrics"]);
    telemetryState.counters.get("carapace.telemetry.exporter.events")?.add.mockClear();
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("carapace.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["carapace.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["carapace.blocked_by"]).toBe("policy-plugin");
  ...
  test("run.completed error span carries the redacted message off the metric attrs", async () => {
    ...
    expect(startedSpanOptions("carapace.run")?.attributes?.["carapace.error"]).toBe(
      "upstream model stream stalled then aborted",
    );
    expect(spanByName("carapace.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "upstream model stream stalled then aborted",
    });
    // The raw message must never widen metric cardinality.
    const runDuration = lastHistogramRecord("carapace.run.duration_ms");
    expect(runDuration?.[1]?.["carapace.outcome"]).toBe("error");
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "carapace.error")).toBe(false);
  });
  ...
  test("run.completed bounds sensitive error text before export", async () => {
    ...
    const status = mockCallArg(spanByName("carapace.run").setStatus, 0) as {
      message?: string;
    };
  ...
  test("harness.run.completed error span carries the redacted message", async () => {
    ...
    expect(startedSpanOptions("carapace.harness.run")?.attributes?.["carapace.error"]).toBe(
      "model run failed during resolve phase",
    );
    expect(spanByName("carapace.harness.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "model run failed during resolve phase",
    });
    const harnessDuration = lastHistogramRecord("carapace.harness.duration_ms");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "carapace.error")).toBe(false);
  });
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("carapace.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["carapace.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = startedSpanOptions("carapace.model.call")?.attributes;
    expect(attrs?.["carapace.content.input_messages"]).toBe("user prompt");
  });
  ...
  test("lets explicit Carapace sampling override the inherited sampler environment", async () => {
  ...
  test("preserves OTLP TLS env options when passing env proxy agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "carapace-otel-tls-"));
    try {
  ...
  test("pins validated collector TLS material on direct HTTPS exporter agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "carapace-otel-direct-tls-"));
    try {
      const rootCertificatePath = path.join(certDir, "root.pem");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    await startServiceFixture(["logs"], {
      ...
      captureContent: true,
      configure: (ctx) => {
        ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-carapace";
      },
    });
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("carapace.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-carapace");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "carapace.log.level": "WARN",
        "carapace.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["carapace.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["carapace.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "carapace.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "carapace.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "carapace.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/carapace/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/carapace/src/private.ts:42",
      },
    ...
    expect(emitCall.attributes["carapace.good"]).toBe(`${"y".repeat(4095)}...(truncated)`);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(Object.hasOwn(emitCall.attributes, `carapace.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "carapace.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "carapace.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "carapace.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "carapace.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "carapace.code.location")).toBe(false);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "carapace.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("carapace.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "carapace.channel": "webchat",
      "carapace.agent": "ops",
      "carapace.provider": "openai",
      "carapace.model": "gpt-5.4",
      "carapace.token": "input",
    });
  ...
  test("advertises explicit duration buckets on the carapace run/harness/context histograms", async () => {
    const priorSdkBoundaries = [
      0, 5, 10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000, 7500, 10000,
    ];
    await startServiceFixture(["metrics"]);

    const runDurationOptions = histogramCreateOptions("carapace.run.duration_ms");
    ...
    const harnessDurationOptions = histogramCreateOptions("carapace.harness.duration_ms");
    ...
    const contextOptions = histogramCreateOptions("carapace.context.tokens");
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("carapace.tokens")?.add).toHaveBeenCalledWith(2, {
      "carapace.channel": "unknown",
      "carapace.agent": "unknown",
      "carapace.provider": "openai",
      "carapace.model": "gpt-5.4",
      "carapace.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("carapace.tokens")?.add.mock.calls),
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("carapace.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "carapace.lane": expected,
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("carapace.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("carapace.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "carapace.sessionId")).toBe(false);
  ...
  test("separates request and turn GenAI client duration by operation", async () => {
    ...
    const carapaceModelCallDuration = telemetryState.histograms.get(
      "carapace.model_call.duration_ms",
    );
    expect(carapaceModelCallDuration?.record).toHaveBeenCalledTimes(4);
    expect(
      carapaceModelCallDuration?.record.mock.calls.map(
        (call) => call[1]?.["carapace.model_call.observation_unit"],
      ),
    ).toEqual(["request", "request", "turn", "turn"]);
    const spanObservationUnits = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "carapace.model.call")
      .map(
        (call) =>
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "carapace.agent": "main",
      "carapace.skill.activation": "read",
      "carapace.skill.name": "tiny-llm-brainstorm",
      "carapace.skill.source": "workspace",
      "carapace.toolName": "read",
    };
    expect(telemetryState.counters.get("carapace.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "carapace.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("carapace.run");
    expect(spanNames).toContain("carapace.model.call");
    expect(spanNames).toContain("carapace.harness.run");
    expect(spanNames).toContain("carapace.tool.execution");

    const runOptions = startedSpanOptions("carapace.run");
    expect(runOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["carapace.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["carapace.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["carapace.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "carapace.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "carapace.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("carapace.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelOptions?.kind).toBe(2);
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("carapace.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["carapace.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["carapace.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["carapace.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["carapace.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["carapace.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["carapace.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["carapace.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["carapace.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["carapace.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["carapace.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "carapace.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "carapace.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "carapace.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("carapace.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["carapace.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["carapace.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["carapace.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["carapace.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["carapace.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "carapace.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "carapace.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    ...
    const modelCallDuration = lastHistogramRecord("carapace.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["carapace.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["carapace.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("carapace.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["carapace.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["carapace.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("carapace.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["carapace.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["carapace.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("carapace.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["carapace.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["carapace.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("carapace.model.call");
    expect(modelSpanAttributes["carapace.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["carapace.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["carapace.model_call.time_to_first_byte_ms"]).toBe(45);
    expect(modelSpanAttributes["carapace.model_call.prompt.input_messages_count"]).toBe(2);
    expect(modelSpanAttributes["carapace.model_call.prompt.input_messages_chars"]).toBe(3456);
    expect(modelSpanAttributes["carapace.model_call.prompt.system_prompt_chars"]).toBe(789);
    expect(modelSpanAttributes["carapace.model_call.prompt.tool_definitions_count"]).toBe(4);
    expect(modelSpanAttributes["carapace.model_call.prompt.tool_definitions_chars"]).toBe(2345);
    expect(modelSpanAttributes["carapace.model_call.prompt.total_chars"]).toBe(6590);
    expect(modelSpanAttributes["carapace.model_call.usage.input_tokens"]).toBe(100);
    expect(modelSpanAttributes["carapace.model_call.usage.output_tokens"]).toBe(20);
    expect(modelSpanAttributes["carapace.model_call.usage.cache_read_input_tokens"]).toBe(30);
    expect(modelSpanAttributes["carapace.model_call.usage.cache_creation_input_tokens"]).toBe(5);
    expect(modelSpanAttributes["carapace.model_call.usage.reasoning_output_tokens"]).toBe(8);
    expect(modelSpanAttributes["carapace.model_call.usage.prompt_tokens"]).toBe(135);
    expect(modelSpanAttributes["carapace.model_call.usage.total_tokens"]).toBe(155);
    ...
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "carapace.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("carapace.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["carapace.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["carapace.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["carapace.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "carapace.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "carapace.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("carapace.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["carapace.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "carapace.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "carapace.runId")).toBe(false);

    const toolSpan = spanByName("carapace.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("carapace.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  });
  ...
  test("closes a tracked blocked tool span once without creating a duplicate", async () => {
    ...
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "carapace.tool.execution",
    );
    expect(toolSpans).toHaveLength(1);
    expect(startedSpanOptions("carapace.tool.execution")?.startTime).toBe(1_000);
  ...
  test("uses authoritative source timestamps for terminal-only tool spans", async () => {
    ...
    const toolSpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "carapace.tool.execution",
    );
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "carapace.tool.execution",
    );
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("carapace.model.failover");
    expect(failoverOptions?.attributes?.["carapace.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["carapace.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["carapace.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["carapace.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["carapace.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["carapace.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["carapace.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["carapace.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "carapace.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("carapace.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("carapace.model.failover")).toStrictEqual([
      1,
      {
        "carapace.failover.reason": "overloaded",
        "carapace.failover.suspended": "true",
        "carapace.lane": "main",
        "carapace.model": "claude-opus-4-6",
        "carapace.provider": "anthropic",
        "carapace.failover.to_model": "gpt-5.4",
        "carapace.failover.to_provider": "openai",
      },
    ]);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("carapace.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "carapace.toolName": "browser",
        "carapace.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "carapace.tool.owner": "browser-tools",
        "carapace.tool.params.kind": "object",
        "carapace.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("carapace.model.failover");
    expect(failoverOptions?.attributes?.["carapace.lane"]).toBe("session");
  ...
  test("maps model call APIs and preserves zero usage on terminal spans", async () => {
    ...
    for (const attrs of [modelCallAttrs[0], modelCallAttrs[2]]) {
      expect(attrs).toMatchObject({
        ...
        "carapace.model_call.usage.cache_read_input_tokens": 0,
        "carapace.model_call.usage.cache_creation_input_tokens": 0,
        "carapace.model_call.usage.reasoning_output_tokens": 0,
        "carapace.model_call.usage.prompt_tokens": 0,
        "carapace.model_call.usage.total_tokens": 0,
    ...
    for (const key of [
      "carapace.model_call.request_bytes",
      "carapace.model_call.response_bytes",
    ...
    expect(
      Object.keys(modelCallAttrs[1] ?? {}).filter(
        (key) => key.startsWith("carapace.model_call.usage.") || key.startsWith("gen_ai.usage."),
      ),
    ).toEqual([]);
    expect(modelCallAttrs[3]).toMatchObject({
      "carapace.model_call.usage.cache_creation_input_tokens": 7,
      "carapace.model_call.usage.prompt_tokens": 7,
      "gen_ai.usage.input_tokens": 7,
      "gen_ai.usage.cache_creation.input_tokens": 7,
    });
    expect(modelCallAttrs[3]).not.toHaveProperty("carapace.model_call.usage.input_tokens");
  });
  ...
  test("uses latest GenAI request and agent span shapes only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("carapace.model.call")).toBeUndefined();
    ...
    expect(agentTurnOptions?.attributes?.["carapace.model_call.observation_unit"]).toBe("turn");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("carapace.model.call");
    expect(modelCallOptions?.attributes?.["carapace.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "carapace.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "carapace.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("carapace.provider.request", {
      "carapace.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("carapace.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["carapace.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "carapace.upstreamRequestIdHash")).toBe(
      false,
    );
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("carapace.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "carapace.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["carapace.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["carapace.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["carapace.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["carapace.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["carapace.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["carapace.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["carapace.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["carapace.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["carapace.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["carapace.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["carapace.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["carapace.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["carapace.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("carapace.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "carapace.toolName": "process",
      "carapace.loop.level": "critical",
      "carapace.loop.action": "block",
      "carapace.loop.detector": "known_poll_no_progress",
      "carapace.loop.count": 20,
      "carapace.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("carapace.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["carapace.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["carapace.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["carapace.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["carapace.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["carapace.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["carapace.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "carapace.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("carapace.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("carapace.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "carapace.memory.level": "critical",
        "carapace.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("carapace.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "carapace.memory.level": "critical",
      "carapace.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("carapace.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["carapace.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["carapace.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["carapace.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["carapace.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["carapace.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["carapace.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["carapace.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["carapace.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["carapace.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["carapace.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "carapace.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("carapace.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "carapace.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "carapace.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "carapace.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["carapace.run"]).toBeUndefined();
    expect(parentBySpanName["carapace.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["carapace.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("carapace.message.processed");
    const harnessSpan = spanByName("carapace.harness.run");
    const runSpan = spanByName("carapace.run");
    const usageSpan = spanByName("carapace.model.usage");
    const modelCallSpan = spanByName("carapace.model.call");
    ...
    expect(parentBySpanName["carapace.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["carapace.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["carapace.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["carapace.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["carapace.model.call"]?.spanId).toBe(runSpanContext.spanId);
  });
  ...
  test("prepares model and tool spans synchronously for propagation without duplicate spans", async () => {
    ...
    const modelSpanContext = spanByName("carapace.model.call").spanContext();
    const runSpanContext = spanByName("carapace.run").spanContext();
    expect(startedSpanParentContexts("carapace.model.call")).toEqual([runSpanContext]);
    expect(formatDiagnosticTraceparent(modelTrace)).toBe(
      `00-${modelTrace.traceId}-${modelTrace.spanId}-01`,
    );
    ...
    expect(startedSpanParentContexts("carapace.tool.execution")).toEqual([modelSpanContext]);
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "carapace.model.call",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("carapace.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("carapace.message.processed")[0]).toBeUndefined();
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("carapace.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("carapace.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("carapace.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("carapace.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("carapace.message.processed")["carapace.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("carapace.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("carapace.harness.run")?.[2]).toBeUndefined();
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("carapace.run")).toBeTypeOf("number");
  });
  ...
  test.each([
    ...
    const runContexts = startedSpanParentContextsByName("carapace.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("carapace.run").spanContext();
    const modelParentContext = startedSpanParentContexts("carapace.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("carapace.model.call");
  ...
  test("bounds retained run contexts by evicting the oldest completed runs", async () => {
    ...
    const newestRunSpan = telemetryState.spans.findLast((span) => span.name === "carapace.run");
    telemetryState.tracer.startSpan.mockClear();
    ...
    const usageParents = startedSpanParentContexts("carapace.model.usage");
  ...
  test("clears retained run contexts when the service stops", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("carapace.model.usage")?.[2]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(parentBySpanName["carapace.run"]).toBeUndefined();
    expect(parentBySpanName["carapace.model.call"]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(startedSpanParentContexts("carapace.harness.run")[0]).toBeUndefined();
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["carapace.run"]).toBeUndefined();
    expect(parentBySpanName["carapace.model.call"]).toBeUndefined();
    expect(parentBySpanName["carapace.tool.execution"]).toBeUndefined();
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "carapace.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "carapace.model.call",
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "carapace.harness.run",
      ),
    ).toHaveLength(1);
  });
  ...
  test("nests exec spans under the run when the trace context is Carapace-owned", async () => {
    await startServiceFixture(["traces", "metrics"]);
    ...
    const execParents = startedSpanParentContexts("carapace.exec");
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("carapace.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["carapace.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["carapace.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["carapace.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["carapace.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("carapace.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["carapace.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["carapace.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["carapace.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["carapace.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["carapace.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["carapace.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["carapace.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "carapace.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "carapace.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "carapace.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("carapace.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("carapace.exec")).toBeTypeOf("number");
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("carapace.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "carapace.channel": "matrix",
      "carapace.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "carapace.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["carapace.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["carapace.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["carapace.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["carapace.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["carapace.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["carapace.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["carapace.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "carapace.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["carapace.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["carapace.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["carapace.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["carapace.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["carapace.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["carapace.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["carapace.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "carapace.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "carapace.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("carapace.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["carapace.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["carapace.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["carapace.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("carapace.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["carapace.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["carapace.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["carapace.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["carapace.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("carapace.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["carapace.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["carapace.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["carapace.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("carapace.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["carapace.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["carapace.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["carapace.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("carapace.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["carapace.status"]).toBe("released");
    expect(telemetryState.counters.get("carapace.talk.event")?.add).toHaveBeenCalledWith(1, {
      "carapace.talk.brain": "agent-consult",
      "carapace.talk.event_type": "input.audio.delta",
      "carapace.talk.mode": "realtime",
      "carapace.talk.provider": "openai",
      "carapace.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("carapace.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "carapace.talk.brain": "agent-consult",
        "carapace.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("carapace.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "carapace.talk.brain": "agent-consult",
      "carapace.talk.event_type": "latency.metrics",
      "carapace.talk.mode": "realtime",
      "carapace.talk.provider": "openai",
      "carapace.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("carapace.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("carapace.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "carapace.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("carapace.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "carapace.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "carapace.content.tool_output")).toBe(
      false,
    );
  ...
  test("exports bounded redacted content when capture is enabled", async () => {
    ...
    const modelAttrs = startedSpanOptions("carapace.model.call")?.attributes;
    const toolAttrs = startedSpanOptions("carapace.tool.execution")?.attributes;

    expect(modelAttrs?.["carapace.content.output_messages"]).toBe("model reply");
    expect(Object.hasOwn(modelAttrs ?? {}, "carapace.content.system_prompt")).toBe(false);
    expect(String(modelAttrs?.["carapace.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["carapace.content.tool_input"]).toBe("tool input");
    ...
    expect(toolAttrs?.["gen_ai.tool.call.arguments"]).toBe(
      toolAttrs?.["carapace.content.tool_input"],
    );
    expect(typeof toolAttrs?.["carapace.content.tool_output"]).toBe("string");
    expect(String(toolAttrs?.["carapace.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["carapace.content.tool_output"])).not.toContain("a".repeat(11));
    expect(toolAttrs?.["gen_ai.tool.call.result"]).toBe(
      toolAttrs?.["carapace.content.tool_output"],
    );
  });
  ...
  test("omits absent model content fields when capture is enabled", async () => {
    ...
    const attrs = startedSpanOptions("carapace.model.call")?.attributes ?? {};
    expect(attrs["carapace.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "carapace.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "carapace.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "carapace.content.tool_definitions")).toBe(false);
  ...
  test("exports Claude CLI turn content through the existing Phoenix GenAI keys", async () => {
    ...
    emitTrustedModelCallCompletedWithContent(
      {
        ...
        systemPrompt: "Carapace appended instructions",
      },
      {
    ...
    const attrs = startedSpanOptions("carapace.model.call")?.attributes;
    expect(attrs?.["carapace.api"]).toBe("claude-code");
    expect(attrs?.["carapace.transport"]).toBe("stdio-live");
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.input.messages"))).toEqual([
    ...
    const compatibilityOutput = stringAttribute(attrs, "carapace.content.output_messages");
  ...
  test("never exports provider-internal thinking payloads in model message attributes", async () => {
    ...
    for (const key of [
      ...
      "carapace.content.input_messages",
      "carapace.content.output_messages",
    ]) {
    ...
    expect(
      JSON.parse(stringAttribute(attrs, "carapace.content.input_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "carapace.content.input_messages"))[0]?.content[1],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "carapace.content.output_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "carapace.content.input_messages"))[0]?.content[2],
    ).toEqual({ type: "text", text: "visible input" });
    expect(
      JSON.parse(stringAttribute(attrs, "carapace.content.output_messages"))[0]?.content[2],
    ).toEqual({
      type: "toolCall",
      id: "tool-1",
      name: "lookup",
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("carapace.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["carapace.reason"])).toContain("…");
    expect(typeof attrs?.["carapace.reason"]).toBe("string");
    expect(String(attrs?.["carapace.reason"])).not.toContain(
