  ...
  for (const { plugin, marketplace, readPluginName } of params.plugins) {
    if (plugin.enabled !== true) {
      evaluated.push({
        ...
        message: `Codex plugin "${plugin.pluginName ?? plugin.name}" is installed in Codex but disabled; enable it in Codex before migrating it to Carapace.`,
      });
      continue;
    }
...
function appInventoryBlockMessage(
  ...
  return `Codex plugin "${plugin.pluginName ?? plugin.name}" owns${appLabel} but the source app inventory reports it is ${status}; authenticate or enable the app in Codex before migrating it to Carapace.`;
}

export function codexPluginMigrationSubscriptionWarning(): string {
  return "Codex app-backed plugin migration requires the Codex app-server source account to be logged in with a ChatGPT subscription account. Log in to the Codex app with subscription auth; Carapace auth or API-key auth does not satisfy Codex app connector access.";
}
