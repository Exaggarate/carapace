...
import { asBoolean, isRecord } from "carapace/plugin-sdk/string-coerce-runtime";
  ...
  for (const memory of params.memoryFiles) {
    ...
    if (!targetNotRegular) {
      ...
      if (isPathInside(source, destination) || isPathInside(destination, source)) {
        throw new Error(
          "Codex memory source and Carapace import destination must be separate paths.",
        );
      }
    }
      ...
      createMigrationItem({
        ...
        reason: targetNotRegular
          ...
          : targetConflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: "Copy consolidated Codex memory into the Carapace memory index.",
        details: {
      ...
      return createMigrationItem({
        ...
        reason: collision
          ...
          : conflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this Carapace agent workspace.`,
    ...
    if (
      ...
      items.push(
        createMigrationItem({
          ...
          message: `Install Codex plugin "${plugin.pluginName}" in the Carapace-managed Codex app-server runtime.`,
          details: {
            configKey,
            marketplaceName: CODEX_PLUGINS_MARKETPLACE_NAME,
            pluginName: plugin.pluginName,
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with carapace plugins install <path> --force.",
      }),
    );
  }
  return items;
...
function buildPluginConfigItem(
  ...
  return createMigrationItem({
    ...
    applyPhase: "after-promotion",
    message:
      "Enable Carapace's Codex plugin integration and record migrated source-installed curated plugins.",
    details: {
...
export async function buildCodexMigrationPlan(
  ...
  return {
    ...
    nextSteps: memoryOnly
      ? []
      : [
          "Run carapace doctor after applying the migration.",
          "Review skipped or auth-required Codex plugin/config/hook items before exposing them in Carapace sessions.",
        ],
    metadata: {
