...
describe("Codex plugin thread config", () => {
  ...
  it("does not expose apps for plugins that Carapace policy leaves disabled", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
  ...
  it("uses app-level destructive policy for plugins without Carapace tool-name knowledge", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
