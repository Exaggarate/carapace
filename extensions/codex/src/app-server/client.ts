...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to Carapace. */
  ...
  addNotificationHandler(handler: CodexServerNotificationHandler): () => void {
    ...
    // Carapace can reserve the first thread or install its shared turn router.
...
function defaultServerRequestResponse(
  ...
  if (request.method === "mcpServer/elicitation/request") {
    return createCodexElicitationResponse("decline", null, {
      message: "Carapace has no interactive handler for this elicitation.",
    });
  }
  return {};
}
...
function timeoutServerRequestResponse(timeoutMs: number): JsonValue {
  return {
    contentItems: [
      {
        type: "inputText",
        text: `Carapace dynamic tool call timed out after ${timeoutMs}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "Carapace could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Carapace, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods Carapace can answer. */
