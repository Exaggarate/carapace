...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Carapace dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Carapace dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "Carapace dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "Carapace dynamic tool call aborted.";
    const terminalReason = resolveCodexToolAbortTerminalReason(params.signal);
    params.onFallbackSelected?.();
  ...
  try {
    ...
    const message = formatToolExecutionErrorMessage(error, "Carapace dynamic tool call failed.");
    notifyFailedToolResult(message, terminalReason);
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Carapace dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "Carapace dynamic tool call failed.";
}

/** Strips Carapace-only metadata before sending a dynamic tool response to Codex. */
...
type TerminalDynamicToolReleaseState = {
  ...
  pendingCarapaceDynamicToolCompletionIdsCount: number;
};
...
export function shouldReleaseTurnAfterTerminalDynamicTool(
  ...
  return (
    ...
    state.pendingCarapaceDynamicToolCompletionIdsCount === 0
  );
}
...
export function resolveDynamicToolCallTimeoutMs(params: {
  ...
  if (
    ...
    try {
      ...
      // Carapace delegation uses that default for its ten-minute approval plus
      // staging/application; it has no model-authored timeout override.
      const timeoutSeconds = params.call.tool === "carapace" ? undefined : args?.timeoutSeconds;
      return (
