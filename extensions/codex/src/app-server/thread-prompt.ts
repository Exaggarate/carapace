...
import { listRegisteredPluginAgentPromptGuidance } from "carapace/plugin-sdk/plugin-runtime";
import {
...
export function buildDeveloperInstructions(
  ...
  for (const spec of options.dynamicTools ?? []) {
    const isDirectNamespace =
      ...
      spec.name.trim() === CODEX_CARAPACE_DIRECT_DYNAMIC_TOOL_NAMESPACE;
  ...
  const sections = [
    ...
    deferredToolNames.size > 0
      ? `Deferred searchable Carapace dynamic tools available: ${[...deferredToolNames]
    ...
    nativeDelegationAvailable
      ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred.${hasSessionsSpawn ? " Use Carapace `sessions_spawn` only for Carapace or ACP delegation, never as a substitute for `spawn_agent` on internal legwork." : ""}`
      : undefined,
