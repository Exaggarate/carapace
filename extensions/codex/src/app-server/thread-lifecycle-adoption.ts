...
async function preparePendingCodexThreadResume(
  ...
  const fail = (reason: string) =>
    new Error(
      `Cannot configure resumed Codex thread ${binding.threadId}: ${reason}. ` +
        "The thread is preserved; continue it in native Codex or use /new for the current Carapace tools.",
    );
  ...
  if (
    ...
    throw fail("configuration adoption requires an Carapace-owned local Codex home");
  }
  ...
  try {
    ...
    if (
      ...
      throw fail("its immutable native tool catalog does not match the current Carapace tools");
    }
    assertCurrent();
    return {
...
function observeCodexThreadConfiguration(
  ...
  return {
    ...
    assertConfigured: () => {
      ...
      // reservation against native-internal reloads outside Carapace's thread queue.
      if (!unloaded) {
        throw new Error(
