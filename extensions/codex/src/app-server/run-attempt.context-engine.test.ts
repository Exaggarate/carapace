...
import type { EmbeddedRunAttemptParamsV2 as EmbeddedRunAttemptParams } from "carapace/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
  supportsModelTools,
...
import { readStringValue } from "carapace/plugin-sdk/string-coerce-runtime";
...
/** Keeps native Codex bindings reusable while omitting Carapace tools and search. */
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  ...
  it.each(["compaction", "branch"] as const)(
    ...
    async (boundary) => {
      ...
      expectRequestInputTextContains(harness, "Carapace assembled context for this turn:");
  ...
  it("projects thread-bootstrap context only once for a matching context-engine epoch", async () => {
    ...
    expectRequestInputTextContains(firstHarness, "Carapace assembled context for this turn:");
  ...
  it("fails first-turn Codex context overflow instead of falling back to Carapace compaction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const compact = vi.fn<ContextEngine["compact"]>(async () => ({
      ok: true,
  ...
  it("keeps current inbound context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentInboundContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] Carapace: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 Carapace: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("Carapace assembled context for this turn:");
    expect(inputText).toContain("Current user request:\nhello");
    expect(inputText).toContain("[reply target] Carapace: anchor REPLYCTX");
  ...
  it("persists the admitted user prompt before an async item buffered during turn startup", async () => {
    ...
    expect(messages[1]).toMatchObject({ carapaceAsyncDelivery: { itemId: "startup-async" } });
  });
