...
vi.mock("carapace/plugin-sdk/node-selection-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("carapace/plugin-sdk/node-selection-runtime")>();
...
type CarapaceCodingToolsOptionsForTest = NonNullable<
  Parameters<NonNullable<typeof dynamicToolBuildState.carapaceCodingToolsFactory>>[0]
>;

function setCarapaceCodingToolsFactoryForTests(
...
function resetCarapaceCodingToolsFactoryForTests(): void {
...
describe("Codex app-server dynamic tool build", () => {
  it("forwards private yield context and acknowledgment to the lifecycle owner", async () => {
    ...
    let capturedOnYield:
      ...
      | undefined;
    setCarapaceCodingToolsFactoryForTests((options) => {
  ...
  it("hands the question tools this run's own way to show a prompt", async () => {
    ...
    let capturedQuestionPrompt: CarapaceCodingToolsOptionsForTest["questionPrompt"];
    setCarapaceCodingToolsFactoryForTests((options) => {
  ...
  it("binds a resolver-backed constructed tool surface exactly once", async () => {
    ...
    setCarapaceCodingToolsFactoryForTests(factory);
  ...
  it("keeps host and plugin tools while native paired-device execution owns filesystem and shell", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    const factory = vi.fn((options: Parameters<typeof createCarapaceCodingTools>[0]) => [
      ...createCarapaceCodingTools(options).filter((tool) => tool.name === "message"),
  ...
  it("keeps policy-filterable Carapace coding replacements when native tools are disabled", () => {
    const tools = [
      "read",
      "write",
      "edit",
  ...
  it.each<{
    ...
    async ({ source, config, conversationToolPolicy, sandboxAgentId, denied }) => {
      ...
      try {
        ...
        setCarapaceCodingToolsFactoryForTests((options) =>
          createCarapaceCodingTools(options).filter((tool) =>
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-codex-tools-"));
  });
  ...
  afterEach(async () => {
    ...
    resetCarapaceCodingToolsFactoryForTests();
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
  ...
  it("maps sandbox exec-server cwd through the remote workspace mapping", () => {
    expect(
      resolveCodexAppServerExecutionCwd({
        effectiveCwd: "/Users/kevinlin/code/carapace",
        environment: {
          id: "sandbox-1",
          cwd: "/Users/kevinlin/code/carapace/sandbox",
        } as never,
        nativeToolSurfaceEnabled: true,
        localWorkspaceRoot: "/Users/kevinlin/code/carapace",
        remoteWorkspaceRoot: "/home/oai/carapace-workspaces",
      }),
    ).toBe("/home/oai/carapace-workspaces/sandbox");
  });
  ...
  it.each([
    ...
    params.runtimePlan = createCodexRuntimePlanFixture();
    setCarapaceCodingToolsFactoryForTests((options) =>
      createCarapaceCodingTools(options).filter((tool) => ["exec", "process"].includes(tool.name)),
    );
    ...
    const gatewayExec = expectDefined(
      ...
      `${testCase.mode} Carapace shell replacement`,
    );
  ...
  it.each(["searchable", "direct"] as const)(
    ...
    async (loading) => {
      ...
      setCarapaceCodingToolsFactoryForTests((options) =>
        createCarapaceCodingTools(options).filter((tool) =>
          ["carapace", "message", "session_status"].includes(tool.name),
        ),
      );
      ...
      expect(bridge.specs).toContainEqual({
        ...
        description: "",
        tools: [expect.objectContaining({ type: "function", name: "carapace" })],
      });
      expect(bridge.specs).toContainEqual(
  ...
  it.each(["core policy", "Codex excludes", "turn allowlist"])(
    ...
    async (policy) => {
      ...
      params.disableTools = false;
      params.runtimePlan = createCodexRuntimePlanFixture();
      params.config = policy === "core policy" ? { tools: { deny: ["carapace"] } } : {};
      params.toolsAllow = policy === "turn allowlist" ? ["message"] : ["carapace", "message"];
      setCarapaceCodingToolsFactoryForTests((options) =>
        createCarapaceCodingTools(options).filter((tool) =>
          ["carapace", "message"].includes(tool.name),
        ),
      );
      ...
      const tools = await buildDynamicToolsForTest(params, workspaceDir, {
        sandbox: null,
        isHostScopedToolActive: () => false,
        pluginConfig: policy === "Codex excludes" ? { codexDynamicToolsExclude: ["carapace"] } : {},
      });
  ...
  it("preserves the host-provided Carapace tool through the Codex allowlist", async () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    ...
    setCarapaceCodingToolsFactoryForTests(() => [
      { ...createRuntimeDynamicTool("carapace"), catalogMode: "direct-only" },
    ]);

    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      isHostScopedToolActive: (toolName) => toolName === "carapace",
      pluginConfig: { codexDynamicToolsExclude: ["carapace"] },
    });

    expect(tools.map((tool) => tool.name)).toEqual(["carapace"]);
  });

  it.each([
    { label: "host scope is inactive", hostActive: false, toolsAllow: ["carapace"] },
    {
      label: "the public allowlist is not exact",
      hostActive: true,
      toolsAllow: ["carapace", "read"],
    },
  ...
  it("reports hosted search denied when effective tool policy removes web_search", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setCarapaceCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
  ...
  it("quarantines unreadable tool entries before Codex-specific filtering", async () => {
    ...
    setCarapaceCodingToolsFactoryForTests(() => sourceTools);
  ...
  it("quarantines non-object plugin schemas before Codex-specific filtering", async () => {
    ...
    setCarapaceCodingToolsFactoryForTests(() => [brokenTool, messageTool]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("exposes Carapace sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    const execTool = expectDefined(
      createCarapaceCodingTools({ workspaceDir: tempDir }).find((tool) => tool.name === "exec"),
      "assembled exec tool",
    );
    setCarapaceCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("read"),
  ...
  it("exposes Docker sandbox shell tools when Carapace sandboxing disables native Code Mode", async () => {
    setCarapaceCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("keeps a pinned Gateway shell path beside Codex native shell", async () => {
    ...
    expect(gatewayExec?.description).toContain("Carapace-managed Gateway environment access");
  ...
  it("shares discovery across attempt catalogs but refreshes the next attempt", async () => {
    setCarapaceCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("exec")]);
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("exposes pinned node shell tools for node-targeted Codex app-server runs", async () => {
    ...
    setCarapaceCodingToolsFactoryForTests(() => [execTool, createRuntimeDynamicTool("message")]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("does not expose Gateway process sessions as remote-node control in auto host runs", async () => {
    ...
    expect(nodeList.contentItems).toEqual([
      { type: "inputText", text: "Unknown Carapace tool: node_process" },
    ]);
    const nodeExec = tools.find((tool) => tool.name === "node_exec");
  ...
  it("restores the policy-filtered Carapace shell when a finite allowlist disables native Code Mode", async () => {
    const execTool = createRuntimeDynamicTool("exec");
    const processTool = createRuntimeDynamicTool("process");
    const messageTool = createRuntimeDynamicTool("message");
    setCarapaceCodingToolsFactoryForTests(() => [execTool, processTool, messageTool]);
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/carapace-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("quarantines exposed Codex memory writes and edits after a network tool", async () => {
    ...
    await fs.mkdir(path.join(workspaceDir, "memory"), { recursive: true });
    {
      ...
      setCarapaceCodingToolsFactoryForTests((options) => {
        const filesystemTools = createCarapaceCodingTools(options).filter((tool) =>
  ...
  it("preserves before-tool wrapping through Codex runtime normalization", async () => {
    ...
    setCarapaceCodingToolsFactoryForTests(() => [wrappedTool]);
  ...
  it.each(["ultra", "off"] as const)(
    "passes active %s thinking into shared Carapace tool construction",
    async (thinkLevel) => {
      const sessionFile = path.join(tempDir, "session.jsonl");
      const workspaceDir = path.join(tempDir, "workspace");
      const params = createParams(sessionFile, workspaceDir);
  ...
  it("disables Codex native tool surfaces whenever an Carapace sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/carapace-data:/tmp/carapace-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/carapace-data:/tmp/carapace-data:rw",
            "/tmp/carapace-data/secrets:/tmp/carapace-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it.each([
    ...
    async ({ sessionKey, required, expected }) => {
      ...
      const factory = vi.fn((_options: Parameters<typeof createCarapaceCodingTools>[0]) => []);
