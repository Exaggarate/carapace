...
export function createCodexAttemptLifecycleController(
  ...
  const { state, activeTurnItemIds, pendingCarapaceDynamicToolCompletionIds } = turnRuntime;
  const releaseTurnAfterTerminalDynamicTool = (value: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingCarapaceDynamicToolCompletionIdsCount: pendingCarapaceDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const scheduleTerminalDynamicToolReleaseCheck = () => {
    ...
    const immediate = setImmediate(() => {
      ...
      if (
        ...
        pendingCarapaceDynamicToolCompletionIds.size === 0
      ) {
  ...
  const emitFastModeAutoProgress = async (payload: {
    ...
    try {
      await params.onToolResult?.({
        text: summary,
        channelData: { carapaceProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
      });
    } catch (error) {
