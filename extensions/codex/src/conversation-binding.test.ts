...
import type { PluginConversationBinding } from "carapace/plugin-sdk/plugin-entry";
...
vi.mock("carapace/plugin-sdk/conversation-binding-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("carapace/plugin-sdk/conversation-binding-runtime")>();
  return {
    ...actual,
...
vi.mock("carapace/plugin-sdk/agent-scope-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("carapace/plugin-sdk/agent-scope-runtime")>()),
...
vi.mock("carapace/plugin-sdk/provider-auth-aliases", async (importOriginal) => ({
  ...(await importOriginal<typeof import("carapace/plugin-sdk/provider-auth-aliases")>()),
...
describe("codex conversation binding", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-codex-binding-"));
  });
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "Carapace native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("rejects attaching a conversation to another session's owned thread", async () => {
    ...
    ).rejects.toThrow("owned by another Carapace session");
    expect(request).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the current Carapace session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await writeTestConversationBinding(sessionFile, { threadId: "thread-1", cwd: tempDir });
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because Carapace sandboxing is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because Carapace exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("Carapace exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current Carapace session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because Carapace sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("moves session history while clamping rootless inherited cwd to the agent workspace", async () => {
    ...
    expect(requests[0]?.params).toMatchObject({
      ...
      approvalPolicy: "on-request",
      approvalsReviewer: "auto_review",
      developerInstructions: expect.stringContaining("bound to an Carapace conversation"),
  ...
  it("refreshes stale network-proxy bound app-server threads before the turn", async () => {
    ...
    await writeTestConversationBinding(sessionFile, {
      threadId: "thread-old",
      cwd: tempDir,
      networkProxyProfileName: "carapace-network-stale",
