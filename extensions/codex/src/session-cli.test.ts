...
vi.mock("carapace/plugin-sdk/gateway-runtime", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/gateway-runtime")>(
      ...
      sessions: [
        {
          ...
          cwd: "/Users/test/Projects/carapace",
          status: "idle",
          activeFlags: [],
...
describe("registerCodexSessionCli", () => {
  ...
  describe("sessions", () => {
    ...
    it("renders connected sessions, metadata, pagination, and offline host errors", async () => {
      ...
      expect(output).toContain("/Users/test/Projects/carapace");
  ...
  describe("continue", () => {
    it("continues a Gateway-local thread and sanitizes the human-readable session key", async () => {
      ...
      expect(output).toBe("Carapace session: harness:codex:supervision:branch\n");
      expect(output).not.toContain("\u001b");
    });
