...
- Start from `carapace wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `carapace wiki obsidian status` before shelling out, then prefer the dedicated helpers like `carapace wiki obsidian search`, `carapace wiki obsidian open`, `carapace wiki obsidian command`, and `carapace wiki obsidian daily`.
