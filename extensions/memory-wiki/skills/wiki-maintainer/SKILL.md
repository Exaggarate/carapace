...
description: Maintain the Carapace memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `carapace wiki ingest`, `carapace wiki compile`, and `carapace wiki lint` as the default maintenance loop.
- In `bridge` mode, run `carapace wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `carapace wiki unsafe-local import` only when the user explicitly opted into private local path access.
