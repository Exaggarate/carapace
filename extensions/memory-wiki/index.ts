// Memory Wiki plugin entrypoint registers its Carapace integration.
import fs from "node:fs/promises";
import path from "node:path";
import { definePluginEntry, type CarapaceConfig } from "./api.js";
import {
...
async function loadConfiguredVaultIdentity(vaultRoot: string): Promise<{
  ...
  try {
    const stat = await fs.stat(path.join(vaultRoot, ".carapace-wiki", "log.jsonl"));
    if (!stat.isFile()) {
      return null;
    }
  } catch (error) {
...
export default definePluginEntry({
  id: "memory-wiki",
  name: "Memory Wiki",
  description: "Persistent wiki compiler and Obsidian-friendly knowledge vault for Carapace.",
  configSchema: memoryWikiConfigSchema,
  register(api) {
    const config = resolveMemoryWikiConfig(api.pluginConfig);
    const getAppConfig = () =>
      (api.runtime.config?.current?.() ?? api.config) as CarapaceConfig | undefined;
