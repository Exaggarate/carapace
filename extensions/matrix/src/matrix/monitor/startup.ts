...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleCarapaceDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Carapace devices detected for ${params.auth.userId}: ${deviceHealth.staleCarapaceDevices.map((device) => device.deviceId).join(", ")}. Run 'carapace matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
