...
describe("Matrix inbound event dedupe", () => {
  ...
  function createStateEnv(): NodeJS.ProcessEnv {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-matrix-inbound-dedupe-"));
    tempDirs.push(dir);
    return { ...process.env, CARAPACE_STATE_DIR: dir };
  }
  ...
  it("keeps committed events in memory when plugin-state persistence fails", async () => {
    ...
    const blockedDir = createStateEnv().CARAPACE_STATE_DIR as string;
    ...
    const deduper = createMatrixInboundEventDeduper({
      auth,
      env: { ...process.env, CARAPACE_STATE_DIR: path.join(filePath, "nested") },
    });
