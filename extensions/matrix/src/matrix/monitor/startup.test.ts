...
function createDeps(
  ...
  return {
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleCarapaceDevices: [] as MatrixManagedDeviceInfo[],
      currentCarapaceDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleCarapaceDevices: [
        { deviceId: "DEV123", displayName: "Carapace Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Carapace devices detected for @bot:example.org: DEV123. Run 'carapace matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'carapace matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
