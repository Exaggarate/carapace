...
import { chunkTextForOutbound, type CarapaceConfig } from "../runtime-api.js";
...
describe("matrixOutbound cfg threading", () => {
  ...
  it("passes resolved cfg to sendMessageMatrix for media sends", async () => {
    ...
    expect(options.mediaLocalRoots).toEqual(["/tmp/carapace"]);
  ...
  it("renders MessagePresentation into Matrix custom content metadata", async () => {
    ...
    expect(matrixData.extraContent?.["com.carapace.presentation"]).toEqual({
      ...presentation,
      version: 1,
    ...
    await matrixOutbound.sendPayload!({
      ...
      payload: {
        ...
        channelData: {
          matrix: {
            extraContent: {
              "com.carapace.presentation": presentationContent,
            },
          },
        },
      },
