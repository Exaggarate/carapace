...
import { expectExplicitMusicGenerationCapabilities } from "carapace/plugin-sdk/provider-test-contracts";
...
vi.mock("carapace/plugin-sdk/provider-http", async (importOriginal) => {
  const original = await importOriginal<typeof import("carapace/plugin-sdk/provider-http")>();
  return {
    ...original,
