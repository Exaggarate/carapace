...
// canonical key; `carapace doctor --fix` repairs shipped `nativeBaseUrl` and
...
const FIX_HINT = `Run "carapace doctor --fix" (api.deepinfra.com endpoints migrate automatically; custom hosts must set ${PROVIDER_PATH}.baseUrl to an OpenAI-compatible videos endpoint manually).`;
