...
- Extension production code should import from `carapace/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `carapace.plugin.json` and the package
  `carapace` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `carapace/plugin-sdk/*`.
