...
import { isBlockedHostnameOrIp } from "carapace/plugin-sdk/ssrf-runtime";
import {
  filterStringRecord,
...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support Carapace provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by Carapace SSRF policy";
...
/**
 * Maps Carapace's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; Carapace only supplies
