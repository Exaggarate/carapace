...
import { readNonEmptyStringPreservingWhitespace as readNonEmptyString } from "carapace/plugin-sdk/string-coerce-runtime";
 ...
 *     use cases.
 *   - Falls back to CARAPACE_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<carapaceHome>/.carapace/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `CARAPACE_HOME` for the home directory root.
 ...
 *   4. CARAPACE_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with Carapace's own files
  ...
  const carapaceHome = readNonEmptyString(args.env.CARAPACE_HOME);
  const rootHome = carapaceHome ? resolve(carapaceHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <carapaceHome>/.carapace/agents/<agentId>/copilot
  return resolve(join(rootHome, ".carapace", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // CARAPACE_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "CARAPACE_GITHUB_TOKEN", value: readNonEmptyString(env.CARAPACE_GITHUB_TOKEN) },
