...
import type { ChannelSetupInput } from "carapace/plugin-sdk/channel-setup";
...
async function prepare(input: ClickClackSetupInput, cfg: CarapaceConfig = {}) {
...
describe("ClickClack setup adapter", () => {
  ...
  it("claims a full setup URL and prepares the token, workspace, and defaults", async () => {
    claimClickClackSetupCode.mockResolvedValue({
      token: "test-token",
      bot: { id: "usr_bot", handle: "carapace", display_name: "Carapace" },
      workspace: {
        id: "wsp_1",
        route_id: "clickclack",
        slug: "default",
  ...
  it("writes normalized default and named account config", () => {
    ...
    expect(
      clickClackSetupContract.applyAccountConfig({
        cfg: { channels: { clickclack: { name: "Legacy" } } } as CarapaceConfig,
        accountId: "Work Team",
        input: {
          name: "Work",
