...
function createGatewayEventsHarness() {
  const handlers = new Set<(event: CarapacePluginSessionsChangedEvent) => void>();
  ...
  } satisfies CarapacePluginGatewayEvents;
  return {
    gatewayEvents,
    unsubscribe,
    emit(event: CarapacePluginSessionsChangedEvent) {
