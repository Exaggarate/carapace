...
describe("ClickClack discussion service", () => {
  ...
  it("opens a managed channel once and returns stable info URLs", async () => {
    ...
    expect(opened).toEqual({
      state: "open",
      embedUrl:
        "https://clickclack.example/embed/channel/team-route/discussion-route?carapaceHostTheme=1",
  ...
  it("adopts a remotely created channel by external reference after an interrupted open", async () => {
    ...
    expect(opened).toEqual({
      state: "open",
      embedUrl:
        "https://clickclack.example/embed/channel/team-route/recovered-route?carapaceHostTheme=1",
