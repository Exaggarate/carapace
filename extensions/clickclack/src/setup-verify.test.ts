...
vi.mock("carapace/plugin-sdk/gateway-runtime", () => ({
...
describe("ClickClack post-write setup verification", () => {
  beforeEach(() => {
    ...
    mocks.me.mockReset().mockResolvedValue({ id: "usr_bot", handle: "carapace" });
  ...
  it("prints the resolved bot and workspace without blocking setup", async () => {
    ...
    expect(runtime.log).toHaveBeenNthCalledWith(
      1,
      "Connected as @carapace — workspace clickclack resolved.",
    );
    expect(runtime.log).toHaveBeenNthCalledWith(
      2,
      "Carapace is running — ClickClack will connect automatically.",
    );
  });
  ...
  it.each([
    {
      ...
      expected: "Carapace is running — ClickClack will connect automatically.",
    },
    {
      name: "not running",
      arrange: () =>
      ...
      expected: "Start Carapace to connect: carapace gateway",
    },
    {
      name: "unavailable",
      arrange: () => mocks.callGatewayFromCli.mockRejectedValue(new Error("probe failed")),
      expected:
        "If Carapace is running it connects automatically; otherwise start it with: carapace gateway",
    },
