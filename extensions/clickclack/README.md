# ClickClack Carapace channel

Official Carapace channel plugin for ClickClack.

## Install

```sh
carapace plugins install @carapace/clickclack
```
...
`/api/bot-setup-codes/claim#CODE` endpoint. Carapace validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
carapace channels add clickclack \
...
Run `carapace onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local Carapace
...
extension publishes Carapace's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each Carapace session:
...
Carapace Control UI, preserving base paths. Main sessions use `/chat/<agent>`.
...
two Carapace gateways using the same ClickClack workspace do not adopt each
...
See `docs/channels/clickclack.md` in the Carapace repository, or the published docs at `../../docs/channels/clickclack.md`.
