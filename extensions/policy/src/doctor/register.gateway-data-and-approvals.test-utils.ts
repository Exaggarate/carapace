...
function runRegisteredPolicyDoctor(configPath: string, cfg: CarapaceConfig) {
...
describe("registerPolicyDoctorChecks", () => {
    ...
    expect(evidence.dataHandling).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          source: "oc://carapace.invariant/logging/redaction",
          value: true,
        }),
        expect.objectContaining({
          kind: "telemetryContentCapture",
          source: "oc://carapace.config/diagnostics/otel/captureContent",
          value: true,
        }),
        expect.objectContaining({
          kind: "sessionRetentionMode",
          source: "oc://carapace.config/session/maintenance/mode",
          value: "warn",
        }),
        expect.objectContaining({
          kind: "memorySessionTranscriptIndexing",
          source: "oc://carapace.config/memory/search/rememberAcrossConversations",
          value: true,
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/diagnostics/otel/captureContent",
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/session/maintenance/mode",
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/memory/search/rememberAcrossConversations",
  ...
  it("uses CARAPACE_HOME for the default exec approvals artifact path", async () => {
    const carapaceHome = join(workspaceDir, "home");
    const approvalsDir = join(carapaceHome, ".carapace");
    ...
    process.env.CARAPACE_HOME = carapaceHome;
    try {
      const result = await runRegisteredPolicyDoctor(configPath, cfgWithPolicy());
  ...
  it("uses CARAPACE_STATE_DIR for the exec approvals artifact path", async () => {
