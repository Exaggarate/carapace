...
async function runPolicyScenario(cfg: CarapaceConfig, policy: object, mode: PolicyScenarioMode) {
...
function configWithPolicy(overrides: object): CarapaceConfig {
  return { ...cfgWithPolicy(), ...overrides } as unknown as CarapaceConfig;
}

function configWithAgents(agents: object): CarapaceConfig {
  return configWithPolicy({ agents });
}

function configWithSandbox(sandbox: object): CarapaceConfig {
  ...
  it("uses Feishu open-group mention defaults", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/ingress-group-mention-required",
          ocPath: "oc://carapace.config/channels/feishu/requireMention",
        }),
      ]),
    );
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: `oc://carapace.config/channels/${channel}/dmPolicy`,
        }),
      ]),
    );
  });
  ...
  it("evaluates implicit default account posture with named accounts", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/ingress-dm-policy-unapproved",
          ocPath: "oc://carapace.config/channels/discord/dmPolicy",
        }),
      ]),
    );
  });
    ...
    expect(evidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          groupId: "ops",
          kind: "channelRequireMention",
          source: "oc://carapace.config/channels/telegram/groups/ops/requireMention",
          value: false,
        }),
        expect.objectContaining({
          accountId: "personal",
    ...
    expect(evidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          groupId: "ops",
          kind: "channelRequireMention",
          source: "oc://carapace.config/channels/slack/groups/ops/requireMention",
          value: false,
        }),
        expect.objectContaining({
          accountId: "personal",
    ...
    expect(evidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "channelDmPolicy",
          source: "oc://carapace.config/channels/slack/dmPolicy",
          value: "allowlist",
        }),
      ]),
    );
  ...
  it("reports private-network SSRF settings denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      ...
      expect.objectContaining({
        checkId: "policy/network-private-access-enabled",
        severity: "error",
        ocPath: "oc://carapace.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
        requirement: "oc://policy.jsonc/network/privateNetwork/allow",
      }),
    ]);
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          severity: "error",
          ocPath: "oc://carapace.config/models/providers/anthropic/apiKey",
          requirement: "oc://policy.jsonc/secrets/requireManagedProviders",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          severity: "error",
          ocPath: "oc://carapace.config/secrets/providers/command",
    ...
    expect(evidence.secrets).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/models/providers/openai/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/models/providers/openai/request/tls/passphrase",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: 'oc://carapace.config/models/providers/"z.ai"/headers/Authorization',
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          ...
          refProvider: "rogue",
          source:
            "oc://carapace.config/plugins/entries/acpx/config/mcpServers/github/env/GITHUB_TOKEN",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/tools/media/models/#0/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/tools/media/models/#0/request/tls/key",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/tools/media/audio/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://carapace.config/tools/media/models/#1/request/auth/token",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/models/providers/openai/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://carapace.config/models/providers/openai/request/auth/token",
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/models/providers/openai/request/tls/passphrase",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://carapace.config/models/providers/openai/request/tls/passphrase",
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: 'oc://carapace.config/models/providers/"z.ai"/headers/Authorization',
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath:
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/tools/media/models/#0/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://carapace.config/tools/media/audio/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/tools/media/models/#1/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/tools/media/models/#0/request/tls/key",
        }),
      ]),
    );
  });
  ...
  it("reports SecretRefs that use a managed provider alias with the wrong source", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/secrets-unmanaged-provider",
        severity: "error",
        ocPath: "oc://carapace.config/models/providers/openai/apiKey",
        requirement: "oc://policy.jsonc/secrets/requireManagedProviders",
      }),
    ]);
  });
  ...
  it("checks configured channel encryptKey SecretRefs", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://carapace.config/channels/feishu/encryptKey",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://carapace.config/channels/feishu/encryptKey",
  ...
  it("reports agent workspace posture denied by policy", async () => {
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/agents-tool-not-denied",
          ocPath: "oc://carapace.config/agents/list/#0/tools/deny",
        }),
      ]),
    );
  });
    ...
    expect(evidence.agentWorkspace).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          denied: true,
          source: "oc://carapace.config/tools/sandbox/tools/deny",
        }),
        expect.objectContaining({
          id: "locked-tool-apply_patch",
          denied: true,
          source: "oc://carapace.config/agents/list/#0/tools/sandbox/tools/deny",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/agents/defaults/sandbox/mode",
          requirement: "oc://policy.jsonc/agents/workspace/allowedAccess",
        }),
        expect.objectContaining({
          checkId: "policy/agents-tool-not-denied",
  ...
  it("reports global and agent-scoped workspace claims independently", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/agents-workspace-access-denied",
          ocPath: "oc://carapace.config/agents/list/#0/sandbox/workspaceAccess",
          requirement: "oc://policy.jsonc/agents/workspace/allowedAccess",
        }),
        expect.objectContaining({
          checkId: "policy/agents-workspace-access-denied",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ocPath: "oc://carapace.config/agents/list/#1/sandbox/workspaceAccess",
        }),
      ]),
    );
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/agents/list/#0/tools/exec/host",
  ...
  it("applies main agent-scoped claims to implicit default agent posture", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          checkId: "policy/tools-exec-host-unapproved",
          ocPath: "oc://carapace.config/tools/exec/host",
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/agents/defaults/sandbox/docker/binds/#0",
          requirement: "oc://policy.jsonc/sandbox/containers/denyContainerRuntimeSocketMounts",
        }),
        expect.objectContaining({
          checkId: "policy/sandbox-container-runtime-socket-mount",
          ocPath: "oc://carapace.config/agents/defaults/sandbox/docker/binds/#2",
          requirement: "oc://policy.jsonc/sandbox/containers/denyContainerRuntimeSocketMounts",
        }),
        expect.objectContaining({
          checkId: "policy/sandbox-container-runtime-socket-mount",
          ocPath: "oc://carapace.config/agents/defaults/sandbox/docker/binds/#3",
          requirement: "oc://policy.jsonc/sandbox/containers/denyContainerRuntimeSocketMounts",
        }),
      ]),
    );
    ...
    expect(evidence.sandboxPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "containerMount",
          bindSurface: "browser",
          source: "oc://carapace.config/agents/defaults/sandbox/browser/binds/#0",
        }),
        expect.objectContaining({
          kind: "containerNetwork",
          value: "host",
          source: "oc://carapace.config/agents/defaults/sandbox/browser/network",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://carapace.config/agents/defaults/sandbox/browser/network",
        }),
        expect.objectContaining({
          checkId: "policy/sandbox-container-mount-mode-required",
          ocPath: "oc://carapace.config/agents/defaults/sandbox/browser/binds/#0",
        }),
        expect.objectContaining({
          checkId: "policy/sandbox-container-runtime-socket-mount",
          ocPath: "oc://carapace.config/agents/defaults/sandbox/browser/binds/#0",
