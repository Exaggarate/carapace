import type { HealthCheckContext, HealthFinding } from "carapace/plugin-sdk/health";
...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://carapace.config/plugins/entries/policy/config/expectedHash",
  ...
  if (requiredMetadata.size > 0) {
    ...
    if (legacyToolsFile !== null) {
      unmigratedToolsFinding = {
        ...
        fixHint:
          "Run `carapace doctor --fix` to migrate TOOLS.md into the AGENTS.md `## Tools` section.",
      };
    }
  ...
  return [
    {
      ...
      requirement: "oc://carapace.config/plugins/entries/policy/config/expectedAttestationHash",
