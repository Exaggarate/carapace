...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://carapace.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://carapace.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://carapace.config/channels/${ocPathSegment(channel)}`;
