...
export function scanPolicyGatewayExposure(
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://carapace.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://carapace.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://carapace.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://carapace.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "controlUi",
    false,
    "oc://carapace.invariant/gateway/controlUi/deviceIdentity",
  );
  pushGatewayBooleanEvidence(
    entries,
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://carapace.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://carapace.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://carapace.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://carapace.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://carapace.config/gateway/http/endpoints/${endpoint}`;
...
function pushGatewayNodeCommandEvidence(
  ...
  if (Array.isArray(denyCommands)) {
    denyCommands.forEach((command, index) => {
      ...
      entries.push({
        ...
        source: `oc://carapace.config/gateway/nodes/commands/deny/#${index}`,
        value: normalized,
        command: normalized,
      });
    });
  ...
  allowCommands.forEach((command, index) => {
    ...
    entries.push({
      ...
      kind: "nodeCommand",
      source: `oc://carapace.config/gateway/nodes/commands/allow/#${index}`,
      value: normalized,
      command: normalized,
    });
  });
