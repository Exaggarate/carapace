...
async function installStatefulOpenCode(initialSessions: StatefulOpenCodeSession[]) {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-opencode-activity-"));
...
describe("OpenCode session upstream activity", () => {
  ...
  it.runIf(process.platform !== "win32")(
    "keeps real text mixed with ignored text and suppresses Carapace self-echo",
    async () => {
      const session: StatefulOpenCodeSession = {
        id: "ses_a",
        title: "Session A",
