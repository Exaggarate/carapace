...
function resolveKimiBaseUrl(kimi?: KimiConfig, carapaceConfig?: CarapaceConfig): string {
  ...
  const moonshotBaseUrl = carapaceConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: CarapaceConfig; searchConfig?: SearchConfigRecord },
