...
export function buildComfyConfig(config: Record<string, unknown>): CarapaceConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): CarapaceConfig {
  return {
    models: {
      providers: {
        comfy: config,
