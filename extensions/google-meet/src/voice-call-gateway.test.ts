...
type GatewayRuntime = typeof import("carapace/plugin-sdk/gateway-runtime");
...
describe("Google Meet voice-call gateway", () => {
  ...
  afterAll(() => {
    vi.doUnmock("carapace/plugin-sdk/gateway-runtime");
    vi.resetModules();
  });

  it("stops the real gateway client's referenced reconnect after a localhost connection fails", async () => {
    const actual = await vi.importActual<GatewayRuntime>("carapace/plugin-sdk/gateway-runtime");
