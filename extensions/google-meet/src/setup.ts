...
import { asRecord, normalizeOptionalString } from "carapace/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Carapace browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Carapace browser profile; configure browser.defaultProfile to choose another profile",
  });
