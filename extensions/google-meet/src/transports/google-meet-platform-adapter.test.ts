...
async function runAudioStatus(
  ...
  const outputLabel = label.includes("Carapace") ? "Carapace Meeting Audio" : label;
...
describe("GOOGLE_MEET_PLATFORM_ADAPTER audio routing", () => {
  it.each(["BlackHole 2ch", "Monitor of Carapace Meeting Audio"])(
    ...
    async (label) => {
      ...
      expect(health).toMatchObject({
        ...
        audioOutputDeviceLabel: label.includes("Carapace") ? "Carapace Meeting Audio" : label,
        micMuted: false,
      });
  ...
  it("parses input-route health and retries until both routes are ready", () => {
    const pending = GOOGLE_MEET_PLATFORM_ADAPTER.browser.parseStatus({
      result: JSON.stringify({
        ...
        micMuted: false,
        audioInputRouted: true,
        audioInputDeviceLabel: "Carapace Meeting Audio",
  ...
  it("keeps Meet muted and reports manual action when input selection cannot be verified", async () => {
    ...
    expect(health).toMatchObject({
      ...
      audioInputRouteError: "Meet did not confirm Carapace Meeting Audio as its microphone.",
      audioOutputRouted: true,
      micMuted: true,
