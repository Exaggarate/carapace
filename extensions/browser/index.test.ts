...
type BrowserAutoEnableProbe = Parameters<CarapacePluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "carapace.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/carapace", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/carapace", logger: { warn: vi.fn() } });
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "carapace" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "carapace", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
