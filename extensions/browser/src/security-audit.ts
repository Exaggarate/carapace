 ...
 */
import type { CarapacePluginSecurityAuditContext } from "carapace/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "carapace/plugin-sdk/ssrf-policy";
import {
  hasNonEmptyString,
...
export function collectBrowserSecurityAuditFindings(ctx: CarapacePluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("carapace security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.CARAPACE_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.CARAPACE_GATEWAY_PASSWORD) ||
