/**
 * Carapace-managed Chrome profile decoration.
 *
...
import { loadJsonFile, saveJsonFile } from "carapace/plugin-sdk/json-store";
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".carapace-profile-decorated");
}
...
export function usesCarapaceMockKeychain(userDataDir: string): boolean {
  const localState = safeReadJson(path.join(userDataDir, "Local State"));
  return readDefaultProfileInfo(localState)?.carapace_mock_keychain === true;
}

/** Disable Chromium network prediction in an Carapace-managed Chrome profile. */
 ...
 */
export function decorateCarapaceProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string; mockKeychain?: boolean },
) {
  const desiredName = opts?.name ?? DEFAULT_CARAPACE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CARAPACE_BROWSER_COLOR).toUpperCase();
  ...
  if (opts?.mockKeychain) {
    ...
    setDeep(localState, ["profile", "info_cache", "Default", "carapace_mock_keychain"], true);
  }
