...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CARAPACE_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "carapace",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/carapace" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    carapace: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/carapace.json" } });
...
vi.mock("./chrome.js", () => ({
  ...
  launchCarapaceChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveCarapaceUserDataDir: vi.fn(() => chromeUserDataDir.dir),
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "carapace";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.CARAPACE_GATEWAY_PORT;
  process.env.CARAPACE_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CARAPACE_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CARAPACE_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CARAPACE_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CARAPACE_GATEWAY_PASSWORD;
  } else {
    process.env.CARAPACE_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
