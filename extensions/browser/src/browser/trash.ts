...
import { CONFIG_DIR } from "carapace/plugin-sdk/text-utility-runtime";
...
export async function movePathToTrash(targetPath: string): Promise<string> {
  const { movePathToTrash: movePathToTrashWithAllowedRoots } =
    await import("carapace/plugin-sdk/browser-config");
  return await movePathToTrashWithAllowedRoots(targetPath, {
    // Managed browser data follows CARAPACE_STATE_DIR/CARAPACE_CONFIG_PATH, which
