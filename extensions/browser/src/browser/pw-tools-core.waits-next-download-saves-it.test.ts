...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCarapaceTmpDir: vi.fn(() => "/tmp/carapace"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-carapace-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-carapace-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCarapaceTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCarapaceTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCarapaceTmpDir.mockReturnValue("/tmp/carapace");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCarapaceTmpDir.mockReturnValue("/tmp/carapace-preferred");
    ...
    const expectedRootedDownloadsDir = await fs.realpath(
      path.resolve(path.join(path.sep, "tmp", "carapace-preferred", "downloads")),
    );
    const expectedDownloadsTail = `${path.join("tmp", "carapace-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCarapaceTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "carapace-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredCarapaceTmpDir.mockReturnValue(tempDir);
