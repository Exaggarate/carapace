/**
 * Carapace-managed Chrome lifecycle and CDP helpers.
 *
...
import {
  decorateCarapaceProfile,
  ensureProfileCleanExit,
  ensureProfileNetworkPredictionDisabled,
  isProfileDecorated,
  usesCarapaceMockKeychain,
...
async function ensureManagedChromePortAvailable(
  ...
  // Chromium tries IPv4 loopback first, while Carapace polls the configured endpoint.
...
function chromeLaunchHints(params: {
  ...
  if (missingDisplay && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set CARAPACE_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  ...
  if (singletonInUse) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.carapace/browser/${params.profile.name}/user-data.`,
    );
  }
...
/** Resolve the user-data-dir path for a managed Carapace Chrome profile. */
export function resolveCarapaceUserDataDir(profileName = DEFAULT_CARAPACE_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed Carapace browser. */
function buildCarapaceChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
  ...
  if (platform === "darwin" && params.useMockKeychain) {
    // This is an isolated Carapace-owned profile, not the user's Chrome profile.
...
/** Launch or attach to the managed Carapace Chrome profile. */
export async function launchCarapaceChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const useMockKeychain =
    ...
    (usesCarapaceMockKeychain(userDataDir) || (profileIsNew && headlessMode.headless));

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CARAPACE_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = async (onStderr?: (chunk: Buffer | string) => void) => {
    signal?.throwIfAborted();
    const args = buildCarapaceChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
      ...headlessOptions,
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredCarapaceTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateCarapaceProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
        mockKeychain: useMockKeychain,
      });
      log.info(`🦞 carapace browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`carapace browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`carapace browser network-prediction prefs failed: ${String(err)}`);
  }

  try {
    ensureProfileCleanExit(userDataDir);
  } catch (err) {
    log.warn(`carapace browser clean-exit prefs failed: ${String(err)}`);
  }
  signal?.throwIfAborted();
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 carapace browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopOwnedCarapaceChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  timeoutMs = CHROME_STOP_TIMEOUT_MS,
): Promise<boolean> {
  if (!profile.cdpIsLoopback || profile.attachOnly || profile.driver !== "carapace") {
    return false;
  }
...
export async function stopCarapaceChrome(
