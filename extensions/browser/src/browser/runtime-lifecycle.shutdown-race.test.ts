...
const mocks = vi.hoisted(() => ({
  ...
  stopCarapaceChrome: vi.fn(async (_running: RunningChrome) => {}),
}));

vi.mock("./chrome.js", () => ({
  stopCarapaceChrome: mocks.stopCarapaceChrome,
}));
...
beforeEach(() => {
  ...
  mocks.stopCarapaceChrome.mockReset().mockResolvedValue(undefined);
});
...
describe("browser runtime shutdown profile races", () => {
  it("invalidates every deferred start before draining late children and clearing state", async () => {
    ...
    expect(mocks.stopCarapaceChrome.mock.calls.map(([running]) => running)).toEqual(children);
