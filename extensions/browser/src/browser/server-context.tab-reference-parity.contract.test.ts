...
function setupProfile() {
  ...
  return { fetchMock, carapace: ctx.forProfile("carapace") };
}
...
async function setupProfileWithLabels() {
  ...
  await ctx.carapace.labelTab("ABCDEF123456", "docs");
  await ctx.carapace.labelTab("ABC999", "app");
  return ctx;
}
...
describe("tab reference parity contract", () => {
  it("listTabs attaches tabId, label, and suggestedTargetId for every fixture tab", async () => {
    const { carapace } = await setupProfileWithLabels();
    const tabs = await carapace.listTabs();
    expect(tabs).toEqual(
  ...
  it("rejects an ambiguous raw prefix consistently across label/ensure/focus/close", async () => {
    const labelCtx = await setupProfileWithLabels();
    await expect(labelCtx.carapace.labelTab("ABC", "parity")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const ensureCtx = await setupProfileWithLabels();
    await expect(ensureCtx.carapace.ensureTabAvailable("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const focusCtx = await setupProfileWithLabels();
    await expect(focusCtx.carapace.focusTab("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const closeCtx = await setupProfileWithLabels();
    await expect(closeCtx.carapace.closeTab("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
  });
});
