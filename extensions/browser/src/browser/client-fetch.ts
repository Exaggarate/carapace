 ...
 */
import { parseBrowserHttpUrl } from "carapace/plugin-sdk/browser-cdp";
import {
  extractErrorCode,
  formatErrorMessage,
  toErrorObject,
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-carapace-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-carapace-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-carapace-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to Carapace; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the Carapace gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the Carapace gateway (Carapace.app menubar, or \`${formatCliCommand("carapace gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the Carapace browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
  ...
  if (kind === "transient-network") {
    return new Error(
      `Can't reach the Carapace browser control service. ${operatorHint} (${msg}) ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
      err instanceof Error ? { cause: err } : undefined,
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Carapace browser control service. ${operatorHint} (${msg})`,
