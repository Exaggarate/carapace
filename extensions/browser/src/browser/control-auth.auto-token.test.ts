...
const mocks = vi.hoisted(() => ({
  ...
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: CarapaceConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<CarapaceConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: CarapaceConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): CarapaceConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: CarapaceConfig) {
...
describe("ensureBrowserControlAuth", () => {
  ...
  it.each(
    ...
    const cfg: CarapaceConfig = { gateway: { auth: { mode } } };
    let latest = cfg;
