...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        carapace: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CARAPACE_GATEWAY_PORT;
    process.env.CARAPACE_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CARAPACE_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CARAPACE_GATEWAY_PASSWORD;
