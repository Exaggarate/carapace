  ...
  const callTool = vi.fn(async ({ name, arguments: args }: ToolCall) => {
    if (name === "list_pages") {
      const pageLines = [
        ...
        "2: https://github.com/Exaggarate/carapace/pull/45318",
      ];
      if (createdPageOpen) {
...
describe("chrome MCP page parsing", () => {
  ...
  it("parses list_pages text responses when structuredContent is missing", async () => {
    ...
    expect(tabs).toEqual([
      ...
      },
      {
        ...
        title: "",
        url: "https://github.com/Exaggarate/carapace/pull/45318",
        type: "page",
      },
    ]);
  });
  ...
  it("redacts remote CDP URL secrets from attach failures", async () => {
    ...
    const carapaceState = await createCarapaceTestState({
      layout: "state-only",
      prefix: "carapace-chrome-mcp-test-",
    });
    await carapaceState.writeConfig({ logging: { redactSensitive: "off" } });
    const tempDir = carapaceState.root;
  ...
  it("redacts home-relative user data dirs from attach failures", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-chrome-mcp-test-"));
