...
describe("system profile import state", () => {
  ...
  it("reuses only the recorded target and otherwise avoids collisions", () => {
    expect(resolveSuggestedImportTarget({ profileNames: ["carapace"] })).toBe("imported");
    expect(resolveSuggestedImportTarget({ profileNames: ["carapace", "imported"] })).toBe(
      "imported-2",
    );
    expect(
      resolveSuggestedImportTarget({
        profileNames: ["carapace", "imported", "imported-2"],
        state: {
          version: 1,
          status: "imported",
          browser: "chrome",
