...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const opened = await carapace.openTab("about:blank");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    await carapace.focusTab("T2");
    await carapace.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(carapace.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(carapace.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(carapace.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
