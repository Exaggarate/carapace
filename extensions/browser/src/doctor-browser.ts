...
import { DEFAULT_CARAPACE_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectChromeMcpProfiles(cfg: CarapaceConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: CarapaceConfig): ManagedProfile[] {
  const browser = asNullableRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asNullableRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "carapace";
    if (driver !== "existing-session") {
...
function isLegacyClawdProfileConfigured(cfg: CarapaceConfig, legacyProfileDir: string): boolean {
  const browser = asNullableRecord(cfg.browser);
  if (!browser) {
    return false;
  }
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_CARAPACE_BROWSER_PROFILE_NAME ||
...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- The canonical Carapace-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("carapace doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (fs.existsSync(extensionCopyPath)) {
    noteFn(
      [
        ...
        `- Run ${formatCliCommand("carapace browser extension status --json")} to inspect it explicitly; this may request browser profile access.`,
        `- Run ${formatCliCommand("carapace browser extension install")} if setup or repair is needed.`,
      ].join("\n"),
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Carapace-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for Carapace-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Carapace-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Carapace connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Carapace does not bundle Chrome.`,
...
export async function maybeRepairOwnedChromeExtensionNativeHosts(): Promise<{
  ...
  return {
    ...
    changes: [],
    warnings: [
      `Chrome extension native-host repair skipped: Doctor does not inspect personal browser profiles. Run ${formatCliCommand("carapace browser extension install")} to repair explicitly.`,
    ],
  };
}
