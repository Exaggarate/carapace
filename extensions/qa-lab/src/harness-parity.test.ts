...
const LEFT: HarnessVariant = { id: "left", label: "Left", runtime: "carapace" };
const RIGHT: HarnessVariant = { id: "right", label: "Right", runtime: "carapace" };
...
function classify(
  ...
  return buildHarnessParityResult({
    ...
    left: buildHarnessParityCell({
      variant: LEFT,
      cell: makeCell("carapace", left),
      tokenUsageSource: "live-usage",
    }),
    right: buildHarnessParityCell({
      variant: RIGHT,
      cell: makeCell("carapace", right),
      tokenUsageSource: "live-usage",
    }),
...
describe("harness parity", () => {
  ...
  it("labels mock token estimates separately from live usage", () => {
    const sourceCell = makeCell("carapace", {
