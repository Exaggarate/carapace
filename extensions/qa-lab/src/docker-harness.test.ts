...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "carapace.json"),
    ...
    expect(compose).toContain("image: carapace:qa-local-prebaked");
    ...
    expect(services["qa-mock-openai"]?.environment).not.toHaveProperty("CARAPACE_CONFIG_PATH");
    expect(services["qa-mock-openai"]?.volumes).toBeUndefined();
    expect(services["qa-lab"]?.environment).toMatchObject({
      CARAPACE_ENABLE_PRIVATE_QA_CLI: "1",
      CARAPACE_CONFIG_PATH: "/opt/carapace-scaffold/carapace.json",
      CARAPACE_STATE_DIR: "/tmp/carapace/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/carapace-scaffold:ro");
    expect(services["qa-lab"]?.volumes).toContain(
      `${path.relative(outputDir, "/repo/carapace/taxonomy.yaml").split(path.sep).join("/")}:/app/taxonomy.yaml:ro`,
    );
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/carapace-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://carapace-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/carapace-qa-lab-ui");
    expect(compose).toContain(":/opt/carapace-repo:ro");
    expect(compose).toContain("./state:/opt/carapace-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/carapace-scaffold/seed-workspace/. /tmp/carapace/workspace/ && rm -rf /tmp/carapace/workspace/repo && ln -s /opt/carapace-repo /tmp/carapace/workspace/repo",
    );
    expect(compose).toContain("CARAPACE_CONFIG_PATH: /tmp/carapace/carapace.json");
    expect(compose).toContain("CARAPACE_STATE_DIR: /tmp/carapace/state");
    expect(compose).toContain('CARAPACE_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("CARAPACE_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "carapace.json"), "utf8");
    ...
    expect(configText).toContain('"carapaceToolsMcpBridge": true');
    expect(configText).toContain("/app/dist/control-ui");
    expect(configText).toContain("C-3PO QA");
    expect(configText).toContain('"/tmp/carapace/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("carapace:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t carapace:qa-local-prebaked --build-arg CARAPACE_EXTENSIONS=acpx qa-channel qa-lab -f Dockerfile . @/repo/carapace",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    const compose = await readFile(path.join(outputDir, "docker-compose.qa.yml"), "utf8");
    const services = parseComposeServices(compose);
    expect(compose).toContain('CARAPACE_EXTENSIONS: "acpx qa-channel qa-lab"');
    expect(services["qa-mock-openai"]?.build?.context).toBe("../repo #hash");
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/carapace-qa-lab-ui:ro",
    );
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/taxonomy.yaml:/app/taxonomy.yaml:ro",
    );
    expect(services["carapace-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/carapace-repo:ro",
    );
  });
});
