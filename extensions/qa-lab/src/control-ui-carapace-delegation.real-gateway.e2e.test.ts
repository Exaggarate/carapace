...
const suite = createControlUiE2eSuite({
  name: "Control UI Carapace delegation with a real Gateway",
  startServerBeforeBrowser: true,
});

const captureUiProof = process.env.CARAPACE_CAPTURE_UI_PROOF === "1";
const prompt = "tool search qa check target=carapace carapace_fixture=logging-level-info";
  ...
  it("renders one native approval card without exposing fallback approval text", async () => {
    ...
    try {
      ...
      await gateway.call("sessions.create", {
        key: sessionKey,
        label: "Carapace native approval",
      });
        ...
        async ({ page }) => {
          ...
          page.on("websocket", (socket) => {
            ...
            socket.on("framereceived", ({ payload }) => {
              ...
              if (typeof frame.event === "string" && frame.event.startsWith("carapace.approval.")) {
          ...
          await page.addInitScript(
            ({ gatewayUrl, token }) => {
              const proofWindow = window as Window & {
                __CARAPACE_APPROVAL_UI_SEQUENCE__?: string[];
                __CARAPACE_APPROVAL_FALLBACK_OBSERVED__?: boolean;
                __CARAPACE_NATIVE_CONTROL_AUTH__?: { gatewayUrl: string; token: string };
              };
              proofWindow["__CARAPACE_NATIVE_CONTROL_AUTH__"] = { gatewayUrl, token };
              proofWindow["__CARAPACE_APPROVAL_UI_SEQUENCE__"] = [];
              proofWindow["__CARAPACE_APPROVAL_FALLBACK_OBSERVED__"] = false;
              const fallbackMarkers = [
                "Carapace change pending approval",
                "/approve",
                "needsApproval",
                "proposalId",
              ];
              ...
              new MutationObserver((mutations) => {
                for (const record of mutations) {
                  ...
                  if (changedText.some(containsFallback)) {
                    proofWindow["__CARAPACE_APPROVAL_FALLBACK_OBSERVED__"] = true;
                  }
                }
                ...
                constructor(url: string | URL, protocols?: string | string[]) {
                  ...
                  this.addEventListener("message", (event) => {
                    try {
                      ...
                      if (frame.event === "carapace.approval.resolved") {
                        const applicationStatus =
                          typeof frame.payload?.applicationStatus === "string"
                            ? `:${frame.payload.applicationStatus}`
                            : "";
                        proofWindow["__CARAPACE_APPROVAL_UI_SEQUENCE__"]?.push(
                          `carapace.approval.resolved${applicationStatus}`,
                        );
                      }
                    } catch {
          ...
          expect(eventSequence).toEqual(["carapace.approval.requested"]);
          ...
          expect(pendingPageText).not.toContain("Carapace change pending approval");
          ...
          const approvalFallbackWasObserved = () =>
            page.evaluate(
              () =>
                ...
                )["__CARAPACE_APPROVAL_FALLBACK_OBSERVED__"] === true,
            );
          ...
          expect(reloadedPendingPageText).not.toContain("Carapace change pending approval");
          ...
          await approvalCard.evaluate((card) => {
            ...
            const observer = new MutationObserver(() => {
              ...
              observer.disconnect();
              proofWindow["__CARAPACE_APPROVAL_UI_SEQUENCE__"].push("approval.card.removed");
            });
          ...
          expect(eventSequence).toEqual([
            ...
            "carapace.approval.resolved:applied",
            "chat.final",
          ]);
          const approvalUiSequence = await page.evaluate(
            () =>
              (window as unknown as { __CARAPACE_APPROVAL_UI_SEQUENCE__: string[] })[
                "__CARAPACE_APPROVAL_UI_SEQUENCE__"
              ],
          );
          ...
          expect(approvalUiSequence.at(-1)).toBe("carapace.approval.resolved:applied");
          ...
          expect(reloadedPageText).not.toContain("Carapace change pending approval");
    ...
    if (errors.length > 0) {
      throw new AggregateError(errors, "native Carapace approval proof failed");
    }
  }, 180_000);
            ...
            page.on("websocket", (socket) => {
              ...
              socket.on("framereceived", ({ payload }) => {
                ...
                if (
                  ...
                  frame.event.startsWith("carapace.approval.")
                ) {
