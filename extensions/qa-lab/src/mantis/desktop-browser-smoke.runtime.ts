...
const DEFAULT_BROWSER_URL = "https://github.com/Exaggarate/carapace";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "CARAPACE_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "CARAPACE_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/carapace-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.CARAPACE_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/carapace-mantis-desktop-${startedAt
    .toISOString()
