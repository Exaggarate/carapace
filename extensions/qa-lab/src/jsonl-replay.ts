...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { carapace: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { carapace: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      carapace: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.carapace.push(parity.cells.carapace);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# Carapace JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
