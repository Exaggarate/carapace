...
const QA_LAB_UI_OVERLAY_DIR = "/opt/carapace-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        CARAPACE_EXTENSIONS: "${QA_DOCKER_PLUGIN_SELECTION}"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/carapace-scaffold:ro
...
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - CARAPACE_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/carapace-scaffold/carapace.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://carapace-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      CARAPACE_CONFIG_PATH: /tmp/carapace/carapace.json
      CARAPACE_STATE_DIR: /tmp/carapace/state
    ...
    volumes:
      - ./state:/opt/carapace-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/carapace-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/carapace/workspace /tmp/carapace/state && cp /opt/carapace-scaffold/carapace.json /tmp/carapace/carapace.json && cp -R /opt/carapace-scaffold/seed-workspace/. /tmp/carapace/workspace/ && rm -rf /tmp/carapace/workspace/repo && ln -s /opt/carapace-repo /tmp/carapace/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t carapace:qa-local-prebaked --build-arg CARAPACE_EXTENSIONS="${QA_DOCKER_PLUGIN_SELECTION}" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "carapace:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "carapace.json"),
  ];
