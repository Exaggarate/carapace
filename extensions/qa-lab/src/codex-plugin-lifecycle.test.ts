...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults Carapace pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "carapace" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=carapace"],
    },
    {
      name: "mixed profile with main-agent Carapace pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "carapace" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=carapace"],
    },
  ])(
    "keeps codex auth and strips stale Carapace runtime pins for $name",
