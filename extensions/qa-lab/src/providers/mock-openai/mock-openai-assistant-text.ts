...
export function buildAssistantText(input: ResponsesInputItem[], body: Record<string, unknown>) {
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Carapace repo from source + docs before acting.`;
  }
