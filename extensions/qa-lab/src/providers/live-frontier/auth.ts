...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "CARAPACE_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "CARAPACE_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "CARAPACE_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["CARAPACE_LIVE_ANTHROPIC_KEY"],
  gemini: ["CARAPACE_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "CARAPACE_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: CarapaceConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: CarapaceConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: CarapaceConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.CARAPACE_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "carapace") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or CARAPACE_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Carapace OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
