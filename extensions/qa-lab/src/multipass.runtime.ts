...
import { coerceErrorMessage, toStringifiedError } from "carapace/plugin-sdk/error-runtime";
...
import { appendRegularFile } from "carapace/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/carapace-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/carapace";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/carapace-codex-home";
...
type QaMultipassPlan = {
  ...
  channelDriverSelection?: CarapaceCrablineChannelDriverSelection;
...
function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `carapace-qa-${createVmSuffix()}`;
...
function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "CARAPACE_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `CARAPACE_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `CARAPACE_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# Carapace QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm carapace qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredCarapaceTmpDir(), `${plan.vmName}-qa-suite-`),
  );
