...
async function createBuiltInQaTransport(
  ...
  if (context.driver === "crabline") {
    const { resolveCarapaceCrablineChannelDriverSelection } = await import("@openclaw/crabline");
    const selection = resolveCarapaceCrablineChannelDriverSelection({ channel: context.channelId });
