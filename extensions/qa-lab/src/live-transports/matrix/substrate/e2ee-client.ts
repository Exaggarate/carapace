...
import type {
  ...
  MessageEventContent,
} from "@carapace/matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@carapace/matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.CARAPACE_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("carapace/plugin-sdk/qa-runner-runtime");
