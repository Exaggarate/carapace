...
import { startMatrixQaCarapaceCli } from "./scenario-runtime-cli.js";
...
async function createCliRoot(): Promise<{
  ...
  const root = await mkdtemp(
    path.join(resolvePreferredCarapaceTmpDir(), "matrix-qa-cli-stream-error-"),
  );
...
describe("Matrix QA CLI runtime stream errors", () => {
  ...
  it.each([
    ...
    async (streamName, afterExit) => {
      ...
      let session: ReturnType<typeof startMatrixQaCarapaceCli> | undefined;
      let childClosed = false;
      try {
        session = startMatrixQaCarapaceCli({
