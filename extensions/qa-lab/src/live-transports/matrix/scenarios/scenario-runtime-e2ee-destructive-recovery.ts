...
import {
  createMatrixQaCarapaceCliRuntime,
...
export type MatrixQaCliRuntime = Awaited<ReturnType<typeof createMatrixQaCarapaceCliRuntime>>;
...
export function requireMatrixQaGatewayConfigPath(context: MatrixQaScenarioContext) {
  const configPath = requireMatrixQaCliRuntimeEnv(context).CARAPACE_CONFIG_PATH?.trim();
  if (!configPath) {
...
export async function createMatrixQaRecoveryCliRuntime(params: {
  ...
  return await createMatrixQaCarapaceCliRuntime({
