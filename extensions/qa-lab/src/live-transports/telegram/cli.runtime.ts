...
import type { LiveTransportQaCommandOptions } from "carapace/plugin-sdk/qa-runtime";
...
const TELEGRAM_QA_SUT_CARAPACE_COMMAND_ENV = "CARAPACE_QA_TELEGRAM_SUT_CARAPACE_COMMAND";
const TELEGRAM_QA_SUT_UID_ENV = "CARAPACE_QA_TELEGRAM_SUT_UID";
const TELEGRAM_QA_SUT_GID_ENV = "CARAPACE_QA_TELEGRAM_SUT_GID";
const TELEGRAM_QA_SUT_BOUNDARY_DIR_ENV = "CARAPACE_QA_TELEGRAM_SUT_PROCESS_BOUNDARY_DIR";
const TELEGRAM_QA_SUT_CLEANUP_TIMEOUT_ENV = "CARAPACE_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS";
const TELEGRAM_QA_SUT_RUNTIME_EXECUTABLE_ENV = "CARAPACE_QA_TELEGRAM_SUT_RUNTIME_EXECUTABLE";
const TELEGRAM_QA_SUT_PRELOAD_PATH_ENV = "CARAPACE_QA_TELEGRAM_SUT_PRELOAD_PATH";
const TELEGRAM_QA_SUT_FORWARDED_ENV_KEYS_ENV = "CARAPACE_QA_TELEGRAM_SUT_FORWARDED_ENV_KEYS";
...
async function resolveTelegramQaSutCarapaceCommand(
  repoRoot: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<QaGatewayChildCommand | undefined> {
  const configuredCommand = env[TELEGRAM_QA_SUT_CARAPACE_COMMAND_ENV];
  if (configuredCommand === undefined) {
    return undefined;
  }
  const command = await resolveRegularPath({
    env,
    key: TELEGRAM_QA_SUT_CARAPACE_COMMAND_ENV,
    executable: true,
  });
...
export async function runQaTelegramSuite(opts: TelegramQaSuiteOptions) {
  ...
  const result = await runQaFlowSuiteFromRuntime({
    ...
    scenarioIds,
    sutCarapaceCommand: opts.sutCarapaceCommand,
...
export async function runQaTelegramCommand(opts: LiveTransportQaCommandOptions) {
  ...
  const sutCarapaceCommand = await resolveTelegramQaSutCarapaceCommand(runOptions.repoRoot);
