...
describe("qa tool coverage report", () => {
      ...
      scenarios: [
        makeScenario("tool-read", "read", {
          toolCoverage: {
            ...
            capabilityLayer: "carapace-dynamic-direct",
            required: true,
            tracking: "#80236",
          },
        }),
  ...
  it("keeps searchable Carapace dynamic tool rows report-only by default", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-searchable-web-search", "web-search", {
          ...
          toolCoverage: {
            bucket: "carapace-dynamic-integration",
            expectedLayer: "carapace-dynamic",
            capabilityLayer: "carapace-dynamic-searchable",
          },
        }),
      ],
      summary: {
  ...
  it("passes required Carapace dynamic tool coverage when both runtimes exercise the tool", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
  ...
  it("passes required Carapace dynamic tool coverage when Codex reports a soft tool error", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",
  ...
  it("fails required Carapace dynamic tool coverage when a runtime skips the tool", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",
  ...
  it("projects skipped runtime execution as non-passing coverage", () => {
    ...
    expect(report.pass).toBe(false);
    expect(report.rows[0]).toMatchObject({ carapace: "pass", codex: "skip" });
    expect(report.failures).toEqual(["web-fetch status carapace=pass codex=skip"]);
  });
  ...
  it("does not let skipped execution hide a runtime cell failure", () => {
    ...
    expect(report.rows[0]).toMatchObject({ carapace: "pass", codex: "fail" });
    expect(report.failures).toEqual(["web-fetch status carapace=pass codex=fail"]);
  });
  ...
  it("fails required Carapace dynamic tool coverage when the fixture failure mode is preserved", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",
