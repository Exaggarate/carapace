...
describe("tool search gateway e2e fetch helper", () => {
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readToolSearchGatewayFetchLimits({
        CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: 1e3");
    expect(() =>
      readToolSearchGatewayFetchLimits({
        CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: 1000ms");
    expect(
      readToolSearchGatewayFetchLimits({
        CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "4096",
        CARAPACE_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
      bodyMaxBytes: 4096,
      timeoutMs: 5_000,
...
describe("tool search gateway e2e session log scanner", () => {
  it("counts JSONL mentions without treating prompt text as a call", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-tool-search-log-"));
    try {
  ...
  it("counts target mentions from SQLite transcript rows", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-tool-search-sqlite-"));
    const sqlitePath = path.join(stateDir, "agents", "qa", "agent", "carapace-agent.sqlite");
...
describe("tool search gateway e2e lane result", () => {
  it("preserves surrogate pairs in provider request snippets", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-tool-search-lane-"));
    ...
    const env: QaSuiteRuntimeEnv = {
      ...
      gateway: {
        ...
        runtimeEnv: { CARAPACE_GATEWAY_TOKEN: "test-token" },
        tempRoot,
