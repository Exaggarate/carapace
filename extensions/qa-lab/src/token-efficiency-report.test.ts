...
describe("token efficiency report", () => {
  it("does not fail live reports solely because Codex uses fewer tokens", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("carapace", { inputTokens: 120, outputTokens: 80, totalTokens: 200 }),
    ...
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          ...
          makeCell("carapace", { inputTokens: 72_000, outputTokens: 381, totalTokens: 72_381 }, [
  ...
  it("detects a real cache regression even when cached-inclusive totals hide it", () => {
    const carapace = makeCell("carapace", {
      inputTokens: 7_403,
      outputTokens: 220,
      totalTokens: 435_810,
      cacheRead: 415_492,
    ...
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([makeRuntimeParity("first-hour-cache-miss", carapace, codex)]),
    });
    ...
    expect(report.rows[0]).toMatchObject({
      classification: "regression",
      flagged: true,
      carapace: { processedTokens: 20_318, cacheReadTokens: 415_492 },
      codex: {
  ...
  it("reports unavailable cache-miss telemetry as unknown rather than zero", () => {
    ...
    for (const cell of [carapace, codex]) {
      cell.cacheDiagnostics = buildRuntimeParityCacheDiagnostics([cell.usage]);
    }

    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([makeRuntimeParity("unknown-cache-telemetry", carapace, codex)]),
    });

    expect(report.pass).toBe(true);
    expect(report.rows[0]).toMatchObject({
      carapace: { cacheReadTokens: null, cacheWriteTokens: null, cacheMisses: null },
      codex: { cacheReadTokens: null, cacheWriteTokens: null, cacheMisses: null },
    });
    expect(report.aggregate.carapace).toMatchObject({
      cacheMissCount: null,
  ...
  it("fails live proof when processed tokens cannot be derived from partial cache telemetry", () => {
    ...
    expect(report.rows[0]).toMatchObject({
      classification: "neutral",
      flagged: false,
      carapace: { processedTokenEvidence: "derived" },
  ...
  it("does not derive cache writes from partially observed post-warm cache reads", () => {
    ...
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity("incomplete-cache-read-telemetry", carapace, codex),
      ]),
    });
  ...
  it("does not certify incomplete cache-write telemetry with unaccounted cache input", () => {
    ...
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity("incomplete-cache-write-telemetry", carapace, codex),
      ]),
    });
  ...
  it("keeps mixed post-warm telemetry unknown without discarding measured misses", () => {
    ...
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([makeRuntimeParity("mixed-cache-telemetry", carapace, codex)]),
    });
  ...
  it("preserves unmeasured warm turns when only partial cache telemetry is available", () => {
    ...
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([makeRuntimeParity("partial-warm-telemetry", carapace, codex)]),
    });
  ...
  it("keeps live zero-usage rows failing instead of passing as neutral", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          "missing-live-usage",
          makeCell("carapace", { inputTokens: 0, outputTokens: 0, totalTokens: 0 }),
    ...
    expect(report.pass).toBe(false);
    expect(report.failures).toEqual([
      "missing-live-usage carapace live usage totalTokens=0",
  ...
  it("excludes explicit non-assistant scenarios from live usage aggregates", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          "usage-applicable",
          makeCell("carapace", { inputTokens: 80, outputTokens: 20, totalTokens: 100 }),
    ...
    expect(report.aggregate.carapace.totalTokens).toBe(100);
  ...
  it("fails live reports with non-integer token usage evidence", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          ...
          makeCell("carapace", { inputTokens: 100.5, outputTokens: 0, totalTokens: 100.5 }),
    ...
    expect(report.pass).toBe(false);
    expect(report.failures).toEqual([
      "fractional-live-usage carapace live usage inputTokens must be a non-negative integer",
      "fractional-live-usage carapace live usage totalTokens must be a non-negative integer",
    ]);
  });
        ...
        scenarios: [
          {
            ...
            runtimeParity: makeRuntimeParity(
              "mock-regression",
              makeCell("carapace", { inputTokens: 100, outputTokens: 0, totalTokens: 100 }),
  ...
  it("renders savings and regression classifications in the markdown report", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("carapace", { inputTokens: 100, outputTokens: 100, totalTokens: 200 }),
