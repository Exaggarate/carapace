...
vi.mock("carapace/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/logging-core")>(
...
vi.mock("carapace/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/channel-pairing")>(
    "carapace/plugin-sdk/channel-pairing",
  );
  return {
    ...actual,
    readChannelAllowFromStore(...args: unknown[]) {
...
vi.mock("carapace/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/media-store")>(
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("carapace/plugin-sdk/plugin-state-test-runtime")>(
      "carapace/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/carapace-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `carapace-inbound-media-${crypto.randomUUID()}`);
...
let resetLogger: typeof import("carapace/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("carapace/plugin-sdk/runtime-env").setLoggerOverride;

const LOG_PATH = path.join(os.tmpdir(), `carapace-inbound-media-${crypto.randomUUID()}.log`);
...
describe("web inbound media saves with extension", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("carapace/plugin-sdk/runtime-env"));
