...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("carapace:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-session-"));
