...
import { DEFAULT_ACCOUNT_ID, type CarapaceConfig } from "carapace/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: CarapaceConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  ...
  resolveWhatsAppAuthDir: vi.fn<
    (params: { cfg: CarapaceConfig; accountId: string }) => { authDir: string }
  >(() => ({
    authDir: "/tmp/carapace-whatsapp-test",
  })),
}));
...
vi.mock("carapace/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/setup")>(
    "carapace/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: CarapaceConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/carapace-whatsapp-test" });
  });
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as CarapaceConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as CarapaceConfig,
      }),
    );
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        cfg: createWhatsAppRootAllowFromConfig() as CarapaceConfig,
      }),
    );
