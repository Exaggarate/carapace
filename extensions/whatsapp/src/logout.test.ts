...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "carapace-test-web-logout-"));
    previousOAuthDir = process.env.CARAPACE_OAUTH_DIR;
    process.env.CARAPACE_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete custom auth directories outside the Carapace auth root", async () => {
  ...
  it("does not delete through symlinked auth dirs inside the Carapace auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Carapace auth root", async () => {
