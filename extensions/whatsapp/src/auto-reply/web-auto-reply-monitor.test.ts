...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-group-gating-"));
...
const makeConfig = (overrides: Record<string, unknown>) =>
  ({
    ...
    ...overrides,
  }) as unknown as import("carapace/plugin-sdk/config-contracts").CarapaceConfig;
...
function makeInboundCfg(responsePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/carapace" } },
  ...
  it("processes a pattern-matching message that also @-mentions another member (#109488)", async () => {
    ...
    const { result } = await runGroupGating({
      ...
      msg: createGroupMessage({
        ...
        body: "@carapace please check this",
        mentionedJids: ["15550000000@s.whatsapp.net"],
        selfE164: "+15551234567",
        selfJid: "15551234567@s.whatsapp.net",
      }),
