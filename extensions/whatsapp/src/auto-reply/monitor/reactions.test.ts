...
type AckReactionConfig = NonNullable<
  NonNullable<NonNullable<CarapaceConfig["channels"]>["whatsapp"]>["ackReaction"]
>;
...
function createConfig(
  ...
  extras?: Partial<NonNullable<CarapaceConfig["channels"]>["whatsapp"]>,
): CarapaceConfig {
  return {
...
function createAckEmojiConfig(ackReaction?: AckReactionConfig): CarapaceConfig {
  const cfg = {
    agents: {
...
function resolveAckEmoji(cfg: CarapaceConfig, agentId = "agent") {
...
const expectAckReactionSent = (accountId: string, cfg: CarapaceConfig = createConfig("ack")) => {
