...
import { sanitizeTerminalText } from "carapace/plugin-sdk/test-fixtures";
...
const testState = vi.hoisted(() => ({
  authDir: `${(process.env.TMPDIR ?? "/tmp").replace(/\/+$/, "")}/carapace-wa-creds-${process.pid}-${Math.random().toString(16).slice(2)}`,
}));
