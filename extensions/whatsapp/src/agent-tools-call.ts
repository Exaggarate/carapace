...
import { createActionGate, stringEnum } from "carapace/plugin-sdk/channel-actions";
import type {
  AnyAgentTool,
  CarapacePluginApi,
  CarapacePluginToolContext,
} from "carapace/plugin-sdk/core";
import { mulawToPcm } from "carapace/plugin-sdk/realtime-voice";
import { detectBinary } from "carapace/plugin-sdk/setup-tools";
import { resolveOAuthDir } from "carapace/plugin-sdk/state-paths";
...
async function resolveRequesterE164(params: {
  accountId: string;
  cfg: NonNullable<CarapacePluginToolContext["config"]>;
...
function resolveRuntimeConfig(api: CarapacePluginApi, context: CarapacePluginToolContext) {
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      if (params.action === "status") {
        return jsonResult({
          ...
          note: "MeowCaller uses a separate WhatsApp linked-device session; it cannot reuse Carapace's Baileys credentials.",
        });
      }
      ...
      if (linkedSelf === target) {
        throw new Error(
          "WhatsApp cannot call the linked account itself; use a dedicated Carapace WhatsApp number",
        );
      }
      ...
      try {
        ...
        const tempDir = await fs.mkdtemp(
          path.join(resolvePreferredCarapaceTmpDir(), "carapace-whatsapp-call-"),
        );
...
export function registerWhatsAppCallTool(api: CarapacePluginApi): void {
