...
import { buildRuntimeAccountStatusSnapshot } from "carapace/plugin-sdk/status-helpers";
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: carapace channels login (scan QR on the gateway host).",
      },
    ]);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expected: {
        ...
        fix: "Run: carapace doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    },
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: carapace logs --follow and run carapace channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
