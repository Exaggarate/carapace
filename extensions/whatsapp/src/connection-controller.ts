...
import { info } from "carapace/plugin-sdk/runtime-env";
...
const WHATSAPP_LOGGED_OUT_RELINK_MESSAGE =
  "WhatsApp reported the session is logged out. Cleared cached web session; please rerun carapace channels login and scan the QR again.";
...
export function closeWaSocket(
  ...
  try {
    if (typeof sock?.end === "function") {
      void Promise.resolve(sock.end(new Error("Carapace WhatsApp socket close"))).catch(
...
export class WhatsAppConnectionController {
  ...
  private async finishSocketCleanup(cleanup: WhatsAppSocketCleanup): Promise<void> {
    if (!cleanup.socketClosed) {
      await closeWhatsAppSocketAndWait(cleanup.sock, "Carapace WhatsApp socket close");
