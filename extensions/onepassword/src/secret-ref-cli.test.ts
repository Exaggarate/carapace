...
function createProgram(config: CarapaceConfig = {}): Command {
  const program = new Command().exitOverride();
  const onepassword = program.command("onepassword");
  registerOnePasswordSecretRefCommands({
    command: onepassword,
    config,
    tokenFile: path.join(os.tmpdir(), "carapace-onepassword-missing-token"),
    env: { PATH: "" },
  });
  return program;
}
...
async function createSetupPlan(args: string[]): Promise<OnePasswordPlan> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-onepassword-cli-"));
...
describe("1Password SecretRef setup", () => {
  it("builds provider config and model API-key targets", async () => {
    const plan = await createSetupPlan([
      "--anthropic-id",
      "op://carapace/Anthropic/credential",
      "--openrouter-id",
      "carapace/OpenRouter/credential",
      "--provider-key",
      "xai=op://carapace/xAI/credential",
    ]);
    ...
    expect(plan.targets).toEqual([
      expect.objectContaining({
        ...
        ref: { source: "exec", provider: "onepassword", id: "op://carapace/Anthropic/credential" },
      }),
  ...
  it("builds arbitrary known Carapace and auth-profile targets", async () => {
    const plan = await createSetupPlan([
      "--target",
      "channels.telegram.botToken=op://carapace/Telegram/botToken",
      "--target",
      "models.providers.openai.headers.x-api-key=op://carapace/OpenAI/proxyKey",
      "--target",
      "auth-profiles:main:profiles.openai.key=op://carapace/OpenAI/credential",
    ]);
  ...
  it.each([
    [
      ...
      [
        "--openai-id",
        "op://carapace/OpenAI/credential",
        "--provider-key",
        "OpenAI=op://carapace/OpenAI/other",
      ],
      "Duplicate model provider id",
    ],
    [
      "non-canonical auth-profile agent ids",
      ["--target", "auth-profiles:../main:profiles.openai.key=op://carapace/OpenAI/credential"],
      "Invalid --target auth-profiles target for 1Password",
    ],
    [
      "traversal secret ids",
      ["--provider-key", "openai=op://carapace/../credential"],
      "Invalid --provider-key openai 1Password SecretRef id",
    ],
    [
      "unsupported targets",
      ["--target", "secrets.github_pat=op://carapace/GitHub/pat"],
    ...
    [
      ...
      [
        ...
        "op://carapace/OpenAI/credential",
        "--target",
        "models.providers.openai.apiKey=op://carapace/OpenAI/other",
      ],
  ...
  it.each(["/absolute/path", "op://carapace\\OpenAI\\credential", "op://vault/clé"])(
  ...
  it("prints a quoted canonical plan path after the readiness command", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-1password-setup-test-"));
    ...
    try {
      const output = await runSetup(planPath, ["--openai-id", "op://carapace/OpenAI/credential"]);
      expect(output).toContain("carapace onepassword secretref status");
      expect(output).toContain(
        `carapace secrets apply --from '${canonicalPlanPath}' --dry-run --allow-exec`,
      );
      expect(output).toContain(`carapace secrets apply --from '${canonicalPlanPath}' --allow-exec`);
    } finally {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-secret-plan-test-"));
      const planPath = path.join(tempDir, "plan.json");
      try {
        await fs.chmod(tempDir, 0o777);
        await expect(
          runSetup(planPath, ["--openai-id", "op://carapace/OpenAI/credential"]),
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const trustedDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-secret-plan-trusted-"));
      const aliasParent = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-secret-plan-alias-"));
