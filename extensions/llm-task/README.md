...
`carapace.invoke --each`) without adding new Carapace code per workflow.
...
default. Run `carapace doctor --fix` once for entries created by older releases;
...
This extension depends on Carapace internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Carapace extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.carapace/extensions` as a standalone plugin directory.
