...
import { buildModelAliasIndex, resolveModelRefFromString } from "carapace/plugin-sdk/agent-runtime";
import {
...
export const llmTaskToolDefinition = {
  ...
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via carapace.invoke.",
...
export function createLlmTaskTool(api: CarapacePluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams, signal?: AbortSignal) {
      ...
      let thinkLevel: ReturnType<CarapacePluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
