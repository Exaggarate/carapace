...
describe("probeNextcloudTalkBotResponseFeature", () => {
  ...
  it.each([1 | 2 | 8, "+011"])(
    ...
    async (features) => {
      ...
      await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
        ...
        botName: "Carapace",
        features: 11,
        message: 'Nextcloud Talk bot "Carapace" has the response feature.',
      });
    },
  );
  ...
  it("reports missing response feature for the matching webhook bot", async () => {
    ...
    await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
      ...
      features: 9,
      message:
        'Nextcloud Talk bot "Carapace" (7) is missing the response feature (features=9); outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
    });
  });
  ...
  it.each(["2response", -1])(
    ...
    async (features) => {
      ...
      await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
        ...
        botName: "Carapace",
        message:
          'Nextcloud Talk bot "Carapace" (7) is missing the response feature; outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
      });
    },
  );
