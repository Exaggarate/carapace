...
vi.mock("carapace/plugin-sdk/memory-core-host-runtime-core", { spy: true });

const { createTempWorkspace } = createMemoryCoreTestHarness();
const DREAMING_TEST_BASE_TIME = new Date("2026-04-05T10:00:00.000Z");
const DREAMING_TEST_DAY = "2026-04-05";
const LIGHT_SLEEP_EVENT_TEXT = "__carapace_memory_core_light_sleep__";
const REM_SLEEP_EVENT_TEXT = "__carapace_memory_core_rem_sleep__";
const originalDreamingTestFast = process.env.CARAPACE_TEST_FAST;
const originalDreamingStateDir = process.env.CARAPACE_STATE_DIR;
const memoryArtifactProvenanceMock = vi.mocked(listMemoryArtifactProvenance);
memoryArtifactProvenanceMock.mockResolvedValue([]);
const LIGHT_DREAMING_TEST_CONFIG: CarapaceConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function setDreamingTestEnv(stateDir: string): void {
  Reflect.set(process.env, "CARAPACE_TEST_FAST", "1");
...
function restoreDreamingTestEnv(): void {
  if (originalDreamingTestFast === undefined) {
    Reflect.deleteProperty(process.env, "CARAPACE_TEST_FAST");
  } else {
    Reflect.set(process.env, "CARAPACE_TEST_FAST", originalDreamingTestFast);
  }
  if (originalDreamingStateDir === undefined) {
    Reflect.deleteProperty(process.env, "CARAPACE_STATE_DIR");
  } else {
    Reflect.set(process.env, "CARAPACE_STATE_DIR", originalDreamingStateDir);
  }
  clearRuntimeConfigSnapshot();
}
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("carapace-dreaming-phases-");
...
function createNarrativeDreamingSweepConfig(workspaceDir: string): CarapaceConfig {
  return {
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__carapace_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __carapace_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__carapace_memory_core_light_sleep__",
            "",
  ...
  it("records policy exclusions and keeps forgotten sessions excluded after policy removal and resweeps", async () => {
    ...
    const excludedConfig: CarapaceConfig = {
      agents: { list: [{ id: "main", workspace: workspaceDir }] },
      plugins: {
        entries: {
          "memory-core": {
  ...
  it("promotes one recurring bullet across three contextualized day files", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__carapace_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: CarapaceConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
