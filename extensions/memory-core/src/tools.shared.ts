...
export function buildMemorySearchUnavailableResult(
  ...
  const deadlineAction = overrides?.agentId
    ? `Retry memory_search after a short wait: a memory-corpus timeout pauses retries for up to a minute. If memory-corpus timeouts persist, run: carapace memory status --deep --agent ${overrides.agentId}, and rebuild with carapace memory index --force --agent ${overrides.agentId} only if it reports the index dirty or incomplete`
