...
import * as sqliteRuntime from "carapace/plugin-sdk/sqlite-runtime";
...
describe("memory manager shared agent connection", () => {
  ...
  it.each(["default", "cli", "maintenance"] as const)(
    ...
    async (purpose) => {
      const shared = sqliteRuntime.openCarapaceAgentDatabase({ agentId: "main" });
  ...
  it("opens a hot-created agent through the same canonical owner", async () => {
    ...
    fixture.trackManager(manager);
    const shared = sqliteRuntime.openCarapaceAgentDatabase({ agentId: "hot" });
  ...
  it("rejects shared integrity failure before exposing a manager", async () => {
    ...
    expect(() => sqliteRuntime.openCarapaceAgentDatabase({ agentId: "main" })).toThrow(
      /foreign_key_check/,
    );
  ...
  it("retains the borrowed connection through agent-cache eviction until manager close", async () => {
    ...
    for (let index = 0; index < 65; index += 1) {
      sqliteRuntime.openCarapaceAgentDatabase({ agentId: `churn-${index}` });
    }
    ...
    await manager.close();
    for (let index = 0; index < 65; index += 1) {
      sqliteRuntime.openCarapaceAgentDatabase({ agentId: `released-${index}` });
    }
  ...
  it("shares retained manager handles and trims released handles on the next open", async () => {
    ...
    for (const { id: agentId } of agents) {
      handles.add(sqliteRuntime.openCarapaceAgentDatabase({ agentId }).db);
    ...
    handles.add(sqliteRuntime.openCarapaceAgentDatabase({ agentId: "after-release" }).db);
