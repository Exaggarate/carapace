    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["carapace memory status", "Show index and provider status."],
          [
            "carapace memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["carapace memory status --deep", "Probe embedding provider readiness."],
          ["carapace memory index --force", "Force a full reindex."],
          ['carapace memory search "meeting notes"', "Quick search using positional query."],
          [
            'carapace memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            "carapace memory forget --hook-source gmail --dry-run",
            "Preview deletion of memories derived from matching sessions.",
          ],
          [
            `carapace memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "carapace memory rem-backfill --path ./memory",
          ...
          [
            "carapace memory session-backfill --agent main --from 2026-01-01",
            "Preview trusted candidates from retained session history.",
          ],
          ["carapace memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "github.com/Exaggarate/carapace")}\n`,
    );
