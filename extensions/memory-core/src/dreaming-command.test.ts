...
import type { PluginCommandContext } from "carapace/plugin-sdk/core";
...
function resolveStoredDreaming(config: CarapaceConfig): Record<string, unknown> {
...
function createHarness(initialConfig: CarapaceConfig = {}) {
  let runtimeConfig: CarapaceConfig = initialConfig;
  ...
  const runtime = {
    config: {
      ...
      writeConfigFile: vi.fn(async (nextConfig: CarapaceConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
