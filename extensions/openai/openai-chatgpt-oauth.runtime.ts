...
import type { OAuthCredentials } from "carapace/plugin-sdk/provider-oauth-runtime";
...
const openAICodexOAuthOriginator = "carapace";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Carapace process and then retry `carapace models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this Carapace process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://github.com/Exaggarate/carapace", "OAuth help");
