...
async function captureOpenRouterWrappedPayload(params: {
  ...
  const baseStreamFn = vi.fn(
    (
      ...args: Parameters<import("carapace/plugin-sdk/agent-core").StreamFn>
    ): ReturnType<import("carapace/plugin-sdk/agent-core").StreamFn> => {
...
describe("openrouter provider hooks", () => {
  ...
  it("forwards resolved API keys as explicit OpenRouter auth headers", async () => {
    ...
    expect(headers.get("http-referer")).toBe("https://github.com/Exaggarate/carapace");
    expect(headers.get("x-openrouter-title")).toBe("Carapace");
  });
