...
type ConversationRuntimeModule = typeof import("carapace/plugin-sdk/conversation-binding-runtime");
...
vi.mock("carapace/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("carapace/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("carapace/plugin-sdk/conversation-binding-runtime")>(
        "carapace/plugin-sdk/conversation-binding-runtime",
      ),
  );
});
...
vi.mock("carapace/plugin-sdk/agent-runtime", () => ({
  ...
  resolveDefaultModelForAgent: (params: { cfg: CarapaceConfig }) => {
...
vi.mock("carapace/plugin-sdk/models-provider-runtime", () => ({
  buildPreparedModelsProviderData: buildPreparedModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `carapace-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("carapace/plugin-sdk/command-auth-native").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("carapace/plugin-sdk/command-auth-native").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("carapace/plugin-sdk/command-auth-native");
...
describe("discord native /think autocomplete", () => {
  ...
  it.each([
    ...
    { sessionRuntime: "carapace", expectedRuntime: "carapace", supportsUltra: true },
  ])(
