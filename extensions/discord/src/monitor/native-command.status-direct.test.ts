...
vi.mock("carapace/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("carapace/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): CarapaceConfig {
  return {
    commands: {
...
async function createStatusCommand(cfg: CarapaceConfig, pluginExecute?: ReturnType<typeof vi.fn>) {
