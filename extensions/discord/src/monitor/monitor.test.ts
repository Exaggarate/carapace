...
import { registerPluginInteractiveHandler } from "carapace/plugin-sdk/plugin-runtime";
import { getActivePluginRegistry } from "carapace/plugin-sdk/plugin-test-runtime";
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): CarapaceConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/carapace-sessions-test.json");
