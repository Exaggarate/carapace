...
type ResolveStorePath = typeof import("carapace/plugin-sdk/session-store-runtime").resolveStorePath;

const hoisted = vi.hoisted(() => {
  const deleteSessionEntry = vi.fn();
  const listSessionEntries = vi.fn();
  const resolveStorePath = vi.fn<ResolveStorePath>(() => "/tmp/carapace-sessions.json");
...
describe("closeDiscordThreadSessions", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveStorePath.mockReturnValue("/tmp/carapace-sessions.json");
  });
