...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as CarapaceConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "carapace",
        channelDescription: "Carapace development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "carapace");
    expectGeneratedTitleField(
      "channelDescription",
      "Carapace development coordination and release planning",
    );
