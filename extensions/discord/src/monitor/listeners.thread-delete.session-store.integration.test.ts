...
import { withEnvAsync, withStateDirEnv } from "carapace/plugin-sdk/test-env";
...
describe("DiscordThreadDeleteListener session-store integration", () => {
  it("deletes matching sessions from every configured agent store", async () => {
    await withStateDirEnv("carapace-discord-thread-delete-", async ({ tempRoot, stateDir }) => {
      ...
      await withEnvAsync({ CARAPACE_STATE_DIR: canonicalStateDir }, async () => {
