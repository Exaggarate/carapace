...
vi.mock("carapace/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("carapace/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<CarapaceConfig["channels"]>["discord"]>["execApprovals"],
): CarapaceConfig {
  return {
    channels: {
      discord: {
