    ...
    it.each([
      ...
      async ({ occupied, recovery, stage, overlap }) => {
        ...
        try {
          ...
          vi.useFakeTimers();
          // Native speaking state exists before Carapace installs its receive listeners.
    ...
    it.each([
      ...
      async ({ mode, origin }) => {
        ...
        try {
          ...
          // A native restart inside Carapace's grace retains this same owned receive stream.
