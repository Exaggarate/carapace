...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: CarapaceConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): CarapaceConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("carapace/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("carapace/plugin-sdk/acp-runtime")>(
...
vi.mock("carapace/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("carapace/plugin-sdk/native-command-config-runtime")
  >("carapace/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
