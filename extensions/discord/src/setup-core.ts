...
import type { DiscordGuildEntry, CarapaceConfig } from "carapace/plugin-sdk/config-contracts";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "carapace/plugin-sdk/setup-runtime";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CarapaceConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CarapaceConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CarapaceConfig; accountId: string }) =>
