...
- Polling and webhook both: enqueue then pump
  ...
  loops.
- Stall timeout: `CARAPACE_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS` →
...
- Do not reintroduce `sendMessageDraft` for answer streaming. Telegram drafts
  ...
  requires a separate `sendMessage`. Carapace uses `sendMessage` plus
...
- Reply context comes from Carapace-observed messages. Bot API updates expose
