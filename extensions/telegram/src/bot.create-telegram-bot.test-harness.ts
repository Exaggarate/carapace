...
type GetRuntimeConfigFn =
  typeof import("carapace/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type GetSessionEntryFn = typeof import("carapace/plugin-sdk/session-store-runtime").getSessionEntry;
type ResolveStorePathFn =
  typeof import("carapace/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("carapace/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
type SessionEntry = import("carapace/plugin-sdk/session-store-runtime").SessionEntry;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}carapace-telegram-${
      process.pid
...
function resolveDefaultModelForAgentForTest(params: { cfg: CarapaceConfig }): {
  provider: string;
  model: string;
} {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: CarapaceConfig = {
  agents: {
    defaults: {
      userTimezone: "UTC",
    },
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: CarapaceConfig) => {
