...
// fix for the leak observed in carapace#68128 is the transport lifecycle that
...
function resolveCarapaceProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.CARAPACE_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveCarapaceProxyUrlForTelegram() : undefined;
