...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    carapaceState = await createCarapaceTestState({
      layout: "state-only",
      prefix: "carapace-telegram-bindings-",
    });
    resetPluginStateStoreForTests({ closeDatabase: false });
    installThreadBindingStore(createThreadBindingStore());
    threadBindingStore.clear();
    readAcpSessionEntryMock.mockReset();
    const acpRuntime = await vi.importActual<typeof import("carapace/plugin-sdk/acp-runtime")>(
      "carapace/plugin-sdk/acp-runtime",
    );
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:carapace-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:carapace-codex-app-server:still-valid",
    );
