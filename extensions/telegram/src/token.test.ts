...
describe("resolveTelegramToken", () => {
  ...
  function createUnknownAccountConfig(): CarapaceConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: CarapaceConfig) {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as CarapaceConfig;
  ...
  it("applies account file, account config, channel file, channel config, then env precedence", () => {
    ...
    const resolve = (telegram: Record<string, unknown>) =>
      resolveTelegramToken({ channels: { telegram } } as CarapaceConfig);
  ...
  // Regression: https://github.com/Exaggarate/carapace/issues/53876
