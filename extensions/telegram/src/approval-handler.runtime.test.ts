...
describe("telegramApprovalNativeRuntime", () => {
  ...
  it("renders a cancelled system-agent result as lifecycle cancellation", () => {
    ...
    ).toBe("⚠️ Carapace change was cancelled because its run ended. No change was made. Retry.");
  });
  ...
  it("renders a system-agent approval with an optional Control UI link", async () => {
    const payload = (await telegramApprovalNativeRuntime.presentation.buildPendingPayload({
      ...
      view: {
        ...
        phase: "pending",
        title: "Carapace change requires approval",
        description: "set config gateway.port to 19001",
      ...
      [
        {
          ...
          url: "https://control.example.com/carapace/approve/system-agent%3Achange-1",
        },
      ],
      [
  ...
  it.each([
    {
      ...
      expected: "✅ Carapace change approved and applied: set config gateway.port to 19001",
    },
    {
      ...
      summary: "set config gateway.port to 19001",
      expected: "❌ Carapace change denied. No change was made.",
    },
    {
      ...
      expected: `✅ Carapace change approved and applied: ${"x".repeat(2798)}…`,
    },
  ] as const)(
  ...
  it("sends one terminal origin result and releases its dedupe entry after finalization", async () => {
    ...
    await telegramApprovalNativeRuntime.transport.updateEntry?.({
      ...
      request,
      approvalKind: "system-agent",
      payload: { text: "✅ Carapace change approved. Applying: restart the Gateway" },
      phase: "resolved",
    });

    expect(sendMessage).toHaveBeenCalledWith(
      "1234",
      "✅ Carapace change approved. Applying: restart the Gateway",
      {
        cfg: {},
        token: "tg-token",
        accountId: "default",
  ...
  it("sends the origin result when the approver card edit fails", async () => {
    ...
    await expect(
      telegramApprovalNativeRuntime.transport.updateEntry?.({
        ...
        request,
        approvalKind: "system-agent",
        payload: { text: "⚠️ Carapace change approved, but it was not applied." },
        phase: "resolved",
      }),
    ).rejects.toThrow("message was deleted");
    expect(sendMessage).toHaveBeenCalledWith(
      "1234",
      "⚠️ Carapace change approved, but it was not applied.",
      {
        cfg: {},
        token: "tg-token",
        accountId: "default",
  ...
  it("sends origin notices only through the originating Telegram account", async () => {
    ...
    const payload = { text: "✅ Carapace change approved and applied." };
