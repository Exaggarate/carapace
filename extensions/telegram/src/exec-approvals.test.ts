...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<CarapaceConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<CarapaceConfig["channels"]>["telegram"]>>,
): CarapaceConfig {
  return {
    channels: {
      telegram: {
...
describe("telegram exec approvals", () => {
  ...
  it("scopes non-telegram turn sources to the stored telegram account", async () => {
    const workspace = tempWorkspaceSync({
      ...
      prefix: "carapace-telegram-exec-approvals-",
    });
