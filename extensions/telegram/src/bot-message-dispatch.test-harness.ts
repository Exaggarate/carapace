...
const getAgentScopedMediaLocalRootsHoisted = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.carapace/workspace-${agentId}`]),
);
...
vi.mock("carapace/plugin-sdk/channel-inbound", async (importOriginal) => {
  ...
  return {
    ...
    runChannelInboundEvent: async (params: RunParams) => {
      ...
      const delivery =
        resolved.delivery as unknown as import("carapace/plugin-sdk/channel-inbound").ChannelInboundTurnPlan<"provider_message_sending">["delivery"];
