...
vi.mock("carapace/plugin-sdk/secret-file-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("carapace/plugin-sdk/secret-file-runtime")>()),
  ...
  it.each([
    {
      ...
      telegram: {
        accounts: {
          sut: {
            tokenFile: "/tmp/carapace-telegram-reply-mode-must-not-read",
    ...
    const cfg = { channels: { telegram } } as unknown as CarapaceConfig;
