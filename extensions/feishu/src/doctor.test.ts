...
function feishuConfig(): CarapaceConfig {
  return {
    channels: {
      feishu: {
        appId: "cli_xxx",
...
async function runDoctor(shouldRepair: boolean, cfg: CarapaceConfig = feishuConfig()) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-feishu-doctor-"));
    vi.stubEnv("HOME", tempHome);
    vi.stubEnv("CARAPACE_HOME", tempHome);
    vi.stubEnv("CARAPACE_STATE_DIR", path.join(tempHome, ".carapace"));
  ...
  it("removes only unhealthy Feishu direct sessions while preserving state, config, and other sessions", async () => {
    ...
    expect(
      fs.existsSync(path.join(backupDir, "session-stores", "main", "carapace-agent.sqlite")),
    ).toBe(true);
    ...
    },
    {
      ...
      arrange: async () => {
        ...
        return {
          ...
          session,
          sqlitePath: path.join(path.dirname(customStorePath), "carapace-agent.support.sqlite"),
          verifyTranscript: true,
        };
      },
    },
