...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "carapace-feishu-agent-"));
});
...
function createRuntime(
  ...
  persistedCfg?: CarapaceConfig,
  mutationCfg?: CarapaceConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as CarapaceConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: CarapaceConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: CarapaceConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: createCanonicalAgentRoster(), bindings: [] } as CarapaceConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
