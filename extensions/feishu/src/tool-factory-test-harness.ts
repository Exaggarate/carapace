...
export function createToolFactoryHarness(cfg: CarapacePluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CarapacePluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
