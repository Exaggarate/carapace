...
function collectConfiguredAgentIds(cfg: CarapaceConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: CarapaceConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "carapace doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: CarapaceConfig): boolean {
