...
vi.mock("./runtime.js", async () => {
  const replyChunking = await import("carapace/plugin-sdk/reply-chunking");
  const textChunking = await import("carapace/plugin-sdk/text-chunking");
  const markdownTables = await import("carapace/plugin-sdk/markdown-table-runtime");
  const runtime = {
    media: {
      loadWebMedia: async () => {
