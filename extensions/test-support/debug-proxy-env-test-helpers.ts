...
export function installDebugProxyTestResetHooks() {
  ...
  afterEach(async () => {
    const { closeDebugProxyCaptureStore } = await import("carapace/plugin-sdk/proxy-capture");
    const { closeCarapaceStateDatabaseForTest } =
      await import("carapace/plugin-sdk/sqlite-runtime-testing");
