...
import { createAgentToolResultMiddlewareRunner } from "carapace/plugin-sdk/agent-harness";
import { createTestPluginApi } from "carapace/plugin-sdk/plugin-test-api";
import { beforeEach, describe, expect, it, vi } from "vitest";

const { tokenjuiceFactory, createTokenjuiceCarapaceEmbeddedExtension } = vi.hoisted(() => {
  const tokenjuiceFactoryLocal = vi.fn();
  const createTokenjuiceCarapaceEmbeddedExtensionLocal = vi.fn(() => tokenjuiceFactoryLocal);
  return {
    tokenjuiceFactory: tokenjuiceFactoryLocal,
    createTokenjuiceCarapaceEmbeddedExtension: createTokenjuiceCarapaceEmbeddedExtensionLocal,
  };
});

vi.mock("./runtime-api.js", () => ({
  createTokenjuiceCarapaceEmbeddedExtension,
}));
...
describe("tokenjuice plugin", () => {
  beforeEach(() => {
    createTokenjuiceCarapaceEmbeddedExtension.mockClear();
  ...
  it("registers tokenjuice tool result middleware for Carapace and Codex runtimes", () => {
    ...
    expect(createTokenjuiceCarapaceEmbeddedExtension).toHaveBeenCalledTimes(1);
    ...
    expect(registration?.[1]).toEqual({ runtimes: ["carapace", "codex"] });
  });
    ...
    await middleware(
      {
        ...
        toolName: "bash",
        args: { command: "cat /tmp/out.txt", workdir: "/tmp/carapace-tokenjuice-test" },
        result: {
  ...
  it("keeps compacted exec results below the middleware details limit", async () => {
    ...
    const runner = createAgentToolResultMiddlewareRunner({ runtime: "carapace" }, [
    ...
    const result = await middleware(
      {
        ...
        toolName: "bash",
        args: { command: "printf 'hello\\n'", workdir: "/tmp/carapace-tokenjuice-test" },
