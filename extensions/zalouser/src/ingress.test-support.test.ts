...
import { openCarapaceAgentDatabase } from "carapace/plugin-sdk/sqlite-runtime-testing";
...
describe("withZalouserIngressTestQueue", () => {
  it("does not write through the parent state environment during teardown", async () => {
    const parentDir = await fs.realpath(
      await fs.mkdtemp(path.join(resolvePreferredCarapaceTmpDir(), "carapace-zalouser-parent-")),
    );
    const previousStateDir = process.env.CARAPACE_STATE_DIR;
    process.env.CARAPACE_STATE_DIR = parentDir;
    try {
      ...
      // untouched; the old order created carapace.sqlite* files here.
