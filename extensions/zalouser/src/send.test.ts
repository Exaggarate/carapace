...
describe("zalouser send helpers", () => {
  ...
  it("delegates link helper to JS transport", async () => {
    ...
    const result = await sendLinkZalouser("thread-3", "https://github.com/Exaggarate/carapace", {
      profile: "p3",
      isGroup: true,
    });

    expect(mockSendLink).toHaveBeenCalledWith("thread-3", "https://github.com/Exaggarate/carapace", {
      profile: "p3",
      isGroup: true,
    });
