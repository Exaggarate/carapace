// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatCarapaceUserAgent`.

import { CARAPACE_VERSION as PACKAGE_VERSION } from "carapace/plugin-sdk/agent-harness-runtime";
import { normalizeOptionalString } from "carapace/plugin-sdk/string-coerce-runtime";

const ORIGINATOR = "carapace";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = normalizeOptionalString(process.env.CARAPACE_VERSION);
  if (envVersion) {
    return envVersion;
  }
...
// an empty record so the carapace identity is not forwarded to the proxy.
