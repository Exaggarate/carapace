...
vi.mock("carapace/plugin-sdk/provider-auth-runtime", () => providerAuthRuntimeMocks);
...
type XaiAutoEnableProbe = Parameters<CarapacePluginApi["registerAutoEnableProbe"]>[0];
...
function registerXaiBilledToolFactories() {
  const tools = new Map<string, Parameters<CarapacePluginApi["registerTool"]>[0]>();
  plugin.register(
    createTestPluginApi({
