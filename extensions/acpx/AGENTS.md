...
The ACPX extension is a thin Carapace wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `carapace/acpx`, not in this extension.
...
Use this flow when Carapace needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `carapace/acpx` repo first.
2. In Carapace, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Carapace and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Carapace ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Carapace-specific glue, move it to `carapace/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
