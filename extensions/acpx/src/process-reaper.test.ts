...
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} --label owner's-choice ${CARAPACE_ACPX_LEASE_ID_ARG} lease-1 ${CARAPACE_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
const PLUGIN_DEPS_CODEX_APP_SERVER_COMMAND =
  "node /tmp/carapace/plugin-runtime-deps/node_modules/@openai/codex/bin/codex.js app-server";
const PLUGIN_DEPS_CODEX_PLATFORM_COMMAND =
  "/tmp/carapace/plugin-runtime-deps/node_modules/@openai/codex-linux-x64/vendor/codex app-server";
...
type CleanupDeps = NonNullable<Parameters<typeof cleanupCarapaceOwnedAcpxProcessTree>[0]["deps"]>;
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isCarapaceLeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isCarapaceLeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });
    ...
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${CARAPACE_ACPX_LEASE_ID_ARG} other-lease ${CARAPACE_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
  ...
  it("recovers a pending lease only from its exact wrapper and gateway identity", async () => {
    const { deps, killed } = cleanupDeps([
      ...
      { pid: 120, ppid: 1, command: CODEX_WRAPPER_COMMAND_WITH_LEASE },
      {
        pid: 121,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${CARAPACE_ACPX_LEASE_ID_ARG} lease-1 ${CARAPACE_GATEWAY_INSTANCE_ID_ARG} gateway-foreign`,
      },
  ...
  it("does not kill a reused pid when the live command is not Carapace-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-carapace-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale Carapace-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleCarapaceOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
