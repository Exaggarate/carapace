...
Official Carapace provider plugin for the **Meta API** — an OpenAI-compatible
...
Carapace defaults to `high`. Meta's
[reasoning documentation](https://dev.meta.ai/docs/reasoning/) says that an
omitted effort uses a model-determined level, so `high` is an Carapace policy rather
