...
vi.mock("carapace/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("carapace/plugin-sdk/provider-auth-runtime")>();
  return {
    ...actual,
...
vi.mock("carapace/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("carapace/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("carapace/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(
  ...
  config: Omit<CarapaceConfig, "models"> = {},
): CarapaceConfig {
  return {
    ...config,
    models: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.CARAPACE_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("CARAPACE_DOCKER_SETUP", "1");
