...
function getA2uiPaths(env = process.env) {
  const srcDir = env.CARAPACE_A2UI_SRC_DIR ?? path.join(pluginDir, "src", "host", "a2ui");
  const outDir = env.CARAPACE_A2UI_OUT_DIR ?? path.join(rootDir, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

function shouldSkipMissingA2uiAssets(env = process.env) {
  return env.CARAPACE_A2UI_SKIP_MISSING === "1" || Boolean(env.CARAPACE_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because CARAPACE_A2UI_SKIP_MISSING=1 or CARAPACE_SPARSE_PROFILE is set.`,
      );
      return;
    }
