...
Read-only diff viewer plugin for **Carapace** agents.

## Install

```bash
carapace plugins install @carapace/diffs
```
...
- Minimum Carapace host: `2026.4.30`
...
Before:
Carapace supports plugins.

After:
Carapace supports plugins and hosted diff views.
```
...
- Rendered PNG/PDF files remain temporary materializations in `$TMPDIR/carapace-diffs` because delivery APIs require a file path. No JSON metadata sidecars are written or imported.
