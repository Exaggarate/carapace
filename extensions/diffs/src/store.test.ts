...
import type { PluginBlobStore } from "carapace/plugin-sdk/plugin-state-runtime";
...
describe("DiffArtifactStore", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("carapace-diffs-store-"));
  });
...
describe("createDiffsHttpHandler", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("carapace-diffs-http-"));
  });
  ...
  it("serves the shared viewer runtime asset", async () => {
    ...
    expect(handled).toBe(true);
    expect(res.statusCode).toBe(200);
    expect(String(res.body)).toContain("carapaceDiffsReady");
