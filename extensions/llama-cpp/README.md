...
Official llama.cpp provider for managed and external Carapace model servers.

The `llama-cpp` provider either installs a pinned, integrity-verified
`llama-server` under Carapace's `localService` supervisor or connects to a
server that you already operate. Both choices use `llama-cpp/<model>` references
and Carapace's normal OpenAI-compatible chat transport. Local embeddings require
the managed choice.
...
After explicit consent, Carapace installs the matching server build and a
...
declined, Carapace offers a separate embedding-only setup. After explicit
...
setup after changing the model so Carapace can verify the file and regenerate
the managed router preset.

See the [llama.cpp provider guide](../../docs/plugins/llama-cpp.md)
...
optional API key. Carapace passively discovers single-model and router catalogs.
...
- Minimum Carapace host: `2026.6.2`
