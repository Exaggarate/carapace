...
vi.mock("carapace/plugin-sdk/process-runtime", () => ({
...
const baseParams = {
  config: baseConfig,
  runtimeId: "carapace-mxc-test-abc12345",
  workdir: "/workspace",
};
...
const MXC_TEST_ENV_KEYS = [
  ...
  "CARAPACE_MXC_HOST_SECRET_TEST",
  "CARAPACE_MXC_SECRET_TEST",
  "ProgramData",
  "ProgramFiles",
  "ProgramFiles(x86)",
  "ProgramW6432",
...
describe("createMxcSandboxBackendFactory", () => {
  test("hashes workspace-qualified scopes without truncating their identity", async () => {
    ...
    expect(handle.runtimeId).toMatch(/^carapace-mxc-workspace-[a-f0-9]{32}$/u);
  });
});
...
describeOnWindows("createMxcSandboxBackendHandle (Windows-only MXC backend tests)", () => {
  ...
  test("buildExecSpec returns a launcher argv with Windows process containment by default", async () => {
    ...
    expect(processContainer).toEqual({
      name: "carapace-mxc-test-abc12345",
      leastPrivilege: true,
      capabilities: [],
      ui: {
        isolation: "container",
  ...
  test("buildExecSpec keeps command and env payload out of process argv", async () => {
    await withProcessEnv({ CARAPACE_MXC_HOST_SECRET_TEST: "host-secret" }, async () => {
      const handle = createMxcSandboxBackendHandle(baseParams);
      const spec = await handle.buildExecSpec({
      ...
      expect(spec.env.CARAPACE_MXC_HOST_SECRET_TEST).toBeUndefined();
      expect(spec.argv[2]).toBe("--payload-file");
    ...
    await withProcessEnv(
      {
        ...
        USERPROFILE: "C:\\Users\\carapace",
        APPDATA: "C:\\Users\\carapace\\AppData\\Roaming",
        LOCALAPPDATA: "C:\\Users\\carapace\\AppData\\Local",
        ProgramData: "C:\\ProgramData",
        "ProgramFiles(x86)": "C:\\Program Files (x86)",
        NUMBER_OF_PROCESSORS: "8",
        CARAPACE_MXC_SECRET_TEST: "do-not-leak",
      },
      async () => {
        ...
        expect(env).toContain("USERPROFILE=C:\\Users\\carapace");
        expect(env).toContain("APPDATA=C:\\Users\\carapace\\AppData\\Roaming");
        expect(env).toContain("LOCALAPPDATA=C:\\Users\\carapace\\AppData\\Local");
        ...
        expect(env.some((entry) => entry.startsWith("CARAPACE_MXC_SECRET_TEST="))).toBe(false);
  ...
  test("Windows process containment caps long AppContainer names", async () => {
    const handle = createMxcSandboxBackendHandle({
      ...baseParams,
      runtimeId: `carapace-mxc-${"a".repeat(80)}-12345678`,
    });
    const spec = await handle.buildExecSpec({ command: "echo hello", env: {}, usePty: false });
  ...
  test("workspaceAccess rw fails closed when protected skill roots exist", async () => {
    ...
    try {
      ...
      mkdirSync(path.join(workdir, ".carapace", "sandbox-skills", "skills", "demo"), {
        recursive: true,
      });
  ...
  test("filesystem bridge protects skill overlays when workspaceAccess is rw", async () => {
    ...
    try {
      ...
      await expect(
        bridge?.rename({
          from: "normal.txt",
          to: ".carapace/sandbox-skills/skills/demo/new.md",
          cwd: workdir,
        }),
      ).rejects.toThrow(/read-only/u);
      await expect(
  ...
  test("runShellCommand uses the inline Windows command line when no args are passed", async () => {
    ...
    const expectedShell = process.env.ComSpec?.trim() || "cmd.exe";
    expect(processConfig?.commandLine).toBe(`${expectedShell} /d /s /c "echo hello"`);
    expect(String(processConfig?.commandLine)).not.toContain(".carapace-mxc-cmd-");
  });
  ...
  test("runShellCommand uses curated Windows env and passes stdin through unchanged", async () => {
    await withProcessEnv(
      ...
      async () => {
        ...
        expect(commandLine).toMatch(/ \/c ""[^"]*\.carapace-mxc-cmd-[^"]+\.cmd" /u);
        ...
        expect(launcherEnv?.CARAPACE_MXC_SECRET_TEST).toBeUndefined();
