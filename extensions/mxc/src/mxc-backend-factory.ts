...
import type { CreateSandboxBackendParams, SandboxBackendHandle } from "carapace/plugin-sdk/sandbox";
...
function sanitizeRuntimeId(value: string): string {
  if (/:workspace:[a-f0-9]{32}$/i.test(value.trim())) {
    ...
    return `carapace-mxc-workspace-${hash}`;
  }
  const slug = value
    .toLowerCase()
    .replace(/[^a-z0-9_.-]+/g, "-")
  ...
  return `carapace-mxc-${slug || "sandbox"}-${hash}`;
}

/** Factory function called by Carapace when sandbox.backend=mxc. */
