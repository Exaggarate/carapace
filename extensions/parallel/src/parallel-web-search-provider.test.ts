...
describe("parallel web search provider", () => {
  ...
  it("returns a stable missing-key payload that points at the real config path", async () => {
    await expect(paidTool({}).execute({ search_queries: ["carapace"] })).resolves.toEqual({
  ...
  it("resolves and validates configured search endpoints at the request boundary", async () => {
    ...
    }).execute({ search_queries: ["carapace"] });
  ...
  it("treats objective as optional and omits it from the request when absent", async () => {
    enqueueJson();
    const result = await paidTool().execute({ search_queries: ["carapace"] });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readBody();
    expect(body).not.toHaveProperty("objective");
    expect(body).toMatchObject({ search_queries: ["carapace"] });
  ...
  it("returns an error payload when search_queries is missing or empty", async () => {
    const tool = paidTool();
    expect(await tool.execute({ objective: "Find Carapace on GitHub" })).toMatchObject({
      error: "invalid_search_queries",
    });
    expect(
      await tool.execute({ objective: "Find Carapace on GitHub", search_queries: [] }),
  ...
  it("promotes a generic `query` arg into search_queries when search_queries is absent (no synthesized objective)", async () => {
    enqueueJson();
    const result = await paidTool().execute({ query: "Carapace GitHub", count: 3 });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readBody();
    expect(body).not.toHaveProperty("objective");
    expect(body).toMatchObject({
      search_queries: ["Carapace GitHub"],
  ...
  it("honors top-level web search settings and sends the native Parallel payload shape", async () => {
    ...
    const result = await paidTool({
      ...
      objective: "Find the Carapace repository on GitHub",
      search_queries: ["carapace github", "carapace repository"],
    });
    expect(endpointMockState.calls).toHaveLength(1);
    ...
    expect(headers["User-Agent"]).toMatch(/^carapace-parallel\/\d+\.\d+\.\d+/);
  ...
  it("always sends max_results matching the Carapace web_search default when no count is provided", async () => {
    enqueueJson();
    await paidTool().execute({ objective: "Find Carapace", search_queries: ["carapace"] });
    expect(endpointMockState.calls).toHaveLength(1);
  ...
  it("does not surface a Parallel-generated sessionId on a cache hit", async () => {
    ...
    const tool = paidTool();
    const firstResult = await tool.execute({ objective, search_queries: ["carapace github"] });
    expect(firstResult.sessionId).toBe("session-generated-by-parallel");
    const secondResult = await tool.execute({ objective, search_queries: ["carapace github"] });
    expect(endpointMockState.calls).toHaveLength(1);
  ...
  it("preserves caller-supplied sessionId across cache hits", async () => {
    ...
    const tool = paidTool();
    await tool.execute({ objective, search_queries: ["carapace github"], session_id: sessionId });
...
describe("runParallelMcpSearch", () => {
  ...
  it("runs the 3-step handshake and maps results into the REST-compatible shape", async () => {
    ...
    for (const call of endpointMockState.calls) {
      expect(headerOf(call, "User-Agent")).toMatch(/^carapace-parallel\//);
    }
