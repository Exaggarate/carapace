import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "carapace/plugin-sdk/chat-channel-ids";
...
/**
 * Provenance marker appended to every Carapace-injected inbound context header
...
const CONTEXT_HEADER_RE = /^Context:[ \t]*⟦carapace:ctx⟧[ \t]*$/m;

/**
 * Matches JSON blobs that look like Carapace transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains Carapace-injected envelope or
...
/**
 * Strips Carapace-injected envelope metadata from a user message so that only
