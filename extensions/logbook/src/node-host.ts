...
// Nodes without the Carapace app (plain `carapace node host run`) advertise
...
export async function handleLogbookSnapshot(rawParams: unknown): Promise<LogbookSnapshotPayload> {
  ...
  // keeps captures out of the broader Carapace temp namespace.
  const captureDir = path.join(resolvePreferredCarapaceTmpDir(), "logbook");
