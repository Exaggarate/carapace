...
import { parseDateStringTimestampMs as readTimestamp } from "carapace/plugin-sdk/number-runtime";
  ...
  const items = HERMES_REAUTH_PROVIDER_MAPPINGS.flatMap(({ sourceProvider, targetProvider }) => {
    ...
    return [
      createMigrationManualItem({
        ...
        source: sourcePath,
        message: `Hermes ${sourceProvider} credentials cannot be reused safely by Carapace.`,
        recommendation:
          targetProvider === "qwen"
            ? "Authenticate qwen with an API key after migration: carapace onboard --auth-choice qwen-api-key."
            : `Authenticate ${targetProvider} in Carapace after migration.`,
      }),
    ];
  });
