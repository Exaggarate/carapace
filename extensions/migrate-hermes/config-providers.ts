    ...
    if (transport && !HERMES_TRANSPORTS[transport]) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an equivalent Carapace provider plugin or API adapter manually.",
        }),
      );
    ...
    if (normalizeOptionalString(inlineKey) && !readEnvReference(inlineKey)) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" contains an inline API key that was not copied into Carapace config.`,
          recommendation: "Move the key to an environment variable or Carapace secret provider.",
        }),
      );
    }
    ...
    if (isRecord(raw.extra_body) && Object.keys(raw.extra_body).length > 0) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" adds request body fields that Carapace cannot import generically.`,
          recommendation:
    ...
    if (apiKeyEnv && !env[apiKeyEnv]?.trim()) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an Carapace auth profile for this provider or expose the variable to the Carapace runtime.",
        }),
      );
    }
  }
