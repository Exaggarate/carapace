...
import { runFfmpeg } from "carapace/plugin-sdk/media-runtime";
...
const liveEnabled = process.env.CARAPACE_LIVE_TEST === "1" && apiKey.length > 0;
const hasSay =
  liveEnabled &&
...
describeLive("SenseAudio live", () => {
  it("transcribes generated speech", async () => {
    await withTempDir("carapace-senseaudio-live-", async (tempDir) => {
