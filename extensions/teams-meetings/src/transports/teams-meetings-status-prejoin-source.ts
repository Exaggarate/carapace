...
export function teamsMeetingStatusPreludeSource(params: MeetingStatusPreludeParams): string {
  ...
  if (canMutateSession && (identityVerifiedBeforeCall || identityPreservedInCall)) {
    ...
    delete window.__carapaceTeamsMeeting;
  }
  ...
  const ensureVirtualAudioInput = async () => {
    ...
    try {
      ...
      const preparedInput = window.__carapaceTeamsMeeting;
      ...
      if (selected && window.__carapaceTeamsMeeting?.identity === expectedIdentity) {
        window.__carapaceTeamsMeeting.audioInputDeviceId = input.deviceId;
      }
      return selected;
    } catch (error) {
  ...
  if (identityVerified && !inCall && allowMicrophone && microphone) {
    ...
    if (!audioInputRouted) {
      ...
      controlManualAction = manualActionFor("teams-audio-choice-required", "Select the Carapace virtual audio device as the Teams microphone and verify it is selected before enabling talk-back.");
  ...
  if (committedOwnerConflict && !canMutateSession) {
    ...
    manualAction = manualActionFor("teams-login-required", tenantLoginRequired ? "This Teams tenant requires sign-in or email verification. Complete it in the Carapace browser profile, then retry." : "Sign in to Microsoft Teams in the Carapace browser profile, then retry the meeting join.");
  } else if (!inCall && lobbyWaiting) {
    manualAction = manualActionFor("teams-admission-required", "Admit the Carapace guest from the Microsoft Teams lobby, then retry speech.");
  } else if (!inCall && permissionRequired) {
    manualAction = manualActionFor("teams-permission-required", allowMicrophone ? "Allow microphone permission for Teams in the Carapace browser profile, then retry." : "Dismiss the Teams device-permission prompt or continue without devices, then retry.");
