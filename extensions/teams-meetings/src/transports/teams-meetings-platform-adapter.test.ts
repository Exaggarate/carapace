...
describe("Microsoft Teams meeting platform adapter", () => {
  ...
  it("does not stamp meeting identity onto unrelated Teams pages", async () => {
    ...
    expect(window).not.toHaveProperty("__carapaceTeamsMeeting");
  });
  ...
  it("does not trust a stale identity marker or post-call screen from another SPA call", () => {
    ...
    expect(window["__carapaceTeamsAudioOutputs"]).toHaveLength(1);
  });
  ...
  it("does not leave a call owned by a newer Carapace session", () => {
    const leave = control({ label: "Leave" });
    const inCallUrl = "https://teams.microsoft.com/v2/";
    const { result } = runLeaveScript({
      currentUrl: inCallUrl,
  ...
  it("retires only the departing session's audio bridges", () => {
    ...
    expect(window["__carapaceTeamsAudioOutputs"]).toEqual([foreignBridge]);
  });
  ...
  it("retires the page-owned audio bridge from the required URL-only leave callback", () => {
    ...
    expect(bridge.pause).toHaveBeenCalledOnce();
    expect(bridge.remove).toHaveBeenCalledOnce();
    expect(window).not.toHaveProperty("__carapaceTeamsAudioOutputs");
  });
  ...
  it.each([
    ...
    ["Carapace Meeting Audio", "Carapace Meeting Audio"],
  ])(
  ...
  it.each(["Carapace Meeting Audio (Virtual)", "Monitor of Carapace Meeting Audio"])(
    ...
    async (deviceLabel) => {
      ...
      expect(result).toMatchObject({
        ...
        manualAction: {
          message:
            "Select the Carapace virtual audio device as the Teams microphone and verify it is selected before enabling talk-back.",
  ...
  it("bridges a live Teams MediaStream when its unloaded audio element rejects setSinkId", async () => {
    ...
    expect(bridge.sinkId).toBe("blackhole-output");
    expect(source.muted).toBe(true);
    expect(window).toHaveProperty("__carapaceTeamsAudioOutputs");
    expect((window["__carapaceTeamsAudioOutputs"] as Array<{ bridge: PageMedia }>)[0]?.bridge).toBe(
      bridge,
    );
    ...
    const repeated = await runAudioStatusScript({
      bridgeMedia: bridge,
      media: [source, bridge],
      priorAudioOutputs: window["__carapaceTeamsAudioOutputs"] as unknown[],
      priorMeeting: window[MEETING_STATE_KEY] as Record<string, unknown>,
    });
    expect(repeated.result.audioOutputRouted).toBe(true);
    expect(
      (repeated.window["__carapaceTeamsAudioOutputs"] as Array<{ bridge: PageMedia }>)[0]?.bridge,
    ).toBe(bridge);
  });
});
