---
name: imsg
description: "Use the imsg CLI from Carapace agents for iMessage/SMS DMs, groups, replies, reactions, polls, watching, and private-API actions."
...
Do not use this skill for Telegram, Signal, WhatsApp, Discord, Slack, or for replying inside the current Carapace conversation when the configured channel already routes the reply.
...
- Private API mode is strongly encouraged for Carapace iMessage. It unlocks replies, precise tapbacks, effects, polls, attachment replies, read/typing actions, and group management. Basic mode is a fallback for reads plus plain text/file send.
- Full Disk Access for the process context that runs `imsg` or Carapace; reads fail without Messages DB access.
...
Use the private API bridge for the native iMessage actions Carapace users normally expect:
...
For Carapace channel setup, check bridge availability early:
