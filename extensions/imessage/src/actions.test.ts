...
function cfg(actions?: Record<string, boolean | undefined>): CarapaceConfig {
  return {
    channels: {
      imessage: {
        cliPath: "imsg",
...
describe("imessage message actions", () => {
  ...
  describe("reply with attachment (carapace/imsg#114 plumbing)", () => {
    ...
    it("rejects reply + attachment when imsg does not advertise send-rich --file", async () => {
      ...
      // drop the attachment (the original carapace/carapace#79822 symptom).
      probeMock.getCachedIMessagePrivateApiStatus.mockReturnValue({
        available: true,
        v2Ready: true,
        selectors: {},
