...
import { normalizeScpRemoteHost } from "carapace/plugin-sdk/host-runtime";
import {
  type CommandOptions,
...
import { sanitizeTempFileName } from "carapace/plugin-sdk/temp-path";
...
export async function withIMessageRemoteFile<T>(params: {
  ...
  const remoteDir = `/tmp/carapace-imessage-${token}`;
