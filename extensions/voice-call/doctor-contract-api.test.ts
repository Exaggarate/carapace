...
describe("voice-call doctor state migration", () => {
  ...
  beforeAll(async () => {
    resetPluginStateStoreForTests();
    const warmStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-voice-call-doctor-"));
    const warmStorePath = createTestStorePath();
    const warmEnv = {
      ...process.env,
      HOME: warmStateDir,
      CARAPACE_STATE_DIR: warmStateDir,
    };
    try {
      installStateRuntime();
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-voice-call-doctor-"));
  ...
  it("reports top-level and per-number session-store agents", () => {
    ...
    expect(
      resolveSessionStoreAgentIds({
        cfg: {
          plugins: { entries: { "@carapace/voice-call": { config: {} } } },
        },
      }),
    ).toEqual(["main"]);
    expect(
  ...
  it("honors CARAPACE_STATE_DIR for the default store", async () => {
  ...
  it("keeps literal $ patterns in home when resolving a tilde-configured store", async () => {
    ...
    const dollarEnv = { ...process.env, HOME: dollarHome, CARAPACE_STATE_DIR: stateDir };
  ...
  it("repairs the plugin-local SQLite schema without a legacy call log", async () => {
    const databasePath = path.join(storePath, "state", "carapace.sqlite");
