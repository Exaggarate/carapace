...
export function createScopedVitestConfig(
  ...
  const externalIncludePatterns = loadPatternListFromEnv("CARAPACE_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeCarapaceRuntimeSetup === false ? [] : ["test/setup-carapace-runtime.ts"]),
    ]),
