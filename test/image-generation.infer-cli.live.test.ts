...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.CARAPACE_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through carapace infer", () => {
    const outputBase = path.join(os.tmpdir(), `carapace-infer-image-${process.pid}.png`);
