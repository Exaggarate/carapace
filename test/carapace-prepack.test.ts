// Carapace prepack tests validate package prepack output.
...
import {
  ...
  runPrepackCommand,
} from "../scripts/carapace-prepack.ts";
...
function createBundledChannelSmokeFixture(
  ...
  const rootDir = tempDirs.make("carapace-prepack-standalone-smoke-");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      name: "@carapace/fixture-channel",
      carapace: { channel: true, extensions: ["./index.ts"] },
    })}\n`,
  );
...
function createPreparedPrepackFixture(entrySource: string) {
  ...
  const sourceFiles = {
    "package.json": '{"name":"carapace","version":"2026.8.1","type":"module","files":["dist"]}\n',
...
function createPrepackLifecycleFixture() {
  ...
  Object.assign(packageJson, {
    ...
    files: ["dist", "docs/docs_map.md", "CHANGELOG.md", ".carapace-lifecycle-pending"],
...
const owner = process.argv[2] === "prepack"
  ? ${JSON.stringify(path.resolve("scripts/carapace-prepack.ts"))}
  : ${JSON.stringify(path.resolve("scripts/carapace-postpack.mjs"))};
...
`,
  ...
  const pack = (prepared: boolean) => {
    ...
    delete env.CARAPACE_PREPACK_PREPARED;
    if (prepared) {
      env.CARAPACE_PREPACK_PREPARED = "1";
    }
...
function runStandaloneBundledChannelSmoke(
  ...
  if (layout === "installed-path") {
    const installedRoot = path.join(rootDir, "node_modules", "carapace");
  ...
  delete env.CARAPACE_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT;
  if (layout === "installed-env") {
    env.CARAPACE_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT = "1";
  }
    ...
    ({ layout, outcome }) => {
      const entrySource = `
        ...
        if (${layout !== "installed-env"}) {
          ...
          assert.ok(modulePath.includes("/node_modules/carapace/dist/"));
        }
        export default ${
...
describe("prepared prepack ownership", () => {
  it.each([
    ...
    async ({ invalid, incumbent }) => {
      ...
      const ownerUrl = pathToFileURL(path.resolve("scripts/carapace-prepack.ts")).href;
      const result = spawnSync(
        process.execPath,
        [
          "--import",
      ...
      expect(existsSync(path.join(rootDir, ".carapace-lifecycle-pending"))).toBe(false);
...
describe("prepack lifecycle", () => {
  it.each([true, false])("packs and restores source artifacts with prepared=%s", (prepared) => {
    ...
    expect(readPacked(".carapace-lifecycle-pending")).toBe("pending\n");
...
describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @carapace/ai", () => {
    const rootDir = tempDirs.make("carapace-source-pack-workspace-");
    ...
    const rootPackageJson = {
      ...
      name: "carapace-source-pack-regression",
      packageManager: rootPackageManager,
      version,
    };
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@carapace/ai", version }, null, 2)}\n`,
    );
    ...
    expect(packedPackageJson.dependencies?.["@carapace/ai"]).toBe(version);
    expect(packedPackageJson.bundleDependencies).toBeUndefined();
    expect(existsSync(path.join(extractDir, "package", "node_modules", "@carapace", "ai"))).toBe(
      false,
    );
    ...
    expect(collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {})).toEqual([
      "plain root packing cannot safely resolve @carapace/ai from workspace:*: pnpm rewrites the workspace dependency to an exact version without bundling the package",
      "use `node scripts/package-carapace-for-docker.mjs --allow-unreleased-changelog` for a self-contained source package; official npm release automation prepares and publishes @carapace/ai separately",
    ]);
    expect(
    ...
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        ...
        CARAPACE_OCM_WORKSPACE_DEPENDENCY_DIRS: aiDir,
      }),
    ).toEqual([]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        npm_command: "pack",
        OCM_INTERNAL_NPM_BIN: path.join(rootDir, "scripts", "ocm-npm-workspace-deps.mts"),
        CARAPACE_OCM_WORKSPACE_DEPENDENCY_DIRS: rootDir,
      }),
    ).toHaveLength(2);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
  ...
  it("omits build-only workspace dependencies from direct pnpm pack manifests", () => {
    const rootDir = tempDirs.make("carapace-direct-pack-manifest-");
    const packDir = path.join(rootDir, "pack");
    const extractDir = path.join(rootDir, "extract");
    const scriptsDir = path.join(rootDir, "scripts");
    const originalPackageJson = `${JSON.stringify(
      {
        name: "carapace-direct-pack-manifest",
        packageManager: rootPackageManager,
...
describe("resolvePrepackAllowUnreleasedChangelog", () => {
  it("requires an explicit non-publish opt-in", () => {
    for (const raw of [undefined, "", "0", "false"]) {
      expect(
        resolvePrepackAllowUnreleasedChangelog({
          CARAPACE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: raw,
        }),
      ).toBe(false);
    }
    ...
    expect(() =>
      resolvePrepackAllowUnreleasedChangelog({
        CARAPACE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: "yes",
      }),
    ).toThrow("invalid CARAPACE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: yes");
  });
});
...
describe("resolvePrepackBuildEnvironment", () => {
  it("pins one timestamp across package and Control UI builds", () => {
    ...
    expect(
      ...
      GIT_COMMIT: commit,
      CARAPACE_BUILD_TIMESTAMP: "2026-07-10T12:34:56.000Z",
    });
    expect(
      resolvePrepackBuildEnvironment(
        { CARAPACE_BUILD_TIMESTAMP: "2026-07-10T01:02:03.7Z" },
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ CARAPACE_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ CARAPACE_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ CARAPACE_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid CARAPACE_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
