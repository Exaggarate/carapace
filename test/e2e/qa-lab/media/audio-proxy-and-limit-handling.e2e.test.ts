...
function createConfig(providerBaseUrl: string, maxBytes: number): CarapaceConfig {
  return {
    models: {
      providers: {
        openai: {
...
async function createAudioContext(
  ...
  tempRoot ??= await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "carapace-audio-qa-")));
