// Package Carapace For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packCarapacePackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCaptureForTest,
  runCommandForTest,
  writePackageInventoryForDocker,
} from "../../../../scripts/package-carapace-for-docker.mts";
...
function createSelectedPluginPackageFixture() {
  const sourceDir = createPackageSourceFixture("carapace-selected-plugin-source-");
  const outputDir = tempDirs.make("carapace-selected-plugin-output-");
  const packageJson = {
    name: "carapace",
    version: "2026.8.1",
    files: [
      "dist",
      ".carapace-lifecycle-pending",
  ...
  const pluginPackage = {
    ...
    carapace: { extensions: ["./index.ts"], release: { publishToNpm: true } },
  };
  const files = {
    "package.json": JSON.stringify(packageJson),
    "extensions/demo/package.json": JSON.stringify(pluginPackage),
    "extensions/demo/carapace.plugin.json": '{"id":"demo"}',
    "extensions/demo/index.ts": "export {};",
    "dist/extensions/demo/package.json": JSON.stringify({
      ...pluginPackage,
      carapace: { ...pluginPackage.carapace, extensions: ["./index.js"] },
    }),
    "dist/extensions/demo/carapace.plugin.json": '{"id":"demo"}',
...
describe("package-carapace-for-docker", () => {
  it.each([false, true])(
    ...
    async (linkedDependencies) => {
      ...
      const selected = await packCarapacePackageForDocker(sourceDir, outputDir, {
        ...options,
        bundlePlugins: ["demo"],
      });
      const extractDir = tempDirs.make("carapace-selected-plugin-extract-");
      await tar.x({ file: selected, cwd: extractDir });
      const packedRoot = path.join(extractDir, "package");
      expect(fs.readFileSync(path.join(packedRoot, ".carapace-lifecycle-pending"), "utf8")).toBe(
        "pending\n",
      );
      ...
      expect(fs.existsSync(path.join(sourceDir, ".carapace-lifecycle-pending"))).toBe(false);

      const ordinary = await packCarapacePackageForDocker(sourceDir, outputDir, options);
      const ordinaryEntries: string[] = [];
      await tar.t({ file: ordinary, onentry: (entry) => ordinaryEntries.push(entry.path) });
      expect(ordinaryEntries).toContain("package/.carapace-lifecycle-pending");
      expect(
  ...
  it.skipIf(process.platform === "win32").each(["pnpm", "corepack"])(
    ...
    async (tool) => {
      const tempDir = tempDirs.make("carapace-package-corepack-runner-");
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("carapace-package-pnpm-runner-");
      fs.writeFileSync(
  ...
  it.runIf(process.platform === "win32")(
    ...
    async ({ signal }) => {
      const tempDir = tempDirs.make("carapace-package-pnpm-timeout-");
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "const pidPath = process.env.CARAPACE_TEST_CHILD_PID;",
      ...
      env.PATHEXT = ".CMD";
      env.npm_execpath = "";
      env.CARAPACE_TEST_CHILD_PID = childPidPath;
  ...
  it("parses package artifact output options", () => {
    expect(
      ...
      outputName: "carapace-current.tgz",
  ...
  it.each([
    ...
    await expect(
      packCarapacePackageForDocker(sourceDir, outputDir, {
        ...skipDocsMapLifecycle,
        prepareChangelog: async () => {},
        restoreChangelog: async () => {},
  ...
  it("restores selected package metadata and inventory after npm pack fails", async () => {
    ...
    expect(fs.existsSync(path.join(sourceDir, "dist/carapace-install-guard"))).toBe(false);
    expect(fs.existsSync(path.join(sourceDir, ".carapace-lifecycle-pending"))).toBe(false);
  });
  ...
  it("loads from a trusted harness checkout without installed dependencies", async () => {
    const tempRoot = tempDirs.make("carapace-package-harness-");
    const copiedFiles = [
      "scripts/package-carapace-for-docker.mts",
        ...
        (resolve, reject) => {
          const child = spawn(
            ...
            [
              "--import",
              tsxImport,
              path.join(tempRoot, "scripts/package-carapace-for-docker.mts"),
              "--invalid",
            ],
  ...
  it("writes inventory for a frozen source checkout without the trusted helper", async () => {
    const sourceDir = tempDirs.make("carapace-package-frozen-source-");
    ...
    fs.writeFileSync(path.join(sourceDir, "package.json"), '{"name":"carapace"}\n');
    fs.writeFileSync(
  ...
  it("rejects package artifact output names that escape the output directory", () => {
    for (const outputName of [
      "../carapace-current.tgz",
      "nested/carapace-current.tgz",
      "carapace-current.zip",
      ".carapace-current.tgz",
    ]) {
    ...
    expect(parseArgs(["--output-name", "carapace-current.tar.gz"]).outputName).toBe(
      "carapace-current.tar.gz",
    );
  });

  it("uses the source package build entrypoint with declaration generation", async () => {
    const sourceDir = tempDirs.make("carapace-package-build-source-");
    const calls: Array<{
      command: string;
      args: string[];
      cwd: string;
    ...
    const previousTimeout = process.env.CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.CARAPACE_RUN_NODE_SKIP_DTS_BUILD;
    const previousPackageExtensions = process.env.CARAPACE_EXTENSIONS;
    const previousDockerBuildExtensions = process.env.CARAPACE_DOCKER_BUILD_EXTENSIONS;
    const previousInternalPluginIds = process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV];
    const previousPrivateQa = process.env.CARAPACE_BUILD_PRIVATE_QA;
    process.env.CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.CARAPACE_RUN_NODE_SKIP_DTS_BUILD = "1";
    process.env.CARAPACE_EXTENSIONS = "clickclack";
    process.env.CARAPACE_DOCKER_BUILD_EXTENSIONS = "slack";
    process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV] = "msteams";
    process.env.CARAPACE_BUILD_PRIVATE_QA = "1";
    ...
    try {
      await buildPackageArtifacts(sourceDir, {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.CARAPACE_BUILD_ALL_NO_PNPM,
            packageExtensions: options.env?.CARAPACE_EXTENSIONS,
            dockerBuildExtensions: options.env?.CARAPACE_DOCKER_BUILD_EXTENSIONS,
            internalDockerBuildPluginIds: options.env?.[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV],
            privateQa: options.env?.CARAPACE_BUILD_PRIVATE_QA,
            skipDts: options.env?.CARAPACE_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.CARAPACE_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.CARAPACE_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
      for (const [envName, previousValue] of [
        ["CARAPACE_EXTENSIONS", previousPackageExtensions],
        ["CARAPACE_DOCKER_BUILD_EXTENSIONS", previousDockerBuildExtensions],
        [DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV, previousInternalPluginIds],
        ["CARAPACE_BUILD_PRIVATE_QA", previousPrivateQa],
      ] as const) {
  ...
  it("keeps root package exclusions in parity with reused private QA build inventory", async () => {
    const sourceDir = createPackageSourceFixture("carapace-package-qa-exclusions-source-");
    const outputDir = tempDirs.make("carapace-package-qa-exclusions-output-");
    const { files, version } = JSON.parse(
      fs.readFileSync(new URL("../../../../package.json", import.meta.url), "utf8"),
    ) as { files: string[]; version: string };
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      JSON.stringify({ name: "carapace", version, files }),
    );
    ...
    const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
      ...skipDocsMapLifecycle,
      prepareChangelog: async () => {},
      restoreChangelog: async () => {},
    });
    const extractDir = tempDirs.make("carapace-package-qa-exclusions-extract-");
  ...
  it("omits stale hashed dist output when frozen sources expose only their own build", async () => {
    const sourceDir = createPackageSourceFixture("carapace-package-clean-dist-source-");
    const outputDir = tempDirs.make("carapace-package-clean-dist-output-");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const sourceDir = createPackageSourceFixture("carapace-package-modes-source-");
      const outputDir = tempDirs.make("carapace-package-modes-output-");
      ...
      fs.writeFileSync(path.join(sourceDir, "carapace.mjs"), "#!/usr/bin/env node\n", {
        mode: 0o700,
      });
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
      ...
      expect(entryModes.get("package/carapace.mjs")).toBe(0o755);
      ...
      expect(fs.readdirSync(outputDir).toSorted()).toEqual(["carapace-current.tgz", "pack.json"]);
    },
  );
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "CARAPACE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-ai-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        dependencies: { "@carapace/ai": "workspace:*", "dep-a": "workspace:1.2.3" },
        devDependencies: { "@carapace/session-url-contract": "workspace:*" },
        files: ["dist"],
        name: "carapace",
        version: "2026.6.17",
      },
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@carapace", "ai");
    const aiPackageJsonPath = path.join(sourceDir, "packages", "ai", "package.json");
    const originalAiPackageJson =
      '{"name":"@carapace/ai","version":"2026.6.17","devDependencies":{"@carapace/normalization-core":"workspace:*"}}\n';
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "carapace-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
          extractAiRuntime: async (_tarballPath: string, destination: string) => {
            fs.writeFileSync(
              ...
              `${JSON.stringify({
                dependencies: {
                  "@carapace/private-runtime": "0.0.0-private",
                  "dep-a": "1.2.3",
                },
                name: "@carapace/ai",
                version: "2026.6.17",
      ...
      expect(packageJson.dependencies["@carapace/ai"]).toBe("2026.6.17");
      expect(packageJson.dependencies["@carapace/private-runtime"]).toBeUndefined();
      expect(packageJson.dependencies["dep-a"]).toBe("1.2.3");
      expect(packageJson.devDependencies?.["@carapace/session-url-contract"]).toBe("workspace:*");
      expect(packageJson.bundleDependencies).toContain("@carapace/ai");
      ...
      expect(fs.existsSync(path.join(outputDir, "carapace-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("keeps real AI runtime pack failures visible for installer diagnostics", async () => {
    const sourceDir = tempDirs.make("carapace-docker-ai-failure-source-");
    const outputDir = tempDirs.make("carapace-docker-ai-failure-output-");
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
    ...
    const aiPackageJsonPath = path.join(sourceDir, "packages", "ai", "package.json");
    const originalAiPackageJson =
      '{"name":"@carapace/ai","devDependencies":{"@carapace/normalization-core":"workspace:*"}}\n';
    fs.mkdirSync(path.join(sourceDir, "packages", "ai"), { recursive: true });
    fs.writeFileSync(aiPackageJsonPath, originalAiPackageJson);
    fs.writeFileSync(packageJsonPath, originalPackageJson);
  ...
  it("reuses the source manifest lifecycle for ignore-scripts package artifacts", async () => {
    const sourceDir = createPackageSourceFixture("carapace-docker-manifest-source-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-manifest-output-"));
    const scriptsDir = path.join(sourceDir, "scripts");
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        devDependencies: {
          "@carapace/session-url-contract": "workspace:*",
          vitest: "4.1.10",
        },
        name: "carapace",
        version: "2026.8.1",
    ...
    try {
      const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          expect(fs.readFileSync(aiPackageJsonPath, "utf8")).toBe(originalAiPackageJson);
          const packedPath = path.join(outputDir, "carapace-2026.8.1.tgz");
          fs.writeFileSync(packedPath, "package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "carapace-2026.8.1.tgz"));
      expect(fs.readFileSync(packageJsonPath, "utf8")).toBe(originalPackageJson);
      expect(fs.readFileSync(aiPackageJsonPath, "utf8")).toBe(originalAiPackageJson);
      expect(
        fs.existsSync(
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
  ...
  it("rejects incomplete AI workspace package sources", async () => {
    const cases = [
      {
        dependencies: { "@carapace/ai": "workspace:*" },
        expected: "@carapace/ai dependency requires the packages/ai workspace",
        withWorkspace: false,
      },
      {
        dependencies: {},
        expected: "root package.json must declare @carapace/ai as a dependency",
        withWorkspace: true,
      },
    ];

    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "carapace" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const outputDir = tempDirs.make("carapace-package-output-");
    const sourceDir = createPackageSourceFixture("carapace-package-source-");
    const calls: string[] = [];
    const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
      ...skipTarballModeNormalization,
      prepareBundledAiRuntime: skipBundledAiRuntime,
      ...
      runCaptureImpl: async (command: string, args: string[], cwd: string) => {
        ...
        return "carapace-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join(outputDir, "carapace-2026.5.28.tgz"));
    expect(calls).toEqual([
  ...
  it("does not touch other source artifacts when the docs-map lock fails", async () => {
    ...
    await expect(
      packCarapacePackageForDocker("/repo", "/out", {
  ...
  it.each([false, true])(
    ...
    async (firstFails) => {
      const { sourceDir, outputDir, files } = createSelectedPluginPackageFixture();
      const secondOutput = tempDirs.make("carapace-next-package-");
      const rejectedOutput = tempDirs.make("carapace-rejected-package-");
      ...
      const markerPath = path.join(sourceDir, ".carapace-lifecycle-pending");
      ...
      const first = packCarapacePackageForDocker(sourceDir, outputDir, {
        ...lifecycle,
        runCaptureImpl: async () => {
          const receipt = fs.readFileSync(receiptPath, "utf8");
          const marker = fs.readFileSync(markerPath, "utf8");
          await expect(
            packCarapacePackageForDocker(sourceDir, rejectedOutput, lifecycle),
          ...
          fs.writeFileSync(path.join(outputDir, "carapace-2026.8.1.tgz"), "package");
          return "carapace-2026.8.1.tgz\n";
        },
        restoreDocsMap: async (cwd) => {
          await restorePackageDocsMap(cwd);
          second = packCarapacePackageForDocker(sourceDir, secondOutput, {
            ...lifecycle,
            runCaptureImpl: async () => {
              markerAtCapture = fs.readFileSync(markerPath, "utf8");
              ready.resolve();
              await finishSecond.promise;
              fs.writeFileSync(path.join(secondOutput, "carapace-2026.8.1.tgz"), "package");
              return "carapace-2026.8.1.tgz\n";
            },
          });
  ...
  it("keeps the docs-map lock when changelog restoration fails", async () => {
    const sourceDir = createPackageSourceFixture("carapace-package-source-");
    const outputDir = tempDirs.make("carapace-package-restore-order-");
    const calls: string[] = [];
  ...
  it("packages Unreleased notes for explicitly non-publish stable artifacts", async () => {
    const sourceDir = createPackageSourceFixture("carapace-unreleased-package-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-unreleased-output-"));
    const sourceChangelog = [
      "# Changelog",
      "",
      "## Unreleased",
    ...
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      '{"name":"carapace","version":"2026.5.29"}\n',
    );
    fs.writeFileSync(path.join(sourceDir, "CHANGELOG.md"), sourceChangelog);
    fs.mkdirSync(path.join(sourceDir, "docs"));
    ...
    try {
      const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "carapace-2026.5.29.tgz");
          fs.writeFileSync(packedPath, "package");
          return "carapace-2026.5.29.tgz\n";
        },
      });

      expect(tarball).toBe(path.join(outputDir, "carapace-2026.5.29.tgz"));
      expect(fs.readFileSync(path.join(sourceDir, "CHANGELOG.md"), "utf8")).toBe(sourceChangelog);
  ...
  it("keeps the docs surface of a pre-map source package unchanged", async () => {
    const sourceDir = createPackageSourceFixture("carapace-frozen-package-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-frozen-output-"));
    const docsDir = path.join(sourceDir, "docs");
    fs.mkdirSync(docsDir);
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      '{"name":"carapace","version":"2026.6.33"}\n',
    );
    fs.writeFileSync(
      path.join(docsDir, "index.md"),
      '---\nsummary: "Frozen Carapace docs"\n---\n\n# Carapace\n',
    );
    ...
    try {
      const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "carapace-2026.6.33.tgz");
          fs.writeFileSync(packedPath, "frozen package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "carapace-2026.6.33.tgz"));
      expect(fs.existsSync(path.join(docsDir, "docs_map.md"))).toBe(false);
    } finally {
      fs.rmSync(sourceDir, { recursive: true, force: true });
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("packs the bundled AI runtime with isolated workspace configuration", async () => {
    const sourceDir = createPackageSourceFixture("carapace-pnpm-bundled-source-");
    const outputDir = tempDirs.make("carapace-pnpm-bundled-output-");
    ...
    const installedAi = path.join(sourceDir, "node_modules/@carapace/ai");
    fs.mkdirSync(path.join(sourceDir, "dist"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@carapace/ai", version, files: ["dist"] }),
    );
    ...
    const tarball = await withEnvAsync(
      process.platform === "win32"
        ...
        : {
            ...
            CARAPACE_TEST_GZIP_MARKER: gzipMarkerPath,
          },
      async () => {
        try {
          return await packCarapacePackageForDocker(sourceDir, outputDir, {
            ...skipDocsMapLifecycle,
            pnpmPack: true,
            prepareChangelog: async () => {},
            restoreChangelog: async () => {},
    ...
    const extracted = tempDirs.make("carapace-pnpm-bundled-extracted-");
    ...
    expect(
      fs.readFileSync(path.join(packedRoot, "node_modules/@carapace/ai/dist/index.js"), "utf8"),
    ).toBe("export const runtime = true;\n");
    expect(
      JSON.parse(fs.readFileSync(path.join(packedRoot, "package.json"), "utf8")),
    ).toMatchObject({
      dependencies: { "@carapace/ai": version },
      bundleDependencies: ["@carapace/ai"],
    });
    expect(
      fs.existsSync(path.join(packedRoot, "node_modules/@carapace/ai/source-only-marker")),
    ).toBe(false);
  ...
  it("normalizes npm 12 pack metadata for renamed package artifacts", async () => {
    const sourceDir = createPackageSourceFixture("carapace-docker-pack-source-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-pack-json-"));
    const packJsonPath = path.join(outputDir, "pack.json");
    const npmPackOutput = JSON.stringify({
      carapace: {
        entryCount: 15_000,
        filename: "carapace-2026.5.28.tgz",
    ...
    try {
      const tarball = await packCarapacePackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async (_command, _args, cwd, options) => {
          if (!options.stdoutFilePath) {
            fs.writeFileSync(path.join(outputDir, "carapace-2026.5.28.tgz"), "package");
            return "carapace-2026.5.28.tgz\n";
          }
      ...
      expect(tarball).toBe(path.join(outputDir, "carapace-current.tgz"));
  ...
  it("cleans receipts without obscuring runner and parse failures", async () => {
    ...
    for (const failure of ["runner", "parse"] as const) {
      for (const cleanupFails of [false, true]) {
        const outputDir = tempDirs.make(`carapace-docker-pack-${failure}-`);
        ...
        try {
          const packPromise = packCarapacePackageForDocker(sourceDir, outputDir, {
            ...skipDocsMapLifecycle,
            ...skipTarballModeNormalization,
            packJsonPath: path.join(outputDir, "pack.json"),
            prepareBundledAiRuntime: skipBundledAiRuntime,
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    ...
    for (const filename of [
      "../carapace-2026.6.17.tgz",
      "/tmp/carapace-2026.6.17.tgz",
      String.raw`C:\temp\carapace-2026.6.17.tgz`,
      "carapace-nested/evil.tgz",
      String.raw`carapace-nested\evil.tgz`,
      "carapace-C:evil.tgz",
    ]) {
      await expect(
        packCarapacePackageForDocker(sourceDir, outputDir, {
      ...
      ).rejects.toThrow("npm pack reported unsafe Carapace tarball filename");
    }
  });

  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const sourceDir = createPackageSourceFixture("carapace-package-source-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-pack-"));
    try {
      if (process.platform === "win32") {
        const nestedDir = path.join(outputDir, "carapace-nested");
        fs.mkdirSync(nestedDir);
        fs.writeFileSync(path.join(nestedDir, "evil.tgz"), "");
      } else {
        fs.writeFileSync(path.join(outputDir, "carapace-C:evil.tgz"), "");
        fs.writeFileSync(path.join(outputDir, String.raw`carapace-nested\evil.tgz`), "");
      }
      await expect(
        packCarapacePackageForDocker(sourceDir, outputDir, {
          ...skipDocsMapLifecycle,
      ...
      ).rejects.toThrow("missing packed Carapace tarball");
      ...
      await expect(
        packCarapacePackageForDocker(sourceDir, outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "carapace-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "carapace-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const sourceDir = createPackageSourceFixture("carapace-package-source-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "carapace-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packCarapacePackageForDocker(sourceDir, outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "carapace-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "carapace-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "carapace-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "carapace-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-package-timeout-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid = 0;
    try {
      const childScript = [
        "const fs = require('node:fs');",
        "process.on('SIGTERM', () => {});",
        "fs.writeFileSync(process.env.CARAPACE_TEST_CHILD_PID, String(process.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");
      const parentScript = [
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid = 0;
    try {
      const childScript = [
  ...
  it("does not fire delayed SIGKILL after a timed-out child exits during grace", async () => {
    ...
    const tempDir = tempDirs.make("carapace-package-grace-exit-");
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid = 0;
  ...
  it("copies exact stdin bytes to a file and rejects capture conflicts", async () => {
    const tempDir = tempDirs.make("carapace-package-stdout-file-");
  ...
  it("restores source artifacts before exiting after receipt-read termination", async () => {
    ...
    const sourceDir = createPackageSourceFixture("carapace-package-source-");
    const tempDir = tempDirs.make("carapace-package-receipt-signal-");
    const markerPath = path.join(tempDir, "restored");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-carapace-for-docker.mts")).href;
    const runnerScript = `
import fs from "node:fs";
...
const { packCarapacePackageForDocker } = await import(${JSON.stringify(scriptUrl)});
try {
  await packCarapacePackageForDocker(${JSON.stringify(sourceDir)}, ${JSON.stringify(tempDir)}, { packJsonPath: "result.json", normalizeTarballModes: async () => {}, prepareBundledAiRuntime: async () => async () => {}, prepareChangelog: async () => {}, prepareDocsMap: async () => {}, prepareManifest: async () => {}, restoreChangelog: async () => {}, restoreDocsMap: async () => { fs.writeFileSync(${JSON.stringify(markerPath)}, "done"); }, restoreManifest: async () => {}, runCaptureImpl: async (_command, _args, _cwd, options) => { if (options.stdoutFilePath) fs.writeFileSync(options.stdoutFilePath, '[{"filename":"carapace-2026.5.28.tgz"}]'); fs.writeFileSync(${JSON.stringify(path.join(tempDir, "carapace-2026.5.28.tgz"))}, "package"); return "carapace-2026.5.28.tgz\\n"; } });
...
`;
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-carapace-for-docker.mts")).href;
    let childPid = 0;
    let runnerPid;
