...
test("Carapace executes and controls the complete real process lifecycle", async () => {
  ...
  try {
    ...
    await expect(
      getProcessSupervisor().spawn({
        mode: "child",
        argv: ["/definitely/not/a/real-carapace-command"],
        env: { CARAPACE_CHILD_OOM_SCORE_ADJ: "0" },
        runId: missingRunId,
      }),
    ...
    const foregroundCommand =
      process.platform === "win32"
        ? `Write-Output -NoNewline ${shellQuote(shellMarker)}; Write-Output -NoNewline "|$env:CARAPACE_SHELL"`
        : `printf '%s' ${shellQuote(shellMarker)} && printf '|%s' "$CARAPACE_SHELL"`;
    ...
    const pty = await foregroundExecTool.execute("foreground-pty", {
      command: nodeEvalCommand(
        `process.stdout.write(${JSON.stringify(ptyMarker + ":")} + String(Boolean(process.stdout.isTTY)) + ":" + (process.env.CARAPACE_SHELL || ""));`,
      ),
      pty: true,
    });
