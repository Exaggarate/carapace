...
const describeOnTestbox = process.env.CARAPACE_TESTBOX === "1" ? describe : describe.skip;
...
const producerReadyMarker = "CARAPACE_QA_PRODUCER_READY";
const execFile = promisify(execFileCallback);
const producerChildSource = `
import { pathToFileURL } from "node:url";
const producerPath = process.env.CARAPACE_QA_PRODUCER_PATH;
const artifactBase = process.env.CARAPACE_QA_ARTIFACT_BASE;
const fixtureReadyPath = process.env.CARAPACE_QA_FIXTURE_READY_PATH;
const fixtureProcessPath = process.env.CARAPACE_QA_FIXTURE_PROCESS_PATH;
const fixtureRoot = process.env.CARAPACE_QA_FIXTURE_ROOT;
...
describe("Gateway SSH tunnel QA preflight", () => {
  it("records blocked evidence outside Testbox without privileged setup", async () => {
    const artifactBase = tempDirs.make("carapace-gateway-ssh-guard-");
    const evidence = await withEnvAsync({ CARAPACE_TESTBOX: undefined }, async () =>
...
function startProducer(
  ...
  const child = spawn(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", producerChildSource],
    {
      ...
      env: {
        ...process.env,
        CARAPACE_QA_ARTIFACT_BASE: artifactBase,
        CARAPACE_QA_FIXTURE_PROCESS_PATH: fixture?.processPath,
        CARAPACE_QA_FIXTURE_READY_PATH: fixture?.readyPath,
        CARAPACE_QA_FIXTURE_ROOT: fixture?.root,
        CARAPACE_QA_PRODUCER_PATH: producerPath,
      },
...
describeOnTestbox("Gateway SSH tunnel QA producer", () => {
  it("proves real forwarding, cleanup, and operator diagnostics", async () => {
    const artifactBase = tempDirs.make("carapace-gateway-ssh-evidence-");
  ...
  it("keeps overlapping and killed producers isolated from account SSH state", async () => {
    ...
    const firstArtifactBase = tempDirs.make("carapace-gateway-ssh-overlap-first-");
    const secondArtifactBase = tempDirs.make("carapace-gateway-ssh-overlap-second-");
    ...
    const killedArtifactBase = tempDirs.make("carapace-gateway-ssh-killed-");
    const killedRoot = tempDirs.make("carapace-gateway-ssh-killed-root-");
