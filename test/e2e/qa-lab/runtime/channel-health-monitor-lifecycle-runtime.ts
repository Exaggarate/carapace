...
async function captureMonitorLogs(
  ...
  // carapace-temp-dir: standalone producer removes this log root in its finally block
  const logDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-health-monitor-proof-"));
