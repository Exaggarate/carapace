...
describe("telemetry runtime boundaries", () => {
  it("writes exact QA Lab evidence for real bus telemetry and task followthrough", async () => {
    ...
    const outputDir = tempDirs.make("carapace-telemetry-runtime-", artifactRoot);
    ...
    expect(path.basename(runtime.result.outputDir)).toMatch(/^carapace-telemetry-runtime-/u);
  }, 180_000);
});
