...
const SSH_NAMESPACE_MARKER = "CARAPACE_QA_SSH_NAMESPACE";
...
async function runInSshNamespace(options: ProducerOptions): Promise<QaEvidenceSummaryJson> {
  ...
  await fs.writeFile(
    ...
    [
      "#!/bin/sh",
      ': "${CARAPACE_QA_REAL_SSH:?missing system SSH path}"',
      ': "${CARAPACE_QA_SSH_KNOWN_HOSTS:?missing isolated known-hosts path}"',
      'exec "$CARAPACE_QA_REAL_SSH" -F /dev/null -o "UserKnownHostsFile=$CARAPACE_QA_SSH_KNOWN_HOSTS" -o GlobalKnownHostsFile=/dev/null -o UpdateHostKeys=no "$@"',
      "",
    ].join("\n"),
    { mode: 0o700 },
  );
  ...
  const childSource = `
import { pathToFileURL } from "node:url";
const modulePath = process.env.CARAPACE_QA_PRODUCER_PATH;
const rawOptions = process.env.CARAPACE_QA_PRODUCER_OPTIONS;
...
export CARAPACE_TESTBOX=1
export CARAPACE_QA_REAL_SSH="$2"
export CARAPACE_QA_PRODUCER_PATH="$4"
export CARAPACE_QA_PRODUCER_OPTIONS="$5"
shift 5
exec "$@"
`;
...
export async function runGatewaySshTunnels(
  ...
  if (process.env.CARAPACE_TESTBOX !== "1") {
    return await writer.write({
      details: "Gateway SSH tunnel QA requires CARAPACE_TESTBOX=1 before privileged setup",
      durationMs: 1,
      status: "blocked",
    });
  }
  ...
  // carapace-temp-dir: normal runs remove the fixture root; the SIGKILL test tracks its injected root
  const root =
    options.fixtureRoot ?? (await fs.mkdtemp(path.join(os.tmpdir(), "carapace-gateway-ssh-")));
    ...
    const result = await withEnvAsync(
      {
        ...
        CARAPACE_QA_SSH_KNOWN_HOSTS: isolatedSshd.knownHostsPath,
