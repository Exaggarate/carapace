...
import { resolveCarapaceAgentSqlitePath } from "../../../../src/state/carapace-agent-db.paths.js";
import {
...
async function seedCompletedUsageSession(state: CarapaceTestState): Promise<{
  databasePath: string;
}> {
  const agentId = "main";
  const storePath = path.join(state.sessionsDir(agentId), "sessions.json");
  const databasePath = resolveCarapaceAgentSqlitePath({ agentId, env: state.env });
