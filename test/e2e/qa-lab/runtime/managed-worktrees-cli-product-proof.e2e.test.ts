...
type CommandResult = Awaited<ReturnType<CarapaceTestInstance["cli"]>>;
...
async function initializeRepository(root: string): Promise<{ baseCommit: string; repo: string }> {
  ...
  await fs.mkdir(path.join(repo, ".carapace"), { recursive: true });
  ...
  const setupScript = path.join(repo, ".carapace", "worktree-setup.sh");
  await fs.writeFile(
    setupScript,
    '#!/bin/sh\nset -eu\nprintf "%s\\n%s\\n" "$CARAPACE_SOURCE_TREE_PATH" "$CARAPACE_WORKTREE_PATH" > "$CARAPACE_WORKTREE_PATH/setup-marker.txt"\n',
  );
...
describe("managed worktrees child CLI product proof", () => {
  it(
    ...
    async () => {
      ...
      tempRoot = await fs.mkdtemp(path.join(canonicalTmp, "carapace-managed-worktree-cli-"));
      const { baseCommit, repo } = await initializeRepository(tempRoot);
      instance = await createCarapaceTestInstance({ name: "qa-managed-worktree-cli" });
      ...
      const expectedSnapshotRef = `refs/carapace/snapshots/${created.id}`;
