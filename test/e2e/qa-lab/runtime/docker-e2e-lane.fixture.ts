...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      CARAPACE_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.CARAPACE_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "carapace@latest",
      CARAPACE_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      CARAPACE_UPGRADE_SURVIVOR_SCENARIO:
        env.CARAPACE_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
  ...
  "update-restart-auth": {
    env: (env) => ({
      CARAPACE_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.CARAPACE_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "carapace@latest",
      CARAPACE_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.CARAPACE_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      CARAPACE_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
