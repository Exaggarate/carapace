...
import { createCarapaceCodingTools } from "../../../../src/agents/agent-tools.js";
...
function toolNames(
  ...
  options: Pick<CarapaceCodingToolsOptions, "sandbox"> = {},
) {
  return new Set(
    createCarapaceCodingTools({
      ...
      agentId: "policy",
      workspaceDir: path.join(os.tmpdir(), "carapace-tool-policy-workspace"),
      agentDir: path.join(os.tmpdir(), "carapace-tool-policy-agent"),
...
test("Carapace applies every configured tool policy as a restrictive intersection", () => {
  const profileConfig: CarapaceConfig = {
    tools: { profile: "coding" },
  };
  expectIncluded(toolNames(profileConfig), ["read", "write", "edit", "exec"], ["message"]);

  const globalConfig: CarapaceConfig = {
    tools: {
      profile: "coding",
  ...
  const providerConfig: CarapaceConfig = {
    tools: {
      ...globalConfig.tools,
      byProvider: {
        openai: {
  ...
  const agentConfig: CarapaceConfig = {
  ...
  const sandboxDir = path.join(os.tmpdir(), "carapace-tool-policy-sandbox");
  const sandbox = createAgentToolsSandboxContext({
    workspaceDir: sandboxDir,
    agentWorkspaceDir: path.join(os.tmpdir(), "carapace-tool-policy-workspace"),
    workspaceAccess: "rw",
