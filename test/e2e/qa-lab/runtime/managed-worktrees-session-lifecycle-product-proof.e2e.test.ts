...
describe("managed worktrees session-owner product proof", () => {
  it(
    ...
    async () => {
      ...
      const fixtureRoot = tempDirs.make("carapace-managed-worktree-session-", canonicalTmp);
      ...
      expect(clean.worktree).toMatchObject({
        ...
        branch: "carapace/qa-session-clean",
      ...
      const cleanSnapshotRef = `refs/carapace/snapshots/${clean.worktree.id}`;
      ...
      const dirtySnapshotRef = `refs/carapace/snapshots/${dirty.worktree.id}`;
