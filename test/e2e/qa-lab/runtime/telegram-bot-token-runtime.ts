...
type TelegramRuntimeDependencies = {
  ...
  createInstance: (
    options: Parameters<typeof createCarapaceTestInstance>[0],
...
const defaultDependencies: TelegramRuntimeDependencies = {
  acquireCredential: async (env) => {
    ...
    return await loadQaRuntimeModule().acquireQaCredentialLease({
      env,
      kind: "telegram",
      source: directCredential ? "env" : env.CARAPACE_QA_CREDENTIAL_SOURCE,
      resolveEnvPayload: () => {
        if (!directCredential) {
  ...
  createInstance: createCarapaceTestInstance,
...
export async function runTelegramBotTokenRuntime(
  ...
  const startedAt = Date.now();
  const directCredential = resolveLeasedToken(env);
  const configuredSource = env.CARAPACE_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
