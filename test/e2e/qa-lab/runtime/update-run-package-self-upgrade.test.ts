...
describe("update.run package self-upgrade producer", () => {
  it("requires an explicit destructive opt-in", () => {
    expect(resolveUpdateRunSelfUpgradePermission({})).toEqual({
      allowed: false,
      reason:
        "blocked destructive package self-upgrade; set CARAPACE_QA_ALLOW_UPDATE_RUN_SELF=1 to run",
    });
    expect(
      resolveUpdateRunSelfUpgradePermission({ CARAPACE_QA_ALLOW_UPDATE_RUN_SELF: "1" }),
  ...
  it("uses one supervised update handoff without a manual target restart", async () => {
    ...
    expect(script).toContain("systemctl --user start carapace-gateway.service");
    expect(script).toContain("CARAPACE_SYSTEMD_UNIT=carapace-gateway.service");
    ...
    expect(script).not.toContain("carapace_e2e_stop_process");
    ...
    expect(script.indexOf("carapace gateway install --force --json")).toBeLessThan(
      script.indexOf("CARAPACE_SYSTEMD_UNIT=carapace-gateway.service"),
    );
  });
  ...
  it("falls back to the exact tag clone when commit or tag objects are missing", async () => {
    ...
    expect(script).not.toContain(
      '-v "$QA_CHANNEL_FIXTURE_ROOT/checkout:/tmp/carapace-update-run-build:ro"',
    );
