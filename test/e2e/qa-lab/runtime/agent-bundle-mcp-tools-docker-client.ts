// Carapace bundle MCP tools Docker harness.
...
const probeTool = server.tool("docker_probe", "Docker Carapace MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("carapace-agent-bundle-mcp-");
