...
const DEFAULT_TARBALL_ENV = "CARAPACE_QA_RELEASE_CANDIDATE_TARBALL";
const CHECKOUT_BUILD_RESULT_PREFIX = "__CARAPACE_QA_RELEASE_CANDIDATE_TARBALL__";
...
function parseOptions(
  ...
  let artifactBase = "";
  let buildFromCheckout = env.CARAPACE_QA_RELEASE_CANDIDATE_BUILD === "1";
...
async function buildCandidateTarballFromCheckout(options: ProducerOptions) {
  ...
  const evalScript = `
    import { packCarapace } from "./scripts/e2e/parallels/package-artifact.ts";
    const artifact = await packCarapace({ destination: ${JSON.stringify(destination)} });
