// Carapace runtime test setup installs runtime mocks and cleanup.
...
const WORKER_RUNTIME_STATE = Symbol.for("carapace.testSetupRuntimeState");
const WORKER_CLEANUP_HELPERS = Symbol.for("carapace.testSetupCleanupHelpers");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: CarapaceConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: CarapaceConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
