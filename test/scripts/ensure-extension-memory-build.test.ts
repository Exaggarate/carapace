...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "carapace-extension-memory-build-"));
  tempRoots.push(root);
...
describe("ensure-extension-memory-build", () => {
  it.each([
    ...
    writeFixture(root, "extensions/internal/carapace.plugin.json", '{"id":"internal"}');
  ...
  it("requires all expected bundled entries by default even when local output exists", () => {
    ...
    for (const id of ["internal-a", "internal-b", "external"]) {
      writeFixture(root, `extensions/${id}/carapace.plugin.json`, JSON.stringify({ id }));
      writeFixture(root, `extensions/${id}/index.ts`);
    }
    writeFixture(
      root,
      "extensions/external/package.json",
      JSON.stringify({ carapace: { build: { bundledDist: false } } }),
    );
  ...
  it("uses the configured extension memory build timeout", () => {
    ...
    ensureExtensionMemoryBuild({
      rootDir: root,
      env: { CARAPACE_EXTENSION_MEMORY_BUILD_TIMEOUT_MS: "1234" },
      requiredExtensionIds: ["discord"],
      spawnSync: (command, args, options) => {
        calls.push({ command, args, options });
        return { status: 0 };
...
describe("resolveExtensionMemoryBuildTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(
      resolveExtensionMemoryBuildTimeoutMs({ CARAPACE_EXTENSION_MEMORY_BUILD_TIMEOUT_MS: "" }),
    ).toBe(10 * 60 * 1000);
    expect(
      resolveExtensionMemoryBuildTimeoutMs({ CARAPACE_EXTENSION_MEMORY_BUILD_TIMEOUT_MS: "4321" }),
    ).toBe(4321);

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolveExtensionMemoryBuildTimeoutMs({
          CARAPACE_EXTENSION_MEMORY_BUILD_TIMEOUT_MS: raw,
        }),
      ).toThrow(`invalid CARAPACE_EXTENSION_MEMORY_BUILD_TIMEOUT_MS: ${raw}`);
    }
  });
});
