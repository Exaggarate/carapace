...
describe("scripts/test-live-codex-harness-docker.sh", () => {
  it("retains the Codex auth, isolation, forwarding, and diagnostic contracts", () => {
    ...
    for (const required of [
      ...
      "CARAPACE_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image",
      "If this is a Testbox/API-key run, set CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key and run through carapace-testbox-env.",
      ...
      "ERROR: Codex auth cannot extract accountId from the available token; refresh CARAPACE_CODEX_AUTH_JSON or use CARAPACE_LIVE_CODEX_HARNESS_AUTH=api-key.",
    ...
    expect(authHelper).toContain("carapace_live_is_ci");
    ...
    for (const [before, after] of [
      ["requires ~/.codex/auth.json before building", 'CARAPACE_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR"'],
      ["OPENAI_API_KEY=%s", "CODEX_API_KEY=%s"],
      ['PROFILE_STATUS="api-key-env"', "carapace_live_append_array DOCKER_RUN_ARGS PROFILE_MOUNT"],
  ...
  it("keeps the staged Gateway and Codex plugin on the same source module graph", () => {
    ...
    const selection = script
      .split('carapace_live_link_runtime_tree "$tmp_dir"\n')[1]
      ?.split('carapace_live_stage_state_dir "$tmp_dir/.carapace-state"')[0];
    expect(selection).toBeDefined();

    const root = fs.mkdtempSync(
      path.join(process.env.TMPDIR ?? "/tmp", "carapace-codex-plugin-roots-"),
    );
    try {
      ...
      fs.writeFileSync(path.join(stagedPlugin, "carapace.plugin.json"), "{}");
      ...
      const result = spawnSync(
        "bash",
        ["-c", `${executableSelection}\nprintf '%s' "$CARAPACE_BUNDLED_PLUGINS_DIR"`],
        {
          encoding: "utf8",
  ...
  it("rejects invalid setup timeout values before auth or Docker setup", () => {
    const result = spawnSync("bash", [SCRIPT_PATH], {
      ...
      env: {
        ...process.env,
        CARAPACE_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS: "180s",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain(
      "invalid CARAPACE_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS: 180s",
    );
