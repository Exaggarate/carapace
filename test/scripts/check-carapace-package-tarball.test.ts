// Check Carapace Package Tarball tests cover check carapace package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-carapace-package-tarball.mts";
const PUBLIC_CHECK_SCRIPT = "scripts/check-carapace-package-tarball.mjs";
...
function checkCraftedTarball(
  ...
  const root = tempDirs.make("carapace-package-tarball-crafted-");
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "carapace-package-tarball-test-"));
  try {
    ...
    const controlUiFiles =
      options.includeControlUi === false
        ? {}
        : {
            "dist/control-ui/index.html": "<!doctype html><carapace-app></carapace-app>",
      ...
      (options.includeShrinkwrap ?? declaredFiles.includes("npm-shrinkwrap.json"))
        ? {
            "npm-shrinkwrap.json": `${JSON.stringify({
              ...
              version,
              lockfileVersion: 3,
              packages: { "": { name: "carapace", version } },
            })}\n`,
          }
        : {};
    const tarFiles = {
    ...
    const tarball = options.pnpmPack
      ? join(root, `carapace-${version}.tgz`)
      : join(root, process.platform === "win32" ? "carapace.tgz" : "carapace:local.tgz");
...
function checkTarball({
  ...
  withTarball(
    ...
    (tarball) => {
      ...
      if (successText) {
        expect(result.stdout).toContain("Carapace package tarball integrity passed.");
      }
    },
    version,
    options,
...
describe("check-carapace-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    const result = spawnSync("node", [PUBLIC_CHECK_SCRIPT, "--help"], { encoding: "utf8" });

    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain(
      "Usage: node scripts/check-carapace-package-tarball.mjs [--require-bundled-workspace-deps] <carapace.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown Carapace package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("Carapace package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "carapace.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected Carapace package tarball check argument: extra");
    expect(extra.stderr).not.toContain("Carapace package tarball does not exist");
  });
      ...
      [
        {
          ...
          type: "File",
          mode: 0o600,
          body: '{"name":"carapace","version":"2026.9.4"}\n',
        },
      ],
    ...
    withTarball(
      ...
      (tarball, root) => {
        ...
        const result = spawnSync(process.execPath, [resolve(CHECK_SCRIPT), tarball], {
          ...
          env: {
            ...process.env,
            CARAPACE_TEST_LIFECYCLE_MARKER: marker,
          },
        });
      ...
      "2026.9.4",
      {
        packageJson: {
          scripts: {
            prepack:
              "node -e \"require('node:fs').writeFileSync(process.env.CARAPACE_TEST_LIFECYCLE_MARKER, 'ran')\"",
            prepare:
              "node -e \"require('node:fs').writeFileSync(process.env.CARAPACE_TEST_LIFECYCLE_MARKER, 'ran')\"",
          },
        },
  ...
  it("rejects an archive package.json symlink before changing its external target", () => {
    const root = tempDirs.make("carapace-package-tarball-link-");
    const externalRoot = tempDirs.make("carapace-package-link-target-");
    const externalManifestPath = join(externalRoot, "package.json");
    const capturePath = join(root, "npm-pack-capture.json");
    const originalBytes = Buffer.from(
      '{"name":"carapace","version":"2026.9.4","scripts":{"prepack":"exit 99"}}\n',
    );
...
childProcess.spawnSync = function patchedSpawnSync(...callArgs) {
  ...
  if (Array.isArray(args) && args.includes("pack") && args.includes("--dry-run")) {
    fs.writeFileSync(process.env.CARAPACE_TEST_NPM_CAPTURE, "called");
...
`,
    ...
    const result = spawnSync(process.execPath, [resolve(CHECK_SCRIPT), tarball], {
      ...
      env: {
        ...
        NODE_OPTIONS: [process.env.NODE_OPTIONS, `--import=${pathToFileURL(preload).href}`]
          .filter(Boolean)
          .join(" "),
        CARAPACE_TEST_NPM_CAPTURE: capturePath,
      },
    });
  ...
  it.each([
    ["bundled plugin manifest", "dist/extensions/example/carapace.plugin.json", "{}\n"],
  ...
  it("rejects package .npmrc without loading its external log policy", () => {
    ...
    const externalLogsDir = tempDirs.make("carapace-npmrc-logs-");
  ...
  it("accepts package-less extension roots without metadata-declared assets", () => {
    ...
    const extensionManifest = "dist/extensions/example/carapace.plugin.json";
    checkTarball({
  ...
  const packageContractCases: NamedTarballCheck[] = [
    ...
    },
    {
      ...
      version: "2026.6.11",
      options: { packageJson: { dependencies: { "@carapace/ai": "workspace:*" } } },
      status: "nonzero",
      stderr: [
        "package.json dependencies.@carapace/ai must not use workspace protocol workspace:*",
      ],
    },
    {
  ...
  const bundledRuntimeCases: NamedTarballCheck[] = [
    ...
    },
    {
      ...
      version: "2026.6.11",
      options: { packageJson: { dependencies: { "@carapace/ai": "2026.6.11" } } },
      status: 0,
      successText: true,
    },
    {
      ...
      status: "nonzero",
      stderr: [
        "package.json dependencies.@carapace/ai must be listed in bundleDependencies because it is private to the Carapace workspace",
        "package.json dependencies.@carapace/ai must be bundled in node_modules/@carapace/ai",
      ],
    },
    {
      ...
      files: {
        "dist/index.js": "export {};\n",
        "node_modules/@carapace/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
      },
      version: "2026.6.11",
      options: {
        packageJson: {
          dependencies: { "@carapace/ai": "2026.6.11" },
      ...
      status: "nonzero",
      stderr: [
        "bundled @carapace/ai is missing required runtime entry dist/index.mjs",
        "bundled @carapace/ai is missing required runtime entry dist/providers.mjs",
        "bundled @carapace/ai is missing required runtime entry dist/internal/runtime.mjs",
      ],
    },
    {
      ...
      files: {
        "dist/index.js": "export {};\n",
        "node_modules/@carapace/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
        "node_modules/@carapace/ai/dist/index.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/transports.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/internal/openai-responses-payload-policy.mjs":
          "export {};\n",
        "node_modules/@carapace/ai/dist/internal/runtime.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/internal/tool-schema.mjs": "export {};\n",
      },
      version: "2026.6.11",
      options: {
        packageJson: {
    ...
    },
    {
      ...
      files: {
        "dist/index.js": "export {};\n",
        "node_modules/@carapace/ai/package.json": LEGACY_AI_RUNTIME_PACKAGE_JSON,
        "node_modules/@carapace/ai/dist/index.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@carapace/ai/dist/internal/runtime.mjs": "export {};\n",
      },
      ...
      options: {
        packageJson: {
          dependencies: { "@carapace/ai": "2026.7.2-beta.4" },
          bundleDependencies: ["@carapace/ai"],
        },
      },
      strict: true,
    ...
    ...["providers", "internal/tool-schema"].map((missingEntry): NamedTarballCheck => ({
      ...
      files: {
        ...
        ...Object.fromEntries(
          ["providers", "internal/tool-schema"]
            ...
            .map((entry) => [`node_modules/@carapace/ai/dist/${entry}.mjs`, "export {};\n"]),
        ),
      },
      version: "2026.6.11",
      options: {
      ...
      strict: true,
      status: "nonzero",
      stderr: [`bundled @carapace/ai is missing required runtime entry dist/${missingEntry}.mjs`],
    })),
    {
      name: "rejects bundled AI entries that its manifest does not export",
      files: {
        "dist/index.js": "export {};\n",
        "node_modules/@carapace/ai/package.json": JSON.stringify({
          name: "@carapace/ai",
          version: "2026.6.11",
          exports: {
            ".": "./dist/index.mjs",
      ...
      strict: true,
      status: "nonzero",
      stderr: ["bundled @carapace/ai runtime specifier @carapace/ai/providers is not resolvable"],
    },
    {
      ...
      files: {
        ...
        "node_modules/@carapace/ai/dist/internal/openai-responses-payload-policy.mjs":
          "export {};\n",
        "node_modules/@carapace/ai/dist/internal/runtime.mjs": 'export * from "./missing.mjs";\n',
        "node_modules/@carapace/ai/dist/internal/tool-schema.mjs": "export {};\n",
      },
      version: "2026.6.11",
      options: {
      ...
      status: "nonzero",
      stderr: [
        "bundled @carapace/ai dist/internal/runtime.mjs imports missing dist/internal/missing.mjs",
      ],
    },
    {
