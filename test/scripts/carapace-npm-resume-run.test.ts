...
import {
  resolveCarapaceNpmResumeRun,
  runCarapaceNpmResumeGh,
  validateCarapaceNpmResumeRun,
} from "../../scripts/carapace-npm-resume-run.mts";
import type { CarapaceNpmResumeValidationInput } from "../../scripts/carapace-npm-resume-run.mts";

const SHA = "a".repeat(40);
const TAG_OBJECT_SHA = "b".repeat(40);
const BRANCH = `release-publish/${SHA.slice(0, 12)}-123`;
const URL = "https://github.com/Exaggarate/carapace/actions/runs/456";

function fixture(
  overrides: Partial<CarapaceNpmResumeValidationInput> = {},
): CarapaceNpmResumeValidationInput {
  return {
...
describe("carapace npm resume run identity", () => {
  it("bounds each GitHub lookup", () => {
    const execFileSyncImpl = vi.fn(() => "result");

    expect(
      runCarapaceNpmResumeGh(["api", "repos/carapace/carapace/actions/runs/456"], {
        execFileSyncImpl,
      }),
    ).toBe("result");
    expect(execFileSyncImpl).toHaveBeenCalledWith(
      "gh",
      ["api", "repos/carapace/carapace/actions/runs/456"],
      {
        encoding: "utf8",
        killSignal: "SIGKILL",
  ...
  it("accepts a successful run bound to a signed main-reachable tooling tag", () => {
    expect(validateCarapaceNpmResumeRun(fixture())).toEqual({
  ...
  it.each([
    ...
    expect(() => validateCarapaceNpmResumeRun(fixture(overrides))).toThrow(message);
  });

  it("loads the exact run, workflow, signed tag, ancestry, and approval job", () => {
    const responses = new Map<string, unknown>([
      [`api repos/carapace/carapace/actions/runs/456 --method GET`, fixture().run],
      [
        `api repos/carapace/carapace/actions/workflows/carapace-npm-release.yml --method GET`,
        { id: 101 },
      ],
      [`api repos/carapace/carapace/git/ref/tags/${BRANCH} --method GET`, fixture().tagRef],
      [`api repos/carapace/carapace/git/tags/${TAG_OBJECT_SHA} --method GET`, fixture().tag],
      [`api repos/carapace/carapace/compare/${SHA}...main --method GET`, { status: "identical" }],
      [`run view 456 --repo carapace/carapace --json jobs --jq .jobs`, fixture().jobs],
    ]);
  ...
  it("loads a lightweight protected tag without requiring tag metadata or main ancestry", () => {
    ...
    const responses = new Map<string, unknown>([
      [`api repos/carapace/carapace/actions/runs/456 --method GET`, lightweight.run],
      [
        `api repos/carapace/carapace/actions/workflows/carapace-npm-release.yml --method GET`,
        { id: 101 },
      ],
      [`api repos/carapace/carapace/git/ref/tags/${BRANCH} --method GET`, lightweight.tagRef],
      [`run view 456 --repo carapace/carapace --json jobs --jq .jobs`, lightweight.jobs],
    ]);
    const runGh = vi.fn((args: string[]) => {
      const response = responses.get(args.join(" "));
      if (!response) {
