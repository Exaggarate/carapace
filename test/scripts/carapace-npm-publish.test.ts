// Carapace NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/carapace-npm-publish.sh";
...
function makeReleaseCheckout(root: string, version: string): string {
  ...
  copyFileSync(
    "scripts/carapace-npm-extended-stable-release.mjs",
    path.join(checkout, "scripts/carapace-npm-extended-stable-release.mjs"),
  );
...
function makePackageTarball(root: string, packageJson?: string): string {
  ...
  const tarball = path.join(root, "carapace.tgz");
...
describe("carapace npm publish wrapper", () => {
  it("prints help without resolving release metadata", () => {
    const result = runPublishWrapper(["--help"]);

    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe(
      "usage: bash scripts/carapace-npm-publish.sh --publish [package.tgz]",
    );
  ...
  it("rejects extra publish arguments before npm publish", () => {
    const tempRoot = tempDirs.make("carapace-npm-publish-");
    const tarball = path.join(tempRoot, "carapace.tgz");
  ...
  it.each(["beta", "latest"])("publishes the prepared tarball to the %s dist-tag", (distTag) => {
    ...
    const result = runPublishWrapper(
      ["--publish", tarball],
      {
        CARAPACE_NPM_PUBLISH_TAG: distTag,
  ...
  it.each([
    ...
    ["missing version", JSON.stringify({ name: "carapace" }), "has no valid version"],
  ...
  it("rejects publishing the current pre-.33 final version to extended-stable", () => {
    ...
    const result = runPublishWrapper(
      ["--publish"],
      {
        CARAPACE_NPM_PUBLISH_TAG: "extended-stable",
      },
      checkout,
    );
  ...
  it("rejects unknown requested dist-tags instead of falling back to beta", () => {
    const result = runPublishWrapper(["--publish"], {
      CARAPACE_NPM_PUBLISH_TAG: "nightly",
    });
