  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      entries: packages.map((entry) => ({
        ...
        source: "official",
        carapace: { install: { npmSpec: entry.manifest.name } },
      })),
    })}\n`,
  );
...
describe("npm placeholder publication", () => {
  it("preserves selected multi-package order and binds unique release-enabled manifests", async () => {
    const names = ["@carapace/zoom-meetings", "@carapace/comfy-provider"] as const;
  ...
  it("creates deterministic canonical two-file placeholder tarballs", () => {
    const first = createPlaceholderTarball("@carapace/comfy-provider");
    const second = createPlaceholderTarball("@carapace/comfy-provider");
  ...
  it("requires three stable registry observations during planning", async () => {
    const packageName = "@carapace/comfy-provider";
  ...
  it("classifies E404, existing-version backfill, and exact idempotent reruns", () => {
    const packageName = "@carapace/meta-provider";
  ...
  it("publishes serially and preserves every non-placeholder dist-tag", async () => {
    const names = ["@carapace/byteplus-provider", "@carapace/meta-provider"] as const;
    const root = createRepo([
  ...
  it("rejects malicious package input, duplicate identity, and unsafe manifest paths", () => {
    expect(() => parseSelectedPackages("@carapace/good,../../evil")).toThrow(
      "Invalid Carapace package name",
    );
    expect(() => parseSelectedPackages("@carapace/good,@carapace/good")).toThrow("duplicates");

    const duplicateRoot = createRepo([
      { dir: "one", manifest: packageJson("@carapace/good") },
      { dir: "two", manifest: packageJson("@carapace/good") },
    ]);
    expect(() => resolveSelectedPackageSources(duplicateRoot, ["@carapace/good"])).toThrow(
      "must map uniquely",
    );

    const unsafeRoot = createRepo([{ dir: "good", manifest: packageJson("@carapace/good") }]);
    writeFileSync(join(unsafeRoot, "extensions", "good", "package.json"), "{}\n");
    expect(() => resolveSelectedPackageSources(unsafeRoot, ["@carapace/good"])).toThrow(
      "must map uniquely",
    );
    ...
    const privateRoot = createRepo([
      {
        dir: "private",
        manifest: {
          ...packageJson("@carapace/private"),
          publishConfig: { access: "private" },
        },
      },
    ]);
    expect(() => resolveSelectedPackageSources(privateRoot, ["@carapace/private"])).toThrow(
  ...
  it("fails final verification when any pre-existing dist-tag changes", () => {
    ...
    const entry = {
      ...
      tarball: {
        name: "carapace-meta-provider-0.0.0.tgz",
        sha256: "d".repeat(64),
        sizeBytes: 1,
        ...expected,
      },
