...
function installerTempFiles(dir: string) {
  return readdirSync(dir).filter((entry) => entry.startsWith("carapace-installer-"));
}
...
describe("cross-OS installer fetch", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("carapace-cross-os-installer-");
      ...
      const server = createServer((request, response) => {
        ...
        if (request.url === "/healthy") {
          response.end('printf "%s" "$1|$2|$3" > "$CARAPACE_PROOF_HEALTHY_MARKER"\n');
          return;
        }
        response.write('printf executed > "$CARAPACE_PROOF_STALL_MARKER"\n');
      });
      ...
      try {
        ...
        const env = {
          ...process.env,
          TMPDIR: dir,
          CARAPACE_PROOF_HEALTHY_MARKER: healthyMarker,
          CARAPACE_PROOF_STALL_MARKER: stalledMarker,
        };
