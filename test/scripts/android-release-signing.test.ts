...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `CARAPACE_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "CARAPACE_ANDROID_KEY_ALIAS=carapace-upload",
      `CARAPACE_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  it("runs from an isolated copy without node_modules", () => {
    const isolatedRoot = tempRoots.make("carapace-android-signing-");
  ...
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:carapace/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/carapace");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      const tempRoot = tempRoots.make("carapace-android-signing-");
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "carapace", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `CARAPACE_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(`CARAPACE_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`);
      expect(materializedProperties).toContain("CARAPACE_ANDROID_KEY_ALIAS=carapace-upload");
      expect(materializedProperties).toContain(`CARAPACE_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
