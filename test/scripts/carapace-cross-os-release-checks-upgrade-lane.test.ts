...
vi.mock("../../scripts/lib/cross-os-release-checks/installed.ts", async (importOriginal) => ({
  ...
  runInstalledCli: mocks.runCarapace,
...
const candidate = {
  candidateTgz: "/tmp/carapace-candidate.tgz",
  candidateVersion: "2026.8.28-beta.1",
  candidateFileName: "carapace-candidate.tgz",
...
function upgradeParams() {
  return {
    baselineSpec: "carapace@2026.7.1",
    baselineTgz: "",
    build: candidate,
    candidateUrl: "http://127.0.0.1:49951/carapace-candidate.tgz",
    ref: "main",
...
function arrangeSuccessfulLane() {
  ...
  mocks.runCarapace.mockResolvedValue({ exitCode: 0, stdout: "{}", stderr: "" });
  ...
  mocks.verifyFreshShellCommand.mockResolvedValue({ cliPath: join(logsDir, "carapace") });
  mocks.ensureDevUpdateGitInstall.mockResolvedValue({ cliPath: join(logsDir, "carapace") });
...
describe("cross-OS manual gateway lane evidence", () => {
  beforeEach(() => {
    logsDir = createTempDir("carapace-upgrade-lane-test-");
  ...
  it.each([
    ...
    async (outcome, publishedVersion) => {
      ...
      mocks.runCarapace.mockImplementation(async ({ args }: { args: string[] }) => {
        if (args[0] === "update") {
          return {
      ...
      if (outcome === "pass") {
        ...
        expect(
          mocks.runCarapace.mock.calls.filter(([call]) => call.args[0] === "agent"),
        ).toHaveLength(2);
      } else {
        expect(result).toMatchObject({
          status: "fail",
      ...
      expect(
        mocks.runCarapace.mock.calls.filter(([call]) => call.args[0] === "update"),
      ).toHaveLength(1);
      expect(mocks.runCarapace.mock.calls.some(([call]) => call.args[0] === "doctor")).toBe(false);
  ...
  it("records bounded evidence when the supported Windows timeout fallback succeeds", async () => {
    ...
    mocks.runCarapace.mockImplementationOnce(async ({ env }) => {
      ...
      throw new Error(
        "Command timed out: C:\\prefix\\node_modules\\carapace\\carapace.mjs update --tag http://127.0.0.1:49951/carapace-candidate.tgz --yes --json --no-restart --timeout 600",
      );
    });
  ...
  it("retains sanitized updater timings when a later upgrade phase fails", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    arrangeSuccessfulLane();
    mocks.runCarapace.mockResolvedValue({
      exitCode: 0,
      stdout: JSON.stringify({
        durationMs: 622_000,
        root: String.raw`C:\npm-updater-private-fixture\carapace`,
        steps: [
          {
            name: "global update",
