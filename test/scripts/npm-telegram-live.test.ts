...
function mkTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "carapace-npm-telegram-live-"));
  tempRoots.push(root);
  return root;
}
...
function runHotpathCandidate(consentSupported: boolean) {
  ...
  const hotpath = script.slice(
    script.indexOf('if [ "${CARAPACE_NPM_TELEGRAM_SKIP_HOTPATH:-0}" != "1" ]'),
    script.indexOf('\nexport CARAPACE_NPM_TELEGRAM_SUT_COMMAND="$sut_command"'),
  );
  writeFileSync(
    path.join(root, "carapace"),
    `#!/usr/bin/env bash
...
sut_command="$FIXTURE_ROOT/carapace"
${hotpath}`,
    ],
    {
...
describe("package Telegram live Docker E2E", () => {
  it("forwards npm-specific credential aliases through the Docker boundary", () => {
    ...
    for (const contract of [
      "CARAPACE_NPM_TELEGRAM_CREDENTIAL_SOURCE",
      "CARAPACE_NPM_TELEGRAM_CREDENTIAL_ROLE",
  ...
  it("installs the package candidate before forwarding runtime secrets", () => {
    ...
    expect(installRun).not.toContain(
      "running package install without CARAPACE_E2E_NPM_INSTALL_TIMEOUT",
    );
  ...
  it("uses unique direct-run output dirs by default", () => {
    ...
    expect(
      testing.resolvePackageTelegramOutputDir(
        { CARAPACE_NPM_TELEGRAM_OUTPUT_DIR: ".artifacts/custom" },
        repoRoot,
      ),
    ).toBe(".artifacts/custom");
  });

  it("keeps the installed Carapace command as the package SUT", async () => {
    const prefix = mkTempRoot();
    const command = path.join(prefix, "bin", "carapace");
    const harnessCommand = path.join(mkTempRoot(), "bin", "carapace");
    ...
    await expect(
      testing.resolveTrustedCarapaceCommand(command, {
    ...
    await expect(
      testing.resolveTrustedCarapaceCommand(harnessCommand, {
        NPM_CONFIG_PREFIX: prefix,
      }),
    ).rejects.toThrow("CARAPACE_NPM_TELEGRAM_SUT_COMMAND must resolve inside NPM_CONFIG_PREFIX.");
  });
  ...
  it("requires Convex leases instead of static Telegram credentials", () => {
    ...
    expect(script).not.toContain("CARAPACE_QA_TELEGRAM_DRIVER_BOT_TOKEN");
    expect(script).not.toContain("CARAPACE_QA_TELEGRAM_SUT_BOT_TOKEN");
  });
  ...
  it("mounts configured output paths before entering the container", () => {
    ...
    expect(dockerEnv).not.toContain('-e CARAPACE_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR_CONTAINER"');
    expect(dockerEnv).not.toContain('-e CARAPACE_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"');
    expect(script).toContain('-v "$OUTPUT_DIR_HOST:$OUTPUT_DIR_CONTAINER"');
  });

  it("uses the container temp root for Carapace runtime scratch files", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    const dockerEnvStart = script.indexOf("docker_env=(");
    const dockerEnvEnd = script.indexOf(")\n\nforward_env_if_set", dockerEnvStart);
    const dockerEnv = script.slice(dockerEnvStart, dockerEnvEnd);
  ...
  it("isolates the trusted private QA harness from the installed package candidate", () => {
    ...
    expect(script).toContain("link_harness_dependency /app carapace");
    expect(script).not.toContain('carapace_package_dir="/npm-global/lib/node_modules/carapace"');
    expect(script).not.toContain('cp "$carapace_package_dir/package.json" /app/package.json');
    expect(script).not.toContain("/app/node_modules/carapace/package.json");
  ...
  it.each([false, true])(
    ...
    (supported) => {
      ...
      expect(calls.filter((call) => call.startsWith("plugins install @carapace/codex"))).toEqual(
        supported ? ["plugins install @carapace/codex --accept-capabilities"] : [],
      );
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      testing.resolveCredentialSource({
        CARAPACE_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        CARAPACE_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      testing.resolveCredentialRole({
        CARAPACE_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
        CARAPACE_QA_CREDENTIAL_ROLE: "maintainer",
      }),
    ).toBe("ci");
  });
  ...
  it("maps repeated RTT env onto package Telegram live options", () => {
    expect(
      testing.resolveRttOptions({
        CARAPACE_NPM_TELEGRAM_RTT_SAMPLES: "7",
        CARAPACE_NPM_TELEGRAM_RTT_TIMEOUT_MS: "45000",
        CARAPACE_NPM_TELEGRAM_RTT_MAX_FAILURES: "2",
        CARAPACE_NPM_TELEGRAM_RTT_CHECKS: "channel-canary",
      }),
    ).toEqual({
      scenarioId: "channel-canary",
      count: 7,
  ...
  it("selects an explicit exact-marker RTT scenario without SCENARIOS duplication", () => {
    const env = {
      CARAPACE_NPM_TELEGRAM_RTT_CHECKS: "telegram-reply-chain-exact-marker",
    };
  ...
  it("rejects multiple explicit RTT scenario ids", () => {
    expect(() =>
      testing.resolvePackageTelegramScenarioSelection({
        CARAPACE_NPM_TELEGRAM_RTT_CHECKS: "channel-canary,telegram-reply-chain-exact-marker",
      }),
    ).toThrow("CARAPACE_NPM_TELEGRAM_RTT_CHECKS accepts at most one scenario id; got 2");
  });
  ...
  it("rejects unknown explicit RTT scenario ids through canonical selection", () => {
    expect(() =>
      testing.resolvePackageTelegramScenarios(
        {
          CARAPACE_NPM_TELEGRAM_RTT_CHECKS: "telegram-unknown-rtt-check",
        },
        (scenarioIds) => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: { CARAPACE_NPM_TELEGRAM_RTT_CHECKS: "channel-canary" },
      requested: ["telegram-status-command"],
      resolved: ["telegram-status-command"],
      expected: ["channel-canary", "telegram-status-command"],
    },
  ...
  it("rejects invalid repeated RTT env", () => {
    expect(() =>
      testing.resolveRttOptions({
        CARAPACE_NPM_TELEGRAM_RTT_SAMPLES: "7samples",
      }),
    ).toThrow("invalid CARAPACE_NPM_TELEGRAM_RTT_SAMPLES: 7samples");
  });
  ...
  it.each(["2026.6.33", "2026.7.1-beta.6", "2026.7.1", "2026.7.2-beta.2", "2026.7.2-beta.3"])(
    ...
    (packageVersion) => {
      const mutateConfig = testing.resolvePackageConfigMutation({
        CARAPACE_NPM_TELEGRAM_PACKAGE_VERSION: packageVersion,
      });
      const config = {
        agents: {
          defaults: {
  ...
  it("preserves the frozen 2026.6.35 package projection", () => {
    const mutateConfig = testing.resolvePackageConfigMutation({
      CARAPACE_NPM_TELEGRAM_PACKAGE_VERSION: "2026.6.35",
    });
    const config = {
      agents: {
        defaults: {
  ...
  it("does not read package Telegram summaries when failures are allowed", async () => {
    await expect(
      testing.shouldFailPackageTelegramRun(
        ...
        { CARAPACE_NPM_TELEGRAM_ALLOW_FAILURES: "1" },
      ),
    ).resolves.toBe(false);
  });
});
