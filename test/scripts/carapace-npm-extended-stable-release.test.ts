...
} from "../../scripts/carapace-npm-extended-stable-release.mjs";
...
describe("npm preflight publication channels", () => {
  ...
  it("allows only qualified regular releases with both SDK receipts to cross beta/latest", () => {
    const receipt = { schema: "carapace.plugin-sdk-api-release-evidence/v1" };
    const manifest = {
      version: 3,
      packageVersion: "2026.8.1",
      npmDistTag: "beta",
      pluginSdkApi: {
        schema: "carapace.plugin-sdk-api-release-evidence-set/v1",
    ...
    const command = spawnSync(
      process.execPath,
      ["scripts/carapace-npm-extended-stable-release.mjs", "verify-preflight-channel"],
      {
        encoding: "utf8",
...
describe("npm extended-stable publication boundary", () => {
  ...
  it("prints exactly channel then publish tag from the dependency-free CLI", () => {
    const result = spawnSync(
      process.execPath,
      ["scripts/carapace-npm-extended-stable-release.mjs", "publish-plan"],
      {
        encoding: "utf8",
        env: {
          ...process.env,
...
describe("extended-stable selector repair", () => {
  it("points the selector at the expected published version", () => {
    expect(extendedStableSelectorRepairCommand("v2026.6.33")).toBe(
      "npm dist-tag add carapace@2026.6.33 extended-stable",
    );
  });
