...
describe("session log mention scanner", () => {
  it("counts mentions across bounded session logs", async () => {
    const root = tempRoots.make("carapace-session-log-mentions-");
  ...
  it("counts mentions from SQLite transcript rows", async () => {
    ...
    const sqlitePath = path.join(root, "agents", "main", "agent", "carapace-agent.sqlite");
  ...
  it("returns zero counts when the sessions directory is absent", async () => {
    await expect(
      countSessionLogMentions({
        sessionsDir: path.join(tempRoots.make("carapace-session-log-mentions-"), "missing"),
        needles: {
          apiFileRead: "API.read",
        },
      }),
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readSessionLogMentionLimits({
        CARAPACE_SESSION_LOG_MENTION_FILE_MAX_BYTES: "1e3",
      }),
    ).toThrow("invalid CARAPACE_SESSION_LOG_MENTION_FILE_MAX_BYTES: 1e3");
    expect(() =>
      readSessionLogMentionLimits({
        CARAPACE_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid CARAPACE_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: 1000ms");
  });
});
