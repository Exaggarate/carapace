...
export function writeNpmInstallRetryFixture(path: string) {
  writeFileSync(
    ...
    [
      ...
      '    printf "#!/bin/sh\\nprintf \'2026.8.1\\\\n\'\\n" > "$NPM_FAKE_PACKAGE_DIR/carapace.mjs"',
      '    printf "#!/usr/bin/env node\\n" > "$NPM_FAKE_PACKAGE_DIR/dist/entry.js"',
      '    chmod +x "$NPM_FAKE_PACKAGE_DIR/carapace.mjs" "$NPM_FAKE_PACKAGE_DIR/dist/entry.js"',
      "  fi",
      "  exit 0",
      "fi",
...
export function writeNpmLifecycleFixture(path: string) {
  writeFileSync(
    ...
    [
      ...
      'mkdir -p "$NPM_FAKE_PACKAGE_DIR/dist"',
      'printf "#!/usr/bin/env node\\n" > "$NPM_FAKE_PACKAGE_DIR/dist/entry.js"',
      'if [[ "${NPM_FAKE_KEEP_GUARD:-0}" == "1" ]]; then : > "$NPM_FAKE_PACKAGE_DIR/.carapace-lifecycle-pending"; else rm -f "$NPM_FAKE_PACKAGE_DIR/.carapace-lifecycle-pending"; fi',
      "exit 0",
      "",
    ].join("\n"),
  );
