...
export function preparePerformanceFixture(root: string, options: PerformanceFixtureOptions) {
  ...
  const dest = "carapace-performance/main/123-1/mock-provider";
  const previous = "carapace-performance/main/100-1/mock-provider";
  const pointer = "carapace-performance/main/latest-mock-provider.json";
  ...
  if (template) {
    ...
    if (options.mode === "publish") {
      ...
      run(reports, "remote", "add", "origin", "https://github.com/Exaggarate/carapace/clawgrit-reports.git");
  ...
  if (!template && reuseDefault) {
    ...
    const templateRoot = templateDirs.make(`carapace-performance-${templateKind}-template-`);
  ...
  if (options.mode === "publish" && options.race) {
    commitReport(seed, "carapace-performance/main/200-1/mock-provider");
    run(seed, "push", remote, "HEAD:main");
  }
  ...
  write(
    path.join(input, "carapace-performance-source-123-1/mock-provider/index.md"),
    "source fixture\n",
  );
  const env = {
    ...
    GITHUB_SHA: target,
    GITHUB_WORKFLOW: "Carapace Performance",
    ...
    DEST_REL: dest,
    REPORT_URL: `https://github.com/Exaggarate/carapace/clawgrit-reports/tree/main/${dest}`,
