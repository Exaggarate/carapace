...
const AUTH_MONITOR_SERVICE_PATH = "scripts/systemd/carapace-auth-monitor.service";
const AUTH_MONITOR_TIMER_PATH = "scripts/systemd/carapace-auth-monitor.timer";
...
function createAuthMonitorHarness() {
  const home = mkdtempSync(join(tmpdir(), "carapace-auth-monitor-"));
  const binDir = join(home, "bin");
  const curlLog = join(home, "curl.log");
  const carapaceLog = join(home, "carapace.log");
  const stateFile = join(home, ".carapace", "auth-monitor-state");
  mkdirSync(binDir);
  writeFileSync(
    join(binDir, "curl"),
  ...
  return {
    ...
    enablePhoneAuth: () => {
      ...
      mkdirSync(join(home, ".carapace", "agents", "main", "agent"), { recursive: true });
      writeFileSync(
        join(home, ".claude", ".credentials.json"),
        JSON.stringify({ claudeAiOauth: { expiresAt } }),
      );
      writeFileSync(
        join(home, ".carapace", "agents", "main", "agent", "auth-profiles.json"),
        JSON.stringify({
    ...
    run: ({
      ...
      spawnSync("bash", [AUTH_MONITOR_PATH], {
        ...
        env: {
          ...
          FAKE_CARAPACE_EXIT_CODE: String(carapaceExitCode),
          FAKE_CARAPACE_LOG: carapaceLog,
          HOME: home,
...
describe("auth monitoring scripts", () => {
  it("keeps systemd install rendering free of checked-in host paths", () => {
    ...
    expect(service).toContain("ExecStart=@CARAPACE_AUTH_MONITOR_PATH@");
  ...
  it("keeps public helper scripts free of private host defaults", () => {
    ...
    expect(joined).toContain("Run on the Carapace host: ${SCRIPT_DIR}/mobile-reauth.sh");
    for (const script of TERMUX_WIDGET_PATHS.map(readScript)) {
      expect(script).toContain('SERVER="${CARAPACE_SERVER:-carapace-host}"');
    }
    expect(readScript("scripts/termux-sync-widget.sh")).toContain(
      "'$HOME/carapace/scripts/sync-claude-code-auth.sh'",
    );
  });
  ...
  it("rate-limits after any configured notification channel succeeds", () => {
    ...
    try {
      ...
      expect(throttled.stdout).toContain("Skipping notification (sent recently)");
      expect(readFileSync(harness.carapaceLog, "utf8").trim().split("\n")).toHaveLength(1);
      expect(readFileSync(harness.curlLog, "utf8").trim().split("\n")).toHaveLength(1);
    } finally {
      harness.cleanup();
    }
  ...
  it("keeps mobile reauth wired to local auth status and Claude token setup", () => {
    ...
    expect(script).toContain("systemctl --user restart carapace");
  });
});
