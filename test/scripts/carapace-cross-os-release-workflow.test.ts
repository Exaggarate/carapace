// Carapace Cross Os Release Workflow tests cover carapace cross os release workflow script behavior.
...
const WORKFLOW_PATH = ".github/workflows/carapace-cross-os-release-checks-reusable.yml";
const RELEASE_CHECKS_PATH = ".github/workflows/carapace-release-checks.yml";
const WRAPPER_PATH = "scripts/github/run-carapace-cross-os-release-checks.sh";
const SCRIPT_PATH = "scripts/carapace-cross-os-release-checks.ts";
const HARNESS = "bash workflow/scripts/github/run-carapace-cross-os-release-checks.sh";
...
describe("cross-OS release checks workflow", () => {
  ...
  it("reuses npm downloads across isolated lane homes without caching installed state", () => {
    ...
    expect(cacheRoot).toBe("${{ github.workspace }}/.cache/carapace-cross-os-npm-cache");
    expect(restore.with?.path).toBe(".cache/carapace-cross-os-npm-cache/_cacache");
    ...
    expect(restore.with?.["restore-keys"]).toContain("carapace-cross-os-npm-v1-seed-\n");
    expect(save.with?.key).toBe("${{ steps.npm_downloads.outputs.cache-primary-key }}");
    expect(save.if).toBe(
      "github.repository == 'carapace/carapace' && github.event_name == 'workflow_dispatch' && steps.npm_downloads.outputs.cache-hit != 'true'",
    );
    expect(step(consumer, "Setup Node.js").id).toBe("node");
    expect(restore.with?.key).toBe(
      "carapace-cross-os-npm-v1-${{ runner.os }}-${{ runner.arch }}-${{ steps.node.outputs.node-version }}-${{ matrix.suite }}-${{ needs.prepare.outputs.candidate_sha256 }}-${{ needs.prepare.outputs.baseline_sha256 }}",
    );
  ...
  it("retries only an interrupted Windows dashboard probe", () => {
    ...
    expect(run).toContain('"${CARAPACE_RELEASE_CHECK_OS}" != "windows"');
  ...
  it("bounds npm baseline packing during prepare", () => {
    ...
    expect(baseline.env).toMatchObject({
      CANDIDATE_JSON: "${{ runner.temp }}/carapace-cross-os-release-checks/prepare/candidate.json",
    ...
    expect(baseline.run).not.toContain("npm view carapace@latest");
  ...
  it("derives baselines from the candidate owner and passes them to every consumer", () => {
    ...
    expect(docker.with?.published_upgrade_survivor_baseline).toBe(
      "${{ format('carapace@{0}', needs.prepare_release_package.outputs.upgrade_baseline) }}",
    );
  ...
  it("binds the prepared release package to an immutable artifact and package tuple", () => {
    ...
    expect(metadata.run).toContain("file_name=carapace-current.tgz");
  ...
  it("downloads and re-exports exact candidate artifacts only by immutable id", () => {
    ...
    expect(resolve.run).toContain("resolve-carapace-package-candidate.mts");
    ...
    expect(upload.with?.name).toBe(
      "carapace-cross-os-release-checks-candidate-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(baselineUpload.with?.name).toBe(
      "carapace-cross-os-release-checks-baseline-${{ github.run_id }}-${{ github.run_attempt }}",
    );
  ...
  it("passes and consumes the immutable prerelease companion registry generically", () => {
    ...
    expect(run.run).not.toContain("CARAPACE_PLUGIN_INSTALL_OVERRIDES");
    expect(JSON.stringify(workflow)).not.toContain("@carapace/codex");
  });
  ...
  it.each([
    ...
    const result = spawnSync(
      ...
      [WRAPPER_PATH, "--resolve-provider-required-companion-packages", "--provider", provider],
      {
        cwd: process.cwd(),
        encoding: "utf8",
        env: { ...process.env, CARAPACE_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH },
      },
    );
  ...
  it("executes the release harness directly with Node", () => {
    ...
    expect(windowsCiCoverage).toContain("test/scripts/carapace-cross-os-release-workflow.test.ts");
    const result = spawnSync(
      BASH_BIN,
      [
        WRAPPER_PATH,
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          CARAPACE_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH,
        },
      },
    );
