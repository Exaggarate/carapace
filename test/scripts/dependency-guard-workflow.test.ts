...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectRunStep.env?.CARAPACE_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubRunStep.env?.CARAPACE_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubRunStep.env?.CARAPACE_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalRunStep.env?.CARAPACE_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.CARAPACE_SECURITY_TEAM_SLUG).toBe("carapace-secops");
    expect(runStep.env?.CARAPACE_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("carapace-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @carapace/carapace-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @carapace/carapace-secops",
    );
    expect(codeowners).toContain("/scripts/github/dependency-guard.mjs @carapace/carapace-secops");
    expect(codeowners).toContain("/package-lock.json @carapace/carapace-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @carapace/carapace-secops");
    expect(codeowners).toContain("/pnpm-lock.yaml @carapace/carapace-secops");
  });
});
