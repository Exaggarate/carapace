...
function writeBaseReleaseFiles(repository: string): void {
  ...
  writeFile(
    ...
    [
      ...
      "",
      "CARAPACE_ANDROID_VERSION_NAME=2026.9.1",
      "CARAPACE_ANDROID_VERSION_CODE=2026090101",
      "",
    ].join("\n"),
  );
  writeFile(
...
if (gitArgs[0] === "remote" && gitArgs[1] === "get-url") {
  process.stdout.write("https://github.com/Exaggarate/carapace.git\\n");
  process.exit(0);
}
...
} else if (endpoint.includes("/git/ref/carapace/mobile-releases/")) {
...
function createFixture(options: FixtureOptions = {}): Fixture {
  ...
  const root = tempRoots.make("carapace-mobile-release-authority-");
...
describe("mobile release authority", () => {
  ...
  it("rechecks permission after live ref reads immediately before immutable ref creation", () => {
    ...
    expect(trace.some(({ args }) => args[1]?.includes("/git/ref/carapace/mobile-releases/"))).toBe(
      true,
    );
  ...
  it("keeps upload and recovery credentials inside one protected platform boundary", () => {
    ...
    for (const {
      ...
      expect(release.steps[uploadIndex]?.env?.CARAPACE_MOBILE_RELEASE_INTENT_PATH).toBe(intentPath);
