...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  const cleanupFunction = script.slice(
    script.indexOf("kill_all_carapace()"),
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/Carapace"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/Carapace"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/Carapace"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$CARAPACE_TEST_KILL_CALLS"',
      "  return 0",
      "}",
      "kill_all_carapace",
    ].join("\n"),
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      CARAPACE_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
function runManagedSupervisorClassifier(
  ...
  const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-supervisor-test-"));
  tempRoots.push(root);
  ...
  const classifierFunctions = script.slice(
    script.indexOf("print_managed_carapace_supervisor_label()"),
    script.indexOf("kill_managed_carapace()"),
  );
  ...
  writeFileSync(
    ...
    [
      ...
      classifierFunctions,
      "loaded_launch_jobs() {",
      '  [[ "${CARAPACE_TEST_FAIL_ENUMERATION:-0}" != "1" ]] || return 1',
      "  cut -d'|' -f1,2 \"$CARAPACE_TEST_LOADED_JOBS\"",
      "}",
      "launch_job_snapshot() {",
      '  grep "^$1|$2|" "$CARAPACE_TEST_LOADED_JOBS" |',
      "    awk -F'|' '{ print \"program = \" $3; print \"properties = \" $4 }'",
      "}",
      'TARGET_EXECUTABLE="/worktree/dist/Carapace.app/Contents/MacOS/Carapace"',
      'INSTALLED_EXECUTABLE="/Applications/Carapace.app/Contents/MacOS/Carapace"',
  ...
  return spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      CARAPACE_TEST_FAIL_ENUMERATION: options.failEnumeration ? "1" : "0",
      CARAPACE_TEST_LOADED_JOBS: recordsPath,
    },
  });
}
...
function runSigningEnvironmentBlock(signIdentity: string) {
  const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-signing-test-"));
  tempRoots.push(root);
  const script = readFileSync(restartScriptPath, "utf8");
...
function runProfileGuard(profile: string) {
  const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-profile-test-"));
  tempRoots.push(root);
  const script = readFileSync(restartScriptPath, "utf8");
  const start = script.indexOf('if [[ -n "${CARAPACE_PROFILE:-}" ]]');
  ...
  return spawnSync("/bin/bash", [harnessPath], {
    encoding: "utf8",
    env: { ...process.env, CARAPACE_PROFILE: profile },
  });
}
...
function runForeignProcessClassifier(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      functions,
      'APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Carapace"',
      'TARGET_EXECUTABLE="/Users/steipete/carapace/dist/Carapace.app/Contents/MacOS/Carapace"',
      'INSTALLED_EXECUTABLE="/Applications/Carapace.app/Contents/MacOS/Carapace"',
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${CARAPACE_RESTART_LOG:-/tmp/carapace-restart.log}"');
  });

  it("rejects named app profiles before global process or launchd cleanup", () => {
    const script = readFileSync(restartScriptPath, "utf8");

    expect(script).toContain('if [[ -n "${CARAPACE_PROFILE:-}"');
  ...
  it("does not remove a live restart lock it did not acquire", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-test-"));
    tempRoots.push(root);
    const lockDir = join(root, "carapace-restart-lock");
    mkdirSync(lockDir);
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "CARAPACE_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Carapace.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/Carapace.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Carapace.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/Carapace.app"),
    );
  });

  it("keeps restart cleanup scoped to known Carapace app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/Carapace.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
    ...
    expect(cleanupBlock).not.toContain('pkill -x "Carapace"');
    expect(cleanupBlock).not.toContain("pgrep");
    expect(cleanupBlock).not.toContain('pgrep -x "Carapace"');
  });

  it("stops launchd supervision before killing app processes", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    const stopIndex = script.indexOf("stop_launch_agent\n  log");
    const killIndex = script.indexOf("if ! kill_all_carapace");
  ...
  it("target-only mode refuses foreign app processes without broad cleanup", () => {
    ...
    expect(initialTargetBlock).toContain("foreign_carapace_process_pids");
    expect(initialTargetBlock).not.toContain("kill_managed_carapace");
    expect(initialTargetBlock).not.toContain("stop_launch_agent");
    expect(initialTargetBlock).not.toContain("kill_all_carapace");
    expect(switchTargetBlock).toContain("foreign_carapace_process_pids");
    expect(switchTargetBlock).toContain("kill_managed_carapace");
  ...
  it("finds persistent launchd supervisors across explicit domains", () => {
    const result = runManagedSupervisorClassifier([
      {
        domain: "gui/501",
        label: "ai.carapace.mac.custom",
        program: "/Applications/Carapace.app/Contents/MacOS/Carapace",
        properties: "keepalive | runatload",
      },
      {
        domain: "user/501",
        label: "ai.carapace.mac.target",
        program: "/worktree/dist/Carapace.app/Contents/MacOS/Carapace",
        properties: "keepalive",
      },
      {
        domain: "gui/501",
        label: "application.ai.carapace.mac.123",
        program: "/Applications/Carapace.app/Contents/MacOS/Carapace",
      },
      {
        domain: "system",
    ...
    expect(result.stdout.trim().split("\n").toSorted()).toEqual([
      "ai.carapace.mac.custom",
      "ai.carapace.mac.target",
    ]);
    expect(result.stderr).toBe("");
  });
  ...
  it("checks managed launchd supervisors before starting the Swift package build", () => {
    ...
    const supervisorIndex = script.indexOf(
      'managed_supervisors="$(managed_carapace_supervisor_labels',
    );
  ...
  it("keeps the managed app alive until the signed replacement is ready", () => {
    ...
    expect(script).toContain('CARAPACE_PACKAGE_APP_ROOT="${STAGED_APP_BUNDLE}"');
  ...
  it("escalates only exact managed app processes when graceful shutdown stalls", () => {
    ...
    const broadKillBlock = script.slice(
      script.indexOf("kill_all_carapace()"),
      script.indexOf("known_carapace_executables()"),
    );

    expect(managedKillBlock).toContain('kill -KILL "${pid}"');
    expect(managedKillBlock).toContain("managed_carapace_process_pids");
  ...
  it("treats the canonical installed app as managed but temp bundles as foreign", () => {
    const result = runForeignProcessClassifier(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  101 /Applications/Carapace.app/Contents/MacOS/Carapace --attach-only'",
        "printf '%s\\n' '  102 /Users/steipete/carapace/dist/Carapace.app/Contents/MacOS/Carapace --attach-only'",
        "printf '%s\\n' '  103 /tmp/agent/Carapace.app/Contents/MacOS/Carapace --attach-only'",
        "printf '%s\\n' '  104 /bin/sh test.sh /Applications/Carapace.app/Contents/MacOS/Carapace'",
      ].join("\n"),
    );
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "Carapace.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../Carapace.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/Carapace.app/Contents/MacOS/Carapace --attach-only'",
      ].join("\n"),
    );
  ...
  it("waits beyond the app signal failsafe for scoped processes to exit", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$CARAPACE_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("keeps the restart grace period longer than the app signal failsafe", () => {
    ...
    const watcher = readFileSync(
      "apps/macos/Sources/Carapace/TerminationSignalWatcher.swift",
      "utf8",
    );
  ...
  it("does not kill unrelated Carapace app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/Carapace.app/Contents/MacOS/Carapace'",
      ].join("\n"),
    );
