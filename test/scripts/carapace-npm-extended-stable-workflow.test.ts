...
const workflowPath = ".github/workflows/carapace-npm-release.yml";
const preflightWorkflowPath = ".github/workflows/carapace-npm-preflight.yml";
...
function runControlUiArtifactStep(options: { artifactPresent: boolean }) {
  const root = mkdtempSync(join(tmpdir(), "carapace-npm-preflight-ui-"));
...
`,
  ...
  const ensureControlUi = step(
    workflow(preflightWorkflowPath).jobs?.prepare_carapace_npm,
...
describe("minimal npm extended-stable workflow", () => {
  ...
  it("routes source and Tideclaw history through the trusted ancestry owner", () => {
    ...
    const sourceAncestry = step(
      parsed.jobs?.check_carapace_npm,
      "Establish source ancestry with main",
    );
    const tideclawAncestry = step(
      parsed.jobs?.prepare_carapace_npm,
    ...
    const sourceSteps = parsed.jobs?.check_carapace_npm?.steps ?? [];
    const prepareSteps = parsed.jobs?.prepare_carapace_npm?.steps ?? [];
  ...
  it("allows an explicit default-off GitHub-hosted preflight runner", () => {
    ...
    expect(parsed.jobs?.preflight_carapace_npm?.uses).toBe(`./${preflightWorkflowPath}`);
    expect(parsed.jobs?.preflight_carapace_npm?.with?.use_github_hosted_runners).toBe(
  ...
  it("binds intentional Plugin SDK release changes to the reported digest", () => {
    ...
    const publishProvenance = step(
      parsed.jobs?.publish_carapace_npm,
    ...
    expect(preflightDiff.run).toContain("['view', 'carapace', 'dist-tags', '--json']");
    ...
    expect(preflightDiff.run).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter carapace",
    );
    ...
    expect(publishProvenanceRun).toContain('npm view "carapace@${RELEASE_NPM_DIST_TAG}" version');
    ...
    expect(
      ...
      publishProvenanceRun.indexOf('npm view "carapace@${RELEASE_NPM_DIST_TAG}" version'),
    );
  ...
  it("reuses the prepared tarball and guards source, preparation, and publication", () => {
    ...
    expect(raw).toContain("carapace-npm-preflight-${{ inputs.tag }}");
    const preflight = workflow(preflightWorkflowPath);
    for (const job of [preflight.jobs?.check_carapace_npm, preflight.jobs?.prepare_carapace_npm]) {
      const request = step(job, "Validate npm release request");
      expect(request.run).toContain("carapace-npm-extended-stable-release.mjs validate-request");
    ...
    ).toContain("carapace-npm-extended-stable-release.mjs validate-request");
    expect(step(parsed.jobs?.publish_carapace_npm, "Recheck npm release request").run).toContain(
      "carapace-npm-extended-stable-release.mjs validate-request",
    );
    expect(
  ...
  it("threads an explicit, default-off extended-stable bypass through every policy gate", () => {
    ...
    const policySteps = [
      step(
        workflow(preflightWorkflowPath).jobs?.check_carapace_npm,
      ...
      step(parsed.jobs?.publish_carapace_npm, "Recheck npm release request"),
      step(parsed.jobs?.publish_carapace_npm, "Publish"),
    ];
  ...
  it("lets main promote only the canonical immutable extended-stable candidate", () => {
    ...
    const recheck = step(parsed.jobs?.publish_carapace_npm, "Recheck npm release request");
  ...
  it("accepts arbitrary SHA preflight targets and exercises every publishable plugin package", () => {
    ...
    expect(
      step(parsed.jobs?.prepare_carapace_npm, "Validate npm package source metadata").run,
    ...
    expect(plugins.env).toMatchObject({
      CARAPACE_PLUGIN_NPM_PUBLISH_TAG: "extended-stable",
    });
    ...
    expect(plugins.run).toContain("CARAPACE_PLUGIN_NPM_PACK_OUTPUT_DIR");
  ...
  it("restores same-SHA preflight build outputs and keeps validation steps running", () => {
    const parsed = workflow(preflightWorkflowPath);
    const preflight = parsed.jobs?.prepare_carapace_npm;
    ...
    expect(build.env?.CARAPACE_CONTROL_UI_RELEASE_BUILD).toBe("1");
    ...
    expect(
      step(parsed.jobs?.check_carapace_npm, "Check source, test types, and architecture").if,
    ).toBeUndefined();
    ...
    expect(verifyReleaseContents.run).toMatch(
      /TSX_TSCONFIG_PATH="\$tooling_dir\/tsconfig\.json" \\\n\s+node --import "\$tooling_dir\/scripts\/tsx\.mjs" "\$tooling_dir\/scripts\/carapace-npm-prepublish-verify\.ts"/,
    );
  ...
  it("uses the trusted Full Validation evidence verifier", () => {
    ...
    expect(raw.match(/carapace-npm-extended-stable-release\.mjs verify-run/g)).toHaveLength(2);
    expect(raw).not.toContain("carapace-npm-extended-stable-release.mjs verify-manifest");
  });
  ...
  it("requires and authenticates the plugin npm run before an extended-stable core publish", () => {
    ...
    expect(verify.run).toContain("carapace-npm-extended-stable-release.mjs verify-run");
  });

  it("captures selector fail closed, publishes extended-stable, retries, and summarizes", () => {
    const parsed = workflow();
    const publish = parsed.jobs?.publish_carapace_npm;
    ...
    expect(capture.run).toContain("carapace-npm-extended-stable-release.mjs capture-selector");
    expect(step(publish, "Publish").run).toContain("carapace-npm-publish.sh");
    expect(readback.run).toContain("carapace-npm-extended-stable-release.mjs verify-readback");
    expect(summary.if).toContain("always()");
    expect(summary.run).toContain("carapace-npm-extended-stable-release.mjs repair-command");
  ...
  it("publishes only the tarball path verified from the preflight manifest", () => {
    const publish = workflow().jobs?.publish_carapace_npm;
  ...
  it("publishes gateway packages in manifest order before the root package", () => {
    ...
    const publish = step(parsed.jobs?.publish_carapace_npm, "Publish");
    ...
    expect(publish.run).toContain(
      'bash scripts/carapace-npm-publish.sh --publish "${publish_target}"',
    );
  });
});
