...
describe("gh-read helpers", () => {
  ...
  it("prints wrapper usage before reading auth env", () => {
    ...
    expect(stderr).toContain("CARAPACE_GH_READ_APP_ID");
  });

  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "carapace/carapace"])).toBe("carapace/carapace");
    expect(parseRepoArg(["run", "list", "--repo=carapace/docs"])).toBe("carapace/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("carapace/carapace")).toBe("carapace/carapace");
    expect(normalizeRepo("github.com/Exaggarate/carapace")).toBe("carapace/carapace");
    expect(normalizeRepo("github:carapace/carapace")).toBe("carapace/carapace");
    expect(normalizeRepo("https://github.com/Exaggarate/carapace.git")).toBe("carapace/carapace");
    expect(normalizeRepo("git@github.com:Exaggarate/carapace.git")).toBe("carapace/carapace");
    expect(normalizeRepo("https://gitlab.com/carapace/carapace.git")).toBeNull();
  ...
  it("bounds GitHub App private key files", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "carapace-gh-read-key-"));
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /CARAPACE_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});
