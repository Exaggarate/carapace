...
describe("bundled plugin build entries", () => {
  ...
  it("retains manifest-owned config repairs independently of runtime package exclusions", () => {
    const cwd = tempDirs.make("carapace-config-doctor-entries-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@carapace/external-owner",
        carapace: { build: { bundledDist: false } },
      }),
    );
    const manifest = {
      id: "external-owner",
  ...
  it("filters bundled plugin build entries for bounded script lanes", () => {
    const entries = listBundledPluginBuildEntries({
      env: {
        ...process.env,
        CARAPACE_BUNDLED_PLUGIN_BUILD_IDS: "active-memory,acpx",
      },
    });
  ...
  it("rejects unknown bounded bundled plugin build ids", () => {
    expect(() =>
      listBundledPluginBuildEntries({
        env: {
          ...process.env,
          CARAPACE_BUNDLED_PLUGIN_BUILD_IDS: "missing-plugin",
        },
      }),
    ).toThrow(
      "CARAPACE_BUNDLED_PLUGIN_BUILD_IDS references unknown bundled plugin id(s): missing-plugin",
    );
  });
  ...
  it("packs the runtime core support package without requiring a plugin manifest", () => {
    ...
    expect(artifacts).not.toContain("dist/extensions/image-generation-core/carapace.plugin.json");
  });
  ...
  it("sorts Docker-selected build entries without git metadata", () => {
    const repoDir = tempDirs.make("carapace-docker-build-entries-");
  ...
  it("excludes externalized model providers from bundled artifacts", () => {
    ...
    for (const pluginId of [
      ...
      expect(artifacts).not.toContain(`dist/extensions/${pluginId}/carapace.plugin.json`);
  ...
  it("keeps OpenCode Go bundled until its companion artifact is available", () => {
    ...
    expect(artifacts).toEqual(
      expect.arrayContaining([
        ...
        "dist/extensions/opencode-go/carapace.plugin.json",
  ...
  it("excludes the externalized Vydra provider from bundled artifacts", () => {
    ...
    expect(artifacts).not.toContain("dist/extensions/vydra/carapace.plugin.json");
  ...
  it("excludes the externalized DuckDuckGo plugin from bundled artifacts", () => {
    ...
    expect(artifacts).not.toContain("dist/extensions/duckduckgo/carapace.plugin.json");
  ...
  it("excludes the externalized Voyage provider from bundled artifacts", () => {
    ...
    expect(artifacts).not.toContain("dist/extensions/voyage/carapace.plugin.json");
  ...
  it("excludes the externalized Volcengine provider from bundled artifacts", () => {
    ...
    expect(artifacts).not.toContain("dist/extensions/volcengine/carapace.plugin.json");
