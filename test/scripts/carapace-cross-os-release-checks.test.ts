// Carapace Cross Os Release Checks tests cover carapace cross os release checks script behavior.
...
describe("scripts/carapace-cross-os-release-checks", () => {
  it("uses the host account identity for managed installer services", () => {
    const env = resolveManagedGatewayInstallerEnv({
      env: {
        ...
        CARAPACE_HOME: "C:\\temp\\lane",
        CARAPACE_PROFILE: "work",
        CARAPACE_STATE_DIR: "C:\\temp\\lane\\.carapace",
        CARAPACE_CONFIG_PATH: "C:\\temp\\lane\\.carapace\\carapace.json",
        CARAPACE_WINDOWS_TASK_NAME: "Carapace Gateway (work)",
        CARAPACE_TASK_SCRIPT_NAME: "work.cmd",
        CARAPACE_TASK_SCRIPT: "C:\\temp\\work.cmd",
        CARAPACE_SERVICE_KIND: "node",
        Carapace_Home: "C:\\temp\\case-variant",
        carapace_config_path: "C:\\temp\\case-variant\\carapace.json",
    ...
    expect(env.CARAPACE_TASK_SCRIPT_NAME).toBeUndefined();
    expect(env.CARAPACE_TASK_SCRIPT).toBeUndefined();
  ...
  it("keeps isolated installer state when no managed service is used", () => {
    const env = { CARAPACE_HOME: "/tmp/carapace-installer" };
  ...
  it("fails closed before borrowing an occupied managed-service account", () => {
    ...
    expect(() =>
      assertManagedGatewayInstallerHostAvailable({
        ...
        pathExists: (path) => path.endsWith(".carapace"),
      }),
    ).toThrow(/pristine host account/);
  });
  ...
  it("holds an exclusive managed-service host lease until release", () => {
    withTempDir("carapace-managed-host-", (accountHome) => {
  ...
  it("bounds public installer fetches on Windows and POSIX", () => {
    const windowsScript = buildInstallerSmokeScript({
      installerUrl: "https://github.com/Exaggarate/carapace",
    ...
    expect(windowsScript).toContain(
      "curl.exe -fsSL --connect-timeout 10 --max-time 120 -o $installerPath 'https://github.com/Exaggarate/carapace'",
    );
    expect(windowsScript).toContain("carapace-installer-");
    ...
    expect(posixScript).toContain(
      'installer_path="$(mktemp "${TMPDIR:-/tmp}/carapace-installer-XXXXXX")"',
    );
    expect(posixScript).toContain("trap 'rm -f \"$installer_path\"' EXIT");
    expect(posixScript).toContain(
      "curl -fsSL --connect-timeout 10 --max-time 120 -o \"$installer_path\" 'https://github.com/Exaggarate/carapace'",
    );
  ...
  it("requires dashboard root markers and same-origin asset URLs", () => {
    const html = [
      "<title>Carapace Control</title>",
      "<carapace-app></carapace-app>",
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: carapace gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("restarts an exited manual gateway once after the exact startup migration refusal", async () => {
    await withTempDirAsync("carapace-cross-os-gateway-restart-", async (dir) => {
      const refusal =
        "Carapace plugin migration inputs changed during startup convergence; refusing to report the gateway ready.";
  ...
  it.each([
    ...
    await withTempDirAsync("carapace-cross-os-gateway-no-restart-", async (dir) => {
  ...
  it("does not restart a live gateway after a readiness failure", async () => {
    await withTempDirAsync("carapace-cross-os-gateway-live-", async (dir) => {
      const gateway = createGatewayHandleFixture({
        dir,
        name: "gateway",
        afterLaunch:
  ...
  it("fails after a second startup migration refusal without looping", async () => {
    await withTempDirAsync("carapace-cross-os-gateway-second-refusal-", async (dir) => {
      const refusal =
        "Carapace plugin migration inputs changed during startup convergence; refusing to report the gateway ready.\n";
      const first = createGatewayHandleFixture({
        dir,
  ...
  it("routes the Discord manual relaunch through the bounded retry wait", async () => {
    await withTempDirAsync("carapace-cross-os-discord-gateway-", async (dir) => {
      ...
      await restartManualGatewayForDiscordSmoke({
        lane,
        cliPath: join(dir, "carapace"),
        env: { CARAPACE_HOME: lane.homeDir },
        gatewayHolder,
        gatewayLogPath,
        statusLogPath,
        operations: {
  ...
  it("gives the Windows packaged updater wrapper enough headroom for Carapace timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("CARAPACE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });
  ...
  it("retains only bounded allowlisted packaged-upgrade timings", () => {
    expect(
      parsePackagedUpgradeUpdateTimings(
        JSON.stringify({
          durationMs: 622_000,
          root: String.raw`C:\private\carapace`,
          steps: [
            {
              name: "global update",
            ...
            { name: "carapace doctor", durationMs: 66_000 },
      ...
      parsePackagedUpgradeUpdateTimings(
        JSON.stringify({
          ...
          steps: [
            ...
            { name: "carapace doctor", durationMs: "66000" },
          ],
        }),
      ),
    ).toEqual([]);
  ...
  it("renders runner, runtime, and sanitized updater timing evidence", () => {
    withTempDir("carapace-cross-os-summary-", (dir) => {
      writeSummary(dir, {
        platform: "win32",
        runnerOs: "Windows",
        ...
        baselineSpec: "carapace@2026.8.27",
        result: {
          status: "pass",
          updateFallback: {
            reason: "timeout",
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    withTempDir("carapace-cross-os-agent-output-", (dir) => {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    withTempDir("carapace-cross-os-agent-output-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ CARAPACE_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ CARAPACE_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("skips optional live agent turns only for model availability failures", () => {
    withTempDir("carapace-cross-os-agent-skip-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        JSON.stringify({
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    withTempDir("carapace-cross-os-agent-skip-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    withTempDir("carapace-cross-os-agent-skip-retry-", (dir) => {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        CARAPACE_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        CARAPACE_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.6 Luna", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/carapace-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "CARAPACE_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.CARAPACE_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.6-luna' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("carapace@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "carapace@2026.5.2",
      "--omit=dev",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("carapace-2026.6.17.tgz")).toBe("carapace-2026.6.17.tgz");
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/carapace-pack");

    expect(resolvePackDestinationTarball("carapace-2026.6.17.tgz", packDir, "pnpm pack")).toEqual({
      fileName: "carapace-2026.6.17.tgz",
      path: resolvePath(packDir, "carapace-2026.6.17.tgz"),
    });
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "carapace-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/carapace.tgz",
      "nested\\carapace.tgz",
      resolvePath(dirname(packDir), "carapace.tgz"),
      resolvePath(packDir, "nested", "carapace.tgz"),
      "carapace\u0000.tgz",
      "carapace.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    withTempDir("carapace-cross-os-pack-command-", (dir) => {
      ...
      const helperPath = join(dir, "scripts", "package-carapace-for-docker.mjs");
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/carapace-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/carapace-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "carapace" }');
  ...
  it("preflights standalone source candidates before cross-OS dependency installation", async () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-cross-os-source-preflight-"));
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'carapace.cmd')");
    expect(script).toContain("$cmd = Get-Command carapace -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        CARAPACE_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        CARAPACE_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        CARAPACE_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("carapace-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-static-server-"));
    const filePath = join(dir, "carapace-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-static-server-close-"));
    const filePath = join(dir, "carapace-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "carapace-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "carapace/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "CARAPACE_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/carapace/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/carapace/dist/plugin-sdk/plugin-runtime.js"',
    );
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view carapace@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\Carapace Prefix`, "carapace.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Carapace Prefix\carapace.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    await withTempDirAsync("carapace-cross-os-run-command-", async (dir) => {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    await withTempDirAsync("carapace-cross-os-run-command-output-", async (dir) => {
      const logPath = join(dir, "command.log");
  ...
  it("keeps multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("carapace-cross-os-run-command-utf8-tail-", async (dir) => {
      const logPath = join(dir, "command.log");
      const result = await runCommand(
        process.execPath,
  ...
  it("keeps rolling multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("carapace-cross-os-run-command-utf8-rolling-", async (dir) => {
      const logPath = join(dir, "command.log");
      const script = [
  ...
  it.each(["stdout", "stderr"] as const)(
    ...
    async (stream) => {
      await withTempDirAsync("carapace-cross-os-run-command-utf8-split-", async (dir) => {
        const logPath = join(dir, "command.log");
        const script = [
  ...
  it.each([1, 2, 3])(
    ...
    async (maxOutputBytes) => {
      await withTempDirAsync("carapace-cross-os-run-command-utf8-budget-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
  ...
  it.each([
    ...
    async ({ maxOutputBytes, expected }) => {
      await withTempDirAsync("carapace-cross-os-run-command-utf8-incomplete-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
          [
  ...
  it("flushes command logs before resolving", async () => {
    await withTempDirAsync("carapace-cross-os-run-command-flush-", async (dir) => {
  ...
  it("resolves Windows and configured npm diagnostic directories", () => {
    const homeDir = join(tmpdir(), "carapace-npm-diagnostics-home");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    withTempDir("carapace-cross-os-npm-relative-logs-", (dir) => {
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-run-command-timeout-"));
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            CARAPACE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "200",
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "carapace-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            CARAPACE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "3000",
            CARAPACE_TEST_CHILD_PID: childPidPath,
          },
          stdio: ["ignore", "ignore", "pipe"],
        },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/carapace", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "carapace-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "carapace");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "carapace" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "carapace"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "carapace"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "carapace"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-carapace-cache",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[carapace] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/carapace/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\carapace-upgrade-q9DsA7\\prefix\\node_modules\\carapace\\carapace.mjs update --tag http://127.0.0.1:49951/carapace-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\carapace\\carapace.mjs update --tag http://127.0.0.1:49951/carapace-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node carapace.mjs update --tag carapace@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.carapace-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("carapace@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("carapace@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    withTempDir("carapace-cross-os-installed-version-", (prefixDir) => {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    withTempDir("carapace-cross-os-scripts-", (packageRoot) => {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
          name: "carapace",
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    await withTempDirAsync("carapace-cross-os-stage-debris-", async (packageRoot) => {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Carapace-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Carapace-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    await withTempDirAsync("carapace-cross-os-local-stamps-", async (packageRoot) => {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
          files: ["dist/"],
          name: "carapace-fixture",
