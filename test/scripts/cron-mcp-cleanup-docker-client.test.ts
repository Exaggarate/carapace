...
describe("cron MCP cleanup docker client", () => {
  it("rejects malformed probe pid wait limits", () => {
    ...
    expect(readCronMcpCleanupProbePidWaitMs({ CARAPACE_CRON_MCP_CLEANUP_PID_WAIT_MS: "250" })).toBe(
      250,
    );
    for (const value of ["1.5", "1e3", "10ms", "0"]) {
      expect(() =>
        readCronMcpCleanupProbePidWaitMs({
          CARAPACE_CRON_MCP_CLEANUP_PID_WAIT_MS: value,
        }),
      ).toThrow("invalid CARAPACE_CRON_MCP_CLEANUP_PID_WAIT_MS");
    }
  });

  it("bounds missing probe pid waits", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-cron-mcp-client-"));
    try {
