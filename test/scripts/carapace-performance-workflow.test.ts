// Carapace Performance Workflow tests cover carapace performance workflow script behavior.
...
function runCandidateTrustClassification({
  ...
  const root = tempDirs.make("carapace-performance-candidate-trust-");
...
describe("Carapace performance workflow", () => {
  ...
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    ...
    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('Carapace Performance {0}', inputs.dispatch_id) || 'Carapace Performance' }}",
    );
  ...
  it("pins the Kova evaluator with release validation contracts", () => {
    ...
    const resolveTarget = findStep("Resolve Carapace target ref", "resolve_target");
    ...
    expect(readWorkflow().jobs?.kova?.env?.KOVA_CARAPACE_CONFIG_CONTRACT).toBe(
  ...
  it("keeps live credentials away from custom Kova refs", () => {
    ...
    const root = mkdtempSync(join(realpathSync(tmpdir()), "carapace-kova-live-ref-"));
  ...
  it("replaces candidate-owned nested setup actions with the trusted workflow copy", () => {
    ...
    for (const stage of stages) {
      const root = tempDirs.make("carapace-performance-action-graph-");
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const targetCheckout = findStep("Checkout target metadata", "resolve_target");
    const resolveTarget = findStep("Resolve Carapace target ref", "resolve_target");
    const checkout = findStep("Checkout Carapace");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout Carapace source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve Carapace target ref"),
    ).toHaveLength(1);
  });
  ...
  it("sparse-fetches only the public source baseline without publisher credentials", () => {
    ...
    expect(run).toContain('remote = "https://github.com/Exaggarate/carapace/clawgrit-reports.git"');
  ...
  it("builds only the QA and startup artifacts required by source probes", () => {
    const run = findStep("Run Carapace source performance probes", "source_performance").run ?? "";
    const typedBuild =
      "CARAPACE_BUILD_PRIVATE_QA=1 node --import tsx scripts/build-all.mts sourcePerformance";
    const nativeBuild = "CARAPACE_BUILD_PRIVATE_QA=1 node scripts/build-all.mjs sourcePerformance";
  ...
  it("runs trusted CLI performance cases against the frozen candidate entrypoint", () => {
    ...
    expect(run).toContain('--entry "$GITHUB_WORKSPACE/carapace.mjs"');
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run Carapace source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run Carapace source performance probes",
      ),
    ).toBe(true);
  ...
  it("keeps app credentials out of artifact processing and scopes them to report Git operations", () => {
    ...
    expect(prepare.run).toContain(
      'source_path="${input_root}/carapace-performance-source-${GITHUB_RUN_ID}-${SOURCE_PRODUCER_ATTEMPT}/${LANE_ID}"',
    );
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://github.com/Exaggarate/carapace/clawgrit-reports.git"',
    );
    ...
    expect(publish.run).toContain(
      'git_auth_environment("https://github.com/Exaggarate/carapace/clawgrit-reports.git", token)',
    );
  ...
  it("publishes bounded bundle metadata while retaining full diagnostics as an artifact", () => {
    ...
    expect(upload.with?.path).not.toContain(".artifacts/carapace-performance/source");
    expect(sourceUpload.with).toMatchObject({
      name: "carapace-performance-source-${{ github.run_id }}-${{ github.run_attempt }}",
      path: ".artifacts/carapace-performance/source",
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "carapace-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	carapace-performance-mock-provider-9001-1' \
  '202	carapace-performance-source-9001-2' \
  '303	carapace-performance-mock-provider-9001-3'
`,
    );
    ...
    async () => {
      ...
      expect(report.performance?.remoteFiles).toEqual(
        expect.arrayContaining([
          "carapace-performance/main/123-1/mock-provider/report.json",
          "carapace-performance/main/200-1/mock-provider/report.json",
        ]),
      );
      expect(JSON.parse(report.performance!.pointer)).toEqual({
        path: "carapace-performance/main/123-1/mock-provider",
      });
    },
    55_000,
  );
  ...
  it("preserves required PARTIAL failures and clears only advisory PARTIAL failures", () => {
    ...
    const root = tempDirs.make("carapace-kova-partial-gate-");
  ...
  it("lets OCM discover its native workspace dependency adapter", () => {
    ...
    expect(workflowText).not.toContain("CARAPACE_OCM_NPM_WRAPPER");
    expect(workflowText).not.toContain("CARAPACE_OCM_WORKSPACE_DEPENDENCY_DIRS");
