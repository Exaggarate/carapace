// Resolve Carapace ref tests cover the release workflow ref resolver script.
import { execFileSync, spawnSync } from "node:child_process";
import { join } from "node:path";
import { afterAll, beforeAll, describe, expect, it } from "vitest";
import { createTempDirTracker } from "../helpers/temp-dir.js";

const SCRIPT_PATH = "scripts/github/resolve-carapace-ref.sh";
...
function createRemoteRepo() {
  const repo = tempDirs.make("carapace-ref-remote-");
...
describe("scripts/github/resolve-carapace-ref.sh", () => {
  ...
  it("writes fallback outputs for unresolved refs when a caller supplies an expected SHA", () => {
    const outputPath = join(tempDirs.make("carapace-ref-output-"), "github-output.txt");
  ...
  it("does not let fallback mode hide remote lookup failures", () => {
    const missingRemote = join(tempDirs.make("carapace-ref-missing-"), "missing.git");
