...
const CHECK_SCRIPT = resolve("scripts/check-carapace-package-tarball.mjs");
...
function withPackedPackage(
  ...
  const root = mkdtempSync(join(tmpdir(), "carapace-control-ui-package-inventory-"));
  ...
  try {
    ...
    writeFixtureFile(packageRoot, ".carapace-lifecycle-pending", "pending\n");
...
function installPackedPackage(root: string, tarball: string) {
  ...
  return join(installRoot, "node_modules", "carapace");
}
