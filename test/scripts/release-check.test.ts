...
describe("release-check", () => {
  ...
  it("loads sparse release tooling and checks the target worker contract", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-target-"));
    try {
      const toolingRoot = join(root, "tooling");
      const workflow = parse(readFileSync(".github/workflows/carapace-npm-preflight.yml", "utf8"));
  ...
  it("resolves prepacked publishable core package tarballs", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-tarball-test-"));
    try {
      writeFileSync(join(root, "carapace-ai-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "carapace-gateway-client-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "carapace-gateway-protocol-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "SHA256SUMS"), "fixture");
      expect(resolveReleaseCheckLocalPackageTarballs(root)).toEqual([
        join(root, "carapace-ai-2026.6.33.tgz"),
        join(root, "carapace-gateway-client-2026.6.33.tgz"),
        join(root, "carapace-gateway-protocol-2026.6.33.tgz"),
      ]);
  ...
  it("accepts gateway core packages when the root does not require AI", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-tarball-test-"));
    try {
      const gatewayTarball = join(root, "carapace-gateway-protocol-2026.7.2.tgz");
      const gatewayClientTarball = join(root, "carapace-gateway-client-2026.7.2.tgz");
  ...
  it("writes an explicit local project for unpublished core package tarballs", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-install-test-"));
    try {
      writePackedTarballInstallManifest(root, "/tmp/carapace.tgz", [
        "/tmp/carapace-ai.tgz",
        "/tmp/carapace-gateway-client.tgz",
        "/tmp/carapace-gateway-protocol.tgz",
      ]);
      ...
      expect(manifest.dependencies).toEqual({
        "@carapace/ai": "file:///tmp/carapace-ai.tgz",
        "@carapace/gateway-client": "file:///tmp/carapace-gateway-client.tgz",
        "@carapace/gateway-protocol": "file:///tmp/carapace-gateway-protocol.tgz",
        carapace: "file:///tmp/carapace.tgz",
      });
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  ...
  it("writes a gateway-packages-only local project when the root does not require AI", () => {
    ...
    try {
      writePackedTarballInstallManifest(
        root,
        "/tmp/carapace.tgz",
        ["/tmp/carapace-gateway-client.tgz", "/tmp/carapace-gateway-protocol.tgz"],
        false,
      );
      const manifest = JSON.parse(readFileSync(join(root, "package.json"), "utf8")) as {
        dependencies?: Record<string, string>;
  ...
  it("packs the local AI workspace when no prepared tarball is supplied", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-ai-pack-test-"));
    try {
      const tarballs = prepareReleaseCheckLocalPackageTarballs({
        tmpRoot: root,
        packLocalAi: (packDestination) => {
          const filename = "carapace-ai-2026.7.1-beta.3.tgz";
          writeFileSync(join(packDestination, filename), "fixture");
          return [{ filename }];
        },
      });
      expect(tarballs).toEqual([join(root, "ai-pack", "carapace-ai-2026.7.1-beta.3.tgz")]);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("prefers prepared core package tarballs over packing the AI workspace", () => {
    ...
    try {
      ...
      const preparedTarball = join(preparedDir, "carapace-ai-2026.7.1-beta.3.tgz");
      const gatewayProtocolTarball = join(
        preparedDir,
        "carapace-gateway-protocol-2026.7.1-beta.3.tgz",
      );
      const gatewayClientTarball = join(preparedDir, "carapace-gateway-client-2026.7.1-beta.3.tgz");
  ...
  it("rejects a packed install without the local AI tarball", () => {
    const root = mkdtempSync(join(tmpdir(), "carapace-release-check-install-test-"));
    try {
      expect(() => writePackedTarballInstallManifest(root, "/tmp/carapace.tgz", [])).toThrow(
        "requires exactly one @carapace/ai tarball",
      );
      expect(() =>
        writePackedTarballInstallManifest(root, "/tmp/carapace.tgz", [
          "/tmp/carapace-ai-one.tgz",
          "/tmp/carapace-ai-two.tgz",
        ]),
      ).toThrow("requires exactly one @carapace/ai tarball");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("rejects missing, incomplete, or ambiguous local package tarball directories", () => {
    ...
    try {
      ...
      expect(() => resolveReleaseCheckLocalPackageTarballs(empty)).toThrow(
        "must contain exactly one @carapace/ai tarball",
      );
  ...
  it("seeds packaged activation smoke with an included channel plugin", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "carapace-release-check-test-"));
    try {
      writePackedBundledPluginActivationConfig(homeDir);
      const config = JSON.parse(
        readFileSync(join(homeDir, ".carapace", "carapace.json"), "utf8"),
      ) as {
      ...
      for (const channel of channels) {
        ...
        const manifest = JSON.parse(
          readFileSync(join("extensions", channel, "carapace.plugin.json"), "utf8"),
        ) as { channels: string[] };
        expect(manifest.channels).toContain(channel);
        expect(artifacts).toContain(`dist/extensions/${channel}/carapace.plugin.json`);
      }
    } finally {
