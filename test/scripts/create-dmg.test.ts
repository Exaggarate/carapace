...
function makeApp(plistEntries: string[]): string {
  const dir = mkdtempSync(path.join(tmpdir(), "carapace-create-dmg-"));
  tempDirs.push(dir);
  const app = path.join(dir, "Carapace.app");
...
function makeFakeDmgTools() {
  const dir = mkdtempSync(path.join(tmpdir(), "carapace-create-dmg-tools-"));
  tempDirs.push(dir);
...
function expectPrivateDmgMount(log: string): string {
  ...
  expect(path.relative(tmpdir(), runRoot)).toMatch(/^carapace-dmg\.[^/]+$/);
...
describe("create-dmg plist validation", () => {
  ...
  it("keeps temporary DMG artifacts scoped to one run", () => {
    ...
    expect(script).toContain('DMG_TEMP="$(mktemp -d "${TMPDIR:-/tmp}/carapace-dmg.XXXXXX")"');
    ...
    expect(script).toContain(
      'DMG_OUTPUT_TEMP="$(mktemp -d "$(dirname "$OUT_PATH")/.carapace-dmg.XXXXXX")"',
    );
    ...
    expect(script).not.toContain("/tmp/carapace-dmg-limits.txt");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      const app = makeApp(["<key>CFBundleName</key>", "<string>Carapace</string>"]);
...
describe.runIf(process.platform === "darwin")("create-dmg ownership boundaries", () => {
  it("uses private intermediate paths without deleting caller-owned siblings", () => {
    ...
    const outputDir = mkdtempSync(path.join(tmpdir(), "carapace-create-dmg-output-"));
    tempDirs.push(outputDir);
    const output = path.join(outputDir, "Carapace.dmg");
    const sibling = path.join(outputDir, "Carapace-rw.dmg");
    ...
    expect(log).toContain(`${outputDir}${path.sep}.carapace-dmg.`);
    expect(log).not.toContain(sibling);
  });

  it("creates a caller-provided output directory before finalizing the DMG", () => {
    const app = makeValidApp();
    const root = mkdtempSync(path.join(tmpdir(), "carapace-create-dmg-output-"));
    tempDirs.push(root);
  ...
  it("styles the private mount without closing unrelated Finder windows", () => {
    ...
    expect(applescript).not.toContain('tell disk "Carapace"');
