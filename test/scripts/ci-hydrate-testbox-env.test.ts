...
describe("scripts/ci-hydrate-testbox-env.sh", () => {
  it("bakes custom profile paths into the generated helper default", () => {
    const root = makeTempDir(tempDirs, "carapace-testbox-env-");
    const home = join(root, "home");
    const profilePath = join(root, "custom profile.env");
    const helperPath = join(root, "bin", "carapace-testbox-env");
    ...
    expect(helper).not.toContain(".carapace-testbox-live.profile");
