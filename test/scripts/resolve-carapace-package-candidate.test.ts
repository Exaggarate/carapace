// Resolve Carapace Package Candidate tests cover resolve carapace package candidate script behavior.
...
import {
  ...
  cleanPackedCarapaceTarballsForTest,
  downloadUrl,
  ...
  runCommandForTest,
  validateCarapacePackageSpec,
} from "../../scripts/resolve-carapace-package-candidate.mts";
...
async function createPackageTarball(
  ...
  await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "carapace", version }));
  ...
  const tarball = path.join(dir, "carapace.tgz");
...
describe("resolve-carapace-package-candidate", () => {
  it("preflights package-acceptance ref candidates before dependency installation", () => {
    const script = readFileSync("scripts/resolve-carapace-package-candidate.mts", "utf8");
    ...
    expect(refPackageBuild).toContain('"scripts/package-carapace-for-docker.mjs"');
    expect(refPackageBuild).toContain('"--allow-unreleased-changelog"');
  });

  it("accepts only Carapace release package specs for npm candidates", () => {
    for (const spec of [
      "carapace@beta",
      "carapace@alpha",
      "carapace@extended-stable",
      "carapace@latest",
      "carapace@2026.4.27",
      "carapace@2026.4.27-1",
      "carapace@2026.4.27-beta.2",
      "carapace@2026.4.27-alpha.2",
    ]) {
      expect(validateCarapacePackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateCarapacePackageSpec("@evil/carapace@1.0.0")).toThrow(
      "package_spec must be carapace@alpha",
    );
    expect(() => validateCarapacePackageSpec("carapace@canary")).toThrow(
      "package_spec must be carapace@alpha",
    );
    expect(() => validateCarapacePackageSpec("carapace@2026.04.27")).toThrow(
      "package_spec must be carapace@alpha",
    );
    expect(() => validateCarapacePackageSpec("carapace@npm:other-package")).toThrow(
      "package_spec must be carapace@alpha",
    );
    expect(() => validateCarapacePackageSpec("carapace@file:../other-package.tgz")).toThrow(
      "package_spec must be carapace@alpha",
    );
  });
  ...
  it("rejects duplicate package candidate CLI options", () => {
    ...
    const duplicateCases = [
      ...
      [
        "--package-spec",
        [...requiredArgs, "--package-spec", "carapace@beta", "--package-spec", "carapace@latest"],
      ],
      [
        "--package-url",
        [...requiredArgs, "--package-url", "", "--package-url", "https://example.com/carapace.tgz"],
      ],
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "carapace@2026.5.26-beta.1",
      "C:\\carapace\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack carapace@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\carapace\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "carapace-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "carapace-2026.6.17.tgz" }]),
        "carapace-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "carapace-current.tgz"));
    await expect(readFile(path.join(dir, "carapace-current.tgz"), "utf8")).resolves.toBe("package");
  });

  it("keeps the first packed identity when a moving npm tag changes", async () => {
    const dir = autoTempDirs.make("carapace-package-moving-tag-");
    const binDir = path.join(dir, "bin");
...
cp "$source" "$FAKE_PACK_OUTPUT/carapace-$version.tgz"
printf '[{"filename":"carapace-%s.tgz"}]\\n' "$version"
`,
    );
    await chmod(path.join(binDir, "npm"), 0o755);
    await mkdir(outputDir);
    const runner = resolveNpmPackageCandidatePackRunner("carapace@beta", outputDir, {
      env: {
        ...process.env,
    ...
    expect(JSON.parse(packageJson)).toMatchObject({ name: "carapace", version: "2026.8.1" });
  ...
  it("reads npm 12 name-keyed package candidate filenames", async () => {
    ...
    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify({ carapace: { filename: "carapace-2026.6.17.tgz" } }),
        "carapace-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "carapace-current.tgz"));
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/carapace-2026.6.17.tgz",
      "nested\\carapace-2026.6.17.tgz",
      "/tmp/carapace-2026.6.17.tgz",
      "C:\\temp\\carapace-2026.6.17.tgz",
      "carapace-2026.6.17.tar.gz",
    ];

    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "carapace-current.tgz"),
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "carapace-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../carapace-2026.6.17.tgz", "C:carapace-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "carapace-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "carapace-C:evil.tgz"), "unsafe");

    await cleanPackedCarapaceTarballsForTest(dir);
    await writeFile(path.join(dir, "carapace-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "carapace-current.tgz"),
    ).resolves.toBe(path.join(dir, "carapace-current.tgz"));
    await expect(missing(path.join(dir, "carapace-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "carapace-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "carapace-current.tgz"), "utf8")).resolves.toBe("current");
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = autoTempDirs.make("carapace-package-runner-timeout-");
  ...
  it("clamps oversized package runner kill grace before scheduling", async () => {
    ...
    const dir = autoTempDirs.make("carapace-package-runner-grace-");
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = autoTempDirs.make("carapace-package-runner-timeout-clean-");
    const childPidPath = path.join(dir, "child.pid");
    const cleanupPath = path.join(dir, "child.cleanup");
    const childScript = [
      "const fs = require('node:fs');",
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = autoTempDirs.make("carapace-package-runner-signal-");
    const childPidPath = path.join(dir, "child.pid");
    const killPath = path.join(dir, "child.kill");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-carapace-package-candidate.mts"),
    ).href;
    const childScript = [
      "process.on('SIGTERM', () => {});",
      "setInterval(() => {}, 1000);",
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/carapace-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/carapace-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/carapace/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "carapace.tgz");

    await expect(
      downloadUrl("http://packages.example/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/carapace/carapace.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/carapace/carapace.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/carapace/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/carapace.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("matches trusted package_url path prefixes on path segment boundaries", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/carapace/pkg.tgz", target, {
      fetchImpl: async (url: URL) => {
        requestedUrls.push(url.toString());
    ...
    expect(requestedUrls).toEqual(["https://packages.internal:8443/artifactory/carapace/pkg.tgz"]);
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/carapace-malicious/pkg.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/carapace/carapace.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/carapace/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "carapace.tgz");
    const previousToken = process.env.CARAPACE_TRUSTED_PACKAGE_TOKEN;
    process.env.CARAPACE_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
          ...
          fetchImpl: async (_url: URL, init?: RequestInit) => {
            ...
            if (requestHeaders.length === 1) {
              return new Response(null, {
                headers: {
                  location: "https://mirror.internal:8443/artifactory/carapace/carapace.tgz",
                },
                status: 302,
              });
            }
      ...
      if (previousToken === undefined) {
        delete process.env.CARAPACE_TRUSTED_PACKAGE_TOKEN;
      } else {
        process.env.CARAPACE_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/carapace.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/carapace.tgz", target, {
      fetchImpl: async () =>
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "carapace.tgz");
    let bodyCancelled = false;
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/carapace.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("normalizes artifact package source SHAs before workflow output", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-candidate-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it.each([
    ...
    const fixture = await createArtifactFixture("carapace-artifact-provenance-mismatch-", {
  ...
  it("uses normalized artifact build-info provenance when metadata is absent", async () => {
    ...
    const fixture = await createArtifactFixture("carapace-artifact-build-info-fallback-", {
  ...
  it("requires artifact build-info only when preparing a registry", async () => {
    const sourceSha = "66ce632b9b7c5c7fdd3e66c739687d51638ad6e2";
    const fixture = await createArtifactFixture("carapace-artifact-missing-build-info-", {
  ...
  it("rejects malformed artifact build-info before package validation", async () => {
    const fixture = await createArtifactFixture("carapace-artifact-malformed-build-info-", {
      buildInfo: "{not-json",
      packageSourceSha: "66ce632b9b7c5c7fdd3e66c739687d51638ad6e2",
    });
  ...
  it("validates the normalized artifact source SHA before registry preparation", async () => {
    const dir = autoTempDirs.make("carapace-artifact-registry-source-");
    const artifactDir = path.join(dir, "artifact");
    const binDir = path.join(dir, "bin");
    const gitLog = path.join(dir, "git.log");
    const sourceSha = "66ce632b9b7c5c7fdd3e66c739687d51638ad6e2";
...
`,
    ...
    try {
      await expect(
        ...
        `package_ref ${sourceSha} resolved to ${sourceSha}, which is not reachable from an Carapace branch or release tag`,
      );
    } finally {
      process.env.PATH = previousPath;
      delete process.env.FAKE_GIT_LOG;
  ...
  it("normalizes whitespace-only artifact package source SHAs to absent", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-candidate-empty-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("rejects malformed artifact package source SHAs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-candidate-bad-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "carapace.tgz");
    await writeFile(file, "carapace package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-artifact-scan-"));
    tempDirs.push(dir);
    const maxEntries = 3;
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "carapace-a.tgz"), "a");
    ...
    expect(message).toContain("carapace-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "carapace-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "carapace-package-build-info-"));
    tempDirs.push(dir);
