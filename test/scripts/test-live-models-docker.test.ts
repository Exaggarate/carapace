...
describe("live model Docker wrappers", () => {
  it.each([
    ["test-live-gateway-models-docker.sh", "CARAPACE_LIVE_GATEWAY_MAX_MODELS", "two"],
    ["test-live-gateway-models-docker.sh", "CARAPACE_LIVE_GATEWAY_STEP_TIMEOUT_MS", "45s"],
    ["test-live-gateway-models-docker.sh", "CARAPACE_LIVE_GATEWAY_MODEL_TIMEOUT_MS", "90s"],
    ["test-live-models-docker.sh", "CARAPACE_LIVE_MAX_MODELS", "3models"],
    ["test-live-models-docker.sh", "CARAPACE_LIVE_MODEL_TIMEOUT_MS", "45s"],
    ...
    expect(fs.readFileSync(scriptPath, "utf8")).toContain(
      `-e ${envName}="$${envName.replace("CARAPACE_", "")}"`,
    );
  });
});
