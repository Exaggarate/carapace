// Carapace source performance isolation executes the workflow's real shared-shell path.
...
const WORKFLOW = ".github/workflows/carapace-performance.yml";
...
describe("Carapace source performance isolation", () => {
  it("isolates the source performance gateway from network and scheduled work", () => {
    ...
    const targetStepName = "Run Carapace source performance probes";
    ...
    const cronEnv = "CARAPACE_SKIP_CRON";
    ...
    expect(workflowSlice.match(/CARAPACE_SKIP_CRON/g) ?? []).toHaveLength(1);
    expect(workflowSlice.match(/CARAPACE_SKIP_CHANNELS/g) ?? []).toHaveLength(1);

    const fixtureRoot = tempDirs.make("carapace-performance-isolation-");
...
record_env() {
  {
    printf 'CARAPACE_HOME=%s\\n' "\${CARAPACE_HOME-<unset>}"
    printf 'CARAPACE_STATE_DIR=%s\\n' "\${CARAPACE_STATE_DIR-<unset>}"
    printf 'CARAPACE_CONFIG_PATH=%s\\n' "\${CARAPACE_CONFIG_PATH-<unset>}"
    printf 'CARAPACE_GATEWAY_PORT=%s\\n' "\${CARAPACE_GATEWAY_PORT-<unset>}"
    printf 'CARAPACE_GATEWAY_TOKEN=%s\\n' "\${CARAPACE_GATEWAY_TOKEN-<unset>}"
    printf 'CARAPACE_SKIP_CHANNELS=%s\\n' "\${CARAPACE_SKIP_CHANNELS-<unset>}"
    printf 'CARAPACE_SKIP_CRON=%s\\n' "\${CARAPACE_SKIP_CRON-<unset>}"
  } > "$CAPTURE_DIR/$1.env"
}
record_argv() {
  name="$1"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "run" ]; then
  record_env gateway
  record_argv gateway "$@"
  /bin/cp "$CARAPACE_CONFIG_PATH" "$CAPTURE_DIR/gateway.config"
  printf '%s\\n' "$CARAPACE_GATEWAY_PORT" > "$CAPTURE_DIR/gateway.port"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "health" ]; then
  record_env health
  record_argv health "$@"
  /bin/cp "$CARAPACE_CONFIG_PATH" "$CAPTURE_DIR/health.config"
...
if [ "\${1-}" = "--import" ] && [ "\${2-}" = "tsx" ]; then
  ...
  /bin/cp "$CARAPACE_CONFIG_PATH" "$CAPTURE_DIR/benchmark.config"
  printf 'benchmark\\n' >> "$EVENTS_FILE"
  exit 0
fi
...
`,
    ...
    for (const [branch, rgStatus, expectsCatalogRefresh] of [
      ...
      const gatewayHome = expectDefined(gatewayEnv.CARAPACE_HOME, "gateway home capture");
      const gatewayState = expectDefined(gatewayEnv.CARAPACE_STATE_DIR, "gateway state capture");
      const gatewayConfigPath = expectDefined(
        gatewayEnv.CARAPACE_CONFIG_PATH,
        "gateway config capture",
      );
      const healthHome = expectDefined(healthEnv.CARAPACE_HOME, "readiness home capture");
      const healthState = expectDefined(healthEnv.CARAPACE_STATE_DIR, "readiness state capture");
      const healthConfigPath = expectDefined(
        healthEnv.CARAPACE_CONFIG_PATH,
        "readiness config capture",
      );
      expect(gatewayEnv.CARAPACE_SKIP_CRON).toBe("1");
      expect(gatewayEnv.CARAPACE_SKIP_CHANNELS).toBe("1");
      expect(benchmarkEnv.CARAPACE_SKIP_CRON).toBe("<unset>");
      expect(benchmarkEnv.CARAPACE_SKIP_CHANNELS).toBe("<unset>");
      expect(healthEnv.CARAPACE_SKIP_CRON).toBe("<unset>");
      expect(healthEnv.CARAPACE_SKIP_CHANNELS).toBe("<unset>");
      expect(benchmarkEnv.CARAPACE_HOME).toBe(gatewayHome);
      expect(benchmarkEnv.CARAPACE_STATE_DIR).toBe(gatewayState);
      expect(benchmarkEnv.CARAPACE_CONFIG_PATH).toBe(gatewayConfigPath);
      expect(benchmarkEnv.CARAPACE_GATEWAY_PORT).toBe(gatewayPort);
      expect(gatewayEnv.CARAPACE_GATEWAY_PORT).toBe(gatewayPort);
      expect(gatewayEnv.CARAPACE_GATEWAY_TOKEN).toMatch(/^[0-9a-f]{64}$/u);
      expect(healthEnv.CARAPACE_GATEWAY_TOKEN).toBe(gatewayEnv.CARAPACE_GATEWAY_TOKEN);
      expect(benchmarkEnv.CARAPACE_GATEWAY_TOKEN).toBe(gatewayEnv.CARAPACE_GATEWAY_TOKEN);
      ...
      expect(gatewayState).toBe(join(gatewayHome, ".carapace"));
      expect(gatewayConfigPath).toBe(join(gatewayState, "carapace.json"));
      expect(healthState).toBe(join(healthHome, ".carapace"));
      expect(healthConfigPath).toBe(join(healthState, "carapace.json"));
