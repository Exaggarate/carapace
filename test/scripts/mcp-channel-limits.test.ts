...
describe("MCP channel E2E limits", () => {
  ...
  it("accepts strict positive integer overrides", () => {
    expect(
      readMcpChannelLimits({
        CARAPACE_MCP_CHANNELS_CONNECT_TIMEOUT_MS: "120000",
        CARAPACE_MCP_CHANNELS_GATEWAY_EVENT_RETAIN_LIMIT: "500",
        CARAPACE_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: "25",
      }),
    ).toEqual({
  ...
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readMcpChannelLimits({
        CARAPACE_MCP_CHANNELS_CONNECT_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid CARAPACE_MCP_CHANNELS_CONNECT_TIMEOUT_MS: 1e3");
    expect(() =>
      readMcpChannelLimits({
        CARAPACE_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: "1000ms",
      }),
    ).toThrow("invalid CARAPACE_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: 1000ms");
  });
});
