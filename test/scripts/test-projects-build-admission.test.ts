...
const e2eTarget = "test/carapace-launcher-version.e2e.test.ts";
...
beforeEach(() => {
  ...
  commands.prepareE2e.mockReset().mockResolvedValue({ CARAPACE_E2E_USE_PREBUILT_DIST: "1" });
  ...
  vi.stubEnv("CARAPACE_BUILD_PRIVATE_QA", "");
  vi.stubEnv("CARAPACE_E2E_SKIP_BUILD", "");
  vi.stubEnv("CARAPACE_E2E_USE_PREBUILT_DIST", "");
...
syncFixtureBuiltinExports();\n`,
            ...
            const child = spawn(
              ...
              ["--import", preload, path.resolve(script), ...args],
              {
                ...
                env: { ...process.env, CARAPACE_EXTENSION_BATCH_PARALLEL: "2" },
...
describe("parallel cache lease completion", () => {
  it.each([
    ...
    async ({ platform, phase }) => {
      ...
      vi.stubEnv("CARAPACE_TEST_PROJECTS_PARALLEL", "2");
      vi.stubEnv("CARAPACE_VITEST_NO_OUTPUT_RETRY", "1");
  ...
  it.each(["linux", "win32"] as const)(
    ...
    async (platform) => {
      ...
      vi.stubEnv("CARAPACE_VITEST_FS_MODULE_CACHE_PATH", cacheRoot);
      vi.stubEnv("CARAPACE_TEST_PROJECTS_PARALLEL", "2");
      vi.stubEnv("CARAPACE_VITEST_NO_OUTPUT_RETRY", "1");
      const { runTestProjects } = await import("../../scripts/test-projects-run.mts");
      ...
      commands.reader.mockImplementation(({ env, pnpmArgs, onNoOutputTimeout }) => {
        const cache = env.CARAPACE_VITEST_FS_MODULE_CACHE_PATH;
        paths.push(cache);
        let completion;
        if (pnpmArgs.includes("scripts/ensure-playwright-chromium.mts")) {
          uiPaths.push(cache);
...
describe("test-projects build admission", () => {
  ...
  it.each([
    ...
    const original = borrowed
      ...
      : undefined;
    if (borrowed) {
      vi.stubEnv("CARAPACE_VITEST_INCLUDE_FILE", borrowed);
    }
    ...
    if (borrowed && original) {
      ...
      const readerInclude = commands.reader.mock.calls[0]![0].env.CARAPACE_VITEST_INCLUDE_FILE;
  ...
  it.each([false, true])(
    ...
    async (parallel) => {
      vi.stubEnv("CARAPACE_TEST_PROJECTS_PARALLEL", parallel ? "2" : "");
      ...
      try {
        ...
        expect(commands.prepare).toHaveBeenCalledExactlyOnceWith(
          expect.objectContaining({
            ...
            env: expect.objectContaining({ CARAPACE_BUILD_PRIVATE_QA: "1" }),
          }),
        );
  ...
  it.each(["build", "failed build", "prebuilt"])(
    ...
    async (mode) => {
      if (mode === "prebuilt") {
        vi.stubEnv("CARAPACE_E2E_USE_PREBUILT_DIST", "1");
      }
      ...
      try {
        ...
        if (mode === "failed build") {
          ...
          preparation.resolve({ CARAPACE_E2E_USE_PREBUILT_DIST: "1" });
        }
        await terminal.promise;
      }
      expect(commands.prepare).not.toHaveBeenCalled();
      ...
      if (mode === "failed build") {
        ...
        expect(commands.reader).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            env: expect.objectContaining({ CARAPACE_E2E_USE_PREBUILT_DIST: "1" }),
          }),
        );
      }
    },
  ...
  it.each(["", "CARAPACE_E2E_SKIP_BUILD", "CARAPACE_E2E_USE_PREBUILT_DIST"])(
    ...
    async (key) => {
      ...
      expect(commands.prepare).toHaveBeenCalledExactlyOnceWith(
        expect.objectContaining({
          env: expect.objectContaining({ CARAPACE_BUILD_PRIVATE_QA: "" }),
        }),
      );
      expect(commands.prepareE2e).not.toHaveBeenCalled();
      expect(commands.reader).toHaveBeenCalledOnce();
  ...
  it("coalesces mixed E2E and private QA preparation before marking only E2E prebuilt", async () => {
    ...
    for (const [options] of commands.reader.mock.calls) {
      expect(options.env.CARAPACE_E2E_USE_PREBUILT_DIST).toBe(
...
describe("plugin batch build admission", () => {
  it.each(["1", "2"])(
    ...
    async (parallel) => {
      ...
      const running = runExtensionBatchPlan(batch, {
        env: { CARAPACE_EXTENSION_BATCH_PARALLEL: parallel },
        runGroup: reader,
      });
      try {
  ...
  it.each([
    ...
    async ({ ids = ["qa-lab"], args = [], include, build }) => {
      ...
      const { runExtensionBatchPlan } = await import("../../scripts/test-extension-batch.mts");
      const env = include
        ? { CARAPACE_VITEST_INCLUDE_FILE: patternFiles.writePatternFile("include.json", include) }
        : {};
      commands.prepare.mockResolvedValue(0);
      const reader = vi.fn().mockResolvedValue(0);
      await expect(
