...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "carapace-ios-run-"));
  tempDirs.push(root);
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${CARAPACE_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.CARAPACE_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${CARAPACE_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/Carapace.app"
mkdir -p "$app_dir"
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_CARAPACE_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("installs and launches the configured app bundle identifier", () => {
    const fixture = makeFixture("ai.carapacefoundation.app");

    runIosRun(fixture, { SIMCTL_BOOT_MODE: "booted" });

    expect(readFileSync(fixture.logFile, "utf8")).toContain(
      "simctl launch iPhone 17 ai.carapacefoundation.app",
    );
  });
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        CARAPACE_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
    ...
    expect(log).toContain("CARAPACE_PUSH_MODE=simulatorSandbox");
    expect(log).toContain(
      "CARAPACE_PUSH_RELAY_BASE_URL=https://github.com/Exaggarate/carapace",
    );
    expect(log).toContain("simctl launch iPhone 17 ai.carapacefoundation.app");
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      CARAPACE_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      CARAPACE_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",
