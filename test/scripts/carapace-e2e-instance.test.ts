...
const helperPath = path.resolve("scripts/lib/carapace-e2e-instance.sh");
const hostPath = [
...
function runHelper(payload: string) {
  return runSourcedHelper(
    [
      `carapace_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
      'printf "value=%s" "${CARAPACE_E2E_INSTANCE_TEST:-unset}"',
    ].join("; "),
    null,
  );
}
...
function writePackageFixture(packagePath: string): void {
  withTempDir("carapace-e2e-package-", (root) => {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "carapace-e2e-fixture", version: "0.0.0" }),
    );
...
function createPackageInstallFixture(tempDir: string) {
  const fixture = {
    ...
    packagePath: path.join(tempDir, "carapace.tgz"),
...
function runPackageInstall(
  ...
  return runBashWithHelper(
    [
      `carapace_e2e_install_package ${shellQuote(fixture.logPath)} ${shellQuote("fixture package")} ${shellQuote(fixture.prefixPath)}`,
    ],
    { ...env, CARAPACE_E2E_REDACTOR_MODULE: fixture.redactorPath },
    undefined,
    "; ",
  );
}
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/carapace-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/carapace-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export CARAPACE_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid Carapace test-state base64 payload");
  });
  ...
  it("reads positive integer env values without treating decimal input as durations", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(carapace_e2e_read_positive_int_env CARAPACE_E2E_SAMPLE_SECONDS 180)"',
    );
    const leadingZero = runSourcedHelper(
      'printf "%s" "$(carapace_e2e_read_positive_int_env CARAPACE_E2E_SAMPLE_SECONDS 180)"',
      { CARAPACE_E2E_SAMPLE_SECONDS: "008" },
    );
    const duration = runSourcedHelper(
      "carapace_e2e_read_positive_int_env CARAPACE_E2E_SAMPLE_SECONDS 180",
      { CARAPACE_E2E_SAMPLE_SECONDS: "30s" },
    );
    ...
    expect(duration.stderr).toContain("invalid CARAPACE_E2E_SAMPLE_SECONDS: 30s");
  });

  it("reads non-negative integer env values without accepting shell-style sizes", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(carapace_e2e_read_nonnegative_int_env CARAPACE_E2E_SAMPLE_BYTES 262144)"',
    );
    const zero = runSourcedHelper(
      'printf "%s" "$(carapace_e2e_read_nonnegative_int_env CARAPACE_E2E_SAMPLE_BYTES 262144)"',
      { CARAPACE_E2E_SAMPLE_BYTES: "0" },
    );
    const size = runSourcedHelper(
      "carapace_e2e_read_nonnegative_int_env CARAPACE_E2E_SAMPLE_BYTES 262144",
      { CARAPACE_E2E_SAMPLE_BYTES: "64kb" },
    );
    ...
    expect(size.stderr).toContain("invalid CARAPACE_E2E_SAMPLE_BYTES: 64kb");
  });

  it("probes default and explicit mock OpenAI base URLs", () => {
    withTempDir("carapace-e2e-mock-openai-url-", (tempDir) => {
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runSourcedHelper(
        [
          `carapace_e2e_probe_http() { printf "%s\\n" "$1" >>${shellQuote(probePath)}; return 0; }`,
          "carapace_e2e_wait_mock_openai 44080 1 400",
          "carapace_e2e_wait_mock_openai 443 1 400 https://api.openai.com:443",
        ].join("; "),
      );
  ...
  it("probes /readyz on the explicit gateway port", () => {
    withTempDir("carapace-e2e-readyz-port-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runBashWithHelper(
        [
          ...
          `carapace_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
        ],
        {},
        5_000,
      );
  ...
  it.each([
    ...
    withTempDir("carapace-e2e-readyz-legacy-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      ...
      const result = runBashWithHelper(
        [
          ...
          `if carapace_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 "$port" ${shellQuote(mode)}; then`,
  ...
  it("wraps package installs with the configured timeout", () => {
    withTempDir("carapace-e2e-instance-", (tempDir) => {
      ...
      const result = runPackageInstall(fixture, {
        ...
        CARAPACE_E2E_NPM_INSTALL_TIMEOUT: "42s",
        CARAPACE_TEST_TIMEOUT_ARGS: fixture.timeoutArgsPath,
        CARAPACE_TEST_NPM_ARGS: fixture.npmArgsPath,
        CARAPACE_TEST_NPM_BIN: path.join(tempDir, "npm"),
      });
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    withTempDir("carapace-e2e-instance-plain-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    withTempDir("carapace-e2e-instance-gtimeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    withTempDir("carapace-e2e-instance-no-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    withTempDir("carapace-e2e-instance-install-log-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
      writeFakeTimeout(path.join(tempDir, "timeout"), true);
      ...
      const result = runPackageInstall(fixture, {
        PATH: `${tempDir}${path.delimiter}${hostPath}`,
        CARAPACE_CURRENT_PACKAGE_TGZ: fixture.packagePath,
        CARAPACE_E2E_LOG_TAIL_BYTES: "80",
        CARAPACE_E2E_NPM_INSTALL_TIMEOUT: "42s",
        CARAPACE_TEST_TIMEOUT_ARGS: fixture.timeoutArgsPath,
      });
  ...
  it.each([
    ["bytes", "CARAPACE_E2E_LOG_TAIL_BYTES", "64kb"],
    ["lines", "CARAPACE_E2E_LOG_TAIL_LINES", "25 lines"],
  ])("rejects invalid E2E log tail %s before invoking tail", (_label, envName, value) => {
    withTempDir("carapace-e2e-instance-log-tail-", (tempDir) => {
      const logPath = path.join(tempDir, "install.log");
      fs.writeFileSync(logPath, "old log\nrecent log\n", "utf8");

      const result = runSourcedHelper(`carapace_e2e_print_log ${shellQuote(logPath)}`, {
        [envName]: value,
      });
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    withTempDir("carapace-e2e-instance-node-watchdog-", (tempDir) => {
      writeNodeShim(tempDir);
      const startedAt = Date.now();
      const result = runBashWithHelper(
        [
          `carapace_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
        ],
        { PATH: tempDir },
        5_000,
        "; ",
      ...
      expect(result.stderr).toContain("Carapace E2E command timed out after 200ms");
    });
  });

  it("preserves child stdin, arguments, and exit status through the Node watchdog", () => {
    withTempDir("carapace-e2e-instance-node-watchdog-stdin-", (tempDir) => {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `carapace_e2e_maybe_timeout 5s node -e ${shellQuote(child)} -- ${args.map(shellQuote).join(" ")}`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("treats Node-watchdog timeout values as data, not Node options", () => {
    withTempDir("carapace-e2e-instance-node-watchdog-option-", (tempDir) => {
      writeNodeShim(tempDir);
      const result = runBashWithHelper(
        ["carapace_e2e_maybe_timeout --version node --version"],
        { PATH: tempDir },
        5_000,
      );
      expect(result.status).toBe(1);
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      withTempDir("carapace-e2e-instance-node-watchdog-signal-", (tempDir) => {
        writeNodeShim(tempDir);
...
set -euo pipefail
source ${shellQuote(helperPath)}
export CARAPACE_E2E_TIMEOUT_KILL_GRACE_MS=100
carapace_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    withTempDir("carapace-e2e-gateway-terminate-", (tempDir) => {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$CARAPACE_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
carapace_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
carapace_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$CARAPACE_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = runBash(
        script,
        {
          ...
          CARAPACE_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        },
        5_000,
      );
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(carapace_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
carapace_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    withTempDir("carapace-e2e-http-probe-", (tempDir) => {
      const portPath = path.join(tempDir, "port.txt");
      ...
      const result = runBash(
        [
          ...
          `port="$(cat ${shellQuote(portPath)})"`,
          `source ${shellQuote(helperPath)}`,
          'carapace_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
        ],
        undefined,
        3_000,
        "; ",
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    withTempDir("carapace-e2e-readyz-incremental-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const grepArgsPath = path.join(tempDir, "grep-args.txt");
      const result = runBashWithHelper(
        [
          "carapace_e2e_probe_http() { return 0; }",
          "grep() {",
          ...
          `printf 'old log line\\n' >${shellQuote(logPath)}`,
          `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
          `carapace_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
        ],
        {},
        5_000,
      );
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    withTempDir("carapace-e2e-readyz-split-marker-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const result = runBashWithHelper(
        [
          "carapace_e2e_probe_http() { return 0; }",
          ...
          `carapace_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
        ],
        {},
        5_000,
      );
  ...
  it("derives the readiness port only from gateway ready log lines", () => {
    withTempDir("carapace-e2e-readyz-port-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          `carapace_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
          "sleep 30 &",
          'gateway_pid="$!"',
          "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged Carapace E2E commands with the configured timeout", () => {
    withTempDir("carapace-e2e-instance-run-logged-", (tempDir) => {
      ...
      writeTimeoutDispatcher(tempDir, "fixture-command", "CARAPACE_TEST_COMMAND_BIN");
      writeBashExecutable(path.join(tempDir, "fixture-command"), [
        'printf "%s\\n" "$*" >"$CARAPACE_TEST_COMMAND_ARGS"',
        'printf "fixture output\\n"',
      ]);

      const result = runBashWithHelper(
        [
          `carapace_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
          `printf "%s" "$CARAPACE_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          CARAPACE_E2E_LOG_DIR: logDir,
          CARAPACE_E2E_COMMAND_TIMEOUT: "17s",
          CARAPACE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          CARAPACE_TEST_COMMAND_ARGS: commandArgsPath,
          CARAPACE_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
        },
        undefined,
        "; ",
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^carapace-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    withTempDir("carapace-e2e-instance-run-log-tail-", (tempDir) => {
      const logLabel = path.basename(tempDir);
      const logDir = path.join(tempDir, "logs");
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = runBashWithHelper(
        [`carapace_e2e_run_logged ${shellQuote(logLabel)} fixture-command`],
        {
          PATH: `${tempDir}${path.delimiter}${hostPath}`,
          CARAPACE_E2E_COMMAND_TIMEOUT: "17s",
          CARAPACE_E2E_LOG_DIR: logDir,
          CARAPACE_E2E_LOG_TAIL_BYTES: "80",
          CARAPACE_E2E_REDACTOR_MODULE: redactorPath,
          CARAPACE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
        },
        undefined,
        "; ",
      ...
      const logFile = expectDefined(fs.readdirSync(logDir)[0], "Carapace E2E command log file");
  ...
  it.each([false, true])(
    ...
    (initializeStdin) => {
      withTempDir("carapace-e2e-instance-run-log-redact-", (tempDir) => {
        const redactorPath = path.join(tempDir, "redactor.mjs");
        fs.writeFileSync(
          redactorPath,
        ...
        const result = runBashWithHelper(
          ["carapace_e2e_run_logged redacted-failure fixture-command"],
          {
            PATH: `${tempDir}${path.delimiter}${hostPath}`,
            CARAPACE_E2E_LOG_DIR: path.join(tempDir, "logs"),
            CARAPACE_E2E_REDACTOR_MODULE: redactorPath,
            CARAPACE_TEST_TIMEOUT_ARGS: path.join(tempDir, "timeout-args.txt"),
          },
          undefined,
          "; ",
        );
  ...
  it("installs the trash shim under isolated test state", () => {
    withTempDir("carapace-e2e-trash-shim-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          ...
          `printf "%s" "$CARAPACE_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
          "command -v trash >/dev/null",
        ],
        { HOME: homeDir, CARAPACE_STATE_DIR: stateDir, PATH: hostPath },
        undefined,
        "; ",
      );
      ...
      expect(binDir).not.toBe("/tmp/carapace-bin");
      expect(pathEntries.filter((entry) => entry === binDir)).toHaveLength(1);
      expect(fs.existsSync(path.join(binDir, "trash"))).toBe(true);
    });
  });

  it("wraps package-installed Carapace CLI calls with the configured timeout", () => {
    withTempDir("carapace-e2e-instance-carapace-cli-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "carapace-args.txt");
      writeTimeoutDispatcher(tempDir, path.join(tempDir, "carapace"), "CARAPACE_TEST_CARAPACE_BIN");
      writeBashExecutable(path.join(tempDir, "carapace"), [
        'printf "%s\\n" "$*" >"$CARAPACE_TEST_COMMAND_ARGS"',
      ]);
      ...
      const result = runBashWithHelper(
        ...
        {
          PATH: `${tempDir}:${hostPath}`,
          CARAPACE_E2E_COMMAND_TIMEOUT: "23s",
          CARAPACE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          CARAPACE_TEST_COMMAND_ARGS: commandArgsPath,
          CARAPACE_TEST_CARAPACE_BIN: path.join(tempDir, "carapace"),
        },
        undefined,
        "; ",
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "carapace")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    withTempDir("carapace-e2e-instance-pty-timeout-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const scriptArgsPath = path.join(tempDir, "script-args.txt");
      const logPath = path.join(tempDir, "pty.log");
      writeTimeoutDispatcher(tempDir, "script", "CARAPACE_TEST_SCRIPT_BIN");
      writeBashExecutable(path.join(tempDir, "script"), [
        'if [ "${1:-}" = "--version" ]; then exit 0; fi',
        'printf "%s\\n" "$*" >"$CARAPACE_TEST_SCRIPT_ARGS"',
      ]);

      const result = runBashWithHelper(
        [
          `carapace_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          CARAPACE_E2E_COMMAND_TIMEOUT: "31s",
          CARAPACE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          CARAPACE_TEST_SCRIPT_ARGS: scriptArgsPath,
          CARAPACE_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
        },
        undefined,
        "; ",
      );
