...
const WORKFLOW_PATH = ".github/workflows/carapace-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const trustedWorkflowRef = `${repository}/.github/workflows/carapace-release-telegram-qa.yml@${workflowRefName}`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/carapace-release-checks.yml@${workflowRefName}`;
  const workdir = tempDirs.make("carapace-telegram-identity-");
  ...
  const payload = {
    aud: "carapace-release-telegram-qa",
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("carapace-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
function runCandidateProvenance(
  ...
  const workdir = tempDirs.make("carapace-telegram-provenance-");
  const fakeBin = join(workdir, "bin");
  mkdirSync(fakeBin);
          ...
          associatedPullRequests: {
            nodes: [
                ...
                ? [
                    {
                      ...
                      headRepository: { nameWithOwner: "carapace/carapace" },
                    },
                  ]
                : []),
              ...(params.mergedPullRequests ?? []).map((pullRequest) => ({
                ...
                baseRepository: {
                  nameWithOwner: pullRequest.baseRepository ?? "carapace/carapace",
                },
...
describe("release Telegram QA workflow", () => {
  it("keeps the workflow wiring explicit and secret-scoped", () => {
    ...
    expect(extractCandidate.run).not.toContain("carapace-release-telegram-qa.yml@refs/heads/main");
    ...
    for (const provenanceBlock of PROVENANCE_BLOCKS) {
      const cases = [
        ...
        },
        {
          ...
          params: {
            candidateVersion: "2026.7.1",
            mergedPullRequests: [{ baseRepository: "fork/carapace" }],
            signature: "web-flow" as const,
            targetContextRef: "release/2026.7.1",
          },
        },
  ...
  it.runIf(process.platform === "linux")("retains only bounded, allowlisted diagnostics", () => {
    ...
    const workdir = tempDirs.make("carapace-telegram-diagnostics-");
  ...
  it.runIf(process.platform === "linux")("keeps the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("carapace-telegram-log-selector-");
    ...
    const paths = Array.from({ length: 12 }, (_, index) => {
      const path = join(runtimeRoot, "tmp", `carapace-${index}.log`);
  ...
  it("keeps generated SUT programs syntactically valid", () => {
    ...
    const workdir = tempDirs.make("carapace-telegram-preload-");
    const preloadPath = join(workdir, "preload.mjs");
    writeFileSync(preloadPath, preload);
    const env = { ...process.env };
    delete env.CARAPACE_QA_SUT_PREENTRY_STOP;
    expect(
  ...
  it("lets the SUT create suite locks without exposing the runner-owned config", () => {
    ...
    expect(createSut).toContain('"${temp_root}/state/qa-auth-bootstrap/carapace.json")');
    expect(createSut).toContain(
    ...
    expect(createSut).toContain('export CARAPACE_CONFIG_PATH="$projection_dir/carapace.json"');
    expect(createSut).toContain('"${CARAPACE_STATE_DIR}/qa-runtime-config/carapace.json") ;;');
  });
  ...
  it("mounts an isolated SUT-owned tmp without exposing the host tmp tree", () => {
    ...
    expect(launcher).toContain("carapace-telegram-sut-tmp");
