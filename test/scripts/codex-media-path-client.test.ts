...
describe("codex media path limits", () => {
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readPositiveIntEnv("CARAPACE_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", 180, {
        CARAPACE_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "1e3",
      }),
    ).toThrow("invalid CARAPACE_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
    expect(() =>
      readPositiveIntEnv("CARAPACE_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", 2 * 1024 * 1024, {
        CARAPACE_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: "64bytes",
      }),
    ).toThrow("invalid CARAPACE_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: 64bytes");
  });
  ...
  it("writes strict positive timeout and port values into generated config", () => {
    const root = tempRoots.make("carapace-codex-media-path-");
    const result = runWriteConfig(root, {
      CARAPACE_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "240",
      PORT: "19002",
    });
  ...
  it("rejects loose write-config timeout env values", () => {
    ...
    expect(result.stderr).toContain("invalid CARAPACE_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
  });
...
describe("codex media path fake app-server", () => {
  it("advertises the managed Codex version across initialization and thread creation", async () => {
    const requestLog = path.join(tempRoots.make("carapace-codex-media-path-"), "requests.jsonl");
    ...
    const child = spawn(process.execPath, [fakeAppServerPath], {
      env: { ...process.env, CARAPACE_CODEX_MEDIA_PATH_APP_SERVER_LOG: requestLog },
  ...
  it("returns a structured error when request logging fails", async () => {
    const requestLogDirectory = tempRoots.make("carapace-codex-media-path-");
    const child: ChildProcessWithoutNullStreams = spawn(process.execPath, [fakeAppServerPath], {
      env: {
        ...process.env,
        CARAPACE_CODEX_MEDIA_PATH_APP_SERVER_LOG: requestLogDirectory,
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
...
describe("codex media path JSONL tailer", () => {
  it("keeps parsed app-server requests and reads only appended lines", () => {
    const logPath = path.join(tempRoots.make("carapace-codex-media-path-"), "app-server.jsonl");
