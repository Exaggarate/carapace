...
`,
  ...
  const { createCarapaceTestInstance } = await import(
    fromRepo("test/helpers/carapace-test-instance.ts")
  );
  const { hasUnjoinedWork } = await import(fromRepo("scripts/lib/managed-child-process.mts"));
  instance = await createCarapaceTestInstance({ name: "native-cli-output-owner", cwd });
