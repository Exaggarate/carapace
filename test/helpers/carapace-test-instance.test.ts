// Carapace test instance tests cover spawned test instance lifecycle.
...
import { createCarapaceTestInstance, testing } from "./carapace-test-instance.js";
...
const RESTART_MARKER =
  "[carapace-test-instance] restarting gateway after migration convergence refusal";
const fakeInstances: {
...
async function createFakeGateway(
  ...
  const cwd = await fs.mkdtemp(path.join(tmpdir(), "carapace-test-instance-gateway-"));
  fakeRoots.push(cwd);
...
const registry = ${JSON.stringify(process.env.CARAPACE_HELPER_PROOF_PID_REGISTRY ?? null)};
...
import { createServer } from "node:http";
${processReceipt}
const tracePath = process.env.CARAPACE_FAKE_GATEWAY_TRACE;
const controlUrl = process.env.CARAPACE_FAKE_GATEWAY_CONTROL;
...
const env = Object.fromEntries(["HOME", "CARAPACE_CONFIG_PATH", "CARAPACE_GATEWAY_TOKEN", "CARAPACE_STATE_DIR"].map((key) => [key, process.env[key]]));
appendFileSync(tracePath, JSON.stringify({ argv, config: JSON.parse(readFileSync(process.env.CARAPACE_CONFIG_PATH, "utf8")), cwd: process.cwd(), env, pid: process.pid, port }) + "\\n");
const kind = (process.env.CARAPACE_FAKE_GATEWAY_SEQUENCE || "ready").split(",")[attempt - 1] || "ready";
if (kind === "cli-json") {
...
`,
  ...
  const instance = await createCarapaceTestInstance({
    ...
    cwd,
    env: {
      CARAPACE_FAKE_GATEWAY_SEQUENCE: sequence,
      CARAPACE_FAKE_GATEWAY_TRACE: tracePath,
      CARAPACE_FAKE_GATEWAY_CONTROL: control?.url,
    },
    startTimeoutMs,
    stopTimeoutMs,
  });
...
describe("carapace test instance", () => {
  ...
  it.runIf(process.platform !== "win32").for(["inherit", "ignore"] as const)(
    ...
    async (stderrMode, { signal }) => {
      ...
      const exerciseGroup = async () => {
        ...
        const registry = process.env.CARAPACE_HELPER_PROOF_PID_REGISTRY;
        if (registry) {
          await fs.appendFile(
            `${registry}.stops`,
  ...
  it.each([
    ...
    try {
      ...
      if (!stopped) {
        const prefix = "[carapace-test-instance] Windows shutdown ";
  ...
  describe("UTF-8 log trimming", () => {
    ...
    beforeEach(async ({ signal }) => {
      ...
      import { testing } from ${JSON.stringify(new URL("./carapace-test-instance.ts", import.meta.url).href)};
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".carapace"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "carapace.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.CARAPACE_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.CARAPACE_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.CARAPACE_SKIP_CRON).toBe("0");
