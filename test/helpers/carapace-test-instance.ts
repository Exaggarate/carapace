// Carapace test instance helper spawns isolated Carapace processes.
...
type CarapaceTestStateOptions = NonNullable<Parameters<typeof createCarapaceTestState>[0]>;

type CarapaceTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<CarapaceTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
...
type CarapaceTestInstanceCommandResult = {
  code: number | null;
...
type CarapaceTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type CarapaceTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: CarapaceTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<CarapaceTestInstanceCommandResult>;
...
type CarapaceTestProcessReadiness = Pick<CarapaceTestProcess, "exitCode" | "signalCode"> & {
...
async function waitForGatewayReady(
  proc: CarapaceTestProcessReadiness,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
function hasGatewayProcessClosed(child: CarapaceTestProcess, platform: NodeJS.Platform): boolean {
...
async function stopGatewayProcess(
  ...
  if (platform === "win32") {
    ...
    const failed = (
      ...
      appendLogChunk(
        stopLog,
        `[carapace-test-instance] Windows shutdown ${JSON.stringify(diagnostic)}\n`,
      );
      return false;
    };
...
function hasChildExited(child: Pick<CarapaceTestProcess, "exitCode" | "signalCode">) {
...
export async function createCarapaceTestInstance(
  options: CarapaceTestInstanceOptions,
): Promise<CarapaceTestInstance> {
  ...
  let child: { process: CarapaceTestProcess; ready: boolean } | undefined;
  const commands = new Set<Promise<CarapaceTestInstanceCommandResult>>();
  ...
  const spawnGatewayProcess = (args: string[], attemptStderr: string[]): CarapaceTestProcess => {
    const [command = "node", ...prefixArgs] = options.gatewayCommandPrefix ?? [];
    const next = spawn(command, [...prefixArgs, ...args], {
      cwd,
      env,
      stdio: ["ignore", "pipe", "pipe"],
      detached: shouldUseCarapaceTestProcessGroup(),
    });
  ...
  const instance: CarapaceTestInstance = {
    name: options.name,
    port,
    ...
    startGateway: () => {
      ...
      return enqueue("start", async () => {
        ...
        while (true) {
          ...
          await releasePort();
          let attempt: CarapaceTestProcess;
          try {
...
function shouldUseCarapaceTestProcessGroup(): boolean {
