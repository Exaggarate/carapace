...
const embeddedAgentMocks = getSharedMocks("carapace.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("carapace.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("carapace.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("carapace.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    carapaceHome: process.env.CARAPACE_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("CARAPACE_HOME", snapshot.carapaceHome);
  restoreKey("CARAPACE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.CARAPACE_STATE_DIR = join(home, ".carapace");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "carapace-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".carapace", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): CarapaceConfig {
