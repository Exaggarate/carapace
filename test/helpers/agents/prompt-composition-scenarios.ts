...
async function createToolRichScenario(workspaceDir: string): Promise<PromptScenario> {
  const skillsPrompt = [
    ...
    "<skill><name>release</name><description>Release Carapace safely.</description><location>/skills/release/SKILL.md</location></skill>",
    "</available_skills>",
  ].join("\n");
...
async function createWorkspaceWithPromptCompositionFiles(): Promise<string> {
  const workspaceDir = await makeTempWorkspace("carapace-prompt-cache-");
  await writeWorkspaceFile({
    dir: workspaceDir,
    name: "AGENTS.md",
    content: [
...
export async function createPromptCompositionScenarios(): Promise<{
  ...
  const warningWorkspaceDir = await makeTempWorkspace("carapace-prompt-cache-warning-");
  const scenarios = [
