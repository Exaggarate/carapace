// Carapace launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "carapace-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "carapace.mjs"),
    path.join(fixtureRoot, "carapace.mjs"),
  );
  await fs.copyFile(
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/carapace\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.CARAPACE_COMPILE_CACHE_DISABLED_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
function launcherEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.CARAPACE_BUNDLED_PLUGINS_DIR;
  delete env.CARAPACE_CONFIG_PATH;
  delete env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
  delete env.CARAPACE_HOME;
  delete env.CARAPACE_STATE_DIR;
  delete env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
describe("carapace launcher", () => {
  ...
  it("keeps the bootstrap Node range aligned with the package engine", async () => {
    ...
    for (const version of NODE_RELEASE_VERSION_CASES) {
      ...
      if (isSupportedCarapaceNodeVersion(version)) {
        ...
        expect(result.stderr, version).toContain(
          `carapace: Node.js >=24.16.0 <25, or >=26.1.0 is required (current: v${version}).`,
        );
      }
    }
  });
  ...
  it("prints recovery guidance before legacy-incompatible modules can load", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(legacyRuntimePath).href, path.join(fixtureRoot, "carapace.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stderr).toContain(
      "carapace: Node.js >=24.16.0 <25, or >=26.1.0 is required (current: v20.0.0).",
    );
  ...
  it("rejects Bun without node:sqlite even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "carapace.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-carapace-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-carapace-launcher-dep");
  ...
  it("executes an entry.mjs-only compiled entry through the root launcher", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", import.meta.resolve("tsx"), path.join(fixtureRoot, "carapace.mjs"), "--profile"],
      {
        cwd: process.cwd(),
        env: launcherEnv({ CARAPACE_NO_RESPAWN: "1" }),
        encoding: "utf8",
      },
    );
  ...
  it.runIf(process.env.CARAPACE_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
    ...
    async () => {
      ...
      const result = spawnSync(bunBin, [path.join(fixtureRoot, "carapace.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
      });
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["config", "doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "carapace.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), "--log-level", "warn", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), ...args], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ CARAPACE_CONTAINER: "demo" }),
        encoding: "utf8",
      },
    );
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ CARAPACE_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ CARAPACE_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it("checks the CARAPACE_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const carapaceHome = path.join(fixtureRoot, "home");
    const configDir = path.join(carapaceHome, ".carapace");
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ CARAPACE_HOME: carapaceHome }),
      encoding: "utf8",
    });
  ...
  it("keeps literal $ patterns in HOME when expanding a tilde CARAPACE_HOME", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const home = path.join(fixtureRoot, "home$&d");
    const configDir = path.join(home, "oc", ".carapace");
    await fs.mkdir(configDir, { recursive: true });
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
      JSON.stringify({ rootHelpText: "PRECOMPUTED memory help\n" }),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, CARAPACE_HOME: "~/oc" }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, CARAPACE_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:carapace/carapace#<ref>");
  });
  ...
  it("respawns source-checkout launchers without inherited NODE_COMPILE_CACHE", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "carapace.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv({
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "carapace-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "carapace.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
  ...
  it.runIf(process.platform !== "win32").each([
    ...
    await fs.writeFile(
      ...
      [
        'import { writeFileSync } from "node:fs";',
        `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
        'process.title = "carapace-launcher-default-signal-test-child";',
        "setInterval(() => {}, 1000);",
        "",
      ].join("\n"),
      "utf8",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          'process.title = "carapace-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          "setInterval(() => {}, 1000);",
          "",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "carapace-launcher-link-");
      const linkedRoot = path.join(linkParent, "carapace-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "carapace.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const globalRoot = makeTempDir(fixtureRoots, "carapace-pnpm-global-");
      const packageRoot = path.join(globalRoot, "v11", "active", "node_modules", "carapace");
      await fs.mkdir(path.dirname(packageRoot), { recursive: true });
      await fs.symlink(fixtureRoot, packageRoot, "dir");
      const launcher = path.join(packageRoot, "carapace.mjs");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        path.join(fixtureRoot, "dist", "entry.js"),
        'process.stdout.write(process.env.CARAPACE_PACKAGED_COMPILE_CACHE_RESPAWNED ?? "0");\n',
        "utf8",
      );

      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "carapace.mjs"), "hooks", "relay"],
        {
          cwd: fixtureRoot,
          env: launcherEnv({
            NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "carapace", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "carapace-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "carapace-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "carapace", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "carapace"));
  });
