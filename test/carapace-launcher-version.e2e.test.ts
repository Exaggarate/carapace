...
async function makeLauncherVersionFixture(
  ...
  const fixtureRoot = makeTempDir(fixtureRoots, "carapace-launcher-version-");
  await fs.copyFile(
  ...
  if (options.pendingLifecycle) {
    await fs.writeFile(path.join(fixtureRoot, ".carapace-lifecycle-pending"), "pending\n");
    ...
    await fs.writeFile(
      ...
      options.pendingLifecycle === "fail"
        ...
        : 'import fs from "node:fs/promises"; export async function completePendingPackageLifecycle({ packageRoot }) { await fs.rm(new URL(".carapace-lifecycle-pending", `file://${packageRoot}/`)); }\n',
    );
  }
...
function runLauncherVersion(
  ...
  return spawnSync(
    process.execPath,
    [path.join(fixtureRoot, "carapace.mjs"), options.flag ?? "--version"],
    {
      cwd: fixtureRoot,
      env: {
        ...process.env,
...
describe("carapace launcher version provenance", () => {
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.stdout).toBe(`Carapace ${packageVersion} (${buildCommit.slice(0, 7)})\n`);
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.status).toBe(0);
      expect(result.stdout).toBe(`Carapace ${packageVersion} (${checkoutCommit.slice(0, 7)})\n`);
      expect(result.stderr).toBe("");
    },
  );
  ...
  it("completes a pending package lifecycle before the version fast path", async () => {
    ...
    expect(result.stdout).toBe(`Carapace ${packageVersion} (${buildCommit.slice(0, 7)})\n`);
    await expect(
      fs.access(path.join(fixtureRoot, ".carapace-lifecycle-pending")),
  ...
  it("fails closed before reporting a version when package lifecycle completion fails", async () => {
    ...
    await expect(
      fs.readFile(path.join(fixtureRoot, ".carapace-lifecycle-pending"), "utf8"),
  ...
  it.each([
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toBe(`Carapace ${packageVersion} (${expected})\n`);
    expect(result.stderr).toBe("");
  });
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    const argumentResult = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "carapace.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: { ...process.env, CARAPACE_CONTAINER: undefined },
        encoding: "utf8",
      },
    );
