// Carapace npm release check tests validate package release checks.
...
import { assertPreparedCarapaceAiDependency } from "../scripts/carapace-npm-prepublish-verify.ts";
import {
...
} from "../scripts/carapace-npm-release-check.ts";
...
describe("prepared Carapace AI dependency", () => {
  it("requires the packed root to depend on the exact prepared AI version", () => {
    expect(() =>
      assertPreparedCarapaceAiDependency({
        aiManifest: { name: "@carapace/ai", version: "2026.7.2" },
        rootManifest: {
          name: "carapace",
          version: "2026.7.1",
          dependencies: { "@carapace/ai": "2026.7.2" },
        },
      }),
    ).toThrow("Prepared root and @carapace/ai tarballs must both be version 2026.7.2.");
    ...
    expect(() =>
      assertPreparedCarapaceAiDependency({
        ...
        rootManifest: {
          name: "carapace",
          version: "2026.7.2",
          dependencies: { "@carapace/ai": "2026.7.1" },
        },
      }),
    ).toThrow("Prepared root tarball must depend on exact @carapace/ai@2026.7.2.");
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        CARAPACE_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("resolveNpmCommandInvocation", () => {
  it("uses npm_execpath when it points to npm", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "carapace", "version"],
        nodeExecPath: "/usr/local/bin/node",
        platform: "linux",
      }),
    ).toEqual({
      command: "/usr/local/bin/node",
      args: ["/usr/local/lib/node_modules/npm/bin/npm-cli.js", "view", "carapace", "version"],
    });
  });
  ...
  it("wraps the Windows npm command when npm_execpath is missing", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "carapace@beta", "version"],
        npmExecPath: "",
        platform: "win32",
      }),
    ).toEqual({
      command: "C:\\Windows\\System32\\cmd.exe",
      args: ["/d", "/s", "/c", "npm.cmd view carapace@beta version"],
  ...
  it("wraps Windows npm_execpath command shims", () => {
    expect(
      resolveNpmCommandInvocation({
        comSpec: "C:\\Windows\\System32\\cmd.exe",
        npmArgs: ["install", "-g", "C:\\tmp\\carapace package.tgz"],
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\Program Files\\nodejs\\npm.cmd" install -g "C:\\tmp\\carapace package.tgz""',
      ],
      windowsVerbatimArguments: true,
    });
  });
  ...
  if (process.platform === "win32") {
    it("executes fallback npm.cmd through cmd.exe on Windows", () => {
      const dir = mkdtempSync(join(tmpdir(), "carapace-fake-npm-cmd-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.CARAPACE_FAKE_NPM_OUT, JSON.stringify(process.argv.slice(2)));",
          ].join("\n"),
        );
        writeFileSync(
          join(dir, "npm.cmd"),
        ...
        execFileSync(invocation.command, invocation.args, {
          ...
          env: {
            ...process.env,
            CARAPACE_FAKE_NPM_OUT: outputPath,
...
describe("resolveNpmReleaseCheckCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({ CARAPACE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "" }),
    ).toBe(10 * 60 * 1000);
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({
        CARAPACE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1234",
      }),
    ).toBe(1234);
    ...
    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolveNpmReleaseCheckCommandTimeoutMs({
          CARAPACE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: raw,
        }),
      ).toThrow(`invalid CARAPACE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("preserves filename-only pnpm receipts and npm size metadata", () => {
    const receipt = { filename: "carapace.tgz", unpackedSize: 120_354_302 };
  ...
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"carapace.tgz","files":[]}]')).toEqual([
      { filename: "carapace.tgz", files: [] },
    ]);
  });
  ...
  it("parses npm 12 name-keyed pack output", () => {
    expect(
      parseNpmPackJsonOutput(
        '{"carapace":{"filename":"carapace.tgz","files":[{"path":"dist/control-ui/index.html"}]}}',
      ),
    ).toEqual([
      {
  ...
  it("parses trailing npm 12 output after lifecycle logs", () => {
    const stdout = [
      "> carapace@2026.7.2 prepack",
      '{"carapace":{"filename":"carapace.tgz","files":[]}}',
    ].join("\n");

    expect(parseNpmPackJsonOutput(stdout)).toEqual([{ filename: "carapace.tgz", files: [] }]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> carapace@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"carapace.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> carapace@2026.3.23 prepack")).toBeNull();
  });
  ...
  it.each([{}, { path: 42 }])("rejects incomplete packed file inventories: %j", (file) => {
    expect(
      parseNpmPackJsonOutput(
        JSON.stringify([
          { filename: "carapace.tgz", files: [{ path: "dist/control-ui/index.html" }, file] },
        ]),
      ),
    ).toBeNull();
  });
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects private qa artifacts in npm pack output", () => {
    expect(
      collectForbiddenPackedPathErrors([
        ...
        "dist/Carapace.app/Contents/MacOS/Carapace",
      ...
      'npm package must not include local application build output "dist/Carapace.app/Contents/MacOS/Carapace".',
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "carapace-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "carapace-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/Exaggarate/carapace.git" },
