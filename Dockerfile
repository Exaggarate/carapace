...
# Example: docker build --build-arg CARAPACE_EXTENSIONS="diagnostics-otel,matrix" .
#
...
ARG CARAPACE_EXTENSIONS=""
ARG CARAPACE_BUNDLED_PLUGIN_DIR=extensions
ARG CARAPACE_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"
ARG CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""
ARG CARAPACE_DOCKER_BUILD_SKIP_DTS=1
ARG CARAPACE_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:934240a162082fd8b8a2f90cd5114446443f1eba1c5378f6687167ca405e6584"
ARG CARAPACE_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:3638d9a6fe4030bd716be989438248074489337ba3275657f93595428be4fc03"
ARG CARAPACE_NODE_BOOKWORM_SLIM_DIGEST="sha256:3638d9a6fe4030bd716be989438248074489337ba3275657f93595428be4fc03"
# Keep in sync with .github/actions/setup-node-env/action.yml bun-version.
# To update: docker buildx imagetools inspect docker.io/oven/bun:<version> and use the manifest-list digest.
ARG CARAPACE_BUN_IMAGE="docker.io/oven/bun:1.4.0@sha256:5ff609364c049b54eb0ff560ec96319729a972078ef2c755d758f0c6ef89c2d6"
...
FROM ${CARAPACE_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG CARAPACE_EXTENSIONS
ARG CARAPACE_BUNDLED_PLUGIN_DIR
...
COPY ${CARAPACE_BUNDLED_PLUGIN_DIR} /tmp/${CARAPACE_BUNDLED_PLUGIN_DIR}
RUN mkdir -p /out/packages "/out/${CARAPACE_BUNDLED_PLUGIN_DIR}" && \
    ...
    node /tmp/docker-plugin-selection.mjs "/tmp/${CARAPACE_BUNDLED_PLUGIN_DIR}" "$CARAPACE_EXTENSIONS" \
      > /out/carapace-selected-plugin-dirs && \
    node /tmp/docker-plugin-selection.mjs "/tmp/${CARAPACE_BUNDLED_PLUGIN_DIR}" "$CARAPACE_EXTENSIONS" \
      --required-platform-packages > /out/carapace-required-platform-packages && \
    node /tmp/docker-plugin-selection.mjs "/tmp/${CARAPACE_BUNDLED_PLUGIN_DIR}" "$CARAPACE_EXTENSIONS" \
      --required-bundled /tmp/package.json > /tmp/carapace-workspace-plugin-dirs && \
    while IFS= read -r ext; do \
      ext_dir="/tmp/${CARAPACE_BUNDLED_PLUGIN_DIR}/$ext"; \
      if [ -f "$ext_dir/package.json" ]; then \
        mkdir -p "/out/${CARAPACE_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "$ext_dir/package.json" "/out/${CARAPACE_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done < /tmp/carapace-workspace-plugin-dirs
...
FROM ${CARAPACE_NODE_BOOKWORM_IMAGE} AS dependency-inputs
ARG CARAPACE_BUNDLED_PLUGIN_DIR
...
COPY --from=workspace-deps /out/${CARAPACE_BUNDLED_PLUGIN_DIR}/ ./${CARAPACE_BUNDLED_PLUGIN_DIR}/
COPY --from=workspace-deps /out/carapace-selected-plugin-dirs /tmp/carapace-selected-plugin-dirs
COPY --from=workspace-deps /out/carapace-required-platform-packages /tmp/carapace-required-platform-packages

# ── Production dependencies ────────────────────────────────────
FROM dependency-inputs AS production-deps
RUN --mount=type=cache,id=carapace-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
...
# ── Build ──────────────────────────────────────────────────────
FROM ${CARAPACE_BUN_IMAGE} AS bun-binary
FROM dependency-inputs AS build
ARG CARAPACE_DOCKER_BUILD_NODE_OPTIONS
ARG CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB
ARG CARAPACE_DOCKER_BUILD_SKIP_DTS
...
ARG GIT_COMMIT=""
ARG CARAPACE_BUILD_TIMESTAMP=""
ARG CARAPACE_DOCKER_BUILD_VERSION=""
ENV GIT_COMMIT=${GIT_COMMIT} \
    CARAPACE_BUILD_TIMESTAMP=${CARAPACE_BUILD_TIMESTAMP}
...
RUN for dir in /app/${CARAPACE_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \
      if [ -d "$dir" ]; then \
...
RUN pnpm_config_verify_deps_before_run=false pnpm canvas:a2ui:bundle || \
    (echo "A2UI bundle: creating stub (non-fatal)" && \
     ...
     rm -rf vendor/a2ui apps/shared/CarapaceKit/Tools/CanvasA2UI)
# Force pnpm for UI build (Bun may fail on ARM/Synology architectures)
ENV CARAPACE_PREFER_PNPM=1
...
RUN set -eu; \
    selected_plugin_dirs="$(cat /tmp/carapace-selected-plugin-dirs)"; \
    if [ -n "$CARAPACE_DOCKER_BUILD_VERSION" ]; then \
      pnpm pkg set "version=$CARAPACE_DOCKER_BUILD_VERSION"; \
    fi; \
    if [ -z "$CARAPACE_BUILD_TIMESTAMP" ]; then \
      CARAPACE_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; \
      export CARAPACE_BUILD_TIMESTAMP; \
    fi; \
    if grep -qx 'qa-lab' /tmp/carapace-selected-plugin-dirs; then \
      export CARAPACE_BUILD_PRIVATE_QA=1 CARAPACE_ENABLE_PRIVATE_QA_CLI=1; \
    fi; \
    CARAPACE_INTERNAL_DOCKER_BUILD_PLUGIN_IDS="$selected_plugin_dirs" CARAPACE_RUN_NODE_SKIP_DTS_BUILD="$CARAPACE_DOCKER_BUILD_SKIP_DTS" CARAPACE_TSDOWN_MAX_OLD_SPACE_MB="$CARAPACE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB" NODE_OPTIONS="$CARAPACE_DOCKER_BUILD_NODE_OPTIONS" pnpm_config_verify_deps_before_run=false pnpm build:docker; \
    pnpm_config_verify_deps_before_run=false pnpm ui:build
RUN if grep -qx 'qa-lab' /tmp/carapace-selected-plugin-dirs; then \
...
RUN rm -rf node_modules ui/node_modules && \
    find packages "${CARAPACE_BUNDLED_PLUGIN_DIR}" -name node_modules -prune -exec rm -rf {} +
...
RUN node scripts/postinstall-bundled-plugins.mjs && \
    CARAPACE_EXTENSIONS="$(cat /tmp/carapace-selected-plugin-dirs)" CARAPACE_BUNDLED_PLUGIN_DIR="$CARAPACE_BUNDLED_PLUGIN_DIR" node scripts/prune-docker-plugin-dist.mjs && \
    find dist -type f \( -name '*.d.ts' -o -name '*.d.mts' -o -name '*.d.cts' -o -name '*.map' \) -delete && \
    if [ -L /app/node_modules/@carapace/ai ]; then \
      ai_runtime_target="$(readlink -f /app/node_modules/@carapace/ai)" && \
      ai_runtime_tmp="$(mktemp -d)" && \
      cp -a "$ai_runtime_target" "$ai_runtime_tmp/ai" && \
      rm /app/node_modules/@carapace/ai && \
      mv "$ai_runtime_tmp/ai" /app/node_modules/@carapace/ai && \
      rmdir "$ai_runtime_tmp"; \
    fi && \
    rm -rf \
      /app/node_modules/carapace \
      /app/node_modules/.bin/carapace \
      /app/node_modules/.pnpm/carapace@*/node_modules/carapace && \
    if ! grep -q '^@anthropic-ai/claude-agent-sdk-' /tmp/carapace-required-platform-packages; then \
...
# ── Runtime base image ──────────────────────────────────────────
FROM ${CARAPACE_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
ARG CARAPACE_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${CARAPACE_NODE_BOOKWORM_SLIM_DIGEST}"
...
LABEL org.opencontainers.image.source="https://github.com/Exaggarate/carapace" \
  org.opencontainers.image.url="https://github.com/Exaggarate/carapace" \
  org.opencontainers.image.documentation="https://github.com/Exaggarate/carapace" \
  org.opencontainers.image.licenses="MIT" \
  org.opencontainers.image.title="Carapace" \
  org.opencontainers.image.description="Carapace gateway and CLI runtime container image"
...
RUN --mount=type=cache,id=carapace-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=carapace-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/carapace.mjs .
COPY --from=runtime-assets --chown=node:node /app/${CARAPACE_BUNDLED_PLUGIN_DIR} ./${CARAPACE_BUNDLED_PLUGIN_DIR}
...
ARG CARAPACE_DOCKER_BUILD_VERSION
RUN if [ -n "$CARAPACE_DOCKER_BUILD_VERSION" ]; then \
      test "$(node -p "require(\"/app/package.json\").version")" = "$CARAPACE_DOCKER_BUILD_VERSION"; \
      test "$(node -p "require(\"/app/dist/build-info.json\").version")" = "$CARAPACE_DOCKER_BUILD_VERSION"; \
      test "$(node /app/carapace.mjs --version | cut -d ' ' -f 2)" = "$CARAPACE_DOCKER_BUILD_VERSION"; \
    fi
...
# Example: docker build --build-arg CARAPACE_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: CARAPACE_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG CARAPACE_IMAGE_APT_PACKAGES
ARG CARAPACE_DOCKER_APT_PACKAGES=""
ENV PATH="/home/node/.local/bin:${PATH}"
RUN --mount=type=cache,id=carapace-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=carapace-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${CARAPACE_IMAGE_APT_PACKAGES:-$CARAPACE_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg CARAPACE_IMAGE_PIP_PACKAGES="requests humanize" .
ARG CARAPACE_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=carapace-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=carapace-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$CARAPACE_IMAGE_PIP_PACKAGES" ]; then \
...
# Build with: docker build --build-arg CARAPACE_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG CARAPACE_INSTALL_BROWSER=""
ENV PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright
RUN --mount=type=cache,id=carapace-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=carapace-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$CARAPACE_INSTALL_BROWSER" ]; then \
      apt-get update && \
...
# Build with: docker build --build-arg CARAPACE_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG CARAPACE_INSTALL_DOCKER_CLI=""
ARG CARAPACE_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=carapace-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=carapace-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$CARAPACE_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update CARAPACE_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL --connect-timeout 10 --max-time 120 \
        https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$CARAPACE_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/carapace.mjs /usr/local/bin/carapace \
 && chmod 755 /app/carapace.mjs
...
# the leaf /home/node/.config/carapace inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.carapace \
      /home/node/.carapace/workspace \
      /home/node/.config/carapace && \
    stat -c '%U:%G %a' /home/node/.carapace | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.carapace/workspace | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.config | grep -qx 'node:node 755' && \
    stat -c '%U:%G %a' /home/node/.config/carapace | grep -qx 'node:node 700'
