...
function withCodexSupervisionEnabled(config: CarapaceConfig): CarapaceConfig {
  const next = structuredClone(normalizePluginTargetConfig(config, "codex")) as CarapaceConfig &
  ...
  if (process.env.CARAPACE_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS === "1") {
    supervision.allowRawTranscripts = true;
  }
  if (process.env.CARAPACE_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS === "1") {
    supervision.allowWriteControls = true;
  }
  return next;
}

function resolveCodexSupervisionTools(config: CarapaceConfig): AnyAgentTool[] {
  const context = {
    config,
...
function requireCompleteCodexSupervisionToolSet(tools: readonly AnyAgentTool[]): void {
  ...
  throw new Error(
    `Codex supervision MCP could not load the official @carapace/codex plugin tools (missing: ${missing.join(", ") || "none"}). Install or update @carapace/codex, then enable Codex supervision.`,
  );
}

export function createCodexSupervisionToolsMcpServer(
  params: { config?: CarapaceConfig; tools?: AnyAgentTool[] } = {},
): Server {
  ...
  return createToolsMcpServer({ name: "carapace-codex-supervisor", tools });
}
