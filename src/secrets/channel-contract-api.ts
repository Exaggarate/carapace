...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `carapace.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built carapace
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.CARAPACE_DEBUG_CHANNEL_CONTRACT_API === "1") {
