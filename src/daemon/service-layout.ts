/** Summarizes installed service command paths and Carapace package layout. */
...
async function resolveCarapacePackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveCarapacePackageRoot(entrypointReal) : undefined;
