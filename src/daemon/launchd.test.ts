...
function createLaunchdEnvWithGatewayPort(port: string): Record<string, string | undefined> {
  return { ...createDefaultLaunchdEnv(), CARAPACE_GATEWAY_PORT: port };
}
...
async function installLaunchAgent(
  ...
  const serviceTarget = `${domain}/ai.carapace.gateway`;
  if (
  ...
  const ownership =
    status === "installed"
      ? {
          ...
          plistPath: "/Library/LaunchDaemons/custom-carapace.plist",
        }
      : status === "unverifiable"
        ? {
            status,
        ...
        : { status, serviceTarget: "system/ai.carapace.gateway" };
...
describe("launchd process ancestry guards", () => {
  it.each([
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.carapace.gateway`;
      ...
      await withEnvAsync(
        ...
        async () => {
          await expect(
            ...
            "Refusing to stop LaunchAgent ai.carapace.gateway from inside the same launchd service",
          );
        },
      );

      expect(state.launchctlCalls).toEqual([["print", `${domain}/ai.carapace.gateway`]]);
    },
  );
      ...
      await withEnvAsync(
        ...
        async () => {
          await expect(
            ...
            `Refusing to ${action} LaunchAgent ai.carapace.gateway from inside ai.carapace.gateway`,
          );
        },
      );
  ...
  it("refuses install from a legacy Gateway ancestor after probing the target first", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.carapace.gateway`;
    const legacyServiceId = `${domain}/ai.carapace.legacy-gateway`;
    ...
    await withEnvAsync(
      {
        ...
        CARAPACE_SERVICE_MARKER: undefined,
        CARAPACE_SERVICE_KIND: undefined,
        CARAPACE_LAUNCHD_LABEL: "ai.carapace.legacy-gateway",
      },
      async () => {
        await expect(installLaunchAgent(defaultLaunchAgentFixture(env))).rejects.toThrow(
          "Refusing to install LaunchAgent ai.carapace.gateway from inside ai.carapace.legacy-gateway",
        );
      },
    );
...
describe("launchd runtime parsing", () => {
  it.each([
    ['disabled services = {\n\t"ai.carapace.gateway" => enabled\n}', true],
    ['disabled services = {\n\t"ai.carapace.gateway" => disabled\n}', false],
    ['disabled services = {\n\t"ai.carapace.gateway" => false\n}', true],
    ['disabled services = {\n\t"ai.carapace.gateway" => true\n}', false],
    ['disabled services = {\n\t"other.service" => disabled\n}', true],
  ])("parses the LaunchAgent enabled override", (output, expected) => {
    expect(parseLaunchAgentEnabled(output, "ai.carapace.gateway")).toBe(expected);
  });

  it("rejects an unrecognized LaunchAgent enabled override", () => {
    expect(() =>
      parseLaunchAgentEnabled(
        'disabled services = {\n\t"ai.carapace.gateway" => unexpected\n}',
        "ai.carapace.gateway",
      ),
...
describe("launchd runtime state", () => {
  it.runIf(process.platform === "darwin").each(["runtime", "enabled"] as const)(
    ...
    async (read) => {
      ...
      const tempDir = await realFs.mkdtemp(`${process.env.TMPDIR ?? "/tmp"}/carapace-launchd-`);
  ...
  it("reports an installed but unloaded LaunchAgent as stopped", async () => {
    ...
    state.printError = [
      "Bad request.",
      'Could not find service "ai.carapace.gateway" in domain for user gui: 501',
    ].join("\n");
    state.printFailuresRemaining = 1;
  ...
  it("reports a loaded system LaunchDaemon even when the user job is also loaded", async () => {
    ...
    expect(runtime).toEqual({
      status: "unknown",
      detail: "System LaunchDaemon system/ai.carapace.gateway already owns this gateway label.",
      systemLaunchDaemon: {
        status: "loaded",
        serviceTarget: "system/ai.carapace.gateway",
      },
...
describe("launchctl list detection", () => {
  it("parses stale Carapace updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListCarapaceUpdateJobs(
      [
        "123 0 ai.carapace.gateway",
        "- 127 ai.carapace.update.2026.5.12",
        "- 0 ai.carapace.manual-update.1717168800",
        "8142 0 ai.carapace.update.2026.5.13-beta.1",
        "915 0 ai.carapace.tayoun.update.20260625T201026-0400",
        "- 0 ai.carapace.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      ...
      {
        label: "ai.carapace.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale Carapace updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.carapace.update.2026.5.12\n";

      const jobs = await findStaleCarapaceUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const env = createDefaultLaunchdEnv();
      const updaterLabel = "ai.carapace.tayoun.update.20260625T201026-0400";
      const gatewayLikeLabel = "ai.carapace.dev.team.update.20260625T201026-0400";
      const nonCarapaceLabel = "ai.carapace.fake.update.20260625T201026-0400";
      const prefixedCliLabel = "ai.carapace.helper.update.20260625T201026-0400";
      state.listOutput = [
        `4321 0 ${updaterLabel}`,
        `9876 0 ${gatewayLikeLabel}`,
        `2468 0 ${nonCarapaceLabel}`,
      ...
      setLaunchAgentPlist(env, gatewayLikeLabel, ["/opt/homebrew/bin/carapace", "gateway", "run"]);
      setLaunchAgentPlist(env, nonCarapaceLabel, ["/bin/echo", "update", "--yes"]);
      setLaunchAgentPlist(env, prefixedCliLabel, [
        "/usr/local/bin/carapace-helper",
        "update",
        "--yes",
      ]);

      const jobs = await findStaleCarapaceUpdateLaunchdJobs(env as NodeJS.ProcessEnv);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      setLaunchAgentPlist(env, updaterLabel, ["/opt/homebrew/bin/carapace", "gateway", "run"], {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const env = createDefaultLaunchdEnv();
      const label = "ai.carapace.tayoun.update.20260625T201026-0400";
      const envDir = "/Users/test/.carapace-tayoun/service-env";
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      const envFilePath = `${envDir}/${label}.env`;
      state.listOutput = `4321 0 ${label}`;
      state.files.set(envFilePath, "export CARAPACE_UPDATE_RUN_HANDOFF='1'\n");
      setLaunchAgentPlist(env, label, [
        LAUNCH_AGENT_ENV_WRAPPER_SHELL,
        wrapperPath,
        envFilePath,
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.carapace.manual-update.1717168800",
        "812 0 ai.carapace.manual-update.profile",
        "913 0 ai.carapace.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleCarapaceUpdateLaunchdJobs({
        CARAPACE_PROFILE: "manual-update.profile",
        CARAPACE_LAUNCHD_LABEL: "ai.carapace.manual-update.custom-label",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.carapace.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.carapace.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.carapace.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.carapace.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from Carapace label env",
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          CARAPACE_LAUNCHD_LABEL: "ai.carapace.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the Carapace update label",
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.carapace.update.2026.5.12",
          CARAPACE_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.carapace.manual-update.1717168800",
          CARAPACE_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const env = createDefaultLaunchdEnv();
      const label = "ai.carapace.dev.team.update.20260625T201026-0400";
      setLaunchAgentPlist(env, label, ["/opt/homebrew/bin/carapace", "gateway", "run"]);

      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          ...env,
          CARAPACE_LAUNCHD_LABEL: label,
          CARAPACE_UPDATE_RUN_HANDOFF: "1",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const env = createDefaultLaunchdEnv();
      const label = "ai.carapace.tayoun.update.20260625T201026-0400";
      setLaunchAgentPlist(env, label, ["/opt/homebrew/bin/carapace", "update", "--yes"]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentCarapaceUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.carapace.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableCarapaceUpdateLaunchdJob("ai.carapace.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableCarapaceUpdateLaunchdJob("ai.carapace.manual-update.1717168800"),
    ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      await expect(
        disableCarapaceUpdateLaunchdJob(label, {
          ...env,
          CARAPACE_UPDATE_RUN_HANDOFF: "1",
        }),
      ).resolves.toBe(false);
...
describe("launchd bootstrap repair", () => {
  ...
  it("migrates inline secrets before making an existing plist readable", async () => {
    const env = createDefaultLaunchdEnv();
    const plistPath = resolveLaunchAgentPlistPath(env);
    const wrapperPath = "/Users/test/.carapace/service-env/ai.carapace.gateway-env-wrapper.sh";
    const warn = vi.fn();
    ...
    expect(state.files.get("/Users/test/.carapace/service-env/ai.carapace.gateway.env")).toContain(
      secret,
    );
  });
...
describe("launchd uninstall", () => {
  ...
  it("refuses an in-band uninstall before bootout or plist removal", async () => {
    ...
    await withEnvAsync({ XPC_SERVICE_NAME: "ai.carapace.gateway" }, async () => {
      await expect(uninstallLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
        "Refusing to uninstall LaunchAgent ai.carapace.gateway from inside ai.carapace.gateway",
      );
    });
...
describe("launchd install", () => {
  it("refuses an in-band reinstall before booting out its own LaunchAgent", async () => {
    ...
    await withEnvAsync({ XPC_SERVICE_NAME: "ai.carapace.gateway" }, async () => {
      await expect(
        ...
        "Refusing to install LaunchAgent ai.carapace.gateway from inside ai.carapace.gateway",
      );
    });
  ...
  it("aborts before mutation when prior LaunchAgent supervision is ambiguous", async () => {
    ...
    const previous = createTestLaunchAgentPlist({
      label: "ai.carapace.gateway",
      programArguments: ["/previous/node", "/previous/carapace.mjs", "gateway"],
    });
    state.files.set(plistPath, previous);
    state.printError = "launchctl print permission denied";
  ...
  it("aborts before mutation when launchd has the only copy of the prior definition", async () => {
    const env = createDefaultLaunchdEnv();
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    state.serviceStates.set(`${domain}/ai.carapace.gateway`, "running");
  ...
  it("removes generated artifacts after a failed fresh install", async () => {
    const env = createDefaultLaunchdEnv();
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.carapace/service-env/ai.carapace.gateway.env";
    const wrapperPath = "/Users/test/.carapace/service-env/ai.carapace.gateway-env-wrapper.sh";
    state.serviceLoaded = false;
    state.serviceRunning = false;
    state.bootstrapError = "Operation not permitted";
    ...
    const error = await installLaunchAgent(
      defaultLaunchAgentFixture(env, {
        environment: { CARAPACE_GATEWAY_PORT: "19000" },
      }),
  ...
  it("restores the exact prior plist and supervision after external bootstrap failure", async () => {
    ...
    const envFilePath = "/Users/test/.carapace/service-env/ai.carapace.gateway.env";
    const wrapperPath = "/Users/test/.carapace/service-env/ai.carapace.gateway-env-wrapper.sh";
    const previousEnv = "export CARAPACE_GATEWAY_PORT='18789'\n";
  ...
  it.each([
    ...
    },
    {
      ...
      env: { HOME: "/Users/test", CARAPACE_PROFILE: "qa" },
      label: "ai.carapace.qa",
      programArguments: defaultProgramArguments,
    },
    {
      name: "node service",
      env: { HOME: "/Users/test", CARAPACE_LAUNCHD_LABEL: "ai.carapace.node" },
      label: "ai.carapace.node",
    ...
    async ({ env, label, programArguments }) => {
      ...
      expect(plist).not.toContain("CARAPACE_SERVICE_VERSION");
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    ...
    expect(state.fileModes.get(envFilePath)).toBe(0o600);
    expect(state.fileModes.get(wrapperPath)).toBe(0o700);
    expect(state.dirModes.get("/Users/test/.carapace/service-env")).toBe(0o700);
  ...
  it("warns before overwriting a customized generated LaunchAgent env wrapper", async () => {
    ...
    expect(output).toContain("carapace gateway install --wrapper <path>");
    expect(output).toContain("CARAPACE_WRAPPER");
  ...
  it("rewrites legacy LaunchAgent environment wrappers to a system shell executable", async () => {
    ...
    await installLaunchAgent(
      defaultLaunchAgentFixture(env, {
        environment: { CARAPACE_GATEWAY_PORT: "19007" },
      }),
    );
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      CARAPACE_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent(
      defaultLaunchAgentFixture(serviceEnv, {
        environment: {
          CARAPACE_GATEWAY_PORT: "18789",
          CARAPACE_STATE_DIR: serviceEnv.CARAPACE_STATE_DIR,
        },
      }),
    );

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.carapace.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.carapace.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.carapace/service-env/ai.carapace.gateway.env";
    const callerWrapperPath =
      "/Users/test/.carapace/service-env/ai.carapace.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.carapace.gateway.env](http:/ai.carapace.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.carapace.gateway-env-wrapper.sh](http:/ai.carapace.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.CARAPACE_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.CARAPACE_STATE_DIR).toBe(serviceEnv.CARAPACE_STATE_DIR);
    expect(command?.environmentValueSources?.CARAPACE_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export CARAPACE_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export CARAPACE_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/carapace/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    setLegacyGatewayLaunchAgentPlist(plistPath, [
      "    <key>EnvironmentVariables</key>",
      "    <dict>",
      "      <key>CARAPACE_SERVICE_VERSION</key>",
  ...
  it.each([undefined, true])(
    ...
    async (disable) => {
      const env = createDefaultLaunchdEnv();
      await withEnvAsync({ LAUNCH_JOB_LABEL: "ai.carapace.gateway" }, async () => {
  ...
  it("disables the current LaunchAgent before scheduling maintenance bootout", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    expect(state.launchctlCalls).toEqual([["disable", `${domain}/ai.carapace.gateway`]]);
    expect(launchdRestartHandoffState.scheduleDetachedLaunchdMaintenancePark).toHaveBeenCalledWith({
      env,
      waitForPid: process.pid,
    });
  ...
  it("re-enables the LaunchAgent when the maintenance handoff cannot spawn", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    expect(state.launchctlCalls).toEqual([
      ["disable", `${domain}/ai.carapace.gateway`],
      ["enable", `${domain}/ai.carapace.gateway`],
    ]);
  });
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      CARAPACE_LAUNCHD_LABEL: "com.example.carapace.gateway",
    };
    let output = "";
    const stdout = capturePassThroughOutput((text) => (output += text));
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.carapace.gateway`;
    expect(state.launchctlCalls).toEqual([
      ["print", serviceId],
      ["bootout", serviceId],
    ]);
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent(
      defaultLaunchAgentFixture(env, {
        environment: { CARAPACE_GATEWAY_PORT: "19006" },
      }),
    );
    state.launchctlCalls.length = 0;
  ...
  it("does not treat a co-located Gateway's own port as busy when stopping a node-host LaunchAgent", async () => {
    ...
    setLaunchAgentPlist(env, "ai.carapace.node", [
      "node",
      "node",
      "run",
      "--host",
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    setLegacyGatewayLaunchAgentPlist(plistPath, [
      ...
      "    <string>/Users/test/.carapace-default/logs/gateway.log</string>",
    ]);
  ...
  it("audits reload bootout before a later bootstrap failure", async () => {
    const env = createLaunchdEnvWithGatewayPort("18789");
    setLaunchAgentPlist(env, "ai.carapace.gateway", ["node", "gateway.js"]);
    state.bootstrapError = "Operation not permitted";
    state.bootstrapCode = 5;
    const onMutation = vi.fn();
  ...
  it("reports the LaunchAgent as unloaded when bootstrap teardown never clears", async () => {
    ...
    expect(message).toContain(`LaunchAgent ${domain}/ai.carapace.gateway is not loaded`);
    expect(message).toContain("The gateway is down and launchd has no job left to respawn it.");
    expect(message).toContain("carapace gateway start");
  });
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent(
      defaultLaunchAgentFixture(env, {
        environment: { CARAPACE_GATEWAY_PORT: "65536" },
      }),
    );
    state.launchctlCalls.length = 0;
  ...
  it.each([
    ...
    async ({ listeners }) => {
      ...
      await expect(
        ...
        "gateway port 19002 is busy but is not verifiably owned by LaunchAgent ai.carapace.gateway",
      );
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withEnvAsync({ LAUNCH_JOB_LABEL: "ai.carapace.gateway" }, async () =>
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/carapace/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withEnvAsync({ LAUNCH_JOB_LABEL: "ai.carapace.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://github.com/Exaggarate/carapace");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when CARAPACE_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.carapace.gateway.plist",
    },
    {
      name: "uses profile-specific label when CARAPACE_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", CARAPACE_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.carapace.jbphoenix.plist",
    },
    {
      name: "prefers CARAPACE_LAUNCHD_LABEL over CARAPACE_PROFILE",
      env: {
        HOME: "/Users/test",
        CARAPACE_PROFILE: "jbphoenix",
        CARAPACE_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from CARAPACE_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        CARAPACE_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty CARAPACE_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        CARAPACE_PROFILE: "myprofile",
        CARAPACE_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.carapace.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        CARAPACE_LAUNCHD_LABEL: "../evil/label",
      }),
