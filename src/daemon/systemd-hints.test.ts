...
describe("renderSystemdUnavailableHints", () => {
  ...
  it("renders generic Linux recovery hints outside WSL", () => {
    expect(renderSystemdUnavailableHints({ kind: "generic_unavailable" })).toEqual([
      ...
      `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("carapace gateway")}\`.`,
    ]);
  });
  ...
  it("skips headless recovery hints when container context is known", () => {
    const env = { CARAPACE_CONTAINER_HINT: "sandbox" };
    expect(
      ...
      "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
      `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("carapace gateway", env)}\`.`,
    ]);
  });
});
