...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to carapace gateway install --wrapper <path> or CARAPACE_WRAPPER.`,
  ];
}
...
export async function publishLaunchAgentPlist(params: {
  ...
  const temporaryPath = `${params.plistPath}.carapace-${randomUUID()}.tmp`;
    ...
    try {
      ...
      try {
        if (previousContents === null) {
          ...
          const rollbackPath = `${params.plistPath}.carapace-${randomUUID()}.rollback`;
          try {
...
export async function rewriteLaunchAgentPlistForRestart({
  ...
  const canonicalEnvironment = {
    ...
    CARAPACE_SERVICE_VERSION: undefined,
  };
  const prepared = await prepareLaunchAgentProgramArguments({
    env,
    label,
