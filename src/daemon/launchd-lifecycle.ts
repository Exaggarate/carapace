...
function formatLaunchAgentLeftUnloadedError(params: {
  ...
  return [
    ...
    `Fix: run \`carapace gateway start\`, or \`launchctl bootstrap ${params.domain} ${params.plistPath}\`.`,
  ].join("\n");
}
...
export async function restartLaunchAgent({
  ...
  // `carapace gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    try {
      await bootstrapLaunchAgentOrThrow({
        ...
        serviceTarget,
        plistPath,
        actionHint: "carapace gateway restart",
        onMutation: reportMutation,
