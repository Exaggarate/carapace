// Classifies whether a user's chat message approves a pending Carapace proposal.
...
export type SystemAgentApprovalClassifier = (params: {
  ...
  proposal?: string;
  /** Exact execution owner that completed the live Carapace inference gate. */
...
export async function classifySystemAgentApprovalIntent(
  ...
  try {
    ...
    if (!route || route.runner !== "embedded" || route.agentHarnessRuntimeOverride !== "carapace") {
      return "other";
    }
