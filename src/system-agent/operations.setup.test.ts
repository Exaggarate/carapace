...
beforeAll(() => {
  ...
  mockConfig.setPluginMetadataBinder((config) => {
    pluginMetadataSnapshot?.bindForConfig(config as CarapaceConfig);
  });
  mockConfig.reset();
});
...
describe("parseSystemAgentOperation", () => {
  ...
  it("runs setup bootstrap only after approval and audits it", async () => {
    const tempDir = opTempDirs.make("carapace-setup-");
    ...
    const applySetup = vi.fn(async () => ({
      configPath: path.join(tempDir, "carapace.json"),
    ...
    expect(lines.join("\n")).toContain("[carapace] done: carapace.setup");
  ...
  it("rejects setup without a default model before any workspace or Gateway write", async () => {
    const tempDir = opTempDirs.make("carapace-no-inference-setup-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const { runtime, lines } = createSystemAgentTestRuntime();
    ...
    expect(applySetup).not.toHaveBeenCalled();
    expect(lines.join("\n")).not.toContain("[carapace] running: carapace.setup");
  ...
  it("rejects setup when the current route fails its live inference check", async () => {
    const tempDir = opTempDirs.make("carapace-failed-inference-setup-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    mockConfig.setConfig({
      agents: {
        defaults: { model: { primary: "openai/gpt-5.5" } },
  ...
  it("rejects a setup model switch before writing", async () => {
    const tempDir = opTempDirs.make("carapace-model-switch-setup-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const { runtime } = createSystemAgentTestRuntime();
    const applySetup = vi.fn();
    ...
    ).rejects.toThrow("`carapace onboard` on the machine running Carapace");

    expect(applySetup).not.toHaveBeenCalled();
  });

  it("allows the same requested model while preserving it without a model write", async () => {
    const tempDir = opTempDirs.make("carapace-same-model-setup-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const { runtime } = createSystemAgentTestRuntime();
    mockConfig.setConfig({
      agents: {
  ...
  it("live-verifies a staged default model before writing and preserves concurrent edits", async () => {
    const tempDir = opTempDirs.make("carapace-verified-model-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    mockConfig.setConfig({
      agents: {
        defaults: {
        ...
        mockConfig.setConfig({
          ...
          agents: {
            ...
            defaults: {
              ...
              models: { "google/unrelated": { agentRuntime: { id: "carapace" } } },
            },
            entries: {
  ...
  it.each([
    ...
    async ({ initial, change }) => {
      const tempDir = opTempDirs.make("carapace-route-conflict-");
      setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
      ...
      expect(mockConfig.mutateConfigFile).toHaveBeenCalledOnce();
      expect(lines.join("\n")).not.toContain("[carapace] done: config.setDefaultModel");
      await expect(fs.access(path.join(tempDir, "audit", "system-agent.jsonl"))).rejects.toThrow();
    },
  );

  it("keeps the working model and writes no audit when live inference fails", async () => {
    const tempDir = opTempDirs.make("carapace-rejected-model-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
  ...
  it("writes nothing when the exact latest route fails its locked recheck", async () => {
    const tempDir = opTempDirs.make("carapace-latest-route-rejected-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const originalConfig = {
      agents: {
        defaults: { model: { primary: "anthropic/claude-sonnet-4-6" } },
  ...
  it("rejects a live result from a different model before opening the write boundary", async () => {
    const tempDir = opTempDirs.make("carapace-mismatched-model-result-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const originalConfig = {
      agents: {
        defaults: { model: { primary: "anthropic/claude-sonnet-4-6" } },
  ...
  it("rejects a different model result from the final commit-boundary probe", async () => {
    const tempDir = opTempDirs.make("carapace-final-mismatched-model-result-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const originalConfig = {
      agents: {
        defaults: { model: { primary: "anthropic/claude-sonnet-4-6" } },
  ...
  it("rechecks the existing inference binding inside the locked model transform", async () => {
    const tempDir = opTempDirs.make("carapace-model-binding-rotated-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const originalConfig = {
      agents: {
        defaults: { model: { primary: "anthropic/claude-sonnet-4-6" } },
  ...
  it("rechecks the existing inference binding after the candidate's final live probe", async () => {
    const tempDir = opTempDirs.make("carapace-model-binding-final-probe-rotated-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    const originalConfig = {
      agents: {
        defaults: { model: { primary: "anthropic/claude-sonnet-4-6" } },
  ...
  it("stages and persists model changes at the effective default-agent owner", async () => {
    const tempDir = opTempDirs.make("carapace-default-agent-model-");
    setTestEnvValue("CARAPACE_STATE_DIR", tempDir);
    mockConfig.setConfig({
      agents: {
    ...
    const verifyInferenceConfig = vi.fn(async ({ config }: { config: TestConfig }) => {
      ...
      const work = listAgentEntries(config as CarapaceConfig).find((entry) => entry.id === "work");
    ...
    const work = listAgentEntries(mockConfig.currentConfig() as CarapaceConfig).find(
      (entry) => entry.id === "work",
    );
    expect(work?.model).toEqual({
      primary: "openai/gpt-5.5",
