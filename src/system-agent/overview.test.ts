// Carapace overview tests cover summary output for rescue diagnostics.
...
describe("loadSystemAgentOverview", () => {
  it("summarizes config, agents, model, tools, and gateway", async () => {
    ...
    const overview = await loadSystemAgentOverview({
      ...
      deps: {
        ...
        resolveConfigPath: () => "/tmp/carapace.json",
    ...
    expect(overview.references.sourceUrl).toBe("https://github.com/Exaggarate/carapace");
    ...
    expect(startup).toContain("Hi, I'm Carapace — caretaker");
  ...
  it("fails closed in startup copy when inference is unavailable", () => {
    ...
    const startup = formatSystemAgentStartupMessage(overview);
    expect(formatSystemAgentOverview(overview)).toContain(
      'Next: run "carapace onboard" to establish inference',
    );
    expect(startup).toContain("Inference is unavailable");
    expect(startup).toContain("Run `carapace onboard`");
    expect(startup.match(/`[^`]+`/g)).toEqual(["`carapace onboard`"]);
