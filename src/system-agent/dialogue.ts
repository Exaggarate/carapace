// Carapace dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into Carapace operations.
 *
...
/** Resolve user input to an Carapace operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[carapace] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[carapace] interpreted: ${plan.command}`);
}
