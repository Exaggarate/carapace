...
async function loadAuthoredSetupConfig(params: {
  ...
  authoredConfig?: import("../config/types.carapace.js").CarapaceConfig;
...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "Carapace onboarding requires working inference first. Run `carapace onboard` on the machine running Carapace to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm Carapace — let's hatch your agent.",
    "",
    ...
    "",
    "Heads up: your agent gets real access to this machine — https://github.com/Exaggarate/carapace",
