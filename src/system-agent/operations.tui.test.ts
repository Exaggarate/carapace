...
describe("system-agent TUI operations", () => {
  it("refuses doctor repairs before any write or audit", async () => {
    await withTempHome(async (home) => {
      ...
      expect(lines.join("\n")).toContain("with Carapace stopped");
      expect(lines.join("\n")).toContain("carapace doctor --fix");
      expect(lines.join("\n")).not.toContain("[carapace] running: doctor.fix");
      await expect(
        fs.access(path.join(home, ".carapace", "audit", "system-agent.jsonl")),
      ).rejects.toThrow();
    });
  });

  it("returns from the agent TUI back to Carapace", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();
    ...
    expect(lines.join("\n")).toContain(
      "[carapace] returned from agent with request: restart gateway",
    );
  });
  ...
  it("re-enters the Carapace shell when the agent TUI returns without a request", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();
    const runTui = vi.fn(async () => ({
      exitReason: "return-to-system-agent" as const,
    }));
    ...
    expect(lines.join("\n")).toContain("[carapace] returned from agent");
  });
});
