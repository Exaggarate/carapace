// Carapace CLI runner selects JSON, one-shot, or interactive setup-helper mode.
...
/** Options accepted by the Carapace command runner. */
...
/** Run Carapace in JSON, one-shot message, or interactive TUI mode. */
export async function runSystemAgent(
  ...
  if (route.runner !== "embedded" || route.agentHarnessRuntimeOverride === "carapace") {
    return await run();
  }
...
async function runBoundSystemAgent(
  ...
  if (boundOpts.message?.trim()) {
    ...
    const overview = await withProgress(
      {
        label: "Loading Carapace overview…",
        indeterminate: true,
        delayMs: 0,
        fallback: "none",
      },
  ...
  if (!inputIsTty || !outputIsTty) {
    // Without a TTY, Carapace cannot safely ask for confirmation; require --message instead.
