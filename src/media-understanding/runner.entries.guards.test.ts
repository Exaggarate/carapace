...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*carapace plugins install/);
    expect(message).toContain("@carapace/groq-provider");
    expect(message).toContain("carapace plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("carapace doctor --fix");
  });
