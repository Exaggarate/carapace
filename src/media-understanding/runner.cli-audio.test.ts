...
async function runAudioEntry(params: {
  ...
  await withAudioFixture(`carapace-cli-${params.command}`, async ({ ctx, media, cache }) => {
    result = await runCliEntry({
      capability: "audio",
      entry: { type: "cli", command: params.command, args: params.args },
      cfg: { tools: { media: { audio: {} } } } as CarapaceConfig,
      ctx,
...
describe("media-understanding CLI audio entry", () => {
  ...
  it.each([
    ...
    await withAudioFixture("carapace-cli-language", async ({ ctx, media, cache }) => {
      await runCliEntry({
        capability: "audio",
        entry: {
          type: "cli",
  ...
  it.each([
    ...
    async ({ count, leadingEmpty }) => {
      await withTestDir({ prefix: "carapace-cli-media-template-" }, async (base) => {
  ...
  it("removes the CLI scratch directory when audio conversion fails", async () => {
    ...
    await withMediaFixture(
      {
        filePrefix: "carapace-cli-whisper-conversion-failure",
        extension: "mp3",
  ...
  it("treats sherpa structured JSON with empty text as empty output", async () => {
    ...
    await withAudioFixture("carapace-cli-audio-empty-sherpa", async ({ ctx, media, cache }) => {
  ...
  it("extracts sherpa text from the final structured output line", async () => {
    ...
    await withAudioFixture("carapace-cli-audio-sherpa-json", async ({ ctx, media, cache }) => {
      const result = await runCliEntry({
        capability: "audio",
        entry: {
          type: "cli",
