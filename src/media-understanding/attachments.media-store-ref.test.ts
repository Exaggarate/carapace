...
function createOpenAiAudioCfg(): CarapaceConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createUrlDisabledFileCfg(): CarapaceConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function withStoredInboundAudio<T>(
  ...
  const stateDir = await fs.realpath(tempDirs.make("carapace-media-store-ref-", os.tmpdir()));
  return await withEnvAsync({ CARAPACE_STATE_DIR: stateDir, PATH: "" }, async () => {
