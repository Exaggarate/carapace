...
async function withIsolatedAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-local-audio-auth-"));
  try {
...
function createVideoCfg(params: { provider: string; model: string }): CarapaceConfig {
  return {
    tools: {
      media: {
        models: [
...
describe("runCapability local no-auth audio providers", () => {
  it("runs provider-owned audio without resolving an unrelated API key", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withAudioFixture("carapace-prepared-audio", async ({ ctx, media, cache }) => {
        const provider = {
          id: "prepared-audio",
  ...
  it("allows a local no-auth audio provider when configured as a local models provider", async () => {
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture("carapace-local-audio-configured", async ({ ctx, media, cache }) => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture(
          "carapace-local-audio-plugin-only",
  ...
  it("prefers resolver env credentials over plugin-only media no-auth", async () => {
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync({ ...AUTH_ENV, OPENAI_API_KEY: "env-openai-audio-key" }, async () => {
        await withAudioFixture("carapace-openai-audio-env-key", async ({ ctx, media, cache }) => {
          const transcribeAudio = vi.fn(async (req: AudioTranscriptionRequest) => ({
            text: `env:${req.apiKey}`,
            model: req.model,
          }));
  ...
  it("uses OpenAI API key auth for audio when the default OpenAI profile is OAuth", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync({ ...AUTH_ENV, OPENAI_API_KEY: "env-openai-audio-key" }, async () => {
        await withAudioFixture(
          "carapace-openai-audio-oauth-env-key",
          async ({ ctx, media, cache }) => {
            const transcribeAudio = vi.fn(async (req: AudioTranscriptionRequest) => ({
  ...
  it("prefers stored auth profile credentials over plugin-only media no-auth", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture(
          "carapace-local-audio-stored-profile",
          async ({ ctx, media, cache }) => {
            const transcribeAudio = vi.fn(async (req: AudioTranscriptionRequest) => ({
  ...
  it("still rejects a remote audio provider without credentials", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture("carapace-remote-audio-no-auth", async ({ ctx, media, cache }) => {
  ...
  it("prefers literal configured provider apiKey over media no-auth hook", async () => {
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture(
          "carapace-local-audio-literal-key",
          async ({ ctx, media, cache }) => {
            const transcribeAudio = vi.fn(async (req: AudioTranscriptionRequest) => ({
  ...
  it("allows a media auth hook to provide an api key after normal auth misses", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture("carapace-local-audio-hook-key", async ({ ctx, media, cache }) => {
          const transcribeAudio = vi.fn(async (req: AudioTranscriptionRequest) => ({
  ...
  it("does not allow plugin-only media provider without explicit no-auth", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture("carapace-local-audio-no-hook", async ({ ctx, media, cache }) => {
          const transcribeAudio = vi.fn(async () => ({
            text: "should not run",
            model: "whisper-local",
          }));
  ...
  it("does not allow plugin-only media provider when no-auth hook returns null", async () => {
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture("carapace-local-audio-null-hook", async ({ ctx, media, cache }) => {
          const transcribeAudio = vi.fn(async () => ({
            text: "should not run",
            model: "whisper-local",
          }));
  ...
  it("does not let plugin-only no-auth override an explicit missing profile", async () => {
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture(
          "carapace-local-audio-plugin-missing-profile",
          async ({ ctx, media, cache }) => {
            const transcribeAudio = vi.fn(async () => ({
              text: "should not run",
              model: "whisper-local",
  ...
  it("does not let media no-auth override an explicit missing profile", async () => {
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withAudioFixture(
          "carapace-local-audio-missing-profile",
          async ({ ctx, media, cache }) => {
            const transcribeAudio = vi.fn(async () => ({
              text: "should not run",
              model: "whisper-local",
    ...
    await withIsolatedAgentDir(async (agentDir) => {
      await withEnvAsync(AUTH_ENV, async () => {
        await withVideoFixture(
          "carapace-local-video-plugin-only",
          async ({ ctx, media, cache }) => {
