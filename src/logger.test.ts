...
describe("logger helpers", () => {
  ...
  it("writes to configured log file at configured level", async () => {
    await withTestDir({ prefix: "carapace-log-test-" }, async (dir) => {
      const logPath = path.join(dir, "carapace.log");
  ...
  it("uses daily rolling log files and prunes old ones", async () => {
    await withTestDir({ prefix: "carapace-log-test-" }, async (dir) => {
      ...
      const todayPath = path.join(dir, `carapace-${today}.log`);
      setLoggerOverride({ level: "info", file: todayPath });

      // create an old file to be pruned
      const oldPath = path.join(dir, "carapace-2000-01-01.log");
