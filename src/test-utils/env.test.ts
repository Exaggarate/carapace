...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "CARAPACE_ENV_TEST_A";
    const keyB = "CARAPACE_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "CARAPACE_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "CARAPACE_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "CARAPACE_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "CARAPACE_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "CARAPACE_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "CARAPACE_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "CARAPACE_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    setTestEnvValue(key, "outer");
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    setTestEnvValue("CARAPACE_STATE_DIR", "/srv/carapace-state");
    setTestEnvValue("CARAPACE_BUNDLED_PLUGINS_DIR", "/srv/carapace-bundled");
    ...
    try {
      ...
      expect(env.CARAPACE_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.CARAPACE_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("CARAPACE_HOME", previousCarapaceHome);
      restoreEnvKey("CARAPACE_STATE_DIR", previousStateDir);
      restoreEnvKey("CARAPACE_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processCarapaceHome: process.env.CARAPACE_HOME,
          processBundledDir: process.env.CARAPACE_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.CARAPACE_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processCarapaceHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.CARAPACE_HOME).toBe("/srv/carapace-home");
    } finally {
      restoreEnvKey("CARAPACE_HOME", previousCarapaceHome);
    }
  });
