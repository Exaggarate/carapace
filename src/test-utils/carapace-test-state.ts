// Creates isolated Carapace state directories for integration-style tests.
...
type CarapaceTestStateLayout = "home" | "state-only" | "split";

type CarapaceTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
type CarapaceTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: CarapaceTestStateLayout;
  scenario?: CarapaceTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "carapace-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "carapace.json"),
...
function scenarioConfig(options: CarapaceTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "carapace-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: CarapaceTestStateOptions): Record<string, string | undefined> {
...
export async function createCarapaceTestState(
  options: CarapaceTestStateOptions = {},
): Promise<CarapaceTestState> {
    ...
    const state: CarapaceTestState = {
      ...
      applyEnv: () => {
        if (releasePromise || cleanupPromise) {
          throw new Error("Cannot apply a released Carapace test state");
        }
        resetConfigRuntimeStateForTest();
        // A later write can throw after earlier keys changed; restoration still owns them.
        envApplied = true;
        for (const [key, value] of Object.entries(envVars)) {
          // Test fixtures apply a fixed Carapace env set, not plugin-provided host env.
          if (value === undefined) {
...
export async function withCarapaceTestState<T>(
  options: CarapaceTestStateOptions,
  fn: (state: CarapaceTestState) => Promise<T>,
): Promise<T> {
  const state = await createCarapaceTestState(options);
