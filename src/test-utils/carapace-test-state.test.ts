// Tests isolated Carapace test-state setup and cleanup behavior.
...
import { createCarapaceTestState, withCarapaceTestState } from "../plugin-sdk/test-state.js";
...
describe("carapace test state", () => {
  it("joins callback descendants before beginning state release", async () => {
    ...
    const operation = withCarapaceTestState({ label: "callback-descendant" }, async (state) => {
      fixtureStateDir = state.stateDir;
      background = trackAsyncWork(async () => {
        entered.resolve();
        await gate.promise;
        selectedStateDir = process.env.CARAPACE_STATE_DIR;
      });
    });
    try {
      ...
      expect.soft(process.env.CARAPACE_STATE_DIR).toBe(fixtureStateDir);
    } finally {
      gate.resolve();
      await background;
      await operation;
  ...
  it.each(["cleanup", "restoreEnv"] as const)(
    ...
    async (method) => {
      const state = await createCarapaceTestState({ label: "concurrent-release" });
      const gate = createDeferredCore();
      const drain = vi
        .spyOn(sessionCleanup, "cleanupSessionStateForTest")
      ...
      try {
        ...
        expect.soft(process.env.CARAPACE_STATE_DIR).toBe(state.stateDir);
      ...
      expect(() => state.applyEnv()).toThrow("released Carapace test state");
    },
  );
  ...
  it.each(["cleanup", "restoreEnv"] as const)(
    ...
    async (method) => {
      ...
      const state = await createCarapaceTestState({ label: "failed-release" });
  ...
  it.each([
    ...
    const unrelated = openCarapaceStateDatabase({
      env: { CARAPACE_STATE_DIR: path.join(parent, "unrelated") },
    });
    try {
      await withEnvAsync(
        {
          CARAPACE_AGENT_DIR: path.join(parent, "previous-agent"),
          PI_CODING_AGENT_DIR: path.join(parent, "previous-legacy-agent"),
          CARAPACE_ACQUISITION_EMPTY: "",
          CARAPACE_ACQUISITION_ABSENT: undefined,
        },
        async () => {
          const keys = [
            "HOME",
          ...
          try {
            await expect(
              createCarapaceTestState({
                ...
                env: {
                  CARAPACE_ACQUISITION_EMPTY: "changed",
                  CARAPACE_ACQUISITION_ABSENT: "added",
                },
              }),
            ).rejects.toBe(fault);
            ...
            const recovered = await createCarapaceTestState({
              prefix,
              layout: "split",
              scenario: "minimal",
              applyEnv: false,
  ...
  it("creates an isolated home layout with spawn env and restores process env", async () => {
    ...
    const previousCarapaceHome = process.env.CARAPACE_HOME;
    ...
    try {
      ...
      expect(state.stateDir).toBe(path.join(state.home, ".carapace"));
      expect(state.configPath).toBe(path.join(state.stateDir, "carapace.json"));
      expect(state.workspaceDir).toBe(path.join(state.home, "workspace"));
      expect(state.env.HOME).toBe(state.home);
      expect(state.env.CARAPACE_HOME).toBe(state.home);
      expect(state.env.CARAPACE_STATE_DIR).toBe(state.stateDir);
      expect(state.env.CARAPACE_CONFIG_PATH).toBe(state.configPath);
      expect(process.env.HOME).toBe(state.home);
      expect(process.env.CARAPACE_HOME).toBe(state.home);
    ...
    expect(process.env.CARAPACE_HOME).toBe(previousCarapaceHome);
    expect(process.env.CARAPACE_STATE_DIR).toBe(previousStateDir);
    expect(process.env.CARAPACE_CONFIG_PATH).toBe(previousConfigPath);
  ...
  it.each([
    ...
    async ({ agentEnv, applyEnv }) => {
      const inherited = {
        CARAPACE_AGENT_DIR: "/tmp/outside-carapace-agent",
      ...
      await withEnvAsync(inherited, async () => {
        ...
        try {
          ...
          expect(state.env.CARAPACE_AGENT_DIR).toBe(expectedAgentDir);
          expect(state.env.PI_CODING_AGENT_DIR).toBeUndefined();
          expect(process.env.CARAPACE_AGENT_DIR).toBe(
            applyEnv ? expectedAgentDir : inherited.CARAPACE_AGENT_DIR,
          );
        ...
        expect(process.env.CARAPACE_AGENT_DIR).toBe(inherited.CARAPACE_AGENT_DIR);
  ...
  it.each([undefined, "main"] as const)(
    ...
    async (agentEnv) => {
      await withEnvAsync({ CARAPACE_AGENT_DIR: undefined, PI_CODING_AGENT_DIR: "" }, async () => {
        const overrides = {
          CARAPACE_AGENT_DIR: "/tmp/explicit-carapace-agent",
          PI_CODING_AGENT_DIR: "/tmp/explicit-legacy-agent",
        };
        const state = await createCarapaceTestState({ agentEnv, applyEnv: false, env: overrides });
        try {
          expect(state.env.CARAPACE_AGENT_DIR).toBe(overrides.CARAPACE_AGENT_DIR);
          ...
          expect(process.env.PI_CODING_AGENT_DIR).toBe("");
          state.applyEnv();
          expect(process.env.CARAPACE_AGENT_DIR).toBe(overrides.CARAPACE_AGENT_DIR);
  ...
  it("writes scenario configs and auth profile stores", async () => {
    await withCarapaceTestState(
      ...
      async (state) => {
        ...
        expect(profilePath).toBe(path.join(state.agentDir(), "carapace-agent.sqlite"));
  ...
  it("closes only fixture-owned databases before restoring env", async () => {
    ...
    const unrelatedRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "carapace-test-state-unrelated-"),
    );
    const unrelatedEnv = {
      ...process.env,
      CARAPACE_STATE_DIR: path.join(unrelatedRoot, "state"),
    };
    const state = await createCarapaceTestState({
      layout: "state-only",
    ...
    const fixtureShared = openCarapaceStateDatabase({ env: state.env });
    const fixtureAgent = openCarapaceAgentDatabase({
      agentId: "worker",
      env: state.env,
    });
    const unrelatedShared = openCarapaceStateDatabase({ env: unrelatedEnv });
    const unrelatedAgent = openCarapaceAgentDatabase({
      agentId: "outside",
      env: unrelatedEnv,
    });
    ...
    try {
      ...
      closeCarapaceAgentDatabaseByPath(fixtureAgent.path);
      closeCarapaceAgentDatabaseByPath(unrelatedAgent.path);
      closeCarapaceStateDatabaseByPath(fixtureShared.path);
      closeCarapaceStateDatabaseByPath(unrelatedShared.path);
      openSpy.mockRestore();
      rmSpy.mockRestore();
  ...
  it("does not recreate fixture databases from a deferred transcript reconcile", async () => {
    const state = await createCarapaceTestState({ label: "deferred-reconcile" });
    const options = { agentId: "main", env: state.env };
    const agent = openCarapaceAgentDatabase(options);
    const shared = openCarapaceStateDatabase({ env: state.env });
    ...
    try {
      ...
      expect(isCarapaceAgentDatabaseOpen(agent.path)).toBe(false);
      expect(agent.db.isOpen).toBe(false);
  ...
  it("preserves callback failures after closing fixture databases", async () => {
    ...
    await expect(
      withCarapaceTestState({ layout: "state-only", label: "callback-failure" }, async (state) => {
        root = state.root;
        shared = openCarapaceStateDatabase({ env: state.env });
  ...
  it("keeps external-service env scoped to the fixture", async () => {
    const previousPolicy = process.env.CARAPACE_SERVICE_REPAIR_POLICY;
    ...
    await withCarapaceTestState(
      ...
      async (state) => {
        expect(process.env.CARAPACE_SERVICE_REPAIR_POLICY).toBe("external");
        expect(state.env.CARAPACE_SERVICE_REPAIR_POLICY).toBe("external");
      },
    );

    expect(process.env.CARAPACE_SERVICE_REPAIR_POLICY).toBe(previousPolicy);
  });
});
