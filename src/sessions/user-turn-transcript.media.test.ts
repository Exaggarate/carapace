...
describe("buildPersistedUserTurnMediaInputsFromFields", () => {
  ...
  it("resolves relative canonical paths against each fact workspace", () => {
    const workspaceDir = "/tmp/carapace-user-turn-workspace";
    expect(
      buildPersistedUserTurnMediaInputsFromFields({
        __carapace: {
...
describe("buildLateMediaAttachedProjection canonical persistence", () => {
  it.each([
    ...
    {
      name: "facts-only",
      message: { __carapace: { media: [{ path: "/media/fact.png" }] } },
    ...
    },
    {
      ...
      message: {
        ...
        __carapace: { media: [{ path: "/media/equal.png" }] },
      },
    ...
    const metadata = (testCase.message as { __carapace?: Record<string, unknown> })["__carapace"];
    const projection = buildLateMediaAttachedProjection({
      role: "user",
      content: "",
      ...testCase.message,
      __carapace: { ...metadata, lateMedia: true },
    } as never);
...
describe("buildPersistedUserTurnMessage media projection", () => {
  it.each([
    ...
    {
      name: "explicit MIME",
      media: [{ path: "/tmp/blob.bin", contentType: "application/x-carapace" }],
      expectedLegacy: {
        MediaPath: "/tmp/blob.bin",
        MediaPaths: ["/tmp/blob.bin"],
        MediaType: "application/x-carapace",
        MediaTypes: ["application/x-carapace"],
      },
      expectedMedia: [{ path: "/tmp/blob.bin", contentType: "application/x-carapace" }],
    },
    {
      name: "bare kind",
      media: [{ kind: "image" }],
    ...
    expect(
      (message as unknown as { __carapace?: { media?: unknown } })["__carapace"]?.media,
