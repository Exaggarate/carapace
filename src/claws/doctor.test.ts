...
function snapshotMcpServers(config: CarapaceConfig): Record<string, Record<string, unknown>> {
...
async function fixture(
  ...
  const root = tempDirs.make("carapace-claw-doctor-");
  if (params.withFile) {
...
describe("collectClawStateHealthFindings", () => {
  ...
  it("reports a newer state schema instead of interpreting it", async () => {
    ...
    await mkdir(dirname(databasePath), { recursive: true });
    const database = new DatabaseSync(databasePath);
    const newerSchemaVersion = CARAPACE_STATE_SCHEMA_VERSION + 1;
  ...
  it("stays hidden when the experimental Claws surface is disabled", async () => {
    ...
    await expect(
      collectClawStateHealthFindings({
        env: { ...current.env, CARAPACE_EXPERIMENTAL_CLAWS: "" },
        cfg: {},
      }),
    ).resolves.toEqual([]);
  });
