...
import { parseClawManifest, parseClawCarapaceProfile } from "./schema.js";
import type { ClawManifest, ClawCarapaceProfile, ClawSourceIdentity } from "./types.js";
...
const baseCarapaceProfile: ClawCarapaceProfile = {
  schemaVersion: 1,
  agent: {
...
async function createPlanSource(): Promise<{ source: ClawSourceIdentity; workspace: string }> {
  const root = tempDirs.make("carapace-claw-plan-");
...
describe("parseClawManifest", () => {
  ...
  it("rejects the prototype flat entries contract", () => {
    const result = parseClawManifest({
      schemaVersion: "carapace.claw.v1",
      id: "old-claw",
  ...
  it("rejects invalid heartbeat durations and cron expressions", () => {
    const heartbeat = parseClawCarapaceProfile({
      schemaVersion: 1,
...
describe("readClawManifestFile", () => {
  it("takes published identity from package.json", async () => {
    const root = tempDirs.make("carapace-claw-package-");
    await writeFile(
      join(root, "package.json"),
      JSON.stringify({
        name: "@acme/github-triage",
        version: "3.2.1",
        carapace: { claw: "carapace.claw.json" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  it("reads a human-authored CLAW.md package through the same manifest schema", async () => {
    const root = tempDirs.make("carapace-claw-markdown-");
    await writeFile(
      join(root, "package.json"),
      JSON.stringify({
        name: "@acme/github-triage",
  ...
  it("accepts a UTF-8 BOM and includes its bytes in snapshot integrity", async () => {
    const root = tempDirs.make("carapace-claw-markdown-bom-");
    await writeFile(
      join(root, "package.json"),
      JSON.stringify({
        name: "@acme/github-triage",
  ...
  it("rejects CLAW.md without YAML frontmatter", async () => {
    const root = tempDirs.make("carapace-claw-markdown-invalid-");
  ...
  it.each([
    ...
    const root = tempDirs.make("carapace-claw-markdown-alias-");
    const path = join(root, "CLAW.md");
    await writeFile(
      path,
      [
  ...
  it("rejects a CLAW.md body that is not valid UTF-8", async () => {
    const root = tempDirs.make("carapace-claw-markdown-original-bytes-");
    const path = join(root, "CLAW.md");
  ...
  it("rejects two competing SOUL.md sources", async () => {
    const root = tempDirs.make("carapace-claw-markdown-soul-conflict-");
    const path = join(root, "CLAW.md");
    await writeFile(
      path,
      [
  ...
  it("synthesizes explicit development identity for a standalone manifest", async () => {
    const root = tempDirs.make("carapace-claw-development-");
  ...
  it("rejects workspace sources through an intermediate symlink", async () => {
    const root = tempDirs.make("carapace-claw-reader-symlink-");
  ...
  it("rejects a workspace source over the per-file byte limit", async () => {
    const root = tempDirs.make("carapace-claw-reader-file-limit-");
  ...
  it("rejects aggregate workspace bytes before reading source contents", async () => {
    const root = tempDirs.make("carapace-claw-reader-aggregate-limit-");
    const files = [];
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = tempDirs.make("carapace-claw-markdown-link-");
      await writeFile(
        join(root, "package.json"),
        JSON.stringify({
          name: "@acme/github-triage",
  ...
  it("rejects package manifests that escape the package root", async () => {
    const parent = tempDirs.make("carapace-claw-escape-");
...
describe("buildClawAddPlan", () => {
  ...
  it("plans one new agent, workspace, packages, MCP servers, and agent-pinned cron jobs", async () => {
    ...
    const plan = await buildClawAddPlan({
      manifest: requireManifest(),
      carapaceProfile: baseCarapaceProfile,
      source,
      context: { workspace },
    });
