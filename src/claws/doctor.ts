...
type ClawDoctorOptions = CarapaceStateDatabaseOptions & {
  cfg?: CarapaceConfig;
  ...
  if (record.install.status !== "complete") {
    findings.push(
      finding({
        ...
        fixHint: "Inspect `carapace claws status` before retrying or removing this Claw.",
      }),
    );
  }
  ...
  for (const pkg of record.packages) {
    ...
    findings.push(
      finding({
        ...
        fixHint:
          "Inspect package state with `carapace claws status` before updating or removing the Claw.",
      }),
    );
  }
...
function orphanedAgentIds(options: CarapaceStateDatabaseOptions): string[] {
...
export async function collectClawStateHealthFindings(
  ...
  try {
    database = await openExistingCarapaceStateDatabaseReadOnly(options);
    if (!database) {
      return [];
    }
