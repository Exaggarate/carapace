// Safe loader for the conventional package-local Carapace profile.
...
import type { ClawDiagnostic, ClawCarapaceProfile } from "./types.js";
import { parseClawYaml } from "./yaml-document.js";

const MAX_PROFILE_BYTES = 256 * 1024;
const CLAW_PROFILE_PATH = "profiles/carapace.yml";
const LEGACY_PROFILE_POINTER_KEY = "carapace.config";
const LEGACY_PROFILE_POINTER_PATH = "$.metadata.carapace.config";
const CONVENTIONAL_PROFILE_PATH = "$.profiles.carapace";
...
function migrateLegacyDynamicToolProfile(value: unknown): {
  value: unknown;
  legacyProfile?: ClawCarapaceProfile;
} {
  ...
  const validationProbe = parseClawCarapaceProfile({
    ...profile,
    agent: {
      ...agent,
      tools: {
  ...
  const legacyProfile: ClawCarapaceProfile = {
...
/**
 * Resolves the Carapace profile for a package.
 *
 * `profiles/carapace.yml` is the conventional location. The retired
 * `metadata.carapace.config` pointer is still read for compatibility with
 ...
 */
export async function readClawCarapaceProfile(params: {
  packageRoot: string;
...
> {
  ...
  if (legacyPointer !== undefined) {
    ...
    if (conventionalExists && legacyPointer !== CLAW_PROFILE_PATH) {
      return {
        ...
        diagnostics: [
          diagnostic(
            "conflicting_carapace_profile_pointer",
    ...
    diagnostics.push(
      warning(
        "deprecated_carapace_profile_pointer",
  ...
  try {
    ...
    return {
      ...
      diagnostics: [
        diagnostic(
          unsafe
            ? "carapace_profile_unsafe"
            : tooLarge
              ? "carapace_profile_too_large"
              : "carapace_profile_read_failed",
          unsafe
            ? "The Carapace profile must be a regular, non-symlinked, non-hardlinked file."
            : tooLarge
              ? `The Carapace profile exceeds ${MAX_PROFILE_BYTES} bytes.`
  ...
  const migration = params.allowLegacyDynamicToolProfile
    ...
    : { value: yaml.value };
  const parsed = parseClawCarapaceProfile(migration.value);
  if (!parsed.ok) {
    return {
      ok: false,
