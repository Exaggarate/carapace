...
export async function buildClawUpdatePlan(params: {
  ...
  targetCarapaceProfile?: ClawCarapaceProfile;
  targetSource: ClawSourceIdentity;
  config: CarapaceConfig;
  sourceMcpServers: Record<string, Record<string, unknown>>;
  stateOptions?: CarapaceStateDatabaseOptions & { packageDeps?: PackageRemovalDeps };
  ...
  const database =
    ...
    (await openExistingCarapaceStateDatabaseReadOnly(params.stateOptions));
  if (!database) {
  ...
  const readOnlyStateOptions: CarapaceStateDatabaseOptions & {
  ...
  try {
    ...
    const targetPlan = await buildClawAddPlan({
      ...
      carapaceProfile: params.targetCarapaceProfile,
      source: params.targetSource,
      diagnostics: params.diagnostics,
      context: {
        agentId,
    ...
    const targetPackages = clawTargetPackages(params.targetManifest, params.targetCarapaceProfile);
    ...
    for (const [key, target] of targetPackages) {
      ...
      if (failedPackageMutationPreflight) {
        ...
        const extensionIndex =
          params.targetCarapaceProfile?.extensions?.findIndex(
            (extension) => clawPackageKey(extension) === key,
          ) ?? -1;
        const path =
          packageIndex >= 0
            ? `$.packages[${packageIndex}]`
            : `$.profiles.carapace.extensions[${extensionIndex}]`;
