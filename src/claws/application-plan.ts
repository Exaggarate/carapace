...
export function clawProfileExtensionPackages(
  profile: ClawCarapaceProfile | undefined,
): ClawPackage[] {
    ...
    if (declaredPackageIds.has(packageId)) {
      collisions.push({
        ...
        diagnostic: blocker(
          ...
          `$.profiles.carapace.extensions[${index}]`,
...
function extensionCapabilityChange(params: {
  extension: ClawCarapaceExtension;
  ...
  const change = {
    ...
    path: `carapace.extensions.${params.extension.id}`,
    action: params.preflight.action === "reuse" ? ("reuse" as const) : ("install" as const),
    reason:
      params.preflight.action === "reuse"
        ? "The Carapace profile requires access to an existing native extension."
        : "The Carapace profile requires installation of native extension content or executable code.",
    effect,
  };
  return {
    ...change,
    ...
    const formatMismatch =
      preflight.ok && completeProvenance && preflight.detectedFormat !== extension.format
        ? blocker(
            ...
            `$.profiles.carapace.extensions[${index}].format`,
