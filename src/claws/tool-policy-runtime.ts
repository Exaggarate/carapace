...
let preparedStateOptions: CarapaceStateDatabaseOptions = {};
let readPreparedSchemaVersions = readCachedClawInstallSchemaVersions;
const uninitializedStateError = new Error(
  "Carapace state database has not initialized Claw consent provenance.",
);
    ...
    super(
      options.unboundedFullProfile
        ? `Claw-managed agent ${JSON.stringify(agentId)} uses the legacy unbounded full tool profile. ` +
            "Add an explicit tools.allow list to its package Carapace profile, then " +
            `run \`carapace claws update ${agentId}\` and approve the refreshed tool authority.`
        : `Claw-managed agent ${JSON.stringify(agentId)} uses a legacy dynamic tool policy. ` +
            `Run \`carapace claws update ${agentId}\` and approve the refreshed tool authority before running it.`,
    );
  ...
  constructor(agentId: string, cause: unknown) {
    super(
      `Cannot verify the installed tool authority for Claw-managed agent ${JSON.stringify(agentId)}. ` +
        "Repair the Carapace state database before running it.",
      { cause },
    );
