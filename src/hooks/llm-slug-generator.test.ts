...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/carapace-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/carapace-agent/.carapace-agent"),
}));
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded Carapace agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded Carapace agent run options");
  }
