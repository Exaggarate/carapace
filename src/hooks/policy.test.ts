...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "carapace-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "carapace-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "carapace-bundled");
      const plugin = makeHookEntry("shared", "carapace-plugin");
      const managed = makeHookEntry("shared", "carapace-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("carapace-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "carapace-managed");
      const workspace = makeHookEntry("shared", "carapace-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "carapace-workspace");
      const second = makeHookEntry("shared", "carapace-workspace");
      second.hook.handlerPath = "/tmp/carapace-workspace/shared/handler-2.js";
