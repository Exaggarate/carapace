...
type CanonicalInboundMessageHookContext = {
  ...
  // `mediaPath(s)` are files Carapace has already staged locally. `mediaUrl(s)`
...
export function toInternalMessageTranscribedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: CarapaceConfig,
): MessageTranscribedHookContext & { cfg: CarapaceConfig } {
...
export function toInternalMessagePreprocessedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: CarapaceConfig,
): MessagePreprocessedHookContext & { cfg: CarapaceConfig } {
  const shared = toInternalInboundMessageHookContextBase(canonical);
  return {
    ...shared,
    transcript: canonical.transcript,
