...
import { type CarapaceConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<CarapaceConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("carapace@gmail.com");
