...
function createBootstrapExtraConfig(paths: string[]): CarapaceConfig {
  return {
    hooks: {
      internal: {
        entries: {
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("carapace-bootstrap-extra-");
  ...
  it("appends configured nested memory without applying session policy", async () => {
    const tempDir = await makeTempWorkspace("carapace-bootstrap-extra-memory-");
    const extraDir = path.join(tempDir, "packages", "core");
  ...
  it("leaves subagent allowlist enforcement to the final resolver", async () => {
    const tempDir = await makeTempWorkspace("carapace-bootstrap-extra-subagent-");
