...
const FIXED_DESTINATION = "carapace-rsync-destination";
...
const receiverTarget =
  receiverMode === "git-pack"
    ? path.posix.join(workspace, ".carapace-base.pack")
    : receiverMode === "accepted-next"
      ? path.posix.join(
          path.posix.dirname(workspace),
          `.carapace-accepted-${createHash("sha256").update(workspace).digest("hex")}-${receiverNonce}`,
          "next",
        )
      : workspace;
