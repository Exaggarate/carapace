/**
 * Realtime voice tool definition and helpers for delegating work to Carapace.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "carapace_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured Carapace agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final Carapace result before answering with the actual result.`,
  };
}
...
/** Resolve the Carapace tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call carapace_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call carapace_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's Carapace-specific context.",
...
export function buildRealtimeVoiceSessionInstructions(params: {
  ...
  if (params.isAgentProxy) {
    return [
      ...
      "Mode: Carapace agent proxy.",
      "You are the realtime voice surface for the same Carapace agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with carapace_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to Carapace and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      'While waiting for Carapace data or tool results, use at most one short natural backchannel such as "yeah", "mm-hmm", "got it", or "one sec"; vary it and do not treat it as the final answer.',
      "When Carapace sends an internal exact answer to speak, do not call tools. Say only that answer.",
...
/** Build the delegated Carapace agent prompt for a live voice consult. */
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured Carapace agent on behalf of this user. Use available tools when the request asks you to do work.",
