...
import { CarapaceDatabaseSchemaPreflightError } from "../../state/carapace-database-preflight.js";
...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv, _config?: CarapaceConfig) => [
...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/carapace-stability.json",
  path: "/tmp/carapace-stability.json",
}));
const bootLifecycle = vi.hoisted(() => ({
  manualChannelStartHint: `Start a channel manually with: carapace gateway call channels.start --params '{"channel":"<id>"}'`,
  decisions: [] as Array<{
    tripped: boolean;
    uncleanBoots: number;
    windowMs: number;
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/carapace-test-missing-config.json",
...
vi.mock("../../config/paths.js", async (importOriginal) => ({
  ...
  CONFIG_PATH: "/tmp/carapace-test-missing-config.json",
  ...
  resolveConfigPath: () => "/tmp/carapace-test-missing-config.json",
  resolveStateDir: () => "/tmp",
...
vi.mock("../terminal-interactivity.js", () => ({
  ...
  NON_INTERACTIVE_GATEWAY_RUN_FORCE_MESSAGE:
    "Refusing to kill the operator's running gateway service from a non-interactive shell. Use an isolated dev gateway (carapace gateway run --dev, or --profile <name> with a free port) for testing.",
}));
...
describe("gateway run option collisions", () => {
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ CARAPACE_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { CARAPACE_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            CARAPACE_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.CARAPACE_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["CARAPACE_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(resolveShellEnvExpectedKeys).toHaveBeenCalledWith(
        expect.objectContaining({ CARAPACE_GATEWAY_TOKEN: "config-token" }),
        finalConfig,
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { CARAPACE_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["CARAPACE_GATEWAY_TOKEN"]);
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ...
              CARAPACE_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.CARAPACE_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.CARAPACE_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.CARAPACE_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.CARAPACE_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              CARAPACE_DEFER_SHELL_ENV_FALLBACK: "1",
              CARAPACE_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.CARAPACE_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { CARAPACE_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("admits deterministic legacy repairs to gateway preflight and rejects unrelated drift", async () => {
    const selectedStateDir = "/tmp/carapace-stable-upgrade-state";
    await withEnvAsync({ CARAPACE_STATE_DIR: undefined }, async () => {
      const stableConfig = {
        meta: {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/carapace-prepared-selected-state";
    await withEnvAsync({ CARAPACE_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        CARAPACE_GATEWAY_TOKEN: undefined,
        CARAPACE_PROXY_URL: undefined,
        CARAPACE_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              CARAPACE_PROXY_URL: "http://127.0.0.1:19876",
              CARAPACE_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        };
        const newConfig = {
          env: { vars: { CARAPACE_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.CARAPACE_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.CARAPACE_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.CARAPACE_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.CARAPACE_PROXY_URL).toBeUndefined();
        expect(process.env.CARAPACE_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("refuses non-interactive --force when a verified gateway appears before signaling", async () => {
    ...
    expect(startGatewayServer).not.toHaveBeenCalled();
    expect(runtimeErrors.join("\n")).toContain("carapace gateway run --dev");
  ...
  it("reports forced port cleanup failures before startup", async () => {
    ...
    expect(runtimeErrors.join("\n")).toContain("carapace gateway status --deep");
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.CARAPACE_SERVICE_MARKER).toBe("carapace");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.CARAPACE_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.CARAPACE_PROFILE).toBe("dev");
          expect(process.env.CARAPACE_STATE_DIR).toBeUndefined();
          expect(process.env.CARAPACE_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.CARAPACE_WORKSPACE_DIR).toBe("/tmp/carapace-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.CARAPACE_GATEWAY_TOKEN ??= "trusted-token";
          process.env.CARAPACE_PROFILE ??= "dev";
          if (process.env.CARAPACE_WORKSPACE_DIR === undefined) {
            setTestEnvValue("CARAPACE_WORKSPACE_DIR", "/tmp/carapace-reset-workspace");
          }
        });
        ...
        expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ CARAPACE_STATE_DIR: "/tmp/carapace-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/carapace-reset-original/carapace.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue("CARAPACE_STATE_DIR", "/tmp/carapace-reset-retargeted");
        return {
          dotenvPresentKeys: ["CARAPACE_STATE_DIR"],
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["CARAPACE_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.CARAPACE_STATE_DIR).toBe("/tmp/carapace-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    await withEnvAsync({ [selector]: "/tmp/carapace-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue(selector, "/tmp/carapace-reset-retargeted");
      });
      ...
      expect(process.env[selector]).toBe("/tmp/carapace-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ CARAPACE_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { CARAPACE_STATE_DIR: "/tmp/carapace-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ CARAPACE_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { CARAPACE_STATE_DIR: "/tmp/carapace-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.CARAPACE_STATE_DIR).toBe("/tmp/carapace-guarded-state");

      const finalConfig = {
        env: { vars: { CARAPACE_STATE_DIR: "/tmp/carapace-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.CARAPACE_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.CARAPACE_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
      ...
      withEnvAsync(
        {
          ...
          CARAPACE_SERVICE_MANAGED_ENV_KEYS: "OPENAI_API_KEY,ANTHROPIC_API_KEY",
        },
        async () => {
          const { prepareGatewayRunBootstrap, selectGatewayRunEnvironment } =
            await import("./pre-bootstrap.js");
  ...
  it("limits inherited service-managed dotenv refresh to systemd launches", async () => {
    ...
    expect(
      serviceManagedEnv.readManagedSystemdServiceEnvKeysFromEnvironment(
        {
          INVOCATION_ID: "systemd-invocation",
          CARAPACE_SERVICE_MANAGED_ENV_KEYS: "OPENAI_API_KEY",
        },
        "linux",
      ),
  ...
  it("clears only missing managed keys after reading the selected config", async () => {
    ...
    await withMockedPlatform("linux", () =>
      withEnvAsync(
        {
          INVOCATION_ID: "systemd-invocation",
          CARAPACE_SERVICE_MANAGED_ENV_KEYS: "REMOVED_KEY,SECRET_REF_KEY",
  ...
  it.each(["initial", "restart", "cause", "aggregate"] as const)(
    ...
    async (kind) => {
      ...
      try {
        ...
        expect(message).toBe(
          `Legacy session store requires migration: ${storePath}. Run "carapace doctor --fix" against the same state/config before starting Carapace.`,
        );
        const failure =
          kind === "cause"
  ...
  it("exits 78 when the only startup blocker is legacy workspace setup state", async () => {
    ...
    const source = path.join(workspaceDir, "carapace-workspace-state.json");
  ...
  it("exits 78 and parks launchd for a repairable shared-state schema", async () => {
    ...
    startGatewayServer.mockRejectedValueOnce(
      new CarapaceStateDatabaseSchemaMigrationRequiredError(
  ...
  it("does not park launchd for a nonrepairable shared-state schema", async () => {
    startGatewayServer.mockRejectedValueOnce(
      new Error(
        "Carapace state database /tmp/carapace.sqlite has a noncanonical agent database registry schema that cannot be repaired automatically.",
      ),
    );
  ...
  it.each([
    ...
    const error =
      kind === "state" || kind === "agent"
        ? new CarapaceDatabaseSchemaPreflightError([
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `carapace onboard --mode local` or `carapace setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/carapace-test-missing-config.json",
      config: {},
      parsed: null,
  ...
  it("does not offer doctor repair after --allow-unconfigured reaches startup", async () => {
    ...
    startGatewayServer.mockRejectedValueOnce(
      createInvalidConfigError("/tmp/carapace.json", "gateway.mode: invalid"),
    );
  ...
  it("reads gateway password from --password-file", async () => {
    ...
    expect(runtimeErrors).not.toContain(
      "Warning: --password can be exposed via process listings. Prefer --password-file or CARAPACE_GATEWAY_PASSWORD.",
    );
  });
