...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<CarapaceConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.CARAPACE_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).CARAPACE_SERVICE_MARKER?.trim()
        : undefined,
    );
