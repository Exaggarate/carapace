...
export async function handleGatewayStartupMaintenance(error: unknown): Promise<boolean> {
  ...
  const stop = `Stop the service with ${formatCliCommand("carapace gateway stop")} (or its service owner), then`;
  const guidance =
    reason === "a newer Carapace build"
      ? `${stop} restore your pre-update backup created with ${formatCliCommand("carapace backup")}, then start it again with ${formatCliCommand("carapace gateway start")}. See https://github.com/Exaggarate/carapace#rollback.`
      : `${stop} run ${formatCliCommand("carapace doctor --fix")}, then start it again with ${formatCliCommand("carapace gateway start")}.`;
  let parked = false;
  try {
