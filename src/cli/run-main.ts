...
async function resolveLocalGatewayProbeTargets(
  ...
  const hasExplicitPort = Boolean(normalizeOptionalString(process.env.CARAPACE_GATEWAY_PORT));
...
function isDebugProxyCaptureEnvEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    isTruthyEnvValue(env.CARAPACE_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.CARAPACE_DEBUG_PROXY_REQUIRE)
  );
}
...
function createAllowlistAgnosticCliLookupConfig(config: CarapaceConfig): CarapaceConfig {
...
async function runCliWithPreparedOutputMode(
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.CARAPACE_CONTAINER) ?? null;
  ...
  if (shouldEnsureCliPath(normalizedArgv)) {
    const { ensureCarapaceCliOnPath } = await import("../infra/path-env.js");
  ...
  let bestEffortConfigPromise: Promise<CarapaceConfig> | null = null;
  ...
  const readBestEffortCliConfig = async (): Promise<CarapaceConfig> => {
  ...
  const replaceStartedProxy = async (config: CarapaceConfig["proxy"]) => {
  ...
  try {
    ...
    // `carapace <typo> --help` surfaces the same Unknown command error as
    // `carapace <typo>` instead of silently showing generic top-level help.
