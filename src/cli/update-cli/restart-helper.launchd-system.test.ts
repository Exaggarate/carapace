...
describe("macOS update restart system ownership", () => {
  ...
  it("refuses the detached handoff before user activation when a system owner appears", async () => {
    ...
    process.getuid = () => 501;
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-restart-system-"));
...
`,
    ...
    try {
      ...
      expect(log).toContain("carapace restart blocked source=update");
      expect(log).toContain("loaded system LaunchDaemon system/ai.carapace.gateway");
    } finally {
