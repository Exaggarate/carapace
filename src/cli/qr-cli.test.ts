...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("carapace devices approve <requestId>");
  });
  ...
  it("does not let CARAPACE_GATEWAY_PASSWORD mask a local password SecretRef", async () => {
    vi.stubEnv("CARAPACE_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
