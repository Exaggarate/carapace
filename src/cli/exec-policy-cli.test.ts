...
const mocks = vi.hoisted(() => {
  ...
  let configState: CarapaceConfig = {
    tools: {
      exec: {
        host: "auto",
        mode: "ask",
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: CarapaceConfig) => void }) => {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: CarapaceConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: CarapaceConfig }>
    >(async () => ({
      path: "/tmp/carapace.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: CarapaceConfig) => void }) => {
  ...
  it("renders an unstored fresh-install policy as defaults instead of missing", async () => {
    mocks.readExecApprovalsSnapshot.mockImplementationOnce(() => ({
      path: "~/.carapace/state/carapace.sqlite#exec_approvals_config",
      exists: false,
      raw: null,
      hash: "missing-hash",
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/carapace.json");
    ...
    expect(output).not.toContain("/tmp/carapace.json\nforged");
