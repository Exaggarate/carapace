...
const previousClawHubUrl = process.env.CARAPACE_CLAWHUB_URL;
...
afterEach(async () => {
  ...
  if (previousClawHubUrl === undefined) {
    ...
    process.env.CARAPACE_CLAWHUB_URL = previousClawHubUrl;
  }
...
describe("carapace plugins search ClawHub E2E", () => {
  it("keeps plugin discovery separate from skills and surfaces empty and failed lookups", async () => {
    ...
    process.env.CARAPACE_CLAWHUB_URL = baseUrl;
    ...
    expect(terminalOutput).toContain("Install: carapace plugins install clawhub:@acme/calendar");
