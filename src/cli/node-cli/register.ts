...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "carapace node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["carapace node status", "Check node host service status."],
          ["carapace node install", "Install the node host service."],
          ["carapace node start", "Start the installed node host service."],
          ["carapace node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "github.com/Exaggarate/carapace")}\n`,
    );
  ...
  node
    ...
    .option("--context-path <path>", "Gateway WebSocket context path (e.g. /carapace-gw)")
