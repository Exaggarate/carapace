...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["carapace security audit", "Run a local security audit."],
          [
            "carapace security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["carapace security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
          ...
          ["carapace security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["carapace security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "github.com/Exaggarate/carapace")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Carapace security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("carapace security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("carapace security audit --fix")}`));
        if (!fixResult) {
