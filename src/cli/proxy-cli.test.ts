...
describe("proxy cli", () => {
  ...
  it.each([
    ...
    expect(() => program.parse(["node", "carapace", ...args])).toThrow(expected);
  });

  it("normalizes signed decimal numeric options through the shared parser", async () => {
    const program = createProgram();

    await program.parseAsync(["node", "carapace", "proxy", "start", "--port", "+08080"]);
    await program.parseAsync(["node", "carapace", "proxy", "validate", "--timeout-ms", "+01000"]);
    await program.parseAsync(["node", "carapace", "proxy", "sessions", "--limit", "+05"]);
