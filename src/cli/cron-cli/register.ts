...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "github.com/Exaggarate/carapace")}\n${theme.muted("Upgrade tip:")} run \`carapace doctor --fix\` to normalize legacy automation storage.\n`,
    );
