...
async function promptResumeSession(
  ...
  if (choices.length === 0) {
    throw new Error(
      "No recent sessions found. Run `carapace sessions` to inspect sessions or `carapace tui` to start one.",
    );
  }
...
function reportResumeFailure(
  ...
  defaultRuntime.error(
    "Run `carapace resume` to choose from recent sessions or `carapace sessions` to inspect all sessions.",
  );
}
