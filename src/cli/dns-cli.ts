...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "github.com/Exaggarate/carapace")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. carapace.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($CARAPACE_CONFIG_PATH, default ~/.carapace/carapace.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by carapace dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active Carapace config ($CARAPACE_CONFIG_PATH, default ~/.carapace/carapace.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
