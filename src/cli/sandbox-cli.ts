...
const SANDBOX_EXAMPLES = {
  main: [
    ["carapace sandbox list", "List all sandbox containers."],
    ["carapace sandbox list --browser", "List only browser containers."],
    ["carapace sandbox recreate --all", "Recreate all containers."],
    ["carapace sandbox recreate --session main", "Recreate a specific session."],
    ["carapace sandbox recreate --agent mybot", "Recreate agent containers."],
    ["carapace sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["carapace sandbox list", "List all sandbox containers."],
    ["carapace sandbox list --browser", "List only browser containers."],
    ["carapace sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["carapace sandbox recreate --all", "Recreate all containers."],
    ["carapace sandbox recreate --session main", "Recreate a specific session."],
    ["carapace sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["carapace sandbox recreate --browser --all", "Recreate only browser containers."],
    ["carapace sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["carapace sandbox explain", "Show effective sandbox config."],
    ["carapace sandbox explain --session agent:main:main", "Explain a specific session."],
    ["carapace sandbox explain --agent work", "Explain an agent sandbox."],
    ["carapace sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "github.com/Exaggarate/carapace")}\n`,
    )
    .action(() => {
