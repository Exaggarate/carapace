...
describe("Claw project CLI", () => {
  ...
  it("runs create, validate, build, and offline dev against the built artifact", async () => {
    const root = join(tempDirs.make("carapace-claw-author-"), "author-flow");
    const artifact = join(tempDirs.make("carapace-claw-author-output-"), "author-flow.tgz");
    ...
    expect(payloads.map((payload) => payload.schemaVersion)).toEqual([
      "carapace.clawProject.v1",
      "carapace.clawProject.v1",
      "carapace.clawBuild.v1",
      "carapace.clawDev.v1",
    ]);
  ...
  it("emits stable dev plans without deleted extraction paths", async () => {
    const root = join(tempDirs.make("carapace-claw-dev-stable-"), "stable-dev");
    await runClawsCreateCommand(root, { json: true });
    const options = {
    ...
    expect(JSON.stringify(first.plan)).not.toContain("carapace-claw-artifact-");
  ...
  it("uses command-specific schemas for build and dev failures", async () => {
    const missing = join(tempDirs.make("carapace-claw-errors-"), "missing");

    await expect(
      runClawsBuildCommand(missing, {
        out: join(tempDirs.make("carapace-claw-error-output-"), "missing.tgz"),
        json: true,
      }),
    ...
    expect(payloads).toMatchObject([
      { schemaVersion: "carapace.clawBuild.v1", ok: false },
      { schemaVersion: "carapace.clawDev.v1", ok: false },
    ]);
  });
});
