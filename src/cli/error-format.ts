...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "carapace channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `carapace channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON. For structured changes, use a JSON5 config patch object file with ${formatInlineCliCommand(
      "carapace config patch --file <path> --dry-run",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?carapace doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "carapace gateway status --deep";
...
export function formatMissingPluginMessage(params: {
  ...
  const listCommand = params.listCommand ?? "carapace plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("carapace plugins search " + params.id)} to look for installable plugins`
    : "";
