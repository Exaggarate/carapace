...
describe("nodes-cli coverage", () => {
  ...
  beforeAll(async () => {
    ...
    await registerNodesCli(sharedProgram, ["node", "carapace", "nodes", "status"]);
  });
  ...
  it("shows the registered pending command in node pairing help", () => {
    ...
    expect(output.join("")).toContain("carapace nodes pending");
    expect(output.join("")).not.toContain("carapace nodes pairing pending");
  });
  ...
  it("explains unknown nodes approve request ids with the current pending requests", async () => {
    ...
    expect(output).toContain("carapace nodes pending");
