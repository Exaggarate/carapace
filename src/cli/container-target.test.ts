...
function expectContainerExec(
  ...
  expect(spawnSync).toHaveBeenNthCalledWith(
    ...
    [
      ...
      ...(params.tty ? ["-t"] : []),
      envFlag,
      `CARAPACE_CONTAINER_HINT=${params.container ?? "demo"}`,
      envFlag,
      "CARAPACE_CLI_CONTAINER_BYPASS=1",
      ...(params.proxyUrl ? [envFlag, `CARAPACE_PROXY_URL=${params.proxyUrl}`] : []),
    ...
    {
      stdio: "inherit",
      env: { ...params.env, CARAPACE_CONTAINER: "" },
    },
  );
}
...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "carapace", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carapace", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "carapace", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carapace", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "carapace", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "carapace", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "carapace", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "carapace", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "carapace", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to CARAPACE_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "carapace", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "carapace", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "carapace", "status"], {
        CARAPACE_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "carapace", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "carapace", "status"],
    });
  });
  ...
  it.each([
    ...
    expect(
      maybeRunCliInContainer(["node", "carapace", "status"], {
        env: { CARAPACE_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("uses CARAPACE_CONTAINER when the flag is absent", () => {
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "carapace", "status"], {
      env: {
        ...
        CARAPACE_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("passes the proxy URL env fallback into the child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "carapace", "status"], {
      env: {
        CARAPACE_CONTAINER: "demo",
        CARAPACE_PROXY_URL: " http://proxy.internal:3128 ",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expectContainerExec(spawnSync, {
      ...
      env: { CARAPACE_PROXY_URL: " http://proxy.internal:3128 " },
    });
  });
  ...
  it("redacts proxy URL credentials and URL suffixes before rejecting loopback container proxy forwarding", () => {
    ...
    expect(message).toContain("CARAPACE_PROXY_URL=http://redacted:redacted@127.0.0.1:3128/");
  ...
  it("allows explicitly overridden loopback proxy URL forwarding into a child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "carapace", "status"], {
      env: {
        ...
        CARAPACE_CONTAINER_ALLOW_LOOPBACK_PROXY_URL: "1",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expect(podmanCall[1]).toContain("CARAPACE_PROXY_URL=http://127.0.0.1:3128");
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "carapace", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "carapace", "--container", "demo", "health"], {
        env: { USER: "carapace" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "carapace", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over CARAPACE_CONTAINER", () => {
    const spawnSync = mockSpawn(runningContainer, missingContainer, successfulExec);

    expect(
      maybeRunCliInContainer(["node", "carapace", "--container", "flag-demo", "health"], {
        env: { CARAPACE_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "carapace", "--container", "demo", "status"], {
        env: { CARAPACE_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "carapace", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "carapace", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
      "carapace update is not supported with --container; rebuild or restart the container image instead.",
    );
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "carapace", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
