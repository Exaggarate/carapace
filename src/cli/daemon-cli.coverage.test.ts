...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      CARAPACE_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { CARAPACE_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { CARAPACE_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-daemon-cli-"));
    ...
    setTestEnvValue("CARAPACE_STATE_DIR", path.join(tmpDir, ".carapace"));
    setTestEnvValue("CARAPACE_CONFIG_PATH", path.join(tmpDir, ".carapace", "carapace.json"));
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        CARAPACE_STATE_DIR: "/tmp/carapace-daemon-state",
        CARAPACE_CONFIG_PATH: "/tmp/carapace-daemon-state/carapace.json",
        CARAPACE_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.carapace.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).CARAPACE_WRAPPER).toBe(
      "/usr/local/bin/carapace-doppler",
    );
  });
