...
const CONFIG_SET_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("carapace config set gateway.port 19001 --strict-json"),
  formatCliCommand(
    "carapace config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
  ),
  formatCliCommand(
    "carapace config set secrets.providers.vault --provider-source file --provider-path /etc/carapace/secrets.json --provider-mode json",
  ),
  formatCliCommand("carapace config set --batch-file ./config-set.batch.json --dry-run"),
].join("\n");
...
const CONFIG_PATCH_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("carapace config patch --file ./carapace.patch.json5 --dry-run"),
  formatCliCommand("carapace config patch --stdin"),
].join("\n");
  ...
  try {
    ...
    if (!res.found) {
      const message = isConfigSchemaPath(schema, parsedPath)
        ? `Config path is valid but unset: ${opts.path}. The runtime default applies until you set an authored value with ${formatCliCommand(`carapace config set ${quoteCliArg(opts.path)} <value>`)}.`
        : `Unknown config path: ${opts.path}. Run ${formatCliCommand("carapace config schema")} to inspect valid paths.`;
      if (opts.json) {
...
async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "carapace.json";
  try {
    const read = await readConfigFileSnapshotWithPluginMetadata({ observe: false });
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("carapace onboard")} or run ${formatCliCommand("carapace doctor --fix")}.`,
        );
      }
      exitCliAfterOutput(runtime, 1);
    }
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        writeRuntimeJson(runtime, {
          ...formatCliJsonFailure(`Carapace config is invalid: ${shortPath}`),
          valid: false,
          path: outputPath,
          issues,
        });
      } else {
        runtime.error(danger(`Carapace config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("carapace config validate")}.`);
      }
      exitCliAfterOutput(runtime, 1);
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "github.com/Exaggarate/carapace")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only", ({ argv }) => isConfigSetJsonParseOnly(argv))
    ...
    .option(
      "--dry-run",
      "Validate changes without writing carapace.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing carapace.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for carapace.json")
    .option("--json", "Output JSON", false)
