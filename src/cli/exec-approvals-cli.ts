...
import { readBestEffortConfig, type CarapaceConfig } from "../config/config.js";
...
async function loadPendingApprovals(
  ...
  const [exec, plugin, systemAgent] = await Promise.all([
    ...
    listCall("carapace.approval.list"),
  ]);
  return [
...
function renderStandingGrants(grants: StandingGrantCliEntry[]): void {
  ...
  defaultRuntime.log(theme.muted("Revoke with: carapace approvals grants revoke <grant-id>"));
}
  ...
  if (!params.approvals) {
    return {
      ...
      note: params.nativePolicy
        ? "This node enforces a host-native exec policy; Carapace approvals-file policy math does not apply."
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "github.com/Exaggarate/carapace")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatExample(
          'carapace approvals allowlist add "~/Projects/**/bin/rg"',
          ...
          'carapace approvals allowlist remove "~/Projects/**/bin/rg"',
