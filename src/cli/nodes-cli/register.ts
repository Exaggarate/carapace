    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["carapace nodes status", "List known nodes with live status."],
          ["carapace nodes pending", "Show pending node pairing requests."],
          ["carapace nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'carapace nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["carapace nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "github.com/Exaggarate/carapace")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `carapace nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
