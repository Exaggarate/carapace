...
describe("Claw authoring process state", () => {
  it("leaves migration-pending operator state unchanged for claws dev", async () => {
    const root = tempDirs.make("carapace-claws-dev-state-");
    const external = tempDirs.make("carapace-claws-dev-external-");
    ...
    await fs.writeFile(path.join(root, "config", "carapace.json"), "{}\n");
