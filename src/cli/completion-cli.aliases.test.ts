...
describe("completion-cli command aliases", () => {
  itWithFish.each([
    ["a canonical root command", "carapace --profile work inf", "infer"],
    ["an aliased root command", "carapace --profile work cap", "capability"],
    ["an inline profile and alias", "carapace --profile=work cap", "capability"],
    ["an alias-shaped profile value", "carapace --profile capability cap", "capability"],
    ["a repeated profile and alias", "carapace --profile first --profile second cap", "capability"],
  ...
  it("completes root and nested aliases in zsh lists and dispatch", () => {
    ...
    expect(script).toContain("(infer|capability) _carapace_infer ;;");
    expect(script).toContain("'create[Add a job]'");
    expect(script).toContain("(add|create) _carapace_cron_add ;;");
  });
  ...
  it("completes aliases and their subtrees in fish", () => {
    ...
    expect(script).toContain(
      'complete -c carapace -n "__carapace_command_path_matches -- --profile" -a "capability" -d \'Run inference\'',
    );
    expect(script).toContain(
      'complete -c carapace -n "__carapace_command_path_matches capability -- --profile" -a "embed" -d \'Embed text\'',
    );
    expect(script).toContain(
      'complete -c carapace -n "__carapace_command_path_matches cron -- --profile" -a "create" -d \'Add a job\'',
    );
    expect(script).toContain(
      "complete -c carapace -n \"__carapace_command_path_matches cron create -- --profile --at\" -l at -r -d 'Schedule time'",
    );
  });

  itWithFish.each([
    ["an aliased nested command", "carapace cron create -"],
    ["a canonical nested command", "carapace cron add -"],
    ["a global profile", "carapace --profile work cron create -"],
    ["an inline global profile", "carapace --profile=work cron create -"],
    ["repeated global profiles", "carapace --profile first --profile second cron create -"],
    ["an inherited global profile", "carapace cron --profile work create -"],
    ["a parent long option", "carapace cron --timezone UTC create -"],
    ["a parent short option", "carapace cron -z UTC create -"],
    ["an inline parent option", "carapace cron --timezone=UTC create -"],
    ["a parent boolean option", "carapace cron --verbose create -"],
  ...
  itWithFish.each([
    ["an aliased positional argument", "carapace cron create meeting -"],
    ["a canonical positional argument", "carapace cron add meeting -"],
    ["a profiled positional argument", "carapace --profile work cron create meeting -"],
    ["a parent option and positional argument", "carapace cron -z UTC create meeting -"],
  ...
  itWithPowerShell.each([
    ["a global option", "carapace --profile work cron create --a"],
    ["an inline global option", "carapace --profile=work cron create --a"],
    ["repeated global options", "carapace --profile first --profile second cron create --a"],
    ["an inherited option after the parent", "carapace cron --profile work create --a"],
    ["the canonical nested command", "carapace --profile work cron add --a"],
