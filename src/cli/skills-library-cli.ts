...
export function registerSkillsLibraryCli(skills: Command): void {
  const library = addGatewayClientOptions(
    ...
    .addHelpText(
      "after",
      "\nCreate: carapace skills library create ./my-skill --slug my-skill\nRead:   carapace skills library read <skill-id> --json\nUpdate: carapace skills library update <skill-id> ./SKILL.md --expected-revision <hash>\nPersonal libraries require a signed-in Gateway profile. Workspace installs remain under skills install.\n",
    );
