    ...
    approval: {
      ...
      presentation:
        kind === "exec"
          ...
          : {
              kind,
              title: kind === "plugin" ? "Plugin action" : "Carapace change",
...
describe("exec approvals pending and resolve CLI", () => {
  ...
  it("renders pending approvals from all three approval kinds", async () => {
    ...
    callGatewayFromCli.mockImplementation(async (method: string) => {
      ...
      if (method === "carapace.approval.list") {
        return [
          {
            id: "system-agent:1",
            request: {
    ...
    expect(output).toContain("Carapace change: Change the system configuration");
