...
function readDevicePairPublicUrlFromConfig(cfg: CarapaceConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.CARAPACE_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: CarapaceConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "github.com/Exaggarate/carapace")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        ...
        const lines: string[] = [
          ...
          "Scan this with the Carapace mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("carapace devices list")}`,
          `  ${theme.command("carapace devices approve <requestId>")}`,
        );
