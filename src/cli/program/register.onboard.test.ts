...
describe("registerOnboardCommand", () => {
  ...
  it("forwards --gateway-token-ref-env", async () => {
    await runCli(["onboard", "--gateway-token-ref-env", "CARAPACE_GATEWAY_TOKEN"]);
    expect(setupWizardOptions().gatewayTokenRefEnv).toBe("CARAPACE_GATEWAY_TOKEN");
  });
  ...
  it("routes --modern through the inference-gated Carapace entrypoint", async () => {
