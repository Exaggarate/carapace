...
/** Parse `carapace health` flags for the route-first status family. */
...
/** Parse `carapace status` flags without registering the full command tree. */
...
/** Parse `carapace gateway status` RPC-only flags accepted by the fast route. */
...
/** Parse machine-readable `carapace gateway health` calls for route-first execution. */
...
/** Parse `carapace sessions` filters for JSON/list route execution. */
...
/** Parse `carapace agents list` display switches for route-first execution. */
...
/** Parse `carapace config get <path>` while preserving root option handling. */
...
/** Parse `carapace config unset <path>` and its mutation guard flags. */
...
/** Parse `carapace models list` filters for the lightweight model catalog route. */
...
/** Parse both parent aliases and `carapace models status` through one status owner. */
...
/** Parse `carapace channels list` display flags for the route-first list path. */
...
/** Parse `carapace channels status` probe flags without full CLI registration. */
...
/** Parse `carapace plugins list` flags for the metadata-only inventory path. */
...
/** Parse both `carapace tasks --json` and `carapace tasks list --json` aliases. */
...
/** Parse JSON-only `carapace tasks audit` filters for the route-first audit path. */
