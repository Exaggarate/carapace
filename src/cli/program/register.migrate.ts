...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration Carapace backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["carapace migrate list", "Show available migration providers."],
          ["carapace migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["carapace migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "carapace migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "carapace migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Carapace state"),
