...
async function registerAllBuiltInCommands(): Promise<Command> {
  ...
  const argv = ["node", "carapace", "completion"];
...
describe("root command descriptions", () => {
  beforeEach(() => {
    vi.stubEnv("CARAPACE_ENABLE_PRIVATE_QA_CLI", "");
  });
  ...
  it("keeps startup policy catalog paths registered or explicitly reserved", async () => {
    ...
    const lazyProgram = new Command().name("carapace");
    registerSubCliCommands(lazyProgram, ["node", "carapace", "--help"]);
