...
describe("private-qa-cli", () => {
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.CARAPACE_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });

  it("loads the private QA CLI from a source checkout path", async () => {
    process.env.CARAPACE_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-private-qa-source-"));
  ...
  it("loads the private QA CLI from a raw synced source checkout path", async () => {
    process.env.CARAPACE_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-private-qa-raw-source-"));
    tempDirs.push(repoRoot);
    const expectedPaths = new Set([
  ...
  it("rejects non-source package roots even when private QA is enabled", () => {
    process.env.CARAPACE_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "carapace" }), "utf8");
    const importModule = vi.fn(async () => ({}));
    ...
    ).toThrow("Private QA CLI is only available from an Carapace source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an Carapace source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
