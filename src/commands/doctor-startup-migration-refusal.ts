...
export function throwStartupMigrationGuardRejected(): never {
  throw new Error(
    "Carapace startup migrations were skipped because the selected config changed during startup; refusing to report the gateway ready. Retry startup so the new config can be validated.",
  );
}

export function throwStartupMigrationIdentityChanged(): never {
  throwStartupMigrationRefusal(
    "Carapace migration inputs changed during startup; refusing to report the gateway ready. Restart Carapace so state migrations run against the final config and plugin inventory.",
  );
}
...
async function describeLiveGatewayOwnerStartupBlocker(
  ...
  return `Another gateway (pid ${activeGateway.pid}) already owns this state directory; refusing to run automatic startup migrations or report the gateway ready. Stop it with "carapace gateway stop" (or select a different CARAPACE_STATE_DIR), then retry startup.`;
}
