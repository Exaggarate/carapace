...
function createTelegramCfg(botToken: string, enabled?: boolean): CarapaceConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@carapace/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
    ...
    install: {
      npmSpec: "@carapace/external-chat",
    },
  };
}
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: CarapaceConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: CarapaceConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: CarapaceConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: CarapaceConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: CarapaceConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as CarapaceConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: CarapaceConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("shows explicit dmScope config command in channel primer", async () => {
    ...
    await runSetupChannels({} as CarapaceConfig, prompter);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@carapace/external-chat-plugin": { enabled: true },
          },
        },
      } as CarapaceConfig,
      prompter,
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: CarapaceConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: CarapaceConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: CarapaceConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: CarapaceConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
