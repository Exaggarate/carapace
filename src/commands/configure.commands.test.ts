...
describe("configureCommandFromSectionsArg", () => {
  ...
  it("fails closed on a non-interactive terminal before reaching the wizard (#93953)", async () => {
    ...
    expect(message).toContain(formatCliCommand("carapace config set"));
    expect(message).toContain(formatCliCommand("carapace config validate"));
  ...
  it("rejects a lone invalid section before unrestricted wizard dispatch", async () => {
    ...
    expect(runtime.error.mock.calls[0]?.[0]).toBe(
      "Invalid --section: bogus. Expected one of: workspace, model, web, gateway, daemon, channels, plugins, skills, health. Run carapace configure without --section to use the full wizard.",
    );
    expect(runConfigureWizardMock).not.toHaveBeenCalled();
  });
