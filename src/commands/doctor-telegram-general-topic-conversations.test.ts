...
describe("doctor Telegram General-topic conversation repair", () => {
  ...
  beforeEach(() => {
    const root = tempDirs.make("carapace-doctor-telegram-topic-");
    storePath = path.join(root, "sessions.json");
    cfg = { session: { store: storePath } };
    env = {
      ...process.env,
      CARAPACE_CONFIG_PATH: path.join(root, "missing-carapace.json"),
  ...
  it("merges an upgraded General-topic related binding exactly once", async () => {
    ...
    const database = openCarapaceAgentDatabase(toDatabaseOptions(resolveSqliteReadScope(scope)));
