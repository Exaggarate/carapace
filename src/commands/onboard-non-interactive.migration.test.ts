...
vi.mock("../config/config.js", () => ({
  ...
  replaceConfigFile: async ({ sourceConfig }: { sourceConfig: CarapaceConfig }) => {
    configStore.set(configPath(), structuredClone(sourceConfig));
    return { nextConfig: sourceConfig };
  },
  resolveGatewayPort: (config: CarapaceConfig) => config.gateway?.port ?? 18789,
}));
...
describe("non-interactive migration onboarding", () => {
  ...
  it("stages, promotes, and acknowledges an explicit import", async () => {
    const stateDir = tempRoots.make("carapace-noninteractive-migration-");
    ...
    expect(applyContext?.reportDir).toContain(".carapace-migration-state-");
    expect(stagedPlan?.items[0]?.target).toContain(".carapace-migration-workspace-");
