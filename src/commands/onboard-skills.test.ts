...
describe("setupSkills", () => {
  ...
  it("bounds skill hints and install failures through the onboarding flow", async () => {
    ...
    await setupSkills({} as CarapaceConfig, "/tmp/ws", runtime, prompter);
  ...
  it("displays a clear empty state note when all skill dependencies are ready", async () => {
    ...
    expect(emptyStateNote?.message).toContain("carapace skills list --verbose");
    expect(emptyStateNote?.message).toContain("carapace skills check");
  });
  ...
  it("does not ask for API keys when skills are missing env vars", async () => {
    ...
    const { prompter } = createPrompter({});
    const next = await setupSkills({} as CarapaceConfig, "/tmp/ws", runtime, prompter);
