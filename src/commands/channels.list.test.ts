...
describe("channels list", () => {
  ...
  it("default output shows configured official external channels when the plugin is missing", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      installCommand: "carapace plugins install @carapace/discord",
      doctorFixCommand: "carapace doctor --fix",
      repairHint:
        "Install the official external plugin with: carapace plugins install @carapace/discord, or run: carapace doctor --fix.",
    });
    mocks.readConfigFileSnapshot.mockResolvedValue(
      createTestConfigSnapshot({
        channels: {
    ...
    expect(output).toContain(
      "run carapace plugins install @carapace/discord or carapace doctor --fix",
    );
