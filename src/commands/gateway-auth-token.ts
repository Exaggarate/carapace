...
export async function gatewayAuthTokenCommand(
  ...
  if (!(options.interactive ?? isTerminalInteractive())) {
    throw new Error(
      "Refusing to print the Gateway token outside an interactive terminal. Run `carapace gateway auth-token --show` directly in a terminal on the Gateway host.",
    );
  }

  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    throw new Error("Gateway config is invalid. Run `carapace doctor --fix`, then try again.");
  }
  ...
  if (resolvedAuth.mode !== "token" || !resolvedAuth.token) {
    throw new Error(
      "No configured Gateway token is available. Run `carapace doctor --generate-gateway-token`, restart the Gateway, then try again.",
    );
  }
