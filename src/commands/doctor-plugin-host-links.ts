...
import { coerceErrorMessage as formatPackageReadFailure } from "@carapace/normalization-core/error-coercion";
...
import {
  ...
  type CarapacePeerLinkAuditIssue,
...
type PluginHostLinkAudit = {
  peerLinkIssues: CarapacePeerLinkAuditIssue[];
  packageReadFailures: PluginPackageReadFailure[];
  registeredPeerLinkIssues: CarapacePeerLinkAuditIssue[];
...
export async function listPluginCarapaceHostLinkIssues(
  params: InstalledPluginIndexRecordStoreOptions,
  ...
  const registeredAudit = await reconcileRegisteredCarapaceHostLinks({
...
/** Relinks npm-owned plugin packages to the current Carapace host package. */
export async function maybeRepairPluginCarapaceHostLinks(
  params: PluginHostLinkDoctorParams,
): Promise<boolean> {
  const npmRoots = resolveDoctorPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audit = await listPluginCarapaceHostLinkIssues(params);
    if (audit.peerLinkIssues.length > 0) {
      note(
        [
          "Managed npm Carapace host peer links need repair:",
          ...audit.peerLinkIssues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("carapace doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
    ...
    if (audit.registeredPeerLinkIssues.length > 0) {
      note(
        [
          "Registered npm plugin Carapace host links need repair:",
          ...audit.registeredPeerLinkIssues.map(
            (issue) => `- ${issue.packageName}: ${issue.reason}`,
          ),
          `Repair with ${formatCliCommand("carapace doctor --fix")} to relink registered npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Carapace host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
