...
describe("buildNodeInstallPlan", () => {
  ...
  it("marks node gateway credentials as file-backed service env", async () => {
    ...
    mocks.renderSystemNodeWarning.mockReturnValue(undefined);
    mocks.buildNodeServiceEnvironment.mockReturnValue({
      CARAPACE_GATEWAY_TOKEN: "node-token",
      CARAPACE_GATEWAY_PASSWORD: "node-password",
      CARAPACE_SERVICE_MARKER: "carapace",
    });
    ...
    expect(plan.environment.CARAPACE_GATEWAY_TOKEN).toBe("node-token");
    expect(plan.environment.CARAPACE_GATEWAY_PASSWORD).toBe("node-password");
    expect(plan.description).toBe("Carapace Node Host");
    expect(plan.environmentValueSources).toEqual({
      CARAPACE_GATEWAY_TOKEN: "file",
      CARAPACE_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
      CF_ACCESS_CLIENT_ID: "file",
