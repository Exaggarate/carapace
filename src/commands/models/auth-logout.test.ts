...
vi.mock("./shared.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveModelsTargetAgent: (_cfg: CarapaceConfig, rawAgentId?: string) => ({
...
function applyCapturedConfigUpdate(cfg: CarapaceConfig): CarapaceConfig {
  const mutator = mocks.updateConfig.mock.calls[0]?.[0] as
    | ((current: CarapaceConfig) => CarapaceConfig)
    | undefined;
  if (!mutator) {
...
describe("models auth logout", () => {
  beforeEach(() => {
    ...
    mocks.updateConfig.mockResolvedValue({} as CarapaceConfig);
