...
export async function modelsAuthLogoutCommand(
  ...
  if (!profileId) {
    throw new Error(
      `Missing profile id. Run ${formatCliCommand("carapace models auth list")} to see saved profile ids.`,
    );
  }
  ...
  if (!credential) {
    throw new Error(
      `Auth profile "${profileId}" not found for agent "${agentId}". Run ${formatCliCommand(`carapace models auth list --agent ${agentId}`)} to see saved profile ids.`,
    );
  }
  ...
  if (boundProvider) {
    throw new Error(
      `Auth profile "${profileId}" is referenced by models.providers.${boundProvider}.apiKey. Change that config value first, then rerun ${formatCliCommand(`carapace models auth logout ${profileId}`)}.`,
    );
  }
  ...
  if (remaining.length === 0) {
    runtime.log(
      `No auth profiles remain for ${credential.provider}. Run ${formatCliCommand(`carapace models auth login --provider ${credential.provider}`)} to sign in again.`,
    );
  }
}
