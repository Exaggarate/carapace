...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: CarapaceConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: CarapaceConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let lastUpdatedConfig: CarapaceConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/carapace/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/carapace/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/carapace/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: CarapaceConfig) => CarapaceConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/carapace/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: CarapaceConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/carapace/agents/coder" : "/tmp/carapace/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: CarapaceConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/carapace/workspaces/coder" : "/tmp/carapace/workspace",
    );
  ...
  it("runs plugin-owned openai login", async () => {
    ...
    expect(upsertCall.agentDir).toBe("/tmp/carapace/agents/main");
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Default model available: openai/gpt-5.5 (current default unchanged; run carapace models set openai/gpt-5.5 to apply)",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Configure the `web_search` tool with `carapace configure --section web`. Docs: https://github.com/Exaggarate/carapace",
    );
  ...
  it("shows local agent scope before provider auth login", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "For personal model accounts on a Gateway, run carapace models accounts login --help.",
      ].join("\n"),
      "Provider sign-in",
    );
    ...
    expect(authRunCall.agentDir).toBe("/tmp/carapace/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/carapace/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileAfterLoginWithLock) as UpsertAuthProfileCall)
        .agentDir,
    ).toBe("/tmp/carapace/agents/coder");
    expect(mocks.callGateway).toHaveBeenCalledWith({
      method: "models.authStatus",
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/carapace/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/carapace/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/carapace/workspace");
  ...
  it("--force fails before login when purge cannot update the profile store", async () => {
    ...
    await expect(
      ...
      'Could not clear cached profiles for "openai" before re-login: auth store is busy; close other Carapace commands using this state directory and retry',
    );
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai. Verify plugins via `carapace plugins list --json`.',
    );
  });
  ...
  it("opens the provider picker when a CLI backend has no provider auth method", async () => {
    ...
    expect(prompter.note).toHaveBeenCalledWith(
      'Provider "claude-cli" uses its own CLI login. Select a provider with an Carapace auth flow.',
      "Provider auth",
    );
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Carapace prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
  ...
  it("rejects OpenAI API keys pasted as OpenAI Codex token material", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like an OpenAI API key. Use carapace models auth paste-api-key --provider openai for API-key auth.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects token material pasted into the OpenAI Codex API-key command", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like token or OAuth material, not an OpenAI API key. Use carapace models auth paste-token --provider openai for token auth material.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "carapace agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/carapace/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/carapace/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileAfterLoginWithLock) as UpsertAuthProfileCall)
        .agentDir,
    ).toBe("/tmp/carapace/agents/coder");
