...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("carapace security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("carapace security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    ...
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("carapace status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("carapace logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("carapace node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("carapace status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("carapace gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`carapace devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("carapace devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("carapace devices list")}`),
  ];
}
