// Carapace command gate: prove inference before starting conversational setup.
...
/**
 * Start Carapace only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("Carapace --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  const oneShot = isOneShotRequest(opts);
  if (!oneShot && !hasInteractiveTty(opts)) {
    runtime.error("Carapace needs an interactive TTY. Use --message for one command.");
    runtime.exit(1);
    return;
  }
  ...
  if (oneShot) {
    ...
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ...
        error: `Carapace requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error(
        [`Carapace requires working inference: ${inference.error}`, guidance].join("\n"),
      );
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
  ...
  runtime.log("Carapace requires working inference. Starting guided AI setup…");
