...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (configForTest: unknown, agentId: string) =>
    `${process.env.CARAPACE_STATE_DIR ?? "/tmp/carapace-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (configForTest: unknown, agentId: string) =>
    `/tmp/carapace-workspaces/${agentId}`,
...
vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/carapace-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.CARAPACE_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<CarapaceConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as CarapaceConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as CarapaceConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as CarapaceConfig, "main"));
  }
  ...
  beforeAll(async () => {
    const authTestEnv = await setupAuthTestEnv("carapace-auth-");
  ...
  it("fails fast when a removed provider auth choice is passed to the interactive flow", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "openai-chatgpt-import" is no longer supported. Use "openai" instead, or run carapace onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("escapes removed provider auth choice guidance for terminal output", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "legacy\\u001b[31mchoice" is no longer supported. Use "modern\\nchoice" instead, or run carapace onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("guides external provider auth-choice replacements before the plugin is installed", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "modelstudio-api-key" is no longer supported. Use "qwen-api-key" instead, or run carapace onboard to choose interactively.',
      );
    } finally {
