...
describe("formatMigrationPreview", () => {
  ...
  it("renders migration warnings with a warning glyph", () => {
    ...
    expect(output).toContain(
      "⚠️  Some Codex plugins could not be migrated. Run `carapace migrate codex` after onboarding.",
    );
  });
...
describe("formatMigrationResult", () => {
  ...
  it("renders provider warnings absent from migration next steps", () => {
    ...
    const output = formatMigrationResult({
      ...
      warnings: [warning],
      nextSteps: ["Run carapace doctor after applying the migration."],
    })
      .map(stripAnsi)
      .join("\n");
    ...
    expect(output).toContain("• Run carapace doctor after applying the migration.");
  });
  ...
  it("renders warning-backed next steps with a warning glyph", () => {
    ...
    const output = formatMigrationResult({
      ...
      warnings: [warning],
      nextSteps: [warning, "Run carapace doctor after applying the migration."],
    })
      .map(stripAnsi)
      .join("\n");
