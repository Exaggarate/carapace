...
/** Prompt for configured channel sections to remove from carapace.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in carapace.json.",
          `Tip: \`${formatCliCommand("carapace channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as CarapaceConfig["channels"];
    } else {
