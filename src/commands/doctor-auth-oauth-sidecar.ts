    ...
    note(
      [
        ...
        ...(unreferencedSidecars.length > 0
          ...
          : []),
        `- ${formatCliCommand("carapace doctor --fix")} migrates active profiles back to inline OAuth credentials and removes only sidecar files it successfully migrated.`,
      ].join("\n"),
      "Auth profiles",
    );
  }
