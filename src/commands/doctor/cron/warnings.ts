...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("carapace automations list")} and ${formatCliCommand("carapace automations show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("carapace automations list")} and ${formatCliCommand("carapace automations show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.carapace/bin/ensure-whatsapp.sh` is not maintained by current Carapace and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("carapace channels status --probe")}, ${formatCliCommand("carapace doctor")}, and ${formatCliCommand("carapace gateway status")} for current health checks.`,
