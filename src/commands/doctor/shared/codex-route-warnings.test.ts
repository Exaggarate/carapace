...
const asLegacyConfig = (cfg: unknown): CarapaceConfig => cfg as CarapaceConfig;
...
function legacyRouteWarning(...routes: string[]): string {
  return [
    ...
    ...routes,
    "- Run `carapace doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
  ].join("\n");
}
...
function codexCompactionWarning(...details: string[]): string {
  return [
    "- Codex runtime uses native server-side compaction and ignores Carapace compaction summarizer overrides.",
    ...details,
  ].join("\n");
}
...
describe("collectCodexRouteWarnings", () => {
  ...
  it("ignores CARAPACE_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings(
      {
        agents: { defaults: { model: "openai-codex/gpt-5.5" } },
      },
      { env: { CARAPACE_AGENT_RUNTIME: "codex" } },
    );
  ...
  it("warns without executing a custom Codex app-server command", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.carapace\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.carapace\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
      ...
      [
        ...
        "- Remove the override to use managed Codex startup, or verify the custom binary matches the Codex version bundled with this Carapace release.",
      ].join("\n"),
    ]);
  });
  ...
  it("preserves and reports authored params across effective Codex sources", () => {
    ...
    expect(result.warnings.join("\n")).not.toContain("gpt-5.6-carapace.params.serviceTier");
  });
  ...
  it("warns when Codex runtime has Carapace compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      agents: {
        defaults: {
          model: "openai/gpt-5.5",
    ...
    expect(warnings).toStrictEqual([
      codexCompactionWarning(
        ...
        "- Run `carapace doctor --fix`: it removes unsupported Codex compaction overrides.",
      ),
    ]);
  });
  ...
  it("preserves Codex runtime policy for each migrated per-agent Lossless model", () => {
    const result = maybeRepairCodexRoutes({
      models: {
        providers: {
          openai: { baseUrl: "https://proxy.example.test/v1", agentRuntime: { id: "carapace" } },
        },
      },
      agents: {
        defaults: {
        ...
        list: [
          {
            ...
            model: "anthropic/claude-sonnet-4-6",
            models: {
              "anthropic/claude-sonnet-4-6": { agentRuntime: { id: "carapace" } },
            },
          },
        ],
      },
  ...
  it("does not broaden runtime policy from kept compaction-only refs", () => {
    ...
    expectAgentRuntime(result.cfg, "carapace");
  });
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the Carapace runtime", () => {
    const result = maybeRepairCodexRoutes({
      plugins: DISABLED_CODEX_PLUGIN_CONFIG,
      models: {
        providers: {
          openai: { agentRuntime: { id: "carapace" }, models: [] },
        },
      },
      agents: { defaults: { model: "gpt-5.5" } },
    });
    ...
    expectCodexPluginDisabled(result);
    expectAgentRuntime(result.cfg, "carapace", { modelId: "gpt-5.5" });
  });

  itKeepsCodexPluginDisabled(
    "keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the Carapace runtime",
    {
      agents: {
        defaults: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise Carapace/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      models: {
        providers: {
          openai: {
  ...
  it("preserves explicit listed-agent canonical refs when default legacy model repair adds Codex policy", () => {
    ...
    expectAgentRuntime(result.cfg, "codex", { modelId: "gpt-5.5" });
    expectAgentRuntime(result.cfg, "carapace", { modelId: "gpt-5.5", agentId: "worker" });
  });
  ...
  it("preserves inherited default legacy runtime pins for listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai-codex/gpt-5.4": { agentRuntime: { id: "carapace" } },
          },
        },
    ...
    expectAgentRuntime(cfg, "carapace", { provider: "openai-codex", agentId: "worker" });
    ...
    expectAgentRuntime(result.cfg, "carapace", { agentId: "worker" });
  });
  ...
  it("preserves inherited default wildcard runtime pins for listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai-codex/*": { agentRuntime: { id: "carapace" } },
          },
        },
        list: [{ id: "worker", model: "openai-codex/gpt-5.4" }],
      },
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expectAgentRuntime(cfg, "carapace", { provider: "openai-codex" });
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "carapace"',
    );
    expectAgentRuntime(result.cfg, "carapace");
  });
  ...
  it("preserves legacy provider catalog runtime pins while repairing default legacy refs", () => {
    const cfg = {
      models: {
        providers: {
          "openai-codex": {
            models: [{ id: "gpt-5.4", agentRuntime: { id: "carapace" } }],
          },
        },
      },
      agents: { defaults: { model: "openai-codex/gpt-5.4" } },
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "carapace"',
    );
    expectAgentRuntime(result.cfg, "carapace", { agentId: "worker" });
  });
  ...
  it("shields listed canonical refs when provider-level legacy default pins migrate", () => {
    ...
    expectAgentRuntime(cfg, "carapace", { provider: "openai-codex", agentId: "main" });
    ...
    expectAgentRuntime(result.cfg, "carapace", { agentId: "main" });
  ...
  it("does not apply pre-existing canonical default runtime pins to listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai/*": { agentRuntime: { id: "carapace" } },
          },
        },
        list: [{ id: "worker", model: "openai-codex/gpt-5.4" }],
      },
  ...
  it("preserves explicit model-scoped runtime pins when repairing legacy model map keys", () => {
    const result = maybeRepairCodexRoutes({
      agents: {
        defaults: {
          models: {
            "openai-codex/gpt-5.5": { alias: "legacy-codex", agentRuntime: { id: "carapace" } },
          },
        },
      },
    });
    ...
    expect(result.cfg.agents?.defaults?.models).toEqual({
      "openai/gpt-5.5": { alias: "legacy-codex", agentRuntime: { id: "carapace" } },
    });
    expect(result.changes.join("\n")).not.toContain(
  ...
  it("preserves explicit Carapace runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves Codex runtime intent alongside explicit Carapace harness pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to Carapace", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(getSession(store, "main").updatedAt).toBe(1);
    expect(getSession(store, "main").agentHarnessId).toBe("carapace");
    expect(getSession(store, "main").agentRuntimeOverride).toBe("carapace");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "carapace" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });
