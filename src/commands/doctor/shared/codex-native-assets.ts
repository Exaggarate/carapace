...
function isCodexRuntimeConfigured(cfg: CarapaceConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: CarapaceConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: CarapaceConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- To review or promote them: install the Codex plugin (carapace plugins install npm:@carapace/codex), then run carapace migrate plan codex.",
    ].join("\n"),
  ];
}

if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("carapace.codexNativeAssetsTestApi")] = {
