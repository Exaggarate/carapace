...
function createFixture(config: Record<string, unknown>, stateEntries: string[] = []) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pristine-startup-"));
  roots.push(root);
...
function addConfiguredPlugin(
  ...
  const config = JSON.parse(fs.readFileSync(env.CARAPACE_CONFIG_PATH, "utf8")) as {
  ...
  fs.writeFileSync(env.CARAPACE_CONFIG_PATH, `${JSON.stringify(config)}\n`);
  return env;
}
...
describe("pristine startup state", () => {
  it("accepts a missing explicitly selected profile root", () => {
    const root = tempDirs.make("carapace-pristine-profile-");
    const stateDir = path.join(root, ".carapace-typo");
      ...
      hooks: {
        internal: {
          ...
          entries: {
            "session-memory": {
              enabled: true,
              env: { CARAPACE_HOOK_TEST: "enabled" },
              customOption: "value",
            },
          },
        },
