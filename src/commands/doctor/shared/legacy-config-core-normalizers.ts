...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as CarapaceConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as CarapaceConfig["channels"],
  };
}
...
>;
type ModelsConfigPatch = Partial<NonNullable<CarapaceConfig["models"]>>;
...
function normalizeLegacyCodexCliProviderRuntimePins(
  cfg: CarapaceConfig,
  changes: string[],
): { config: CarapaceConfig; changed: boolean } {
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<CarapaceConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as CarapaceConfig["agents"],
      }
    : cfgWithProviders;
  return nextCfg;
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<CarapaceConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as CarapaceConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<CarapaceConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
