...
const commandSecretState = vi.hoisted(() => ({
  ...
  resolvedConfig: undefined as CarapaceConfig | undefined,
...
vi.mock("../../../cli/command-secret-gateway.js", () => ({
  resolveCommandSecretRefsViaGateway: vi.fn(async (params: { config: CarapaceConfig }) => ({
...
vi.mock("./stale-plugin-config.js", () => ({
  scanStalePluginConfig: (cfg: {
    ...
    if (cfg.channels?.["carapace-weixin"]) {
      hits.push({ id: "carapace-weixin", surface: "channel" });
    }
    return hits.filter(
      (hit, index) =>
        hits.findIndex(
...
describe("doctor preview warnings", () => {
  ...
  it("reports merged gateway owner profiles without applying the repair", async () => {
    const env = { CARAPACE_STATE_DIR: "/tmp/carapace-doctor-preview" };
    const warning = 'Gateway owner profile is merged. Run "carapace doctor --fix" to repair it.';
  ...
  it("routes personal Codex asset notices to info instead of warnings", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-preview-codex-assets-"));
    tempRoots.add(root);
  ...
  it("includes stale plugin config warnings", async () => {
    ...
    expect(warning).toContain('Run "carapace doctor --fix"');
  ...
  it("includes stale channel config warnings without plugin config", async () => {
    ...
    expectSingleWarningContaining(warnings, "channels.carapace-weixin: dangling channel config");
  });

  it("includes bundled plugin load path migration warnings", async () => {
    const packageRoot = path.resolve("app-node-modules", "carapace");
  ...
  it("includes stale OAuth profile shadow warnings", async () => {
    staleOAuthShadowState.warnings = [
      '- ~/.carapace/agents/telegram/agent/auth-profiles.json has stale OAuth auth profile openai-codex:default. Run "carapace doctor --fix".',
    ];
  ...
  it("includes stale configured auth-order warnings", async () => {
    staleAuthOrderState.warnings = [
      "- auth.order.anthropic references only missing profiles while compatible stored credentials exist; run carapace doctor --fix to remove the stale override and restore automatic selection.",
    ];
  ...
  it("warns but skips auto-removal when plugin discovery has errors", async () => {
    ...
    expect(warning).toContain('rerun "carapace doctor --fix"');
  });
