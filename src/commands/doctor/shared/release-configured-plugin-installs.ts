...
function isPluginsGloballyDisabled(cfg: CarapaceConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: CarapaceConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: CarapaceConfig): string[] {
...
function isPluginEntryDisabled(cfg: CarapaceConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: CarapaceConfig, channelId: string): boolean {
  const channels = asNullableRecord(cfg.channels);
  const entry = asNullableRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: CarapaceConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: CarapaceConfig): string[] {
  const entries = asNullableRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: CarapaceConfig): string[] {
...
function collectConfiguredChannelIds(cfg: CarapaceConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: CarapaceConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: CarapaceConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: CarapaceConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: CarapaceConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: CarapaceConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: CarapaceConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: CarapaceConfig): string[] {
...
function addEligiblePluginId(cfg: CarapaceConfig, pluginIds: Set<string>, pluginId: string): void {
...
function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareCarapaceVersions(
  ...
  const touchedComparedToRelease = compareCarapaceVersions(params.touchedVersion, releaseVersion);
