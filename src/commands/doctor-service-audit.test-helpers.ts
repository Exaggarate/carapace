...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.CARAPACE_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.CARAPACE_GATEWAY_TOKEN);
}
