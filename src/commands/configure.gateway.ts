...
import { parseIpAddressOrCidr } from "@carapace/net-policy/ip";
...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "CARAPACE_GATEWAY_TOKEN",
          placeholder: "CARAPACE_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like CARAPACE_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Carapace will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Carapace trusts user identity from a reverse proxy.",
