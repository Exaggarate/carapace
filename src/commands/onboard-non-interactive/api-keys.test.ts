...
describe("resolveNonInteractiveApiKey", () => {
  it("resolves provider environment auth against the staged config and agent workspace", async () => {
    ...
    const workspaceDir = "/tmp/carapace-example-workspace";
  ...
  it.each([
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      testCase.resolvedEnv
        ? "Paste the API key value, not an Carapace onboarding command. Check ZAI_API_KEY."
  ...
  it("rejects a command-shaped explicit env key before a secret-ref flag", async () => {
    ...
    process.env.ZAI_API_KEY = "carapace onboard --non-interactive --auth-choice zai-api-key"; // pragma: allowlist secret
    resolveEnvApiKey.mockImplementation(() => {
  ...
  it.each([
    ...
    },
    {
      ...
      flagValue: "carapace onboard --non-interactive --auth-choice fixture-api-key",
    ...
    const result = await resolveNonInteractiveApiKey({
      ...
      envVar: testCase.envVar ?? "CARAPACE_ONBOARD_MISSING_FIXTURE_KEY",
