...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
    case "module-missing":
      return `Fix: run \`${formatCliCommand("carapace doctor --fix")}\`.`;
    case "service-missing":
      return `Fix: run \`${formatCliCommand("carapace gateway install --force")}\`.`;
    case "service-stopped":
      return `Fix: run \`${formatCliCommand("carapace gateway restart")}\`.`;
    case "startup-blocked":
      return `Fix: run \`${formatCliCommand("carapace gateway status --deep")}\`.`;
    case "not-listening":
      return `Fix: start \`${formatCliCommand("carapace gateway run")}\`, or run \`${formatCliCommand("carapace gateway restart")}\` for a managed gateway.`;
    default:
      return undefined;
  }
}
