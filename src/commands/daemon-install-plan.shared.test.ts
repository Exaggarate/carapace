  ...
  it("detects src ts entrypoints when devMode is not overridden", async () => {
    ...
    try {
      for (const [entrypoint, expected] of [
        ["/Users/me/carapace/src/cli/index.ts", true],
        ["C:\\Users\\me\\carapace\\src\\cli\\index.ts", true],
        ["/Users/me/carapace/dist/cli/index.js", false],
      ] as const) {
...
describe("resolveDaemonServicePathDirs carapace discovery", () => {
  it("uses the active carapace command directory", () => {
    expect(
      resolveDaemonServicePathDirs({
        argv: ["node", "/Users/testuser/.npm-global/bin/carapace", "gateway", "install"],
        env: { PATH: "" },
        platform: "darwin",
      }),
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-daemon-path-"));
      try {
        const binDir = path.join(root, "bin");
        const packageDir = path.join(root, "lib", "node_modules", "carapace");
        const entrypoint = path.join(packageDir, "carapace.mjs");
        fs.mkdirSync(binDir, { recursive: true });
        fs.mkdirSync(packageDir, { recursive: true });
        fs.writeFileSync(entrypoint, "");
        fs.symlinkSync(entrypoint, path.join(binDir, "carapace"));
  ...
  it.skipIf(process.platform === "win32")(
    "ignores unrelated carapace commands elsewhere on PATH",
    () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-daemon-path-"));
      try {
        const binDir = path.join(root, "bin");
        const activeEntrypoint = path.join(root, "active", "carapace.mjs");
        const otherEntrypoint = path.join(root, "other", "carapace.mjs");
        fs.mkdirSync(binDir, { recursive: true });
        ...
        fs.symlinkSync(otherEntrypoint, path.join(binDir, "carapace"));
...
describe("resolveDaemonServicePathDirs", () => {
  it("combines runtime and active carapace command directories", () => {
    expect(
      resolveDaemonServicePathDirs({
