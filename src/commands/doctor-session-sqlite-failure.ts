...
export function writeSessionSqliteMigrationFailureReports(
  ...
  const payload = {
    ...
    recoveryCommand: "carapace doctor --session-sqlite recover --github-issue",
...
export function createSessionSqliteMigrationFailureIssue(
  ...
  const body = [
    "Carapace doctor generated this sanitized report from a local session SQLite migration recovery.",
    "",
    reportBody,
  ].join("\n");
...
function renderFailureMarkdown(payload: {
  ...
  const lines = [
    ...
    `- Carapace version: ${payload.version}`,
