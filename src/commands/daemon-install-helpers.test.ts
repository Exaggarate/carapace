...
const mocks = vi.hoisted(() => ({
  ...
  resolveCarapaceWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveCarapaceWrapperPath: mocks.resolveCarapaceWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { CARAPACE_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveCarapaceWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { CARAPACE_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  // Prevent tests from reading the developer's real ~/.carapace/.env when
  ...
  it("uses provided runtimePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ CARAPACE_PORT: "3000" });
  ...
  it("resolves and forwards Bun for a Bun Gateway install plan", async () => {
    ...
    mocks.resolveGatewayProgramArguments.mockResolvedValue({
      programArguments: [bunPath, "/opt/carapace/dist/index.js", "gateway"],
    });
  ...
  it("adds the active carapace command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const carapaceBinPath = path.join(isolatedHome, ".npm-global", "bin", "carapace");
    process.argv = ["node", carapaceBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(carapaceBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".carapace"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes CARAPACE_WRAPPER through program args and managed service env", async () => {
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.CARAPACE_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.CARAPACE_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".carapace", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        CARAPACE_PORT: "3000",
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        CARAPACE_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.CARAPACE_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring CARAPACE_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.CARAPACE_PORT).toBe("3000");
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("keeps first-install provider API keys file-backed without capturing unrelated credentials", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  ...
  it("keeps durable provider API keys authoritative over first-install shell credentials", async () => {
    await writeStateDirDotEnv("OPENAI_API_KEY=durable-openai\n", {
      stateDir: path.join(isolatedHome, ".carapace"),
    });
    mockNodeGatewayPlanFixture({ serviceEnvironment: { CARAPACE_PORT: "3000" } });
    mocks.hasAnyAuthProfileStoreSource.mockReturnValue(false);
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it.each(["linux", "win32"] as const)(
    ...
    async (platform) => {
      ...
      const env = isolatedPlanEnv({
        ...
        BASH_ENV: "/tmp/carapace-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/carapace-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        CARAPACE_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
    ...
    expect(plan.environment.CARAPACE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe("CARAPACE_GATEWAY_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".carapace"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains config SecretRef env values for macOS LaunchAgent env files", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe("TELEGRAM_DEFAULT_BOTTOKEN");
  });
  ...
  it("retains existing generated env-file SecretRef values for macOS LaunchAgent regeneration", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        CARAPACE_SERVICE_MANAGED_ENV_KEYS:
  ...
  it("retains active gateway auth and channel SecretRefs through real LaunchAgent regeneration", async () => {
    ...
    const existing = await readGeneratedLaunchAgentFixture({
      root: tmpDir,
      environment: {
        CARAPACE_GATEWAY_AUTH_TOKEN: "gateway-existing-env-file-token",
        OLD_GATEWAY_AUTH_TOKEN: "stale-gateway-token",
        RETIRED_BOTTOKEN: "retired-channel-token",
        TELEGRAM_DEFAULT_BOTTOKEN: "telegram-existing-env-file-token",
        CARAPACE_SERVICE_MANAGED_ENV_KEYS:
          "OLD_GATEWAY_AUTH_TOKEN,CARAPACE_GATEWAY_AUTH_TOKEN,RETIRED_BOTTOKEN,TELEGRAM_DEFAULT_BOTTOKEN",
      },
    });
        ...
        gateway: {
          auth: {
            ...
            token: {
              source: "env",
              provider: "default",
              id: "CARAPACE_GATEWAY_AUTH_TOKEN",
            },
          },
        },
        channels: {
    ...
    expect(plan.environment.CARAPACE_GATEWAY_AUTH_TOKEN).toBe("gateway-existing-env-file-token");
    expect(plan.environment.TELEGRAM_DEFAULT_BOTTOKEN).toBe("telegram-existing-env-file-token");
    expect(plan.environmentValueSources?.CARAPACE_GATEWAY_AUTH_TOKEN).toBe("file");
    expect(plan.environmentValueSources?.TELEGRAM_DEFAULT_BOTTOKEN).toBe("file");
    expect(plan.environment.OLD_GATEWAY_AUTH_TOKEN).toBeUndefined();
    expect(plan.environment.RETIRED_BOTTOKEN).toBeUndefined();
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe(
      "CARAPACE_GATEWAY_AUTH_TOKEN,TELEGRAM_DEFAULT_BOTTOKEN",
    );
    ...
    await stageLaunchAgent({
      env: { HOME: home, CARAPACE_STATE_DIR: stateDir, CARAPACE_LAUNCHD_LABEL: label },
    ...
    expect(envFile).toContain(
      "export CARAPACE_GATEWAY_AUTH_TOKEN='gateway-existing-env-file-token'",
    );
    expect(envFile).toContain(
    ...
    expect(plist).not.toContain("CARAPACE_GATEWAY_AUTH_TOKEN");
    ...
    expect(rewritten?.environment?.CARAPACE_GATEWAY_AUTH_TOKEN).toBe(
    ...
    expect(rewritten?.environmentValueSources?.CARAPACE_GATEWAY_AUTH_TOKEN).toBe("file");
  });
  ...
  it.each([
    {
      ...
      surface: "token",
      mode: "token",
      configuredKey: "CARAPACE_GATEWAY_AUTH_TOKEN",
      existingKey: "CARAPACE_GATEWAY_AUTH_TOKEN",
      existingSource: "file",
    ...
    },
    {
      ...
      surface: "password",
      mode: "password",
      configuredKey: "CARAPACE_GATEWAY_AUTH_PASSWORD",
      existingKey: "CARAPACE_GATEWAY_AUTH_PASSWORD",
      existingSource: "file",
      expectedValue: "existing-secret",
    },
    {
    ...
    const existingEnvironment = existingKey
      ? {
          ...
          CARAPACE_SERVICE_MANAGED_ENV_KEYS: existingKey,
        }
      : undefined;
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment,
      existingEnvironmentValueSources,
      config: { gateway: { auth } } as unknown as CarapaceConfig,
    });
  ...
  it("lets the state-dir dotenv source replace an older file-backed gateway credential", async () => {
    await writeStateDirDotEnv("CARAPACE_GATEWAY_AUTH_TOKEN=authoritative-state-token\n", {
      stateDir: path.join(tmpDir, ".carapace"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      platform: "darwin",
      existingEnvironment: {
        CARAPACE_GATEWAY_AUTH_TOKEN: "stale-file-token",
        CARAPACE_SERVICE_MANAGED_ENV_KEYS: "CARAPACE_GATEWAY_AUTH_TOKEN",
      },
      existingEnvironmentValueSources: {
        CARAPACE_GATEWAY_AUTH_TOKEN: "file",
      },
      config: {
        gateway: {
          auth: {
    ...
    expect(plan.environment.CARAPACE_GATEWAY_AUTH_TOKEN).toBe("authoritative-state-token");
    expect(plan.environmentValueSources?.CARAPACE_GATEWAY_AUTH_TOKEN).toBe("inline");
  });
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        CARAPACE_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        CARAPACE_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        CARAPACE_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.CARAPACE_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps differently-cased source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.CARAPACE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.CARAPACE_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:carapace|carapace)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale CARAPACE_ALLOW_ROOT", async () => {
    const result = await buildEnvironment({ CARAPACE_ALLOW_ROOT: "1" });
    expect(result.CARAPACE_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves CARAPACE_CLI_CONTAINER_BYPASS and CARAPACE_CONTAINER_HINT", async () => {
    const result = await buildEnvironment({
      CARAPACE_CLI_CONTAINER_BYPASS: "1",
      CARAPACE_CONTAINER_HINT: "ci",
    });
    expect(result.CARAPACE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.CARAPACE_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary CARAPACE_FOO", async () => {
    const result = await buildEnvironment({ CARAPACE_FOO: "bar" });
    expect(result.CARAPACE_FOO).toBeUndefined();
  });

  it("drops legacy install-time version metadata from canonical rewrites", async () => {
    const result = await buildEnvironment({ CARAPACE_SERVICE_VERSION: "2026.4.24" });
    expect(result.CARAPACE_SERVICE_VERSION).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated CARAPACE_* keys", async () => {
    const result = await buildEnvironment({
      CARAPACE_CLI_CONTAINER_BYPASS: "1",
      CARAPACE_CONTAINER_HINT: "ci",
      CARAPACE_BAZ: "qux",
    ...
    expect(result.CARAPACE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.CARAPACE_CONTAINER_HINT).toBe("ci");
    expect(result.CARAPACE_BAZ).toBeUndefined();
  });
});
