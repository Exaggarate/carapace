...
import { resolvePrimaryStringValue } from "@carapace/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.CARAPACE_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: CarapaceConfig): {
  ...
  const parsed = CarapaceSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteImplicitFallbackClobberWarnings(cfg: CarapaceConfig): void {
...
export function noteSandboxOriginProxyWarning(cfg: CarapaceConfig): void {
...
export function noteMcpOriginWarning(cfg: CarapaceConfig): void {
