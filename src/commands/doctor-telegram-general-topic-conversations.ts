...
function resolveRepairScopes(cfg: CarapaceConfig, env: NodeJS.ProcessEnv) {
...
function repairLegacyRow(database: CarapaceAgentDatabase, legacyConversationId: string): boolean {
...
export async function repairTelegramGeneralTopicConversations(params: {
  ...
  for (const { scope } of resolveRepairScopes(params.cfg, env)) {
    await runExclusiveSqliteSessionWrite(scope, async () => {
      repaired += runCarapaceAgentWriteTransaction(
        (database) => {
