...
export async function migrateListCommand(runtime: RuntimeEnv, opts: { json?: boolean } = {}) {
  ...
  if (providers.length === 0) {
    runtime.log(
      `No migration providers found. Run ${formatCliCommand("carapace plugins list")} to verify provider plugins are installed and enabled.`,
    );
    return;
  }
  runtime.log(
...
export async function migratePlanCommand(
  ...
  if (!providerId) {
    throw new Error(
      `Migration provider is required. Run ${formatCliCommand("carapace migrate list")} to choose one.`,
    );
  }
...
export async function migrateApplyCommand(
  ...
  if (!opts.yes && !process.stdin.isTTY) {
    throw new Error(
      `carapace migrate apply requires --yes in non-interactive mode. Preview first with ${formatCliCommand(`carapace migrate plan '${providerId.replaceAll("'", "'\\''")}'`)}.`,
    );
  }
