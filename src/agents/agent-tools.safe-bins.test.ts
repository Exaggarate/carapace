...
beforeAll(async () => {
  await withEnvAsync(
    {
      CARAPACE_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "carapace-test-no-bundled-extensions"),
    },
    async () => {
...
vi.mock("carapace/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        CARAPACE_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createCarapaceCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "carapace-safe-bins-grep-",
        safeBins: ["grep"],
