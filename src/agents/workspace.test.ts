...
beforeEach(async () => {
  ...
  testState = await createCarapaceTestState({
    layout: "state-only",
    prefix: "carapace-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CARAPACE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CARAPACE_HOME: "/srv/carapace-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/carapace-home"), ".carapace", "workspace"));
  });
  ...
  it("roots named profile workspaces inside the profile state directory", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/carapace-home"), ".carapace-work", "workspace"));
  });

  it("rejects invalid environment-only profile names", () => {
    expect(() =>
      resolveDefaultAgentWorkspaceDir({
        CARAPACE_PROFILE: "../escape",
        HOME: "/home/peter",
      } as NodeJS.ProcessEnv),
    ).toThrow('Invalid profile name: "../escape"');
  });

  it("prefers CARAPACE_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
    ...
    expect(dir).toBe(path.resolve("/srv/carapace-workspace"));
  });
});
...
describe("ensureAgentWorkspace", () => {
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not carapace state\n";
  ...
  it("requires Doctor before using legacy JSON setup state", async () => {
    ...
    ).rejects.toThrow(/run carapace doctor --fix/u);
    await expect(
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".carapace"), { recursive: true });
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as unreadable", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-workspace-hardlink-"));
    try {
