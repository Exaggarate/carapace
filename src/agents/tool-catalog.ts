...
type CoreToolDefinition = {
  ...
  includeInCarapaceGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  {
    id: "gateway",
    description: "Read Gateway config/schema; owner-only Carapace self-update",
    sectionId: "automation",
    profiles: [],
    includeInCarapaceGroup: true,
  },
...
function buildCoreToolGroupMap() {
  ...
  const carapaceTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInCarapaceGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:carapace": carapaceTools,
