 ...
 */
export async function readLiveTestConfig(): Promise<CarapaceConfig> {
...
export function isLiveTestEnabled(
  ...
  return [...extraEnvVars, "LIVE", "CARAPACE_LIVE_TEST"].some((name) =>
...
export function isLiveProfileKeyModeEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.CARAPACE_LIVE_REQUIRE_PROFILE_KEYS);
}
