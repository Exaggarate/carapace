...
describe("buildAuthProfileUnusableHint", () => {
  it.each(["auth", "auth_permanent"] as const)(
    ...
    (reason) => {
      expect(
        ...
        "Re-authenticate with `carapace models auth login --provider openai --profile-id 'openai:default'`.",
      );
    },
  );
  ...
  it("keeps Claude subscription and Anthropic API-key recovery distinct", () => {
    expect(
      ...
      "claude auth login && carapace models auth login --provider anthropic --method cli --profile-id 'anthropic:claude-cli'",
    );
    expect(
      buildAuthProfileUnusableHint({
        kind: "cooldown",
    ...
    ).toContain("carapace models auth login --provider anthropic --profile-id 'anthropic:api-key'");
    expect(
      buildAuthProfileUnusableHint({
        kind: "cooldown",
        reason: "auth",
  ...
  it("routes legacy Gemini CLI profiles to supported Google API-key setup", () => {
    ...
    expect(hint).toBe(
      "Gemini CLI OAuth cannot be repaired by Carapace. Connect Google with an AI Studio API key using `carapace models auth login --provider google`, then select that Google profile for the Gemini CLI runtime.",
    );
...
describe("oauth refresh failure hints", () => {
  ...
  it("includes the profile id in refresh-failure login hints when known", () => {
    ...
    ).toBe("carapace models auth login --provider openai --profile-id 'Work Profile'");
  });
  ...
  it("renders login commands containing backticks as valid Markdown code spans", () => {
    ...
    expect(formatOAuthRefreshFailureLoginCommandMarkdown(command)).toBe(
      "``carapace models auth login --provider openai --profile-id 'openai:work`slot'``",
    );
  });
    ...
    await withLoopbackServer(
      ...
      async (baseUrl) => {
        ...
        expect(error?.message).toContain(
          "Re-auth with `claude auth login && carapace models auth login --provider anthropic --method cli`",
        );
        console.log(
