...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = tempDirs.make("carapace-tilde-test-edit-", osHome());
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = tempDirs.make("carapace-tilde-test-write-", osHome());
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = tempDirs.make("carapace-tilde-test-mkdir-", osHome());
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const dir = tempDirs.make("carapace-tilde-test-win32-", osHome());
  ...
  it("ignores CARAPACE_HOME for write operations", async () => {
    const carapaceHome = tempDirs.make("carapace-home-override-", os.tmpdir());
    const dir = tempDirs.make("carapace-tilde-test-write-", osHome());
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ CARAPACE_HOME: carapaceHome }, async () => {
      createHostWorkspaceWriteTool(carapaceHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(carapaceHome, path.basename(testFile))));
    });
  });

  it("ignores CARAPACE_HOME for mkdir operations", async () => {
    const carapaceHome = tempDirs.make("carapace-home-override-", os.tmpdir());
    const dir = tempDirs.make("carapace-tilde-test-mkdir-", osHome());
    ...
    await withEnvAsync({ CARAPACE_HOME: carapaceHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(carapaceHome, path.basename(newDir))));
    });
  });

  it("ignores CARAPACE_HOME for readFile operations", async () => {
    const carapaceHome = tempDirs.make("carapace-home-override-", os.tmpdir());
    const dir = tempDirs.make("carapace-tilde-test-edit-", osHome());
    ...
    await withEnvAsync({ CARAPACE_HOME: carapaceHome }, async () => {
      createHostWorkspaceEditTool(carapaceHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores CARAPACE_HOME for access operations", async () => {
    const carapaceHome = tempDirs.make("carapace-home-override-", os.tmpdir());
    const dir = tempDirs.make("carapace-tilde-test-edit-", osHome());
...
describe("createHostWorkspaceEditTool host access mapping", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-edit-access-test-"));
      try {
