...
let configOverride: CarapaceConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: CarapaceConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/Exaggarate/carapace/issues/31298
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // CARAPACE_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Carapace runtime context (internal):");
