...
const announceProvenance = {
  kind: "inter_session",
  sourceSessionKey: "agent:carapace:subagent:child",
...
const subagentCompletionEvent: AgentInternalEvent = {
  ...
  source: "subagent",
  childSessionKey: "agent:carapace:subagent:child",
  ...
  it("keeps malformed completion announces in the broad deny class", () => {
    expect(
      isSubagentAnnounceCompletionHandoff({
        inputProvenance: announceProvenance,
        internalEvents: [
          { ...subagentCompletionEvent, childSessionKey: "agent:carapace:subagent:forged" },
        ],
      }),
    ).toBe(true);
    expect(
