...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-subagent-"));
  ...
  it("reuses the persisted registry cache on hot internal read snapshots", async () => {
    ...
    const previousFlag = process.env.CARAPACE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
    let cloneSpy: { mockRestore(): void } | undefined;
    try {
      process.env.CARAPACE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = "1";
      ...
      if (previousFlag === undefined) {
        delete process.env.CARAPACE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
      } else {
        process.env.CARAPACE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = previousFlag;
      }
    }
  });
  ...
  it("prefers active runs and can resolve them from persisted registry snapshots", async () => {
    ...
    const resolved = withEnv({ CARAPACE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE: "1" }, () =>
