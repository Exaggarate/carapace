...
export interface ExtensionUIContext {
   ...
   * extend `CustomEditor` from `carapace/plugin-sdk/agent-sessions` and call
   * `super.handleInput(data)` for keys you don't handle.
   *
   * @example
   * ```ts
   * import { CustomEditor } from "carapace/plugin-sdk/agent-sessions";
   *
...
export interface ExtensionContext {
  ...
  /** Gracefully shut down Carapace and exit. Available in all contexts. */
  shutdown(): void;
...
interface ProviderModelConfig {
  ...
  reasoning: boolean;
  /** Maps Carapace thinking levels to provider/model-specific values; null marks a level unsupported. */
