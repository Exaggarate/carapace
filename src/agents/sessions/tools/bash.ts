...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("carapace.bashToolTestApi")] = {
    resolveBashTimeoutMs,
  };
}

/**
 * Create bash operations using Carapace runtime's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want Carapace runtime's
...
export function createBashToolDefinition(
  ...
  return {
    ...
    async execute(
      ...
      const output = new OutputAccumulator({ tempFilePrefix: "carapace-bash" });
