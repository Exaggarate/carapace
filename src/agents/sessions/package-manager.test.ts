...
describe("DefaultPackageManager", () => {
  it("keeps manifest resource entries inside the package root", async () => {
    ...
    const root = tempDirs.make("carapace-package-manager-");
    ...
    await writeFile(
      ...
      JSON.stringify({ carapace: { skills: entries } }),
      "utf-8",
    );
  ...
  it("expands manifest resource globs without hidden paths", async () => {
    ...
    await writeFile(
      join(packageRoot, "package.json"),
      JSON.stringify({ carapace: { skills: ["skills/*"] } }),
      "utf-8",
    );
  ...
  it("loads home-scoped personal skills only for the default state directory", async () => {
    const root = tempDirs.make("carapace-package-manager-personal-");
    ...
    const resolveSkillPaths = async (stateDir: string) =>
      await withEnvAsync(
        { HOME: home, CARAPACE_HOME: undefined, CARAPACE_STATE_DIR: stateDir },
        async () => {
          const manager = new DefaultPackageManager({
            cwd: workspace,
    ...
    expect(await resolveSkillPaths(join(home, ".carapace"))).toContain(personalSkill);
  ...
  it("keeps auto-discovered project resources inside their resource roots", async () => {
    ...
    const root = tempDirs.make("carapace-package-manager-");
    const configRoot = join(root, ".carapace");
    const outsideRoot = join(root, "outside");
  ...
  it("does not auto-install missing npm package resources", async () => {
    ...
    const manager = new DefaultPackageManager({
      cwd: root,
      agentDir: join(root, "agent"),
      settingsManager: SettingsManager.inMemory({ packages: ["npm:@carapace/missing-test"] }),
    });
  ...
  it("honors filters on direct local extension files", async () => {
    const root = tempDirs.make("carapace-package-manager-filter-");
  ...
  it("treats object local directories without filters like string sources", async () => {
    const root = tempDirs.make("carapace-package-manager-object-");
  ...
  it.each([
    ...
    ["npm", "npm:@carapace/missing-test"],
    ["git", "https://github.com/Exaggarate/carapace/missing-test.git"],
  ])("reports missing %s package sources through the owner callback", async (_kind, source) => {
    const root = tempDirs.make("carapace-package-manager-missing-");
  ...
  it("keeps temporary package paths in a private per-agent directory", async () => {
    const root = tempDirs.make("carapace-package-manager-temp-");
    ...
    const npmSource = manager.parseSource("npm:@carapace/example");
    const gitSource = manager.parseSource("https://github.com/Exaggarate/carapace/example.git");
