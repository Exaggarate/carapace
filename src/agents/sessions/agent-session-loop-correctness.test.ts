...
describe("AgentSession loop correctness", () => {
      ...
      try {
        ...
        for (const message of originalMessages) {
          const identitySymbol = Object.getOwnPropertySymbols(message).find(
            (symbol) => symbol === Symbol.for("carapace.steeringMessageIdentity"),
          );
  ...
  it("does not append when a compaction extension rejects the finalized summary", async () => {
    const dir = tempDirs.make("carapace-rejected-compaction-");
    const target = {
      agentId: "main",
  ...
  it.each([
    ...
    const pluginDir = tempDirs.make("carapace-terminate-plugin-");
