...
const EXEC_SHELL_SNAPSHOT_ENV = "CARAPACE_EXEC_SHELL_SNAPSHOT";
...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-disabled-state-"));
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-disabled-home-"));
    ...
    expect(wrapped).toBe(command);
    expect(fs.existsSync(resolveShellSnapshotDirForTest({ CARAPACE_STATE_DIR: stateDir }))).toBe(
      false,
    );
  });
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const trustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "carapace-snapshot-trusted-state-"),
    );
    const untrustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "carapace-snapshot-untrusted-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-state-home-"));
    const untrustedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "carapace-snapshot-untrusted-home-"),
    );
    ...
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      ...
      env: {
        ...
        CARAPACE_STATE_DIR: untrustedStateDir,
    ...
    expect(
      fs.existsSync(resolveShellSnapshotDirForTest({ CARAPACE_STATE_DIR: untrustedStateDir })),
    ).toBe(false);
    expect(
      fs.existsSync(resolveShellSnapshotDirForTest({ CARAPACE_STATE_DIR: trustedStateDir })),
    ).toBe(true);
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-cwd-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '[ "$CARAPACE_SHELL" = exec ] && alias oc_snap_exec_alias="printf marker-ok"',
  ...
  it("captures bash aliases behind common interactive-only guards", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-interactive-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-interactive-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-interactive-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("preserves per-call safe env overrides after trusted capture", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("treats a blank trusted HOME as unset when resolving the snapshot home", async () => {
    ...
    const homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-blank-home-"));
    const home = path.join(homeRoot, " trusted home ");
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-blank-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-blank-cwd-"));
    fs.mkdirSync(home);
  ...
  it("uses the OS account home when trusted home variables are blank", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-account-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-account-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-account-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setTestEnvValue("CARAPACE_STATE_DIR", stateDir);
    setTestEnvValue("HOME", "");
    ...
    const shellArgs = getPosixShellArgs(bash);
    const env = { ...process.env, HOME: home, CARAPACE_STATE_DIR: stateDir };
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      command: 'printf "%s" "$PNPM_HOME"',
      shell: bash,
      shellArgs,
  ...
  it("preserves per-call env outside the snapshot allowlist", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-plugin-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-plugin-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-plugin-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("does not let non-fingerprinted env change captured shell state", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-branch-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-branch-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-branch-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("refreshes stale snapshot files when startup files source alias fragments", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-refresh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-refresh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-refresh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refreshes fresh cached snapshots that no longer parse", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-corrupt-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-corrupt-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-corrupt-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    ...
    fs.writeFileSync(
      snapshotPath,
      [
        "# Carapace exec shell snapshot. Generated; do not edit.",
  ...
  it("refuses to persist aliases or functions with literal secret-looking values", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-secret-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-secret-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-secret-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("captures zsh aliases in sourceable form", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zsh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zsh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zsh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
  ...
  it("captures zsh startup state from ZDOTDIR", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zdot-home-"));
    const zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zdot-dir-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zdot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-snapshot-zdot-cwd-"));
