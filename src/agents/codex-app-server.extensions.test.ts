...
function createTempDir(): string {
  return createTempPluginDir(tempDirs, "carapace-codex-ext-");
}
...
describe("agent tool result middleware", () => {
  it("includes plugin-registered middleware and restores it from cache", async () => {
    ...
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = tmp;
  ...
  api.registerAgentToolResultMiddleware(async (event) => ({
    ...
    expect(listAgentToolResultMiddlewares("carapace")).toHaveLength(0);
  ...
  it("rejects middleware when the manifest omits the runtime contract", () => {
    ...
    writeTempPlugin({
      ...
      manifest: {
        contracts: {
          agentToolResultMiddleware: ["carapace"],
        },
      },
      body: `export default { id: "tool-result-middleware", register(api) {
  ...
  it("allows middleware from installed plugins when they declare the runtime contract", () => {
    ...
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
...
export default { id: "tool-result-middleware", register(api) {
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["carapace"] });
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["codex"] });
    ...
    expect(listAgentToolResultMiddlewares("carapace")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
  });
  ...
  api.registerAgentToolResultMiddleware(async (event) => ({
    ...
    resetActivePluginRegistryForTest();
    loadCarapacePlugins({ config, loadModules: false, onlyPluginIds: ["tool-result-middleware"] });
  ...
  config: {
    ...
    loadCarapacePlugins({ config, channelPluginLoadIntent: "setup" });
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(0);
    setRuntimeConfigSnapshot(config);
  ...
  api.registerAgentToolResultMiddleware(async (event) => ({
    ...
    loadCarapacePlugins({ config, onlyPluginIds: ["bundled-tool-result-middleware"] });
    setRuntimeConfigSnapshot(config);
