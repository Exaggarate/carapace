...
describe("sanitizeUserFacingText", () => {
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      "Visible intro.",
      "⚠️ 🛠️ `run carapace definitely-not-a-real-subcommand (agent)` failed",
      "⚠️ 🛠️ gh search issues --repo carapace/carapace --state open --no-search-pages.jsonl /tmp/carapace_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo carapace/carapace --state open (agent) failed: command timed out",
      "🛠️ run git status",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `Carapace runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "Carapace runtime context (internal): is the label used by the old runtime block formatter.";
