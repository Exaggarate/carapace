...
type PluginModelCatalogDatabase = Pick<CarapaceAgentKyselyDatabase, "cache_entries">;
...
function repairPersistedPluginModelCatalogs(params: {
  ...
  const applied = runCarapaceAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<PluginModelCatalogDatabase>(database.db);
...
/** Detects model catalogs generated by Carapace rather than user-authored JSON. */
