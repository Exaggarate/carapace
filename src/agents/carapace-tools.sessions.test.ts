...
import "./test-helpers/fast-carapace-tools-sessions.js";
...
function createCarapaceTools(options?: {
...
function getSessionTool(
  ...
  options?: Parameters<typeof createCarapaceTools>[0],
) {
  const tool = createCarapaceTools(options).find((candidate) => candidate.name === name);
  if (!tool) {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    const storePath = path.join(tempDirs.make("carapace-sessions-mailbox-"), "sessions.json");
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      const request = opts as { method?: string };
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "carapace", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "carapace", model: "delivery-mirror" }),
    );
  ...
  it("keeps scoped sends from creating post-return work or durable watches", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-scoped-session-send-"));
    const storePath = path.join(tmpDir, "sessions.json");
  ...
  it.each([
    ...
    async ({ timeoutSeconds, admitted }) => {
      const storePath = path.join(
        tempDirs.make("carapace-session-send-participant-"),
        "agents",
        "research",
        "agent",
  ...
  it("sessions_send never reroutes an exact-incarnation grant to a Cron parent", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-exact-cron-send-"));
    const storePath = path.join(tmpDir, "sessions.json");
    const requesterKey = "agent:main:main";
  ...
  it.each([true, false])(
    ...
    async (supportsTranscriptCommitWait) => {
      ...
      const runScopedCallerKey = "agent:leasing-ops:cron:monthly-utility:run:run-fast";
      const requesterKey = "agent:re-portal:main";
      const dir = tempDirs.make("carapace-sessions-steered-provenance-");
      const scope = {
        agentId: "leasing-ops",
