...
} from "./carapace-tools.subagents.sessions-spawn.test-harness.js";
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.CARAPACE_TEST_FAST;
...
describe("carapace-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.CARAPACE_TEST_FAST = fastModeEnv.previous;
  });
  ...
  it("sessions_spawn retires bundle MCP runtime when run-mode cleanup completes", async () => {
    ...
    await getOrCreateSessionMcpRuntime({
      ...
      workspaceDir: "/tmp/carapace-subagent-mcp-retire",
