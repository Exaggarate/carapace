...
describe.runIf(process.platform === "win32")("CarapaceStdioClientTransport on Windows", () => {
  it("applies configured environment overrides regardless of key case", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-mcp-env-case-"));
    ...
    let transport: CarapaceStdioClientTransport | undefined;
    ...
    try {
      const resolved = resolveStdioMcpServerLaunchConfig({
        ...
        args: [
          "-e",
          'require("node:fs").writeFileSync(process.env.CARAPACE_MCP_ENV_CAPTURE, process.env.TEMP)',
        ],
        env: {
          temp: configuredTemp,
          CARAPACE_MCP_ENV_CAPTURE: capturePath,
        },
      });
      ...
      transport = new CarapaceStdioClientTransport(resolved.config);
