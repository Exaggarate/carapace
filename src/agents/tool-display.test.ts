...
describe("tool display details", () => {
  ...
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await carapace.tools.describe('carapace:core:exec'); return await carapace.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "carapace:core:exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await carapace.tools.call("web_search", { query: "Carapace", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "Carapace",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "Carapace docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "Carapace docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the Carapace repository on GitHub", "carapace github", "carapace repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.carapace/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.carapace/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/carapace" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/carapace" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.carapace/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "Carapace docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "Carapace docs"');
  });
    ...
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          ...
          workdir: "/Users/example/.carapace/workspace",
        },
        detailMode: "explain",
      }),
    );
  ...
  it("keeps heredoc body separators out of top-level stage splitting", () => {
    const stages = splitTopLevelStages(
      [
        "mkdir -p .carapace/tmp/farm-notices",
        "cat > .carapace/tmp/farm-notices/ventura.txt <<'EOF'",
