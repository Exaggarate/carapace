import type { AgentEvent } from "carapace/plugin-sdk/agent-core";
...
describe("handleToolExecutionEnd cron mutation tracking", () => {
  ...
  it.each([
    ["exec", "carapace cron add --at +1h --message 'follow up' --name reminder"],
    ["exec", "npx carapace cron add --at=+1h --message 'follow up'"],
    ["exec", "bunx carapace cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm exec carapace cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx carapace cron add --at +1h --message 'follow up'"],
    ["exec", "npx -y carapace cron add --at +1h --message 'follow up'"],
    ["exec", "bunx --bun carapace cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx carapace@latest cron add --at +1h --message 'follow up'"],
    ["exec", "npx carapace@latest cron add --at +1h --message 'follow up'"],
    ["exec", "bunx carapace@latest cron add --at +1h --message 'follow up'"],
    ["exec", "/usr/local/bin/carapace cron add --at +1h --message 'follow up'"],
    ["bash", "corepack pnpm exec carapace cron add --at +1h --message 'follow up'"],
    ["exec", "env CARAPACE_PROFILE=test carapace cron add --at +1h --message 'follow up'"],
    ["exec", "carapace cron create --at +1h --message 'follow up'"],
    ["exec", "carapace --profile work cron create --at +1h --message 'follow up'"],
    ["exec", "carapace --dev cron add --at +1h --message 'follow up'"],
    ["exec", "carapace --log-level debug --no-color cron add --at +1h --message 'follow up'"],
    ["exec", "carapace --container helper cron add --at +1h --message 'follow up'"],
    ["exec", "carapace cron add --at +1h --message 'follow up || wait'"],
    ["exec", "carapace cron add --at +1h --message 'follow up' 2>&1"],
  ...
  it("does not increment successfulCronAdds when shell cron add fails", async () => {
    ...
    await executeTool(ctx, {
      ...
      args: {
        command: "carapace cron add --at +1h --message 'follow up' --name reminder",
      },
      isError: false,
      result: {
        details: {
  ...
  it.each([
    ["carapace cron list --json", "a different cron action"],
    ["echo carapace cron add --at +1h", "a command that only mentions cron add"],
    ["carapace cron add --at '+1h", "an unterminated shell argument"],
    ["cd /tmp && carapace cron add --at +1h", "a compound command"],
    ["carapace cron add --help", "the add command help"],
    ["carapace cron create -h", "the create alias help"],
    ["carapace cron add --bad||true", "a masked cron failure"],
    ["carapace cron add --at +1h; true", "a semicolon suffix"],
    ["carapace cron add --at +1h | cat", "a pipeline suffix"],
    ["carapace cron add --at +1h & true", "a background suffix"],
    ["carapace cron add --at +1h\ntrue", "a newline-separated suffix"],
    ["carapace cron add --bad # ignored\ntrue", "a comment-masked cron failure"],
    ["npx -y echo carapace cron add --at +1h", "a package runner for another executable"],
    ["pnpm carapace cron add --at +1h", "a bare pnpm package script"],
    ["corepack pnpm carapace cron add --at +1h", "a corepack pnpm package script"],
    ["carapace@latest cron add --at +1h", "a package spec without a package runner"],
    ["pnpm exec carapace@latest cron add --at +1h", "a package spec passed to pnpm exec"],
    ["carapace cron add --bad &>/tmp/cron.log", "a bash-only combined redirection"],
    ["carapace cron add --bad &>>/tmp/cron.log", "a bash-only append redirection"],
...
describe("handleToolExecutionEnd timeout metadata", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      args: { command: "git status", workdir: "/Users/agent/Projects/Carapace" },
...
describe("handleToolExecutionEnd exec approval prompts", () => {
  ...
  it("emits a deterministic unavailable payload when the initiating surface cannot approve", async () => {
    ...
    expect(text).toContain(
      "Inspect the node's effective exec policy with `carapace approvals get --node node-mac-1`.",
    );
    ...
    await executeTool(ctx, {
      ...
      result: {
        details: {
          ...
          view: { id: "cv_1", url: "/__carapace__/canvas/documents/cv_1/index.html" },
        },
      },
    });
...
describe("control UI credential redaction (issue #72283)", () => {
  ...
  it("redacts secrets in exec aggregated stdout before emitting command_output", async () => {
    ...
    await executeTool(ctx, {
      ...
      args: { command: "cat ~/.carapace/carapace.json" },
      isError: false,
      result: {
        details: {
          status: "completed",
