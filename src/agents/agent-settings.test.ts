...
describe("isSilentOverflowProneModel", () => {
  ...
  // carapace#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable Carapace runtime's secondary compaction path;
  // Carapace's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of carapace#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering Carapace runtime's
  ...
  // state.messages intact; Carapace's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Carapace runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Carapace runtime's
