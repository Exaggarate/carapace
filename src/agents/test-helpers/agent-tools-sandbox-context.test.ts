...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("carapace-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("carapace-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("carapace-sbx-");
