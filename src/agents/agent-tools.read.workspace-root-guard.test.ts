...
describe("wrapToolWorkspaceRootGuardWithOptions", () => {
  ...
  it("adds a workspace-safe temp hint when rejecting paths outside the workspace", async () => {
    ...
    await expect(
      ...
      /Path escapes sandbox root .* Use a relative path under `.carapace\/tmp\/` inside the workspace/,
    );
...
describe("createCarapaceReadTool malformed XML arg-value suffix handling", () => {
  it("strips the suffix from read paths before invoking the base tool", async () => {
    ...
    const tool = createCarapaceReadTool(base);
