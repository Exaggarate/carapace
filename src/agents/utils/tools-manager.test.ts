...
beforeEach(() => {
  originalAgentDir = process.env.CARAPACE_AGENT_DIR;
  tempAgentDir = mkdtempSync(join(tmpdir(), "carapace-tools-manager-"));
...
afterEach(() => {
  ...
  if (originalAgentDir === undefined) {
    ...
    setTestEnvValue("CARAPACE_AGENT_DIR", originalAgentDir);
  }
  if (tempAgentDir) {
...
describe("ensureTool", () => {
  ...
  it("treats trimmed on as the canonical offline opt-in", async () => {
    vi.stubEnv("CARAPACE_OFFLINE", " ON ");
    const { ensureTool } = await import("./tools-manager.js");
