...
import { createReadTool } from "carapace/plugin-sdk/agent-sessions";
...
describe("workspace path resolution", () => {
  it("uses cwd for coding filesystem tools while workspaceDir remains the agent workspace", async () => {
    await withTempDir("carapace-agent-ws-", async (workspaceDir) => {
      await withTempDir("carapace-task-cwd-", async (cwd) => {
        const tools = createCarapaceCodingTools({ workspaceDir, cwd });
  ...
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("carapace-ws-", async (workspaceDir) => {
      await withTempDir("carapace-cwd-", async (otherDir) => {
        ...
        try {
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "carapace" }],
          });
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      await withTempDir("carapace-windows-case-", async (workspaceDir) => {
        const cfg: CarapaceConfig = { tools: { fs: { workspaceOnly: true } } };
        const tools = createCarapaceCodingTools({ workspaceDir, config: cfg });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("carapace-ws-", async (workspaceDir) => {
      ...
      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-carapace.txt");
      await expect(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("carapace-ws-symlink-write-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("carapace-ws-symlink-edit-", async (workspaceDir) => {
        const realDir = path.join(workspaceDir, "oc_system", "memory");
        const aliasDir = path.join(workspaceDir, "memory");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("carapace-ws-symlink-escape-", async (rootDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("carapace-ws-symlink-leaf-", async (workspaceDir) => {
  ...
  it("allows workspaceOnly reads for resolved skill roots without allowing other filesystem access", async () => {
    await withTempDir("carapace-skill-read-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("rejects symlink escapes inside resolved skill roots", async () => {
    ...
    await withTempDir("carapace-skill-read-symlink-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
      const skillDir = path.join(rootDir, "global-skills", "demo");
      await fs.mkdir(workspaceDir, { recursive: true });
      await fs.mkdir(skillDir, { recursive: true });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("carapace-sandbox-", async (sandboxDir) => {
      await withTempDir("carapace-workspace-", async (workspaceDir) => {
        ...
        const tools = createCarapaceCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
...
function resolveApplyPatchTool(params: { sandbox: UnsafeMountedSandbox; config: CarapaceConfig }) {
  ...
  it("allows read-only materialized sandbox skills for sandbox reads only", async () => {
    await withUnsafeMountedSandboxHarness(
      async ({ sandbox, skillsWorkspaceDir }) => {
        ...
        const tools = createSandboxFsTools({ sandbox, workspaceOnly: true });
        const { readTool } = expectReadWriteEditTools(tools);
        const containerSkillPath = "/workspace/.carapace/sandbox-skills/skills/demo/SKILL.md";
        ...
        const relativeReadResult = await readTool?.execute("t2", {
          path: ".carapace/sandbox-skills/skills/demo/SKILL.md",
        });
        ...
        const fileUrlReadResult = await readTool?.execute("t3", {
          path: "file:///workspace/.carapace/sandbox-skills/skills/demo/SKILL.md",
        });
...
vi.mock("carapace/plugin-sdk/llm", async () => {
  const original =
    await vi.importActual<typeof import("carapace/plugin-sdk/llm")>("carapace/plugin-sdk/llm");
  return {
    ...original,
  };
});
...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createCarapaceReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-test-"));
