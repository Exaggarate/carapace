...
function expectWidget(tools: ReturnType<typeof createCarapaceTools>) {
...
describe("pinned show_widget registration", () => {
  ...
  it.each([undefined, "agent:main:cron:job:run:detached"])(
    ...
    (agentSessionKey) => {
      const tools = createCarapaceTools({ agentSessionKey, pinnedWidgetAuthoring: true });
  ...
  it("applies tool policy to recovered dashboard authoring", () => {
    ...
    expectPinnedOnlySchema(expectWidget(createCarapaceCodingTools(options)));
    const deniedTools = createCarapaceCodingTools({
      ...options,
