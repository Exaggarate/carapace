/** Regular-agent client for the Carapace system agent. */
...
const CarapaceDelegateSchema = Type.Object({
  message: Type.String({ description: "What system must do." }),
  sessionId: Type.Optional(Type.String({ description: "Continue prior Carapace talk." })),
});

const CarapaceDelegateOutputSchema = Type.Object(
  {
    reply: Type.String(),
...
export function createCarapaceDelegateToolsForRun(
  options: Pick<
    CarapaceToolsOptions,
    | "sandboxed"
    | "runSessionKey"
    | "agentSessionKey"
    | "agentChannel"
  ...
  const tool: AnyAgentTool = {
    ...
    parameters: CarapaceDelegateSchema,
    outputSchema: CarapaceDelegateOutputSchema,
    execute: async (_toolCallId, args, signal) => {
      ...
      const result = await withGatewayToolCallerIdentity(caller, () =>
        callInProcessGatewayTool<CarapaceDelegateResult>("carapace.chat", {
          sessionId,
          message,
          delegation: {
            agentId: options.sessionAgentId,
