...
import { MAX_TIMER_TIMEOUT_SECONDS } from "@carapace/normalization-core/number-coercion";
...
describe("web cache keys", () => {
  it("keeps case-sensitive request components distinct in cache keys", () => {
    const upper = normalizeCacheKey(" fetch:https://example.com/get?marker=CarapaceCase ");
    const lower = normalizeCacheKey("fetch:https://example.com/get?marker=carapacecase");

    expect(upper).toBe("fetch:https://example.com/get?marker=CarapaceCase");
    expect(lower).toBe("fetch:https://example.com/get?marker=carapacecase");
