...
function hasExplicitDefaultPrimaryModel(cfg?: CarapaceConfig): boolean {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("carapace.imageToolTestApi")] = {
    ...testing,
...
function pickMaxBytes(cfg?: CarapaceConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const providerCfg: CarapaceConfig = effectiveCfg ?? {};
  const preparedProviders =
    params.preparedModelRuntime?.mediaCapabilityProviders?.mediaUnderstandingProviders;
