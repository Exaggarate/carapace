...
import { resolveImageToolFactoryAvailable } from "../carapace-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeApiKey?: boolean }): CarapaceConfig {
...
function createBedrockSdkConfig(): CarapaceConfig {
  return {
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-image-auth-regression-"));
  try {
