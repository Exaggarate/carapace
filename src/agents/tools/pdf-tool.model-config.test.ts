...
const TEST_AGENT_DIR = "/tmp/carapace-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: CarapaceConfig }) => {
  ...
  resolveDefaultModelRef: (cfg?: CarapaceConfig) => {
