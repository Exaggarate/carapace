...
import { coerceErrorMessage as formatLiveError, expectDefined } from "@carapace/normalization-core";
...
const LIVE_IMAGE_TOOL_ENABLED = isLiveTestEnabled(["CARAPACE_LIVE_IMAGE_TOOL_TEST"]);
const LIVE_OPENAI_MODEL =
  process.env.CARAPACE_LIVE_IMAGE_TOOL_OPENAI_MODEL?.trim() ||
  process.env.CARAPACE_LIVE_IMAGE_TOOL_MODEL?.trim() ||
  "gpt-4.1-mini";
const LIVE_ANTHROPIC_MODEL =
  process.env.CARAPACE_LIVE_IMAGE_TOOL_ANTHROPIC_MODEL?.trim() || "claude-sonnet-4-6";
...
function createLiveConfig(testCase: LiveProviderCase): CarapaceConfig {
  return {
    agents: {
      defaults: {
...
async function withLiveWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-image-tool-live-"));
  try {
