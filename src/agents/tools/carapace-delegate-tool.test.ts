...
import { createCarapaceDelegateToolsForRun } from "./carapace-delegate-tool.js";
...
describe("carapace delegation tool", () => {
  it("relays context and the completed approval outcome", async () => {
    ...
    const tool = createCarapaceDelegateToolsForRun({
      sessionAgentId: "main",
    ...
    if (!tool) {
      throw new Error("expected Carapace delegation tool");
    }
    ...
    expect(callGateway).toHaveBeenCalledWith("carapace.chat", {
  ...
  it.each([
    ...
    name: string;
    options: Omit<Parameters<typeof createCarapaceDelegateToolsForRun>[0], "sessionAgentId">;
    full: boolean;
  ...
  it("uses the owner-only core gate", () => {
    expect(GATEWAY_OWNER_ONLY_CORE_TOOLS).toContain("carapace");
  });
});
