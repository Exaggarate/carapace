...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<CarapaceConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<CarapaceConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    const sourceProviders: NonNullable<NonNullable<CarapaceConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
