...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      carapaceReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__carapace_replay");
    expect(thinkingBlock.carapaceReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.CARAPACE_DEBUG_MODEL_PAYLOAD;
    process.env.CARAPACE_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.CARAPACE_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.CARAPACE_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.CARAPACE_DEBUG_MODEL_PAYLOAD;
    process.env.CARAPACE_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Carapace", () => {
  ...
  it("adds Carapace attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds Carapace attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("CARAPACE_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      makeResponsesModel({
        id: "gpt-5.4-codex",
  ...
  it("adds SSE Accept only to native ChatGPT/Codex Responses stream requests", () => {
    ...
    const transportAliasModel = {
      ...codexModel,
      api: "carapace-openai-chatgpt-responses-transport" as Api,
    } satisfies Model;
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("carapace-openai-responses-transport");
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("carapace-anthropic-messages-transport");
    expectRecordFields(prepared, {
