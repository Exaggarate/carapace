// Verifies Carapace-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "carapace/plugin-sdk/agent-core";
import {
  installCarapaceOwnedToolHooks,
  resetCarapaceOwnedToolHooks,
  textToolResult,
...
describe("Carapace-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetCarapaceOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installCarapaceOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installCarapaceOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/carapace-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/carapace-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/carapace-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installCarapaceOwnedToolHooks({ blockReason: "blocked by policy" });
