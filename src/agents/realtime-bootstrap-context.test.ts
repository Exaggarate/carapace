...
async function makeWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-realtime-bootstrap-"));
  tempDirs.push(dir);
  return dir;
}

function makeConfig(workspaceDir: string): CarapaceConfig {
...
describe("resolveRealtimeBootstrapContextInstructions", () => {
  it("formats the default profile bootstrap files without exposing local paths", async () => {
    ...
    expect(instructions).toContain("Carapace realtime voice profile context");
    ...
    expect(instructions).not.toContain("carapace_agent_consult");
