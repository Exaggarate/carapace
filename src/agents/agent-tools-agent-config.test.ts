...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: CarapaceConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<CarapaceConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<CarapaceConfig["agents"]>["list"]>[number]["tools"];
  }): CarapaceConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("keeps core tools for owner WebChat while restricting non-owners", () => {
    ...
    expect(ownerTools).toContain("carapace");
    ...
    expect(nonOwnerTools).not.toContain("carapace");
