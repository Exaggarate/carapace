...
vi.mock("../../infra/carapace-root.js", () => ({
  resolveCarapacePackageRootSync: () => mocks.packageRoot,
}));
...
describe("native hook relay executable resolution", () => {
  ...
  it("prefers the isolated packaged relay over the general CLI", () => {
    ...
    const generalCli = path.join(mocks.packageRoot, "carapace.mjs");
  ...
  it("preserves the explicit CLI path override", () => {
    const override = path.resolve("/custom/carapace-entry.mjs");
    vi.stubEnv("CARAPACE_CLI_PATH", override);
