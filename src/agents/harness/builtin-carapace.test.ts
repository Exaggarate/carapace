// Built-in Carapace harness tests cover logical thinking-mode boundaries.
...
describe("createCarapaceAgentHarness", () => {
  beforeEach(() => {
  ...
  it("brands only host-created instances as the built-in runtime", () => {
    expect(isBuiltInCarapaceAgentHarness(createCarapaceAgentHarness())).toBe(true);
    expect(
      isBuiltInCarapaceAgentHarness({
        id: "carapace",
        label: "forged",
  ...
  it("preserves logical Ultra for the embedded attempt", async () => {
    ...
    await createCarapaceAgentHarness().runAttempt(params);
  ...
  it("runs isolated completion through the prepared zero-tool transport", async () => {
    const params = {
      ...
      NonNullable<ReturnType<typeof createCarapaceAgentHarness>["runIsolatedCompletionV2"]>
    >[0];

    await expect(createCarapaceAgentHarness().runIsolatedCompletionV2?.(params)).resolves.toEqual({
  ...
  it("rejects harness-owned isolated authorization", async () => {
    ...
    await expect(createCarapaceAgentHarness().runIsolatedCompletionV2?.(params)).rejects.toThrow(
