...
type CreateCarapaceToolsOpts = Parameters<CreateSessionsSpawnTool>[0];
...
const sessionStorePath = path.join(
  os.tmpdir(),
  `carapace-sessions-spawn-test-store-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
);
...
export async function getSessionsSpawnTool(opts: CreateCarapaceToolsOpts) {
