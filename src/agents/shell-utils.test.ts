...
function createTempCommandDir(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-shell-"));
  tempDirs.push(dir);
...
describe("getShellConfig", () => {
  ...
  it("rejects a missing explicit custom shell path", () => {
    expect(() => getShellConfig(path.join(os.tmpdir(), "missing-carapace-shell"))).toThrow(
...
describe("getBashShellConfig", () => {
  ...
  it("finds Git Bash under ProgramFiles", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-git-bash-"));
  ...
  it("prepends coreutils for a standard Git for Windows install", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-git-bash-env-"));
    tempDirs.push(programFiles);
  ...
  it("recognizes portable Git for Windows installs", () => {
    const gitRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-portable-git-"));
  ...
  it("leaves unrelated MSYS2 installs unchanged", () => {
    const msysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-msys2-"));
...
describe("getBashShellEnv", () => {
  ...
  it("returns an env object with the Carapace bin dir on PATH", () => {
    process.env.PATH = "/usr/bin";
    const env = getBashShellEnv();

    expect(env.PATH).toContain("/usr/bin");
    expect(env.PATH).toContain(".carapace");
  });
...
describe("getShellConfig on Windows", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-bin-"));
  ...
  it("prefers a native pwsh.exe over earlier bare shims, batch wrappers, and PowerShell 5.1", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pfiles-"));
    const shimDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-shim-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-bin-"));
    const systemRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-sysroot-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-sysroot-"));
