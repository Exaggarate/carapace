...
describe("Carapace MCP HTTP lifecycle adapters", () => {
  it.each([
  ...
  it("rejects an oversized JSON message before the SDK parses it", async () => {
    ...
    const transport = new CarapaceStreamableHTTPClientTransport(new URL("http://mcp.invalid/mcp"), {
      fetch: fetchMock,
    });
  ...
  it("keeps a legacy SSE stream open beyond the cumulative message limit", async () => {
    ...
    const transport = new CarapaceSSEClientTransport(new URL("http://mcp.invalid/sse"), {
      fetch: fetchMock,
