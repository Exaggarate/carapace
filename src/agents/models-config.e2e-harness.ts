...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousCarapaceAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousCarapaceAgentDir = process.env.CARAPACE_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousCarapaceAgentDir === undefined) {
      delete process.env.CARAPACE_AGENT_DIR;
    } else {
      process.env.CARAPACE_AGENT_DIR = previousCarapaceAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const CUSTOM_PROXY_MODELS_CONFIG: CarapaceConfig = {
  models: {
    providers: {
      "custom-proxy": {
