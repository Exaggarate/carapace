...
import { isIncognitoCarapaceAgentSqlitePath } from "../state/carapace-agent-db.js";
...
export function resolveInternalSessionEffectsTarget(params: {
  ...
  const incognito = isIncognitoCarapaceAgentSqlitePath(params.storePath, {
...
export async function prepareInternalSessionEffectsSession(params: {
  ...
  const entry = await upsertSessionEntryCore(scope, {
    ...
    ...(isIncognitoCarapaceAgentSqlitePath(params.storePath, { agentId: params.agentId })
