...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const carapaceTmpDir = path.resolve(resolvePreferredCarapaceTmpDir());
  if (!isPathInside(carapaceTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: carapaceTmpDir });
  return resolved;
}
