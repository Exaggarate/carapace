// Verifies Carapace plugin tools are resolved with browser/runtime context.
...
const TEST_AGENT_DIR = path.join(os.tmpdir(), "carapace-plugin-tool-auth-test");
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from Carapace tool resolution.
...
describe("createCarapaceTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveCarapacePluginToolsForOptions({
      options: { config },
  ...
  it.each(["agent:main:telegram:group:123", undefined])("binds delivery for %s", async (key) => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-plugin-delivery-"));
    const mediaUrl = path.join(workspaceDir, "photo.png");
    const outsideMediaUrl = `${workspaceDir}-outside.png`;
    await fs.copyFile(
      path.join(
        process.cwd(),
        "apps/ios/WatchApp/Assets.xcassets/CarapaceIcon.imageset/carapace-icon.png",
      ),
      mediaUrl,
    );
  ...
  it("does not expose plugin delivery without a host turn capability", () => {
    ...
    resolveCarapacePluginToolsForOptions({
      ...
      resolvedConfig: {} as CarapaceConfig,
    });
  ...
  it("forwards the lifecycle registry to workspace-scoped plugin tools", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { enabled: true } } as CarapaceConfig;
  ...
  it("keeps provider availability and credential resolution aligned for env-only auth", async () => {
    const envName = "CARAPACE_PLUGIN_TOOL_AUTH_TEST_KEY";
  ...
  it("preserves ungated plugin resolution when no authoritative auth store is supplied", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { allow: ["browser"] } } as CarapaceConfig;
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: CarapaceConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: CarapaceConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it.each(["custom", "source-less", "absent"] as const)(
    ...
    (initialRuntime) => {
      const pinnedRuntimeConfig: CarapaceConfig = { plugins: { allow: ["old-plugin"] } };
      const explicitConfig: CarapaceConfig = {
        plugins: { allow: ["browser"] },
        tools: { updatePlan: true },
      };
      let capturedRuntimeConfig: CarapaceConfig | undefined;
      let getRuntimeConfig: (() => CarapaceConfig | undefined) | undefined;
      hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
        const context = (
          params as {
            context?: {
  ...
  it.each(["source", "runtime", "ambient"] as const)(
    ...
    (inputKind) => {
      ...
      const firstRuntimeConfig: CarapaceConfig = {
        ...sourceConfig,
        plugins: {
      ...
      const nextRuntimeConfig: CarapaceConfig = {
        ...firstRuntimeConfig,
        ...nextSourceConfig,
      };
      let getRuntimeConfig: (() => CarapaceConfig | undefined) | undefined;
      hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
        getRuntimeConfig = (
          params as { context?: { getRuntimeConfig?: () => CarapaceConfig | undefined } }
...
function requirePluginTool(name: string, overrides?: Parameters<typeof createCarapaceTools>[0]) {
  hoisted.resolvePluginTools.mockReturnValue([
    {
      name,
...
describe("createCarapaceTools Gateway caller identity", () => {
  afterEach(() => {
