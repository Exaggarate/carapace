...
describe("fallback-skip-cache", () => {
  ...
  it("does not enable the cache for a suffixed TTL value", () => {
    vi.stubEnv("CARAPACE_FALLBACK_SKIP_TTL_MS", "1000ms");
    markFallbackCandidateSkipped({
      sessionId: "s1",
      provider: "anthropic",
      model: "claude-opus-4-7",
  ...
  it("uses CARAPACE_FALLBACK_SKIP_TTL_MS as an opt-in default TTL", () => {
    vi.stubEnv("CARAPACE_FALLBACK_SKIP_TTL_MS", "60000");
    markFallbackCandidateSkipped({
      sessionId: "s1",
      provider: "anthropic",
      model: "claude-opus-4-7",
