...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "CARAPACE_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { CARAPACE_LIVE_MODEL_IMAGE_PROBE: "false" },
        "CARAPACE_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { CARAPACE_LIVE_MODEL_IMAGE_PROBE: "1" },
        "CARAPACE_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
