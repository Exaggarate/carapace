...
import { resolveCarapacePluginToolInputs } from "./carapace-tools.plugin-context.js";

describe("carapace plugin tool context", () => {
  it("forwards trusted requester sender identity", () => {
    const result = resolveCarapacePluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("defaults missing and unknown conversation-read origins to delegated", () => {
    const missing = resolveCarapacePluginToolInputs({
      options: { config: {} as never },
    });
    const unknown = resolveCarapacePluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("forwards host-prepared active project keys", () => {
    const activeProjectKeys = ["github.com/Carapace/Carapace"];
    const result = resolveCarapacePluginToolInputs({
