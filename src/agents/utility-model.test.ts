...
describe("readUtilityModelSetting", () => {
  it("distinguishes unset, explicit, and empty-string disable", () => {
    expect(readUtilityModelSetting({} as CarapaceConfig, "main")).toEqual({ kind: "auto" });
    expect(
      readUtilityModelSetting(
        { agents: { defaults: { utilityModel: " openai/gpt-5.4-mini " } } } as CarapaceConfig,
        "main",
      ),
    ).toEqual({ kind: "explicit", modelRef: "openai/gpt-5.4-mini" });
    expect(
      readUtilityModelSetting(
        { agents: { defaults: { utilityModel: "" } } } as CarapaceConfig,
        "main",
      ),
...
describe("resolveUtilityModelRefForAgent", () => {
  ...
  it("returns undefined when utility routing is disabled", () => {
    const cfg = { agents: { defaults: { utilityModel: "" } } } as CarapaceConfig;
