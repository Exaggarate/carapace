// Verifies createCarapaceTools wires shared config and context into the TTS tool.
...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/carapace/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./carapace-tools.nodes-workspace-guard.js", () => ({
...
describe("createCarapaceTools TTS config wiring", () => {
  beforeEach(() => {
...
describe("createCarapaceTools transcript ownership wiring", () => {
  beforeEach(() => {
    mocks.createTranscriptsToolOptions.mockClear();
  });

  it("uses trusted caller authority instead of the delivery account", () => {
    const injectedConfig = { transcripts: { enabled: true } } satisfies CarapaceConfig;
...
describe("createCarapaceTools media generation session wiring", () => {
  beforeEach(() => {
    mocks.createImageGenerateToolOptions.mockClear();
    mocks.createMusicGenerateToolOptions.mockClear();
    mocks.createVideoGenerateToolOptions.mockClear();
...
describe("createCarapaceTools session status route context wiring", () => {
  beforeEach(() => {
...
describe("createCarapaceTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });
