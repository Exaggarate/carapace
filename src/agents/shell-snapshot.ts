...
const CAPTURE_MARKER = "__CARAPACE_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__CARAPACE_SHELL_SNAPSHOT_ENV__";
  ...
  return await withTempWorkspace(
    {
      ...
      prefix: "carapace-shell-snapshot-",
      dirMode: 0o700,
      mode: 0o600,
    },
    async (workspace) => {
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // CARAPACE_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.CARAPACE_SHELL === "exec") {
    env.CARAPACE_SHELL = "exec";
  }
  return env;
}
