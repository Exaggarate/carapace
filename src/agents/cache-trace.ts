...
import { sanitizeSurrogates } from "@carapace/ai/internal/shared";
...
function resolveCacheTraceConfig(params: CacheTraceInit): CacheTraceConfig {
  ...
  const envEnabled = parseBooleanValue(env.CARAPACE_CACHE_TRACE);
  const enabled = envEnabled ?? config?.enabled ?? false;
  const fileOverride = env.CARAPACE_CACHE_TRACE_FILE?.trim();
  const filePath = fileOverride
    ? resolveUserPath(fileOverride)
    : path.join(resolveStateDir(env), "logs", "cache-trace.jsonl");

  const includeMessages = parseBooleanValue(env.CARAPACE_CACHE_TRACE_MESSAGES);
  const includePrompt = parseBooleanValue(env.CARAPACE_CACHE_TRACE_PROMPT);
  const includeSystem = parseBooleanValue(env.CARAPACE_CACHE_TRACE_SYSTEM);
