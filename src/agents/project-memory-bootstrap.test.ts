...
describe("project memory bootstrap", () => {
  ...
  const entries: MemorySearchResult[] = [
    {
      ...
      score: 0.8,
      snippet:
        "Use the release helper. <!-- trigger: release helper --> <!-- importance: 8 --> <!-- project: github.com/Carapace/Carapace -->",
      source: "memory" as const,
      projectKey: "github.com/Carapace/Carapace",
      importance: 8,
      provenance: {
  ...
  async function prepareEntries(
    ...
    activeProjectKeys: string[] = ["github.com/Carapace/Carapace"],
  ): Promise<string[]> {
  ...
  it("includes entries from every project retained in the session active set", async () => {
    const rendered = (
      await prepareEntries(entries, ["github.com/example/other", "github.com/Carapace/Carapace"])
    ).join("\n");
    expect(rendered).toContain("Use the release helper.");
  ...
  it("uses the dedicated curated listing instead of a daily-note-crowded search", async () => {
    ...
    const rendered = (
      await prepareProjectMemoryBootstrap({
        cfg: {},
        agentId: "main",
        activeProjectKeys: ["github.com/Carapace/Carapace"],
      })
    ).join("\n");
    expect(rendered).toContain("Use the release helper.");
  ...
  it("builds scoped write guidance without capturing global memory", () => {
    const instruction = buildProjectMemoryWriteInstruction("github.com/Carapace/Carapace");
    expect(instruction).toContain("<!-- project: github.com/Carapace/Carapace -->");
