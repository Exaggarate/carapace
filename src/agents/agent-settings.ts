...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. Carapace config reloads create a new
...
export function resolveEffectiveCompactionMode(cfg?: CarapaceConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Carapace runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (carapace#75799).
 *
...
/**
 * Disable Carapace runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Carapace or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (carapace#75799).
 * Default-mode runs against ordinary providers keep Carapace runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Carapace runtime's
