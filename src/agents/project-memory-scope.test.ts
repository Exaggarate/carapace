...
async function makeRepo(remote?: string): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-project-scope-"));
  cleanup.push(root);
  await git(root, "init");
  if (remote) {
...
describe("project memory scope", () => {
  it.each([
    ["https://GitHub.COM/Carapace/Carapace.git", "github.com/Carapace/Carapace"],
    ["git@GITHUB.com:Carapace/Carapace.git", "github.com/Carapace/Carapace"],
    ["https://github.com/Carapace/Repo;Prod.git", "github.com/Carapace/Repo%3bProd"],
  ...
  it("preserves filesystem case so local path identities remain distinct", async () => {
    const parent = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-project-case-"));
    cleanup.push(parent);
  ...
  it("escapes the list delimiter in local repository paths", async () => {
    const parent = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-project-delimiter-"));
    cleanup.push(parent);
  ...
  it("converges a linked worktree and its source repository", async () => {
    const repo = await makeRepo("https://github.com/Carapace/Carapace.git");
    ...
    ).resolves.toEqual(["github.com/Carapace/Carapace", "github.com/Carapace/Carapace"]);
  });

  it.runIf(process.platform !== "win32")(
    "falls back to the path key when the git lookup hangs",
    async () => {
      const parent = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-project-hang-"));
      cleanup.push(parent);
      ...
      await fs.mkdir(repo);
      await git(repo, "init");
      await git(repo, "remote", "add", "origin", "https://github.com/Carapace/Carapace.git");
