...
import type { ExtensionAPI, ExtensionContext } from "carapace/plugin-sdk/agent-sessions";
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("carapace.runtime-context");
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-compaction-workspace-"));
