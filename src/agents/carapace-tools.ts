...
import { filterToolsByClientCaps } from "./carapace-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./carapace-tools.media-factory-plan.js";
import { createMediaGenerationAsyncStartCallback } from "./carapace-tools.media-yield.js";
import { applyNodesToolWorkspaceGuard } from "./carapace-tools.nodes-workspace-guard.js";
import {
...
import { createRequesterYieldCallback } from "./carapace-tools.requester-yield.js";
import { createCarapaceSwarmToolGroups } from "./carapace-tools.swarm.js";
import { resolveTranscriptsTool } from "./carapace-tools.transcripts.js";
import type { CarapaceToolsOptions } from "./carapace-tools.types.js";
import { resolveWidgetPresentationForRun } from "./carapace-tools.widget-presentation.js";
...
export { filterToolsByClientCaps } from "./carapace-tools.client-caps.js";
export function createCarapaceTools(options?: CarapaceToolsOptions): AnyAgentTool[] {
  ...
  const swarmToolGroups = createCarapaceSwarmToolGroups({
    config: resolvedConfig,
  ...
  options?.recordToolPrepStage?.("carapace-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("carapace-tools:image-tool");
  ...
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("carapace-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("carapace-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("carapace-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("carapace-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("carapace-tools:nodes-tool");
  ...
  const progressCardTool = shouldIncludeProgressCardToolForCarapaceTools({
    ...options,
    agentId: sessionAgentId,
  })
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentCarapaceTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentCarapaceTools([transcriptsTool]),
    ...collectPresentCarapaceTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
          ...
          ...createCarapaceDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentCarapaceTools([progressCardTool]),
    ...
    ...collectPresentCarapaceTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("carapace-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("carapace-tools:plugin-tools");
  }

  allTools = finalizeAgentToolAvailability(filterToolsByClientCaps(allTools, options?.clientCaps));
  options?.recordToolPrepStage?.("carapace-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("carapace-tools:tool-hooks");
  return allTools
    .map((tool) =>
