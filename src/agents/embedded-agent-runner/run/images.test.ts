...
describe("detectImageReferences", () => {
  ...
  it("ignores Carapace CLI image cache paths from prior prompt transcripts", () => {
    ...
    const refs = detectImageReferences(
      [
        '<system-reminder>Called the Read tool with {"file_path":"/Users/ada/.carapace/workspace/.carapace-cli-images/stale.png"}</system-reminder>',
  ...
  it("ignores temporary Carapace CLI image cache paths", () => {
    expectNoImageReferences(
      `Prior turn wrote ${path.join(resolvePreferredCarapaceTmpDir(), "carapace-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `[media attached: ${path.join(resolvePreferredCarapaceTmpDir(), "carapace-cli-images", "stale.jpg")} (image/jpeg)]`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "carapace", "carapace-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "carapace-501", "carapace-cli-images", "stale.jpg")}`,
    );
  });

  it("ignores file URLs into the Carapace CLI image cache", () => {
    const stalePath = path.join(os.tmpdir(), "carapace", "carapace-cli-images", "stale.png");
  ...
  it("detects normal user image paths in similarly named directories", () => {
    expect(detectImageReferences("/workspace/carapace-cli-images/current.png")).toStrictEqual([
      {
        raw: "/workspace/carapace-cli-images/current.png",
        type: "path",
        resolved: "/workspace/carapace-cli-images/current.png",
      },
    ]);
  });
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("skips generated media-note refs already supplied inline", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-dedupe-"));
  ...
  it("uses a described fact identity to suppress its generated media-note path", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-described-dedupe-"));
  ...
  it("dedupes a relative fact projection against the fact workspace", async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-fact-workspace-dedupe-"));
  ...
  it("keeps offloaded-only facts when existing images have no order metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-unordered-images-"));
  ...
  it("classifies prompt and attachment refs while preserving mixed attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-order-"));
  ...
  it("preserves an empty described-image slot in mixed attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-described-image-order-"));
    const inboundDir = path.join(stateDir, "media", "inbound");
  ...
  it("does not load an explicit prompt ref twice when the same fact owns it", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-fact-"));
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-sandbox-"));
  ...
  it("loads managed inbound absolute paths when workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-managed-"));
  ...
  it("keeps the fact hydration size limit", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-native-image-size-"));
...
describe("hydratePromptMediaMessages", () => {
  it("hydrates queued facts in attachment order without mutating cache-stable input", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-queued-image-facts-"));
    const inboundDir = path.join(stateDir, "media", "inbound");
  ...
  it("hydrates a fact-owned user message whose content is a string", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-string-image-facts-"));
    const imagePath = path.join(workspaceDir, "photo.png");
    await fs.writeFile(imagePath, Buffer.from(TINY_PNG_BASE64, "base64"));
    const message = attachRuntimePromptMediaFacts(
  ...
  it("reconstructs recent facts from canonical transcript media", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-replayed-image-"));
    const imagePath = path.join(workspaceDir, "photo.png");
    await fs.writeFile(imagePath, Buffer.from(TINY_PNG_BASE64, "base64"));
    const message = {
      role: "user" as const,
      content: "describe the replayed image",
      __carapace: { media: [{ path: imagePath, contentType: "image/png" }] },
    } as unknown as AgentMessage;
  ...
  it.each(["runtime", "recorder"] as const)(
    ...
    async (owner) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-replay-order-"));
  ...
  it("keeps duplicate fact slots across serialize and restore", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-replay-duplicates-"));
