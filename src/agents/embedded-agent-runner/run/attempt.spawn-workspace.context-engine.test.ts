...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createCarapaceCodingToolsMock,
        0,
  ...
  it("enables Tool Search controls for embedded Carapace runs when configured", async () => {
  ...
  it("carries the resolved context budget into Carapace tool construction", async () => {
    await createContextEngineAttemptRunner({
    ...
    expect(
      mockParams(hoisted.createCarapaceCodingToolsMock, 0, "tool construction params")
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createCarapaceCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createCarapaceCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createCarapaceCodingTools tools config",
    );
  ...
  it.each([false, true])("keeps lean replies direct (private: %s)", async (privateReply) => {
    hoisted.createCarapaceCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createCarapaceCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.carapaceCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "carapace.runtime-context",
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("CARAPACE_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.carapace/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.carapace/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.carapace/skills/smaug",
              source: "carapace-workspace",
              sourceInfo: {
                path: "/Users/alice/.carapace/skills/smaug/SKILL.md",
                source: "carapace-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.carapace/skills/smaug",
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/carapace-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/carapace-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("CARAPACE_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the Carapace runtime event.");
    expect(result.finalPromptText).toBe("Continue the Carapace runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the Carapace runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = await readTrajectoryEvents(tempPaths);
    const contextCompiled = trajectoryEvents.find((event) => event.type === "context.compiled");
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toBe("[Carapace room event]");
    expect(seenPrompt).not.toContain("inbound_event_kind: room_event");
    expect(seenPrompt).not.toBe("Continue the Carapace runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[Carapace room event]");
  });
  ...
  it("flushes the embedded session transcript before afterTurn", async () => {
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        currentInboundEventKind: "room_event",
        currentInboundContext: { text: "[Carapace room event]" },
        suppressNextUserMessagePersistence: true,
        transcriptPrompt: "",
      },
      sessionPrompt: async (session) => {
  ...
  it("uses SQLite transcript messages for bootstrap without treating the marker as a file", async () => {
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ctx-engine-sqlite-"));
