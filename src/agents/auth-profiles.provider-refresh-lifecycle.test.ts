...
const START_AUTH_CALLBACK = "__carapaceProviderRefreshLifecycleStart";
...
describe("provider OAuth refresh lifecycle", () => {
  it("resumes refresh after synchronous provider registration completes", async () => {
    await withCarapaceTestState(
      { layout: "state-only", prefix: "carapace-provider-refresh-", agentEnv: "main" },
      async (state) => {
        ...
        const loadOptions: NonNullable<Parameters<typeof loadCarapacePlugins>[0]> = {
          cache: false,
