...
function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`carapace models auth login --provider ${providerId}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("carapace configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("carapace configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
