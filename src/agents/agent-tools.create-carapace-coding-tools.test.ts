/**
 * Broad coverage for createCarapaceCodingTools.
...
import type { AgentTool, AgentToolResult } from "carapace/plugin-sdk/agent-core";
...
import * as carapacePluginTools from "./carapace-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createCarapaceCodingTools>) {
...
type CarapaceCodingTool = ReturnType<typeof createCarapaceCodingTools>[number];
type CarapaceToolsOptions = NonNullable<Parameters<typeof createCarapaceTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: CarapaceCodingTool[], name: string): CarapaceCodingTool {
...
function requireToolExecute(tool: CarapaceCodingTool): NonNullable<CarapaceCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateCarapaceToolsOptions(): CarapaceToolsOptions {
  const calls = vi.mocked(createCarapaceTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createCarapaceTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: CarapaceToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createCarapaceCodingTools", () => {
  it("forwards the session web-search gate to core tool materialization", () => {
    vi.mocked(createCarapaceTools).mockClear();
    createCarapaceCodingTools({ webSearchEnabled: false });

    expect(latestCreateCarapaceToolsOptions().webSearchEnabled).toBe(false);
  });
  ...
  it.each([
    ...
    async ({ backend, content, oversized }) => {
      ...
      const baseDir = virtual
        ...
        : tempDirs.make("carapace-assembled-local-skill-");
          ...
          resolvedSkills: [
            {
              ...
              source: virtual ? "carapace-node" : "test",
              sourceInfo: {
                source: virtual ? "carapace-node" : "test",
                path: locator,
                scope: "temporary",
  ...
  it("exposes only gateway config reads to owner sessions", () => {
    const tools = createCarapaceCodingTools({ config: testConfig });
  ...
  it("passes explicit channel and requester facts to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-hook-channel-"));
  ...
  it("binds configured MCP cron authority only to the exact admitted run", async () => {
    ...
    const forgedTools = runWithCronCreatorAuthorityCapabilityResolver({
      ...
      runId: "forged-run",
      resolve,
      run: () => createCarapaceCodingTools({ runId: "forged-run", senderIsOwner: false }),
    });
    expect(toolNameList(forgedTools)).not.toContain("automations");
    expect(
      vi.mocked(createCarapaceTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority,
    ).toBeUndefined();
    ...
    const activeRun = runWithCronCreatorAuthorityCapability(capability, async () => {
      const wrongRunTools = runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "other-run",
        resolve,
        run: () => createCarapaceCodingTools({ runId: "admitted-run", senderIsOwner: false }),
      });
      ...
      expect(admittedToolNames).not.toContain("carapace");
      retainedResolver =
        vi.mocked(createCarapaceTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority;
    ...
    expect(
      toolNameList(createCarapaceCodingTools({ runId: "admitted-run", senderIsOwner: false })),
  ...
  it("drops senderless Automations retention when exact authority aborts or errors", async () => {
    ...
    const buildTools = (capability: ReturnType<typeof createCronCreatorAuthorityCapability>) =>
      runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "lifecycle-run",
        resolve,
        run: () => createCarapaceCodingTools({ runId: "lifecycle-run", senderIsOwner: false }),
      });
    ...
    expect(
      toolNameList(createCarapaceCodingTools({ runId: "lifecycle-run", senderIsOwner: false })),
    ).not.toContain("automations");
  });
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createCarapaceTools).mockReturnValueOnce([wrapped as never]);

    const tools = createCarapaceCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Carapace tools are not materialized", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
  ...
  it("keeps the injected ring-zero tool under policy and rejects a same-name replacement", () => {
    ...
    vi.mocked(createCarapaceTools).mockReturnValueOnce([duplicateTool]);

    const tools = runWithAgentRingZeroTools([injectedTool], () =>
      createCarapaceCodingTools({
        config: { tools: { allow: ["read"], deny: ["carapace"] } },
    ...
    expect(tools[0]?.name).toBe("carapace");
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateCarapaceToolsOptions();
  ...
  it("does not inherit native-harness bridge runtime allowlists", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateCarapaceToolsOptions().pluginToolAllowlist).toEqual([
      "sessions_spawn",
      "memory_search",
    ]);
    expect(latestCreateCarapaceToolsOptions().inheritedToolAllowlist).toEqual([]);
  });
  ...
  it("inherits embedded runtime toolsAllow when explicitly marked as parent capability", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateCarapaceToolsOptions().inheritedToolAllowlist;
  ...
  it("keeps restricted spawn inheritance in the caller-owned runtime snapshot", () => {
    ...
    expect(latestCreateCarapaceToolsOptions().inheritedToolAllowlist).toBe(
      inheritedToolAllowlistRef,
    );
  ...
  it("does not snapshot additive alsoAllow policies for spawn inheritance", () => {
    ...
    expect(latestCreateCarapaceToolsOptions().inheritedToolDenylist).toContain("exec");
  });
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:carapace"], ["*"]]) {
      const tools = createCarapaceCodingTools({
        config: { tools: { profile: "minimal" } },
        runtimeToolAllowlist,
        toolConstructionPlan: {
  ...
  it("passes source reply delivery mode to Carapace tool construction", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateCarapaceToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it.each([
    ...
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-completion-grant-"));
    ...
    const config: CarapaceConfig = { session: { store: storeTemplate } };
    ...
    try {
      ...
      expect(latestCreateCarapaceToolsOptions().sourceReplyOnly).toBe(testCase.expected);
    } finally {
      await fs.rm(storeDir, { recursive: true, force: true });
    }
  });

  it("passes configured filesystem policy to Carapace tool construction", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    ...
    expect(latestCreateCarapaceToolsOptions().fsPolicy).toEqual({ workspaceOnly: true });
  });
  ...
  it.each([
    ...
    expect(latestCreateCarapaceToolsOptions().sandboxWorkspaceMediaReadAllowed).toBe(
      testCase.expected,
    );
  });

  it("allows workspace-only reads from declared sandbox bind mounts", async () => {
    const workspaceDir = tempDirs.make("carapace-sandbox-workspace-");
    const bindRoot = tempDirs.make("carapace-sandbox-bind-");
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateCarapaceToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateCarapaceToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("continues oversized data through the assembled shell-disabled read tool", async () => {
    const workspaceDir = tempDirs.make("carapace-read-no-shell-");
  ...
  it("passes plugin suppression into Carapace tool construction plans", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateCarapaceToolsOptions().disablePluginTools).toBe(true);
  });

  it("forwards trusted conversation recall to Carapace tool construction", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    ...
    expect(latestCreateCarapaceToolsOptions().conversationRecall).toEqual(conversationRecall);
  });

  it("keeps plugin-only construction off the Carapace core factory", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(carapacePluginTools, "resolveCarapacePluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("forwards the native channel id through standard tool construction", () => {
    ...
    expect(latestCreateCarapaceToolsOptions().nativeChannelId).toBe("oc_native_chat");
    expect(latestCreateCarapaceToolsOptions().currentChatType).toBe("group");
    expect(latestCreateCarapaceToolsOptions().messageActionTurnCapability).toBe(
      "turn-capability-1",
    );
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateCarapaceToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
  ...
  it("materializes additive runtime tools while preserving normal deny policy", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateCarapaceToolsOptions().pluginToolAllowlist).not.toContain(
      "workboard_heartbeat",
    );
    expect(
      createPluginToolAllowlist(latestCreateCarapaceToolsOptions().pluginToolAllowlist).allowsTool(
        "workboard",
        "workboard_heartbeat",
      ),
    ).toBe(true);
    expect(latestCreateCarapaceToolsOptions()).not.toHaveProperty("runtimePluginToolGrant");
    expectListIncludes(latestCreateCarapaceToolsOptions().pluginToolDenylist, ["workboard_block"]);
  });

  it("passes explicit denylist entries to Carapace tool factory planning", () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateCarapaceToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("removes message from persisted visible child sessions on every turn", async () => {
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-visible-subagent-message-"));
    const storeTemplate = path.join(storeDir, "{agentId}", "sessions.json");
  ...
  it("passes inherited allowlist entries to Carapace plugin discovery", async () => {
    const createCarapaceToolsMock = vi.mocked(createCarapaceTools);
    createCarapaceToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `carapace-session-store-${agentId}-{agentId}.json`,
    );
    await writeSessionStore(storeTemplate, agentId, {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateCarapaceToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateCarapaceToolsOptions().cronCreatorToolAllowlist;
  ...
  it("passes the final unrestricted tool surface to cron-created agent turns", () => {
    ...
    createCarapaceCodingTools({ config: {} });

    expect(createCarapaceToolsMock).toHaveBeenCalledTimes(1);
    const cronAllowNames = cronCreatorToolNames(
      latestCreateCarapaceToolsOptions().cronCreatorToolAllowlist,
    );
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      CarapaceToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("carapace-tools:test-helper")).toBeLessThan(
      stages.indexOf("carapace-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createCarapaceCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createCarapaceCodingTools({
      config: testConfig,
    ...
    const codexTools = createCarapaceCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledOpenAiTools = createCarapaceCodingTools({
    ...
    const anthropicTools = createCarapaceCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: CarapaceConfig = {
      tools: {
        exec: {
    ...
    const allowed = createCarapaceCodingTools({
    ...
    const denied = createCarapaceCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createCarapaceCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCarapaceCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    ...
    expect(latestCreateCarapaceToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createCarapaceCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createCarapaceCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createCarapaceCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createCarapaceCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createCarapaceCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createCarapaceCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createCarapaceCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createCarapaceCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createCarapaceCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-"));
    try {
      ...
      const contents = "Hello from carapace read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "carapace-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "carapace-workspace"),
    ...
    const tools = createCarapaceCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-canonical-"));
    try {
      const tools = createCarapaceCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("records restricted memory flush writes without an active memory provider", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-memory-cwd-"));
  ...
  it("records turn taint and source-session lineage for memory writes, edits, and patches", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-memory-write-taint-"));
    let tainted = false;
    try {
      const tools = createCarapaceCodingTools({
        workspaceDir,
  ...
  it("records agent provenance after an untainted same-turn delete and recreate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-memory-recreate-"));
    try {
  ...
  it.each(["relative", "container"])(
    ...
    async (pathKind) => {
      const workspaceDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "carapace-memory-sandbox-taint-"),
      );
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-legacy-alias-"));
    try {
      const tools = createCarapaceCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-structured-write-"));
    try {
      const tools = createCarapaceCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-structured-edit-"));
    try {
...
describe("createCarapaceCodingTools read behavior", () => {
  it("protects materialized sandbox skill identities when no skill snapshot exists", async () => {
    const root = tempDirs.make("carapace-sandbox-skill-whole-");
  ...
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-sbx-encoding-"));
  ...
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "carapace-outside.txt");
  ...
  it("rejects sandbox directory reads before calling the bridge read operation", async () => {
    const tmpDir = tempDirs.make("carapace-sbx-directory-");
  ...
  it("resolves Unicode-equivalent filenames through sandbox operations", async () => {
    const tmpDir = tempDirs.make("carapace-sbx-unicode-");
  ...
  it("classifies sandbox AVIF reads from the already-read buffer", async () => {
    const tmpDir = tempDirs.make("carapace-sbx-avif-");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-cap-"));
  ...
  it.each([
    ...
    const root = tempDirs.make("carapace-read-first-page-cap-");
  ...
  it("describes explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-offset-eof-"));
  ...
  it("ignores a trailing newline when describing offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createCarapaceReadTool(
      baseRead as unknown as Parameters<typeof createCarapaceReadTool>[0],
    );
...
describe("createCarapaceCodingTools message provider policy", () => {
