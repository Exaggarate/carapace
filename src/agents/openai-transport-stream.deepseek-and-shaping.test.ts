...
describe("openai transport stream", () => {
  ...
  it.each(["none", "short", "long"] as const)(
    ...
    (cacheRetention) => {
      const params = buildOpenAIResponsesParams(
        ...
        {
          carapace_session_id: "session-123",
          carapace_turn_id: "turn-123",
        },
      ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shaping when simple completions use the Carapace transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { carapace_session_id: "session-123" },
