...
import { withCarapaceMcpCaptureHeader, writeTemporaryBundleMcpJson } from "./bundle-mcp-runtime.js";
...
async function writeGeminiSettings(
  ...
  const temporary = await writeTemporaryBundleMcpJson("carapace-gemini-mcp-", settings);
  return {
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const temporary = await writeTemporaryBundleMcpJson(
    "carapace-gemini-mcp-attempt-",
    withCarapaceMcpCaptureHeader(settings, params.captureKey),
  );
  return {
    env: {
      ...params.inheritedEnv,
