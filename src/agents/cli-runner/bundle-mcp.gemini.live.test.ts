...
const LIVE = isLiveTestEnabled(["CARAPACE_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "carapace-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("carapace_live_probe", "Carapace Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Carapace-configured streamable-http server", async () => {
    const geminiCommand = process.env.CARAPACE_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("carapaceLiveProbe");
