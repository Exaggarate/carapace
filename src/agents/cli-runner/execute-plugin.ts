...
function createPluginToolPermissionHandler(params: {
  ...
  return async (request) => {
    ...
    try {
      ...
      return denyTool("Carapace denied native tool use: the admitted run is no longer active.");
    }

    const toolName = request.toolName.trim();
    if (!toolName) {
      return denyTool("Carapace denied an unnamed native tool.");
    }
    if (run.cliToolAvailability && !run.cliToolAvailability.native.includes(toolName)) {
      return denyTool(`Carapace denied native tool ${toolName}: it is unavailable to this run.`);
    }
    ...
    if (nativeFileTool) {
      ...
      if (typeof nativePath !== "string") {
        return denyTool("Carapace denied native file tool use: invalid file path.");
      }
      if (Object.hasOwn(request.toolInput, "path") && request.toolInput.path !== nativePath) {
        return denyTool("Carapace denied native file tool use: conflicting file paths.");
      }
      ...
      if (canonicalToolName === "edit") {
        ...
        if (typeof oldText !== "string" || typeof newText !== "string") {
          return denyTool("Carapace denied native edit tool use: invalid replacement.");
        }
        if (
          edits !== undefined &&
          (!Array.isArray(edits) ||
          ...
          return denyTool("Carapace denied native edit tool use: conflicting replacements.");
        }
    ...
    try {
      ...
      return denyTool("Carapace denied native tool use: the admitted run closed during policy.");
    }
    if (hookResult.blocked) {
      return denyTool(hookResult.reason);
    }
    if (!isRecord(hookResult.params)) {
      return denyTool("Carapace denied native tool use: before_tool_call returned invalid input.");
    }
    let toolInput = hookResult.params;
    // SDK permission replies must return the native schema, never policy-only aliases.
    if (nativeFileTool) {
      if (typeof toolInput.path !== "string") {
        return denyTool("Carapace denied native file tool use: invalid rewritten file path.");
      }
      if (toolInput === policyInput) {
        ...
        if (canonicalToolName === "edit") {
          ...
          if (
            ...
            return denyTool("Carapace denied an unrepresentable native edit rewrite.");
          }
    ...
    if (plan === "deny") {
      return denyTool(
        `Carapace exec policy denied native tool use (security=${permission.security}, ask=${permission.ask}).`,
      );
    }
    ...
    try {
      ...
      return denyTool("Carapace denied native tool use: the admitted run closed during approval.");
    }
    if (outcome.kind !== "allow") {
      return denyTool(
        outcome.message ??
          (outcome.reason === "user"
            ? `Carapace user denied native tool use (${toolName}).`
            : `Carapace approval was not granted for native tool use (${toolName}).`),
      );
    }
  ...
  return async (request) => {
    ...
    try {
      ...
      return cancelUserInput(
        "Carapace cancelled operator input: the admitted run is no longer active.",
      );
    }
    ...
    if (
      ...
      return cancelUserInput(
        toolName
          ? `Carapace cancelled operator input from ${toolName}: it is unavailable to this run.`
          : "Carapace cancelled an unnamed operator input request.",
      );
    }
    if (request.questions.length === 0 || request.questions.length > 12) {
      return cancelUserInput("Carapace cancelled an invalid operator input request.");
    }
    ...
    try {
      ...
      try {
        ...
        return cancelUserInput(
          "Carapace cancelled operator input: the admitted run closed before the answer was committed.",
        );
      }
      return result.status === "answered"
        ...
        : cancelUserInput(
            result.message ??
              "Carapace cancelled operator input; continue with your best judgment.",
          );
    } catch {
      return cancelUserInput(
        "Carapace could not collect operator input; continue with your best judgment.",
      );
    } finally {
      params.onPendingInput(-1);
    }
