...
function hasExplicitImageModelConfig(config: CarapaceConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: CarapaceConfig | undefined): boolean {
  return (
