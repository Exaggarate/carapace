...
function formatSandboxRecreateHint(params: { scope: SandboxScope; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`carapace sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`carapace sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("carapace sandbox recreate --all");
}
