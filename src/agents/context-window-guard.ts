...
export function formatContextWindowWarningMessage(params: {
  ...
  if (params.guard.source === "modelsConfig") {
    return (
      `${base}; Carapace is using the configured model context limit for this model, ` +
...
export function formatContextWindowBlockMessage(params: {
  ...
  return (
    ...
    `Carapace local/self-hosted runs work best at ${params.guard.warnBelowTokens}+ tokens.`
  );
}
