...
describe("ManagedWorktreeService provisioned state", () => {
  ...
  beforeAll(async () => {
    ...
    templateRoot = await fs.mkdtemp(path.join(tempRoot, "carapace-worktree-state-template-"));
  ...
  beforeEach(async () => {
    const tempRoot = await fs.realpath(os.tmpdir());
    root = await fs.mkdtemp(path.join(tempRoot, "carapace-worktree-state-"));
    repo = path.join(root, "repo");
    await fs.cp(templateRepo, repo, { mode: fsConstants.COPYFILE_FICLONE, recursive: true });
    repo = await fs.realpath(repo);
    env = { ...process.env, CARAPACE_STATE_DIR: path.join(root, "carapace-state") };
  ...
  it("fails closed for pre-ledger worktrees whose ignored state is unknown", async () => {
    ...
    await git(repo, "worktree", "add", "-b", "carapace/legacy", legacyPath, "HEAD");
