 ...
 * Protects Carapace web_search routing when provider/model compatibility also
...
vi.mock("./carapace-plugin-tools.js", () => ({
  resolveCarapacePluginToolsForOptions: () => [{ name: "browser" }],
}));
...
const testing = {
  applyModelProviderToolPolicy(
    ...
    options?: NonNullable<Parameters<typeof createCarapaceCodingTools>[0]>,
  ): AnyAgentTool[] {
    const actualTools = createCarapaceCodingTools({
      ...options,
      cwd: "/tmp/carapace-agent-tools-policy-test",
      workspaceDir: "/tmp/carapace-agent-tools-policy-test",
...
describe("applyModelProviderToolPolicy", () => {
  ...
  it("keeps web_search for OpenRouter xAI model ids so Carapace tool routing stays authoritative", () => {
    const filtered = testing.applyModelProviderToolPolicy(baseTools, {
      modelCompat: {
