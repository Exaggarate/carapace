...
async function withHostedDocumentServer<T>(
  ...
  const stateDir = await mkdtemp(path.join(tmpdir(), "carapace-canvas-gateway-"));
  ...
  return await withEnvAsync(
    {
      CARAPACE_SKIP_CANVAS_HOST: params.skipHost,
...
describe("core Canvas Gateway host switches", () => {
  ...
  it.each([
    ...
    { label: "CARAPACE_SKIP_CANVAS_HOST", config: {}, skipHost: "1" },
