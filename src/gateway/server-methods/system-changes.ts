    ...
    source === "doctor"
      ...
      : source === "config-rpc"
        ...
        : source === "plugin-install"
          ...
          : source === "system-agent"
            ? "Carapace updated configuration"
            : source === "cli"
  ...
  if (value.event === "config.external") {
    ...
    return {
      entry: {
        ...
        kind: "external-edit",
        source: "external",
        summary: summarizePaths("Configuration edited outside Carapace", changedPaths),
...
export const systemChangesHandlers: GatewayRequestHandlers = {
  "carapace.changes.list": ({ params, respond }) => {
    if (
      !assertValidParams(params, validateSystemChangesListParams, "carapace.changes.list", respond)
    ) {
      return;
    }
    try {
