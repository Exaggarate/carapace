...
export function buildNodeCommandRejectionHint(
  ...
  if (reason === "node did not declare commands") {
    if (node?.declaredCommands?.includes(command)) {
      return "node command not allowed: the node's declared command surface is pending approval; run `carapace nodes pending`, then `carapace nodes approve <requestId>`";
    }
