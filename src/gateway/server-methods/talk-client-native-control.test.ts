...
describe("native Talk through the public OpenAI plugin registration", () => {
    ...
    async (text, eventOrder) => {
      await withParkedNativeTask(
        async ({ create, offer, result, socket, activeRun, abortOwned, chatAbortControllers }) => {
          ...
          const idleReply =
            text === "cancel"
              ? "There is no active Carapace run to cancel."
          ...
          const reply =
            text === "cancel"
              ...
              : "Carapace is working on the current voice request.";
          ...
          expect(
            socket.sent
              ...
              .filter((frame) => frame.includes("Internal Carapace voice control result.")),
          ).toHaveLength(1);
          expect(
  ...
  it.each([
    ...
    ["cancel", "There is no active Carapace run to cancel."],
            ...
            expect(sentFrames()).toContainEqual(
              expect.objectContaining({
                ...
                content: [
                  expect.objectContaining({
                    type: "input_text",
                    text: expect.stringContaining("Cancelled the active Carapace run."),
                  }),
                ],
              }),
            ),
