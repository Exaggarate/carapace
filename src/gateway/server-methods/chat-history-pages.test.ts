...
describe("enrichChatHistoryCompactionMarkers", () => {
  it("joins checkpoint token metrics to the matching transcript marker", () => {
    const marker = {
      role: "system",
      __carapace: { kind: "compaction", id: "compact-entry-1", seq: 4 },
    };
    const entry = {
    ...
    expect(marker["__carapace"]).not.toHaveProperty("tokensBefore");
  });

  it("preserves message identity without a matching checkpoint", () => {
    const marker = {
      role: "system",
      __carapace: { kind: "compaction", id: "compact-entry-1" },
    };
