...
export async function prepareDelegatedSystemAgentApproval(params: {
  ...
  if (!approvalAuthority) {
    throw new Error("delegated Carapace approval requires an active run authority");
  }
  ...
  const assertLiveApprovalAuthority = () => {
    if (!isAuthorityActive() || params.sessions.get(params.sessionId) !== params.session) {
      throw new Error(
        "Carapace change cancelled: system-agent approval authority is no longer active. Retry the request if it is still needed.",
      );
    }
  };
  ...
  return async (proposal) => {
    ...
    return await withProposalFailureCleanup(async (): ReturnType<DelegatedProposalResolver> => {
      ...
      if (params.session.pendingApproval) {
        ...
        throw new Error("Carapace change is no longer pending. Retry the request.");
      }
      ...
      if (!manager) {
        throw new Error("Carapace approval registry unavailable");
      }
      ...
      const cancelledReply = {
        text: "Carapace change cancelled. No change. Retry the request if it is still needed.",
        action: "none" as const,
        applied: false,
      };
      const failedReply = {
        text: "Carapace change failed to complete. Check the current settings and Carapace status before retrying.",
        action: "none" as const,
        applied: false,
      };
      ...
      void handlePendingApprovalRequest({
        ...
        respond: () => undefined,
        context: params.context,
        requestEventName: "carapace.approval.requested",
        requestEvent,
        twoPhase: true,
        approvalKind: "system-agent",
        ...
        afterDecisionErrorLabel: "Carapace approval apply failed",
      }).catch((error: unknown) => {
