...
const mocks = vi.hoisted(() => ({
  ...
  resolveCarapaceAgentSqlitePath: vi.fn(
    (params?: { path?: string }) => params?.path ?? "/agents/test-agent/carapace-agent.sqlite",
  ),
  closeCarapaceAgentDatabaseByPath: vi.fn((_pathname?: string, _expectedAgentId?: string) => true),
  listCarapaceRegisteredAgentDatabases: vi.fn(() => [] as Array<Record<string, unknown>>),
  unregisterCarapaceAgentDatabase: vi.fn(),
  assertNoCarapaceAgentDatabaseLeases: vi.fn(),
  ...
  return {
    ...
    mutateConfigFileWithRetry: async (params: {
      ...
      const result = await params.mutate(draft, {
        snapshot: { path: "/tmp/carapace/config.json" },
...
vi.mock("../../agents/auth-profiles/path-resolve.js", async () => ({
  ...
  resolveSharedAuthStorePath: () => "/resolved/agents/main/agent/carapace-agent.sqlite",
}));
...
vi.mock("../../state/carapace-agent-db.js", () => ({
  closeCarapaceAgentDatabaseByPath: mocks.closeCarapaceAgentDatabaseByPath,
  listCarapaceRegisteredAgentDatabases: mocks.listCarapaceRegisteredAgentDatabases,
  resolveCarapaceAgentSqlitePath: mocks.resolveCarapaceAgentSqlitePath,
  resolveIncognitoCarapaceAgentSqlitePath: () =>
    "/agents/test-agent/incognito-carapace-agent.sqlite",
}));
...
vi.mock("../../state/carapace-agent-db-registry.js", () => ({
  isSameCarapaceAgentDatabasePath: (left: string, right: string) =>
    path.resolve(left) === path.resolve(right),
  unregisterCarapaceAgentDatabase: mocks.unregisterCarapaceAgentDatabase,
}));

vi.mock("../../state/carapace-agent-db-lease.js", () => ({
  assertNoCarapaceAgentDatabaseLeases: mocks.assertNoCarapaceAgentDatabaseLeases,
}));
...
beforeEach(() => {
  ...
  mocks.resolveCarapaceAgentSqlitePath
    .mockReset()
    .mockImplementation(
      (params?: { path?: string }) => params?.path ?? "/agents/test-agent/carapace-agent.sqlite",
    );
  mocks.closeCarapaceAgentDatabaseByPath.mockReset().mockReturnValue(true);
  mocks.listCarapaceRegisteredAgentDatabases.mockReset().mockReturnValue([]);
  mocks.unregisterCarapaceAgentDatabase.mockReset();
...
describe("agents.delete", () => {
  ...
  it("removes only the deleted agent's authority before committing its roster removal", async () => {
    ...
    mocks.closeCarapaceAgentDatabaseByPath.mockImplementation(() => {
    ...
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledWith(
      "/agents/test-agent/carapace-agent.sqlite",
      "test-agent",
    );
    expect(mocks.unregisterCarapaceAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/agents/test-agent/carapace-agent.sqlite",
    });
  ...
  it("rolls cron back and keeps the roster when authority cleanup fails", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledTimes(2);
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
  });
  ...
  it("keeps deletion fenced when config persistence succeeds before reporting failure", async () => {
    ...
    expect(mocks.beginAgentDeletionRollback).not.toHaveBeenCalled();
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalled();
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
  });
  ...
  it("does not perform destructive cleanup without a config deletion result", async () => {
    ...
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledTimes(2);
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
    expect(mocks.unregisterCarapaceAgentDatabase).not.toHaveBeenCalled();
    expect(mocks.beginAgentDeletionRollback).toHaveBeenCalledOnce();
  });
  ...
  it("resumes a journaled deletion while its directory is still owned by the deleted agent", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledWith(
      "/journal/agent/carapace-agent.sqlite",
      "test-agent",
    );
    expect(mocks.unregisterResolvedAgentDir).toHaveBeenCalledWith({
      agentId: "test-agent",
  ...
  it("extends a recovery cleanup snapshot with newly discovered database files", async () => {
    ...
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue(journal);
    mocks.listCarapaceRegisteredAgentDatabases.mockReturnValue([
  ...
  it("settles recovery without touching a journaled directory claimed by another agent", async () => {
    ...
    const databaseRows = [
      { agentId: "test-agent", path: "/journal/agent/carapace-agent.sqlite" },
      { agentId: "other-agent", path: "/journal/agent/carapace-agent.sqlite" },
    ];
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue({
      agentId: "test-agent",
    ...
    mocks.listCarapaceRegisteredAgentDatabases.mockImplementation(() => databaseRows);
    mocks.unregisterCarapaceAgentDatabase.mockImplementation(
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.listCarapaceRegisteredAgentDatabases).toHaveBeenCalled();
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledWith(
      "/journal/agent/carapace-agent.sqlite",
      "test-agent",
    );
    expect(mocks.assertNoCarapaceAgentDatabaseLeases).toHaveBeenCalledWith("test-agent", {});
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
    expect(mocks.unregisterCarapaceAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/journal/agent/carapace-agent.sqlite",
    });
  ...
  it("revalidates database ownership after earlier filesystem cleanup", async () => {
    ...
    mocks.listCarapaceRegisteredAgentDatabases.mockImplementation(() =>
      claimed ? [{ agentId: "other-agent", path: "/journal/agent/survivor.sqlite" }] : [],
    );
    mocks.movePathToTrash.mockImplementation(async (pathname?: string) => {
      if (pathname === "/journal/agent/carapace-agent.sqlite") {
        claimed = true;
      }
      return "/trashed";
    });
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/agent/carapace-agent.sqlite");
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/workspace");
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
  ...
  it("cleans a relocated deleted-agent database while preserving a claimed agent directory", async () => {
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledTimes(3);
    expect(mocks.closeCarapaceAgentDatabaseByPath).toHaveBeenCalledWith(
  ...
  it("preserves a parent directory containing another agent's relocated database", async () => {
    ...
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/agents/test-agent/carapace-agent.sqlite");
    expect(mocks.beginAgentDeletionFinish).toHaveBeenCalledOnce();
  });
  ...
  it("unregisters a closed database row after committing deletion", async () => {
    mocks.closeCarapaceAgentDatabaseByPath.mockReturnValueOnce(false);
    ...
    expect(mocks.writeConfigFile.mock.invocationCallOrder[0]).toBeLessThan(
      expectDefined(
        mocks.unregisterCarapaceAgentDatabase.mock.invocationCallOrder[0],
  ...
  it("closes and archives every registered database path owned by the deleted agent", async () => {
    ...
    expect(mocks.closeCarapaceAgentDatabaseByPath).not.toHaveBeenCalledWith(
  ...
  it("sweeps WAL sidecars recreated between deletion preparation and cleanup", async () => {
    ...
    const walPath = `${agentDir}/carapace-agent.sqlite-wal`;
    const shmPath = `${agentDir}/carapace-agent.sqlite-shm`;
  ...
  it("reports trash failures without deleting the retained directory", async () => {
    ...
    const workspaceDir = await actualFs.realpath(
      await actualFs.mkdtemp(path.join(os.tmpdir(), "carapace-agent-delete-trash-failure-")),
    );
  ...
  it("rejects deleting the main agent", async () => {
    ...
    expectRespondErrorContaining(respond, "carapace doctor --fix");
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
  });
