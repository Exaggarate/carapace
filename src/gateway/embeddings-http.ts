...
// Bridges /v1/embeddings requests to configured Carapace memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps Carapace agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (!isCarapaceAgentModelId(requestModel)) {
    sendInvalidRequest(res, "Invalid `model`. Use `carapace` or `carapace/<agentId>`.");
    return true;
  }
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-carapace-model")) ||
