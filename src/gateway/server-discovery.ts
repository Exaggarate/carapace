...
/** Formats the Bonjour instance name while preserving user-provided Carapace names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "Carapace";
  }
  if (/carapace/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Carapace)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.CARAPACE_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "carapace");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.CARAPACE_TAILNET_DNS?.trim();
