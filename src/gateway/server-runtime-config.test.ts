...
describe("resolveGatewayRuntimeConfig", () => {
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.CARAPACE_GATEWAY_TOKEN;
      delete process.env.CARAPACE_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.CARAPACE_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.CARAPACE_GATEWAY_TOKEN;
      }
    });
    ...
    it.each([
      {
        ...
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or CARAPACE_GATEWAY_TOKEN)",
      },
      {
