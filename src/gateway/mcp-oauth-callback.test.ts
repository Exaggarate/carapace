...
const AUTHORIZATION_URL =
  "https://accounts.example.com/authorize?state=state-1234567890&client_id=carapace";

function callbackConfig(serverName = "calendar"): CarapaceConfig {
  return {
    mcp: {
      servers: {
        [serverName]: {
...
async function dispatch(
  path: string,
  options?: { config?: CarapaceConfig; method?: string },
): Promise<{
  handled: boolean;
