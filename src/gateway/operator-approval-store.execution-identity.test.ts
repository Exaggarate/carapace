...
function databaseOptions(): CarapaceStateDatabaseOptions {
  const stateDir = fs.realpathSync(tempDirs.make("carapace-approval-id-"));
...
describe("operator approval execution identity", () => {
  it("creates the side table only for the first exact bound write", () => {
    ...
    expect(
      openCarapaceStateDatabase(unbound)
        .db.prepare(
    ...
    const userVersionBefore = openCarapaceStateDatabase(bound)
    ...
    expect(
      openCarapaceStateDatabase(bound)
        .db.prepare(
    ...
    expect(openCarapaceStateDatabase(bound).db.prepare("PRAGMA user_version").get()).toEqual(
      userVersionBefore,
    );
  });
  ...
  it("never late-binds or binds a mismatched source run", () => {
    ...
    expect(
      openCarapaceStateDatabase(mismatch)
        .db.prepare(
          "SELECT name FROM sqlite_schema WHERE type = 'table' AND name = 'operator_approval_execution_identities'",
        )
        .get(),
