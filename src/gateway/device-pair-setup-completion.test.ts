...
describe("device pair setup completion", () => {
  it.each([
    ...
    const baseDir = await tempDirs.make(`carapace-setup-completion-${access}-`);
  ...
  it("keeps the completion recoverable when a slow subscriber drops the frame", async () => {
    const baseDir = await tempDirs.make("carapace-setup-completion-slow-");
    const slowSocket = {
      readyState: 1,
  ...
  it("ignores generic bootstrap records without setup correlation", async () => {
    const baseDir = await tempDirs.make("carapace-setup-completion-generic-");
    persistDeviceBootstrapTokenRecords(
      {
        generic: {
          token: "generic",
