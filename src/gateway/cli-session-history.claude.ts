...
// Converts Claude project JSONL into Carapace transcript-compatible messages.
...
import { attachCarapaceTranscriptMeta } from "./session-transcript-readers.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; Carapace history
...
export function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    ...
    return attachCarapaceTranscriptMeta(
      {
        role: "user",
        content,
        ...(harnessInjected
