...
function resolveGatewayTokenOrEnv(): string {
  ...
  if (typeof token !== "string") {
    throw new Error("expected gateway token in test state or CARAPACE_GATEWAY_TOKEN");
  }
  return token;
}
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = nextAuthIdentityPath("carapace-auth-rate-limit");
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = nextAuthIdentityPath("carapace-auth-device");
