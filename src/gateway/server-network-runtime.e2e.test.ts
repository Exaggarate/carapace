...
describe("gateway network runtime", () => {
  ...
  it("bootstraps env proxy dispatching when the gateway starts directly", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-gw-proxy-home-"));
    ...
    try {
      ...
      process.env.CARAPACE_BUNDLED_PLUGINS_DIR = path.join(tempHome, "empty-bundled-plugins");
      await fs.mkdir(process.env.CARAPACE_BUNDLED_PLUGINS_DIR, { recursive: true });
  ...
  it.each(["lan", "loopback", "tailnet", "auto", "custom", undefined] as const)(
    ...
    async (bind) => {
      const envSnapshot = captureEnv([...NETWORK_GATEWAY_ENV_KEYS]);
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-gw-bind-home-"));
      let server: Awaited<ReturnType<typeof startGatewayServer>> | undefined;
