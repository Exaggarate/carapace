...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: CarapaceConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    ...
    options.localPortOverride !== undefined
      ? undefined
      : normalizeOptionalString(process.env.CARAPACE_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.CARAPACE_ALLOW_INSECURE_PRIVATE_WS === "1";
