...
import { parseControlUiSessionPath } from "@carapace/session-url-contract/parse";
...
export function serveControlUiShareDocument(
  ...
  const title = session.namespace === "dashboard" ? "Carapace dashboard" : "Carapace session";
  const description = "Open this shared link in Carapace. Access to the session is required.";
...
<meta property="og:image:alt" content="Carapace logo and lobster mascot">
...
<img src="${image}" width="1200" height="630" alt="Carapace logo and lobster mascot">
<h1>${title}</h1><p>${description}</p><a href="${target}">Open ${title === "Carapace dashboard" ? "dashboard" : "session"}</a>
</main></body></html>`;
  res.statusCode = 200;
