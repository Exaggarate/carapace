...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__carapace_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.CARAPACE_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (isVitestRuntimeEnv(env) ? env.CARAPACE_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP : undefined);
  if (!configured) {
