...
describe("formatGatewayAuthFailureMessage", () => {
  it.each(["bootstrap_token_invalid", undefined])(
    ...
    (reason) => {
      ...
      expect(message).toBe(
        "unauthorized: setup code invalid, expired, revoked, or already used (create a new code; review `carapace devices list`)",
      );
  ...
  it("makes a missing Control UI token actionable within the WebSocket close limit", () => {
    ...
    expect(message).toBe(
      "unauthorized: gateway token missing (paste in Control UI settings or carapace doctor --generate-gateway-token; restart)",
    );
    expect(truncateCloseReason(message)).toBe(message);
  });
  ...
  it("tells rejected node hosts how to diagnose identity-header auth", () => {
    expect(
      ...
      "gateway rejected this node: trusted-proxy identity-header authentication is required and no usable machine credential was accepted; run `carapace doctor` on the Gateway",
    );
  });
