...
const ROOT_RESPONSE_CASES: RootResponseCase[] = [
  ...
  },
  {
    path: "/root-failed",
    root: { kind: "failed" },
    body: "Control UI assets could not be prepared. Check the Gateway logs or run `carapace doctor --fix`.",
    missingHeaders: ["retry-after"],
  },
  {
    path: "/root-missing",
  ...
  },
  {
    ...
    root: { kind: "resolved", path: "/carapace-test/missing-control-ui-root" },
    body: "Control UI assets not found. Build them with `pnpm ui:build` (auto-installs UI deps), or run `pnpm ui:dev` during development.",
    missingHeaders: ["retry-after"],
  },
];
