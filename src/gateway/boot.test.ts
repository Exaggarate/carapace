...
describe("runBootOnce", () => {
  ...
  const resolveMainStore = (cfg: CarapaceConfig = testConfig()) => {
  ...
  const withBootWorkspace = async (
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-boot-"));
    try {
  ...
  it("wraps BOOT.md content in internal-runtime-context delimiters so verbatim echoes get stripped", async () => {
    ...
    expect(message).toContain("<<<BEGIN_CARAPACE_INTERNAL_CONTEXT>>>");
    expect(message).toContain("<<<END_CARAPACE_INTERNAL_CONTEXT>>>");
  ...
  it("escapes literal internal-runtime-context delimiters in user-supplied BOOT.md to prevent confusion with the wrapper", async () => {
    ...
    expect((message.match(/<<<BEGIN_CARAPACE_INTERNAL_CONTEXT>>>/g) ?? []).length).toBe(1);
    expect((message.match(/<<<END_CARAPACE_INTERNAL_CONTEXT>>>/g) ?? []).length).toBe(1);
    expect(message).toContain("[[CARAPACE_INTERNAL_CONTEXT_BEGIN]]");
    expect(message).toContain("[[CARAPACE_INTERNAL_CONTEXT_END]]");
  });
