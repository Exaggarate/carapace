...
const originalTrace = process.env.CARAPACE_GATEWAY_STARTUP_TRACE;

describe("gateway server boundary", () => {
  beforeEach(() => {
    vi.resetModules();
    process.env.CARAPACE_GATEWAY_STARTUP_TRACE = "1";
  });
  ...
  afterEach(() => {
    ...
    if (originalTrace === undefined) {
      delete process.env.CARAPACE_GATEWAY_STARTUP_TRACE;
    } else {
      process.env.CARAPACE_GATEWAY_STARTUP_TRACE = originalTrace;
    }
  });
