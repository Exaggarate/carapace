...
function stateDatabaseOptions(stateDir?: string): CarapaceStateDatabaseOptions {
  return stateDir
...
export function readManagedImageRecord(
  ...
  const database = openCarapaceStateDatabase(stateDatabaseOptions(stateDir));
...
export function listManagedImageRecordEntries(params: {
  ...
  const database = openCarapaceStateDatabase(stateDatabaseOptions(params.stateDir));
