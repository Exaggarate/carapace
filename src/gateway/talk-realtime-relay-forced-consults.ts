...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with Carapace.",
    "Do not answer the request yet. Wait for the Carapace result before giving the actual answer.",
  ].join(" ");
}
  ...
  session.harness.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking carapace_agent_consult, so Carapace is forcing the consult for realtime Talk.",
