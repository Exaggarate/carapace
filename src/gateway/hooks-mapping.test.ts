...
describe("hooks mapping", () => {
  ...
  it("validates sessionMode returned by hook transforms", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-hook-session-mode-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    fs.writeFileSync(
  ...
  it("runs transform module", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("treats transform-provided session keys as templated by default", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "carapace-config-sessionkey-xform-",
  ...
  it("uses transform-provided static session key source metadata", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "carapace-config-sessionkey-static-",
      sessionKey: "hook:gmail:{{messages[0].subject}}",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
  ...
  it("treats empty transform session keys as absent for source tracking", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "carapace-config-sessionkey-empty-",
      sessionKey: "hook:gmail:{{messages[0].subject}}",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
  ...
  it("defaults invalid transform session key source metadata to templated", async () => {
    const result = await applyGmailTransformSessionKey({
      tempPrefix: "carapace-config-sessionkey-invalid-",
      transformLines: [
        "export default () => ({",
        '  kind: "agent",',
        '  message: "Transformed",',
  ...
  it("rejects transform module traversal outside transformsDir", () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-traversal-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("rejects absolute transform module path outside transformsDir", () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-abs-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("rejects transformsDir traversal outside the transforms root", () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-xformdir-trav-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("rejects transformsDir absolute path outside the transforms root", () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-xformdir-abs-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    expect(() =>
      resolveHookMappings(
  ...
  it("accepts transformsDir subdirectory within the transforms root", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-xformdir-ok-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const configDir = makeTempDir(hooksTempDirs, "carapace-config-symlink-module-");
      const transformsRoot = path.join(configDir, "hooks", "transforms");
      fs.mkdirSync(transformsRoot, { recursive: true });
      const outsideDir = makeTempDir(hooksTempDirs, "carapace-outside-module-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const configDir = makeTempDir(hooksTempDirs, "carapace-config-symlink-dir-");
      const transformsRoot = path.join(configDir, "hooks", "transforms");
      fs.mkdirSync(transformsRoot, { recursive: true });
      const outsideDir = makeTempDir(hooksTempDirs, "carapace-outside-dir-");
  ...
  it.runIf(process.platform !== "win32")("accepts in-root transform module symlink", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-symlink-ok-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
  ...
  it("treats null transform as a handled skip", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-config-skip-");
  ...
  it("caches transform functions by module path and export name", async () => {
    const configDir = makeTempDir(hooksTempDirs, "carapace-hooks-export-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("uses one transform module instance per mapping reload", async () => {
    const configDir = autoCleanupTempDirs.make("carapace-hooks-generation-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    ...
    fs.writeFileSync(
      modPath,
      [
        "globalThis.__carapaceHookTransformInstance = (globalThis.__carapaceHookTransformInstance ?? 0) + 1;",
        "const instance = globalThis.__carapaceHookTransformInstance;",
  ...
  it("reloads a transform when the module file changes", async () => {
    const configDir = autoCleanupTempDirs.make("carapace-hooks-reload-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
  ...
  it("does not invalidate the active transform cache while resolving a rejected reload", async () => {
    const configDir = autoCleanupTempDirs.make("carapace-hooks-rejected-reload-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    const modPath = path.join(transformsRoot, "reloadable.mjs");
  ...
  it("does not let an older in-flight transform import repopulate the reload cache", async () => {
    const configDir = autoCleanupTempDirs.make("carapace-hooks-overlap-");
    const transformsRoot = path.join(configDir, "hooks", "transforms");
    fs.mkdirSync(transformsRoot, { recursive: true });
    const modPath = path.join(transformsRoot, "reloadable.mjs");
