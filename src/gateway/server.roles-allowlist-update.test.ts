...
const connectNodeClient = async (params: {
  ...
  if (!token) {
    throw new Error("CARAPACE_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("CARAPACE_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    ...
    try {
      const systemDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(
          os.tmpdir(),
          `carapace-node-system-run-${Date.now()}-${Math.random()}.sqlite`,
        ),
      });
      const emptyDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(os.tmpdir(), `carapace-node-empty-${Date.now()}-${Math.random()}.sqlite`),
      });
      const allowedDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(os.tmpdir(), `carapace-node-allowed-${Date.now()}-${Math.random()}.sqlite`),
      });
  ...
  test("rechecks current allowlist before exposing approved live commands", async () => {
    ...
    try {
      const deviceIdentity = createDeviceIdentityForTest("carapace-node-current-allowlist");
      nodeClient = await connectNodeClientWithPairing({
        port,
        commands: ["canvas.snapshot"],
        platform: "macos",
  ...
  test("records only allowlisted commands in pending node pairing requests", async () => {
    const deviceIdentity = createDeviceIdentityForTest("carapace-allowlisted-pending");
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    const deviceIdentity = createDeviceIdentityForTest("carapace-spoof-test-device");
  ...
  test("does not promote paired desktop client id changes into host command defaults", async () => {
    const deviceIdentity = createDeviceIdentityForTest("carapace-client-id-promotion");
  ...
  test("allows canonical node-host reconnect for legacy pinned platform metadata", async () => {
    const deviceIdentity = createDeviceIdentityForTest("carapace-node-host-platform-upgrade");
  ...
  test("filters system.run for confusable iOS metadata at connect time", async () => {
    const deviceIdentity = createDeviceIdentityForTest("carapace-confusable-node-greek-omicron");
