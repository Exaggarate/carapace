...
type CreateCarapaceCodingToolsArg = {
...
const hoisted = vi.hoisted(() => {
  ...
  return {
    ...
    createCarapaceCodingToolsMock: vi.fn(
      (_args: CreateCarapaceCodingToolsArg): ReturnType<typeof makeTool>[] => [],
    ),
    createCarapaceToolsMock: vi.fn((_args: CreateCarapaceToolsArg) => [
      makeTool("read"),
...
vi.mock("../agents/carapace-tools.js", () => ({
  createCarapaceTools: (args: CreateCarapaceToolsArg) => hoisted.createCarapaceToolsMock(args),
}));

vi.mock("../agents/agent-tools.js", () => ({
  createCarapaceCodingTools: (args: CreateCarapaceCodingToolsArg) =>
    hoisted.createCarapaceCodingToolsMock(args),
}));
...
describe("resolveGatewayScopedTools excludeToolNames", () => {
  beforeEach(() => {
    ...
    hoisted.createCarapaceCodingToolsMock.mockReset();
    hoisted.createCarapaceCodingToolsMock.mockReturnValue([]);
    hoisted.getLoadedChannelPluginMock.mockReset();
  });

  function readCreateToolsArgs(index = 0): CreateCarapaceToolsArg {
    const args = hoisted.createCarapaceToolsMock.mock.calls[index]?.[0];
    if (!args || typeof args !== "object") {
      throw new Error("expected createCarapaceTools args");
    }
    return args;
  }
  ...
  it("constructs exact coding tools for a server-minted mediated grant", () => {
    hoisted.createCarapaceCodingToolsMock.mockReturnValueOnce([hoisted.makeTool("write")]);

    const result = resolveGatewayScopedTools({
      cfg: { tools: { exec: { host: "node" } } } as CarapaceConfig,
  ...
  it("denies loopback tools after the scheduled owner account is removed", () => {
    ...
    hoisted.getLoadedChannelPluginMock.mockReturnValue({
      config: {
        listAccountIds: (cfg: CarapaceConfig) => Object.keys(cfg.channels?.discord?.accounts ?? {}),
      },
  ...
  it("does not fall back when policy removes a mediated coding tool", () => {
    hoisted.createCarapaceToolsMock.mockReturnValueOnce([
  ...
  it("omits all exec variants when host policy forbids node execution", () => {
    ...
    const sandboxAuto = resolveGatewayScopedTools({
      cfg: { agents: { defaults: { sandbox: { mode: "all" } } } } as CarapaceConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,
  ...
  it("filters node exec through the existing gateway deny policy", () => {
    const result = resolveGatewayScopedTools({
      cfg: { gateway: { tools: { deny: ["exec"] } } } as CarapaceConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,
  ...
  it("uses persisted delegated policy instead of the sender wildcard", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-gateway-delegated-policy-"));
  ...
  it("does not inherit node-only exec as a generic child or cron capability", () => {
    const result = resolveGatewayScopedTools({
      cfg: { tools: { allow: ["exec", "sessions_spawn", "automations"] } } as CarapaceConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,
