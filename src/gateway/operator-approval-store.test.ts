...
function createDatabaseOptions(): CarapaceStateDatabaseOptions {
  const stateDir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "carapace-operator-approval-")),
  );
...
function rawApprovalRow(options: CarapaceStateDatabaseOptions, id: string) {
...
describe("operator approval store", () => {
  ...
  it("reads the default clock after waiting for the SQLite write lock", async () => {
    ...
    const databasePath = openCarapaceStateDatabase(databaseOptions).path;
