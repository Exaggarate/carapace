...
\`\`\`json
{"label":"carapace-tui (gateway-client)","id":"gateway-client"}
\`\`\``;
...
describe("session titles with inbound Gateway metadata", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = tempDirs.make("carapace-session-title-inbound-");
  ...
  test("keeps a user-assigned session label ahead of the Gateway display name", () => {
    ...
    expect(deriveSessionTitle(entry, senderMetadata, "carapace-tui")).toBe(
