...
describe("Gateway GitHub publication boundaries", () => {
  ...
  it("refuses a matching pull request owned by another GitHub account", async () => {
    ...
    mocks.runCommand.mockImplementation(async (argv: string[], options?: { input?: string }) => {
      const command = argv.join(" ");
      if (command.includes(" repos/carapace/carapace/pulls ")) {
        return commandResult(
          JSON.stringify([
            {
              url: "https://github.com/Exaggarate/carapace/pull/foreign",
              userId: 99,
              state: "open",
              body: "",
              headSha: "b".repeat(40),
  ...
  it.each([
    ...
    const fallback = mocks.runCommand.getMockImplementation()!;
    mocks.runCommand.mockImplementation(async (argv: string[], options?: { input?: string }) => {
      if (argv.join(" ").includes(" repos/carapace/carapace/pulls ")) {
