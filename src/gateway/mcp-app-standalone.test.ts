...
describe("MCP App standalone host", () => {
  ...
  it("serves a hash-protected static shell without per-view data", async () => {
    const result = await request({ url: "/__carapace__/mcp-app" });
  ...
  it.each([
    { label: "public shell", path: "/__carapace__/mcp-app", expectedStatus: 200 },
    {
      label: "authenticated multibyte view",
      path: "/__carapace__/mcp-app/view",
      expectedStatus: 200,
      authorized: true,
    },
    {
  ...
  it("returns capabilities only for handlers installed on the live view", async () => {
    const issued = issueTicket({ sessionKey: "agent:main:main", view, nowMs, secret });
    const route = "/__carapace__/mcp-app/view";
  ...
  it("is path-scoped and rejects malformed operations", async () => {
    const issued = issueTicket({ sessionKey: "agent:main:main", view, nowMs, secret });
    expect((await request({ url: "/__carapace__/mcp-app", method: "POST" })).res.statusCode).toBe(
      404,
    );
    expect((await request({ url: "/__carapace__/mcp-app/other" })).handled).toBe(false);
    expect(
      (
        await request({
          url: "/__carapace__/mcp-app/view",
