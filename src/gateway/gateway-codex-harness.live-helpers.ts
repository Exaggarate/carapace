...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Carapace session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsCarapaceStatus =
    normalized.includes("carapace is running on") ||
    /carapace\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("carapace status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsCarapaceStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from carapace status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live carapace config shows") ||
