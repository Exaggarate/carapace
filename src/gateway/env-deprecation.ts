...
// Emits a one-shot notice for ignored pre-Carapace environment prefixes.
...
export function warnLegacyCarapaceEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but Carapace only reads CARAPACE_* names now.`,
      "Rename them by replacing the legacy prefix with CARAPACE_; the old names are ignored.",
    ].join("\n"),
    { code: "CARAPACE_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}
