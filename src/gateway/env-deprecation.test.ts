...
let warnLegacyCarapaceEnvVars: typeof import("./env-deprecation.js").warnLegacyCarapaceEnvVars;

describe("warnLegacyCarapaceEnvVars", () => {
  let envSnapshot: ReturnType<typeof captureEnv>;
  let emitWarning: ReturnType<typeof vi.spyOn>;

  beforeEach(async () => {
    vi.resetModules();
    ({ warnLegacyCarapaceEnvVars } = await import("./env-deprecation.js"));
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with CARAPACE_");
    ...
    expect(options).toEqual({
      code: "CARAPACE_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current CARAPACE names", () => {
    warnLegacyCarapaceEnvVars({ CARAPACE_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyCarapaceEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyCarapaceEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyCarapaceEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyCarapaceEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
