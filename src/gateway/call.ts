...
type CallGatewayBaseOptions = {
   ...
   * Bypasses CARAPACE_GATEWAY_URL and CARAPACE_GATEWAY_PORT for this call only.
   */
  localPortOverride?: number;
  /** Keep a caller-supplied config target authoritative over CARAPACE_GATEWAY_URL. */
...
const defaultGetRuntimeConfig = async (): Promise<CarapaceConfig> =>
...
async function loadGatewayConfig(): Promise<CarapaceConfig> {
 ...
 */
async function loadGatewayConfigForExplicitConnection(): Promise<CarapaceConfig> {
  try {
...
function loadGatewayConfigForConnectionDetails(): CarapaceConfig {
...
function resolveGatewayPortValue(config?: CarapaceConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: CarapaceConfig) {
...
function resolveGatewayCallTimeout(timeoutValue: unknown): {
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.CARAPACE_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(isVitestRuntimeEnv() && process.env.CARAPACE_TEST_HANDSHAKE_TIMEOUT_MS);
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `carapace doctor` for diagnostics.";
  }
  return message;
}
    ...
    if (!storedAuth?.token && deviceAuthScope) {
      throw new GatewayStoredDeviceAuthUnavailableError(
        [
          ...
          `Run \`carapace tui --url ${deviceAuthScope}\` to send a pairing request, approve it in that gateway's Control UI (Settings -> Devices) or run \`carapace devices approve --latest\` on the gateway host, then retry.`,
        ].join("\n"),
      );
    }
  }
