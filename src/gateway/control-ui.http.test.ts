...
describe("handleControlUiHttpRequest", () => {
  function createAvatarConfig(workspace: string, avatar: string): CarapaceConfig {
    return {
      agents: {
        defaults: { workspace },
  ...
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-"));
    try {
  ...
  async function runTrustedProxyAssistantMediaRequest(params: {
    ...
    return await runAssistantMediaRequest({
      url: `/__carapace__/assistant-media?${params.meta ? "meta=1&" : ""}source=${encodeURIComponent(params.filePath)}`,
      method: "GET",
  ...
  async function withAllowedAssistantMediaRoot<T>(params: {
    ...
    const tmpRoot = await fs.mkdtemp(path.join(resolvePreferredCarapaceTmpDir(), params.prefix));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-root-"));
    try {
  ...
  async function withPairedOperatorDeviceToken<T>(params: {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-device-token-"));
    try {
      return await withEnvAsync({ CARAPACE_HOME: tempHome }, async () => {
  ...
  async function withScopedPairedOperatorDevice<T>(params: {
    ...
    const tempHome = testTempDirs.make("carapace-ui-scoped-device-");
    return await withEnvAsync({ CARAPACE_HOME: tempHome }, async () => {
  ...
  it("sets security headers for Control UI responses", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        expect(responseBody(end)).toContain('data-carapace-terminal-enabled="true"');
      },
    });
  });
  ...
  it("uses effective terminal availability instead of raw restart-pending config", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        expect(responseBody(end)).toContain('data-carapace-terminal-enabled="false"');
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
    ...
    async (mutation) => {
      await withAllowedAssistantMediaRoot({
        ...
        fn: async (tmpRoot) => {
          ...
          const url = `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}`;
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?playback=1&source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
  ...
  it("caps long assistant media filenames in content disposition", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("fixture"));
        const { res, handled } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&token=t`,
          method: "GET",
  ...
  it("serves assistant media from canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled } = await runAssistantMediaRequest({
        url: `/__carapace__/assistant-media?source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("reports assistant media metadata for canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled, end } = await runAssistantMediaRequest({
        url: `/__carapace__/assistant-media?meta=1&source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("rejects assistant local media outside allowed preview roots", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-media-blocked-"));
    try {
  ...
  it("reports assistant local media availability metadata", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const meta = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
          headers: {
        ...
        const media = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        ...
        const rejected = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(shortenedTicket)}`,
          method: "GET",
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const refresh = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
  ...
  it("rejects assistant local media with an invalid scoped media ticket", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=v1.invalid.invalid`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
  ...
  it("reports assistant local media availability failures with a reason", async () => {
    const { res, handled, end } = await runAssistantMediaRequest({
      url: `/__carapace__/assistant-media?meta=1&source=${encodeURIComponent("/Users/test/Documents/private.pdf")}&token=test-token`,
      method: "GET",
      auth: { mode: "token", token: "test-token", allowTailscale: false },
    });
    expect(handled).toBe(true);
  ...
  it("rejects assistant local media without a valid auth token when auth is enabled", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
      ...
      fn: async (operatorToken) => {
        await withAllowedAssistantMediaRoot({
          ...
          fn: async (tmpRoot) => {
            ...
            await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
            const { res, handled } = await runAssistantMediaRequest({
              url: `/__carapace__/assistant-media?source=${encodeURIComponent(filePath)}&token=${encodeURIComponent(operatorToken)}`,
              method: "GET",
              auth: { mode: "token", token: "shared-token", allowTailscale: false },
            });
            expect(handled).toBe(true);
    ...
    await withControlUiRoot({
      ...
      fn: async (tmp) => {
        ...
        expect(end).toHaveBeenCalledWith(
          html.replace(
            "<html",
            '<html data-carapace-control-ui-base-path="" data-carapace-terminal-enabled="true"',
          ),
        );
      },
    });
    ...
    await withControlUiRoot({
      ...
      fn: async (tmp) => {
        ...
        expect(responseBody(documentResponse.end)).toContain(
          'data-carapace-environment="{&quot;label&quot;:&quot;edge &amp; team&quot;,&quot;color&quot;:&quot;amber&quot;}"',
        );
  ...
  it("rewrites public asset hrefs in index.html when Control UI uses a configured base path (#94157)", async () => {
    ...
    await withControlUiRoot({
      ...
      fn: async (tmp) => {
        ...
        expect(body).toContain('data-carapace-control-ui-base-path="/carapace"');
        expect(body).toContain('href="/carapace/manifest.webmanifest"');
        expect(body).toContain('href="/carapace/favicon.svg"');
  ...
  it.each([
    ...
    },
    {
      ...
      requestPath: "/carapace/focus/desktop/control",
      basePath: "/carapace",
      expectedResourceBasePath: "/carapace",
    },
    {
    ...
    },
    {
      ...
      requestPath: "/__carapace__/new",
      basePath: undefined,
      expectedResourceBasePath: "",
    },
    {
      name: "base-mounted ordinary deep routes",
      requestPath: "/carapace/settings/approvals",
      basePath: "/carapace",
      expectedResourceBasePath: "/carapace",
    },
  ])(
      ...
      await withControlUiRoot({
        ...
        fn: async (tmp) => {
          ...
          expect(body).toContain(
            `data-carapace-control-ui-base-path="${expectedResourceBasePath}"`,
          );
  ...
  it("penalizes both credential scopes when a Control UI read token is invalid", async () => {
    const tempHome = testTempDirs.make("carapace-ui-invalid-token-");
    await withEnvAsync({ CARAPACE_HOME: tempHome }, async () => {
      await withControlUiRoot({
        fn: async (tmp) => {
          const rateLimiter = createAuthRateLimiterSpy();
          const { res, handled, end } = await runBootstrapConfigRequest({
  ...
  it("rejects a rate-limited Control UI read when no valid device token is presented", async () => {
    const tempHome = testTempDirs.make("carapace-ui-rate-limited-token-");
    await withEnvAsync({ CARAPACE_HOME: tempHome }, async () => {
      await withControlUiRoot({
        fn: async (tmp) => {
          const rateLimiter = createAuthRateLimiterSpy();
      ...
      fn: async (tmp) => {
        ...
        expect(
          cookieNames.every((name) =>
            /^__carapace_plugin_tab_auth_[0-9a-f]{16}_[0-9a-f]{64}$/.test(name),
          ),
        ).toBe(true);
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          { url: `/carapace${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/carapace",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/carapace");
  ...
  it("serves bootstrap config under the configured /__carapace__ basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/__carapace__/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
            basePath: "/__carapace__",
            root: { kind: "resolved", path: tmp },
            config: {
              agents: {
                defaults: { workspace: tmp },
        ...
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/__carapace__");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  // so the SPA is served at the default `/__carapace__/` namespace. The browser opens
  // the default entry, `inferBasePathFromPathname("/__carapace__/")` yields `/__carapace__`,
  // and the loader fetches `/__carapace__/control-ui-config.json`. Before this fix the
  ...
  it("serves bootstrap config at the default /__carapace__ entry with no configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
  ...
  // single-underscore `/__carapace/control-ui-config.json` endpoint under an empty
  ...
  // vacuous.
  it("still serves bootstrap config at the legacy /__carapace/control-ui-config.json with no configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/__carapace/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
  ...
  // `gateway.controlUi.basePath` is set (e.g. `/carapace`), current main and
  // v2026.6.1 serve the bootstrap config at `${basePath}/__carapace/control-ui-config.json`
  ...
  it("still serves bootstrap config at the legacy ${basePath}/__carapace/control-ui-config.json under a configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/carapace/__carapace/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
  ...
  it("does not serve bootstrap config from the doubled /__carapace__/__carapace path (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end, handled } = await runControlUiRequest({
          url: "/__carapace__/__carapace/control-ui-config.json",
          method: "GET",
          rootPath: tmp,
        });
        expectNotFoundResponse({ handled, res, end });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = testTempDirs.make("carapace-avatar-http-");
    try {
  ...
  it.each([
    ...
    async ({ contentType, filename }) => {
      const tmp = testTempDirs.make("carapace-avatar-head-metadata-");
  ...
  it.each(["", "/carapace"])(
    ...
    async (basePath) => {
      const tmp = testTempDirs.make("carapace-avatar-head-base-");
  ...
  it("does not expose avatar HEAD representation length before authentication", async () => {
    const tmp = testTempDirs.make("carapace-avatar-head-unauthorized-");
    try {
  ...
  it.each([
    ...
    async (_name, url, method) => {
      const tmp = testTempDirs.make("carapace-avatar-no-read-");
      const read = vi.spyOn(fsSync, "read");
      const closeSync = vi.spyOn(fsSync, "closeSync");
      try {
        await fs.writeFile(path.join(tmp, "main.png"), REAL_PNG);
  ...
  it("rejects hardlinked avatar bytes and reports matching metadata", async () => {
    const tmp = testTempDirs.make("carapace-avatar-http-hardlink-");
    try {
      await fs.writeFile(path.join(tmp, "original.png"), REAL_PNG);
      await fs.link(path.join(tmp, "original.png"), path.join(tmp, "avatar.png"));
      const config = createAvatarConfig(tmp, "avatar.png");
  ...
  it("bounds an avatar route file that grows after its descriptor is pinned", async () => {
    const tmp = testTempDirs.make("carapace-avatar-http-growth-");
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-avatar-http-outside-"));
    try {
  ...
  it("serves local avatar bytes when auth is enabled and the token is valid", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-avatar-auth-"));
    try {
      const avatarPath = path.join(tmp, "main.png");
      await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("serves local avatar bytes when paired device-token auth is valid", async () => {
    await withPairedOperatorDeviceToken({
      fn: async (operatorToken) => {
        const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-avatar-device-token-"));
        try {
          const avatarPath = path.join(tmp, "main.png");
          await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-outside-"));
        try {
  ...
  it("serves a missing bundled asset from an exact retained generation", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const retainedRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-retained-"));
        try {
  ...
  it.each([
    ...
    },
    {
      ...
      basePath: "/carapace",
      url: "/carapace/approve/Approval%3AMobile%2F%E6%9D%B1%E4%BA%AC%20100%25%20%F0%9F%A6%9E",
    },
    {
    ...
    },
    {
      ...
      basePath: "/carapace",
      url: "/carapace/approve/plugin%3Arequest.js",
    },
    {
    ...
    },
    {
      ...
      basePath: "/carapace",
      url: "/carapace/focus/desktop/control/session/agent%3Amain%3Amain",
    },
      ...
      fn: async (tmp) => {
        for (const method of ["GET", "HEAD"] as const) {
          ...
          if (method === "HEAD") {
            ...
            if (basePath) {
              expect(responseBody(end)).toContain('data-carapace-control-ui-base-path="/carapace"');
            }
          }
        }
      },
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-index-outside-"));
        try {
  ...
  it("rejects hardlinked index.html for non-package control-ui roots", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ui-index-hardlink-"));
        try {
          const outsideIndex = path.join(outsideDir, "index.html");
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        for (const [url, expectedType] of [
          ["/__carapace__/favicon.svg", "image/svg+xml"],
          ["/__carapace__/manifest.webmanifest", "application/manifest+json; charset=utf-8"],
          ["/__carapace__/apple-touch-icon.png", "image/png"],
          ["/__carapace__/sw.js", "application/javascript; charset=utf-8"],
        ] as const) {
          const { res, end, handled } = await runControlUiRequest({
            url,
            method: "GET",
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = await handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/carapace", root: { kind: "resolved", path: tmp } },
        );
        expect(handled).toBe(false);
      },
    });
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/carapace", "/carapace/", "/carapace/some-page"]) {
          const { handled, end } = await runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
