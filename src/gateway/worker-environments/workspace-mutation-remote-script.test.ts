...
describe("remote workspace mutation receiver script", () => {
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = tempDirs.make("carapace-workspace-receiver-lock-");
      ...
      const relative = ".carapace-worker/workspaces/env/session/1";
      ...
      await fs.writeFile(
        path.join(bin, "rsync"),
        '#!/bin/sh\nset -eu\n( : > "$CARAPACE_TEST_RECEIVER_MARKER"; read -r _ < "$CARAPACE_TEST_RECEIVER_GATE"; printf "late\\n" > "$CARAPACE_TEST_RECEIVER_WORKSPACE/late.txt" ) </dev/null >/dev/null 2>&1 &\nexit 0\n',
        { mode: 0o755 },
      );
      await fs.writeFile(
        preload,
...
process.kill = function(pid, signal) {
  if (signal === 0 && pid < 0 && process.argv[4] === process.env.CARAPACE_TEST_RESET_NONCE) {
...
`,
      ...
      const env = {
        ...
        CARAPACE_TEST_RECEIVER_GATE: gate,
        CARAPACE_TEST_RECEIVER_MARKER: receiverMarker,
        CARAPACE_TEST_RECEIVER_WORKSPACE: workspace,
        CARAPACE_TEST_RESET_NONCE: resetNonce,
      ...
      try {
        ...
        const lock = path.join(path.dirname(workspace), `.carapace-accepted-lock-${workspaceKey}`);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = tempDirs.make("carapace-workspace-receiver-controller-lock-");
      let home = path.join(root, "home");
      const bin = path.join(root, "bin");
...
fs.renameSync = function(source, destination) {
  if (
    process.argv[4] === process.env.CARAPACE_TEST_RECEIVER_NONCE &&
    destination.includes(".released.")
  ) {
    fs.writeFileSync(process.env.CARAPACE_TEST_RELEASE_MARKER, "");
    if (fs.readFileSync(process.env.CARAPACE_TEST_RELEASE_GATE, "utf8").trim() !== "release") {
...
process.kill = function(pid, signal) {
  ...
  if (
    signal === 0 &&
    process.argv[4] === process.env.CARAPACE_TEST_RESET_NONCE &&
    pid === Number(process.env.CARAPACE_TEST_CONTROLLER_PID)
  ) {
    fs.writeFileSync(process.env.CARAPACE_TEST_CONTENDER_MARKER, "");
  }
  return result;
...
`,
      ...
      const env = {
        ...
        HOME: home,
        PATH: `${bin}:${process.env.PATH ?? ""}`,
        CARAPACE_TEST_RECEIVER_NONCE: receiverNonce,
        CARAPACE_TEST_RESET_NONCE: resetNonce,
        CARAPACE_TEST_RELEASE_GATE: releaseGate,
        CARAPACE_TEST_RELEASE_MARKER: releaseMarker,
        CARAPACE_TEST_CONTENDER_MARKER: contenderMarker,
      };
      const receiverCommand = createWorkerWorkspaceRsyncReceiverPathFactory({
        receiverEntryPath: workerWorkspaceRsyncReceiverEntryPath(BUNDLE_HASH),
      ...
      try {
        ...
        reset = runCommandWithTimeout(
          ...
          {
            timeoutMs: 10_000,
            baseEnv: { ...env, CARAPACE_TEST_CONTROLLER_PID: String(owner.controllerPid) },
          },
        );
        void reset.then(
          () => {
