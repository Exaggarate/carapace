...
it("rejects a full workspace above 4 GiB before hashing its files", async () => {
  const { home, workspace, baseCommit } = await gitWorkspace("carapace-manifest-byte-budget");
...
it("rejects prior manifests above the full-inventory entry limit", async () => {
  const { home, workspace, baseCommit } = await gitWorkspace("carapace-manifest-entry-budget");
...
it("budgets raw Git candidates separately from materialized eligible inventory", async () => {
  ...
  const { home, workspace, baseCommit } = await gitWorkspace("carapace-raw-git-candidate-budget");
  ...
  await fs.writeFile(
    mockGit,
    `#!/usr/bin/env node
const count = Number(process.env.CARAPACE_TEST_GIT_CANDIDATES);
...
`,
  ...
  const baseEnv = {
    ...
    CARAPACE_TEST_GIT_CANDIDATES: String(MAX_WORKSPACE_INVENTORY_ENTRIES + 1),
  };
  ...
  const rejected = await runCommandWithTimeout(
    [process.execPath, "-e", REMOTE_WORKSPACE_MANIFEST_JS, workspace, baseCommit, "eligible"],
    {
      ...
      baseEnv: {
        ...baseEnv,
        CARAPACE_TEST_GIT_CANDIDATES: String(MAX_WORKSPACE_GIT_CANDIDATES + 1),
      },
    },
  );
