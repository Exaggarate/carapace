...
export function isUpdateDoctorRun(
  ...
  const value = env.CARAPACE_UPDATE_IN_PROGRESS;
  return value === "1" || value === "true";
}

export function resolveDoctorMode(cfg: CarapaceConfig): "local" | "remote" {
  return cfg.gateway?.mode === "remote" ? "remote" : "local";
}
export function resolveDoctorWorkspaceDir(cfg: CarapaceConfig, env = process.env) {
