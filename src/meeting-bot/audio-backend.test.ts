...
describe("meeting audio backend", () => {
  ...
  it("builds PipeWire-Pulse raw command pairs", () => {
    ...
    expect(runtime).toMatchObject({
      ...
      deviceLabel: "Carapace Meeting Audio",
    });
  ...
  it("idempotently provisions a PipeWire-Pulse null sink", async () => {
    ...
    expect(calls).toContainEqual([
      ...
      `sink_properties='device.description="Carapace Meeting Audio"'`,
    ]);
    expect(calls).toContainEqual([
      "pactl",
      "load-module",
      ...
      "channel_map=front-left,front-right",
      "remix=no",
      `source_properties='device.description="Carapace Meeting Audio"'`,
    ]);
  });
