...
describe("configured meeting node host", () => {
  ...
  it("provisions PipeWire-Pulse and probes node-local default commands", async () => {
    ...
    childProcessMocks.spawnSync.mockImplementation((command: string, args: string[]) => {
      ...
      if (args[0] === "list" && args[2] === "sinks") {
        return {
          status: 0,
          stdout: sinkLoaded ? "42\tcarapace_meeting_audio\tPipeWire" : "",
          stderr: "",
        };
      }
      if (args[0] === "list" && args[2] === "sources") {
        return {
          status: 0,
          stdout: sourceLoaded ? "43\tcarapace_meeting_audio\tPipeWire" : "",
          stderr: "",
        };
      }
      return { status: 0, stdout: "", stderr: "" };
    ...
    expect(result).toEqual({
      ...
      audioDeviceLabel: "Carapace Meeting Audio",
    });
  ...
  it("probes explicit command overrides after provisioning the backend", async () => {
    ...
    childProcessMocks.spawnSync.mockImplementation((command: string, args: string[]) => {
      ...
      if (args[0] === "list" && args[2] === "sinks") {
        return { status: 0, stdout: "42\tcarapace_meeting_audio\tPipeWire", stderr: "" };
      }
      if (args[0] === "list" && args[2] === "sources") {
        return { status: 0, stdout: "43\tcarapace_meeting_audio\tPipeWire", stderr: "" };
      }
      return { status: 0, stdout: "", stderr: "" };
    });
