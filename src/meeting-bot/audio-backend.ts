...
const PIPEWIRE_SINK_NAME = "carapace_meeting_audio";
const PIPEWIRE_MONITOR_NAME = `${PIPEWIRE_SINK_NAME}.monitor`;
const PIPEWIRE_SOURCE_NAME = PIPEWIRE_SINK_NAME;
const PIPEWIRE_MEETING_AUDIO_DEVICE_LABEL = "Carapace Meeting Audio";
...
export async function ensureMeetingAudioBackend(params: {
  ...
  if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the Carapace PipeWire-Pulse sink");
      }
    }
  }
  ...
  if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the Carapace PipeWire-Pulse source");
      }
    }
  }
  sinks = await params.run(["pactl", "list", "short", "sinks"], params.timeoutMs);
  ...
  if (
    ...
    throw new Error("Carapace PipeWire-Pulse sink or monitor source was not created.");
  }
}
