...
beforeAll(() => {
  sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-run-plan-fixtures-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "carapace-node-import-inline-equals-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "carapace-ruby-require-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "carapace-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "carapace-perl-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "carapace-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "carapace-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "carapace-pnpm-dlx-unknown-prefix-value-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32")("handles approval hardening cases", () => {
    for (const testCase of cases) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("carapace-approval-hardening-");
  ...
  it("captures mutable runtime operands in approval plans", () => {
    const tmp = createFixtureDir("carapace-approval-script-plan-");
    withFakeRuntimeBins({
  ...
  it("captures mutable shell script operands in approval plans", () => {
    withScriptOperandPlanFixture(
      {
        tmpPrefix: "carapace-approval-script-plan-",
      },
      (fixture, tmp) => {
        expectMutableFileOperandApprovalPlan(fixture, tmp);
      },
  ...
  it("keeps fail-closed behavior for relative native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("carapace-shell-relative-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for writable absolute native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("carapace-shell-absolute-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for owner-controlled read-only absolute binaries", () => {
    ...
    const tmp = createFixtureDir("carapace-shell-owned-readonly-binding-");
  ...
  it("keeps fail-closed behavior for symlinked binaries with writable targets", () => {
    ...
    const tmp = createFixtureDir("carapace-shell-symlink-binary-binding-");
  ...
  it("keeps fail-closed behavior for mutable or ambiguous shell payload files", () => {
    for (const testCase of [
      {
        tmpPrefix: "carapace-shell-script-binding-",
        fileName: "run.sh",
        body: "#!/bin/sh\necho SAFE\n",
      },
      {
        tmpPrefix: "carapace-shell-empty-binding-",
        fileName: "empty",
        body: "",
      },
      {
        tmpPrefix: "carapace-shell-mz-text-binding-",
        fileName: "mz-script",
        body: "MZ not really a PE file\n",
      },
      {
        tmpPrefix: "carapace-shell-nul-header-binding-",
        fileName: "nul-script",
  ...
  it("keeps fail-closed behavior when the shell payload probe stops seeing a file", () => {
    ...
    const tmp = createFixtureDir("carapace-shell-race-binding-");
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "carapace-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = createFixtureDir("carapace-pnpm-dlx-shell-mode-");
        fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
        expect(
          resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind mutable local files", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const casesResult = [
          {
            prefix: "carapace-pnpm-dlx-package-bin-",
            command: ["pnpm", "dlx", "cowsay", "hello"],
          },
          {
            prefix: "carapace-pnpm-dlx-package-runtime-token-",
            command: ["pnpm", "dlx", "cowsay", "node"],
          },
          {
            prefix: "carapace-pnpm-dlx-package-runtime-token-multi-",
            command: ["pnpm", "dlx", "cowsay", "node", "hello"],
          },
          {
            prefix: "carapace-pnpm-dlx-package-file-",
          ...
          {
            prefix: "carapace-pnpm-dlx-package-data-tail-",
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "carapace-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    ...
    for (const testCase of casesValue) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("carapace-shell-option-value-");
        const scriptPath = path.join(tmp, "run.sh");
        fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
  ...
  it("does not bind opaque shell command operands as mutable script files", () => {
    const tmp = createFixtureDir("carapace-opaque-shell-operand-");
    const scriptPath = path.join(tmp, "run.sh");
    fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(scriptPath, 0o755);
    const snapshot = resolveMutableFileOperandSnapshotSync({
  ...
  it("denies opaque shell inline payloads hidden by startup options", () => {
    const tmp = createFixtureDir("carapace-opaque-shell-hidden-inline-");
  ...
  it("denies nushell execute payloads hidden by startup modes", () => {
    const tmp = createFixtureDir("carapace-nu-startup-execute-");
    const scriptPath = path.join(tmp, "run.sh");
    fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(scriptPath, 0o755);
  ...
  it("captures fish script operands with plus-prefixed filenames", () => {
    ...
    for (const testCase of casesLocal) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("carapace-fish-plus-script-");
