...
export function snapshotNodeWorkerEnv(source: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  const hostCacheFenced =
    ...
    source.CARAPACE_SERVICE_KIND === NODE_SERVICE_KIND &&
    source.CARAPACE_LAUNCHD_LABEL === resolveNodeLaunchAgentLabel();
  ...
  if (!workerCacheDisabled) {
    ...
    snapshot.NODE_COMPILE_CACHE =
      requestedCache || path.join(resolvePreferredCarapaceTmpDir(), "node-worker-compile-cache");
  } else {
  ...
  snapshot.CARAPACE_NO_RESPAWN = "1";
  return snapshot;
}
