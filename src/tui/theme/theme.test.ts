...
type ThemeEnvOverrides = {
  CARAPACE_THEME?: string | undefined;
...
async function importThemeWithEnv(env: ThemeEnvOverrides) {
  if (Object.hasOwn(env, "CARAPACE_THEME")) {
    vi.stubEnv("CARAPACE_THEME", env.CARAPACE_THEME);
  }
...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      CARAPACE_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when CARAPACE_THEME=light", async () => {
    const mod = await importThemeWithEnv({ CARAPACE_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when CARAPACE_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ CARAPACE_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats CARAPACE_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ CARAPACE_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("CARAPACE_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      CARAPACE_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ CARAPACE_THEME: "light" });
    const darkMod = await importThemeWithEnv({ CARAPACE_THEME: "dark" });
