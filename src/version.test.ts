...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "carapace-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "carapace", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "carapace", version: "2.3.4" });
  ...
  it("ignores non-carapace package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers CARAPACE_VERSION over package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CARAPACE_VERSION: "9.9.9",
  ...
  it("prefers runtime VERSION over stale CARAPACE_VERSION for compatibility checks", () => {
    const previousCompatibility = process.env.CARAPACE_COMPATIBILITY_HOST_VERSION;
    const previous = process.env.CARAPACE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      delete process.env.CARAPACE_COMPATIBILITY_HOST_VERSION;
      process.env.CARAPACE_VERSION = "2026.3.25";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("CARAPACE_COMPATIBILITY_HOST_VERSION", previousCompatibility);
      restoreEnvValue("CARAPACE_VERSION", previous);
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        CARAPACE_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        CARAPACE_VERSION: "2026.3.99",
        npm_package_version: "2026.3.97",
      }),
    ).toBe("2026.4.8");
