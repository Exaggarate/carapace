...
beforeEach(() => {
  envSnapshot = captureEnv(["CARAPACE_TEST_FILE_LOG", "CARAPACE_LOG_LEVEL"]);
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    logging.setLoggerConfigLoaderForTests(() => ({
      level: "debug",
      file: "/tmp/carapace-configured.log",
      maxFileBytes: 2048,
    }));
    ...
    expect(settings.file).toBe("/tmp/carapace-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^carapace-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/carapace/carapace-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
