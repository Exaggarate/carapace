...
const logPathTracker = createSuiteLogPathTracker("carapace-test-env-log-level-");

describe("CARAPACE_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CARAPACE_LOG_LEVEL"]);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.CARAPACE_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("CARAPACE_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid CARAPACE_LOG_LEVEL="nope"');
  });
  ...
  it("structures invalid env warnings for JSON console output", () => {
    ...
    const warning = stderrSpy.mock.calls
      .map(([firstArg]) => String(firstArg))
      .find((line) => line.includes("CARAPACE_LOG_LEVEL"));
    expect(JSON.parse(warning ?? "")).toMatchObject({
      level: "warn",
      message: expect.stringContaining('Ignoring invalid CARAPACE_LOG_LEVEL="nope"'),
    });
  });
});
