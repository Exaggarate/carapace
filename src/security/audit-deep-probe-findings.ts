...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("carapace status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("carapace security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set CARAPACE_GATEWAY_TOKEN/CARAPACE_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("carapace security audit --deep")}".`,
    });
  }
  return findings;
}
