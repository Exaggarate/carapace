...
describe("security audit exec surface findings", () => {
  // Redirect the Carapace home (CARAPACE_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.carapace/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-exec-approvals-"));
  });

  beforeEach(async () => {
    envSnapshot = captureEnv(["CARAPACE_HOME", "HOME", "USERPROFILE"]);
    const tempDir = path.join(tempRoot, `case-${++tempCaseIndex}`);
    await fs.mkdir(path.join(tempDir, ".carapace"), { recursive: true });
    // CARAPACE_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.CARAPACE_HOME = tempDir;
