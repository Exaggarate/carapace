...
describe("security audit config include permissions", () => {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = tempDirs.make("carapace-include-perms-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = tempDirs.make("carapace-include-perms-allowed-");
      ...
      const configSnapshot: ConfigFileSnapshot = {
        path: path.join(configDir, "carapace.json"),
        exists: true,
      ...
      const findings = await collectIncludeFilePermFindings({
        configSnapshot,
        env: { CARAPACE_INCLUDE_ROOTS: sharedDir },
        platform: "linux",
      });
