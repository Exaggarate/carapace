...
import type { CarapaceConfig, SecurityConfig } from "../config/types.carapace.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "carapace" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load Carapace config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    carapaceVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
