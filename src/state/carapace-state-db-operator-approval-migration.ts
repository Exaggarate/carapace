...
export function assertCanonicalOperatorApprovalKinds(db: DatabaseSync, pathname: string): void {
  if (!hasCanonicalOperatorApprovalKinds(db)) {
    throw new Error(
      `Carapace state database ${pathname} has a legacy operator approval schema; run carapace doctor --fix to migrate it.`,
    );
  }
}
...
function canonicalOperatorApprovalCreateSql(): string {
  ...
  const start = CARAPACE_STATE_SCHEMA_SQL.indexOf(marker);
  const end = CARAPACE_STATE_SCHEMA_SQL.indexOf(
  ...
  return CARAPACE_STATE_SCHEMA_SQL.slice(start, end + tableTerminator.length);
}
...
function operatorApprovalIndexSql(): string {
  const statements = CARAPACE_STATE_SCHEMA_SQL.split(";")
