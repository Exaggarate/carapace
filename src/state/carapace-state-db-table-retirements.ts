...
function assertRecognizedRetiredCommitmentsSchema(db: DatabaseSync): void {
  ...
  assertSqliteSchemaContains(
    db,
    "retired Carapace commitments schema",
    RETIRED_COMMITMENTS_SCHEMA_SQL,
    RETIRED_COMMITMENTS_SCHEMA_COMPATIBILITY,
  );
  throw new Error(
    "Retired Carapace commitments schema has unsupported additional indexes; refusing destructive migration.",
  );
}
...
function assertNoRetiredCommitmentsForeignKeys(db: DatabaseSync): void {
  ...
  for (const table of tables) {
    ...
    if (
      ...
      throw new Error(
        `Retired Carapace commitments schema is referenced by table ${table.name}; refusing destructive migration.`,
      );
    }
  }
}
...
function assertNoRetiredCommitmentsSchemaDependencies(db: DatabaseSync): void {
  const probeTable = "__carapace_retired_commitments_probe";
  if (tableExists(db, probeTable)) {
    throw new Error(
      `Carapace state database already contains ${probeTable}; refusing destructive migration.`,
    );
  }
  const before = collectRetainedSchemaSql(db);
  const savepoint = "carapace_probe_commitments_dependencies";
  ...
  if (changedObject) {
    ...
    throw new Error(
      `Retired Carapace commitments schema is referenced by ${type} ${name}; refusing destructive migration.`,
    );
  }
}
...
function migrateRetiredCommitmentsSchema(db: DatabaseSync, previousVersion: number): boolean {
  ...
  const savepoint = "carapace_retire_commitments_v7";
  db.exec(`SAVEPOINT ${savepoint};`);
  try {
