...
import { assertCarapaceStateDatabaseForMaintenance } from "./carapace-state-db.js";
import { CARAPACE_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY } from "./carapace-state-schema-compatibility.js";
import { CARAPACE_STATE_SCHEMA_SQL } from "./carapace-state-schema.js";

describe("Carapace database maintenance schema validation", () => {
  it("accepts the current global and agent schemas", () => {
    ...
    try {
      expect(() =>
        assertCarapaceStateDatabaseForMaintenance(globalDatabase, {
  ...
  it("accepts a global schema produced by an additive column migration", () => {
    const schemaWithoutMigratedColumn = CARAPACE_STATE_SCHEMA_SQL.replace(
    ...
    try {
      ...
      expect(() =>
        assertCarapaceStateDatabaseForMaintenance(database, {
          pathname: "global.sqlite",
        }),
      ).not.toThrow();
    } finally {
  ...
  it("keeps a newer nullable shared-state column compatible with the previous schema", () => {
    ...
    try {
      expect(previousSchema).not.toBe(CARAPACE_STATE_SCHEMA_SQL);
      expect(() =>
...
`;
    const previousSchema = CARAPACE_STATE_SCHEMA_SQL.replace(additiveSchema, "");
    const database = createGlobalDatabase();
    try {
      expect(previousSchema).not.toBe(CARAPACE_STATE_SCHEMA_SQL);
      expect(() =>
  ...
  it("keeps the cron authority companion table compatible with the previous schema", () => {
    ...
    const previousSchema = `${CARAPACE_STATE_SCHEMA_SQL.slice(
      0,
      start,
    )}${CARAPACE_STATE_SCHEMA_SQL.slice(end + endMarker.length)}`;
    const database = createGlobalDatabase();
    try {
      expect(() =>
        assertSqliteSchemaContains(database, "previous global schema", previousSchema),
  ...
  it("keeps lifecycle bindings additive and keyed only by canonical owner identity", () => {
    ...
    expect(start).toBeGreaterThanOrEqual(0);
    expect(end).toBeGreaterThan(start);
    const previousSchema = `${CARAPACE_STATE_SCHEMA_SQL.slice(0, start)}${CARAPACE_STATE_SCHEMA_SQL.slice(end + endMarker.length)}`;
    const database = createGlobalDatabase();
    try {
      expect(() =>
        assertSqliteSchemaContains(database, "previous global schema", previousSchema),
  ...
  it("accepts the historical checked shared-host column but rejects other constraints", () => {
    const historicalSchema = CARAPACE_STATE_SCHEMA_SQL.replace(
      "  shared_host INTEGER\n) STRICT;",
      "  shared_host INTEGER CHECK (shared_host IN (0, 1))\n) STRICT;",
    );
    const database = createGlobalDatabase(historicalSchema);
    try {
      expect(historicalSchema).not.toBe(CARAPACE_STATE_SCHEMA_SQL);
      expect(() =>
        assertCarapaceStateDatabaseForMaintenance(database, {
          pathname: "global.sqlite",
        }),
  ...
  it("keeps every same-version additive column bare and canonical", () => {
    ...
    expect(CARAPACE_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY.allowedMissingColumns).toEqual(
      additiveColumns,
    );
    expect(
...
function createGlobalDatabase(schemaSql = CARAPACE_STATE_SCHEMA_SQL): DatabaseSync {
...
function createAgentDatabase(): DatabaseSync {
  ...
  database.exec(`PRAGMA user_version = ${CARAPACE_AGENT_SCHEMA_VERSION};`);
  database
    .prepare(
      `
        INSERT INTO schema_meta (
    ...
    .run(CARAPACE_AGENT_SCHEMA_VERSION);
  return database;
}
