...
export type CarapaceExternalStateOwnership = {
  claimedAt: number;
  managerId: string;
  mode: "external";
  version: 1;
...
export class CarapaceStateOwnershipError extends Error {}

export function isCarapaceStateWriteContentionError(error: unknown): boolean {
  return error instanceof StateDatabaseCoordinatorContentionError || isSqliteLockError(error);
}

export class CarapaceStateOwnershipMetadataError extends CarapaceStateOwnershipError {
  constructor(
    readonly databasePath: string,
    message: string,
  ) {
    super(
      `Carapace shared state ownership metadata is invalid at ${databasePath}: ${message}. ` +
        "Repair it with CARAPACE_SUPERVISOR_MODE=external carapace database ownership claim --manager <manager-id>.",
    );
    this.name = "CarapaceStateOwnershipMetadataError";
  }
}

class CarapaceStateExternalOwnershipError extends CarapaceStateOwnershipError {
  constructor(
    readonly databasePath: string,
    readonly managerId: string,
  ) {
    super(
      `Carapace shared state database ${databasePath} is externally supervised by ${managerId}. ` +
        "Use that external supervisor with CARAPACE_SUPERVISOR_MODE=external for writable operations.",
    );
    this.name = "CarapaceStateExternalOwnershipError";
  }
}

export function normalizeCarapaceStateManagerId(managerId: string): string {
...
function parseExternalOwnership(
  ...
  try {
    ...
    throw new CarapaceStateOwnershipMetadataError(databasePath, "reserved value is not valid JSON");
  }
  ...
  if (
    ...
    throw new CarapaceStateOwnershipMetadataError(
      databasePath,
...
export function inspectCarapaceStateOwnershipFromDatabase(
  database: DatabaseSync,
  databasePath: string,
  configMachineStateTableReady = false,
): CarapaceExternalStateOwnership | null {
  ...
  if (typeof row.value_json !== "string") {
    throw new CarapaceStateOwnershipMetadataError(databasePath, "reserved value is not text");
  }
...
function inspectOwnershipThroughConnection(
  ...
  try {
    ...
    return inspectCarapaceStateOwnershipFromDatabase(database, databasePath);
  } finally {
    database.close();
  }
}
...
function inspectOwnershipWhileCoordinatorHeld(databasePath: string, busyTimeoutMs: number) {
  ...
  try {
    ...
    return inspectCarapaceStateOwnershipFromDatabase(database, resolvedPath);
  } finally {
    database.close();
  }
}

function acquireCarapaceStateOwnershipCoordinator(databasePath: string, busyTimeoutMs: number) {
...
export function runWithCarapaceStateOwnershipCoordinator<T>(
  databasePath: string,
  operationLabel: string,
  operation: () => T,
): T {
  return runWithSqliteCoordinator(
    acquireCarapaceStateOwnershipCoordinator(databasePath, CARAPACE_SQLITE_BUSY_TIMEOUT_MS),
    operationLabel,
    operation,
  );
}
...
export function inspectCarapaceStateOwnershipAtPath(
  databasePath: string,
): CarapaceExternalStateOwnership | null {
  const resolvedPath = path.resolve(databasePath);
  if (!existsSync(resolvedPath)) {
...
function assertOwnershipAllowsWrite(
  status: CarapaceExternalStateOwnership | null,
  databasePath: string,
  env: NodeJS.ProcessEnv,
): void {
  if (status && !isGatewayExternallySupervised(env)) {
    throw new CarapaceStateExternalOwnershipError(databasePath, status.managerId);
  }
}

/** Fence and hold one path-based mutation until its main-file preamble is complete. */
function acquireCarapaceStateWriteAccess(options: {
  databasePath: string;
  ...
  const busyTimeoutMs = normalizeSqliteNonNegativeInteger(
    options.busyTimeoutMs ?? CARAPACE_SQLITE_BUSY_TIMEOUT_MS,
    "busyTimeoutMs",
  );
  const access = acquireCarapaceStateOwnershipCoordinator(resolvedPath, busyTimeoutMs);
  try {
...
export function runWithCarapaceStateWriteAccess<T>(
  options: { databasePath: string; busyTimeoutMs?: number; env?: NodeJS.ProcessEnv },
  operationLabel: string,
  operation: () => T,
): T {
  return runWithSqliteCoordinator(
    acquireCarapaceStateWriteAccess(options),
    operationLabel,
    operation,
  );
}
...
export async function assertCarapaceStateWriteAllowedAtPath(options: {
  databasePath: string;
  env?: NodeJS.ProcessEnv;
...
export function assertCarapaceStateWriteAllowed(options: {
  database: DatabaseSync;
  databasePath: string;
  env?: NodeJS.ProcessEnv;
  ...
  const resolvedPath = path.resolve(options.databasePath);
  const status = inspectCarapaceStateOwnershipFromDatabase(
    options.database,
    resolvedPath,
    options.schemaReady,
  );
