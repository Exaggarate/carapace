...
function standingIntentsSchemaSql(): string {
  const start = CARAPACE_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_START);
  const end = CARAPACE_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Carapace standing-intents schema markers are missing.");
  }
  return CARAPACE_AGENT_SCHEMA_SQL.slice(start, end);
}
...
export function ensureCarapaceAgentStandingIntentsSchema(db: DatabaseSync): void {
  const ensure = () => {
