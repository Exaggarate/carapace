...
export const CARAPACE_AGENT_SCHEMA_VERSION = 19;
export const AGENT_MEDIA_SCHEMA_VERSION = 17;

/** Open per-agent SQLite database handle plus lifecycle maintenance. */
export type CarapaceAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type CarapaceAgentDatabaseOptions = CarapaceStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type CarapaceRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
export type CarapaceAgentDatabaseOwnerInspection =
