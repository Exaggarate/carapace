...
function messageToolRunOutcomeSchemaSql(): string {
  const start = CARAPACE_AGENT_SCHEMA_SQL.indexOf(SCHEMA_START);
  const end = CARAPACE_AGENT_SCHEMA_SQL.indexOf(SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Carapace message-tool run outcome schema markers are missing.");
  }
