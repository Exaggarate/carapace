...
function fixture() {
  ...
  const database = openCarapaceAgentDatabase({ agentId: "worker", env });
...
function withAbortableMaintenance<T>(
  ...
  return withCarapaceStateLease(
    ...
    (maintenance) => {
      assertNoCarapaceAgentDatabaseLeases(maintenance, { env: f.env });
...
describe("asynchronous agent database maintenance admission", () => {
  it("yields during real integrity admission while retaining the maintenance fence", async () => {
    ...
    await withAgentDatabaseMaintenanceLease({ env: f.env }, async (maintenance) => {
      ...
      try {
        await migrateCarapaceAgentDatabaseForMaintenance(f.options, maintenance);
        expect(progressed).toBe(true);
        maintenance.assertOwned();
        expect(() => openCarapaceAgentDatabase({ agentId: "worker", env: f.env })).toThrow(
    ...
    try {
      await expect(
        withAbortableMaintenance(f, abort.signal, (maintenance) =>
          migrateCarapaceAgentDatabaseForMaintenance(f.options, maintenance),
        ),
  ...
  it("rejects a retained closed maintenance context before opening or scanning", async () => {
    ...
    await expect(migrateCarapaceAgentDatabaseForMaintenance(f.options, retained!)).rejects.toThrow(
  ...
  it.each(["outside", "copied"] as const)(
    ...
    async (scope) => {
      const f = fixture();
      const ready = createDeferred<CarapaceStateLeaseContext>();
