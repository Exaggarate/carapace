...
import {
  ...
  ensureCarapaceAgentBoardSchemaInTransaction,
} from "./carapace-agent-board-schema.js";
import { CONTEXT_ENGINE_TURN_OUTBOX_TABLE } from "./carapace-agent-context-engine-turn-outbox-schema.js";
import { FIRST_USE_ADDITIVE_AGENT_COLUMN_DEFINITIONS } from "./carapace-agent-db-additive-columns.js";
import {
...
} from "./carapace-agent-db-session-migrations.js";
import { SESSION_GOAL_OPERATIONS_TABLE } from "./carapace-agent-goal-operations-schema.js";
import { MESSAGE_TOOL_RUN_OUTCOMES_TABLE } from "./carapace-agent-message-tool-outcome-schema.js";
import { LEGACY_PARTICIPANT_OPTIONAL_COLUMNS } from "./carapace-agent-participants-migration.js";
import { SESSION_PENDING_INPUTS_TABLE } from "./carapace-agent-pending-inputs-schema.js";
import {
  ensureCarapaceAgentProgressCardSchemaInTransaction,
  AGENT_PROGRESS_CARD_SCHEMA_SQL,
  SESSION_PROGRESS_CARDS_TABLE,
} from "./carapace-agent-progress-card-schema.js";
import { CARAPACE_AGENT_SCHEMA_SQL } from "./carapace-agent-schema.js";
import { SESSION_PARTICIPANTS_TABLE } from "./carapace-agent-session-participants-schema.js";
import {
  AGENT_V14_ADDITIVE_SCHEMA_SQL,
  AGENT_V14_CORE_SCHEMA_SQL,
  AGENT_V14_SESSION_SHARING_SCHEMA_SQL,
} from "./carapace-agent-session-sharing-schema.js";
import { SESSION_TRANSCRIPT_ARCHIVES_TABLE } from "./carapace-agent-session-transcript-archive-schema.js";
import {
  STANDING_INTENTS_FTS_SHADOW_TABLES,
  STANDING_INTENTS_FTS_TABLE,
  STANDING_INTENTS_TABLE,
} from "./carapace-agent-standing-intents-schema.js";
...
export function assertCarapaceAgentSchemaContains(
...
export function assertCarapaceAgentCurrentRuntimeSchema(
  database: DatabaseSync,
  ...
  if (metadata.schemaVersion !== CARAPACE_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `Carapace agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${CARAPACE_AGENT_SCHEMA_VERSION}; run carapace doctor --fix before using it.`,
    );
  }
  if (hasRetiredAgentStateLeaseSchema(database)) {
    throw new Error(
      `Carapace agent database ${options.pathname} retains retired state_leases storage; run carapace doctor --fix before using it.`,
    );
  }
...
function repairAndAssertAgentSchemaGroup(
  ...
  assertCarapaceAgentSchemaContains(database, pathname, schemaSql, "legacy");
}
...
export function ensureSessionKeyContractSchemaInTransaction(db: DatabaseSync): void {
  const start = CARAPACE_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_START);
  const end = CARAPACE_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Carapace agent session-key contract schema markers are missing.");
  }
  db.exec(CARAPACE_AGENT_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- Idempotent additive lazy ensure.
}

export function repairAndAssertCarapaceAgentV14SchemaForMigration(
  database: DatabaseSync,
  options: { agentId: string; pathname: string },
): void {
  ...
  if (userVersion !== 14) {
    throw new Error(
      `Carapace agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `Carapace agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
  ...
  if (hasAnyCanonicalTable(database, AGENT_V14_BOARD_SCHEMA_SQL)) {
    ...
    ensureCarapaceAgentBoardSchemaInTransaction(database);
  ...
  if (hasAnyCanonicalTable(database, AGENT_PROGRESS_CARD_SCHEMA_SQL)) {
    ...
    ensureCarapaceAgentProgressCardSchemaInTransaction(database);
...
export function assertCanonicalAgentPersistenceVersion(
  ...
  if (userVersion < AGENT_MEDIA_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new CarapaceAgentDatabaseMediaMigrationRequiredError(pathname, userVersion);
  }
  if (userVersion < CARAPACE_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new Error(
      `Carapace agent database ${pathname} uses schema version ${userVersion}; stop active agents and run carapace doctor --fix to migrate session identities before using it.`,
    );
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `Carapace agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`Carapace agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `Carapace agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}
...
export function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `Carapace agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
