...
import {
  type CarapaceDatabaseVerifyResult,
  type CarapaceDatabaseVerifyTarget,
  verifyCarapaceDatabases,
...
function quarantineStorePath(stateDir: string): string {
  return path.join(stateDir, "state", "carapace-quarantine.sqlite");
}

function terminalVerificationResult(pathname: string): CarapaceDatabaseVerifyResult {
  return {
    path: pathname,
    ok: false,
...
describe("Carapace database integrity verifier", () => {
  it.each(["absent", "installed"])(
    "verifies the %s additive transcript eligibility projection",
    async (shape) => {
      const stateDir = tempDirs.make("carapace-database-verify-eligibility-");
      ...
      const targets: CarapaceDatabaseVerifyTarget[] = [
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("carapace-database-verify-live-locks-");
      const env = { CARAPACE_STATE_DIR: stateDir };
      const agent = openCarapaceAgentDatabase({ agentId: "worker-1", env });
      agent.db
        .prepare(
      ...
      const targets: CarapaceDatabaseVerifyTarget[] = [
        { kind: "agent", label: "Carapace agent database worker-1", path: agent.path },
      ];
      ...
      const reader = spawnSync(
        ...
        [
          ...
          `
            ...
            const database = new DatabaseSync(process.env.CARAPACE_VERIFY_TEST_PATH);
            database.prepare("PRAGMA schema_version;").get();
            database.close();
          `,
        ],
        {
          env: { ...process.env, CARAPACE_VERIFY_TEST_PATH: agent.path },
          encoding: "utf8",
        },
      );
  ...
  it("detects corruption off-thread, quarantines it, and latches later opens", async () => {
    const stateDir = tempDirs.make("carapace-database-verify-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    createUnsafeIndexDrift(agentPath);
    const targets: CarapaceDatabaseVerifyTarget[] = [
      { kind: "agent", label: "Carapace agent database worker-1", path: agentPath },
    ];
    ...
    const quarantine = readCarapaceDatabaseQuarantine(agentPath, { env });
    ...
    clearCarapaceAgentDatabaseOpenFailure(agentPath, { env });
    expect(() => openCarapaceAgentDatabase({ agentId: "worker-1", env })).toThrow(
      expect.objectContaining({ name: "SqliteIntegrityError" }),
    );
  });
  ...
  it("does not quarantine a healthy database that replaced the verified file", async () => {
    const stateDir = tempDirs.make("carapace-database-verify-replacement-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    closeCarapaceAgentDatabasesForTest();
    closeCarapaceStateDatabaseForTest();
    ...
    applyCarapaceDatabaseVerificationResults({ env, results, targets });

    expect(readCarapaceDatabaseQuarantine(agentPath, { env })).toBeUndefined();
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("carapace-database-verify-live-agent-replace-");
      const env = { CARAPACE_STATE_DIR: stateDir };
      const agent = openCarapaceAgentDatabase({ agentId: "worker-1", env });
      ...
      expect(readCarapaceDatabaseQuarantine(agent.path, { env })).toBeUndefined();
      expect(openCarapaceAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
    },
  );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("carapace-database-verify-live-state-replace-");
      const env = { CARAPACE_STATE_DIR: stateDir };
      ...
      const targets: CarapaceDatabaseVerifyTarget[] = [
        { kind: "state", label: "Carapace state database", path: state.path },
      ];
      const results = preparedVerificationResults(targets);
      ...
      expect(readCarapaceDatabaseQuarantine(state.path, { env })).toBeUndefined();
      expect(openCarapaceStateDatabase({ env }).db.isOpen).toBe(true);
    },
  );

  it("reconfirms and quarantines a corrupt closed database", async () => {
    const stateDir = tempDirs.make("carapace-database-verify-closed-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    closeCarapaceAgentDatabasesForTest();
    closeCarapaceStateDatabaseForTest();
    ...
    expect(readCarapaceDatabaseQuarantine(agentPath, { env })?.reason).toMatch(
  ...
  it("does not quarantine a repaired database after same-inode mutation", async () => {
    const stateDir = tempDirs.make("carapace-database-verify-repair-race-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    closeCarapaceAgentDatabasesForTest();
    closeCarapaceStateDatabaseForTest();
    ...
    await expect(verifyCarapaceDatabases(targets)).resolves.toEqual([
  ...
  it("rejects stale terminal results after draining healthy owners", () => {
    const stateDir = tempDirs.make("carapace-database-verify-live-stale-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const state = openCarapaceStateDatabase({ env });
    const agent = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    const targets: CarapaceDatabaseVerifyTarget[] = [
  ...
  it("revalidates agent schema ownership after confirmation drains a pathname", () => {
    const stateDir = tempDirs.make("carapace-database-verify-agent-revalidate-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agent = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    ...
    replacement.close();
    expect(closeCarapaceAgentDatabaseByPath(agent.path)).toBe(true);
  ...
  it("expires a generation-bound process latch after the database changes", () => {
    const stateDir = tempDirs.make("carapace-database-verify-latch-generation-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    closeCarapaceAgentDatabasesForTest();
    ...
    expect(recordCarapaceAgentDatabaseOpenFailure(agentPath, error, generation)).toBe(true);
    const { DatabaseSync } = requireNodeSqlite();
  ...
  it("reports an uncleared quarantine row instead of claiming repair success", () => {
    const stateDir = tempDirs.make("carapace-database-verify-clear-failure-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    ...
    error.name = "SqliteIntegrityError";
    recordCarapaceAgentDatabaseOpenFailure(agentPath, error);
    closeCarapaceStateDatabaseForTest();
    ...
    try {
      expect(clearCarapaceAgentDatabaseOpenFailure(agentPath, { env })).toBe(false);
      expect(
        recordCarapaceDatabaseQuarantine({
          env,
          kind: "agent",
    ...
    expect(clearCarapaceAgentDatabaseOpenFailure(agentPath, { env })).toBe(true);
    expect(openCarapaceAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
  });

  it("keeps healthy opens on the missing-store fast path", () => {
    const stateDir = tempDirs.make("carapace-database-verify-clean-");
    const env = { CARAPACE_STATE_DIR: stateDir };
  ...
  it("records and clears dedicated quarantine rows with rollback journaling", () => {
    const stateDir = tempDirs.make("carapace-database-verify-store-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);

    expect(clearCarapaceDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(
      recordCarapaceDatabaseQuarantine({
        env,
        kind: "agent",
    ...
    expect(readCarapaceDatabaseQuarantine(databasePath, { env })).toEqual({
      kind: "agent",
      quarantinedAt: expect.any(Number),
      reason: "corrupt index",
    });
    ...
    expect(clearCarapaceDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(clearCarapaceDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(readCarapaceDatabaseQuarantine(databasePath, { env })).toBeUndefined();
  });

  it("expires a persisted quarantine when the verified database generation changes", () => {
    const stateDir = tempDirs.make("carapace-database-verify-store-generation-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
    ...
    expect(readCarapaceDatabaseQuarantine(databasePath, { env })?.reason).toBe(
      "corrupt generation",
    );
  ...
  it("reads schema-v1 quarantine rows and migrates them on the next write", () => {
    const stateDir = tempDirs.make("carapace-database-verify-store-v1-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
  ...
  it("recovers an interrupted empty quarantine-store initialization", () => {
    const stateDir = tempDirs.make("carapace-database-verify-empty-store-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    ...
    expect(readCarapaceDatabaseQuarantine(databasePath, { env })?.reason).toBe("corrupt index");
  });

  it.skipIf(process.platform === "win32")(
    "recovers a hot rollback journal before reading quarantine",
    () => {
      const stateDir = tempDirs.make("carapace-database-verify-hot-journal-");
      const env = { CARAPACE_STATE_DIR: stateDir };
      const databasePath = path.join(stateDir, "agent.sqlite");
      const storePath = quarantineStorePath(stateDir);
      expect(
      ...
      const crashed = spawnSync(
        ...
        [
          ...
          `
            import { DatabaseSync } from "node:sqlite";
            const database = new DatabaseSync(process.env.CARAPACE_QUARANTINE_TEST_PATH);
        ...
        { env: { ...process.env, CARAPACE_QUARANTINE_TEST_PATH: storePath } },
      );
  ...
  it("does not latch transient verifier errors", () => {
    const stateDir = tempDirs.make("carapace-database-verify-transient-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    const agentPath = openCarapaceAgentDatabase({ agentId: "worker-1", env }).path;
    closeCarapaceAgentDatabasesForTest();
    closeCarapaceStateDatabaseForTest();
  ...
  it("persists state failure quarantine across restart until doctor repair", () => {
    const stateDir = tempDirs.make("carapace-database-verify-state-failure-");
    const env = { CARAPACE_STATE_DIR: stateDir };
    ...
    const error = new Error("corrupt index");
    error.name = "SqliteIntegrityError";
    recordCarapaceStateDatabaseOpenFailure(statePath, error);
    ...
    expect(repairCarapaceStateDatabaseSchema({ env }).warnings).toEqual([]);
    expect(openCarapaceStateDatabase({ env }).db.isOpen).toBe(true);
  });
});
