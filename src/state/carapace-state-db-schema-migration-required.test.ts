...
function createV15Database(version: string | null = "2026.9.2") {
  const options = { env: { CARAPACE_STATE_DIR: tempDirs.make("carapace-schema-publication-") } };
...
function reopen(options: Parameters<typeof openCarapaceStateDatabase>[0]) {
  closeCarapaceStateDatabaseForTest();
  return openCarapaceStateDatabase(options).db;
}
...
describe("shared state schema publication", () => {
    ...
    (entry) => {
      ...
      expect(
        db
          ...
          .get(),
      ).toEqual({ value_json: String(CARAPACE_STATE_SCHEMA_VERSION) });
      expect(
        db
          .prepare(
  ...
  it("preserves migrated Workshop rows and skips content on repeated deferred opens and repair", () => {
    ...
    closeCarapaceStateDatabaseForTest();
    const repair = repairCarapaceStateDatabaseSchema(options);
    ...
    const after = openCarapaceStateDatabase(options).db;
  ...
  it.each([
    ...
    expectVersion(openCarapaceStateDatabase(options).db, CARAPACE_STATE_SCHEMA_VERSION);
  });

  it("uses finished_at_ms even when a terminal record was enriched more recently", () => {
    const { options } = createV15Database();
    finishRun(openCarapaceStateDatabase(options).db, now - graceMs);
    expectVersion(reopen(options), CARAPACE_STATE_SCHEMA_VERSION);
  });

  it("publishes deferred content when its legacy ledger row is absent", () => {
    const { options } = createV15Database();
    openCarapaceStateDatabase(options).db.exec("DELETE FROM update_runs");
    expectVersion(reopen(options), CARAPACE_STATE_SCHEMA_VERSION);
  });

  it("requires every legacy row to clear its own deadline, including a new running update", () => {
    const { options } = createV15Database();
    let db = openCarapaceStateDatabase(options).db;
