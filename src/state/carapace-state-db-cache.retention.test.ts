...
it("releases closed shared database wrappers after path and global retirement", () => {
  const stateDir = tempDirs.make("carapace-state-retention-");
  ...
  const script = `
    ...
    function retire(byPath) {
      const owner = openCarapaceStateDatabase();
      const ref = new WeakRef(owner.db);
      for (let i = 0; i < 3; i++) {
        assert.equal(openCarapaceStateDatabase(), owner);
      }
      if (byPath) {
        assert.equal(closeCarapaceStateDatabaseByPath(owner.path), true);
      } else {
        closeCarapaceStateDatabase();
      }
