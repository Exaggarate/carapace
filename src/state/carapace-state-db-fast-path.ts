...
import { assertCarapaceStateDatabaseForMaintenance } from "./carapace-state-db-maintenance.js";
import {
  assertCanonicalStateSchemaShape,
  detectCarapaceStateDatabaseSchemaMigrationsFromDatabase,
...
export function needsCarapaceStateDatabaseSchemaRepair(pathname: string): boolean {
  let database: DatabaseSync | undefined;
  try {
    database = openNodeSqliteDatabase(pathname, { readOnly: true });
    assertSupportedStateSchemaVersion(database, pathname);
    const needsRepair =
      readStateSchemaContentVersion(database) !== CARAPACE_STATE_SCHEMA_VERSION ||
      detectCarapaceStateDatabaseSchemaMigrationsFromDatabase(database, pathname).length > 0;
    if (!needsRepair) {
...
export function assertCurrentStateRuntimeSchema(
  ...
  assertCarapaceStateDatabaseForMaintenance(database, { pathname }, readTable);
}

export function isCarapaceStateSchemaFastPathEligible(
  database: DatabaseSync,
  pathname: string,
): boolean {
  return runSqliteDeferredTransactionSync(database, () => {
    ...
    const startupRepairRequired = collectSqliteSchemaIssues(
      ...
      STATE_PERSISTENT_SCHEMA_COMPATIBILITY,
      readTable,
    ).some(isCarapaceStateStartupRepairableSchemaIssue);
