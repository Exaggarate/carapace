...
import { runDatabaseVerifyWorker } from "./carapace-database-verify.impl.js";
import { verifyCarapaceDatabases } from "./carapace-database-verify.worker.js";

const tempDirs = useAutoCleanupTempDirTracker(afterEach);

async function importVerifierInUnrelatedFork(): Promise<unknown[]> {
  const fixtureDir = tempDirs.make("carapace-database-verify-process-");
...
describe("database verifier child process entrypoint", () => {
  it("retains native SQLite diagnostics across IPC without changing the source", async () => {
    const fixtureDir = tempDirs.make("carapace-database-verify-invalid-");
  ...
  it("preserves inherited Node flags for a JavaScript fork", async () => {
    const fixtureDir = tempDirs.make("carapace-database-verify-flags-");
...
describe("database verifier bounded diagnostics", () => {
  ...
  it.each([
    ...
    await expect(verifyCarapaceDatabases([target])).resolves.toEqual([
  ...
  it("bounds cyclic and deep causes without serializing their prose", async () => {
    ...
    await expect(verifyCarapaceDatabases([target, target])).resolves.toEqual([
      {
        path: target.path,
        ok: false,
