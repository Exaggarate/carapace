...
function createWalFixture(): { header: Buffer; withFrames: Buffer } {
  const fixtureDir = fs.realpathSync(tempStateDirs.make("carapace-wal-header-"));
...
function prepareCase(kind: (typeof databaseKinds)[number]) {
  const stateDir = fs.realpathSync(tempStateDirs.make("carapace-orphan-sidecar-"));
  const env = { CARAPACE_STATE_DIR: stateDir };
  const databasePath =
    kind === "state"
      ? resolveCarapaceStateSqlitePath(env)
