...
let carapaceStateCanonicalNamedIndexSet: ReadonlySet<string> | undefined;

function getCarapaceStateCanonicalNamedIndexSet(): ReadonlySet<string> {
  carapaceStateCanonicalNamedIndexSet ??= new Set(
    getCanonicalSqliteNamedIndexContracts(CARAPACE_STATE_SCHEMA_SQL).map((index) => index.name),
  );
  return carapaceStateCanonicalNamedIndexSet;
}

/** Project canonical SQL to the tables the shared runtime may create during this open. */
export function getCarapaceStateRuntimeSchema(options: {
  includeVersionLazyAdditiveTables: boolean;
}): string {
  let schema = CARAPACE_STATE_SCHEMA_SQL;
...
export const CARAPACE_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY: SqliteSchemaCompatibility = {
...
export function isCarapaceStateStartupRepairableSchemaIssue(issue: SqliteSchemaIssue): boolean {
  ...
  return (
    ...
    getCarapaceStateCanonicalNamedIndexSet().has(issue.objectName)
  );
}

/** Identify compatible schema differences repaired only by their feature owner. */
export function isCarapaceStateFirstUseSchemaIssue(issue: SqliteSchemaIssue): boolean {
  return (
