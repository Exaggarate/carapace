// Carapace agent database tests cover agent-scoped DB storage and migrations.
...
import { AGENT_MEDIA_SCHEMA_VERSION } from "./carapace-agent-db-contract.js";
import {
...
import {
  ...
  unregisterCarapaceAgentDatabases,
} from "./carapace-agent-db-registry.js";
import type { DB as CarapaceAgentKyselyDatabase } from "./carapace-agent-db.generated.js";
import {
  ...
  openCarapaceAgentDatabase as openCarapaceAgentDatabaseRuntime,
...
import { resolveIncognitoCarapaceAgentSqlitePath } from "./carapace-agent-db.paths.js";
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "carapace-agent-db-");
}
...
function ensureSharedStateDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "carapace-agent-db-shared-state-");
...
function openCarapaceAgentDatabase(
  options: Parameters<typeof openCarapaceAgentDatabaseRuntime>[0],
) {
  materializeSharedStateDatabase(options.env);
  return openCarapaceAgentDatabaseRuntime(options);
}
...
function ensureCurrentWorkerAgentDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "carapace-agent-db-current-worker-");
  const env = { CARAPACE_STATE_DIR: stateDir };
  const template = openCarapaceAgentDatabase({ agentId: "worker-1", env });
  ...
  try {
    ...
    if (readSqliteNumberPragma(verified, "user_version") !== CARAPACE_AGENT_SCHEMA_VERSION) {
...
async function migrateAndOpenLegacyAgentDatabaseForTest(
  options: Parameters<typeof openCarapaceAgentDatabase>[0],
) {
  ...
  try {
    await withAgentDatabaseMaintenanceLease({ env: options?.env }, async () =>
      ensureCarapaceAgentDatabaseSchema(database, options),
    );
  } finally {
    database.close();
  }
  return openCarapaceAgentDatabase(options);
}
...
function readRegisteredAgentDatabaseLastSeenAt(params: {
  ...
  const row = openCarapaceStateDatabase({ env: params.env })
...
function launchAgentSchemaOpener(params: {
  ...
  const agentModuleUrl = new URL("./carapace-agent-db.ts", import.meta.url).href;
  const stateModuleUrl = new URL("./carapace-state-db.ts", import.meta.url).href;
  const child = spawn(
    process.execPath,
    [
      "--import",
      ...
      `
        ...
        const db = new DatabaseSync(process.env.CARAPACE_AGENT_DB_RACE_PATH);
        ...
        process.stdin.once("data", () => {
          let result;
          try {
            ensureCarapaceAgentDatabaseSchema(observedDb, {
              agentId: process.env.CARAPACE_AGENT_DB_RACE_AGENT,
              env: { CARAPACE_STATE_DIR: process.env.CARAPACE_AGENT_DB_RACE_STATE_DIR },
              path: process.env.CARAPACE_AGENT_DB_RACE_PATH,
              register: true,
            });
            result = { agentId: process.env.CARAPACE_AGENT_DB_RACE_AGENT, ok: true };
          } catch (error) {
            result = {
              agentId: process.env.CARAPACE_AGENT_DB_RACE_AGENT,
              ok: false,
    ...
    ],
    {
      env: {
        ...process.env,
        CARAPACE_AGENT_DB_RACE_AGENT: params.agentId,
        CARAPACE_AGENT_DB_RACE_PATH: params.databasePath,
        CARAPACE_AGENT_DB_RACE_STATE_DIR: params.stateDir,
      },
...
describe("carapace agent database", () => {
  it.each([false, true])(
    "keeps agent state writable without SQLite extension support (incognito=%s)",
    (incognito) => {
      const options = { agentId: "worker-1", env: { CARAPACE_STATE_DIR: createTempStateDir() } };
      const capability = vi
      ...
      try {
        const { db } = openCarapaceAgentDatabase({
          ...options,
          ...(incognito ? { path: resolveIncognitoCarapaceAgentSqlitePath(options) } : {}),
        });
  ...
  it("uses the canonical state schema for deletion journal reads and updates", () => {
    ...
    const schemaSql = fs.readFileSync(
      new URL("./carapace-state-schema.sql", import.meta.url),
      "utf8",
    );
    ...
    const databasePaths = [path.join(entry.agentDir, "carapace-agent.sqlite")];
    const cleanupPaths = [
      {
        path: entry.agentDir,
  ...
  it("fences registration and lease claims beneath another agent's pending deletion", () => {
    ...
    registerCarapaceAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });
    const deletion = beginAgentDeletionJournal(
      {
    ...
    unregisterCarapaceAgentDatabase({ agentId: "deleted", path: relocatedDatabasePath, env });
    unregisterCarapaceAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });

    try {
      for (const fencedPath of [databasePath, relocatedDatabasePath, sidecarClaimPath]) {
        expect(() =>
          registerCarapaceAgentDatabase({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
        expect(() =>
          claimCarapaceAgentDatabaseLease({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
      }
    } finally {
    ...
    expect(() =>
      registerCarapaceAgentDatabase({ agentId: "survivor", path: databasePath, env }),
    ).not.toThrow();
    ...
    unregisterCarapaceAgentDatabase({ agentId: "survivor", path: databasePath, env });
  });
  ...
  it("blocks deletion on a foreign lease beneath its workspace until release", () => {
    ...
    expect(() => assertNoCarapaceAgentDatabaseLeases("deleted", { env })).toThrow("deletion owns");
    releaseCarapaceAgentDatabaseLease(leaseId, { env });
    expect(() => assertNoCarapaceAgentDatabaseLeases("deleted", { env })).not.toThrow();
    removeAgentDeletionJournal(deletion.agentId, deletion.operationId, { env });
  });
  ...
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "carapace-agent.sqlite"));
  });
  ...
  it("opens the agent and its shared registry under the default HOME state owner", () => {
    ...
    const agentPath = path.join(stateDir, "agents", "main", "agent", "carapace-agent.sqlite");

    // Fail before opening if path resolution escapes the test-owned home.
    expect(resolveCarapaceAgentSqlitePath(options)).toBe(agentPath);
    const agent = openCarapaceAgentDatabaseRuntime(options);
    const shared = openCarapaceStateDatabase({ env });

    expect(agent.path).toBe(agentPath);
    expect(shared.path).toBe(path.join(stateDir, "state", "carapace.sqlite"));
  ...
  it("distinguishes absent, unowned and unsupported read-only databases", () => {
    ...
    expect(withCarapaceAgentDatabaseReadOnly(() => "unused", options)).toEqual({
      found: false,
    ...
    try {
      unsupported.exec(`PRAGMA user_version = ${CARAPACE_AGENT_SCHEMA_VERSION + 1};`);
    } finally {
      unsupported.close();
    }
    expect(() => withCarapaceAgentDatabaseReadOnly(() => "unused", options)).toThrow(
      `newer schema version ${CARAPACE_AGENT_SCHEMA_VERSION + 1}`,
    );
  });
  ...
  it("opens extension-capable reads separately from the owner without enabling writes", () => {
    ...
    const owner = openCarapaceAgentDatabase(options);
  ...
  it.each([
    {
      version: CARAPACE_AGENT_SCHEMA_VERSION + 1,
      expectedError: {
    ...
    },
    {
      ...
      expectedError: {
        name: "CarapaceAgentDatabaseMediaMigrationRequiredError",
        message: expect.stringContaining("run carapace doctor --fix to migrate persisted media"),
      },
    },
    {
      version: CARAPACE_AGENT_SCHEMA_VERSION - 1,
      expectedError: {
        name: "Error",
        message: expect.stringContaining(
          "stop active agents and run carapace doctor --fix to migrate session identities",
        ),
      },
    },
  ])(
  ...
  it("rotates the registry token on pathname switches and committed writes", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const otherEnv = { CARAPACE_STATE_DIR: createTempStateDir() };
    ...
    const firstToken = readCarapaceAgentDatabaseRegistryToken({ env });
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([]);
    expect(readCarapaceAgentDatabaseRegistryToken({ env })).toBe(firstToken);
    expect(readCarapaceAgentDatabaseRegistryToken({ env: otherEnv })).not.toBe(firstToken);
    const returnedToken = readCarapaceAgentDatabaseRegistryToken({ env });
    expect(returnedToken).not.toBe(firstToken);

    registerCarapaceAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    const registeredToken = readCarapaceAgentDatabaseRegistryToken({ env });
    expect(registeredToken).not.toBe(returnedToken);
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-1", path: databasePath }),
    ]);
    expect(readCarapaceAgentDatabaseRegistryToken({ env })).toBe(registeredToken);

    unregisterCarapaceAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    expect(readCarapaceAgentDatabaseRegistryToken({ env })).not.toBe(registeredToken);
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([]);
  });
  ...
  it("stores state-owned registrations relative while preserving external paths", () => {
    ...
    const env = { CARAPACE_STATE_DIR: stateDir };
    const inRootPath = path.join(stateDir, "agents", "worker-1", "agent", "carapace-agent.sqlite");
    const externalPath = path.join(externalStateDir, "external.sqlite");

    registerCarapaceAgentDatabase({ agentId: "worker-1", path: inRootPath, env });
    registerCarapaceAgentDatabase({ agentId: "external", path: externalPath, env });
    ...
    expect(
      ...
      {
        agent_id: "worker-1",
        path: path.join("agents", "worker-1", "agent", "carapace-agent.sqlite"),
      },
    ]);
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([
    ...
    stateDatabase.db
      ...
      .run("worker-1", inRootPath, CARAPACE_AGENT_SCHEMA_VERSION);
    unregisterCarapaceAgentDatabase({ agentId: "worker-1", path: inRootPath, env });
    expect(
      stateDatabase.db
    ...
    unregisterCarapaceAgentDatabase({ agentId: "external", path: externalPath, env });
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([]);
  });
  ...
  it("keeps incompatible schema versions maintenance-only", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "legacy", "agent", "carapace-agent.sqlite");
    registerCarapaceAgentDatabase({
      agentId: "legacy",
      path: databasePath,
      env,
      schemaVersion: CARAPACE_AGENT_SCHEMA_VERSION - 1,
    });
  ...
  it("does not persist archived import databases in the registry", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const archivedPath = path.join(stateDir, "imports", "archive", "carapace-agent.sqlite");

    registerCarapaceAgentDatabase({ agentId: "archive", path: archivedPath, env });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const env = { CARAPACE_STATE_DIR: stateDir };
      const activePath = path.join(stateDir, "active", "carapace-agent.sqlite");
      const archivedPath = path.join(stateDir, "imports", "archive", "carapace-agent.sqlite");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const missingPath = path.join(aliasDir, "carapace-agent.sqlite");

      registerCarapaceAgentDatabase({ agentId: "archive", path: missingPath, env });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(() => listCarapaceRegisteredAgentDatabases({ env })).toThrow("is unavailable");
    },
  );
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    const database = openCarapaceAgentDatabaseRuntime({
      agentId: "worker-1",
      env: { CARAPACE_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./carapace-agent-schema.sql", import.meta.url)),
    );
    expect(
      database.db
    ...
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "carapace-agent.sqlite"),
    );

    const registered = listCarapaceRegisteredAgentDatabases({
      env: { CARAPACE_STATE_DIR: stateDir },
  ...
  it("opens a v13 database that already contains additive board storage", async () => {
    expect(CARAPACE_AGENT_SCHEMA_VERSION).toBe(19);
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);
    expect(
      reopened.db
  ...
  it("migrates v13 session entries, routes, and generations into nodes and windows", async () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);
  ...
  it("migrates version 1 memory source identities before registering version 2", async () => {
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);
    expect(
    ...
    expect(
      listCarapaceRegisteredAgentDatabases({ env }).find((entry) => entry.agentId === "worker-1"),
    ).toMatchObject({ path: databasePath, schemaVersion: CARAPACE_AGENT_SCHEMA_VERSION });

    closeCarapaceAgentDatabasesForTest();
    const reopened = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    expect(
  ...
  it("rolls back a failed version 1 migration without retaining ownership", () => {
    ...
    expect(() => openCarapaceAgentDatabase({ agentId: "worker-1", env })).toThrow();
  ...
  it("retains a failed-open handle and lease when initialization cleanup also fails", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const database = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    ...
    expect(() =>
      openCarapaceAgentDatabase({ agentId: "worker-2", env, path: database.path }),
    ).toThrow("initialization close failed");
    close.mockRestore();
    expect(inspectCarapaceAgentDatabaseOwner(database.path)).toEqual({
      status: "owned",
      agentId: "worker-1",
    });
    expect(() => assertNoCarapaceAgentDatabaseLeases("worker-2", { env })).toThrow(
    ...
    expect(() =>
      withCarapaceAgentDatabaseReadOnly(() => "unused", {
        agentId: "worker-2",
        env,
        path: database.path,
      }),
    ...
    expect(disposeCarapaceAgentDatabaseByPath(database.path, { env })).toBe(true);
    expect(() => assertNoCarapaceAgentDatabaseLeases("worker-2", { env })).not.toThrow();
  });
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const defaultDatabase = openCarapaceAgentDatabase({
      agentId: "worker-1",
      env,
    });

    expect(
      listCarapaceRegisteredAgentDatabases({ env })
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      try {
        const matchesPath = createCarapaceAgentDatabasePathMatcher();
      ...
      expect(createCarapaceAgentDatabasePathMatcher()(realPath, aliasPath)).toBe(false);
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(() => isSameCarapaceAgentDatabasePath(loopPath, loopPath)).toThrow(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(isSameCarapaceAgentDatabasePath(realPath, aliasPath)).toBe(true);
      ...
      expect(isSameCarapaceAgentDatabasePath(futureRealPath, danglingAliasPath)).toBe(true);
      ...
      expect(isSameCarapaceAgentDatabasePath(relativeRealPath, relativeAliasPath)).toBe(true);
      ...
      expect(isSameCarapaceAgentDatabasePath(redirectedRealPath, redirectedAliasPath)).toBe(true);
      expect(
        isSameCarapaceAgentDatabasePath(
      ...
      expect(isSameCarapaceAgentDatabasePath(danglingTraversalAlias, livePath)).toBe(false);
      const rawMissingTraversalPath = `${stateDir}${path.sep}missing${path.sep}..${path.sep}live-worker.sqlite`;
      expect(isSameCarapaceAgentDatabasePath(rawMissingTraversalPath, livePath)).toBe(false);
      ...
      expect(() => isSameCarapaceAgentDatabasePath(loopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      expect(() => isSameCarapaceAgentDatabasePath(expandingLoopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      const database = openCarapaceAgentDatabase({ agentId: "worker", env, path: realPath });
      unregisterCarapaceAgentDatabase({ agentId: "worker", env, path: database.path });
      registerCarapaceAgentDatabase({ agentId: "worker", env, path: aliasPath });
    },
  );
  ...
  it("matches volume semantics for non-ASCII case aliases", () => {
    ...
    expect(isSameCarapaceAgentDatabasePath(upperPath, lowerPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for Unicode simple case-fold aliases", () => {
    ...
    expect(isSameCarapaceAgentDatabasePath(sigmaPath, finalSigmaPath)).toBe(aliases);
  });
  ...
  it("keeps case and normalization semantics distinct for missing aliases", () => {
    ...
    expect(isSameCarapaceAgentDatabasePath(composedUpperPath, decomposedLowerPath)).toBe(aliases);
  });
  ...
  it("requires raw normalization after an ASCII-only case difference", () => {
    ...
    expect(isSameCarapaceAgentDatabasePath(composedUpperPath, decomposedMixedPath)).toBe(aliases);
  });
  ...
  it.runIf(tempVolumeIsCaseInsensitive)(
    ...
    () => {
      ...
      expect(
        isSameCarapaceAgentDatabasePath(path.join(stateDir, upper), path.join(stateDir, lower)),
      ).toBe(true);
    },
  );
  ...
  it("uses the candidate code points for missing-path normalization semantics", () => {
    ...
    expect(isSameCarapaceAgentDatabasePath(leftPath, rightPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for normalization-only missing directory components", () => {
    ...
    expect(
      isSameCarapaceAgentDatabasePath(
        path.join(leftDir, "carapace-agent.sqlite"),
        path.join(rightDir, "carapace-agent.sqlite"),
      ),
    ).toBe(aliases);
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(isSameCarapaceAgentDatabasePath(driveRelativePath, absolutePath)).toBe(true);
    },
  );
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      expect(
        isSameCarapaceAgentDatabasePath(path.join(hardLinkedNested, "d", "link"), firstHardLink),
      ).toBe(true);
    },
  );
  ...
  it("does not refresh global registry metadata on cached opens", () => {
    ...
    try {
      const first = openCarapaceAgentDatabase({
        agentId: "worker-1",
        env,
      });
      expect(
      ...
      const second = openCarapaceAgentDatabase({
        agentId: "worker-1",
        env,
      });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    legacyDb.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/carapace-agent.sqlite',
        1,
        10,
        20
      );
    ...
    expect(() => listCarapaceRegisteredAgentDatabases({ env })).toThrow(
    ...
    ).toThrow(/run carapace doctor --fix/);

    fs.rmSync(stateDatabasePath);
    const reopened = openCarapaceAgentDatabase({
      agentId: "worker-1",
      env,
    });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-agent-db-state-"));
          const env = { CARAPACE_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-agent-db-relative-"));
          ...
          try {
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: listCarapaceRegisteredAgentDatabases({ env })
                .filter((entry) => entry.agentId === "worker-1")
                .map((entry) => entry.path),
  ...
  it("closes only the cached database at the exact resolved path", () => {
    ...
    const first = openCarapaceAgentDatabase({ agentId: "worker-1", env, path: firstPath });
    const second = openCarapaceAgentDatabase({ agentId: "worker-2", env, path: secondPath });

    expect(closeCarapaceAgentDatabaseByPath(path.join(stateDir, "missing.sqlite"))).toBe(false);
    expect(closeCarapaceAgentDatabaseByPath(firstPath, "worker-2")).toBe(false);
    ...
    expect(second.db.isOpen).toBe(true);
    expect(closeCarapaceAgentDatabaseByPath(firstPath)).toBe(false);

    const reopened = openCarapaceAgentDatabase({ agentId: "worker-1", env, path: firstPath });
  ...
  it("retains its durable lease when closing the handle fails", () => {
    ...
    expect(() => closeCarapaceAgentDatabaseByPath(database.path)).toThrow("wal close failed");
    expect(database.db.isOpen).toBe(true);
    expect(() => assertNoCarapaceAgentDatabaseLeases("worker-1", { env })).toThrow(
      "database is still open",
    );

    close.mockRestore();
    expect(closeCarapaceAgentDatabaseByPath(database.path)).toBe(true);
    expect(() => assertNoCarapaceAgentDatabaseLeases("worker-1", { env })).not.toThrow();
  });
  ...
  it("settles a Worker lease after a recoverable handle-close failure", () => {
    ...
    const settled = settleCarapaceAgentDatabaseWorkerClose(database.path);
  ...
  it("disposes only its exact cached owner and unregisters that registry row", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const first = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    const second = openCarapaceAgentDatabase({ agentId: "worker-2", env });

    expect(disposeCarapaceAgentDatabaseByPath(first.path, { env })).toBe(true);
    expect(first.db.isOpen).toBe(false);
    expect(second.db.isOpen).toBe(true);
    expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-2", path: second.path }),
    ]);
    expect(disposeCarapaceAgentDatabaseByPath(first.path, { env })).toBe(false);
    expect(second.db.isOpen).toBe(true);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const realPath = path.join(realDir, "carapace-agent.sqlite");
      const aliasPath = path.join(aliasDir, "carapace-agent.sqlite");
      const database = openCarapaceAgentDatabase({ agentId: "probe", env, path: realPath });

      expect(disposeCarapaceAgentDatabaseByPath(aliasPath, { env })).toBe(true);
      expect(database.db.isOpen).toBe(false);
      expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([]);
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const real = openCarapaceAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(realDir, "carapace-agent.sqlite"),
      });
      const alias = openCarapaceAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(aliasDir, "carapace-agent.sqlite"),
      });

      expect(disposeCarapaceAgentDatabaseByPath(real.path, { env })).toBe(false);
  ...
  it("reopens a recreated agent id on a fresh database after archival", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const original = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    ...
    try {
      expect(disposeCarapaceAgentDatabaseByPath(original.path, { env })).toBe(true);
      ...
      const recreated = openCarapaceAgentDatabase({ agentId: "worker-1", env });
      expect(recreated.db.isOpen).toBe(true);
      expect(fs.statSync(recreated.path).ino).not.toBe(originalInode);
      expect(fs.existsSync(path.join(archivedDir, "carapace-agent.sqlite"))).toBe(true);
      expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual([
  ...
  it("keeps the deleted id fenced until its completed tombstone is claimed", () => {
    ...
    expect(() =>
      registerCarapaceAgentDatabase({ agentId: "worker-2", path: databasePath, env }),
    ).not.toThrow();
    unregisterCarapaceAgentDatabase({ agentId: "worker-2", path: databasePath, env });
  ...
  it("serializes database leases with the durable deletion fence", () => {
    const stateDir = createTempStateDir();
    const env = { CARAPACE_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "worker-1", "carapace-agent.sqlite");
    const leaseId = claimCarapaceAgentDatabaseLease({
      agentId: "worker-1",
      path: databasePath,
      env,
    ...
    try {
      ...
      expect(() =>
        claimCarapaceAgentDatabaseLease({ agentId: "worker-1", path: databasePath, env }),
      ).toThrow("agent worker-1 is deleted");
      releaseCarapaceAgentDatabaseLease(leaseId, { env });
      expect(() => assertNoCarapaceAgentDatabaseLeases("worker-1", { env })).not.toThrow();
    } finally {
  ...
  it("closes cached agent databases and fences new writers during maintenance", async () => {
    ...
    await withAgentDatabaseMaintenanceLease({ env }, async (maintenance) => {
      ...
      expect(() =>
        runCarapaceStateWriteTransaction(({ db }) => maintenance.assertOwnedInTransaction(db), {
          env,
        }),
      ).not.toThrow();
    });
    ...
    expect(openCarapaceAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
  });
  ...
  it("serializes concurrent ownership claims for one unowned database", async () => {
    ...
    try {
      ...
      expect(
        listCarapaceRegisteredAgentDatabases({ env: { CARAPACE_STATE_DIR: stateDir } }),
      ).toEqual([
        expect.objectContaining({
          agentId: winner?.agentId,
          path: databasePath,
  ...
  it("rechecks schema version after waiting for a concurrent writer", async () => {
    ...
    const futureVersion = CARAPACE_AGENT_SCHEMA_VERSION + 1;
    try {
      await opener.ready;
      opener.child.stdin.end("go\n");
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const reopenedStateDatabase = openCarapaceStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { CARAPACE_STATE_DIR: parentDir };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      runCarapaceAgentWriteTransaction(() => {
        runCarapaceAgentWriteTransaction(() => undefined, options);
  ...
  it("installs same-version session additions before maintenance index repair", async () => {
    ...
    try {
      ...
      expect(() =>
        assertCarapaceAgentDatabaseForMaintenance(repaired, {
          agentId: "worker-1",
  ...
  it("installs transcript eligibility without parsing or rewriting older same-version rows", () => {
    ...
    const repaired = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    expect(readSqliteNumberPragma(repaired.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);
    expect(
      repaired.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
  ...
  it.each(["TEXT", "INTEGER NOT NULL DEFAULT 1", "INTEGER CHECK (context_eligible IN (0, 1))"])(
    ...
    (declaration) => {
      ...
      expect(() =>
        openCarapaceAgentDatabase({ agentId: "worker-1", env: { CARAPACE_STATE_DIR: stateDir } }),
  ...
  it("keeps memory provenance tables independently lazy and accepts their same-version installation", () => {
    ...
    const opened = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    ...
    expect(readSqliteNumberPragma(opened.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);

    for (const name of ["memory_entry_origins", "memory_session_tombstones"]) {
      const canonicalSchema = CARAPACE_AGENT_SCHEMA_SQL.match(
  ...
  it("rejects same-name transcript index drift when duplicate rows block repair", () => {
    ...
    try {
      ...
      expect(
        after
          .prepare(
            "SELECT name FROM sqlite_schema WHERE type = 'index' AND name LIKE 'carapace_probe_%'",
          )
          .all(),
      ).toEqual([]);
    } finally {
  ...
  it("adds durable conversation delivery state when upgrading the canonical v10 schema", async () => {
    ...
    expect(() =>
      assertCarapaceAgentDatabaseForMaintenance(database.db, {
        agentId: "worker-1",
  ...
  it("inspects registered database ownership without mutating the database", () => {
    ...
    expect(inspectCarapaceAgentDatabaseOwner(databasePath)).toEqual(expectedOwner);
    closeCarapaceAgentDatabasesForTest();
  ...
  it.each([null, "", "   "])(
    ...
    (agentId) => {
      ...
      expect(inspectCarapaceAgentDatabaseOwner(databasePath)).toEqual({ status: "unreadable" });
    },
  );
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    ...
    try {
      expect(() =>
        ensureCarapaceAgentDatabaseSchema(independentlyManaged, {
          agentId: "worker-1",
          env,
        }),
  ...
  it.each(["path", "agent"] as const)(
    ...
    (scope) => {
      ...
      try {
        const databasePaths = [
          openCarapaceAgentDatabase({ agentId: "worker-1", env }).path,
          openCarapaceAgentDatabase({
            agentId: "worker-1",
            env,
        ...
        const unrelatedPath = openCarapaceAgentDatabase({ agentId: "worker-2", env }).path;
        for (const databasePath of [...databasePaths, unrelatedPath]) {
          expect(closeCarapaceAgentDatabaseByPath(databasePath)).toBe(true);
        }
        if (scope === "agent") {
          unregisterCarapaceAgentDatabases({ agentId: "worker-1", env });
        } else {
        ...
        openCarapaceAgentDatabase({ agentId: "worker-2", env });
        expect(listCarapaceRegisteredAgentDatabases({ env })).toEqual(
          expect.arrayContaining([
  ...
  it("initializes an empty replacement after physical close without a registry mutation", () => {
    ...
    const reopened = openCarapaceAgentDatabase({ agentId: "worker-1", env, path: databasePath });
  ...
  it("reopens a validated current-schema database promptly while a WAL writer is active", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const writer = new DatabaseSync(databasePath);
    let reopened: ReturnType<typeof openCarapaceAgentDatabase> | undefined;
    try {
      writer.exec("PRAGMA journal_mode = WAL; BEGIN IMMEDIATE;");
      const startedAt = performance.now();
      reopened = openCarapaceAgentDatabase({ agentId: "worker-1", env });
      ...
      writer.close();
      if (reopened) {
        closeCarapaceAgentDatabaseByPath(reopened.path);
      }
    }
  });
  ...
  it.each([
    ...
    ({ role, agentId, expectedError }) => {
      ...
      try {
        ...
        let failure: unknown;
        try {
          openCarapaceAgentDatabase({ agentId: "worker-1", env });
        } catch (error) {
          failure = error;
        }
        const elapsedMs = performance.now() - startedAt;
  ...
  it.each([0, 16])(
    ...
    (version) => {
      ...
      expect(() => openCarapaceAgentDatabase({ agentId: "worker-1", env })).toThrow(
        "run carapace doctor --fix to migrate persisted media",
      );
    },
  );
  ...
  it.each([
    {
      ...
      expectedError: /uses schema version 0; run carapace doctor --fix/,
    },
    {
    ...
    ({ schema, expectedError }) => {
      ...
      expect(() =>
        openCarapaceAgentDatabase({ agentId: "worker-1", env, path: databasePath }),
      ).toThrow(expectedError);
    },
  );
  ...
  it("runs full integrity before a pending agent schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${CARAPACE_AGENT_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("latches newer per-agent schema failures on a validated physical reopen", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    db.exec(`PRAGMA user_version = ${CARAPACE_AGENT_SCHEMA_VERSION + 1};`);
    db.close();
    ...
    expect(firstFailure).toMatchObject({
      name: "SqliteSchemaVersionError",
      message: expect.stringContaining("https://github.com/Exaggarate/carapace"),
    });
    expect(isCarapaceAgentDatabaseOpen(databasePath)).toBe(false);
    expect(
      openCarapaceStateDatabase({ env })
    ...
    clearCarapaceAgentDatabaseOpenFailure(databasePath);
    const reopened = openCarapaceAgentDatabase({ agentId: "worker-1", env });
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(CARAPACE_AGENT_SCHEMA_VERSION);
    expect(
      reopened.db
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openCarapaceAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openCarapaceAgentDatabase({
            agentId: "worker-1",
            env: { CARAPACE_STATE_DIR: process.env.CARAPACE_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, CARAPACE_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {
