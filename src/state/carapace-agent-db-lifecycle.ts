...
import type {
  ...
  CarapaceAgentDatabaseOwnerInspection,
...
export const CARAPACE_AGENT_DB_OPEN_HANDLE_CAP = 64;
const agentDbLog = createSubsystemLogger("state/agent-db");
const CARAPACE_AGENT_DB_SLOW_OPEN_MS = 1_000;
...
type AgentDatabaseLifecycle = {
  databases: Map<string, CarapaceAgentDatabase>;
  borrowers: WeakMap<DatabaseSync, Set<object>>;
  incognito: WeakSet<CarapaceAgentDatabase>;
  generation: number;
...
export type PendingAgentDatabaseOpen = {
  ...
  promise: Promise<CarapaceAgentDatabase>;
  assertHeld?: () => void;
  operations: number;
  releaseBorrow?: () => void;
};
...
const cache = resolveGlobalSingleton<AgentDatabaseLifecycle>(
  Symbol.for("carapace.agentDatabaseLifecycle"),
  () => ({
    databases: new Map(),
    ...
    failures: new Map(),
    leases: new Map(),
    terminal: createSqliteTerminalOpenLatch({ closeByPath: closeCarapaceAgentDatabaseByPath }),
...
export function startAgentDatabaseOpenTiming(
  ...
  return (phase: keyof typeof phaseDurationsMs): void => {
    ...
    if (phase === "registration" && elapsedMs >= CARAPACE_AGENT_DB_SLOW_OPEN_MS) {
      agentDbLog.warn("slow Carapace agent database open", {
        agentId,
        elapsedMs,
        path: pathname,
        pid: process.pid,
        ...
        phaseDurationsMs,
        ...diagnostics,
        thresholdMs: CARAPACE_AGENT_DB_SLOW_OPEN_MS,
      });
    }
  };
}
...
export function closeCachedCarapaceAgentDatabase(
  ...
  if (lease) {
    releaseCarapaceAgentDatabaseLease(lease.leaseId, { env: lease.env });
...
export function evictLruAgentDatabaseHandles(): void {
  ...
  while (cache.databases.size >= CARAPACE_AGENT_DB_OPEN_HANDLE_CAP) {
    let evicted = false;
    for (const [pathname, database] of cache.databases) {
      ...
      closeCachedCarapaceAgentDatabase(database, { eviction: true });
      cache.databases.delete(pathname);
      cache.failures.delete(pathname);
      if (cache.incognito.has(database)) {
        cache.generation += 1;
      }
      agentDbLog.debug("evicted Carapace agent database handle", {
    ...
    if (!evicted) {
      ...
      agentDbLog.warn("agent database handle cap exceeded; all cached handles are retained", {
        cap: CARAPACE_AGENT_DB_OPEN_HANDLE_CAP,
        openHandles: cache.databases.size,
      });
      return;
    }
...
export function closeCarapaceAgentDatabaseByPath(
  pathname: string,
  ...
  closeCachedCarapaceAgentDatabase(database);
...
export type CarapaceAgentDatabaseWorkerCloseResult = {
  errors: Error[];
  settled: boolean;
};
 ...
 */
export function settleCarapaceAgentDatabaseWorkerClose(
  pathname: string,
): CarapaceAgentDatabaseWorkerCloseResult {
  const resolvedPath = path.resolve(pathname);
  const errors: Error[] = [];
  const database = cache.databases.get(resolvedPath);
  if (database) {
...
export function closeCarapaceAgentDatabases(rootPath?: string): void {
...
export async function closeCarapaceAgentDatabasesAsync(rootPath?: string): Promise<void> {
  while (true) {
...
export function inspectCarapaceAgentDatabaseOwner(
  pathname: string,
): CarapaceAgentDatabaseOwnerInspection {
  ...
  try {
    ...
    setSqliteBusyTimeout(db, CARAPACE_SQLITE_BUSY_TIMEOUT_MS);
