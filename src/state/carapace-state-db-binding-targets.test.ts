...
import {
  ...
  reconcileCarapaceStateSchemaPublication,
...
function createVersion14Bindings() {
  const stateDir = tempDirs.make("carapace-binding-targets-v14-");
...
describe("conversation binding target migration", () => {
  it("admits deferred content without schema mutation while another Gateway owns the state", async () => {
    const options = { env: { CARAPACE_STATE_DIR: tempDirs.make("carapace-deferred-reader-") } };
    ...
    initial.db
      ...
      .run(String(CARAPACE_STATE_SCHEMA_VERSION), Date.now());
    ...
    try {
      ...
      const reader = openCarapaceStateDatabase(options);
      ...
      expect(() => reconcileCarapaceStateSchemaPublication(options)).not.toThrow();
      expect(openCarapaceStateDatabase(options).db).toBe(reader.db);
      expect(reader.db.prepare("PRAGMA user_version").get()).toEqual({ user_version: 15 });
      closeCarapaceStateDatabaseForTest();
      const agedReader = openCarapaceStateDatabase(options);
    ...
    expect(openCarapaceStateDatabase(options).db.prepare("PRAGMA user_version").get()).toEqual({
    ...
    async (migrationPath) => {
      ...
      expect(
        migrated.db
          ...
          .get(),
      ).toEqual({ schema_version: CARAPACE_STATE_SCHEMA_VERSION, app_version: VERSION });
      expect(
        migrated.db
          .prepare(
      ...
      const holder = await holdGatewayLifecycle(databasePath);
      try {
        expect(readMigrationSnapshot(openCarapaceStateDatabase(options).db)).toEqual(after);
      } finally {
        closeCarapaceStateDatabaseForTest();
        await holder.release();
      }
  ...
  it.each(
    ...
    ({ migrationPath, failure }) => {
      ...
      if (migrationPath === "runtime open") {
        expect(() => openCarapaceStateDatabase(options)).toThrow(reason);
      } else {
