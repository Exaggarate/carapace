...
import { isCarapaceStateSchemaFastPathEligible } from "./carapace-state-db-fast-path.js";
import {
...
describe("ACP replay accounting repair", () => {
  ...
  it.each(["UTF-8", "UTF-16le"])(
    ...
    async (encoding) => {
      await withTestDir({ prefix: "carapace-acp-repair-" }, async (dir) => {
        ...
        seed.close();
        const initial = openCarapaceStateDatabase(options).db;
        seedLegacyReplay(initial);
        const before = canonicalReplay(initial);
        closeCarapaceStateDatabaseForTest();
        const repaired = openCarapaceStateDatabase(options).db;
        ...
        try {
          expect(isCarapaceStateSchemaFastPathEligible(reopened, options.path)).toBe(true);
        } finally {
  ...
  it("rolls back event repairs and the release checkpoint when a later session repair fails, then doctor retries atomically", async () => {
    await withTestDir({ prefix: "carapace-acp-repair-" }, async (dir) => {
      ...
      closeCarapaceStateDatabaseForTest();
      const failed = repairCarapaceStateDatabaseSchema(options);
