...
import {
  assertCarapaceStateWriteAllowedAtPath,
  inspectCarapaceStateOwnershipAtPath,
...
function createEnv(external = false): NodeJS.ProcessEnv {
  return {
    CARAPACE_STATE_DIR: tempDirs.make("carapace-state-ownership-"),
    ...(external ? { CARAPACE_SUPERVISOR_MODE: "external" } : {}),
  };
}

function withoutExternalMarker(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const next = { ...env };
  delete next.CARAPACE_SUPERVISOR_MODE;
  return next;
}

function claimFixture(managerId = "gateway-supervisor") {
  const externalEnv = createEnv(true);
  const ownership = claimCarapaceStateOwnership(managerId, { env: externalEnv });
...
describe("external shared-state ownership", () => {
  it("returns unowned for a missing path without creating its state tree", async () => {
    const rootDir = tempDirs.make("carapace-state-ownership-missing-");
    const missingStateDir = path.join(rootDir, "missing-state");
    const databasePath = path.join(missingStateDir, "state", "carapace.sqlite");

    expect(fs.existsSync(missingStateDir)).toBe(false);
    expect(inspectCarapaceStateOwnershipAtPath(databasePath)).toBeNull();
    expect(fs.existsSync(missingStateDir)).toBe(false);
    await assertCarapaceStateWriteAllowedAtPath({ databasePath });
    expect(fs.existsSync(missingStateDir)).toBe(false);
  });
  ...
  it.each(["missing", "orphaned", "existing"])(
    ...
    async (layout) => {
      ...
      try {
        ...
        await expect(assertCarapaceStateWriteAllowedAtPath(options)).rejects.toBe(reason);
  ...
  it("keeps missing-database admission eligible for pristine startup", async () => {
    const home = tempDirs.make("carapace-state-ownership-pristine-");
    ...
    await assertCarapaceStateWriteAllowedAtPath({ databasePath, env });
    expect(fs.readdirSync(stateDir)).toEqual(["carapace.json"]);
    expect(planPristineStartupStateMigrations(env)).toEqual({
      skipAllStateMigrations: true,
      skipCoreStateMigrations: true,
    });
  ...
  it("preserves ordinary unowned database behavior", () => {
    ...
    expect(inspectCarapaceStateOwnershipAtPath(database.path)).toBeNull();
  });

  it("checks Doctor startup admission without staging a public snapshot", async () => {
    const fixture = claimFixture();
    const home = tempDirs.make("carapace-state-ownership-doctor-");
    ...
    const runPreflight = async (
      ...
      await withEnvAsync(
        {
          HOME: home,
          CARAPACE_CONFIG_PATH: path.join(home, "carapace.json"),
          CARAPACE_PROFILE: undefined,
          CARAPACE_STATE_DIR: env.CARAPACE_STATE_DIR,
          CARAPACE_SUPERVISOR_MODE: env.CARAPACE_SUPERVISOR_MODE,
        },
        async () =>
  ...
  it("reads ownership from a WAL when the SHM index is absent", () => {
    ...
    try {
      ...
      const copyDir = tempDirs.make("carapace-state-ownership-wal-only-");
      const copyPath = path.join(copyDir, "carapace.sqlite");
      ...
      expect(inspectCarapaceStateOwnershipAtPath(copyPath)).toEqual(ownership);
    } finally {
      writer.close();
    }
  });
  ...
  it("rejects unmarked WAL ownership without modifying the SQLite family", async () => {
    ...
    const copyDir = tempDirs.make("carapace-state-ownership-wal-rejection-");
    const copyPath = path.join(copyDir, "carapace.sqlite");
    try {
      writer.exec("PRAGMA journal_mode = WAL; PRAGMA wal_autocheckpoint = 0;");
      writer
    ...
    ).rejects.toThrow(CarapaceStateOwnershipError);
  ...
  it("observes committed ownership that is still resident in the live WAL", () => {
    ...
    try {
      ...
      expect(inspectCarapaceStateOwnershipAtPath(databasePath)).toEqual(ownership);
    } finally {
      writer.close();
    }
  });
  ...
  it("does not expose rolled-back ownership from a rollback-journal race", () => {
    ...
    try {
      const inspected = inspectCarapaceStateOwnershipAtPath(databasePath);
      ...
      expect(
        runWithCarapaceStateWriteAccess(
          ...
          () => inspectCarapaceStateOwnershipAtPath(databasePath),
        ),
      ).toBeNull();
      expect(transactionStarted).toBe(true);
      expect(transientOwnershipObserved).toBe(true);
  ...
  it("inspects consolidated ownership without modifying its SQLite family or state tree", () => {
    const fixture = claimFixture();
    const stateDir = fixture.externalEnv.CARAPACE_STATE_DIR;
    if (!stateDir) {
    ...
    try {
      expect(inspectCarapaceStateOwnershipAtPath(fixture.databasePath)).toEqual(fixture.ownership);
    } finally {
      if (process.platform !== "win32") {
  ...
  it("uses one external coordinator path across temporary-directory environments", () => {
    ...
    const databasePath = openCarapaceStateDatabase({ env }).path;
    closeCarapaceStateDatabaseForTest();
    const stateDir = resolveCarapaceStateDirForDatabasePath(databasePath);
    ...
    for (const temporaryDirectory of [
      ...
      withEnv({ TMPDIR: temporaryDirectory }, () =>
        runWithCarapaceStateOwnershipCoordinator(databasePath, "test coordinator path", () => {}),
      );
  ...
  it("closes an unpublished fresh handle when coordinator release fails", () => {
    ...
    let cachedDuringRelease: ReturnType<
      typeof carapaceStateDatabaseCache.getCarapaceStateDatabaseIfOpenAtPath
    > = undefined;
  ...
  it("requires the external marker and makes claims idempotent only for one manager", () => {
    const env = createEnv();
    expect(() => claimCarapaceStateOwnership("gateway-supervisor", { env })).toThrow(
      /CARAPACE_SUPERVISOR_MODE=external/u,
    );
    const externalEnv = { ...env, CARAPACE_SUPERVISOR_MODE: "external" };
    const first = claimCarapaceStateOwnership("gateway-supervisor", { env: externalEnv });
    expect(claimCarapaceStateOwnership("gateway-supervisor", { env: externalEnv })).toEqual(first);
    expect(
      inspectCarapaceStateOwnershipAtPath(openCarapaceStateDatabase({ env: externalEnv }).path),
    ).toEqual(first);
    expect(() => claimCarapaceStateOwnership("replacement-manager", { env: externalEnv })).toThrow(
  ...
  it("refuses unmarked writable opens before changing the SQLite family", () => {
    const fixture = claimFixture();
    const pending = openCarapaceStateDatabase({ env: fixture.externalEnv });
    pending.db.exec(`
    ...
    expect(() => openCarapaceStateDatabase({ env: fixture.unmarkedEnv })).toThrow(
      CarapaceStateOwnershipError,
    );
    ...
    const repaired = openCarapaceStateDatabase({ env: fixture.externalEnv });
  ...
  it("fences a claim made immediately before cold-open schema repair", () => {
    ...
    try {
      expect(() => openCarapaceStateDatabase({ env })).toThrow(CarapaceStateOwnershipError);
    } finally {
      exec.mockRestore();
    }
  ...
  it("fences injected and pre-claim handles on their next canonical write", () => {
    const externalEnv = createEnv(true);
    const opened = openCarapaceStateDatabase({ env: externalEnv });
    claimCarapaceStateOwnership("gateway-supervisor", { env: externalEnv });
    const unmarkedEnv = withoutExternalMarker(externalEnv);

    expect(() => openCarapaceStateDatabase({ env: unmarkedEnv })).toThrow(
      CarapaceStateOwnershipError,
    );
    expect(() => openCarapaceStateDatabase({ env: unmarkedEnv, database: opened })).toThrow(
      CarapaceStateOwnershipError,
    );
    expect(() =>
  ...
  it("reports checkpoint failure and lets the same durable claim retry", () => {
    ...
    const ownership = claimCarapaceStateOwnership("gateway-supervisor", { env });
    expect(inspectCarapaceStateOwnershipAtPath(database.path)).toEqual(ownership);
  });
  ...
  it("reports lock cleanup separately after a durable claim and permits idempotent retry", () => {
    ...
    const ownership = claimCarapaceStateOwnership("gateway-supervisor", { env });
    expect(inspectCarapaceStateOwnershipAtPath(openCarapaceStateDatabase({ env }).path)).toEqual(
      ownership,
    );
  });
  ...
  it("fails closed when unmarked and lets an external claim repair malformed metadata", () => {
    ...
    expect(() => openCarapaceStateDatabase({ env: withoutExternalMarker(env) })).toThrow(
      CarapaceStateOwnershipMetadataError,
    );
    expect(() => openCarapaceStateDatabase({ env })).toThrow(CarapaceStateOwnershipMetadataError);
    const ownership = claimCarapaceStateOwnership("gateway-supervisor", { env });
    expect(inspectCarapaceStateOwnershipAtPath(database.path)).toEqual(ownership);
    expect(
      openCarapaceStateDatabase({ env }).db.prepare("PRAGMA table_info(worktrees)").all(),
  ...
  it("fences state repair and config health writes while allowing health reads", async () => {
    ...
    const before = snapshotSqliteFamily(fixture.databasePath);
    expect(() => repairCarapaceStateDatabaseSchema({ env: fixture.unmarkedEnv })).toThrow(
      CarapaceStateOwnershipError,
    );
    expect(() => repairCarapaceStateDatabaseSchemaIfNeeded({ env: fixture.unmarkedEnv })).toThrow(
      CarapaceStateOwnershipError,
    );
    expect(() =>
      withCarapaceStateStartupMigrationCheckpointDatabase(() => undefined, {
    ...
    const healthDeps = {
      env: fixture.unmarkedEnv,
      homedir: () => fixture.unmarkedEnv.CARAPACE_STATE_DIR ?? "",
      logger: { warn: () => undefined },
    };
    expect(readConfigHealthStateFromStore(healthDeps)).toEqual({ entries: {} });
    expect(() =>
      writeConfigHealthStateToStore(healthDeps, {
        entries: { "/tmp/carapace.json": { lastObservedSuspiciousSignature: "test" } },
      }),
    ).toThrow(CarapaceStateOwnershipError);
    assert.deepStrictEqual(snapshotSqliteFamily(fixture.databasePath), before);
  });
  ...
  it("allows read-only access without the external marker", async () => {
    const fixture = claimFixture();
    const database = await openExistingCarapaceStateDatabaseReadOnly({ env: fixture.unmarkedEnv });
    expect(database?.db.isOpen).toBe(true);
