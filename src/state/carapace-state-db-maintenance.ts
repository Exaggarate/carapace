...
import { migrateJsonCanonicalWideRowsV13 } from "./carapace-state-db-schema-v13-widerow.js";
import {
...
import { CarapaceStateOwnershipError } from "./carapace-state-ownership.js";
import {
...
type CarapaceStateMigrationVersion = keyof typeof STATE_MIGRATION_ALLOWED_MISSING_TABLES;

/** Require canonical shared-state ownership without requiring the latest schema. */
export function assertCarapaceStateDatabaseOwner(
  ...
  if (metadata?.role !== "global") {
    ...
    throw new Error(
      `Carapace state database ${options.pathname} has schema role ${role}; expected global.`,
    );
  }
}

/** Require the canonical shared-state owner and schema before offline file maintenance. */
export function assertCarapaceStateDatabaseForMaintenance(
  database: DatabaseSync,
  options: { pathname: string },
  readTable?: SqliteTableContractReader,
): void {
  const userVersion = assertSupportedStateSchemaVersion(database, options.pathname);
  if (readStateSchemaContentVersion(database) !== CARAPACE_STATE_SCHEMA_VERSION) {
    throw new Error(
      `Carapace state database ${options.pathname} uses schema version ${userVersion}; run carapace doctor --fix before compacting it.`,
    );
  }

  assertCarapaceStateDatabaseOwner(database, options);
  ...
  if (metadata?.schema_version !== userVersion) {
    ...
    throw new Error(
      `Carapace state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${userVersion}; run carapace doctor --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
...
function assertCarapaceStateDatabaseVersionForMigration(
  database: DatabaseSync,
  options: { pathname: string; version: CarapaceStateMigrationVersion },
): void {
  const userVersion = readSqliteUserVersion(database);
  if (userVersion !== options.version) {
    throw new Error(
      `Carapace state database ${options.pathname} uses schema version ${userVersion}; expected ${options.version} before migrating it.`,
    );
  }
  assertCarapaceStateDatabaseOwner(database, options);
  const metadata = database
  ...
  if (metadata?.schema_version !== options.version) {
    const schemaVersion =
      typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `Carapace state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${options.version}; repair the ownership metadata before migrating it.`,
    );
  }
  assertSqliteSchemaTablesPresent(database, options.pathname, CARAPACE_STATE_SCHEMA_SQL, {
...
function assertCarapaceStateDatabaseV5ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 5 });
}

/** Require every stable v6 table before the v7 retirement migration can run. */
function assertCarapaceStateDatabaseV6ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 6 });
}

/** Require every stable v7 table before the v8 placement migration can run. */
function assertCarapaceStateDatabaseV7ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 7 });
}

/** Require every stable v8 table before the v9 registry migration can run. */
function assertCarapaceStateDatabaseV8ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 8 });
}

/** Require every stable v9 table before the v10 retirement migration can run. */
function assertCarapaceStateDatabaseV9ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 9 });
}

/** Require every stable v10 table before the v11 curator retirement can run. */
function assertCarapaceStateDatabaseV10ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 10 });
}

/** Require every stable v11 table before singleton state folds into the v12 store. */
function assertCarapaceStateDatabaseV11ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 11 });
}

/** Require every stable v12 table before wide rows become JSON-canonical. */
function assertCarapaceStateDatabaseV12ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 12 });
}

/** Keep historical migration gates beside their version-specific ownership assertions. */
export const carapaceStateMigrationAssertions = new Map([
  [5, assertCarapaceStateDatabaseV5ForMigration],
  [6, assertCarapaceStateDatabaseV6ForMigration],
  [7, assertCarapaceStateDatabaseV7ForMigration],
  [8, assertCarapaceStateDatabaseV8ForMigration],
  [9, assertCarapaceStateDatabaseV9ForMigration],
  [10, assertCarapaceStateDatabaseV10ForMigration],
  [11, assertCarapaceStateDatabaseV11ForMigration],
  [12, assertCarapaceStateDatabaseV12ForMigration],
  [
    13,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 13 }),
  ],
  [
    14,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 14 }),
  ],
  [
    15,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertCarapaceStateDatabaseVersionForMigration(database, { ...options, version: 15 }),
  ],
]);
...
export function resolveDatabasePath(options: CarapaceStateDatabaseOptions = {}): string {
...
export function runStateSchemaMigrationTransaction<T>(
  ...
  return runSqliteImmediateTransactionSync(
    ...
    () => {
      ...
      const blocker =
        publishedVersion < CARAPACE_STATE_SCHEMA_VERSION
      ...
      try {
        ...
        if (cause instanceof CarapaceStateOwnershipError) {
          throw cause;
        }
...
export function executeCanonicalStateSchema(
  ...
  database.exec(getCarapaceStateRuntimeSchema(options));
}
