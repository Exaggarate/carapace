// State database path helpers resolve shared Carapace state DB paths.
import path from "node:path";
import { resolveStateDir } from "../config/paths.js";

/** Resolve the directory that contains the shared state SQLite file. */
export function resolveCarapaceStateSqliteDir(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveStateDir(env), "state");
}

/** Resolve the shared state SQLite file path. */
export function resolveCarapaceStateSqlitePath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveCarapaceStateSqliteDir(env), "carapace.sqlite");
}

/** Resolve the state owner directory for a canonical or explicit shared database path. */
export function resolveCarapaceStateDirForDatabasePath(databasePath: string): string {
...
export function resolveCarapaceAgentDatabaseStoredPath(
  registryDatabasePath: string,
  agentDatabasePath: string,
): string {
  const stateDir = resolveCarapaceStateDirForDatabasePath(registryDatabasePath);
...
export function resolveCarapaceRegisteredAgentDatabasePath(
  registryDatabasePath: string,
  storedPath: string,
): string {
  return path.isAbsolute(storedPath)
    ? storedPath
    : `${resolveCarapaceStateDirForDatabasePath(registryDatabasePath)}${path.sep}${storedPath}`;
}
