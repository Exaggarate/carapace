...
it("shares agent ownership, savepoints, and commit observers across transformed SDK modules", async () => {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "carapace-agent-module-")));
  const repo = process.cwd();
  const dist = path.join(root, "dist");
  const source = (relativePath: string) => JSON.stringify(path.join(repo, relativePath));
  const ownerExports = `
    export {
      openCarapaceAgentDatabase, closeCarapaceAgentDatabaseByPath,
      closeCarapaceAgentDatabases, runCarapaceAgentWriteTransaction,
      deferCarapaceAgentPostCommitPublication,
      listCarapaceRegisteredAgentDatabases, readCarapaceAgentDatabaseRegistryToken,
    } from ${source("src/state/carapace-agent-db.ts")};
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      'export * from "carapace/plugin-sdk/sqlite-runtime";\n',
    );
    ...
    const result = spawnNodeEvalSync(
      String.raw`
        ...
        const agentPath = path.join(process.env.CARAPACE_STATE_DIR, "agents", "main", "agent", "carapace-agent.sqlite");
        ...
        try {
          ...
          const canonical = host.openCarapaceAgentDatabase(options);
          const nativeBorrow = nativeSdk.borrowCarapaceAgentDatabase(options);
          ...
          plugin = host.getCachedPluginModuleLoader({
            ...
            aliasMap: { "carapace/plugin-sdk/sqlite-runtime": path.join(root, "dist/sqlite-runtime.js") },
          })(modulePath);
          borrowed = plugin.borrowCarapaceAgentDatabase(options);
          ...
          host.runCarapaceAgentWriteTransaction(() => {
            insert("outer");
            assert.equal(plugin.deferCarapaceAgentPostCommitPublication(canonical, () => publish("outer")), true);
            plugin.runCarapaceAgentWriteTransaction(() => {
              insert("committed");
              assert.equal(host.deferCarapaceAgentPostCommitPublication(canonical, () => publish("committed")), true);
              assert.throws(() => host.runCarapaceAgentWriteTransaction(() => {
                insert("discarded");
                assert.equal(plugin.deferCarapaceAgentPostCommitPublication(canonical, () => publish("discarded")), true);
          ...
          assert.throws(() => plugin.runCarapaceAgentWriteTransaction(() => {
            host.runCarapaceAgentWriteTransaction(() => {
              insert("rolled-back-outer");
              assert.equal(host.deferCarapaceAgentPostCommitPublication(canonical, () => publish("rolled-back-outer")), true);
            }, options);
          ...
          assert.deepEqual(host.listCarapaceRegisteredAgentDatabases().map(entry => entry.agentId), ["main"]);
          const registryToken = host.readCarapaceAgentDatabaseRegistryToken();
          const hotOptions = {
            agentId: "hot-created",
            path: path.join(process.env.CARAPACE_STATE_DIR, "agents", "hot-created", "agent", "carapace-agent.sqlite"),
          };
          const hotBorrow = plugin.borrowCarapaceAgentDatabase(hotOptions);
          assert.equal(host.openCarapaceAgentDatabase(hotOptions).db === hotBorrow.db, true);
          hotBorrow.release();
          assert.notEqual(host.readCarapaceAgentDatabaseRegistryToken(), registryToken,
            "transformed registration invalidates native discovery");
          assert.deepEqual(host.listCarapaceRegisteredAgentDatabases().map(entry => entry.agentId), ["hot-created", "main"]);

          borrowed.release();
          assert.equal(db.isOpen, true, "borrow release leaves disposal with the host");
          borrowed = plugin.borrowCarapaceAgentDatabase(options);
          assert.equal(host.closeCarapaceAgentDatabaseByPath(agentPath), true);
          assert.equal(borrowed.db.isOpen, false, "explicit owner disposal revokes retained borrowers");
        } finally {
          borrowed?.release();
          plugin?.closeCarapaceAgentDatabases();
          host?.closeCarapaceAgentDatabases();
          plugin?.closeCarapaceStateDatabase();
