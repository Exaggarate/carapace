...
import {
  resolveCarapaceStateSqliteDir,
  resolveCarapaceStateSqlitePath,
} from "./carapace-state-db.paths.js";

const CARAPACE_STATE_DIR_MODE = 0o700;
const CARAPACE_STATE_FILE_MODE = 0o600;
...
export function ensureCarapaceStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveCarapaceStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveCarapaceStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`Carapace state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: CARAPACE_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, CARAPACE_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      try {
        bestEffortChmodSync(candidate, CARAPACE_STATE_FILE_MODE);
      } catch (error) {
