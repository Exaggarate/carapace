...
import { CONTENT_VERSION_KEY } from "./carapace-state-db-schema-version.js";
...
export function resolveStateSchemaVersionToPublish(db: DatabaseSync): number {
  ...
  if (published >= CARAPACE_STATE_SCHEMA_VERSION || !readStateSchemaPublicationBlocker(db)) {
    return CARAPACE_STATE_SCHEMA_VERSION;
  }
  ...
  executeSqliteQuerySync(
    ...
    getNodeSqliteKysely<PublicationDatabase>(db)
      ...
      .values({
        ...
        value_json: String(CARAPACE_STATE_SCHEMA_VERSION),
      ...
      .onConflict((conflict) =>
        conflict
          ...
          .where("config_machine_state.value_json", "!=", String(CARAPACE_STATE_SCHEMA_VERSION)),
      ),
  );
  return published;
}
