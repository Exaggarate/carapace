...
import * as permissions from "./carapace-agent-db-permissions.js";
import * as registry from "./carapace-agent-db-registry.js";
import * as schema from "./carapace-agent-db-schema.js";
import {
...
function createTimedOpen(validationMs: number, indexRepairMs = 0) {
  const options = {
    agentId: "timing-test",
    env: { CARAPACE_STATE_DIR: makeTempDir(tempDirs, "carapace-agent-open-timing-") },
  };
  ...
  const ensurePermissions = permissions.ensureCarapaceAgentDatabasePermissions;
  vi.spyOn(permissions, "ensureCarapaceAgentDatabasePermissions").mockImplementation((...args) => {
  ...
  const ensureSchema = schema.ensureCarapaceAgentSchema;
  vi.spyOn(schema, "ensureCarapaceAgentSchema").mockImplementation((...args) => {
    ensureSchema(...args);
    advance(90);
  });
  const register = registry.registerCarapaceAgentDatabase;
  vi.spyOn(registry, "registerCarapaceAgentDatabase").mockImplementation((...args) => {
...
describe("agent database open timings", () => {
  it("reports completed phases at the slow threshold and skips live cache hits", () => {
    ...
    expect(logger.warn).toHaveBeenCalledExactlyOnceWith("slow Carapace agent database open", {
  ...
  it("keeps opens below the existing threshold quiet", () => {
    ...
    expect(openCarapaceAgentDatabase(options).db.isOpen).toBe(true);
    expect(logger.warn).not.toHaveBeenCalled();
  });
  ...
  it.each(["definition", "physical"] as const)(
    ...
    `);
      ...
      expect(logger.warn).toHaveBeenCalledExactlyOnceWith(
        "slow Carapace agent database open",
  ...
  it("includes asynchronous admission waiting once for coalesced callers", async () => {
    ...
    const databases: Array<ReturnType<typeof openCarapaceAgentDatabase>> = [];
    const collect = (database: ReturnType<typeof openCarapaceAgentDatabase>) => {
      databases.push(database);
    };
    const outcomes = Promise.allSettled([
      withCarapaceAgentDatabaseAsync(options, collect),
      withCarapaceAgentDatabaseAsync(options, collect),
    ]);
    try {
