...
export function ensureSessionPendingInputsSchema(db: DatabaseSync): void {
  ...
  if (start < 0) {
    throw new Error("Carapace pending-input schema marker is missing.");
  }
  const nested = db.isTransaction;
  runSqliteImmediateTransactionSync(db, () => {
    db.exec(CARAPACE_AGENT_SCHEMA_SQL.slice(start)); // sqlite-allow-raw -- Canonical additive DDL only.
