...
describe("maintenance lease heartbeat", () => {
  it("retains ownership while synchronous maintenance exceeds the lease duration", async () => {
    await withCarapaceTestState({ label: "maintenance-lease-blocked" }, async (state) => {
      await withCarapaceStateLease({ ...options(state.env), leaseMs: 10_000 }, async (lease) => {
        block(10_250);
  ...
  it("acknowledges ownership checks while the parent holds a state write transaction", async () => {
    await withCarapaceTestState({ label: "maintenance-lease-transaction" }, async (state) => {
      await withCarapaceStateLease(options(state.env), async (lease) => {
        runCarapaceStateWriteTransaction(
          ({ db }) => {
  ...
  it("rejects a terminated worker before its queued exit event reaches the parent", async () => {
    await withCarapaceTestState({ label: "maintenance-lease-worker-loss" }, async (state) => {
      const spawned = once(process, "worker") as Promise<[Worker]>;
      await expect(
        withCarapaceStateLease({ ...options(state.env), leaseMs: 10_000 }, async (lease) => {
  ...
  it("does not enter maintenance when its heartbeat exits before readiness", async () => {
    await withCarapaceTestState({ label: "maintenance-lease-startup-loss" }, async (state) => {
      ...
      try {
        await expect(
          withCarapaceStateLease(options(state.env), async () => {
            entered = true;
          }),
        ).rejects.toMatchObject({ code: "CARAPACE_STATE_LEASE_LOST" });
  ...
  it("accepts published readiness when the parent notification is withheld", async () => {
    await withCarapaceTestState({ label: "maintenance-lease-delayed-ready" }, async (state) => {
      const spawned = new Promise<Worker>((resolve) => {
        process.once("worker", resolve);
      });
      const operation = withCarapaceStateLease(
  ...
  it.each(["replacement", "expiry", "deletion"] as const)(
    ...
    async (failure) => {
      await withCarapaceTestState({ label: `maintenance-lease-${failure}` }, async (state) => {
        let changed: ReturnType<typeof readLease>;
        await expect(
          withCarapaceStateLease(options(state.env), async (lease) => {
            runCarapaceStateWriteTransaction(
              ({ db }) => {
                if (failure === "deletion") {
  ...
  it.each(["return", "throw", "abort"] as const)(
    ...
    async (ending) => {
      await withCarapaceTestState({ label: `maintenance-lease-${ending}` }, async (state) => {
        const controller = new AbortController();
        const spawned = once(process, "worker") as Promise<[Worker]>;
        let retained: CarapaceStateLeaseContext | undefined;
        const operation = withCarapaceStateLease(
