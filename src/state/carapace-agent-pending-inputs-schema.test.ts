...
import { ensureCarapaceAgentDatabaseSchema } from "./carapace-agent-db-schema.js";
import {
  closeCarapaceAgentDatabasesForTest,
  openCarapaceAgentDatabase,
} from "./carapace-agent-db.js";
import { ensureSessionPendingInputsSchema } from "./carapace-agent-pending-inputs-schema.js";
import { CARAPACE_AGENT_SCHEMA_SQL } from "./carapace-agent-schema.js";
import { tableHasColumn, tableExists } from "./carapace-state-db-schema-helpers.js";
...
describe("pending input additive schema", () => {
  it("leaves old stores table-free on reads and preserves accepted input through older-reader use and reopen", async () => {
    const options = {
      agentId: "main",
      env: { CARAPACE_STATE_DIR: tempDirs.make("carapace-input-schema-") },
    };
    const scope = {
      ...options,
    ...
    const filename = openCarapaceAgentDatabase(options).path;
    closeCarapaceAgentDatabasesForTest();
    ...
    const previousSql = CARAPACE_AGENT_SCHEMA_SQL.slice(
      0,
      CARAPACE_AGENT_SCHEMA_SQL.indexOf("CREATE TABLE IF NOT EXISTS session_pending_inputs ("),
    );
    ...
    expect(openCarapaceAgentDatabase(options).db.prepare("PRAGMA user_version").get()).toEqual(
      version,
    );
  });
  ...
  it("rejects a drifted optional table rather than treating it as absent", () => {
    const options = {
      agentId: "main",
      env: { CARAPACE_STATE_DIR: tempDirs.make("carapace-input-schema-drift-") },
    };
    const filename = openCarapaceAgentDatabase(options).path;
    closeCarapaceAgentDatabasesForTest();
    ...
    drifted.close();
    expect(() => openCarapaceAgentDatabase(options)).toThrow(/session_pending_inputs|schema/u);
  });
  ...
  it.each(["open", "doctor", "first-use"] as const)(
    ...
    async (path) => {
      const options = {
        agentId: "main",
        env: { CARAPACE_STATE_DIR: tempDirs.make("carapace-input-column-") },
      };
      ...
      if (path === "open") {
        ...
        if (path === "doctor") {
          ensureCarapaceAgentDatabaseSchema(current, options);
        } else {
  ...
  it.each([false, true])(
    ...
    (existing) => {
      ...
      try {
        database.exec(CARAPACE_AGENT_SCHEMA_SQL.replace("  consumed_event_id TEXT,\n", ""));
