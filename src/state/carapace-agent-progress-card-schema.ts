...
import { CARAPACE_AGENT_SCHEMA_WITHOUT_BOARD_SQL } from "./carapace-agent-board-schema.js";
...
function splitProgressCardSchema(sql: string): {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Carapace agent progress-card schema markers are missing.");
  }
  return {
...
const progressCardSchema = splitProgressCardSchema(CARAPACE_AGENT_SCHEMA_WITHOUT_BOARD_SQL);
...
export function ensureCarapaceAgentProgressCardSchemaInTransaction(db: DatabaseSync): void {
  if (!db.isTransaction) {
