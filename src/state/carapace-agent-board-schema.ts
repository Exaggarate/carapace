...
function splitBoardSchema(sql: string): { board: string; withoutBoard: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Carapace agent board schema markers are missing from the canonical schema.");
  }
  return {
...
const boardSchema = splitBoardSchema(CARAPACE_AGENT_SCHEMA_SQL);

const CARAPACE_AGENT_BOARD_SCHEMA_SQL = boardSchema.board;
export const AGENT_V14_BOARD_SCHEMA_SQL = CARAPACE_AGENT_BOARD_SCHEMA_SQL;
export const CARAPACE_AGENT_SCHEMA_WITHOUT_BOARD_SQL = boardSchema.withoutBoard;

function canonicalBoardWidgetsCreateSql(): string {
  const start = CARAPACE_AGENT_BOARD_SCHEMA_SQL.indexOf(BOARD_WIDGETS_SCHEMA_START);
  const end = CARAPACE_AGENT_BOARD_SCHEMA_SQL.indexOf(BOARD_WIDGETS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Carapace agent board widget schema markers are missing.");
  }
  return CARAPACE_AGENT_BOARD_SCHEMA_SQL.slice(start, end).trim();
}
...
function legacyBoardWidgetsCreateSql(): string {
  ...
  if (legacy === canonical) {
    throw new Error("Carapace agent board widget legacy schema derivation failed.");
  }
  return legacy;
}
...
export function ensureCarapaceAgentBoardSchemaInTransaction(db: DatabaseSync): void {
  if (!db.isTransaction) {
    throw new Error("board schema ensure requires an active transaction");
  }
  db.exec(CARAPACE_AGENT_BOARD_SCHEMA_SQL); // sqlite-allow-raw -- Canonical DDL bootstrap for the lazy board schema.
  ...
  if (typeof row?.sql !== "string") {
    throw new Error("Carapace agent board widget schema is missing after ensure.");
  }
  ...
  if (normalizedSchema !== normalizeBoardWidgetsCreateSql(legacyBoardWidgetsCreateSql())) {
    throw new Error(
      "Carapace agent board widget schema has an unsupported content-kind constraint.",
    );
  }
  ...
  if (existingMigrationTable) {
    throw new Error(
      `Carapace agent board migration table already exists: ${BOARD_WIDGETS_MIGRATION_TABLE}`,
    );
  }
  ...
  `);
  db.exec(CARAPACE_AGENT_BOARD_SCHEMA_SQL); // sqlite-allow-raw -- Restore the canonical board index after the rebuild.
}
