...
} from "./carapace-state-db-audit-migration.js";
import {
  CARAPACE_STATE_STRICT_SCHEMA_VERSION,
  type CarapaceStateDatabaseOptions,
  type CarapaceStateDatabaseSchemaMigration,
} from "./carapace-state-db-contract.js";
import { resolveDatabasePath } from "./carapace-state-db-maintenance.js";
import * as operatorApprovalMigration from "./carapace-state-db-operator-approval-migration.js";
import { withExistingCarapaceStateDatabaseArtifactPreservingReadOnly } from "./carapace-state-db-readonly.js";
import {
  tableExists,
  tableHasColumn,
  tablePrimaryKeyColumns,
} from "./carapace-state-db-schema-helpers.js";
import { CarapaceStateDatabaseSchemaMigrationRequiredError } from "./carapace-state-db-schema-migration-required.js";
import { FOLDED_SINGLETON_STATE_TABLES_V12 } from "./carapace-state-db-schema-v12-foldin.js";
...
} from "./carapace-state-db-table-retirements.js";
import {
  resolveCarapaceAgentDatabaseStoredPath,
...
export function migrateWorkerPlacementExecutionModeSchema(
  ...
  const placementSchema = CARAPACE_STATE_SCHEMA_SQL.slice(start, end + endMarker.length);
  ...
  if (
    ...
    throw new Error("Carapace v7 worker placement columns are not canonical");
  }
  ...
  if (unexpectedObjects.length > 0) {
    throw new Error("Carapace v7 worker placement schema has unsupported attached objects");
  }
  const migrationTable = "worker_session_placements_migration_v8";
  if (tableExists(db, migrationTable)) {
    throw new Error(`Carapace worker placement migration table already exists: ${migrationTable}`);
  }
...
function isDefaultAgentDatabasePath(pathname: string, agentId: string): boolean {
  ...
  return (
    path.basename(pathname) === "carapace-agent.sqlite" &&
...
export function migrateAgentDatabaseRelativePaths(
  ...
  for (const row of rows) {
    ...
    if (typeof agentId !== "string" || typeof registeredPath !== "string") {
      throw new Error("Carapace v8 agent database registry paths are not canonical");
    }
    if (!path.isAbsolute(registeredPath)) {
      continue;
    }
    const storedPath = resolveCarapaceAgentDatabaseStoredPath(databasePath, registeredPath);
  ...
  for (const row of rows) {
    ...
    if (
      ...
      !path.isAbsolute(resolveCarapaceAgentDatabaseStoredPath(databasePath, registeredPath))
    ) {
      continue;
    }
    ...
    if (isDefaultAgentDatabasePath(absolutePath, agentId)) {
      ...
      const counterpartStored = resolveCarapaceAgentDatabaseStoredPath(
        databasePath,
        counterpartAbsolute,
      );
...
export function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAgentDatabasesPrimaryKey(db)) {
    if (canRepairAgentDatabasesPrimaryKey(db)) {
      throw new CarapaceStateDatabaseSchemaMigrationRequiredError(
        "agent-databases-composite-primary-key",
        pathname,
      );
    }
    throw new Error(
      `Carapace state database ${pathname} has a noncanonical agent database registry schema that cannot be repaired automatically; restore the canonical agent_databases shape before retrying.`,
    );
  }
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new CarapaceStateDatabaseSchemaMigrationRequiredError("audit-events-v2", pathname);
    }
    throw new Error(
      `Carapace state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectCarapaceStateDatabaseSchemaMigrations(
  ...
  if (behavior.artifactPreservingReadOnly) {
    return (
      withExistingCarapaceStateDatabaseArtifactPreservingReadOnly(
        ({ db }) => detectCarapaceStateDatabaseSchemaMigrationsFromDatabase(db, pathname),
  ...
  try {
    return detectCarapaceStateDatabaseSchemaMigrationsFromDatabase(db, pathname);
  } finally {
    db.close();
  }
}
 ...
 */
export function detectCarapaceStateDatabaseSchemaMigrationsFromDatabase(
  db: DatabaseSync,
  pathname: string,
): CarapaceStateDatabaseSchemaMigration[] {
  const migrations: CarapaceStateDatabaseSchemaMigration[] = [];
  ...
  if (tableExists(db, "audit_events") && userVersion < CARAPACE_STATE_STRICT_SCHEMA_VERSION) {
