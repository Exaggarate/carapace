...
import { sessionParticipantsSchemaSql } from "./carapace-agent-session-participants-schema.js";
...
describe("participant identity migration", () => {
  it("rejects a missing aggregate count in the current schema", async () => {
    await withCarapaceTestState({ scenario: "minimal" }, async (state) => {
      ...
      closeCarapaceAgentDatabasesForTest();
      expect(() => openCarapaceAgentDatabase(options)).toThrow(/session_participants/);
    });
  });
