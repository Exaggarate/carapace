...
export class UpdateSchemaRefusalError extends Error {
  ...
  constructor(
    ...
    const commands = [
      ...
      `npm install -g carapace@${targetVersion} --allow-scripts=carapace`,
      "carapace doctor --fix",
    ...
    super(
      `Doctor refused update-time schema repair driven by Carapace ${updaterVersion}: this updater reopens the ledger with old code after migration, and version publication could not be deferred safely. ` +
        databases
          .map(
            (database) =>
        ...
        `${commands.join(" && ")}. ` +
        `Use the package manager that owns this install (pnpm: pnpm add -g --allow-build=carapace carapace@${targetVersion}; Bun: bun add -g --trust carapace@${targetVersion}). On npm 11.15 and earlier, omit --allow-scripts=carapace.`,
      options,
    );
