...
import { CARAPACE_DATABASE_SCHEMA_DOCS_URL } from "./carapace-state-db-contract.js";
import { resolveCarapaceStateSqliteDir } from "./carapace-state-db.paths.js";

const CARAPACE_QUARANTINE_SCHEMA_VERSION = 2;
const CARAPACE_QUARANTINE_BUSY_TIMEOUT_MS = 5_000;
const CARAPACE_QUARANTINE_DIR_MODE = 0o700;
const CARAPACE_QUARANTINE_FILE_MODE = 0o600;

type CarapaceDatabaseKind = "agent" | "state";

type CarapaceDatabaseQuarantine = {
  kind: CarapaceDatabaseKind;
  quarantinedAt: number;
  reason: string;
};

// Read admission needs this error without importing schema migrations.
export function createCarapaceDatabaseVerificationError(
  kind: "agent" | "state",
  pathname: string,
  ...
  const error = new Error(
    `Carapace ${kind} database ${pathname} is quarantined after integrity verification failed: ${storedError ?? "unknown integrity error"}. Restore the database from a backup or repair it, then run carapace doctor --fix to clear the quarantine. See ${CARAPACE_DATABASE_SCHEMA_DOCS_URL}.`,
  );
...
function resolveQuarantineStorePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveCarapaceStateSqliteDir(env), "carapace-quarantine.sqlite");
}

function ensureQuarantineStoreDirectory(storePath: string): void {
  const dir = path.dirname(storePath);
  mkdirSync(dir, { recursive: true, mode: CARAPACE_QUARANTINE_DIR_MODE });
  applyPrivateModeSync(dir, CARAPACE_QUARANTINE_DIR_MODE);
}

function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  database.exec(`
    PRAGMA busy_timeout = ${CARAPACE_QUARANTINE_BUSY_TIMEOUT_MS};
    PRAGMA journal_mode = DELETE;
    PRAGMA synchronous = FULL;
  `);
  const userVersion = readQuarantineSchemaVersion(database, storePath);
  if (userVersion > CARAPACE_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Carapace quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === CARAPACE_QUARANTINE_SCHEMA_VERSION) {
    return;
  ...
  if (userVersion === 1) {
    database.exec(`
      ...
      PRAGMA user_version = ${CARAPACE_QUARANTINE_SCHEMA_VERSION};
      COMMIT;
    `);
    return;
  }
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Carapace quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
function withQuarantineWriter<T>(env: NodeJS.ProcessEnv, operation: (db: DatabaseSync) => T): T {
  ...
  try {
    if (!existed) {
      applyPrivateModeSync(storePath, CARAPACE_QUARANTINE_FILE_MODE);
    }
...
export function readCarapaceDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): CarapaceDatabaseQuarantine | undefined {
  ...
  const database = openNodeSqliteDatabase(storePath);
  try {
    database.exec(`PRAGMA busy_timeout = ${CARAPACE_QUARANTINE_BUSY_TIMEOUT_MS};`);
    const userVersion = readQuarantineSchemaVersion(database, storePath);
    if (userVersion === 0) {
      return undefined;
    }
    ...
    if (
      ...
      throw new Error(`Carapace quarantine store ${storePath} contains an invalid row.`);
    }
...
export function recordCarapaceDatabaseQuarantine(options: {
...
export function clearCarapaceDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): boolean {
