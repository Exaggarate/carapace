...
import { normalizeNullableString as migratedText } from "@carapace/normalization-core/string-coerce";
...
import { ensureCarapaceAgentBoardSchemaInTransaction } from "./carapace-agent-board-schema.js";
import {
...
import * as maintenanceAuthority from "./carapace-agent-db-lease.js";
import { ensureCarapaceAgentDatabasePermissions } from "./carapace-agent-db-permissions.js";
import { registerCarapaceAgentDatabase } from "./carapace-agent-db-registry.js";
import {
  assertExistingAgentSchemaOwner,
  assertCarapaceAgentCurrentRuntimeSchema,
  ...
  repairAndAssertCarapaceAgentV14SchemaForMigration,
...
import { migrateSessionNodesAndWindows } from "./carapace-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./carapace-agent-db-session-provenance.js";
...
} from "./carapace-agent-participants-migration.js";
import { hasPendingInputConsumptionColumnMigration } from "./carapace-agent-pending-inputs-schema.js";
...
type CarapaceAgentMetadataDatabase = Pick<CarapaceAgentKyselyDatabase, "schema_meta">;
...
function migrateCarapaceAgentSchema(db: DatabaseSync): void {
...
function backfillCarapaceAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function* agentDatabaseIntegrityBeforeMutationSteps(
  ...
  if (userVersion === CARAPACE_AGENT_SCHEMA_VERSION && !hasPendingCurrentVersionMigration) {
    yield* verifyAndRepairCanonicalSqliteIndexSteps(database, pathname, CARAPACE_AGENT_SCHEMA_SQL, {
      allowMissingColumns: true,
      validateAfterRepair: () =>
        assertCarapaceAgentCurrentRuntimeSchema(database, { agentId, pathname }),
      diagnostics,
    });
    assertCarapaceAgentCurrentRuntimeSchema(database, { agentId, pathname });
  } else if (
...
function persistAgentSchemaMetadata(
  ...
  executeSqliteQuerySync(
    db,
    getNodeSqliteKysely<CarapaceAgentMetadataDatabase>(db)
...
function ensureAgentSchema(
  ...
  agentId: string,
  pathname: string,
  targetVersion = CARAPACE_AGENT_SCHEMA_VERSION,
): void {
  const schemaSql =
    targetVersion < 18
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion > targetVersion) {
        throw new Error(
          `Carapace agent database ${pathname} uses schema version ${previousVersion}; expected at most ${targetVersion} for this migration.`,
        );
      }
      if (previousVersion === AGENT_MEDIA_SCHEMA_VERSION) {
        const legacySql = withLegacySessionParticipantsSchema(CARAPACE_AGENT_SCHEMA_SQL);
      ...
      if (previousVersion === targetVersion) {
        ...
        repairAndAssertCarapaceAgentV14SchemaForMigration(db, { agentId, pathname });
      }
      ...
      migrateCarapaceAgentSchema(db);
      migrateConversationDeliveryTargetColumn(db);
      backfillCarapaceAgentSchema(db, previousVersion);
      ...
      migrateMemoryChunkMetadataSchema(db);
      if (previousVersion < targetVersion) {
        ensureCarapaceAgentBoardSchemaInTransaction(db);
      }
...
export function ensureCarapaceAgentDatabaseSchema(
  db: DatabaseSync,
  options: CarapaceAgentDatabaseOptions & { register?: boolean },
): void {
  runSqliteIntegrityOperationSync(ensureCarapaceAgentDatabaseSchemaSteps(db, options));
}

/** Share one schema sequence between synchronous callers and leased maintenance. */
export function* ensureCarapaceAgentDatabaseSchemaSteps(
  db: DatabaseSync,
  options: CarapaceAgentDatabaseOptions & { register?: boolean },
  ...
  if (databaseOptions.register === true) {
    registerCarapaceAgentDatabase({ agentId, path: pathname, env: databaseOptions.env });
  }
}

/** Upgrade older owned databases to the structural schema required by the media cutover. */
export function migrateCarapaceAgentDatabaseToMediaPrerequisiteSchema(
  db: DatabaseSync,
...
export { ensureAgentSchema as ensureCarapaceAgentSchema };
