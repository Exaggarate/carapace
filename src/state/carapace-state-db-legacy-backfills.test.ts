...
describe("repairLegacySubagentSuspensionReasons", () => {
  it("rewrites the shipped reason on open and stays canonical after a second open", () => {
    const stateDir = tempDirs.make("carapace-subagent-suspension-backfill-");
    ...
    const firstOpen = openCarapaceStateDatabase(options);
    ...
    const secondOpen = openCarapaceStateDatabase(options);
