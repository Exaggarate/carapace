...
} = await import("./carapace-agent-db.js");
const { closeCarapaceStateDatabaseForTest } = await import("./carapace-state-db.js");
...
describe("agent database permission repair", () => {
  ...
  it("rolls back an outer write when pre-commit permission repair fails", () => {
    const stateDir = tempDirs.make("carapace-agent-chmod-");
    const options = {
      agentId: "worker-1",
