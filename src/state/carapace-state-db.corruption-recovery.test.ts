...
type StateDbTestDatabase = Pick<CarapaceStateKyselyDatabase, "diagnostic_events">;
...
function createTempStateDir(): string {
  return fs.realpathSync(tempStateDirs.make("carapace-state-db-corruption-"));
}
...
function prepareCorruptedCachedDatabase(env: NodeJS.ProcessEnv): {
  ...
  poisoned: ReturnType<typeof openCarapaceStateDatabase>;
} {
  const poisoned = openCarapaceStateDatabase({ env });
...
describe("shared state write transaction corruption recovery", () => {
  it("preserves the shared WAL when evicting a poisoned cache owner", () => {
    const env = { CARAPACE_STATE_DIR: createTempStateDir() };
    const cached = openCarapaceStateDatabase({ env });
    ...
    try {
      ...
      expect(
        carapaceStateDatabaseCache.evictCarapaceStateDatabaseAfterCorruption(
          cached,
      ...
      expect(
        carapaceStateDatabaseCache.getCarapaceStateDatabaseIfOpenAtPath(cached.path),
      ).toBeUndefined();
  ...
  it("does not evict a different cached owner when an injected write handle fails", () => {
    ...
    try {
      ...
      expect(carapaceStateDatabaseCache.getCarapaceStateDatabaseIfOpenAtPath(cached.path)).toBe(
        cached,
      );
