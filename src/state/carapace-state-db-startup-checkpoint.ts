...
import { ensureColumn } from "./carapace-state-db-schema-helpers.js";
...
export function withCarapaceStateStartupCheckpointConnection<T>(
...
export function initializeNativeCarapaceStateConnection(
  options: CarapaceStateDatabaseOptions,
  initializeCanonicalSchema: (db: DatabaseSync, pathname: string, env: NodeJS.ProcessEnv) => void,
): void {
  if (
  ...
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
  runWithCarapaceStateWriteAccess({ databasePath: pathname, env }, "native state bootstrap", () => {
    const db = openNodeSqliteDatabase(pathname);
    try {
      if (!isUninitializedNativeStartupDatabase(db)) {
        return;
