...
export function createCarapaceAgentDatabaseAdmissionOwner(
  ...
  function withCarapaceAgentDatabaseAsync<T>(
    inputOptions: CarapaceAgentDatabaseOptions,
    operation: (database: CarapaceAgentDatabase) => T | Promise<T>,
  ): Promise<T> {
    ...
    if (existing?.controller.signal.aborted) {
      return existing.promise.then(
        () => withCarapaceAgentDatabaseAsync(options, operation),
        () => withCarapaceAgentDatabaseAsync(options, operation),
      );
    }
    const pending = existing ?? startCarapaceAgentDatabaseAdmission(options, agentId, pathname);
  ...
  async function withCarapaceAgentDatabaseAdmission<T>(
    inputOptions: CarapaceAgentDatabaseOptions,
    withAdmission: CarapaceAgentDatabaseWriteAdmission,
    operation: (database: CarapaceAgentDatabase) => T | Promise<T>,
  ): Promise<T> {
    const options = { ...inputOptions, env: { ...(inputOptions.env ?? process.env) } };
    const agentId = normalizeAgentId(options.agentId);
    ...
    if (existing) {
      ...
      return withCarapaceAgentDatabaseAdmission(options, withAdmission, operation);
    }
    const admission = createCarapaceAgentDatabaseAdmission(agentId, pathname);
    ...
    try {
      while (true) {
        const outcome = await withAdmission(async (assertCurrent) => {
          try {
            assertCurrent();
            assertCarapaceAgentDatabaseAdmissionCurrent(options, pending, check?.database);
          } catch (error) {
  ...
  function createCarapaceAgentDatabaseAdmission(agentId: string, pathname: string) {
    const completion = createDeferredCore<CarapaceAgentDatabase>();
    ...
    return {
      pending,
      complete: (database: CarapaceAgentDatabase) => {
        retire();
        if (
  ...
  function assertCarapaceAgentDatabaseAdmissionCurrent(
    options: CarapaceAgentDatabaseOptions,
    pending: PendingAgentDatabaseOpen,
  ...
  function startCarapaceAgentDatabaseAdmission(
    options: CarapaceAgentDatabaseOptions,
    agentId: string,
    pathname: string,
    ...
    void (async () => {
      ...
      while (!step.done) {
        ...
        try {
          assertCarapaceAgentDatabaseAdmissionCurrent(options, pending, step.value.database);
        } catch (error) {
          failure = error;
          failed = true;
        }
  ...
  return { withCarapaceAgentDatabaseAsync, withCarapaceAgentDatabaseAdmission };
}
