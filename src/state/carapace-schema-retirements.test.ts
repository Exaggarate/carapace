...
import retirementManifest from "./carapace-schema-retirements.json" with { type: "json" };
...
it("keeps the schema retirement ledger aligned with canonical database schemas", () => {
  ...
  const databases = {
    state: {
      currentVersion: CARAPACE_STATE_SCHEMA_VERSION,
      schemaUrl: new URL("./carapace-state-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
    agent: {
      currentVersion: CARAPACE_AGENT_SCHEMA_VERSION,
      schemaUrl: new URL("./carapace-agent-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
  } satisfies Record<
    DatabaseOwner,
