...
import { startCarapaceStateLeaseHeartbeat } from "./carapace-state-lease-heartbeat.js";
import {
  acquireCarapaceStateLeaseInTransaction,
  readCarapaceStateLeaseExpiry,
  releaseCarapaceStateLeaseInTransaction,
  renewCarapaceStateLeaseInTransaction,
} from "./carapace-state-lease-store.js";

type CarapaceStateLeaseDatabase = {
  scope: "shared";
  options?: CarapaceStateDatabaseOptions;
};

type CarapaceStateLeaseOptions = {
  scope: string;
  key: string;
  database: CarapaceStateLeaseDatabase;
  leaseMs: number;
  waitMs: number;
  signal?: AbortSignal;
...
export type CarapaceStateLeaseContext = {
  signal: AbortSignal;
...
type CarapaceStateLeaseErrorCode =
  | "CARAPACE_STATE_LEASE_INVALID_INPUT"
  | "CARAPACE_STATE_LEASE_TIMEOUT"
  | "CARAPACE_STATE_LEASE_ABORTED"
  | "CARAPACE_STATE_LEASE_LOST"
  | "CARAPACE_STATE_LEASE_STORAGE_FAILED";

export class CarapaceStateLeaseError extends Error {
  readonly code: CarapaceStateLeaseErrorCode;

  constructor(message: string, options: { code: CarapaceStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "CarapaceStateLeaseError";
...
function leaseError(
  code: CarapaceStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): CarapaceStateLeaseError {
  return new CarapaceStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): CarapaceStateLeaseError {
  return leaseError("CARAPACE_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: CarapaceStateLeaseOptions) {
...
function withLeaseWriteTransaction<T>(
  ...
  const stateDatabase = openCarapaceStateDatabase(database.options);
  const run = () =>
    runCarapaceStateWriteTransaction(
      ({ db }) => operation(db),
...
function tryAcquire(
  ...
  return withLeaseWriteTransaction(params.database, params.operationLabel, (db) =>
    acquireCarapaceStateLeaseInTransaction(db, params, params.leaseMs),
  );
}
...
function renew(
  ...
  return withLeaseWriteTransaction(params.database, params.operationLabel, (db) => {
    const expiresAt = renewCarapaceStateLeaseInTransaction(db, params, params.leaseMs);
...
function assertLeaseOwnedInDatabase(database: DatabaseSync, params: LeaseIdentity): number {
  const expiresAt = readCarapaceStateLeaseExpiry(database, params);
  if (expiresAt === undefined) {
    throw leaseError(
      "CARAPACE_STATE_LEASE_LOST",
      `${params.leaseLabel} ${params.scope}/${params.key} was lost`,
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: CarapaceStateLeaseDatabase; transaction?: DatabaseSync },
): number {
  try {
    ...
    return assertLeaseOwnedInDatabase(
      openCarapaceStateDatabase(params.database.options).db,
      params,
    );
  } catch (error) {
    if (error instanceof CarapaceStateLeaseError) {
      throw error;
    }
    throw leaseError(
      "CARAPACE_STATE_LEASE_STORAGE_FAILED",
...
function release(
  ...
  withLeaseWriteTransaction(params.database, params.operationLabel, (db) =>
    releaseCarapaceStateLeaseInTransaction(db, params),
  );
}
...
export async function withCarapaceStateLease<T>(
  options: CarapaceStateLeaseOptions,
  ...
  let closed = false;
  let workerHeartbeat: ReturnType<typeof startCarapaceStateLeaseHeartbeat> | undefined;
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof CarapaceStateLeaseError
          ? cause
          : leaseError(
              "CARAPACE_STATE_LEASE_LOST",
  ...
  const renewFromTimer = () => {
    try {
      ...
      if (error instanceof CarapaceStateLeaseError && error.code === "CARAPACE_STATE_LEASE_LOST") {
        abortLost(error);
  ...
  try {
    ...
    try {
      if (validated.heartbeat === "worker") {
        workerHeartbeat = startCarapaceStateLeaseHeartbeat({
          path: openCarapaceStateDatabase(validated.database.options).path,
          identity,
          leaseMs: validated.leaseMs,
          heartbeatMs,
