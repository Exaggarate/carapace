...
function sessionTranscriptArchiveSchemaSql(): string {
  const start = CARAPACE_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_START);
  const end = CARAPACE_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Carapace session transcript archive schema markers are missing.");
  }
