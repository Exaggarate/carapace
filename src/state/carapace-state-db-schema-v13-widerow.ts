...
import { repairLegacySubagentRetainedResults } from "./carapace-state-db-legacy-backfills.js";
...
function reprojectLegacyCronJson(db: DatabaseSync): void {
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Carapace v12 cron job row is not canonical");
    }
...
function rebuildJsonCanonicalTable(db: DatabaseSync, tableName: string): void {
  ...
  if (tableExists(db, migrationTable)) {
    throw new Error(`Carapace v13 migration table already exists: ${migrationTable}`);
  }
  ...
  const migrationSchema = CARAPACE_STATE_SCHEMA_SQL.slice(start, end + endMarker.length).replace(
    startMarker,
