...
import { invalidateRegisteredAgentDatabasesMemo } from "./carapace-agent-db-registry-listing.js";
import {
  invalidateCarapaceAgentDatabaseValidation,
  invalidateCarapaceAgentDatabaseValidationsForAgent,
...
import { resolveCarapaceAgentDatabaseStoredPath } from "./carapace-state-db.paths.js";

export {
  inspectCarapaceRegisteredAgentDatabases,
  listCarapaceRegisteredAgentDatabases,
  readCarapaceAgentDatabaseRegistryToken,
} from "./carapace-agent-db-registry-listing.js";
...
export function createCarapaceAgentDatabasePathMatcher(): (left: string, right: string) => boolean {
...
export function isSameCarapaceAgentDatabasePath(left: string, right: string): boolean {
...
export function registerCarapaceAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
  schemaVersion?: number;
}): void {
  if (!isPersistentCarapaceAgentDatabasePath(params.path, params.env)) {
    return;
  }
  ...
  runCarapaceStateWriteTransaction(
    (database) => {
      ...
      const storedPath = resolveCarapaceAgentDatabaseStoredPath(database.path, params.path);
      const db = getNodeSqliteKysely<CarapaceAgentRegistryDatabase>(database.db);
      executeSqliteQuerySync(
        ...
        db
          ...
          .values({
            ...
            path: storedPath,
            schema_version: params.schemaVersion ?? CARAPACE_AGENT_SCHEMA_VERSION,
  ...
  invalidateCarapaceAgentDatabaseValidation(params.path);
...
export function isPersistentCarapaceAgentDatabasePath(
  pathname: string,
...
export function unregisterCarapaceAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
}): void {
...
export function unregisterCarapaceAgentDatabases(params: {
  agentId: string;
  env?: NodeJS.ProcessEnv;
  ...
  invalidateCarapaceAgentDatabaseValidationsForAgent(params.agentId);
