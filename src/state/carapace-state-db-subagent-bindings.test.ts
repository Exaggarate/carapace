...
// Exact affected tables from v2026.6.34:src/state/carapace-state-schema.sql (schema v1).
...
function createReleasedDatabase(execution: "running" | "terminal") {
  const stateDir = fs.realpathSync(tempDirs.make("carapace-released-subagent-binding-"));
...
describe("released subagent task bindings", () => {
  it.each(["terminal", "running"] as const)(
    ...
    (execution) => {
      ...
      const upgraded = openCarapaceStateDatabase();
      ...
      closeCarapaceStateDatabaseForTest();
      expect(snapshot(openCarapaceStateDatabase().db)).toEqual(firstOpen);
  ...
  it("rolls back every binding and earlier repair when one binding write fails", () => {
    ...
    expect(() => openCarapaceStateDatabase()).toThrow("binding write refused");
