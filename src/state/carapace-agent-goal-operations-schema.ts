...
export function ensureSessionGoalOperationsSchema(db: DatabaseSync): void {
  ...
  if (start < 0 || end < 0) {
    throw new Error("Carapace Goal operation schema markers are missing.");
  }
  runSqliteImmediateTransactionSync(db, () => {
    db.exec(CARAPACE_AGENT_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- Canonical additive DDL only.
  });
