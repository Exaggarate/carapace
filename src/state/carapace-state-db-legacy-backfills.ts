...
import { compactLegacyDeliveryQueueFailures } from "./carapace-state-db-delivery-queue-backfill.js";
import * as operatorApprovalMigration from "./carapace-state-db-operator-approval-migration.js";
import { ensureColumn, tableExists, tableHasColumn } from "./carapace-state-db-schema-helpers.js";
...
export function backfillAcpReplayEstimatedBytes(db: DatabaseSync): void {
  ...
  const replayDb =
    getNodeSqliteKysely<
      Pick<CarapaceStateKyselyDatabase, "acp_replay_events" | "acp_replay_sessions">
    >(db);
