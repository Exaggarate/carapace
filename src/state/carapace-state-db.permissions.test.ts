...
vi.mock("node:fs", async (importOriginal) => {
  ...
  const chmodSync: typeof actual.chmodSync = ((target: unknown, mode: unknown) => {
    ...
    const isProbe = String(target).includes(".carapace-chmod-probe-");
...
} = await import("./carapace-state-db.js");
...
describe("state database permission hardening without chmod support", () => {
  ...
  it("opens the state database when chmodSync throws ENOTSUP", () => {
    const stateDir = tempDirs.make("carapace-state-chmod-");
  ...
  it("rethrows EPERM when existing permissions are too broad", () => {
    ...
    expect(() => openCarapaceStateDatabase({ env: { CARAPACE_STATE_DIR: stateDir } })).toThrow(
      /EPERM/,
    );
  });
  ...
  it.each(["-wal", "-shm", "-journal"])(
    ...
    (suffix) => {
      ...
      closeCarapaceStateDatabaseForTest();
      const sidecarPath = join(stateDir, "state", `carapace.sqlite${suffix}`);
  ...
  it("rethrows when the main database vanishes between the existence check and the chmod", () => {
    ...
    openCarapaceStateDatabase(options);
    closeCarapaceStateDatabaseForTest();
    chmodFailHook.removeTargetSuffix = "carapace.sqlite";

    expect(() => openCarapaceStateDatabase(options)).toThrow(/ENOENT/);
    // Guards against a vacuous pass if the main file were skipped before chmod.
    expect(chmodFailHook.targets.some((target) => target.endsWith("carapace.sqlite"))).toBe(true);
  });
  ...
  it("repairs the schema when chmodSync throws ENOTSUP", () => {
    ...
    expect(() =>
      repairCarapaceStateDatabaseSchema({ env: { CARAPACE_STATE_DIR: stateDir } }),
    ).not.toThrow();
  });
  ...
  it("commits write transactions when chmodSync throws ENOTSUP", () => {
    ...
    const result = runCarapaceStateWriteTransaction((database) => {
      expect(database.db.isOpen).toBe(true);
      return "committed";
    }, options);
