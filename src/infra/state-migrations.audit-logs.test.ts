  ...
  it("imports config and system audit JSONL only through explicit doctor repair", async () => {
    await withAuditMigrationFixture(async (audit) => {
      ...
      const laterConfigRecord = {
        ...
        argv: ["carapace", "config", "set", "token", "later-redaction-marker"],
      };
    ...
    await withAuditMigrationFixture(async (audit) => {
      ...
      const records = Array.from({ length: 10_000 }, (_, index) =>
        JSON.stringify({
          ...
          source: "config-io",
          event: "config.write",
          argv: ["carapace", "config", "set", `key-${index}`, "value"],
          execArgv: [],
        }),
      );
      ...
      const retainedRecord = configAuditRecord("value", {
        ...
        argv: ["carapace", "config", "set", "later", "value"],
      });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const externalAuditDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "carapace-audit-migration-external-"),
      );
      try {
        await withAuditMigrationFixture(async (audit) => {
