...
describe.skipIf(process.platform !== "win32")("native Windows source CLI shim", () => {
  it.each(["off", "on"])(
    ...
    async (delayedExpansion) => {
      await withTempDir("carapace-source-cli-win-", async (root) => {
        ...
        const shimPath = path.join(stateDir, "tmp", "agent-cli", "carapace.cmd");
  ...
  it("preserves literal forwarded bangs and percent signs", async () => {
    await withTempDir("carapace-source-cli-args-win-", async (root) => {
      const fixture = await createSourceCliFixture(root);
      const stateDir = path.join(root, "state");
  ...
  it.each(["generic Node host", "bare TSX source parent"])(
    ...
    async (parent) => {
      await withTempDir("carapace-source-cli-host-win-", async (root) => {
        const fixture = await createSourceCliFixture(root);
        ...
        await fs.writeFile(hostEntry, "// A generic Node host is not the Carapace CLI entry.\n");
        ...
        const invocation = resolveCurrentCarapaceCliInvocation(["probe"], {
