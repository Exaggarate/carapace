...
async function writePnpmIsolatedPackage(params: {
  ...
  const packageRoot = path.join(installRoot, "node_modules", "carapace");
...
describe("pnpm isolated install preflight (v11 layout)", () => {
  it("rejects grouped installs before dropping sibling packages", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-group-" }, async (base) => {
  ...
  it("rejects multiple standalone installs before an alias-wide update", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-multiple-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      await writePnpmIsolatedPackage({
        globalRoot,
        installName: "other",
  ...
  it("rejects an orphaned invoking install before manager probes", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-invoking-orphan-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const packageRoot = path.join(globalRoot, "orphan", "node_modules", "carapace");
  ...
  it("rejects an orphan whose package symlink shares the active store target", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-shared-store-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
  ...
  it.each(["unset", "conflicting", "empty"] as const)(
    ...
    async (aliases) => {
      await withTestDir({ prefix: "carapace-package-update-pnpm-isolated-" }, async (base) => {
        ...
        const oldPackageRoot = path.join(globalRoot, "old", "node_modules", "carapace");
        const newPackageRoot = path.join(globalRoot, "new", "node_modules", "carapace");
        ...
        await fs.symlink(
          ...
          path.join(globalRoot, "hash-carapace"),
          "dir",
        );
        ...
        const runStep = vi.fn(
          async ({ name, argv, cwd, env }): Promise<PackageUpdateStepResult> => {
            if (name === "global update") {
              ...
              await fs.rm(path.join(globalRoot, "hash-carapace"), { force: true });
              ...
              await Promise.all([
                ...
                fs.writeFile(
                  path.join(newPackageRoot, ".carapace-lifecycle-pending"),
                  "pending\n",
                  "utf8",
                ),
                fs.writeFile(
                ...
                fs.writeFile(
                  ...
                  JSON.stringify({ private: true, dependencies: { carapace: "2.0.0" } }),
                  "utf8",
                ),
              ]);
              await fs.symlink(
              ...
              await expect(
                fs.readFile(path.join(newPackageRoot, ".carapace-lifecycle-pending"), "utf8"),
              ...
              await fs.rm(path.join(newPackageRoot, ".carapace-lifecycle-pending"));
            } else {
        ...
        await expectPathMissing(path.join(newPackageRoot, ".carapace-lifecycle-pending"));
  ...
  it("accepts a replacement pnpm project that reuses the same shared-store package", async () => {
    await withTestDir(
      { prefix: "carapace-package-update-pnpm-shared-replacement-" },
      async (base) => {
        const globalDir = path.join(base, "pnpm-home", "global");
        const globalRoot = path.join(globalDir, "v11");
        ...
        const oldPackageRoot = path.join(oldInstallRoot, "node_modules", "carapace");
        const newPackageRoot = path.join(newInstallRoot, "node_modules", "carapace");
        const sharedPackageRoot = path.join(base, "store", "carapace");
        const activeLink = path.join(globalRoot, "hash-carapace");
        await Promise.all([
  ...
  it("preserves pnpm local specs before mutating from the owner root", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-relative-spec-" }, async (base) => {
      const globalDir = path.join(base, "pnpm-home", "global");
      const globalRoot = path.join(globalDir, "v11");
      const globalBinDir = path.join(base, "pnpm-home", "bin");
      const callerProjectDir = path.join(base, "caller-project");
      ...
      const cases: Array<{
      ...
      }> = [
        {
          ...
          expectedInstallSpec: `carapace@file:${candidateTarball}`,
        },
        { installSpec: candidateTarball, expectedInstallSpec: `carapace@file:${candidateTarball}` },
        { installSpec: "candidate.tgz", expectedInstallSpec: `carapace@file:${candidateTarball}` },
        { installSpec: callerProjectDir, expectedInstallSpec: `carapace@link:${callerProjectDir}` },
        { installSpec: ".", expectedInstallSpec: `carapace@link:${callerProjectDir}` },
        {
          installSpec: "../checkout",
          expectedInstallSpec: `carapace@link:${path.join(base, "checkout")}`,
        },
        {
          installSpec: "file:./candidate",
          expectedInstallSpec: `carapace@file:${path.join(callerProjectDir, "candidate")}`,
        },
        {
          installSpec: "carapace@link:./candidate",
          expectedInstallSpec: `carapace@link:${path.join(callerProjectDir, "candidate")}`,
        },
        {
        ...
        { installSpec: "./candidate.tar", expectedInstallSpec: `carapace@file:${candidateTar}` },
        {
          installSpec: "carapace@file:./candidate.tar",
          expectedInstallSpec: `carapace@file:${candidateTar}`,
        },
        { installSpec: "candidate.tar", expectedInstallSpec: "candidate.tar" },
        { installSpec: "carapace@candidate.tar", expectedInstallSpec: "carapace@candidate.tar" },
        {
          installSpec: "file:~/candidate.tgz",
          expectedInstallSpec: "carapace@file:~/candidate.tgz",
        },
        { installSpec: "~/candidate.tgz", expectedInstallSpec: "carapace@file:~/candidate.tgz" },
        { installSpec: "~/checkout", expectedInstallSpec: "carapace@link:~/checkout" },
        { installSpec: "carapace@latest", expectedInstallSpec: "carapace@latest" },
        {
        ...
        { installSpec: "@scope/candidate.tgz", expectedInstallSpec: "@scope/candidate.tgz" },
        {
          ...
          expectedInstallSpec: `carapace@file:${path.join(callerProjectDir, "package@1.0.0.tgz")}`,
        },
        {
          installSpec: "carapace@npm:other@1.0.0",
          expectedInstallSpec: "carapace@npm:other@1.0.0",
        },
        {
        ...
        { installSpec: "C:\\checkout", expectedInstallSpec: "carapace@link:C:\\checkout" },
        {
          installSpec: "C:\\candidate.tgz",
          expectedInstallSpec: "carapace@file:C:\\candidate.tgz",
        },
        {
          installSpec: "\\\\server\\checkout",
          expectedInstallSpec: "carapace@link:\\\\server\\checkout",
        },
        {
          installSpec: ".\\checkout",
          expectedInstallSpec: "carapace@link:C:\\caller\\checkout",
          installCwd: "C:\\caller",
        },
      ];
      const { packageRoot } = await writePnpmIsolatedPackage({
      ...
      const runStep = vi.fn(async ({ name, argv, cwd, env }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toEqual(["pnpm", "add", "-g", "--allow-build=carapace", expectedInstallSpec]);
        return {
          name,
          command: argv.join(" "),
          cwd: cwd ?? process.cwd(),
    ...
    async (installCwd) => {
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        expect(argv.at(-1)).toBe(
          `carapace@file:${path.join(installCwd ?? process.cwd(), "candidate.tgz")}`,
        );
  ...
  it("rejects a pnpm command that owns another global root", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-root-" }, async (base) => {
      ...
      expect(result.failedStep?.stderrTail).toContain("not the invoking Carapace install");
  ...
  it("rejects a pnpm update that leaves only an orphaned old package root", async () => {
    await withTestDir({ prefix: "carapace-package-update-pnpm-orphan-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const globalBinDir = path.join(base, "pnpm-home", "bin");
  ...
  it.each(["pnpm package preinstall", "pnpm package postinstall"])(
    ...
    async (failedLifecycleStep) => {
      await withTestDir({ prefix: "carapace-package-update-pnpm-lifecycle-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          if (name === "global update" && firstAttempt) {
            ...
            await Promise.all([
              ...
              fs.writeFile(
                path.join(packageRoot, ".carapace-lifecycle-pending"),
                "pending\n",
                "utf8",
              ),
              fs.writeFile(
          ...
          if (name === "pnpm package postinstall" && exitCode === 0) {
            await fs.rm(path.join(packageRoot, ".carapace-lifecycle-pending"));
          }
          return {
            name,
            command: argv.join(" "),
        ...
        await expect(
          fs.readFile(path.join(packageRoot, ".carapace-lifecycle-pending"), "utf8"),
        ).resolves.toBe("pending\n");
        ...
        await expectPathMissing(path.join(packageRoot, ".carapace-lifecycle-pending"));
      });
    },
  );
});
