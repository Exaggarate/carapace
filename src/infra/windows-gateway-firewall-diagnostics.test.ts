...
function ruleRow(params?: {
  ...
  return {
    DisplayName: params?.displayName ?? "Carapace Gateway",
...
describe("Windows Gateway firewall diagnostics", () => {
  ...
  it.each([
    [
      ...
      "Carapace could not quickly inspect Windows Firewall LAN Gateway policy.",
    ],
    [
      "truncated stdout",
    ...
    [
      "malformed JSON",
      async () => ({ code: 0, stdout: "{" }),
      "Carapace could not parse Windows Firewall LAN Gateway policy.",
    ],
    [
      "a runner exception",
      async () => {
