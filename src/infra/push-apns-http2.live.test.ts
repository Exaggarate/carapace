...
const LIVE =
  (isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.CARAPACE_LIVE_TEST)) &&
  isTruthyEnvValue(process.env.CARAPACE_LIVE_APNS_REACHABILITY);
const describeLive = LIVE ? describe : describe.skip;

function getLiveTimeoutMs(): number {
  const raw = process.env.CARAPACE_LIVE_APNS_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_TIMEOUT_MS;
  }
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`CARAPACE_LIVE_APNS_TIMEOUT_MS must be a positive number, got ${raw}`);
  }
