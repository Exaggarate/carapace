...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "CARAPACE_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "carapace", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("carapace channels login --channel whatsapp");
  });
