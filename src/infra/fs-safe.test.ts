...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("carapace-fs-safe-");
  ...
  it("writes external command output within an allowed root", async () => {
    const dir = await tempDirs.make("carapace-fs-safe-output-");
  ...
  it.each([
    ...
    const dir = await tempDirs.make("carapace-fs-safe-output-name-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("carapace-fs-safe-root-");
    const sourceDir = await tempDirs.make("carapace-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("carapace-fs-safe-root-");
    const outside = await tempDirs.make("carapace-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    withEnv({ HOME: fakeHome, CARAPACE_HOME: fakeHome }, () => {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    });

    const root = await tempDirs.make("carapace-tilde-test-");
    await withEnvAsync({ HOME: root, CARAPACE_HOME: root }, async () => {
    ...
    const outsideRoot = await tempDirs.make("carapace-tilde-outside-");
    await expectRejectCode(
