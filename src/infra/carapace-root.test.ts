// Covers Carapace package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__carapace_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "carapace-root");
...
function setPackageRoot(root: string, name = "carapace") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./carapace-root.js").resolveCarapacePackageRootSync,
  asyncResolver: typeof import("./carapace-root.js").resolveCarapacePackageRoot,
  opts: Parameters<typeof import("./carapace-root.js").resolveCarapacePackageRootSync>[0],
...
vi.mock("./carapace-root.fs.runtime.js", () => ({
  carapaceRootFsSync: mockFsModule(),
  carapaceRootFs: mockFsPromisesModule(),
}));

describe("resolveCarapacePackageRoot", () => {
  let resolveCarapacePackageRoot: typeof import("./carapace-root.js").resolveCarapacePackageRoot;
  let resolveCarapacePackageRootSync: typeof import("./carapace-root.js").resolveCarapacePackageRootSync;

  beforeAll(async () => {
    ({ resolveCarapacePackageRoot, resolveCarapacePackageRootSync } =
      await import("./carapace-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "carapace");
        const pkgRoot = path.join(project, "node_modules", "carapace");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "carapace");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "carapace.mjs")));
        setPackageRoot(realPkg);
        return { opts: { argv1: bin }, expected: realPkg };
      },
    },
    {
      name: "prefers a symlink target nested under another carapace package",
      setup: () => {
        const sourceRoot = fx("nested-symlink-scenario");
        const bin = path.join(sourceRoot, ".artifacts", "prefix", "bin", "carapace");
        ...
        state.realpaths.set(abs(bin), abs(path.join(installedRoot, "carapace.mjs")));
    ...
    {
      name: "falls through from a non-carapace moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-carapace");
    ...
    {
      name: "returns null for non-carapace package roots",
      setup: () => {
        const pkgRoot = fx("not-carapace");
        setPackageRoot(pkgRoot, "not-carapace");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "carapace.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "carapace");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    {
      name: "still resolves the carapace package below a node_modules boundary",
      setup: () => {
