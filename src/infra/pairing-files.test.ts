...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/carapace-state", "devices")).toEqual({
      dir: path.join("/tmp/carapace-state", "devices"),
      pendingPath: path.join("/tmp/carapace-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/carapace-state", "devices", "paired.json"),
    });
  });
