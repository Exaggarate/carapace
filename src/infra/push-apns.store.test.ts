...
type TestDatabase = Pick<CarapaceStateKyselyDatabase, "apns_registrations">;

async function makeTempDir(): Promise<string> {
  return await tempDirs.make("carapace-push-apns-store-test-");
}
...
function databaseEnv(baseDir: string): NodeJS.ProcessEnv {
  return { ...process.env, CARAPACE_STATE_DIR: baseDir };
}
...
describe("push APNs registration store", () => {
  ...
  it("clears transport-specific columns when a node changes registration transport", async () => {
    ...
    const database = openCarapaceStateDatabase({ env: databaseEnv(baseDir) });
  ...
  it("fails loudly for a malformed canonical row", async () => {
    ...
    runCarapaceStateWriteTransaction(
      ({ db }) => {
        ...
        ).run("corrupt-node", "unknown", "ai.carapace.ios", "sandbox", 1);
      },
      { env },
    );
