...
export class DirectoryCache<T> {
  private cachesByConfig = new WeakMap<CarapaceConfig, Map<string, CacheEntry<T>>>();
   ...
   */
  get(key: string, cfg: CarapaceConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: CarapaceConfig): void {
    const cache = this.cacheForConfig(cfg);
    const now = Date.now();
   ...
   */
  clearMatching(match: (key: string) => boolean, cfg: CarapaceConfig): void {
  ...
  private cacheForConfig(cfg: CarapaceConfig): Map<string, CacheEntry<T>> {
