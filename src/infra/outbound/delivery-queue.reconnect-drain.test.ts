...
const MAX_RETRIES = 5;
const stubCfg = {} as CarapaceConfig;
...
describe("drainPendingDeliveriesCore for reconnect", () => {
  ...
  it("rejects recovered delivery when the current channel config disables its account", async () => {
    ...
    const cfg: CarapaceConfig = { channels: { discord: { enabled: false } } };
    const admitDeferredDelivery = vi.fn(({ cfg: currentConfig }: { cfg: CarapaceConfig }) =>
  ...
  it("skips entries that an in-flight live delivery has actively claimed", async () => {
    // Regression for carapace/carapace#70386: a reconnect drain that runs
