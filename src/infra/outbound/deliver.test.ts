...
const matrixChunkConfig: CarapaceConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as CarapaceConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredCarapaceTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: CarapaceConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as CarapaceConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
    ...
    await expect(
      deliverMatrix({
        deliveryCompletion: {
          ...
          storePath: "/tmp/carapace-missing-stale-writer.sqlite",
  ...
  it("finalizes owner state only after a chunked batch completes", async () => {
    ...
    await deliverMatrix({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as CarapaceConfig,
      payloads: [{ text: "abcd" }],
      deps: { matrix: sendMatrix },
      queuePolicy: "required",
      deliveryCompletion: {
  ...
  it("strips complete inline runtime context blocks before channel delivery", async () => {
    ...
    await deliverOutboundPayloads({
      ...
      payloads: [
        {
          text: "before <<<BEGIN_CARAPACE_INTERNAL_CONTEXT>>>private runtime metadata<<<END_CARAPACE_INTERNAL_CONTEXT>>> after",
        },
      ],
    });
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as CarapaceConfig;
    const sendPayload = installPayloadOutbound(
  ...
  it.each([
    ...
    async ({ ordinaryMedia, caption, mediaUrls }) => {
      ...
      const cfg = { channels: { line: {} } } as CarapaceConfig;
      ...
      expect(
        sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
          root.endsWith(path.join(".carapace", "workspace-work")),
        ),
      ).toBe(true);
      expect(sendMedia).not.toHaveBeenCalled();
    },
  ...
  it("includes Carapace tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverMatrix({
      cfg: { channels: { matrix: {} } } as CarapaceConfig,
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as CarapaceConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("queues a spool copy while the live send keeps the producer's path", async () => {
    ...
    // Carapace temp root that TTS actually writes to. Staging must resolve the
    ...
    const sourceDir = await fsPromises.realpath(
      await fsPromises.mkdtemp(path.join(resolvePreferredCarapaceTmpDir(), "deliver-spool-")),
    );
    const carapaceState = await createCarapaceTestState({
      layout: "state-only",
      prefix: "carapace-deliver-spool-state-",
    });
  ...
  it("stages agent-workspace media that only the agent-scoped capability can read", async () => {
    ...
    // Deliberately outside the Carapace temp root: that root is itself a default
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for carapace/carapace#70386: if a reconnect or startup drain
