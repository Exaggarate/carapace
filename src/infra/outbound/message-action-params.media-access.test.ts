...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Carapace tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideCarapace = path.join(os.tmpdir(), "outside-carapace", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
