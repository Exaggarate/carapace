...
const resolvePreferredCarapaceTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".carapace", "gateway.cmd");
  }),
);
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredCarapaceTmpDirMock.mockReset();
    resolvePreferredCarapaceTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ CARAPACE_PROFILE: "work" });
    const cmdExePath = getWindowsCmdExePath();

    expect(result.ok).toBe(true);
    expect(result.method).toBe("schtasks");
    expect(result.tried).toContain('schtasks /Run /TN "Carapace Gateway (work)"');
    ...
    expect(script).toContain(
      'carapace restart attempt source=windows-task-handoff target="Carapace Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$task = Get-ScheduledTask -TaskName 'Carapace Gateway (work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1" >nul 2>&1`,
    );
    expect(script).not.toContain("findstr");
    expect(script).toContain('schtasks /Run /TN "Carapace Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "Carapace Gateway (work)"'),
    );
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers CARAPACE_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Carapace Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      CARAPACE_WINDOWS_TASK_NAME: "Carapace Gateway (Bob's work)",
    });
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      `-Command "$task = Get-ScheduledTask -TaskName 'Carapace Gateway (Bob''s work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1"`,
    );
    expect(script).not.toContain("findstr");
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredCarapaceTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ CARAPACE_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-state-"));
  ...
  const asciiPathEnv = {
    ...
    CARAPACE_STATE_DIR: "C:\\ocw-test\\state",
  };
  ...
  it("writes marked code-page bytes for CJK task names that decode back exactly", () => {
    ...
    const result = relaunchGatewayScheduledTask({
      ...asciiPathEnv,
      CARAPACE_WINDOWS_TASK_NAME: "Carapace Gateway (隆)",
    });
    ...
    expect(
      raw
        .toString("latin1")
        .startsWith("@chcp 936 >nul\r\n@rem carapace-launcher-encoding=gbk\r\n"),
    ).toBe(true);
    ...
    expect(script.startsWith("@echo off\r\n")).toBe(true);
    expect(script).toContain('schtasks /Run /TN "Carapace Gateway (隆)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });
