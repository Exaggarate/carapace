...
describe("gateway-watch tmux wrapper", () => {
  it("derives stable session names from profile and port", () => {
    expect(resolveGatewayWatchTmuxSessionName({ args: ["gateway", "--force"], env: {} })).toBe(
      "carapace-gateway-watch-main",
    );
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force", "--port", "19001"],
        env: { CARAPACE_PROFILE: "Dev Profile" },
      }),
    ).toBe("carapace-gateway-watch-dev-profile-19001");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["--dev", "gateway", "--port=18789"],
        env: {},
      }),
    ).toBe("carapace-gateway-watch-dev");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--dev", "--port=18789"],
        env: {},
      }),
    ).toBe("carapace-gateway-watch-main");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--profile", "work", "--port=18789"],
        env: { CARAPACE_PROFILE: "main" },
      }),
    ).toBe("carapace-gateway-watch-work");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force"],
        env: { CARAPACE_GATEWAY_PORT: "127.0.0.1:18789" },
      }),
    ).toBe("carapace-gateway-watch-main");
  });
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      cwd: "/repo with spaces/carapace",
      env: {
        CARAPACE_DEV_SOURCE_ROOT: "/selected checkout",
        CARAPACE_GATEWAY_PORT: "19001",
        CARAPACE_GATEWAY_RESTART_TRACE: "1",
        CARAPACE_GATEWAY_STARTUP_TRACE: "1",
        CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR: "0",
        CARAPACE_PROFILE: "Dev Profile",
        CARAPACE_TRACE_SYNC_IO: "0",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
      sessionName: "carapace-gateway-watch-main",
    });

    expect(command).toContain("exec '/bin/zsh' -lc");
    expect(command).toContain("/repo with spaces/carapace");
    expect(command).toContain("'CARAPACE_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'CARAPACE_GATEWAY_WATCH_SESSION=carapace-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'CARAPACE_DEV_SOURCE_ROOT=/selected checkout'");
    expect(command).toContain("'CARAPACE_GATEWAY_PORT=19001'");
    expect(command).toContain("'CARAPACE_GATEWAY_RESTART_TRACE=1'");
    expect(command).toContain("'CARAPACE_GATEWAY_STARTUP_TRACE=1'");
    expect(command).toContain("'CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR=0'");
    expect(command).toContain("'CARAPACE_PROFILE=Dev Profile'");
    expect(command).toContain("'CARAPACE_TRACE_SYNC_IO=0'");
    expect(command).toContain("'\\''-u'\\'' '\\''CARAPACE_SKIP_CHANNELS'\\''");
  ...
  it("runs managed service handoff before watching and clears an omitted development selector", () => {
    ...
    expect(command).toContain("scripts/gateway-watch-tmux.mts");
    expect(command).toContain("'\\''-u'\\'' '\\''CARAPACE_DEV_SOURCE_ROOT'\\''");
    expect(command).not.toContain("CARAPACE_DEV_SOURCE_ROOT=");
  ...
  it("stops a loaded service on the watched port with CLI selector precedence", () => {
    ...
    const code = runGatewayWatchServiceHandoff({
      ...
      cwd: "/repo",
      env: { CARAPACE_GATEWAY_PORT: "19001", CARAPACE_PROFILE: "main" },
      nodePath: "/opt/node",
      spawnSync,
    ...
    expect(requireRecord(stopOptions.env, "stop env").CARAPACE_GATEWAY_PORT).toBe("18789");
  ...
  it("leaves a loaded managed Gateway running on a different port", () => {
    ...
    const code = runGatewayWatchServiceHandoff({
      args: ["gateway", "--force", "--port=19001"],
      cwd: "/repo",
      env: { CARAPACE_GATEWAY_PORT: "18789" },
      nodePath: "/opt/node",
      spawnSync,
      stderr: stderr.stream,
    });
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain("'CARAPACE_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'");
    expect(command).toContain("'CARAPACE_RUN_NODE_CPU_PROF_MAX_FILES=40'");
    expect(command).toContain("'CARAPACE_TRACE_SYNC_IO=0'");
  ...
  it("preserves an explicit benchmark CPU profile retention cap", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { CARAPACE_RUN_NODE_CPU_PROF_MAX_FILES: "8", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync);
    expect(command).toContain("'CARAPACE_RUN_NODE_CPU_PROF_MAX_FILES=8'");
  });
  ...
  it("preserves explicit sync I/O tracing in benchmark mode", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { CARAPACE_TRACE_SYNC_IO: "1", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync);
    expect(command).toContain("'CARAPACE_TRACE_SYNC_IO=1'");
    expect(command).toContain(
      "'CARAPACE_RUN_NODE_OUTPUT_LOG=.artifacts/gateway-watch-profiles/gateway-watch-output.log'",
    );
    expect(command).toContain("'CARAPACE_RUN_NODE_FILTER_SYNC_IO_STDERR=1'");
    expect(stderr.chunks.join("")).toContain(
  ...
  it("creates a detached tmux session when none exists", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 0, "tmux", ["has-session", "-t", "carapace-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
    ...
    expect(
      expectSpawn(spawnSync, 5, "tmux", [
        ...
        "-t",
        "carapace-gateway-watch-main",
        "CARAPACE_GATEWAY_WATCH_CWD",
        "/repo",
      ]).encoding,
    ).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch started in tmux session carapace-gateway-watch-main",
    );
    expect(stdout.chunks.join("")).toContain("tmux attach -t carapace-gateway-watch-main");
    expect(stdout.chunks.join("")).toContain(
      "tmux capture-pane -ep -t carapace-gateway-watch-main -S -200",
    );
    expect(stdout.chunks.join("")).toContain(
      "tmux show-options -v -t carapace-gateway-watch-main @carapace.gateway_watch.cwd",
    );
  });
  ...
  it("auto-attaches in an interactive terminal after creating a session", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 6, "tmux", ["attach-session", "-t", "carapace-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  ...
  it("switches tmux clients instead of nesting attach when already inside tmux", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 5, "tmux", ["switch-client", "-t", "carapace-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  });
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    expectSpawn(spawnSync, 1, "tmux", [
      ...
      "-w",
      "-t",
      "carapace-gateway-watch-dev-19001",
      "remain-on-exit",
      "on",
    ]);
    ...
    expect(String(respawnArgs[6])).toContain("CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR=0");
    expect(String(respawnArgs[6])).toContain("CARAPACE_SKIP_CHANNELS");
    expect(requireRecord(respawnCall[2], "spawn options").encoding).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch restarted in tmux session carapace-gateway-watch-dev-19001",
    );
  });
  ...
  it("recreates a stale session when its active pane target is missing", () => {
    ...
    expect(
      expectSpawn(spawnSync, 2, "tmux", ["kill-session", "-t", "carapace-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
  ...
  it("runs the raw foreground watcher when tmux mode is disabled", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force"],
      cwd: "/repo",
      env: { CARAPACE_GATEWAY_WATCH_TMUX: "0" },
      nodePath: "/node",
      spawnSync,
    });
