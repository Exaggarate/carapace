...
describe("tmp-carapace-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__carapaceTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredCarapaceTmpDir } from "./src/infra/tmp-carapace-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: DEFAULT_POSIX_TMP_ROOT,
            resolverType: typeof resolvePreferredCarapaceTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-carapace-dir-browser-entry.ts",
      },
      write: false,
    });
