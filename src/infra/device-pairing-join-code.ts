...
import { ensureDevicePairingJoinCodeSchema } from "../state/carapace-state-db-schema-additive.js";
...
type DevicePairingJoinCodeDatabase = Pick<CarapaceStateKyselyDatabase, "device_pairing_join_codes">;
...
export function redeemDevicePairingJoinCode(params: {
  ...
  const payloadJson = runCarapaceStateWriteTransaction(({ db }) => {
    ensureJoinCodeSchema(db);
    const kysely = getNodeSqliteKysely<DevicePairingJoinCodeDatabase>(db);
    executeSqliteQuerySync(
      db,
