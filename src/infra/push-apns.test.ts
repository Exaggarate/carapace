...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.carapace.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const carapacePayload = requireRecord(payload.carapace, "carapace payload");
    expectRecordFields(carapacePayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof carapacePayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.carapace.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      ...
      sound: "default",
      category: "carapace.exec-approval",
      "content-available": 1,
    });
    const carapacePayload = requireRecord(payload.carapace, "carapace payload");
    expectRecordFields(carapacePayload, {
    ...
    expect(typeof carapacePayload.ts).toBe("number");
    expectNoProperties(carapacePayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("builds plugin approval alerts with request copy and a bounded body", async () => {
    ...
    expect(payload.aps).toEqual({
      ...
      sound: "default",
      category: "carapace.plugin-approval",
      "content-available": 1,
    });
    const carapacePayload = requireRecord(payload.carapace, "carapace payload");
    expectRecordFields(carapacePayload, {
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    const payload = requirePayload(requireSendRequest(send));
    expectRecordFields(requireRecord(payload.carapace, "carapace payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
