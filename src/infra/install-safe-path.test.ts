...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "carapace-install-safe-dir-symlink-probe-"),
    );
...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@carapace/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@carapace/matrix", id: "matrix", expected: true },
    { packageName: "@carapace/matrix", id: "@carapace/matrix", expected: true },
    { packageName: "@carapace/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@carapace/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@carapace__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTestDir({ prefix: "carapace-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTestDir({ prefix: "carapace-install-safe-" }, async (baseDir) => {
        await withTestDir({ prefix: "carapace-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTestDir({ prefix: "carapace-install-safe-" }, async (parentDir) => {
