 ...
 * `carapace doctor --fix` repairs the default root while a scoped install stays blocked.
...
function doctorFixInstruction(filePath: string): string {
  const command = "Run `carapace doctor --fix`";
  return process.env.CARAPACE_STATE_DIR?.trim()
    ? `${command} with CARAPACE_STATE_DIR set to ${path.dirname(filePath)}`
    : command;
}
