...
import { createRequireRecord, bundledPluginFile } from "carapace/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "carapace.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTestDir({ prefix: "carapace-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.CARAPACE_WATCH_MODE).toBe("1");
      expect(spawnEnv.CARAPACE_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.CARAPACE_NO_RESPAWN).toBe("1");
      expect(spawnEnv.CARAPACE_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.CARAPACE_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTestDir({ prefix: "carapace-watch-node-" }, async (cwd) => {
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).CARAPACE_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { CARAPACE_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.carapace.gateway");
    expect(spawnEnv.CARAPACE_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/carapace/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/carapace/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        ...
        ["[carapace] Invalid package config: /tmp/carapace/.pnpm/chokidar/package.json"],
        ["[carapace] This usually means a file in node_modules is empty or truncated."],
        ["[carapace] Recommended recovery:"],
        ["[carapace]   rm -rf node_modules"],
        ["[carapace]   pnpm store prune"],
        ["[carapace]   pnpm install"],
        [""],
        ["[carapace] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTestDir({ prefix: "carapace-watch-node-lock-" }, async (cwd) => {
