...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "carapace-gateway-lock-workshop-",
});
let fixtureRoot = "";
...
describe("Gateway lock roles", () => {
  ...
  it("keeps a live Workshop apply owner while Gateway startup races", async () => {
    ...
    const readProcessCmdline = () => ["carapace", "skills", "workshop", "apply", "proposal-id"];
  ...
  it("keeps agent-embedded ownership distinct from a running Gateway", async () => {
    ...
    const readProcessCmdline = () => ["carapace", "agent", "--local", "--message", "hello"];
    const lock = await acquireGatewayLock({
      allowInTests: true,
      env,
      lockDir,
