// Resolves Carapace home and platform-specific config directories.
...
function resolveRawHomeDir(env: NodeJS.ProcessEnv, homedir: () => string): string | undefined {
  const explicitHome = normalize(env.CARAPACE_HOME);
  if (!explicitHome) {
...
/** Resolves Carapace's effective home, honoring CARAPACE_HOME before OS homes. */
...
/** Resolves the underlying OS user home, ignoring CARAPACE_HOME overrides. */
...
export function resolveRequiredHomeDir(
  ...
  throw new Error(
    "Unable to resolve an Carapace home: set CARAPACE_HOME, HOME, or USERPROFILE, or run from an existing directory.",
  );
}
...
/** Resolves a user-supplied path against the OS home, ignoring CARAPACE_HOME. */
