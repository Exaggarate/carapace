...
// update.
//
// `runGatewayUpdate` (git mode) runs `carapace doctor --fix` with
// `CARAPACE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `carapace update` CLI resumes
...
// binary's hidden `carapace update finalize` entrypoint — the designed
...
> {
  ...
  return {
    ...
    authoredConfig: isRecord(snapshot.parsed)
      ? (snapshot.parsed as CarapaceConfig) // SAFETY: the valid snapshot supplies a parsed config object.
...
function buildFinalizeEnv(
  ...
  delete env.CARAPACE_SERVICE_MARKER;
  delete env.CARAPACE_SERVICE_KIND;
  delete env[GATEWAY_SERVICE_RUNTIME_PID_ENV];
  env[UPDATE_EFFECTIVE_CHANNEL_ENV] = effectiveChannel;
  if (serviceRepairPolicy) {
    env.CARAPACE_SERVICE_REPAIR_POLICY = serviceRepairPolicy;
  }
  return env;
}
...
// re-enters the full `carapace update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "carapace update finalize",
