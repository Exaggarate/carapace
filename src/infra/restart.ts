...
function emitGatewayRestart(reasonOverride?: string, intent?: GatewayRestartIntent): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerCarapaceRestart() uses schtasks to restart the gateway.
...
export function triggerCarapaceRestart(): RestartAttempt {
  ...
  const label =
    process.env.CARAPACE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CARAPACE_PROFILE);
