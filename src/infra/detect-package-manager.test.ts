...
async function withPackageManagerRoot<T>(
  ...
  return await withTestDir({ prefix: "carapace-detect-pm-" }, async (root) => {
...
async function writePublishedCarapaceRoot(
  root: string,
...
describe("detectPackageManager", () => {
  ...
  it.each(
    ...
    async ({ manager, layout, shrinkwrap }) => {
      await withTestDir({ prefix: `carapace-detect-pm-${manager}-` }, async (base) => {
        ...
        const packageRoot =
          layout === "virtual"
            ? path.join(nodeModulesRoot, ".pnpm", "carapace@2026.5.27", "node_modules", "carapace")
            : path.join(nodeModulesRoot, "carapace");
        await writePublishedCarapaceRoot(packageRoot, { shrinkwrap });
        if (manager === "pnpm") {
          await fs.writeFile(
  ...
  it.each([
    ...
    async ({ installEnv, customGlobalDir }) => {
      await withTestDir({ prefix: "carapace-detect-pm-bun-owner-" }, async (base) => {
        ...
        const packageRoot = path.join(globalProject, "node_modules", "carapace");
        await writePublishedCarapaceRoot(packageRoot, { shrinkwrap: false });
  ...
  it("keeps pnpm ownership through markerless global v11 virtual-store symlinks", async () => {
    await withTestDir({ prefix: "carapace-detect-pm-pnpm-global-store-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const installRoot = path.join(globalRoot, "install-carapace");
      const linkedPackageRoot = path.join(installRoot, "node_modules", "carapace");
      ...
      await writePublishedCarapaceRoot(storePackageRoot, { shrinkwrap: false });
      await fs.mkdir(path.dirname(linkedPackageRoot), { recursive: true });
      await fs.writeFile(
        path.join(installRoot, "package.json"),
        JSON.stringify({ private: true, dependencies: { carapace: "2026.7.2" } }),
        "utf8",
      );
      ...
      await fs.symlink(installRoot, path.join(globalRoot, "hash-carapace"), symlinkType);
