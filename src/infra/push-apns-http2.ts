...
import { decodeTextPrefix } from "@carapace/normalization-core";
  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.carapace.proxy.validation",
        "apns-topic": "ai.carapace.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "Carapace APNs proxy validation" } }));
    });
  } finally {
