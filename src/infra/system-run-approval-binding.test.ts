...
describe("mutable file operand binding", () => {
  it("binds every script in a compound command and detects drift", async () => {
    await withTempDir("carapace-system-run-binding-", async (rawCwd) => {
  ...
  it("binds direct script executables", async () => {
    await withTempDir("carapace-system-run-direct-", async (cwd) => {
  ...
  it.each([
    ...
    await withTempDir("carapace-system-run-wrapper-", async (cwd) => {
  ...
  it("binds mutable native executables", async () => {
    await withTempDir("carapace-system-run-native-", async (cwd) => {
  ...
  it("fails closed for shell startup file operands", async () => {
    await withTempDir("carapace-system-run-startup-", async (cwd) => {
  ...
  it("fails closed for shell source built-ins", async () => {
    await withTempDir("carapace-system-run-source-", async (cwd) => {
  ...
  it("fails closed for runtime code-loading and cwd options", async () => {
    await withTempDir("carapace-system-run-bun-", async (cwd) => {
  ...
  it("binds mutable scripts resolved through PATH", async () => {
    await withTempDir("carapace-system-run-path-script-", async (cwd) => {
  ...
  it("binds both a PATH-resolved interpreter shim and its script operand", async () => {
    await withTempDir("carapace-system-run-path-python-", async (cwd) => {
      const binDir = path.join(cwd, "bin");
  ...
  it("binds both an explicit interpreter shim and its script operand", async () => {
    await withTempDir("carapace-system-run-explicit-python-", async (cwd) => {
  ...
  it("fails closed when an earlier shell segment changes cwd", async () => {
    await withTempDir("carapace-system-run-cd-", async (cwd) => {
  ...
  it("fails closed when a script operand does not exist", async () => {
    await withTempDir("carapace-system-run-missing-", async (cwd) => {
      await expect(
        prepareSystemRunMutableFileBinding({
  ...
  it("does not let inline eval bypass a mutable loader operand", async () => {
    await withTempDir("carapace-system-run-loader-", async (cwd) => {
  ...
  it.runIf(process.platform !== "win32" && process.getuid?.() !== 0)(
    ...
    async () => {
      const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-system-run-unreadable-"));
