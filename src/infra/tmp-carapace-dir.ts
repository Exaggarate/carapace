// Creates temporary Carapace directories for runtime scratch work.
import { getSealedRuntimeSecureTempRoot } from "./sealed-runtime-registry.js";

/** Preferred shared Carapace temp root on POSIX systems when ownership and permissions are safe. */
export const DEFAULT_POSIX_TMP_ROOT = "/tmp/carapace";
...
export type ResolvePreferredCarapaceTmpDirOptions = {
...
/** Resolves a safe Carapace temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredCarapaceTmpDir(
  options: ResolvePreferredCarapaceTmpDirOptions = {},
): string {
  return loadResolveSecureTempRoot()({
    ...options,
    preferredDir: options.preferredDir ?? DEFAULT_POSIX_TMP_ROOT,
    fallbackPrefix: "carapace",
    warningPrefix: "[carapace]",
    unsafeFallbackLabel: "Carapace temp dir",
