...
const DEFAULT_APNS_RELAY_BASE_URL = "https://github.com/Exaggarate/carapace";
const DEFAULT_APNS_SANDBOX_RELAY_BASE_URL = "https://github.com/Exaggarate/carapace";
...
const GATEWAY_DEVICE_ID_HEADER = "x-carapace-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-carapace-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-carapace-gateway-signed-at-ms";
...
function normalizeApnsRelayBaseUrlWithPolicy(
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !allowLoopbackHttpWithoutEnvOptIn) {
      throw new Error(
        "http relay URLs require CARAPACE_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
...
export function normalizeApnsRelayBaseUrl(
  ...
  return normalizeApnsRelayBaseUrlWithPolicy(
    baseUrl,
    readAllowHttp(env.CARAPACE_APNS_RELAY_ALLOW_HTTP),
  );
}
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.CARAPACE_APNS_RELAY_BASE_URL);
  ...
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or CARAPACE_APNS_RELAY_BASE_URL for relay registrations without the hosted relay origin",
    };
  }
  ...
  return {
    ...
    value: {
      ...
      timeoutMs: normalizeTimeoutMs(
        normalizeNonEmptyString(env.CARAPACE_APNS_RELAY_TIMEOUT_MS) ?? configuredRelay?.timeoutMs,
      ),
    },
  };
}
