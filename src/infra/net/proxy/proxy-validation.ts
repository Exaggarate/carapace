...
const DENIED_CANARY_HEADER = "x-carapace-proxy-validation-canary";
...
type ResolveProxyValidationConfigOptions = {
  config?: ProxyConfig;
  env?: NodeJS.ProcessEnv | Partial<Record<"CARAPACE_PROXY_URL", string | undefined>>;
...
function validateProxyUrl(value: string | undefined): string[] {
  if (!value) {
    return ["proxy validation requires proxy.proxyUrl, --proxy-url, or CARAPACE_PROXY_URL"];
  }
  if (!isHttpUrl(value)) {
...
function resolveProxyValidationConfig(
  ...
  const envUrl = normalizeProxyUrl(options.env?.CARAPACE_PROXY_URL);
  if (envUrl) {
    const proxyCaFile = resolveManagedProxyCaFileForUrl({
      proxyUrl: envUrl,
      config: options.config,
