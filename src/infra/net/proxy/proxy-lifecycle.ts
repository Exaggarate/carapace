import { isLoopbackIpAddress } from "@carapace/net-policy/ip";
import { isHttpUrl, isWebSocketUrl } from "@carapace/net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    CARAPACE_PROXY_ACTIVE: process.env["CARAPACE_PROXY_ACTIVE"],
    CARAPACE_PROXY_LOOPBACK_MODE: process.env["CARAPACE_PROXY_LOOPBACK_MODE"],
    CARAPACE_PROXY_CA_FILE: process.env["CARAPACE_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["CARAPACE_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["CARAPACE_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["CARAPACE_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["CARAPACE_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or CARAPACE_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["CARAPACE_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["CARAPACE_PROXY_CA_FILE"],
  });
...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  if (
    ...
    (!config?.proxyUrl?.trim() && !process.env["CARAPACE_PROXY_URL"]?.trim())
  ) {
    return null;
  }
 ...
 * loopback CDP probe to a Chromium instance Carapace spawned itself.
 *
