...
describe("GitHub issue transport", () => {
  ...
  it("submits the prepared body through stdin and accepts only the canonical issue URL", async () => {
    ...
    const runGh = vi
      ...
      .mockResolvedValueOnce(
        cliResult({
          ...
          status: 0,
          stdout: Buffer.from(
            "HTTP/1.1 200 Connection established\r\n\r\nHTTP/2.0 201 Created\r\n\r\nhttps://github.com/Exaggarate/carapace/issues/123\n",
          ),
        }),
      );
    ...
    expect(runGh).toHaveBeenNthCalledWith(
      ...
      [
        ...
        "--method",
        "POST",
        "repos/carapace/carapace/issues",
        "--input",
        "-",
        "--jq",
        ".html_url",
            ...
            JSON.stringify([
              {
                body: issue.body,
                title: issue.title,
                url: "https://github.com/Exaggarate/carapace/issues/456",
              },
            ]),
          ),
        }),
  ...
  it.each([
    "javascript:alert(1)",
    "https://github.com.evil.example/carapace/carapace/issues/123",
    "https://github.com/Exaggarate/carapace/issues/123?token=secret",
            ...
            JSON.stringify([
              {
                body: `${issue.body}\nchanged`,
                title: issue.title,
                url: "https://github.com/Exaggarate/carapace/issues/999",
              },
            ]),
          ),
        }),
  ...
  it("deduplicates concurrent submissions with the same marker", async () => {
    ...
    const runGh = vi
      ...
      .mockResolvedValueOnce(
        cliResult({
          started: true,
          status: 0,
          stdout: Buffer.from("https://github.com/Exaggarate/carapace/issues/789\n"),
        }),
      );
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      { status: "created", url: "https://github.com/Exaggarate/carapace/issues/789" },
      { status: "created", url: "https://github.com/Exaggarate/carapace/issues/789" },
    ]);
    expect(runGh).toHaveBeenCalledTimes(2);
  });
        ...
        child.stdin.end.mockImplementation(() =>
          queueMicrotask(() => {
            ...
            if (args[0] === "api") {
              child.stdout.emit(
                "data",
                Buffer.from("https://github.com/Exaggarate/carapace/issues/123\n"),
              );
            }
            child.emit("close", 0);
          }),
