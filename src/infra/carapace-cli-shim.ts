...
import {
  resolveCurrentCarapaceCliInvocation,
  type CarapaceCliInvocation,
} from "./carapace-cli-invocation.js";

const AGENT_CLI_BIN_DIR = path.join("tmp", "agent-cli");
const GATEWAY_AGENT_CLI_STATE_KEY = Symbol.for("carapace.gatewayAgentCliShim");
...
function renderPosixShim(invocation: CarapaceCliInvocation, profile: string | null): string {
...
function renderWindowsShim(invocation: CarapaceCliInvocation, profile: string | null): string {
  const args = [...invocation.args, ...(profile ? ["--profile", profile] : [])];
...
export async function prepareGatewayAgentCliShim(
  options: {
    ...
    invocation?: CarapaceCliInvocation;
  ...
  const invocation = options.invocation ?? resolveCurrentCarapaceCliInvocation([]);
  ...
  const executablePath = path.join(binDir, platform === "win32" ? "carapace.cmd" : "carapace");
  const content =
    platform === "win32"
