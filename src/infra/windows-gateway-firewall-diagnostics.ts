...
$targetPort = ${port}
function Test-CarapacePortMatch($value) {
...
function Resolve-CarapaceProgramScope($rule) {
...
function Get-CarapaceManagedRules {
  try {
    ...
    foreach ($rule in $rules) {
      foreach ($portFilter in @($rule | Get-NetFirewallPortFilter)) {
        ...
        if (($protocol -eq 'Any' -or $protocol -eq 'TCP') -and (Test-CarapacePortMatch $portFilter.LocalPort)) {
...
$managedMatchingRules = @(Get-CarapaceManagedRules)
...
foreach ($rule in $policy.Rules) {
  ...
  if (($protocol -ne 'TCP' -and $protocol -ne 'Any') -or -not (Test-CarapacePortMatch $rule.LocalPorts)) { continue }
  [void]$matchingRules.Add([pscustomobject]@{
    ...
    Program = (Resolve-CarapaceProgramScope $rule)
...
export async function inspectWindowsGatewayFirewall(
  ...
  if (quickJson === null) {
    return {
      ...
      message: "Carapace could not quickly inspect Windows Firewall LAN Gateway policy.",
      details: [
        "Run `carapace gateway status --deep` again, or verify the advertised LAN URL from another device.",
      ],
    };
  }
  ...
  if (!quickPayload || typeof quickPayload !== "object" || Array.isArray(quickPayload)) {
    return {
      ...
      severity: "warning",
      code: "windows_firewall_inspection_failed",
      message: "Carapace could not parse Windows Firewall LAN Gateway policy.",
      details: [
        "Run `carapace gateway status --deep` again, or verify the advertised LAN URL from another device.",
      ],
    };
