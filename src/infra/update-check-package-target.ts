import { normalizeNullableString as toOptionalTrimmedString } from "@carapace/normalization-core/string-coerce";
...
function parseNpmPackageTargetMetadata(raw: string): {
  ...
  const carapace = rec.carapace && typeof rec.carapace === "object" ? rec.carapace : null;
  const schemaVersions =
    parseCarapaceSchemaVersions(rec["carapace.schemaVersions"]) ??
    (carapace
      ? parseCarapaceSchemaVersions((carapace as Record<string, unknown>).schemaVersions)
      : undefined);
  return {
...
function packageTargetSpec(params: { target: string; spec?: string }): string {
  ...
  return spec || `carapace@${params.target.trim() || "latest"}`;
}
...
async function fetchNpmPackageTargetStatusFromRegistry(params: {
  ...
  try {
    ...
    const json = await readProviderJsonResponse<{
      ...
      carapace?: { schemaVersions?: unknown };
    }>(res, "npm package target status");
    const schemaVersions = parseCarapaceSchemaVersions(json.carapace?.schemaVersions);
    return {
      target: params.target,
