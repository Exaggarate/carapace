...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "CARAPACE_LIVE_ANTHROPIC_KEYS",
  "CARAPACE_LIVE_GEMINI_KEY",
  "CARAPACE_LIVE_OPENAI_KEY",
  "CARAPACE_GATEWAY_TOKEN",
  "CARAPACE_GATEWAY_PASSWORD",
  "CARAPACE_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "CARAPACE_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "CARAPACE_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "CARAPACE_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "CARAPACE_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  it("loads ~/.carapace/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.carapace/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".carapace");
        setTestEnvValue("CARAPACE_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "carapace", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CARAPACE_STATE_DIR=./evil-state",
            "CARAPACE_CONFIG_PATH=./evil-config.json",
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CARAPACE_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "CARAPACE_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "CARAPACE_LIVE_GEMINI_KEY=sk-gemini-live",
            "CARAPACE_LIVE_OPENAI_KEY=sk-openai-live",
            "CARAPACE_GATEWAY_TOKEN=attacker-token",
            "CARAPACE_GATEWAY_PASSWORD=attacker-password",
            "CARAPACE_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "CARAPACE_AGENT_DIR=./evil-agent",
            `CARAPACE_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "CARAPACE_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks CARAPACE_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "CARAPACE_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.CARAPACE_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CARAPACE_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CARAPACE_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'CARAPACE_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.CARAPACE_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.CARAPACE_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CARAPACE_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.CARAPACE_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "CARAPACE_PINNED_PYTHON=./attacker-python",
            "CARAPACE_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.CARAPACE_PINNED_PYTHON;
        delete process.env.CARAPACE_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.CARAPACE_PINNED_PYTHON).toBeUndefined();
        expect(process.env.CARAPACE_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CARAPACE_PINNED_PYTHON=/trusted/python",
            "CARAPACE_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.CARAPACE_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.CARAPACE_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "CARAPACE_LIVE_GEMINI_KEY=sk-gemini-live",
            "CARAPACE_LIVE_OPENAI_KEY=sk-openai-live",
            "CARAPACE_GATEWAY_TOKEN=trusted-token",
            "CARAPACE_GATEWAY_PASSWORD=trusted-password",
            "CARAPACE_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.CARAPACE_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.CARAPACE_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.CARAPACE_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.CARAPACE_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.CARAPACE_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.CARAPACE_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.CARAPACE_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "CARAPACE_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks CARAPACE_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "CARAPACE_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when CARAPACE_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("CARAPACE_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "carapace", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.CARAPACE_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.CARAPACE_STATE_DIR).toBe(stateDir);
        expect(process.env.CARAPACE_CONFIG_PATH).toBeUndefined();
        expect(process.env.CARAPACE_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
