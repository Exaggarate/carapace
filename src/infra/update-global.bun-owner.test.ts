...
describe("custom Bun global installation ownership", () => {
  it("preserves ownership when the original BUN_INSTALL is unavailable", async () => {
    await withTestDir({ prefix: "carapace-update-custom-bun-root-" }, async (base) => {
      await withEnvAsync(
  ...
  it.each([
    ...
    await withTestDir({ prefix: "carapace-package-update-bun-owner-" }, async (base) => {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const runStep = vi.fn(async ({ name, argv, cwd, env }) => {
            expect(argv).toEqual(["bun", "add", "-g", "--trust", "carapace@2.0.0"]);
