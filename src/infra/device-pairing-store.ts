...
function resolveDevicePairingStateDbOptions(baseDir?: string): CarapaceStateDatabaseOptions {
  return baseDir ? { env: { ...process.env, CARAPACE_STATE_DIR: baseDir } } : {};
}
...
function invalidateDevicePairingStoreCache(database: CarapaceStateDatabase): void {
  if (
...
function runDevicePairingStoreMutation<T>(
  ...
  mutate: (database: CarapaceStateDatabase) => DevicePairingStoreMutation<T>,
): T {
  const databaseOptions = resolveDevicePairingStateDbOptions(baseDir);
  const database = openCarapaceStateDatabase(databaseOptions);
  const result = runCarapaceStateWriteTransaction(mutate, { ...databaseOptions, database });
  if (result.mutated) {
...
export function readDevicePairingStoreStateFromDatabase(db: DatabaseSync): DevicePairingStoreState {
  const kysely = getNodeSqliteKysely<CarapaceStateKyselyDatabase>(db);
...
export function loadDevicePairingStoreState(baseDir?: string): DevicePairingStoreState {
  const database = openCarapaceStateDatabase(resolveDevicePairingStateDbOptions(baseDir));
  const { db } = database;
...
export function loadPairedDevicePairingStoreRecord(
  ...
  const { db } = openCarapaceStateDatabase(resolveDevicePairingStateDbOptions(baseDir));
