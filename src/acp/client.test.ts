...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets CARAPACE_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "carapace",
    });

    expect(env.CARAPACE_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("carapace");
  });

  it("overrides pre-existing CARAPACE_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      CARAPACE_SHELL: "wrong",
    });
    expect(env.CARAPACE_SHELL).toBe("acp-client");
  });
  ...
  it("preserves CARAPACE_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        CARAPACE_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["CARAPACE_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Carapace bridge", () => {
    ...
    expect(env.CARAPACE_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.CARAPACE_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "carapace",
        serverArgs: ["acp"],
        defaultServerCommand: "carapace",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("CARAPACE_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "carapace", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    const dir = tempDirs.make("carapace-acp-client-test-");
    const scriptPath = path.join(dir, "carapace", "dist", "entry.js");
    const shimPath = path.join(dir, "carapace.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\carapace\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
  ...
  it("prompts for search when rawInput path escapes cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/carapace-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
  ...
  it("auto-approves search when query-like title text contains a path label", async () => {
    await expectAutoAllowWithoutPrompt({
      ...
      cwd: "/tmp/carapace-acp-cwd/workspace",
    });
  });
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/carapace-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/carapace-acp-cwd",
    });
  ...
  it.each(["FILE:///tmp/outside/marker.txt", "file:/tmp/outside/marker.txt"])(
    ...
    async (fileUrl) => {
      ...
      const res = await resolvePermissionRequest(
        ...
        { prompt, log: () => {}, cwd: "/tmp/carapace-acp-cwd" },
      );
      expect(prompt).toHaveBeenCalledTimes(1);
