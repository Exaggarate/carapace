...
export function isAcpEnabledByPolicy(cfg: CarapaceConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: CarapaceConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: CarapaceConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: CarapaceConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: CarapaceConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: CarapaceConfig, agentId: string): boolean {
