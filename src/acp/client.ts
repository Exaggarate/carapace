/** Interactive stdio ACP client used to connect a terminal session to an Carapace ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "carapace";
  ...
  try {
    ...
    await client.initialize({
      ...
      clientInfo: { name: "carapace-acp-client", version: "1.0.0" },
    });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("Carapace ACP client");
