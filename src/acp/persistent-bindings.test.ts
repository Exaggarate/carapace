...
type ConfiguredBinding = NonNullable<CarapaceConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: CarapaceConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/carapace" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
