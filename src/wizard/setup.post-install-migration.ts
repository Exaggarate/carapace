...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`carapace migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `carapace migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`carapace migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
