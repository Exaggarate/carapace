...
function hasMeaningfulConfig(config: CarapaceConfig): boolean {
...
function buildSetupMigrationSnapshotConfig(config: CarapaceConfig): Record<string, unknown> {
...
/** Serializes onboarding writes that share one Carapace state target. */
export async function withSetupMigrationTargetLock<T>(
  ...
  if (activeStateDir) {
    if (activeStateDir !== resolvedStateDir) {
      throw new Error("nested onboarding target lock cannot switch the Carapace state directory");
    }
    return await fn();
  }
  ...
  try {
    ...
    throw new SetupTargetLockedError(holderPid, process.env.CARAPACE_PROFILE?.trim(), error);
  }
}
