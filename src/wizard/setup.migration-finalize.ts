...
function deferredRetryInstruction(providerId: string): string {
  return `Some post-promotion migration activation steps are still pending. Retry only those steps with carapace onboard --flow import --import-from ${providerId}.`;
}
