...
const config: CarapaceConfig = {
  agents: {
    defaults: { workspace: "/tmp/carapace-memory-step" },
...
function planFor(
  ...
  const items: MigrationPlan["items"] = [
    ...planned.map((id) => ({
      ...
      target: `/tmp/carapace-memory-step/memory/imports/${providerId}/${id}.md`,
    })),
    ...conflicts.map((id) => ({
      id,
      kind: "memory" as const,
  ...
  return {
    ...
    target: "/tmp/carapace-memory-step",
    items,
    summary: {
      total: items.length,
...
describe("runSetupMemoryImportStep", () => {
  ...
  it("does not apply when a single-provider import is declined", async () => {
    ...
    // The skip hint must not suggest `carapace migrate <id>`: that command runs
    // the full provider migration, not a memory-only retry.
    expect(notes).toContain("Memory import page");
    expect(notes).not.toContain("carapace migrate");
  });
