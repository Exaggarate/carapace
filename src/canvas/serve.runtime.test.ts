...
async function createStateDir(): Promise<string> {
  const stateDir = await mkdtemp(path.join(tmpdir(), "carapace-canvas-serve-"));
...
describe("core canvas document host", () => {
  ...
  it("rejects unsupported methods and traversal paths", async () => {
    ...
    const missingResponse = await capture("/__carapace__/canvas/documents/widget-1/index.html");
