...
function getConfigPath(config: CarapaceConfig | undefined, path: string): unknown {
...
function setConfigPath(target: CarapaceConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: CarapaceConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: CarapaceConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: CarapaceConfig) =>
