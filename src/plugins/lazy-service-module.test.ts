...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.CARAPACE_LAZY_SERVICE_SKIP;
    delete process.env.CARAPACE_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.CARAPACE_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "CARAPACE_LAZY_SERVICE_OVERRIDE",
  ...
  it("leaves caller-supplied override loaders responsible for their own specifiers", async () => {
    process.env.CARAPACE_LAZY_SERVICE_OVERRIDE = "C:\\Users\\alice\\browser-service.mjs";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));
  ...
  it("surfaces override validation failures", async () => {
    process.env.CARAPACE_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
