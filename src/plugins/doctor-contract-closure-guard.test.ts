...
const FORBIDDEN_SPECIFIER = "carapace/plugin-sdk/agent-runtime";
...
const FORBIDDEN_SPECIFIER_RULES = new Map<string, { reason: string; kinds: Set<ClosureKind> }>([
  ...
  [
    FORBIDDEN_SPECIFIER,
    {
      reason:
        ...
        "use carapace/plugin-sdk/agent-scope-runtime or another focused subpath",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/doctor-repair-runtime",
    {
      reason:
        "install-path, uninstall, and state-db schema repair cold-load the state-db/kysely graph; " +
        "use carapace/plugin-sdk/runtime-doctor-migrations, or defer the repair behind a dynamic import",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/runtime-doctor",
    {
      reason:
        ...
        "current source must use carapace/plugin-sdk/runtime-doctor-migrations",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/ssrf-runtime",
    {
      reason:
        ...
        "legacy private-network config migration lives in carapace/plugin-sdk/runtime-doctor-migrations",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/provider-model-shared",
    {
      reason:
        "the provider-model barrel cold-loads replay/endpoint/catalog helpers; " +
        "use carapace/plugin-sdk/model-ref-parse for provider/model reference parsing",
      kinds: new Set(["doctor-contract"]),
    },
  ],
  [
    "carapace/plugin-sdk/acp-runtime",
    {
      reason:
  ...
  [
    "carapace/plugin-sdk/provider-auth",
    {
      reason:
        ...
        "install graph (execa, kysely, commander); use carapace/plugin-sdk/secret-provider-alias " +
        "for the default secret provider alias",
      kinds: new Set(["doctor-contract"]),
    },
  ],
  [
    "carapace/plugin-sdk/channel-secret-basic-runtime",
    {
      reason:
        "the channel-secret barrel cold-loads secret-ref/account-routing modules; " +
        "the canonical record guard is carapace/plugin-sdk/string-coerce-runtime",
      kinds: new Set(["doctor-contract"]),
    },
  ],
  [
  ...
  [
    "carapace/plugin-sdk/logging-core",
    {
      reason:
        "the logging barrel makes doctor enumeration cold-load the diagnostic/config graph; " +
        "use carapace/plugin-sdk/security-runtime for redaction helpers",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/realtime-voice",
    {
      reason:
        ...
        "use carapace/plugin-sdk/realtime-voice-activation for activation-name helpers",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
  [
    "carapace/plugin-sdk/channel-outbound",
    {
      reason:
        ...
        "use carapace/plugin-sdk/channel-streaming-config for streaming config helpers",
      kinds: new Set(["doctor-contract"]),
    },
  ],
  [
    "carapace/plugin-sdk/memory-host-core",
    {
      reason:
        ...
        "use carapace/plugin-sdk/agent-scope-runtime for agent scope resolvers",
      kinds: new Set(["doctor-contract", "legacy-setup"]),
    },
  ],
]);
...
function collectClosureEntries(): ClosureEntry[] {
  ...
  const env = {
    ...process.env,
    CARAPACE_BUNDLED_PLUGINS_DIR: path.join(REPO_ROOT, "extensions"),
  };
  for (const record of loadBundledPluginManifestRegistry({ env }).plugins) {
    ...
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8")) as {
      carapace?: { setupEntry?: unknown };
    };
    const setupEntry = packageJson.carapace?.setupEntry;
...
function collectHeavyRuntimeDoctorMigrationImports(): string[] {
  ...
  const forbiddenPrefixes = ["src/plugin-state/plugin-state-store", "src/state/carapace-state-db"];
  const violations: string[] = [];
  const visited = new Set<string>();
...
const PLUGIN_SDK_SPECIFIER_PREFIX = "carapace/plugin-sdk/";
...
// deep-relative package bridges) plus carapace/plugin-sdk/* subpaths. Other bare
