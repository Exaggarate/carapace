...
function normalizePluginApiVersionForComparator(version: string, target: string): string {
  const normalizedCorrection = normalizeCarapaceNumericCorrectionForPluginApi(version);
  if (normalizedCorrection) {
    return normalizedCorrection;
  }
  return shouldPreservePluginApiPrereleaseFloor(target)
    ? version
    : normalizeCarapaceReleaseSuffixForPluginApi(version);
}
...
const CARAPACE_RELEASE_SUFFIX_PATTERN =
  /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)(?:-\d+|-(?:alpha|beta|rc)\.\d+)$/i;
const CARAPACE_NUMERIC_CORRECTION_PATTERN = /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)-\d+$/;

function normalizeCarapaceNumericCorrectionForPluginApi(
  pluginApiVersion: string,
): string | undefined {
  return CARAPACE_NUMERIC_CORRECTION_PATTERN.exec(pluginApiVersion.trim())?.[1];
}

function normalizeCarapaceReleaseSuffixForPluginApi(pluginApiVersion: string): string {
  const match = CARAPACE_RELEASE_SUFFIX_PATTERN.exec(pluginApiVersion.trim());
  return match?.[1] ?? pluginApiVersion;
}

/** Result of reading package.json carapace.compat.pluginApi metadata. */
...
export function resolvePackagePluginApiRange(
  ...
  if (!isRecord(compat)) {
    return { ok: false, error: "package.json carapace.compat must be an object" };
  }
  ...
  if (typeof pluginApi !== "string") {
    return { ok: false, error: "package.json carapace.compat.pluginApi must be a string" };
  }
  const range = pluginApi.trim();
  if (!range) {
    return { ok: false, error: "package.json carapace.compat.pluginApi must not be empty" };
  }
