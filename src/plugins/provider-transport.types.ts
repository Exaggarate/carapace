...
/**
 * Provider-owned stream wrapper hook after Carapace applies its generic
 ...
 *
 * Return `true` or `false` to override Carapace's built-in provider cache TTL
 ...
 *
 * Runs only after Carapace exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Carapace should append to the generic `Unknown model`
 * error.
 */
