...
function createEnabledBundleConfig(pluginIds: string[]): CarapaceConfig {
  return {
    plugins: {
...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("carapace-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("carapace-bundle-mcp-workspace-");
  ...
  it("loads MCP servers declared by an enabled native plugin", async () => {
    const workspaceDir = await tempHarness.createTempDir("carapace-native-mcp-workspace-");
    const pluginRoot = await tempHarness.createTempDir("carapace-native-mcp-plugin-");
    const loaded = loadEnabledBundleMcpConfig({
      workspaceDir,
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "carapace-bundle-inline-placeholder",
      async ({ homeDir, workspaceDir }) => {
  ...
  it("loads Agent Plugins MCP config with placeholders, injected env, and canonical transports", async () => {
    await withBundleHomeEnv(
      ...
      async ({ homeDir, workspaceDir }) => {
        ...
        const pluginDataPath = path.join(homeDir, ".carapace", "plugin-data", "portable-mcp");
  ...
  it("ignores dot MCP config and inline MCP fields for Agent Plugins", async () => {
    await withBundleHomeEnv(
      ...
      async ({ homeDir, workspaceDir }) => {
        ...
        expectNoDiagnostics(loaded.diagnostics);
        expect(loaded.config.mcpServers).toStrictEqual({});
        await expectPathMissing(path.join(homeDir, ".carapace", "plugin-data", "closed-agent"));
  ...
  it("keeps Agent Plugins inspection pure when PLUGIN_DATA collides", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "carapace-agent-bundle-data-collision",
      async ({ homeDir, workspaceDir }) => {
        ...
        const pluginDataPath = path.join(homeDir, ".carapace", "plugin-data", pluginId);
  ...
  it("skips invalid Agent Plugins MCP entries while retaining valid siblings", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "carapace-agent-bundle-entry-isolation",
      async ({ homeDir, workspaceDir }) => {
