  ...
  it.each([
    {
      ...
      input: {
        name: "@carapace/demo",
        version: "1.2.3",
        resolvedSpec: "@carapace/demo@1.2.3",
...
describe("resolveNpmInstallRecordSpec", () => {
  it("uses an exact resolved registry spec when managed installs request pinning", () => {
    expect(
      resolveNpmInstallRecordSpec({
        requestedSpec: "@carapace/codex",
        resolution: {
          name: "@carapace/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@carapace/codex@2026.5.30-beta.1",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@carapace/codex@2026.5.30-beta.1");
  });
    ...
    expect(
      resolveNpmInstallRecordSpec({
        ...
        resolution: {
          name: "@carapace/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@carapace/codex@beta",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@carapace/codex");
...
describe("recordPluginInstall", () => {
  ...
  it("replaces local npm-pack provenance on a registry reinstall", () => {
    const localInstall = recordPluginInstall(
      {},
      {
        pluginId: "diffs",
        source: "npm",
        spec: "/tmp/carapace-diffs.tgz",
        sourcePath: "/tmp/carapace-diffs.tgz",
        installPath: "/tmp/carapace/plugins/diffs-local",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmTarballName: "carapace-diffs-1.0.0.tgz",
    ...
    const registryInstall = recordPluginInstall(localInstall, {
      ...
      source: "npm",
      spec: "@carapace/diffs",
      installPath: "/tmp/carapace/plugins/diffs",
      resolvedName: "@carapace/diffs",
      resolvedVersion: "1.1.0",
      resolvedSpec: "@carapace/diffs@1.1.0",
      installedAt: "2026-05-15T00:00:03.000Z",
    });
  ...
  it("moves an exact prior npm load path on same-version reinstall", () => {
    const previousInstallPath = "/tmp/carapace/npm/projects/alpha-v1/node_modules/alpha";
    const nextInstallPath = "/tmp/carapace/npm/projects/alpha-v2/node_modules/alpha";
    const customPath = `${previousInstallPath}/custom-child`;
    const adjacentPath = "/tmp/carapace/npm/projects/beta/node_modules/beta";
    const existing = {
      plugins: {
