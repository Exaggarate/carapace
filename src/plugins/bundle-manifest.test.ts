/** Tests bundle manifest parsing for Agent, Codex, Claude, Cursor, and Carapace formats. */
...
function makeTempDir() {
  return makeTrackedTempDir("carapace-bundle-manifest", tempDirs);
}
...
describe("bundle manifest parsing", () => {
  it("does not treat carapace.bundle.json as a bundle manifest", () => {
    const rootDir = makeTempDir();
    writeBundleManifest(rootDir, "carapace.bundle.json", {
      name: "Not Real",
      skills: ["skills"],
    });
  ...
  it("keeps client-specific and native formats ahead of portable Agent Plugins", () => {
    ...
    writeBundleFixtureFiles(nativeRoot, {
      "carapace.plugin.json": { id: "native", configSchema: { type: "object" } },
      [AGENT_BUNDLE_MANIFEST_RELATIVE_PATH]: {
        $schema: AGENT_BUNDLE_MANIFEST_SCHEMA,
        name: "Agent",
      },
  ...
  it("loads ai.carapace activation like an equivalent Claude bundle", () => {
    const activation = {
      onStartup: true,
  ...
  it.each([
    ...
    {
      name: "non-object ai.carapace extension",
      extensions: { "ai.carapace": "invalid" },
    },
    {
  ...
  it("ignores unknown fields inside the ai.carapace extension", () => {
    const rootDir = makeTempDir();
    writeBundleManifest(rootDir, AGENT_BUNDLE_MANIFEST_RELATIVE_PATH, {
      $schema: AGENT_BUNDLE_MANIFEST_SCHEMA,
      name: "portable",
