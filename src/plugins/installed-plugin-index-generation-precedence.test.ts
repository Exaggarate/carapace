...
// `__carapace-generation__` dirs when reconciling persisted install records.
...
/** Writes a managed plugin version into an `__carapace-generation__` dir. */
...
describe("managed npm generation-dir loader precedence", () => {
  it("loads the authoritative generation after an upgrade leaves the old flat install", async () => {
    const stateDir = tempDirs.make("carapace-plugin-generation-precedence-");
  ...
  it("warns when install recency replaces a dangling managed authority", async () => {
    ...
    expect(emitWarning).toHaveBeenCalledWith(
      expect.stringContaining("without an authoritative active path"),
      expect.objectContaining({ code: "CARAPACE_PLUGIN_INSTALL_RECOVERY_FALLBACK" }),
    );
  });
  ...
  it("does not treat unrelated legacy-root metadata changes as plugin recency", async () => {
    ...
    writeManagedNpmPlugin({
      stateDir,
      packageName: "@carapace/unrelated",
      pluginId: "unrelated",
      version: "1.0.0",
      layout: "legacy",
    });
