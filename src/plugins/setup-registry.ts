...
function collectConfiguredPluginEntryIds(config: CarapaceConfig): string[] {
...
function resolveRegister(mod: CarapacePluginModule): {
...
function resolveSetupRegistration(
  ...
  let mod: CarapacePluginModule;
  try {
    mod = getModuleLoader(setupSource, setupArtifact.rootDir)(setupSource) as CarapacePluginModule;
  } catch (error) {
  ...
  const resolved = resolveRegister((mod as { default?: CarapacePluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Carapace ignored the plugin setup runtime entry. Remove setup-api/carapace.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
