...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@carapace/media-core/mime";
import { registerTestPlugin } from "carapace/plugin-sdk/plugin-test-contracts";
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredCarapaceTmpDir(), "carapace-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: CarapacePluginApi }).proofAttachmentApi;
