...
const excludeOptionalEnv = { CARAPACE_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageCarapace ? { carapace: params.packageCarapace } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "carapace.plugin.json"),
      "utf8",
    ),
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@carapace/tlon",
    manifest: { skills: [skillPath] },
    packageCarapace: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-meta-");
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "carapace.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.carapace?.extensions).toEqual(["./index.js"]);
  });

  it("ignores a non-file package icon and removes stale output", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-invalid-icon-");
    const pluginDir = createPlugin(repoRoot, {
      id: "acpx",
      packageName: "@carapace/acpx",
      packageCarapace: { extensions: ["./index.ts"] },
  ...
  it("copies generated bundled channel config schemas into dist manifests", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-channel-config-meta-");
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "carapace.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
      skills: ["./bundled-skills/@scope/skill"],
    });
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "package.json"), {
      name: "@carapace/removed-plugin",
    });
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "carapace.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@carapace/google-gemini-cli-auth",
    });
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    const repoRoot = makeRepoRoot("carapace-private-qa-metadata-");
    createPlugin(repoRoot, {
      id: "qa-lab",
      packageName: "@carapace/qa-lab",
      packageCarapace: { extensions: ["./index.ts"] },
    });
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { CARAPACE_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "carapace.plugin.json"))).toBe(true);
  ...
  it.each([
    {
      ...
      pluginId: "acpx",
      packageName: "@carapace/acpx-plugin",
      packageCarapace: { extensions: ["./index.ts"] },
    ...
    },
    {
      ...
      packageCarapace: {
        ...
        install: { npmSpec: "@carapace/whatsapp" },
      },
      env: {},
      seedStaleDist: false,
      expectedExists: false,
    ...
    "$name",
    ({ pluginId, packageName, packageCarapace, env, seedStaleDist, expectedExists }) => {
      const repoRoot = makeRepoRoot(`carapace-bundled-plugin-${pluginId}-`);
      createPlugin(repoRoot, {
        id: pluginId,
        packageName,
        packageCarapace,
  ...
  it("removes build-excluded bundled plugin metadata", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-excluded-meta-");
    createPlugin(repoRoot, {
      id: "selected",
      packageName: "@carapace/selected",
      packageCarapace: { extensions: ["./index.ts"] },
    });
    createPlugin(repoRoot, {
      id: "whatsapp",
    ...
    copyBundledPluginMetadata({
      repoRoot,
      env: { CARAPACE_BUNDLED_PLUGIN_BUILD_IDS: "selected" },
    });

    expect(fs.existsSync(staleDistDir)).toBe(false);
    expect(readBundledPackageJson(repoRoot, "selected").carapace?.extensions).toEqual([
      "./index.js",
    ]);
  });

  it("preserves isolated source-checkout output for an external plugin", () => {
    const repoRoot = makeRepoRoot("carapace-external-plugin-local-dist-meta-");
    createPlugin(repoRoot, {
      id: "sms",
      packageName: "@carapace/sms",
      packageCarapace: {
        extensions: ["./index.ts"],
        build: { bundledDist: false },
    ...
    expect(fs.existsSync(path.join(distPluginDir, "carapace.plugin.json"))).toBe(true);
    expect(readBundledPackageJson(repoRoot, "sms").carapace?.extensions).toEqual(["./index.js"]);
  });

  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@carapace/image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "carapace.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
  ...
  it("refuses to remove dist plugin trees through a symlinked dist root", () => {
    const repoRoot = makeRepoRoot("carapace-bundled-plugin-meta-symlink-");
    ...
    createPlugin(repoRoot, { id: "acpx", packageName: "@carapace/acpx" });
