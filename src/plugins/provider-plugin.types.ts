...
export type ProviderPlugin = {
   ...
   *
   * Carapace calls this only from async model resolution paths. Return the
   ...
   *
   * Runs after Carapace performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Carapace registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Carapace wrappers.
   *
   ...
   *
   * Called after Carapace resolves the raw configured credential but before the
   ...
   * hint than Carapace's generic auth-store guidance.
   */
   ...
   * Carapace's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Carapace should surface a provider-specific hint for
   ...
   * merging. Carapace deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Carapace stores the
   ...
   * without replacing the full Carapace-owned system prompt.
   */
   ...
   *
   * Runs after Carapace's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Carapace's default fallback policy.
   */
   ...
   * rewrites after Carapace has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Carapace handles
   ...
   *
   * Carapace uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that generic auth must ignore and `carapace doctor` should remove.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `carapace doctor` should offer.
   *
   ...
   *
   * Carapace calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Carapace's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Keep process/network I/O here; Carapace publishes the completed result for this generation.
   */
   ...
   * Carapace falls back to that stored profile.
   */
