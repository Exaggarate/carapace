...
function formatPluginInspectCommand(pluginId: string): string {
  return `carapace plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'carapace plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'carapace plugins inspect <id>', and add the ones you trust to plugins.allow in carapace.json."
    : `To trust them explicitly, set plugins.allow in carapace.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'carapace plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
