...
import type { CarapacePluginModule, PluginLogger } from "./types.js";
...
export function loadRuntimePluginCandidate(params: {
  ...
  let mod: CarapacePluginModule | null = null;
  ...
  try {
    ...
    mod = withProfile(
      ...
      () => params.loadPluginModule(safeSource) as CarapacePluginModule,
    );
  } catch (error) {
    recordPluginError({
...
function recordBundleDiagnostics(params: {
  ...
  for (const capability of unsupportedCapabilities) {
    params.registry.diagnostics.push({
      ...
      message: `bundle capability detected but not wired into Carapace yet: ${capability}`,
    });
  }
  if (
