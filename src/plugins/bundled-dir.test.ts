...
const originalDisableBundledPlugins = process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
const originalTrustBundledPlugins = process.env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
function createCarapaceRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "carapace" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@carapace/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "carapace.mjs"),
...
afterEach(() => {
  ...
  if (originalDisableBundledPlugins === undefined) {
    ...
    process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
  if (originalTrustBundledPlugins === undefined) {
    delete process.env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "carapace-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "carapace-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "carapace-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "carapace-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "carapace-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "carapace-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
    ...
    const repoRoot = createCarapaceRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("uses source extensions in pnpm workspace mirrors without git metadata", () => {
    const repoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-source-mirror-",
      hasExtensions: true,
      hasSrc: true,
      hasPnpmWorkspace: true,
    });
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "carapace.mjs");

    expect(resolveSourceCheckoutDependencyDiagnostic()).toEqual({
      source: repoRoot,
      message:
        "Carapace source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    });
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("reuses the prepared bundled root until its cache owner changes", () => {
    const repoRoot = fs.realpathSync(
      createCarapaceRoot({
        prefix: "carapace-bundled-dir-owner-",
        hasExtensions: true,
        hasSrc: true,
        hasPnpmWorkspace: true,
      }),
    ...
    const resolveRoot = vi.spyOn(carapaceRoot, "resolveCarapacePackageRootSync");
  ...
  it.each(["CARAPACE_HOME", "HOME", "USERPROFILE", "cwd"] as const)(
    "separates relative override resolution by %s within one cache owner",
    (homeSource) => {
      const homeA = makeRepoRoot("carapace-bundled-dir-home-a-");
      const homeB = makeRepoRoot("carapace-bundled-dir-home-b-");
      seedBundledPluginTree(homeA, "bundled", "memory-core");
      seedBundledPluginTree(homeB, "bundled", "discord");
      const envBase = {
        CARAPACE_BUNDLED_PLUGINS_DIR: homeSource === "cwd" ? "./bundled" : "~/bundled",
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "carapace.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    delete process.env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("carapace-bundled-dir-vitest-override-reject-");
    ...
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.CARAPACE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("carapace-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("carapace-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("carapace-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("carapace-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "carapace.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("ignores an enclosing checkout reached through node_modules tooling argv1", () => {
    ...
    const outerRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-enclosing-",
      hasExtensions: true,
      hasSrc: true,
      hasDistExtensions: true,
      hasGitCheckout: true,
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createCarapaceRoot({
      prefix: "carapace-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createCarapaceRoot({
          prefix: "carapace-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createCarapaceRoot({
          prefix: "carapace-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "carapace.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createCarapaceRoot({
          prefix: "carapace-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
