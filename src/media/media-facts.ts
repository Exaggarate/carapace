...
const RUNTIME_PROMPT_MEDIA_FACTS = Symbol.for("carapace.runtimePromptMediaFacts");
...
function readPersistedMediaFactInputs(message: object): Array<MediaFactInput | null> | undefined {
  const metadata = asNonArrayRecord(asNonArrayRecord(message)["__carapace"]);
...
export function canonicalizePersistedUserMessageMedia<T extends object>(
  ...
  const carapace = { ...asNonArrayRecord(record["__carapace"]) };
  if (media.length > 0 || canonical !== undefined || topLevelMedia !== undefined) {
    carapace.media = media;
  }
  if (Object.keys(carapace).length > 0) {
    next["__carapace"] = carapace;
  } else {
