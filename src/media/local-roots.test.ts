...
function loadedConfig(config: CarapaceConfig): CarapaceConfig {
  return migratePersistedImplicitMainRoster(config).config as CarapaceConfig;
}

function getAgentScopedMediaLocalRoots(config: CarapaceConfig, agentId: string) {
...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ CARAPACE_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "carapace-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "carapace-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "carapace-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "carapace-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "carapace-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "carapace-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
