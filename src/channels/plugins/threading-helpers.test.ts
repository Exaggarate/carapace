...
describe("createStaticReplyToModeResolver", () => {
  it.each(["off", "all"] as const)("always returns the configured mode %s", (mode) => {
    expect(createStaticReplyToModeResolver(mode)({ cfg: {} as CarapaceConfig })).toBe(mode);
  });
});
  ...
  it.each([
    {
      ...
      cfg: { channels: { "demo-top-level": { replyToMode: "first" } } } as CarapaceConfig,
      expected: "first",
    },
    {
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as CarapaceConfig, chatType: "group" })).toBe("first");
