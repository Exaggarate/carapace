...
async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-ingress-queue-"));
  try {
...
function openIngressStateDatabase(stateDir: string) {
  return openCarapaceStateDatabase({ env: { CARAPACE_STATE_DIR: stateDir } });
}
  ...
  describe("corrupt JSON resilience", () => {
    function insertCorruptRow(
      ...
      executeSqliteQuerySync(
        ...
        } as Insertable<CarapaceStateKyselyDatabase["channel_ingress_events"]>),
      );
    }
