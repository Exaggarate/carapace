...
/** Parses the JSON5 Carapace manifest block embedded inside a string frontmatter field. */
export function resolveCarapaceManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
/** Extracts normalized runtime requirement lists from an Carapace manifest block. */
export function resolveCarapaceManifestRequires(
  metadataObj: Record<string, unknown>,
): CarapaceManifestRequires | undefined {
  const requiresRaw =
...
export function resolveCarapaceManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an Carapace manifest block. */
export function resolveCarapaceManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedCarapaceManifestInstallBase = {
...
export function parseCarapaceManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedCarapaceManifestInstallBase | undefined {
  ...
  const spec: ParsedCarapaceManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyCarapaceManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedCarapaceManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
