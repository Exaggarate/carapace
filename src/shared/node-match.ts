...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Carapace nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentCarapaceClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("carapace-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentCarapaceClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> Carapace migration, a unique current client should win only
