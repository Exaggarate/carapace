...
// Carapace process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/Exaggarate/carapace/issues/78063.
...
type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the Carapace service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the Carapace service environment, then restart the gateway.`,
  );
  return lines;
}
