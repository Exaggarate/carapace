...
describe("Skill Workshop history scan", () => {
  ...
  it("rejects an opposite-direction scan while one is active", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-history-scan-test-"));
    try {
      ...
      const params = {
        ...
        env: { ...process.env, CARAPACE_STATE_DIR: path.join(tempDir, "state") },
        workspaceDir,
      };
  ...
  it("keeps scan cursors separate when the transcript store changes", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-history-store-scope-"));
    try {
      const workspaceDir = path.join(tempDir, "workspace");
