...
const fixtureSuite = createFixtureSuite("carapace-skills-suite-");
let tempHome: TempHomeEnv | null = null;
let skillsHomeEnv: SkillsHomeEnvSnapshot | null = null;
const pluginEnvSnapshot = captureEnv(["CARAPACE_DISABLE_BUNDLED_PLUGINS"]);
...
function rawSkillApiKeyRefConfig(skillName: string): CarapaceConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
function resolvedSkillApiKeyConfig(skillName: string, apiKey: string): CarapaceConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("carapace-skills-home-");
  skillsHomeEnv = setMockSkillsHomeEnv(tempHome.home);
  await fs.mkdir(path.join(tempHome.home, ".carapace", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native Carapace slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const config = {
      plugins: {
        entries: {
    ...
    const pluginRoot = path.join(workspaceDir, ".carapace", "extensions", "compound-bundle");
...
describe("shouldIncludeSkill", () => {
  const envName = "CARAPACE_TEST_SKILL_REQUIREMENT";
  const entry = makeSkillEntry("env-skill", {
    primaryEnv: envName,
    requires: { env: [envName] },
  });

  function shouldInclude(config?: CarapaceConfig): boolean {
