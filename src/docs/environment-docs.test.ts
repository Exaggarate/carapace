...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "Carapace ignores provider credentials and protected runtime controls from workspace `.env`",
    );
    expect(markdown).toContain("~/.carapace/.env");
    expect(markdown).toContain("$CARAPACE_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("CARAPACE_LOAD_SHELL_ENV=1");
