...
const tempRoot = createSuiteTempRootTracker({ prefix: "carapace-fleet-doctor-" });
...
function healthyInspection(): Extract<FleetContainerInspectResult, { kind: "ok" }> {
  return {
    ...
    environment: { CARAPACE_GATEWAY_TOKEN: "secret" },
...
function healthyNetwork(): Extract<FleetNetworkInspectResult, { kind: "ok" }> {
  return {
    ...
    attachedContainers: [{ id: "cell", name: "carapace-cell-acme" }],
    internal: false,
  };
}
...
describe("fleet doctor", () => {
  ...
  it.each([
    ...
    [
      ...
      "fail",
      (inspection: ReturnType<typeof healthyInspection>) => {
        inspection.labels["carapace.fleet.owner"] = "foreign";
      },
    ],
    [
      "gateway-token-env",
      "fail",
      (inspection: ReturnType<typeof healthyInspection>) => {
        inspection.environment.CARAPACE_GATEWAY_TOKEN = "";
      },
    ],
    [
      "disk-limit",
      ...
      (inspection: ReturnType<typeof healthyInspection>) => {
        ...
        inspection.labels["carapace.fleet.disk-limit"] = "10g";
    ...
    [
      ...
      (inspection: ReturnType<typeof healthyInspection>) => {
        ...
        inspection.labels["carapace.fleet.disk-limit"] = "not-a-size";
