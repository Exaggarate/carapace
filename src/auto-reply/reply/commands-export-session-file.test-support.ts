...
function makeWorkspace(): string {
  return tempDirs.make("carapace-session-export-");
}
...
describe("writeSessionExportFile", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const aliasParent = tempDirs.make("carapace-session-export-alias-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const outsideDir = tempDirs.make("carapace-session-export-outside-");
