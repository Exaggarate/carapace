// Formats Carapace CLI command snippets for chat-facing command responses.
import { resolveCurrentCarapaceCliInvocation } from "../../infra/carapace-cli-invocation.js";

const TEST_RUNNER_ENV_PREFIXES = ["VITEST_", "CARAPACE_VITEST_"];
...
export function buildCurrentCarapaceCliExecRequest(
  args: string[],
  env: NodeJS.ProcessEnv = process.env,
) {
  const invocation = resolveCurrentCarapaceCliInvocation(args);
