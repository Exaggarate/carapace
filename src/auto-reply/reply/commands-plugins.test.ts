...
vi.mock("../../plugins/toggle-config.js", () => ({
  setPluginEnabledInConfig: vi.fn((config: CarapaceConfig, id: string, enabled: boolean) => ({
    ...config,
    plugins: {
      ...config.plugins,
      entries: {
...
describe("handlePluginsCommand", () => {
  ...
  it.each(["list", "inspect pack/one", "enable pack/one"])(
    ...
    async (action) => {
      readConfigFileSnapshotMock.mockResolvedValue({ valid: false, path: "/tmp/carapace.json" });
      const result = await handlePluginsCommand(
        buildPluginsParams(`/plugins ${action}`, buildCfg(), {
