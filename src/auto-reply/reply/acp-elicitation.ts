...
export function parseAcpElicitationRequest(
  ...
  if (!isStructuredInputRecord(snapshot)) {
    return unsupported("Carapace declined a malformed or over-limit ACP input request.");
  }
  ...
  if (mode !== "form") {
    return unsupported(
      `Carapace does not support ACP elicitation mode ${JSON.stringify(mode ?? "unknown")}.`,
    );
  }
  return {
    correlation,
...
function readScope(
  ...
  if (hasSession === hasRequest) {
    return "Carapace declined an ACP input request with an invalid or ambiguous scope.";
  }
  if (hasSession) {
    const normalizedSessionId = readCorrelationText(sessionId);
    if (!normalizedSessionId) {
      return "Carapace declined an ACP input request with an invalid session id.";
    }
    ...
    if (toolCallId !== undefined && toolCallId !== null && !normalizedToolCallId) {
      return "Carapace declined an ACP input request with an invalid tool-call id.";
    }
    return {
  ...
  return normalizedRequestId
    ...
    : "Carapace declined an ACP input request with an invalid request scope.";
}
