...
const { formattingHintCalls } = vi.hoisted(() => ({
  formattingHintCalls: [] as Array<{ cfg: CarapaceConfig; accountId?: string | null }>,
}));
...
describe("buildInboundMetaSystemPrompt", () => {
  it("includes stable routing fields and omits chat ids", () => {
    ...
    expect(payload["schema"]).toBe("carapace.inbound_meta.v2");
...
describe("buildInboundUserContextPrefix", () => {
  ...
  it("omits conversation label block for direct chats", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "direct",
      ConversationLabel: "carapace-tui",
    } as TemplateContext);
  ...
  it.each<{
    ...
    },
    {
      ...
      context: {
        ChatType: "group",
        SenderId: "carapace-control-ui",
        MessageSid: "123",
        ConversationLabel: "some-label",
      },
      expected: {
        message_id: "123",
        sender: { id: "carapace-control-ui" },
    ...
    },
    {
      ...
      includes: ["Conversation info: ⟦carapace:ctx⟧", '"conversation_label":"ops-room"'],
    },
    {
  ...
  it("keeps fenced json delimiters while neutralizing markdown fence tokens in content", () => {
    ...
    expect(text).toContain("Thread starter: ⟦carapace:ctx⟧\n```json");
  ...
  it("renders chat window structured context as compact transcript text", () => {
              ...
              messages: [
                {
                  ...
                  media_path: "/home/user/.carapace/media/inbound/sticker.webp",
    ...
    expect(text).toContain(
      "Current local chat window (chronological, before current message): ⟦carapace:ctx⟧",
    );
    ...
    expect(text).not.toContain("/home/user/.carapace/media/inbound/sticker.webp");
    expect(text).not.toContain("Current local chat window: ⟦carapace:ctx⟧");
  ...
  it("omits forwarded metadata blocks unless ForwardedFrom is present", () => {
    ...
    expect(text).not.toContain("Forwarded message context: ⟦carapace:ctx⟧");
    ...
    expect(withForwardedFrom).toContain("Forwarded message context: ⟦carapace:ctx⟧");
  ...
  it("includes inbound history media metadata without leaking paths or URLs", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/carapace-secret-image.png",
    ...
    expect(text).not.toContain("/tmp/carapace-secret-image.png");
  ...
  it("preserves every media content type for a history message with multiple attachments", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/carapace-secret-image-1.png",
            ...
            {
              path: "/tmp/carapace-secret-image-2.jpg",
    ...
    expect(text).not.toContain("/tmp/carapace-secret-image-1.png");
    expect(text).not.toContain("/tmp/carapace-secret-image-2.jpg");
  ...
  it("renders chat history as per-message prose instead of a raw JSON dump", () => {
    ...
    expect(text).toContain(
      [
        "Chat history since last reply: ⟦carapace:ctx⟧",
        "#1001 sam.rivera: did anyone see the game last night",
        "#1002 lee.chen: yeah it was wild",
      ].join("\n"),
    );
    expect(text).not.toContain("Chat history since last reply: ⟦carapace:ctx⟧\n```json");
  });
});
