...
export function resolveAcpInstallCommandHint(cfg: CarapaceConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `carapace plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^carapace plugins install /u, "");
    ...
    return "carapace plugins install acpx";
  }
