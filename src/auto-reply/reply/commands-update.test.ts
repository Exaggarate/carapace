...
describe("handleUpdateCommand", () => {
  ...
  it.each(["text", "native"] as const)(
    ...
    async (source) => {
      ...
      expect(await handleUpdateCommand(params, true)).toEqual({
        ...
        reply: {
          text: "You are not authorized to use this owner-only command. Ask the operator to run `carapace config set commands.ownerAllowFrom '[\"telegram:123456789\"]'` in a terminal to make this sender a command owner.",
        },
      });
      expect(dispatch).not.toHaveBeenCalled();
    },
  ...
  it("relays owner recovery instructions when the gateway revokes an admitted owner", async () => {
    const message =
      "Ask the operator to run `carapace config set commands.ownerAllowFrom '[\"telegram:owner\"]'` in a terminal.";
  ...
  it("hands off an owner update with session routing and a 20-minute timeout", async () => {
    ...
    dispatch.mockImplementationOnce(async () => {
      ...
      return {
        ...
        handoff: { status: "started", command: "carapace update" },
      };
    });
  ...
  it.each([true, false])(
    ...
    async (ackQueued) => {
      const acknowledgement = "⬆️ Updating Carapace 2026.9.1 → 2026.9.2.";
      dispatch.mockResolvedValueOnce({
        ok: true,
        runId,
        ackDelivered: false,
  ...
  it.each([
    ...
    const command = "carapace update --channel stable";
    getRun.mockReturnValue(
