...
const cases: Array<{ state: Partial<SystemAgentView>; label: string; text: string }> = [
  {
    state: {},
    label: "Allowed once",
    text: "✅ Carapace change approved. Applying: restart the Gateway",
  },
  {
  ...
  },
  {
    state: { decision: "deny" },
    label: "Denied",
    text: "❌ Carapace change denied. No change was made.",
  },
  {
    state: { applicationStatus: "applied" },
    label: "Applied",
    text: "✅ Carapace change approved and applied: restart the Gateway",
  },
  {
    state: { applicationStatus: "not-applied" },
    label: "Not applied",
    text: "⚠️ Carapace change approved, but it was not applied. Check the Gateway and retry.",
  },
  {
  ...
  },
  {
    ...
    label: "Cancelled",
    text: "⚠️ Carapace change was cancelled because its run ended. No change was made. Retry.",
  },
];
...
describe("approval terminal presentation", () => {
  ...
  it("uses the resolved event's decision for channel prose", () => {
    ...
    ).toBe("❌ Carapace change denied. No change was made.");
  });
});
