...
export function defineStrayPluginEntryConfigMigration(params: {
  ...
  normalizeConfig: (params: { cfg: CarapaceConfig }) => {
  ...
  const readStrayEntryConfig = (cfg: CarapaceConfig): Record<string, unknown> | null => {
  ...
  return {
    legacyConfigRule: {
      ...
      message: `${entryConfigPath} is not read by the ${channelId} channel; run "carapace doctor --fix" to move its keys to channels.${channelId}.`,
      match: (value) => {
