...
export function lookupFailedDenialSuffix(kind: LookupFailureKind): string {
  if (kind === "transient") {
    return "spawned-session ownership lookup failed (transient); retry once, then ask the operator to inspect Carapace logs.";
  }
  if (kind === "credentials") {
    return "spawned-session ownership lookup failed; ask the operator to check gateway configuration and credentials.";
  }
  return "spawned-session ownership lookup failed; ask the operator to inspect Carapace logs.";
}
...
export function lookupFailedOperationMessage(
  ...
  const guidance =
    kind === "transient"
      ? "retry once, then ask the operator to inspect Carapace logs"
      : kind === "credentials"
        ? "ask the operator to check gateway configuration and credentials"
        : "ask the operator to inspect Carapace logs";
