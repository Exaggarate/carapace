...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "carapace-qa-test-api-root-");
    resolveCarapacePackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.CARAPACE_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.CARAPACE_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "carapace-qa-runner-root-");
    resolveCarapacePackageRootSync.mockReturnValue(sourceRoot);
    ...
    const manifestCall = loadBundledPluginManifestRegistry.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(manifestCall?.env?.CARAPACE_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.CARAPACE_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.CARAPACE_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.CARAPACE_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-example" exported "extra" from its QA runner surface but did not declare it in carapace.plugin.json',
    );
  });
});
