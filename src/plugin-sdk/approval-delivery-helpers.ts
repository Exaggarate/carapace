...
import type { CarapaceConfig } from "./config-runtime.js";
...
type ApproverRestrictedNativeApprovalFlatParams = {
  ...
  isNativeDeliveryEnabled: (params: { cfg: CarapaceConfig; accountId?: string | null }) => boolean;
...
type StandardNativeApprovalRoutingParams = {
  ...
  isTransportEnabled: (params: { cfg: CarapaceConfig; accountId?: string | null }) => boolean;
  /** Lists channel account ids for route and DM availability checks. */
  listAccountIds: (cfg: CarapaceConfig) => readonly string[];
  /** Resolves the channel's default account id. */
  resolveDefaultAccountId: (cfg: CarapaceConfig) => string;
...
function buildApproverRestrictedNativeApprovalCapability(
  ...
  return createChannelApprovalCapability({
    ...
    delivery: {
      hasConfiguredDmRoute: ({ cfg }: { cfg: CarapaceConfig }) =>
