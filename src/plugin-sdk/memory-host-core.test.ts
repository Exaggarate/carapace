...
describe("memory-host-core helpers", () => {
    ...
    async () => {
      ...
      try {
        ...
        const externalOwner = path.join(
          ...
          ".carapace-memory-host-events-owner.json",
        );
  ...
  it("does not replace or delete an unowned event export path", async () => {
    ...
    const workspaceDir = path.join(fixtureRoot, "workspace");
    try {
      vi.stubEnv("CARAPACE_STATE_DIR", fixtureRoot);
      await fs.mkdir(workspaceDir);
      await appendMemoryHostEvent(workspaceDir, {
        type: "memory.recall.recorded",
        timestamp: "2026-05-18T12:00:00.000Z",
      ...
      await createPluginStateKeyedStore("memory-core", {
        ...
        maxEntries: 10_000,
        env: { ...process.env, CARAPACE_STATE_DIR: fixtureRoot },
      }).clear();
  ...
  it("does not let an orphaned owner marker authorize a later workspace file", async () => {
    ...
    try {
      ...
      const ownerPath = path.join(exportDir, ".carapace-memory-host-events-owner.json");
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          schemaVersion: 3,
          kind: "carapace-memory-host-events-export",
          stateHash,
          workspaceHash,
      ...
      vi.spyOn(fs, "open").mockImplementation(async (...args: Parameters<typeof fs.open>) => {
        ...
        if (typeof target === "string" && path.resolve(target) === path.resolve(exportPath)) {
          ...
          if (exportOpenCount === 4) {
            await fs.rename(exportPath, `${exportPath}.carapace-created`);
  ...
  it("lists shared public artifacts from memory workspaces", async () => {
    ...
    try {
      ...
      const exportOwner = JSON.parse(
        await fs.readFile(
          path.join(path.dirname(eventExportPath), ".carapace-memory-host-events-owner.json"),
          "utf8",
        ),
      ...
      vi.spyOn(fs, "open").mockImplementation(async (...args: Parameters<typeof fs.open>) => {
        ...
        if (typeof target === "string" && path.resolve(target) === expectedExportPath) {
          ...
          if (exportOpenCount === 2) {
            await fs.rename(expectedExportPath, `${expectedExportPath}.carapace-owned`);
  ...
  it("keeps public event exports isolated across state directories", async () => {
    ...
    try {
      ...
      for (const profile of ["profile-a", "profile-b"]) {
        vi.stubEnv("CARAPACE_STATE_DIR", path.join(fixtureRoot, profile));
        await appendMemoryHostEvent(workspaceDir, {
          type: "memory.recall.recorded",
          timestamp: "2026-05-18T12:00:00.000Z",
          query: profile,
