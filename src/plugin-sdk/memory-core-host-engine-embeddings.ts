...
export function createLocalEmbeddingProvider(..._args: unknown[]): Promise<never> {
  return Promise.reject(
    new Error(
      "The legacy in-process llama.cpp embedding runtime is retired. Run `carapace update repair` to install the managed llama-server provider, then restart Carapace.",
    ),
  );
}
