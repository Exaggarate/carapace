...
import type { CarapacePluginGatewayEvents } from "../plugins/gateway-events.js";
import {
  isPluginJsonValue,
...
export type FeatureInvocationContext = { api: CarapacePluginApi } & (
  | { source: "session-action"; action: PluginSessionActionContext }
  | {
      source: "tool";
      tool: CarapacePluginToolContext;
      toolCallId: string;
      signal?: AbortSignal;
...
export type DefineFeaturePluginOptions<C extends FeatureContract> = {
  ...
  setup: (api: CarapacePluginApi, events: FeatureEventEmitter<C>) => FeatureHandlers<C>;
...
export function defineFeaturePlugin<C extends FeatureContract>(
  ...
  const entry = definePluginEntry({
    ...
    register(api) {
      ...
      let gatewayEvents: CarapacePluginGatewayEvents | undefined;
