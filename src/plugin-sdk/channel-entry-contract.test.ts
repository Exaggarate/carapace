...
import type { CarapacePluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "carapace.pluginModuleLoaderJitiFactoryOverride",
);
...
function createApi(registrationMode: PluginRegistrationMode): CarapacePluginApi {
  return {
    registrationMode,
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: CarapacePluginApi) => void;
  registerFull?: (api: CarapacePluginApi) => void;
  registerCapabilities?: (api: CarapacePluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  ...
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = tempDirs.make("carapace-bundled-entry-tools-");
    ...
    const registerCliMetadata = vi.fn<(api: CarapacePluginApi) => void>();
    const registerCapabilities = vi.fn<(api: CarapacePluginApi) => void>();
    const registerFull = vi.fn<(api: CarapacePluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = tempDirs.make("carapace-bundled-entry-runtime-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: CarapacePluginApi) => void>();
    const registerFull = vi.fn<(api: CarapacePluginApi) => void>();
    const registerCapabilities = vi.fn<(api: CarapacePluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = tempDirs.make("carapace-bundled-setup-entry-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: CarapacePluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("CARAPACE_DIAGNOSTICS", "plugin.load-profile");
  ...
  try {
    ...
    const tempRoot = tempDirs.make("carapace-channel-entry-contract-");
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  ...
  fs.writeFileSync(path.join(tempRoot, "carapace.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  fs.writeFileSync(
    ...
    'import { defineChannelMessageAdapter } from "carapace/plugin-sdk/channel-outbound";\nexport const sentinel = defineChannelMessageAdapter();\n',
    "utf8",
  );
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, CARAPACE_DIAGNOSTICS: "plugin.load-profile" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    const importerUrl = "file:///C:/Users/alice/carapace/dist/extensions/feishu/index.js";
      ...
      return {
        ...
        openRootFileSync: (params: Parameters<typeof actual.openRootFileSync>[0]) => {
          ...
          return opened.ok
            ? {
                ...opened,
                path: "C:\\Users\\alice\\carapace\\dist\\extensions\\feishu\\helper.ts",
              }
            : opened;
        },
      };
    ...
    await withMockedWindowsPlatform(async () => {
      try {
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/carapace/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("transforms Carapace SDK dependencies after a native built sidecar load declines", async () => {
  ...
  it("preserves presence-based source fallback disable semantics and cache modes", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"carapace"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("CARAPACE_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "enabled");
    ...
    for (const value of ["", "   ", "off", "no", " ON ", "arbitrary"]) {
      vi.stubEnv("CARAPACE_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", value);
      expect(loadSecretContract).toThrow(
        `resolved "${path.join(pluginRoot, "src", "secret-contract.js")}"`,
      );
    }
