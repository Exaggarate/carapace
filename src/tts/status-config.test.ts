...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "carapace-tts-status-"));
});
...
describe("resolveStatusTtsSnapshot", () => {
  it("treats null prefs as empty settings", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".carapace", "settings", "tts.json");
  ...
  it("derives the default prefs path from CARAPACE_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
