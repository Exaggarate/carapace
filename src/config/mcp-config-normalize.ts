...
import { normalizeLowercaseStringOrEmpty as normalizeMcpString } from "@carapace/normalization-core/string-coerce";
import { isRecord } from "../utils.js";

type ConfigMcpServers = Record<string, Record<string, unknown>>;
type CarapaceMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_CARAPACE_TRANSPORT: Record<string, CarapaceMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to Carapace HTTP transport names. */
export function resolveCarapaceMcpTransportAlias(
  value: unknown,
): CarapaceMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_CARAPACE_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias Carapace can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_CARAPACE_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveCarapaceMcpTransportAlias(next.type);
  // `transport` is Carapace's canonical field; legacy `type` only fills a gap.
