...
describe("sandbox docker config", () => {
  ...
  it("accepts Windows drive-letter binds in sandbox.docker config", () => {
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/carapace/src:/src:ro", "D:\\data\\carapace\\output:/output:rw"],
            },
          },
        },
      },
    ...
    if (res.ok) {
      expect(res.config.agents?.defaults?.sandbox?.docker?.binds).toEqual([
        "D:/data/carapace/src:/src:ro",
        "D:\\data\\carapace\\output:/output:rw",
      ]);
    }
  });
...
describe("sandbox browser binds config", () => {
          ...
          sandbox: {
            browser: {
              binds: [
                ...
                "D:/data/carapace/chrome:/data/chrome-windows:rw",
              ],
            },
          },
        },
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("carapace-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "carapace-sandbox-browser-global" },
      agentBrowser: { network: "carapace-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("carapace-sandbox-browser-agent");
  });
