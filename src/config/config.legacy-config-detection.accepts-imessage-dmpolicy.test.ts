...
function expectCarapaceSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = CarapaceSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectCarapaceSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = CarapaceSchema.safeParse(config);
