...
describe("physical session disk usage", () => {
  it("propagates worker transport failure and measures successfully after recovery", async () => {
    await withTestDir({ prefix: "carapace-disk-usage-worker-error-" }, async (directory) => {
      const storePath = path.join(directory, "carapace-agent.sqlite");
  ...
  it("propagates an unreadable custom-store inventory instead of reporting zero bytes", async () => {
    await withTestDir({ prefix: "carapace-disk-usage-error-" }, async (directory) => {
  ...
  it("does not add synchronous realpath work as session artifacts grow", async () => {
    await withTestDir({ prefix: "carapace-disk-scan-scaling-" }, async (directory) => {
      const storePath = path.join(directory, "carapace-agent.sqlite");
      await fs.writeFile(storePath, Buffer.alloc(321));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "carapace-disk-scan-alias-" }, async (directory) => {
        ...
        await fs.symlink(database, path.join(sessionsDirectory, "carapace-agent.sqlite"));
        await fs.symlink(wal, path.join(sessionsDirectory, "carapace-agent.sqlite-wal"));
        ...
        const usage = await measureSessionPhysicalDiskUsage(
          path.join(aliasDirectory, "carapace-agent.sqlite"),
        );
