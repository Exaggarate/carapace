...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "carapace-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CARAPACE_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-carapace",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-carapace#general");
  });
  ...
  it("derives session transcripts dir from CARAPACE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDirForAgent(
      "main",
      { CARAPACE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CARAPACE_STATE_DIR differs from stored paths", () => {
