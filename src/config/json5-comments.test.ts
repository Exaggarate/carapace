...
describe("warnIfJSON5CommentsWillBeStripped", () => {
  it.each([
    ...
    warnIfJSON5CommentsWillBeStripped({ raw, filePath: "/tmp/carapace.json", warn });

    expect(warn).toHaveBeenCalledWith(
      "Config write will strip JSON5 comments from /tmp/carapace.json.",
    );
  });
