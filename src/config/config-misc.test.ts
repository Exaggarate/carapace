...
import { buildWebSearchProviderConfig, withTempHome, writeCarapaceConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = CarapaceSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://github.com/Exaggarate/carapace");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = CarapaceSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://github.com/Exaggarate/carapace");
    }
  });
});
...
describe("models.catalogRefresh", () => {
  ...
  it("rejects invalid refresh values", () => {
    expect(
      CarapaceSchema.safeParse({ models: { catalogRefresh: { enabled: "false" } } }).success,
    ).toBe(false);
    expect(
      CarapaceSchema.safeParse({ models: { catalogRefresh: { url: "file:///tmp/catalog.json" } } })
        .success,
    ).toBe(false);
    expect(
      CarapaceSchema.safeParse({ models: { catalogRefresh: { url: "not a url" } } }).success,
    ).toBe(false);
    expect(
      CarapaceSchema.safeParse({
...
describe("gateway.controlUi.environment", () => {
  ...
  it.each([
    ...
    expect(CarapaceSchema.safeParse({ gateway: { controlUi: { environment } } }).success).toBe(
      false,
    );
  });
});
...
describe("mcp.sessionIdleTtlMs", () => {
  it.each([0, 1000.9, 7_200_000])("accepts the historical value %s", (sessionIdleTtlMs) => {
    expect(CarapaceSchema.safeParse({ mcp: { sessionIdleTtlMs } }).success).toBe(true);
  });

  it.each([-1, Number.NaN, Number.POSITIVE_INFINITY, "1000"])(
    "rejects invalid idle TTL %s",
    (sessionIdleTtlMs) => {
      expect(CarapaceSchema.safeParse({ mcp: { sessionIdleTtlMs } }).success).toBe(false);
    },
  );
});
...
describe("mcp.apps.enabled", () => {
  it.each([true, false])("accepts %s", (enabled) => {
    expect(CarapaceSchema.safeParse({ mcp: { apps: { enabled } } }).success).toBe(true);
  });

  it("rejects non-boolean values", () => {
    expect(CarapaceSchema.safeParse({ mcp: { apps: { enabled: "yes" } } }).success).toBe(false);
  });
  ...
  it("accepts only a bare HTTP(S) sandbox origin", () => {
    ...
    expect(CarapaceSchema.safeParse({ mcp: { apps: { sandboxPort: 65536 } } }).success).toBe(false);
    for (const sandboxOrigin of [
      ...
      expect(CarapaceSchema.safeParse({ mcp: { apps: { sandboxOrigin } } }).success).toBe(false);
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@carapace"]);
    }
  });
...
describe("config strict validation", () => {
  ...
  it("rejects legacy sandbox perSession without read-time auto-migration", async () => {
    await withTempHome(async (home) => {
      ...
      expect(issuePaths(snap.issues)).toContain("agents.entries.carapace.sandbox");
      ...
      expect(snap.sourceConfig.agents?.entries?.carapace?.sandbox).toEqual({
        perSession: false,
      });
    });
  });
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeCarapaceConfig(home, {
        gateway: { bind: "${CARAPACE_BIND}" },
      });

      const prev = process.env.CARAPACE_BIND;
      process.env.CARAPACE_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.CARAPACE_BIND;
        } else {
          process.env.CARAPACE_BIND = prev;
        }
      }
    });
  });
