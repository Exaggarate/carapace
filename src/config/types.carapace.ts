// Defines the top-level Carapace configuration type.
...
/** Top-level Carapace config as read from user/project config files. */
export type CarapaceConfig = {
  ...
  $schema?: string;
  meta?: {
    /** Last Carapace version that wrote this config. */
  ...
  ui?: {
    /** Accent color for Carapace UI chrome (hex). */
    seamColor?: string;
    /**
...
export type CarapaceConfigInput = Omit<CarapaceConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const carapaceConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = CarapaceConfig & {
  readonly [carapaceConfigStateBrand]?: TState;
};
