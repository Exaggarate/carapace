...
describe("config io EACCES handling", () => {
  it("logs config load failures without exposing a raw error stack", () => {
    const configPath = "/data/.carapace/carapace.json";
  ...
  it("includes configPath in the chown hint for the correct remediation command", async () => {
    const configPath = "/home/myuser/.carapace/carapace.json";
    const io = createConfigIO({
      configPath,
      fs: makeEaccesFs(configPath),
      logger: { error: () => {}, warn: () => {} },
...
describe("config write guard after unreadable config", () => {
  ...
  it.each([
    ...
    async ({ writeOptions }) => {
      const home = fsNode.mkdtempSync(path.join(os.tmpdir(), "carapace-unreadable-"));
      tempRoots.push(home);
      ...
      const skeletal: CarapaceConfig = { channels: { telegram: { enabled: true } } };
      ...
      const rejectedArtifacts = fsNode
        ...
        .filter((name) => name.startsWith("carapace.json.rejected."));
