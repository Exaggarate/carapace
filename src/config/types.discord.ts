...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Carapace agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Carapace, or the agent id plus Carapace. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Carapace agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Carapace conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Carapace Discord conversation. */
