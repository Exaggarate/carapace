...
describe("readBestEffortConfig", () => {
  ...
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.carapace/logs/config-health.json`;
  ...
  it("can read snapshots without applying config env vars to the process", async () => {
    await withTempHome(async (home) => {
      const key = "CARAPACE_ISOLATED_CONFIG_READ_TEST";
  ...
  it("resolves config env above exact lower-precedence values in isolated snapshots", async () => {
    await withTempHome(async (home) => {
      const key = "CARAPACE_GATEWAY_TOKEN";
  ...
  it("can read best-effort config without applying env vars or recording observation", async () => {
    await withTempHome(async (home) => {
      const key = "CARAPACE_ISOLATED_BEST_EFFORT_CONFIG_TEST";
      await withEnvAsync({ [key]: undefined }, async () => {
        await writeCarapaceConfig(home, {
          env: { vars: { [key]: "from-config" } },
          gateway: { mode: "local" },
        ...
        expect(process.env[key]).toBeUndefined();
        await expect(fs.stat(`${home}/.carapace/logs/config-health.json`)).rejects.toMatchObject({
          code: "ENOENT",
        });
      });
    });
  ...
  it("records why an unparseable config was ignored by best-effort reads", async () => {
    await withTempHome(async (home) => {
      const configPath = `${home}/.carapace/carapace.json`;
      await fs.mkdir(`${home}/.carapace`, { recursive: true });
  ...
  it("preserves Windows case-insensitive env lookup in isolated reads", async () => {
    await withTempHome(async (home) => {
      const mixedCaseKey = "Carapace_Config_Path";
      const customConfigPath = `${home}/custom-carapace.json`;
      await withEnvAsync({ CARAPACE_CONFIG_PATH: undefined }, async () => {
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      ...
      const entries = await fs.readdir(`${home}/.carapace`);
      expect(entries.some((entry) => entry.startsWith("carapace.json.clobbered."))).toBe(false);
    });
  });
