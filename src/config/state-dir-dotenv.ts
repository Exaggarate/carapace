...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Carapace env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Carapace-managed keys:
 ...
 * literal references for keys Carapace previously managed.
 */
...
/**
 * Read and parse `~/.carapace/.env` (or `$CARAPACE_STATE_DIR/.env`), returning
