...
if (
  ...
  (existsSync(new URL("./.carapace-lifecycle-pending", import.meta.url)) ||
    existsSync(new URL("./dist/carapace-install-guard", import.meta.url)))
) {
  try {
...
const { isSupportedCarapaceNodeVersion } = await import("./node-version.mjs");
...
const ensureSupportedRuntimeVersion = () => {
  if (process.versions.bun) {
    ...
    // rejecting Bun outright so capable Bun builds can run Carapace.
    let hasNodeSqlite;
    try {
    ...
    process.stderr.write(
      "carapace: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
        `Use Node.js ${SUPPORTED_NODE_RANGE}; Bun remains supported for installs and package scripts.\n`,
    );
    process.exit(1);
  }
  if (isSupportedCarapaceNodeVersion(process.versions.node)) {
    return;
  }

  process.stderr.write(
    `carapace: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[carapace] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.CARAPACE_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    CARAPACE_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["carapace: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:carapace/carapace#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g carapace@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.CARAPACE_CONTAINER)) {
    return true;
  }
  const args = argv.slice(2);
...
const isHelpFastPathDisabled = () =>
  process.env.CARAPACE_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.CARAPACE_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.CARAPACE_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.CARAPACE_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "carapace.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".carapace", "carapace.json"),
    path.join(homeDir, ".carapace", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "carapace.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.CARAPACE_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.CARAPACE_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Carapace ${version} (${commit})\n` : `Carapace ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.CARAPACE_BUNDLED_VERSION) ?? "0.0.0";
}
